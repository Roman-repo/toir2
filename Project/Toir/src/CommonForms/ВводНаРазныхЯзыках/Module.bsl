...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Реквизит = Неопределено Тогда
		ШаблонОшибки = НСтр("es='A la apertura de la forma de VvodNaRaznyhYazykah en el parámetro de ImyaRekvizita es indicado el requisito existente %1';ru='При открытии формы ВводНаРазныхЯзыках в параметре ИмяРеквизита указан не существующий реквизит %1';en='When open the ВводНаРазныхЯзыках form, the ИмяРеквизита parameter specifies a non-existing attribute%1'");
	...
	Иначе
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='%1 en las lenguas diferentes';ru='%1 на разных языках';en='%1 in different languages'"), Реквизит.Представление());
	КонецЕсли;
