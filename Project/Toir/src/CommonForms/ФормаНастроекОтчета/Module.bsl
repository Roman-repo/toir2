...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("НастройкиОтчета", НастройкиОтчета) Тогда
		ВызватьИсключение НСтр("es='el parámetro ""NastroykiOtcheta"" de servicio.';ru='Не передан служебный параметр ""НастройкиОтчета"".';en='The service parameter ""НастройкиОтчета"" was not transferred.'");
	КонецЕсли;
	Если Не Параметры.Свойство("ВариантНаименование", ВариантНаименование) Тогда
		ВызватьИсключение НСтр("es='el parámetro ""VariantNaimenovanie"" de servicio.';ru='Не передан служебный параметр ""ВариантНаименование"".';en='The service parameter ""ВариантНаименованияя"" was not transferred.'");
	КонецЕсли;
	...
	Если ТипЗнч(ИдентификаторЭлементаСтруктурыНастроек) = Тип("ИдентификаторКомпоновкиДанных") Тогда
		...
		Если Не Параметры.Свойство("Заголовок", Заголовок) Тогда
			ВызватьИсключение НСтр("es='""el Encabezamiento"".';ru='Не передан служебный параметр ""Заголовок"".';en='The service parameter ""Title"" was not transferred.'");
		КонецЕсли;
		
		Если Не Параметры.Свойство("ТипЭлементаСтруктурыНастроек", ТипЭлементаСтруктурыНастроек) Тогда
			ВызватьИсключение НСтр("es='el parámetro ""TipElementaStrukturyNastroek"" de servicio.';ru='Не передан служебный параметр ""ТипЭлементаСтруктурыНастроек"".';en='The service parameter ""ТипЭлементаСтруктурыНастроек"". was not transferred.'");
		КонецЕсли;
	Иначе
		...
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Los ajustes del informe ""%1""';ru='Настройки отчета ""%1""';en='Report Settings ""%1""'"), ВариантНаименование);
	КонецЕсли;
...
Процедура Подключаемый_Список_ОбработкаВыбора(Элемент, РезультатВыбора, СтандартнаяОбработка)
	...
	Если Дополнение.Всего > 0 Тогда
		Если Дополнение.Всего = 1 Тогда
			ЗаголовокОповещения = НСтр("es='El elemento es añadido en la lista';ru='Элемент добавлен в список';en='Element added to list.'");
		Иначе
			ЗаголовокОповещения = НСтр("es='Los elementos son añadidos en la lista';ru='Элементы добавлены в список';en='Elements added to list.'");
		КонецЕсли;
...
Процедура ВыбранныеПоля_Разгруппировать(Команда)
	...
	Если ИдентификаторыСтрок.Количество() <> 1 Тогда 
		ПоказатьПредупреждение(, НСтр("es='Escojan un grupo.';ru='Выберите одну группу.';en='Select one group.'"));
		Возврат;
	КонецЕсли;
	...
	Если ТипЗнч(РодительСтрокиИсточника.Идентификатор) <> Тип("ИдентификаторКомпоновкиДанных") Тогда 
		ПоказатьПредупреждение(, НСтр("es='Escojan el grupo.';ru='Выберите группу.';en='Select a group.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОтборыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Строка.ПредставлениеПользовательскойНастройки = "Объект иерархии" Тогда
		СвойствоЭлементаСтруктуры = СвойствоЭлементаСтруктурыНастроек(Отчет.КомпоновщикНастроек, "Отбор", ИдентификаторЭлементаСтруктурыНастроек);
		ЭлементНастройки = ЭлементНастройки(СвойствоЭлементаСтруктуры, Строка);
		ТекущиеНастройки = ЭлементСтруктурыНастроек(Отчет.КомпоновщикНастроек.Настройки, ИдентификаторЭлементаСтруктурыНастроек);
		ПараметрыВыбора = ОтчетыКлиентСервер.ПараметрыВыбора(ТекущиеНастройки, Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы, ЭлементНастройки, РасширенныйРежим = 1);
		Если ПараметрыВыбора.Количество() Тогда
			ТипыЭлементовВИерархии = торо_РаботаСИерархией20КлиентСервер.ПолучитьОписаниеТиповЭлементовВИерархии(ПараметрыВыбора[0].Значение);
			Элементы.ОтборыПравоеЗначение.ОграничениеТипа = ТипыЭлементовВИерархии;
			Строка.ТипЗначения = ТипыЭлементовВИерархии;
			Элементы.ОтборыПравоеЗначение.ДоступныеТипы = Строка.ТипЗначения;
		КонецЕсли;
	КонецЕсли;
	// ТОИР--
...
Процедура СтруктураВарианта_СохранитьВФайл(Команда)
	...
	ПолучитьФайл(Адрес, НСтр("es='Настройки.xml';ru='Настройки.xml';en='Settings.xml'"), Истина);
КонецПроцедуры
...
Процедура ОбновитьПоляГруппировки()
	...
	Для Каждого ЭлементНастройки Из СвойствоЭлементаСтруктуры.Элементы Цикл 
		...
		Если ТипЗнч(ЭлементНастройки) = Тип("АвтоПолеГруппировкиКомпоновкиДанных") Тогда 
			Строка.Заголовок  = НСтр("es='Авто (por todos los campos)';ru='Авто (по всем полям)';en='Auto (in all fields)'");
...
Процедура ОбновитьПараметрыДанных()
	...
	Раздел.Заголовок = НСтр("es='Los parámetros';ru='Параметры';en='Options'");
...
Процедура ОбновитьОтборы(Строки = Неопределено, ЭлементыНастроек = Неопределено)
	...
	Если Строки = Неопределено Тогда 
		Раздел = Отборы.ПолучитьЭлементы().Добавить();
		Раздел.ЭтоРаздел = Истина;
		Раздел.Заголовок = НСтр("es='Las selecciones';ru='Отборы';en='Selections'");
...
Процедура ОтборыПослеВыбораПоля(ОписаниеНастройки, ИдентификаторСтроки) Экспорт
	...
	// ТОИР++
	Поле = Новый ПолеКомпоновкиДанных("ОбъектИерархии");
	Если Поле = ОписаниеНастройки.Поле Тогда
		ТекущиеНастройки = ЭлементСтруктурыНастроек(Отчет.КомпоновщикНастроек.Настройки, ИдентификаторЭлементаСтруктурыНастроек);
		ПараметрыВыбора = ОтчетыКлиентСервер.ПараметрыВыбора(ТекущиеНастройки, Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы, ЭлементНастройки, РасширенныйРежим = 1);
		Если ПараметрыВыбора.Количество() Тогда
			ТипыЭлементовВИерархии = торо_РаботаСИерархией20КлиентСервер.ПолучитьОписаниеТиповЭлементовВИерархии(ПараметрыВыбора[0].Значение);
			ЭлементНастройки.ПравоеЗначение = ТипыЭлементовВИерархии.ПривестиЗначение();
		КонецЕсли;
	Иначе
	// ТОИР--	
...
Функция ПараметрыГруппировкиОтборов()
	...
	Если Строки.Количество() = 0 Тогда 
		ПоказатьПредупреждение(, НСтр("es='Escojan los elementos.';ru='Выберите элементы.';en='Select items.'"));
		Возврат Неопределено;
	КонецЕсли;
	
	Родители = ОбщегоНазначенияКлиентСервер.СвернутьМассив(Родители);
	Если Родители.Количество() > 1 Тогда 
		ПоказатьПредупреждение(, НСтр("es='Los elementos escogidos no pueden ser agrupados, ya que pertenecen a los padres diferentes.';ru='Выбранные элементы не могут быть сгруппированы, поскольку они принадлежат разным родителям.';en='Selected items cannot be grouped because they belong to different parents.'"));
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПараметрыСдвигаОтборов()
	...
	Родители = ОбщегоНазначенияКлиентСервер.СвернутьМассив(Родители);
	Если Родители.Количество() > 1 Тогда 
		ПоказатьПредупреждение(, НСтр("es='Los elementos escogidos no pueden ser trasladados, ya que pertenecen a los padres diferentes.';ru='Выбранные элементы не могут быть перемещены, поскольку они принадлежат разным родителям.';en='Selected items cannot be moved because they belong to different parents.'"));
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ОбновитьВыбранныеПоля(Строки = Неопределено, ЭлементыНастроек = Неопределено)
	...
	Если Строки = Неопределено Тогда 
		...
		Раздел.ЭтоРаздел = Истина;
		Раздел.Заголовок = НСтр("es='Campos';ru='Поля';en='Fields'");
	...
	Для Каждого ЭлементНастройки Из ЭлементыНастроек Цикл 
		...
		Если ТипЗнч(ЭлементНастройки) = Тип("АвтоВыбранноеПолеКомпоновкиДанных") Тогда 
			Строка.Заголовок = НСтр("es='Авто (el campo de los padres)';ru='Авто (поля родителя)';en='Auto (parent fields)'");
			Строка.Картинка = 6;
			Продолжить;
		КонецЕсли;
...
Процедура УстановитьПараметрыРедактирования(Строка)
	...
	// ТОИР++
	ПараметрыВыбора = ОтчетыКлиентСервер.ПараметрыВыбора(ТекущиеНастройки, ПользовательскиеНастройки, ЭлементНастройки, РасширенныйРежим = 1);
	ПолеЗначения.ПараметрыВыбора = ПараметрыВыбора;
	
	Если Строка.ПредставлениеПользовательскойНастройки = "Объект иерархии" И ПараметрыВыбора.Количество() Тогда
		ТипыЭлементовВИерархии = торо_РаботаСИерархией20КлиентСервер.ПолучитьОписаниеТиповЭлементовВИерархии(ПараметрыВыбора[0].Значение);
		ПолеЗначения.ОграничениеТипа = ТипыЭлементовВИерархии;
		Строка.ТипЗначения = ТипыЭлементовВИерархии;
		ПолеЗначения.ДоступныеТипы = Строка.ТипЗначения;
	Иначе
	// ТОИР--
...
Процедура ОбновитьСортировку()
	...
	Раздел.ЭтоРаздел = Истина;
	Раздел.Заголовок = НСтр("es='Las selecciones';ru='Сортировки';en='Sortings'");
	...
	Для Каждого ЭлементНастройки Из ЭлементыНастроек Цикл 
		...
		Если ТипЗнч(ЭлементНастройки) = Тип("АвтоЭлементПорядкаКомпоновкиДанных") Тогда 
			Строка.Заголовок = НСтр("es='Авто (la selección de los padres)';ru='Авто (сортировки родителя)';en='Auto (parent sort)'");
...
Процедура ОбновитьОформление()
	...
	Иначе
		...
		Раздел.Заголовок = НСтр("es='La formalización condicional';ru='Условное оформление';en='Conditional execution'");
		Раздел.Представление = НСтр("es='La formalización condicional';ru='Условное оформление';en='Conditional execution'");
...
Процедура ПрочитатьПредопределенныеПараметрыОформления()
	...
	Строка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El encabezamiento: %1';ru='Заголовок: %1';en='Heading:%1'"),
		?(ЗначениеЗаполнено(Объект.Значение), Объект.Значение, "<Отсутствует>"));
	Строка.Представление = НСтр("es='El encabezamiento';ru='Заголовок';en='Headlng'");
	...
	Строка.Заголовок = НСтр("es='Sacar los parámetros y las selecciones';ru='Выводить параметры и отборы';en='Display parameters and selections'");
	Строка.Представление = НСтр("es='Sacar los parámetros y las selecciones';ru='Выводить параметры и отборы';en='Display parameters and selections'");
...
Процедура ОформлениеИзменитьЭлемент(ИдентификаторСтроки = Неопределено, Строка = Неопределено)
	...
	ПараметрыФормы.Вставить("Заголовок", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El elemento de la formalización condicional del informe ""%1""';ru='Элемент условного оформления отчета ""%1""';en='Conditional report element ""%1""'"), ВариантНаименование));
...
Процедура ОбновитьСтруктуру()
	...
	Раздел.Представление = НСтр("es='El informe';ru='Отчет';en='Report'");
	Раздел.ЭтоРаздел = Истина;
	Раздел.Картинка = -1;
...
Функция СвойстваЭлементаКоллекцииСтруктуры(Элемент)
	...
	ИначеЕсли ТипЭлемента = Тип("НастройкиВложенногоОбъектаКомпоновкиДанных") Тогда 
		...
		Если Не ЗначениеЗаполнено(СвойстваЭлемента.Представление) Тогда
			СвойстваЭлемента.Представление = НСтр("es='La agrupación impuesta';ru='Вложенная группировка';en='Nested Grouping'");
		КонецЕсли;
		ИменаКоллекций = "Структура";
	ИначеЕсли ТипЭлемента = Тип("ТаблицаКомпоновкиДанных") Тогда 
		СвойстваЭлемента.Представление = НСтр("es='La tabla';ru='Таблица';en='Table'");
		ИменаКоллекций = "Строки, Колонки";
	ИначеЕсли ТипЭлемента = Тип("ДиаграммаКомпоновкиДанных") Тогда 
		СвойстваЭлемента.Представление = НСтр("es='El diagrama';ru='Диаграмма';en='Chart'");
...
Функция ПредставлениеПолейГруппировки(ЭлементНастройки, ПометкаУдаления)
	...
	Если Поля.Элементы.Количество() = 0 Тогда 
		Возврат НСтр("es='<las anotaciones Detalladas>';ru='<Детальные записи>';en='<Детальные записи>'");
	КонецЕсли;
...
Процедура ДобавитьЭлементСтруктурыНастроек(ТипЭлемента)
	...
	Если Строка.Тип = "ДиаграммаКомпоновкиДанных" Тогда
		...
		Строка.Представление = НСтр("es='El diagrama';ru='Диаграмма';en='Chart'");
		...
		ПодчиненнаяСтрока.Представление = НСтр("es='Los puntos';ru='Точки';en='Points'");
		...
		ПодчиненнаяСтрока.Идентификатор = СвойствоЭлементаСтруктуры.ПолучитьИдентификаторПоОбъекту(ПодчиненныйЭлементНастройки);
		ПодчиненнаяСтрока.Картинка = -1;
		ПодчиненнаяСтрока.Представление = НСтр("es='Unas series';ru='Серии';en='Series'");
	ИначеЕсли Строка.Тип = "ТаблицаКомпоновкиДанных" Тогда
		Строка.Представление = НСтр("es='La tabla';ru='Таблица';en='Table'");
		...
		ПодчиненнаяСтрока.Идентификатор = СвойствоЭлементаСтруктуры.ПолучитьИдентификаторПоОбъекту(ПодчиненныйЭлементНастройки);
		ПодчиненнаяСтрока.Картинка = -1;
		ПодчиненнаяСтрока.Представление = НСтр("es='Las líneas';ru='Строки';en='Lines'");
		...
		ПодчиненнаяСтрока.Идентификатор = СвойствоЭлементаСтруктуры.ПолучитьИдентификаторПоОбъекту(ПодчиненныйЭлементНастройки);
		ПодчиненнаяСтрока.Картинка = -1;
		ПодчиненнаяСтрока.Представление = НСтр("es='Las columnas';ru='Колонки';en='Speakers'");
	КонецЕсли;
...
Процедура СтруктураВариантаПослеВыбораПоля(ОписаниеНастройки, ПараметрыВыполнения) Экспорт
	...
	Если ОписаниеНастройки = "<>" Тогда
		...
		Представление = НСтр("es='<las anotaciones Detalladas>';ru='<Детальные записи>';en='<Детальные записи>'");
	Иначе
...
Процедура ИзменитьЭлементСтруктуры(Строка, ИмяСтраницы = Неопределено, ИспользоватьФормуВарианта = Неопределено)
	...
	ШаблонЗаголовка = НСтр("es='El ajuste %1 informes ""%2""';ru='Настройка %1 отчета ""%2""';en=""Setting %1 of report '%2'""");
	Если Строка.Тип = "ДиаграммаКомпоновкиДанных" Тогда
		ПредставлениеЭлемента = НСтр("es='Los diagramas';ru='диаграммы';en='charts'");
	Иначе
		ПредставлениеЭлемента = НСтр("es='Las agrupaciones';ru='группировки';en='groupings'");
	КонецЕсли;
...
Процедура ОпределитьВыделенныеСтроки(Контекст)
	...
	Если Контекст.СтрокиДерева.Количество() = 0 Тогда
		Контекст.ПричинаОтказа = НСтр("es='Escojan los elementos.';ru='Выберите элементы.';en='Select items.'");
		Возврат;
	КонецЕсли;
	Если Контекст.ТекущаяСтрока = Неопределено Тогда
		Если Контекст.Действие = "ИзменитьГруппу" Тогда
			Контекст.ПричинаОтказа = НСтр("es='Escojan el grupo.';ru='Выберите группу.';en='Select a group.'");
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Функция СтрокаДобавлена(Строки, СтрокаДерева, Особенности)
	...
	Если (Особенности.МогутБытьПараметры И СтрокаДерева.ЭтоПараметр)
		...
		Если Строки.Действие = "Переместить" Тогда
			Строки.ПричинаОтказа = НСтр("es='Los parámetros no pueden ser trasladados.';ru='Параметры не могут быть перемещены.';en='Parameters cannot be moved.'");
		ИначеЕсли Строки.Действие = "Сгруппировать" Тогда
			Строки.ПричинаОтказа = НСтр("es='Los parámetros no pueden ser los participantes de los grupos.';ru='Параметры не могут быть участниками групп.';en='Parameters cannot be members of groups.'");
		ИначеЕсли Строки.Действие = "Удалить" Тогда
			Строки.ПричинаОтказа = НСтр("es='Los parámetros no pueden ser quitados.';ru='Параметры не могут быть удалены.';en='Parameters cannot be deleted.'");
		КонецЕсли;
		Возврат Ложь;
	КонецЕсли;
	Если Особенности.ТребоватьОдногоРодителя Тогда
		...
		ИначеЕсли Строки.ТекущийРодитель <> Родитель Тогда
			Если Строки.Действие = "Переместить" Тогда
				Строки.ПричинаОтказа = НСтр("es='Los elementos escogidos no pueden ser trasladados, ya que pertenecen a los padres diferentes.';ru='Выбранные элементы не могут быть перемещены, поскольку они принадлежат разным родителям.';en='Selected items cannot be moved because they belong to different parents.'");
			ИначеЕсли Строки.Действие = "Сгруппировать" Тогда
				Строки.ПричинаОтказа = НСтр("es='Los elementos escogidos no pueden ser agrupados, ya que pertenecen a los padres diferentes.';ru='Выбранные элементы не могут быть сгруппированы, поскольку они принадлежат разным родителям.';en='Selected items cannot be grouped because they belong to different parents.'");
			КонецЕсли;
			Возврат Ложь; 
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыбратьРежимОтображения(ОтборыУзлаНастроек, ИмяКоллекции, ИдентификаторСтроки, ПоказыватьРежимыВвода, ПоказыватьРежимыФлажков, ТекущийРежимОтображения = Неопределено)
	...
	Если ПоказыватьРежимыВвода Тогда
		Список.Добавить("ПоказыватьВШапкеОтчета", НСтр("es='En el gorro del informe';ru='В шапке отчета';en='In the header of the report'"), , БиблиотекаКартинок.БыстрыйДоступ);
	КонецЕсли;
	Если ПоказыватьРежимыФлажков Тогда
		Список.Добавить("ПоказыватьТолькоФлажокВШапкеОтчета", НСтр("es='Solamente el banderín en el gorro del informe';ru='Только флажок в шапке отчета';en='Only flag in report header'"), , БиблиотекаКартинок.БыстрыйДоступСФлажком);
	КонецЕсли;
	Если ПоказыватьРежимыВвода Тогда
		Список.Добавить("ПоказыватьВНастройкахОтчета", НСтр("es='En los ajustes del informe';ru='В настройках отчета';en='In report settings'"), , БиблиотекаКартинок.Реквизит);
	КонецЕсли;
	Если ПоказыватьРежимыФлажков Тогда
		Список.Добавить("ПоказыватьТолькоФлажокВНастройкахОтчета", НСтр("es='Solamente el banderín en los ajustes del informe';ru='Только флажок в настройках отчета';en='Only checkbox in report settings'"), , БиблиотекаКартинок.ОбычныйДоступСФлажком);
	КонецЕсли;
	Список.Добавить("НеПоказывать", НСтр("es='No mostrar';ru='Не показывать';en='Do not show'"), , БиблиотекаКартинок.СкрытаяНастройкаОтчета);
...
Процедура ОбновитьСвойстваЭлементовФормы()
	...
	Если ЭтоРасширенныйРежим Тогда
		Элементы.СтраницаОтборы.Заголовок = НСтр("es='Las selecciones';ru='Отборы';en='Selections'");
	Иначе
		Элементы.СтраницаОтборы.Заголовок = НСтр("es='Básico';ru='Основное';en='Main'");
	КонецЕсли;
	...
	Если ОтображатьВыбранныеПоля И ОтображатьСортировки Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = НСтр("es='Campos y la selección';ru='Поля и сортировки';en='Fields and Sorting'");
	ИначеЕсли ОтображатьВыбранныеПоля Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = НСтр("es='Campos';ru='Поля';en='Fields'");
	ИначеЕсли ОтображатьСортировки Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = НСтр("es='Las selecciones';ru='Сортировки';en='Sortings'");
	КонецЕсли;
	...
	Если РежимИзмененияЭлементаСтруктурыНастроек Тогда
		Элементы.ЗакрытьИСформировать.Заголовок = НСтр("es='Acabar la redacción';ru='Завершить редактирование';en='Complete editing'");
		Элементы.Закрыть.Заголовок = НСтр("en='cancel';es='La anulación';ru='Отмена'");
	Иначе
		Элементы.ЗакрытьИСформировать.Заголовок = НСтр("es='Cerrar y formar';ru='Закрыть и сформировать';en='Close and shape'");
