	...
	Для каждого СтрАдрес Из АдресаИсключений Цикл
		Если НЕ ПустаяСтрока(СтрАдрес.АдресСервера) Тогда
			...
			Если НЕ ПустаяСтрока(НедопустимыеСимволы) Тогда
				...
				ТекстСообщения = СтрЗаменить(НСтр("es='En la dirección son encontrados los símbolos inadmisibles: %1';ru='В адресе найдены недопустимые символы: %1';en='Invalid characters found in the address:%1'"),
					"%1",
					НедопустимыеСимволы);
