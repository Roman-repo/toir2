...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ШаблонЗаголовка = Нстр("es='Los análogos del material ""%1"" (%2 %3)';ru='Аналоги материала ""%1"" (%2 %3)';en='Analogs of material ""%1"" (%2%3)'");
...
Процедура СписокВыборЗначения(Элемент, Значение, СтандартнаяОбработка)
	...
	Если ТекущиеДанные.КоличестваНеДостаточноДляЗамены Тогда
		...
		ШаблонПредупреждения = НСтр("es='No es posible sustituir %1 %2 materiales en %3 %4';ru='Нельзя заменить %1 %2 материала на %3 %4';en='Can not replace%1%2 material with%3%4'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='<cualquier característica>';ru='<любая характеристика>';en='<любая характеристика>'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='<componente>';ru='<составной>';en='<составной>'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", WebЦвета.НейтральноСерый);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='No';ru='нет';en='no'"));
...
Процедура ОбновитьПредставлениеОбластиДействия(ТекущиеДанные)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ДатаОкончанияДействия) Тогда
		ШаблонПредставления = НСтр("es='Con %1 por %2';ru='С %1 по %2';en='From %1 to %2'") + " ";
	...
	Иначе
		ШаблонПредставления = НСтр("es='CON %1';ru='С %1';en='From %1'") + " ";
		ТекстПредставления = СтрШаблон(ШаблонПредставления,
			Формат(ТекущиеДанные.ДатаНачалаДействия, "ДЛФ=D"));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ВидРемонта)
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.ВидРемонта) Тогда
			ШаблонПредставления = НСтр("es='En los límites de las reparaciones del tipo ""%1""';ru='в рамках ремонтов вида ""%1""';en='as part of repairs of type ""%1""'") + " ";
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.Подразделение) Тогда
			ШаблонПредставления = НСтр("es=' Cumplido por subdivisión ""%1""';ru=' выполняемых подразделением ""%1""';en="" performed by department '%1'""") + " ";
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.ТехКарта) Тогда
			ШаблонПредставления = НСтр("es=' Por la tarjeta tecnológica ""%1""';ru=' по технологической карте ""%1""';en=' on the routing ""%1""'") + " ";
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ОбъектРемонта) Тогда
		ШаблонПредставления = НСтр("es='Para el objeto de la reparación ""%1""';ru='для объекта ремонта ""%1""';en='for repair item ""%1""'") + " ";
	...
	ИначеЕсли 
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.Организация) Тогда
			ШаблонПредставления = НСтр("es='Que pertenecen ""%1"" ';ru='принадлежащих ""%1"" ';en='owned by ""%1"" '") + " ";
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.НаправлениеОбъектаРемонта) Тогда
			ШаблонПредставления = НСтр("es='(La dirección ""%1"") ';ru='(направление ""%1"") ';en='(direction ""%1"") '") + " ";
	...
	Иначе
		ПредставлениеОбластиДействия = ПредставлениеОбластиДействия + НСтр("es='Por la empresa en total';ru='по предприятию в целом';en='for the enterprise as a whole'");
	КонецЕсли;
