...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(СсылкаНаОбъект) Тогда
		ВызватьИсключение НСтр("es='el propietario de los ajustes de los derechos.';ru='Не указан владелец настроек прав.';en='The owner of the rights settings is not specified.'");
	КонецЕсли;
	...
	Если ВозможныеПрава.ПоТипамСсылок.Получить(ТипСсылкиНаОбъект) = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Los derechos de admisión para cada objeto no se disponen "
"para los objetos como ""%1"".';ru='Права доступа для каждого объекта не настраиваются"
"для объектов типа ""%1"".';en='Access rights for each object are not configurable"
"for objects of type ""%1 "". '"),
	...
	Если Не ПравоДоступа("Просмотр", Метаданные.НайтиПоТипу(ТипСсылкиНаОбъект)) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No hay derecho la Presentación a los objetos como ""%1"".';ru='Нет права Просмотр на объекты типа ""%1"".';en='No view rights to objects of type ""%1"".'"), Строка(ТипСсылкиНаОбъект));
	КонецЕсли;
...
Процедура Перечитать(Команда)
	...
	Иначе
		ПоказатьВопрос(
			...
			НСтр("es='Los datos son cambiados. ¿Leer sin conservación?';ru='Данные изменены. Прочитать без сохранения?';en='Data changed. Read without saving?'"),
...
Процедура ЗаписатьНачало(Закрыть = Ложь)
	...
	Если ПодтвердитьОтказОтУправленияПравами = Истина Тогда
		...
		Кнопки.Добавить("ЗаписатьИЗакрыть", НСтр("es='Anotar y cerrar';ru='Записать и закрыть';en='Write and close'"));
		...
		ПоказатьВопрос(
			...
			НСтр("es='Después de la anotación el ajuste de los derechos será inaccesible.';ru='После записи настройка прав станет недоступной.';en='After recording, the rights setting will become unavailable.'"),
			Кнопки,, "Отмена");
	Иначе
		Если Закрыть Тогда
			Закрыть();
...
Процедура ДобавитьРеквизитыИлиЭлементыФормы(НовыеРеквизиты = Неопределено)
	...
	Для каждого ОписаниеПрава Из ОписанияВозможныхПрав Цикл
		...
		Иначе
			...
			Элемент.Формат                        = НСтр("es='ЧЦ=1; ЧН =; БЛ=Нет; БИ=Да';ru='ЧЦ=1; ЧН=; БЛ=Нет; БИ=Да';en='ЧЦ=1; ЧН=; БЛ=No; БИ=Yes'");
	...
	Если НовыеРеквизиты = Неопределено И Параметры.СсылкаНаОбъект.Метаданные().Иерархический Тогда
		...
		Элемент.Заголовок = НСтр("es='Para "
"подпапок';ru='Для"
"подпапок';en='For"
"subfolders'");
		Элемент.Подсказка = НСтр("es='Los derechos no sólo para la carpeta corriente, "
"sino también para sus carpetas inferiores';ru='Права не только для текущей папки,"
"но и для ее нижестоящих папок';en='Rights not only for the current folder,"
"but also for her subordinate folders '");
		...
		Элемент.Заголовок = НСтр("es='Es heredado de';ru='Наследуется от';en='Inherited from'");
		Элемент.Подсказка = НСтр("es='La carpeta, de que son heredados el ajuste de los derechos';ru='Папка, от которой наследуются настройка прав';en='The folder from which the rights setting is inherited'");
	...
	Пока НЕ Отказ И НомерСтроки >= 0 Цикл
		...
		Если НетЗаполненногоПрава Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='No es rellenado ni un derecho de admisión.';ru='Не заполнено ни одно право доступа.';en='Not a single access right has been filled.'"),
				,
		...
		Если НЕ ЗначениеЗаполнено(ТекущаяСтрока["Пользователь"]) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='el usuario o el grupo.';ru='Не заполнен пользователь или группа.';en='The user or group is not full.'"),
				,
		...
		Если ГруппыПрав.НайтиСтроки(Отбор).Количество() > 1 Тогда
			Если ТипЗнч(Отбор.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекстСообщения = НСтр("es='El ajuste para el usuario ""%1"" es ya.';ru='Настройка для пользователя ""%1"" уже есть.';en='The setting for user ""%1"" already exists.'");
			Иначе
				ТекстСообщения = НСтр("es='El ajuste para el grupo de los usuarios ""%1"" es ya.';ru='Настройка для группы пользователей ""%1"" уже есть.';en=""The setting for user group '%1' already exists.""");
			КонецЕсли;
			ОбщегоНазначенияКлиент.СообщитьПользователю(
...
Процедура ЗаписатьПрава()
	...
	Попытка
		...
		Иначе
			...
			ВызватьИсключение НСтр("es='Después de la anotación el ajuste de los derechos será inaccesible.';ru='После записи настройка прав станет недоступной.';en='After recording, the rights setting will become unavailable.'");
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекущийВладелецНастройки)
	   И ТекущийВладелецНастройки <> Параметры.СсылкаНаОбъект Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Estos derechos son heredados, se puede cambiarlos en forma del ajuste de los derechos "
"de la carpeta superior ""%1"".';ru='Эти права унаследованы, их можно изменить в форме настройки прав"
"вышестоящей папки ""%1"".';en='These rights are inherited, you can change them in the rights settings form"
"parent folder ""%1"".'"),
		...
		Если ПроверкаУдаления Тогда
			ТекстСообщения = ТекстСообщения + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Para la desaparición de todos los derechos heredados debe "
"quitar el banderín ""%1"".';ru='Для удаления всех унаследованных прав следует"
"снять флажок ""%1"".';en='To remove all inherited rights, you need to "
"uncheck ""%1"".'"),
...
Процедура ПроверитьРазрешениеНаУправлениеПравами()
	...
	ВызватьИсключение НСтр("es='El ajuste de los derechos es inaccesible.';ru='Настройка прав недоступна.';en='Rights setting is not available.'");
