...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(Параметры.Отправитель) = Тип("СписокЗначений") Тогда
		...
		Если НаборУчетныхЗаписей.Количество() = 0 Тогда
			ТекстСообщения = НСтр("es='del mensaje, se dirijan al administrador del sistema.';ru='Не указаны учетные записи для отправки сообщения, обратитесь к администратору системы.';en='Accounts are not specified for sending messages, contact your system administrator.'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,,,, Отказ);
			Возврат;
		КонецЕсли;
	...
	Если ТипЗнч(Параметры.Получатель) = Тип("СписокЗначений") Тогда
		...
		Для Каждого ЭлементПочтовыйАдрес Из Параметры.Получатель Цикл
			...
			НовыйПолучатель.ВариантОтправки = НСтр("es='A quien:';ru='Кому:';en='To:'");
	...
	Если ПочтовыеАдресаПолучателей.Количество() = 0 Тогда
		...
		НоваяСтрока.ВариантОтправки = НСтр("es='A quien:';ru='Кому:';en='To:'");
...
Процедура ПочтовыйАдресаПолучателейПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если СтрНайти(Адрес, "@") = 0 ИЛИ СтрНайти(Адрес, ".") = 0 Тогда
		ПоказатьПредупреждение(, НСтр("es='Es necesario introducir la dirección del correo electrónico';ru='Необходимо ввести адрес электронной почты';en='Email is required'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Для каждого ПолучательПисьма Из ПочтовыеАдресаПолучателей Цикл
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("es='Tal dirección del correo electrónico es ya en la lista.';ru='Такой адрес электронной почты уже есть в списке.';en='This email address is already on the list.'"));
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	КонецЦикла;
...
Процедура ОтправитьПисьмо()
	...
	Если ПоляЗаполненыКорректно() И ОтправитьПочтовоеСообщение() Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("es='El mensaje es enviado:';ru='Сообщение отправлено:';en='Message sent:'"), ,
			?(ПустаяСтрока(ТемаПисьма), НСтр("es='<Sin tema>';ru='<Без темы>';en='<Без темы>'"), ТемаПисьма), БиблиотекаКартинок.Информация32);
...
Функция ПоляЗаполненыКорректно()
	...
	Если ПочтовыеАдресаПолучателей.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='Es necesario rellenar al destinatario de la carta';ru='Необходимо заполнить получателя письма';en='Required to fill out the recipient of the letter'"), , "ПочтовыеАдресаПолучателей");
		Результат = Ложь;
	КонецЕсли;
	Для каждого ПолучательПочты Из ПочтовыеАдресаПолучателей Цикл
		...
		Если ПустаяСтрока(Адрес) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='Es necesario rellenar al destinatario de la carta';ru='Необходимо заполнить получателя письма';en='Required to fill out the recipient of the letter'"),, "ПочтовыеАдресаПолучателей[" + Формат(ПочтовыеАдресаПолучателей.Индекс(ПолучательПочты), "ЧГ=0") + "].Представление");
			Результат = Ложь;
		ИначеЕсли СтрНайти(Адрес, "@") = 0 Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='La dirección equivocada del correo electrónico';ru='Неверный адрес электронной почты';en='Incorrect E-Mail Address'"),, "ПочтовыеАдресаПолучателей[" + Формат(ПочтовыеАдресаПолучателей.Индекс(ПолучательПочты), "ЧГ=0") + "].Представление");
			Результат = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьТабличныйДокументПоДвоичнымДанным(Знач ДвоичныеДанные)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("es='La recepción del documento tabular';ru='Получение табличного документа';en='Retrieving a spreadsheet document'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , , 
...
Функция СформироватьПараметрыПисьма()
	...
	Для каждого Получатель Из ПочтовыеАдресаПолучателей Цикл
		...
		Для каждого ПочтаПолучателя Из ПочтаПолучателей Цикл
			Если Получатель.ВариантОтправки = НСтр("es='La copia escondida:';ru='Скрытая копия:';en='Hidden copy:'") Тогда
				СкрытыеКопии.Добавить(Новый Структура("Адрес, Представление", ПочтаПолучателя.Адрес, ПочтаПолучателя.Псевдоним));
			ИначеЕсли Получатель.ВариантОтправки = НСтр("es='La copia:';ru='Копия:';en='Copy:'") Тогда
...
Функция ПолучитьПриведенныйПочтовыйАдресВФормате(Текст)
	...
	Если Адреса.Количество() > 1 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='Se puede indicar solamente una dirección para la respuesta.';ru='Можно указывать только один адрес для ответа.';en='You can specify only one address for a response.'"), , "АдресОтвета");
		Возврат Текст;
	КонецЕсли;
...
Процедура ПоказатьВопросПередЗакрытиемФормы()
	ТекстВопроса = НСтр("es='El mensaje no es enviado. ¿Para cerrar forma?';ru='Сообщение еще не отправлено. Закрыть форму?';en='Message has not sent yet. Close the form?'");
	...
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки,,
		КодВозвратаДиалога.Отмена, НСтр("es='El envío del mensaje';ru='Отправка сообщения';en='Message sending'"));
КонецПроцедуры
	...
	Если Результат <> Неопределено Тогда
		...
		Если ТипЗнч(Результат.Получатель) = Тип("СписокЗначений") Тогда
			Для Каждого Получатель Из Результат.Получатель Цикл
				...
				АдресПолучателя.ВариантОтправки = НСтр("es='A quien:';ru='Кому:';en='To:'");
