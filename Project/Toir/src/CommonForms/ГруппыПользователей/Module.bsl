...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Пользователь <> Неопределено Тогда
		...
		Элементы.ФормаЗаписатьИЗакрыть.Заголовок = НСтр("es='Anotar';ru='Записать';en='Write down'");
	...
	Если КоличествоПользователей = 0 Тогда
		ВызватьИсключение НСтр("es='No es escogido ni un usuario.';ru='Не выбрано ни одного пользователя.';en='No user selected.'");
	КонецЕсли;
	...
	Для Каждого ПользовательИзМассива Из МассивПользователей Цикл
		...
		Если ТипПользователяИзМассива <> Тип("СправочникСсылка.Пользователи")
		   И ТипПользователяИзМассива <> Тип("СправочникСсылка.ВнешниеПользователи") Тогда
			...
			ВызватьИсключение НСтр("es='La orden no puede ser cumplida para el objeto indicado.';ru='Команда не может быть выполнена для указанного объекта.';en='The command could not be executed for the specified object.'");
		КонецЕсли;
		
		Если ТипПользователей <> ТипПользователяИзМассива Тогда
			ВызватьИсключение НСтр("es='La orden no puede ser cumplida en seguida para dos tipos diferentes de los usuarios.';ru='Команда не может быть выполнена сразу для двух разных видов пользователей.';en='The command cannot be executed immediately for two different kinds of users.'");
		КонецЕсли;
	КонецЦикла;
	...
	Если КоличествоПользователей > 1
	   И Параметры.Пользователь = Неопределено Тогда
		...
		Заголовок = НСтр("es='Los grupos de los usuarios';ru='Группы пользователей';en='Groups of users'");
	...
	Если УведомлениеПользователя.ЕстьОшибки = Ложь Тогда
		Если УведомлениеПользователя.Сообщение <> Неопределено Тогда
			ПоказатьОповещениеПользователя(
				НСтр("es='El traslado de los usuarios';ru='Перемещение пользователей';en='User movement'"), , УведомлениеПользователя.Сообщение, БиблиотекаКартинок.Информация32);
		КонецЕсли;
	Иначе
	...
	Если КоличествоНеПеремещенныхПользователей > 0 Тогда
		...
		Если КоличествоНеПеремещенныхПользователей = 1 Тогда
			...
			СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió incluir al usuario ""%1"" en los grupos escogidos, "
"porque a ellos se diferencia el tipo o cerca de los grupos es establecido el indicio ""los Todos usuarios del tipo dado"".';ru='Пользователя ""%1"" не удалось включить в выбранные группы,"
"т.к. у них различается тип или у групп установлен признак ""Все пользователи заданного типа"".';en='User ""%1"" could not be included in the selected groups, because they have different types or groups have the attribute ""All users of the specified type"".'"),
				Предмет);
		Иначе
			Предмет = Формат(КоличествоНеПеремещенныхПользователей, "ЧДЦ=0") + " "
				+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоНеПеремещенныхПользователей,
					"", НСтр("es='пользователю,пользователям,пользователям,,,,,,0';ru='пользователю,пользователям,пользователям,,,,,,0';en='user, users, users ,,,,,, 0'"));
			СообщениеПользователю =
				НСтр("es='No consiguió incluir a todos los usuarios en los grupos escogidos, "
"porque a ellos se diferencia el tipo o cerca de los grupos es establecido el indicio ""los Todos usuarios del tipo dado"".';ru='Не всех пользователей удалось включить в выбранные группы,"
"т.к. у них различается тип или у групп установлен признак ""Все пользователи заданного типа"".';en='Not all users were able to be included in the selected groups,"
"because they have different types or groups have the attribute "" All users of the specified type '");
			Для Каждого НеПеремещенныйПользователь Из НеПеремещенныеПользователи Цикл
			...
			УведомлениеПользователя.ПолныйТекстСообщения =
				НСтр("es='Los usuarios siguientes no eran incluidos en los grupos:';ru='Следующие пользователи не были включены в группы:';en='The following users were not included in the groups:'")
	...
	ИначеЕсли КоличествоПользователей = 1 Тогда
		...
		УведомлениеПользователя.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es cambiada la composición de los grupos al usuario ""%1""';ru='Изменен состав групп у пользователя ""%1""';en='Group composition changed for user ""%1""'"),
	...
	ИначеЕсли КоличествоПользователей > 1 Тогда
		СтрокаОбъект = Формат(КоличествоПользователей, "ЧДЦ=0") + " "
			+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоПользователей,
				"", НСтр("es='пользователя,пользователей,пользователей,,,,,,0';ru='пользователя,пользователей,пользователей,,,,,,0';en='user, users, users ,,,,,, 0'"));
		
		УведомлениеПользователя.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es cambiada la composición de los grupos a %1';ru='Изменен состав групп у %1';en='%1 group composition changed'"), СтрокаОбъект);
	КонецЕсли;
...
Процедура ЗаписатьИЗакрытьОбработкаВопроса(Ответ, ПолныйТекстСообщения) Экспорт
	...
	Если Ответ = "Ок" Тогда
		...
		ЗаписатьИЗакрытьЗавершение();
	Иначе
		ЗаголовокСообщения = НСтр("es='Los usuarios no incluidos en el grupo';ru='Пользователи, не включенные в группы';en='Users not in groups'");
