...
Процедура ОпределитьПоведениеВМобильномКлиенте()
	...
	Элементы.ТекстСлеваСверху.ПодсказкаВвода = НСтр("es='A la izquierda de arriba';ru='Слева сверху';en='Top left'");
	Элементы.ТекстВЦентреСверху.ПодсказкаВвода = НСтр("es='En el centro de arriba';ru='В центре сверху';en='In the center above'");
	Элементы.ТекстСправаСверху.ПодсказкаВвода = НСтр("es='A la derecha de arriba';ru='Справа сверху';en='Top right'");
	
	Элементы.ТекстСлеваСнизу.ПодсказкаВвода = НСтр("es='A la izquierda de abajo';ru='Слева снизу';en='Bottom left'");
	Элементы.ТекстВЦентреСнизу.ПодсказкаВвода = НСтр("es='En el centro de abajo';ru='В центре снизу';en='Center bottom'");
	Элементы.ТекстСправаСнизу.ПодсказкаВвода = НСтр("es='A la derecha de abajo';ru='Справа снизу';en='Bottom right'");
...
Процедура ПодготовитьОбразец()
	...
	Образец.Область(3, 2).Текст      = Символы.ПС + НСтр("es='El modelo del informe';ru='Образец отчета';en='Sample report'") + Символы.ПС + " ";
...
Функция ЗаполнитьШаблон(Шаблон, НачальнаяСтраница)
	...
	Иначе
		...
		Результат = СтрЗаменить(Результат, "[&НазваниеОтчета]", НСтр("es='El informe estandartizado';ru='Стандартный отчет';en='Standard report'"));
