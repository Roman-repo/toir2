...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ПараметрыОтчета") Тогда
		НастройкиОтчета.Вставить("ПараметрыОтчета", Параметры.ПараметрыОтчета);
	КонецЕсли;
	// ТОиР--
	...
	ОтобразитьСостояниеОтчета(НСтр("es='El informe no es formado. Presionen ""Formar"" para la recepción del informe.';ru='Отчет не сформирован. Нажмите ""Сформировать"" для получения отчета.';en='The report is not generated. Click ""Generate"" to get a report.'"));
...
Процедура ПриЗагрузкеВариантаНаСервере(НовыеНастройкиКД)
	...
	// ТОиР++
	торо_ПоказателиKPI.ОтчетРасшифровка_ПриЗагрузкеВариантаНаСервере(РеквизитФормыВЗначение("Отчет"), ЭтаФорма, НовыеНастройкиКД);
	// ТОиР--
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого ЭлементНастройки Из ЭлементыНастроек Цикл
		...
		Если ДатаНачала.АвтоОтметкаНезаполненного = Истина
			...
			ТекстОшибки = НСтр("es='el período';ru='Не указан период';en='Period not specified'");
			ПутьКДанным = ДатаНачала.ПутьКДанным;
		ИначеЕсли Значение.ДатаНачала > Значение.ДатаОкончания Тогда
			ТекстОшибки = НСтр("es='El fin del período debe ser más de comienzo';ru='Конец периода должен быть больше начала';en='End of period must be greater than beginning'");
...
Процедура ОтправитьПоПочте(Команда)
	...
	Если ОтображениеСостояния.Видимость = Истина
		...
		ТекстВопроса = НСтр("es='El informe no es formado. ¿Formar?';ru='Отчет не сформирован. Сформировать?';en='The report is not generated. Form?'");
...
Процедура ОтчетСкомпоноватьРезультат(Команда)
	...
	// ТОИР++
	Если ЭтотОбъект.ИмяФормы = "Отчет.торо_ПланМТОТрудозатратИнструментов.Форма" 
		ИЛИ ЭтотОбъект.ИмяФормы = "Отчет.торо_ПланФактЗатрат.Форма"
		ИЛИ ЭтотОбъект.ИмяФормы = "Отчет.торо_ПланФактТрудозатрат.Форма" Тогда
		
		ОписаниеОшибки = "";
		Если слкМенеджерЗащиты.ПроверитьЛицензииСеанса(ОписаниеОшибки) <> Истина Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("es='¡La licencia no es descubierta! La formación del informe es imposible.';ru='Лицензия не обнаружена! Формирование отчета невозможно.';en='No license found! Report generation is not possible.'"));
			Возврат;
		КонецЕсли;	
	КонецЕсли;
	// ТОИР--
...
Процедура ЗагрузитьСхему(Команда)
	...
	ПараметрыЗагрузки.Диалог.Фильтр = НСтр("es='Los ficheros XML (*.xml) |*.xml';ru='Файлы XML (*.xml) |*.xml';en='XML Files (* .xml) | * .xml'");
...
#Иначе
	ПоказатьПредупреждение(, (НСтр("es='Para redactar el esquema de la composición, es necesario poner en marcha la aplicación en el régimen del cliente gordo.';ru='Для того чтобы редактировать схему компоновки, необходимо запустить приложение в режиме толстого клиента.';en='In order to edit the layout, you must run the application in the thick client mode.'")));
#КонецЕсли
КонецПроцедуры
...
Процедура Подключаемый_ЗагрузитьВариантОтчета(Команда)
	...
	Если Найденные.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("es='La variante del informe no es encontrada.';ru='Вариант отчета не найден.';en='Replace report option marked for deletion? ""'"));
		Возврат;
	КонецЕсли;
...
Процедура Сформировать()
	ВыполнятьЗамеры = НастройкиОтчета.ВыполнятьЗамеры И ЗначениеЗаполнено(НастройкиОтчета.КлючЗамеров);
	Если ВыполнятьЗамеры Тогда
		Комментарий = НастройкиОтчета.ПрефиксЗамеров + "; " + НСтр("es='Directamente:';ru='Непосредственно:';en='Directly:'") + " " + Строка(Непосредственно);
		МодульОценкаПроизводительностиКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ОценкаПроизводительностиКлиент");
		ИдентификаторЗамера = МодульОценкаПроизводительностиКлиент.ЗамерВремени(
...
Процедура ПослеФормирования(Результат, ЗагрузитьРезультатФормированияОтчета) Экспорт 
	...
	Если Результат.Статус = "Выполнено" Тогда 
		...
		КонецЕсли;	
		ПоказатьОповещениеПользователя(НСтр("es='El informe es formado';ru='Отчет сформирован';en='Report generated'"),, Заголовок);
	ИначеЕсли Результат.Статус = "Ошибка" Тогда
		ПоказатьОшибкиФормирования(Результат.КраткоеПредставлениеОшибки);
		ПоказатьОповещениеПользователя(НСтр("es='El informe no es formado';ru='Отчет не сформирован';en='Report not generated'"),, Заголовок);
	КонецЕсли;
...
Процедура ПоказатьСписокВыбора(Элемент, СтандартнаяОбработка)
	...
	ВыборГруппИЭлементов = ОтчетыКлиент.ЗначениеТипаИспользованиеГруппИЭлементов(
		?(Сведения.Описание = Неопределено, Неопределено, Сведения.Описание.ВыборГруппИЭлементов), Условие);
...
Процедура УстановитьВидимостьДоступность(ПриСохраненииВарианта = Ложь)
	...
	Если РежимРасшифровки Тогда
		Заголовок = Заголовок + " (" + НСтр("es='El desciframiento';ru='Расшифровка';en='Decryption'") + ")";
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗагрузитьВариант(КлючВарианта)
	...
	ОтобразитьСостояниеОтчета(НСтр("es='Es escogida otra variante del informe. Presionen ""Formar"" para la recepción del informe.';ru='Выбран другой вариант отчета. Нажмите ""Сформировать"" для получения отчета.';en='Another report option selected. Click ""Generate"" to get a report.'"),
...
Процедура ОпределитьПоведениеВМобильномКлиенте()
	...
	Элементы.КомандыИПоказатели.Заголовок = НСтр("es='Los índices';ru='показатели';en='Showing%1 -%2 of%3'");
...
Процедура ОбновитьЭлементыФормыНастроекНаСервере(ПараметрыОбновления = Неопределено)
	...
	Если ПараметрыОбновления.ИмяСобытия <> "ПослеФормирования" Тогда
		...
		ИначеЕсли ПараметрыОбновления.ВариантМодифицирован
			...
			ОтобразитьСостояниеОтчета(НСтр("es='Se han cambiado los ajustes. Presionen ""Formar"" para la recepción del informe.';ru='Изменились настройки. Нажмите ""Сформировать"" для получения отчета.';en='Changed settings. Click ""Generate"" to get a report.'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ПоказатьОшибкиФормирования(ИнформацияОбОшибке)
	Если ТипЗнч(ИнформацияОбОшибке) = Тип("ИнформацияОбОшибке") Тогда
		...
		ПодробноеПредставлениеОшибки = НСтр("es='La falta a la formación:';ru='Ошибка при формировании:';en='Formation error:'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
