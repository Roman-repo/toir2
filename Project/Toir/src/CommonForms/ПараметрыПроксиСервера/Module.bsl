...
""
"El ajuste del proksi-servidor se cumple por el administrador.';ru='Недостаточно прав доступа."
""
"Настройка прокси-сервера выполняется администратором.';en='Not enough access rights."
""
"The proxy is configured by the administrator.'");
	КонецЕсли;
	...
	Иначе
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("es='Los parámetros del proksi-servidor en el servidor 1С:Предприятия';ru='Параметры прокси-сервера на сервере 1С:Предприятия';en='Parameters of the proxy server on the 1C: Enterprise server'");
...
#Если ВебКлиент Тогда
		ПоказатьПредупреждение(, НСтр("es='En el cliente web es necesario dar los parámetros del proksi-servidor en los ajustes del navegador.';ru='В веб-клиенте параметры прокси-сервера необходимо задавать в настройках браузера.';en='In the web client, the proxy server settings must be set in the browser settings.'"));
		Отказ = Истина;
		Возврат;
#КонецЕсли
