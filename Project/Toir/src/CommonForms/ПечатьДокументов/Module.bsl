...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Параметры.ИсточникДанных) Тогда 
		ОбщегоНазначенияКлиентСервер.Проверить(ТипЗнч(Параметры.ПараметрКоманды) = Тип("Массив") Или ОбщегоНазначения.ЗначениеСсылочногоТипа(Параметры.ПараметрКоманды),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El significado inadmisible del parámetro De ParametrKomandy con la llamada del método УправлениеПечатьюКлиент.ВыполнитьКомандуПечати. "
"Se espera: el Macizo, LyubayaSsylka. "
"Es entregado: %1';ru='Недопустимое значение параметра ПараметрКоманды при вызове метода УправлениеПечатьюКлиент.ВыполнитьКомандуПечати."
"Ожидалось: Массив, ЛюбаяСсылка."
"Передано: %1';en='Invalid value of the parameter ПараметрКоманды when calling the method УправлениеПечатьюКлиент.ВыполнитьКомандуПечати."
"Expected: Array, AnyReference."
"Submitted: %1'"), ТипЗнч(Параметры.ПараметрКоманды)));
	КонецЕсли;
	...
	Если Не ОбщегоНазначения.ЭтоМобильныйКлиент() И ЭтоПечатьКомплекта() Тогда
		Элементы.Копий.Заголовок = НСтр("es='De las copias del juego';ru='Копий комплекта';en='Copies of the kit'");
	КонецЕсли;
	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег("ОбщаяФорма.СохранениеПечатнойФормы") Тогда
		...
		Если ВыбранноеЗначение <> Неопределено И ВыбранноеЗначение <> КодВозвратаДиалога.Отмена Тогда
			...
			Иначе
				...
				КонецЦикла;
				ПоказатьОповещениеПользователя(, , НСтр("es='La conservación es acabada';ru='Сохранение завершено';en='Saving complete'"), БиблиотекаКартинок.Информация32);
			КонецЕсли;
		КонецЕсли;
...
Процедура ПерейтиКДокументу(Команда)
	...
	СписокВыбора.ПоказатьВыборЭлемента(ОписаниеОповещения, НСтр("es='Pasar a la forma de imprenta';ru='Перейти к печатной форме';en='Go to print'"));
	...
	Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
		Если НастройкаПечатнойФормы.Количество <> 1 Тогда
			НастройкаПечатнойФормы.Представление = НастройкаПечатнойФормы.Представление 
				+ " (" + НастройкаПечатнойФормы.Количество + " " + НСтр("es='La copia';ru='экз.';en='instance'") + ")";
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
	...
	Если ЕстьРазрешенныйВывод Тогда
		Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
			...
			Если ПолеТабличногоДокумента.Вывод = ИспользованиеВывода.Запретить Тогда
				НастройкаПечатнойФормы.Представление = НастройкаПечатнойФормы.Представление + " (" + НСтр("es='La conclusión no es accesible';ru='вывод не доступен';en='no output available'") + ")";
...
Процедура УстановитьИмяПринтераВПодсказкеКнопкиПечать()
	Если НастройкиПечатныхФорм.Количество() > 0 Тогда
		...
		Если Не ПустаяСтрока(ИмяПринтера) Тогда
			ЭтотОбъект.Команды["Печать"].Подсказка = НСтр("es='Imprimir sobre la impresora';ru='Напечатать на принтере';en='Print to printer'") + " (" + ИмяПринтера + ")";
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура УстановитьЗаголовокФормы()
	...
	Иначе
		Если ЭтоПечатьКомплекта() Тогда
			Заголовок = НСтр("es='La prensa del juego';ru='Печать комплекта';en='Printing set'");
		ИначеЕсли ТипЗнч(Параметры.ПараметрКоманды) <> Тип("Массив") Или Параметры.ПараметрКоманды.Количество() > 1 Тогда
			Заголовок = НСтр("es='La prensa del juego';ru='Печать комплекта';en='Printing set'");
		Иначе
			Заголовок = НСтр("es='La prensa de los documentos';ru='Печать документов';en='Printing documents'");
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Функция ПолучитьИмяФайлаДляАрхива(ПереводитьИменаФайловВТранслит)
	...
	Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
		...
		Иначе
			Результат = НСтр("es='Los documentos';ru='Документы';en='Documents'");
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура СохранитьПечатныеФормыВПапку(СписокФайловВоВременномХранилище, Знач Папка = "")
	...
	ПоказатьОповещениеПользователя(НСтр("es='La conservación es con éxito acabada';ru='Сохранение успешно завершено';en='Saving completed successfully'"), "file:///" + Папка, НСтр("es='En la carpeta:';ru='в папку:';en='to folder:'") + " " + Папка, БиблиотекаКартинок.Информация32);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСФайлами") Тогда
		...
		Для Каждого Файл Из ФайлыВоВременномХранилище Цикл
			...
			Результат.Добавить(МодульРаботаСФайлами.ДобавитьФайл(
				ПараметрыФайла, Файл.АдресВоВременномХранилище, , НСтр("es='La forma de imprenta';ru='Печатная форма';en='Printing form'")));
		КонецЦикла;
	КонецЕсли;
	Возврат Результат;
КонецФункции
...
Процедура ОткрытьМакетДляРедактирования()
	...
	ОтобразитьСостояниеТекущейПечатнойФормы(НСтр("es='La maqueta es redactada';ru='Макет редактируется';en='The layout is being edited.'"));
...
Процедура ПереформироватьПечатнуюФорму(ИмяМакета, ИмяРеквизита)
	...
	Если Отказ Тогда
		ВызватьИсключение НСтр("es='La forma de imprenta no era reformada.';ru='Печатная форма не была переформирована.';en='The print form has not been reorganized.'");
	КонецЕсли;
