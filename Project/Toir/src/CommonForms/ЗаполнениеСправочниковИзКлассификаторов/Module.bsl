...
Процедура СообщитьОЗагруженностиВсегоКлассификатора()
	ТекстПредупреждения = НСтр("es='Es cargado todo el clasificador';ru='Загружен весь классификатор';en='All classifier downloaded'");
	ПоказатьПредупреждение(, ТекстПредупреждения, , НСтр("es='La carga del clasificador';ru='Загрузка классификатора';en='Classifier loading'"));
КонецПроцедуры
...
Функция ПолучитьМакетКлассификатора(ПолныйПутьКМакету)
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La maqueta del clasificador ""%1"" no es encontrada. La operación es interrumpida.';ru='Макет классификатора ""%1"" не найден. Операция прервана.';en='Classifier layout ""%1"" not found. Operation aborted.'"), ПолныйПутьКМакету);
	КонецЕсли;
	...
	ИначеЕсли НЕ ЗагружаемыйЭлемент.Загружен Тогда
		...
		Исключение
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió cargar el elemento %1 (el código: %2)';ru='Не удалось загрузить элемент %1 (код: %2)';en='Failed to load item%1 (code:%2)'"),
