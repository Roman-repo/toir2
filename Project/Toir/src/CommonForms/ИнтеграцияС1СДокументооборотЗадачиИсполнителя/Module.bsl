...
Процедура ОбработатьФормуСогласноВерсииСервиса()
	...
	Если ПустаяСтрока(ВерсияСервиса) Тогда // идет подключение
		...
		Заголовок = НСтр("es='La circulación de documentos: Mis tareas (se espera la conexión...)';ru='Документооборот: Мои задачи (ожидается подключение...)';en='Document management: My tasks (connection expected ...)'");
		
	Иначе
		
		Заголовок = НСтр("es='La circulación de documentos: Mis tareas';ru='Документооборот: Мои задачи';en='Document management: My tasks'");
		
		Если ВерсияСервиса = "0.0.0.0" Тогда
		
			Элементы.ГруппаФункционалНеПоддерживается.Видимость = Истина;
			Элементы.ГруппаПроверкаАвторизации.Видимость = Истина;
			Элементы.ДекорацияФункционалНеПоддерживается.Заголовок = НСтр("es='No hay acceso a 1С:Документообороту.';ru='Нет доступа к 1С:Документообороту.';en='No access to 1C: Document management.'");
