...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	торо_ЛокализацияМодификацияФорм.ДобавитьНастройкуЯзыкаВПользовательскихНастройках(ЭтаФорма);
	// << [Деснол] [Ушаков А.Д.] [10.10.2019] 
	// <==ТОиР=
...
Процедура ОбновитьГруппуУстановкиРасширенияРаботыСФайлами()
	...
	Текст = ?(Подключено, НСтр("es='La ampliación del trabajo con los ficheros en el navegador web es establecida.';ru='Расширение работы с файлами в веб-браузере установлено.';en='The extension for working with files in a web browser is installed.'"), 
		НСтр("es='Para la ejecución serie de las acciones en el trabajo en el cliente web "
"es necesario establecer la ampliación del trabajo con los ficheros en el ordenador dado.';ru='Для выполнения ряда действий при работе в веб-клиенте"
"требуется установить расширение работы с файлами на данном компьютере.';en='To perform a series of actions when working in the web client"
"you need to install the file extension on this computer.'"));
...
#КонецЕсли
	...
	// >> [Деснол] [Ушаков А.Д.] [10.10.2019] 
	торо_Локализация.СохранитьЯзыкТекущегоПользователя(ЭтаФорма.дс_ЯзыкРедактирования);
	// << [Деснол] [Ушаков А.Д.] [10.10.2019] 
