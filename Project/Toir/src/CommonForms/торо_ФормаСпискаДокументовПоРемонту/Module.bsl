		...
		Если Параметры.Свойство("ТабЧасть") Тогда
			Для каждого Стр Из Документ[Параметры.ТабЧасть] Цикл
				...
				ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.торо_ВнешнееОснованиеДляРабот") Тогда 
					НС.ВидРемонтов = Константы.торо_ВидРемонтаПриВводеНаОснованииВнешнихОснований.Получить();
				Иначе
...
Процедура Провести(Команда)
	...
	Если ТекДанные.ПометкаУдаления Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("es='¡El documento, marcado a la desaparición, no puede ser pasado!';ru='Помеченный на удаление документ не может быть проведен!';en='A document marked for deletion cannot be posted!'"));
		Возврат;
	ИначеЕсли ПолучитьДатуДокумента(ТекДанные.Документ) > КонецДня(ТекущаяДата()) Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("es='¡No es posible pasar los documentos por la próxima fecha!';ru='Нельзя проводить документы будущей датой!';en='Can not post documents with a future date!'"));
		Возврат;
	КонецЕсли;
...
Процедура ОтменаПроведения(Команда)
	...
	Если НЕ ТекДанные.Проведен Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("es='El documento no es pasado.';ru='Документ не проведен.';en='The document is not posted.'"));
		Возврат;
	КонецЕсли;
	Попытка
