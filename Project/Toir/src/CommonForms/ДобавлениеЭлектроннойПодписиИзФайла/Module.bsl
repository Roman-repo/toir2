...
Процедура ОК(Команда)
	...
	Если Подписи.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("es='No es escogido ni un fichero de la firma';ru='Не выбрано ни одного файла подписи';en='No signature files selected'"));
		Возврат;
	КонецЕсли;
...
Процедура ОКЗавершение(Результат, Контекст = Неопределено) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("es='No consiguió añadir la firma electrónica del fichero';ru='Не удалось добавить электронную подпись из файла';en='Failed to add electronic signature from file'"), "", Ошибка, Новый Структура);
		Возврат;
	КонецЕсли;
...
Процедура ВыбратьФайл(ДобавитьНовуюСтроку = Ложь)
	...
	ПараметрыЗагрузки.Диалог.Заголовок = НСтр("es='Escojan el fichero de la firma electrónica';ru='Выберите файл электронной подписи';en='Select an electronic signature file'");
	ПараметрыЗагрузки.Диалог.Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Los ficheros de la firma (*. El %1) | *. El %1|Все los ficheros (*. *) |*.*';ru='Файлы подписи (*.%1)|*.%1|Все файлы(*.*)|*.*';en='Signature files (*.%1) | *.%1 | All Files (*. *) | *. *'"),
...
Процедура ВыбратьФайлПослеОшибкиПолученияСертификатовИзПодписи(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='A la recepción de los certificados del fichero de la firma había una falta: "
"%1';ru='При получении сертификатов из файла подписи произошла ошибка:"
"%1';en='An error occurred while getting certificates from the signature file:"
"%1'"),
...
Процедура ВыбратьФайлПослеПолученияСертификатовИзПодписи(Сертификаты, Контекст) Экспорт
	...
	Если Сертификаты.Количество() = 0 Тогда
		ОшибкаНаКлиенте = Новый Структура("ОписаниеОшибки",
			НСтр("es='En el fichero de la firma no existe ni un certificado.';ru='В файле подписи нет ни одного сертификата.';en='There are no certificates in the signature file.'"));
...
"%1'"),
	...
	Если Сертификаты.Количество() = 0 Тогда
		ОшибкаНаСервере.Вставить("ОписаниеОшибки", НСтр("es='En el fichero de la firma no existe ni un certificado.';ru='В файле подписи нет ни одного сертификата.';en='There are no certificates in the signature file.'"));
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПоказатьОшибку(ОшибкаНаКлиенте, ОшибкаНаСервере)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("es='No consiguió recibir la firma del fichero';ru='Не удалось получить подпись из файла';en='Failed to get signature from file.'"), "", ОшибкаНаКлиенте, ОшибкаНаСервере);
