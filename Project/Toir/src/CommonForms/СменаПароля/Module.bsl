		...
		Исключение
			...
			Если Не ПриВходеВПрограмму Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No consiguió abrir la forma del cambio de la consigna por la causa: "
""
"%1';ru='Не удалось открыть форму смены пароля по причине:"
""
"%1';en='Failed to open password reset form due to:"
""
"%1'"),
	...
	Если ПриВходеВПрограмму Тогда
		...
		Иначе
			...
			Элементы.ПояснениеПриВходе.Заголовок =
				НСтр("es='Para la entrada en el programa es necesario establecer la consigna.';ru='Для входа в программу нужно установить пароль.';en='To enter the program password must be setted.'")
		КонецЕсли;
	Иначе
		...
		Элементы.ФормаЗакрытьФорму.Заголовок = НСтр("en='cancel';es='La anulación';ru='Отмена'");
	КонецЕсли;
	...
	Если Не ДополнительныеПараметры.ЭтоТекущийПользовательИБ
	 Или Не ДополнительныеПараметры.ПарольУстановлен Тогда
		...
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("es='La instalación de la consigna';ru='Установка пароля';en='Password setting'");
...
Процедура УстановитьПароль(Команда)
	...
	Если Не ПоказыватьНовыйПароль И НовыйПароль <> Подтверждение Тогда
		...
		ПоказатьПредупреждение(, НСтр("es='La confirmación de la consigna es indicada es incorrecto.';ru='Подтверждение пароля указано некорректно.';en='Password confirmation is incorrect.'"));
		Возврат;
	КонецЕсли;
