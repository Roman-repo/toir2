...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() И Параметры.ЗапрашиватьПараметрыАдминистрированияКластера Тогда
		ВызватьИсключение НСтр("es='El ajuste de los parámetros del clúster de los servidores es accesible solamente en el cliente-del servidor el régimen del trabajo.';ru='Настройка параметров кластера серверов доступна только в клиент-серверном режиме работы.';en='Configuring server cluster parameters is available only in client-server operation mode.'");
	КонецЕсли;
	
	Если Параметры.ЗапрашиватьПараметрыАдминистрированияКластера
		И ОбщегоНазначения.ЭтоOSXКлиент() Тогда
...
Процедура ПриОткрытии(Отказ)
	...
	Если Параметры.ЗапрашиватьПараметрыАдминистрированияКластера
		И ОбщегоНазначенияКлиент.ЭтоOSXКлиент() Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("es='La conexión al clúster de los servidores es inaccesible en el cliente bajo la dirección de la SO X.';ru='Подключение к кластеру серверов недоступно в клиенте под управлением ОС X.';en='Connection to a server cluster is not available in a client running OS X.'");
