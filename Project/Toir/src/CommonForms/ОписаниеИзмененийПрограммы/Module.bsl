...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Que nuevo en las configuraciones %1';ru='Что нового в конфигурации %1';en=""What's new in configuration%1"""), Метаданные.Синоним);
	...
	Если ДокументОписаниеОбновлений.ВысотаТаблицы = 0 Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La configuración es con éxito renovada a la versión %1';ru='Конфигурация успешно обновлена на версию %1';en='Configuration successfully updated to version%1'"), Метаданные.Версия);
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		ЗаголовокСообщения = НСтр("es='Es necesario cumplir los procedimientos adicionales del tratamiento de los datos';ru='Необходимо выполнить дополнительные процедуры обработки данных';en='Additional data processing required'");
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь(, Истина) Тогда
		Элементы.ОтложенноеОбновлениеДанных.Заголовок =
			НСтр("es='de los datos';ru='Не выполнены дополнительные процедуры обработки данных';en='Additional data processing procedures not performed'");
	КонецЕсли;
...
Процедура ОтключеныРегламентныеЗаданияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ТекстВопроса = НСтр("es='¿Перезапустить el programa?';ru='Перезапустить программу?';en='Restart the program?'");
