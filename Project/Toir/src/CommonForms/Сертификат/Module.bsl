	...
	ИначеЕсли ЗначениеЗаполнено(Параметры.Ссылка) Тогда
		...
		Если АдресСертификата = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió abrir el certificado ""%1"", "
"porque él no es encontrado en la guía.';ru='Не удалось открыть сертификат ""%1"","
"т.к. он не найден в справочнике.';en='Failed to open certificate ""%1"","
"because it is not found in the directory.'"), Параметры.Ссылка);
		КонецЕсли;
	Иначе // Отпечаток
		...
		Если АдресСертификата = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió abrir el certificado, porque él no es encontrado por la huella "
"""%1"".';ru='Не удалось открыть сертификат, т.к. он не найден"
"по отпечатку ""%1"".';en='Failed to open certificate because it was not found"
"by fingerprint ""%1"".'"), Параметры.Отпечаток);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьЗавершение(Результат, Контекст) Экспорт
	...
	Если Результат = Истина Тогда
		ПоказатьПредупреждение(, НСтр("es='El certificado es válido.';ru='Сертификат действителен.';en='The certificate is valid.'"));
		
	ИначеЕсли Результат <> Неопределено Тогда
		ПараметрыПредупреждения = Новый Структура;
		
		ПараметрыПредупреждения.Вставить("ТекстПредупреждения", Результат);
		ПараметрыПредупреждения.Вставить("ЗаголовокПредупреждения", НСтр("es='El certificado es no válido por la causa:';ru='Сертификат недействителен по причине:';en='Certificate is not valid due to:'"));
...
Процедура ЗаполнитьСвойстваСубъекта(Сертификат)
	...
	ПредставленияСвойств["ОбщееИмя"] = НСтр("es='El nombre general';ru='Общее имя';en='General name'");
	ПредставленияСвойств["Страна"] = НСтр("es='El país';ru='Страна';en='Country'");
	ПредставленияСвойств["Регион"] = НСтр("es='La región';ru='Регион';en='Region'");
	ПредставленияСвойств["НаселенныйПункт"] = НСтр("es='La localidad';ru='Населенный пункт';en='Locality'");
	ПредставленияСвойств["Улица"] = НСтр("es='La calle';ru='Улица';en='Street'");
	ПредставленияСвойств["Организация"] = НСтр("es='La organización';ru='Организация';en='Organization'");
	ПредставленияСвойств["Подразделение"] = НСтр("es='La subdivisión';ru='Подразделение';en='Subdivision'");
	ПредставленияСвойств["ЭлектроннаяПочта"] = НСтр("es='El correo electrónico';ru='Электронная почта';en='Email'");
...
Процедура ЗаполнитьВнутренниеПоляСертификата()
	...
	Если ГруппаВнутреннихПолей = "Общие" Тогда
		...
		ДобавитьСвойство(Сертификат, "Версия",                    НСтр("es='La versión';ru='Версия';en='Version'"));
		ДобавитьСвойство(Сертификат, "ДатаНачала",                НСтр("es='La fecha ha comenzado';ru='Дата начала';en='Start date'"));
		ДобавитьСвойство(Сертификат, "ДатаОкончания",             НСтр("es='La versión';ru='Версия';en='Version'"));
		ДобавитьСвойство(Сертификат, "ИспользоватьДляПодписи",    НСтр("es='La fecha ha comenzado';ru='Дата начала';en='Start date'"));
		ДобавитьСвойство(Сертификат, "ИспользоватьДляШифрования", НСтр("es='La fecha de la terminación';ru='Дата окончания';en='End date'"));
		ДобавитьСвойство(Сертификат, "ОткрытыйКлюч",              НСтр("es='Usar para la firma';ru='Использовать для подписи';en='Use for signature'"), Истина);
		ДобавитьСвойство(Сертификат, "Отпечаток",                 НСтр("es='Usar para la cifración';ru='Использовать для шифрования';en='Use for encryption'"), Истина);
		ДобавитьСвойство(Сертификат, "СерийныйНомер",             НСтр("es='La llave abierta';ru='Открытый ключ';en='Open key'"), Истина);
