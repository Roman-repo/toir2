...
Процедура Сохранить(Команда)
	...
	Если ВариантСохранения = "СохранитьВПапку" И ПустаяСтрока(ВыбраннаяПапка) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("es='Es necesario indicar la carpeta.';ru='Необходимо указать папку.';en='Required to specify a folder.'"),,"ВыбраннаяПапка");
		Возврат;
	КонецЕсли;
	#КонецЕсли
	...
	Если ФорматыСохранения.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("es='Es necesario indicar como mínimo un de los formatos propuestos.';ru='Необходимо указать как минимум один из предложенных форматов.';en='Requirerd to specify at least one of the proposed formats.'"));
		Возврат;
	КонецЕсли;
