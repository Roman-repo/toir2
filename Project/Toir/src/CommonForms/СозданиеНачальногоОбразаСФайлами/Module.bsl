...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Узел = Менеджер.ЭтотУзел() Тогда
		ВызватьИсключение
			НСтр("es='La creación de la imagen inicial para el nudo dado es imposible.';ru='Создание начального образа для данного узла невозможно.';en='Creating an initial image for this node is not possible.'");
	Иначе
	...
	Если Не ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		Элементы.ПутьКАрхивуСФайламиТомов.ПодсказкаВвода = НСтр("es='\\el nombre del servidor \resource\files.zip';ru='\\имя сервера\resource\files.zip';en='\\ server name \ resource \ files.zip'");
...
Процедура СоздатьНачальныйОбраз(Команда)
	...
	Если ВидБазы = 0 И НЕ МожноСоздатьФайловуюБазу Тогда
		...
		ВызватьИсключение
			НСтр("es='La creación de la imagen inicial de la base de fichero informativa "
"en la plataforma dada no es apoyada.';ru='Создание начального образа файловой информационной базы"
"на данной платформе не поддерживается.';en='Creating an initial image of a file infobase"
"not supported on this platform. '");
	Иначе
		...
		Если ВидБазы = 0 Тогда
			...
			ПараметрыЗадания.Вставить("НаименованиеЗадания", НСтр("es='La creación de la imagen de fichero inicial';ru='Создание файлового начального образа';en='Creating a file initial image'"));
		...
		Иначе
			...
			ПараметрыЗадания.Вставить("НаименованиеЗадания", НСтр("es='La creación de la imagen del servidor inicial';ru='Создание серверного начального образа';en='Creating a server starter image'"));
...
Процедура ОбработчикСохраненияФайла(
	...
	Диалог.Заголовок = НСтр("es='Escojan el fichero para la conservación';ru='Выберите файл для сохранения';en='Select a file to save'");
...
Процедура ЗапуститьСозданиеНачальногоОбраза()
	...
	Иначе
		ВызватьИсключение НСтр("es='No consiguió crear la imagen inicial por la causa:';ru='Не удалось создать начальный образ по причине:';en='Failed to create initial image due to:'") + " " + Результат.КраткоеПредставлениеОшибки; 
	КонецЕсли;
...
Процедура СоздатьНачальныйОбразНаСервереЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Неопределено Тогда
		ПроцентВыполнения = 0;
		ДопИнформацияВыполнение = НСтр("es='La acción es anulada por el administrador.';ru='Действие отменено администратором.';en='The action was canceled by the administrator.'");
	...
	Если Результат.Статус = "Ошибка" Тогда
		ПроцентВыполнения = 0;
		Элементы.СтатусГотово.Заголовок = НСтр("es='No consiguió crear la imagen inicial por la causa:';ru='Не удалось создать начальный образ по причине:';en='Failed to create initial image due to:'") + " " + Результат.КраткоеПредставлениеОшибки;
		ЗапуститьПереходРезультат();
		Возврат;
	КонецЕсли;
