...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Элементы.Программы.Заголовок =
			НСтр("es='La lista de los programas previstos por el administrador, que se puede usar en el ordenador';ru='Список программ, предусмотренных администратором, которые можно использовать на компьютере';en='List of programs provided by the administrator that can be used on the computer'");
	КонецЕсли;
...
Процедура ПрограммыLinuxПутьКПрограммеПриИзменении(Элемент)
	...
	Если БезПраваСохранениеДанныхПользователя Тогда
		...
		ПоказатьПредупреждение(,
			НСтр("es='Es imposible conservar la vía al programa. Falta el derecho de la conservación de los datos. "
"Se dirijan al administrador.';ru='Невозможно сохранить путь к программе. Отсутствует право сохранения данных."
"Обратитесь к администратору.';en='Unable to save the path to the program. There is no right to save data."
...
Процедура ПрограммыУстановитьПометкуУдаления(Команда)
	...
	Если ТекущиеДанные.ПометкаУдаления Тогда
		ТекстВопроса = НСтр("es='¿Quitar con ""%1"" marca a la desaparición?';ru='Снять с ""%1"" пометку на удаление?';en='Unmark deletion from ""%1""?'");
	Иначе
...
Процедура ОбновитьТекущуюВидимостьЭлементов()
	...
	Если Константы.ИспользоватьШифрование.Получить()
	 Или ЭлектроннаяПодпись.ОбщиеНастройки().ЗаявлениеНаВыпускСертификатаДоступно Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"СертификатыСоздать", "Заголовок", НСтр("es='Añadir...';ru='Добавить...';en='Add ...'"));
		
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"СертификатыКонтекстноеМенюСоздать", "Заголовок", НСтр("es='Añadir...';ru='Добавить...';en='Add ...'"));
	...
	Если Константы.ИспользоватьШифрование.Получить() Тогда
		Если ЭлектроннаяПодписьСлужебный.ИспользоватьЭлектроннуюПодписьВМоделиСервиса() Тогда
			Элементы.ДобавитьИзУстановленныхНаКомпьютере.Заголовок =
				НСтр("es='De establecido en el servicio nuboso y en el ordenador...';ru='Из установленных в облачном сервисе и на компьютере...';en='From installed in the cloud service and on the computer...'");
		Иначе
			Элементы.ДобавитьИзУстановленныхНаКомпьютере.Заголовок =
				НСтр("es='De establecido en el ordenador...';ru='Из установленных на компьютере...';en='From installed on the computer...'");
		КонецЕсли;
	Иначе
		Если ЭлектроннаяПодписьСлужебный.ИспользоватьЭлектроннуюПодписьВМоделиСервиса() Тогда
			Элементы.ДобавитьИзУстановленныхНаКомпьютере.Заголовок =
	...
	Если Константы.ИспользоватьЭлектронныеПодписи.Получить() Тогда
		ЗаголовокФлажка = НСтр("es='Comprobar las firmas y los certificados en el servidor';ru='Проверять подписи и сертификаты на сервере';en='Check signatures and certificates on the server'");
		ПодсказкаФлажка =
			НСтр("es='Permite no establecer el programa al ordenador del usuario "
"para la comprobación de las firmas electrónicas y los certificados. "
""
"Importante: a cada ordenador, donde trabaja el servidor 1С:Предприятия "
"o el servidor web que usa la base de fichero informativa, "
"debe ser establecido por lo menos un de los programas en la lista.';ru='Позволяет не устанавливать программу на компьютер пользователя"
...
"должна быть установлена хотя бы одна из программ в списке.';en=""Allows not to install the program on the user's computer"
"for verification of electronic signatures and certificates."
""
"Important: for each computer where the 1C: Enterprise server is running"
"or a web server using a file infobase,"
"at least one of the programs in the list must be installed.""");
	...
	Иначе
		ЗаголовокФлажка = НСтр("es='Comprobar los certificados en el servidor';ru='Проверять сертификаты на сервере';en='Check certificates on the server'");
		ПодсказкаФлажка =
			НСтр("es='Permite no establecer el programa al ordenador del usuario "
"para la comprobación de los certificados. "
""
"Importante: a cada ordenador, donde trabaja el servidor 1С:Предприятия "
"o el servidor web que usa la base de fichero informativa, "
"debe ser establecido por lo menos un de los programas en la lista.';ru='Позволяет не устанавливать программу на компьютер пользователя"
...
"at least one of the programs in the list must be installed.""");
	...
	Если Не Константы.ИспользоватьЭлектронныеПодписи.Получить() Тогда
		ЗаголовокФлажка = НСтр("es='Cifrar y descifrar en el servidor';ru='Шифровать и расшифровывать на сервере';en='Encrypt and decrypt on server'");
		ПодсказкаФлажка =
			НСтр("es='Permite no establecer el programa y el certificado "
"al ordenador del usuario para la cifración y el desciframiento. "
""
"Importante: a cada ordenador, donde trabaja el servidor 1С:Предприятия "
"o el servidor web que usa la base de fichero informativa, "
"debe ser establecido el programa y el certificado con la llave cerrada.';ru='Позволяет не устанавливать программу и сертификат"
"на компьютер пользователя для шифрования и расшифровки."
""
"Важно: на каждый компьютер, где работает сервер 1С:Предприятия"
"или веб-сервер, использующий файловую информационную базу,"
"должна быть установлена программа и сертификат с закрытым ключом.';en=""Allows not to install the program and certificate"
"to the user's computer for encryption and decryption."
""
"Important: for each computer where the 1C: Enterprise server is running"
"or a web server using a file infobase,"
"the program and certificate with the private key must be installed. """);
	ИначеЕсли Не Константы.ИспользоватьШифрование.Получить() Тогда
		ЗаголовокФлажка = НСтр("es='Firmar en el servidor';ru='Подписывать на сервере';en='Sign on server'");
		ПодсказкаФлажка =
			НСтр("es='Permite no establecer el programa y el certificado "
"al ordenador del usuario para la firma. "
""
"Importante: a cada ordenador, donde trabaja el servidor 1С:Предприятия "
"o el servidor web que usa la base de fichero informativa, "
"debe ser establecido el programa y el certificado con la llave cerrada.';ru='Позволяет не устанавливать программу и сертификат"
...
"или веб-сервер, использующий файловую информационную базу,"
"должна быть установлена программа и сертификат с закрытым ключом.';en=""Allows not to install the program and certificate"
"to the user's computer for signing."
""
"Important: for each computer where the 1C: Enterprise server is running"
"or a web server using a file infobase,"
"the program and certificate with the private key must be installed.""");
	Иначе
		ЗаголовокФлажка = НСтр("es='Firmar y cifrar en el servidor';ru='Подписывать и шифровать на сервере';en='Sign and encrypt on the server'");
		ПодсказкаФлажка =
			НСтр("es='Permite no establecer el programa y el certificado "
"al ordenador del usuario para la firma, la cifración y el desciframiento. "
""
"Importante: a cada ordenador, donde trabaja el servidor 1С:Предприятия "
"o el servidor web que usa la base de fichero informativa, "
"debe ser establecido el programa y el certificado con la llave cerrada.';ru='Позволяет не устанавливать программу и сертификат"
...
"или веб-сервер, использующий файловую информационную базу,"
"должна быть установлена программа и сертификат с закрытым ключом.';en=""Allows not to install the program and certificate"
"to the user's computer for signing, encryption and decryption."
""
"Important: for each computer where the 1C: Enterprise server is running"
"or a web server using a file infobase,"
"the program and certificate with the private key must be installed.""");
...
Процедура ОбработчикОжиданияОпределитьУстановленныеПрограммыЦиклНачало(Контекст)
	...
	ИначеЕсли ОписаниеПрограммы.ЭтоПрограммаОблачногоСервиса Тогда
		ОбновитьЗначение(ОписаниеПрограммы.РезультатПроверки, НСтр("es='Es accesible.';ru='Доступен.';en='Available.'"));
...
Процедура ОбработчикОжиданияОпределитьУстановленныеПрограммыЦиклПродолжение(Менеджер, Контекст) Экспорт
	...
	Если Менеджер <> Неопределено Тогда
		ОбновитьЗначение(ОписаниеПрограммы.РезультатПроверки, НСтр("es='Es establecida en el ordenador.';ru='Установлена на компьютере.';en='Installed on the computer.'"));
		ОбновитьЗначение(ОписаниеПрограммы.Установлена, Истина);
		ОбработчикОжиданияОпределитьУстановленныеПрограммыЦиклНачало(Контекст);
		Возврат;
	КонецЕсли;
	...
	Если Ошибка.НеУказанПуть Тогда
		ОбновитьЗначение(ОписаниеПрограммы.РезультатПроверки, НСтр("es='al programa.';ru='Не указан путь к программе.';en='The path to the program is not specified.'"));
		ОбновитьЗначение(ОписаниеПрограммы.Установлена, "");
	Иначе
		...
		Если Ошибка.КАдминистратору И Не ЭтоПолноправныйПользователь Тогда
			ТекстОшибки = ТекстОшибки + " " + НСтр("es='Se dirijan al administrador.';ru='Обратитесь к администратору.';en='Contact administrator.'");
		КонецЕсли;
...
Процедура ЗаполнитьПрограммыИНастройки(ОбновитьПовтИсп = Ложь)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Строка.ЭтоПрограммаОблачногоСервиса И Не Строка.ПометкаУдаления Тогда
			ОбновитьЗначение(Строка.РезультатПроверки, НСтр("es='Es accesible.';ru='Доступен.';en='Available.'"));
