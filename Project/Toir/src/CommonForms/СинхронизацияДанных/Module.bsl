...
Процедура УдалитьНастройкуСинхронизации(Команда)
	...
	Если ТекущиеДанные.ЭтоОбменСПриложениемВСервисе 
		...
		ПоказатьПредупреждение(, НСтр("es='Para la desaparición del ajuste de la sincronización de los datos pasen en el manager del servicio. "
"En el manager del servicio se aprovechen de la orden ""la Sincronización de los datos"".';ru='Для удаления настройки синхронизации данных перейдите в менеджер сервиса."
"В менеджере сервиса воспользуйтесь командой ""Синхронизация данных"".';en='To remove the data synchronization settings, go to the service manager."
...
Процедура НачальнаяВыгрузкаДанных(Команда)
	...
	Если ПродолжитьНастройкуНовойСинхронизации(ТекущиеДанные, ДанныеДляЗавершенияНастройки) Тогда
		...
		ПоказатьВопрос(ОповещениеЗавершения,
			НСтр("es='Ante la descarga de los datos la comparación tiene que acabar el ajuste de la sincronización. "
"¿Abrir forma del ayudante del ajuste?';ru='Перед выгрузкой данных для сопоставления необходимо завершить настройку синхронизации."
"Открыть форму помощника настройки?';en='Before uploading data for mapping, you must complete the synchronization setup."
"Open the configuration assistant form?'"),
			РежимДиалогаВопрос.ДаНет);
	ИначеЕсли Не ТекущиеДанные.ДоступнаИнтерактивнаяОтправка Тогда
		ПоказатьПредупреждение(, НСтр("es='Para la variante escogida del ajuste de la sincronización la descarga de los datos para la comparación no es apoyada.';ru='Для выбранного варианта настройки синхронизации выгрузка данных для сопоставления не поддерживается.';en='For the selected synchronization configuration option, uploading data for matching is not supported.'"));
		Возврат;
	Иначе
		ПараметрыПомощника = Новый Структура;
		ПараметрыПомощника.Вставить("УзелОбмена", ТекущиеДанные.УзелИнформационнойБазы);
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ПравоеЗначение = НСтр("es='No era puesto en marcha';ru='Не запускалась';en='Did not start'");
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='н/д';ru='н/д';en='n/a'"));
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СписокПриложений.СостояниеПредставление");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = НСтр("es='El ajuste no es acabado';ru='Настройка не завершена';en='Setup is not completed'");
...
Процедура ОбработкаКомандыВыполненияСинхронизации(ИспользоватьДопОтборы = Ложь)
	...
	Если ПродолжитьНастройкуНовойСинхронизации(ТекущиеДанные, ДанныеДляЗавершенияНастройки) Тогда
		...
		ОповещениеЗавершения = Новый ОписаниеОповещения("ВопросПродолжитьНастройкуСинхронизацииЗавершение", ЭтотОбъект, ДополнительныеПараметры);
		ПоказатьВопрос(ОповещениеЗавершения,
			НСтр("es='Ante el lanzamiento de la sincronización de los datos es necesario acabar su ajuste. "
"¿Abrir forma del ayudante del ajuste?';ru='Перед запуском синхронизации данных необходимо завершить ее настройку."
"Открыть форму помощника настройки?';en='Before starting data synchronization, you must complete its configuration."
"Open the configuration assistant form?'"),
			РежимДиалогаВопрос.ДаНет);
	ИначеЕсли ТекущиеДанные.ЗапускОбменаИзКорреспондента
		...
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El lanzamiento de la sincronización con ""%1"" de este programa no es apoyado. "
"Pasen en ""%1"" y pongan en marcha la sincronización de ella.';ru='Запуск синхронизации с ""%1"" из этой программы не поддерживается."
"Перейдите в ""%1"" и запустите синхронизацию из нее.';en='Launching synchronization with ""%1"" ""from this program is not supported. Go to ""%1"" and start synchronization from it. ""'"), ТекущиеДанные.НаименованиеКорреспондента));
	Иначе
		ДополнительныеПараметры = Новый Структура;
...
Процедура ОбработатьРезультатПолученияВариантовНастроекНаСервере(АдресРезультата, Отказ, СообщениеОбОшибке, ПараметрыПомощника)
	...
	Если Не ЗначениеЗаполнено(Результат.КодОшибки) Тогда
		...
		Иначе
			Отказ = Истина;
			СообщениеОбОшибке = НСтр("es='El ajuste de la conexión al servicio dado es inaccesible.';ru='Настройка подключения к данному сервису недоступна.';en='Configuring connection to this service is not available.'");
		КонецЕсли;
	Иначе
		Отказ = Истина;
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
			...
			Если Не МодульОбменДаннымиВМоделиСервисаПовтИсп.СинхронизацияДанныхПоддерживается() Тогда
		 		ТекстСообщения = НСтр("es='La posibilidad del ajuste de la sincronización de los datos en el programa dado no es prevista.';ru='Возможность настройки синхронизации данных в данной программе не предусмотрена.';en='The ability to configure data synchronization in this program is not provided.'");
				Отказ = Истина;
			КонецЕсли;
		Иначе
			ТекстСообщения = НСтр("es='En el régimen no dividido el ajuste de la sincronización de los datos con otros programas es inaccesible.';ru='В неразделенном режиме настройка синхронизации данных с другими программами недоступна.';en='In unshared mode, setting data synchronization with other programs is not available.'");
			Отказ = Истина;
		КонецЕсли;
	Иначе
...
Функция СостояниеСинхронизацииДанных(СтрокаПриложение)
	...
	Если Не СтрокаПриложение.НастройкаЗавершена Тогда
		Состояние.Представление = НСтр("es='El ajuste no es acabado';ru='Настройка не завершена';en='Setup is not completed'");
		Состояние.Картинка = 3;
		
		Если СтрокаПриложение.ПолученоСообщениеДляСопоставленияДанных Тогда
			Состояние.Представление = НСтр("es='El ajuste no son acabados, recibidos los datos para la comparación';ru='Настройка не завершена, получены данные для сопоставления';en='Setup is not completed, data for matching is received'");
		КонецЕсли;
	Иначе
		...
		ИначеЕсли СтрокаПриложение.ДатаНачалаПоследнейВыгрузки > СтрокаПриложение.ДатаОкончанияПоследнейВыгрузки Тогда
			Состояние.Представление = НСтр("es='La descarga de los datos...';ru='Выгрузка данных...';en='Uploading data ...'");
			Состояние.Картинка = 4;
		ИначеЕсли Не ЗначениеЗаполнено(СтрокаПриложение.ДатаПоследнегоЗапуска) Тогда
			Состояние.Представление = НСтр("es='No era puesto en marcha';ru='Не запускалась';en='Did not start'");
			
			Если СтрокаПриложение.ПолученоСообщениеДляСопоставленияДанных Тогда
				Состояние.Представление = НСтр("es='Son recibidos los datos para la comparación';ru='Получены данные для сопоставления';en='Received data for comparison'");
			КонецЕсли;
		Иначе
			Состояние.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El lanzamiento pasado: %1';ru='Прошлый запуск: %1';en='Last run:%1'"),
...
Процедура ПроверитьСовместимостьПравилКонвертации(ОбработкаПродолжения)
	...
	Иначе
		...
		Кнопки.Добавить("ПерейтиКЗагрузкеПравил", НСтр("es='Cargar las reglas';ru='Загрузить правила';en='Download Rules'"));
		...
		Если ОписаниеОшибки.ВидОшибки = "НекорректнаяКонфигурация" Тогда
			ПараметрыФормы.Заголовок = НСтр("es='La sincronización de los datos no puede ser cumplida';ru='Синхронизация данных не может быть выполнена';en='Data synchronization could not be performed.'");
		Иначе
			ПараметрыФормы.Заголовок = НСтр("es='La sincronización de los datos puede ser cumplida es incorrecto';ru='Синхронизация данных может быть выполнена некорректно';en='Data synchronization may not be performed correctly.'");
		КонецЕсли;
