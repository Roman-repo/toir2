...
Процедура ПерейтиКСписку()
	...
	Для Каждого ОкноКлиентскогоПриложения Из ПолучитьОкна() Цикл
		Если ОкноКлиентскогоПриложения.ПолучитьНавигационнуюСсылку() = НавигационнаяСсылка Тогда
			...
			Кнопки.Добавить("Переоткрыть", НСтр("es='Переоткрыть';ru='Переоткрыть';en='Rediscover'"));
			Кнопки.Добавить("Отмена", НСтр("es='No переоткрывать';ru='Не переоткрывать';en='Do not reopen'"));
			ТекстВопроса = 
				НСтр("es='La lista es abierta ya. ¿Переоткрыть la lista "
"para ver los cambios del menú la prensa?';ru='Список уже открыт. Переоткрыть список, "
"чтобы увидеть изменения меню печать?';en='The list is already open. Reopen the list,"
"to see print menu changes?'");
