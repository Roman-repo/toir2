...
Процедура ПриОткрытии(Отказ)
	Если ПоказатьОповещениеОПодсказках И ПоказыватьПодсказки Тогда
		ПоказатьОповещениеПользователя(
			НСтр("es='Una nueva posibilidad';ru='Новая возможность';en='New opportunity'"),
			"e1cib/data/ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.ОписаниеНовойВозможностиПоВыводуОписаний",
			НСтр("es='La conclusión de las descripciones en los paneles de los informes';ru='Вывод описаний в панелях отчетов';en='Display descriptions in report panels'"),
...
Функция ВведенаСлишкомКороткаяСтрокаПоиска(Текст)
	...
	Если СтрДлина(Текст) < 2 Тогда
		ПоказатьПредупреждение(, НСтр("es='Es introducida la línea demasiado corta de la búsqueda.';ru='Введена слишком короткая строка поиска.';en='The search string is too short.'"));
		Возврат Истина;
	КонецЕсли;
	...
	Если Не ЕстьНормальноеСлово Тогда
		ПоказатьПредупреждение(, НСтр("es='Son introducidas las palabras demasiado cortas para la búsqueda.';ru='Введены слишком короткие слова для поиска.';en='Search terms are too short.'"));
		Возврат Истина;
	КонецЕсли;
...
Процедура СброситьНастройки(Команда)
	ТекстВопроса = НСтр("es='¿Arrojar los ajustes de la disposición de los informes?';ru='Сбросить настройки расположения отчетов?';en='Reset report location settings?'");
...
Функция НачатьЗамер(Событие, Комментарий = Неопределено)
	...
	Если Событие = "СписокОтчетов.Открытие" Или Событие = "ПанельОтчетов.Открытие" Тогда
		Замер.Имя = Событие;
		Комментарий = Комментарий + "; " + НСтр("es='Del panel de los informes:';ru='Из панели отчетов:';en='From the report panel:'") + " " + ПараметрыКлиента.ПутьКПодсистеме;
	Иначе
		...
		Комментарий = Комментарий + "; " + НСтр("es='Los soplos:';ru='Подсказки:';en='Hint:'") + " " + Строка(ПоказыватьПодсказки);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
		Комментарий = Комментарий
			+ "; " + НСтр("es='La búsqueda:';ru='Поиск:';en='Search:'") + " " + Строка(СтрокаПоиска)
			+ "; " + НСтр("es='En todas las secciones:';ru='Во всех разделах:';en='In all sections:'") + " " + Строка(ИскатьВоВсехРазделах);
	Иначе
		Комментарий = Комментарий + "; " + НСтр("es='Sin búsqueda';ru='Без поиска';en='Without search'");
	КонецЕсли;
	
	Если Событие = "ВыходИзРежимаНастройки" Тогда
		Комментарий = Комментарий + "; " + НСтр("es='La salida del régimen del ajuste';ru='Выход из режима настройки';en='Exit setup mode'");
	КонецЕсли;
...
Функция ОбновитьПанельОтчетовНаСервере(Знач Событие = "")
	...
	// Заголовок.
	СуффиксРежимаНастройки = " (" + НСтр("es='El ajuste';ru='настройка';en='customization'") + ")";
...
Процедура ОпределитьПоведениеВМобильномКлиенте()
	...
	ПодстрокаПоиска = НСтр("es='Presionen sobre el informe por el botón derecho del ratón y';ru='нажмите на отчете правой кнопкой мыши и';en='right click on the report and'");
	ПодстрокаЗамены = НСтр("es='En el menú de contexto';ru='в контекстном меню';en='in the context menu'");
...
Процедура ОпределитьПодсистемыИЗаголовок(Параметры)
	...
	ЗаголовокПанели = ?(ЗаданЗаголовок, Параметры.Заголовок, НСтр("es='Los informes';ru='Отчеты';en='Reports'"));
	...
	Если ТекущийРазделСсылка = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Para el panel de los informes es indicada sección no existente ""%1"" (cm. ВариантыОтчетовПереопределяемый.ОпределитьРазделыСВариантамиОтчетов).';ru='Для панели отчетов указан несуществующий раздел ""%1"" (см. ВариантыОтчетовПереопределяемый.ОпределитьРазделыСВариантамиОтчетов).';en='The report panel has a non-existent section ""%1"" (see ВариантыОтчетовПереопределяемый.ОпределитьРазделыСВариантамиОтчетов).'"),
...
Процедура ЗаполнитьПанельОтчетов(ПараметрыЗаполненияВременноеХранилище)
	...
	Иначе
		...
		Если ПараметрыЗаполнения.ДругиеРазделы.Количество() = 0 Тогда
			...
			Надпись.Заголовок = НСтр("es='Los informes en otras secciones no son encontrados.';ru='Отчеты в других разделах не найдены.';en='No reports found in other sections.'") + Символы.ПС;
			Надпись.Высота = 2;
		КонецЕсли;
		...
		Если ПараметрыЗаполнения.НеВыведено > 0 Тогда // Вывод информационной надписи.
			ЗаголовокНадписи = НСтр("es='Son sacados los primeros %1 informes de otras secciones, precisen la interpelación de prospección.';ru='Выведены первые %1 отчетов из других разделов, уточните поисковый запрос.';en='The first%1 reports from other sections are displayed, specify the search query.'");
...
Процедура ВывестиВариантыРаздела(ПараметрыЗаполнения, РазделСсылка)
	...
	Если ПараметрыЗаполнения.ВариантыКоличество = 0 Тогда
		...
		Если ПараметрыЗаполнения.ВыводятсяВариантыТекущегоРаздела Тогда
			...
			Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
				Если ПараметрыЗаполнения.ТолькоТекущийРаздел Тогда
					Надпись.Заголовок = НСтр("es='Los informes no son encontrados.';ru='Отчеты не найдены.';en='No reports found.'");
				Иначе
					Надпись.Заголовок = НСтр("es='Los informes en la sección corriente no son encontrados.';ru='Отчеты в текущем разделе не найдены.';en='No reports were found in the current section.'");
					Надпись.Высота = 2;
				КонецЕсли;
			Иначе
				Надпись.Заголовок = НСтр("es='En el panel de los informes de esta sección no es instalado ni un informe.';ru='В панели отчетов этого раздела не размещено ни одного отчета.';en='There are no reports posted in the report panel of this section.'");
			КонецЕсли;
...
Функция ДобавитьГруппуПодсистемы(ПараметрыЗаполнения, СтрокаПорядкаВывода, ВГруппу)
	...
	Если ТребуетсяПодсветка Тогда
		Если СтрокаПорядкаВывода.ЭтоПродолжение Тогда
			Суффикс = НСтр("es='(La continuación)';ru='(продолжение)';en='(continued)'");
	...
	Иначе
		Если СтрокаПорядкаВывода.ЭтоПродолжение Тогда
			ПодсистемаПредставление = ПодсистемаПредставление + " " + НСтр("es='(La continuación)';ru='(продолжение)';en='(continued)'");
		КонецЕсли;
...
Функция ДобавитьЭлементыВариантаОтчета(ПараметрыЗаполнения, Вариант, ВГруппу, УровеньВложенности = 0)
	...
	Если ЗначениеЗаполнено(Вариант.Автор) Тогда
		Надпись.Подсказка = СокрЛ(Надпись.Подсказка + Символы.ПС) + НСтр("es='El autor:';ru='Автор:';en='Author:'") + " " + СокрЛП(Строка(Вариант.Автор));
	КонецЕсли;
	...
	Если ПараметрыЗаполнения.ИспользоватьПодсветку Тогда
		...
		Если ПараметрыПодсветки <> Неопределено Тогда
			...
			Если ПараметрыПодсветки.ПредставлениеАвтора.КоличествоНайденныхСлов > 0 Тогда
				...
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("es='El autor:';ru='Автор:';en='Author:'") + " ");
			...
			Если ПараметрыПодсветки.НаименованияПользовательскихНастроек.КоличествоНайденныхСлов > 0 Тогда
				Если СоставПодсказки.Количество() > 0 Тогда
					СоставПодсказки.Добавить(Символы.ПС);
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("es='Los ajustes conservados:';ru='Сохраненные настройки:';en='Saved settings:'") + " ");
			...
			Если ПараметрыПодсветки.НаименованияПолей.КоличествоНайденныхСлов > 0 Тогда
				Если СоставПодсказки.Количество() > 0 Тогда
					СоставПодсказки.Добавить(Символы.ПС);
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("es='Campos:';ru='Поля:';en='Fields:'") + " ");
			...
			Если ПараметрыПодсветки.НаименованияПараметровИОтборов.КоличествоНайденныхСлов > 0 Тогда
				Если СоставПодсказки.Количество() > 0 Тогда
					СоставПодсказки.Добавить(Символы.ПС);
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("es='Los ajustes:';ru='Настройки:';en='Settings:'") + " ");
			...
			Если ПараметрыПодсветки.КлючевыеСлова.КоличествоНайденныхСлов > 0 Тогда
				Если СоставПодсказки.Количество() > 0 Тогда
					СоставПодсказки.Добавить(Символы.ПС);
				КонецЕсли;
				СоставПодсказки.Добавить(НСтр("es='Las palabras claves:';ru='Ключевые слова:';en='Keywords:'") + " ");
