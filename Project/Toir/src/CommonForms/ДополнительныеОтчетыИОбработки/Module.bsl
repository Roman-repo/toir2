...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВидОбработок = Перечисления.ВидыДополнительныхОтчетовИОбработок.ЗаполнениеОбъекта Тогда
		...
		Заголовок = НСтр("es='Las órdenes del relleno de los objetos';ru='Команды заполнения объектов';en='Object fill commands'");
	ИначеЕсли ВидОбработок = Перечисления.ВидыДополнительныхОтчетовИОбработок.Отчет Тогда
		ЭтоНазначаемыеОбработки = Истина;
		ЭтоОтчеты = Истина;
		Заголовок = НСтр("es='Los informes';ru='Отчеты';en='Reports'");
	ИначеЕсли ВидОбработок = Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма Тогда
		ЭтоНазначаемыеОбработки = Истина;
		Заголовок = НСтр("es='Las formas de imprenta adicionales';ru='Дополнительные печатные формы';en='Additional printing forms'");
	ИначеЕсли ВидОбработок = Перечисления.ВидыДополнительныхОтчетовИОбработок.СозданиеСвязанныхОбъектов Тогда
		ЭтоНазначаемыеОбработки = Истина;
		Заголовок = НСтр("es='Las órdenes de la creación de los objetos vinculados';ru='Команды создания связанных объектов';en='Commands for creating related objects'");
	ИначеЕсли ВидОбработок = Перечисления.ВидыДополнительныхОтчетовИОбработок.ДополнительнаяОбработка Тогда
		...
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Los tratamientos adicionales (%1)';ru='Дополнительные обработки (%1)';en='Additional treatments (%1)'"), 
	...
	ИначеЕсли ВидОбработок = Перечисления.ВидыДополнительныхОтчетовИОбработок.ДополнительныйОтчет Тогда
		...
		ЭтоОтчеты = Истина;
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Los informes adicionales (%1)';ru='Дополнительные отчеты (%1)';en='Additional reports (%1)'"), 
			ДополнительныеОтчетыИОбработки.ПредставлениеРаздела(СсылкаРаздела));
	КонецЕсли;
...
Функция ЗапуститьФоновоеЗадание(Знач ВыполняемаяКоманда, Знач УникальныйИдентификатор)
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Доп. Los informes y los tratamientos: la Ejecución de la orden ""%1""';ru='Доп. отчеты и обработки: Выполнение команды ""%1""';en='Add. reporting and processing: Executing command ""%1""'"),
...
Процедура ВыполнитьСерверныйМетодОбработкиЗавершение(Задание, ДополнительныеПараметры) Экспорт
	Если Задание.Статус = "Выполнено" Тогда
		...
		Если ВыполняемаяКоманда.ПоказыватьОповещение Тогда
			ПоказатьОповещениеПользователя(
				НСтр("es='La orden es cumplida';ru='Команда выполнена';en='Command completed'"),
				,
				ВыполняемаяКоманда.Представление);
		КонецЕсли;
		Если Открыта() Тогда
	...
	Иначе
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La orden ""%1"" no es cumplida:';ru='Команда ""%1"" не выполнена:';en='Command ""%1"" failed:'"),
			ВыполняемаяКоманда.Представление);
		Если Открыта() Тогда
			Закрыть();
		КонецЕсли;
