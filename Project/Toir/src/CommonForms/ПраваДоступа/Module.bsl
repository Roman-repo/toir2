...
Процедура ИсключитьИзГруппы(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ТекущаяГруппаДоступа) Тогда
		ПоказатьПредупреждение(, НСтр("es='El grupo del acceso no es escogido.';ru='Группа доступа не выбрана.';en='Access group not selected.'"));
		Возврат;
	КонецЕсли;
...
Процедура ИзменитьГруппу(Команда)
	...
	Иначе
		ПоказатьПредупреждение(,
			НСтр("es='Es insuficientemente derecho para la redacción del grupo del acceso. "
"Redactar el grupo del acceso pueden responsable de los participantes del grupo del acceso y el administrador.';ru='Недостаточно прав для редактирования группы доступа."
"Редактировать группу доступа могут ответственный за участников группы доступа и администратор.';en='Not enough rights to edit access group."
"The access group can be edited by the person in charge of the access group and the administrator.'"));
		Возврат;
	КонецЕсли;
...
Процедура ИзменитьСоставГруппы(Знач ГруппаДоступа, Знач Добавить, ОписаниеОшибки = "")
	...
	Если НЕ РазрешеноИзменениеСоставаПользователейГруппы(ГруппаДоступа) Тогда
		Если Добавить Тогда
			ОписаниеОшибки =
				НСтр("es='Es imposible incluir al usuario en el grupo del acceso, "
"puesto que el usuario corriente "
"no responsable de los participantes del grupo del acceso y "
"no el administrador con todos los derechos.';ru='Невозможно включить пользователя в группу доступа,"
"так как текущий пользователь"
"не ответственный за участников группы доступа и"
"не полноправный администратор.';en='Unable to include user in access group,"
"since current user"
"not responsible for access group members and"
"not a full administrator.'");
		Иначе
			ОписаниеОшибки =
				НСтр("es='Es imposible excluir al usuario del grupo del acceso, "
"puesto que el usuario corriente "
"no responsable de los participantes del grupo del acceso y "
"no el administrador con todos los derechos.';ru='Невозможно исключить пользователя из группы доступа,"
"так как текущий пользователь"
"не ответственный за участников группы доступа и"
"не полноправный администратор.';en='Unable to exclude user from access group,"
"since current user"
"not responsible for access group members and"
"not a full administrator. '");
		КонецЕсли;
	...
	Если НЕ Добавить И НЕ ПользовательВключенВГруппуДоступа(ТекущаяГруппаДоступа) Тогда
		ОписаниеОшибки =
			НСтр("es='Es imposible excluir al usuario del grupo del acceso, "
"puesto que él es incluido en ella indirectamente.';ru='Невозможно исключить пользователя из группы доступа,"
"так как он включен в нее косвенно.';en='Unable to exclude user from access group,"
"since it’s included in it indirectly. '");
		Возврат;
	КонецЕсли;
