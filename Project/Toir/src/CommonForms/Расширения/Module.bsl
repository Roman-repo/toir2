...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		ВызватьИсключение НСтр("es='No basta los derechos de admisión. Se dirijan al administrador.';ru='Недостаточно прав доступа. Обратитесь к администратору.';en='Not enough access rights. Contact your administrator.'");
	КонецЕсли;
	...
	Если СтандартныеПодсистемыСервер.ЭтоБазоваяВерсияКонфигурации() Тогда
		ВызватьИсключение НСтр("es='Las ampliaciones son inaccesibles en la versión básica del programa.';ru='Расширения недоступны в базовой версии программы.';en='Extensions are not available in the basic version of the program.'");
	КонецЕсли;
...
Процедура СохранитьКак(Команда)
	...
	ПараметрыСохранения.Диалог.Заголовок = НСтр("es='Escojan el fichero para la conservación de la ampliación de la configuración';ru='Выберите файл для сохранения расширения конфигурации';en='Select a file to save the configuration extension'");
	ПараметрыСохранения.Диалог.Фильтр    = НСтр("es='Los ficheros de las ampliaciones de la configuración (*.cfe) | *.cfe|Все los ficheros (*. *) |*.*';ru='Файлы расширений конфигурации (*.cfe)|*.cfe|Все файлы (*.*)|*.*';en='Configuration extension files (* .cfe) | * .cfe | All Files (*. *) | *. *'");
...
Процедура ОбновитьКэшиРасширений(Команда)
	...
	ПоказатьПредупреждение(,
		НСтр("es='Es cumplida la renovación de la versión de los parámetros del trabajo de las ampliaciones "
"conectadas en esta sesión.';ru='Выполнено обновление версии параметров работы расширений,"
"подключенных в этом сеансе.';en='The version of the extension operation parameters has been updated,"
...
Процедура УдалитьУстаревшиеКэшиРасширений(Команда)
	...
	ПоказатьПредупреждение(, НСтр("es='Es cumplida la desaparición de las versiones que han caducado de los parámetros del trabajo de las ampliaciones.';ru='Выполнено удаление устаревших версий параметров работы расширений.';en='The removal of outdated versions of the extension operation parameters has been completed.'"));
...
Процедура ОбновитьСписок(ПослеДобавления = Ложь)
	...
	Для Каждого Расширение Из Расширения Цикл
		...
		Если ПодключенныеРасширения[КлючРасширения] <> Неопределено Тогда
			...
			ЭлементРасширения.СостояниеПодключения = НСтр("es='Es conectado';ru='Подключено';en='Connected'");
		ИначеЕсли ОтключенныеРасширения[КлючРасширения] <> Неопределено Тогда
			ЭлементРасширения.Подключено = 2;
			ЭлементРасширения.СостояниеПодключения = НСтр("es='Es desconectado';ru='Отключено';en='Disabled'");
		Иначе
			ЭлементРасширения.Подключено = 1;
			ЭлементРасширения.СостояниеПодключения = НСтр("es='Es necesario el relanzamiento';ru='Требуется перезапуск';en='Restart required'");
		КонецЕсли;	
...
Процедура УдалитьРасширенияНаСервере(ВыделенныеСтроки)
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió quitar la ampliación ""%1"" por la causa: "
""
"%2';ru='Не удалось удалить расширение ""%1"" по причине:"
""
"%2';en='Failed to remove extension ""%1"" due to:"
""
"%2'"),
			УдаляемоеРасширение,
	...
	Если Не ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Después de la desaparición, con la preparación de las ampliaciones que se han quedado para el trabajo, había una falta: "
""
"%1';ru='После удаления, при подготовке оставшихся расширений к работе, произошла ошибка:"
""
"%1';en='After uninstallation, while preparing the remaining extensions for work, an error occurred:"
""
		...
		Исключение
			...
			ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Durante la tentativa de restablecer las ampliaciones alejadas había todavía una falta: "
"%1';ru='При попытке восстановить удаленные расширения произошла еще одна ошибка:"
"%1';en='Another error occurred while trying to recover deleted extensions:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибкеВосстановления));
		...
		Если ВосстановлениеВыполнялось
		   И ИнформацияОбОшибкеВосстановления = Неопределено Тогда
			...
			ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
				+ НСтр("es='Las ampliaciones alejadas eran restablecidas.';ru='Удаленные расширения были восстановлены.';en='Deleted extensions have been restored.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗагрузитьРасширениеПослеПодтверждения(Ответ, Контекст) Экспорт
	...
	ПараметрыЗагрузки.Диалог.Фильтр = НСтр("es='La ampliación de la configuración';ru='Расширение конфигурации';en='Configuration extension'", "ru")+ " (*.cfe)|*.cfe";
	ПараметрыЗагрузки.Диалог.Заголовок = НСтр("es='Escojan el fichero de la ampliación de la configuración';ru='Выберите файл расширения конфигурации';en='Select configuration extension file'", "ru");
...
Процедура ЗагрузитьРасширениеПослеПомещенияФайлов(ПомещенныеФайлы, Контекст) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОбновлениеКонфигурации") Тогда
		...
		ИначеЕсли ВыбранныеФайлыСодержатИсправления(ПомещенныеФайлы, МодульОбновлениеКонфигурацииКлиент) Тогда 
			ПоказатьПредупреждение(,
				НСтр("es='Los ficheros escogidos no deben al mismo tiempo contener la corrección (патчи) y otros tipos de las ampliaciones.';ru='Выбранные файлы не должны одновременно содержать исправления (патчи) и другие виды расширений.';en='The selected files should not simultaneously contain corrections (patches) and other types of extensions.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗагрузитьРасширениеЗавершение()
	...
	Если Контекст.ИдентификаторРасширения = Неопределено Тогда
		Если Контекст.ПомещенныеФайлы.Количество() > 1 Тогда
			ТекстОповещения = НСтр("es='Las ampliaciones de la configuración son añadidas';ru='Расширения конфигурации добавлены';en='Configuration extensions added'", "ru");
		Иначе
			ТекстОповещения = НСтр("es='La ampliación de la configuración es añadida';ru='Расширение конфигурации добавлено';en='Configuration extension added'", "ru");
		КонецЕсли;
	Иначе
		ТекстОповещения = НСтр("es='La ampliación de la configuración es renovada';ru='Расширение конфигурации обновлено';en='Configuration extension updated'", "ru");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(НеподключенныеРасширения) Тогда
		Если Контекст.ПомещенныеФайлы.Количество() > 1 Тогда
			Если СтрНайти(НеподключенныеРасширения, ",") > 0 Тогда
				Пояснение = НСтр("es='Algunas ampliaciones no se conectan:';ru='Некоторые расширения не подключаются:';en='Some extensions are not connected:'");
			Иначе
				Пояснение = НСтр("es='Una ampliación no se conecta:';ru='Одно расширение не подключается:';en='One extension does not connect:'");
			КонецЕсли;
			Пояснение = Пояснение + " " + НеподключенныеРасширения;
		Иначе
			Пояснение = НСтр("es='La ampliación no se conecta.';ru='Расширение не подключается.';en='The extension does not connect.'");
		КонецЕсли;
	Иначе
		Пояснение = "";
	КонецЕсли;
...
Процедура ИзменитьРасширенияНаСервере(ПомещенныеФайлы, ИдентификаторРасширения, НеподключенныеРасширения, РасширенияИзменены)
	...
	Попытка
		Если ИдентификаторРасширения <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(ТекстОшибки) Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Una nueva ampliación no puede ser aplicada por la causa: "
"%1';ru='Новое расширение не может быть применено по причине:"
"%1';en='The new extension cannot be applied due to:"
"%1'"),
					ТекстОшибки);
			Иначе
				...
				Иначе
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='No es posible sustituir la ampliación ""%1"" en ""%2"".';ru='Нельзя заменить расширение ""%1"" на ""%2"".';en='Can not replace extension ""%1"" with ""%2"".'"),
	...
	Исключение
		...
		Если ИдентификаторРасширения <> Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió renovar la ampliación por la causa: "
""
"%1';ru='Не удалось обновить расширение по причине:"
""
"%1';en='Failed to update extension due to:"
""
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
			
		ИначеЕсли ЗначениеЗаполнено(ИмяФайлаДобавляемогоРасширения) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió añadir la ampliación del fichero "
"""%1"" "
"por la causa: "
""
"%2';ru='Не удалось добавить расширение из файла"
"""%1"""
"по причине:"
""
"%2';en='Failed to add extension from file"
"""%1"""
"due to:"
""
"%2'"),
		...
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió añadir por la causa: "
""
"%1';ru='Не удалось добавить по причине:"
""
"%1';en='Failed to add due to:"
""
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		КонецЕсли;
	КонецПопытки;
	...
	Если Не ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Después de la adición, con la preparación de las ampliaciones para el trabajo, había una falta: "
""
"%1';ru='После добавления, при подготовке расширений к работе, произошла ошибка:"
""
"%1';en='After adding, while preparing extensions for work, an error occurred:"
""
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		КонецПопытки;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		Исключение
			...
			Если ИдентификаторРасширения <> Неопределено Тогда
				ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Durante la tentativa de restablecer la ampliación cambiada había todavía una falta: "
"%1';ru='При попытке восстановить измененное расширение произошла еще одна ошибка:"
"%1';en='Another error occurred while trying to restore the changed extension:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибкеВосстановления));
			Иначе
				ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Durante la tentativa de quitar las ampliaciones añadidas había todavía una falta: "
"%1';ru='При попытке удалить добавленные расширения произошла еще одна ошибка:"
"%1';en='Another error occurred while trying to remove the added extensions:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибкеВосстановления));
			КонецЕсли;
		КонецПопытки;
		Если ВосстановлениеВыполнялось
		   И ИнформацияОбОшибкеВосстановления = Неопределено Тогда
			...
			Если ИдентификаторРасширения <> Неопределено Тогда
				Если НовыеДвоичныеДанныеЗаписаны Тогда
					ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
						+ НСтр("es='La ampliación cambiada era restablecida.';ru='Измененное расширение было восстановлено.';en='The changed extension has been restored.'");
				Иначе
					ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
						+ НСтр("es='La ampliación no era cambiada.';ru='Расширение не было изменено.';en='The extension has not been changed.'");
				КонецЕсли;
			Иначе
				ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
					+ НСтр("es='Las ampliaciones añadidas eran quitadas.';ru='Добавленные расширения были удалены.';en='Added extensions have been removed.'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
		...
		Попытка
			...
			Иначе
				...
				Если ВременноеРасширение = Неопределено Тогда
					ВызватьИсключение
						НСтр("es='La ampliación corriente no es encontrada en la base de datos, "
"es posible era quitado en otra sesión.';ru='Текущее расширение не найдено в базе данных,"
"возможно оно было удалено в другом сеансе.';en='The current extension was not found in the database,"
		...
		Исключение
			...
			Если ИдентификаторРасширения = Неопределено Тогда
				Если ПомещенныеФайлы.Количество() > 1 Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='Durante la tentativa temporalmente de añadir la ampliación del fichero "
"""%1"" "
"ha surgido la falta: "
"%2';ru='При попытке временно добавить расширение из файла"
"""%1"""
"возникла ошибка:"
"%2';en='When trying to temporarily add an extension from a file"
"""%1"""
"an error occurred:"
"%2'"),
							ПомещенныйФайл.Имя,
				...
				Иначе
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='Durante la tentativa temporalmente de añadir la ampliación ha surgido la falta: "
"%1';ru='При попытке временно добавить расширение возникла ошибка:"
"%1';en='An error occurred while trying to temporarily add the extension:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
				КонецЕсли;
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Durante la tentativa temporalmente de renovar la ampliación ha surgido la falta: "
"%1';ru='При попытке временно обновить расширение возникла ошибка:"
"%1';en='An error occurred while trying to temporarily update the extension:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
			КонецЕсли;
				...
				Исключение
					...
					Если ИдентификаторРасширения = Неопределено Тогда 
						Если ПомещенныеФайлы.Количество() > 1 Тогда
							ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
								+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("es='Durante la tentativa de quitar la ampliación temporalmente añadida del fichero "
"%1 "
"ha surgido todavía una falta: "
"%2';ru='При попытке удалить временно добавленное расширение из файла"
"%1"
"возникла еще одна ошибка:"
"%2';en='When trying to remove a temporarily added extension from a file"
"%1"
...
"%2'"),
						...
						Иначе
							ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
								+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("es='Durante la tentativa de quitar la ampliación temporalmente añadida ha surgido todavía una falta: "
"%1';ru='При попытке удалить временно добавленное расширение возникла еще одна ошибка:"
"%1';en='Another error occurred while trying to remove the temporarily added extension:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
						КонецЕсли;
					Иначе
						ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='Durante la tentativa de restablecer la ampliación temporalmente cambiada ha surgido todavía una falta: "
"%1';ru='При попытке восстановить временно измененное расширение возникла еще одна ошибка:"
"%1';en='Another error occurred while trying to restore a temporarily modified extension:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
					КонецЕсли;
				КонецПопытки;
			КонецЕсли;
