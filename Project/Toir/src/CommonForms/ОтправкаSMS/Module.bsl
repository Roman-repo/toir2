...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтатусОтправки = НСтр("es='El mensaje sale...';ru='Сообщение отправляется...';en='Message is being sent ...'");
	...
	Заголовок = НСтр("es='El envío SMS a teléfono';ru='Отправка SMS на телефон';en='Sending SMS to phone'") + НомераПолучателей;
КонецПроцедуры
...
Процедура Отправить(Команда)
	...
	Если СтрДлина(ТекстСообщения) = 0 Тогда
		ПоказатьПредупреждение(, НСтр("es='Es necesario introducir el texto del mensaje';ru='Необходимо ввести текст сообщения';en='Required to enter a message text'"));
		Возврат;
	КонецЕсли;
...
Процедура ОтправитьSMS()
	...
	Иначе
		...
		ШаблонСообщения = НСтр("es='El envío no es cumplido: "
"%1 "
"Detalles de cm. En la revista del registro.';ru='Отправка не выполнена:"
"%1"
"Подробности см. в журнале регистрации.';en='Sending failed:"
"%1"
...
Процедура ПроверитьСтатусДоставки()
	...
	ШаблонСостояния = НСтр("es='El envío del mensaje es cumplido. El estado de la entrega: "
"%1.';ru='Отправка сообщения выполнена. Состояние доставки:"
"%1.';en='Sending message completed. Delivery status:"
"%1.'");
...
Функция СтатусДоставки(ИдентификаторСообщения)
	...
	СтатусыДоставки.Вставить("Ошибка", НСтр("es='Había una falta a la conexión al proveedor SMS';ru='произошла ошибка при подключении к провайдеру SMS';en='an error occurred while connecting to the SMS provider'"));
	СтатусыДоставки.Вставить("НеОтправлялось", НСтр("es='El mensaje no salía por el proveedor';ru='сообщение не отправлялось провайдером';en='message was not sent by provider'"));
	СтатусыДоставки.Вставить("Отправляется", НСтр("es='Se cumple el envío por el proveedor';ru='выполняется отправка провайдером';en='sending by provider'"));
	СтатусыДоставки.Вставить("Отправлено", НСтр("es='Es enviado por el proveedor';ru='отправлено провайдером';en='sent by provider'"));
	СтатусыДоставки.Вставить("НеОтправлено", НСтр("es='El mensaje no es enviado por el proveedor';ru='сообщение не отправлено провайдером';en='message not sent by provider'"));
	СтатусыДоставки.Вставить("Доставлено", НСтр("es='El mensaje es llevado';ru='сообщение доставлено';en='message delivered'"));
	СтатусыДоставки.Вставить("НеДоставлено", НСтр("es='El mensaje no es llevado';ru='сообщение не доставлено';en='message not delivered'"));
