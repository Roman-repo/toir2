...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		...
		Элементы.ОчиститьРабочийКаталог.Заголовок = НСтр("es='Limpiar';ru='Очистить';en='Clear'");
...
Процедура РабочийКаталогПользователяНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Заголовок = НСтр("es='Escojan el catálogo de trabajo';ru='Выберите рабочий каталог';en='Choose a working directory'");
...
Процедура ОчиститьЛокальныйКэшФайлов(Команда)
	...
	ТекстВопроса =
		НСтр("es='Del catálogo de trabajo serán quitados todos los ficheros, "
"excepto ocupado para la redacción. "
""
"¿Continuar?';ru='Из рабочего каталога будут удалены все файлы,"
"кроме занятых для редактирования."
""
"Продолжить?';en='All files will be deleted from the working directory,"
"except inaccessible for editing."
""
"Continue?'");
...
Процедура ОчиститьЛокальныйКэшФайловЗавершение(Результат, ПараметрыВыполнения) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("es='El catálogo de trabajo';ru='Рабочий каталог';en='Working directory'"),, НСтр("es='La limpieza del catálogo de trabajo es con éxito acabada.';ru='Очистка рабочего каталога успешно завершена.';en='Clearing the working directory completed successfully.'"));
