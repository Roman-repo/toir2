...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
		Элементы.Профили.Заголовок = НСтр("es='Los perfiles del usuario exterior';ru='Профили внешнего пользователя';en='External user profiles'");
	Иначе
		Элементы.Профили.Заголовок = НСтр("es='Los perfiles del usuario';ru='Профили пользователя';en='User profiles'");
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ТипЗнч(ВладелецФормы) <> Тип("УправляемаяФорма")
	 Или ВладелецФормы.Окно <> Окно Тогда
		...
		АвтоЗаголовок = Ложь;
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Los derechos de admisión (%1)';ru='Права доступа (%1)';en='Access rights (%1)'"), Строка(Параметры.Пользователь));
	КонецЕсли;
