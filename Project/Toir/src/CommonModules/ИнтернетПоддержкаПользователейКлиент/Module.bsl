...
Процедура ПодключитьИнтернетПоддержкуПользователей(
	...
	Если Не ДоступноПодключениеИнтернетПоддержки() Тогда
		...
		ПоказатьПредупреждение(
			ОповещениеАвторизацияНедоступна,
			НСтр("es='Es insuficientemente derecho para la conexión del Apoyo De Internet de los usuarios. Se dirijan al administrador.';ru='Недостаточно прав для подключения Интернет-поддержки пользователей. Обратитесь к администратору.';en='Not enough rights to connect online user support. Contact your administrator.'"));
		Возврат;
...
//	ЗаголовокОкна - Строка - заголовок окна для метода
...
Процедура ОткрытьСтраницуИнтегрированногоСайта(URLСтраницыСайта, ЗаголовокОкна = "") Экспорт
	...
	Если Не ПустаяСтрока(РезультатПолученияURL.КодОшибки) И РезультатПолученияURL.КодОшибки <> "НеверныйЛогинИлиПароль" Тогда
		ПоказатьОповещениеПользователя(
			,
			,
			НСтр("es='La falta de la entrada al Portal 1С:ИТС. "
		...
		Исключение
			ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No consiguió conectar el elaborador de la interpelación de los ajustes del cliente del licenciamiento. "
"%1';ru='Не удалось подключить обработчик запроса настроек клиента лицензирования."
"%1';en='Failed to connect the licensing client settings request handler."
"%1'"),
...
Процедура ИнтернетПоддержкаИСервисы_БИПВойтиИлиВыйти(Форма, Команда) Экспорт
	...
	Иначе
		ПоказатьВопрос(
			...
			НСтр("es='El login y la consigna para la conexión a los servicios del Apoyo De Internet de los usuarios serán quitados del programa. "
"¿Desconectar el Apoyo De Internet?';ru='Логин и пароль для подключения к сервисам Интернет-поддержки пользователей будут удалены из программы."
"Отключить Интернет-поддержку?';en='Login and password for connecting to Internet user support services will be deleted from the program."
"Turn off online support?'"),
			РежимДиалогаВопрос.ДаНет,
			,
			КодВозвратаДиалога.Нет,
			НСтр("es='La salida del Apoyo De Internet de los usuarios';ru='Выход из Интернет-поддержки пользователей';en='Exiting Internet User Support'"));
	КонецЕсли;
...
Процедура ИнтернетПоддержкаИСервисы_СообщениеВСлужбуТехническойПоддержки(Форма, Команда) Экспорт
	...
	ОтправитьСообщениеВТехПоддержку(
		...
		НСтр("es='<Rellenen el texto del mensaje>';ru='<Заполните текст сообщения>';en='<Заполните текст сообщения>'"));
...
Процедура ОткрытьВебСтраницуСДополнительнымиПараметрами(
	...
	Если НеобходимаАвторизации И ПараметрыОткрытия.ЭтоПолноправныйПользователь Тогда
		...
		Состояние(, , НСтр("es='El tránsito al Portal 1С:ИТС';ru='Переход на Портал 1С:ИТС';en='Going to Portal 1C: ITS'"));
...
Процедура ОтправитьСообщениеВТехПоддержку(
	...
	Если Получатель <> "webIts" И Получатель <> "taxcom" И Получатель <> "backup" Тогда
		ВызватьИсключение НСтр("es='El destinatario desconocido del mensaje.';ru='Неизвестный получатель сообщения.';en='Unknown message recipient.'");
	КонецЕсли;
	...
	Состояние(, , НСтр("es='La preparación del mensaje en el servicio del soporte técnico';ru='Подготовка сообщения в службу технической поддержки';en='Preparation of a message to technical support'"));
...
Процедура ОткрытьЛичныйКабинетПользователя() Экспорт
	...
	ОткрытьВебСтраницу(
		...
		НСтр("es='El gabinete personal del usuario';ru='Личный кабинет пользователя';en='User account'"));
...
Процедура ОткрытьГлавнуюСтраницуПортала() Экспорт
	...
	ОткрытьВебСтраницу(
		ИнтернетПоддержкаПользователейКлиентСервер.URLСтраницыПорталаПоддержки(
			...
			НастройкиСоединенияССерверами().ДоменРасположенияСерверовИПП),
		НСтр("es='El portal 1С:ИТС';ru='Портал 1С:ИТС';en='Portal 1C: ITS'"));
...
Процедура ОткрытьСтраницуВосстановленияПароля() Экспорт
	...
	ОткрытьВебСтраницу(
		...
		НСтр("es='La reconstitución de la consigna';ru='Восстановление пароля';en='Password recovery'"));
...
Процедура ОткрытьСтраницуОфициальнаяПоддержка() Экспорт
	...
	ОткрытьВебСтраницу(
		ИнтернетПоддержкаПользователейКлиентСервер.URLСтраницыПорталаПоддержки(
			...
			НастройкиСоединенияССерверами().ДоменРасположенияСерверовИПП),
		НСтр("es='El apoyo oficial';ru='Официальная поддержка';en='Official support'"));
	...
	Иначе
		...
		ФайловаяСистемаКлиент.ПодключитьРасширениеДляРаботыСФайлами(
			Новый ОписаниеОповещения(
				...
				ЭтотОбъект,
				ПараметрыСообщения),
			НСтр("es='Para la continuación es necesario establecer la ampliación para el trabajo con los ficheros.';ru='Для продолжения необходимо установить расширение для работы с файлами.';en='To continue, you must install the extension for working with files.'"));
...
Процедура ПриОтправкеДанныхСообщенияВТехПоддержку(РезультатОтправки, ПараметрыСообщения)
	...
	Если Результат Тогда
		...
		ПараметрыОткрытия = Новый Структура;
		ПараметрыОткрытия.Вставить("ЗаголовокОкна", НСтр("es='El envío del mensaje en el servicio del soporte técnico';ru='Отправка сообщения в службу технической поддержки';en='Sending a message to technical support'"));
...
Функция ВидПриложения()
	...
	#Если ВебКлиент Тогда
	Возврат НСтр("es='El cliente Web';ru='Веб-клиент';en='Web client'");
	#ИначеЕсли ТолстыйКлиентОбычноеПриложение Тогда
	Возврат НСтр("es='El cliente gordo, la aplicación regular';ru='Толстый клиент, обычное приложение';en='Fat client, regular application'");
