...
Процедура ФоновыйРасчетППР() Экспорт
	...
	|ГДЕ
	|	НЕ торо_ППРДляРасчетаВФоновомРежимеСрезПоследних.Рассчитан &ПересчетППРПослеОшибки
	|
	|УПОРЯДОЧИТЬ ПО
	|	Период";
	...
	Если НеПересчитыватьППРВФонеПослеОшибки Тогда
	    Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПересчетППРПослеОшибки", "И ВЫРАЗИТЬ(торо_ППРДляРасчетаВФоновомРежимеСрезПоследних.ОписаниеОшибки КАК СТРОКА(1)) = """" ");
	Иначе
	    Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПересчетППРПослеОшибки", "");
	КонецЕсли; 
	...
	Пока ВыборкаДляРасчета.Следующий() Цикл
		...
		Попытка
			...
			Иначе
				ТекстСообщения = НСтр("es='¡El cálculo no es cumplido! Comprueben los ajustes del sistema y el ajuste del usuario para la ejecución de la tarea de fondo.';ru='Расчет не выполнен! Проверьте настройки системы и настройки пользователя для выполнения фонового задания.';en='Calculation failed! Check your system settings and user settings for the background task.'");
				ЗаписьФоновыйППР.ДокументППР = ВыборкаДляРасчета.ДокументППР;
				ЗаписьФоновыйППР.Период = ТекущаяДата();
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("es='La excepción';ru='Исключение';en='An exception'"), УровеньЖурналаРегистрации.Предупреждение, 
				Метаданные.ОбщиеМодули.торо_РасчетППР, ОписаниеОшибки());			
			ЗаписьФоновыйППР.ДокументППР = ВыборкаДляРасчета.ДокументППР;
			ЗаписьФоновыйППР.Период = ТекущаяДата();
