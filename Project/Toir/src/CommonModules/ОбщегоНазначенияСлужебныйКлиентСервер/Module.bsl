...
"por la causa: "
"la línea Inadmisible de la orden "
"%1 "
"por la causa: "
"la Línea de la orden no debe contener los símbolos: ""$"", ""`"", ""|"", "";"", ""y"".';ru='Не удалось запустить программу"
"по причине:"
"Недопустимая строка команды"
"%1"
"по причине:"
"Строка команды не должна содержать символы: ""$"", ""`"", ""|"", "";"", ""&"".';en='Failed to start program"
"due to:"
"Invalid command line"
"%1"
"due to:"
"The command line must not contain the characters: ""$"", ""`"", ""|"", "";"", ""&"".'"),
				КомандаЗапуска);
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(КомандаЗапуска) = Тип("Массив") Тогда
		...
		Если КомандаЗапуска.Количество() > 0 Тогда 
			...
			Если СодержитНебезопасныеДействия(КомандаЗапуска[0]) Тогда
				...
				НСтр("es='No consiguió poner en marcha el programa "
"por la causa: "
"la orden Inadmisible o la vía al fichero cumplido "
"%1 "
"por la causa: "
"la Orden no debe contener los símbolos: ""$"", ""`"", ""|"", "";"", ""y"".';ru='Не удалось запустить программу"
"по причине:"
"Недопустимая команда или путь к исполняемому файлу"
"%1"
"по причине:"
"Команда не должна содержать символы: ""$"", ""`"", ""|"", "";"", ""&"".';en='Failed to start program"
"due to:"
"Invalid command or executable path"
"%1"
"due to:"
"The command must not contain the characters: ""$"", ""`"", ""|"", "";"", ""&"".'"),
				КомандаЗапуска[0]);
			КонецЕсли;
		...
		Иначе
			ВызватьИсключение
				НСтр("es='Se espera que el primer elemento del macizo de KomandaZapuska será la orden o la vía al fichero cumplido.';ru='Ожидалось, что первый элемент массива КомандаЗапуска будет командой или путем к исполняемому файлу.';en='The first element of the КомандаЗапуска array was expected to be a command or path to an executable file.'");
		КонецЕсли;
		
	Иначе 
		ВызватьИсключение 
			НСтр("es='Se espera que el significado de KomandaZapuska será <la Línea> o <el Macizo>';ru='Ожидалось, что значение КомандаЗапуска будет <Строка> или <Массив>';en='The КомандаЗапуска value was expected to be <Строка> or <Массв>'");
	КонецЕсли;
...
Функция ТекстОшибкиПредопределенноеЗначениеНеНайдено(ПолноеИмяПредопределенного) Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El significado predestinado ""%1"" no es encontrado.';ru='Предопределенное значение ""%1"" не найдено.';en='Predefined value ""%1"" was not found.'"), ПолноеИмяПредопределенного);
...
#Если МобильныйКлиент Тогда
	...
	СтрокаСообщенияОбОшибке = НСтр("es='La conexión a otro programa no es accesible en el cliente móvil.';ru='Подключение к другой программе не доступно в мобильном клиенте.';en='Connection to another program is not available in the mobile client.'");
...
#Иначе
	...
	Исключение
		...
		СтрокаСообщенияОбОшибке = НСтр("es='No consiguió se conectará a otro programa: %1';ru='Не удалось подключится к другой программе: %1';en='Failed to connect to another program:%1'");
