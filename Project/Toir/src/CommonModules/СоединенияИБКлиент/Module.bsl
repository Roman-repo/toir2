...
Процедура ЗавершитьРаботуЭтогоСеанса(ВыводитьВопрос = Истина) Экспорт
	...
	ТекстСообщения = НСтр("es='El trabajo de los usuarios con el programa es prohibido. ¿Acabar trabajo de esta sesión?';ru='Работа пользователей с программой запрещена. Завершить работу этого сеанса?';en='User work with the program is prohibited. Close this session?'");
	Заголовок = НСтр("es='La terminación del trabajo de la sesión corriente';ru='Завершение работы текущего сеанса';en='Current session work completing'");
...
Процедура ПослеНачалаРаботыСистемы() Экспорт
	...
	// ТОиР++
	Если НЕ ПараметрыРаботы.Свойство("ПараметрыБлокировкиСеансов") Тогда
		Возврат;
	КонецЕсли;
	// ТОиР--
...
Процедура ПередЗавершениемРаботыСистемы(Отказ, Предупреждения) Экспорт
	...
	Если РаботаПользователейЗавершается() Тогда
		...
		ПараметрыПредупреждения.ТекстГиперссылки = НСтр("es='El bloqueo del trabajo de los usuarios';ru='Блокировка работы пользователей';en='Blocking user work'");
		ПараметрыПредупреждения.ТекстПредупреждения = НСтр("es='De la sesión corriente se cumple la terminación del trabajo de los usuarios';ru='Из текущего сеанса выполняется завершение работы пользователей';en='Users are shutting down from the current session'");
...
Процедура ИнтерактивнаяОбработкаПередНачаломРаботыСистемы(Параметры, Контекст) Экспорт
	...
	Если Не ПустаяСтрока(ТекстВопроса) Тогда
		...
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("es='Entrar';ru='Войти';en='To log in'"));
		Если ПараметрыКлиента.ВозможноСнятьБлокировку Тогда
			Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("es='Quitar el bloqueo y entrar';ru='Снять блокировку и войти';en='Unlock and login'"));
		КонецЕсли;
...
Процедура ПоказатьПредупреждениеПриЗавершенииРаботы(ТекстСообщения) Экспорт
	...
	Если ПараметрыПриложения[ИмяПараметра] <> Истина Тогда
		ПоказатьОповещениеПользователя(НСтр("es='El trabajo del programa será acabado';ru='Работа программы будет завершена';en='Program work will be completed'"),, ТекстСообщения,, 
...
Процедура ЗадатьВопросПриЗавершенииРаботы(ТекстСообщения) Экспорт
	...
	ИмяПараметра = "СтандартныеПодсистемы.ПоказаноПредупреждениеПередЗавершениемРаботы";
	Если ПараметрыПриложения[ИмяПараметра] <> Истина Тогда
		ПоказатьОповещениеПользователя(НСтр("es='El trabajo del programa será acabado';ru='Работа программы будет завершена';en='Program work will be completed'"),, ТекстСообщения,, СтатусОповещенияПользователя.Важное);
		ПараметрыПриложения.Вставить("СтандартныеПодсистемы.ПоказаноПредупреждениеПередЗавершениемРаботы", Истина);
	КонецЕсли;	
		
	ТекстВопроса = НСтр("es='¿%1 "
"Acabar el trabajo?';ru='%1"
"Завершить работу?';en='%1 Finish work?'");
...
Функция ОбработатьПараметрыЗапуска(Знач ПараметрыЗапуска)
	...
	Если ПараметрыЗапуска.Найти("РазрешитьРаботуПользователей") <> Неопределено Тогда
		...
		Если НЕ СоединенияИБВызовСервера.РазрешитьРаботуПользователей() Тогда
			ТекстСообщения = НСтр("es='El parámetro del lanzamiento de RazreshitRabotuPolzovateley no es trabajado. No hay derechos a la administración de la base informativa.';ru='Параметр запуска РазрешитьРаботуПользователей не отработан. Нет прав на администрирование информационной базы.';en='Launch parameter РазрешитьРаботуПользователей not worked out. No rights to administer the infobase.'");
			ПоказатьПредупреждение(,ТекстСообщения);
			Возврат Ложь;
		КонецЕсли;
		
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),,
			НСтр("es='Es cumplido el lanzamiento con el parámetro ""RazreshitRabotuPolzovateley"". El trabajo del programa será acabado.';ru='Выполнен запуск с параметром ""РазрешитьРаботуПользователей"". Работа программы будет завершена.';en='Launched with the option ""РазрешитьРаботуПользователей"". The program will be completed.'"), ,Истина);
		ЗавершитьРаботуСистемы(Ложь);
		Возврат Истина;
	...
	ИначеЕсли ПараметрыЗапуска.Найти("ЗавершитьРаботуПользователей") <> Неопределено Тогда
		...
		Если НЕ СоединенияИБВызовСервера.УстановитьБлокировкуСоединений() Тогда
			ТекстСообщения = НСтр("es='El parámetro del lanzamiento de ZavershitRabotuPolzovateley no es trabajado. No hay derechos a la administración de la base informativa.';ru='Параметр запуска ЗавершитьРаботуПользователей не отработан. Нет прав на администрирование информационной базы.';en='Launch parameter ЗавершитьРаботуПользователей not worked out. No rights to administer the infobase.'");
			ПоказатьПредупреждение(,ТекстСообщения);
			Возврат Ложь;
		КонецЕсли;
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("es='La terminación del trabajo de los usuarios';ru='Завершение работы пользователей';en='User work completing'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
