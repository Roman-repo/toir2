...
Процедура ВключитьВерсионированиеОбъекта(ИмяОбъекта, Знач ВариантВерсионирования = Неопределено) Экспорт
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La variante desconocida версионирования: ""%1""';ru='Неизвестный вариант версионирования: ""%1""';en='Unknown versioning option: ""%1""'"), ВариантВерсионирования);
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("es='Версионирование';ru='Версионирование';en='Versioning'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка, НаборЗаписей.Метаданные(),
				,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No consiguió renovar las noticias sobre la versión № %1 objetos ""%2"" por la causa: "
"%3';ru='Не удалось обновить сведения о версии №%1 объекта ""%2"" по причине:"
"%3';en='Failed to update version number%1 of object ""%2"" due to:                          %3'", ОбщегоНазначения.КодОсновногоЯзыка()),
					Выборка.НомерВерсии,
	...
	Если Выборка.Количество() > 0 Тогда
		Если ОбработаноЗаписей = 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es=""Al procedimiento ObnovitSvedeniyaOVersiyahOb'ektov no consiguió tratar algunas anotaciones del registro de las noticias VersiiOb'ektov (son dejados pasar): %1"";ru='Процедуре ОбновитьСведенияОВерсияхОбъектов не удалось обработать некоторые записи регистра сведений ВерсииОбъектов (пропущены): %1';en='A procedure ОбновитьСведенияОВерсияхОбъектов failed to process some entries of the register of information ВерсииОбъектов (skipped):%1'"), 
					Выборка.Количество());
...
Процедура ПриПолученииДанныхОтПодчиненного(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад, Отправитель) Экспорт
	...
	Если ТипЗнч(ЭлементДанных) = Тип("РегистрСведенийНаборЗаписей.ВерсииОбъектов") И ЭлементДанных.Количество() > 0 Тогда
		...
		Если НомерВерсииСКоллизией = Неопределено Тогда
			...
			Запись.Комментарий = НСтр("es='La versión declinada (el permiso automático del conflicto).';ru='Отклоненная версия (автоматическое разрешение конфликта).';en='Rejected version (automatic conflict resolution).'");
			НаборЗаписей.Записать();
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("es='La renovación de las noticias sobre las versiones anotadas de los objetos. Después de la ejecución de la renovación se aumentará la velocidad de la anotación версионируемых de los objetos.';ru='Обновление сведений о записанных версиях объектов. После выполнения обновления увеличится скорость записи версионируемых объектов.';en='Updating information about recorded versions of objects. After the update is completed, the write speed of the versioned objects will increase.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "2.2.4.13";
	Обработчик.Процедура = "ВерсионированиеОбъектов.ПеренестиНастройкиВерсионирования";
	Обработчик.Комментарий = НСтр("es='La renovación de los ajustes версионирования.';ru='Обновление настроек версионирования.';en='Update versioning settings.'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Подсказка = НСтр("es='De las versiones que han caducado: %1 (%2)';ru='Устаревших версий: %1 (%2)';en='Outdated versions:%1 (%2)'");
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("es='Las versiones que han caducado de los objetos';ru='Устаревшие версии объектов';en='Outdated object versions'");
		...
		Если Не Объект.ЭтоНовый() И (ИзмененаПроведенность И СведенияОВерсииОбъекта.НомерВерсии > 1 Или ВерсияОтличаетсяОтРанееЗаписанной(Объект)) Тогда
			...
			Если СведенияОВерсииОбъекта.НомерВерсии = 1 Тогда
				Если ОбъектВерсионируется(Объект.Ссылка, Ложь) Тогда
					...
					ПараметрыВерсии.Вставить("Комментарий", НСтр("es='La versión es creada por ya objeto que hay';ru='Версия создана по уже имеющемуся объекту';en='Version created on an existing object'"));
...
Процедура ПроверитьНаличиеПравНаИзменениеОбъекта(ОбъектМетаданных)
	...
	Если Не ПривилегированныйРежим() И Не ПравоДоступа("Изменение", ОбъектМетаданных)Тогда
		ТекстСообщения = НСтр("es='Es insuficientemente derecho a cambio ""%1"".';ru='Недостаточно прав на изменение ""%1"".';en=""Not enough rights to change '%1'.""");
...
Функция ОтчетПоВерсииОбъекта(СсылкаНаОбъект, Знач ВерсияОбъекта = Неопределено, ПользовательскийНомерВерсии = Неопределено) Экспорт
	...
	Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='№ %1 / (%2) / %3';ru='№ %1 / (%2) / %3';en='№ %1 / (%2) / %3'"), ПользовательскийНомерВерсии,
...
Функция ВосстановитьОбъектПоXML(ДанныеОбъекта, ТекстСообщенияОбОшибке = "")
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("es='Версионирование';ru='Версионирование';en='Versioning'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ТекстСообщенияОбОшибке = НСтр("es='No consiguió pasar a la versión escogida. "
"La causa posible: la versión del objeto era anotada en otra versión del programa. "
"La información técnica sobre la falta: %1';ru='Не удалось перейти на выбранную версию."
"Возможная причина: версия объекта была записана в другой версии программы."
"Техническая информация об ошибке: %1';en='Failed to upgrade to the selected version."
"Possible reason: the version of the object was recorded in another version of the program."
"Technical error information:%1 '");
...
Функция СведенияОВерсииОбъекта(Знач Ссылка, Знач НомерВерсии) Экспорт
	СообщениеНеУдалосьПолучитьВерсию = НСтр("es='No consiguió recibir la versión anterior del objeto.';ru='Не удалось получить предыдущую версию объекта.';en='Failed to get the previous version of the object.'");
	...
	Если Результат.ВерсияОбъекта = Неопределено Тогда
		ВызватьИсключение НСтр("es='La versión escogida del objeto falta en el programa.';ru='Выбранная версия объекта отсутствует в программе.';en='The selected version of the object is missing from the program.'");
	КонецЕсли;
...
Функция РазмерДанныхСтрокой(Знач РазмерДанных)
	...
	ЕдиницаИзмерения = НСтр("es='El byte';ru='байт';en='byte'");
	Если 1024 <= РазмерДанных И РазмерДанных < 1024 * 1024 Тогда
		РазмерДанных = РазмерДанных / 1024;
		ЕдиницаИзмерения = НСтр("es='Кб';ru='Кб';en='Kb'");
	...
	ИначеЕсли 1024 * 1024 * 1024 <= РазмерДанных Тогда
		...
		ЕдиницаИзмерения = НСтр("es='El Mb';ru='Мб';en='Mb'");
	КонецЕсли;
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 %2';ru='%1 %2';en='12%'"), РазмерДанных, ЕдиницаИзмерения);
...
Функция ПерейтиНаВерсиюСервер(Ссылка, НомерВерсии, ТекстСообщенияОбОшибке, ОтменаПроведения = Ложь) Экспорт
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		ВызватьИсключение НСтр("es='Es insuficientemente derecho para la ejecución de la operación.';ru='Недостаточно прав для выполнения операции.';en='Not enough rights to perform the operation.'");
	КонецЕсли;
	...
	Объект.ДополнительныеСвойства.Вставить("ВерсионированиеОбъектовКомментарийКВерсии",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Es cumplido el tránsito a la versión № %1 de %2';ru='Выполнен переход к версии №%1 от %2';en='Migrated to version% 1 from% 2'"),
