...
Процедура ДобавитьКонтактнуюИнформацию(СсылкаИлиОбъект, ЗначениеИлиПредставление, ВидКонтактнойИнформации, Дата = Неопределено, Замещать = Истина) Экспорт
	...
	Если МетаданныеОбъекта = Неопределено
		...
		ВызватьИсключение НСтр("es='La adición de la información de contacto es imposible, el objeto no tiene tabla con la información de contacto.';ru='Добавление контактной информации невозможно, у объекта нет таблицы с контактной информацией.';en='Adding contact information is not possible, the object does not have a table with contact information.'");;
	КонецЕсли;
...
Функция СтранаАдресаКонтактнойИнформации(Знач Адрес) Экспорт
	...
	ИначеЕсли ТипЗнч(Адрес) <> Тип("Структура") Тогда
		...
		ВызватьИсключение НСтр("es='Es imposible determinar el país, se espera la dirección.';ru='Невозможно определить страну, ожидается адрес.';en='Unable to determine country, address expected.'");
...
Функция ДоменАдресаКонтактнойИнформации(Знач КонтактнаяИнформация) Экспорт
	...
	ВызватьИсключение НСтр("es='Es imposible determinar el dominio, se esperan el correo electrónico o la referencia web.';ru='Невозможно определить домен, ожидается электронная почта или веб-ссылка.';en='Unable to determine domain, e-mail or web link expected.'");
КонецФункции
...
Функция НомерТелефонаКонтактнойИнформации(Знач КонтактнаяИнформация) Экспорт
	...
	ВызватьИсключение НСтр("es='Es imposible determinar el número, se espera teléfono o el fax.';ru='Невозможно определить номер, ожидается телефона или факс.';en='Unable to determine number, expected phone or fax.'");
...
Процедура СоздатьВТКонтактнаяИнформация(МенеджерВременныхТаблиц, МассивОбъектов, ТипыКонтактнойИнформации = Неопределено, ВидыКонтактнойИнформации = Неопределено, Дата = Неопределено) Экспорт
	...
	Если ТипЗнч(МассивОбъектов) <> Тип("Массив") ИЛИ МассивОбъектов.Количество() = 0 Тогда
		ВызватьИсключение НСтр("es='El significado equivocado para el macizo de los propietarios de la información de contacto.';ru='Неверное значение для массива владельцев контактной информации.';en='Invalid value for an array of contact information owners.'");
	КонецЕсли;
	...
	Для каждого ОбъектСКонтактнойИнформацией Из ОбъектыСГруппировкойПоТипам Цикл
		...
		Если МетаданныеОбъекта.ТабличныеЧасти.Найти("КонтактнаяИнформация") = Неопределено Тогда
			ВызватьИсключение  МетаданныеОбъекта.Имя + " " + НСтр("es='No contiene la información de contacto.';ru='не содержит контактную информацию.';en='does not contain contact information.'");
		КонецЕсли;
	...
	Для каждого ОбъектСКонтактнойИнформацией Из ОбъектыСГруппировкойПоТипам Цикл
		...
		Если МетаданныеОбъекта.ТабличныеЧасти.КонтактнаяИнформация.Реквизиты.Найти("ДействуетС") <> Неопределено Тогда
			Если ТипЗнч(Дата) <> Тип("Дата") Тогда
				ВызватьИсключение НСтр("es='Para la recepción de la información de contacto que guarda la historia de los cambios, "
"es necesario indicar la fecha, de que funciona la anotación de la información de contacto.';ru='Для получения контактной информации, хранящей историю изменений,"
"требуется указывать дату, с которой действует запись контактной информации.';en='For contact information that stores the change history,"
"the date on which the contact information entry is valid is required.'");
			КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из Форма.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов Цикл
		...
		Если СтрокаТаблицы.ЭтоРеквизитТабличнойЧасти Тогда
			...
			Для Каждого СтрокаТабличнойЧастиФормы Из ТабличнаяЧастьФормы Цикл
				...
				Если ОбязательноеЗаполнение И ПустаяСтрока(Представление) И Не ВидИнформации.ПометкаУдаления Тогда
					...
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Campo ""%1"" no es rellenado.';ru='Поле ""%1"" не заполнено.';en='Field ""%1"" is empty.'"), ВидИнформации.Наименование),,Поле);
		...
		Иначе
			...
			Если ВидИнформации <> ПредыдущийВид И ОбязательноеЗаполнение И ПустаяСтрока(Представление)
				...
				ОбщегоНазначения.СообщитьПользователю(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Campo ""%1"" no es rellenado.';ru='Поле ""%1"" не заполнено.';en='Field ""%1"" is empty.'"), ВидИнформации.Наименование),,, ИмяРеквизита);
				ТекущийУровеньОшибок = 2;
...
Процедура КолонкиДляЗагрузкиДанных(МетаданныеСправочника, ИнформацияПоКолонкам) Экспорт
	...
	Для каждого ВидКонтактнойИнформации Из ВидыКонтактнойИнформации Цикл
		...
		Если ИнформацияПоКолонкам.Найти(ИмяКолонки, "ИмяКолонки") = Неопределено Тогда
			...
			СтрокаИнфоПроКолонки.Группа = НСтр("es='La información de contacto';ru='Контактная информация';en='Contact Information'");
...
Процедура ДобавитьКнопкуДополнительногоПоляКонтактнойИнформации(Знач Форма, Знач ИмяЭлементаДляРазмещения)
	...
	Описание = НСтр("es='Añadir el campo adicional de la información de contacto';ru='Добавить дополнительное поле контактной информации';en='Add additional contact information field'");
	...
	Кнопка.Заголовок   = НСтр("es='Añadir';ru='Добавить';en='Add'");
...
Процедура ДобавитьПояснениеПоСбросуНастроекФормы(Знач Форма, Знач ИмяЭлементаДляРазмещения, Знач ОтложеннаяИнициализация)
	...
	Если ОтложеннаяИнициализация
		...
		ПредупреждениеОРазмещении = НСтр("es='Para la representación de la información de contacto es necesario instalar grupo ""%1"" no por el primer elemento (después de cualquier otro grupo) en grupo ""%2"" (el menú Todavía - Cambiar la forma).';ru='Для отображения контактной информации необходимо разместить группу ""%1"" не первым элементом (после любой другой группы) в группе ""%2"" (меню Еще - Изменить форму).';en='To display contact information, it is necessary to place the group ""% 1"" not the first element (after any other group) in the group ""% 2"" (menu More - Change form).'");
		...
		ТекстПодсказки = НСтр("es='Es posible establecer también los ajustes estandartizados de la forma:"
" • en el menú escoger Todavía el punto Cambiar la forma...;"
" • en la forma que se ha abierto ""el Ajuste de la forma"" en el menú escoger Todavía el punto ""Establecer los ajustes estandartizados"".';ru='Также можно установить стандартные настройки формы:"
"   • в меню Еще выбрать пункт Изменить форму...;"
"   • в открывшейся форме ""Настройка формы"" в меню Еще выбрать пункт ""Установить стандартные настройки"".';en='You can also set the default form settings:"
"    • in the More menu, select the Change form ... item;"
"    • in the opened form ""Form Settings"" in the Still menu, select the item "" Set default settings"".'");
...
Функция СоздатьПолеВвода(Форма, Родитель, ИнформацияОВидеКИ, ИмяРеквизита, ИмяЭлементаДляРазмещения,ЭтоНовыйВидКИ = Ложь, ОбязательноеЗаполнение = Ложь)
	...
	Если ИнформацияОВидеКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
		// ТОИР++
		Если ИнформацияОВидеКИ.ИмяПредопределенногоВида = "АдресОР" Тогда 
	...
	Если ИнформацияОВидеКИ.Тип <> Перечисления.ТипыКонтактнойИнформации.Другое И Не ИнформацияОВидеКИ.ПометкаУдаления Тогда
		...
		Кнопка.Заголовок = НСтр("es='Introducir el comentario';ru='Ввести комментарий';en='Enter a comment'");
		Команда = Форма.Команды.Добавить(ИмяКоманды);
		Команда.Подсказка = НСтр("es='Introducir el comentario';ru='Ввести комментарий';en='Enter a comment'");
	...
	Если ИнформацияОВидеКИ.ХранитьИсториюИзменений И Не ИнформацияОВидеКИ.ПометкаУдаления Тогда
		...
		Кнопка = Форма.Элементы.Добавить(ИмяКоманды, Тип("КнопкаФормы"), Элемент.КонтекстноеМеню);
		Кнопка.Заголовок = НСтр("es='La historia de los cambios...';ru='История изменений...';en='Change history ...'");
		Команда = Форма.Команды.Добавить(ИмяКоманды);
		Команда.Картинка = БиблиотекаКартинок.ИсторияИзменений;
		Команда.Подсказка = НСтр("es='Muestra la historia del cambio de la información de contacto';ru='Показывает историю изменения контактной информации';en='Shows contact change history'");
		Команда.Действие = "Подключаемый_КонтактнаяИнформацияВыполнитьКоманду";
	...
	Если ИнформацияОВидеКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
		...
		Кнопка.Заголовок = НСтр("es='La dirección al Yandex. Las tarjetas';ru='Адрес на Яндекс.Картах';en='Address on Yandex.Maps'");
		Команда = Форма.Команды.Добавить(ИмяКоманды);
		Команда.Картинка = БиблиотекаКартинок.ЯндексКарты;
		Команда.Подсказка = НСтр("es='Muestra la dirección en las tarjetas el Yandex. Las tarjetas';ru='Показывает адрес на картах Яндекс.Карты';en='Shows the address on Yandex.Maps'");
		Команда.Действие = "Подключаемый_КонтактнаяИнформацияВыполнитьКоманду";
		Кнопка.ИмяКоманды = ИмяКоманды;
		...
		Кнопка = Форма.Элементы.Добавить(ИмяКоманды,Тип("КнопкаФормы"), ГруппаРазделительКарты);
		Кнопка.Заголовок = НСтр("es='La dirección en Google Maps';ru='Адрес на Google Maps';en='Address on Google Maps'");
		Команда = Форма.Команды.Добавить(ИмяКоманды);
		Команда.Картинка = БиблиотекаКартинок.GoogleMaps;
		Команда.Подсказка = НСтр("es='Indica la dirección en el mapa Google Maps';ru='Показывает адрес на карте Google Maps';en='Shows the address on the map Google Maps'");
		Команда.Действие = "Подключаемый_КонтактнаяИнформацияВыполнитьКоманду";
		Кнопка.ИмяКоманды = ИмяКоманды;
		...
		Если Не ИнформацияОВидеКИ.ПометкаУдаления Тогда
			...
			ГруппаПодменюАдреса.Заголовок = НСтр("es='Rellenar';ru='Заполнить';en='Fill in'");
		КонецЕсли;
...
Процедура ПунктКонтекстногоМенюПеремещения(ПредыдущийЭлемент, Форма, Направление, ИмяЭлементаДляРазмещения)
	...
	Команда.Действие = "Подключаемый_КонтактнаяИнформацияВыполнитьКоманду";
	Если Направление > 0 Тогда 
		ТекстКоманды = НСтр("es='Trasladar hacia arriba';ru='Переместить вверх';en='Move up'");
...
Функция ОшибкиЗаполненияЭлектроннойПочты(АдресЭП, ВидИнформации, Знач ИмяРеквизита = "", ПолеРеквизита = "")
	...
	Попытка
		...
		Если Результат.Количество() > 1 Тогда
			...
			СтрокаОшибки = НСтр("es='Se permite la introducción solamente una dirección del correo electrónico';ru='Допускается ввод только одного адреса электронной почты';en='Only one email address is allowed'");
...
Процедура СоздатьДействие(Форма, ВидКонтактнойИнформации, ИмяРеквизита, ГруппаДействий, КоличествоАдресов, ЕстьКомментарий = Ложь, ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация")
	...
	Если СоздаватьДействиеДляТипа[Тип] = Истина Тогда
		...
		Если Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
			...
			ГруппаПодменю.Заголовок = НСтр("es='La información de contacto';ru='Контактная информация';en='Contact Information'");
		...
		ИначеЕсли Тип = Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты Тогда
			...
			Элемент.Заголовок = НСтр("es='Escribir la carta';ru='Написать письмо';en='Write a letter'");
			Команда.Подсказка = НСтр("es='Escribir la carta';ru='Написать письмо';en='Write a letter'");
		...
		ИначеЕсли Тип = Перечисления.ТипыКонтактнойИнформации.ВебСтраница Тогда
			...
			Элемент.Заголовок = НСтр("es='Pasar';ru='Перейти';en='Go to'");
			Команда.Подсказка = НСтр("es='Pasar por la referencia';ru='Перейти по ссылке';en='Go to the link'");
		...
		ИначеЕсли Тип = Перечисления.ТипыКонтактнойИнформации.Телефон Тогда
			Если Форма.ПараметрыКонтактнойИнформации[ИмяЭлементаДляРазмещения].МожноОтправлятьSMS Тогда
				Элемент.Заголовок = НСтр("es='Llamar o enviar SMS';ru='Позвонить или отправить SMS';en='Call or send SMS'");
				Команда.Подсказка = НСтр("es='Llamar o enviar SMS';ru='Позвонить или отправить SMS';en='Call or send SMS'");
				Команда.Картинка = БиблиотекаКартинок.ПозвонитьИлиОтправитьSMS;
			Иначе
				Элемент.Заголовок = НСтр("es='Llamar o enviar SMS';ru='Позвонить или отправить SMS';en='Call or send SMS'");
				Команда.Подсказка = НСтр("es='Llamar por teléfono';ru='Позвонить по телефону';en='Make a phone call'");
				Команда.Картинка = БиблиотекаКартинок.Позвонить;
			КонецЕсли;
			
		ИначеЕсли Тип = Перечисления.ТипыКонтактнойИнформации.Skype Тогда
			Элемент.Заголовок = НСтр("es='Skype';ru='Skype';en='Skype'");
			Команда.Подсказка = НСтр("es='Skype';ru='Skype';en='Skype'");
	...
	Для Каждого СтрокаКИ Из ВсеСтроки Цикл
		...
		Для Каждого Адрес Из НайденныеСтроки Цикл
			...
			Если Команда = Неопределено Тогда
				Команда = Форма.Команды.Добавить(ИмяКоманды);
				Команда.Подсказка = НСтр("es='Copiar la dirección';ru='Скопировать адрес';en='Copy address'");
				Команда.Действие = "Подключаемый_КонтактнаяИнформацияВыполнитьКоманду";
				Команда.ИзменяетСохраняемыеДанные = Истина;
...
Функция ПараметрыИзВидаКонтактнойИнформации(Знач ВидКонтактнойИнформации)
	...
	Если РезультатЗапроса.Количество() = 0 Тогда
		ШаблонТекстаОшибки = НСтр("es='A la recepción de las propiedades de la información de contacto era entregado el tipo equivocado de la información de contacto. %1';ru='При получении свойств контактной информации был передан неверный вид контактной информации. %1';en='Upon receipt of the contact information properties, the wrong type of contact information was transmitted. %1'");
