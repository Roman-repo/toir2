...
Процедура ВыбратьЗначениеДополнительногоРеквизита(Форма, Элемент, СтандартнаяОбработка) Экспорт
	...
	Если СписокДоступныхТипов.Количество() > 1 Тогда
		ЗаголовокФормы = НСтр("es='El tipo del requisito ""%Свойство del %""';ru='Тип реквизита ""%Свойство%""';en='Type of props ""%Свойство%""'");
...
Процедура ВыбратьЗначениеРеквизитаСоставногоТипаВТаблице(Форма, ТекущиеДанные, Реквизит, СтандартнаяОбработка) Экспорт
	...
	Если СписокДоступныхТипов.Количество() > 1 Тогда
		ЗаголовокФормы = НСтр("es='La elección del tipo de datos';ru='Выбор типа данных';en='Data type selection'");
		ПараметрыФормы = Новый Структура("СписокДоступныхТипов, ЗаголовокФормы", СписокДоступныхТипов, ЗаголовокФормы);
		ИмяФормы = "Обработка.ИнтеграцияС1СДокументооборот.Форма.ВыборОдногоТипаИзСоставногоТипа";
...
Процедура ОткрытьОбъект(ТипОбъекта, ID, ВладелецФормы = Неопределено, 
	...
	Иначе
		Если ЗначениеЗаполнено(ТипОбъекта) Тогда
			ПоказатьПредупреждение(, НСтр("es='La presentación de los objetos de tal tipo no es apoyada en esta versión 1С:Документооборота.';ru='Просмотр объектов такого типа не поддерживается в этой версии 1С:Документооборота.';en='Viewing objects of this type is not supported in this version of 1C: Document management.'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыбратьЗначениеПоТипуОбъекта(ТипОбъектаВыбора, Отбор = Неопределено, ВыбранноеЗначение = "", 
	...
	СоответствиеЗаголовков.Вставить("DMFile", 			НСтр("es='La elección del fichero';ru='Выбор файла';en='File selection'")); 
	СоответствиеЗаголовков.Вставить("DMProject", 		НСтр("es='La elección del proyecto';ru='Выбор проекта';en='Project selection'"));
	СоответствиеЗаголовков.Вставить("DMWorkType", 		НСтр("es='La elección del tipo de los trabajos';ru='Выбор вида работ';en='The choice of the type of work'")); 
	СоответствиеЗаголовков.Вставить("DMProjectTask", 	НСтр("es='La elección de la tarea de proyectos';ru='Выбор проектной задачи';en='Design task selection'"));
	СоответствиеЗаголовков.Вставить("DMFileFolder", 	НСтр("es='La elección de la carpeta de los ficheros';ru='Выбор папки файлов';en='File folder selection'"));
	СоответствиеЗаголовков.Вставить("DMOrganization", 	НСтр("es='La elección de la organización';ru='Выбор организации';en='Organization Choice'"));
	СоответствиеЗаголовков.Вставить("DMUser", 			НСтр("es='La elección del usuario';ru='Выбор пользователя';en='User Choice'"));
	СоответствиеЗаголовков.Вставить("DMActivityMatter", НСтр("es='La elección de la pregunta de la actividad';ru='Выбор вопроса деятельности';en='Choice of a question of activity'"));
	СоответствиеЗаголовков.Вставить("DMCurrency", 		НСтр("es='La elección de la divisa';ru='Выбор валюты';en='Currency selection'"));
	
	Если ИнтеграцияС1СДокументооборотКлиентПовтИсп.ИспользоватьТерминКорреспонденты() Тогда
		СоответствиеЗаголовков.Вставить("DMCorrespondent", 	НСтр("es='La elección del corresponsal';ru='Выбор корреспондента';en='Correspondent choice'"));
	Иначе
		СоответствиеЗаголовков.Вставить("DMCorrespondent", 	НСтр("es='La elección del contratante';ru='Выбор контрагента';en='Counterparty selection'"));
	КонецЕсли;
	
	СоответствиеЗаголовков.Вставить("DMContactPerson", 	НСтр("es='La elección de la persona de contacto';ru='Выбор контактного лица';en='Contact selection'"));
	СоответствиеЗаголовков.Вставить("DMSubdivision", 	НСтр("es='La elección de la subdivisión';ru='Выбор подразделения';en='Unit selection'"));
	СоответствиеЗаголовков.Вставить("DMAccessLevel", 	НСтр("es='La elección del grifo del acceso';ru='Выбор грифа доступа';en='Access Bar Selection'"));
	СоответствиеЗаголовков.Вставить("DMRelationType", 	НСтр("es='La elección como el enlace';ru='Выбор типа связи';en='Communication type selection'"));
	СоответствиеЗаголовков.Вставить("DMPrivatePerson", 	НСтр("es='La elección de la persona física';ru='Выбор физического лица';en='Individual selection'"));
	СоответствиеЗаголовков.Вставить("DMDocumentStatus", НСтр("es='La elección del estado del documento';ru='Выбор состояния документа';en='Selection of document status'"));
	СоответствиеЗаголовков.Вставить("DMDeliveryMethod", НСтр("es='La elección del modo de la entrega';ru='Выбор способа доставки';en='Choosing a Delivery Method'"));
	СоответствиеЗаголовков.Вставить("DMInternalDocument", НСтр("es='La elección del documento interior';ru='Выбор внутреннего документа';en='Select an internal document'"));
	СоответствиеЗаголовков.Вставить("DMIncomingDocument", НСтр("es='La elección del documento recibido';ru='Выбор входящего документа';en='Select Incoming Document'"));
	СоответствиеЗаголовков.Вставить("DMOutgoingDocument", НСтр("es='La elección del documento saliente';ru='Выбор исходящего документа';en='Outgoing Document Selection'"));
	СоответствиеЗаголовков.Вставить("DMIncomingDocumentType", 	НСтр("es='La elección del tipo del documento recibido';ru='Выбор вида входящего документа';en='Select the type of incoming document'"));
	СоответствиеЗаголовков.Вставить("DMOutgoingDocumentType", 	НСтр("es='La elección del tipo del documento saliente';ru='Выбор вида исходящего документа';en='Selecting the type of outgoing document'"));
	СоответствиеЗаголовков.Вставить("DMInternalDocumentType", 	НСтр("es='La elección del tipo del documento interior';ru='Выбор вида внутреннего документа';en='Choosing the type of internal document'"));
	СоответствиеЗаголовков.Вставить("DMInternalDocumentFolder", НСтр("es='La elección de la carpeta del documento interior';ru='Выбор папки внутреннего документа';en='Select an internal document folder'"));
	СоответствиеЗаголовков.Вставить("DMProlongationProcedure", 	НСтр("es='La elección del modo de la prolongación';ru='Выбор способа продления';en='Choosing an extension method'"));
	СоответствиеЗаголовков.Вставить("DMBusinessProcessOrderTemplate", 		  НСтр("es='La elección del modelo del encargo';ru='Выбор шаблона поручения';en='Choosing an order template'"));
	СоответствиеЗаголовков.Вставить("DMBusinessProcessConsiderationTemplate", НСтр("es='La elección del modelo del examen';ru='Выбор шаблона рассмотрения';en='Review Template Selection'"));
	СоответствиеЗаголовков.Вставить("DMBusinessProcessRegistrationTemplate",  НСтр("es='La elección del modelo del registro';ru='Выбор шаблона регистрации';en='Registration Template Selection'"));
	СоответствиеЗаголовков.Вставить("DMBusinessProcessApprovalTemplate", 	  НСтр("es='La elección del modelo de la coordinación';ru='Выбор шаблона согласования';en='Matching pattern selection'"));
	СоответствиеЗаголовков.Вставить("DMBusinessProcessConfirmationTemplate",  НСтр("es='La elección del modelo de la afirmación';ru='Выбор шаблона утверждения';en='Approval Template Selection'"));
	СоответствиеЗаголовков.Вставить("DMBusinessProcessPerformanceTemplate",   НСтр("es='La elección del modelo de la realización';ru='Выбор шаблона исполнения';en='Choosing a Performance Template'"));
	СоответствиеЗаголовков.Вставить("DMBusinessProcessAcquaintanceTemplate",  НСтр("es='La elección del modelo del conocimiento';ru='Выбор шаблона ознакомления';en='Choosing a familiarization template'"));
	СоответствиеЗаголовков.Вставить("DMCompoundBusinessProcessTemplate", 	  НСтр("es='La elección del modelo del proceso de negocios componente';ru='Выбор шаблона составного бизнес-процесса';en='Selecting a composite business process template'"));
	СоответствиеЗаголовков.Вставить("DMComplexBusinessProcessTemplate", 	  НСтр("es='La elección del modelo del proceso de negocios complejo';ru='Выбор шаблона комплексного бизнес-процесса';en='Choosing an integrated business process template'"));
	СоответствиеЗаголовков.Вставить("DMBusinessProcessImportance", 			  НСтр("es='La elección de la variante de la importancia';ru='Выбор варианта важности';en='Choosing an Importance Option'"));
	
	СоответствиеЗаголовков.Вставить("DMIncomingEMail", НСтр("es='La elección de la carta entrante';ru='Выбор входящего письма';en='Inbox selection'"));
	СоответствиеЗаголовков.Вставить("DMOutgoingEMail", НСтр("es='La elección de la carta saliente';ru='Выбор исходящего письма';en='Outgoing email selection'"));
	СоответствиеЗаголовков.Вставить("DMDailyReport", НСтр("es='La elección del informe diario';ru='Выбор ежедневного отчета';en='Daily Report Selection'"));
...
Процедура НачатьСозданиеСвязанногоОбъектаДО(ОбъектИС, ОписаниеОповещения, ОткрытьФорму = Ложь) Экспорт
	...
	ИначеЕсли Правила.Количество() = 0 Тогда
		...
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("Заголовок", НСтр("es='Crear en 1С:Документообороте';ru='Создать в 1С:Документообороте';en='Create in 1C: Document management'"));
		ПараметрыФормы.Вставить("ЗаголовокКоманды", НСтр("es='Crear';ru='Создать';en='Create'"));
	...
	Иначе // без открытия формы
		...
		Если ТипЗнч(ОбъектДОИлиСообщение) = Тип("Строка") Тогда // сообщение об ошибке
			...
			ПоказатьПредупреждение(ОписаниеОповещения, ОбъектДОИлиСообщение,,
				НСтр("es='No consiguió anotar el objeto 1С:Документооборота';ru='Не удалось записать объект 1С:Документооборота';en='Could not write object 1C: Document management'"));
...
Процедура СоздатьИнтегрированныйОбъектБезПравила(Форма, ТипОбъектаИС, ТипОбъектаДО, ИдентификаторОбъектаДО) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("es='De las reglas convenientes no es encontrado.';ru='Подходящих правил не найдено.';en='No matching rules found.'"));
	...
	Если ПарольСохранен Тогда
		...
		Исключение // файл удален или поврежден
			...
			ИнтеграцияС1СДокументооборотВызовСервера.ЗаписатьПредупреждение(
				НСтр("es='No son encontrados los datos conservados de descuento del usuario 1С:Документооборота';ru='Не найдены сохраненные учетные данные пользователя 1С:Документооборота';en='No saved credentials of 1C: Document management user'"));
		КонецПопытки;
		Если ПарольСохранен
...
Процедура ПоказатьВопросДаНет(ОписаниеОповещенияОЗавершении, ТекстВопроса,
	...
	Если ТекстКнопкиДа = Неопределено Тогда
		Кнопки.Добавить(КодВозвратаДиалога.ОК, НСтр("es='Sí';ru='Да';en='Yes'"));
	Иначе
		Кнопки.Добавить(КодВозвратаДиалога.ОК, ТекстКнопкиДа);
	КонецЕсли;
	Если ТекстКнопкиНет = Неопределено Тогда
		Кнопки.Добавить(КодВозвратаДиалога.Отмена, НСтр("es='No';ru='Нет';en='No'"));
	Иначе
	...
	Иначе
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El significado inadmisible del botón por reticencia: %1';ru='Недопустимое значение кнопки по умолчанию: %1';en='Invalid default button value:%1'"),
			КнопкаПоУмолчанию);
...
Процедура НачатьВыборТипаБизнесПроцесса(ОписаниеОповещенияОЗакрытии) Экспорт
	...
	СписокДоступныхТиповБизнесПроцессов.Добавить(Новый Структура("XDTOClassName, Presentation", "DMBusinessProcessPerformance", 	НСтр("es='La realización';ru='Исполнение';en='Execution'")));
	СписокДоступныхТиповБизнесПроцессов.Добавить(Новый Структура("XDTOClassName, Presentation", "DMBusinessProcessAcquaintance", 	НСтр("es='El conocimiento';ru='Ознакомление';en='Acquaintance'")));
	СписокДоступныхТиповБизнесПроцессов.Добавить(Новый Структура("XDTOClassName, Presentation", "DMBusinessProcessOrder", 			НСтр("es='El encargo';ru='Поручение';en='Commission'")));
	СписокДоступныхТиповБизнесПроцессов.Добавить(Новый Структура("XDTOClassName, Presentation", "DMBusinessProcessConsideration", 	НСтр("es='El examen';ru='Рассмотрение';en='Review'")));
	СписокДоступныхТиповБизнесПроцессов.Добавить(Новый Структура("XDTOClassName, Presentation", "DMBusinessProcessRegistration", 	НСтр("es='El registro';ru='Регистрация';en='registration'")));
	СписокДоступныхТиповБизнесПроцессов.Добавить(Новый Структура("XDTOClassName, Presentation", "DMBusinessProcessApproval", 		НСтр("es='La coordinación';ru='Согласование';en='Approval'")));
	СписокДоступныхТиповБизнесПроцессов.Добавить(Новый Структура("XDTOClassName, Presentation", "DMBusinessProcessConfirmation", 	НСтр("es='La afirmación';ru='Утверждение';en='Approval  '")));
	СписокДоступныхТиповБизнесПроцессов.Добавить(Новый Структура("XDTOClassName, Presentation", "DMBusinessProcessInternalDocumentProcessing", 	НСтр("es='El tratamiento del documento interior';ru='Обработка внутреннего документа';en='Internal document processing'")));
	СписокДоступныхТиповБизнесПроцессов.Добавить(Новый Структура("XDTOClassName, Presentation", "DMBusinessProcessIncomingDocumentProcessing", 	НСтр("es='El tratamiento del documento recibido';ru='Обработка входящего документа';en='Inbound processing'")));
	СписокДоступныхТиповБизнесПроцессов.Добавить(Новый Структура("XDTOClassName, Presentation", "DMBusinessProcessOutgoingDocumentProcessing", 	НСтр("es='El tratamiento del documento saliente';ru='Обработка исходящего документа';en='Outbound processing'")));
	
	ЗаголовокФормы = НСтр("es='El tipo del proceso de negocios';ru='Тип бизнес-процесса';en='Business process type'");
...
Процедура НачатьВыборШаблонаБизнесПроцесса(ОписаниеОповещенияОЗакрытии, Форма) Экспорт
	...
	Иначе
		ТекстПредупреждения = НСтр("es='No es encontrado ni un modelo conveniente del proceso de negocios.';ru='Не найдено ни одного подходящего шаблона бизнес-процесса.';en='No matching business process template found.'");
...
Процедура НачатьСогласование(Параметры) Экспорт
	...
	Если Не ИнтеграцияС1СДокументооборотВызовСервера.ПользователюРазрешенЗапускСогласования(
		...
		Если ТекстПредупреждения = "" Тогда // текст по умолчанию
			ТекстПредупреждения = НСтр("es='Es insuficientemente derecho para el lanzamiento de la coordinación en 1С:Документообороте.';ru='Недостаточно прав для запуска согласования в 1С:Документообороте.';en='Not enough rights to start coordination in 1C: Document management.'");
		КонецЕсли;
...
Процедура НачатьСогласованиеПослеВопроса(Результат, Параметры) Экспорт
	...
	Если ПодходящиеШаблоны.Количество() = 0 Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("es='De los modelos convenientes no es encontrado.';ru='Подходящих шаблонов не найдено.';en='No matching templates found.'"));
	...
	Иначе // есть шаблон(ы)
		...
		Иначе // предложим выбор
			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("Заголовок", НСтр("es='La elección del modelo de la coordinación';ru='Выбор шаблона согласования';en='Matching pattern selection'"));
...
Процедура НачатьСогласованиеПослеВыбораШаблона(Результат, Параметры) Экспорт
	...
	Если ИнтеграцияС1СДокументооборотВызовСервера.ЗапуститьСогласованиеПоШаблону(Шаблон, Параметры.Предмет) Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("es='La coordinación es puesta en marcha.';ru='Согласование запущено.';en='Reconciliation is running.'"));
...
Процедура ПрерватьСогласование(Параметры) Экспорт
	...
	Если Результат = Неопределено Тогда
		ТекстПредупреждения = НСтр("es='En 1С:Документообороте no es puesta en marcha la coordinación.';ru='В 1С:Документообороте не запущено согласование.';en='In 1C: Документообороте is not running approval.'");
		ПоказатьПредупреждение(ОписаниеОповещенияПослеПредупреждения, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
	Параметры.Вставить("Предмет", Результат);
	...
	Если Не ИнтеграцияС1СДокументооборотВызовСервера.ПользователюРазрешеноПрерываниеСогласования(
		...
		Если ТекстПредупреждения = "" Тогда // предупреждение по умолчанию
			ТекстПредупреждения = НСтр("es='Es insuficientemente derecho para la interrupción de la coordinación en 1С:Документообороте.';ru='Недостаточно прав для прерывания согласования в 1С:Документообороте.';en='Not enough rights to interrupt approval in 1C: Document flow.'");
		КонецЕсли;
		ПоказатьПредупреждение(ОписаниеОповещенияПослеПредупреждения, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура ПрерватьСогласованиеПослеВопроса(Результат, Параметры) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("es='La coordinación es interrumpida.';ru='Согласование прервано.';en='Reconciliation aborted.'"));
...
Процедура ВыполнитьСогласование(Параметры) Экспорт
	...
	Если ЗадачиСогласования.Количество() = 0 Тогда
		ТекстВопроса = НСтр("es='En 1С:Документообороте no hay tareas, dirigidas usted, de la coordinación.';ru='В 1С:Документообороте нет адресованных вам задач согласования.';en='In 1C: Документообороте there are no coordination tasks addressed to you.'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("ОК", НСтр("es='ОК';ru='ОК';en='OK'"));
		Кнопки.Добавить("ЛистСогласования", НСтр("es='La hoja de la coordinación';ru='Лист согласования';en='Approval sheet'"));
		ОписаниеОповещенияПослеВопроса = Новый ОписаниеОповещения(
...
Процедура ВыполнитьСогласованиеПослеВводаКомментария(Результат, Параметры) Экспорт
	...
	Если Параметры.Результат = "Согласовано" Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("es='Es arreglado.';ru='Согласовано.';en='Agreed.'"));
	...
	ИначеЕсли Параметры.Результат = "СогласованоСЗамечаниями" Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("es='Es arreglado con las observaciones.';ru='Согласовано с замечаниями.';en='Agreed with comments.'"));
	...
	ИначеЕсли Параметры.Результат = "НеСогласовано" Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("es='No es arreglado.';ru='Не согласовано.';en='Not agreed.'"));
...
Процедура ОстановитьПроцесс(Форма) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("es='El proceso es parado';ru='Процесс остановлен';en='Process stopped'"),,
		Форма.Заголовок,
...
Процедура ПродолжитьПроцесс(Форма) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("es='El proceso es continuado';ru='Процесс продолжен';en='Process continued'"),,
		Форма.Заголовок,
		БиблиотекаКартинок.Информация32);
...
Функция ОткрытьФайлПриложением(ПолноеИмяФайла) Экспорт
	...
	Исключение
		...
		ТекстПредупреждения = СтрШаблон(
			НСтр("es='La descripción = ""el %1""';ru='Описание=""%1""';en='Description = ""%1""'"),
			...
			Если Файл.Существует() Тогда
				...
				Иначе // уверенности нет, требуется вмешательство пользователя
					...
					Если РазницаДат > 0 Тогда // файл на диске свежее
						ПараметрыФормы.Вставить("Заголовок", НСтр("es='Sobre el disco el nuevo fichero';ru='На диске новый файл';en='New file on disk'"));
						ПараметрыФормы.Вставить("Сообщение",
							НСтр("es='El fichero sobre el disco новее, que el fichero en 1С:Документообороте. "
"Probablemente, él era cambiado. "
""
"¿Usar el fichero existente o sustituirlo por el fichero de 1æ:¿ä«¬Ò¼Ñ¡Ô««í«Ó«Ôá, "
"probablemente, con la pérdida de los cambios?';ru='Файл на диске новее, чем файл в 1С:Документообороте."
"Возможно, он был изменен."
""
"Использовать существующий файл или заменить его на файл из 1С:Документооборота,"
"возможно, с потерей изменений?';en='The file on the disk is newer than the file in 1C: Document management."
"Perhaps it has been changed."
""
"Use an existing file or take a file from 1C: Document management, perhaps with a loss of change?'"));
					
					Иначе // файл на диске старше
						ПараметрыФормы.Вставить("Заголовок", НСтр("es='En 1С:Документообороте un nuevo fichero';ru='В 1С:Документообороте новый файл';en='In 1C: Документообороте new file'"));
						ПараметрыФормы.Вставить("Сообщение",
							НСтр("es='El fichero en 1С:Документообороте новее, que el fichero sobre el disco. "
"Probablemente, él era cambiado. "
""
"¿Tomar el fichero de 1æ:¿ä«¬Ò¼Ñ¡Ô««í«Ó«Ôá, habiendo sustituido el fichero sobre el disco con la pérdida "
"de los cambios, o usar el fichero existente?';ru='Файл в 1С:Документообороте новее, чем файл на диске."
"Возможно, он был изменен."
""
"Взять файл из 1С:Документооборота, заменив файл на диске с потерей"
"изменений, или использовать существующий файл?';en='File in 1C: Document management is newer than a file on disk."
"Perhaps it has been changed."
""
"Take file from 1C: Document management, replacing the file on the disk with loss"
"changes, or use an existing file?'"));
		...
		Если ЗначениеЗаполнено(СведенияОРедактировании.РекомендуемаяПапка) Тогда
			...
			Иначе // попытаемся создать эту папку
				...
				Исключение // нет доступа или иные проблемы.
					...
					ИнтеграцияС1СДокументооборотВызовСервера.ЗаписатьПредупреждение(
						СтрШаблон(
							НСтр("es='No hay acceso a la carpeta de trabajo: %1';ru='Нет доступа к рабочей папке: %1';en='No access working folder:%1'"),
		...
		Иначе
			...
			Исключение
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La falta a la creación de catálogo ""%1"": "
"""%2"".';ru='Ошибка при создании каталога ""%1"":"
"""%2"".';en='Error creating directory ""%1"":"
"""%2"".'"),
					КаталогССуффиксом,
	...
	КонецЦикла;
	Если Не Успех Тогда
		ВызватьИсключение НСтр("es='No consigue crear el fichero en el catálogo de los ficheros temporales.';ru='Не удается создать файл в каталоге временных файлов.';en='Cannot create file in temporary files directory.'");
	КонецЕсли;
	...
	Если Результат = "ВзятьИзДокументооборота" Тогда
		...
		Исключение
			ПоказатьПредупреждение(, 
				НСтр("es='No consiguió sustituir el fichero sobre el disco por el fichero de 1С:Документооборота. "
"Probablemente, él es bloqueado por otra aplicación.';ru='Не удалось заменить файл на диске файлом из 1С:Документооборота."
"Возможно, он заблокирован другим приложением.';en='Failed to replace the file on the disk with a file from 1C: Workflow."
"It may be blocked by another application. '"));
	...
	ИначеЕсли Результат = "ИспользоватьСуществующий" Тогда
		...
		Если Параметры.СведенияОРедактировании.НаЧтение И Не Параметры.НаЧтение Тогда
			...
			Исключение
				ПоказатьПредупреждение(, 
					НСтр("es='No consiguió abrir el fichero para la redacción. Probablemente, "
"él es bloqueado por otra aplicación.';ru='Не удалось открыть файл для редактирования. Возможно, "
"он заблокирован другим приложением.';en='Failed to open file for editing. Perhaps"
"it is blocked by another application'"));
				ПолучитьФайлыНачало(Параметры);
			КонецПопытки;
...
Процедура ОткрытьКаталогФайла(Идентификатор) Экспорт
	...
	Если Не РасширениеРаботыСФайламиПодключено() Тогда
		ПоказатьПредупреждение(, НСтр("es='Abrir el catálogo del fichero se puede solamente después de la conexión "
"de la ampliación para el trabajo con los ficheros.';ru='Открывать каталог файла можно только после подключения"
"расширения для работы с файлами.';en='Can open the file directory only after connecting"
"file extensions.'"));
		Возврат;
	КонецЕсли;
	...
	Если СведенияОРедактировании.Сохранен Тогда
		...
		Если Файл.Существует() Тогда
			...
			КонецЕсли;
		Иначе
			ПоказатьПредупреждение(, НСтр("es='El fichero no es encontrado en el catálogo de trabajo.';ru='Файл не найден в рабочем каталоге.';en='File not found in working directory.'"));
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(, НСтр("es='El fichero no es encontrado en el catálogo de trabajo.';ru='Файл не найден в рабочем каталоге.';en='File not found in working directory.'"));
		Возврат;
...
Процедура СделатьВерсиюАктивной(ИдентификаторФайла, ИдентификаторВерсии, 
	...
	Если ИнтеграцияС1СДокументооборотВызовСервера.СделатьВерсиюАктивной(ИдентификаторФайла,
		...
		Если ТекстСообщения = "" Тогда
			ТекстСообщения = НСтр("es='La versión activa es cambiada.';ru='Активная версия изменена.';en='Active version changed.'");
		КонецЕсли;
	...
	Если Не РасширениеРаботыСФайламиПодключено() Тогда
		...
		Исключение
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió conservar los cambios por la causa: "
"%1';ru='Не удалось сохранить изменения по причине:"
"%1';en='Failed to save changes due to:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если Не СведенияОРедактировании.Сохранен 
		...
		ПоказатьВопросДаНет(ОписаниеОповещения, 
			НСтр("es='El fichero no es encontrado en el catálogo de trabajo. ¿Liberarlo?';ru='Файл не найден в рабочем каталоге. Освободить его?';en='File not found in working directory. Free it up?'"));
		Возврат;
	...
	ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Va la conservación del fichero ""%1"" (%2). Por favor, esperen...';ru='Идет сохранение файла ""%1"" (%2). Пожалуйста, подождите...';en='The file ""%1"" (%2) is being saved. Please, wait…'"),
...
Процедура ЗакончитьРедактированиеСРасширениемПослеПомещения(Результат, Адрес, ПолныйПуть, Параметры) Экспорт
	...
	Если ИнтеграцияС1СДокументооборотВызовСервера.
		...
		Если Не Файл.ПолучитьТолькоЧтение() Тогда
			Файл.УстановитьТолькоЧтение(Истина);
		КонецЕсли;
		Состояние(НСтр("es='El fichero es renovado y liberado.';ru='Файл обновлен и освобожден.';en='File updated and freed.'"));
		Если ОписаниеОповещения <> Неопределено Тогда
			ВыполнитьОбработкуОповещения(ОписаниеОповещения, Истина);
		КонецЕсли;
	КонецЕсли;
...
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если Не СведенияОРедактировании.Сохранен Тогда
		ПоказатьПредупреждение(, НСтр("es='No consiguió conservar los cambios del fichero, "
"puesto que él no es capturado ahora a la redacción.';ru='Не удалось сохранить изменения файла,"
"так как он сейчас не захвачен на редактирование.';en='Failed to save file changes,"
"because it is not being captured for editing right now.'"));
		Возврат;
	КонецЕсли;
	Если СведенияОРедактировании.НаЧтение Тогда
		ПоказатьПредупреждение(, НСтр("es='No consiguió conservar los cambios del fichero, "
"puesto que él es conservado ahora solamente para la presentación.';ru='Не удалось сохранить изменения файла,"
"так как он сейчас сохранен только для просмотра.';en='Failed to save file changes,"
"because now it is saved for viewing only. '"));
		Возврат;
	КонецЕсли;
	Параметры.Вставить("СведенияОРедактировании", СведенияОРедактировании);
...
Процедура СохранитьИзмененияСРасширениемПослеПомещения(Результат, Адрес, ПолныйПуть, Параметры) Экспорт
	...
	Если ИнтеграцияС1СДокументооборотВызовСервера.
		ОбновитьФайлДвоичнымиДаннымиВременногоХранилища(Параметры) Тогда
		Состояние(НСтр("es='Los cambios son conservados.';ru='Изменения сохранены.';en='Changes saved.'"));
		Если ОписаниеОповещения <> Неопределено Тогда
			ВыполнитьОбработкуОповещения(ОписаниеОповещения, Истина);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОтменитьРедактированиеФайла(Идентификатор, ОписаниеОповещенияОЗавершении = Неопределено) Экспорт
	...
	ПоказатьВопросДаНет(ОписаниеОповещения,
		НСтр("es='La anulación de la redacción del fichero puede llevar a la pérdida de los cambios. ¿Continuar?';ru='Отмена редактирования файла может привести к потере изменений. Продолжить?';en='Canceling file editing may result in loss of changes. Continue?'"));
...
Процедура СохранитьКакНачало(Параметры)
	...
	Если Не РасширениеРаботыСФайламиПодключено() Тогда
		...
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Va la conservación del fichero ""%1"". Por favor, esperen...';ru='Идет сохранение файла ""%1"". Пожалуйста, подождите...';en='The file ""%1"" is being saved. Please, wait...'"),
			Параметры.ИмяФайла);
		Состояние(ТекстПояснения);
	...
	Если Параметры.ОставшиесяФайлы.Количество() = 0 Тогда
		...
		Диалог.Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Todos los ficheros (*. El %1) | *. El %1';ru='Все файлы (*.%1)|*.%1';en='All files (*.% 1) | *.% 1'"), Параметры.Расширение, Параметры.Расширение);
	Иначе
...
"Probablemente, él era cambiado. "
""
"¿Usar el fichero existente o tomar el fichero de 1æ:¿ä«¬Ò¼Ñ¡Ô««í«Ó«Ôá?';ru='Файл на диске новее, чем файл в 1С:Документообороте."
"Возможно, он был изменен."
""
"Использовать существующий файл или взять файл из 1С:Документооборота?';en='The file on the disk is newer than the file in 1C: Document management."
"Perhaps it has been changed."
""
"Use an existing file or take a file from 1C: Document management?'"));
					ПараметрыФормы.Вставить("ПолныйПуть", СведенияОРедактировании.ПолныйПуть);
					ПараметрыФормы.Вставить("ДатаВремяНаДиске", ДатаВремяНаДиске);
					ПараметрыФормы.Вставить("ДатаВремяВДокументообороте", ДатаВремяВДокументообороте);
					ПараметрыФормы.Вставить("РазмерНаДиске", РазмерНаДиске);
...
Процедура СохранитьКакПослеВыбораИсточника(Результат, Параметры) Экспорт
	...
	ИначеЕсли Результат = "ИспользоватьСуществующий" Тогда
		...
		Исключение
			Инфо = ИнформацияОбОшибке();
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió copiar el fichero (""%1"").';ru='Не удалось скопировать файл (""%1"").';en='Failed to copy file (""%1"").'"),
				КраткоеПредставлениеОшибки(Инфо));
			ПоказатьПредупреждение(, ТекстПредупреждения); 
			Возврат;
		КонецПопытки;
	...
	Состояние(НСтр("es='El fichero es conservado.';ru='Файл сохранен.';en='File saved.'"), , Параметры.ПолныйПуть);
...
Процедура ОбновитьИзФайлаНаДиске(Идентификатор, Имя, Расширение, УникальныйИдентификаторФормы,
	...
	Если Не РасширениеРаботыСФайламиПодключено() Тогда
		...
		Исключение
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió renovar el fichero por la causa: "
"%1';ru='Не удалось обновить файл по причине:"
"%1';en='Failed to update file due to:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			ПоказатьПредупреждение(, ТекстПредупреждения);
		КонецПопытки;
		Возврат;
...
Процедура ОбновитьИзФайлаНаДискеПослеПомещенияФайлаСРасширением(Результат, Адрес, ПолныйПуть, Параметры) Экспорт
	...
	Если ИнтеграцияС1СДокументооборотВызовСервера.
		ОбновитьФайлДвоичнымиДаннымиВременногоХранилища(Параметры) Тогда
		Состояние(НСтр("es='El fichero es renovado.';ru='Файл обновлен.';en='File updated.'"));
		Если ОписаниеОповещения <> Неопределено Тогда
			ВыполнитьОбработкуОповещения(ОписаниеОповещения, Параметры);
		КонецЕсли;
	КонецЕсли;
...
Процедура СоздатьФайлСДискаПослеПодключенияРасширения(Подключено, Параметры) Экспорт
	...
	Если Подключено Тогда
		...
		ВыборФайла.Фильтр = НСтр("es='Todos los ficheros (*. *) |*.*';ru='Все файлы (*.*)|*.*';en='All files (*. *) | *. *'");
	...
	Иначе // веб-клиент без расширения
		...
		Исключение
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió crear el fichero por la causa: "
"%1';ru='Не удалось создать файл по причине:"
"%1';en='Failed to create file due to:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			ПоказатьПредупреждение(, ТекстПредупреждения);
		КонецПопытки;
...
Процедура СоздатьФайлСДискаПослеПолученияРазмера(Размер, Параметры) Экспорт
	...
	Если Размер > МаксРазмерФайла Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				 НСтр("es='La dimensión del fichero ""%1"" (%2) supera máximamente admisible (%3).';ru='Размер файла ""%1"" (%2) превышает максимально допустимый (%3).';en='File size ""%1"" (%2) exceeds the maximum size (%3).'"),
				 Параметры.ПолноеИмяФайла, 
...
Процедура НачатьУдалениеСвязи(ИсходныйДокумент, СвязанныйДокумент, ТипСвязи,
	...
	ПоказатьВопросДаНет(ОповещениеПродолжения,
		НСтр("es='¿Quitar el enlace?';ru='Удалить связь?';en='Delete link?'"));
...
Процедура ИнформироватьОПодписанииОбъекта(ИмяФайла, ИдентификаторФайла) Экспорт
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Es establecida la firma para ""%1""';ru='Установлена подпись для ""%1""';en='Signed for ""%1""'"), ИмяФайла);
	Состояние(ТекстСообщения);
...
Процедура НачатьСохранениеВместеСЭППослеПодключенияРасширения(Подключено, Параметры) Экспорт	
	...
	Иначе
		ПоказатьПредупреждение(, НСтр("es='Para la ejecución de la operación es necesario establecer la ampliación del trabajo con los ficheros.';ru='Для выполнения операции нужно установить расширение работы с файлами.';en='To perform the operation, you need to install the file extension.'"));
	КонецЕсли;
...
Процедура ПроверитьОднуПодпись(ДанныеСтроки, МенеджерКриптографии, УникальныйИдентификатор, 
	...
	Попытка
		...
		ДанныеСтроки.Статус = НСтр("es='Es justa';ru='Верна';en='Verna'");
		ДанныеСтроки.Неверна = Ложь;
	Исключение
		ДанныеСтроки.Статус = НСтр("es='No es justo';ru='Не верна';en='Incorrect'");
		Инфо = ИнформацияОбОшибке();
...
Процедура ИнформироватьОПодписании(Представление) Экспорт
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Es establecida la firma para ""%1""';ru='Установлена подпись для ""%1""';en='Signed for ""%1""'"),
		Представление);
	Состояние(ТекстСообщения);
...
Процедура НачатьФормированиеПодписиОбъекта(Форма, ОписаниеОповещенияОЗакрытии) Экспорт
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция", НСтр("es='La firma del documento';ru='Подписание документа';en='Document signing'"));
	ОписаниеДанных.Вставить("ЗаголовокДанных", НСтр("es='El documento';ru='Документ';en='Document'"));
...
Процедура НачатьДобавлениеЭПИзФайлаЗавершение(ОписаниеДанных, ПараметрыОповещения) Экспорт
	...
	Если МассивДанныхДляЗанесенияВБазу.Количество() = 1 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es añadida la firma del fichero para ""%1""';ru='Добавлена подпись из файла для ""%1""';en='Added signature from file for ""% 1""'"), СвойстваФайла.ИмяФайла);
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Son añadidas las firmas de los ficheros para ""%1""';ru='Добавлены подписи из файлов для ""%1""';en='Added captions from files for ""% 1""'"), СвойстваФайла.ИмяФайла);
	КонецЕсли;
...
Процедура ВыбратьЗначениеРеквизитаСоставногоТипаВТаблицеОбработка(ИмяТипа, Форма, ТекущиеДанные, Реквизит)
	...
	Если ИмяТипа = "integer" 
		...
		ПоказатьВводЗначения(Оповещение, ТекущиеДанные[Реквизит], НСтр("es='Introduzcan el significado';ru='Введите значение';en='Enter value'"), ТипЗначения);
...
Процедура ПереключитьХронометражЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Если ПараметрыХронометража.ВключенХронометраж Тогда
			...
			НСтр("es='Es incluido el cronometraje por ""%1""';ru='Включен хронометраж по ""%1""';en='Timing by ""% 1"" is enabled'"), Строка(ПараметрыОповещения.ОбъектИС));
			Состояние(Текст);
		КонецЕсли;
...
Процедура ПереключитьХронометражПослеПоискаСвязанногоОбъектаДО(Результат, Параметры) Экспорт
	...
	Иначе
		...
		Если ПараметрыХронометража.ВключенХронометраж Тогда
			...
			Если ПараметрыХронометража.ВключенХронометраж Тогда
				Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Es incluido el cronometraje por ""%1""';ru='Включен хронометраж по ""%1""';en='Timing by ""% 1"" is enabled'"), Строка(Параметры.ОбъектИС));
				Состояние(Текст);
			КонецЕсли;
		...
		Иначе
			...
			Иначе
				...
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Es incluido el cronometraje por otros objetos: "
"¿%1 "
"Desconectar el cronometraje y fijar трудозатраты ante la inclusión del nuevo cronometraje?';ru='Включен хронометраж по другим объектам:"
"%1"
"Отключить хронометраж и зафиксировать трудозатраты перед включением нового хронометража?';en='Timing on other objects is included:"
"%1"
"Disable timing and record labor before starting a new timing?'"),
						СтрокаОбъектовХронометража);
				Кнопки = новый СписокЗначений;
				Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("es='Sí';ru='Да';en='Yes'"));
...
Процедура ПрерватьПроцессПослеВопроса(Результат, Параметры) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("es='El proceso es interrumpido';ru='Процесс прерван';en='Process aborted'"),,
