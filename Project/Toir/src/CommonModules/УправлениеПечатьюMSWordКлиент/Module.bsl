...
#КонецЕсли
	...
	Исключение
		...
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка",
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),,Истина);
		ВызватьИсключение(НСтр("es='La falta a la apertura del fichero del modelo.';ru='Ошибка при открытии файла шаблона.';en='Error opening template file.'") + Символы.ПС 
...
Функция СобытиеЖурналаРегистрации()
	Возврат НСтр("es='La prensa';ru='Печать';en='Printing'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
КонецФункции
...
#Иначе		
	ТекстУточнения = НСтр("es='Para la conclusión de las formas de imprenta en el formato Microsoft Word es necesario que en el ordenador sea establecido el paquete Microsoft Office.';ru='Для вывода печатных форм в формате Microsoft Word требуется, чтобы на компьютере был установлен пакет Microsoft Office.';en='Microsoft Word requires that Microsoft Office software is installed on the computer to display printed forms in Microsoft Word format.'");
#КонецЕсли
