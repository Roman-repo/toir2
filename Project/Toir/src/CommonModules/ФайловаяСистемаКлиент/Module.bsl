...
Процедура ЗагрузитьФайл(
	...
	ИначеЕсли Не ПараметрыЗагрузки.Интерактивно
		...
		ВызватьИсключение НСтр("es='el régimen no interactivo.';ru='Не указано имя файла для загрузки в неинтерактивном режиме.';en='File name specified for silent download is not specified.'");
	КонецЕсли;
...
Процедура ЗагрузитьФайлы(
	...
	Если Не ПараметрыЗагрузки.Интерактивно
		...
		ВызватьИсключение НСтр("es='el régimen no interactivo.';ru='Не указаны файлы для загрузки в неинтерактивном режиме.';en='Files for downloading in non-interactive mode are not specified.'");
...
// Пример:
//  ПараметрыЗагрузки = ФайловаяСистемаКлиент.ПараметрыЗагрузкиФайла();
//  ПараметрыЗагрузки.Диалог.Заголовок = НСтр("es='Escojan el documento';ru='Выберите документ';en='Select a document'");
//  ПараметрыЗагрузки.Диалог.Фильтр = НСтр("es='Los ficheros MS Word (*.doc; *.docx) | *.doc; *.docx|Все los ficheros (*. *) |*.*';ru='Файлы MS Word (*.doc;*.docx)|*.doc;*.docx|Все файлы(*.*)|*.*';en='MS Word files (* .doc; *. Docx) | * .doc; *. Docx | All files (*. *) | *. *'");
//  ФайловаяСистемаКлиент.ЗагрузитьФайл(Оповещение, ПараметрыЗагрузки);
//
...
// Пример:
//  ПараметрыСохранения = ФайловаяСистемаКлиент.ПараметрыСохраненияФайла();
//  ПараметрыСохранения.Диалог.Заголовок = НСтр("es='Conservar el perfil de las operaciones claves en el fichero';ru='Сохранить профиль ключевых операций в файл';en='Save key operations profile to file'");
...
// Пример:
//  ПараметрыСохранения = ФайловаяСистемаКлиент.ПараметрыСохраненияФайлов();
//  ПараметрыСохранения.Диалог.Заголовок = НСтр("es='La elección de la carpeta para la conservación del documento formado';ru='Выбор папки для сохранения сформированного документа';en='Select a folder to save the generated document'");
//  ФайловаяСистемаКлиент.СохранитьФайлы(Оповещение, ПолучаемыеФайлы, ПараметрыСохранения);
//
...
Процедура ОткрытьФайл(
	...
	Если ЭтоАдресВременногоХранилища(РасположениеФайла) Тогда
		...
		Если ПустаяСтрока(ИмяФайла) Тогда
			ВызватьИсключение НСтр("es='del fichero.';ru='Не указано имя файла.';en='File name is not specified.'");
		КонецЕсли;
...
Процедура ОткрытьПроводник(ПутьККаталогуИлиФайлу) Экспорт
	...
	ТекстПредложения = НСтр("es='Para la apertura de la carpeta es necesario establecer la ampliación del trabajo con los ficheros.';ru='Для открытия папки необходимо установить расширение работы с файлами.';en='To open a folder, you must install the file extension.'");
...
Процедура ОткрытьНавигационнуюСсылку(НавигационнаяСсылка, Знач Оповещение = Неопределено) Экспорт
	...
	ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='No consiguió pasar por la referencia ""%1"" por la causa: "
"es incorrectamente dada la referencia de navegación.';ru='Не удалось перейти по ссылке ""%1"" по причине: "
"Неверно задана навигационная ссылка.';en='Failed to follow the link ""%1"" due to:"
...
#КонецЕсли
	...
	Иначе 
		...
		ТекстПредложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Para la apertura de la referencia ""%1"" es necesario establecer la ampliación del trabajo con los ficheros.';ru='Для открытия ссылки ""%1"" необходимо установить расширение работы с файлами.';en='To open the link ""% 1"", you must install the file extension.'"),
			НавигационнаяСсылка);
		ПодключитьРасширениеДляРаботыСФайлами(Оповещение, ТекстПредложения, Ложь);
...
#КонецЕсли
	...
	ТекстПредложения = 
		НСтр("es='Para la creación del catálogo temporal es necesario establecer la ampliación del trabajo con los ficheros.';ru='Для создания временного каталога необходимо установить расширение работы с файлами.';en='To create a temporary directory, you must install the file extension.'");
	ПодключитьРасширениеДляРаботыСФайлами(Оповещение, ТекстПредложения, Ложь);
...
Процедура СоздатьВременныйКаталог(Знач Оповещение, Расширение = "") Экспорт 
	...
	ТекстПредложения = НСтр("es='Para la creación del catálogo temporal es necesario establecer la ampliación del trabajo con los ficheros.';ru='Для создания временного каталога необходимо установить расширение работы с файлами.';en='To create a temporary directory, you must install the file extension.'");
	ПодключитьРасширениеДляРаботыСФайлами(Оповещение, ТекстПредложения, Ложь);
...
//  ТекстСообщения = НСтр("es='Para la prensa del documento es necesario establecer la ampliación del trabajo con los ficheros.';ru='Для печати документа необходимо установить расширение работы с файлами.';en='To print a document, you must install the file extension.'");
