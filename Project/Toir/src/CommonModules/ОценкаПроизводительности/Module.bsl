...
Процедура ЗафиксироватьДлительностьКлючевойОперации(Параметры)
	...
	Если БезопасныйРежим() <> Ложь Тогда
		ЗаписьЖурналаРегистрации(НСтр("es='El trabajo del subsistema la apreciación de la productividad en el régimen seguro no es apoyado';ru='Работа подсистемы оценка производительности в безопасном режиме не поддерживается';en='Subsystem performance assessment in safe mode is not supported'", КодОсновногоЯзыка),
	...
	Если Не ЗначениеЗаполнено(ДатаНачалаКлючевойОперации) Тогда
		ЗаписьЖурналаРегистрации(НСтр("es='Es imposible fijar la medición con la fecha vacía del comienzo';ru='Невозможно зафиксировать замер с пустой датой начала';en='Unable to commit metering with empty start date'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Информация,,,Строка(КлючеваяОперация));
...
Процедура ЗаписатьЗамерыВремени(МассивЗамеров)
	...
	Если НаборЗаписей.Количество() > 0 Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("es='No consiguió conservar las mediciones de la productividad';ru='Не удалось сохранить замеры производительности';en='Failed to save performance measurements'", КодОсновногоЯзыка),
...
Процедура ЗагрузитьФайлОценкиПроизводительности(ИмяФайла, АдресХранилища) Экспорт
	...
	Исключение
		...
		ШаблонСтроки = НСтр("es='No consiguió распаковать el archivo %1 %2';ru='Не удалось распаковать архив %1 %2';en='Failed to unzip archive%1%2'");
