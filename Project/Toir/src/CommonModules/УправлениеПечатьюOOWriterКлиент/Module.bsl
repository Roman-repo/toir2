...
#Если Не МобильныйКлиент Тогда
	...
	Исключение
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка",
			НСтр("es='La falta al enlace con el servicio por el manager (com.sun.star. ServiceManager).';ru='Ошибка при связи с сервис менеджером (com.sun.star.ServiceManager).';en='Error communicating with service manager (com.sun.star.ServiceManager).'")
	...
	Исключение
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка",
			НСтр("es='La falta durante el lanzamiento del servicio Desktop (com.sun.star.frame. Desktop).';ru='Ошибка при запуске сервиса Desktop (com.sun.star.frame.Desktop).';en='Error while starting the Desktop service (com.sun.star.frame.Desktop).'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),,Истина);
		НеУдалосьСформироватьПечатнуюФорму(ИнформацияОбОшибке());
	КонецПопытки;
...
Функция ПолучитьПозициюНачалаОбласти(Знач xDocument, Знач ИмяОбласти)
	...
	Если xFound = Неопределено Тогда
		ВызватьИсключение НСтр("es='No es encontrado el comienzo de la esfera de la maqueta:';ru='Не найдено начало области макета:';en='The beginning of the layout area was not found:'") + " " + ИмяОбласти;	
	КонецЕсли;
	Возврат xFound.End;
...
Функция ПолучитьПозициюОкончанияОбласти(Знач xDocument, Знач ИмяОбласти)
	...
	xFound = xDocument.findFirst(xSearchDescr);
	Если xFound = Неопределено Тогда
		ВызватьИсключение НСтр("es='de la maqueta:';ru='Не найден конец области макета:';en='Not found end of layout area:'") + " " + ИмяОбласти;	
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрации()
	Возврат НСтр("es='La prensa';ru='Печать';en='Printing'");
КонецФункции

Процедура НеУдалосьСформироватьПечатнуюФорму(ИнформацияОбОшибке)
#Если ВебКлиент Или МобильныйКлиент Тогда
	ТекстУточнения = НСтр("es='Para la formación de esta forma de imprenta es necesario aprovecharse al cliente delgado.';ru='Для формирования этой печатной формы необходимо воспользоваться тонким клиентом.';en='To create this plate, you must use a thin client.'");
#Иначе
	ТекстУточнения = НСтр("es='Para la conclusión de las formas de imprenta en el formato OpenOffice.org Writer es necesario que en el ordenador sea establecido el paquete OpenOffice.org.';ru='Для вывода печатных форм в формате OpenOffice.org Writer требуется, чтобы на компьютере был установлен пакет OpenOffice.org.';en='Printing output in OpenOffice.org Writer format requires that the OpenOffice.org package is installed on the computer.'");
#КонецЕсли
	ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='No consiguió formar la forma de imprenta: %1. "
"%2';ru='Не удалось сформировать печатную форму: %1. "
"%2';en='Failed to generate plate:%1."
"%2'"),
