...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Взаимодействия") Тогда
		...
		Обработчик.Комментарий = НСтр("es='Renueva las noticias sobre las anotaciones personales de descuento del correo electrónico. "
"Antes de la terminación del tratamiento la lista de las anotaciones personales de descuento del correo electrónico puede ser incompleta.';ru='Обновляет сведения о персональных учетных записях электронной почты."
"До завершения обработки список персональных учетных записей электронной почты может быть неполным.';en='Updates personal email account information."
"Prior to processing, the list of personal email accounts may not be complete. '");
...
Процедура ПриЗаполненииВидовДоступа(ВидыДоступа) Экспорт
	...
	ВидДоступа.Представление = НСтр("es='Las anotaciones de descuento del correo electrónico';ru='Учетные записи электронной почты';en='Email accounts'");
	...
	Если СоздаватьСоединение Тогда
		...
		Исключение
			Если ПротоколПолученияПочты = ПротоколИнтернетПочты.IMAP И Не РеквизитыОтправителя.ИспользоватьДляПолучения Тогда
				ТекстОшибки = НСтр("es='No consiguió conectarse al servidor IMAP. Comprueben los ajustes de la anotación de descuento.';ru='Не удалось подключиться к серверу IMAP. Проверьте настройки учетной записи.';en='Failed to connect to the IMAP server. Check your account settings.'", ОбщегоНазначения.КодОсновногоЯзыка());
				ЗаписьЖурналаРегистрации(НСтр("es='El trabajo con los mensajes postales. El envío de los correos';ru='Работа с почтовыми сообщениями.Отправка почты';en='Work with mail messages. Sending mail'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПроверитьВозможностьОтправкиИПолученияЭлектроннойПочты(УчетнаяЗапись, СообщениеОбОшибке, ДополнительноеСообщение) Экспорт
	...
	Если НастройкиУчетнойЗаписи.ИспользоватьДляОтправки Тогда
		...
		Исключение
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No consiguió conectarse al servidor SMTP: %1';ru='Не удалось подключиться к серверу SMTP: %1';en='Failed to connect to SMTP server:%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
		Если Не НастройкиУчетнойЗаписи.ИспользоватьДляПолучения Тогда
			ДополнительноеСообщение = Символы.ПС + НСтр("es='(Es cumplida la comprobación del envío de los mensajes electrónicos.)';ru='(Выполнена проверка отправки электронных сообщений.)';en='(Sending emails checking is completed.)'");
		КонецЕсли;
	КонецЕсли;
	...
	Если НастройкиУчетнойЗаписи.ИспользоватьДляПолучения 
		...
		Исключение
			...
			СообщениеОбОшибке = СообщениеОбОшибке + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No consiguió conectarse al servidor %1: %2';ru='Не удалось подключиться к серверу %1: %2';en='Failed to connect to server%1:%2'"),
		...
		Если Не НастройкиУчетнойЗаписи.ИспользоватьДляОтправки Тогда
			ДополнительноеСообщение = Символы.ПС + НСтр("es='(Es cumplida la comprobación de la recepción de los mensajes electrónicos.)';ru='(Выполнена проверка получения электронных сообщений.)';en='(Receiving emails checking is completed.)'");
		КонецЕсли;
...
Процедура ПодготовитьВложения(Вложения, НастройкиСохранения) Экспорт
	...
	Для Индекс = -Вложения.ВГраница() По 0 Цикл
		...
		Для Каждого ВыбранныйФормат Из ВыбранныеФорматыСохранения Цикл
			...
			Иначе
				ИмяФайлаДляАрхива = НСтр("es='Los documentos';ru='Документы';en='Documents'") + ".zip";
			КонецЕсли;
...
Функция ПредставлениеПисьма(ТемаПисьма, ДатаПисьма)
	...
	ШаблонПредставления = НСтр("es='%1 de %2';ru='%1 от %2';en='% 1 from% 2'");
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПредставления,
		?(ПустаяСтрока(ТемаПисьма), НСтр("es='<Sin tema>';ru='<Без темы>';en='<Без темы>'"), ТемаПисьма),
