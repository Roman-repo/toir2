...
Функция ОтправитьПочтовоеСообщение(Знач УчетнаяЗапись, Знач ПараметрыОтправки,
	...
	Если ТипЗнч(УчетнаяЗапись) <> Тип("СправочникСсылка.УчетныеЗаписиЭлектроннойПочты")
		...
		ВызватьИсключение НСтр("es='La anotación de descuento no es rellenada o rellenada incorrectamente.';ru='Учетная запись не заполнена или заполнена неправильно.';en='The account is empty or incorrect.'");
	КонецЕсли;
	
	Если ПараметрыОтправки = Неопределено Тогда
		ВызватьИсключение НСтр("es='del envío.';ru='Не заданы параметры отправки.';en='No send options specified.'");
	КонецЕсли;
	...
	Если ТипЗнчКому = Неопределено И ТипЗнчКопии = Неопределено И СкрытыеКопии = Неопределено Тогда
		ВызватьИсключение НСтр("es='No es indicado ni un destinatario.';ru='Не указано ни одного получателя.';en='No recipients specified.'");
	КонецЕсли;
	...
	Если ПараметрыОтправки.ОшибочныеПолучатели.Количество() > 0 Тогда
		ТекстОшибки = НСтр("es='Las direcciones siguientes postales no eran aceptadas por el servidor postal:';ru='Следующие почтовые адреса не были приняты почтовым сервером:';en='The following mail addresses were not accepted by the mail server:'");
...
Функция ЗагрузитьПочтовыеСообщения(Знач УчетнаяЗапись, Знач ПараметрыЗагрузки = Неопределено) Экспорт
	...
	Если НЕ ИспользоватьДляПолучения Тогда
		ВызватьИсключение НСтр("es='La anotación de descuento no es destinada a la recepción de los mensajes.';ru='Учетная запись не предназначена для получения сообщений.';en='The account is not intended to receive messages.'");
	КонецЕсли;
