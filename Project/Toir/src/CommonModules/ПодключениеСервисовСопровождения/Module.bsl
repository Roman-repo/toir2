...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Важное         = Ложь;
	Дело.Представление  = НСтр("es='La conexión del período de test';ru='Подключение тестового периода';en='Test period connection'");
	...
	Дело.Представление  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La conexión del período de test (%1)';ru='Подключение тестового периода (%1)';en='Test period connection (%1)'"),
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.Подключен Тогда
			...
			Представление    = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es conectado %1';ru='Подключен %1';en='Connected%1'"),
		...
		Иначе
			...
			Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió conectar %1';ru='Не удалось подключить %1';en='Failed to connect %1'"),
				ВыборкаДетальныеЗаписи.НаименованиеТестовогоПериода);
		КонецЕсли;
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		443,
		НСтр("es='El servicio de la conexión de los períodos de test (ru)';ru='Сервис подключения тестовых периодов (ru)';en='Test Period Connection Service (ru)'"));
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		443,
		НСтр("es='El servicio de la conexión de los períodos de test (eu)';ru='Сервис подключения тестовых периодов (eu)';en='Test period connection service (eu)'"));
	НовыеРазрешения.Добавить(Разрешение);
...
Процедура ОбновитьДанныеСервисовСопровождения() Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("es='La renovación de los identificadores dados de los servicios del acompañamiento. El comienzo de la renovación.';ru='Обновление данных идентификаторов сервисов сопровождения. Начало обновления.';en='Updating the data of service tracking identifiers. Start of update.'"),
		Ложь);
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("es='La renovación de los identificadores dados de los servicios del acompañamiento. Es con éxito acabado.';ru='Обновление данных идентификаторов сервисов сопровождения. Успешно завершено.';en='Updating the data of service tracking identifiers. Successfully completed.'"),
		Ложь);
...
Процедура СоздатьРегламентноеЗаданиеПроверкиПодключения(
	...
	Если Не РазрешеноПодключениеТестовыхПериодов() Тогда
		ВызватьИсключение НСтр("es='La conexión del período de test es inaccesible.';ru='Подключение тестового периода недоступно.';en='Test period connection is not available.'");
	КонецЕсли;
	...
	Попытка
		...
		ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es creada la tarea de reglamento para la comprobación de la interpelación por la conexión: %1';ru='Создано регламентное задание для проверки запроса на подключение: %1';en='Scheduled task was created to check connection request:%1'"),
			ИдентификаторЗапроса);
	...
	Исключение
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la creación de la tarea de reglamento de la comprobación de la conexión de los períodos de test: "
"%1';ru='Ошибка при создании регламентного задания проверки подключения тестовых периодов:"
"%1';en='Error while creating a scheduled task for testing periods connection:"
"%1'"),
...
Процедура УдалитьРегламентноеЗаданиеПроверкиПодключения(ИдентификаторЗапроса)
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Es quitada la tarea de reglamento de la comprobación de la interpelación de la conexión: %1';ru='Удалено регламентное задание проверки запроса на подключение: %1';en='Deleted scheduled task for checking connection request:%1'"),
		ИдентификаторЗапроса);
...
Функция ОбновитьКэшТестовыхПериодов(Знач Идентификаторы)
	...
	Если ЗначениеЗаполнено(НеКорректныеИдентификаторы) Тогда
		...
		РезультатОбновления.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Las descripciones de los servicios: %1, no son añadido.';ru='Описания сервисов: %1, не добавлены.';en='Service descriptions:%1, not added.'"),
			НеКорректныеИдентификаторы);
	...
	Если Не РезультатОперации.СодержаниеОтвета.ПодключениеДоступно Тогда
		РезультатОбновления.Ошибка             = Истина;
		РезультатОбновления.ИнформацияОбОшибке = НСтр("es='La conexión de los períodos de test es temporalmente inaccesible, "
"se cumple el tratamiento de la interpelación. Repitan la tentativa más tarde.';ru='Подключение тестовых периодов временно недоступно,"
"выполняется обработка запроса. Повторите попытку позже.';en='Connecting test periods is temporarily unavailable,"
"request processing is in progress. Please try again later. '");
		Возврат РезультатОбновления;
	КонецЕсли;
	...
	Если РезультатОбновления.Ошибка Тогда
		РезультатОбновления.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Son descubiertos los identificadores incorrectos de los servicios: "
"%1';ru='Обнаружены некорректные идентификаторы сервисов:"
"%1';en='Incorrect service identifiers found:"
"%1'"),
			НеКорректныеИдентификаторы);
		Возврат РезультатОбновления;
	КонецЕсли;
...
Процедура ИзменитьИнформациюОЗапросеПодключения(
	...
	Если ИдентификаторыЗапросовНабор.Количество() = 0 Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='la conexión. "
"El servicio del acompañamiento: %1 "
"Identificador: %2';ru='Не обнаружен идентификатор запроса на подключение."
"Сервис сопровождения: %1"
"Идентификатор: %2';en='Connection request id not found."
"Escort service:%1"
"ID:%2 '"),
			СервисСопровождения,
			ИдентификаторЗапроса);
...
Функция ПроверитьДоступностьСервиса(РезультатОперации, ПараметрыПодключения)
	...
	Если Не ПустаяСтрока(РезультатПроверки.ИмяОшибки) Тогда
		...
		ПодробнаяИнформацияОбОшибке          = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió comprobar la accesibilidad del servicio de los períodos de test: "
"%1. "
"La causa: "
"%2';ru='Не удалось проверить доступность сервиса тестовых периодов:"
"%1."
"Причина:"
"%2';en='Failed to check the availability of the trial period service:"
"%1."
"Cause:"
"%2'"),
			URLОперацииPing,
...
Функция ИнформацияОДоступныхТестовыхПериодах(ИдентификаторыСервисов)
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Ha comenzado la renovación del cache para los servicios: %1';ru='Начало обновление кэша для сервисов: %1';en='Started cache update for services:%1'"),
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		РезультатОперации.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la recepción de los períodos accesibles de test: "
"%1';ru='Ошибка при получении доступных тестовых периодов:"
"%1';en='Error getting available test periods:"
"%1'"),
			РезультатОтправки.СообщениеОбОшибке);
			
		ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir los períodos accesibles de test. "
"A la recepción de la información sobre los períodos accesibles de test el servicio ha devuelto la falta: %1. "
"La causa: "
"%2';ru='Не удалось получить доступные тестовые периоды."
"При получении информации о доступных тестовых периодах сервис вернул ошибку: %1."
"Причина:"
"%2';en='Failed to get available test periods."
"When receiving information about available test periods, the service returned an error:%1."
"Cause:"
"%2 '"),
			URLОперации,
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Es acabada la renovación del cache para los servicios: %1';ru='Завершено обновление кэша для сервисов: %1';en='Completed cache update for services: %1'"),
		СтрСоединить(ИдентификаторыСервисов, ","));
	ЗаписатьИнформациюВЖурналРегистрации(
		ТекстЖурналРегистрации,
		Ложь);
...
Функция ПрочитатьДанные_availableList(ТелоJSON)
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Es recibida la respuesta del servicio de la Conexión de los períodos de test: "
"%1';ru='Получен ответ сервиса Подключения тестовых периодов:"
"%1';en='Received the response of the service Connecting test periods:"
"%1'"),
		ТелоJSON);
...
Процедура ПодключитьТестовыйПериод(ПараметрыЗапроса, АдресХранилища) Экспорт
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La conexión del período de test: "
"el Servicio del acompañamiento: %1 "
"Identificador del período de test: %2';ru='Подключение тестового периода:"
"Сервис сопровождения: %1"
"Идентификатор тестового периода: %2';en='Test period connection:"
"Escort service:%1"
"Test period identifier:%2'"),
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		РезультатОперации.Ошибка             = Истина;
		РезультатОперации.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la conexión del período de test: "
"%1';ru='Ошибка при подключении тестового периода:"
"%1';en='Error connecting test period:"
"%1'"),
			РезультатОтправки.СообщениеОбОшибке);
			
		ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió conectar el período de test. "
"A la activación del período de test el servicio ha devuelto la falta: %1. "
"La causa: "
"%2';ru='Не удалось подключить тестовый период."
"При активации тестового периода сервис вернул ошибку: %1."
"Причина:"
"%2';en='Failed to connect the test period."
"When activating the test period, the service returned an error: %1."
"Cause:"
"%2 '"),
			URLОперации,
			РезультатОтправки.ИнформацияОбОшибке);
...
Функция СостояниеЗапросаНаПодключение(
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La comprobación del estado de la interpelación por la conexión del período de test: "
"el Identificador de la interpelación: %1 "
"Identificador del período de test: %2';ru='Проверка состояния запроса на подключение тестового периода:"
"Идентификатор запроса: %1"
"Идентификатор тестового периода: %2';en='Checking the status of the request for connecting a test period:"
"Request ID:%1"
"Trial ID:%2'"),
		ИдентификаторЗапроса,
		ИдентификаторТестовогоПериода);
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		РезультатОперации.Ошибка             = Истина;
		РезультатОперации.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta durante la comprobación de la interpelación por la conexión: "
"%1';ru='Ошибка при проверке запроса на подключение:"
"%1';en='Error checking connection request:"
"%1'"),
			РезультатОтправки.СообщениеОбОшибке);
		
		ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió comprobar el estado de la interpelación por la conexión. "
"Durante la comprobación de la interpelación por la conexión de los períodos de test el servicio ha devuelto la falta: %1. "
"La causa: "
"%2';ru='Не удалось проверить состояние запроса на подключение."
"При проверке запроса на подключение тестовых периодов сервис вернул ошибку: %1."
"Причина:"
"%2';en='Failed to check the status of the connection request."
"When checking the request for connecting test periods, the service returned an error:%1."
"Cause:"
"%2'"),
			URLОперации,
			РезультатОтправки.ИнформацияОбОшибке);
...
Процедура ОбработатьРезультатПроверкиПодключения(
	...
	ПодробноеПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La falta durante la comprobación del estatus de la interpelación por la conexión del período de test: "
"el Código de la falta: %1 "
"Información sobre la falta: %2';ru='Ошибка при проверке статуса запроса на подключение тестового периода:"
"Код ошибки: %1"
"Информация об ошибке: %2';en='Error while checking the status of the request for connecting a test period:"
"Error code:%1"
"Error information:%2 '"),
	...
	Если РезультатПодключения.КодОшибки = "HAS_ACTIVE_OPTION_PACKAGES_BY_APPLICATION"
		...
		Иначе
			...
			РезультатПодключения.ИнформацияОбОшибке = НСтр("es='No consiguió conectar el período de test. "
"Más detalladamente cm. En la revista del registro.';ru='Не удалось подключить тестовый период."
"Подробнее см. в журнале регистрации.';en='Failed to connect the test period."
"See the logbook for details.'");
		КонецЕсли;
		
	ИначеЕсли РезультатПодключения.КодОшибки = "UNKNOWN_CAUSE" Тогда
		РезультатПодключения.ИнформацияОбОшибке = НСтр("es='La falta desconocida al tratamiento de la interpelación. "
"Más detalladamente cm. En la revista del registro.';ru='Неизвестная ошибка при обработке запроса."
"Подробнее см. в журнале регистрации.';en='Unknown error processing request."
"See the logbook for details.'");
	Иначе
		РезультатПодключения.ИнформацияОбОшибке = НСтр("es='La falta interior al tratamiento de la interpelación. "
"Más detalladamente cm. En la revista del registro.';ru='Внутренняя ошибка при обработке запроса."
"Подробнее см. в журнале регистрации.';en='Internal error processing request."
...
Функция ТестовыйПериодПодключен(ИдентификаторТестовогоПериода, ПараметрыПодключения)
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La comprobación de la actividad del período de test: "
"%1';ru='Проверка активности тестового периода:"
"%1';en='Test period activity check:"
"%1'"),
		ИдентификаторТестовогоПериода);
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió comprobar la actividad del período de test. "
"Durante la comprobación de la actividad del período de test el servicio ha devuelto la falta: %1. "
"La causa: "
"%2';ru='Не удалось проверить активность тестового периода."
"При проверке активности тестового периода сервис вернул ошибку: %1."
"Причина:"
"%2';en='Failed to check the activity of the test period."
"When checking the activity of the test period, the service returned an error:%1."
"Cause:"
"%2 '"),
			URLОперации,
			РезультатОтправки.ИнформацияОбОшибке);
...
"See the log for details. '");
			ПодробнаяИнформацияОбОшибке =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No consiguió llamar la operación %1. "
"No consiguió cumplir аутентификацию. "
"%2';ru='Не удалось вызвать операцию %1."
"Не удалось выполнить аутентификацию."
"%2';en='Failed to call operation%1."
...
"%2 '"),
	...
	Иначе
		...
		Если Результат.ДанныеАутентификации = Неопределено Тогда
			...
			Результат.ИнформацияОбОшибке =
				НСтр("es='Para el uso de los períodos de test, es necesario conectar el Apoyo De Internet de los usuarios.';ru='Для использования тестовых периодов, необходимо подключить Интернет-поддержку пользователей.';en='To use the test periods, you need to connect online user support.'");
			ЗаписатьИнформациюВЖурналРегистрации(
				Результат.ИнформацияОбОшибке,
				Истина);
		КонецЕсли;
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("es='La conexión de los períodos de test.';ru='Подключение тестовых периодов.';en='Connecting test periods.'", ОбщегоНазначения.КодОсновногоЯзыка());
