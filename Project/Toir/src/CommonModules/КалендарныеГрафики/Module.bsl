...
Функция ДатыПоКалендарю(Знач ГрафикРаботы, Знач ДатаОт, Знач МассивДней, Знач РассчитыватьСледующуюДатуОтПредыдущей = Ложь, ВызыватьИсключение = Истина) Экспорт
	...
	Если Не ЗначениеЗаполнено(ГрафикРаботы) Тогда
		Если ВызыватьИсключение Тогда
			ВызватьИсключение НСтр("es='del trabajo o el calendario de producción.';ru='Не указан график работы или производственный календарь.';en='Not specified work schedule or production calendar.'");
		КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ДатаПоКалендарю = Неопределено Тогда
			СообщениеОбОшибке = НСтр("es='El calendario de producción «%1» no es rellenado de la fecha de %2 a la cantidad indicada de los días de trabajo.';ru='Производственный календарь «%1» не заполнен с даты %2 на указанное количество рабочих дней.';en='Production calendar “%1” has not been completed from the date%2 for the specified number of working days.'");
			Если ВызыватьИсключение Тогда
...
Функция РазностьДатПоКалендарю(Знач ГрафикРаботы, Знач ДатаНачала, Знач ДатаОкончания, ВызыватьИсключение = Истина) Экспорт
	...
	Если Результат.Пустой() Тогда
		Если ВызыватьИсключение Тогда
			СообщениеОбОшибке = НСтр("es='El gráfico del trabajo «%1» no es rellenado en el período de %2.';ru='График работы «%1» не заполнен на период %2.';en='Schedule ""%1"" is not filled for the period%2.'");
...
Функция ДатыБлижайшихРабочихДней(График, НачальныеДаты, ПолучатьПредшествующие = Ложь, ВызыватьИсключение = Истина, 
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе 
			...
			КонецЕсли;
			Если ВызыватьИсключение Тогда
				ТекстСообщения = НСтр("es='Es imposible determinar la próxima fecha de trabajo para la fecha %1, probablemente, el gráfico del trabajo no es rellenado.';ru='Невозможно определить ближайшую рабочую дату для даты %1, возможно, график работы не заполнен.';en='It is not possible to determine the nearest business date for date%1, or the schedule may not be completed.'");
...
Функция РасписанияРаботыНаПериод(Графики, ДатаНачала, ДатаОкончания) Экспорт
	...
	ВызватьИсключение НСтр("es='El subsistema «los Gráficos del trabajo» no es descubierto.';ru='Подсистема «Графики работы» не обнаружена.';en='The subsystem “Schedules of work” was not found.'");
...
Процедура ПриДобавленииКлассификаторов(Классификаторы) Экспорт
	...
	Описатель.Наименование = НСтр("es='Los calendarios de producción';ru='Производственные календари';en='Production calendars'");
...
Функция ДанныеКлассификатора() Экспорт
	...
	Если ДанныеФайлов = Неопределено Тогда
		ТекстСообщения = НСтр("es='No consiguió recibir los datos del calendario. "
"El trabajo con los clasificadores no es apoyado o falta.';ru='Не удалось получить данные календаря."
"Работа с классификаторами не поддерживается или отсутствует.';en='Failed to get calendar data."
"Work with classifiers is not supported or missing. '");
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	
	Если Не ПустаяСтрока(ДанныеФайлов.КодОшибки) Тогда
		ИмяСобытия = НСтр("es='Los gráficos de calendario. La recepción del fichero del clasificador';ru='Календарные графики.Получение файла классификатора';en='Calendar charts. Getting a classifier file'", ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(
			ИмяСобытия, 
			УровеньЖурналаРегистрации.Ошибка,,, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió recibir los datos del calendario. "
"%1';ru='Не удалось получить данные календаря. "
"%1';en='Failed to get calendar data."
"%1.'"), 
...
"%1.'"), 
	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		ТекстСообщения = НСтр("es='No consiguió recibir los datos del calendario. "
"Los clasificadores recibidos no contienen los calendarios.';ru='Не удалось получить данные календаря."
"Полученные классификаторы не содержат календарей.';en='Failed to get calendar data."
"The resulting classifiers do not contain calendars. '");
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	...
	Если СведенияФайла.Версия < ВерсияКалендарей() Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió tratar los datos recibidos del calendario por el conflicto de las versiones. "
"La versión de los calendarios "
"- el clasificador %1 recibido, "
"- empotrado en la configuración %2, "
"- el clasificador cargado antes %3.';ru='Не удалось обработать полученные данные календаря из-за конфликта версий."
"Версия календарей"
"- полученного классификатора %1, "
"- встроенных в конфигурацию %2, "
"- загруженного ранее классификатора %3.';en='Failed to process received calendar data due to version conflict."
"Calendars version"
"- obtained classifier%1,"
"- built into the%2 configuration,"
"- previously loaded classifier%3. '"),
	...
	Исключение
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió tratar los datos recibidos del calendario. "
"%1.';ru='Не удалось обработать полученные данные календаря."
"%1.';en='Failed to process received calendar data."
"%1.'"),
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Обработчик.Комментарий = НСтр("es='La renovación de los gráficos del trabajo etc. los datos dependientes de los calendarios de producción.';ru='Обновление графиков работы и др. данных, зависимых от производственных календарей.';en='Updating work schedules and other data dependent on production calendars.'");
...
Процедура ПроверитьНачалоЭлемента(Знач ЧтениеXML, Знач Имя)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента Или ЧтениеXML.Имя <> Имя Тогда
		ИмяСобытия = НСтр("es='Los gráficos de calendario. El tratamiento del fichero del clasificador';ru='Календарные графики.Обработка файла классификатора';en='Calendar charts. Processing classifier file'", ОбщегоНазначения.КодОсновногоЯзыка());
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El formato equivocado del fichero de los datos. Se Espera el comienzo del elemento %1';ru='Неверный формат файла данных. Ожидается начало элемента %1';en='Invalid data file format. Expected start of item %1'"), 
			Имя);
...
Процедура ПроверитьКонецЭлемента(Знач ЧтениеXML, Знач Имя)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.КонецЭлемента Или ЧтениеXML.Имя <> Имя Тогда
		ИмяСобытия = НСтр("es='Los gráficos de calendario. El tratamiento del fichero del clasificador';ru='Календарные графики.Обработка файла классификатора';en='Calendar charts. Processing classifier file'", ОбщегоНазначения.КодОсновногоЯзыка());
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El formato equivocado del fichero de los datos. Se Espera el fin del elemento %1';ru='Неверный формат файла данных. Ожидается конец элемента %1';en='Invalid data file format. Expected end of item %1'"), 
			Имя);
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка, , , ТекстСообщения);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
