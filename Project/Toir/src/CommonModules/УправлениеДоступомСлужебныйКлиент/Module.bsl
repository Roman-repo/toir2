	...
	Если Строки.Количество() > 0
	   И Строки[0].ПолучитьИдентификатор() <> Форма.Элементы.ВидыДоступа.ТекущаяСтрока Тогда
		...
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El tipo del acceso ""%1"" es escogido ya. "
"Escojan otro.';ru='Вид доступа ""%1"" уже выбран."
"Выберите другой.';en='Access type ""%1"" ""is already selected. Choose another.'"),
			ВыбранноеЗначение));
