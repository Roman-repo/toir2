...
Функция ПредставлениеРазмераФайла(Знач Размер) Экспорт
	...
	Если Размер < 1024 Тогда
		Возврат Формат(Размер, "ЧДЦ=1") + " " + НСтр("es='El byte';ru='байт';en='byte'");
	ИначеЕсли Размер < 1024 * 1024 Тогда
		Возврат Формат(Размер / 1024, "ЧДЦ=1") + " " + НСтр("es='КБ';ru='КБ';en='KB'");
	ИначеЕсли Размер < 1024 * 1024 * 1024 Тогда
		Возврат Формат(Размер / (1024 * 1024), "ЧДЦ=1") + " " + НСтр("es='EL MB';ru='МБ';en='MB'");
	Иначе
		Возврат Формат(Размер / (1024 * 1024 * 1024), "ЧДЦ=1") + " " + НСтр("es='ГБ';ru='ГБ';en='GB'");
	КонецЕсли;
...
Функция ПредставлениеРасписания(Расписание) Экспорт
	...
	Если Расписание = Неопределено Тогда
		Возврат НСтр("es='Ajustar el horario';ru='Настроить расписание';en='Set schedule'");
	Иначе
...
Функция ПользовательскоеПредставлениеИнтервалаВерсий(ВерсияОТ, ВерсияДО) Экспорт
	...
	Если (ВерсияОТ = "00.00.000.0000") И (ВерсияДО = "99.99.999.9999") Тогда
		Результат = НСтр("es='Cualquier versión';ru='Любая версия';en='Any version'");
	ИначеЕсли (ВерсияОТ <> "00.00.000.0000") И (ВерсияДО <> "99.99.999.9999") Тогда
		...
		Иначе // Интервал версий
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El intervalo %1... %2';ru='Интервал %1...%2';en='Interval%1 ...%2'"),
	...
	ИначеЕсли (ВерсияОТ <> "00.00.000.0000") И (ВерсияДО = "99.99.999.9999") Тогда // От версии и выше
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='De %1 y es más alto';ru='От %1 и выше';en='From %1 and up'"),
			ПользовательскоеПредставлениеНомераВерсии(ВерсияОТ, Истина));
	ИначеЕсли (ВерсияОТ = "00.00.000.0000") И (ВерсияДО <> "99.99.999.9999") Тогда // До версии включительно
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Hasta la versión %1 inclusivamente';ru='До версии %1 включительно';en='Up to version%1 inclusive'"),
			ПользовательскоеПредставлениеНомераВерсии(ВерсияДО, Истина));
...
Функция ПредставлениеДняНедели(НомерДняНедели) Экспорт
	...
	Если НомерДняНедели = 1 Тогда
		Результат = НСтр("es='El lunes';ru='Понедельник';en='Monday'");
	ИначеЕсли НомерДняНедели = 2 Тогда
		Результат = НСтр("es='El martes';ru='Вторник';en='Tuesday'");
	ИначеЕсли НомерДняНедели = 3 Тогда
		Результат = НСтр("es='El ambiente';ru='Среда';en='Wednesday'");
	ИначеЕсли НомерДняНедели = 4 Тогда
		Результат = НСтр("es='El jueves';ru='Четверг';en='Thursday'");
	ИначеЕсли НомерДняНедели = 5 Тогда
		Результат = НСтр("es='El viernes';ru='Пятница';en='Friday'");
	ИначеЕсли НомерДняНедели = 6 Тогда
		Результат = НСтр("es='El sábado';ru='Суббота';en='Saturday'");
	ИначеЕсли НомерДняНедели = 7 Тогда
		Результат = НСтр("es='El domingo';ru='Воскресенье';en='Sunday'");
	КонецЕсли;
...
Функция ЗавершитьРегистрациюРезультатаВыполненияОперации(КонтекстВыполнения, КодРезультата, ОписаниеРезультата, ВложенныйКонтекстВыполнения = Неопределено) Экспорт
	...
	Если ТипЗнч(ТекущийШагВыполнения.ВремяНачала) <> Тип("Число") Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No es registrado el comienzo del paso de la ejecución %1, %2.';ru='Не зарегистрировано начало шага выполнения %1, %2.';en='The start of the execution step%1,%2 has not been registered.'"),
...
Функция ПредставлениеЗаписиРезультатовВыполненияОпераций(
	...
	Для Каждого ТекущийШаг Из КонтекстВыполнения.ЛогШаговВыполнения Цикл
		...
		ИначеЕсли ВариантФормата = "ПодробноПоШагам" Тогда
			...
			ШаблонШага = НСтр("es='%1Шаг: %2 "
"%1Длительность (мс): %3 "
"%1Результат de la ejecución: %4';ru='%1Шаг: %2"
"%1Длительность (мс): %3"
"%1Результат выполнения: %4';en='%1Step:%2"
"%1 Duration (ms):       %3"
"%1Result result:%4'")
				+ Символы.ПС;
			Результат = Результат
...
Процедура ОтобразитьСостояниеПодключенияИПП(Форма) Экспорт
	...
	Если Форма.БИПДанныеАутентификации = Неопределено Тогда
		Элементы.ДекорацияЛогинИПП.Заголовок = НСтр("es='La conexión al Apoyo De Internet no es cumplida.';ru='Подключение к Интернет-поддержке не выполнено.';en='Connection to online support failed.'");
		Элементы.ВойтиИлиВыйтиИПП.Заголовок = НСтр("es='Conectar';ru='Подключить';en='Connect'");
		Элементы.ВойтиИлиВыйтиИПП.ОтображениеПодсказки = ОтображениеПодсказки.Нет;
	Иначе
		ШаблонЗаголовка = ПодставитьДомен(
			НСтр("es='<body> Es conectado el Apoyo De Internet para el usuario <a href = ""action:openUsersSite""> %1 </body>';ru='<body>Подключена Интернет-поддержка для пользователя <a href=""action:openUsersSite"">%1</body>';en='<body> Online support is enabled for user <a href=""action:openUsersSite"">% 1 </body>'"));
		Элементы.ДекорацияЛогинИПП.Заголовок =
			ФорматированныйЗаголовок(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонЗаголовка,
					Форма.БИПДанныеАутентификации.Логин));
		Элементы.ВойтиИлиВыйтиИПП.Заголовок = НСтр("es='Desconectar';ru='Отключить';en='Disable'");
