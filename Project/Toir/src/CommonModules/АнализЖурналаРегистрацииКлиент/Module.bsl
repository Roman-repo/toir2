...
Процедура ФормаОтчетаОбработкаРасшифровки(ФормаОтчета, Элемент, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	Если ТипРасшифровки = "РасшифровкаРегламентногоЗадания" Тогда
		...
		ВариантРасшифровки.Добавить("СведенияОРегламентномЗадании", НСтр("es='Las noticias sobre la tarea de reglamento';ru='Сведения о регламентном задании';en='Scheduled task information'"));
		ВариантРасшифровки.Добавить("ОткрытьЖурналРегистрации", НСтр("es='Pasar a la revista del registro';ru='Перейти к журналу регистрации';en='Go to the logbook'"));
...
Процедура РезультатОбработкаРасшифровкиЗавершение(ВыбранныйВариант, ПараметрыОбработчика) Экспорт
	...
	Если Действие = "СведенияОРегламентномЗадании" Тогда
		...
		Для Каждого ТочкаДиаграммыГанта Из СписокТочек Цикл
			...
			Если ТочкаДиаграммыГанта.Значение = НСтр("es='Las tareas de fondo';ru='Фоновые задания';en='Background tasks'") Тогда
				Продолжить;
			КонецЕсли;
