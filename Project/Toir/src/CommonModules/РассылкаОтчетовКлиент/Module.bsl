...
Процедура ВыбратьПолучателя(ОбработчикРезультата, Объект, МножественныйВыбор, ВозвращатьСоответствие) Экспорт
	...
	Если МножественныйВыбор Тогда
		ВозможныеПолучатели.Вставить(0, Неопределено, НСтр("es='A todos los destinatarios';ru='Всем получателям';en='To all recipients'"));
	КонецЕсли;
	...
	ВозможныеПолучатели.ПоказатьВыборЭлемента(Обработчик, НСтр("es='La elección del destinatario';ru='Выбор получателя';en='Recipient Selection'"));
КонецПроцедуры
...
Процедура ВыполнитьСейчасВФоне(Получатели, Параметры) Экспорт
	...
	Если Параметры.ЭтоФормаЭлемента Тогда
		...
		КонецЕсли;
		ТекстСостояния = НСтр("es='Se cumple el envío de los informes.';ru='Выполняется рассылка отчетов.';en='Reporting is in progress.'");
	Иначе
		ТекстСостояния = НСтр("es='Se cumplen los envíos de los informes.';ru='Выполняются рассылки отчетов.';en='Reporting is in progress.'");
	КонецЕсли;
...
Процедура ВыполнитьСейчасВФонеЗавершение(Задание, Параметры) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("es='No consiguió cumplir los envíos de los informes:';ru='Не удалось выполнить рассылки отчетов:';en='Failed to send reports:'")
...
Функция СписокВариантовЗаполненияРасписаний() Экспорт
	...
	СписокВариантов.Добавить(1, НСтр("es='Cada día';ru='Каждый день';en='Every day'"));
	СписокВариантов.Добавить(2, НСтр("es='Cada segundo día';ru='Каждый второй день';en='Every second day'"));
	СписокВариантов.Добавить(3, НСтр("es='Cada cuarto día';ru='Каждый четвертый день';en='Every fourth day'"));
	СписокВариантов.Добавить(4, НСтр("es='Entre semana';ru='По будням';en='On weekdays'"));
	СписокВариантов.Добавить(5, НСтр("es='Por el fin de semana';ru='По выходным';en='On weekends'"));
	СписокВариантов.Добавить(6, НСтр("es='Todos los lunes';ru='По понедельникам';en='On Mondays'"));
	СписокВариантов.Добавить(7, НСтр("es='Los viernes';ru='По пятницам';en='On Fridays'"));
	СписокВариантов.Добавить(8, НСтр("es='Los domingos';ru='По воскресеньям';en='On Sundays'"));
	СписокВариантов.Добавить(9, НСтр("es='En el primer día el mes';ru='В первый день месяца';en='On the first day of the month'"));
	СписокВариантов.Добавить(10, НСтр("es='En el último día el mes';ru='В последний день месяца';en='On the last day of the month'"));
	СписокВариантов.Добавить(11, НСтр("es='Cada trimestre del décimo';ru='Каждый квартал десятого числа';en='Every quarter of the tenth'"));
	СписокВариантов.Добавить(12, НСтр("es='Otro...';ru='Другое...';en='Other ...'"));
