...
Процедура УдалитьЗадание(Знач Идентификатор) Экспорт
	...
	Иначе
		Если ТипЗнч(Идентификатор) = Тип("ОбъектМетаданных") И Идентификатор.Предопределенное Тогда
			ВызватьИсключение( НСтр("es='La tarea predestinada de reglamento de quitar es imposible.';ru='Предопределенное регламентное задание удалить невозможно.';en='The predefined scheduled task cannot be deleted.'") );
...
Функция ПолучитьРегламентноеЗадание(Знач Идентификатор) Экспорт
	...
	Если РегламентноеЗадание = Неопределено Тогда
		ВызватьИсключение( НСтр("es='La tarea de reglamento no es encontrada. "
"Probablemente, es quitado por otro usuario.';ru='Регламентное задание не найдено."
"Возможно, оно удалено другим пользователем.';en='Regulatory task not found."
