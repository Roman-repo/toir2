...
Функция ВыполнитьЗапрос(Прокси, Запрос) Экспорт
	...
	Исключение
		...
		Если Найти(КраткоеПредставлениеОшибки, НСтр("es='Аутентификация del usuario no es cumplido';ru='Аутентификация пользователя не выполнена';en='User Authentication Failed'")) <> 0 Тогда
			ПараметрыСеанса.ИнтеграцияС1СДокументооборотВерсияСервиса = "0.0.0.0";
			КраткоеПредставлениеОшибки = НСтр("es='El nombre equivocado del usuario o la consigna. Comprueben el nombre "
"y la consigna en los ajustes de la autorización en 1С:Документообороте.';ru='Неверное имя пользователя или пароль. Проверьте имя"
"и пароль в настройках авторизации в 1С:Документообороте.';en='Invalid username or password. Verify username"
"and password in the authorization settings in 1C: Document management.'");
		...
		Иначе
			...
			Иначе // получим версию сервиса асинхронно, в фоновом задании
				ЗаписьЖурналаРегистрации(
					...
					НСтр("es='Es comenzada la recepción de la versión del servicio';ru='Начато получение версии сервиса';en='Getting service version started'",
...
Функция ПолучитьВерсиюВызовомСервиса(ИмяПользователя, Пароль, ИспользуетсяАутентификацияОС)
	...
	Если Прокси = Неопределено Тогда
		ЗаписьЖурналаРегистрации(
			ИмяСобытияЖурналаРегистрации(), 
			УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("es='El servicio es inaccesible';ru='Сервис недоступен';en='Service is unavailable'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
		Возврат "0.0.0.0";
	Иначе
		...
		Иначе
			ТекстСообщения = СтрШаблон(
				НСтр("es='Es conectado el servicio de la versión %1';ru='Подключен сервис версии %1';en='Connected service version%1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				Ответ.versionNumber);
			ЗаписьЖурналаРегистрации(
				ИмяСобытияЖурналаРегистрации(), 
				УровеньЖурналаРегистрации.Информация,,,
...
Процедура УстановитьВерсиюСервиса(АдресВХранилище, ИмяПользователя, Пароль, ИспользуетсяАутентификацияОС) Экспорт
	...
	Иначе
		...
		ЗаписьЖурналаРегистрации(
			ИмяСобытияЖурналаРегистрации(), 
			УровеньЖурналаРегистрации.Информация,,,
			НСтр("es='Es acabada la recepción de la versión del servicio';ru='Завершено получение версии сервиса';en='Completed service version retrieval'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	КонецЕсли;
...
Процедура ПроверитьВозвратВебСервиса(Прокси, Ответ) Экспорт
	...
	Если ПроверитьТип(Прокси, Ответ, "DMError") Тогда 
		...
		ПозицияТекстаПоПричине = СтрНайти(ТекстОшибки,
			Символы.ПС + НСтр("es='Por la causa:';ru='по причине:';en='due to:'"),
...
Функция НовоеИсходящееПисьмо(Прокси, Предмет = Неопределено) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("es='Esta versión 1С:Документооборота no apoya la creación de las cartas salientes';ru='Эта версия 1С:Документооборота не поддерживает создание исходящих писем';en='This version of 1C: Workflow does not support the creation of outgoing emails'");
	КонецЕсли;
...
Функция ПолучитьПрокси(ВызыватьИсключение = Истина, ИмяПользователя = Неопределено, Пароль = Неопределено,
	...
	Если Определения = Неопределено Тогда
		...
		Если ВызыватьИсключение Тогда
			ВызватьИсключение НСтр("es='La falta de la conexión a 1С:Документообороту. "
""
"Probablemente, no ha pasado la autorización, es indicada la dirección equivocada del servicio web "
"o la base 1С:Документооборота no es publicado en el servidor web. "
""
"Los detalles en la revista del registro. Se dirijan al administrador del sistema.';ru='Ошибка подключения к 1С:Документообороту."
""
...
""
"Подробности в журнале регистрации. Обратитесь к администратору системы.';en='Error connecting to 1C: Document management."
""
"Perhaps authorization failed, the specified web service address is incorrect"
"or base 1C: Document management is not published on the web server."
""
"Details in the logbook. Contact system administrator. '");
		Иначе
			Возврат Неопределено;
		КонецЕсли;
	КонецЕсли;
...
Функция СоздатьОбъект(Прокси, ТипОбъекта) Экспорт
	...
	Если Прокси = Неопределено Тогда
		ВызватьИсключение НСтр("es='La integración con 1С:Документооборотом no es ajustada, o el servicio "
"1С:Документооборота es inaccesible. Se dirijan al administrador.';ru='Интеграция с 1С:Документооборотом не настроена, или сервис"
"1С:Документооборота недоступен. Обратитесь к администратору.';en='Integration with 1C: Document management is not configured, or service"
"1C: Document management is not available. Contact your administrator. '");
	КонецЕсли;
		...
		Если ОбщегоНазначения.ЭтоСправочник(МетаданныеОбъекта) Тогда
			...
			Если Ошибка <> Неопределено Тогда
				...
				Если Выборка.Следующий() Тогда
					...
					ТекстСообщения = СтрШаблон(
						НСтр("es='Es insuficientemente derecho para el acceso a este objeto (%1). "
"Se dirijan al administrador.';ru='Недостаточно прав для доступа к этому объекту (%1)."
"Обратитесь к администратору.';en='Not enough rights to access this object (%1)."
"Contact administrator. '"),
			...
			Иначе // продолжим поиск по другим реквизитам
				...
				Если Не ЗначениеЗаполнено(Результат) Тогда
					...
					Иначе
						ТекстСообщения = СтрШаблон(
							НСтр("es='Es insuficientemente derecho para la creación de los nuevos elementos (%1). "
"Se dirijan al administrador.';ru='Недостаточно прав для создания новых элементов (%1)."
"Обратитесь к администратору.';en='Not enough rights to create new items (%1)."
...
Функция СокращенноеНаименованиеКонфигурации() Экспорт
	...
	СокращенноеНаименование = НСтр("es='БИД (el demo)';ru='БИД (демо)';en='BID (demo)'");
...
Функция ИмяСобытияЖурналаРегистрации(СобытиеВторогоУровня = "") Экспорт
	...
	Если СобытиеВторогоУровня = "" Тогда
		Возврат НСтр("es='La integración con 1С:Документооборотом';ru='Интеграция с 1С:Документооборотом';en='Integration with 1C: Document management'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Иначе
		Возврат СтрШаблон("%1.%2",
			НСтр("es='La integración con 1С:Документооборотом';ru='Интеграция с 1С:Документооборотом';en='Integration with 1C: Document management'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ПолучитьФайлыСвязанныхДокументов(СвязанныеДокументы) Экспорт
	...
	ФоновыеЗадания.Выполнить(
		...
		МассивПараметров, КлючЗадания, НСтр("es='La recepción de los ficheros de los documentos vinculados';ru='Получение файлов связанных документов';en='Retrieving related document files'"));
		...
		Для Каждого ПодходящийОбъект Из ПодходящиеОбъекты Цикл
			...
			Если ДанныеОбъектаДО = Неопределено Тогда // создадим документ в ДО
				...
				Иначе // сообщение об ошибке
					ИмяСобытия = НСтр("es='La creación automática del objeto vinculado';ru='Автоматическое создание связанного объекта';en='Automatically create a linked object'");
					ЗаписьЖурналаРегистрации(
						ИмяСобытияЖурналаРегистрации(ИмяСобытия), 
						УровеньЖурналаРегистрации.Ошибка,,,
						НовыйОбъектДО);
					ТекстИсключения = СтрШаблон(НСтр("es='No consiguió crear el documento vinculado para "
"""%1: %2""';ru='Не удалось создать связанный документ для ""%1"":"
"%2';en='Failed to create linked document for ""%1"":"
"%2'"),
						ПодходящийОбъект,
						НовыйОбъектДО);
...
Процедура ДобавитьКомандуПереключитьХронометраж(Форма, Знач МестоРазмещенияКоманд)
	...
	Если Форма.Команды.Найти(ИмяКоманды) = Неопределено Тогда
		...
		КомандаФормы.Заголовок = НСтр("es='Cambiar el cronometraje';ru='Переключить хронометраж';en='Toggle timing'");
		КомандаФормы.Подсказка = НСтр("es='Cambiar el cronometraje';ru='Переключить хронометраж';en='Toggle timing'");
...
Процедура ДобавитьКомандуУказатьТрудозатраты(Форма, Знач МестоРазмещенияКоманд)
	...
	Если Форма.Команды.Найти(ИмяКоманды) = Неопределено Тогда
		КомандаФормы = Форма.Команды.Добавить(ИмяКоманды);
		КомандаФормы.Действие = "Подключаемый_ВыполнитьКомандуИнтеграции";
		КомандаФормы.Заголовок = НСтр("es='Indicar трудозатраты';ru='Указать трудозатраты';en='Indicate labor costs'");
		КомандаФормы.Подсказка = НСтр("es='Indicar трудозатраты';ru='Указать трудозатраты';en='Indicate labor costs'");
		КомандаФормы.ИзменяетСохраняемыеДанные = Ложь;
		КомандаФормы.Отображение = ОтображениеКнопки.Авто;
