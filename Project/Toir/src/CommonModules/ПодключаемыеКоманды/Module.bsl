	...
	Если ИсточникиЧерезЗапятую = "" И ТребуетсяУказаниеИсточниковКоманд(ИмяФормы) Тогда
		...
		Если ИсточникиЧерезЗапятую = "" Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='En las formas de los informes, los tratamientos y en las formas generales "
"con la llamada del procedimiento ""%1"" "
"es necesario claramente indicar parámetro ""%2""';ru='В формах отчетов, обработок и в общих формах"
"при вызове процедуры ""%1"""
"требуется явно указывать параметр ""%2""';en='In reporting, processing and general forms"
"when calling procedure ""%1 """
"parameter ""%2 "" must be explicitly specified'"),
				"ПодключаемыеКоманды.ПриСозданииНаСервере",
				"ПараметрыРазмещения.Источники");
		КонецЕсли;
	КонецЕсли;
...
Функция НастройкиПодключаемогоОбъекта(ПолноеИмя, НастройкиПрограммногоИнтерфейса = Неопределено) Экспорт
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió leer los ajustes del objeto %1 del módulo del manager:';ru='Не удалось прочитать настройки объекта %1 из модуля менеджера:';en='Failed to read settings of object%1 from manager module:'"), ПолноеИмя);
		ТекстОшибки = ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		
		ЗаписьЖурналаРегистрации(
			НСтр("es='Las órdenes conectadas';ru='Подключаемые команды';en='Pluggable commands'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ТаблицаПодключаемыхОбъектов(НастройкиПрограммногоИнтерфейса = Неопределено) Экспорт
	...
	Для Каждого Настройка Из НастройкиПрограммногоИнтерфейса Цикл
		...
		Исключение
			ТекстОшибки = НСтр("es='No consiguió registrar el ajuste de la interfaz de programa de los objetos conectados. "
"La llave: ""%1"", la descripción de los tipos: ""%2"", la descripción de la falta: ""%3"".';ru='Не удалось зарегистрировать настройку программного интерфейса подключаемых объектов."
"Ключ: ""%1"", описание типов: ""%2"", описание ошибки: ""%3"".';en='Failed to register the configuration of the program interface of the connected objects."
...
Процедура ПроверитьИмяВидаКоманд(ИмяВида)
	...
	Исключение
		ТекстОшибки = НСтр("es='El nombre del tipo de las órdenes ""%1"" no responde a las exigencias именования tras las variables.';ru='Имя вида команд ""%1"" не удовлетворяет требованиям именования переменных.';en='The name of the command type ""%1"" does not satisfy the variable naming requirements.'");
...
Процедура ВывестиКоманды(Форма, Команды, ПараметрыРазмещения)
	...
	Если КомандаЗаглушка = Неопределено Тогда
		...
		КомандаЗаглушка.Заголовок = НСтр("es='No)';ru='(нет)';en='(no)'");
	КонецЕсли;
...
Функция ЗарегистрироватьПодменю(Элементы, СведенияОВсехПодменю, ИмяПодменю, ШаблонНового = Неопределено, КоманднаяПанель = Неопределено, ПодменюПоУмолчанию = Неопределено)
	...
	Если ЗначениеЗаполнено(ИмяПодменю) Тогда
		...
		Если Не Группы.Свойство("Важное") Тогда
			...
			ГруппаВажное.Заголовок = Подменю.Заголовок + " (" + НСтр("es='Importante';ru='Важное';en='Important'") + ")";
		...
		Если Не Группы.Свойство("Обычное") Тогда
			...
			ГруппаПоУмолчанию.Заголовок = Подменю.Заголовок + " (" + НСтр("es='Regular';ru='Обычное';en='Usual'") + ")";
		...
		Если Не Группы.Свойство("СмТакже") Тогда
			...
			ГруппаСмТакже.Заголовок = Подменю.Заголовок + " (" + НСтр("es='Véase también';ru='См. также';en='See also'") + ")";
...
Функция ОписаниеКоманды(ИмяКомандыВФорме, АдресНастроек) Экспорт
	...
	Если Команда = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Las noticias sobre la orden ""%1"" no son encontradas.';ru='Сведения о команде ""%1"" не найдены.';en=""No information found for command '%1'."""),
			ИмяКомандыВФорме);
	КонецЕсли;
