...
Процедура ВключитьОтключитьАвтоматическуюУстановкуИсправлений(Знач ЗначениеНастройки) Экспорт
	...
	Если Не ПараметрыОбновления.ПолучатьИсправления Тогда
		ВызватьИсключение НСтр("es='La recepción de las correcciones es inaccesible en el régimen corriente del trabajo.';ru='Получение исправлений недоступно в текущем режиме работы.';en='Getting fixes is not available in the current mode of operation.'");
	КонецЕсли;
...
Функция ПараметрыПроверкиВерсииПлатформыПриЗапуске() Экспорт
	...
	ТекстСообщения = "<body>" + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Para el trabajo con el programa %1 usar la versión de la plataforma &quot;1С:Предприятие 8&quot; no más abajo: <b> %2 </b> "
"<br/> ahora versión Usada: %3';ru='Для работы с программой %1 использовать версию платформы &quot;1С:Предприятие 8&quot; не ниже: <b>%2</b>"
"<br />Используемая сейчас версия: %3';en='To work with the program%1 use the platform version &quot;1С:Предприятие 8&quot; не ниже: <b>%2</b>"
"<br />Version used now:%3'"),
		?(ПараметрыБазовойФункциональности.РаботаВПрограммеЗапрещена,
			НСтр("es='Es necesario';ru='необходимо';en='required'"),
			НСтр("es='Es necesario';ru='необходимо';en='required'")),
	...
	Если Не Результат.ЭтоАдминистраторСистемы Тогда
		...
		ТекстСообщения = ТекстСообщения + "<br /><br />"
			+ НСтр("es='La entrada en el programa es imposible. <br/> "
"es necesario dirigirse al administrador para la renovación de la versión de la plataforma 1С:Предприятие.';ru='Вход в программу невозможен.<br />"
"Необходимо обратиться к администратору для обновления версии платформы 1С:Предприятие.';en='Entering the program is not possible. <br />"
"You must contact the administrator to update the version of the 1C: Enterprise platform.'");
	...
	ИначеЕсли ПараметрыБазовойФункциональности.РаботаВПрограммеЗапрещена Тогда
		...
		ТекстСообщения = ТекстСообщения + "<br /><br />"
			+ НСтр("es='La entrada en el programa es imposible. <br/> "
"es necesario preliminarmente renovar la versión de la plataforma 1С:Предприятие.';ru='Вход в программу невозможен.<br />"
"Необходимо предварительно обновить версию платформы 1С:Предприятие.';en='Entering the program is not possible. <br />"
"You must first update the version of the 1C: Enterprise platform.'");
		
	Иначе
		
		ТекстСообщения = ТекстСообщения + "<br /><br />"
			+ НСтр("es='Se recomienda renovar la versión de la plataforma 1С:Предприятия. <br/> "
"En caso contrario algunas posibilidades del programa serán inaccesibles o trabajarán es incorrecto.';ru='Рекомендуется обновить версию платформы 1С:Предприятия.<br />"
"В противном случае некоторые возможности программы будут недоступны или будут работать некорректно.';en='It is recommended to upgrade the version of the 1C: Enterprise platform. <br />"
	...
	Попытка
		...
		Если Результат.Статус = "Отменено" Тогда
			...
			ПолучениеОбновленийПрограммы.ЗаписатьИнформациюВЖурналРегистрации(
				НСтр("es='No consiguió cumplir la comprobación de fondo de la presencia de las renovaciones. La tarea es anulada por el administrador.';ru='Не удалось выполнить фоновую проверку наличия обновлений. Задание отменено администратором.';en='Failed to run background check for updates. Job canceled by administrator.'"));
			Возврат Неопределено;
		...
		ИначеЕсли Результат.Статус = "Ошибка" Тогда
			...
			ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La falta de la comprobación de fondo de la presencia de las renovaciones. No consiguió realizar la tarea de fondo. %1';ru='Ошибка фоновой проверки наличия обновлений. Не удалось выполнить фоновое задание. %1';en='Error checking background for updates. Failed to complete background task. %1'"),
...
Функция РезультатЗаданияПроверкиНаличияОбновлений(Знач ОписаниеЗадания) Экспорт
	...
	Исключение
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió determinar el resultado de la comprobación de fondo de la presencia de las renovaciones. %1';ru='Не удалось определить результат фоновой проверки наличия обновлений. %1';en='Failed to determine the result of the background check for updates. %1'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
		Возврат -1;
	КонецПопытки;
