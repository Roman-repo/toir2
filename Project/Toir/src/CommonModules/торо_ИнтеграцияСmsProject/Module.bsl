...
Процедура ОтправитьВProject(ПараметрКоманды, ПараметрыВыполненияКоманды, ПереданМассивID = Ложь) Экспорт
	...
	Кнопки.Добавить(1, НСтр("ru='В MS Project напрямую';en='To MS Project directly'"));
	Кнопки.Добавить(2, НСтр("ru='В промежуточный XML-файл';en='To intermediate XML file'"));
	Кнопки.Добавить(3, НСтр("en='cancel';es='La anulación';ru='Отмена'"));
	
	ПоказатьВопрос(ОписаниеОповещения, НСтр("es='Indiquen el modo deseable de la transferencia de los datos:';ru='Укажите желаемый способ передачи данных:';en='Specify the desired data transfer method:'"), Кнопки,, 1, НСтр("ru='Интеграция с MS Project';en='Integration with MS Project'"));
...
Процедура ВыгрузитьЗаявку(ПутьКФайлу, Заявка) 
	...
	Если ЗначениеЗаполнено(ПутьКФайлу) Тогда 
		...
		Если ПередатьДанныеПоCOM(Данные, ПутьКФайлу) Тогда
			ПоказатьПредупреждение(, НСтр("es='La descarga en MS Project es acabada.';ru='Выгрузка в MS Project завершена.';en='Unloading in MS Project is completed.'"), 5, НСтр("es='La integración con MS Project';ru='Интеграция с MS Project';en='Integration with MS Project'"));
		КонецЕсли;
...
Функция ПередатьДанныеПоCOM(Данные, ПутьКФайлу)
	...
	Если ТипЗнч(Данные) <> Тип("Соответствие") Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("es='Los datos son equivocados.';ru='Данные неверны.';en='Data is incorrect.'"));
		Возврат Ложь;
	КонецЕсли;
	
	#Если МобильныйКлиент Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("es='Функционал es inaccesible en el cliente móvil.';ru='Функционал недоступен в мобильном клиенте.';en='Functionality is not available in the mobile client.'"));
		Возврат Ложь;
	#Иначе	
	...
	Исключение
	    ПоказатьПредупреждение(Неопределено, Нстр("es='La falta de la conexión a MS Project, es posible MS Project no es establecido en el ordenador dado';ru='Ошибка подключения к MS Project, возможно MS Project не установлен на данном компьютере';en='Error connecting to MS Project, maybe MS Project is not installed on this computer'"),,"Ошибка");
		Возврат Ложь;
	КонецПопытки;   
	...
	Попытка
		Если prjApp.Projects.Count() > 0 Тогда
			prjApp = Неопределено;
			ПоказатьПредупреждение(Неопределено,НСтр("es='La aplicación Microsoft Project es puesta en marcha ya. La carga o la descarga no puede ser hecha';ru='Приложение Microsoft Project уже запущено. Загрузка или выгрузка не может быть произведена';en='The application Microsoft Project is already running. Loading or unloading cannot be done.'"));
			Возврат Ложь;
		КонецЕсли;
	Исключение
	...
	Исключение
		...
		prjApp.Quit();
		ПоказатьПредупреждение(Неопределено, НСтр("es='La falta del relleno del fichero del proyecto';ru='Ошибка заполнения файла проекта';en='Error filling project file'"));
		Возврат Ложь;
	КонецПопытки;
...
Функция ЗагрузитьЗаявки(ПутьКФайлуДляЗагрузки, ТипЗагрузки = "ЗаявкаНаПлатеж") Экспорт
	...
	Если ТекстОшибки = "" Тогда
		ТекстПредупреждения = НСтр("es='La carga de MS Project es acabada.';ru='Загрузка из MS Project завершена.';en='Download from MS Project is completed.'");
	Иначе
		ТекстПредупреждения = ТекстОшибки;
	КонецЕсли;
	ПоказатьПредупреждение(, ТекстПредупреждения, 5, НСтр("es='La integración con MS Project';ru='Интеграция с MS Project';en='Integration with MS Project'"));
...
Функция ПолучитьДанныеПоCOM(ПутьКФайлуДляЗагрузки)
	...
	Попытка
		prjApp = Новый COMОбъект("MSProject.Application");
	Исключение
	    ПоказатьПредупреждение(Неопределено, Нстр("es='La falta de la conexión a MS Project, es posible MS Project no es establecido en el ordenador dado';ru='Ошибка подключения к MS Project, возможно MS Project не установлен на данном компьютере';en='Error connecting to MS Project, maybe MS Project is not installed on this computer'"),,НСтр("es='La falta';ru='Ошибка';en='Error '"));
		Возврат Неопределено;
	КонецПопытки;  
	...
	Исключение
		ОбщегоНазначенияКлиент.СообщитьПользователю(ОписаниеОшибки());
		prjApp.Quit();
		ПоказатьПредупреждение(Неопределено, НСтр("es='La falta a la lectura del fichero del proyecto';ru='Ошибка при чтении файла проекта';en='Error reading project file'"));
	КонецПопытки;
...
Процедура ОткрытьДиалогВыбораФайла(ОписаниеОповещения, Расширение = "xml")
	...
	ЗаголовокДиалога = НСтр("es='Escojan el lugar de la conservación del fichero:';ru='Выберите место сохранения файла:';en='Choose a location to save the file:'");
	...
	Если Расширение = "xml" Тогда
		ДиалогОткрытияФайла.Фильтр = НСтр("es='El formato XML (*.xml) |*.xml';ru='Формат XML(*.xml)|*.xml';en='XML format (*. Xml) | * .xml'");
	Иначе
		ДиалогОткрытияФайла.Фильтр = НСтр("es='El proyecto (*.mpp) | *.mpp';ru='Проект(*.mpp)|*.mpp';en='Project(*.mpp)|*.mpp'");
	КонецЕсли;
...
Функция ЗагрузитьЗаявкиИзXML(Знач ПутьКФайлуДляЗагрузки, ТипЗагрузки = "ЗаявкаНаПлатеж") Экспорт
	...
	Если Данные <> Неопределено Тогда
		...
		КонецЕсли;
	Иначе
		ТекстПредупреждения = НСтр("es='La falta a la creación del COM-objeto «MSXML2.DOMDocument».';ru='Ошибка при создании COM-объекта «MSXML2.DOMDocument».';en='Error creating COM object ""MSXML2.DOMDocument"".'");
	КонецЕсли;
...
Процедура СообщитьОбУдаленныхРемонтах(МассивУдаленныхРемонтов)
	...
	Если КоличествоУдаленных > 0 Тогда
		ШаблонСообщения = НСтр("es='A la redacción en MS Project del fichero ';ru='При редактировании в MS Project из файла ';en='When editing in MS Project from a file '");
		Если КоличествоУдаленных = 1 Тогда
			ШаблонСообщения = ШаблонСообщения + НСтр("es='Era quitada 1 reparación.';ru='был удален 1 ремонт.';en='1 repair has been removed.'");
		Иначе
			Если КоличествоУдаленных<5 Тогда
				ШаблонСообщения = ШаблонСообщения + НСтр("es='Era quitado %1 reparaciones.';ru='было удалено %1 ремонта.';en='%1 repair was deleted.'");
			Иначе
				ШаблонСообщения = ШаблонСообщения + НСтр("es='Era quitado %1 reparaciones.';ru='было удалено %1 ремонтов.';en='%1 repairs were deleted.'");
			КонецЕсли;
