...
Функция ПереместитьЭлемент(ЭлементСписок, ТекущийЭлементСсылка, Направление) Экспорт
	...
	Если Не ПараметрыДоступа.Доступность Тогда
		Возврат НСтр("es='Es insuficientemente derecho para el cambio aproximadamente los elementos.';ru='Недостаточно прав для изменения порядка элементов.';en='Not enough rights to reorder items.'");
	КонецЕсли;
	...
	Если Информация.ЕстьРодитель И ОтображениеСписком И Не СписокСодержитОтборПоРодителю(НастройкиКомпоновкиДанных) Тогда
		Возврат НСтр("es='Para el cambio aproximadamente los elementos es necesario establecer el régimen de la presentación ""el Árbol"" o ""la lista Jerárquica"".';ru='Для изменения порядка элементов необходимо установить режим просмотра ""Дерево"" или ""Иерархический список"".';en='To change the order of elements, you must set the view mode to ""Tree"" or ""Hierarchical list"".'");
	КонецЕсли;
	...
	Если Информация.ЕстьВладелец И Не СписокСодержитОтборПоВладельцу(НастройкиКомпоновкиДанных) Тогда
		Возврат НСтр("es='Para el cambio aproximadamente los elementos es necesario establecer la selección por el campo ""el Propietario"".';ru='Для изменения порядка элементов необходимо установить отбор по полю ""Владелец"".';en='To change the order of elements, it is necessary to set the selection by the ""Owner"" field.'");
	КонецЕсли;
	...
	Если Информация.ЕстьГруппы Тогда
		...
		Если ЭтоГруппа И Не Информация.ДляГрупп Или Не ЭтоГруппа И Не Информация.ДляЭлементов Тогда
			Возврат НСтр("es='El elemento escogido no es posible trasladar.';ru='Выбранный элемент нельзя перемещать.';en='The selected item cannot be moved.'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ДеревоЗначений = Неопределено Тогда
		Возврат НСтр("es='Para el cambio aproximadamente los elementos es necesario arrojar los ajustes de la lista "
"(el Menú Todavía - Establecer los ajustes estandartizados).';ru='Для изменения порядка элементов необходимо сбросить настройки списка"
"(Меню Еще - Установить стандартные настройки).';en='To reorder items, reset the list"
"(More menu - Set default settings).'");
	КонецЕсли;
...
Функция ПроверитьСортировкуВСписке(НастройкиКомпоновкиДанных)
	...
	Если Не СортировкаПравильная Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Para el traslado de los elementos es necesario ajustar la selección "
"en la lista por el campo ""%1"" (por el acrecentamiento)';ru='Для перемещения элементов необходимо настроить сортировку"
"в списке по полю ""%1"" (по возрастанию)';en='To move items, you need to configure sorting"
"in the list by field ""%1"" (ascending)'"), РеквизитДопУпорядочивания.Заголовок);
	КонецЕсли;
