            ...
            Результат = Истина;
				...
				Если НЕ ЗначениеЗаполнено(Форма[Строка.ИмяРеквизитаЗначение]) Тогда
					ОбщегоНазначения.СообщитьПользователю(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Campo ""%1"" no es rellenado.';ru='Поле ""%1"" не заполнено.';en='Field ""%1"" is empty.'"), Строка.Наименование),
						,
   ...
   УправлениеСвойствамиСлужебный.ЗаполнитьНаборыСДополнительнымиРеквизитами(
	...
	Для Каждого ОписаниеСвойства Из ОписаниеСвойств Цикл
		...
		Если ОписаниеСвойства.СтрокаСсылочногоТипа Тогда
			...
			Иначе
				Значение = НСтр("es='No es dado';ru='не задано';en='not set'");
            ...
            ЭлементСписка = Форма.Свойства_ЭлементыГруппДополнительныхРеквизитов.НайтиПоЗначению(
				...
				Иначе
					...
					Если ОписаниеНабора = Неопределено Тогда
						...
						ОписаниеНабора.Заголовок = НСтр("es='Los requisitos alejados';ru='Удаленные реквизиты';en='Deleted details'")
					КонецЕсли;
		...
		Если СоздатьОписаниеДополнительныхРеквизитов Тогда
         ...
         Реквизиты.Добавить(Новый РеквизитФормы(
			...
			Если ПравоДоступа("Изменение", Метаданные.Справочники.НаборыДополнительныхРеквизитовИСведений) Тогда
				...
				Команда.Заголовок = НСтр("es='Cambiar la composición de los requisitos adicionales';ru='Изменить состав дополнительных реквизитов';en='Change the composition of additional details'");
				Команда.Действие = "Подключаемый_СвойстваВыполнитьКоманду";
				Команда.Подсказка = НСтр("es='Cambiar la composición de los requisitos adicionales';ru='Изменить состав дополнительных реквизитов';en='Change the composition of additional details'");
			...
			Команда.Заголовок   = НСтр("es='Comenzar/acabar la redacción';ru='Начать/закончить редактирование';en='Start / End Editing'");
			Команда.Действие    = "Подключаемый_СвойстваВыполнитьКоманду";
			Команда.Подсказка   = НСтр("es='Comenzar/acabar la redacción';ru='Начать/закончить редактирование';en='Start / End Editing'");
...
Процедура ПодготовитьФормуДляОтложеннойИнициализации(Форма, Контекст, ИмяЭлементаДляРазмещения, Индекс)
	...
	Если Родитель <> Неопределено
		...
		ПредупреждениеОРазмещении = НСтр("es='Para la representación de los requisitos adicionales es necesario instalar grupo ""%1"" no por el primer elemento (después de cualquier otro grupo) en grupo ""%2"" (el menú Todavía - Cambiar la forma).';ru='Для отображения дополнительных реквизитов необходимо разместить группу ""%1"" не первым элементом (после любой другой группы) в группе ""%2"" (меню Еще - Изменить форму).';en='To display additional details, it is necessary to place the group ""%1"" not the first element (after any other group) in the group ""%2"" (menu More - Change form).'");
...
Функция КлючНаборовСвойств(Наборы)
	...
	// ТОИР++
	ЭтоСправочникОбъектыРемонта = (Наборы.НайтиПоЗначению(Справочники.НаборыДополнительныхРеквизитовИСведений.Справочник_торо_ТиповыеОР_Общие)<>Неопределено
										ИЛИ Наборы.НайтиПоЗначению(Справочники.НаборыДополнительныхРеквизитовИСведений.Справочник_торо_ОбъектыРемонта_общие)<>Неопределено);
	// ТОИР--
