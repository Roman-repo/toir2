...
Функция ИсправленияРодительскихКонфигураций() Экспорт
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь(, Истина, Истина) Тогда
		...
		Результат.КраткоеОписаниеОшибки   = НСтр("es='Es insuficientemente derecho.';ru='Недостаточно прав.';en='Not enough rights.'");
		Результат.ПодробноеОписаниеОшибки = НСтр("es='Es insuficientemente derecho para la recepción de la información sobre las correcciones.';ru='Недостаточно прав для получения информации об исправлениях.';en='Not enough rights to receive corrections information.'");
		ЗаписатьОшибкуВЖурналРегистрации(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir la información sobre las correcciones accesibles (патчах) de las configuraciones paternales. "
"%1';ru='Не удалось получить информацию о доступных исправлениях (патчах) родительских конфигураций."
"%1';en='Failed to get information about available fixes (patches) of parental configurations."
"%1'"),
	...
	Если Разделы.Количество() > 0 Тогда
		...
		Для Каждого Раздел Из Разделы Цикл
			...
			Если ПараметрыОповещения.РекомендуетсяУстановить Тогда
				...
				ДелоРекомендуетсяУстановить.Представление  = НСтр("es='Se recomienda establecer esta renovación.';ru='Рекомендуется установить это обновление.';en='It is recommended to install this update.'");
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			"downloads.v8.1c.ru",
			80,
			НСтр("es='La recepción de los ficheros de las renovaciones del programa (la zona ru)';ru='Получение файлов обновлений программы (зона ru)';en='Receiving program update files (ru zone)'"));
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			"downloads.v8.1c.eu",
			80,
			НСтр("es='La recepción de los ficheros de las renovaciones del programa (la zona eu)';ru='Получение файлов обновлений программы (зона eu)';en='Receiving program update files (eu zone)'"));
		НовыеРазрешения.Добавить(Разрешение);
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			"update-api.1c.ru",
			443,
			НСтр("es='El servicio de la recepción de las renovaciones del programa (la zona ru)';ru='Сервис получения обновлений программы (зона ru)';en='Service for receiving program updates (zone ru)'"));
		НовыеРазрешения.Добавить(Разрешение);
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			"update-api.1c.eu",
			443,
			НСтр("es='El servicio de la recepción de las renovaciones del programa (la zona eu)';ru='Сервис получения обновлений программы (зона eu)';en='Service for receiving program updates (eu zone)'"));
		НовыеРазрешения.Добавить(Разрешение);
...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	Если Не НастройкиОбновления.ПолучатьИсправления Тогда
		ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("es='La instalación de las correcciones (патчей) es prohibida en los ajustes de la configuración.';ru='Установка исправлений (патчей) запрещена в настройках конфигурации.';en='Installation of patches (patches) is prohibited in the configuration settings.'"));
		Загружать = Ложь;
		Возврат;
	КонецЕсли;
	...
	Для каждого Характеристика Из Дескриптор.Properties.Property Цикл
		Если Характеристика.Code = "ДанныеВерсийПрограмм" Тогда
			...
			Для Каждого ОписаниеВерсии Из ДоступныеВерсии Цикл
				Если ОписаниеВерсии.ИмяКонфигурации = Метаданные.Имя
					И ОписаниеВерсии.ВерсияКонфигурации = Метаданные.Версия Тогда
					Загружать = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;
	...
	Если УдалитьИсправление Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La revocación de la corrección: %1';ru='Отзыв исправления: %1';en='Correction revocation:%1'"),
				Идентификатор));
...
Процедура ОбработатьНовыеДанные(Знач Дескриптор, Знач ПутьКФайлу) Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La instalación nuevo испрвления: "
"%1';ru='Установка нового испрвления:"
"%1';en='Installing a new fix:"
"%1'"),
...
Процедура ОбработкаДанныхОтменена(Знач Дескриптор) Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El tratamiento de los datos suministrados no es cumplido. "
"Los datos suministrados: "
"%1';ru='Обработка поставляемых данных не выполнена."
"Поставляемые данные:"
"%1';en='Delivery data processing failed."
"Supplied data:"
"%1'"),
			МодульПоставляемыеДанные.ПолучитьОписаниеДанных(Дескриптор)));
...
Функция ИнформацияОДоступномОбновленииВНастройках() Экспорт
	...
	Исключение
		...
		ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió recibir la información sobre la renovación accesible de los ajustes del usuario. "
"%1';ru='Не удалось получить информацию о доступном обновлении из настроек пользователя."
"%1';en='Failed to get information about available update from user settings."
"%1'"),
...
Функция ПараметрыОповещенияОДоступномОбновлении(ИнформацияОДоступномОбновлении, ДляТекущихДел = Ложь) Экспорт
	...
	Если Не ПустаяСтрока(ИнформацияОДоступномОбновлении.ИмяОшибки) Тогда
		...
		Если ИнформацияОДоступномОбновлении.ИмяОшибки = "ConnectError"
			...
			Результат = Новый Структура;
			Результат.Вставить("Текст" , НСтр("es='No consiguió comprobar la presencia de las renovaciones del programa.';ru='Не удалось проверить наличие обновлений программы.';en='Failed to check for program updates.'"));
	...
	Иначе
		...
		Если ДоступноОбновление Тогда
			...
			Результат.Текст = НСтр("es='Es accesible la renovación del programa.';ru='Доступно обновление программы.';en='Program update available.'");
			Если РазмерОбновления <> 0 Тогда
				Если ДляТекущихДел Тогда
					Результат.ПредставлениеРазмераДистрибутива = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La dimensión del distributivo: %1.';ru='Размер дистрибутива: %1.';en='Distribution size:%1.'"),
			...
			Если Результат.РекомендуетсяУстановить Тогда
				Если Не ДляТекущихДел Тогда
					Результат.Текст = Результат.Текст + Символы.ПС
						+ НСтр("es='Se recomienda establecer esta renovación.';ru='Рекомендуется установить это обновление.';en='It is recommended to install this update.'");
				КонецЕсли;
			...
			Иначе
				...
				Если УдалятьФайлыИсправлений Тогда
					...
					Исключение
						ЗаписатьОшибкуВЖурналРегистрации(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='No consiguió quitar el fichero de la corrección %1. "
"%2';ru='Не удалось удалить файл исправления %1."
"%2';en='Failed to delete patch file%1."
"%2'"),
	...
	Если ОписаниеИсправлений.Установить.Количество() = 0
		...
		ЗаписатьИнформациюВЖурналРегистрации(НСтр("es='La instalación y la desaparición de las correcciones no son necesario (todas las correcciones eran establecidas antes).';ru='Установка и удаление исправлений не требуются (все исправления были установлены ранее).';en='Installation and removal of patches are not required (all patches were installed earlier).'"));
		Возврат Результат;
	КонецЕсли;
	
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La llamada de la instalación y la desaparición de las correcciones (БСП). "
"Las correcciones para la instalación: "
"%1 "
""
"Correcciones para la desaparición (llamado): "
"%2 "
""
"son establecidos Ya (son excluidos de la instalación): "
"%3.';ru='Вызов установки и удаления исправлений (БСП)."
...
"%3.';en='Call install and uninstall patches (BSP)."
"Fixes for installation:"
"%1"
""
"Corrections for removal (revoked):"
"% 2"
""
...
"% 3.'"),
	...
	Если РезультатУстановки.НеУстановлено > 0 Или РезультатУстановки.НеУдалено > 0 Тогда
		...
		Если РезультатУстановки.НеУстановлено > 0 Тогда
			...
			СообщениеОбОшибке = СообщениеОбОшибке + ?(ПустаяСтрока(СообщениеОбОшибке), "", Символы.ПС)
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No consiguió establecer las correcciones (%1 piezas).';ru='Не удалось установить исправления (%1 шт.).';en='Failed to install fixes (%1 pc.).'"),
		...
		Если РезультатУстановки.НеУдалено > 0 Тогда
			...
			СообщениеОбОшибке = СообщениеОбОшибке + ?(ПустаяСтрока(СообщениеОбОшибке), "", Символы.ПС)
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No consiguió quitar las correcciones (%1 piezas).';ru='Не удалось удалить исправления (%1 шт.).';en='Failed to remove fixes (%1 pc.).'"),
	...
	Иначе
		...
		ЗаписатьИнформациюВЖурналРегистрации(НСтр("es='La instalación y la desaparición de las correcciones son con éxito acabadas.';ru='Установка и удаление исправлений успешно завершены.';en='Installation and uninstallation of patches completed successfully.'"));
...
Процедура ЗагрузитьИУстановитьОбновленияВФоновомРежиме(Параметры, АдресРезультата) Экспорт
	...
	Если Не ДоступноИспользованиеОбновленияПрограммы(Истина, Ложь) Тогда
		ВызватьИсключение НСтр("es='El uso de la renovación del programa es inaccesible en el régimen corriente del trabajo.';ru='Использование обновления программы недоступно в текущем режиме работы.';en='Using the program update is not available in the current mode of operation.'");
	КонецЕсли;
	...
	Если ДанныеАутентификации = Неопределено Тогда
		...
		Контекст.Вставить("Сообщение", НСтр("es='аутентификации.';ru='Не заполнены данные аутентификации.';en='Authentication data is not filled.'"));
	...
	Если Контекст.ОбновитьПлатформу Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La recepción de la renovación de la plataforma 1С:Предприятие: la versión %1; URL: %2';ru='Получение обновления платформы 1С:Предприятие: версия %1; URL: %2';en='Obtaining platform 1C: Enterprise platform update: version%1; URL:%2'"),
		...
		Если Не ПустаяСтрока(Контекст.КаталогДистрибутиваПлатформы) Тогда
			...
			ЗаписатьИнформациюВЖурналРегистрации(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La renovación de la plataforma 1С:Предприятие era cargada ya antes en %1';ru='Обновление платформы 1С:Предприятие уже было загружено ранее в %1';en='Platform update 1C: Enterprise has already been downloaded earlier in%1'"),
		...
		Иначе
			...
			Если КаталогУстановкиПлатформы <> Неопределено Тогда
				...
				ЗаписатьИнформациюВЖурналРегистрации(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La versión de %1 plataformas 1С:Предприятие es establecida ya en el ordenador.';ru='Версия %1 платформы 1С:Предприятие уже установлена на компьютере.';en='Version 1 of the 1C: Enterprise platform is already installed on the computer.'"),
						Контекст.ВерсияПлатформы));
		...
		Если Не ТекИсправление.Получено Тогда
			...
			Иначе
				...
				Исключение
					...
					СообщениеЖурнала =
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='La falta a la recepción del fichero de la corrección (патча) (%1). "
"No consiguió conservar el fichero en el catálogo local. "
"%2';ru='Ошибка при получении файла исправления (патча) (%1)."
"Не удалось сохранить файл в локальном каталоге."
"%2';en='Error getting patch file (%1)."
"Failed to save file to local directory."
"%2 '"),
							ТекИсправление.URLФайла,
							ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
					ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
				...
				ЗаписатьИнформациюВЖурналРегистрации(НСтр("es='El fichero de la corrección (патча) es con éxito recibido.';ru='Файл исправления (патча) успешно получен.';en='The patch file was successfully received.'"));
	...
	Если Контекст.ОбновленияКонфигурации.Количество() = 0
		...
		Контекст.ТекущееДействие = НСтр("es='La instalación de las correcciones (патчей)';ru='Установка исправлений (патчей)';en='Installing fixes (patches)'");
	...
	Если Контекст.ОбновитьПлатформу И Не Контекст.ОбновлениеПлатформыУстановлено Тогда
		...
		Контекст.ТекущееДействие = НСтр("es='La instalación de la plataforma 1С:Предприятие';ru='Установка платформы 1С:Предприятие';en='Installing of the 1C: Enterprise platform'");
...
Процедура ЗагрузитьОбновлениеПлатформы(Контекст, Параметры, ДанныеАутентификации)
	...
	Исключение
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta a la creación del catálogo para la conservación del distributivo (%1).';ru='Ошибка при создании каталога для сохранения дистрибутива (%1).';en='Error creating directory to save distribution (%1).'"),
				КаталогДистрибутива)
			+ Символы.ПС
			+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
		...
		Контекст.ИнформацияОбОшибке = СообщениеЖурнала;
		Контекст.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió crear el catálogo %1 para la conservación del distributivo. %2';ru='Не удалось создать каталог %1 для сохранения дистрибутива. %2';en='Failed to create directory%1 to save distribution. %2'"),
			КаталогДистрибутива,
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		Возврат;
	...
	Исключение
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta a la creación del catálogo para la carga del distributivo (%1).';ru='Ошибка при создании каталога для загрузки дистрибутива (%1).';en='An error occurred while creating the directory for downloading the distribution kit (%1).'"),
				КаталогДляЗагрузки)
			+ Символы.ПС
			+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La recepción del fichero de la renovación de la plataforma 1С:Предприятие: %1';ru='Получение файла обновления платформы 1С:Предприятие: %1';en='Getting the platform 1C: Enterprise update file:%1'"),
	...
	Если Не ПустаяСтрока(РезультатПолучения.КодОшибки) Тогда
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta a la recepción del fichero del distributivo de la plataforma 1С:Предприятие (%1). %2';ru='Ошибка при получении файла дистрибутива платформы 1С:Предприятие (%1). %2';en='An error occurred while getting the 1C: Enterprise platform distribution file (%1). %2'"),
				Контекст.URLФайлаОбновленияПлатформы,
		...
		Контекст.ИнформацияОбОшибке = СообщениеЖурнала;
		Контекст.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la recepción del fichero del distributivo. %1';ru='Ошибка при получении файла дистрибутива. %1';en='Error getting distribution file. %1'"),
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El fichero de la renovación de la plataforma 1С:Предприятие es con éxito recibido: %1';ru='Файл обновления платформы 1С:Предприятие успешно получен: %1';en='Platform update file 1C: Enterprise successfully received:%1'"),
			ПутьПолученногоФайла));
	...
	Попытка
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La extracción del distributivo de la plataforma 1С:Предприятие en %1';ru='Извлечение дистрибутива платформы 1С:Предприятие в %1';en='Extracting the distribution platform 1C: Enterprise in %1'"),
				КаталогДистрибутива));
	...
	Исключение
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta a la extracción de los ficheros del archivo (%1) en el catálogo %2.';ru='Ошибка при извлечении файлов архива (%1) в каталог %2.';en='Error while extracting archive files (%1) to the%2 directory.'"),
				ПутьПолученногоФайла,
				КаталогДистрибутива)
			+ Символы.ПС
			+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		...
		Контекст.ИнформацияОбОшибке = СообщениеЖурнала;
		Контекст.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió sacar los ficheros del distributivo. %1';ru='Не удалось извлечь файлы дистрибутива. %1';en='Failed to extract distribution files. %1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El distributivo de la plataforma 1С:Предприятие es con éxito conservado en %1';ru='Дистрибутив платформы 1С:Предприятие успешно сохранен в %1';en='Distribution platform 1C: Enterprise successfully saved in%1'"),
			КаталогДистрибутива));
...
Процедура УстановитьОбновлениеПлатформы(Контекст, Параметры, УстановкаОтменена)
	...
	Если ПолучениеОбновленийПрограммыКлиентСервер.ФайлСуществует(ПутьФайлаПротокола) Тогда
		...
		Исключение
			ЗаписатьОшибкуВЖурналРегистрации(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La falta a la desaparición del fichero de acta (%1). %2';ru='Ошибка при удалении файла протокола (%1). %2';en='Error deleting log file (%1). %2'"),
					ПутьФайлаПротокола,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
		КонецПопытки;
	КонецЕсли;
	...
	Если Не ПолучениеОбновленийПрограммыКлиентСервер.ФайлСуществует(ПутьФайлаПрограммыУстановки) Тогда
		...
		СообщениеЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es cargado el distributivo incorrecto de la plataforma 1С:Предприятие. Falta el fichero setup.exe (%1).';ru='Загружен некорректный дистрибутив платформы 1С:Предприятие. Отсутствует файл setup.exe (%1).';en='Invalid 1C: Enterprise platform distribution downloaded. Missing setup.exe file (%1).'"),
			ПутьФайлаПрограммыУстановки);
		ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
		
		Контекст.ИмяОшибки = "НекорректныйДистрибутивПлатформы";
		Контекст.ИнформацияОбОшибке = СообщениеЖурнала;
		Контекст.Сообщение = НСтр("es='Es cargado el distributivo incorrecto de la plataforma 1С:Предприятие. Falta el fichero setup.exe.';ru='Загружен некорректный дистрибутив платформы 1С:Предприятие. Отсутствует файл setup.exe.';en='Invalid 1C: Enterprise platform distribution downloaded. The setup.exe file is missing.'");
		Возврат;
	...
	Попытка
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La instalación de la nueva versión de la plataforma 1С:Предприятие (%1). %2';ru='Установка новой версии платформы 1С:Предприятие (%1). %2';en='nstalling a new version of the 1C: Enterprise platform (%1). %2'"),
				Контекст.ВерсияПлатформы,
				КомандаЗапуска));
	...
	Исключение
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta durante el lanzamiento del programa de la instalación de la plataforma 1С:Предприятие (%1). %2';ru='Ошибка при запуске программы установки платформы 1С:Предприятие (%1). %2';en='Error while starting the 1C: Enterprise platform installation program (%1). %2'"),
				КомандаЗапуска,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Контекст.ИнформацияОбОшибке = СообщениеЖурнала;
		Контекст.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta durante el lanzamiento del programa de la instalación de la plataforma 1С:Предприятие. %1';ru='Ошибка при запуске программы установки платформы 1С:Предприятие. %1';en='Error while starting 1C: Enterprise platform installation program. %1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	...
	Если КодВозврата = 0 Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(НСтр("es='Una nueva versión de la plataforma 1С:Предприятие es con éxito establecida.';ru='Новая версия платформы 1С:Предприятие успешно установлена.';en='The new version of the 1C: Enterprise platform has been successfully installed.'"));
	...
	Иначе
		...
		Иначе
			...
			СообщениеЖурнала =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La instalación de la plataforma 1С:Предприятие es acabada con la falta. "
"La versión %1; "
"el Código del regreso: %2; "
"la Orden: %3';ru='Установка платформы 1С:Предприятие завершена с ошибкой."
"Версия %1;"
"Код возврата: %2;"
"Команда: %3';en='Installing the 1C: Enterprise platform completed with an error."
"Version%1;"
"Return Code:%2;"
"Command:%3'"),
					Контекст.ВерсияПлатформы,
			...
			ШаблонСообщенияПользователю =
				НСтр("es='<body> a la instalación de la nueva versión de la plataforma 1С:Предприятие había una falta. "
"<br/> el código del regreso: %1.';ru='<body>При установке новой версии платформы 1С:Предприятие произошла ошибка."
"<br />Код возврата: %1.';en='<body> An error occurred while installing the new version of the 1C: Enterprise platform."
"<br /> Return code:% 1.'");
			
			Если ПолучениеОбновленийПрограммыКлиентСервер.ФайлСуществует(ПутьФайлаПротокола) Тогда
				ШаблонСообщенияПользователю = ШаблонСообщенияПользователю + Символы.ПС
					+ НСтр("es='<br/> la información Técnica contiene en <a href = ""open:debuglog""> la acta de la instalación </a>.';ru='<br />Техническая информация содержится в <a href=""open:debuglog"">протоколе установки</a>.';en='<br /> Technical information is contained in the <a href=""open:debuglog""> installation protocol </a>.'");
			КонецЕсли;
			...
			Если Параметры.РежимУстановкиПлатформы = 0
				...
				ШаблонСообщенияПользователю = ШаблонСообщенияПользователю + Символы.ПС
					+ НСтр("es='<br/> <br/> <p> la falta Dada es vinculada a las restricciones de sistema el político de la seguridad. "
"<br/> se Recomienda <a href = ""action:retruupdateplatfom""> establecer la plataforma con los ajustes de mano </a>, o poner en marcha "
"<br/> el programa en nombre del administrador. </p>';ru='<br /><br /><p>Данная ошибка связана с ограничениями системных политик безопасности."
"<br />Рекомендуется <a href=""action:retruupdateplatfom"">установить платформу с ручными настройками</a>, либо запустить"
"<br />программу от имени администратора.</p>';en='<br /> <br /> <p> This error is due to system security policy restrictions."
"<br /> It is recommended that you <a href=""action:retruupdateplatfom""> install the platform with manual settings </a>, or run"
"<br /> program as administrator. </p>'");
			КонецЕсли;
			
			ШаблонСообщенияПользователю = ШаблонСообщенияПользователю + Символы.ПС
				+ НСтр("es='<br/> <br/> Durante el surgimiento de los problemas escriban en <a href = ""mailto:webits-info@1c.ru""> el apoyo técnico </a>. </body>';ru='<br /><br />При возникновении проблем напишите в <a href=""mailto:webits-info@1c.ru"">техподдержку</a>.</body>';en='<br /> <br /> If you have problems, write to <a href=""mailto:webits-info@1c.ru""> technical support </a>. </body>'");
...
Процедура ЗагрузитьОбновлениеКонфигурации(Обновление, Контекст, Параметры, ДанныеАутентификации)
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La carga del fichero de la renovación de la configuración %1';ru='Загрузка файла обновления конфигурации %1';en='Download configuration update file%1'"),
	...
	ЗаписатьИнформациюВЖурналРегистрации(НСтр("es='El fichero de la renovación es con éxito cargado.';ru='Файл обновления успешно загружен.';en='Update file downloaded successfully.'"));
...
Процедура РегламентноеЗаданиеПолучениеИУстановкаИсправленийКонфигурации() Экспорт
	...
	Если Не ПараметрыОбновления.ПолучатьИсправления Тогда
		ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("es='El uso de la recepción de las correcciones es inaccesible en el régimen corriente del trabajo. "
"La tarea de reglamento ПолучениеИУстановкаИсправленийКонфигурации es desconectada.';ru='Использование получения исправлений недоступно в текущем режиме работы."
"Регламентное задание ПолучениеИУстановкаИсправленийКонфигурации отключено.';en='Using fixes is not available in the current mode of operation. Регламентное задание ПолучениеИУстановкаИсправленийКонфигурации is turned off.'"));
		РегламентныеЗаданияСервер.УстановитьИспользованиеРегламентногоЗадания(
			МетаданныеЗадание,
			Ложь);
		Возврат;
	...
	Если Задание <> Неопределено Тогда
		...
		Если ПериодПовтораВТечениеДня > 0 И ПериодПовтораВТечениеДня < 3600 Тогда
			ВызватьИсключение НСтр("es='El intervalo de la instalación automática no puede ser más a menudo, que una vez por hora.';ru='Интервал автоматической установки не может быть чаще, чем один раз в час.';en='The automatic installation interval cannot be more often than once per hour.'");
		КонецЕсли;
	КонецЕсли;
	...
	УстановитьПривилегированныйРежим(Ложь);
	Если ДанныеАутентификации = Неопределено Тогда
		ЗаписатьИнформациюВЖурналРегистрации(НСтр("es='El apoyo de Internet no es conectado. "
"La ejecución de la tarea de la recepción automática y la instalación es interrumpida.';ru='Интернет-поддержка не подключена."
"Выполнение задания автоматического получения и установки прервано.';en='Online support is not connected."
"The automatic receive and install job has been aborted. '"));
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьИУстановитьИсправленияДляТекущейВерсииКонфигурации(ДанныеАутентификации, ВФоне)
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("es='La recepción automática y la instalación de las correcciones (патчей) para la versión corriente de la configuración';ru='Автоматическое получение и установка исправлений (патчей) для текущей версии конфигурации';en='Automatically receiving and installing patches for the current version of the configuration'"));
	...
	Если ИнформацияОбИсправлениях.Ошибка Тогда
		ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió automáticamente recibir y establecer las correcciones (патчи) para la versión corriente de la configuración. "
"No consiguió recibir la información sobre las correcciones accesibles. "
"%1';ru='Не удалось автоматически получить и установить исправления (патчи) для текущей версии конфигурации."
"Не удалось получить информацию о доступных исправлениях."
"%1';en='Failed to automatically obtain and install patches for the current version of the configuration."
"Failed to get information about available fixes."
"%1'"),
	...
	Если Исправления = Неопределено Или Исправления.Количество() = 0 Тогда
		ЗаписатьИнформациюВЖурналРегистрации(НСтр("es='No hay correcciones accesibles (патчей) para la instalación';ru='Нет доступных исправлений (патчей) для установки';en='No patches available for installation'"));
		Возврат;
	КонецЕсли;
...
"%1'"),
	...
	ЗаписатьИнформациюВЖурналРегистрации(НСтр("es='Es acabada la recepción automática y la instalación de las correcciones (патчей)';ru='Завершено автоматическое получение и установка исправлений (патчей)';en='Completed the automatic receipt and installation of patches (patches)'"));
			...
			Если ИмяСвойства = "patchUeid" Тогда
				...
				Исключение
					ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El identificador incorrecto de la corrección (патча) en patchUpdateList [].uin (%1)';ru='Некорректный идентификатор исправления (патча) в patchUpdateList[].uin (%1)';en='Invalid correction (patch) identifier in patchUpdateList []. uin (%1)'"),
						ЗначениеСвойства);
...
Функция ИнформацияОДоступномОбновленииСлужебная(
		...
		Если ВерсииКонфигураций.Количество() > 0 Тогда
			...
			Если РезультатИнформацияОбИсправлениях.Ошибка Тогда
				...
				Результат.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No consiguió recibir la información sobre las correcciones (патчах). %1';ru='Не удалось получить информацию об исправлениях (патчах). %1';en='Failed to get information about corrections (patches). %one'"),
	...
	Если СценарийОбновления = "РабочееОбновление" Тогда
		ПолучениеОбновленийПрограммыВызовСервера.СохранитьИнформациюОДоступномОбновленииВНастройках(Результат);
	КонецЕсли;
...
Процедура ИнформацияОДоступномОбновленииВызовОперации(
	...
	Если ИмяТекущейПрограммы = "Unknown" Тогда
		...
		Результат.Сообщение = НСтр("es='Los parámetros equivocados de la conexión al servicio.';ru='Неверные параметры подключения к сервису.';en='Incorrect service connection settings.'");
		Результат.ИнформацияОбОшибке =
			НСтр("es='No consiguió recibir la información sobre la renovación accesible. No es determinado el nombre del programa en el método "
"de PriOpredeleniiImeniProgrammy () el módulo general ИнтернетПоддержкаПользователейПереопределяемый.';ru='Не удалось получить информацию о доступном обновлении."
"Не определено имя программы в методе ПриОпределенииИмениПрограммы() общего модуля ИнтернетПоддержкаПользователейПереопределяемый.';en='Failed to get information about available update."
"The program name is not defined in the ПриОпределенииИмениПрограммы() method of the Internet general module. ИнтернетПоддержкаПользователейПереопределяемый.'");
	...
	Если Не ПустаяСтрока(РезультатПроверки.ИмяОшибки) Тогда
		...
		Результат.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir la información sobre la renovación accesible. "
"No consiguió comprobar la accesibilidad del servicio de la renovación automática del programa: %1. "
"La causa: "
"%2';ru='Не удалось получить информацию о доступном обновлении."
"Не удалось проверить доступность сервиса автоматического обновления программы: %1."
"Причина:"
"%2';en='Failed to get information about available update."
"Failed to check the availability of the automatic program update service: %1."
"Cause:"
"%2 '"),
			URLОперацииPing,
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La recepción de la información sobre la renovación accesible (%1).';ru='Получение информации о доступном обновлении (%1).';en='Getting information about an available update (%1).'"),
			URLОперации)
		+ Символы.ПС
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El nombre del programa corriente: %1; "
"la Versión del programa corriente: %2; "
"el Nombre del nuevo programa: %3; "
"el Número de la redacción del nuevo programa: %4; "
"el Guión de la renovación: %5; "
"los parámetros Adicionales: "
"%7';ru='Имя текущей программы: %1;"
"Версия текущей программы: %2;"
"Имя новой программы: %3;"
"Номер редакции новой программы: %4;"
"Сценарий обновления: %5;"
"Дополнительные параметры:"
"%7';en='Name of current program:%1;"
"Current program version:%2;"
"New program name:%3;"
"Edition number of the new program:%4;"
"Update script:%5;"
"Extra options:"
"%7'"),
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		Результат.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la recepción de la información sobre la renovación accesible. "
"%1';ru='Ошибка при получении информации о доступном обновлении."
"%1';en='Error getting information about an available update."
"%1'"),
		...
		Результат.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir la información sobre la renovación accesible. "
"%1';ru='Не удалось получить информацию о доступном обновлении."
"%1';en='Failed to get information about available update."
"%1'"),
	...
	Исключение
		...
		Результат.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió comprobar la presencia de las renovaciones del programa. "
"La falta al tratamiento de la respuesta del servicio. "
"La respuesta incorrecta del servicio. "
"%1 "
"Cuerpo de la respuesta: %2';ru='Не удалось проверить наличие обновлений программы."
"Ошибка при обработке ответа сервиса."
"Некорректный ответ сервиса."
"%1"
"Тело ответа: %2';en='Failed to check for program updates."
...
"Response body:%2'"),
		...
		Результат.Сообщение =
			НСтр("es='No consiguió comprobar la presencia de las renovaciones del programa. "
"La respuesta incorrecta del servicio.';ru='Не удалось проверить наличие обновлений программы."
"Некорректный ответ сервиса.';en='Failed to check for program updates."
"Invalid service response. '");
	...
	Если Не ПустаяСтрока(Результат.ИмяОшибки) Тогда
		...
		ЗаписатьОшибкуВЖурналРегистрации(
			...
			НСтр("es='No consiguió comprobar la presencia de las renovaciones del programa. "
""
"El servicio ha informado sobre la falta. "
""
"La respuesta del servicio: %1';ru='Не удалось проверить наличие обновлений программы."
""
"Сервис сообщил об ошибке."
""
"Ответ сервиса: %1';en='Failed to check for program updates."
""
...
"Service response:%1 '"),
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es recibida la información sobre la renovación accesible. "
"%1';ru='Получена информация о доступном обновлении."
"%1';en='Received information about an available update."
"%1'"),
...
Функция ОписаниеФайловОбновлений(
	...
	Если Не ПустаяСтрока(РезультатПроверки.ИмяОшибки) Тогда
		...
		Результат.Сообщение = РезультатПроверки.СообщениеОбОшибке;
		Результат.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir la información sobre los ficheros de la renovación. "
"No consiguió comprobar la accesibilidad del servicio de la renovación automática del programa: %1. "
"La causa: "
"%2';ru='Не удалось получить информацию о файлах обновления."
"Не удалось проверить доступность сервиса автоматического обновления программы: %1."
"Причина:"
"%2';en='Failed to get update file information."
"Failed to check the availability of the automatic program update service:%1."
"Cause:"
"%2'"),
			URLОперацииPing,
...
"%2'"),
		...
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El login: %1 "
"Renovación de la configuración: "
"%2; "
"la Renovación de la plataforma: %3; "
"los parámetros Adicionales: "
"%5';ru='Логин: %1"
...
"%5'"),
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		Результат.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la recepción de la información sobre los ficheros de la renovación. "
"%1';ru='Ошибка при получении информации о файлах обновления."
"%1';en='Error getting update file information."
"%1'"),
			РезультатОтправки.ИнформацияОбОшибке);
		ЗаписатьОшибкуВЖурналРегистрации(Результат.ИнформацияОбОшибке);
		...
		Результат.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir la información sobre los ficheros de la renovación. "
"%1';ru='Не удалось получить информацию о файлах обновления."
"%1';en='Failed to get update file information."
"%1'"),
			РезультатОтправки.СообщениеОбОшибке);
	...
	Исключение
		...
		Результат.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir la información sobre la renovación del programa. "
"La falta al tratamiento de la respuesta del servicio. "
"La respuesta incorrecta del servicio. "
"%1 "
"Cuerpo de la respuesta: %2';ru='Не удалось получить информацию об обновлении программы."
"Ошибка при обработке ответа сервиса."
"Некорректный ответ сервиса."
"%1"
"Тело ответа: %2';en='Failed to get program update information."
"Error processing service response."
"Invalid service response."
"%1"
"Response body:%2 '"),
		...
		Результат.Сообщение =
			НСтр("es='No consiguió recibir la información sobre la renovación del programa. "
"La respuesta incorrecta del servicio.';ru='Не удалось получить информацию об обновлении программы."
"Некорректный ответ сервиса.';en='Failed to get program update information."
"Invalid service response.'");
...
"El servicio ha informado sobre la falta. "
""
"La respuesta del servicio: %1';ru='Не удалось получить информацию об обновлении программы."
""
"Сервис сообщил об ошибке."
""
"Ответ сервиса: %1';en='Failed to get program update information."
""
"The service reported an error."
""
"Service response:%1 '"),
...
"%1'"),
	...
	Если ОбновлениеКонфигурации <> Неопределено Тогда
		Если ОбновлениеКонфигурации.ФайлыДляЗагрузки.Количество() <> Результат.ОбновленияКонфигурации.Количество() Тогда
			Результат.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es devuelta la información incompleta sobre la renovación de la configuración. "
"No coincide la cantidad de los ficheros pedidos y devueltos (%1 y %2).';ru='Возвращена неполная информация об обновлении конфигурации."
"Не совпадает количество запрошенных и возвращенных файлов (%1 и %2).';en='Incomplete configuration update information returned."
"The number of requested and returned files (% 1 and% 2) does not match.""'"),
	...
	Если ПроверитьДоступностьСервиса Тогда
		...
		Если Не ПустаяСтрока(РезультатПроверки.ИмяОшибки) Тогда
			...
			Результат.ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió recibir la información sobre las correcciones accesibles (патчах) de las configuraciones. "
"No consiguió comprobar la accesibilidad del servicio de la renovación automática del programa: %1. "
"La causa: "
"%2';ru='Не удалось получить информацию о доступных исправлениях (патчах) конфигураций."
"Не удалось проверить доступность сервиса автоматического обновления программы: %1."
"Причина:"
"%2';en='Failed to get information about available configuration fixes (patches)."
"Failed to check the availability of the automatic program update service: %1."
"Cause:"
"%2'"),
				URLОперацииPing,
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La recepción de la información sobre las correcciones (патчах) de las configuraciones (%1).';ru='Получение информации об исправлениях (патчах) конфигураций (%1).';en='Getting information about corrections (patches) of configurations (%1).'"),
			URLОперации)
		+ Символы.ПС
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Las versiones de los subsistemas: "
"%1; "
"ИдентификаторыУстановленныхИсправлений: "
"%2; "
"los parámetros Adicionales: "
"%3';ru='Версии подсистем:"
"%1;"
"ИдентификаторыУстановленныхИсправлений:"
"%2;"
"Дополнительные параметры:"
"%3';en='Subsystem versions:"
"%1;"
...
"% 3'"),
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		Результат.Ошибка = Истина;
		Результат.ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la recepción de la información sobre las correcciones accesibles (патчах) de las configuraciones. "
"%1';ru='Ошибка при получении информации о доступных исправлениях (патчах) конфигураций."
"%1';en='Error getting information about available configuration fixes (patches)."
"%1'"),
			РезультатОтправки.ИнформацияОбОшибке);
		ЗаписатьОшибкуВЖурналРегистрации(Результат.ПодробноеОписаниеОшибки);
		...
		Результат.КраткоеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir la información sobre las correcciones accesibles (патчах) de las configuraciones. "
"%1';ru='Не удалось получить информацию о доступных исправлениях (патчах) конфигураций."
"%1';en='Failed to get information about available configuration fixes (patches)."
"%1'"),
			РезультатОтправки.СообщениеОбОшибке);
	...
	Исключение
		...
		Результат.ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió comprobar la presencia de las correcciones (патчей) de las configuraciones. "
"La falta al tratamiento de la respuesta del servicio. "
"La respuesta incorrecta del servicio. "
"%1 "
"Cuerpo de la respuesta: %2';ru='Не удалось проверить наличие исправлений (патчей) конфигураций."
"Ошибка при обработке ответа сервиса."
"Некорректный ответ сервиса."
"%1"
"Тело ответа: %2';en='Failed to check for fixes (patches) configurations."
"Error processing service response."
"Invalid service response."
"%1"
"Response body:%2 '"),
		...
		Результат.КраткоеОписаниеОшибки =
			НСтр("es='No consiguió comprobar la presencia de las correcciones (патчей) de las configuraciones. "
"La respuesta incorrecta del servicio.';ru='Не удалось проверить наличие исправлений (патчей) конфигураций."
"Некорректный ответ сервиса.';en='Failed to check for fixes (patches) configurations."
"Invalid service response.'");
...
"El servicio ha informado sobre la falta. "
""
"La respuesta del servicio: %1';ru='Не удалось проверить наличие исправлений (патчей) конфигураций."
""
"Сервис сообщил об ошибке."
""
"Ответ сервиса: %1';en='Failed to check for fixes (patches) configurations."
""
"The service reported an error."
""
"Service response:%1'"),
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es recibida la información sobre las correcciones accesibles (патчах) de las configuraciones. "
"%1';ru='Получена информация о доступных исправлениях (патчах) конфигураций."
"%1';en='Received information about available fixes (patches) of configurations."
"%1'"),
			РезультатОтправки.Содержимое));
	...
	Пока ЧтениеJSONПрочитать(ЧтениеОтвета, ТекущийУровень) Цикл
		...
		ИначеЕсли ЧтениеОтвета.ТипТекущегоЗначения = ТипЗначенияJSON.ИмяСвойства И ТекущийУровень = 2 Тогда
			...
			Если ИмяСвойства = "ueid" Тогда
				...
				Исключение
					ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El identificador incorrecto de la corrección en patchUpdateList [].ueid (%1)';ru='Некорректный идентификатор исправления в patchUpdateList[].ueid (%1)';en='Invalid patch identifier in patchUpdateList []. ueid (%1)'"),
						ЗначениеСвойства);
					ВызватьИсключение ТекстИсключения;
				КонецПопытки;
...
Функция ОписаниеФайловИсправлений(ИдентификаторыИсправлений, ДанныеАутентификации, ПроверитьДоступностьСервиса, ПараметрыКлиента)
	...
	Если ПроверитьДоступностьСервиса Тогда
		...
		Если Не ПустаяСтрока(РезультатПроверки.ИмяОшибки) Тогда
			...
			Результат.Сообщение = РезультатПроверки.СообщениеОбОшибке;
			Результат.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió recibir la información sobre los ficheros de las correcciones (патчах). "
"No consiguió comprobar la accesibilidad del servicio de la renovación automática del programa: %1. "
"La causa: "
"%2';ru='Не удалось получить информацию о файлах исправлений (патчах)."
"Не удалось проверить доступность сервиса автоматического обновления программы: %1."
"Причина:"
"%2';en='Failed to get information about files of corrections (patch)."
"Failed to check the availability of the automatic program update service:%1."
"Cause:"
"%2 '"),
				URLОперацииPing,
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La recepción de la información sobre los ficheros de las correcciones (патчей) (%1).';ru='Получение информации о файлах исправлений (патчей) (%1).';en='Getting information about patch files (%1).'"),
			URLОперации)
		+ Символы.ПС
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El login: "
"%1; "
"los Identificadores de las correcciones: "
"%2; "
"los parámetros Adicionales: "
"%3';ru='Логин:"
"%1;"
...
"%3'"),
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		Результат.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la recepción de la información sobre los ficheros de las correcciones (патчах). "
"%1';ru='Ошибка при получении информации о файлах исправлений (патчах)."
"%1';en='Error getting information about patch files."
"%1'"),
			РезультатОтправки.ИнформацияОбОшибке);
		ЗаписатьОшибкуВЖурналРегистрации(Результат.ИнформацияОбОшибке);
		...
		Результат.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir la información sobre los ficheros de las correcciones (патчах). "
"%1';ru='Не удалось получить информацию о файлах исправлений (патчах)."
"%1';en='Failed to get information about files of corrections (patch)."
"%1'"),
			РезультатОтправки.СообщениеОбОшибке);
...
"La respuesta incorrecta del servicio. "
"%1 "
"Cuerpo de la respuesta: %2';ru='Не удалось получить информацию о файлах исправлений (патчах)."
"Ошибка при обработке ответа сервиса."
"Некорректный ответ сервиса."
"%1"
"Тело ответа: %2';en='Failed to get information about files of corrections (patches)."
"Error processing service response."
"Invalid service response."
"%1"
"Response body:%2'"),
		...
		Результат.Сообщение =
			НСтр("es='No consiguió recibir la información sobre los ficheros de las correcciones (патчах). "
"La respuesta incorrecta del servicio.';ru='Не удалось получить информацию о файлах исправлений (патчах)."
"Некорректный ответ сервиса.';en='Failed to get information about files of corrections (patches)."
"Invalid service response.'");
...
"El servicio ha informado sobre la falta. "
""
"La respuesta del servicio: %1';ru='Не удалось получить информацию о файлах исправлений (патчах)."
""
"Сервис сообщил об ошибке."
""
"Ответ сервиса: %1';en='Failed to get information about patch files."
""
"The service reported an error."
""
"Service response: %1'"),
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es recibida la información sobre los ficheros de las correcciones (патчах). "
"%1';ru='Получена информация о файлах исправлений (патчах)."
"%1';en='Received information about patch files."
"%1'"),
			РезультатОтправки.Содержимое));
...
Функция СлужебнаяЗагрузитьИсправления(ИдентификаторыИсправлений, ИдентификаторФормы = Неопределено, ПроверитьДоступностьСервиса = Истина)
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь(, Истина, Истина) Тогда
		Результат.Ошибка = Истина;
		Результат.КраткоеОписаниеОшибки   = НСтр("es='Es insuficientemente derecho.';ru='Недостаточно прав.';en='Not enough rights.'");
		Результат.ПодробноеОписаниеОшибки = НСтр("es='Es insuficientemente derecho para la recepción de los ficheros de las correcciones (патчей).';ru='Недостаточно прав для получения файлов исправлений (патчей).';en='Not enough rights to receive patch files.'");
		ЗаписатьОшибкуВЖурналРегистрации(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir los ficheros de las correcciones (патчей). "
"%1';ru='Не удалось получить файлы исправлений (патчей)."
"%1';en='Failed to get corrections files (patches). "
"%1'"),
			Результат.ПодробноеОписаниеОшибки));
		Возврат Результат;
	КонецЕсли;
	...
	Если ДанныеАутентификации = Неопределено Тогда
		Результат.Ошибка = Истина;
		Результат.КраткоеОписаниеОшибки   = НСтр("es='El apoyo de Internet de los usuarios no es conectado.';ru='Интернет-поддержка пользователей не подключена.';en='Online user support is not connected.'");
		Результат.ПодробноеОписаниеОшибки = НСтр("es='El apoyo de Internet de los usuarios no es conectado.';ru='Интернет-поддержка пользователей не подключена.';en='Online user support is not connected.'");
		ЗаписатьОшибкуВЖурналРегистрации(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir los ficheros de las correcciones (патчей). "
"%1';ru='Не удалось получить файлы исправлений (патчей)."
"%1';en='Failed to get corrections files (patches). "
...
"%1'"),
	...
	Для Каждого ТекИсправление Из РезультатПолученияОписанияФайлов.Исправления Цикл
		...
		Если РезультатПолучения.Ошибка Тогда
			...
			Результат.Ошибка = Истина;
			Результат.КраткоеОписаниеОшибки =
				НСтр("es='No consiguió recibir el fichero de la corrección (патча).';ru='Не удалось получить файл исправления (патча).';en='Failed to get the corrections file (patch).'") + Символы.ПС + РезультатПолучения.КраткоеОписаниеОшибки;
			Результат.ПодробноеОписаниеОшибки =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No consiguió cargar el fichero de la corrección (патча) con el identificador %1. "
"%2';ru='Не удалось загрузить файл исправления (патча) с идентификатором %1."
"%2';en='Failed to load patch (patch) file with identifier%1."
"%2'"),
					ТекИсправление.Идентификатор,
					РезультатПолучения.ИнформацияОбОшибке);
			Возврат Результат;
...
Функция ЗагрузитьФайлИсправления(URLФайла, Идентификатор, ДанныеАутентификации) Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La recepción del fichero de la corrección (патча). "
"URL: %1; "
"el Identificador: %2.';ru='Получение файла исправления (патча)."
"URL: %1;"
"Идентификатор: %2.';en='Getting the patch file."
"URL:%1;"
"ID:%2. '"),
			URLФайла,
			Идентификатор));
	...
	Если Не ПустаяСтрока(РезультатПолучения.КодОшибки) Тогда
		Результат.Ошибка = Истина;
		Результат.КраткоеОписаниеОшибки =
			НСтр("es='No consiguió recibir el fichero de la corrección (патча).';ru='Не удалось получить файл исправления (патча).';en='Failed to get the corrections file (patch).'") + Символы.ПС + РезультатПолучения.СообщениеОбОшибке;
		Результат.ПодробноеОписаниеОшибки =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió cargar el fichero de la corrección (патча) con el identificador %1. "
"%2';ru='Не удалось загрузить файл исправления (патча) с идентификатором %1."
...
"%2'"),
				...
				РезультатПолучения.ИнформацияОбОшибке);
		ЗаписатьОшибкуВЖурналРегистрации(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir el fichero de la corrección (патча). "
"El identificador: %1; "
"URL: %2; "
"%3';ru='Не удалось получить файл исправления (патча)."
"Идентификатор: %1;"
"URL: %2;"
"%3';en='Failed to get the corrections file (patch). Identifier: %1;"
"URL: %2;"
"%3'"),
			Идентификатор,
			URLФайла,
	...
	ЗаписатьИнформациюВЖурналРегистрации(НСтр("es='El fichero de la corrección (патча) es con éxito recibido';ru='Файл исправления (патча) успешно получен';en='Patch file successfully received'"));
...
Процедура ДобавитьОтключенноеДело(ТекущиеДела, ИдентификаторДела, Разделы)
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.ЕстьДела       = Ложь;
		Дело.Важное         = Ложь;
		Дело.Представление  = НСтр("es='Es accesible la renovación del programa';ru='Доступно обновление программы';en='Program update available'");
		Дело.Форма          = "Обработка.ОбновлениеПрограммы.Форма.Форма";
		Дело.Владелец       = Раздел;
	КонецЦикла;
