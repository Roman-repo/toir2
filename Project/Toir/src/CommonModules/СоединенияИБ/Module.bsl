...
Процедура УстановитьБлокировкуСеансовОбластиДанных(Параметры, Знач ПоМестномуВремени = Истина, Знач ОбластьДанных = -1) Экспорт
	...
	Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		ИначеЕсли ОбластьДанных <> ЗначениеРазделителяСеанса Тогда
			ВызватьИсключение НСтр("es='De la sesión con los significados usados разделителей no es posible establecer el bloqueo de las sesiones de la área de los datos excelente de usada en sesión.';ru='Из сеанса с используемыми значениями разделителей нельзя установить блокировку сеансов области данных, отличной от используемой в сеансе.';en='From a session with the delimiter values used, you cannot set the session lock on a data area other than that used in the session.'");
		КонецЕсли;
		
	Иначе
		
		Если ОбластьДанных = -1 Тогда
			ВызватьИсключение НСтр("es='Es imposible establecer el bloqueo de las sesiones de la área de los datos - no es indicada la área de los datos.';ru='Невозможно установить блокировку сеансов области данных - не указана область данных.';en='Unable to set session lock for data area - data area not specified.'");
		КонецЕсли;
...
Функция СообщениеОНеотключенныхСеансах() Экспорт
	...
	Сообщение = НСтр("es='No consiguió desconectar las sesiones:';ru='Не удалось отключить сеансы:';en='Failed to disconnect sessions:'");
...
"que no pueden ser acabados forzadamente: "
""
"%1 %2';ru='Имеются активные сеансы работы с программой,"
"которые не могут быть завершены принудительно:"
"%1"
"%2';en='There are active sessions with the program,"
"which cannot be completed forcibly:"
"%1"
"%2 '"),
...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(ТекущийРежим.Конец) Тогда
		ПериодБлокировки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Para el período con %1 por %2';ru='на период с %1 по %2';en='for the period from %1 to%2'"), ТекущийРежим.Начало, ТекущийРежим.Конец);
	Иначе
		ПериодБлокировки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Con %1';ru='с %1';en='from %1'"), ТекущийРежим.Начало);
	КонецЕсли;
	Если ЗначениеЗаполнено(ТекущийРежим.Сообщение) Тогда
		ПричинаБлокировки = НСтр("es='Por la causa:';ru='по причине:';en='due to:'") + Символы.ПС + ТекущийРежим.Сообщение;
	Иначе
		ПричинаБлокировки = НСтр("es='Para la realización de los trabajos de reglamento';ru='для проведения регламентных работ';en='for routine maintenance'");
	КонецЕсли;
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Por el administrador de la aplicación es establecido el bloqueo del trabajo de los usuarios %1 %2. "
""
"La aplicación es temporalmente inaccesible.';ru='Администратором приложения установлена блокировка работы пользователей %1 %2."
""
"Приложение временно недоступно.';en='The application administrator has blocked the work of users% 1% 2."
""
"The application is temporarily unavailable.'"), ПериодБлокировки, ПричинаБлокировки);
	...
	Если Пользователи.ЭтоПолноправныйПользователь() Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Por el administrador de la aplicación es establecido el bloqueo del trabajo de los usuarios %1 %2. "
""
"¿Entrar en la aplicación bloqueada?';ru='Администратором приложения установлена блокировка работы пользователей %1 %2."
""
"Войти в заблокированное приложение?';en='The application administrator has set the user% 1% 2 to be blocked."
""
"Sign in to a locked app?'"),
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если ПараметрыБлокировки.Установлена Тогда
		Если ТекущаяДатаСеанса < ПараметрыБлокировки.Начало Тогда
			Если ПараметрыБлокировки.Конец <> Дата(1, 1, 1) Тогда
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Es planeada con %1 por %2';ru='Запланирована с %1 по %2';en='Scheduled from %1 to %2'"),
					Формат(ПараметрыБлокировки.Начало, "ДЛФ=DT"), Формат(ПараметрыБлокировки.Конец, "ДЛФ=DT"));
			Иначе
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Es planeada con %1';ru='Запланирована с %1';en='Scheduled from %1'"), Формат(ПараметрыБлокировки.Начало, "ДЛФ=DT"));
			КонецЕсли;
			Важность = Ложь;
		ИначеЕсли ПараметрыБлокировки.Конец <> Дата(1, 1, 1) И ТекущаяДатаСеанса > ПараметрыБлокировки.Конец И ПараметрыБлокировки.Начало <> Дата(1, 1, 1) Тогда
			Важность = Ложь;
			Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No funciona (ha cumplido el plazo %1)';ru='Не действует (истек срок %1)';en='Not valid (expired%1)'"), Формат(ПараметрыБлокировки.Конец, "ДЛФ=DT"));
		Иначе
			Если ПараметрыБлокировки.Конец <> Дата(1, 1, 1) Тогда
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Con %1 por %2';ru='с %1 по %2';en='from %1 to %2'"),
					Формат(ПараметрыБлокировки.Начало, "ДЛФ=DT"), Формат(ПараметрыБлокировки.Конец, "ДЛФ=DT"));
			Иначе
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Con %1';ru='с %1';en='from %1'"), 
					Формат(ПараметрыБлокировки.Начало, "ДЛФ=DT"));
			КонецЕсли;
			Важность = Истина;
		КонецЕсли;
	Иначе
		Сообщение = НСтр("es='No funciona';ru='Не действует';en='It does not work'");
		Важность = Ложь;
	КонецЕсли;
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("es='El bloqueo del trabajo de los usuarios';ru='Блокировка работы пользователей';en='Blocking user work'");
...
Функция СформироватьСообщениеБлокировки(Знач Сообщение, Знач КодРазрешения) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() И ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		ТекстСообщения = ТекстСообщения + НСтр("es='%1 "
"Para el permiso del trabajo de los usuarios es posible abrir la aplicación con el parámetro de RazreshitRabotuPolzovateley. Por ejemplo: "
"¿http:¿//<la dirección web del servidor>/? C=РазрешитьРаботуПользователей';ru='%1"
"Для разрешения работы пользователей можно открыть приложение с параметром РазрешитьРаботуПользователей. Например:"
"http://<веб-адрес сервера>/?C=РазрешитьРаботуПользователей';en='%1"
"To allow users to work, you can open the application with the РазрешитьРаботуПользователей option. For example:"
"http: // <server web address> /? C = РазрешитьРаботуПользователей'");
...
"http: // <server web address> /? C = РазрешитьРаботуПользователей'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, СоединенияИБКлиентСервер.ТекстДляАдминистратора(),
			СтрокаПутиКИнформационнойБазе, НСтр("es='<el código del permiso>';ru='<код разрешения>';en='<код разрешения>'"));
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("es='La terminación del trabajo de los usuarios';ru='Завершение работы пользователей';en='User work completing'", ОбщегоНазначения.КодОсновногоЯзыка());
