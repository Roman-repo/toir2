...
Функция ПодсистемыТекущегоПользователя() Экспорт
	...
	КорневаяСтрока.Представление = НСтр("es='Todas las secciones';ru='Все разделы';en='All sections'");
	...
	Для Каждого ЭлементСписка Из СписокРазделов Цикл
		...
		Если НЕ (ТипЗнч(РазделМетаданные) = Тип("ОбъектМетаданных") И СтрНачинаетсяС(РазделМетаданные.ПолноеИмя(), "Подсистема"))
			...
			ВызватьИсключение НСтр("es='Son determinados los significados de las secciones en el procedimiento ВариантыОтчетовПереопределяемый.ОпределитьРазделыСВариантамиОтчетов';ru='Некорректно определены значения разделов в процедуре ВариантыОтчетовПереопределяемый.ОпределитьРазделыСВариантамиОтчетов';en='Incorrectly defined section values in the procedure ВариантыОтчетовПереопределяемый.ОпределитьРазделыСВариантамиОтчетов'");
		...
		Если ЭтоНачальнаяСтраница Тогда
			...
			СтрокаДерева.Представление = НСтр("es='La página inicial';ru='Начальная страница';en='start page'");
		Иначе
