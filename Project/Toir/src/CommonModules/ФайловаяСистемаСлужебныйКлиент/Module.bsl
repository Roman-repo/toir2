...
Процедура ОткрытьФайлВПрограммеПросмотра(ПутьКФайлу, Знач Оповещение = Неопределено,
	...
	ТекстПредложения = НСтр("es='Para la apertura del fichero es necesario establecer la ampliación del trabajo con los ficheros.';ru='Для открытия файла необходимо установить расширение работы с файлами.';en='To open a file, you must install the file extension.'");
...
Процедура ОткрытьФайлВПрограммеПросмотраПослеПроверкиРасширенияРаботыСФайлами(РасширениеПодключено, Контекст) Экспорт
	...
	Иначе
		...
		ОписаниеОшибки = НСтр("es='La ampliación para el trabajo con los ficheros no es establecida, la apertura del fichero es imposible.';ru='Расширение для работы с файлами не установлено, открытие файла невозможно.';en='The extension for working with files has not been installed; opening a file is not possible.'");
...
Процедура ОткрытьФайлВПрограммеПросмотраПослеПроверкиСуществования(Существует, Контекст) Экспорт
	...
	Иначе 
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='el fichero, que es necesario abrir: "
"%1';ru='Не найден файл, который требуется открыть:"
"%1';en='The file to open was not found:"
"%1'"),
			ФайлИнфо.ПолноеИмя);
		ОткрытьФайлВПрограммеПросмотраОповеститьОбОшибке(ОписаниеОшибки, Контекст);
...
Процедура ОткрытьФайлВПрограммеПросмотраПослеПроверкиЭтоФайл(ЭтоФайл, Контекст) Экспорт
	...
	Если ЭтоФайл Тогда
		...
		Если ПустаяСтрока(ФайлИнфо.Расширение) Тогда 
			...
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El nombre del fichero no contiene la ampliación: "
"%1';ru='Имя файла не содержит расширения:"
"%1';en='The file name does not contain the extension:"
"%1'"),
				ФайлИнфо.ПолноеИмя);
		...
		Если ЭтоРасширениеИсполняемогоФайла(ФайлИнфо.Расширение) Тогда 
			...
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Se prohíbe abrir los ficheros cumplidos: "
"%1';ru='Исполняемые файлы открывать запрещено:"
"%1';en='Executable files are forbidden to open:"
"%one'"),
				ФайлИнфо.ПолноеИмя);
...
Процедура ОткрытьПроводникПослеПроверкиРасширенияРаботыСФайлами(РасширениеПодключено, Контекст) Экспорт
	...
	Если РасширениеПодключено Тогда
		...
		ФайлИнфо.НачатьПроверкуСуществования(Оповещение);
	Иначе
		ОписаниеОшибки = НСтр("es='La ampliación para el trabajo con los ficheros no es establecida, la apertura de la carpeta no es posible.';ru='Расширение для работы с файлами не установлено, открытие папки не возможно.';en='The extension for working with files has not been installed; opening a folder is not possible.'");
...
Процедура ОткрытьПроводникПослеПроверкиСуществования(Существует, Контекст) Экспорт 
	...
	Иначе 
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='la carpeta, que es necesaria abrir en el conductor: "
"""%1""';ru='Не найдена папка, которую требуется открыть в проводнике:"
"""%1""';en='The folder to open in Explorer was not found:"
"""%1""'"),
			ФайлИнфо.ПолноеИмя);
		ОткрытьПроводникОповеститьОбОшибке(ОписаниеОшибки, Контекст);
	КонецЕсли;
...
Процедура ОткрытьНавигационнуюСсылкуПослеПроверкиРасширенияРаботыСФайлами(РасширениеПодключено, Контекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La ampliación para el trabajo con los ficheros no es establecida, el tránsito por la referencia ""%1"" es imposible.';ru='Расширение для работы с файлами не установлено, переход по ссылке ""%1"" невозможен.';en='The extension for working with files has not been installed, clicking the link ""%1"" is not possible.'"),
			НавигационнаяСсылка);
...
Процедура ЗапуститьПрограммуПослеПроверкиРасширенияРаботыСФайлами(РасширениеПодключено, Контекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("es='La ampliación para el trabajo con los ficheros no es establecida, el lanzamiento del programa es imposible.';ru='Расширение для работы с файлами не установлено, запуск программы невозможен.';en='The extension for working with files is not installed, the program cannot be launched.'");
...
Процедура ЗапуститьПрограммуПослеПроверкиСуществования(Существует, Контекст) Экспорт
	...
	Иначе 
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió poner en marcha el programa "
"%1 "
"por la causa: "
"no hay catálogo indicado como por TekuschiyKatalog "
"%2';ru='Не удалось запустить программу"
"%1"
"по причине:"
"Не существует каталог, указанный как ТекущийКаталог"
"%2';en='Failed to start program"
"%1"
"due to:"
"The directory specified as ТекущийКаталог does not exist"
"%2'"),
			СтрокаКоманды,
			ТекущийКаталог);
		ЗапуститьПрограммуОповеститьОбОшибке(ОписаниеОшибки, Контекст);
...
"%1 "
"por la causa: "
"ТекущийКаталог no es el catálogo %2';ru='Не удалось запустить программу"
"%1"
"по причине:"
"ТекущийКаталог не является каталогом %2';en='Failed to start program"
"%1"
"due to:"
"ТекущийКаталог is not a%2 directory'"),
			СтрокаКоманды,
			ТекущийКаталог);
		ЗапуститьПрограммуОповеститьОбОшибке(ОписаниеОшибки, Контекст);
	КонецЕсли;
...
Процедура ЗапуститьПрограммуПослеЗапускаПриложения(КодВозврата, Контекст) Экспорт 
	...
	Если Контекст.ДождатьсяЗавершения И КодВозврата = Неопределено Тогда
		ОписаниеОшибки = НСтр("es='Había una falta desconocida durante el lanzamiento del programa.';ru='Произошла неизвестная ошибка при запуске программы.';en='An unknown error occurred while starting the program.'");
		ЗапуститьПрограммуОповеститьОбОшибке(ОписаниеОшибки, Контекст);
		Возврат;
	КонецЕсли;
...
"%1 "
"por la causa: "
"el Lanzamiento de los programas con el aumento de los privilegios es inaccesible en el cliente web.';ru='Не удалось запустить программу"
"%1"
"по причине:"
"Запуск программ с повышением привилегий недоступен в веб-клиенте.';en='Failed to start program"
"%1"
"due to:"
"Running elevated privileges is not available in the web client.'"),
...
"%1 "
"por la causa: "
"el Lanzamiento de los programas con el aumento de los privilegios es inaccesible en el cliente móvil.';ru='Не удалось запустить программу"
"%1"
"по причине:"
"Запуск программ с повышением привилегий недоступен в мобильном клиенте.';en='Failed to start program"
"%1"
"due to:"
"Running elevated privilege programs is not available on the mobile client. '"),
		Контекст.СтрокаКоманды);
	ЗапуститьПрограммуОповеститьОбОшибке(ОписаниеОшибки, Контекст);
#Иначе
...
"%1 "
"por la causa: "
"el Lanzamiento de los programas con el aumento de los privilegios es accesible solamente en Windows y Linux.';ru='Не удалось запустить программу"
"%1"
"по причине:"
"Запуск программ с повышением привилегий доступен только в Windows и Linux.';en='Failed to start program"
"%1"
"due to:"
"Running privileged escalation programs is only available on Windows and Linux.'"),
			Контекст.СтрокаКоманды);
		ЗапуститьПрограммуОповеститьОбОшибке(ОписаниеОшибки, Контекст);
	КонецЕсли;
...
Процедура СоздатьВременныйКаталогПослеПроверкиРасширенияРаботыСФайлами(РасширениеПодключено, Контекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = 
			НСтр("es='La ampliación para el trabajo con los ficheros no es establecida, la creación del catálogo temporal es imposible.';ru='Расширение для работы с файлами не установлено, создание временного каталога невозможно.';en='The extension for working with files has not been installed; creating a temporary directory is not possible.'");
