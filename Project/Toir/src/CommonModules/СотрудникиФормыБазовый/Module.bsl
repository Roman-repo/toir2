		...
		Если ЗначениеЗаполнено(Форма.ТекущаяОрганизация) Тогда
			...
			Если ЗначениеЗаполнено(Форма.ДатаУвольнения) Тогда
				...
				Если Форма.ДатаУвольнения <= Форма.ДатаПриема Тогда
					...
					ТекстСообщения = НСтр("es='La fecha del despido no puede ser menos o es igual a la fecha del ingreso al trabajo';ru='Дата увольнения не может быть меньше или равна дате приема на работу';en='Date of dismissal cannot be less than or equal to the date of employment'");
				...
				ИначеЕсли НЕ ЗначениеЗаполнено(Форма.ДатаПриема) Тогда
					...
					ТекстСообщения = НСтр("es='Es necesario rellenar la fecha del ingreso al trabajo';ru='Необходимо заполнить дату приема на работу';en='Required to fill out the date of employment'");
...
Функция ЗаголовокКнопкиОткрытияСотрудника(ДанныеСотрудника, РеквизитыОрганизации, ДатаСведений, ВыводитьПодробнуюИнформацию = Ложь) Экспорт
	...
	Если Не ВыводитьПодробнуюИнформацию Тогда
		...
		Если (ЗначениеЗаполнено(ДанныеСотрудника.ДатаПриема) И ДанныеСотрудника.ДатаПриема <= ДатаСведений) И ((Не ЗначениеЗаполнено(ДанныеСотрудника.ДатаУвольнения)) Или (ДанныеСотрудника.ДатаУвольнения > ДатаСведений)) Тогда
			...
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 "
"trabaja con %2 %3';ru='%1"
"работает с %2 %3';en='%1"
"works with %2 %3'"), 
...
"fired from% 2'"), 
		...
		Иначе
			Если ДанныеСотрудника.ДатаПриема > ДатаСведений Тогда
				Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 "
"trabajará con %2';ru='%1  "
"будет работать с %2';en='%1"
"will work with% 2'"), 
																			НаименованиеОрганизации,
																			Формат(ДанныеСотрудника.ДатаПриема, "ДФ=дд.ММ.гггг"));	
			Иначе
				Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 no es admitido "
"al trabajo';ru='%1  "
"не оформлен на работу';en='%1"
"not registered for work'"), 
	...
	Иначе 
		Если (Не ЗначениеЗаполнено(ДанныеСотрудника.ДатаПриема) И (Не ЗначениеЗаполнено(ДанныеСотрудника.ДатаУвольнения))) Тогда	
			Возврат НСтр("es='Admitir en el trabajo';ru='Принять на работу';en='Recruit'")	
		Иначе	
			Возврат НСтр("es='Más detalladamente...';ru='Подробнее...';en='More details …'")
		КонецЕсли;	
	КонецЕсли;	
...
Функция ПоясняющаяНадписьКМестуРаботыСотрудника(ДанныеСотрудника, РеквизитыОрганизации, ДатаСведений) Экспорт
	...
	Если ДанныеСотрудника.СотрудникАктуален Тогда
		СтрокаПериодРаботы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Trabaja desde %1 años';ru='Работает с %1 года';en='Works from %1 year'"), Формат(ДанныеСотрудника.ДатаПриема, "ДФ=дд.ММ.гггг"));
	Иначе
		Если ЗначениеЗаполнено(ДанныеСотрудника.ДатаПриема) И ДанныеСотрудника.ДатаПриема > ДатаСведений Тогда
			СтрокаПериодРаботы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Trabajará desde %1 años';ru='Будет работать с %1 года';en='Will work from% 1 year'"), Формат(ДанныеСотрудника.ДатаПриема, "ДФ=дд.ММ.гггг"));	
		ИначеЕсли ЗначениеЗаполнено(ДанныеСотрудника.ДатаПриема) И ЗначениеЗаполнено(ДанныеСотрудника.ДатаУвольнения) Тогда 
			СтрокаПериодРаботы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Trabajaba con %1 por %2';ru='Работал с %1 по %2';en='Worked from %1 to %2'"), Формат(ДанныеСотрудника.ДатаПриема, "ДФ=дд.ММ.гггг"), Формат(ДанныеСотрудника.ДатаУвольнения, "ДФ=дд.ММ.гггг"));
		ИначеЕсли ЗначениеЗаполнено(ДанныеСотрудника.ДатаУвольнения) Тогда 
			СтрокаПериодРаботы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Trabajará desde %1 años';ru='Будет работать с %1 года';en='Will work from%1 year'"), Формат(ДанныеСотрудника.ДатаУвольнения, "ДФ=дд.ММ.гггг"));
		ИначеЕсли ЗначениеЗаполнено(ДанныеСотрудника.ДатаПриема) Тогда 
			СтрокаПериодРаботы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Trabajaba con %1 por %2';ru='Работал с %1 по %2';en='Worked from %1 to %2'"), Формат(ДанныеСотрудника.ДатаПриема, "ДФ=дд.ММ.гггг")); 		
		Иначе	
			СтрокаПериодРаботы = НСтр("es='del ingreso al trabajo';ru='Не указана дата приема на работу';en='Missing date for hiring'");
		КонецЕсли;				
	КонецЕсли;	
	...
	Результат.Вставить("ИнфоНадписьОрганизация", ?(ЗначениеЗаполнено(РеквизитыОрганизации.Наименование), РеквизитыОрганизации.Наименование,  НСТР("es='No es indicado';ru='не указана';en='not specified'")));
	Результат.Вставить("ИнфоНадписьДолжность", ?(ЗначениеЗаполнено(ДолжностьНаименование), ДолжностьНаименование,  НСТР("es='No es indicado';ru='не указана';en='not specified'")));
	
	Если ДанныеСотрудника.Владелец().Колонки.Найти("ТекущаяТарифнаяСтавка") <> Неопределено Тогда
		Результат.Вставить("ИнфоНадписьОклад", ?(ЗначениеЗаполнено(ДанныеСотрудника.ТекущаяТарифнаяСтавка), ДанныеСотрудника.ТекущаяТарифнаяСтавка, НСТР("es='No es indicado';ru='не указан';en='not specified'")));
	КонецЕсли; 
...
Функция БанковскийСчетИнформацияОПричинахНедоступности() Экспорт
	Возврат НСтр("es='Para la introducción de la cuenta personal rellenen la organización';ru='Для ввода лицевого счета заполните организацию';en='For entering a personal account fill in the organization'");
КонецФункции
		...
		Для Каждого КонтактнаяИнформация Из КоллекцияПолейКонтактнойИнформации Цикл
			...
			Если Элемент <> Неопределено Тогда
				...
				ИначеЕсли КонтактнаяИнформация.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
					...
					Надпись.Заголовок = НСтр("es='La falta';ru='Ошибка';en='Error '");
