...
Функция СловарьКонтейнераВнешнегоМодуля() Экспорт
	...
	Результат.Вставить("Именительный", НСтр("es='El informe adicional o el tratamiento';ru='Дополнительный отчет или обработка';en='Additional report or processing'"));
	Результат.Вставить("Родительный", НСтр("es='Del informe adicional o el tratamiento';ru='Дополнительного отчета или обработки';en='Additional report or processing'"));
...
Процедура ВыполнитьСценарийБезопасногоРежима(Знач КлючСессии, Знач Сценарий, Знач ИсполняемыйОбъект, ПараметрыВыполнения, СохраняемыеПараметры = Неопределено, ОбъектыНазначения = Неопределено) Экспорт
	...
	Для Каждого ШагСценария Из Сценарий Цикл
		...
		Иначе
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El tipo desconocido de la acción para la etapa del guión: %1';ru='Неизвестный вид действия для этапа сценария: %1';en='Unknown action type for scenario step: %1'"), ШагСценария.ВидДействия);
		...
		Для Каждого ПараметрМетода Из ПараметрыМетода Цикл
			...
			Иначе
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El parámetro desconocido para la etapa del guión: %1';ru='Неизвестный параметр для этапа сценария: %1';en='Unknown parameter for script stage: %1'"), ПараметрМетода.Вид);
...
Функция СформироватьПредставлениеРазрешений(Знач Разрешения) Экспорт
	...
	Для Каждого Разрешение Из Разрешения Цикл
		...
		Результат = Результат + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			НСтр("es='Más detalladamente...';ru='Подробнее...';en='More details …'"));
...
Функция СформироватьПодробноеОписаниеРазрешения(Знач ВидРазрешения, Знач ПараметрыРазрешения) Экспорт
	...
	Результат = Результат + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		"<P><FONT size=2>%1%2</FONT></P>", РасшифровкаРазрешения, ?(
			ЕстьПараметры,
			" " + НСтр("es='Con las restricciones siguientes';ru='со следующими ограничениями';en='with the following restrictions'") + ":",
			"."));
