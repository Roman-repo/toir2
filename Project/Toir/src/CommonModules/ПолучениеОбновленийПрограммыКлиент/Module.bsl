...
Процедура ПерейтиНаНовуюРедакцию(НомерРедакции, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(НомерРедакции) Тогда
		ВызватьИсключение НСтр("es='de la redacción.';ru='Не передан номер редакции.';en='Revision number not provided.'");
	КонецЕсли;
...
Процедура ПерейтиНаДругуюПрограмму(
	...
	Если Не ЗначениеЗаполнено(ИмяНовойПрограммы) Тогда
		ВызватьИсключение НСтр("es='programa.';ru='Не передано имя новой программы.';en='The name of the new program was not passed.'");
	КонецЕсли;
	...
	Исключение
		...
		ПолучениеОбновленийПрограммыВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta de la comprobación de la versión de la plataforma durante el lanzamiento. %1';ru='Ошибка проверки версии платформы при запуске. %1';en='Error checking platform version at startup. %1'"),
...
Процедура ОбработатьОповещениеОДоступномОбновлении(ПараметрыОповещения)
	...
	ПоказатьОповещениеПользователя(
		НСтр("es='La renovación del programa';ru='Обновление программы';en='Program update'"),
		Новый ОписаниеОповещения(
...
Процедура ПриИзмененииРасписания(Расписание, Форма) Экспорт
	...
	Если Форма.БИПАвтоматическаяПроверкаОбновлений = 2
		...
		ПоказатьПредупреждение(,
			НСтр("es='El intervalo de la comprobación no puede ser dado más a menudo, que una vez 5 minutos.';ru='Интервал проверки не может быть задан чаще, чем один раз 5 минут.';en='The scan interval cannot be set more than once every 5 minutes.'"));
		Возврат;
	КонецЕсли;
