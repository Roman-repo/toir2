...
Процедура ПоказатьОтключение() Экспорт
	...
	Если Не ОбсужденияСлужебныйВызовСервера.Подключены() Тогда 
		ПоказатьПредупреждение(, НСтр("es='La conexión de las discusiones no es cumplida.';ru='Подключение обсуждений не выполнено.';en='Discussion connection failed.'"));
		Возврат;
	КонецЕсли;
	
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить("Отключить", НСтр("es='Desconectar';ru='Отключить';en='Disable'"));
	Кнопки.Добавить(КодВозвратаДиалога.Нет);
	
	Оповещение = Новый ОписаниеОповещения("ПослеОтветаНаВопросОбОтключении", ЭтотОбъект);
	
	ПоказатьВопрос(Оповещение, НСтр("es='¿Desconectar las discusiones?';ru='Отключить обсуждения?';en='Disable discussions?'"),
...
Процедура ПослеЗаписиПользователя(Форма, ОписаниеЗавершения) Экспорт
	...
	ПараметрыВопроса.Заголовок = НСтр("es='Las discusiones (el sistema de las interacciones)';ru='Обсуждения (система взаимодействий)';en='Discussions (interaction system)'");
