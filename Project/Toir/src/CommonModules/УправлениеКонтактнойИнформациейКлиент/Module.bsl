...
Процедура ВыполнитьКоманду(Знач Форма, Знач ИмяКоманды) Экспорт
	...
	ИначеЕсли ТипКонтактнойИнформации = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Телефон") Тогда
		...
		Если ПустаяСтрока(Параметры.НомерТелефона) Тогда
			Если МожноОтправлятьSMS Тогда
				ТекстПредупреждения = НСтр("es='Para la realización del telefonazo o el envío SMS es necesario introducir el número de teléfono.';ru='Для совершения звонка или отправки SMS требуется ввести номер телефона.';en='To make a call or send SMS, you must enter a phone number.'");
			Иначе
				ТекстПредупреждения = НСтр("es='Para la realización del telefonazo es necesario introducir el número de teléfono.';ru='Для совершения звонка требуется ввести номер телефона.';en='To make a call, you must enter a phone number.'");
			КонецЕсли;
			ПоказатьПредупреждение(, ТекстПредупреждения);
		ИначеЕсли МожноОтправлятьSMS Тогда
			Список = Новый СписокЗначений;
			Список.Добавить("Позвонить", НСтр("es='Llamar';ru='Позвонить';en='To call'"),, БиблиотекаКартинок.Позвонить);
			Список.Добавить("ОтправитьSMS", НСтр("es='Enviar SMS...';ru='Отправить SMS...';en='Send SMS ...'"),, БиблиотекаКартинок.ОтправитьSMS);
	...
	ИначеЕсли ТипКонтактнойИнформации = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Skype") Тогда
		...
		Список = Новый СписокЗначений;
		Список.Добавить("Позвонить", НСтр("es='Llamar';ru='Позвонить';en='To call'"));
		Список.Добавить("НачатьЧат", НСтр("es='Comenzar el chat';ru='Начать чат';en='Start chat'"));
...
Процедура СоздатьЭлектронноеПисьмо(Знач ЗначенияПолей, Знач Представление = "", ОжидаемыйВид = Неопределено, ИсточникКонтактнойИнформации = Неопределено) Экспорт
	...
	Если ТипИнформации <> ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No es posible crear la carta de contacto la información con tipo ""%1""';ru='Нельзя создать письмо по контактной информацию с типом ""%1""';en='Can not create a contact information letter of type ""%1""'"), ТипИнформации);
	КонецЕсли;
	
	Если ЗначенияПолей = "" И ПустаяСтрока(Представление) Тогда
		ПоказатьПредупреждение(,НСтр("es='Para el envío de la carta es necesario introducir la dirección del correo electrónico.';ru='Для отправки письма необходимо ввести адрес электронной почты.';en='To send an email, you must enter an email address.'"));
		Возврат;
	КонецЕсли;
	...
	Если ТипЗнч(АдресПочты) <> Тип("Строка") Тогда
		ВызватьИсключение НСтр("es='La falta de la recepción de la dirección del correo electrónico, el tipo equivocado de la información de contacto';ru='Ошибка получения адреса электронной почты, неверный тип контактной информации';en='Error receiving email address, invalid contact information type'");
	КонецЕсли;
...
Процедура СоздатьSMS(Знач ЗначенияПолей, Знач Представление = "", ОжидаемыйВид = Неопределено, ИсточникКонтактнойИнформации = "") Экспорт
	...
	Если Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОтправкаSMS") Тогда
		ВызватьИсключение НСтр("es='El envío SMS es inaccesible.';ru='Отправка SMS недоступна.';en='SMS sending is not available.'");
	КонецЕсли;
	...
	Если ТипИнформации <> ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Телефон") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No es posible отправитьSMS por de contacto la información con tipo ""%1""';ru='Нельзя отправитьSMS по контактной информацию с типом ""%1""';en='Can not send SMS by contact information with type ""%1""'"), ТипИнформации);
	КонецЕсли;
	
	Если ЗначенияПолей = "" И ПустаяСтрока(Представление) Тогда
		ПоказатьПредупреждение(,НСтр("es='Para el envío SMS es necesario introducir el número de teléfono.';ru='Для отправки SMS необходимо ввести номер телефона.';en='To send SMS, you must enter a phone number.'"));
		Возврат;
	КонецЕсли;
	...
	#Если НЕ ВебКлиент Тогда
		...
		Если ДоступноеИмяПротокола = Неопределено Тогда
			СтрокаСПредупреждением = Новый ФорматированнаяСтрока(
					НСтр("es='Para la realización del telefonazo es necesario establecer el programa de la telefonía, por ejemplo';ru='Для совершения звонка требуется установить программу телефонии, например';en='To make a call you need to install a telephony program, for example'"),
...
Процедура ПерейтиПоВебСсылке(Знач ЗначенияПолей, Знач Представление = "", ОжидаемыйВид = Неопределено) Экспорт
	...
	Если ТипИнформации <> ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.ВебСтраница") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No es posible abrir la referencia según la información de contacto con tipo ""%1""';ru='Нельзя открыть ссылку по контактной информации с типом ""%1""';en='Can not open link on contact information with type ""%1""'"), ТипИнформации);
	КонецЕсли;
	...
	Если ТипЗнч(АдресСсылки) <> Тип("Строка") Тогда
		ВызватьИсключение НСтр("es='La falta de la recepción de la referencia, el tipo equivocado de la información de contacto';ru='Ошибка получения ссылки, неверный тип контактной информации';en='Error receiving link, wrong type of contact information'");
	КонецЕсли;
...
Процедура ОткрытьSkype(СтрокаЗапуска)
	...
	#Если НЕ ВебКлиент Тогда
		Если ПустаяСтрока(ПрограммаТелефонииУстановлена("skype")) Тогда
			ПоказатьПредупреждение(Неопределено, НСтр("es='Para la realización del telefonazo por Skype es necesario establecer el programa.';ru='Для совершения звонка по Skype требуется установить программу.';en='To make a Skype call, you need to install the program.'"));
			Возврат;
		КонецЕсли;
	#КонецЕсли
...
Процедура ВвестиКомментарий(Знач Форма, Знач ИмяРеквизита, Знач НайденнаяСтрока, Знач Результат)
	...
	ОбщегоНазначенияКлиент.ПоказатьФормуРедактированияМногострочногоТекста(Оповещение, Комментарий, 
		НСтр("es='El comentario';ru='Комментарий';en='Comment'"));
КонецПроцедуры
