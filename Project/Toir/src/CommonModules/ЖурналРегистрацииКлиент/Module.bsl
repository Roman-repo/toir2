...
//     НСтр("es='Es imposible conectarse a la red el Internet para la comprobación de las renovaciones.';ru='Невозможно подключиться к сети Интернет для проверки обновлений.';en='Unable to connect to the Internet to check for updates.'"));
//
...
Процедура ОткрытьДанныеДляПросмотра(ТекущиеДанные) Экспорт
	...
	Если ТекущиеДанные = Неопределено Или ТекущиеДанные.Данные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("es='Esta anotación de la revista del registro no es vinculada a los datos (cm. La columna ""los Datos"")';ru='Эта запись журнала регистрации не связана с данными (см. колонку ""Данные"")';en='This log entry is not related to data (see the ""Data"" column)'"));
		Возврат;
	КонецЕсли;
	...
	Исключение
		ТекстПредупреждения = НСтр("es='Esta anotación de la revista del registro es vinculada a los datos, pero reflejarlos es imposible. "
"%1';ru='Эта запись журнала регистрации связана с данными, но отобразить их невозможно."
"%1';en='This log entry is associated with data, but cannot be displayed."
"%1'");
		Если ТекущиеДанные.Событие = "_$Data$_.Delete" Тогда 
			// это - событие удаления
			ТекстПредупреждения =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстПредупреждения, НСтр("es='Los datos son quitados de la base informativa';ru='Данные удалены из информационной базы';en='Data deleted from infobase'"));
		Иначе
			ТекстПредупреждения =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстПредупреждения, НСтр("es='Probablemente, los datos son quitados de la base informativa';ru='Возможно, данные удалены из информационной базы';en='The data may have been deleted from the infobase.'"));
		КонецЕсли;
