...
Процедура ПриПереходеКСпискуХарактеристик(Форма, ПараметрыНоменклатуры)
	...
	ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Las características: ""%1""';ru='Характеристики: ""%1""';en='Characteristics: ""%1""'"), ПараметрыНоменклатуры.Номенклатура);
		...
		Пока ВыборкаПоРеквизитам.Следующий() Цикл
			...
			Пока ВыборкаЗначения.Следующий() Цикл
				...
				Если ТипЗнч(ВыборкаЗначения.ЗначениеОтбора) = Тип("Число")
					...
					СтрокаРеквизита.ПредставлениеОтбора   = НСтр("es='<no es dado>';ru='<не задано>';en='<не задано>'");
...
Процедура ЗаполнитьДеревоЗначенийОтборовХарактеристик(Форма, ДеревоОтборов)
	...
	НеЗадано = НСтр("es='<no es dado>';ru='<не задано>';en='<не задано>'");
...
Процедура ВыполнитьПолнотекстовыйПоискНоменклатуры(Форма, РезультатПоиска)
	...
	//РезультатПоиска = торо_Локализация.ПолучитьРезультатыПоиска(СтрокаПоиска, РезультатПоиска);
	// << [Деснол] [Ушаков А.Д.] [10.10.2019] 
...
Процедура ВыполнитьНеПолнотекстовыйПоискНоменклатуры(Форма, РезультатПоиска)
	...
	// >> [Деснол] [Ушаков А.Д.] [10.10.2019] 
	//СтрокаПоиска = Форма.СтрокаПоискаНоменклатура;
	//РезультатПоиска = торо_Локализация.ПолучитьРезультатыПоиска(СтрокаПоиска, РезультатПоиска, Истина);
	// << [Деснол] [Ушаков А.Д.] [10.10.2019] 
	ДополнитьРезультатыПоискаНоменклатуры(РезультатПоиска);
...
Процедура ОбработатьЭлементСпискаПоиска(ЭлементСписка, РезультатПоиска)
	...
	Если ТипЗнч(МетаданныеЭлемента) = Тип("ОбъектМетаданных") Тогда
		...
		Иначе
			ВызватьИсключение НСтр("es='La falta desconocida';ru='Неизвестная ошибка';en='Unknown error'");
		КонецЕсли;
...
Функция ТекстЗапросаПоискаПоОбласти(ОбластьПоиска, ТочноеСоответствие)
	...
	Если ОбластьПоиска = Метаданные.Справочники.Номенклатура Тогда
		...
		// Гкс+[TOIR20-370] Лебедев Н.А. от 22.06.2023. Добавлено:{
		|			ИЛИ СправочникНоменклатура.Код %ВидСравнения% &СтрокаПоиска
		// } Гкс-[TOIR20-370] Лебедев Н.А. от 22.06.2023.
...
Процедура УстановитьЗаголовкиЭлементовФормыПоПараметрам(Форма) Экспорт
	...
	ТекстЗаголовка = НСтр("es='El precio';ru='Цена';en='Price'");
	
	Если ЗначениеЗаполнено(Валюта) Тогда
		ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El precio (%1)';ru='Цена (%1)';en='Price (%1)'"), Валюта);
	КонецЕсли;
	...
	ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		НСтр("es='Solamente presente sobre el grupo de los almacenes ""%1""';ru='Только в наличии на группе складов ""%1""';en='Only in stock at warehouse group ""%1""'"),
		?(ЗначениеЗаполнено(Склад), НСтр("es='Solamente presente en el almacén ""%1""';ru='Только в наличии на складе ""%1""';en=""Only in stock at '%1'"""), НСтр("es='Solamente presente';ru='Только в наличии';en='Only in stock'"))), 
		Склад);
