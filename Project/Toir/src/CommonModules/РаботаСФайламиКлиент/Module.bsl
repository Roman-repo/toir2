...
Процедура ДобавитьФайлы(Знач ВладелецФайла, Знач ИдентификаторФормы, Знач Фильтр = "", ГруппаФайлов = Неопределено,
	...
	Если Не ЗначениеЗаполнено(ВладелецФайла) Тогда
		ВызватьИсключение НСтр("es='No el valor dado del parámetro de VladeletsFayla en RabotaSFaylamiKlient. ДобавитьФайлы.';ru='Не задано значение параметра ВладелецФайла в РаботаСФайламиКлиент.ДобавитьФайлы.';en='Parameter value not set  ВладелецФайла в РаботаСФайламиКлиент.ДобавитьФайлы.'");
	КонецЕсли;
...
Процедура ДобавитьФайл(ОбработчикРезультата, ВладелецФайла, ФормаВладелец, РежимСоздания = Неопределено, 
	...
	Если Не ЗначениеЗаполнено(ВладелецФайла) Тогда
		ВызватьИсключение НСтр("es='No el valor dado del parámetro de VladeletsFayla en RabotaSFaylamiKlient. ДобавитьФайл.';ru='Не задано значение параметра ВладелецФайла в РаботаСФайламиКлиент.ДобавитьФайл.';en='Parameter value not set ВладелецФайла в РаботаСФайламиКлиент.ДобавитьФайл.'");
	КонецЕсли;
...
Процедура ПоказатьПодтверждениеЗакрытияФормыСФайлами(Форма, Отказ, ЗавершениеРаботы, ВладелецФайлов,
	...
	ТекстВопроса = НСтр("es='Uno o plusieurs los ficheros son ocupados para la redacción. "
""
"¿Continuar?';ru='Один или несколько файлов заняты для редактирования."
""
"Продолжить?';en='One or more files are inaccessible for editing."
""
"Continue?'");
...
Процедура СкопироватьФайл(ВладелецФайла, ФайлОснование, ДополнительныеПараметры = Неопределено,
	...
	Если Не ЗначениеЗаполнено(ВладелецФайла) Тогда
		ВызватьИсключение НСтр("es='No el valor dado del parámetro de VladeletsFayla en RabotaSFaylamiKlient. СкопироватьФайл.';ru='Не задано значение параметра ВладелецФайла в РаботаСФайламиКлиент.СкопироватьФайл.';en='Parameter value not set ВладелецФайла в РаботаСФайламиКлиент.СкопироватьФайл.'");
	КонецЕсли;
...
Процедура ОткрытьФормуВыбораФайлов(Знач ВладелецФайлов, Знач ЭлементФормы, СтандартнаяОбработка = Ложь,
	...
	Если ВладелецФайлов.Пустая() Тогда
		...
		ПоказатьВопрос(ОбработчикОповещенияОЗакрытии,
			НСтр("es='Los datos no son anotados. "
"El tránsito a ""los ficheros Juntados"" es posible solamente después del registro de datos.';ru='Данные еще не записаны. "
"Переход к ""Присоединенные файлы"" возможен только после записи данных.';en='Data not yet recorded."
"The transition to the """" Attached files """" is possible only after recording data.'"),
...
Процедура ПодписатьФайл(ПрисоединенныйФайл, ИдентификаторФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПрисоединенныйФайл) Тогда
		ПоказатьПредупреждение(, НСтр("es='el fichero, que es necesario firmar.';ru='Не выбран файл, который нужно подписать.';en='The file to be signed is not selected.'"));
		Возврат;
	КонецЕсли;
	
	Если Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		ПоказатьПредупреждение(, НСтр("es='La adición de las firmas electrónicas no es apoyada.';ru='Добавление электронных подписей не поддерживается.';en='Adding electronic signatures is not supported.'"));
		Возврат;
	КонецЕсли;
	...
	Если Не МодульЭлектроннаяПодписьКлиент.ИспользоватьЭлектронныеПодписи() Тогда
		ПоказатьПредупреждение(,
			НСтр("es='Para añadir la firma electrónica, incluyan "
"en los ajustes del programa el uso de las firmas electrónicas.';ru='Чтобы добавить электронную подпись, включите"
"в настройках программы использование электронных подписей.';en='To add an electronic signature, enable"
"in the program settings use electronic signatures.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОткрытьФормуНастройкиСканирования() Экспорт
	...
	Если Не РаботаСФайламиСлужебныйКлиент.ДоступноСканирование() Тогда
		ТекстСообщения = НСтр("es='Para el escaneo es necesario usar la versión 32-de bit del programa para la SO Windows.';ru='Для сканирования необходимо использовать 32-битную версию программы  для ОС Windows.';en='For scanning, you must use the 32-bit version of the program for the Windows OS.'");
	...
	Если Не КомпонентаУстановлена Тогда
		ТекстВопроса = 
			НСтр("es='Para la continuación del trabajo es necesario establecer el componente del escaneo. "
"¿Establecer componente?';ru='Для продолжения работы необходимо установить компоненту сканирования. "
"Установить компоненту?';en='To continue, you must install the scan component."
"Install component? '");
...
#Если МобильныйКлиент Тогда
	ПоказатьПредупреждение(, НСтр("es='La prensa de los ficheros del tipo dado no es apoyada en el cliente móvil.';ru='Печать файлов данного типа не поддерживается в мобильном клиенте.';en='Printing files of this type is not supported on the mobile client.'"));
	Возврат;
#Иначе
	...
	Если СтрНайти(РасширенияИсключения, " " + Расширение + ",") > 0 Тогда
		ПоказатьПредупреждение(, НСтр("es='La prensa de los ficheros del tipo dado no es apoyada.';ru='Печать файлов данного типа не поддерживается.';en='Printing files of this type is not supported.'"));
		Возврат;
...
#Если ВебКлиент Тогда
	Если ПараметрыВыполнения.ДанныеФайла.Расширение <> "mxl" Тогда
		Текст = НСтр("es='Es necesario conservar el fichero al ordenador, después de que cumplir "
"la prensa por medio de la aplicación destinada "
"al trabajo con el fichero dado.';ru='Необходимо сохранить файл на компьютер, после чего выполнить"
"печать при помощи приложения, предназначенного"
"для работы с данным файлом.';en='Required to save the file to your computer, and then run"
"printing using an application designed"
...
"El tránsito a los ficheros juntados es posible solamente después del registro de datos. "
"Los datos serán anotados.';ru='Данные еще не записаны."
"Переход к присоединенным файлам возможен только после записи данных."
"Данные будут записаны.';en='Data not yet recorded."
"Transition to the attached files is possible only after data recording."
...
Процедура ОбновитьОбластьПредпросмотра(Форма, НомерЭлемента, Файл)
	...
	Если ЭлементКартинки <> Неопределено Тогда
		...
		ИначеЕсли ДанныеОбновления.ФайлПоврежден Тогда
			Форма[ИмяРеквизита] = Неопределено;
			ЭлементКартинки.ТекстНевыбраннойКартинки = НСтр("es='La imagen falta';ru='Изображение отсутствует';en='No image'");
		Иначе
	...
	Если ЭлементЗаголовка <> Неопределено Тогда
		...
		Если ДанныеФайла = Неопределено Тогда
			ЭлементЗаголовка.Заголовок = НСтр("es='Cargar';ru='загрузить';en='download'");
