...
//			Свойства[ИмяСвойства] = НСтр("es='Es arreglado';ru='Согласовано';en='Agreed'");
...
//
// Пример реализации:
//   СокращенноеНаименование = НСтр("es='1С:ERP УП';ru='1С:ERP УП';en='1C: ERP UP'");
//
Процедура ПриОпределенииСокращенногоНаименованияКонфигурации(СокращенноеНаименование) Экспорт
	
	СокращенноеНаименование = НСтр("es='1С:ТОИР 2 КОРП';ru='1С:ТОИР 2 КОРП';en='1C: TOIR 2 CORP'");
...
//			КомандаПечати.Представление = НСтр("es='La circulación de documentos: la hoja de la coordinación';ru='Документооборот: лист согласования';en='Document management: approval sheet'");
...
//		НСтр("es='Serán creadas las reglas de la integración y los tipos de los documentos interiores para todos los tipos de los contractos.';ru='Будут созданы правила интеграции и виды внутренних документов для всех видов договоров.';en='Integration rules and types of internal documents for all types of contracts will be created.'");
//
//
...
//				ПредставлениеОбъектаПотребителя = НСтр("es='El ingreso de las mercancías';ru='Поступление товаров';en='Receipt of goods'");
//			ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Услуги Тогда
//				ПредставлениеОбъектаПотребителя = НСтр("es='El ingreso de los servicios';ru='Поступление услуг';en='Service receipt'");
//			КонецЕсли;
//		КонецЕсли;
//	КонецЕсли;
//
...
//			ВидОперации = КлючевыеРеквизиты[0].ЗначениеРеквизитаПотребителя;
//			Если ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Товары Тогда
//				ПредставлениеОбъектаПотребителя = НСтр("es='Las mercancías (factura)';ru='Товары (накладная)';en='Goods (invoice)'");
//			КонецЕсли;
//		КонецЕсли;
//	КонецЕсли;
//
...
//
// Пример реализации:
//	Форма.Заголовок = НСтр("es='La forma del documento';ru='Форма документа';en='Document form'");
//	Форма.Автозаголовок = Ложь;
//	Форма.Пояснение = НСтр("es='Escojan el tipo del documento:';ru='Выберите вид документа:';en='Select the type of document:'");
