...
#Иначе
			ВызватьИсключение КраткоеПредставлениеОшибки(ИнформацияОбОшибке()) + Символы.ПС + Символы.ПС
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='En caso de la falta sobre la disconformidad de la versión de los componente ""comcntr"" sigue registrarla en el ordenador %1 "
"para la anotación de descuento la SO Windows, en nombre de que se cumple 1С:Предприятие. Por ejemplo: "
"regsvr32.exe ""%2\comcntr.dll""';ru='В случае ошибки о несоответствии версии компоненты ""comcntr"" следует зарегистрировать ее на компьютере %1"
"для учетной записи ОС Windows, от имени которой выполняется 1С:Предприятие. Например:"
"regsvr32.exe ""%2\comcntr.dll""';en='In case of an error about the incompatibility of the version of the component"" ""comcntr"" "", register it on the computer%1"
"for the Windows OS account on behalf of which 1C: Enterprise is run. For example:"
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда 
	Если БезопасныйРежим() <> Ложь Тогда
		ВызватьИсключение НСтр("es='¡La atención! La administración del clúster es imposible en el régimen seguro';ru='Внимание! Администрирование кластера невозможно в безопасном режиме';en='Warning! Cluster administration is not possible in safe mode'");
	КонецЕсли;
	
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ВызватьИсключение НСтр("es='¡La atención! En el modelo del servicio es inadmisible la ejecución por la base aplicada informativa de las funciones de la administración del clúster';ru='Внимание! В модели сервиса недопустимо выполнение прикладной информационной базой функций администрирования кластера';en='Warning! In the service model, it is unacceptable that the application information base performs cluster administration functions'");
	КонецЕсли;
	
	Возврат Новый COMОбъект(ОбщегоНазначенияКлиентСервер.ИмяCOMСоединителя());
#ИначеЕсли МобильныйКлиент Тогда
	ВызватьИсключение НСтр("es='¡La atención! La administración del clúster es imposible en el cliente móvil';ru='Внимание! Администрирование кластера невозможно в мобильном клиенте';en='Warning! Cluster administration is not possible in the mobile client'");
#Иначе
...
Функция ПолучитьКластер(СоединениеСАгентомСервера, Знач ПортКластера, Знач ИмяАдминистратораКластера, Знач ПарольАдминистратораКластера)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='En el servidor %1 de trabajo no es encontrado el clúster %2';ru='На рабочем сервере %1 не найден кластер %2';en='Cluster %2 not found on production server %1'"),
...
Функция СоединениеСРабочимПроцессом(COMСоединитель, СоединениеСАгентомСервера, Кластер)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='En el clúster de los servidores %1: el %2 no es encontrado los procesos activos de trabajo.';ru='В кластере серверов %1:%2 не найдено активных рабочих процессов.';en='No active worker processes were found in server cluster% 1:% 2.'"),
		Кластер.HostName,
...
Функция ПолучитьОписаниеИБ(СоединениеСАгентомСервера, Кластер, Знач ИмяВКластере)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='En el clúster de los servidores %1: el %2 no es encontrada la base informativa ""%3""';ru='В кластере серверов %1:%2 не найдена информационная база ""%3""';en=""Infobase '% 3' not found in server cluster% 1:% 2"""),
		Кластер.HostName,
		Формат(Кластер.MainPort, "ЧГ=0"),
		ИмяВКластере);
	...
	Для Каждого ИнформационнаяБаза Из СоединениеСРабочимПроцессом.GetInfoBases() Цикл
		...
		Если НРег(ИнформационнаяБаза.Name) = НРег(ИмяВКластере) Тогда
			...
			Если Не ЗначениеЗаполнено(ИнформационнаяБаза.DBMS) Тогда
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Los nombres incorrectos y la consigna del administrador de la base informativa %1 en el clúster de los servidores %2: el %3 (el nombre: ""%4"").';ru='Неправильные имя и пароль администратора информационной базы %1 в кластере серверов %2:%3 (имя: ""%4"").';en='Wrong username and password for administrator of infobase%1 in server cluster%2:%3 (name: ""%4"").'"),
					ИмяВКластере,
					Кластер.HostName, 
					Кластер.MainPort,
					ИмяАдминистратораИБ);
...
Функция ПолучитьПрофильБезопасности(СоединениеСАгентомСервера, Кластер, ИмяПрофиля)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='En el clúster de los servidores %1: el %2 no es encontrado el perfil de la seguridad ""%3""';ru='В кластере серверов %1:%2 не найден профиль безопасности ""%3""';en=""Security profile '% 3' not found in server cluster% 1:% 2"""),
		Кластер.HostName,
		Формат(Кластер.MainPort, "ЧГ=0"),
		ИмяПрофиля);
