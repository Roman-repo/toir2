...
Процедура ЗаписатьОбъект(Знач Объект, Знач РегистрироватьНаУзлахПлановОбмена = Неопределено, 
	...
	Если ДокументРежимЗаписи <> Неопределено Тогда
		Если ТипЗнч(ДокументРежимЗаписи) <> Тип("РежимЗаписиДокумента") Тогда
			ТекстИсключения = НСтр("es='El tipo incorrecto del parámetro de DokumentRezhimZapisi';ru='Неправильный тип параметра ДокументРежимЗаписи';en='Invalid parameter type ДокументРежимЗаписи'");
...
Процедура ПроверитьОбъектОбработан(Данные, Форма = Неопределено, ИмяОтложенногоОбработчика = "", ИмяПроцедурыПрограммногоИнтерфейса = "") Экспорт
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Es inadmisible llamar %1 "
"del elaborador de la renovación "
"%2 "
"puesto que su número del turno menos o es igual al número del turno del elaborador de la renovación "
"%3';ru='Недопустимо вызывать %1"
"из обработчика обновления"
"%2"
"так как его номер очереди меньше или равен номеру очереди обработчика обновления"
"%3';en='Invalid to call %1"
"from update handler"
"%2"
"because its queue number is less or equal to the line number of the update handler"
"%3'"),
...
Функция ОбъектОбработан(Данные) Экспорт
	...
	Если НезавершенныеОбработчики.Количество() > 0 Тогда
		ТекстИсключения = НСтр("es='Las acciones con el objeto son temporalmente prohibidas, puesto que no es acabado el tránsito a una nueva versión del programa. "
"Es el proceso planificado, que se acabará pronto. "
"Había unos procedimientos siguientes del tratamiento de los datos:';ru='Действия с объектом временно запрещены, так как не завершен переход на новую версию программы."
"Это плановый процесс, который скоро завершится."
"Остались следующие процедуры обработки данных:';en='Actions with the object are temporarily prohibited because the transition to the new version of the program has not been completed."
"This is a planned process that will end soon."
"The following data processing procedures remain:'");
...
Процедура ОтметитьВыполнениеОбработки(Данные, ДополнительныеПараметры = Неопределено, Очередь = Неопределено) Экспорт
	...
	Если (ТипЗнч(Данные) = Тип("Массив")
		...
		ТекстИсключения = НСтр("es='Al procedimiento ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки es entregado el macizo vacío. No es posible notar la ejecución del tratamiento.';ru='В процедуру ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки передан пустой массив. Не возможно отметить выполнение обработки.';en='In the procedure ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки passed an empty array. It is not possible to mark processing'");
		ВызватьИсключение ТекстИсключения;
	...
	Иначе
		Если ТипЗнч(Данные) = Тип("ОбъектМетаданных") Тогда
			ТекстИсключения = НСтр("es='No es apoyada la marca de la ejecución del tratamiento de la renovación por la regleta de blanco del objeto de los metadatos. Es necesario notar el tratamiento de los datos concretos.';ru='Не поддерживается отметка выполнения обработки обновления целиком объекта метаданных. Нужно отмечать обработку конкретных данных.';en='The flag to complete processing of updating the entire metadata object is not supported. It is necessary to note the processing of specific data.'");
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
...
Функция МетаданныеИОтборПоДанным(Данные, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИначеЕсли ТипДанных = Тип("ДанныеФормыСтруктура") Тогда
		...
		Иначе
			ТекстИсключения = НСтр("es='El procedimiento ОбновлениеИнформационнойБазы.МетаданныеИОтборПоДанным no puede ser usado para esta forma.';ru='Процедура ОбновлениеИнформационнойБазы.МетаданныеИОтборПоДанным не может быть использована для этой формы.';en='A procedure ОбновлениеИнформационнойБазы.МетаданныеИОтборПоДанным can not be used for this form.'");
		КонецЕсли;
	...
	Иначе
		...
		ИначеЕсли ОбщегоНазначения.ЭтоРегистр(МетаданныеОбъекта) Тогда
			...
			КонецЕсли;
		Иначе
			ТекстИсключения = НСтр("es='Para esto como los metadatos no es apoyado el análisis en la función ОбновлениеИнформационнойБазы.МетаданныеИОтборПоДанным.';ru='Для этого типа метаданных не поддерживается анализ в функции ОбновлениеИнформационнойБазы.МетаданныеИОтборПоДанным.';en='For this type of metadata, the analysis in ОбновлениеИнформационнойБазы.МетаданныеИОтборПоДанным.fubction is not supported.'");
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
...
Процедура ОтметитьКОбработке(ОсновныеПараметры, Данные, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ДополнительныеПараметры.ЭтоДвижения Тогда
		...
		Если ДополнительныеПараметры.ОтметитьВсеРегистраторы Тогда
			...
			Иначе
				ТекстИсключения = НСтр("es='Para el registro de todos los registradores del registro es necesario en el parámetro ""Dado"" entregar Ob''ektMetadannyh:Dokument o DokumentSsylka.';ru='Для регистрации всех регистраторов регистра необходимо в параметре ""Данные"" передать ОбъектМетаданных:Документ или ДокументСсылка.';en='To register all registrars of the register, it is necessary to transfer ОбъектМетаданных:Документ or ДокументСсылка.'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
	...
	Иначе
		...
		Иначе
			Если ТипЗнч(Данные) = Тип("ОбъектМетаданных") Тогда
				ТекстИсключения = НСтр("es='No es apoyado el registro a la renovación por la regleta de blanco del objeto de los metadatos. Es necesario renovar los datos concretos.';ru='Не поддерживается регистрация к обновлению целиком объекта метаданных. Нужно обновлять конкретные данные.';en='Registration for updating the whole metadata object is not supported. It is necessary to update specific data.'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
...
Функция ЕстьДанныеДляОбработки(Очередь, ПолноеИмяМетаданныеОбъекта, Отбор = Неопределено) Экспорт
	...
	Иначе
		ТекстИсключения = НСтр("es='Es entregado el tipo incorrecto del parámetro ""PolnoeImyaMetadannyeOb''ekta"" a la función ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки';ru='Передан неправильный тип параметра ""ПолноеИмяМетаданныеОбъекта"" в функцию ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки';en='Invalid parameter type passed ""ПолноеИмяМетаданныеОбъекта"" in function ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;	
	...
	Для каждого ОбрабатываемыйТип Из ПолныеИменаОбрабатываемыхОбъектов Цикл 
		...
		ИначеЕсли ОбщегоНазначения.ЭтоРегистрСведений(МетаданныеОбъекта)
			...
			Если ПолныеИменаОбрабатываемыхОбъектов.Количество() > 1 Тогда
				ТекстИсключения = НСтр("es='En el macizo de los nombres en el parámetro ""PolnoeImyaMetadannyeOb''ekta"" a la función ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки es entregado el registro independiente de las noticias.';ru='В массиве имен в параметре ""ПолноеИмяМетаданныеОбъекта"" в функцию ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки передан независимый регистр сведений.';en='In the array of names in the parameter ""ПолноеИмяМетаданныеОбъекта"" в функцию ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки an independent register of information has been transferred.'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;	
		...
		Иначе
			ТекстИсключения = НСтр("es='Para el tipo de los metadatos ""%МетаданныеОбъекта del %"" no es apoyada la comprobación en la función ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки';ru='Для типа метаданных ""%МетаданныеОбъекта%"" не поддерживается проверка в функции ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки';en='For metadata type ""%МетаданныеОбъекта%""function check is not supported  ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки'");
...
Функция СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(Очередь, ПолноеИмяОбъекта, МенеджерВременныхТаблиц, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Иначе
		ТекстИсключения = НСтр("es='Para esto como los metadatos no es apoyada la comprobación en la función ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных.';ru='Для этого типа метаданных не поддерживается проверка в функции ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных.';en='For this type of metadata, check in ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных.is not supported.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
...
Функция СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияСсылок(Очередь, ПолныеИменаОбъектов, МенеджерВременныхТаблиц, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Иначе	
		...
		Для Каждого ОбрабатываемыйТип Из ПолныеИменаОбъектовМассив Цикл
			...
			Иначе
				ТекстИсключения = НСтр("es='Para el tipo de los metadatos ""%МетаданныеОбъекта del %"" no es apoyada la comprobación en la función ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияСсылок';ru='Для типа метаданных ""%МетаданныеОбъекта%"" не поддерживается проверка в функции ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияСсылок';en='For metadata type ""%МетаданныеОбъекта%""function check is not supported   ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияСсылок'");
				ТекстИсключения = СтрЗаменить(ТекстИсключения, "%МетаданныеОбъекта%", Строка(МетаданныеОбъекта)); 
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
	...
	Иначе	
		...
		Если ДополнительныеИсточникиСсылки.Количество() > 0 Тогда
			...
			Если ПолноеИмяОбъекта = Неопределено Тогда
				ТекстИсключения = НСтр("es='La falta de la llamada de la función %ИмяФункции del %: no es entregado el nombre del documento, pero son entregadas las fuentes adicionales de los datos.';ru='Ошибка вызова функции %ИмяФункции%: не передано имя документа, но переданы дополнительные источники данных.';en='Error calling the function%ИмяФункции%: the document name was not transferred, but additional data sources were transferred.'");
		...
		Если ДополнительныеИсточникиРегистры.Количество() > 0 Тогда
			...
			Для Каждого ИсточникДанных Из ДополнительныеИсточникиРегистры Цикл
				...
				Если ОбщегоНазначения.ЭтоРегистрСведений(МетаданныеИсточника)
					...
					ТекстИсключения = НСтр("es='El registro %ИсточникДанных del % independiente. Es apoyada la comprobación solamente por los registros, los registradores subordinados.';ru='Регистр %ИсточникДанных% независимый. Поддерживается проверка только по регистрам, подчиненным регистраторам.';en='Register %ИсточникДанных% independent. Validation is supported only for registers subordinate to registrars.'");
...
Процедура ПроверитьПараметрыВыборки(Параметры)
	...
	Если Не Параметры.ВыбиратьПорциями И ЭтоПостраничнаяВыборка(Параметры) Тогда
		ВызватьИсключение НСтр("es='El elaborador mnogopotochnyy de la renovación es obligado a escoger dado por las porciones.';ru='Многопоточный обработчик обновления обязан выбирать данные порциями.';en='A multi-threaded update processor is required to select data in batches.'");
	КонецЕсли;
...
Функция ЭтоПростойИсточникДанных(ДополнительныеИсточникиДанных)
	...
	Если ПростойИсточник И СложныйИсточник Тогда
		Ошибка = НСтр("es='La fuente de los datos es dada incorrectamente (cm. ДополнительныеПараметрыВыборкиДанныхДляОбработки ()).';ru='Источник данных задан неверно (см. ДополнительныеПараметрыВыборкиДанныхДляОбработки()).';en='The data source is not set correctly (see ДополнительныеПараметрыВыборкиДанныхДляОбработки()).'");
