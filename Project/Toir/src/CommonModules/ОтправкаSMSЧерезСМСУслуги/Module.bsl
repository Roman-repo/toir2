...
"%1 (error code: %2) '"), ОтветСервера.descr, КодРезультата);
...
Функция СтатусДоставки(Знач ИдентификаторСообщения, НастройкиОтправкиSMS) Экспорт
	...
	Если КодРезультата = "1" Тогда
		...
		КонецЦикла;
	Иначе
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No consiguió recibir el estatus de la entrega SMS (smsid: ""%3""): "
"%1 (el código de la falta: %2)';ru='Не удалось получить статус доставки SMS (smsid: ""%3""):"
"%1 (код ошибки: %2)';en='Failed to get SMS delivery status (id:"" ""%3"" ""):"
"%1 (error code:%2)'"), ОтветСервера.descr, КодРезультата, ИдентификаторСообщения);
		ЗаписьЖурналаРегистрации(НСтр("es='El envío SMS';ru='Отправка SMS';en='SMS sending'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция Разрешения() Экспорт
	...
	Порт = Неопределено;
	Описание = НСтр("es='El envío SMS a través de ""los SMS-SERVICIOS"".';ru='Отправка SMS через ""СМС-Услуги"".';en='Sending SMS via SMS Services.'");
