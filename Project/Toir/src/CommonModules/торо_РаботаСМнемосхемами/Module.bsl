...
Процедура мнс_СоздатьНовуюКарту(ГрафическаяСхема,Картинка, СтруктураДопСвойств) Экспорт
	...
	Если Обработка = Неопределено Тогда
		...
		КонецЦикла;
		Если Обработка = Неопределено Тогда
			ТекстСообщения = НСтр("es='¡No consiguió recibir el tratamiento protegido, la operación no será cumplida! ¡Comprueben el enlace con el servidor de las licencias!';ru='Не удалось получить защищенную обработку, операция не будет выполнена! Проверьте связь с сервером лицензий!';en='Failed to get secure processing, operation will not be completed! Check the connection with the license server!'");
