...
Функция ЗаголовокФормыСотрудника(Форма) Экспорт
	...
	Если Форма.Параметры.Ключ.Пустая() Тогда
		Возврат НСТр("es='El empleado (creación)';ru='Сотрудник (создание)';en='Employee (creation)'")
	КонецЕсли;
	
	Возврат Форма.СотрудникНаименование + НСТр("es=' (El empleado)';ru=' (Сотрудник)';en=' (Employee)'");
...
Функция ЗаголовокФормыФизическогоЛица(Форма) Экспорт
	...
	Если Форма.Параметры.Ключ.Пустая() Тогда
		Возврат НСТр("es='La persona física (creación)';ru='Физическое лицо (создание)';en='Individual (creation)'")
	КонецЕсли;
	
	Возврат Форма.ФизическоеЛицо.Наименование + НСТр("es=' (La persona física)';ru=' (Физическое лицо)';en=' (Individual)'");
...
Процедура ОбработатьОтображениеПоляИНН(ИНН, Элемент, Форма) Экспорт
	...
	Если НЕ ПустаяСтрока(ИНН) Тогда
		...
		СообщенияПроверки = ?(ПустаяСтрока(СообщенияПроверки), НСтр("es='El INN es indicado correctamente';ru='ИНН указан правильно';en='TIN is correct'"), СообщенияПроверки);
		...
		Иначе
			...
			ТекстНадписи  = НСтр("es='La falta';ru='Ошибка';en='Error '");
			ЭлементЦветТекста = Форма.ЦветСтиляПоясняющийОшибкуТекст;
		КонецЕсли;
		
	Иначе
		
		СообщенияПроверки = НСтр("es='al INN (se usa, por ejemplo, en el informe por la forma 2-NDFL)';ru='Не указан ИНН (используется, например, в отчетности по форме 2-НДФЛ)';en='TIN is not indicated (used, for example, in reporting under form 2-NDFL)'");
		ТекстНадписи  = "";
		ЭлементЦветТекста = Форма.ЦветСтиляЦветТекстаПоля;
...
Процедура ОбработатьОтображениеПоляСтраховойНомерПФР(СтраховойНомерПФР ,Элемент, Форма) Экспорт
	...
	Если НЕ ПустаяСтрока(СтрЗаменить(СтраховойНомерПФР, "-", "")) Тогда
		...
		СообщенияПроверки = ?(ПустаяСтрока(СообщенияПроверки), НСтр("es='СНИЛС (el número ПФР de seguros) es indicado correctamente';ru='СНИЛС (страховой номер ПФР) указан правильно';en='SNILS (PFR insurance number) is indicated correctly'"), СообщенияПроверки);
	...
	Иначе
		...
		СообщенияПроверки = НСтр("es='СНИЛС (el número ПФР de seguros)';ru='Не указан СНИЛС (страховой номер ПФР)';en='SNILS (PFR insurance number) not specified'");
		ТекстНадписи  = "";
		ЭлементЦветТекста = Форма.ЦветСтиляЦветТекстаПоля;
		Картинка = Новый Картинка;
...
Процедура ОбработатьОтображениеСерияДокументаФизическогоЛица(ВидДокумента, Серия ,Элемент, Форма) Экспорт
	...
	Если ЗначениеЗаполнено(ВидДокумента) И ТипСерии > 0 Тогда
		Если НЕ ПустаяСтрока(Серия) Тогда
			...
			СообщенияПроверки = ?(ПустаяСтрока(СообщенияПроверки), НСтр("es='Una serie es indicada correctamente';ru='Серия указана правильно';en='Series is correct'"), СообщенияПроверки);
...
Процедура ОбработатьОтображениеНомерДокументаФизическогоЛица(ВидДокумента, Номер ,Элемент, Форма) Экспорт
	...
	Если ЗначениеЗаполнено(ВидДокумента) И ТипНомера > 0 Тогда
		Если НЕ ПустаяСтрока(Номер) Тогда
			...
			СообщенияПроверки = ?(ПустаяСтрока(СообщенияПроверки), НСтр("es='El número del documento es indicado correctamente';ru='Номер документа указан правильно';en='Document number is correct'"), СообщенияПроверки);
			Если СерияУказанаПравильно Тогда
				Картинка = БиблиотекаКартинок.Успешно;
				ТекстНадписи  = "";
				ЭлементЦветТекста = Форма.ЦветСтиляЦветТекстаПоля;
...
Процедура УстановитьПодсказкуКДатеРождения(Форма) Экспорт
	...
	Иначе
		...
		Форма.ФизическоеЛицоДатаРожденияНадпись = НСтр("es='La fecha del nacimiento no es indicada, "
"el salario no será contado';ru='Дата рождения не указана,"
"зарплата не будет рассчитана';en='Date of birth not specified,"
"salary will not be calculated '");
		ТекстПодсказки = НСтр("es='La fecha del nacimiento se usa al cálculo de los pagos de seguros en ПФР';ru='Дата рождения используется при расчете страховых взносов в ПФР';en='Date of birth used in calculating insurance premiums in the FIU'");
