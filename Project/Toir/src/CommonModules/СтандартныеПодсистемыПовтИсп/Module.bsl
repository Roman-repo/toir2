	...
	Для Каждого ИмяМодуля Из МодулиПодсистем Цикл
		...
		Если ОписанияПодсистем.ПоИменам.Получить(Описание.Имя) <> Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta con la preparación de las descripciones de los subsistemas: "
"en la descripción del subsistema (cm. El procedimiento %1. ПриДобавленииПодсистемы) "
"es indicado el nombre del subsistema ""%2"", que es registrado ya antes.';ru='Ошибка при подготовке описаний подсистем:"
"в описании подсистемы (см. процедуру %1.ПриДобавленииПодсистемы)"
"указано имя подсистемы ""%2"", которое уже зарегистрировано ранее.';en='Error preparing subsystem descriptions:"
"in the description of the subsystem (see procedure %1.ПриДобавленииПодсистемы) the name of the subsystem ""%2"", which is already registered earlier, is indicated.'"),
	...
	Если ОписаниеКонфигурацииНайдено Тогда
		...
		Если Описание.Версия <> Метаданные.Версия Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta con la preparación de las descripciones de los subsistemas: "
"la versión de ""%2"" configuraciones ""%1"" (cm. El procedimiento %3. ПриДобавленииПодсистемы) "
"no coincide con la versión de la configuración en los metadatos ""%4"".';ru='Ошибка при подготовке описаний подсистем:"
"версия ""%2"" конфигурации ""%1"" (см. процедуру %3.ПриДобавленииПодсистемы)"
"не совпадает с версией конфигурации в метаданных ""%4"".';en='Error preparing subsystem descriptions:"
"version ""%2"" of configuration ""%1"" (see procedure%3.ПриДобавленииПодсистемы)"
"does not match the configuration version in the metadata ""%4"".'"),
	...
	Иначе
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta con la preparación de las descripciones de los subsistemas: "
"en los módulos generales indicados en el procedimiento ПодсистемыКонфигурацииПереопределяемый.ПриДобавленииПодсистемы no es encontrada la descripción "
"de subsistema que coincide con el nombre de la configuración ""%1"".';ru='Ошибка при подготовке описаний подсистем:"
"в общих модулях, указанных в процедуре ПодсистемыКонфигурацииПереопределяемый.ПриДобавленииПодсистемы"
"не найдено описание подсистемы, совпадающей с именем конфигурации ""%1"".';en='Error preparing subsystem descriptions:"
"in the general modules specified in the procedure ПодсистемыКонфигурацииПереопределяемый.ПриДобавленииПодсистемы no description of subsystem matching configuration name ""%1"" was found.'"),
			Метаданные.Имя);
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	...
	Для каждого КлючИЗначение Из ВсеТребуемыеПодсистемы Цикл
		Если ОписанияПодсистем.ПоИменам.Получить(КлючИЗначение.Ключ) = Неопределено Тогда
			...
			КонецЦикла;
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta con la preparación de las descripciones de los subsistemas: no es encontrado "
"el subsistema ""%1"" exigido para los subsistemas: %2.';ru='Ошибка при подготовке описаний подсистем:"
"не найдена подсистема ""%1"" требуемая для подсистем: %2.';en='Error preparing subsystem descriptions:"
"subsystem ""%1"" not found required for the subsystems:%2. '"),
				КлючИЗначение.Ключ,
				ЗависимыеПодсистемы);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
...
Функция ОтключитьИдентификаторыОбъектовМетаданных() Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВариантыОтчетов")
	 ...
	 ИЛИ ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
		...
		ВызватьИсключение
			НСтр("es='Es imposible desconectar la guía los Identificadores de los objetos de los metadatos, "
"si se usa cualquier de los subsistemas siguientes: "
"- VariantyOtchetov, "
"- DopolnitelnyeOtchetyIObrabotki, "
"- RassylkaOtchetov, "
"- UpravlenieDostupom.';ru='Невозможно отключить справочник Идентификаторы объектов метаданных,"
"если используется любая из следующих подсистем:"
"- ВариантыОтчетов,"
"- ДополнительныеОтчетыИОбработки,"
"- РассылкаОтчетов,"
"- УправлениеДоступом.';en='Cannot disable the directory Metadata object identifiers,"
