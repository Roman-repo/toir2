...
Процедура ДобавитьУсловиеФильтра(Фильтр, Знач Свойство, Знач ВидСравненияЗначений, Знач Значение) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("es='El tipo equivocado del parámetro el Filtro, se espera <la Estructura> o <el Macizo>';ru='Неверный тип параметра Фильтр, ожидалось <Структура> или <Массив>';en='Invalid filter parameter type, expected <Структура> or <Массив>'");
...
Функция ПроверитьУсловияФильтра(Знач ПроверяемыйОбъект, Знач Фильтр = Неопределено) Экспорт
	...
	Для Каждого Условие Из Фильтр Цикл
		...
		Иначе
			...
			ВызватьИсключение НСтр("es='El tipo equivocado del significado del parámetro el Filtro, se espera <la Estructura> o <KlyuchIZnachenie>';ru='Неверный тип значения параметра Фильтр, ожидалось <Структура> или <КлючИЗначение>';en='Invalid filter parameter type, expected <Структура> or <КлючИЗначениие>'");
...
Функция МенеджерАдминистрирования(Знач ПараметрыАдминистрирования)
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El tipo desconocido del parámetro De ParametryAdministrirovaniya (%1), se espera ""COM"" o ""RAS""';ru='Неизвестный тип параметра ПараметрыАдминистрирования (%1), ожидалось ""COM"" или ""RAS""';en='Unknown type of AdministrationParameters parameter (%1), expected to be ""COM"" or ""RAS""'"), 
