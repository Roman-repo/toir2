...
Функция НовыйКонтекстВзаимодействия(
	...
	Если МестоЗапуска = "taxcomGetID" Тогда
		Результат.Вставить("СообщениеОНедоступностиДействия",
			НСтр("es='La recepción del identificador único del abonado Taksky es inaccesible para esta configuración.';ru='Получение уникального идентификатора абонента Такском недоступно для этой конфигурации.';en='Obtaining a unique Takcom subscriber identifier is not available for this configuration.'"));
	ИначеЕсли МестоЗапуска = "taxcomPrivat" Тогда
		Результат.Вставить("СообщениеОНедоступностиДействия",
			НСтр("es='El trabajo con el gabinete personal del abonado Taksky es inaccesible para esta configuración.';ru='Работа с личным кабинетом абонента Такском недоступна для этой конфигурации.';en='Work with Taxkom personal account is not available for this configuration.'"));
	КонецЕсли;
...
Процедура ДобавитьКомандыСервиса(
	...
	Исключение
		...
		Если ТипЗнч(КонтекстОбработчика) = Тип("Структура") Тогда
			...
			КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
				НСтр("es='La falta a la conexión al servicio del Apoyo De Internet';ru='Ошибка при подключении к сервису Интернет-поддержки';en='Error connecting to online support service'");
		КонецЕсли;
	...
	Исключение
		...
		Если ТипЗнч(КонтекстОбработчика) = Тип("Структура") Тогда
			...
			КонтекстОбработчика.ДействиеПриОшибкеДляКлиента = "ОтобразитьФормуОтсутствуетПодключение";
			КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
				НСтр("es='La falta a la recepción de los datos del servidor del Apoyo De Internet.';ru='Ошибка при получении данных с сервера Интернет-поддержки.';en='An error occurred while receiving data from the online support server.'");
		КонецЕсли;
...
Процедура ЗаписатьОбщиеПараметрыИПП(Знач ОбщиеПараметры)
	...
	// Проверка прав.
	Если Не Пользователи.РолиДоступны("ИспользованиеСервиса1СТакском", , Ложь) Тогда
		ВызватьИсключение НСтр("es='Es insuficientemente derecho para la anotación de los parámetros del Apoyo De Internet.';ru='Недостаточно прав для записи параметров Интернет-поддержки.';en='Not enough rights to write Internet support settings.'");
	КонецЕсли;
...
Процедура ПодготовитьСертификатЭПКОтправке(КСКонтекст, КонтекстОбработчика)
	...
	Если СертификатЭП = Неопределено Тогда
		...
		КонтекстОбработчика.ПроизошлаОшибка = Истина;
		КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
			НСтр("es='No consiguió recibir los datos binarios del certificado. El certificado no es descubierto en la lista de los parámetros.';ru='Не удалось получить двоичные данные сертификата. Сертификат не обнаружен в списке параметров.';en='Failed to get binary certificate data. The certificate was not found in the parameter list.'");
		КонтекстОбработчика.ДействиеПриОшибкеДляКлиента = "ПоказатьСообщение";
		КонтекстОбработчика.ДействияПриОшибкеДляСервера.Добавить("ПрерватьБизнесПроцесс");
		Возврат;
	...
	Если ДвоичныеДанныеСертификата = Неопределено Тогда
		...
		СообщениеДляЖурналаРегистрации = НСтр("es='La falta a la recepción de los datos binarios del certificado. Los datos binarios del certificado faltan, o el certificado era quitado.';ru='Ошибка при получении двоичных данных сертификата. Двоичные данные сертификата отсутствуют, либо сертификат был удален.';en='Error retrieving binary certificate data. The binary data for the certificate is missing or the certificate has been deleted.'");
		...
		КонтекстОбработчика.ДействияПриОшибкеДляСервера.Добавить("ПрерватьБизнесПроцесс");
		КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
			НСтр("es='La falta a la recepción de los datos del certificado. Más detalladamente cm. En la revista del registro.';ru='Ошибка при получении данных сертификата. Подробнее см. в журнале регистрации.';en='Error getting certificate data. See the logbook for more details.'");
		КонтекстОбработчика.ДействиеПриОшибкеДляКлиента = "ПоказатьСообщение";
		Возврат;
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("es='La conexión 1S-Takskom';ru='Подключение 1С-Такском';en='Connection 1C-Taxkom'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПодготовитьДанныеОбОрганизации(КСКонтекст, КонтекстОбработчика)
	...
	Исключение
		...
		КонтекстОбработчика.ПолноеОписаниеОшибки =
			НСтр("es='No consiguió recibir las organizaciones de registro dadas.';ru='Не удалось получить регистрационные данные организации.';en='Failed to get organization registration information.'")
			+ " " + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		КонтекстОбработчика.ДействияПриОшибкеДляСервера.Добавить("СоздатьЗаписьВЖурналеРегистрации");
		КонтекстОбработчика.ДействияПриОшибкеДляСервера.Добавить("ПрерватьБизнесПроцесс");
		
		КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
			НСтр("es='La falta a la recepción de los datos de la organización. "
"Más detalladamente cm. En la revista del registro';ru='Ошибка при получении данных организации."
"Подробнее см. в журнале регистрации';en='Error retrieving organization data."
"For more information, see the logbook'");
		КонтекстОбработчика.ДействиеПриОшибкеДляКлиента = "ПоказатьСообщение";
		Возврат;
