...
Функция СоздатьАдминистратора(ПользовательИБ = Неопределено) Экспорт
	...
	Если Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение НСтр("es='La guía los Usuarios es inaccesible en el régimen no dividido.';ru='Справочник Пользователи недоступен в неразделенном режиме.';en='Directory Users not available in unshared mode.'");
	КонецЕсли;
	...
	Если ПользовательИБ = Неопределено Тогда
		...
		Если ПользователиИБ.Количество() = 0 Тогда
			Если ОбщегоНазначения.РазделениеВключено() Тогда
				ВызватьИсключение
					НСтр("es='Es imposible automáticamente crear al primer administrador de la área de los datos.';ru='Невозможно автоматически создать первого администратора области данных.';en='Unable to automatically create first data area administrator.'");
			КонецЕсли;
		...
		Иначе
			...
			ТекстОшибки =
				НСтр("es='No consiguió "
"encontrar la lista de los usuarios de la base informativa no vacío, sin embargo ni un usuario con los papeles los Plenos derechos y el Administrador del sistema. "
""
"Es probable, los usuarios se creaban en конфигураторе. "
"Es necesario fijar los papeles los Plenos derechos y el Administrador del sistema por lo menos a un usuario.';ru='Список пользователей информационной базы не пустой, однако не удалось"
...
"Требуется назначить роли Полные права и Администратор системы хотя бы одному пользователю.';en='The list of users of the infobase is not empty, but failed"
"find no users with roles Full rights and System administrator."
""
"Probably, users were created in the configurator."
"At least one user must be assigned Full Rights and System Administrator roles.'");
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
	Иначе
		Если Не ПользователиСлужебный.РолиАдминистратораДоступны(ПользовательИБ) Тогда
			ТекстОшибки =
				НСтр("es='Es imposible crear al usuario en la guía para el usuario "
"de la base ""%1"" informativa, "
"puesto que él no tiene papeles los Plenos derechos y el Administrador del sistema. "
""
"Es probable, el usuario era creado en конфигураторе. "
"La creación automática del usuario en la guía tiene que "
"fijar a ello los papeles los Plenos derechos y el Administrador del sistema.';ru='Невозможно создать пользователя в справочнике для пользователя"
...
"назначить ему роли Полные права и Администратор системы.';en='Unable to create user in user directory"
"infobase ""%1"","
"since it does not have the roles Full rights and System administrator."
""
"The user was probably created in the configurator."
"To automatically create a user in the directory requires"
"Assign him the roles Full Rights and System Administrator.'");
	...
	Пользователь.ДополнительныеСвойства.Вставить("СозданиеАдминистратора",
		?(ПользовательИБ = Неопределено,
		  НСтр("es='Es cumplida la creación del primer administrador.';ru='Выполнено создание первого администратора.';en='Created first administrator.'"),
		  ?(ПользовательСоздан,
		    НСтр("es='El administrador es confrontado con un nuevo usuario de la guía.';ru='Администратор сопоставлен с новым пользователем справочника.';en='The administrator is mapped to the new directory user.'"),
		    НСтр("es='El administrador es confrontado con el usuario existente de la guía.';ru='Администратор сопоставлен с существующим пользователем справочника.';en='The administrator is mapped to an existing directory user.'")) ) );
...
Функция ПолноеИмяНеуказанногоПользователя() Экспорт
	...
	Возврат НСтр("es='<no es indicado>';ru='<Не указан>';en='<Не указан>'");
...
Процедура УстановитьСвойстваПользователяИБ(Знач ИмяИлиИдентификатор, Знач ОбновляемыеСвойства,
	...
	Если Не ПользовательСуществует Тогда
		Если Не СоздатьНового Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El usuario de la base informativa ""%1"" no existe.';ru='Пользователь информационной базы ""%1"" не существует.';en='The user of the infobase ""%1"" does not exist.'"),
				ИмяИлиИдентификатор);
		КонецЕсли;
		ПользовательИБ = ПользователиИнформационнойБазы.СоздатьПользователя();
	Иначе
		Если СоздатьНового Тогда
			ВызватьИсключение ОписаниеОшибкиПриЗаписиПользователяИБ(
				НСтр("es='Es imposible crear al usuario de la base %1 informativa, puesto que él existe ya.';ru='Невозможно создать пользователя информационной базы %1, так как он уже существует.';en='Unable to create the infobase user%1 because he already exists.'"),
				СтарыеСвойства.Имя,
		...
		Если ОбновляемыеСвойства.Свойство("СтарыйПароль")
		   И ТипЗнч(ОбновляемыеСвойства.СтарыйПароль) = Тип("Строка") Тогда
			...
			Если Не СтарыйПарольСовпадает Тогда
				ВызватьИсключение ОписаниеОшибкиПриЗаписиПользователяИБ(
					НСтр("es='A la anotación del usuario de la base informativa %1 consigna vieja es indicada no justo.';ru='При записи пользователя информационной базы %1 старый пароль указан не верно.';en='When writing the user of the infobase%1, the old password is incorrect.'"),
					СтарыеСвойства.Имя,
					СтарыеСвойства.УникальныйИдентификатор);
			КонецЕсли;
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекстОшибкиПароля) Тогда
		ВызватьИсключение ОписаниеОшибкиПриЗаписиПользователяИБ(
			НСтр("es='No consiguió anotar las propiedades del usuario de la base informativa %1 por la causa: "
"%2.';ru='Не удалось записать свойства пользователя информационной базы %1 по причине:"
"%2.';en='Failed to write user properties of infobase%1 due to:"
"%2.'"),
			ПользовательИБ.Имя,
	...
	Если ТипЗнч(Приемник) = Тип("ПользовательИнформационнойБазы")
	 ...
	 Или ТипЗнч(Источник) = Тип("ПользовательИнформационнойБазы")
	   ...
	   И ТипЗнч(Приемник) <> Тип("УправляемаяФорма") Тогда
		...
		ВызватьИсключение
			НСтр("es='El significado inadmisible del parámetro el Receptor o la Fuente "
"en el procedimiento de SkopirovatSvoystvaPolzovatelyaIB del módulo general los Usuarios.';ru='Недопустимое значение параметра Приемник или Источник"
"в процедуре СкопироватьСвойстваПользователяИБ общего модуля Пользователи.';en='Invalid value of the parameter Receiver or Source in the procedure СкопироватьСвойстваПользователяИБ general module Users.'");
	КонецЕсли;
					...
					Если Свойство = "Имя"
					   И Приемник[Свойство] <> ЗначениеСвойства Тогда
						...
						Если СтрДлина(ЗначениеСвойства) > 64 Тогда
							ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La falta de la anotación del usuario de la base informativa "
"el Nombre (para la entrada): ""%1"" "
"supera la longitud de 64 símbolos.';ru='Ошибка записи пользователя информационной базы"
"Имя (для входа): ""%1"""
"превышает длину 64 символа.';en='Error writing user infobase"
"Name (for login): ""%1"""
"exceeds 64 characters.'"),
						...
						ИначеЕсли СтрНайти(ЗначениеСвойства, ":") > 0 Тогда
							ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La falta de la anotación del usuario de la base informativa "
"el Nombre (para la entrada): ""%1"" "
"contiene el símbolo prohibido "":"".';ru='Ошибка записи пользователя информационной базы"
"Имя (для входа): ""%1"""
"содержит запрещенный символ "":"".';en='Error writing user infobase"
"Name (for login): """"%1 """""
...
Процедура НайтиНеоднозначныхПользователейИБ(Знач Пользователь,
	...
	ОписаниеОшибки = НСтр("es='La falta en la base de datos.';ru='Ошибка в базе данных.';en='Error in database.'") + Символы.ПС;
		...
		Если Строка.НеоднозначныйИдентификатор <> ТекущийНеоднозначныйИдентификатор Тогда
			...
			Иначе
				...
				Если ТекущийПользовательИБ = Неопределено Тогда
					ИмяДляВхода = НСтр("es='<no es encontrado>';ru='<не найден>';en='<не найден>'");
				Иначе
				...
				Если ИдентификаторПользователяСервиса Тогда
					ОписаниеОшибки = ОписаниеОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Al usuario del servicio con el identificador ""%1"" "
"le corresponde más de un elemento en la guía:';ru='Пользователю сервиса с идентификатором ""%1"""
"соответствует более одного элемента в справочнике:';en='Service user with identifier ""%1"" matches more than one item in the directory:'"),
						ТекущийНеоднозначныйИдентификатор);
				Иначе
					ОписаниеОшибки = ОписаниеОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Al usuario ИБ ""%1"" con el identificador ""%2"" "
"le corresponde más de un elemento en la guía:';ru='Пользователю ИБ ""%1"" с идентификатором ""%2"""
"соответствует более одного элемента в справочнике:';en='IB user ""%1"" with identifier ""%2"""
"matches more than one item in the directory:'"),
						ИмяДляВхода,
						ТекущийНеоднозначныйИдентификатор);
				КонецЕсли;
				ОписаниеОшибки = ОписаниеОшибки + Символы.ПС;
...
Процедура УстановитьПравоОткрытияВнешнихОтчетовИОбработок(ОткрытиеРазрешено) Экспорт
	...
	Для Каждого ПользовательИБ Из ПользователиИБ Цикл
		...
		ОписаниеНастроек.Представление = НСтр("es='La prevención de la seguridad';ru='Предупреждение безопасности';en='Safety warning'");
...
Функция ЗаписатьПользователяИБ(Знач Идентификатор,
	...
	ИначеЕсли СоздатьНового = Истина Тогда
		ОписаниеОшибки = ОписаниеОшибкиПриЗаписиПользователяИБ(
			НСтр("es='Es imposible crear al usuario de la base "
"%1 informativa, "
"puesto que él existe ya.';ru='Невозможно создать пользователя информационной базы"
"%1,"
"так как он уже существует.';en='Unable to create infobase user"
"%1,"
"as he already exists.'"),
			СтарыеСвойства.Имя,
			СтарыеСвойства.УникальныйИдентификатор);
		Возврат Ложь;
	Иначе
		Если ОбновляемыеСвойства.Свойство("СтарыйПароль")
		   И ТипЗнч(ОбновляемыеСвойства.СтарыйПароль) = Тип("Строка") Тогда
			...
			Если НЕ СтарыйПарольСовпадает Тогда
				ОписаниеОшибки = ОписаниеОшибкиПриЗаписиПользователяИБ(
					НСтр("es='A la anotación del usuario de la base "
"%1 informativa, "
"la consigna vieja es indicada no justo.';ru='При записи пользователя информационной базы"
"%1,"
"старый пароль указан не верно.';en='When recording user infobase"
"%1,"
...
"%2.'"),
	...
	Исключение
		...
		ОписаниеОшибки = ОписаниеОшибкиПриЗаписиПользователяИБ(
			НСтр("es='A la anotación del usuario de la base informativa "
"%1 había una falta: "
""
"%2.';ru='При записи пользователя информационной базы"
"%1 произошла ошибка:"
""
"%2.';en='When recording user infobase"
"%1 error occurred:"
""
"%2.'"),
			ПользовательИБ.Имя,
...
Функция ОписаниеОшибкиПриЗаписиПользователяИБ(ШаблонОшибки,
	...
	Если ЗаписатьВЖурнал Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("es='Los usuarios. La falta de la anotación del usuario ИБ';ru='Пользователи.Ошибка записи пользователя ИБ';en='Users. IB user account error'",
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки,
				"""" + ИмяДляВхода + """ (" + ?(ЗначениеЗаполнено(ИдентификаторПользователяИБ),
					НСтр("es='Nuevo';ru='Новый';en='New'"), Строка(ИдентификаторПользователяИБ)) + ")",
