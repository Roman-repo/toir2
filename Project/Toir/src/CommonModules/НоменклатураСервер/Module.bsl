...
Функция НаименованиеПоШаблону (Знач ШаблонНаименования, ОбъектСправочник) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		ТекстИсключения = НСтр("es='La configuración trabaja en el régimen del servicio. La formación del nombre debe pasar sobre el cliente.';ru='Конфигурация работает в режиме сервиса. Формирование наименования должно происходить на клиенте.';en='The configuration works in service mode. The formation of the name should occur on the client.'");
	...
	Если ЗначениеЗаполнено(ФормулаНаименования) Тогда
		...
		Исключение
			...
			ШаблонСообщенияОбОшибке = НСтр("es='Es imposible formar el nombre por dado para el tipo de la nomenclatura ""%ВидНоменклатуры del %"" al modelo. Comprueben la corrección del modelo.';ru='Невозможно сформировать наименование по заданному для вида номенклатуры ""%ВидНоменклатуры%"" шаблону. Проверьте правильность шаблона.';en='Unable to generate a name according to the template specified for the item type ""%ВидНоменклатуры%"". Check the correctness of the template.'");
...
Процедура ПроверитьЗаполнениеХарактеристик(Объект,МассивНепроверяемыхРеквизитов,Отказ,ПараметрыПроверки = Неопределено) Экспорт
	...
	ШаблонСообщения = НСтр("es='"".';ru='Не заполнена колонка ""%Характеристика%"" в строке %НомерСтроки% списка ""%ТаблицаТовары%"".';en='The column ""%Характеристика%"" in the line  %НомерСтроки% of the list ""%ТаблицаТовары%"" is not filled.'");
...
Процедура ПроверитьЗаполнениеСодержания(Объект, Отказ, ИмяТЧ) Экспорт
	...
	ШаблонСообщения = НСтр("es='"".';ru='Не заполнена колонка """"Содержание"""" в строке %НомерСтроки% списка ""%ТаблицаТовары%"".';en='The column """" Content """" in the line %НомерСтроки% of the list ""%ТаблицаТовары%"" is not filled.'");
...
Процедура ПроверитьЗаполнениеУпаковок(Объект,МассивНепроверяемыхРеквизитов,Отказ,ПараметрыПроверки = Неопределено) Экспорт
	...
	ШаблонСообщения = НСтр("es='"".';ru='Не заполнена колонка ""%Упаковка%"" в строке %НомерСтроки% списка ""%ТаблицаТовары%"".';en='The column ""%Упаковка%"" in the line %НомерСтроки% of the list ""%ТаблицаТовары%"" is not filled.'");
...
Процедура ПроверитьВидНоменклатурыОприходования(Объект, Отказ, ИмяТЧДляПроверки = "Товары") Экспорт
	...
	Пока Выборка.Следующий() Цикл
		ТекстСообщения = НСтр("es='El tipo de la nomenclatura de otra cualidad (la mercancía estropeada) debe coincidir con el tipo de la nomenclatura inicial, porque tiene lugar el recuento por серям las nomenclaturas';ru='Вид номенклатуры другого качества (испорченного товара) должен совпадать с видом исходной номенклатуры, т.к. ведется учет по серям номенклатуры';en='The type of nomenclature of a different quality (spoiled goods) must coincide with the type of the original nomenclature, since stock grades are kept'");
...
Процедура ВызватьИсключениеПоОшибкеШаблонаНаименования(ИмяОперанда)
	...
	ШаблонСообщенияОбОшибке = НСтр("es='Es imposible contar el nombre por el modelo. La falta en el nombre del operando ""%ИмяОперанда del %"".';ru='Невозможно рассчитать наименование по шаблону. Ошибка в имени операнда ""%ИмяОперанда%"".';en='Unable to calculate the name of the template. Error in operand name ""%ИмяОперанда%"".'");
...
Процедура ПроверитьЗаполнениеСерий(Объект,Отказ,МассивНепроверяемыхРеквизитов = Неопределено,ВыдаватьСообщения = Истина,ПараметрыПроверки = Неопределено) Экспорт
	...
	ШаблонСообщения = НСтр("es='"".';ru='Не заполнена колонка ""%Серия%"" в строке %НомерСтроки% списка ""%ТаблицаТовары%"".';en='The column ""%Серия%"" in the line %НомерСтроки% of the list ""%ТаблицаТовары%"" is not filled.'");
