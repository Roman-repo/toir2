...
Функция ОбновитьКлассификаторы(Идентификаторы) Экспорт
	...
	Если РезультатОперации.ДанныеКлассификаторов.Количество() = 0 Тогда
		...
		РезультатОбновления.СообщениеОбОшибке  = НСтр("es='La renovación no es necesario. Es cargada la versión actual del clasificador.';ru='Обновление не требуется. Загружена актуальная версия классификатора.';en='No update required. The current version of the classifier is loaded.'");
		РезультатОбновления.ИнформацияОбОшибке = НСтр("es='La renovación no es necesario. Es cargada la versión actual del clasificador.';ru='Обновление не требуется. Загружена актуальная версия классификатора.';en='No update required. The current version of the classifier is loaded.'");
		Возврат РезультатОбновления;
	КонецЕсли;
	...
	Если НеОбработанныеКлассификаторы.Количество() > 0 Тогда
		...
		РезультатОперации.СообщениеОбОшибке  = НСтр("es='No consiguió tratar las renovaciones de los clasificadores.';ru='Не удалось обработать обновления классификаторов.';en='Failed to process classifier updates.'");
		РезультатОперации.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Al tratamiento de las renovaciones cargadas de los clasificadores %1 han surgido las faltas.';ru='При обработке загруженных обновлений классификаторов %1 возникли ошибки.';en='Errors occurred while processing downloaded classifier updates%1.'"),
...
Процедура УстановитьВерсиюКлассификатора(Идентификатор, Версия) Экспорт
	...
	Иначе
		...
		Если Не Запись.Выбран() Тогда
			ВызватьИсключение НСтр("es='El clasificador por el identificador no es descubierto.';ru='Классификатор по идентификатору не обнаружен.';en='Classifier by ID not found.'");
		КонецЕсли;
	КонецЕсли;
...
Функция ВерсияКлассификатора(Идентификатор, ВызыватьИсключение = Ложь) Экспорт
	...
	Если ВызыватьИсключение Тогда
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El clasificador %1 no es registrado.';ru='Классификатор %1 не зарегистрирован.';en='Classifier %1 is not registered.'"),
			Идентификатор);
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		443,
		НСтр("es='El servicio de los clasificadores (ru)';ru='Сервис классификаторов (ru)';en='Classifier service (ru)'"));
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		443,
		НСтр("es='El servicio de los clasificadores (eu)';ru='Сервис классификаторов (eu)';en='Classifier service (eu)'"));
	НовыеРазрешения.Добавить(Разрешение);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий         = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='%1. La renovación de la lista de los clasificadores.';ru='%1. Обновление списка классификаторов.';en='%1. Updating the list of classifiers.'"),
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Обработчик.Комментарий         = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='%1. La renovación de la lista de los clasificadores de las áreas de los datos.';ru='%1. Обновление списка классификаторов областей данных.';en='%1. Updating the list of classifiers for data areas.'"),
			ИмяСобытияЖурналаРегистрации());
		...
		Обработчик.Комментарий         = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='%1. El traslado de los ajustes al registro de las noticias de VersiiKlassifikatorovOblasteyDannyh.';ru='%1. Перенос настроек в регистр сведений ВерсииКлассификаторовОбластейДанных.';en='%1. Transferring settings to the register of information of the Classifications of Data Areas.'"),
			ИмяСобытияЖурналаРегистрации());
...
Процедура ОбновитьНастройкиРаботыСКлассификаторами() Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("es='La renovación de los ajustes del subsistema ""el Trabajo con los clasificadores"". El comienzo de la renovación.';ru='Обновление настроек подсистемы ""Работа с классификаторами"". Начало обновления.';en='Updating the settings of the subsystem ""Working with classifiers"". Start of update.'"),
		Ложь);
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("es='La renovación de los ajustes del subsistema ""el Trabajo con los clasificadores"". Es con éxito acabado.';ru='Обновление настроек подсистемы ""Работа с классификаторами"". Успешно завершено.';en='Updating the settings of the subsystem ""Working with classifiers"". Successfully completed.'"),
		Ложь);
...
Процедура ОбновитьНастройкиРаботыСКлассификаторамиОбластейДанных(Параметры) Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("es='La renovación de los ajustes de las esferas del subsistema ""el Trabajo con los clasificadores"". El comienzo de la renovación.';ru='Обновление настроек областей подсистемы ""Работа с классификаторами"". Начало обновления.';en='Updating the settings of the areas of the subsystem ""Working with classifiers"". Start of update.'"),
		Ложь);
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("es='La renovación de los ajustes de las esferas del subsistema ""el Trabajo con los clasificadores"". Es con éxito acabado.';ru='Обновление настроек областей подсистемы ""Работа с классификаторами"". Успешно завершено.';en='Updating the settings of the areas of the subsystem ""Working with classifiers"". Successfully completed.'"),
		Ложь);
...
Процедура ЗаполнитьДанныеРегистраВерсииКлассификаторовОбластейДанных(Параметры) Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("es='El traslado de los ajustes al registro de las noticias de VersiiKlassifikatorovOblasteyDannyh. El comienzo de la renovación.';ru='Перенос настроек в регистр сведений ВерсииКлассификаторовОбластейДанных. Начало обновления.';en='Transferring settings to the information register ВерсииКлассификаторовОбластейДанных.Start of update.'"),
		Ложь);
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("es='El traslado de los ajustes al registro de las noticias de VersiiKlassifikatorovOblasteyDannyh. Es con éxito acabado.';ru='Перенос настроек в регистр сведений ВерсииКлассификаторовОбластейДанных. Успешно завершено.';en='Transferring settings to the information register ВерсииКлассификаторовОбластейДанных.Successfully completed. '"),
		Ложь);
...
Процедура ПриУдаленииЛогинаИПароляИзИБ() Экспорт
	...
	Если Не ИнтернетПоддержкаПользователей.ДоступноПодключениеИнтернетПоддержки() Тогда
		ВызватьИсключение НСтр("es='La desconexión el Internet del apoyo de los usuarios no es accesible.';ru='Отключение интернет поддержки пользователей не доступно.';en='Disabling online user support is not available.'");
	КонецЕсли;
...
Процедура ОбновлениеКлассификаторов() Экспорт
	...
	Если Идентификаторы.Количество() = 0 Тогда
		ЗаписатьИнформациюВЖурналРегистрации(
			НСтр("es='Faltan los clasificadores para la renovación automática.';ru='Отсутствуют классификаторы для автоматического обновления.';en='There are no classifiers for automatic updates.'"),
			Ложь);
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьРегламентноеЗаданиеОбновленияКлассификаторов()
	...
	Если Не ОбщегоНазначения.РазделениеВключено()
		...
		Если ЗаданияОбновления.Количество() = 0 Тогда
			...
			ПараметрыЗадания.Вставить("Наименование",  НСтр("es='La renovación de los clasificadores';ru='Обновление классификаторов';en='Classifiers update'"));
			
			РегламентныеЗаданияСервер.ДобавитьЗадание(ПараметрыЗадания);
			
			ЗаписатьИнформациюВЖурналРегистрации(
				НСтр("es='Es creada la tarea de reglamento de la renovación de los clasificadores.';ru='Создано регламентное задание обновления классификаторов.';en='A scheduled task for updating classifiers has been created.'"),
				Ложь,
				Метаданные.РегламентныеЗадания.ОбновлениеКлассификаторов);
		КонецЕсли;
...
Процедура УдалитьРегламентноеЗаданиеОбновленияКлассификаторов()
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("es='Es quitada la tarea de reglamento de la renovación de los clasificadores';ru='Удалено регламентное задание обновления классификаторов';en='Classifier update scheduled task deleted'"),
		Ложь,
		Метаданные.РегламентныеЗадания.ОбновлениеКлассификаторов);
...
Функция ИнформацияОбАктуальныхВерсияхКлассификаторов(
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El comienzo de la recepción de la información sobre las versiones actuales de los clasificadores: %1';ru='Начало получения информации об актуальных версиях классификаторов: %1';en='Start of receiving information on current versions of classifiers:%1'"),
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		ЧастиСтрок.Добавить(НСтр("es='No consiguió recibir las versiones actuales de los clasificadores.';ru='Не удалось получить актуальные версии классификаторов.';en='Failed to get current versions of classifiers.'"));
		...
		ЧастиСтрок.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La información técnica sobre la falta: "
"a la recepción de la información sobre las versiones actuales de los clasificadores el servicio ha devuelto la falta. "
"URL: %1 "
"Código de la falta: %2 "
"información Detallada: "
"%3';ru='Техническая информация об ошибке:"
"При получении информации об актуальных версиях классификаторов сервис вернул ошибку."
"URL: %1"
"Код ошибки: %2"
"Подробная информация:"
"%3';en='Technical error information:"
"When receiving information about the current versions of classifiers, the service returned an error."
"URL:%1"
"Error code:%2"
...
"%3 '"),
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Es acabada la recepción de las versiones actuales de los clasificadores: %1';ru='Завершено получение актуальных версий классификаторов: %1';en='Received current versions of classifiers completed:% 1'"),
		СтрСоединить(Идентификаторы, ","));
...
Процедура ПрочитатьДанные_versionlatest(
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Es recibida la respuesta del Servicio de los clasificadores: "
"%1';ru='Получен ответ Сервиса классификаторов:"
"%1';en='Received response from the classifier service:"
"%1'"),
		ТелоJSON);
	...
	Для Каждого ОписательВерсии Из ДанныеКлассификаторов Цикл
		Если Не ЗначениеЗаполнено(ОписательВерсии.Идентификатор)
			...
			СообщениеОбОшибке = НСтр("es='El formato equivocado de la respuesta del Servicio de los clasificadores.';ru='Неверный формат ответа Сервиса классификаторов.';en='Invalid response format of the Classifier service.'");
		...
		Если Прав(НРег(СокрЛП(СтруктураURI.Хост)), 6) <> Прав(НРег(СокрЛП(ХостСервиса)), 6) Тогда
			...
			СообщениеОбОшибке = НСтр("es='La dirección equivocada del fichero de la renovación del clasificador.';ru='Неверный адрес файла обновления классификатора.';en='Invalid classifier update file address.'");
			ЗаписатьИнформациюВЖурналРегистрации(СообщениеОбОшибке);
			ВызватьИсключение СообщениеОбОшибке;
	...
	Для Каждого ОписательКлассификатора Из ДанныеКлассификаторов Цикл
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La recepción del fichero del clasificador: %1';ru='Получение файла классификатора: %1';en='Getting classifier file:%1'"),
		...
		Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
			...
			РезультатОперации.СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta a la recepción del fichero del clasificador "
"%1: %2';ru='Ошибка при получении файла классификатора %1: "
"%2';en='Error getting classifier file%1:"
"%2 '"),
				ОписательКлассификатора.Идентификатор,
				РезультатОтправки.СообщениеОбОшибке);
				
			РезультатОперации.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió recibir el fichero del clasificador %1. "
"%2 "
""
"información Técnica sobre la falta: "
"a la carga del fichero el servicio ha devuelto la falta. "
"El código de la falta: %3. "
"URL del Fichero: %4 "
"información Detallada: "
"%5';ru='Не удалось получить файл классификатора %1."
"%2"
""
...
"URL Файла: %4"
"Подробная информация:"
"%5';en='Failed to get classifier file%1."
"%2"
""
"Technical error information:"
"When downloading a file, the service returned an error."
"Error code:%3."
"File URL:%4"
"Detailed information:"
"%5'"),
...
"%2 '"),
				ОписательКлассификатора.Идентификатор,
				НСтр("es='Es recibido el fichero incorrecto.';ru='Получен некорректный файл.';en='Invalid file received.'"));
				
			РезультатОперации.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió recibir el fichero del clasificador %1. "
"La suma de control de fichero recibido se distingue de esperado.';ru='Не удалось получить файл классификатора %1."
"Контрольная сумма полученного файла отличается от ожидаемой.';en='Failed to get classifier file%1."
"The checksum of the received file is different than expected. '"),
				ОписательКлассификатора.Идентификатор);
	...
	Иначе
		Для Каждого ОписаниеКлассификатора Из ДанныеКлассификаторов Цикл
			Если ОписаниеКлассификатора.ДанныеФайла.Размер() = 0 Тогда
				ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El fichero del clasificador %1 (%2) tiene la dimensión igual 0. "
"La carga de los datos es parada.';ru='Файл классификатора %1 (%2) имеет размер равный 0."
"Загрузка данных остановлена.';en='Classifier file%1 (%2) has a size of 0."
	...
	Если НеОбработанныеКлассификаторы.Количество() > 0 Тогда
		...
		Для Каждого ОписаниеКлассификатора Из ДанныеКлассификаторов Цикл
			Если НеОбработанныеКлассификаторы.Найти(ОписаниеКлассификатора.Идентификатор) <> Неопределено Тогда
				СообщениеОбОшибке = СообщениеОбОшибке
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='%1. La versión %2 clasificadores %3 no es cargada;';ru='%1. Версия %2 классификатора %3 не загружена;';en='%1 Version% 2 of classifier% 3 is not loaded;'"),
							Сч,
		...
		РезультатОбновления.КодОшибки         = КодОшибкиНеОбработан();
			РезультатОбновления.СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió tratar las renovaciones de los clasificadores: "
""
"%1';ru='Не удалось обработать обновления классификаторов:"
""
"%1';en='Failed to process classifier updates:"
""
"%1'"),
				СообщениеОбОшибке);
	КонецЕсли;
...
Функция ДанныеКлассификаторовДляИнтерактивногоОбновления() Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ВызватьИсключение НСтр("es='El uso de la función en el trabajo en el modelo del servicio es prohíbido.';ru='Использование функции при работе в модели сервиса запрещено.';en='Using the function while working in the service model is prohibited.'");
	КонецЕсли;
...
Функция ПереопределитьСообщениеПользователю(КодОшибки)
	...
	Если КодОшибки = "НеизвестныйКлассификаторИлиПрограмма" Тогда
		Возврат НСтр("es='El clasificador o el programa por el identificador no son descubiertos.';ru='Классификатор или программа по идентификатору не обнаружены.';en='Classifier or program by identifier was found.'");
	ИначеЕсли КодОшибки = "НетДоступаКПрограмме" Тогда
		ЧастиСтрок = Новый Массив;
		ЧастиСтрок.Добавить(НСтр("es='El acceso a la renovación del clasificador es imposible, puesto que su programa no se encuentra sobre el apoyo oficial';ru='Доступ к обновлению классификатора невозможен, так как ваша программа не находится на официальной поддержке';en='Access to updating the classifier is not possible, because your program is not officially supported'"));
		ЧастиСтрок.Добавить(" ");
	...
	ИначеЕсли КодОшибки = КодОшибкиНеверныйЛогинИлиПароль() Тогда
		Возврат НСтр("es='La falta de la autorización sobre el Portal 1С:ИТС. "
"Más detalladamente cm. En la revista del registro.';ru='Ошибка авторизации на Портале 1С:ИТС."
"Подробнее см. в журнале регистрации.';en='Authorization error on Portal 1C: ITS."
"See the log for details.'");
	ИначеЕсли КодОшибки = "ПревышеноКоличествоПопыток" Тогда
		Возврат НСтр("es='Es superada la cantidad de las tentativas de la introducción del login y la consigna. "
"Comprueben la corrección de los datos de la autorización y repitan "
...
"Check that the authorization data is correct and retry"
"try in 30 minutes.'");
	ИначеЕсли КодОшибки = "СервисВременноНеДоступен" Тогда
		Возврат НСтр("es='No consiguió conectarse al servicio de los clasificadores. El servicio es temporalmente inaccesible. "
"Repitan la tentativa de la conexión más tarde.';ru='Не удалось подключиться к сервису классификаторов. Сервис временно недоступен."
"Повторите попытку подключения позже.';en='Failed to connect to the classifier service. The service is temporarily unavailable."
"Please try connecting again later.'");
	ИначеЕсли КодОшибки = "ОшибкаСервиса" Тогда
		Возврат НСтр("es='La falta del trabajo con el servicio de los clasificadores.';ru='Ошибка работы с сервисом классификаторов.';en='Error working with the classifier service.'");
	ИначеЕсли КодОшибки = "ОшибкаПодключения" Тогда
		Возврат НСтр("es='No consiguió conectarse al servicio de los clasificadores.';ru='Не удалось подключиться к сервису классификаторов.';en='Failed to connect to the classifier service.'");
	ИначеЕсли КодОшибки = "НеизвестнаяОшибка" Тогда
		Возврат НСтр("es='La falta desconocida a la conexión al servicio.';ru='Неизвестная ошибка при подключении к сервису.';en='Unknown error connecting to the service.'");;
	КонецЕсли;
...
Функция ЗначениеСвойстваJSON(ЧтениеОбъектаJSON, ЗначениеПоУмолчанию = Неопределено)
	...
	Иначе
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió leer el significado de la propiedad %1. "
"El tipo incorrecto del significado de la propiedad (%2).';ru='Не удалось прочитать значение свойства %1."
"Некорректный тип значения свойства (%2).';en='Failed to read property value%1."
...
Функция ДанныеАутентификации()
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		ВызватьИсключение НСтр("es='En el trabajo en el modelo del servicio la información sobre los clasificadores "
"es cargada de los datos suministrados.';ru='При работе в модели сервиса информация о классификаторах"
"загружается из поставляемых данных.';en='When working in a service model, information about classifiers"
"loaded from the supplied data.'");
	...
	Иначе
		...
		Если Результат.ДанныеАутентификации = Неопределено Тогда
			...
			Результат.ИнформацияОбОшибке =
				НСтр("es='Para la recepción de las renovaciones de los clasificadores es necesario conectar el Apoyo De Internet de los usuarios.';ru='Для получения обновлений классификаторов необходимо подключить Интернет-поддержку пользователей.';en='To receive classifier updates, you need to connect Internet user support.'");
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("es='El trabajo con los clasificadores';ru='Работа с классификаторами';en='Work with classifiers'", ОбщегоНазначения.КодОсновногоЯзыка());
