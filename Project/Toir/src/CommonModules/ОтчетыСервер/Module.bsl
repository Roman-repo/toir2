...
Процедура ИнициализироватьКомпоновщикНастроек(КомпоновщикНастроек, Схема, Отчет = Неопределено, ВариантОтчета = Неопределено) Экспорт 
	...
	Исключение
		ИмяСобытия = НСтр("es='La falta de la inicialización компоновщика de los ajustes de la composición de los datos.';ru='Ошибка инициализации компоновщика настроек компоновки данных.';en='Error initializing the data composition settings linker.'",
...
Процедура ВывестиКоманду(ФормаОтчета, КомандаИлиКоманды, ТипГруппы, ВНачалоГруппы = Ложь, ТолькоВоВсехДействиях = Ложь, СуффиксПодгруппы = "") Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Con la llamada del procedimiento ""%1"" es entregado es inadmisible el significado de parámetro ""%2"".';ru='При вызове процедуры ""%1"" передано недопустимо значение параметра ""%2"".';en='When calling the procedure ""%1""value passed for parameter ''%2'' is not passed.'"),
...
Процедура ЗаполнитьПредставлениеИСостояниеНастройки(СвойстваНастройки, НастройкаВариантаКД, ПользовательскаяНастройкаКД, ДоступнаяНастройкаКД)
	...
	ИначеЕсли ЗначениеЗаполнено(СвойстваНастройки.Подтип) Тогда
		Если СвойстваНастройки.Подтип = "ДиаграммаСерии" Тогда
			ПредставлениеПоУмолчанию = НСтр("es='Unas series';ru='Серии';en='Series'");
		ИначеЕсли СвойстваНастройки.Подтип = "ДиаграммаТочки" Тогда
			ПредставлениеПоУмолчанию = НСтр("es='Los puntos';ru='Точки';en='Points'");
		ИначеЕсли СвойстваНастройки.Подтип = "ТаблицаСтроки" Тогда
			ПредставлениеПоУмолчанию = НСтр("es='Las líneas';ru='Строки';en='Lines'");
		ИначеЕсли СвойстваНастройки.Подтип = "ТаблицаКолонки" Тогда
			ПредставлениеПоУмолчанию = НСтр("es='Las columnas';ru='Колонки';en='Speakers'");
		ИначеЕсли СвойстваНастройки.Подтип = "Отчет" Тогда
			ПредставлениеПоУмолчанию = НСтр("es='El informe';ru='Отчет';en='Report'");
		Иначе
	...
	ИначеЕсли СвойстваНастройки.Тип = "Отбор" Тогда
		ПредставлениеПоУмолчанию = НСтр("es='La selección';ru='Отбор';en='Selection'");
	...
	ИначеЕсли СвойстваНастройки.Тип = "Порядок" Тогда
		ПредставлениеПоУмолчанию = НСтр("es='La selección';ru='Сортировка';en='Sorting'");
	ИначеЕсли СвойстваНастройки.Тип = "АвтоЭлементПорядка" Тогда
		ПредставлениеПоУмолчанию = НСтр("es='Авто (la selección de los padres)';ru='Авто (сортировки родителя)';en='Auto (parent sort)'");
	...
	ИначеЕсли СвойстваНастройки.Тип = "ВыбранныеПоля" Тогда
		ПредставлениеПоУмолчанию = НСтр("es='Campos';ru='Поля';en='Fields'");
	ИначеЕсли СвойстваНастройки.Тип = "ВыбранноеПоле" Тогда
		ПредставлениеПоУмолчанию = Строка(НастройкаВариантаКД.Поле);
	ИначеЕсли СвойстваНастройки.Тип = "АвтоВыбранноеПоле" Тогда
		ПредставлениеПоУмолчанию = НСтр("es='Авто (el campo de los padres)';ru='Авто (поля родителя)';en='Auto (parent fields)'");
	...
	ИначеЕсли СвойстваНастройки.Тип = "УсловноеОформление" Тогда
		ПредставлениеПоУмолчанию = НСтр("es='La formalización';ru='Оформление';en='Decor'");
	...
	ИначеЕсли СвойстваНастройки.Тип = "Группировка"
		...
		Если ПустаяСтрока(ПредставлениеПоУмолчанию) Тогда
			ПредставлениеПоУмолчанию = НСтр("es='<las anotaciones Detalladas>';ru='<Детальные записи>';en='<Детальные записи>'");
		Иначе
	...
	ИначеЕсли СвойстваНастройки.Тип = "Таблица" Тогда
		ПредставлениеПоУмолчанию = НСтр("es='La tabla';ru='Таблица';en='Table'");
	ИначеЕсли СвойстваНастройки.Тип = "Диаграмма" Тогда
		ПредставлениеПоУмолчанию = НСтр("es='El diagrama';ru='Диаграмма';en='Chart'");
	ИначеЕсли СвойстваНастройки.Тип = "НастройкиВложенногоОбъекта" Тогда
		...
		Если ПустаяСтрока(ПредставлениеПоУмолчанию) Тогда
			ПредставлениеПоУмолчанию = НСтр("es='La agrupación impuesta';ru='Вложенная группировка';en='Nested Grouping'");
		КонецЕсли;
	ИначеЕсли СвойстваНастройки.Тип = "СтруктураНастроек" Тогда
		ПредставлениеПоУмолчанию = НСтр("es='La estructura';ru='Структура';en='Structure'");
	Иначе
...
Функция ОтчетОбъект(Идентификатор) Экспорт
	...
	Иначе
		ВызватьИсключение СтрЗаменить(НСтр("es='El nombre incorrecto completo del informe ""%1"".';ru='Некорректное полное имя отчета ""%1"".';en='Invalid full report name ""%1"".'"), "%1", ПолноеИмя);
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение СтрЗаменить(НСтр("es='""%1"" no es el informe.';ru='""%1"" не является отчетом.';en='%1 is not a report.'"), "%1", ПолноеИмя);
	КонецЕсли;
КонецФункции
...
Процедура ДобавитьКомандуВыбораПериода(Форма, Группа, ШаблонИмени)
	...
	Команда.Заголовок = НСтр("es='Escoger el período...';ru='Выбрать период...';en='Choose period ...'");
...
Процедура ДобавитьКомандыСписка(Форма, ЭлементНастройки, ЭлементыНастроек, ИмяСписка)
	...
	ЗаголовокКоманды = НСтр("es='La selección';ru='Подбор';en='Selection'");
	...
	ЗаголовокКоманды = НСтр("es='Poner del portapapeles del cambio...';ru='Вставить из буфера обмена...';en='Paste from clipboard ...'");
