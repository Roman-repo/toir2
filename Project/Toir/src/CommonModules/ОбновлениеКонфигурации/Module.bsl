			...
			Если УдалитьИсправление Тогда
				...
				Исключение
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='No consiguió quitar la corrección ""%1"" por la causa: "
""
"%2';ru='Не удалось удалить исправление ""%1"" по причине:"
""
"%2';en='Failed to remove the patch"" ""%1"" due to:"
""
"%2 '"), Исправление.Имя, КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
					ЗаписьЖурналаРегистрации(НСтр("es='Las correcciones. La desaparición';ru='Исправления.Удаление';en='Corrections: Delete'", ОбщегоНазначения.КодОсновногоЯзыка()),
			...
			Иначе
				...
				Если ТребуетсяЗапись Тогда
					...
					Исключение
						ИнформацияОбОшибке = ИнформацияОбОшибке();
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='A la anotación de la corrección ""%1"" había una falta: "
""
"%2';ru='При записи исправления ""%1"" произошла ошибка:"
""
"%2';en='An error occurred while writing the fix ""%1"":"
""
"%2'"), Исправление.Имя, КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
						ЗаписьЖурналаРегистрации(НСтр("es='Las correcciones. El cambio';ru='Исправления.Изменение';en='Corrections: Change'", ОбщегоНазначения.КодОсновногоЯзыка()),
							УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
						ВызватьИсключение ТекстОшибки;
					КонецПопытки;
				КонецЕсли;
	...
	Для Каждого Расширение Из Расширения Цикл
		Если ЭтоИсправление(Расширение) Тогда
			...
			Исключение
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No consiguió quitar la corrección desconectada ""%1"" por la causa: "
""
"%2';ru='Не удалось удалить отключенное исправление ""%1"" по причине:"
""
"%2';en='Failed to remove the disabled fix ""%1"" due to:"
""
"%2 '"), Расширение.Имя, КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	...
	Если Изменения.УдаленныеИсправления.Количество() > 0 Тогда
		Заголовок = НСтр("es='Son quitados que han caducado патчи';ru='Удалены устаревшие патчи';en='Obsolete patches deleted'");
	...
	Если Изменения.НеактивныеИсправления.Количество() > 0 Тогда
		Заголовок = НСтр("es='Son quitados inactivo патчи';ru='Удалены неактивные патчи';en='Inactive patches deleted'");
	...
	Если Изменения.ОтключенаЗащита.Количество() > 0 Тогда
		Заголовок = НСтр("es='Son desconectadas las prevenciones de las acciones peligrosas';ru='Отключены предупреждения об опасных действиях';en='Notifications about hazardous actions disabled'");
		Если ЗначениеЗаполнено(ОписаниеИзменений) Тогда
			ОписаниеИзменений = ОписаниеИзменений + Символы.ПС + Символы.ПС;
		КонецЕсли;
	...
	Если Изменения.СнятБезопасныйРежим.Количество() > 0 Тогда
		Заголовок = НСтр("es='Es desconectado el régimen seguro';ru='Отключен безопасный режим';en='Safe mode disabled'");
		Если ЗначениеЗаполнено(ОписаниеИзменений) Тогда
			ОписаниеИзменений = ОписаниеИзменений + Символы.ПС + Символы.ПС;
		КонецЕсли;
	...
	Если Исправления.Свойство("Установить", Устанавливаемые)
		...
		Для Каждого Патч Из Устанавливаемые Цикл
			...
			Исключение
				...
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='A la instalación de la corrección ""%1"" había una falta: "
""
"%2';ru='При установке исправления ""%1"" произошла ошибка:"
""
"%2';en='An error occurred while installing the hotfix ""%1"":"
""
"%2 '"), Расширение.Имя, КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
...
"%2 '"), Расширение.Имя, КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() И ВФоне Тогда
		ТекстДляАсинхронногоВызова = НСтр("es='La renovación de los parámetros del trabajo de las ampliaciones después de la instalación o "
"la desaparición патчей se ha acabado con la falta.';ru='Обновление параметров работы расширений после установки или"
"удаления патчей завершилось с ошибкой.';en='Updating the operation parameters of extensions after installation or"
"uninstalling patches failed. '")
	КонецЕсли;
...
Процедура ЗавершитьОбновление(Знач РезультатОбновления, Знач ЭлектроннаяПочта, Знач ИмяАдминистратораОбновления, Знач КаталогСкрипта = Неопределено) Экспорт
	...
	ТекстСообщения = НСтр("es='La terminación de la renovación de exterior скрипта.';ru='Завершение обновления из внешнего скрипта.';en='Completing the update from an external script.'");
	...
	Если Не ЕстьПраваНаУстановкуОбновления() Тогда
		ТекстСообщения = НСтр("es='Es insuficientemente derecho para la terminación de la renovación de la configuración.';ru='Недостаточно прав для завершения обновления конфигурации.';en='Not enough rights to complete configuration update.'");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями")
		...
		Попытка
			...
			ТекстСообщения = НСтр("es='El aviso de la renovación es con éxito enviado a la dirección del correo electrónico:';ru='Уведомление об обновлении успешно отправлено на адрес электронной почты:';en='An update notification was sent successfully to the email address:'")
				+ " " + ЭлектроннаяПочта;
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,,,ТекстСообщения);
		Исключение
			ТекстСообщения = НСтр("es='La falta al envío de la carta del correo electrónico:';ru='Ошибка при отправке письма электронной почты:';en='Error sending email:'")
...
Процедура ОтправитьУведомлениеОбОбновлении(Знач ИмяПользователя, Знач АдресНазначения, Знач УспешноеОбновление)
	...
	Тема = ? (УспешноеОбновление, НСтр("es='La renovación exitosa de la configuración ""%1"", la versión %2';ru='Успешное обновление конфигурации ""%1"", версия %2';en=""Successful configuration update '%1', version%2"""), 
		НСтр("es='La falta de la renovación de la configuración ""%1"", la versión %2';ru='Ошибка обновления конфигурации ""%1"", версия %2';en=""Error updating configuration '%1', version%2"""));
	Тема = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Тема, Метаданные.КраткаяИнформация, Метаданные.Версия);
	
	Подробности = ?(УспешноеОбновление, НСтр("es='La renovación de la configuración es acabada con éxito.';ru='Обновление конфигурации завершено успешно.';en='Configuration update completed successfully.'"), 
		НСтр("es='A la renovación de la configuración había unas faltas. Los detalles son anotados en la revista del registro.';ru='При обновлении конфигурации произошли ошибки. Подробности записаны в журнал регистрации.';en='Errors occurred while updating the configuration. Details are recorded in the logbook.'"));
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 "
""
"Configuración: %2 "
"Versión: %3 "
"Línea de la unión: %4';ru='%1"
""
"Конфигурация: %2"
"Версия: %3"
"Строка соединения: %4';en='%1"
""
"Configuration: %2"
"Version: %3"
"Connection string: %4'"),
...
Функция СобытиеЖурналаРегистрации()
	Возврат НСтр("es='La renovación de la configuración';ru='Обновление конфигурации';en='Configuration update'", ОбщегоНазначения.КодОсновногоЯзыка());
КонецФункции
