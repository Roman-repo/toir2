...
Функция ТекущаяНастройкаРезервногоКопирования() Экспорт
	...
	Если НастройкиРезервногоКопирования = Неопределено Тогда
		Возврат НСтр("es='Para el ajuste de la copia de reserva es necesario dirigirse al administrador.';ru='Для настройки резервного копирования необходимо обратиться к администратору.';en='To configure the backup, you must contact the administrator.'");
	КонецЕсли;
	
	ТекущаяНастройка = НСтр("es='La copia de reserva no es ajustada, la base informativa arriesga las pérdidas de los datos.';ru='Резервное копирование не настроено, информационная база подвергается риску потери данных.';en='Backup is not configured, the infobase is at risk of data loss.'");
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		...
		Если НастройкиРезервногоКопирования.ВыполнятьАвтоматическоеРезервноеКопирование Тогда
			...
			Если НастройкиРезервногоКопирования.ВариантВыполнения = "ПриЗавершенииРаботы" Тогда
				ТекущаяНастройка = НСтр("es='La copia de reserva se cumple regularmente a la terminación del trabajo.';ru='Резервное копирование выполняется регулярно при завершении работы.';en='Backups are performed regularly at shutdown.'");
			ИначеЕсли НастройкиРезервногоКопирования.ВариантВыполнения = "ПоРасписанию" Тогда // По расписанию
				...
				Если Не ПустаяСтрока(Расписание) Тогда
					ТекущаяНастройка = НСтр("es='La copia de reserva se cumple regularmente por el horario: %1';ru='Резервное копирование выполняется регулярно по расписанию: %1';en='Scheduled backups regularly:%1'");
		...
		Иначе
			...
			Если НастройкиРезервногоКопирования.РезервноеКопированиеНастроено Тогда
				ТекущаяНастройка = НСтр("es='La copia de reserva no se cumple (es organizado por los programas extraños).';ru='Резервное копирование не выполняется (организовано сторонними программами).';en='Backup is not performed (organized by third-party programs).'");
			КонецЕсли;
	...
	Иначе
		...
		ТекущаяНастройка = НСтр("es='La copia de reserva no se cumple (es organizado por los medios СУБД).';ru='Резервное копирование не выполняется (организовано средствами СУБД).';en='Backup is not performed (organized by DBMS).'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Если Не ОтключеноУведомлениеОНастройкеРезервногоКопирования Тогда
			...
			Дело.Представление  = НСтр("es='Ajustar la copia de reserva';ru='Настроить резервное копирование';en='Set up backup'");
			Дело.Важное         = Истина;
		...
		Если Не ОтключеноУведомлениеОВыполненииРезервногоКопирования Тогда
			...
			Дело.Представление  = НСтр("es='La copia de reserva no es cumplida';ru='Резервное копирование не выполнено';en='Backup failed'");
			Дело.Важное         = Истина;
