...
Процедура ПриВыполненииДоставкиВПапку(ПараметрыДоставки, Вложения) Экспорт
	...
	Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El envío de los informes %1 de %2';ru='Рассылка отчетов %1 от %2';en='Report mailing %1 from%2'"),
...
Функция ИменаСправочниковХраненияФайлов(ВладелецФайлов, НеВызыватьИсключение = Ложь) Экспорт
	...
	Для каждого КлючИЗначение Из ИменаСправочников Цикл
		...
		Если Метаданные.Справочники.Найти(КлючИЗначение.Ключ) = Неопределено Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta a la definición de los nombres de las guías para el almacenaje de los ficheros. "
"Cerca del propietario de los ficheros como ""%1"" "
"es indicada la guía no existente ""%2"".';ru='Ошибка при определении имен справочников для хранения файлов."
"У владельца файлов типа ""%1"""
"указан несуществующий справочник ""%2"".';en='Error while determining the names of directories for storing files."
"The owner of files of type ""%1"""
"specified non-existent directory ""%2"".'"),
		...
		ИначеЕсли Не СтрЗаканчиваетсяНа(КлючИЗначение.Ключ, "ПрисоединенныеФайлы") И Не КлючИЗначение.Ключ ="Файлы" Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta a la definición de los nombres de las guías para el almacenaje de los ficheros. "
"Cerca del propietario de los ficheros como ""%1"" "
"es indicado el nombre de la guía ""%2"" "
"sin terminación de ""PrisoedinennyeFayly"".';ru='Ошибка при определении имен справочников для хранения файлов."
"У владельца файлов типа ""%1"""
"указано имя справочника ""%2"""
"без окончания ""ПрисоединенныеФайлы"".';en='Error while determining the names of directories for storing files."
"The owner of files of type ""%1"""
"Directory name specified ""%2"""
"without ending ""ПрисоединенныеФайлы"".'"),
				Строка(ТипВладельцаФайлов),
				Строка(КлючИЗначение.Ключ));
		...
		ИначеЕсли КлючИЗначение.Значение = Истина Тогда
			Если ОсновнойСправочникУказан Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La falta a la definición de los nombres de las guías para el almacenaje de los ficheros. "
"Cerca del propietario de los ficheros como ""%1"" "
"guía básica es indicado más de una vez.';ru='Ошибка при определении имен справочников для хранения файлов."
"У владельца файлов типа ""%1"""
"основной справочник указан более одного раза.';en='Error while determining the names of directories for storing files."
"The owner of files of type ""%1"""
"the main directory is listed more than once.'"),
					Строка(ТипВладельцаФайлов),
					Строка(КлючИЗначение.Ключ));
			КонецЕсли;
	...
	Если ИменаСправочников.Количество() = 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la definición de los nombres de las guías para el almacenaje de los ficheros. "
"Propietario de los ficheros como ""%1"" "
"no tiene guías para el almacenaje de los ficheros.';ru='Ошибка при определении имен справочников для хранения файлов."
"У владельца файлов типа ""%1"""
"не имеется справочников для хранения файлов.';en='Error while determining the names of directories for storing files."
"The owner of files of type ""%1"""
"there are no directories for storing files.'"),
			Строка(ТипВладельцаФайлов));
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ПутьКФайлу) И ЗначениеЗаполнено(Выборка.Том) Тогда
			...
			Если СуществующийФайл.Количество() = 0 Тогда
				...
				НесуществующийФайл.СтатусПроверки = НСтр("es='Faltan los datos en el tomo sobre el disco';ru='Отсутствуют данные в томе на диске';en='Missing data on volume on disk'");
			...
			Иначе
				...
				СуществующийФайл[0].СтатусПроверки = НСтр("es='Los íntegros datos';ru='Целостные данные';en='Holistic data'");
...
Функция ДобавитьФайлВТом(ДвоичныеДанныеИлиПуть, ВремяИзмененияУниверсальное, ИмяБезРасширения, Расширение,
	...
	Если Выборка.Количество() = 0 Тогда
		ВызватьИсключение НСтр("es='No existe ni un tomo para la instalación del fichero.';ru='Нет ни одного тома для размещения файла.';en='There are no volumes to host the file.'");
	КонецЕсли;
		...
		Попытка
			...
			Иначе // Считаем, что иначе это путь к файлу на диске.
				...
				Иначе
					...
					НСтр("es='No consiguió añadir el fichero ""%1"" ni en un de los tomos, porque él falta. "
"Probablemente, el fichero es quitado por el programa antivírico. "
"Se dirijan al administrador.';ru='Не удалось добавить файл ""%1"" ни в один из томов, т.к. он отсутствует."
"Возможно, файл удален антивирусной программой."
"Обратитесь к администратору.';en='Failed to add file ""%1"" ""to any of the volumes because it is missing.  The file may have been deleted by the antivirus program."
"Contact your administrator.'"),
			...
			Если СсылкаНаТом.МаксимальныйРазмер <> 0 Тогда
				...
				Если НовыйРазмер > СсылкаНаТом.МаксимальныйРазмер Тогда
					...
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Es superada la dimensión máxima del tomo (%1 Mb).';ru='Превышен максимальный размер тома (%1 Мб).';en='Maximum volume size exceeded (%1 Mb).'"),
		...
		Исключение
			...
			ШаблонОписанияОшибки =
				НСтр("es='La falta a la adición del fichero ""%1"" "
"en aquel ""%2"" (%3): "
"""%4"".';ru='Ошибка при добавлении файла ""%1"""
"в том ""%2"" (%3):"
"""%4"".';en='Error adding file ""%1"" "
"in that ""%2"" (%3):"
"""%4"".'");
	...
	ШаблонСообщенияОбОшибке = НСтр("es='No consiguió añadir el fichero ni en un de los tomos. "
"La lista de las faltas: "
""
"%1';ru='Не удалось добавить файл ни в один из томов."
"Список ошибок:"
""
"%1';en='Failed to add file to any of the volumes."
"List of errors:"
""
"%1'");
	
	ЗаписьЖурналаРегистрации(
		НСтр("es='Los ficheros. La adición del fichero';ru='Файлы.Добавление файла';en='Files. Adding a File'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Иначе
		...
		СтрокаИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió añadir el fichero: "
"""%1. El %2"". "
""
"Se dirijan al administrador.';ru='Не удалось добавить файл:"
"""%1.%2""."
""
"Обратитесь к администратору.';en='Failed to add file:"
"""%1.%2""."
""
...
Процедура КриптографияПриСозданииФормыНаСервере(Форма, ЭтоФормаСписка = Истина, ТолькоКартинкаСтрок = Ложь) Экспорт
	...
	Если ЭлектронноеПодписание И Шифрование Тогда
		Заголовок = НСтр("es='La firma electrónica y la cifración';ru='Электронная подпись и шифрование';en='Electronic signature and encryption'");
		Подсказка = НСтр("es='La presencia de la firma electrónica o la cifración';ru='Наличие электронной подписи или шифрования';en='Electronic signature or encryption'");
		Картинка  = БиблиотекаКартинок["ПодписанЗашифрованЗаголовок"];
	ИначеЕсли ЭлектронноеПодписание Тогда
		Заголовок = НСтр("es='La firma electrónica';ru='Электронная подпись';en='Electronic signature'");
		Подсказка = НСтр("es='La presencia de la firma electrónica';ru='Наличие электронной подписи';en='Electronic signature'");
		Картинка  = БиблиотекаКартинок["ПодписанЭП"];
	Иначе // Шифрование
		Заголовок = НСтр("es='La cifración';ru='Шифрование';en='Encryption'");
		Подсказка = НСтр("es='La presencia de la cifración';ru='Наличие шифрования';en='The presence of encryption'");
...
Процедура ПриОпределенииПроверок(ГруппыПроверок, Проверки) Экспорт
	...
	Проверка.Наименование                 = НСтр("es='La búsqueda de las referencias a los ficheros no existentes en los tomos del almacenaje';ru='Поиск ссылок на несуществующие файлы в томах хранения';en='Search for links to nonexistent files in storage volumes'");
	Проверка.Причины                      = НСтр("es='El fichero era físico es quitado o trasladado sobre el disco a consecuencia del trabajo de los programas antivíricos, "
"las acciones impremeditadas del administrador и.т.д.';ru='Файл был физически удален или перемещен на диске вследствие работы антивирусных программ,"
"непреднамеренных действий администратора и.т.д.';en='File has been physically deleted or moved to disk due to antivirus software,"
"unintended actions by the administrator, etc. '");
	Проверка.Рекомендация                 = НСтр("es='• Marcar el fichero en el programa a la desaparición; "
"• O restablecer el fichero sobre el disco en el tomo de la copia de reserva.';ru='• Пометить файл в программе на удаление;"
"• Или восстановить файл на диске в томе из резервной копии.';en='• Mark the file in the program for deletion;"
"• Or restore a file on disk in a volume from a backup.'");
...
Процедура ИзвлечьТекстИзФайлов() Экспорт
	...
	ЗаписьЖурналаРегистрации(
		...
		,
		,
		НСтр("es='Es comenzado de reglamento las la extracciones del texto';ru='Начато регламентное извлечения текста';en='Scheduled text extraction started'"));
		...
		Исключение
			Если ФайлЗаблокирован Тогда
				ЗаписьЖурналаРегистрации(
					...
					,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='No consiguió cumplir la extracción de reglamento del texto del fichero "
"""%1"" "
"por la causa: "
"""%2"".';ru='Не удалось выполнить регламентное извлечение текста из файла"
"""%1"""
"по причине:"
"""%2"".';en='Failed to perform scheduled extraction of text from file"
"""%1"""
"due to"
"""%2 "". '"),
						ИмяСРасширениемФайла,
		...
		Если ЗначениеЗаполнено(ИмяФайлаСДвоичнымиДанными) Тогда
			...
			Если Файл.Существует() Тогда
				...
				Исключение
					ЗаписьЖурналаРегистрации(НСтр("es='Los ficheros. La extracción del texto';ru='Файлы.Извлечение текста';en='Files. Extract text'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	ЗаписьЖурналаРегистрации(
		...
		,
		,
		НСтр("es='Es acabada la extracción de reglamento del texto';ru='Закончено регламентное извлечение текста';en='Scheduled text extraction completed'"));
		...
		Если Не ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
			Если ЕстьФайлыВТомах И Не ПустаяСтрока(ПутьКАрхивуСФайламиТомов) И (Лев(ПутьКАрхивуСФайламиТомов, 2) <> "\\"
				...
				ОбщегоНазначения.СообщитьПользователю(
					НСтр("es='La vía al archivo con los ficheros de los tomos debe ser "
"en el formato UNC (\\servername\resource)';ru='Путь к архиву с файлами томов должен быть"
"в формате UNC (\\servername\resource)';en='The path to the archive with the volume files should be"
"in UNC format (\\ servername \ resource)'"),
					,
			...
			Если Не ПустаяСтрока(ПолноеИмяФайловойБазы) И (Лев(ПолноеИмяФайловойБазы, 2) <> "\\" ИЛИ СтрНайти(ПолноеИмяФайловойБазы, ":") <> 0) Тогда
				ОбщегоНазначения.СообщитьПользователю(
					НСтр("es='La vía a la base de fichero debe ser "
"en el formato UNC (\\servername\resource)';ru='Путь к файловой базе должен быть"
"в формате UNC (\\servername\resource)';en='The path to the file base should be"
"in UNC format (\\ servername \ resource)'"),
					,
	...
	Если ПустаяСтрока(ПолноеИмяФайловойБазы) Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("es='Indiquen el nombre completo de la base de fichero (el fichero 1cv8.1cd)';ru='Укажите полное имя файловой базы (файл 1cv8.1cd)';en='Specify the full name of the file base (file 1cv8.1cd)'"),,
			"ПолноеИмяФайловойБазыWindows");
		Результат.ДанныеПодготовлены = Ложь;
	ИначеЕсли Результат.ДанныеПодготовлены Тогда
		...
		Если ЕстьФайлыВТомах Тогда
			Если ПустаяСтрока(ПутьКАрхивуСФайламиТомов) Тогда
				ОбщегоНазначения.СообщитьПользователю(
					НСтр("es='Indiquen el nombre completo del archivo con los ficheros de los tomos (el fichero *.zip)';ru='Укажите полное имя архива с файлами томов (файл *.zip)';en='Specify the full name of the archive with the volume files (* .zip file)'"),, 
					"ПутьКАрхивуСФайламиТомовWindows");
				Результат.ДанныеПодготовлены = Ложь;
			Иначе
				...
				Если Файл.Существует() И ФайлБазы.Существует() Тогда
					Результат.ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Los ficheros ""%1"" y ""%2"" existen ya. "
"¿Sustituir los ficheros existentes?';ru='Файлы ""%1"" и ""%2"" уже существуют."
"Заменить существующие файлы?';en='Files ""%1"" and ""%2"" already exist. Replace existing files?'"), ПутьКАрхивуСФайламиТомов, ПолноеИмяФайловойБазы);
					Результат.ТребуетсяПодтверждение = Истина;
				ИначеЕсли Файл.Существует() Тогда
					Результат.ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El fichero ""%1"" existe ya. "
"¿Sustituir el fichero existente?';ru='Файл ""%1"" уже существует."
"Заменить существующий файл?';en='File ""%1"" ""already exists."
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("es='Los ficheros redactados';ru='Редактируемые файлы';en='Editable files'");
...
Процедура ПриЗаполненииВозможныхПравДляНастройкиПравОбъектов(ВозможныеПрава) Экспорт
	...
	Право.Имя           = "Чтение";
	Право.Заголовок     = НСтр("es='La lectura';ru='Чтение';en='Reading'");
	Право.Подсказка     = НСтр("es='La lectura de las carpetas y los ficheros';ru='Чтение папок и файлов';en='Reading folders and files'");
	...
	Право.Заголовок     = НСтр("es='El cambio "
"de las carpetas';ru='Изменение"
"папок';en='Change"
"folders '");
	Право.Подсказка     = НСтр("es='La adición, el cambio y "
"la marca de la desaparición de las carpetas de los ficheros';ru='Добавление, изменение и"
"пометка удаления папок файлов';en='Adding, modifying and"
"marking deletion of file folders '");
	...
	Право.Заголовок     = НСтр("es='El cambio "
"de los ficheros';ru='Изменение"
"файлов';en='Change"
"files '");
	Право.Подсказка     = НСтр("es='El cambio de los ficheros en la carpeta';ru='Изменение файлов в папке';en='Change files in a folder'");
	// Права, требуемые для этого права.
	Право.ТребуемыеПрава.Добавить("Чтение");
	// Права для стандартных шаблонов ограничений доступа.
	...
	Право.Заголовок     = НСтр("es='La adición "
"de los ficheros';ru='Добавление"
"файлов';en='Adding"
"files '");
	Право.Подсказка     = НСтр("es='La adición de los ficheros en la carpeta';ru='Добавление файлов в папку';en='Adding files to a folder'");
	// Права, требуемые для этого права.
	...
	Право.Заголовок     = НСтр("es='La marca "
"de la desaparición';ru='Пометка"
"удаления';en='Mark"
"delete '");
	Право.Подсказка     = НСтр("es='La marca de la desaparición de los ficheros en la carpeta';ru='Пометка удаления файлов в папке';en='Mark delete files in a folder'");
	// Права, требуемые для этого права.
	Право.ТребуемыеПрава.Добавить("ИзменениеФайлов");
...
"файлов';en='Change"
"files '");
	Право.Подсказка     = НСтр("es='La dirección de los derechos de la carpeta';ru='Управление правами папки';en='Folder rights control '");
	// Права, требуемые для этого права.
	Право.ТребуемыеПрава.Добавить("Чтение");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		...
		Обработчик.Комментарий =
			НСтр("es='El traslado de las firmas electrónicas y los certificados de la cifración "
"de las partes tabulares en los registros de las noticias.';ru='Перенос электронных подписей и сертификатов шифрования"
"из табличных частей в регистры сведений.';en='Transfer of electronic signatures and encryption certificates"
"from tabular parts to information registers.'");
	...
	Обработчик.Комментарий =
			НСтр("es='El traslado de los ficheros binarios dados al registro de las noticias los datos Binarios de los ficheros.';ru='Перенос двоичных данных файлов в регистр сведений Двоичные данные файлов.';en='Transfer binary file data to the information register Binary file data.'");
	...
	Обработчик.Версия = "2.4.1.1";
	Обработчик.Комментарий =
			НСтр("es='El traslado de la información sobre la presencia de los ficheros al registro de las noticias la Presencia de los ficheros.';ru='Перенос информации о наличии файлов в регистр сведений Наличие файлов.';en='Transfer information about the presence of files in the information register. The presence of files.'");
	...
	Обработчик.Комментарий =
			НСтр("es='La renovación de la fecha universal y como el almacenaje de los elementos de la guía los Ficheros.';ru='Обновление универсальной даты и типа хранения элементов справочника Файлы.';en='Updating the universal date and storage type of directory items Files.'");
	...
	Обработчик.Комментарий =
			НСтр("es='El traslado de la información sobre los ficheros al registro de las noticias de la Noticia sobre los ficheros.';ru='Перенос информации о файлах в регистр сведений Сведения о файлах.';en='Transfer file information to the information register file information.'");
...
Процедура ВыполнитьОчисткуДанныхОВерсии(ФайлСсылка)
	...
	Попытка
		...
		ТекстКомментария = НСтр("es='El fichero es quitado a la limpieza de los ficheros innecesarios.';ru='Файл удален при очистке ненужных файлов.';en='File deleted while cleaning unnecessary files.'")
	...
	Исключение
		...
		Если Не ПустаяСтрока(ИмяФайлаСПутем) Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("es='La limpieza de los ficheros innecesarios';ru='Очистка ненужных файлов';en='Clearing junk files'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ВыполнитьОчисткуДанныхОФайле(ФайлСсылка)
	...
	Попытка
		...
		ТекстОписания = НСтр("es='El fichero es quitado a la limpieza de los ficheros innecesarios.';ru='Файл удален при очистке ненужных файлов.';en='File deleted while cleaning unnecessary files.'")
			+ " " + Формат(ТекущаяДатаСеанса(),"ДЛФ=D") + Символы.ПС;
...
"Contact your administrator.'"),
				...
				ЗаписьЖурналаРегистрации(НСтр("es='Los ficheros. La adición del fichero en aquel';ru='Файлы.Добавление файла в том';en='Files. Adding a File in Volume'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ИмяСправочникаХраненияФайлов(ВладелецФайлов, ИмяСправочника = "",
	...
	Если ИменаСправочников.Количество() = 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			+ НСтр("es='El propietario de los ficheros ""%1"" como ""%2"" "
"no tiene guías para el almacenaje de los ficheros.';ru='У владельца файлов ""%1"" типа ""%2"""
"нет справочников для хранения файлов.';en='File owner ""%1"" of type"" ""%2"" """
"no directories to store files.'"),
	...
	Если ЗначениеЗаполнено(ИмяСправочника) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ЗаголовокОшибки + Символы.ПС
			+ НСтр("es='El propietario de los ficheros ""%1"" como ""%2"" "
"no tiene guía ""%3"" para el almacenaje de los ficheros.';ru='У владельца файлов ""%1"" типа ""%2"""
"нет справочника ""%3"" для хранения файлов.';en='File owner ""%1"" of type ""%2"" "
"no directory ""%3"" for storing files.'"),
			Строка(ВладелецФайлов),
			Строка(ТипЗнч(ВладелецФайлов)),
	...
	ШаблонПричиныОшибки = 
		НСтр("es='Cerca del propietario de los ficheros ""%1"" como ""%2"" no es indicada la guía básica para el almacenaje "
"de los ficheros.';ru='У владельца файлов ""%1"" типа ""%2"""
"не указан основной справочник для хранения файлов.';en='File owner ""%1"" of type ""%2"" "
"directory for storing files is not specified.'") + Символы.ПС;
...
Функция ИменаСправочниковХраненияВерсийФайлов(ВладелецФайлов, НеВызыватьИсключение = Ложь)
	...
	Для каждого КлючИЗначение Из ИменаСправочников Цикл
		...
		Если Метаданные.Справочники.Найти(КлючИЗначение.Ключ) = Неопределено Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta a la definición de los nombres de las guías para el almacenaje de las versiones de los ficheros. "
"Cerca del propietario de los ficheros como ""%1"" "
"es indicada la guía no existente ""%2"".';ru='Ошибка при определении имен справочников для хранения версий файлов."
"У владельца файлов типа ""%1"""
"указан несуществующий справочник ""%2"".';en='Error determining directory names for storing file versions."
"The owner of files of type ""%1"""
"specified non-existent directory ""%2"".'"),
				Строка(ТипВладельцаФайлов),
				Строка(КлючИЗначение.Ключ));
...
"Cerca del propietario de los ficheros como ""%1"" "
"es indicado el nombre de la guía ""%2"" "
"sin terminación ""VersiiPrisoedinennyhFaylov"".';ru='Ошибка при определении имен справочников для хранения версий файлов."
"У владельца файлов типа ""%1"""
"указано имя справочника ""%2"""
"без окончания ""ВерсииПрисоединенныхФайлов"".';en='Error while determining directory names for storing file versions."
"The owner of files of type ""%1"""
"Directory name specified ""%2"""
"without ending the ""ВерсииПрисоединенныхФайлов"".'"),
				Строка(ТипВладельцаФайлов),
				Строка(КлючИЗначение.Ключ));
		...
		ИначеЕсли КлючИЗначение.Значение = Истина Тогда
			Если ОсновнойСправочникУказан Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La falta a la definición de los nombres de las guías para el almacenaje de las versiones de los ficheros. "
"Cerca del propietario de los ficheros como ""%1"" "
"guía básica de las versiones es indicado más de una vez.';ru='Ошибка при определении имен справочников для хранения версий файлов."
"У владельца файлов типа ""%1"""
"основной справочник версий указан более одного раза.';en='Error determining directory names for storing file versions."
"The owner of files of type ""%1"""
"The main version guide is listed more than once.'"),
					Строка(ТипВладельцаФайлов),
					Строка(КлючИЗначение.Ключ));
			КонецЕсли;
			ОсновнойСправочникУказан = Истина;
...
Функция ИмяСправочникаХраненияВерсийФайлов(ВладелецФайлов, ИмяСправочника = "",
	...
	ШаблонПричиныОшибки = 
		НСтр("es='Cerca del propietario de las versiones de los ficheros ""%1"" no es indicada la guía básica para el almacenaje de las versiones "
"de los ficheros.';ru='У владельца версий файлов ""%1"""
"не указан основной справочник для хранения версий файлов.';en='File owner ""%1"" "
"the main directory for storing file versions is not specified. '") + Символы.ПС;
...
Функция ИмпортФайловСформироватьОтчет(МассивИменФайловСОшибками) Экспорт
	...
	ОбластьЗаголовок.Параметры.Описание = НСтр("es='No consiguió cargar los ficheros siguientes:';ru='Не удалось загрузить следующие файлы:';en='Failed to load  the he following files:'");
	...
	Если ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
		Если НЕ ВерсияСсылка.Том.Пустая() Тогда
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(
					НСтр("es='Los ficheros. La apertura del fichero';ru='Файлы.Открытие файла';en='Files. Open a file'",
					     ОбщегоНазначения.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
...
Функция СформироватьТекстОшибкиПолученияФайлСТомаДляАдминистратора(ИнформацияОбОшибкеФункции, ФайлСсылка) Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La referencia al fichero: ""%1"". "
...
Функция ОшибкаФайлНеНайденВХранилищеФайлов(ИмяФайла, ПоискВТоме = Истина, ВладелецФайла = "") Экспорт
	...
	Если ПоискВТоме Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió abrir el fichero: "
"%1 "
"que es juntado a: "
"%2 "
"por la causa: los datos binarios del fichero eran quitados. Probablemente, el fichero es limpiado como innecesario o es quitado por el programa antivírico. "
"Se dirijan al administrador.';ru='Не удалось открыть файл:"
"%1"
...
"Обратитесь к администратору.';en='Could not open file:"
"%1"
"which is attached to:"
"%2"
"for reason: binary file data has been deleted. The file may have been cleaned up as unnecessary or deleted by the antivirus program."
"Contact administrator.'"),
			ИмяФайла,
			ВладелецФайла);
...
"que es juntado a: "
"%2 "
"por la causa: los datos binarios del fichero eran quitados. Probablemente, el fichero es limpiado como innecesario. "
"Se dirijan al administrador.';ru='Не удалось открыть файл:"
"%1"
"который присоединен к:"
"%2"
"по причине: двоичные данные файла были удалены. Возможно, файл очищен как ненужный."
"Обратитесь к администратору.';en='Failed to open file:"
"%1"
"which is attached to:"
"%2"
"for reason: binary file data has been deleted. The file may have been cleaned up as unnecessary."
"Contact administrator.'"),
			ИмяФайла,
			ВладелецФайла);
	КонецЕсли;
...
Процедура УдалитьФайлВТоме(ИмяУдаляемогоФайла)
	...
	Если ФайлВременный.Существует() Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("es='Los ficheros. La desaparición de los ficheros en el tomo';ru='Файлы.Удаление файлов в томе';en='Files. Deleting files in a volume'",
				     ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
...
Функция Кодировки() Экспорт
	...
	СписокКодировок.Добавить("ibm852",       НСтр("es='IBM852 (Tsentralnoevropeysky DOS)';ru='IBM852 (Центральноевропейская DOS)';en='IBM852 (Central European DOS)'"));
	СписокКодировок.Добавить("ibm866",       НСтр("es='IBM866 (Alfabeto ciriliano DOS)';ru='IBM866 (Кириллица DOS)';en='IBM866 (Cyrillic DOS)'"));
	СписокКодировок.Добавить("iso-8859-1",   НСтр("es='ISO-8859-1 (De Europa occidental ISO)';ru='ISO-8859-1 (Западноевропейская ISO)';en='ISO-8859-1 (Western European ISO)'"));
	СписокКодировок.Добавить("iso-8859-2",   НСтр("es='ISO-8859-2 (Tsentralnoevropeysky ISO)';ru='ISO-8859-2 (Центральноевропейская ISO)';en='ISO-8859-2 (Central European ISO)'"));
	СписокКодировок.Добавить("iso-8859-3",   НСтр("es='ISO-8859-3 (El Alfabeto latino 3 ISO)';ru='ISO-8859-3 (Латиница 3 ISO)';en='ISO-8859-3 (Latin 3 ISO)'"));
	СписокКодировок.Добавить("iso-8859-4",   НСтр("es='ISO-8859-4 (Báltico ISO)';ru='ISO-8859-4 (Балтийская ISO)';en='ISO-8859-4 (Baltic ISO)'"));
	СписокКодировок.Добавить("iso-8859-5",   НСтр("es='ISO-8859-5 (Alfabeto ciriliano ISO)';ru='ISO-8859-5 (Кириллица ISO)';en='ISO-8859-5 (Cyrillic ISO)'"));
	СписокКодировок.Добавить("iso-8859-7",   НСтр("es='ISO-8859-7 (Griego ISO)';ru='ISO-8859-7 (Греческая ISO)';en='ISO-8859-7 (Greek ISO)'"));
	СписокКодировок.Добавить("iso-8859-9",   НСтр("es='ISO-8859-9 (Turco ISO)';ru='ISO-8859-9 (Турецкая ISO)';en='ISO-8859-9 (Turkish ISO)'"));
	СписокКодировок.Добавить("iso-8859-15",  НСтр("es='ISO-8859-15 (El Alfabeto latino 9 ISO)';ru='ISO-8859-15 (Латиница 9 ISO)';en='ISO-8859-15 (Latin 9 ISO)'"));
	СписокКодировок.Добавить("koi8-r",       НСтр("es='KOI8-R (Alfabeto ciriliano KOI8-R)';ru='KOI8-R (Кириллица KOI8-R)';en='KOI8-R (Cyrillic KOI8-R)'"));
	СписокКодировок.Добавить("koi8-u",       НСтр("es='KOI8-U (Alfabeto ciriliano KOI8-U)';ru='KOI8-U (Кириллица KOI8-U)';en='KOI8-U (Cyrillic KOI8-U)'"));
	СписокКодировок.Добавить("us-ascii",     НСтр("es='US-ASCII LOS (EEUU)';ru='US-ASCII (США)';en='US-ASCII (USA)'"));
	СписокКодировок.Добавить("utf-8",        НСтр("es='UTF-8 (Yunikod UTF-8)';ru='UTF-8 (Юникод UTF-8)';en='UTF-8 (Unicode UTF-8)'"));
	СписокКодировок.Добавить("utf-8_WithoutBOM", НСтр("es='UTF-8 (Yunikod UTF-8 sin BOM)';ru='UTF-8 (Юникод UTF-8 без BOM)';en='UTF-8 (Unicode UTF-8 without BOM)'"));
	СписокКодировок.Добавить("windows-1250", НСтр("es='Windows-1250 (Tsentralnoevropeysky Windows)';ru='Windows-1250 (Центральноевропейская Windows)';en='Windows-1250 (Central European Windows)'"));
	СписокКодировок.Добавить("windows-1251", НСтр("es='windows-1251 (Alfabeto ciriliano Windows)';ru='windows-1251 (Кириллица Windows)';en='windows-1251 (Cyrillic Windows)'"));
	СписокКодировок.Добавить("windows-1252", НСтр("es='Windows-1252 (De Europa occidental Windows)';ru='Windows-1252 (Западноевропейская Windows)';en='Windows-1252 (West European Windows)'"));
	СписокКодировок.Добавить("windows-1253", НСтр("es='Windows-1253 (Griego Windows)';ru='Windows-1253 (Греческая Windows)';en='Windows-1253 (Greek Windows)'"));
	СписокКодировок.Добавить("windows-1254", НСтр("es='Windows-1254 (Turco Windows)';ru='Windows-1254 (Турецкая Windows)';en='Windows-1254 (Turkish Windows)'"));
	СписокКодировок.Добавить("windows-1257", НСтр("es='Windows-1257 (Báltico Windows)';ru='Windows-1257 (Балтийская Windows)';en='Windows-1257 (Baltic Windows)'"));
...
Функция СобытиеЖурналаРегистрацииДляОбмена() 
	...
	Возврат НСтр("es='Los ficheros. No consiguió enviar el fichero al cambio para los datos';ru='Файлы.Не удалось отправить файл при обмене данными';en='Files. Failed to send file during data exchange.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ОбновитьДвоичныеДанныеФайлаВИнформационнойБазе(ФайлОбъект, ФайлСсылка, ДвоичныеДанные)
	...
	Исключение
		ЗаписьЖурналаРегистрации(
		НСтр("es='Los ficheros. La renovación de los datos del fichero juntado en el depósito de los ficheros';ru='Файлы.Обновление данных присоединенного файла в хранилище файлов';en='Files. Updating file attachment data in file vault'",
		ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,
		,
		,
...
Процедура ОбновитьДвоичныеДанныеФайлаВТоме(ФайлОбъект, ФайлСсылка, ДвоичныеДанные)
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(
			НСтр("es='Los ficheros. El Registro del fichero al disco';ru='Файлы.Запись файла на диск';en='Files. Write a file to disk'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
...
Функция ОбновитьВерсиюФайла(ФайлСсылка,
	...
	Если СведенияОФайле.ХранитьВерсии Тогда
		ЗаголовокОшибки = НСтр("es='La falta a la anotación de la nueva versión de los ficheros juntados.';ru='Ошибка при записи новой версии присоединенных файлов.';en='Error while writing new version of attached files.'");
		ОкончаниеОшибки = НСтр("es='En este caso la anotación de la versión del fichero es imposible.';ru='В этом случае запись версии файла невозможна.';en='In this case, writing the file version is not possible.'");
...
Функция ТекстОшибкиПриСохраненииФайлаВТоме(Знач СообщениеОбОшибке, Знач Файл)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La falta, a la conservación del fichero en el tomo: "
"""%1"". "
""
"La referencia al fichero: ""%2"".';ru='Ошибка, при сохранении файла в томе:"
"""%1""."
""
"Ссылка на файл: ""%2"".';en='Error saving file in volume:"
"""%1""."
""
"File link: ""%2"".'"),
		СообщениеОбОшибке,
...
Процедура ФормаЭлементаПриСозданииНаСервере(Контекст, Отказ, СтандартнаяОбработка, Параметры, ТолькоПросмотр, НастраиватьОбъектФормы = Ложь) Экспорт
	...
	ЗаголовокОшибки = НСтр("es='La falta al ajuste de la forma del elemento de los ficheros juntados.';ru='Ошибка при настройке формы элемента присоединенных файлов.';en='Error while customizing the form of the attached file element.'");
	ОкончаниеОшибки = НСтр("es='En este caso el ajuste de la forma del elemento es imposible.';ru='В этом случае настройка формы элемента невозможна.';en='In this case, adjusting the shape of the element is not possible.'");
	...
	Если РасширениеФайлаВСписке Тогда
		...
		Если Не ЗначениеЗаполнено(Контекст.Кодировка) Тогда
			Контекст.Кодировка = НСтр("es='Por reticencia';ru='По умолчанию';en='Default'");
		КонецЕсли;
...
Процедура ЗаполнитьСписокШифрования(Контекст, Знач Источник = Неопределено) Экспорт
	...
	ТекстЗаголовка = НСтр("es='Está permitido descifrar';ru='Разрешено расшифровывать';en='Decrypt permitted'");
...
Процедура ЗаполнитьСписокПодписей(Контекст, Знач Источник = Неопределено) Экспорт
	...
	ТекстЗаголовка = НСтр("es='Las firmas electrónicas';ru='Электронные подписи';en='Electronic signatures'");
...
Функция ПроверитьИсключениеHTTP1С(Ответ, АдресСервера)
	...
	Если (Ответ.КодСостояния >= 400) И (Ответ.КодСостояния <= 599) Тогда
		...
		ШаблонОшибки = НСтр("es='No consiguió sincronizar el fichero a dirección de %2, porque el servidor ha devuelto HTTP el código: %1. %3';ru='Не удалось синхронизировать файл по адресу %2, т.к. сервер вернул HTTP код: %1. %3';en='Failed to sync file at%2 because the server returned an HTTP code:%1. %3'");
...
Функция ЗагрузитьФайлССервера(ПараметрыФайла, ЭтоФайл = Неопределено)
	...
	ТекстСобытия = НСтр("es='La carga del fichero del servidor: %1';ru='Загрузка файла с сервера: %1';en='Downloading file from server:%1'");
	...
	Если РезультатЗагрузки.Успешно И РезультатЗагрузки.АдресЗагруженногоФайла <> Неопределено Тогда
		...
		ТекстСообщения = НСтр("es='Es cargado el fichero del servicio nuboso: ""%1""';ru='Загружен файл из облачного сервиса: ""%1""';en='Download file from cloud service: ""%1""'");
		СтатусДляЖурналаРегистрации = УровеньЖурналаРегистрации.Информация;
	Иначе
		ТекстСообщения = НСтр("es='No consiguió cargar el fichero ""%1"" del servicio nuboso por la causa:';ru='Не удалось загрузить файл ""%1"" из облачного сервиса по причине:';en='Failed to download file ""%1"" from the cloud service due to:'") + " " + Символы.ПС + РезультатЗагрузки.ТекстОшибки;
...
Функция СобытиеЖурналаРегистрацииСинхронизация()
	...
	Возврат НСтр("es='La sincronización de los ficheros con el servicio nuboso';ru='Синхронизация файлов с облачным сервисом';en='Synchronization files with the cloud service'", ОбщегоНазначения.КодОсновногоЯзыка());
			...
			Если НайденныйPropstat<>Неопределено Тогда
				Для каждого ДочернийУзелPropstat Из НайденныйPropstat.ДочерниеУзлы Цикл
					Если ДочернийУзелPropstat.ЛокальноеИмя = "quota-available-bytes" Тогда
						...
						ИнформацияОСвободномМесте = НСтр("es='El lugar libre: %1 Mb';ru='Свободное место : %1 Мб';en='Free space:%1 Mb'");
					...
					ИначеЕсли ДочернийУзелPropstat.ЛокальноеИмя = "quota-used-bytes" Тогда
						...
						ИнформацияОЗанятомМесте = НСтр("es='Es ocupado: %1 Mb';ru='Занято : %1 Мб';en='Busy:%1 Mb'");
	...
	Попытка
		...
		Пока НайденныйResponse <> Неопределено Цикл
			...
			Если НайденныйHref = Неопределено Тогда
				ТекстОшибки = НСтр("es='La falta de la respuesta del servidor: no es encontrado HREF en %1';ru='Ошибка ответа от сервера: не найден HREF в %1';en='Server response error: HREF not found in %1'");
		...
		Если СтрокаДереваФайлов.ЭтоПапка Тогда
			...
			Если (ТекущаяПапкаФайлов = Неопределено) И (ТаблицаФайлов.Найти(СтрокаДереваФайлов.Href, "Href") = Неопределено) Тогда
				...
				Если НЕ ПроверитьВозможностьUID1C(СтрокаДереваФайлов.Href, Строка(Новый УникальныйИдентификатор), ПараметрыСинхронизации) Тогда
					ТекстСобытия = НСтр("es='Es imposible la conservación de las propiedades adicionales del fichero, él no será cargado: %1';ru='Невозможно сохранение дополнительных свойств файла, он не будет загружен: %1';en='Unable to save additional file properties, it will not be loaded:%1'");
				...
				Попытка
					...
					ТекстСобытия = НСтр("es='Es cargada la carpeta del servidor: %1';ru='Загружена папка с сервера:  %1';en='Folder downloaded from server:%1'");
...
Процедура СинхронизироватьФайлыСОблачнымСервисом(УчетнаяЗапись)
	...
	ТекстСобытия = НСтр("es='El comienzo de la sincronización de los ficheros con el servicio nuboso.';ru='Начало синхронизации файлов с облачным сервисом.';en='Start synchronizing files with the cloud service.'");
	...
	ТекстСобытия = НСтр("es='Es acabada la sincronización de los ficheros con el servicio nuboso';ru='Завершена синхронизация файлов с облачным сервисом';en='File sync with cloud service completed'");
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации.УчетнаяЗапись);
...
Процедура ВыполнитьСинхронизациюФайловОблачнымСервисом(ПараметрыСинхронизации)
	...
	Если Отказ = Истина Тогда
		...
		ТекстСобытия = НСтр("es='No consiguió cargar la estructura de los ficheros del servidor, la sincronización no es cumplida.';ru='Не удалось загрузить структуру файлов с сервера, синхронизация не выполнена.';en='Failed to load file structure from server, synchronization failed.'");
...
Процедура ОбходТаблицыФайлов(ТаблицаФайлов, ПараметрыСинхронизации, АдресСервера, ЭтоУдалениеФайлов = Ложь)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаФайлов Цикл
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No consiguió sincronizar el fichero ""%1"" por la causa:';ru='Не удалось синхронизировать файл ""%1"" по причине:';en=""Failed to sync file '%1' due to:"""), Строка(СтрокаТаблицы.ФайлСсылка))
...
Функция ИзменитьФайлВОблачномСервисе(Знач ИзмененоСодержимоеНаСервере, ОбновитьСтатусСинхронизацииФайла, Знач ПараметрыСинхронизации, Знач СтрокаТаблицы)
	...
	ТекстСобытия = НСтр("es='Es renovado el objeto en la base: %1';ru='Обновлен объект в базе: %1';en='Updated object in database:%1'");
...
Функция УдалитьФайлВОблачномСервисе(Знач ПараметрыСинхронизации, Знач СтрокаТаблицы)
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ТекстСобытия = НСтр("es='la base: %1';ru='Не удалено в базе: %1';en='Not deleted in database:%1'");
		ЗаписатьВЖурналСобытийСинхронизацииФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСобытия, СтрокаТаблицы.Наименование), ПараметрыСинхронизации.УчетнаяЗапись);
		Возврат Ложь;
	...
	ТекстСобытия = НСтр("es='Es quitado en la base: %1';ru='Удалено в базе: %1';en='Deleted in database:%1'");
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСобытия, СтрокаТаблицы.Наименование), ПараметрыСинхронизации.УчетнаяЗапись);
...
Функция СоздатьФайлВОблачномСервисе(Знач АдресСервера, Знач ПараметрыСинхронизации, Знач СтрокаТаблицы, Знач ТаблицаФайлов)
	...
	ТекстСобытия = НСтр("es='Es creado el objeto en el servicio nuboso %1';ru='Создан объект в облачном сервисе %1';en='Created an object in the cloud service%1'");
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСобытия, СтрокаТаблицы.Наименование), ПараметрыСинхронизации.УчетнаяЗапись);
...
Процедура ВыполнитьПроверкуПодключения(УчетнаяЗапись, РезультатПроверки) Экспорт 
	...
	ТекстСобытия = НСтр("es='Es comenzada la comprobación de la sincronización de los ficheros';ru='Начата проверка синхронизации файлов';en='File synchronization check started'") + " " + УчетнаяЗапись.Наименование;
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации.УчетнаяЗапись);
	
	ПрочитатьПараметрыКаталога(РезультатПроверки,АдресСервера, ПараметрыСинхронизации);
	
	ТекстСобытия = НСтр("es='Es acabada la comprobación de la sincronización de los ficheros';ru='Завершена проверка синхронизации файлов';en='File synchronization check completed'") + " " + УчетнаяЗапись.Наименование;
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации.УчетнаяЗапись);
...
Процедура УдалитьНесинхронизируемыеФайлыУчетнойЗаписи(УчетнаяЗапись = Неопределено)
	...
	ТекстСобытия = НСтр("es='El comienzo de la liberación de los ficheros capturados por el servicio nuboso.';ru='Начало освобождения файлов, захваченных облачным сервисом.';en='Start releasing files captured by the cloud service.'");
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации.УчетнаяЗапись);
	...
	Попытка
		...
		СкачатьДеревоФайловРекурсивно(ДеревоФайловСервера.Строки, АдресСервера, ПараметрыСинхронизации, Отказ);
		Если Отказ = Истина Тогда
			ТекстОшибки = НСтр("es='A la carga de la estructura de los ficheros del servidor había una falta, la sincronización no es cumplida.';ru='При загрузке структуры файлов с сервера произошла ошибка, синхронизация не выполнена.';en='An error occurred while loading the file structure from the server; synchronization failed.'");
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
			...
			Для Каждого СтрокаТаблицы Из ТаблицаФайлов Цикл
				...
				Попытка
					...
					Если ЗначениеЗаполнено(СтрокаТаблицы.Href) Тогда
						...
						ТекстСобытия = НСтр("es='Es quitado el objeto en el servicio nuboso %1';ru='Удален объект в облачном сервисе %1';en='Deleted object in cloud service%1'");
	...
	ТекстСобытия = НСтр("es='Es acabado la liberación de los ficheros capturados por el servicio nuboso';ru='Завершено освобождения файлов, захваченных облачным сервисом';en='Completed the release of files captured by the cloud service'");
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации.УчетнаяЗапись);
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió tratar el tomo del almacenaje de los ficheros: %1 por la causa: "
"%2';ru='Не удалось обработать том хранения файлов: %1 по причине:"
"%2';en='Failed to process files storage type:%1 due to: %3'"), 
	...
	Пока Результат.Количество() > 0 Цикл
		...
		Для Каждого СтрокаРезультата Из Результат Цикл
			...
			Если СтрокаРезультата.Владелец <> Неопределено Тогда
				УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La versión ""%1"" ficheros ""%2"" no existe en tomo ""%3"".';ru='Версия ""%1"" файла ""%2"" не существует в томе ""%3"".';en='Version ""% 1"" of file ""% 2"" does not exist on volume ""% 3"".'"),
					СтрокаРезультата.Файл, СтрокаРезультата.Владелец, СтрокаРезультата.Том);
			Иначе
				УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El fichero ""%1"" no existe en tomo ""%2"".';ru='Файл ""%1"" не существует в томе ""%2"".';en='File ""%1"" does not exist on volume ""%2"".'"),
...
Функция ТомДоступен(Том, ПредставлениеТома, Путь, ПараметрыПроверки)
	...
	Если ПустаяСтрока(Путь) Тогда
		...
		УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Cerca del tomo del almacenaje de los ficheros ""%1"" no es dada la vía al catálogo de red. La conservación de los ficheros en él es imposible.';ru='У тома хранения файлов ""%1"" не задан путь к сетевому каталогу. Сохранение файлов в него невозможно.';en=""File storage volume '%1' does not have a network directory path. Saving files to it is not possible."""), 
			ПредставлениеТома);
	...
	Исключение
		...
		УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El tomo del almacenaje de los ficheros ""%1"" es inaccesible por la causa: "
"%2 "
""
"catálogo Indicado de red podía ser desconectado o a él faltan los derechos de admisión. "
"Es imposible el trabajo con todos los ficheros que están en este tomo.';ru='Том хранения файлов ""%1"" недоступен по причине: "
"%2"
""
"Указанный сетевой каталог мог быть отключен или к нему отсутствуют права доступа."
"Невозможна работа со всеми файлами, хранящимися в этом томе.';en='File storage volume ""%1"" is not available due to:"
"%2"
""
"The specified network directory may have been disabled or it does not have access rights."
"Cannot work with all files stored in this volume.'"),
...
Процедура НачальноеЗаполнение() Экспорт
	...
	ПапкаШаблонов.Наименование = НСтр("es='Los modelos de los ficheros';ru='Шаблоны файлов';en='File templates'");
...
Процедура ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведенийДляТаблицы(ПараметрыОбновления, ПолноеИмяОбъектаМетаданных, ОбработкаЗавершена)
	...
	Если ОбъектМетаданных = Неопределено Тогда
		ВызватьИсключение НСтр("es='de las firmas electrónicas y los certificados de la cifración.';ru='Не указан объект для обработки электронных подписей и сертификатов шифрования.';en='No object specified for processing electronic signatures and encryption certificates.'");
	КонецЕсли;
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió tratar el objeto: %1 por la causa: "
"%2';ru='Не удалось обработать объект: %1 по причине:"
"%2';en='Failed to process object: %1 due to:"
"%2'"),
			ОбъектМетаданных,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Al procedimiento ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведений no consiguió tratar algunos objetos (son dejados pasar): %1';ru='Процедуре ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведений не удалось обработать некоторые объекты (пропущены): %1';en='A procedure ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведений failed to process some objects (skipped):%1'"),
			ПроблемныхОбъектов);
	...
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			...
			,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El procedimiento ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведений ha tratado la porción siguiente de objetos: %1';ru='Процедура ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведений обработала очередную порцию объектов: %1';en='A procedure ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведений processed the next batch of objects:%1'"),
				ОбъектовОбработано));
	КонецЕсли;
