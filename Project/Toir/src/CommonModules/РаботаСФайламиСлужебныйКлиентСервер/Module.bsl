...
Функция СтрокаСообщенияОНедопустимостиПодписанияЗанятогоФайла(ФайлСсылка = Неопределено) Экспорт
	...
	Если ФайлСсылка = Неопределено Тогда
		Возврат НСтр("es='No es posible firmar el fichero ocupado.';ru='Нельзя подписать занятый файл.';en='Can not sign inaccessible file.'");
	Иначе
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No es posible firmar el fichero ocupado: %1.';ru='Нельзя подписать занятый файл: %1.';en='Can not sign inaccessible file:%1.'"),
			Строка(ФайлСсылка) );
	КонецЕсли;
...
Функция СтрокаСообщенияОНедопустимостиПодписанияЗашифрованногоФайла(ФайлСсылка = Неопределено) Экспорт
	...
	Если ФайлСсылка = Неопределено Тогда
		Возврат НСтр("es='No es posible firmar el fichero cifrado.';ru='Нельзя подписать зашифрованный файл.';en='Can not sign an encrypted file.'");
	Иначе
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='No es posible firmar el fichero cifrado: %1.';ru='Нельзя подписать зашифрованный файл: %1.';en='Can not sign encrypted file:%1.'"),
						Строка(ФайлСсылка) );
	КонецЕсли;
...
Процедура ЗаполнитьСтатусПодписи(СтрокаПодписи) Экспорт
	...
	Если СтрокаПодписи.ПодписьВерна Тогда
		СтрокаПодписи.Статус = НСтр("es='Es justa';ru='Верна';en='Verna'");
	Иначе
		СтрокаПодписи.Статус = НСтр("es='Es equivocada';ru='Неверна';en='Wrong'");
	КонецЕсли;
