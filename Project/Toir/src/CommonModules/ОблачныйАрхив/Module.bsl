...
Процедура СобратьДанныеПоОблачномуАрхиву(ДополнительныеСвойства, АдресХранилища = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La recogida de los datos en el fondo';ru='Сбор данных в фоне';en='Background data collection'"));

		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			КонтекстВыполненияВложенный,
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
			НСтр("es='La recepción del macizo de los pasos';ru='Получение массива шагов';en='Getting an array of steps'"));
			...
			Если МассивШагов.Количество() > 0 Тогда
				...
				ОписаниеРезультатаВложенный = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Es recibido %1 pasos para la ejecución';ru='Получено %1 шагов для выполнения';en='Received%1 steps to complete'"),
		...
		Для Каждого ТекущийШаг Из МассивШагов Цикл
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La ejecución del paso: %1';ru='Выполнение шага: %1';en='Step completion:%1'"),ТекущийШаг.ОписаниеШага));
			...
			ДлительныеОперации.СообщитьПрогресс(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La ejecución del paso: %1';ru='Выполнение шага: %1';en='Step completion:%1'"), ТекущийШаг.ОписаниеШага),
...
#Область ИнформацияОКлиенте
				...
				Иначе
					КодРезультатаВложенный      = 0;
					ОписаниеРезультатаВложенный = НСтр("es='Los datos recogidos antes no han caducado';ru='Собранные ранее данные еще не устарели';en='Previously collected data is not yet outdated'");
				КонецЕсли;
...
#КонецОбласти
			...
			Если КодРезультатаВложенный > 0 Тогда
				...
				ОписаниеРезультата = ОписаниеРезультата + Символы.ПС
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Había una falta %1 recogidas de los datos sobre el paso "
"%2: %3';ru='Произошла ошибка %1 сбора данных на шаге %2:"
"%3';en='An error occurred%1 data collection in step%2:"
"%3'")
							+ Символы.ПС,
						КодРезультатаВложенный,
						ТекущийШаг.КодШага,
			...
			Иначе
				ОписаниеРезультата = ОписаниеРезультата + Символы.ПС
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La recogida de los datos del paso %1 es con éxito acabada';ru='Сбор данных шага %1 успешно завершен';en='Data collection of step%1 completed successfully'")
							+ Символы.ПС,
						ТекущийШаг.КодШага);
			КонецЕсли;
			...
			ДлительныеОперации.СообщитьПрогресс(
				(НомерШага / МассивШагов.Количество()) * 100,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La ejecución del paso: %1. Es acabado.';ru='Выполнение шага: %1. Завершено.';en='Step completion:% 1. Is completed.'"),
	...
	ДлительныеОперации.СообщитьПрогресс(
		101, // > 100%
		НСтр("es='СобратьДанныеПоОблачномуАрхиву. Es acabado.';ru='СобратьДанныеПоОблачномуАрхиву. Завершено.';en='СобратьДанныеПоОблачномуАрхиву. Completed.'"),
		Новый Структура("КодСостояния, ОписаниеРезультата",
...
"%5'")
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("es='БИП:ОблачныйАрхив. Todas las renovaciones del archivo Nuboso';ru='БИП:ОблачныйАрхив.Все обновления Облачного архива';en='БИП:ОблачныйАрхив.Все обновления Облачного архива'"), // Имя события
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El archivo nuboso. Todas las renovaciones del archivo Nuboso. %1';ru='Облачный архив. Все обновления Облачного архива. %1';en='Cloud archive. All cloud archive updates. %1'"),
...
Функция ДоступнаАктивацияАгентаКопированияНаКомпьютере(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La llamada del procedimiento / las funciones (%1)';ru='Вызов процедуры / функции (%1)';en='Call procedure / function (% 1)'"),
			НаименованиеПроцедурыФункции));

	// Запись в журнал регистрации.
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='%1, ha comenzado "
"el Tiempo del comienzo (мс): %2';ru='%1, начало"
"Время начала (мс): %2';en='% 1 start"
"Start time (ms):% 2'")
			+ Символы.ПС,
		НаименованиеПроцедурыФункции,
		КонтекстВыполненияВложенный.ТекущийШагВыполнения.ВремяНачала);
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("es='БИП:ОблачныйАрхив. Todas las renovaciones del archivo Nuboso';ru='БИП:ОблачныйАрхив.Все обновления Облачного архива';en='БИП:ОблачныйАрхив.Все обновления Облачного архива'"), // ИмяСобытия
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El archivo nuboso. Todas las renovaciones del archivo Nuboso. %1. El comienzo';ru='Облачный архив. Все обновления Облачного архива. %1. Начало';en='Cloud archive. All cloud archive updates. %1. Start'"),
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La carga de la información del servicio web particular (%1)';ru='Загрузка информации из приватного веб-сервиса (%1)';en='Downloading information from a private web service (%1)'"),
			...
			Если (ТипЗнч(Результат) <> ТипОбъектXDTO) // Неправильный тип
				...
				КонтекстВыполненияВложенный.ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El servicio Web ha devuelto el significado del tipo incorrecto: %1';ru='Веб-сервис вернул значение неправильного типа: %1';en='The web service returned the value of the wrong type:% 1'"),
					ТипЗнч(Результат));
				ЗаполнитьЗначенияСвойств(КонтекстВыполнения, КонтекстВыполненияВложенный, "КодРезультата, ОписаниеРезультата"); // Установить код ошибки для самого верхнего уровня Контекста выполнения.
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La desconexión del servicio web (%1)';ru='Отключение от веб-сервиса (%1)';en='Disable from web service (%1)'"),
				"logout"));
...
"%5'")
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("es='БИП:ОблачныйАрхив. Todas las renovaciones del archivo Nuboso';ru='БИП:ОблачныйАрхив.Все обновления Облачного архива';en='БИП:ОблачныйАрхив.Все обновления Облачного архива'"), // ИмяСобытия
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El archivo nuboso. Todas las renovaciones del archivo Nuboso. %1. El fin';ru='Облачный архив. Все обновления Облачного архива. %1. Конец';en='Cloud archive. All cloud archive updates. %1. The end'"),
			НаименованиеПроцедурыФункции), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
...
"Start time (ms):% 2'")
		...
		Если (ТипЗнч(Прокси) = ТипWSПрокси) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				ИначеЕсли (ТипЗнч(Результат) = ТипОбъектXDTO)
					...
					КонтекстВыполненияВложенный.ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El servicio Web ha devuelto la falta con el código: %1';ru='Веб-сервис вернул ошибку с кодом: %1';en='The web service returned an error with code:% 1'"),
						Результат.errorCode);
					ЗаполнитьЗначенияСвойств(КонтекстВыполнения, КонтекстВыполненияВложенный, "КодРезультата, ОписаниеРезультата"); // Установить код ошибки для самого верхнего уровня Контекста выполнения.
					ЕстьОшибки = Истина;
				КонецЕсли;
			...
			Если ЕстьОшибки <> Истина Тогда
				...
				ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
					...
					НСтр("es='La conservación de la información en ИБ';ru='Сохранение информации в ИБ';en='Saving information in information security'"));
	...
	Иначе
		...
		ШагВыполнения = ИнтернетПоддержкаПользователейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
			...
			380,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La renovación no se realizaba: la renovación anterior era menos hace 30 segundos (era %1, es ahora %2)';ru='Обновление не осуществлялось: предыдущее обновление было менее 30 секунд назад (было %1, сейчас %2)';en='The update was not carried out: the previous update was less than 30 seconds ago (it was%1, now%2)'"),
...
Процедура ПанельАдминистрированияБСП_ПриСозданииНаСервере(Форма) Экспорт
	...
	Иначе // Облачный архив
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='La recogida de los datos para el archivo Nuboso';ru='Сбор данных для Облачного архива';en='Data collection for Cloud Archive'");
...
Процедура ПанельАдминистрированияБСП_ПереключитьНаПравильнуюСтраницу(Форма, СобратьДанныеОКлиенте = Ложь) Экспорт
	...
	ИначеЕсли Форма.СпособРезервногоКопирования = "1С:Облачный архив" Тогда
		...
		Если ИнформацияОКлиенте.ЭтоАдминистраторWindows Тогда
			...
			КонецЕсли;
		Иначе
			Элементы.ДекорацияОблачныйАрхивОшибки.Заголовок = НСтр("es='Para la conexión, la ejecución de la copia de reserva y el ajuste del Agente de la copia de reserva son necesarios los derechos del administrador Windows en este ordenador.';ru='Для подключения, выполнения резервного копирования и настройки Агента резервного копирования необходимы права администратора Windows на этом компьютере.';en='To connect, perform backups and configure the Backup Agent, you must have Windows administrator rights on this computer.'");
	...
	Если ОблачныйАрхивПовтИсп.РазрешенаРаботаСОблачнымАрхивом() Тогда
		...
		Если НЕ МодульТекущиеДелаСервер.ДелоОтключено(ИдентификаторДела) Тогда
			...
			Если Результат.ТребуетсяУстановка = Истина Тогда
				...
				Дело.Важное         = Истина;
				Дело.Представление  = НСтр("es='Para el trabajo 1С:Облачный el archivo es necesario establecer al agente de la copia de reserva';ru='Для работы 1С:Облачный архив необходимо установить агента резервного копирования';en='To work with 1C: Cloud Archive, you need to install a backup agent'");
			...
			ИначеЕсли Результат.ТребуетсяОбновление = 1 Тогда // Доступно обновление.
				...
				Дело.Представление  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Es accesible la renovación %1 agentes de la copia de reserva';ru='Доступно обновление %1 агента резервного копирования';en='%1 backup agent update available'"),
			...
			ИначеЕсли Результат.ТребуетсяОбновление = 2 Тогда // Рекомендуется обновление.
				...
				Дело.Важное         = Ложь;
				Дело.Представление  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Se recomienda la renovación del agente de la copia de reserva hasta la versión %1';ru='Рекомендуется обновление агента резервного копирования до версии %1';en='It is recommended to upgrade the backup agent to version%1 '"),
					ИнтернетПоддержкаПользователейКлиентСервер.ПользовательскоеПредставлениеНомераВерсии(
						Результат.ВерсияНовейшегоАгентаКопирования,
						Ложь));
				Дело.Форма          = "Обработка.ОблачныйАрхив.Форма.МастерОбновленияАгента";
			...
			ИначеЕсли Результат.ТребуетсяОбновление = 3 Тогда // Необходимо обновление.
				...
				Дело.Важное         = Истина;
				Дело.Представление  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Es necesario renovar al agente de la copia de reserva hasta la versión %1';ru='Необходимо обновить агента резервного копирования до версии %1';en='Required to upgrade the backup agent to version%1'"),
					ИнтернетПоддержкаПользователейКлиентСервер.ПользовательскоеПредставлениеНомераВерсии(
						Результат.ВерсияНовейшегоАгентаКопирования,
						Ложь));
				Дело.Форма          = "Обработка.ОблачныйАрхив.Форма.МастерОбновленияАгента";
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		443,
		НСтр("es='Los servicios Web de la autorización en 1С';ru='Веб-сервисы авторизации в 1С';en='Authorization web services in 1C'"));
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		443,
		НСтр("es='Los servicios Web del trabajo con el archivo nuboso (público y particular)';ru='Веб-сервисы работы с облачным архивом (публичные и приватные)';en='Cloud archive web services (public and private)'"));
	НовыеРазрешения.Добавить(Разрешение);
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		443,
		НСтр("es='Los servicios Web de la información sobre el agente de la copia';ru='Веб-сервисы информации об агенте копирования';en='Copy Agent Information Web Services'"));
	НовыеРазрешения.Добавить(Разрешение);
	...
	Если (ИнформацияОКлиенте.АгентКопированияУстановлен = Истина)
		...
		Исключение
			...
			ОписаниеРезультатаШага = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta de la desaparición del fichero temporal de los ajustes %1 por la causa: "
"%2';ru='Ошибка удаления временного файла настроек %1 по причине:"
"%2';en='Error deleting temporary settings file%1 due to:"
"%2'")
					+ Символы.ПС,
				ИмяВременногоФайлаНастроекИБ,
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ОписаниеРезультатаШага = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta de la desaparición del catálogo %1 temporal por la causa: "
"%2';ru='Ошибка удаления временного каталога %1 по причине:"
"%2';en='Error deleting temporary directory %1 due to:"
"%2 '")
					+ Символы.ПС,
...
Функция ПолучитьПроксиВебСервисаАвторизации1С(КонтекстВыполнения = Неопределено)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("es='La conexión a прокси del servicio web (la autorización 1С)';ru='Подключение к прокси веб-сервиса (авторизация 1С)';en='Connection to a web service proxy (1C authorization)'"));
	...
	Если ТипЗнч(Прокси) <> Тип("WSПрокси") Тогда
		...
		Прокси = Неопределено;
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La conexión desgraciada al servicio web (la autorización 1С) por la causa: "
"%1';ru='Неудачное подключение к веб-сервису (авторизация 1С) по причине:"
"%1';en='Unsuccessful connection to the web service (1C authorization) due to:"
"%1'"),
			ПодробноеПредставлениеОшибки);
		// Запись в журнал регистрации.
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:ОблачныйАрхив. Los servicios Web';ru='БИП:ОблачныйАрхив.Веб-сервисы';en='БИП:ОблачныйАрхив.Веб-сервисы'"), // Имя события
			НСтр("es='El archivo nuboso. Los servicios Web. La conexión al servidor';ru='Облачный архив. Веб-сервисы. Подключение к серверу';en='Cloud archive. Web services Server connection'"), // ИмяСобытия
...
Функция ПолучитьПроксиВебСервисаБэкап1СПубличный(КонтекстВыполнения = Неопределено)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("es='La conexión a прокси del servicio web (público backup.1c.ru)';ru='Подключение к прокси веб-сервиса (публичный backup.1c.ru)';en='Connection to the web service proxy (public backup.1c.ru)'"));
	...
	Если ТипЗнч(Прокси) <> Тип("WSПрокси") Тогда
		...
		Прокси = Неопределено;
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La conexión desgraciada al servicio web (público, backup.1c.ru) por la causa: "
"%1';ru='Неудачное подключение к веб-сервису (публичный, backup.1c.ru) по причине:"
"%1';en='Unsuccessful connection to the web service (public, backup.1c.ru) due to:"
"%1'"),
			ПодробноеПредставлениеОшибки);
		// Запись в журнал регистрации.
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
...
"Start time (ms):% 2'")
	...
	Если (ТипЗнч(Прокси) = ТипWSПрокси) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции + ".ЗагрузкаДанных",
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La carga de la información del servicio web público (%1)';ru='Загрузка информации из публичного веб-сервиса (%1)';en='Downloading information from a public web service (%1)'"),
				"getBackupAgentInfo"));
...
Функция ПолучитьПроксиВебСервисаБэкап1СПриватный(КонтекстВыполнения = Неопределено) Экспорт
	...
	Если (ТипЗнч(ПроксиЛогин1С) = Тип("WSПрокси")) И (КонтекстВыполнения.КодРезультата = 0) Тогда
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("es='La recepción тикета';ru='Получение тикета';en='Ticket receipt'"));
			...
			Если (ТипЗнч(ПроксиПубличныйБэкап1С) = Тип("WSПрокси")) И (КонтекстВыполнения.КодРезультата = 0) Тогда
				...
				Если ЕстьОшибки <> Истина Тогда
					...
					ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
						КонтекстВыполнения,
						"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
						НСтр("es='La conexión a прокси del servicio web (particular backup.1c.ru)';ru='Подключение к прокси веб-сервиса (приватный backup.1c.ru)';en='Connection to the web service proxy (private backup.1c.ru)'"));
					...
					Если ТипЗнч(ПроксиПриватныйБэкап1С) <> Тип("WSПрокси") Тогда
						...
						ПроксиПриватныйБэкап1С = Неопределено;
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='La conexión desgraciada al servicio web (particular, backup.1c.ru) por la causa: "
"%1';ru='Неудачное подключение к веб-сервису (приватный, backup.1c.ru) по причине:"
"%1';en='Unsuccessful connection to the web service (private, backup.1c.ru) due to:"
"%1'"),
							ПодробноеПредставлениеОшибки);
						// Запись в журнал регистрации.
						ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
...
Процедура УдалитьРезервнуюКопию(КонтекстВыполнения) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("es='La desaparición de la copia de reserva';ru='Удаление резервной копии';en='Deleting backup'"));
...
#Область УдалениеФайла
	...
	Если (ТипЗнч(Прокси) = Тип("WSПрокси")) И (КонтекстВыполненияВложенный.КодРезультата = 0) Тогда
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("es='La desaparición del fichero del servidor';ru='Удаление файла с сервера';en='Deleting a file from the server'"));
			...
			Попытка
				...
				ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La desaparición del fichero con UIN %1 es cumplida con éxito';ru='Удаление файла с UIN %1 выполнено успешно';en='Deleting file with UIN%1 completed successfully'"),
					КонтекстВыполнения.ИдентификаторФайла);
			Исключение
				Результат = -1;
				ИнформацияОбОшибке = ИнформацияОбОшибке();
			...
			ИначеЕсли (ТипЗнч(Результат) = ТипЧисло) Тогда
				Если Результат = 0 Тогда
					// Файл удален
					ОписаниеРезультата = НСтр("es='El fichero de la copia de reserva con el identificador %1 es con éxito quitado del servidor';ru='Файл резервной копии с идентификатором %1 успешно удален с сервера';en='Backup file ID%1 deleted successfully from server'");
					ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El fichero de la copia de reserva con el identificador %1 es con éxito quitado del servidor.';ru='Файл резервной копии с идентификатором %1 успешно удален с сервера.';en='Backup file with ID%1 was successfully deleted from the server.'"),
						КонтекстВыполнения.ИдентификаторФайла);
				Иначе // Ошибка
					ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Había una falta de la desaparición del fichero de la copia de reserva del servidor: "
"el Código de la falta: %1 "
"Identificador del fichero: %2';ru='Произошла ошибка удаления файла резервной копии с сервера:"
"Код ошибки: %1"
"Идентификатор файла: %2';en='An error occurred while deleting the backup file from the server:"
"Error code: %1"
"File ID: %2 '"),
						Результат,
						КонтекстВыполнения.ИдентификаторФайла);
...
#КонецОбласти
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La desaparición del fichero de la copia de reserva del servidor, el informe total. "
"El tiempo del comienzo (мс): %1 "
"Tiempo de la terminación (мс): %2 "
"Duración (мс): %3 "
"Resultado de la ejecución: "
"%4';ru='Удаление файла резервной копии с сервера, итоговый отчет."
"Время начала (мс): %1"
"Время окончания (мс): %2"
"Длительность (мс): %3"
"Результат выполнения:"
"%4';en='Deleting backup file from server, summary report."
"Start Time (ms):%1"
"End Time (ms):%2"
"Duration (ms):%3"
"Execution Result:"
"%four'")
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("es='БИП:ОблачныйАрхив. Los servicios Web';ru='БИП:ОблачныйАрхив.Веб-сервисы';en='БИП:ОблачныйАрхив.Веб-сервисы'"), // Имя события
		НСтр("es='El archivo nuboso. Los servicios Web. La desaparición del fichero de la copia de reserva';ru='Облачный архив. Веб-сервисы. Удаление файла резервной копии';en='Cloud archive. Web Services Delete backup file'"), // ИмяСобытия
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
Процедура ЗагрузитьИнформациюОбАктуальныхВерсияхАгентаКопирования(КонтекстВыполнения = Неопределено)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("es='La carga de la información sobre la versión actual del Agente de la copia';ru='Загрузка информации об актуальной версии Агента копирования';en='Download information on the current version of Copy Agent'"));
...
#Область ПолучениеСпискаОСиВерсий
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La recepción de la lista de la SO y las versiones de la SO';ru='Получение списка ОС и версий ОС';en='Getting a list of OS and OS versions'"));
	...
	ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Es recibida la lista de la SO y las versiones: "
"%1';ru='Получен список ОС и версий:"
"%1';en='Received a list of OS and versions:"
"%1'"),
...
#Область ПреобразованиеСпискаОСиВерсий
	...
	Если ТаблицаВсехОСиВерсий <> Неопределено Тогда
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("es='La transformación de la lista de la SO y las versiones de la SO';ru='Преобразование списка ОС и версий ОС';en='Convert OS list and OS versions'"));
			...
			ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es transformada la lista de la SO y las versiones: "
"%1';ru='Преобразован список ОС и версий:"
"%1';en='Converted list of OS and versions:"
"%1'"),
				ТелоОтправки);
...
#Область ЗагрузкаДанных
	...
	Если ТаблицаВсехОСиВерсий <> Неопределено Тогда
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			КонтекстВыполненияВложенный,
			"ОблачныйАрхив." + НаименованиеПроцедурыФункции + ".ЗагрузкаДанных",
			НСтр("es='La carga de los datos de los servidores 1С';ru='Загрузка данных с серверов 1С';en='Downloading data from 1C servers'"));
			...
			ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Son recibidos los datos para las versiones del Agente de la copia de reserva. "
"El código del estado: %1 "
"Encabezamientos: "
"%2 "
"Resultado: "
"%3 "
"representación Detallada de la falta (a la presencia): "
"%4';ru='Получены данные для версий Агента резервного копирования."
"Код состояния: %1"
"Заголовки:"
"%2"
"Результат:"
...
"%4';en='eceived data for versions of Backup agent."
"Status code:%1"
"Headings:"
"%2"
"Result:"
"%3"
"Detailed error view (if there are):"
"%4'"),
				КодСостояния,
...
#Область ПарсингПолученныхДанных
	...
	Если ЕстьОшибки = Ложь Тогда
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("es='Парсинг de los datos cargados';ru='Парсинг загруженных данных';en='Parsing uploaded data'"));
			...
			Если (ЗагруженныеДанные.Свойство("backupAgentInfoList"))
				...
				ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
					...
					НСтр("es='Парсинг de los datos cargados, la carga de los ficheros de las descripciones';ru='Парсинг загруженных данных, загрузка файлов описаний';en='Parsing downloaded data, loading description files'"));
					...
					Для Каждого ЗагружаемыйФайл Из СодержимоеФайлов Цикл
						...
						Исключение
							...
							ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
								+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("es='La falta de la desaparición del fichero temporal de la descripción en el formato html %1 por la causa: "
"%2';ru='Ошибка удаления временного файла описания в формате html %1 по причине:"
"%2';en='Error deleting temporary description file in html%1 format due to:"
"%2'")
										+ Символы.ПС,
									ИмяВременногоФайла,
									ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
					...
					ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='La lista URL de los ficheros de las descripciones: "
"%1';ru='Список URL файлов описаний:"
"%1';en='List of description file URLs:"
"%1'"),
				...
				ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
					...
					НСтр("es='Парсинг de los datos cargados, la conservación de los datos en ИБ';ru='Парсинг загруженных данных, сохранение данных в ИБ';en='Parsing downloaded data, storing data in information security'"));
						...
						Если ТипЗнч(ТекущееОписаниеВерсииАгентаКопирования) = ТипСтруктура Тогда
							...
							Иначе
								...
								ОшибкиЗаписиВБазуДанных = ОшибкиЗаписиВБазуДанных + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("es='Para la línea %1 no son rellenados los campos obligatorios: %2';ru='Для строки %1 не заполнены обязательные поля: %2';en='Required fields are not filled for line%1:%2'")
										+ Символы.ПС,
					...
					ОписаниеРезультатаВложенный = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Es cargado los datos: %1 "
"Es dejado pasar los datos: %2 "
"Faltas de la anotación en la base de datos: "
"%3';ru='Загружено данных: %1"
"Пропущено данных: %2"
"Ошибки записи в базу данных:"
"%3';en='Data downloaded:%1"
"Missing data:%2"
"Errors in writing to the database:"
"%3 '"),
			...
			ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Son tratados los datos para las versiones del Agente de la copia de reserva: "
"%1';ru='Обработаны данные для версий Агента резервного копирования:"
"%1';en='Processed data for versions of Backup Agent:"
"%1'")
					+ Символы.ПС,
...
Процедура ВсеОбновленияОблачногоАрхива() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("es='La ejecución de las tareas de reglamento del subsistema el archivo Nuboso';ru='Выполнение регламентных заданий подсистемы Облачный архив';en='Performing scheduled tasks of the Cloud Archive subsystem'"));

	ТекстСообщения = НСтр("es='El comienzo de la ejecución de la tarea de reglamento "
"el Tipo de la plataforma: %ТипПлатформы% "
"la Versión de la SO: %ВерсияОС% "
"la Línea de la conexión: %СтрокаСоединенияИнформационнойБазы% "
"de TekuschiyRezhimZapuska: %ТекущийРежимЗапуска% "
"Es la configuración dividida: %ЭтоРазделеннаяКонфигурация% "
"la División es incluida: %РазделениеВключено% "
"Es accesible el uso de los datos divididos: %ДоступноИспользованиеРазделенныхДанных% "
"la Sesión es puesta en marcha sin разделителей: %СеансЗапущенБезРазделителей% "
"la Configuración (metadatos): %Метаданные_Имя%, %Метаданные_Версия% "
"la opción Funcional Es permitida el trabajo con el archivo Nuboso: %ОблачныйАрхивПовтИсп_РазрешенаРаботаСОблачнымАрхивом% "
"el usuario ИБ Corriente:"
"	El Nombre: %ТекущийПользователь_Имя%"
"	УИН: %ТекущийПользователь_УникальныйИдентификатор%"
"	La División de los datos:"
"		%ТекущийПользователь_РазделениеДанных% "
"el Tiempo del comienzo (мс): %ТНачало%';ru='Начало выполнения регламентного задания"
...
"	Разделение данных:"
"		%ТекущийПользователь_РазделениеДанных%"
"Время начала (мс): %ТНачало%';en='Start of the routine task"
"Platform type: %ТипПлатформы%"
"OS version: %ВерсияОС%"
"Connection string: %СтрокаСоединенияИнформационнойБазы%"
"Current Startup Mode: %ТекущийРежимЗапуска%"
"This is a split configuration: %ЭтоРазделеннаяКонфигурация%"
"Split enabled: %РазделениеВключено%"
"Split data usage available: %ДоступноИспользованиеРазделенныхДанных%"
"Session started without delimiters: %СеансЗапущенБезРазделителей%"
"Configuration (metadata): %Метаданные_Имя%, %Метаданные_Версия%"
"Functional option allowed to work with Cloud archive: %ОблачныйАрхивПовтИсп_РазрешенаРаботаСОблачнымАрхивом%"
"The current user information security:"
" Name: %ТекущийПользователь_Имя%"
" Win: %ТекущийПользователь_УникальныйИдентификатор%"
" Data separation:"
" %ТекущийПользователь_РазделениеДанных%"
"Start time (MS): %ТНачало%'")
		+ Символы.ПС;
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("es='БИП:ОблачныйАрхив. Todas las renovaciones del archivo Nuboso';ru='БИП:ОблачныйАрхив.Все обновления Облачного архива';en='БИП:ОблачныйАрхив.Все обновления Облачного архива'"), // ИмяСобытия
		НСтр("es='El archivo nuboso. Todas las renovaciones del archivo Nuboso. El comienzo';ru='Облачный архив. Все обновления Облачного архива. Начало';en='Cloud archive. All cloud archive updates. Start'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
	...
	Если МонопольныйРежим() Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:ОблачныйАрхив. Todas las renovaciones del archivo Nuboso';ru='БИП:ОблачныйАрхив.Все обновления Облачного архива';en='БИП:ОблачныйАрхив.Все обновления Облачного архива'"), // ИмяСобытия
			НСтр("es='El archivo nuboso. Todas las renovaciones del archivo Nuboso. El fin';ru='Облачный архив. Все обновления Облачного архива. Конец';en='Cloud archive. All cloud archive updates. the end'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияОблачногоАрхива, // ОбъектМетаданных
			, // Данные
			НСтр("es='La terminación forzada de la ejecución de la tarea de reglamento: la base informativa trabaja en el régimen monopolizado.';ru='Принудительное завершение выполнения регламентного задания: информационная база работает в монопольном режиме.';en='Forced completion of the scheduled task: the information base works in exclusive mode.'"), // Комментарий
			ОблачныйАрхивПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
		ЕстьОшибки = Истина;
...
#КонецОбласти
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La terminación de la ejecución de la tarea de reglamento "
"el Tiempo del comienzo (мс): %1 "
"Tiempo de la terminación (мс): %2 "
"Duración (мс): %3 "
""
"Por los pasos: "
"%4';ru='Завершение выполнения регламентного задания"
"Время начала (мс): %1"
"Время окончания (мс): %2"
"Длительность (мс): %3"
""
"По шагам:"
"%4';en='Completion of the scheduled task"
"Start time (ms): %1"
"End time (ms): %2"
"Duration (ms): %3"
""
...
Процедура ИзменитьИспользованиеРегламентныхЗаданий(Использовать) Экспорт
	...
	Если Использовать = Истина Тогда
		...
		Если (НайденныеЗадания.Количество() = 0) Тогда
			...
			ПараметрыЗадания.Вставить("Наименование", НСтр("es='Todas las renovaciones del archivo Nuboso';ru='Все обновления Облачного архива';en='All Cloud Archive Updates'"));
		...
		Иначе
			Для Каждого ТекущееРегламентноеЗадание Из НайденныеЗадания Цикл
				...
				ТекущееРегламентноеЗадание.Наименование                             = НСтр("es='Todas las renovaciones del archivo Nuboso';ru='Все обновления Облачного архива';en='All Cloud Archive Updates'");
	...
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='ОблачныйАрхив.ИзменитьИспользованиеРегламентныхЗаданий: "
"en el parámetro Usar es entregado el significado del tipo incorrecto: %1';ru='ОблачныйАрхив.ИзменитьИспользованиеРегламентныхЗаданий:"
"в параметре Использовать передано значение неправильного типа: %1';en='ОблачныйАрхив.ИзменитьИспользованиеРегламентныхЗаданий: incorrect value passed in parameter Use:%1'"),
			ТипЗнч(Использовать));
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:ОблачныйАрхив. El servicio y el reglamento';ru='БИП:ОблачныйАрхив.Сервис и регламент';en='БИП:ОблачныйАрхив.Сервис и регламент'"), // ИмяСобытия
			НСтр("es='El archivo nuboso. El servicio y el reglamento. ИзменениеИспользованияРегламентныхЗаданий';ru='Облачный архив. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий';en='Cloud Archive. Service and regulations. ИзменениеИспользованияРегламентныхЗаданий'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
	...
	Если МассивЧастейВремени.Количество() = 3 Тогда
		...
		Исключение
			...
			ОписаниеРезультата = ОписаниеРезультата
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La descripción del tiempo incorrecto: %1 "
"Había una falta de la transformación: "
"%2';ru='Описание времени некорректное: %1"
"Произошла ошибка преобразования:"
"%2';en='The time description is incorrect:%1"
...
"%2'")
	...
	Иначе
		...
		ОписаниеРезультата = ОписаниеРезультата
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La descripción del tiempo incorrecto: %1';ru='Описание времени некорректное: %1';en='The time description is incorrect:%1'")
					+ Символы.ПС,
				ТекстВремя);
	КонецЕсли;
...
Функция ПолучитьДниНеделиИзТекста(ТекстДниНедели, ОписаниеРезультата)
	...
	Для Каждого ТекущийДеньНедели Из МассивДнейНедели Цикл
		...
		Иначе
			ОписаниеРезультата = ОписаниеРезультата
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El día incorrecto de la semana: %1';ru='Неправильный день недели: %1';en='Wrong day of the week:%1'")
						+ Символы.ПС,
					ТекущийДеньНедели);
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьДниМесяцаИзТекста(ТекстДниМесяца, ОписаниеРезультата)
	...
	Для Каждого ТекущийДеньМесяца Из МассивДнейМесяца Цикл
		...
		Иначе
			...
			ОписаниеРезультата = ОписаниеРезультата
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El día incorrecto del mes: %1';ru='Неправильный день месяца: %1';en='Wrong day of the month:%1'")
						+ Символы.ПС,
					ТекущийДеньМесяца);
		КонецЕсли;
	КонецЦикла;
...
#Область ОсновноеОбновление
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("es='La renovación básica';ru='Основное обновление';en='Main update'"));
...
#Область ПереопределяемоеОбновление
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("es='La renovación redeterminada';ru='Переопределяемое обновление';en='Override update'"));
...
"%5'")
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("es='БИП:ОблачныйАрхив. El servicio y el reglamento';ru='БИП:ОблачныйАрхив.Сервис и регламент';en='БИП:ОблачныйАрхив.Сервис и регламент'"), // ИмяСобытия
		НСтр("es='El archivo nuboso. El servicio y el reglamento. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПервыйЗапуск';ru='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПервыйЗапуск';en='Cloud Archive. Service and regulations. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПервыйЗапуск'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
#Область ОсновноеОбновление_ЗагрузитьВсеСтандартныеЗначения
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("es='La renovación básica, la carga de los significados estandartizados';ru='Основное обновление, загрузка стандартных значений';en='Main update, loading standard values'"));
...
"%5'")
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("es='БИП:ОблачныйАрхив. El servicio y el reglamento';ru='БИП:ОблачныйАрхив.Сервис и регламент';en='БИП:ОблачныйАрхив.Сервис и регламент'"), // ИмяСобытия
		НСтр("es='El archivo nuboso. El servicio y el reglamento. ОбновлениеИнформационнойБазы. ОбластьДанных. ПервыйЗапуск';ru='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПервыйЗапуск';en='Cloud Archive. Service and regulations. ОбновлениеИнформационнойБазы. ОбластьДанных. ПервыйЗапуск'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
"%5'")
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("es='БИП:ОблачныйАрхив. El servicio y el reglamento';ru='БИП:ОблачныйАрхив.Сервис и регламент';en='БИП:ОблачныйАрхив.Сервис и регламент'"), // ИмяСобытия
		НСтр("es='El archivo nuboso. El servicio y el reglamento. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПерейтиНаВерсию';ru='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПерейтиНаВерсию';en='Cloud Archive. Service and regulations. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПерейтиНаВерсию'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
#Область ОсновноеОбновление_ИзменитьИспользованиеРегламентныхЗаданий
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("es='La renovación básica, la instalación del horario';ru='Основное обновление, установка расписания';en='Main update, schedule setting'"));
...
"%5'")
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("es='БИП:ОблачныйАрхив. El servicio y el reglamento';ru='БИП:ОблачныйАрхив.Сервис и регламент';en='БИП:ОблачныйАрхив.Сервис и регламент'"), // ИмяСобытия
		НСтр("es='El archivo nuboso. El servicio y el reglamento. ОбновлениеИнформационнойБазы. ОбластьДанных. ПерейтиНаВерсию';ru='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПерейтиНаВерсию';en='Cloud Archive. Service and regulations. ОбновлениеИнформационнойБазы. ОбластьДанных. ПерейтиНаВерсию'"), // ИдентификаторШага
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
Функция СобратьДанные_ИнформацияОКлиенте(КонтекстВыполнения = Неопределено)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("es='La renovación de la información sobre el cliente';ru='Обновление информации о клиенте';en='Customer information update'"));
...
#Область Шаг_ПолучениеДанныхИзРеестраWindows
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La recepción de la información sobre el agente establecido de la copia y los parámetros del ordenador (del registro Windows)';ru='Получение информации об установленном агенте копирования и параметрах компьютера (из реестра Windows)';en='Getting information about the installed copy agent and computer settings (from the Windows registry)'"));
...
#Область Шаг_ПолучениеДанныхИзРеестраWindows_ТипОС
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La recepción de la información sobre el agente establecido de la copia y los parámetros del ordenador (del registro Windows), el tipo de la SO';ru='Получение информации об установленном агенте копирования и параметрах компьютера (из реестра Windows), тип ОС';en='Getting information about installed copy agent and computer settings (from the Windows registry), OS type'"));
		...
		Исключение
			...
			ОписаниеРезультатаШага2 = ОписаниеРезультатаШага2
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La falta de la recepción de los parámetros del agente de la copia (el tipo del sistema Windows de operaciones) (a través de los objetos OLE, los ambientes variables) por la causa: "
"%1';ru='Ошибка получения параметров агента копирования (тип операционной системы Windows) (через объекты OLE, переменные окружения) по причине:"
"%1';en='Error getting copy agent parameters (type of Windows operating system) (via OLE objects, environment variables) due to:"
"%1'")
						+ Символы.ПС,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		КонецПопытки;
		...
		Иначе
			...
			ОписаниеРезультатаШаг2 = ОписаниеРезультатаШаг2
				+ НСтр("es='La falta de la recepción de los parámetros del agente de la copia (ТипОС). Las causas eran indicadas más arriba.';ru='Ошибка получения параметров агента копирования (ТипОС). Причины были указаны выше.';en='Error getting copy agent parameters (ТипОС). The reasons have been indicated above.'")
				+ Символы.ПС;
		КонецЕсли;
...
#Область Шаг_ПолучениеДанныхИзРеестраWindows_КаталогИВерсияАгента
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La recepción de la información sobre el agente establecido de la copia y los parámetros del ordenador (del registro Windows), el catálogo y las versiones del agente';ru='Получение информации об установленном агенте копирования и параметрах компьютера (из реестра Windows), каталог и версии агента';en='Getting information about the installed copy agent and computer settings (from the Windows registry), directory and agent versions'"));
		...
		Иначе
			...
			ОписаниеРезультатаШаг2 = ОписаниеРезультатаШаг2
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='de la instalación y la versión del agente, porque el sistema de operaciones no se refiere a Windows. "
"El tipo de la plataforma: %1 "
"Tipo de la SO: %2';ru='Не получен каталог установки и версия агента, т.к. операционная система не относится к Windows."
"Тип платформы: %1"
"Тип ОС: %2';en='The installation directory and version of the agent were not received because the operating system does not apply to Windows."
"Platform Type: %1"
"OS Type: %2 '"),
		...
		Если (НЕ ПустаяСтрока(КаталогУстановкиАгентаКопирования))
			...
			ОписаниеРезультатаШаг2 = ОписаниеРезультатаШаг2
				+ НСтр("es='Los datos son con éxito recibidos del registro Windows.';ru='Данные успешно получены из реестра Windows.';en='Data successfully retrieved from the Windows registry.'")
				+ Символы.ПС;
		КонецЕсли;
...
#Область Шаг_ПолучениеИдентификатораКомпьютера
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La recepción de la información sobre el agente establecido de la copia y los parámetros del ordenador (del registro Windows), el identificador del ordenador';ru='Получение информации об установленном агенте копирования и параметрах компьютера (из реестра Windows), идентификатор компьютера';en='Getting information about the installed copy agent and computer settings (from the Windows registry), computer identifier'"));
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				Попытка
					...
					ОписаниеРезультатаШаг2 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La recepción exitosa del identificador del ordenador (MachineGuid) del registro Windows (a través de los objetos OLE): "
"%1';ru='Успешное получение идентификатора компьютера (MachineGuid) из реестра Windows (через объекты OLE):"
"%1';en='Successfully retrieving a computer identifier (MachineGuid) from the Windows registry (through OLE objects):"
"%1'")
							+ Символы.ПС,
						ИдентификаторКомпьютераРеестр);
				Исключение
					...
					ОписаниеРезультатаШаг2 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La falta de la recepción de los parámetros del agente de la copia del registro Windows (a través de los objetos OLE) por la causa: "
"%1';ru='Ошибка получения параметров агента копирования из реестра Windows (через объекты OLE) по причине:"
"%1';en='Error getting copy agent settings from the Windows registry (through OLE objects) due to:"
"%1'")
							+ Символы.ПС,
						ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				КонецПопытки;
...
#Область Шаг_ПолучениеИдентификатораКомпьютера_КомандаReg
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				Попытка
					...
					ОписаниеРезультатаШаг2 = ОписаниеРезультатаШаг2
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='Exitoso las recepciones del identificador del ordenador (MachineGuid) del registro Windows (a través de reg.exe): "
"la Línea de la orden: "
"%1 "
"Código del regreso: %2 "
"Flujo de la conclusión: "
"%3 "
"Flujo de las faltas: "
"%4 "
"MachineGuid: %5';ru='Успешное получения идентификатора компьютера (MachineGuid) из реестра Windows (через reg.exe):"
"Строка команды:"
"%1"
"Код возврата: %2"
"Поток вывода:"
...
"Поток ошибок:"
"%4"
"MachineGuid: %5';en='Successfully retrieving the computer identifier (MachineGuid) from the Windows registry (via reg.exe):"
"Command line:"
"%one"
"Return Code:%2"
"Output stream:"
...
"MachineGuid:%5'")
				...
				Исключение
					...
					ОписаниеРезультатаШаг2 = ОписаниеРезультатаШаг2
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='La falta de la recepción de los parámetros del agente de la copia del registro Windows (a través de reg.exe) por la causa: "
"%1';ru='Ошибка получения параметров агента копирования из реестра Windows (через reg.exe) по причине:"
"%1';en='Error retrieving copy agent settings from the Windows registry (via reg.exe) because:"
"%1'")
								+ Символы.ПС,
							ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				КонецПопытки;
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				Если НЕ ПустаяСтрока(КаталогУстановкиАгентаКопирования) Тогда
					...
					Попытка
						...
						ОписаниеРезультатаШаг2 = ОписаниеРезультатаШаг2
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='Exitoso las recepciones del identificador del ordenador (MachineGuid) a través de BackupAgent.exe: "
"la Línea de la orden: "
"%1 "
"Código del regreso: %2 "
"Flujo de la conclusión: "
...
"Flujo de las faltas: "
"%4 "
"MachineGuid: %5';ru='Успешное получения идентификатора компьютера (MachineGuid) через BackupAgent.exe:"
"Строка команды:"
"%1"
"Код возврата: %2"
"Поток вывода:"
...
"Поток ошибок:"
"%4"
"MachineGuid: %5';en='Successfully retrieving the computer ID (MachineGuid) through BackupAgent.exe:"
"Command line:"
"%one"
"Return Code:%2"
"Output stream:"
...
"MachineGuid:%5 '")
					...
					Исключение
						...
						ОписаниеРезультатаШаг2 = ОписаниеРезультатаШаг2
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La falta de la recepción de los parámetros del agente de la copia a través de BackupAgent.exe por la causa: "
"%1';ru='Ошибка получения параметров агента копирования через BackupAgent.exe по причине:"
"%1';en='Error retrieving copy agent settings through BackupAgent.exe due to:"
"%1'")
									+ Символы.ПС,
								ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
					КонецПопытки;
...
#КонецОбласти
			...
			Иначе
				...
				ОписаниеРезультатаШаг1 = ОписаниеРезультатаШаг1
					+ НСтр("es='La falta de la recepción de los parámetros del agente de la copia (ИдентификаторКомпьютера). Las causas eran indicadas más arriba.';ru='Ошибка получения параметров агента копирования (ИдентификаторКомпьютера). Причины были указаны выше.';en='Error getting copy agent settings (ИдентификаторКомпьютера). The reasons have been indicated above.'")
					+ Символы.ПС;
			КонецЕсли;
...
#Область Шаг_ПроверкаДанныхОбАгенте_Ключи
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("es='La comprobación de la presencia de los ficheros del programa y los ficheros de las llaves';ru='Проверка наличия файлов программы и файлов ключей';en='Checking for program files and key files'"));
			...
			Если КаталогУстановкиАгентаКопирования.Существует() Тогда
				...
				Для Каждого ТекущийФайл Из ФайлыДляПроверки Цикл
					...
					Если МассивНайденныхФайлов.Количество() <= 0 Тогда
						...
						ОписаниеРезультатаШаг1 = ОписаниеРезультатаШаг1
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='El fichero del programa %1 falta';ru='Файл программы %1 отсутствует';en='Program file%1 is missing'")
									+ Символы.ПС,
								ТекущийФайл);
					Иначе
						ОписаниеРезультатаШаг1 = ОписаниеРезультатаШаг1
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='El fichero del programa %1 es encontrado';ru='Файл программы %1 найден';en='Program file%1 found'")
									+ Символы.ПС,
								ТекущийФайл);
					КонецЕсли;
				КонецЦикла;
				...
				Для Каждого ТекущийФайл Из ФайлыДляПроверки Цикл
					...
					Если МассивНайденныхФайлов.Количество() <= 0 Тогда
						...
						ОписаниеРезультатаШаг1 = ОписаниеРезультатаШаг1
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='El fichero de la llave %1 falta';ru='Файл ключа %1 отсутствует';en='Key file%1 is missing'")
									+ Символы.ПС,
								ТекущийФайл);
					Иначе
						ОписаниеРезультатаШаг1 = ОписаниеРезультатаШаг1
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='El fichero de la llave %1 es encontrado';ru='Файл ключа %1 найден';en='Key file%1 found'")
									+ Символы.ПС,
								ТекущийФайл);
					КонецЕсли;
				КонецЦикла;
				...
				Если ЕстьОшибки = Истина Тогда
					...
					ОписаниеРезультатаШаг1 = ОписаниеРезультатаШаг1
						+ НСтр("es='Faltan los ficheros necesarios para el trabajo del Agente de la copia de reserva';ru='Отсутствуют файлы, необходимые для работы Агента резервного копирования';en='Missing files required for Backup agent to work'");
				КонецЕсли;
			...
			Иначе
				...
				ОписаниеРезультатаШаг1 = ОписаниеРезультатаШаг1
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Falta el catálogo de la instalación del Agente de la copia de reserva (%1)';ru='Отсутствует каталог установки Агента резервного копирования (%1)';en='Missing Backup agent installation directory (%1)'"),
...
#КонецОбласти
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='ОблачныйАрхив. El %1 "
"Tipo de la SO: %2 "
"Versión de la SO: %3 "
"Tipo de la plataforma 1С: %4 "
"Versión 1С: %5 "
"Línea de la conexión: %6 "
"TekuschiyRezhimZapuska: %7';ru='ОблачныйАрхив.%1"
"Тип ОС: %2"
"Версия ОС: %3"
"Тип платформы 1С: %4"
"Версия 1С: %5"
"Строка подключения: %6"
"ТекущийРежимЗапуска: %7';en='ОблачныйАрхив.%2                       OS Type:%2"
"OS Version:%3"
"Platform Type 1C:%4"
"Version 1C:%5"
"Connection string:%6 ТекущийРежимЗапуска: %7'")
			+ Символы.ПС,
		НаименованиеПроцедурыФункции,
...
Функция СобратьДанные_НастройкиАгентаКопированияКлиент(КонтекстВыполнения = Неопределено)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОблачныйАрхив." + НаименованиеПроцедурыФункции,
		НСтр("es='La renovación de la información sobre los ajustes del agente de la copia';ru='Обновление информации о настройках агента копирования';en='Update copy agent settings information'"));
...
#Область Шаг_ПроверкаДанныхОбАгенте_Настройки_Каталоги
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("es='La comprobación de los catálogos para el trabajo del Agente de la copia de reserva';ru='Проверка каталогов для работы Агента резервного копирования';en='Checking directories for Backup agent'"));
				...
				КаталогиДляПроверки.Добавить(ИмяКаталогаЛогов, НСтр("es='El catálogo de las guaridas';ru='Каталог логов';en='Log directory'"));
				КаталогиДляПроверки.Добавить(ИмяКаталогаСтатистики, НСтр("es='El catálogo de la estadística';ru='Каталог статистики';en='Statistics directory'"));
				КаталогиДляПроверки.Добавить(ИмяКаталогаРабочий, НСтр("es='El catálogo de trabajo';ru='Рабочий каталог';en='Working directory'"));
				Для Каждого ТекущийКаталог Из КаталогиДляПроверки Цикл
					...
					Если Каталог.Существует() Тогда
						ВложенноеОписаниеРезультата =
							ВложенноеОписаниеРезультата
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='%1 (%2) existe.';ru='%1 (%2) существует.';en='% 1 (% 2) exists.'")
									+ Символы.ПС,
					...
					Иначе
						Попытка
							...
							ВложенноеОписаниеРезультата =
								ВложенноеОписаниеРезультата
								+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("es='%1 (%2) faltaba, pero era con éxito creado.';ru='%1 (%2) отсутствовал, но был успешно создан.';en='% 1 (% 2) was missed, but was created successfully.'")
										+ Символы.ПС,
									ТекущийКаталог.Представление,
									ТекущийКаталог.Значение);
						Исключение
							...
							ВложенноеОписаниеРезультата =
								ВложенноеОписаниеРезультата
								+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("es='%1 (%2) faltaba, no lo consiguió crear por la causa: "
"%3';ru='%1 (%2) отсутствовал, создать его не удалось по причине:"
"%3';en='% 1 (% 2) was missing, it could not be created due to:"
"% 3'")
										+ Символы.ПС,
									ТекущийКаталог.Представление,
									ТекущийКаталог.Значение,
...
#Область Шаг_ПроверкаДанныхОбАгенте_Настройки_Расписание
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("es='La carga de los ajustes de este ИБ del catálogo de trabajo';ru='Загрузка настроек этой ИБ из рабочего каталога';en='Downloading the settings of this information security from the working directory'"));
				...
				Если МассивНайденныхФайлов.Количество() >= 1 Тогда
					...
					Если ФайлНастроекИБ.Существует() Тогда
						...
						ВложенноеОписаниеРезультата = ВложенноеОписаниеРезультата
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='Es encontrado el fichero de los ajustes del Agente de la copia de reserva para ИБ (%1). Es hecha la lectura de los ajustes.';ru='Найден файл настроек Агента резервного копирования для ИБ (%1). Производится считывание настроек.';en='Found backup agent settings file for IB (%1). The settings are being read.'")
									+ Символы.ПС,
								ИмяФайлаНастроекИБ);
							...
							Если ВложенныеЭлементы.Свойство("DatabaseBackupConfiguration") Тогда
								...
								Если ВложенныеЭлементы.Свойство("Schedule") Тогда
									...
									Если СтруктураРасписания.Элементы.Свойство("BackupNow") Тогда
										...
										ВложенноеОписаниеРезультата = ВложенноеОписаниеРезультата
											+ НСтр("es='El ajuste del horario BackupNow es dejado pasar.';ru='Настройка расписания BackupNow пропущена.';en='Scheduled BackupNow skipped.'")
												+ Символы.ПС;
									КонецЕсли;
...
#Область Шаг_ПроверкаДанныхОбАгенте_Настройки_Расписание_Ежедневно
									...
									Если РасписаниеНайдено = Ложь Тогда
										Если СтруктураРасписания.Элементы.Свойство("Time") Тогда
											ВложенноеОписаниеРезультата = ВложенноеОписаниеРезультата
												+ НСтр("es='Es encontrado el ajuste del horario Time.';ru='Найдена настройка расписания Time.';en='Found Time schedule setting.'")
												+ Символы.ПС;
									...
									Если РасписаниеНайдено = Ложь Тогда
										Если СтруктураРасписания.Элементы.Свойство("BackupDaily") Тогда
											ВложенноеОписаниеРезультата = ВложенноеОписаниеРезультата
												+ НСтр("es='Es encontrado el ajuste del horario BackupDaily.';ru='Найдена настройка расписания BackupDaily.';en='Found schedule setting BackupDaily.'")
													+ Символы.ПС;
											...
											Если ТекущийЭлементРасписания.Атрибуты.Свойство("time")
												...
												ВложенноеОписаниеРезультата = ВложенноеОписаниеРезультата
													+ НСтр("es='Es encontrado el ajuste del horario BackupDaily (el comienzo, la terminación, la periodicidad).';ru='Найдена настройка расписания BackupDaily (начало, окончание, периодичность).';en='Found BackupDaily schedule setting (start, end, frequency).'")
														+ Символы.ПС;
											...
											Если РасписаниеНайдено = Ложь Тогда
												Если ТекущийЭлементРасписания.Атрибуты.Свойство("time") Тогда
													ВложенноеОписаниеРезультата = ВложенноеОписаниеРезультата
														+ НСтр("es='Es encontrado el ajuste del horario BackupDaily (cada día).';ru='Найдена настройка расписания BackupDaily (ежедневно).';en='BackupDaily schedule setting found (daily).'")
															+ Символы.ПС;
...
#Область Шаг_ПроверкаДанныхОбАгенте_Настройки_Расписание_Еженедельно
									...
									Если РасписаниеНайдено = Ложь Тогда
										Если СтруктураРасписания.Элементы.Свойство("BackupWeekly") Тогда
											ВложенноеОписаниеРезультата = ВложенноеОписаниеРезультата
												+ НСтр("es='Es encontrado el ajuste del horario BackupWeekly.';ru='Найдена настройка расписания BackupWeekly.';en='Found BackupWeekly schedule setting.'")
													+ Символы.ПС;
...
#Область Шаг_ПроверкаДанныхОбАгенте_Настройки_Расписание_Ежемесячно
									...
									Если РасписаниеНайдено = Ложь Тогда
										Если СтруктураРасписания.Элементы.Свойство("BackupMonthly") Тогда
											ВложенноеОписаниеРезультата = ВложенноеОписаниеРезультата
												+ НСтр("es='Es encontrado el ajuste del horario BackupMonthly.';ru='Найдена настройка расписания BackupMonthly.';en='Found BackupMonthly schedule setting.'")
													+ Символы.ПС;
											...
											Если ТекущийЭлементРасписания.Атрибуты.Свойство("time")
												...
												ВложенноеОписаниеРезультата = ВложенноеОписаниеРезультата
													+ НСтр("es='Es encontrado el ajuste del horario BackupMonthly (el tiempo, los días del mes).';ru='Найдена настройка расписания BackupMonthly (время, дни месяца).';en='Found backupMonthly schedule setting (time, days of the month).'")
														+ Символы.ПС;
												ТекстВремя = ТекущийЭлементРасписания.Атрибуты.time;
												Время = ПолучитьВремяИзТекста(ТекстВремя, ВложенноеОписаниеРезультата);
											...
											Если РасписаниеНайдено = Ложь Тогда
												Если ТекущийЭлементРасписания.Атрибуты.Свойство("time")
														...
														И ТекущийЭлементРасписания.Атрибуты.Свойство("weekDay") Тогда
													ВложенноеОписаниеРезультата = ВложенноеОписаниеРезультата
														+ НСтр("es='Es encontrado el ajuste del horario BackupMonthly (el tiempo, los días de la semana).';ru='Найдена настройка расписания BackupMonthly (время, дни недели).';en='Found backupMonthly schedule setting (time, days of the week).'")
															+ Символы.ПС;
													ТекстВремя = ТекущийЭлементРасписания.Атрибуты.time;
													Время = ПолучитьВремяИзТекста(ТекстВремя, ВложенноеОписаниеРезультата);
...
#КонецОбласти
					...
					Иначе
						...
						ВложенноеОписаниеРезультата = ВложенноеОписаниеРезультата
							+ НСтр("es='El fichero de los ajustes del Agente de la copia de reserva para ИБ no es encontrado. Será ajustado el horario por-reticencia.';ru='Файл настроек Агента резервного копирования для ИБ НЕ найден. Будет настроено расписание по-умолчанию.';en='Backup Agent settings file for IB was NOT found. The default schedule will be configured.'")
								+ Символы.ПС;
					КонецЕсли;
				Иначе
				...
				Если ПолучитьРасписаниеПоУмолчанию = Истина Тогда
					РасписаниеАвтоматическогоРезервногоКопирования = ХранилищаНастроек.НастройкиОблачногоАрхива.ПровестиВалидациюНастроек(
						...
						"");
					ВложенноеОписаниеРезультата = ВложенноеОписаниеРезультата
						+ НСтр("es='Los ajustes del horario son generados por-reticencia.';ru='Настройки расписания сгенерированы по-умолчанию.';en='Schedule settings are generated by default.'")
							+ Символы.ПС;
				Иначе
...
#КонецОбласти
		...
		Иначе
			...
			ИнтернетПоддержкаПользователейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
				...
				124,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Falta el catálogo de los ajustes del Agente establecido de la copia de reserva (%1). Probablemente, él era quitado a mano.';ru='Отсутствует каталог настроек установленного Агента резервного копирования (%1). Возможно, он был удален вручную.';en='The settings directory of the installed Backup Agent (%1) is missing. It may have been deleted manually.'"),
...
#КонецЕсли
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='ОблачныйАрхив. El %1';ru='ОблачныйАрхив.%1';en='ОблачныйАрхив.%1'")
			+ Символы.ПС,
		НаименованиеПроцедурыФункции);
	КонтекстВыполнения.ОписаниеРезультата = ТекстСообщения;
