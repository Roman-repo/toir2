...
Функция ПоддерживаетсяУстановкаОбновлений() Экспорт
	...
	Если ПараметрыРаботыКлиента.РазделениеВключено Тогда 
		Результат.ОписаниеОшибки = 
			НСтр("es='La instalación de las renovaciones para la aplicación en el Internet se cumple centralizadamente a través del Manager del servicio.';ru='Установка обновлений для приложения в Интернете выполняется централизованно через Менеджер сервиса.';en='Installing updates for an application on the Internet is performed centrally through the Service Manager.'");
		Возврат Результат;
	КонецЕсли;
	
	Если Не ПараметрыРаботыКлиента.ЭтоПолноправныйПользователь Тогда 
		Результат.ОписаниеОшибки = НСтр("es='Para la instalación de la renovación son necesarios los derechos de la administración.';ru='Для установки обновления требуются права администрирования.';en='To install the update, you need administrative rights.'");
		Возврат Результат;
	КонецЕсли;
	
#Если ВебКлиент Тогда
	Результат.ОписаниеОшибки =
		НСтр("es='La instalación de la renovación no es accesible en el cliente web. "
"Es necesario establecer el distributivo completo de la plataforma tecnológica 1С:Предприятие para Windows.';ru='Установка обновления не доступна в веб-клиенте."
"Необходимо установить полный дистрибутив технологической платформы 1С:Предприятие для Windows.';en='Installing the update is not available in the web client."
"You must install the full distribution of the technology platform 1C: Enterprise for Windows.'");
	
#ИначеЕсли МобильныйКлиент Тогда
	Результат.ОписаниеОшибки = НСтр("es='La instalación de la renovación es accesible solamente en la SO Windows.';ru='Установка обновления доступна только в ОС Windows.';en='Installing the update is available only on Windows.'");
#Иначе
	...
	Если ОбщегоНазначенияКлиент.КлиентПодключенЧерезВебСервер() Тогда 
		...
		Результат.ОписаниеОшибки = Результат.ОписаниеОшибки
			+ НСтр("es='La instalación de la renovación no es accesible a la conexión a través del servidor web. "
"Es necesario ajustar la conexión directa a la base informativa.';ru='Установка обновления не доступна при подключении через веб-сервер."
"Необходимо настроить прямое подключение к информационной базе.';en='Installing the update is not available when connecting via the web server."
"You must configure a direct connection to the infobase. '");
	КонецЕсли;
	...
	Если Не ПоддерживаетсяПакетныйРежимКонфигуратора() Тогда 
		...
		КонецЕсли;
		Результат.ОписаниеОшибки = Результат.ОписаниеОшибки
			+ НСтр("es='Para la instalación de la renovación es necesario конфигуратор. "
"Es necesario establecer el distributivo completo de la plataforma tecnológica 1С:Предприятие para Windows.';ru='Для установки обновления требуется конфигуратор."
"Необходимо установить полный дистрибутив технологической платформы 1С:Предприятие для Windows.';en='A configurator is required to install the update."
"You must install the full distribution of the technology platform 1C: Enterprise for Windows.'");
	КонецЕсли;
...
Функция ЗаголовокСозданияРезервнойКопии(Параметры) Экспорт
	...
	Если Параметры.СоздаватьРезервнуюКопию = 0 Тогда
		Возврат НСтр("es='No crear la copia ИБ de reserva';ru='Не создавать резервную копию ИБ';en='Do not backup IB'");
	ИначеЕсли Параметры.СоздаватьРезервнуюКопию = 1 Тогда
		Если Параметры.ВосстанавливатьИнформационнуюБазу Тогда 
			Возврат НСтр("es='Crear la copia ИБ temporal de reserva y cumplir el acarreo a la situación supernumeraria';ru='Создавать временную резервную копию ИБ и выполнять откат при нештатной ситуации';en='Create a temporary backup of the information security and roll back in case of emergency'");
		Иначе 
			Возврат НСтр("es='Crear la copia ИБ temporal de reserva y no cumplir el acarreo a la situación supernumeraria';ru='Создавать временную резервную копию ИБ и не выполнять откат при нештатной ситуации';en='Create a temporary backup of IB and do not roll back in case of emergency'");
		КонецЕсли;
	ИначеЕсли Параметры.СоздаватьРезервнуюКопию = 2 Тогда
		Если Параметры.ВосстанавливатьИнформационнуюБазу Тогда 
			Возврат НСтр("es='Crear la copia ИБ de reserva y cumplir el acarreo a la situación supernumeraria';ru='Создавать резервную копию ИБ и выполнять откат при нештатной ситуации';en='Create a back up information security and roll back in case of emergency'");
		Иначе 
			Возврат НСтр("es='Crear la copia ИБ de reserva y no cumplir el acarreo a la situación supernumeraria';ru='Создавать резервную копию ИБ и не выполнять откат при нештатной ситуации';en='Create a back up copy of IB and do not roll back in case of emergency'");
		КонецЕсли;
	Иначе
		Возврат "";
	КонецЕсли;
...
Процедура ОбработатьРезультатОбновления(РезультатОбновления, КаталогСкрипта) Экспорт
	...
	Если ПустаяСтрока(КаталогСкрипта) Тогда
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),
			"Предупреждение",
			НСтр("es='La renovación es cumplida de la versión muy vieja del programa. "
"Los datos de la revista de la renovación no eran cargados, pero "
"se puede encontrar la revista misma en la carpeta de los ficheros temporales %temp del % - "
"en la carpeta del tipo 1Cv8Update. <xxxxxxxx> (las cifras).';ru='Обновление выполнено с очень старой версии программы. "
"Данные журнала обновления не были загружены, но сам журнал "
"можно найти в папке временных файлов %temp% - "
"в папке вида 1Cv8Update.<xxxxxxxx>(цифры).';en='The update was performed with a very old version of the program."
"Update log data was not loaded, but the log itself"
"can be found in the temporary files folder% temp% -"
"in the view folder  1Cv8Update.<xxxxxxxx>(figures).'"),
			, 
			Истина);
	...
	Иначе 
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La carpeta con la revista de la renovación está situada %1';ru='Папка с журналом обновления расположена %1';en='The update log folder is located%1'"), КаталогСкрипта),
			,
			Истина);
...
Процедура ПередЗавершениемРаботыСистемы(Отказ, Предупреждения) Экспорт
	...
	Если ПараметрыПриложения["СтандартныеПодсистемы.ПредлагатьОбновлениеИнформационнойБазыПриЗавершенииСеанса"] = Истина Тогда
		...
		ПараметрыПредупреждения.ТекстФлажка  = НСтр("es='Establecer la renovación de la configuración';ru='Установить обновление конфигурации';en='Install configuration update'");
		ПараметрыПредупреждения.ТекстПредупреждения  = НСтр("es='Es planeada la instalación de la renovación';ru='Запланирована установка обновления';en='Scheduled update installation'");
...
Функция ВозможнаУстановкаОбновления(Параметры, ПараметрыАдминистрирования)
	...
	Если ЭтоФайловаяБаза И Параметры.СоздаватьРезервнуюКопию = 2 Тогда
		...
		Если Не Файл.Существует() Или Не Файл.ЭтоКаталог() Тогда
			ПоказатьПредупреждение(, НСтр("es='Indiquen el catálogo existente para la conservación de la copia ИБ de reserva.';ru='Укажите существующий каталог для сохранения резервной копии ИБ.';en='Specify an existing directory to save the IB backup.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если Параметры.РежимОбновления = 0 Тогда // Обновить сейчас
		...
		Если ЭтоФайловаяБаза И ОбновлениеКонфигурацииВызовСервера.НаличиеАктивныхСоединений(ПараметрыПриложения[ИмяПараметра]) Тогда
			ПоказатьПредупреждение(, НСтр("es='Es imposible continuar la renovación de la configuración, puesto que no son acabadas todas las uniones con la base informativa.';ru='Невозможно продолжить обновление конфигурации, так как не завершены все соединения с информационной базой.';en='Unable to continue updating the configuration because all connections to the infobase have not been completed.'"));
			Возврат Ложь;
		КонецЕсли;
	ИначеЕсли Параметры.РежимОбновления = 2 Тогда
		...
		Если Параметры.ВыслатьОтчетНаПочту
			...
			ПоказатьПредупреждение(, НСтр("es='Indiquen la dirección admisible del correo electrónico.';ru='Укажите допустимый адрес электронной почты.';en='Please enter a valid email address.'"));
			Возврат Ложь;
		КонецЕсли;
		Если Не ПоддерживаетсяПланировщикЗаданий() Тогда
			ПоказатьПредупреждение(, НСтр("es='Планировщик de las tareas es apoyado solamente a partir del sistema de operaciones de la versión 6.0 Vista.';ru='Планировщик заданий поддерживается только начиная с операционной системы версии 6.0 Vista.';en='Task scheduler is only supported starting with operating system version 6.0 Vista.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Функция ДатаОбновленияУказанаВерно(Параметры)
	...
	Если Параметры.ДатаВремяОбновления < ТекущаяДата Тогда
		ТекстСообщения = НСтр("es='La renovación de la configuración puede ser planeada solamente a la próxima fecha y el tiempo.';ru='Обновление конфигурации может быть запланировано только на будущую дату и время.';en='A configuration update can only be scheduled for a future date and time.'");
	ИначеЕсли Параметры.ДатаВремяОбновления > ДобавитьМесяц(ТекущаяДата, 1) Тогда
		ТекстСообщения = НСтр("es='La renovación de la configuración puede ser planeada no más tarde que en un mes acerca de la fecha corriente.';ru='Обновление конфигурации может быть запланировано не позднее, чем через месяц относительно текущей даты.';en='A configuration update can be scheduled no later than a month from the current date.'");
	КонецЕсли;
...
Функция ИмяЗадачиScheduleService(Знач КодЗадачи)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La renovación de la configuración (%1)';ru='Обновление конфигурации (%1)';en='Configuration update (%1)'"), Формат(КодЗадачи, "ЧГ=0"));
...
Функция СобытиеЖурналаРегистрации() Экспорт
	Возврат НСтр("es='La renovación de la configuración';ru='Обновление конфигурации';en='Configuration update'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
КонецФункции
...
#Если НЕ ВебКлиент И НЕ МобильныйКлиент Тогда
	...
	Если НастройкиОбновления.КонфигурацияИзменена Тогда
		ПоказатьОповещениеПользователя(НСтр("es='La renovación de la configuración';ru='Обновление конфигурации';en='Configuration update'"),
			"e1cib/app/Обработка.УстановкаОбновлений",
			НСтр("es='La configuración se distingue de la configuración básica de la base informativa.';ru='Конфигурация отличается от основной конфигурации информационной базы.';en='The configuration is different from the basic configuration of the infobase.'"), 
...
Функция СформироватьФайлыСкриптаОбновления(Знач ИнтерактивныйРежим, Параметры, ПараметрыАдминистрирования)
	...
	Если ЭтоОтложенноеОбновление Тогда 
		...
		ОписаниеЗадачи = НСтр("es='La renovación de la configuración 1С:Предприятие';ru='Обновление конфигурации 1С:Предприятие';en='1C: Enterprise configuration update'");
...
Процедура ЗапуститьСкриптОбновления(Параметры, ПараметрыАдминистрирования)
	...
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Информация",
		НСтр("es='Se cumple el procedimiento de la renovación de la configuración:';ru='Выполняется процедура обновления конфигурации:';en='The configuration update procedure is in progress:'") + " " + ИмяГлавногоФайлаСкрипта);
