...
Процедура ДобавитьУсловиеФильтра(Фильтр, Знач Свойство, Знач ВидСравненияЗначений, Знач Значение) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("es='Es dada la descripción del filtro.';ru='Некорректно задано описание фильтра.';en='The description of the filter is incorrect.'");
...
Функция ПроверитьУсловияФильтра(Знач ПроверяемыйОбъект, Знач Фильтр = Неопределено) Экспорт
	...
	Для Каждого Условие Из Фильтр Цикл
		...
		Иначе
			...
			ВызватьИсключение НСтр("es='Es dado el filtro.';ru='Некорректно задан фильтр.';en='Filter set incorrectly.'");
...
Функция МенеджерАдминистрирования(Знач ПараметрыАдминистрирования)
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El tipo desconocido de la conexión: %1.';ru='Неизвестный тип подключения: %1.';en='Unknown connection type:%1.'"), ПараметрыАдминистрирования.ТипПодключения);
