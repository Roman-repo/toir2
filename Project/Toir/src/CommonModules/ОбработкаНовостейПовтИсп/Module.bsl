...
Функция ПолучитьИмяПользователяИБ(ПользовательСсылка = Неопределено) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("es='La falta de la recepción del nombre del usuario ИБ (ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ ()) por la causa: "
"%ИнформацияОбОшибке del %';ru='Ошибка получения имени пользователя ИБ (ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()) по причине:"
"%ИнформацияОбОшибке%';en='Error getting IB username (ОбработкаНовостейПовтИсп.ПолучитьИмяПользователяИБ()) due to: %ИнформацияОбОшибке%'");
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("es='БИП:Новости. Diferente';ru='БИП:Новости.Разное';en='BIP: News. Miscellaneous'"), // ИдентификаторШага
...
Функция ПолучитьПользователяПоИмениПользователяИБ(ИмяПользователяИБ) Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
		...
		Исключение
			Результат = Справочники.Пользователи.ПустаяСсылка();
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ТекстСообщения = НСтр("es='La falta de la recepción del usuario de la guía los Usuarios por su nombre (%ИмяПользователяИБ del %) en ИБ (ОбработкаНовостейПовтИсп.ПолучитьПользователяПоИмениПользователяИБ ()) por la causa: "
"%ИнформацияОбОшибке del %';ru='Ошибка получения пользователя из справочника Пользователи по его имени (%ИмяПользователяИБ%) в ИБ (ОбработкаНовостейПовтИсп.ПолучитьПользователяПоИмениПользователяИБ()) по причине:"
"%ИнформацияОбОшибке%';en='Error getting user from directory Users by his name (%ИмяПользователяИБ%) in IB (ОбработкаНовостейПовтИсп.ПолучитьПользователяПоИмениПользователяИБ()) due to: %ИнформацияОбОшибке%'");
	...
	Для Каждого ТекущийПользовательИБ Из МассивПользователейИБ Цикл
		...
		Если Добавить = Истина Тогда
			...
			Исключение
				БылиОшибки = Истина;
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				ТекстСообщения = НСтр("es='La falta de la búsqueda del usuario (%ПолныеПрава del %) por el identificador único: "
"el nombre Breve del usuario: %КраткоеИмяПользователя del % "
"el nombre Completo del usuario: %ПолноеИмяПользователя del % "
"el identificador Único: %УникальныйИдентификатор del % "
"la Causa: "
"%ИнформацияОбОшибке del %';ru='Ошибка поиска пользователя (%ПолныеПрава%) по уникальному идентификатору:"
...
"Причина:"
"%ИнформацияОбОшибке%';en='User search error %ПолныеПрава%) by unique identifier:"
"Short username: %КраткоеИмяПользователя% Full username: %ПолноеИмяПользователя% Unique identificator: %УникальныйИдентификатор% Cause: %ИнформацияОбОшибке%'")
					+ Символы.ПС;
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПолныеПрава%", ?(ПолныеПрава = Истина, НСтр("es='Son accesibles los plenos derechos';ru='доступны полные права';en='full rights available'"), НСтр("es='El papel de RedaktorNovostey';ru='роль РедакторНовостей';en='role РедакторНовостей'")));
				...
				Если НЕ НайденныйПользователь.Пустая() Тогда
					Результат.Добавить(
						Новый Структура("Пользователь, ПользовательПредставление, ПолныеПрава",
							...
							?(ПолныеПрава = Истина,
								НайденныйПользователь.Наименование + " " + НСтр("es='(Los plenos derechos)';ru='(полные права)';en='(full rights)'"),
								НайденныйПользователь.Наименование + " " + НСтр("es='(El redactor de las novedades)';ru='(редактор новостей)';en='(news editor)'")),
							ПолныеПрава));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
