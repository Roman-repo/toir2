...
Процедура УдалитьВременныйКаталог(Знач Путь) Экспорт
	...
	Если ЭтоИмяВременногоФайла(Путь) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El significado equivocado del parámetro la Vía a FaylovayaSistema. УдалитьВременныйКаталог: "
"el Catálogo no es temporal ""%1""';ru='Неверное значение параметра Путь в ФайловаяСистема.УдалитьВременныйКаталог:"
"Каталог не является временным ""%1""';en='Invalid Path value in ФайловаяСистема.УдалитьВременныйКаталог: directory is not temporary ""%1""'"), 
			Путь);
	КонецЕсли;
...
Процедура УдалитьВременныйФайл(Знач Путь) Экспорт
	...
	Если ЭтоИмяВременногоФайла(Путь) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El significado equivocado del parámetro la Vía a FaylovayaSistema. УдалитьВременныйФайл: "
"el Fichero no es temporal ""%1""';ru='Неверное значение параметра Путь в ФайловаяСистема.УдалитьВременныйФайл:"
"Файл не является временным ""%1""';en='Invalid Path value in ФайловаяСистема.УдалитьВременныйФайл: file is not temporary ""%1""'"), 
			Путь);
	КонецЕсли;
...
Процедура УдалитьВременныеФайлы(Знач Путь)
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("es='Los subsistemas estandartizados';ru='Стандартные подсистемы';en='Standard subsystems'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió quitar el fichero temporal ""%1"" por la causa: "
"%2';ru='Не удалось удалить временный файл ""%1"" по причине:"
"%2';en='Failed to delete temporary file"" ""%1"" ""due to:"
"%2'"),
				Путь,
