...
//   НСтр("es='El mensaje de error.';ru='Сообщение об ошибке.';en='Error message.'"), ,
...
//  ОбщегоНазначенияКлиент.СообщитьПользователю(
//   НСтр("es='El mensaje de error.';ru='Сообщение об ошибке.';en='Error message.'"), ОбъектИнформационнойБазы, "Ответственный",,Отказ);
//
//  4. Для вывода сообщения по ссылке на объект информационной базы:
//  ОбщегоНазначенияКлиент.СообщитьПользователю(
//   НСтр("es='El mensaje de error.';ru='Сообщение об ошибке.';en='Error message.'"), Ссылка, , , Отказ);
//
...
Функция ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты, ВыбратьРазрешенные = Ложь) Экспорт
	...
	Если ТипЗнч(Ссылка) = Тип("Строка") Тогда 
		...
		Исключение
			...
			НСтр("es='El primer parámetro equivocado la Referencia en la función ОбщегоНазначения.ЗначенияРеквизитовОбъекта: "
"%1';ru='Неверный первый параметр Ссылка в функции ОбщегоНазначения.ЗначенияРеквизитовОбъекта:"
"%1';en='Invalid first parameter Link in the function ОбщегоНазначения.ЗначенияРеквизитовОбъекта:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Если Ссылка = Неопределено Тогда 
			...
			Если Не ПравоДоступа("Чтение", МетаданныеОбъекта) Тогда 
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Es insuficientemente derecho para el trabajo con tabla ""%1""';ru='Недостаточно прав для работы с таблицей ""%1""';en='Not enough rights to work with table ""%1""'"), ПолноеИмяОбъектаМетаданных);
			КонецЕсли;
		КонецЕсли;
	...
	Иначе // Если передана ссылка.
		...
		Исключение
			ВызватьИсключение 
				НСтр("es='El primer parámetro equivocado la Referencia en la función ОбщегоНазначения.ЗначенияРеквизитовОбъекта: "
"- el Significado debe ser la referencia o el nombre del elemento predestinado';ru='Неверный первый параметр Ссылка в функции ОбщегоНазначения.ЗначенияРеквизитовОбъекта: "
"- Значение должно быть ссылкой или именем предопределенного элемента';en='Invalid first parameter Link in the function ОбщегоНазначения.ЗначенияРеквизитовОбъекта:                                     - The value must be a reference or the name of a predefined item.'");
		КонецПопытки;
		...
		Для Каждого Реквизит Из Реквизиты Цикл
			...
			Исключение 
				...
				Если Результат.Ошибка Тогда 
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El segundo parámetro equivocado los Requisitos en la función ОбщегоНазначения.ЗначенияРеквизитовОбъекта: %1';ru='Неверный второй параметр Реквизиты в функции ОбщегоНазначения.ЗначенияРеквизитовОбъекта: %1';en='Invalid second parameter in the function. ОбщегоНазначения.ЗначенияРеквизитовОбъекта: %1'"),
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El tipo equivocado del segundo parámetro los Requisitos en la función ОбщегоНазначения.ЗначенияРеквизитовОбъекта: %1';ru='Неверный тип второго параметра Реквизиты в функции ОбщегоНазначения.ЗначенияРеквизитовОбъекта: %1';en='Invalid type of the second parameter Requisites in the function ОбщегоНазначения.ЗначенияРеквизитовОбъекта: %1'"), 
...
Функция ЗначениеРеквизитаОбъекта(Ссылка, ИмяРеквизита, ВыбратьРазрешенные = Ложь) Экспорт
	...
	Если ПустаяСтрока(ИмяРеквизита) Тогда 
		ВызватьИсключение 
			НСтр("es='El segundo parámetro equivocado de ImyaRekvizita en la función ОбщегоНазначения.ЗначениеРеквизитаОбъекта: "
"- el Nombre del requisito debe ser rellenado';ru='Неверный второй параметр ИмяРеквизита в функции ОбщегоНазначения.ЗначениеРеквизитаОбъекта: "
"- Имя реквизита должно быть заполнено';en='The second parameter of the ИмяРеквизита in the  function ОбщегоНазначения.ЗначениеРеквизитаОбъекта:"
"- The name of the props must be filled in '");
	КонецЕсли;
...
Функция ЗначенияРеквизитовОбъектов(Ссылки, Знач Реквизиты, ВыбратьРазрешенные = Ложь) Экспорт
	...
	Если ПустаяСтрока(Реквизиты) Тогда 
		ВызватьИсключение 
			НСтр("es='El segundo parámetro equivocado los Requisitos en la función ОбщегоНазначения.ЗначенияРеквизитовОбъектов: "
"- Paul del objeto debe ser indicado';ru='Неверный второй параметр Реквизиты в функции ОбщегоНазначения.ЗначенияРеквизитовОбъектов: "
"- Поле объекта должно быть указано';en='Invalid second parameter Requisites in the function   ОбщегоНазначения.ЗначенияРеквизитовОбъектов:"
"- The object field must be specified '");
	...
	Если СтрНайти(Реквизиты, ".") <> 0 Тогда 
		ВызватьИсключение 
			НСтр("es='El segundo parámetro equivocado los Requisitos en la función ОбщегоНазначения.ЗначенияРеквизитовОбъектов: "
"- el Recurso a través del punto no es apoyado';ru='Неверный второй параметр Реквизиты в функции ОбщегоНазначения.ЗначенияРеквизитовОбъектов: "
"- Обращение через точку не поддерживается';en='Invalid second parameter Requisites in the  function. ОбщегоНазначения.ЗначенияРеквизитовОбъектов:"
"- Point-to-point handling not supported'");
	КонецЕсли;
	...
	Исключение
		ВызватьИсключение 
			НСтр("es='El primer parámetro equivocado de la Referencia en la función ОбщегоНазначения.ЗначенияРеквизитовОбъектов: "
"- los Significados del macizo deben ser las referencias';ru='Неверный первый параметр Ссылки в функции ОбщегоНазначения.ЗначенияРеквизитовОбъектов: "
"- Значения массива должны быть ссылками';en='Invalid first parameter Link in the function ОбщегоНазначения.ЗначенияРеквизитовОбъектов:                                   - Array values must be references'");
	КонецПопытки;
	...
	Исключение
		...
		Если Результат.Ошибка Тогда 
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El segundo parámetro equivocado los Requisitos en la función ОбщегоНазначения.ЗначенияРеквизитовОбъектов: %1';ru='Неверный второй параметр Реквизиты в функции ОбщегоНазначения.ЗначенияРеквизитовОбъектов: %1';en='Invalid second parameter Requisites in the function   ОбщегоНазначения.ЗначенияРеквизитовОбъектов: %1'"), 
				Результат.ОписаниеОшибки);
		КонецЕсли;
...
Функция ЗначениеРеквизитаОбъектов(МассивСсылок, ИмяРеквизита, ВыбратьРазрешенные = Ложь) Экспорт
	...
	Если ПустаяСтрока(ИмяРеквизита) Тогда 
		ВызватьИсключение НСтр("es='El segundo parámetro equivocado de ImyaRekvizita en la función ОбщегоНазначения.ЗначениеРеквизитаОбъектов: "
"- el Nombre del requisito debe ser rellenado';ru='Неверный второй параметр ИмяРеквизита в функции ОбщегоНазначения.ЗначениеРеквизитаОбъектов: "
"- Имя реквизита должно быть заполнено';en='The second parameter of the ИмяРеквизита in the  function ОбщегоНазначения.ЗначениеРеквизитаОбъектов:"
"- The name of the props must be filled in '");
	КонецЕсли;
...
Функция ПровестиДокументы(Документы) Экспорт
	...
	Для Каждого ДокументСсылка Из Документы Цикл
		...
		Если ДокументОбъект.ПроверитьЗаполнение() Тогда
			...
			КонецПопытки;
		Иначе
			ПредставлениеОшибки = НСтр("es='Campos del documento no son rellenados.';ru='Поля документа не заполнены.';en='Fields of the document are not filled.'");
		КонецЕсли;
...
Функция МестаИспользования(Знач НаборСсылок, Знач АдресРезультата = "") Экспорт
	...
	Для Каждого МестоИспользования Из МестаИспользования Цикл
		...
		ИначеЕсли МетаданныеКонстант.Содержит(МестоИспользования.Метаданные) Тогда
			Представление = МестоИспользования.Метаданные.Представление() + " (" + НСтр("es='La constante';ru='константа';en='constant'") + ")";
			
		ИначеЕсли МетаданныеПоследовательностей.Содержит(МестоИспользования.Метаданные) Тогда
			Представление = МестоИспользования.Метаданные.Представление() + " (" + НСтр("es='La consecuencia';ru='последовательность';en='sequence'") + ")";
...
Функция ОбщийМодуль(Имя) Экспорт
	...
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El módulo general ""%1"" no es encontrado.';ru='Общий модуль ""%1"" не найден.';en=""The general module '%1' was not found."""),
			Имя);
	КонецЕсли;
...
Функция ОбщиеПараметрыБазовойФункциональности() Экспорт
	...
	Если Не ПустаяСтрока(Минимальная)
		...
		ТекстСообщения = НСтр("es='La versión mínima de la plataforma ""1С:Предприятие 8"" es indicada más arriba recomendado. "
"La versión mínima - ""%1"", la versión recomendada - ""%2"".';ru='Минимальная версия платформы ""1С:Предприятие 8"" указана выше рекомендуемой."
"Минимальная версия - ""%1"", рекомендуемая версия - ""%2"".';en='The minimum version of the platform ""1C: Enterprise 8"" ""is indicated above the recommended one. The minimum version is ""%1 "", the recommended version is ""%2"".'");
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстСообщения,
...
Функция ИнтервалВремениСтрокой(ВремяНачала, ВремяОкончания = Неопределено) Экспорт
	...
	ИначеЕсли ВремяНачала > ВремяОкончания Тогда
		ВызватьИсключение НСтр("es='La fecha de la terminación del intervalo no puede ser menos de fechas del comienzo.';ru='Дата окончания интервала не может быть меньше даты начала.';en='The end date of the interval cannot be less than the start date.'");
	КонецЕсли;
	...
	Если ВеличинаИнтервалаВДнях > 365 Тогда
		ОписаниеИнтервала = НСтр("es='Más de año';ru='более года';en='more than a year'");
	ИначеЕсли ВеличинаИнтервалаВДнях > 31 Тогда
		ОписаниеИнтервала = НСтр("es='Más de mes';ru='более месяца';en='more than a month'");
...
Функция ПрочитатьXMLВТаблицу(Знач XML) Экспорт
	...
	Если Не Чтение.Прочитать() Тогда
		ВызватьИсключение НСтр("es='Vacío XML';ru='Пустой XML';en='Empty XML'");
	ИначеЕсли Чтение.Имя <> "Items" Тогда
		ВызватьИсключение НСтр("es='La falta en la estructura XML';ru='Ошибка в структуре XML';en='XML structure error'");
	КонецЕсли;
...
Функция СократитьСтрокуКонтрольнойСуммой(Строка, МаксимальнаяДлина) Экспорт
	ОбщегоНазначенияКлиентСервер.Проверить(МаксимальнаяДлина >= 32, НСтр("es='El parámetro de MaksimalnayaDlina no puede ser menos 32';ru='Параметр МаксимальнаяДлина не может быть меньше 32';en='Parameter МаксимальнаяДлина cannot be less than 32'"),
...
Функция ПредметСтрокой(СсылкаНаПредмет) Экспорт
	...
	Если СсылкаНаПредмет = Неопределено Или СсылкаНаПредмет.Пустая() Тогда
		Результат = НСтр("es='No es dado';ru='не задан';en='not set'");
...
Функция ИдентификаторCOMСоединителя(Знач ИмяCOMСоединителя) Экспорт
	...
	ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='En es dado CLSID para la clase %1';ru='На задан CLSID для класса %1';en='On set CLSID for class %1'"), ИмяCOMСоединителя);
...
Функция УстановитьВнешнееСоединениеСБазой(Параметры) Экспорт
	...
	КраткоеОписаниеОшибки = НСтр("es='La conexión directa a la base informativa es inaccesible en el servidor bajo la dirección de la SO Linux.';ru='Прямое подключение к информационной базе недоступно на сервере под управлением ОС Linux.';en='Direct connection to the infobase is not available on a server running Linux.'");
...
Функция МенеджерОбъектаПоПолномуИмени(ПолноеИмя) Экспорт
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El tipo desconocido del objeto metadado ""%1""';ru='Неизвестный тип объекта метаданных ""%1""';en=""Unknown metadata object type '%1'"""), ПолноеИмя);
...
Функция ВидОбъектаПоТипу(ТипОбъекта) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El tipo equivocado del significado de parámetro (%1)';ru='Неверный тип значения параметра (%1)';en='Invalid parameter value type (%1)'"), Строка(ТипОбъекта));
...
Функция СоздатьWSПрокси(ПереданныеПараметры) Экспорт
	...
	Если ДелатьКонтрольныйВызов И Таймаут <> Неопределено И Таймаут > 20 Тогда
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió comprobar la accesibilidad del web-servicio "
"%1 "
"por la causa: "
"%2';ru='Не удалось проверить доступность web-сервиса"
"%1"
"по причине:"
"%2';en='Failed to check web service availability"
"%1"
"due to:"
"%2'"),
...
"%2'"),
			...
			ЗаписьЖурналаРегистрации(НСтр("es='WSПрокси';ru='WSПрокси';en='WSProxy'", КодОсновногоЯзыка()),
...
Функция ПолучитьВерсииИнтерфейса(Знач Адрес, Знач Пользователь, Знач Пароль = Неопределено, Знач Интерфейс = Неопределено) Экспорт
	...
	Если Не ПараметрыПодключения.Свойство("URL") 
		...
		ВызватьИсключение(НСтр("es='del servicio.';ru='Не задан URL сервиса.';en='Service URL not set.'"));
	КонецЕсли;
...
Функция ПолучитьВерсииИнтерфейсаЧерезВнешнееСоединение(ВнешнееСоединение, Знач ИмяИнтерфейса) Экспорт
	...
	Исключение
		СтрокаСообщения = НСтр("es='El corresponsal no apoya версионирование las interfaces de programa. "
"La descripción de la falta: %1';ru='Корреспондент не поддерживает версионирование программных интерфейсов."
"Описание ошибки: %1';en='The correspondent does not support versioning of software interfaces."
"Error description: %1 '");
		...
		ЗаписьЖурналаРегистрации(НСтр("es='La recepción de las versiones de la interfaz';ru='Получение версий интерфейса';en='Retrieving interface versions'", КодОсновногоЯзыка()),
...
Процедура ЗаписатьДанныеВБезопасноеХранилище(Владелец, Данные, Ключ = "Пароль") Экспорт
	...
	ОбщегоНазначенияКлиентСервер.Проверить(ЗначениеЗаполнено(Владелец),
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El significado inadmisible del parámetro %1 en %2. "
"El parámetro debe contener la referencia; es entregado el significado: %3 (tipo %4).';ru='Недопустимое значение параметра %1 в %2."
"параметр должен содержать ссылку; передано значение: %3 (тип %4).';en='Invalid value for parameter%1 in%2."
"the parameter must contain a link; passed value:%3 (type%4).'"),
	...
	ОбщегоНазначенияКлиентСервер.Проверить(ТипЗнч(Ключ) = Тип("Строка"),
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El significado inadmisible del parámetro %1 en %2. "
"El parámetro debe contener la línea; es entregado el significado: %3 (tipo %4).';ru='Недопустимое значение параметра %1 в %2."
"параметр должен содержать строку; передано значение: %3 (тип %4).';en='Invalid value for parameter%1 in%2."
"the parameter must contain a string; passed value:%3 (type%4). '"),
...
Процедура ВыполнитьМетодОбъекта(Знач Объект, Знач ИмяМетода, Знач Параметры = Неопределено) Экспорт
	...
	Попытка
		...
		Если Тест = Неопределено Тогда 
			ВызватьИсключение НСтр("es='El test sintético';ru='Синтетический тест';en='Synthetic test'");
		КонецЕсли;
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El significado incorrecto del parámetro de ImyaMetoda (%1) en ObschegoNaznacheniya. ВыполнитьМетодОбъекта';ru='Некорректное значение параметра ИмяМетода (%1) в ОбщегоНазначения.ВыполнитьМетодОбъекта';en='Invalid value of the ИмяМетода parameter (%1) in the ОбщегоНазначения.ВыполнитьМетодОбъекта'"), ИмяМетода);
	КонецПопытки;
...
"Se Recomienda prohibir la ejecución de las tareas de reglamento para el período de renovación.';ru='Вход в программу временно невозможен в связи с обновлением на новую версию."
"Рекомендуется запрещать выполнение регламентных заданий на время обновления.';en='Logging into the program is temporarily impossible due to the upgrade to the new version."
"It’s recommended that you do not perform scheduled tasks during the upgrade.'");
	КонецЕсли;
	...
	Если Не РазделениеВключено()
	   ...
	   И ЗначениеЗаполнено(Константы.ГлавныйУзел.Получить()) Тогда
		...
		ВызватьИсключение
			НСтр("es='La entrada en el programa es temporalmente imposible antes de la reconstitución del enlace con el nudo principal. "
"Se Recomienda prohibir la ejecución de las tareas de reglamento para el período de reconstitución.';ru='Вход в программу временно невозможен до восстановления связи с главным узлом."
"Рекомендуется запрещать выполнение регламентных заданий на время восстановления.';en='Logging into the program is temporarily impossible until communication with the main node is restored."
"We recommend that you prohibit scheduled tasks during recovery.'");
	КонецЕсли;
	...
	Если РегламентноеЗадание <> Неопределено
		...
		Если Не Доступно Тогда
			...
			КонецЦикла;
			ВызватьИсключение
				НСтр("es='La tarea de reglamento es inaccesible por las opciones funcionales o "
"no apoya el trabajo en el régimen corriente del trabajo del programa. "
"La ejecución es interrumpida. La tarea es desconectada.';ru='Регламентное задание недоступно по функциональным опциям или"
"не поддерживает работу в текущем режиме работы программы."
"Выполнение прервано. Задание отключено.';en='Regulatory task is not available for functional options or"
"does not support work in the current program operation mode."
"Execution aborted. Task disabled. '");
		КонецЕсли;
	КонецЕсли;
...
Функция ПодключитьКомпонентуИзМакета(Идентификатор, ПолноеИмяМакета) Экспорт
	...
	Если Не МакетСуществует(ПолноеИмяМакета) Тогда 
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió conectar el componente exterior ""%1"" en el servidor "
"de %2 "
"por la causa: "
"la Conexión en el servidor no de la maqueta es prohíbida';ru='Не удалось подключить внешнюю компоненту ""%1"" на сервере"
"из %2"
"по причине:"
"Подключение на сервере не из макета запрещено';en='Failed to connect the external component ""%1"" on the server"
"from%2"
"due to:"
"Connection to the server not from the layout is prohibited'"),
			Идентификатор,
			ПолноеИмяМакета);
	КонецЕсли;
	...
	Если ПодключитьВнешнююКомпоненту(Местоположение, СимволическоеИмя) Тогда
		...
		Попытка
			...
			Если ПодключаемыйМодуль = Неопределено Тогда 
				ВызватьИсключение НСтр("es='El operador Nuevo ha devuelto Neopredeleno';ru='Оператор Новый вернул Неопределено';en='Operator new returned undefined'");
			КонецЕсли;
		Исключение
			ПодключаемыйМодуль = Неопределено;
		...
		Если ПодключаемыйМодуль = Неопределено Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió crear el objeto de los componente ""%1"" exteriores conectados en el servidor "
"de la maqueta ""%2"", "
"por la causa: "
"%3';ru='Не удалось создать объект внешней компоненты ""%1"", подключенной на сервере"
"из макета ""%2"","
"по причине:"
"%3';en='Failed to create the external component object ""%1"" connected to the server"
"from layout ""%2"","
"due to:"
"%3 '"),
				Идентификатор,
			...
			ЗаписьЖурналаРегистрации(
				НСтр("es='La conexión de los componente exteriores en el servidor';ru='Подключение внешней компоненты на сервере';en='Connecting an external component on the server'",
					КодОсновногоЯзыка()),
...
"de la maqueta ""%2"" "
"por la causa: "
"el Método De PodklyuchitVneshnyuyuKomponentu ha devuelto la Mentira.';ru='Не удалось подключить внешнюю компоненту ""%1"" на сервере"
"из макета ""%2"""
"по причине:"
"Метод ПодключитьВнешнююКомпоненту вернул Ложь.';en='Failed to connect the external component ""%1"" on the server"
"from layout """"%2 """""
"due to: The Method ПодключитьВнешнююКомпоненту returned False.'"),
			Идентификатор,
			Местоположение);
...
Функция Просклонять(Знач ФИО, Падеж, Результат, Пол = Неопределено) Экспорт
	...
	ВызватьИсключение НСтр("es='Para la llamada de ObschegoNaznacheniya. Declinar es necesario el subsistema «la Declinación de las representaciones de los objetos».';ru='Для вызова ОбщегоНазначения.Просклонять необходима подсистема «Склонение представлений объектов».';en='To call ОбщегоНазначения.Просклонят  ""Declination of representations of objects"" is needed.'");
...
Процедура ЗаблокироватьИБ(Знач ПроверитьОтсутствиеДругихСеансов = Истина) Экспорт
	...
	Иначе
		...
		Иначе
			ВызватьИсключение(НСтр("es='El subsistema ""el Trabajo en el modelo del servicio"" no es accesible';ru='Подсистема ""Работа в модели сервиса"" не доступна';en='Subsystem ""Work in the service model"" is not available'"));
		КонецЕсли;
	КонецЕсли;
...
Функция ОбъектПоПолномуИмени(ПолноеИмя) Экспорт
	...
	Если МассивСтрок.Количество() >= 2 Тогда
		...
		Имя = МассивСтрок[1];
	Иначе
		ВызватьИсключение СтрЗаменить(НСтр("es='El nombre incorrecto completo del informe o el tratamiento ""%1"".';ru='Некорректное полное имя отчета или обработки ""%1"".';en='Invalid fully qualified report or processing name ""%1"".'"), "%1", ПолноеИмя);
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение СтрЗаменить(НСтр("es='""%1"" no es el informe o el tratamiento.';ru='""%1"" не является отчетом или обработкой.';en='% 1 is not a report or processing.'"), "%1", ПолноеИмя);
	КонецЕсли;
КонецФункции
...
Функция НайтиОшибкуДоступностиРеквизитовОбъекта(ПолноеИмяОбъектаМетаданных, ПроверяемыеВыражения)
	...
	Если МетаданныеОбъекта = Неопределено Тогда 
		Возврат Новый Структура("Ошибка, ОписаниеОшибки", Истина, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta de la recepción metadado ""%1""';ru='Ошибка получения метаданных ""%1""';en=""Error retrieving metadata '%1'"""), ПолноеИмяОбъектаМетаданных));
	КонецЕсли;
	...
	Для Каждого ТекущееВыражение Из ПроверяемыеВыражения Цикл
		...
		Если Не ПолеИсточникаСхемыЗапросаДоступно(Источник, ТекущееВыражение) Тогда 
			ТекстОшибки = ТекстОшибки + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='- Paul del objeto ""%1"" no es encontrado';ru='- Поле объекта ""%1"" не найдено';en=""- Object field '% 1' is not found"""), ТекущееВыражение);
		КонецЕсли;
...
Функция РазметитьМестаИспользования(Знач ПараметрыВыполнения, Знач Ссылка, Знач ЦелеваяСсылка, Знач ТаблицаПоиска)
	...
	Для Каждого МестоИспользования Из Результат.МестаИспользования Цикл
		...
		Иначе
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La sustitución de las referencias en ""%1"" no es apoyada.';ru='Замена ссылок в ""%1"" не поддерживается.';en='Replacing links in ""%1"" is not supported.'"), Информация.ПолноеИмя);
...
Процедура ЗаменитьСсылкуОднойТранзакцией(Результат, Знач Дубль, Знач ПараметрыВыполнения, Знач ТаблицаПоиска)
	...
	Исключение
		...
		Если СостояниеОперации = "ОшибкаБлокировки" Тогда
			...
			Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No consiguió bloquear todos los lugares del uso %1:';ru='Не удалось заблокировать все места использования %1:';en='Failed to block all places of use%1:'") 
	...
	Попытка
		Если ВнутренняяТранзакция Тогда
			...
			Попытка
				Блокировка.Заблокировать();
			Исключение
				Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No consiguió bloquear la constante %1';ru='Не удалось заблокировать константу %1';en='Failed to lock constant %1'"), 
					ПредставлениеДанных);
				СостояниеОперации = "ОшибкаБлокировки";
				ВызватьИсключение;
			КонецПопытки;
		...
		Исключение
			...
			Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No consiguió anotar %1 por la causa: %2';ru='Не удалось записать %1 по причине: %2';en='Failed to write%1 due to:%2'"), 
	...
	Попытка
		...
		Если ВнутренняяТранзакция Тогда
			...
			Исключение
				СостояниеОперации = "ОшибкаБлокировки";
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No consiguió bloquear el objeto "
"""%1: %2""';ru='Не удалось заблокировать объект ""%1"":"
"%2';en='Failed to lock object ""%1"":                                      %2'"),
					ПредставлениеДанных,
					КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
				ВызватьИсключение;
			КонецПопытки;
		...
		Исключение
			СостояниеОперации = "ОшибкаЗаписи";
			ОписаниеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No consiguió anotar %1 por la causa: %2';ru='Не удалось записать %1 по причине: %2';en='Failed to write%1 due to:%2'"), 
				ПредставлениеДанных, ОписаниеОшибки);
			ВызватьИсключение;
		КонецПопытки;
	...
	Попытка
		...
		Если ВнутренняяТранзакция Тогда
			...
			Попытка
				Блокировка.Заблокировать();
			Исключение
				Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No consiguió bloquear el juego %1';ru='Не удалось заблокировать набор %1';en='Failed to block dialing%1'"), 
					ПредставлениеДанных);
				СостояниеОперации = "ОшибкаБлокировки";
				ВызватьИсключение;
			КонецПопытки;
	...
	Попытка
		...
		Для Каждого Ссылка Из Удаляемые Цикл
			...
			Попытка
				...
				КонецЕсли;
			Исключение
				ТекстОшибки = НСтр("es='La falta de la desaparición';ru='Ошибка удаления';en='Delete error'")
					+ Символы.ПС
...
Процедура ДобавитьРезультатыЗаменыИзмененныхОбъектов(Результат, ТаблицаПовторногоПоиска)
	...
	Для Каждого Строка Из ТаблицаПовторногоПоиска Цикл
		...
		ЗарегистрироватьОшибкуЗамены(Результат, Ссылка, 
			...
			НСтр("es='Son sustituidos no todos los lugares del uso. Probablemente los lugares del uso eran añadidos o cambiados por otro usuario.';ru='Заменены не все места использования. Возможно места использования были добавлены или изменены другим пользователем.';en='Not all locations have been replaced. Perhaps the places of use have been added or changed by another user.'")));
	КонецЦикла;
...
Процедура ЗаписатьОбъект(Знач Объект, Знач ПараметрыЗаписи)
	...
	Если СвойстваОбъекта.Иерархический Или СвойстваОбъекта.ВидыСубконто <> Неопределено Тогда 
		...
		Если Объект.Родитель = Объект.Ссылка Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='A la anotación ""%1"" surge la referencia cíclica en la jerarquía.';ru='При записи ""%1"" возникает циклическая ссылка в иерархии.';en=""Writing '%1' creates a circular reference in the hierarchy."""),
				Строка(Объект));
			КонецЕсли;
	...
	Если СвойстваОбъекта.Владельцы.Количество() > 1 И Объект.Владелец = Объект.Ссылка Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='A la anotación ""%1"" surge la referencia cíclica en la subordinación.';ru='При записи ""%1"" возникает циклическая ссылка в подчинении.';en='When writing ""%1"", a circular reference appears in the subordinate.'"),
			Строка(Объект));
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииЗаменыСсылок()
	...
	Возврат НСтр("es='La búsqueda y la desaparición de las referencias';ru='Поиск и удаление ссылок';en='Search and delete links'", КодОсновногоЯзыка());
...
Процедура ЗарегистрироватьОшибкуВТаблицу(Результат, Дубль, Оригинал, Данные, Информация, ТипОшибки, ИнформацияОбОшибке)
	...
	Если ТипОшибки = "БлокировкаДляРегистра" Тогда
		НовыйШаблон = НСтр("es='No consiguió comenzar la redacción %1: %2';ru='Не удалось начать редактирование %1: %2';en='Failed to start editing%1:%2'");
		Ошибка.ТипОшибки = "ОшибкаБлокировки";
	ИначеЕсли ТипОшибки = "БлокировкаДляУдаленияДубля" Тогда
		НовыйШаблон = НСтр("es='No consiguió comenzar la desaparición: %2';ru='Не удалось начать удаление: %2';en='Failed to start deleting:%2'");
		Ошибка.ТипОшибки = "ОшибкаБлокировки";
	ИначеЕсли ТипОшибки = "УдалитьНаборДубля" Тогда
		НовыйШаблон = НСтр("es='No consiguió limpiar las noticias sobre la toma en %1: %2';ru='Не удалось очистить сведения о дубле в %1: %2';en='Failed to clear duplicate information in %1:%2'");
		Ошибка.ТипОшибки = "ОшибкаЗаписи";
	ИначеЕсли ТипОшибки = "ЗаписатьНаборОригинала" Тогда
		НовыйШаблон = НСтр("es='No consiguió renovar las noticias en %1: %2';ru='Не удалось обновить сведения в %1: %2';en='Failed to update information in%1:%2'");
		Ошибка.ТипОшибки = "ОшибкаЗаписи";
	Иначе
	...
	НовыйШаблон = НовыйШаблон + Символы.ПС + Символы.ПС + НСтр("es='Los detalles en la revista del registro.';ru='Подробности в журнале регистрации.';en='Details in the logbook.'");
...
Функция ОписаниеИзмеренийНабора(Знач МетаданныеРегистра, КэшИзмеренийРегистров)
	...
	Если Метаданные.РегистрыСведений.Содержит(МетаданныеРегистра) Тогда
		...
		Если МетаПериод = Периодичность.ПозицияРегистратора Тогда
			...
			ДанныеИзмерения.Представление = НСтр("es='El registrador';ru='Регистратор';en='Registrar'");
		...
		ИначеЕсли МетаПериод = Периодичность.Год Тогда
			...
			ДанныеИзмерения.Представление = НСтр("es='El período';ru='Период';en='Period'");
			ДанныеИзмерения.Формат        = НСтр("es=""ДФ ='yyyy '; ДП ='Дата no es dado'"";ru='ДФ=''yyyy ""г.""''; ДП=''Дата не задана''';en='ДФ=''yyyy ""г.""''; ДП=''Дата не задана'''");
			ОписаниеИзмерений.Вставить("Период", ДанныеИзмерения);
			
		ИначеЕсли МетаПериод = Периодичность.День Тогда
			ДанныеИзмерения.Тип           = Новый ОписаниеТипов("Дата");
			ДанныеИзмерения.Представление = НСтр("es='El período';ru='Период';en='Period'");
			ДанныеИзмерения.Формат        = НСтр("es=""ДЛФ=D; ДП ='Дата no es dado'"";ru=""ДЛФ=D; ДП='Дата не задана'"";en=""ДЛФ=D; ДП='Date not set '""");
			ОписаниеИзмерений.Вставить("Период", ДанныеИзмерения);
			
		ИначеЕсли МетаПериод = Периодичность.Квартал Тогда
			ДанныеИзмерения.Тип           = Новый ОписаниеТипов("Дата");
			ДанныеИзмерения.Представление = НСтр("es='El período';ru='Период';en='Period'");
			ДанныеИзмерения.Формат        =  НСтр("es='ДФ =''к """" el trimestre """" yyyy """" """" ''; ДП =''Дата no es dado''';ru='ДФ=''к """"квартал """"yyyy """"г.""""''; ДП=''Дата не задана''';en='ДФ=''к """"квартал """"yyyy """"г.""""''; ДП=''Дата не задана'''");
			ОписаниеИзмерений.Вставить("Период", ДанныеИзмерения);
			
		ИначеЕсли МетаПериод = Периодичность.Месяц Тогда
			ДанныеИзмерения.Тип           = Новый ОписаниеТипов("Дата");
			ДанныеИзмерения.Представление = НСтр("es='El período';ru='Период';en='Period'");
			ДанныеИзмерения.Формат        = НСтр("es='ДФ =''ММММ yyyy """" """" ''; ДП =''Дата no es dado''';ru='ДФ=''ММММ yyyy """"г.""""''; ДП=''Дата не задана''';en='ДФ=''ММММ yyyy """"г.""""''; ДП=''Дата не задана'''");
			ОписаниеИзмерений.Вставить("Период", ДанныеИзмерения);
			
		ИначеЕсли МетаПериод = Периодичность.Секунда Тогда
			ДанныеИзмерения.Тип           = Новый ОписаниеТипов("Дата");
			ДанныеИзмерения.Представление = НСтр("es='El período';ru='Период';en='Period'");
			ДанныеИзмерения.Формат        = НСтр("es=""ДЛФ=DT; ДП ='Дата no es dado'"";ru=""ДЛФ=DT; ДП='Дата не задана'"";en=""ДЛФ=DT; ДП='Date not set '""");
			ОписаниеИзмерений.Вставить("Период", ДанныеИзмерения);
...
Функция СерверныйМодульМенеджера(Имя)
	...
	Если Не ОбъектНайден Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El objeto de los metadatos ""%1"" no es encontrado, "
"o para él no es apoyada la recepción del módulo del manager.';ru='Объект метаданных ""%1"" не найден,"
"либо для него не поддерживается получение модуля менеджера.';en='Metadata object ""%1""not found or the manager module is not supported for it.'"),
			Имя);
	КонецЕсли;
...
Функция СравниваемыеКолонки(Знач КоллекцияСтрок, Знач ИменаКолонок, Знач ИсключаяКолонки)
	...
	Если ПустаяСтрока(ИменаКолонок) Тогда
		...
		Иначе	
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Para la colección del tipo %1 es necesario indicar los nombres de los campos, por que es hecha la comparación en ObschegoNaznacheniya. КоллекцииИдентичны';ru='Для коллекции типа %1 необходимо указать имена полей, по которым производится сравнение в ОбщегоНазначения.КоллекцииИдентичны';en='For a collection of type%1, you must specify the names of the fields used to compare them in ОбщегоНазначения.КоллекцииИдентичны'"),
				ТипКоллекции);
		КонецЕсли;
	Иначе
...
Процедура ПроверкаФиксированностиДанных(Данные, ДанныеВЗначенииФиксированныхТипов = Ложь)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La falta como FiksirovannyeDannye del módulo ObschegoNaznacheniya general. "
"Los datos como ""%1"" no pueden ser fijados.';ru='Ошибка в функции ФиксированныеДанные общего модуля ОбщегоНазначения."
"Данные типа ""%1"" не могут быть зафиксированы.';en='Function error ФиксированныеДанные of general module ОбщегоНазначения. Data type ""%1"" cannot be committed.'"),
		Строка(ТипДанных) );
...
Процедура ПроверитьИмяПроцедурыКонфигурации(Знач ИмяПроцедуры)
	...
	Если ЧастиИмени.Количество() <> 2 И ЧастиИмени.Количество() <> 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El formato incorrecto del parámetro de ImyaProtsedury (es entregado el significado: ""%1"") en ОбщегоНазначения.ВыполнитьМетодКонфигурации';ru='Неправильный формат параметра ИмяПроцедуры (передано значение: ""%1"") в ОбщегоНазначения.ВыполнитьМетодКонфигурации';en='The format of the ИмяПроцедуры parameter is incorrect (value passed: ""%1"") in the ОбщегоНазначения.ВыполнитьМетодКонфигурации'"), ИмяПроцедуры);
	КонецЕсли;
	
	ИмяОбъекта = ЧастиИмени[0];
	Если ЧастиИмени.Количество() = 2 И Метаданные.ОбщиеМодули.Найти(ИмяОбъекта) = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El formato incorrecto del parámetro de ImyaProtsedury (es entregado el significado: ""%1"") en ОбщегоНазначения.ВыполнитьМетодКонфигурации: no es encontrado el módulo general "
"""%2"".';ru='Неправильный формат параметра ИмяПроцедуры (передано значение: ""%1"") в ОбщегоНазначения.ВыполнитьМетодКонфигурации:"
"Не найден общий модуль ""%2"".';en='The format of the ИмяПроцедуры parameter is incorrect (value passed: ""%1"") in the ОбщегоНазначения.ВыполнитьМетодКонфигурации:                   The common module ''%2'' is not found.'"),
			ИмяПроцедуры,
			ИмяОбъекта);
	КонецЕсли;
	...
	Если ЧастиИмени.Количество() = 3 Тогда
		...
		Если Менеджер = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El formato incorrecto del parámetro de ImyaProtsedury (es entregado el significado: ""%1"") en ОбщегоНазначения.ВыполнитьМетодКонфигурации: no es encontrado el manager del objeto "
"""%2"".';ru='Неправильный формат параметра ИмяПроцедуры (передано значение: ""%1"") в ОбщегоНазначения.ВыполнитьМетодКонфигурации:"
"Не найден менеджер объекта ""%2"".';en='The format of the ИмяПроцедуры parameter is incorrect (value passed: ""%1"") in the ОбщегоНазначения.ВыполнитьМетодКонфигурации:                   The manager for object ''%2'' is not found.'"),
				ИмяПроцедуры,
				ПолноеИмяОбъекта);
		КонецЕсли;
	КонецЕсли;
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("es='La ejecución segura del método';ru='Безопасное выполнение метода';en='Safe method execution'", КодОсновногоЯзыка()),
...
Функция МенеджерОбъектаПоИмени(Имя)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No consiguió recibir el manager para el objeto ""%1""';ru='Не удалось получить менеджер для объекта ""%1""';en='Failed to get manager for object ""%1""'"), Имя);
