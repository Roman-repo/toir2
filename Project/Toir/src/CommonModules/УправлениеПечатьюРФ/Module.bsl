...
Функция СтрокаОбязательныхРеквизитов(ДанныеДокумента, ТекстСообщения)
	...
	Для Каждого Элемент Из ОбязательныеРеквизиты Цикл
		Если Не ЗначениеЗаполнено(ДанныеДокумента[Элемент.Ключ]) Тогда
			ТекстСообщения = НСтр("es='el requisito obligatorio: %1';ru='Не заполнен обязательный реквизит: %1';en='Required requisite not completed:%1'");
	...
	Если СтрДлина(ОбязательныеДанные) > 300 Тогда
		Шаблон = НСтр("es='Es imposible crear el QR-código para el documento %1 "
"Línea de los requisitos obligatorios debe ser menos de 300 símbolos: "
"""%2""';ru='Невозможно создать QR-код для документа %1"
"Строка обязательных реквизитов должна быть меньше 300 символов:"
"""%2""';en='Unable to create a QR code for documen t 1"
"Required details must be less than 300 characters:"
"""%2""'");
