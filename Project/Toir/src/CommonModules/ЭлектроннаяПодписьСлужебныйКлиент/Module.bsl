...
Процедура НайтиУстановленныеПрограммыЦиклПродолжение(Менеджер, Контекст) Экспорт
	...
	Если Ошибка.НеУказанПуть Тогда
		ОбновитьЗначение(ОписаниеПрограммы.РезультатПроверкиНаКлиенте, НСтр("es='al programa.';ru='Не указан путь к программе.';en='The path to the program is not specified.'"));
		ОбновитьЗначение(ОписаниеПрограммы.Установлена, "");
	Иначе
		ТекстОшибки = НСтр("es='el ordenador.';ru='Не установлена на компьютере.';en='Not installed on the computer.'") + " " + Ошибка.Описание;
...
Процедура ОткрытьСертификатПродолжение(Контекст)
	...
	ИначеЕсли ТипЗнч(Контекст.ДанныеСертификата) <> Тип("Строка") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta con la llamada del procedimiento de OtkrytSertifikat del módulo ElektronnayaPodpisKlient general: "
"el significado Incorrecto del parámetro de DannyeSertifikata ""%1"".';ru='Ошибка при вызове процедуры ОткрытьСертификат общего модуля ЭлектроннаяПодписьКлиент:"
"Некорректное значение параметра ДанныеСертификата ""%1"".';en='Error calling procedure ОткрытьСертификат of general module ЭлектроннаяПодписьКлиент: Invalid parameter value ДанныеСертификата ""%1"".'"),
...
Процедура СохранитьСертификатПродолжение(Контекст)
	...
	ПараметрыСохранения.Диалог.Заголовок = НСтр("es='Escojan el fichero para la conservación del certificado';ru='Выберите файл для сохранения сертификата';en='Select a file to save the certificate'");
	ПараметрыСохранения.Диалог.Фильтр    = НСтр("es='Escojan el fichero para la conservación del certificado';ru='Выберите файл для сохранения сертификата';en='Select a file to save the certificate'");
...
Процедура СохранитьСертификатПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Иначе
		...
		ПоказатьОповещениеПользователя(НСтр("es='El certificado es conservado en el fichero:';ru='Сертификат сохранен в файл:';en='Certificate saved to file:'"),,
...
Процедура СохранитьЗапросНаСертификат(Оповещение, ЗапросНаСертификат, ИмяФайлаБезРасширения = "") Экспорт
	...
	ПараметрыСохранения = ФайловаяСистемаКлиент.ПараметрыСохраненияФайла();
	ПараметрыСохранения.Диалог.Заголовок = НСтр("es='Escojan el fichero para la conservación de la interpelación por el certificado';ru='Выберите файл для сохранения запроса на сертификат';en='Select file to save certificate request'");
	ПараметрыСохранения.Диалог.Фильтр    = НСтр("es='Escojan el fichero para la conservación de la interpelación por el certificado';ru='Выберите файл для сохранения запроса на сертификат';en='Select file to save certificate request'");
...
Процедура СохранитьЗапросНаСертификатПослеПолученияФайлов(ПолученныеФайлы, Оповещение) Экспорт
	...
	Иначе
		ЕстьПолученныеФайлы = Истина;
		ПоказатьОповещениеПользователя(НСтр("es='La interpelación por el certificado es conservada en el fichero:';ru='Запрос на сертификат сохранен в файл:';en='Certificate request saved to file:'"),,
			ПолученныеФайлы[0].Имя);
	КонецЕсли;
...
Процедура СохранитьПодпись(АдресПодписи) Экспорт
	...
	Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Los ficheros de las firmas electrónicas (*. El %1) | *. El %1|Все los ficheros (*. *) |*.*';ru='Файлы электронных подписей (*.%1)|*.%1|Все файлы (*.*)|*.*';en='Electronic signature files (*.%1) | *.%1 | All Files (*. *) | *. *'"),
		ЭлектроннаяПодписьКлиент.ПерсональныеНастройки().РасширениеДляФайловПодписи);
	
	ПараметрыСохранения = ФайловаяСистемаКлиент.ПараметрыСохраненияФайла();
	ПараметрыСохранения.Диалог.Фильтр = Фильтр;
	ПараметрыСохранения.Диалог.Заголовок = НСтр("es='Escojan el fichero para la conservación de la firma';ru='Выберите файл для сохранения подписи';en='Select a file to save the signature'");
...
Процедура СохранитьПодписьПослеПолученияФайла(ПолученныеФайлы, Контекст) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("es='La firma electrónica es conservada en el fichero:';ru='Электронная подпись сохранена в файл:';en='Electronic signature saved to file:'"),,
		ПолученныеФайлы[0].Имя);
...
Процедура ПолучитьСертификатПоОтпечаткуЗавершение(Сертификат, ПредставлениеОшибки, Контекст)
	...
	Если ЗначениеЗаполнено(ПредставлениеОшибки) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El certificado no es encontrado en el ordenador por la causa: "
"%1';ru='Сертификат не найден на компьютере по причине:"
"%1';en='Certificate not found on the computer due to:"
"%1'"),
			ПредставлениеОшибки);
	Иначе
		ТекстОшибки = НСтр("es='El certificado no es encontrado en el ordenador.';ru='Сертификат не найден на компьютере.';en='Certificate not found on computer.'");
	КонецЕсли;
...
Процедура ПроверитьПодписьНаКлиентеПослеПроверкиПодписи(Сертификат, Контекст) Экспорт
	...
	Если Сертификат = Неопределено Тогда
		ВыполнитьОбработкуОповещения(Контекст.Оповещение,
			НСтр("es='El certificado no es encontrado en los datos de la firma.';ru='Сертификат не найден в данных подписи.';en='Certificate not found in signature data.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьСертификатНаКлиентеПослеОшибкиПроверки(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	Если Контекст.ОписаниеОшибкиНаСервере <> Неопределено Тогда
		ОписаниеОшибки = Контекст.ОписаниеОшибкиНаСервере + " " + НСтр("es='(En el servidor)';ru='(на сервере)';en='(on server)'") + Символы.ПС
			+ ОписаниеОшибки + " " + НСтр("es='(Sobre el cliente)';ru='(на клиенте)';en='(on the client)'");
	КонецЕсли;
...
Процедура СоздатьМенеджерКриптографииПослеПодключенияРасширенияРаботыСКриптографией(Подключено, Контекст) Экспорт
	...
	ЗаголовокФормы = НСтр("es='Es necesario el programa de la firma electrónica y la cifración';ru='Требуется программа электронной подписи и шифрования';en='Electronic signature and encryption program required'");
	Операция       = Контекст.Операция;
	
	Если Операция = "Подписание" Тогда
		ЗаголовокОшибки = НСтр("es='No consiguió firmar los datos de la causa:';ru='Не удалось подписать данные по причине:';en='Failed to sign data due to:'");
		
	ИначеЕсли Операция = "ПроверкаПодписи" Тогда
		ЗаголовокОшибки = НСтр("es='No consiguió comprobar la firma por la causa:';ru='Не удалось проверить подпись по причине:';en='Failed to verify signature due to:'");
		
	ИначеЕсли Операция = "Шифрование" Тогда
		ЗаголовокОшибки = НСтр("es='No consiguió cifrar los datos de la causa:';ru='Не удалось зашифровать данные по причине:';en='Failed to encrypt data due to:'");
		
	ИначеЕсли Операция = "Расшифровка" Тогда
		ЗаголовокОшибки = НСтр("es='No consiguió descifrar los datos de la causa:';ru='Не удалось расшифровать данные по причине:';en='Failed to decrypt data due to:'");
		
	ИначеЕсли Операция = "ПроверкаСертификата" Тогда
		ЗаголовокОшибки = НСтр("es='No consiguió comprobar el certificado por la causa:';ru='Не удалось проверить сертификат по причине:';en='Failed to validate certificate due to:'");
		
	ИначеЕсли Операция = "ПолучениеСертификатов" Тогда
		ЗаголовокОшибки = НСтр("es='No consiguió recibir los certificados por la causa:';ru='Не удалось получить сертификаты по причине:';en='Failed to get certificates due to:'");
	...
	ИначеЕсли Операция <> "" Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta como MenedzherKriptografii. "
"El significado equivocado del parámetro la Operación ""%1"".';ru='Ошибка в функции МенеджерКриптографии."
"Неверное значение параметра Операция ""%1"".';en='Function error МенеджерКриптографии. Invalid value for Operation ""%1"".'"), Операция);
	Иначе
		ЗаголовокОшибки = НСтр("es='No consiguió cumplir la operación de la causa:';ru='Не удалось выполнить операцию по причине:';en='The operation failed due to:'");
	КонецЕсли;
	...
	Если Не Подключено Тогда
		ТекстОшибки =
			НСтр("es='En el navegador es necesario establecer la ampliación "
"para el trabajo con la firma electrónica y la cifración.';ru='В браузере требуется установить расширение"
"для работы с электронной подписью и шифрованием.';en='An extension is required in the browser"
"for work with electronic signature and encryption.'");
...
Процедура СоздатьМенеджерКриптографииПослеЦикла(Контекст)
	...
	Если СвойстваОшибки.Ошибки.Количество() = 0 Тогда
		ТекстОшибки = НСтр("es='el uso ni un programa.';ru='Не предусмотрено использование ни одной программы.';en='Not a single program is foreseen.'");
		СвойстваОшибки.Вставить("Описание", ТекстОшибки);
		...
		Если Не СтандартныеПодсистемыКлиент.ПараметрыРаботыКлиента().ЭтоПолноправныйПользователь Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС + НСтр("es='Se dirijan al administrador.';ru='Обратитесь к администратору.';en='Contact administrator.'");
		КонецЕсли;
	...
	Иначе
		Если Контекст.Программа <> Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El programa ""%1"" no es accesible o no es establecido en el ordenador por la causa: "
"%2';ru='Программа ""%1"" не доступна или не установлена на компьютере по причине:"
"%2';en='Program ""%1"" is not available or is not installed on the computer due to:                                              %2 '"),
				Контекст.Программа,
				СвойстваОшибки.Ошибки[0].Описание);
		Иначе
			ТекстОшибки = НСтр("es='Ni un de los programas no es accesible o no es establecido en el ordenador.';ru='Ни одна из программ не доступна или не установлена на компьютере.';en='None of the programs are available or installed on the computer.'");
			Для Каждого Ошибка Из СвойстваОшибки.Ошибки Цикл
				ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='No consiguió usar el programa ""%1"" por la causa: "
"%2';ru='Программу ""%1"" не удалось использовать по причине:"
"%2';en='Program ""%1"" could not be used due to:"
"%2'"),
						Ошибка.Программа,
						Ошибка.Описание);
			КонецЦикла;
...
Процедура ДобавитьСертификатТолькоДляШифрованияИзФайла(ПараметрыСоздания) Экспорт
	...
	ПараметрыЗагрузки.Диалог.Заголовок = НСтр("es='Escojan el fichero del certificado (solamente para la cifración)';ru='Выберите файл сертификата (только для шифрования)';en='Select a certificate file (for encryption only)'");
	ПараметрыЗагрузки.Диалог.Фильтр = НСтр("es='El certificado X.509 (*.cer; *.crt) | *.cer; *.crt|Все los ficheros (*. *) |*.*';ru='Сертификат X.509 (*.cer;*.crt)|*.cer;*.crt|Все файлы(*.*)|*.*';en='Certificate X.509 (*.cer;*.crt)|*.cer;*.crt|Все файлы(*.*)|*.*'");
...
Процедура ДобавитьСертификатТолькоДляШифрованияИзФайлаПослеПомещенияФайла(Адрес, Контекст)
	...
	Если Форма = Неопределено Тогда
		ПоказатьПредупреждение(,
			НСтр("es='El fichero del certificado debe ser en el formato DER X.509, la operación es interrumpida.';ru='Файл сертификата должен быть в формате DER X.509, операция прервана.';en='Certificate file must be in DER X.509 format, the operation is aborted.'"));
		Возврат;
	КонецЕсли;
	
	Если Не Форма.Открыта() Тогда
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("Открыть", НСтр("es='Abrir';ru='Открыть';en='Open'"));
		...
		ПоказатьВопрос(
			...
			НСтр("es='El certificado es añadido ya.';ru='Сертификат уже добавлен.';en='Certificate has already been added.'"), Кнопки);
	КонецЕсли;
...
Процедура ПоказатьОшибкуОбращенияКПрограмме(ЗаголовокФормы, ЗаголовокОшибки, ОшибкаНаКлиенте, ОшибкаНаСервере,
	...
	Если ТипЗнч(ОшибкаНаКлиенте) <> Тип("Структура") Тогда
		ВызватьИсключение
			НСтр("es='Para el procedimiento De PokazatOshibkuObrascheniyaKProgramme "
"es indicado el tipo incorrecto del parámetro de OshibkaNaKliente.';ru='Для процедуры ПоказатьОшибкуОбращенияКПрограмме"
"указан некорректный тип параметра ОшибкаНаКлиенте.';en='For the procedure ПоказатьОшибкуОбращенияКПрограмме"
"invalid parameter type specified  ОшибкаНаКлиенте.'");
	КонецЕсли;
	
	Если ТипЗнч(ОшибкаНаСервере) <> Тип("Структура") Тогда
		ВызватьИсключение
			НСтр("es='Para el procedimiento De PokazatOshibkuObrascheniyaKProgramme "
"es indicado el tipo incorrecto del parámetro de OshibkaNaServere.';ru='Для процедуры ПоказатьОшибкуОбращенияКПрограмме"
"указан некорректный тип параметра ОшибкаНаСервере.';en='For the procedure ПоказатьОшибкуОбращенияКПрограмме"
"invalid parameter type specified  ОшибкаНаСервере.'");
	КонецЕсли;
...
Процедура НачалоВыбораСертификатаПриУстановленномОтборе(Форма) Экспорт
	...
	Текст = НСтр("es='Los certificados, que pueden ser usados para esta operación son limitados.';ru='Сертификаты, которые могут быть использованы для этой операции ограничены.';en='Certificates that can be used for this operation are limited.'");
	...
	Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
		Заголовок = НСтр("es='Los certificados siguientes permitidos son accesibles para la elección:';ru='Следующие разрешенные сертификаты доступны для выбора:';en='The following authorized certificates are available for selection:'");
	...
	Если ЗначениеЗаполнено(НедоступныеСертификаты) Тогда
		Если ЭлектроннаяПодписьКлиент.СоздаватьЭлектронныеПодписиНаСервере() Тогда
			Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
				Заголовок = НСтр("es='Los certificados siguientes permitidos no son encontrados ni en el ordenador, ni en el servidor:';ru='Следующие разрешенные сертификаты не найдены ни на компьютере, ни на сервере:';en='The following permitted certificates were not found on either the computer or the server:'");
			Иначе
				Заголовок = НСтр("es='Ni un de los certificados siguientes permitidos no es encontrado ni en el ordenador, ni en el servidor:';ru='Ни один из следующих разрешенных сертификатов не найден ни на компьютере, ни на сервере:';en='None of the following permitted certificates were found either on the computer or on the server:'");
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(ДоступныеСертификаты) Тогда
				Заголовок = НСтр("es='Los certificados siguientes permitidos no son encontrados en el ordenador:';ru='Следующие разрешенные сертификаты не найдены на компьютере:';en='The following permitted certificates were not found on the computer:'");
			Иначе
				Заголовок = НСтр("es='Los certificados siguientes permitidos no son encontrados ni en el ordenador, ni en el servidor:';ru='Следующие разрешенные сертификаты не найдены ни на компьютере, ни на сервере:';en='The following permitted certificates were not found on either the computer or the server:'");
			КонецЕсли;
		КонецЕсли;
...
Процедура ДобавитьПодписьИзФайлаПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если Контекст.ПроверитьМенеджерКриптографииНаКлиенте
	   И ТипЗнч(Результат) <> Тип("МенеджерКриптографии") Тогда
		...
		ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("es='Es necesario el programa de la firma electrónica y la cifración';ru='Требуется программа электронной подписи и шифрования';en='Electronic signature and encryption program required'"),
...
Процедура СохранитьДанныеВместеСПодписью(ОписаниеДанных, ОбработкаРезультата = Неопределено) Экспорт
	...
	СерверныеПараметры = Новый Структура;
	СерверныеПараметры.Вставить("ЗаголовокДанных",     НСтр("es='Los datos';ru='Данные';en='Data'"));
...
Процедура СохранитьДанныеВместеСПодписьюПослеСохраненияФайлаДанных(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("es='Al registro del fichero ha surgido la falta:';ru='При записи файла возникла ошибка:';en='An error occurred while writing the file:'") + Символы.ПС + Результат.ОписаниеОшибки);
		
		ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("es='No consiguió conservar las firmas junto con el fichero';ru='Не удалось сохранить подписи вместе с файлом';en='Failed to save captions with file'"), "", Ошибка, Новый Структура);
		Возврат;
...
Процедура СохранитьДанныеВместеСПодписьюПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Текст = НСтр("es='La carpeta con los ficheros:';ru='Папка с файлами:';en='Folder with files:'") + Символы.ПС;
	Текст = Текст + Контекст.ПутьКФайлам;
	Текст = Текст + Символы.ПС + Символы.ПС;
	
	Текст = Текст + НСтр("es='Los ficheros:';ru='Файлы:';en='Files:'") + Символы.ПС;
	...
	Если Контекст.НаСторонеКлиента Тогда
		...
		Если ЭтоXMLDSig Тогда
			Если Контекст.Операция <> "Подписание" Тогда
				Ошибка = Новый Структура("ОписаниеОшибки",
					ЭлектроннаяПодписьСлужебныйКлиентСервер.ЗаголовокОшибкиПолученияДанных(Контекст.Операция)
					+ Символы.ПС + НСтр("es='El componente XMLDSig exterior puede usarse solamente para la firma.';ru='Внешняя компонента XMLDSig может использоваться только для подписания.';en='The external XMLDSig component can only be used for signing.'"));
				ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
				Возврат;
			КонецЕсли;
...
Процедура ВыполнитьНаСторонеЦиклПослеЗаписиПодписи(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("es='A la anotación de la firma ha surgido la falta:';ru='При записи подписи возникла ошибка:';en='An error occurred while writing the signature:'") + Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьНаСторонеЦиклПослеЗаписиЗашифрованныхДанных(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("es='A la anotación de los datos cifrados ha surgido la falta:';ru='При записи зашифрованных данных возникла ошибка:';en='An error occurred while writing encrypted data:'")
			+ Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьНаСторонеЦиклПослеЗаписиРасшифрованныхДанных(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("es='A la anotación de los datos descifrados ha surgido la falta:';ru='При записи расшифрованных данных возникла ошибка:';en='An error occurred while writing decrypted data:'")
			+ Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
	 ...
	 Или ТипЗнч(Результат.Данные) <> Тип("ДвоичныеДанные")
	   ...
	   И (Не ЭтоXMLDSig) Тогда
		...
		Если ТипЗнч(Результат) <> Тип("Структура") Или Не Результат.Свойство("ОписаниеОшибки") Тогда
			Ошибка = Новый Структура("ОписаниеОшибки", НСтр("es='El tipo de datos incorrecto.';ru='Некорректный тип данных.';en='Invalid data type.'"));
		Иначе
...
Процедура ПолучитьДанныеИзОписанияДанныхЗавершение(ПомещенныеФайлы, Контекст) Экспорт
	...
	Если ПомещенныеФайлы = Неопределено Или ПомещенныеФайлы.Количество() = 0 Тогда
		Результат = Новый Структура("ОписаниеОшибки",
			НСтр("es='La transferencia de los datos es anulada por el usuario.';ru='Передача данных отменена пользователем.';en='Data transfer canceled by user.'"));
	Иначе
...
Процедура НачатьПроверкуПодписиПослеИнициализацииСертификата(СертификатКриптографии, Контекст) Экспорт
	...
	ПараметрыПодключения.ТекстПояснения = НСтр("es='Para la firma XML es necesaria la instalación de los componente XMLDSig';ru='Для подписания XML необходима установка компоненты XMLDSig';en='XML signing requires installation of the XMLDSig component'");
	...
	Иначе
		...
		Если ПустаяСтрока(Результат.ОписаниеОшибки) Тогда 
			...
			ЗавершитьОперациюСОшибкой(
				Контекст,
				НСтр("es='La operación es imposible. Es necesaria la instalación de los componente para las HTTP-interpelaciones.';ru='Операция невозможна. Требуется установка компоненты для HTTP-запросов.';en='Operation is not possible. Requires installation of components for HTTP requests.'"));
		...
		Иначе 
			...
			ЗавершитьОперациюСОшибкой(
				Контекст,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La operación es imposible. %1';ru='Операция невозможна. %1';en='Operation is not possible. %one'"), Результат.ОписаниеОшибки));
...
Процедура ПослеПолученияИнформацииМодуляКриптографии(ИнформацияМодуляКриптографии, Контекст) Экспорт
	...
	Если ОписаниеПрограммы = Неопределено Тогда
		ЗавершитьОперациюСОшибкой(Контекст,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió determinar el tipo криптопровайдера %1';ru='Не удалось определить тип криптопровайдера %1';en='Could not determine type of cryptographic provider %1'"),
...
Процедура ПослеУстановкиСвойстваЗапретитьПользовательскийИнтерфейс(Контекст) Экспорт
	...
	Иначе
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("es='XMLDSig.';ru='Не установлен режим работы компоненты XMLDSig.';en='XMLDSig component operating mode not set.'"));
	КонецЕсли;
...
Процедура НачатьПодписаниеСообщенияSOAP(Контекст)
	...
	Исключение
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("es='La falta de la llamada del método GetSignOIDFromCert de los componente XMLDSig.';ru='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.';en='Error calling  GetSignOIDFromCert method of the XMLDSig component.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура Подписание_ПослеВыполненияGetSignOIDFromCert(OIDАлгоритмаПодписи, Параметры, Контекст) Экспорт
	...
	Если OIDАлгоритмаПодписи = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("es='A la ejecución del método GetSignOIDFromCert había una falta:';ru='При выполнении метода GetSignOIDFromCert произошла ошибка:';en='An error occurred while executing method GetSignOIDFromCert:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Контекст.ДанныеАлгоритмаПодписания.OIDВыбранногоАлгоритмаПодписи) Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El algoritmo de la firma del certificado de la criptografía se distingue del algoritmo %1.';ru='Алгоритм подписи сертификата криптографии отличается от алгоритма %1.';en='The cryptography certificate signing algorithm is different from the% 1 algorithm.'"),
	...
	Иначе
		...
		Исключение
			...
			ЗавершитьОперациюСОшибкой(
				Контекст,
				НСтр("es='La falta de la llamada del método C14N de los componente XMLDSig.';ru='Ошибка вызова метода C14N компоненты XMLDSig.';en='Error invoking method C14N of XMLDSig component.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Подписание_ПослеВыполненияC14N_ПодписываемыйТег(КаноникализированныйТекстXML, Параметры, Контекст) Экспорт
	...
	Если КаноникализированныйТекстXML = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("es='A la ejecución del método C14N había una falta:';ru='При выполнении метода C14N произошла ошибка:';en='An error occurred while executing method C14N:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("es='La falta de la llamada del método Hash de los componente XMLDSig.';ru='Ошибка вызова метода Hash компоненты XMLDSig.';en='Error calling the Hash method of the XMLDSig component.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Подписание_ПослеВыполненияHash_ПодписываемыйТег(DigestValue, Параметры, Контекст) Экспорт
	...
	Если DigestValue = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("es='A la ejecución del método Hash había una falta:';ru='При выполнении метода Hash произошла ошибка:';en='An error occurred while executing method Hash:'"), Контекст);
		Возврат;
	КонецЕсли;
...
Процедура Подписание_ПослеВыполненияC14N_SignedInfo(КаноникализированныйТекстXMLSignedInfo, Параметры, Контекст) Экспорт
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("es='La falta de la llamada del método Sign de los componente XMLDSig.';ru='Ошибка вызова метода Sign компоненты XMLDSig.';en='Error calling the Sign method of the XMLDSig component.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Подписание_ПослеВыполненияSign(SignatureValue, Параметры, Контекст) Экспорт
	...
	Если SignatureValue = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("es='A la ejecución del método Sign había una falta:';ru='При выполнении метода Sign произошла ошибка:';en='An error occurred while executing method Sign:'"), Контекст);
		Возврат;
	КонецЕсли;
...
Процедура Проверка_ПослеВыполненияC14N_SignedInfo(КаноникализированныйТекстXMLSignedInfo, Параметры, Контекст) Экспорт
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("es='La falta de la llamada del método VerifySign de los componente XMLDSig.';ru='Ошибка вызова метода VerifySign компоненты XMLDSig.';en='Error calling VerifySign method of XMLDSig component.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Проверка_ПослеВыполненияVerifySign(ПодписьВерна, Параметры, Контекст) Экспорт
	...
	Если ПодписьВерна = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("es='A la ejecución del método VerifySign había una falta:';ru='При выполнении метода VerifySign произошла ошибка:';en='An error occurred while executing method VerifySign:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Если Не ПодписьВерна Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("es='La firma no es justo (SignatureValue es incorrecto.';ru='Подпись не верна (SignatureValue некорректно.';en='Signature is incorrect (SignatureValue is incorrect.'"));
...
Процедура Проверка_ПослеВыполненияHash_ПодписываемыйТег(HashValue, Параметры, Контекст) Экспорт
	...
	Если Не ПодписьВерна Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("es='La firma no es justo (SignatureValue correctamente, se distingue DigestValue).';ru='Подпись не верна (SignatureValue корректно, отличается DigestValue).';en='Signature is incorrect (SignatureValue is correct, DigestValue differs).'"));
...
Процедура НачатьПолучениеТекстаОшибки(НачалоОписанияТекстаОшибки, Контекст)
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("es='La falta de la llamada del método GetLastError de los componente XMLDSig.';ru='Ошибка вызова метода GetLastError компоненты XMLDSig.';en='Error calling GetLastError method of XMLDSig component.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
