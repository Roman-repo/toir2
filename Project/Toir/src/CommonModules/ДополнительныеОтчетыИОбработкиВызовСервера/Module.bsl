...
Функция ПоместитьВХранилище(Ссылка, ИдентификаторФормы) Экспорт
	...
	Если НЕ ДополнительныеОтчетыИОбработки.ВозможнаВыгрузкаОбработкиВФайл(Ссылка) Тогда
		ВызватьИсключение НСтр("es='Es insuficientemente derecho para la descarga de los ficheros de los informes adicionales y los tratamientos';ru='Недостаточно прав для выгрузки файлов дополнительных отчетов и обработок';en='Not enough rights to upload additional report and processing files'");
	КонецЕсли;
...
Функция ЗапуститьДлительнуюОперацию(Знач УникальныйИдентификатор, Знач ПараметрыКоманды) Экспорт
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La ejecución del informe adicional o el tratamiento ""%1"", el nombre de la orden ""%2""';ru='Выполнение дополнительного отчета или обработки ""%1"", имя команды ""%2""';en='Executing additional report or processing ""%1"", command name ""%2""'"),
