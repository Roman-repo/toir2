...
Процедура ОбработкаНавигационнойСсылки(Форма, Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка, Параметры) Экспорт
	...
	ИначеЕсли (НавигационнаяСсылкаЛокальная = "backup1C:OpenWebPersonalAccount_Backups") Тогда
		...
		ИнтернетПоддержкаПользователейКлиент.ОткрытьВебСтраницу(
			...
			НСтр("es='Las copias de reserva del servicio el archivo Nuboso';ru='Резервные копии сервиса Облачный архив';en='Cloud service backups'"));
	...
	ИначеЕсли (НавигационнаяСсылкаЛокальная = "backup1C:OpenWebPersonalAccount_DeactivateAgent") Тогда
		...
		ИнтернетПоддержкаПользователейКлиент.ОткрытьВебСтраницу(
			...
			НСтр("es='Деактивация de los Agentes de la copia de reserva del servicio el archivo Nuboso';ru='Деактивация Агентов резервного копирования сервиса Облачный архив';en='Deactivating cloud backup service agents'"));
	...
	ИначеЕсли (НавигационнаяСсылкаЛокальная = "backup1C:OpenWebPersonalAccount_BuyAgent") Тогда
		...
		ИнтернетПоддержкаПользователейКлиент.ОткрытьВебСтраницу(
			...
			НСтр("es='El gabinete personal del cliente del servicio el archivo Nuboso';ru='Личный кабинет клиента сервиса Облачный архив';en='Personal account of the client service Cloud archive'"));
	...
	ИначеЕсли (НавигационнаяСсылкаЛокальная = "backup1C:TechSupport_BackupsList") Тогда
		...
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("es='El archivo nuboso: la lista de las copias de reserva';ru='Облачный архив: список резервных копий';en='Cloud archive: backup list'"), // Тема
			"", // Тело
			"backup", // Получатель
	...
	ИначеЕсли (НавигационнаяСсылкаЛокальная = "backup1C:TechSupport_ConnectionMaster_Login") Тогда
		...
		МассивВложений = ПодготовитьСписокВложенийДляТехподдержки(НавигационнаяСсылкаЛокальная);
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("es='El archivo nuboso: el maestro las conexiones (login)';ru='Облачный архив: мастер подключения (логин)';en='Cloud archive: connection wizard (login)'"), // Тема
			"", // Сообщение
			"backup", // Получатель
			МассивВложений); // Вложения
	...
	ИначеЕсли (НавигационнаяСсылкаЛокальная = "backup1C:TechSupport_ConnectionMaster_BackupAgentInstall") Тогда
		...
		МассивВложений = ПодготовитьСписокВложенийДляТехподдержки(НавигационнаяСсылкаЛокальная);
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("es='El archivo nuboso: el maestro de la conexión (la instalación del Agente de la copia de reserva)';ru='Облачный архив: мастер подключения (установка Агента резервного копирования)';en='Cloud archive: connection wizard (installing Backup agent)'"), // Тема
			"", // Сообщение
			"backup", // Получатель
			МассивВложений); // Вложения
	...
	ИначеЕсли (НавигационнаяСсылкаЛокальная = "backup1C:TechSupport_ConnectionMaster_BackupAgentInstall_Error") Тогда
		...
		МассивВложений = ПодготовитьСписокВложенийДляТехподдержки(НавигационнаяСсылкаЛокальная);
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("es='El archivo nuboso: el maestro de la conexión (otras faltas)';ru='Облачный архив: мастер подключения (прочие ошибки)';en='Cloud archive: connection wizard (other errors)'"), // Тема
			"", // Сообщение
			"backup", // Получатель
			МассивВложений); // Вложения
	...
	ИначеЕсли (НавигационнаяСсылкаЛокальная = "backup1C:TechSupport_BackupCreate_Error") Тогда
		...
		МассивВложений = ПодготовитьСписокВложенийДляТехподдержки(НавигационнаяСсылкаЛокальная);
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("es='El archivo nuboso: la creación de la copia de reserva';ru='Облачный архив: создание резервной копии';en='Cloud archive: back up'"), // Тема
			"", // Сообщение
			"backup", // Получатель
			МассивВложений); // Вложения
	...
	ИначеЕсли (НавигационнаяСсылкаЛокальная = "backup1C:TechSupport_Settings") Тогда
		...
		МассивВложений = ПодготовитьСписокВложенийДляТехподдержки(НавигационнаяСсылкаЛокальная);
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("es='El archivo nuboso: los ajustes';ru='Облачный архив: настройки';en='Cloud archive: settings'"), // Тема
			"", // Сообщение
			"backup", // Получатель
			МассивВложений); // Вложения
	...
	ИначеЕсли (НавигационнаяСсылкаЛокальная = "backup1C:TechSupport_BackupAgentUpdateMaster_Update") Тогда
		...
		МассивВложений = ПодготовитьСписокВложенийДляТехподдержки(НавигационнаяСсылкаЛокальная);
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("es='El archivo nuboso: el maestro de la renovación del Agente de la copia de reserva (la renovación)';ru='Облачный архив: мастер обновления Агента резервного копирования (обновление)';en='Cloud archive: backup agent upgrade wizard (upgrade)'"), // Тема
			"", // Сообщение
			"backup", // Получатель
			МассивВложений); // Вложения
	...
	ИначеЕсли (НавигационнаяСсылкаЛокальная = "backup1C:TechSupport_BackupAgentUpdateMaster_Update_Error") Тогда
		...
		МассивВложений = ПодготовитьСписокВложенийДляТехподдержки(НавигационнаяСсылкаЛокальная);
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("es='El archivo nuboso: el maestro de la renovación del Agente de la copia de reserva (otras faltas)';ru='Облачный архив: мастер обновления Агента резервного копирования (прочие ошибки)';en='Cloud archive: backup agent upgrade wizard (other errors)'"), // Тема
			"", // Сообщение
			"backup", // Получатель
			МассивВложений); // Вложения
	...
	ИначеЕсли (НавигационнаяСсылкаЛокальная = "backup1C:BackupAgentUpdate_WhatIsNew") Тогда
		...
		Если Параметры.Свойство("ТекстЧтоНовогоВВерсии")
			...
			ПараметрыОткрытия = Новый Структура("Заголовок, ТекстХТМЛ",
				НСтр("es='Que nuevo en las versiones';ru='Что нового в версии';en=""What's new in version"""),
	...
	ИначеЕсли (НавигационнаяСсылкаЛокальная = "backup1C:BackupAgentUpdate_HowToUpdate") Тогда
		...
		Если Параметры.Свойство("ТекстПорядокОбновления")
			...
			ПараметрыОткрытия = Новый Структура("Заголовок, ТекстХТМЛ",
				НСтр("es='El orden de la renovación';ru='Порядок обновления';en='Update order'"),
...
Процедура ПанельАдминистрированияБСП_ПроверитьСостояниеОблачногоАрхива(Форма) Экспорт
	...
	Если Форма.ПараметрыОблачногоАрхива.ТекущийСчетчикПроверкиФоновогоЗадания > 12 Тогда // Прошло более 2 минут - прервать
		...
		ТекстСообщения = НСтр("es='Había unas faltas a la conexión al subsistema el archivo Nuboso: "
"la Recogida de los datos ha superado el tiempo admisible.';ru='Произошли ошибки при подключении к подсистеме Облачный архив:"
"Сбор данных превысил допустимое время.';en='Errors occurred while connecting to the Cloud Archive subsystem:"
"Data collection exceeded time limit. '");
					...
					Если Прогресс.ДополнительныеПараметры.Свойство("КодСостояния") Тогда
						...
						ИначеЕсли Прогресс.ДополнительныеПараметры.КодСостояния = "Завершено с ошибками" Тогда // Идентификатор.
							Форма.Элементы.ДекорацияОблачныйАрхивОшибки.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='Había unas faltas a la conexión al subsistema el archivo Nuboso: "
"%1';ru='Произошли ошибки при подключении к подсистеме Облачный архив:"
"%1';en='Errors occurred while connecting to the Cloud Archive subsystem:"
"%1'"),
	...
	Если ОблачныйАрхивКлиентПовтИсп.РазрешенаРаботаСОблачнымАрхивом() Тогда
		...
		Если ТипЗнч(лкРасписаниеСозданияРезервныхКопий) = ТипСписокЗначений Тогда
			...
			Если СписокНапоминаний.Количество() > 0 Тогда
				...
				Кнопки.Добавить("ВыйтиИзПрограммы", НСтр("es='Salir del programa';ru='Выйти из программы';en='Exit the program'"));
				Кнопки.Добавить("НапомнитьПозже", НСтр("es='Recordar más tarde';ru='Напомнить позже';en='Remind me later'"));
				...
				Если НачалоМинуты(СписокНапоминаний[0].Значение) - НачалоМинуты(ВремяНачала) <= 1*60 Тогда
					КогдаНачнетсяКопирование = НСтр("es='1 minuto';ru='1 минуту';en='1 minute'");
				ИначеЕсли НачалоМинуты(СписокНапоминаний[0].Значение) - НачалоМинуты(ВремяНачала) <= 4*60 Тогда
					КогдаНачнетсяКопирование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='%1 minutos';ru='%1 минуты';en='% 1 minutes'"),
				...
				ИначеЕсли НачалоМинуты(СписокНапоминаний[0].Значение) - НачалоМинуты(ВремяНачала) <= 20*60 Тогда
					КогдаНачнетсяКопирование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='%1 minutos';ru='%1 минут';en='% 1 minutes'"),
						(НачалоМинуты(СписокНапоминаний[0].Значение) - НачалоМинуты(ВремяНачала)) / 60);
				Иначе
					КогдаНачнетсяКопирование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='%1 minas';ru='%1 мин';en='%1 min'"),
						(НачалоМинуты(СписокНапоминаний[0].Значение) - НачалоМинуты(ВремяНачала)) / 60);
				КонецЕсли;
				...
				ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Conforme al horario 1С:Облачного del archivo, la copia de reserva debe ser cumplida a través de %1. "
""
"La ejecución de la copia de reserva tiene que salir del programa.';ru='Согласно расписанию 1С:Облачного архива, резервное копирование должно быть выполнено через %1."
""
"Для выполнения резервного копирования надо выйти из программы.';en='According to the schedule 1C: Cloud Archive, the backup should be done through%1."
""
"To perform a backup, you must exit the program.'"),
				...
				ПоказатьВопрос(
					...
					НСтр("es='La copia de reserva 1С:Облачный el archivo por el horario';ru='Резервное копирование 1С:Облачный архив по расписанию';en='1C Backup: Scheduled Cloud archive'"), // Заголовок.
...
Функция ПодготовитьСписокВложенийДляТехподдержки(НавигационнаяСсылка)
	...
	Если ВложитьАрхивЛоговИспользования = Истина Тогда
		...
		Если СписокФайлов.Количество() > 0 Тогда
			...
			СписокФайловЛоговИспользования = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es encontrado %1 ficheros de las guaridas del trabajo del Agente de la copia de reserva.';ru='Найдено %1 файлов логов работы Агента резервного копирования.';en='Found %1 backup agent operation log files.'"),
				ВсегоФайлов);
			Для Каждого ТекущийФайл Из СписокФайлов Цикл
				Результат.Добавить(Новый Структура("Представление, ИмяФайла",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El resultado del trabajo del Agente de la copia de reserva de %1.txt';ru='Результат работы Агента резервного копирования от %1.txt';en='Backup Agent result from%1.txt'"),
	...
	Если ВложитьАрхивЛоговУстановки = Истина Тогда
		...
		Если СписокФайлов.Количество() > 0 Тогда
			...
			СписокФайловЛоговУстановки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es encontrado %1 ficheros de las guaridas de la instalación.';ru='Найдено %1 файлов логов установки.';en='Found %1 installation log files.'"),
				ВсегоФайлов);
			Для Каждого ТекущийФайл Из СписокФайлов Цикл
				Результат.Добавить(Новый Структура("Представление, ИмяФайла",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El resultado de la instalación del Agente de la copia de reserva de %1.txt';ru='Результат установки Агента резервного копирования от %1.txt';en='Result of installing the Backup agent from %1.txt'"),
						Формат(ТекущийФайл.Значение, "ДФ='yyyyMMdd_HHmm'")),
					ТекущийФайл.Представление));
				ВсегоФайлов = ВсегоФайлов - 1;
				Если ВсегоФайлов <= 0 Тогда
	...
	Если ВложитьРасписаниеАгентаРезервногоКопирования = Истина Тогда
		ТекстРасписания = НСтр("es='El agente de la copia de reserva no era puesto en marcha.';ru='Агент резервного копирования не запускался.';en='Backup agent is not started.'");
				...
				Если НЕ ПустаяСтрока(лкКаталогУстановкиАгентаКопирования) Тогда
					...
					Попытка
						...
						ТекстРасписания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='El resultado de la comprobación del horario: "
"%1 "
"Texto de la falta: "
"%2 "
"Código del regreso: %3';ru='Результат проверки расписания:"
"%1"
"Текст ошибки:"
"%2"
"Код возврата: %3';en='Result of schedule check:"
"%1"
"Error text:"
"%2"
"Return code:%3'"),
					...
					Исключение
						...
						ТекстРасписания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='El resultado de la comprobación los horarios (falta): "
"%1';ru='Результат проверки расписания (ошибка):"
"%1';en='Result of schedule check (error):"
"%1'")
								+ Символы.ПС,
			...
			Иначе
				ТекстРасписания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El agente de la copia de reserva de la versión %1 es establecido en este ordenador (%2), "
"pero el usuario no tiene derechos del administrador Windows para la presentación del horario.';ru='Агент резервного копирования версии %1 установлен на этом компьютере (%2),"
"но у пользователя нет прав администратора Windows для просмотра расписания.';en='Backup agent version% 1 is installed on this computer (% 2),"
"but the user does not have Windows administrator privileges to view the schedule.'"),
					ИнформацияОКлиенте.ВерсияУстановленногоАгентаКопирования,
					ИмяКомпьютера());
			КонецЕсли;
		Иначе
			ТекстРасписания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El agente de la copia de reserva no es establecido en este ordenador (%1).';ru='Агент резервного копирования не установлен на этом компьютере (%1).';en='Backup agent is not installed on this computer (% 1).'"),
				ИмяКомпьютера());
		КонецЕсли;
		Результат.Добавить(
			Новый Структура("Представление, Текст",
				НСтр("es='El horario del Agente de reserva копирования.txt';ru='Расписание Агента резервного копирования.txt';en='Backup agent schedule.txt'"),
				ТекстРасписания));
	КонецЕсли;
		...
		Если ПустаяСтрока(РезультатВыгрузкиЖурналаРегистрации.ТекстОшибки) Тогда
			Результат.Добавить(
				Новый Структура("Представление, Адрес",
					НСтр("es='La revista del registro (los todos acontecimientos).txt';ru='Журнал регистрации (все события).txt';en='Registration log (all events) .txt'"),
					РезультатВыгрузкиЖурналаРегистрации.АдресВременногоХранилищаФайла));
		Иначе
			Результат.Добавить(
				Новый Структура("Представление, Текст",
					НСтр("es='La revista los registros (falta).txt';ru='Журнал регистрации (ошибка).txt';en='Registration log (error) .txt'"),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La falta de la descarga de la revista del registro: "
"%1';ru='Ошибка выгрузки журнала регистрации:"
"%1';en='Error log upload:"
"%1'"),
	...
	ПредставлениеПараметровРаботыОблачногоАрхива = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='----- ParametryOkruzheniyaServer----- "
"%1"
""
"----- InformatsiyaOKliente----- "
"%2"
""
"----- NastroykiAgentaKopirovaniyaKlient----- "
"%3"
""
"----- ParametryAvtorizatsiiIPP----- "
"%4"
""
"----- los Ficheros de las guaridas del trabajo----- "
"%5"
""
"----- los Ficheros de las guaridas de la instalación----- "
"%6"
""
"----- el Texto del horario del Agente de la copia de reserva----- "
"%7';ru='----- ПараметрыОкруженияСервер -----"
"%1"
""
"----- ИнформацияОКлиенте -----"
"%2"
...
"%7""'")
		...
		?(ВложитьАрхивЛоговИспользования = Истина, СписокФайловЛоговИспользования, НСтр("es='No son impuesto';ru='Не вложены';en='Not nested'")), // Файлы логов работы.
		?(ВложитьАрхивЛоговУстановки = Истина, СписокФайловЛоговУстановки, НСтр("es='No son impuesto';ru='Не вложены';en='Not nested'")), // Файлы логов установки.
		?(ВложитьРасписаниеАгентаРезервногоКопирования = Истина, ТекстРасписания, НСтр("es='No es impuesto';ru='Не вложено';en='Not nested'"))); // Текст расписания.

	Результат.Добавить(Новый Структура("Представление, Текст",
		НСтр("es='Los parámetros del trabajo del archivo Nuboso';ru='Параметры работы Облачного архива';en='Cloud archive settings'"),
...
#Область ЗапускПриложения
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='ОблачныйАрхивКлиентСервер. La ejecución de la línea de mando / el Lanzamiento del fichero de mando';ru='ОблачныйАрхивКлиентСервер.Выполнение командной строки / Запуск командного файла';en='ОблачныйАрхивКлиентСервер.Command line execution / Launch the command file'"));
		...
		Исключение
			...
			ПотокВывода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió leer el flujo de la conclusión por la causa: "
"%1';ru='Не удалось прочитать поток вывода по причине:"
"%1';en='Failed to read output stream due to:"
"%1'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		КонецПопытки;
		...
		Исключение
			...
			ПотокОшибок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió leer el flujo de las faltas por la causa: "
"%1';ru='Не удалось прочитать поток ошибок по причине:"
"%1';en='Failed to read error stream due to:"
"%1'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		КонецПопытки;
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El lanzamiento del fichero de mando. "
"La línea de la orden: %1 "
"Código del regreso: %2 "
"Contenido del fichero de mando: "
"%ТекстКомандногоФайла del % "
"el Flujo de la conclusión: "
"%ПотокВывода del % "
"el Flujo de las faltas: "
"%ПотокОшибок del %';ru='Запуск командного файла."
"Строка команды: %1"
"Код возврата: %2"
...
"%ПотокВывода%"
"Поток ошибок:"
"%ПотокОшибок%';en='Launching batch file."
"Command line:%1"
"Return code:%2"
...
#Область УдалениеВременныхФайлов
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='ОблачныйАрхивКлиентСервер. La ejecución de la línea de mando / la Desaparición de los ficheros temporales';ru='ОблачныйАрхивКлиентСервер.Выполнение командной строки / Удаление временных файлов';en='ОблачныйАрхивКлиентСервер.Command line execution / Delete temporary files'"));

		Попытка
			УдалитьФайлы(ИмяКомандногоФайла);
			УдалитьФайлы(ИмяФайлаЛога);
			УдалитьФайлы(ИмяФайлаОшибок);
			ПодробноеСообщение = НСтр("es='La desaparición de los ficheros temporales ha pasado con éxito';ru='Удаление временных файлов прошло успешно';en='Deleting temporary files was successful'");
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
	...
	ИнтернетПоддержкаПользователейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
		...
		0,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La desaparición de los ficheros temporales. "
"El nombre del fichero de mando: %1 "
"Nombre del fichero лога: %2 "
"Nombre del fichero de las faltas: %3 "
"Detalles: "
"%4';ru='Удаление временных файлов."
"Имя командного файла: %1"
"Имя файла лога: %2"
"Имя файла ошибок: %3"
"Подробности:"
"%4';en='Deleting temporary files."
"Command file name:%1"
"Log file name:%2"
"Error file name:%3"
"Details:"
...
#КонецОбласти
	...
	ИнтернетПоддержкаПользователейКлиентСервер.ЗарегистрироватьРезультатВыполненияОперации(
		...
		НСтр("es='ОблачныйАрхивКлиентСервер. La ejecución de la línea de mando / la Desaparición de los ficheros temporales';ru='ОблачныйАрхивКлиентСервер.Выполнение командной строки / Удаление временных файлов';en='ОблачныйАрхивКлиентСервер.Command line execution / Delete temporary files'"),
		0,
		НСтр("es='El lanzamiento al cliente web es prohibido';ru='Запуск в веб-клиенте запрещен';en='Launch the web client is prohibited'"),
		Неопределено);
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:ОблачныйАрхив. El trabajo con las utilidades exteriores';ru='БИП:ОблачныйАрхив.Работа с внешними утилитами';en='БИП:ОблачныйАрхив.Работа с внешними утилитами'"), // ИмяСобытия
			НСтр("es='El archivo nuboso. El trabajo con las utilidades exteriores. El lanzamiento de la orden cualquiera';ru='Облачный архив. Работа с внешними утилитами. Запуск произвольной команды';en='Cloud Archive. Work with external utilities. Running an arbitrary command'"), // ИдентификаторШага
