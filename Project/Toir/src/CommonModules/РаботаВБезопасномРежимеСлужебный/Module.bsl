...
Функция КонтрольныеСуммыФайловКомплектаВнешнейКомпоненты(Знач ИмяМакета) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió formar el permiso para el uso de los componente exteriores: "
"el nombre incorrecto de la maqueta %1.';ru='Не удалось сформировать разрешение на использование внешней компоненты:"
"некорректное имя макета %1.';en='Failed to generate permission to use external component:"
"invalid layout name%1.'"), ИмяМакета);
	КонецЕсли;
	
	Если Макет = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió formar el permiso para el uso de componente exteriores "
"suministrados en la maqueta %1: la maqueta %1 no es descubierta como parte de la configuración.';ru='Не удалось сформировать разрешение на использование внешней компоненты,"
"поставляемой в макете %1: макет %1 не обнаружен в составе конфигурации.';en='Failed to generate permission to use an external component,"
"provided in layout%1: layout%1 was not found in the configuration.'"), ИмяМакета);
	КонецЕсли;
	...
	Если ТипМакета <> Метаданные.СвойстваОбъектов.ТипМакета.ДвоичныеДанные И ТипМакета <> Метаданные.СвойстваОбъектов.ТипМакета.ВнешняяКомпонента Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió formar el permiso para el uso de los componente exteriores: "
"la maqueta %1 no contiene los datos binarios.';ru='Не удалось сформировать разрешение на использование внешней компоненты:"
"макет %1 не содержит двоичных данных.';en='Failed to generate permission to use external component:"
"layout%1 does not contain binary data.'"), ИмяМакета);
	КонецЕсли;
	...
	Если ПустаяСтрока(ФайлМанифеста) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió formar el permiso para el uso de componente exteriores "
"suministrados en la maqueta %1: en el archivo no es descubierto el fichero MANIFEST.XML.';ru='Не удалось сформировать разрешение на использование внешней компоненты,"
"поставляемой в макете %1: в архиве не обнаружен файл MANIFEST.XML.';en='Failed to generate permission to use an external component,"
"supplied in layout%1: the MANIFEST.XML file was not found in the archive. '"), ИмяМакета);
	КонецЕсли;
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("es='El trabajo en el régimen seguro. No consiguió quitar el fichero temporal';ru='Работа в безопасном режиме.Не удалось удалить временный файл';en='Working in safe mode. Failed to delete temporary file'", ОбщегоНазначения.КодОсновногоЯзыка()), 
...
Процедура ЗаполнитьРазрешенияНаЦентрЗащитыОбновлений(ЗапросыРазрешений)
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса("HTTPS", "1cv8update.com",, 
		НСтр("es='El sitio ""el Centro de la defensa de las renovaciones"" (ЦЗО) para la comprobación de la legitimidad del uso y la renovación del producto informático.';ru='Сайт ""Центр защиты обновлений"" (ЦЗО) для проверки правомерности использования и обновления программного продукта.';en='The site ""Update Protection Center"" (TSO) to verify the legitimacy of the use and updating of the software product.'"));
...
Процедура ПриЗаписиСлужебныхДанных(Объект) Экспорт
	...
	Если РаботаВБезопасномРежиме.УстановленБезопасныйРежим() Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La anotación del objeto %1 es inaccesible: es establecido el régimen seguro: %2.';ru='Запись объекта %1 недоступна: установлен безопасный режим: %2.';en='Object %1 writing is unavailable: safe mode is set:%2.'"),
