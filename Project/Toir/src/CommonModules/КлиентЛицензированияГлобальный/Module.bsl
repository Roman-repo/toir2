...
Процедура ПриЗапросеНастроекКлиентаЛицензирования() Экспорт
	...
	Если Не ИнтернетПоддержкаПользователейКлиент.ЗначениеПараметраПриложения("БазоваяФункциональностьБИП\ПравоПодключенияИПП", Ложь) Тогда
		...
		ПоказатьПредупреждение(, НСтр("es='Es insuficientemente derecho para la conexión del Apoyo De Internet de los usuarios. "
"Se dirijan al administrador.';ru='Недостаточно прав для подключения Интернет-поддержки пользователей."
"Обратитесь к администратору.';en='Not enough rights to connect online user support."
