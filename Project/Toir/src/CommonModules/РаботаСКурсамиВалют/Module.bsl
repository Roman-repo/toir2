...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("es='Las cotizaciones han caducado';ru='Курсы валют устарели';en='Currency rates are outdated'");
		Дело.Важное         = Истина;
...
Процедура ОбновитьКурсВалюты(Параметры, АдресРезультата) Экспорт
	...
	ИначеЕсли СпособУстановкиКурса = Перечисления.СпособыУстановкиКурсаВалюты.РасчетПоФормуле Тогда
		...
		Если РезультатЗапроса.Пустой() Тогда
			ТекстОшибки = НСтр("es='En la fórmula debe ser usada por lo menos una divisa básica.';ru='В формуле должна быть использована хотя бы одна основная валюта.';en='At least one primary currency must be used in the formula.'");
...
Процедура ОбновитьСведенияОВалюте937() Экспорт
	...
	Если Не Валюта.Пустая() Тогда
		...
		Валюта.НаименованиеПолное = НСтр("es='El bolívar';ru='Боливар';en='Bolivar'");
