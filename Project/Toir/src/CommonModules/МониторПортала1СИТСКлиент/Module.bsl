...
Процедура ОткрытьМониторСПодключениемИнтернетПоддержки() Экспорт
	...
	ТекстСообщения = НСтр("es='Para el trabajo con el Monitor del Portal 1С:ИТС es necesario conectar el Apoyo De Internet de los usuarios.';ru='Для работы с Монитором Портала 1С:ИТС необходимо подключить Интернет-поддержку пользователей.';en='To work with the Monitor Portal 1C: ITS, you must connect Internet user support.'");
	Если Не ИнтернетПоддержкаПользователейКлиент.ДоступноПодключениеИнтернетПоддержки() Тогда
		ПоказатьПредупреждение(, ТекстСообщения + Символы.ПС + НСтр("es='Se dirijan al administrador.';ru='Обратитесь к администратору.';en='Contact administrator.'"));
		Возврат;
	КонецЕсли;
	
	ОбработчикРезультата = Новый ОписаниеОповещения("ПриОтветеНаВопросПодключитьИнтернетПоддержку", ЭтотОбъект);
	ТекстВопроса = ТекстСообщения + Символы.ПС + НСтр("es='¿Conectar el Apoyo De Internet de los usuarios?';ru='Подключить Интернет-поддержку пользователей?';en='Enable online user support?'");
...
Процедура ПриЗавершенииПроверкиНаличияНовойИнформации(Результат)
	...
	Если Результат = "Ошибка" Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("es='El monitor del Portal 1С:ИТС';ru='Монитор Портала 1С:ИТС';en='Portal Monitor 1C: ITS'"),
			Новый ОписаниеОповещения("ПриНажатииОповещенияОНаличииНовойИнформации", ЭтотОбъект),
			НСтр("es='No consiguió comprobar la presencia de la nueva información. "
"Más detalladamente cm. La revista del registro.';ru='Не удалось проверить наличие новой информации."
"Подробнее см. Журнал регистрации.';en='Failed to check for new information."
"See Logbook for more information. '"),
	...
	ИначеЕсли Результат = "ДанныеИзменены" Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("es='El monitor del Portal 1С:ИТС';ru='Монитор Портала 1С:ИТС';en='Portal Monitor 1C: ITS'"),
			Новый ОписаниеОповещения("ПриНажатииОповещенияОНаличииНовойИнформации", ЭтотОбъект),
			НСтр("es='Una nueva información en el Monitor del Portal 1С:ИТС. "
"Presionen para la presentación.';ru='Новая информация в Мониторе Портала 1С:ИТС."
"Нажмите для просмотра.';en='New information in the Portal Monitor 1C: ITS."
"Click to view. '"),
