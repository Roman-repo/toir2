...
// Параметры:
//		Форма - УправляемаяФорма - форма списка, выбора или подбора объектов ремонта.
//
...
Функция ПолучитьПараметрыИерархии(СтруктураИерархии) Экспорт
	...
	СтруктураПараметровИерархии = Новый Структура("ИзменяетсяДокументами, СтроитсяАвтоматически, ИерархическийСправочник, 
																|РеквизитОР, ТипРеквизитаОР, ТекСтруктураИерархии, РазрешенВводНовыхОР, КонечныеЭлементыВДереве",
																Ложь, Ложь, Ложь, "", "", Неопределено, Ложь, Ложь);
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
//				то считается, что вид сранения Равно.
//
Процедура ОбработатьИзменениеИерархииНаСервере(Форма, Список, Дерево, ПолучатьСтатусыВУчете = Ложь, Знач ОтборСтатусыВУчете = Неопределено, ОтборОбъектРемонта = Неопределено, ДополнительныеОтборы = Неопределено) Экспорт
...
Процедура ЗаполнитьМассивРодителейОбъектаРемонта(ОбъектРемонта, МассивРодителей, СтруктураИерархии) Экспорт
	...
	Если ЭлементРодитель <> "<>" И ТипЗнч(ЭлементРодитель) = Тип("Строка") Тогда
		Возврат;
	КонецЕсли;
...
Функция ПометитьНаУдалениеНаСервере(МассивОР, ЗначениеПометки, СПодчиненными = Ложь) Экспорт
	...
	Для каждого ОбъектРемонта из МассивОРДляОбработки Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("es='No consiguió marcar el objeto a la desaparición por la causa: ';ru='Не удалось пометить объект на удаление по причине: ';en='Failed to mark the object for deletion due to: '");
...
Процедура УдалитьИзИерархииНаСервере(МассивОР, СтруктураИерархии, СПодчиненными = Ложь, ЭтоВложенныйУровень = Ложь) Экспорт
	...
	ШаблонСообщенияНельзяУдалять = НСтр("es='El objeto de la reparación ""%1"" no puede ser quitado de la jerarquía dada puesto que se usa solamente en ella.';ru='Объект ремонта ""%1"" не может быть удалён из данной иерархии так как используется только в ней.';en='Repair object ""%1"" can not be deleted from this hierarchy since it is used only in it.'");
...
Процедура ДобавитьВИерархиюНаСервере(МассивОР, СтруктураИерархии, РодительИерархии, СоответствиеОРСуществующихВТекИеррархии, ИерархияИсточник, РодительИсточник, НеСохранятьИерархию, Отказ) Экспорт
	...
	Если СтруктураИерархии.ИзменяетсяДокументами Тогда
		...
		ДанныеЗаполнения = Новый Структура("СтруктураИерархии, ОбъектИерархии, РодительИерархии, НеСохранятьИерархию", СтруктураИерархии, МассивОР, РодительИерархии, НеСохранятьИерархию);
		...
		Исключение
			ОписаниеОшибки = ОписаниеОшибки();
			ШаблонСообщения = НСтр("es='La adición de los Objetos de la reparación no es cumplida por la causa: %1';ru='Добавление Объектов ремонта не выполнено по причине: %1';en='Adding repair objects failed due to:%1'");
...
Процедура ИзменитьПоложениеВИерархииНаСервере(МассивОР, СтруктураИерархии, РодительИерархии, Отказ) Экспорт
	...
	Если СтруктураИерархии.ИзменяетсяДокументами Тогда
		...
		Если ПеремещениеПриведетКЗацикливанию(МассивОР, СтруктураИерархии, РодительИерархии) Тогда
			ТестСообщения = НСтр("es='El traslado del objeto de la reparación llevará a la ciclación. El traslado es imposible.';ru='Перемещение объекта ремонта приведет к зацикливанию. Перемещение невозможно.';en='Moving the repair object will cause a loop. Moving is not possible.'");
		...
		Исключение
			ОписаниеОшибки = ОписаниеОшибки();
			ШаблонСообщения = НСтр("es='El traslado de los Objetos de la reparación no es cumplido por la causa: %1';ru='Перемещение Объектов ремонта не выполнено по причине: %1';en='Relocation of Repair objects failed due to:%1'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, ОписаниеОшибки);
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,,,,Отказ);
		КонецПопытки;
	...
	Иначе
		...
		Если ПеремещениеПриведетКЗацикливанию(МассивОР, СтруктураИерархии, РодительИерархии) Тогда
			ТекстСообщения = НСтр("es='El traslado del objeto de la reparación llevará a la ciclación. El traslado es imposible.';ru='Перемещение объекта ремонта приведет к зацикливанию. Перемещение невозможно.';en='Moving the repair object will cause a loop. Moving is not possible.'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,,,,Отказ);
			Возврат;
		КонецЕсли;
...
//
Функция ПолучитьСоответсвиеПочиненныхВоВсехИерархиях(МассивОР) Экспорт
...
//
Процедура ВключитьНовыйОбъектРемонтаВИерархию(СсылкаНаОбъект, СтруктураИерархии, РодительИерархии, Отказ, ОбменДаннымиЗагрузка = Ложь) Экспорт
	...
	Если НЕ СтруктураИерархии.СтроитсяАвтоматически Тогда
		...
		Если СтруктураИерархии.ИзменяетсяДокументами Тогда
			...
			ПредыдущееПоложение = НСтр("es='Es aportado en la estructura de la jerarquía';ru='Внесен в структуру иерархии';en='Included in the hierarchy structure'");
			ПараметрыЗаполнения = Новый Структура("ОбъектИерархии, СтруктураИерархии, РодительИерархии, ПредыдущееПоложение", СсылкаНаОбъект, СтруктураИерархии, РодительИерархии, ПредыдущееПоложение);
...
Процедура СнаятьСЭлементовОтметкуГруппыВоВсехИерархиях(МассивОР) Экспорт
...
Функция ДополнитьОтобратьМассивВыбранныхСтрок(МассивДанныхВыбранныхСтрок, ВариантУчетаИерархииЭлементов, СтруктураИерархии, РазрешитьВыборГрупп, ДатаСреза = Неопределено, ОтборСтатусыВУчете = Неопределено, ДополнительныеОтборы = Неопределено) Экспорт
...
// Возвращает таблицу значений с колонками:
//		* ОбъектРемонта - СправочникСсылка.торо_ОбъектыРемонта - объект ремонта.
//		* ЕстьПодчиненные - Булево - есть ли у объекта ремонта подчиненные в выбранной иерархии.
//
Функция ТаблицаНаличияПодчиненныхОР(МассивОР, СтруктураИерархии)
...
Функция ДобавлениеПриведетКЗацикливанию(МассивОР, СтруктураИерархии, РодительИерархии) Экспорт
	...
	ШаблонСообщения = НСтр("es='En la rama añadida hay unos elementos, que se encuentran más arriba por el árbol de la jerarquía: %1. ¡La adición es anulada!';ru='В добавляемой ветке присутствуют элементы, которые встречаются выше по дереву иерархии: %1. Добавление отменено!';en='In the added branch there are elements that are found higher in the hierarchy tree:%1. Upload canceled!'");
...
Процедура ВыполнитьПереносЭлементов(МассивОР, СтруктураИерархии, РодительИерархии = Неопределено, Удаление = Ложь)
	
	Если СтруктураИерархии.ИзменяетсяДокументами Тогда
		
		ДанныеЗаполнения = Новый Структура("СтруктураИерархии, ОбъектИерархии, РодительИерархии, Удаление", СтруктураИерархии, МассивОР, РодительИерархии, Удаление);
		...
		Исключение
			ШаблонСообщения = НСтр("es='El traslado de los Objetos de la reparación no es cumplido por la causa: %1';ru='Перенос Объектов ремонта не выполнен по причине: %1';en='Transfer of Repair Objects failed due to:%1'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, ОписаниеОшибки());
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
		КонецПопытки;
...
Функция ПреобразоватьТаблицуРодителейВИерархииДляПереноса(ИсходнаяТаблицаРодителей, РодительИсточник) Экспорт
	...
	Для Каждого текОР Из ИсходнаяТаблицаРодителей Цикл
		нс = ПреобразованнаяТаблица.Добавить();
		...
		Если КореньВетки <> текОР.Ключ И типЗнч(текОР.Значение) <> Тип("СправочникСсылка.торо_ОбъектыРемонта") Тогда
			...
			Иначе
				...
				Элемент = Справочники.торо_ОбъектыРемонта.НайтиПоНаименованию(Наименование,Истина);
