...
Функция ИдентификаторCOMКлассаВРежимеОбратнойСовместимости(Знач ProgId)
	...
	Если CLSID = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El permiso para el uso de la COM-clase ""%1"" no puede ser concedido al tratamiento adicional "
"que trabaja en el régimen de la compatibilidad de vuelta con el mecanismo de los permisos, realizado en las versiones БСП 2.1.3. "
"El uso de la COM-clase tiene que transformar el tratamiento adicional para el trabajo sin régimen de la compatibilidad de vuelta.';ru='Разрешение на использование COM-класса ""%1"" не может быть предоставлено дополнительной обработке,"
"работающей в режиме обратной совместимости с механизмом разрешений, реализованным в версии БСП 2.1.3."
"Для использования COM-класса требуется переработать дополнительную обработку для работы без режима обратной совместимости.';en='Permission to use the COM class""%1"" can not be granted additional processing,"
"working in backward compatibility mode with the permission mechanism implemented in BSP version 2.1.3."
"To use the COM class, you need to rework additional processing to work without backward compatibility mode. '"),
			ProgId);
