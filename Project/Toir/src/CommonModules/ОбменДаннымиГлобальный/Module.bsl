...
Процедура ПроверитьНеобходимостьОбновления(ТребуетсяОбновлениеКонфигурацииУзлаРИБ)
	...
	Если ТребуетсяОбновлениеКонфигурацииУзлаРИБ Тогда
		...
		Пояснение = НСтр("es='Es recibida la renovación del programa de ""%1"". "
"Es necesario establecer la renovación del programa, después de que la sincronización de los datos será continuada.';ru='Получено обновление программы из ""%1""."
"Необходимо установить обновление программы, после чего синхронизация данных будет продолжена.';en='Received program update from ""%1""."
"You must install the program update, after which the data synchronization will continue. '");
		...
		ПоказатьОповещениеПользователя(НСтр("es='Establecer la renovación';ru='Установить обновление';en='Install update'"), "e1cib/app/Обработка.ВыполнениеОбменаДанными",
