...
Процедура УстановитьРасширенныйЗаголовокПриложения(ПриЗапуске = Ложь) Экспорт
	...
	Иначе
		ШаблонЗаголовка = "%1 / %2";
		ЗаголовокПриложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка, НСтр("es='разделители';ru='Не установлены разделители';en='Separators not set'"), ПараметрыКлиента.ПодробнаяИнформация);
	КонецЕсли;
	...
	Если ПараметрыКлиента.Свойство("РаботаСВнешнимиРесурсамиЗаблокирована")
		...
		ЗаголовокПриложения = НСтр("es='LA [COPIA]';ru='[КОПИЯ]';en='[КОПИЯ]'") + " " + ЗаголовокПриложения;
	КонецЕсли;
...
Процедура ОткрытьСписокАктивныхПользователей(ПараметрыФормы = Неопределено, ВладелецФормы = Неопределено) Экспорт
	...
	Иначе
		...
		ПоказатьПредупреждение(,
			НСтр("es='Para abrir la lista de los usuarios activos, pasen en el menú "
"Todas las funciones - Estandartizado - los usuarios Activos.';ru='Для того чтобы открыть список активных пользователей, перейдите в меню"
"Все функции - Стандартные - Активные пользователи.';en='To open the list of active users, go to the menu"
...
#КонецЕсли
	...
	Если Предупреждения.Количество() = 0 Тогда
		Если Не ПараметрКлиента("ЗапрашиватьПодтверждениеПриЗавершенииПрограммы") Тогда
			Возврат;
		КонецЕсли;
		ТекстПредупреждения = НСтр("es='¿Acabar el trabajo con el programa?';ru='Завершить работу с программой?';en='Complete the work with the program?'");
		Отказ = Истина;
	Иначе
		Отказ = Истина;
...
Процедура ПоказатьПредупреждениеИПродолжить(Параметры, ОписаниеПредупреждения) Экспорт
	...
	Если Параметры.Отказ Тогда
		Кнопки.Добавить("Завершить", НСтр("es='Acabar el trabajo';ru='Завершить работу';en='Complete work'"));
		ПараметрыВопроса.КнопкаПоУмолчанию = "Завершить";
	Иначе
		Кнопки.Добавить("Продолжить", НСтр("en='Resume';es='Continuar';ru='Продолжить'"));
		Кнопки.Добавить("Завершить",  НСтр("en='Resume';es='Continuar';ru='Продолжить'"));
...
Процедура ПоказатьПодробнуюИнформацию(Обработчик, Текст, Заголовок = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(НастройкиДиалога.Заголовок) Тогда
		НастройкиДиалога.Заголовок = НСтр("es='Más detalladamente';ru='Подробнее';en='More details'");
	КонецЕсли;
	
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(0, НСтр("es='Cerrar';ru='Закрыть';en='Close'"));
...
Функция ИнформацияДляПоддержки() Экспорт
	...
	Текст = НСтр("es='El nombre del programa: [НазваниеПрограммы] "
"la Versión del programa: [ВерсияПрограммы]; "
"la Versión de la Plataforma 1С:Предприятие: [ВерсияПлатформы] [РазрядностьПлатформы]; "
"la Versión de la Biblioteca de los subsistemas estandartizados: [ВерсияБСП]; "
"el sistema De operaciones: [ОперационнаяСистема]; "
"la Dimensión de la memoria operativa: [ОперативнаяПамять]; "
"el Nombre COM del conectador: [ИмяCOMСоединителя]; "
"Básico: [ЭтоБазоваяВерсияКонфигурации] "
"el usuario Con todos los derechos: [ЭтоПолноправныйПользователь] "
"De estudios: [ЭтоУчебнаяПлатформа] "
"la Configuración es cambiada: [КонфигурацияИзменена]';ru='Название программы: [НазваниеПрограммы] "
...
"Конфигурация изменена: [КонфигурацияИзменена]';en='Program Name: [НазваниеПрограммы]"
"Program Version: [ВерсияПрограммы];"
"Platform Version 1C: Enterprise: [ВерсияПлатформы] [РазрядностьПлатформы];"
"Library version of standard subsystems: [ВерсияБСП];"
"Operating System: [ОперационнаяСистема];"
"Size of random access memory:[ОперативнаяПамять];"
"Connector COM Name: [ИмяCOMСоединителя];"
"Base: [ЭтоБазоваяВерсияКонфигурации]"
"Full User: [ЭтоПолноправныйПользователь]"
"Training: [ЭтоУчебнаяПлатформа]"
...
Процедура ИнтерактивнаяОбработкаПроверкиВерсииПлатформыПриЗапуске(Параметры, Контекст) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Текущая, Минимальная) < 0 Тогда
		Если ПараметрыКлиента.ЕстьДоступДляОбновленияВерсииПлатформы Тогда
			ТекстСообщения =
				НСтр("es='La entrada en el programa es imposible. "
"Es necesario preliminarmente renovar la versión de la plataforma 1С:Предприятие.';ru='Вход в программу невозможен."
"Необходимо предварительно обновить версию платформы 1С:Предприятие.';en='Entering the program is not possible."
"You must first update the version of the 1C: Enterprise platform.'");
		Иначе
			ТекстСообщения =
				НСтр("es='La entrada en el programa es imposible. "
"Es necesario dirigirse al administrador para la renovación de la versión de la plataforma 1С:Предприятие.';ru='Вход в программу невозможен."
"Необходимо обратиться к администратору для обновления версии платформы 1С:Предприятие.';en='Entering the program is not possible."
"You must contact the administrator to update the version of the 1C: Enterprise platform.'");
		КонецЕсли;
	Иначе
		Если ПараметрыКлиента.ЕстьДоступДляОбновленияВерсииПлатформы Тогда
			ТекстСообщения =
				НСтр("es='Se recomienda acabar el trabajo del programa y renovar la versión de la plataforma 1С:Предприятия. "
"En caso contrario algunas posibilidades del programa serán inaccesibles o trabajarán es incorrecto.';ru='Рекомендуется завершить работу программы и обновить версию платформы 1С:Предприятия."
"В противном случае некоторые возможности программы будут недоступны или будут работать некорректно.';en='It is recommended to terminate the program and update the version of the 1C: Enterprise platform."
"Otherwise, some features of the program will be unavailable or will not work correctly.'");
		Иначе
			ТекстСообщения = 
				НСтр("es='La entrada en el programa es imposible. "
"Es necesario dirigirse al administrador para la renovación de la versión de la plataforma 1С:Предприятие.';ru='Вход в программу невозможен."
...
"Se dirijan al administrador detrás de los detalles.';ru='Вход в программу временно невозможен до восстановления связи с главным узлом."
"Обратитесь к администратору за подробностями.';en='Logging into the program is temporarily impossible until communication with the main node is restored."
...
Процедура ПриВыполненииСтандартныхПериодическихПроверок() Экспорт
	...
	ТекстВопроса =
		НСтр("es='La versión del programa es renovada (son hechos los cambios en la configuración de la base informativa). "
"Para el trabajo ulterior se recomienda перезапустить el programa.';ru='Версия программы обновлена (внесены изменения в конфигурацию информационной базы)."
"Для дальнейшей работы рекомендуется перезапустить программу.';en='The version of the program has been updated (changes have been made to the configuration of the infobase)."
"For further work, it is recommended to restart the program.'");
	ПоказатьОповещениеПользователя(НСтр("es='Es necesario el relanzamiento';ru='Требуется перезапуск';en='Restart required'"),,
		ТекстВопроса, БиблиотекаКартинок.Предупреждение32, СтатусОповещенияПользователя.Важное);
	
	КнопкиВопроса = Новый СписокЗначений;
	КнопкиВопроса.Добавить("Перезапустить", НСтр("es='Перезапустить';ru='Перезапустить';en='Restart'"));
	КнопкиВопроса.Добавить("НапомнитьПозже", НСтр("es='Recordar más tarde';ru='Напомнить позже';en='Remind me later'"));
	КнопкиВопроса.Добавить("НеСегодня", НСтр("es='No recordar hoy';ru='Не напоминать сегодня';en='Do not remind today'"));
...
Процедура ИдентификаторыОбъектовМетаданныхФормаСпискаСписокВыборЗначения(Форма, Элемент, Значение, СтандартнаяОбработка) Экспорт
	...
	Если Не Форма.ВыбиратьГруппыОбъектовМетаданных
	   ...
	   И Не ЗначениеЗаполнено(Элемент.ТекущиеДанные.Родитель) Тогда
		...
		Иначе
			ПоказатьПредупреждение(,
				НСтр("es='Es imposible escoger el grupo de los objetos de los metadatos. "
"Escojan el objeto de los metadatos.';ru='Невозможно выбрать группу объектов метаданных."
"Выберите объект метаданных.';en='Unable to select a group of metadata objects."
...
Процедура УстановитьРазделениеСеанса()
	...
	Если ПараметрыЗапуска.Количество() < 2 Тогда
		ВызватьИсключение
			НСтр("es='A la instrucción del parámetro del lanzamiento de VoytiVOblastDannyh, "
"el parámetro adicional es necesario indicar el significado разделителя.';ru='При указании параметра запуска ВойтиВОбластьДанных,"
"дополнительным параметром необходимо указать значение разделителя.';en='When specifying a start parameterВойтиВОбластьДанных, an additional parameter must specify the value of the separator.'");
	КонецЕсли;
	...
	Исключение
		ВызватьИсключение
			НСтр("es='El significado разделителя en el parámetro de VoytiVOblastDannyh debe ser el número.';ru='Значением разделителя в параметре ВойтиВОбластьДанных должно быть число.';en='The separator value in the parameter ВойтиВОбластьДанных must be a number.'");
	КонецПопытки;
...
Процедура ОткрытьПрикладнуюФормуПредупреждения(Параметры, ОбработкаОтвета, ПредупреждениеПользователю, ИмяФормы, ПараметрыФормы)
	...
	Если ДействиеГиперссылка.Свойство("ПрикладнаяФормаПредупреждения", Форма) Тогда
		...
		Если ДействиеГиперссылка.Свойство("ПараметрыПрикладнойФормыПредупреждения", ПараметрыФормы) Тогда
			...
			ПараметрыФормы.Вставить("ЗаголовокКнопкиДа",  НСтр("es='Acabar';ru='Завершить';en='Complete'"));
			ПараметрыФормы.Вставить("ЗаголовокКнопкиНет", НСтр("en='cancel';es='La anulación';ru='Отмена'"));
...
Процедура ОбработатьОшибкуПриЗапускеИлиЗавершении(Параметры, ИнформацияОбОшибке, Событие, ПрекратитьРаботу = Ложь)
	...
	ТекстПредупреждения = НачалоОписанияОшибки + Символы.ПС
		+ НСтр("es='La información técnica sobre la falta es anotada en la revista del registro.';ru='Техническая информация об ошибке записана в журнал регистрации.';en='Technical error information is written to the log.'")
...
Процедура ОповеститьОНехваткеПамяти() Экспорт
	...
	Заголовок = НСтр("es='La velocidad del trabajo es bajada';ru='Скорость работы снижена';en='Work speed ​​reduced'");
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Se recomienda aumentar la capacidad de almacenaje hasta %1 Gb.';ru='Рекомендуется увеличить объем памяти до %1 Гб.';en='It is recommended to increase the memory capacity to%1 GB.'"), РекомендуемыйОбъем);
