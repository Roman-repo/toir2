...
Процедура ОбновитьВидыКонтактнойИнформацииФизическогоЛица() Экспорт
	...
	ПараметрыВида.Вид = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыКонтактнойИнформации.ТелефонМобильныйФизическогоЛица");
...
Процедура ПодразделенияОрганизацийОбработкаПроверкиЗаполненияОбработкаПроверкиЗаполнения(Источник, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если ЗначениеЗаполнено(Источник.ДатаРасформирования) И ЗначениеЗаполнено(Источник.ДатаСоздания) И Источник.ДатаСоздания > Источник.ДатаРасформирования Тогда
		ТекстСообщения = НСтр("es='¡La fecha de la creación de la subdivisión no puede ser más de fechas de la disolución ""!';ru='Дата создания подразделения не может быть больше даты расформирования""!';en='The date the unit was created cannot be greater than the date of disbandment ""!'",);
	...
	Если (Не Источник.Расформировано) И Источник.ПометкаУдаления Тогда
		ТекстСообщения = НСтр("es='¡Cerca de la subdivisión, marcada a la desaparición, no es posible quitar la bandera ""es disuelto""!';ru='У помеченного на удаление подразделения нельзя снять флаг ""Расформировано""!';en='A flag marked for deletion cannot be unchecked ""Disbanded""!'",);
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Источник.Ссылка, , , Отказ);	
	КонецЕсли;	
...
Процедура ПроверкаИспользованияВШтатномРасписании(Отказ, Ссылка, ДатаРасформирования)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ШаблонСообщения = НСтр("es='Subdivisión ""%1"" no puede ser disuelta, porque en %2 hay unas subdivisiones activas subordinadas:';ru='Подразделение ""%1"" не может быть расформировано, т.к. на %2 есть действующие подчиненные подразделения:';en='Unit ""%1"" cannot be disbanded, as %2 has active subordinate units:'"); 
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Ссылка, Формат(ДатаРасформирования, "ДЛФ=DD")); 
        ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка, "Объект.Расформировано" , , Отказ);
		
		Выборка = РезультатЗапроса.Выбрать();
		Пока Выборка.Следующий() Цикл
			ШаблонСообщения = "- " + НСтр("es='La subdivisión';ru='подразделение';en='subdivision'") + " ""%1""";
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Выборка.НаименованиеПодразделения); 
	...
	Если Не Результат.Пустой() Тогда
		...
		ШаблонСообщения = НСтр("es='Subdivisión ""%1"" no puede ser disuelta, porque en %2 ello se usa en las posiciones actuales de la plantilla:';ru='Подразделение ""%1"" не может быть расформировано, т.к. на %2 оно используется в актуальных позициях штатного расписания:';en='Unit ""%1"" cannot be disbanded, as by%2 it is used in the current positions of the staffing table:'"); 
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Ссылка, Формат(ДатаРасформирования, "ДЛФ=DD")); 
        ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка, "Объект.Расформировано" , , Отказ);
		...
		Пока Выборка.Следующий() Цикл
			ШаблонСообщения = "- " + НСтр("es='La posición';ru='позиция';en='position'") + " ""%1""";
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Выборка.ПозицияШтатногоРасписания); 
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Выборка.Ссылка,  "Объект.Расформировано", , Отказ);
		КонецЦикла;	
