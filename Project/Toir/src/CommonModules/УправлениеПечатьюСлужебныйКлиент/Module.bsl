...
Процедура ВыполнитьПодключаемуюКомандуПечатиЗавершение(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		...
		ИмяПоказателя = НСтр("es='La prensa';ru='Печать';en='Printing'") + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("/%1/%2/%3/%4/%5/%6/%7",
...
Процедура ПроверитьПроведенностьДокументовДиалогПроведения(Параметры) Экспорт
	...
	Если УправлениеПечатьюВызовСервера.ДоступноПравоПроведения(Параметры.НепроведенныеДокументы) Тогда
		Если Параметры.НепроведенныеДокументы.Количество() = 1 Тогда
			ТекстВопроса = НСтр("es='Para desellar el documento, es necesario preliminarmente pasar. ¿Cumplirlo realización del documento y continuar?';ru='Для того чтобы распечатать документ, его необходимо предварительно провести. Выполнить проведение документа и продолжить?';en='In order to print a document, it must first be done. Complete the document and continue?'");
		Иначе
			ТекстВопроса = НСтр("es='Para desellar los documentos, es necesario preliminarmente pasar. ¿Cumplirlos realización de los documentos y continuar?';ru='Для того чтобы распечатать документы, их необходимо предварительно провести. Выполнить проведение документов и продолжить?';en='In order to print documents, they must first be held. Do the paperwork and continue?'");
		КонецЕсли;
	Иначе
		Если Параметры.НепроведенныеДокументы.Количество() = 1 Тогда
			ТекстПредупреждения = НСтр("es='Para desellar el documento, es necesario preliminarmente pasarlo. Es insuficientemente derecho para la realización del documento, la prensa es imposible.';ru='Для того чтобы распечатать документ, его необходимо предварительно провести. Недостаточно прав для проведения документа, печать невозможна.';en='In order to print a document, it must first be done. Not enough rights to hold the document, printing is not possible.'");
		Иначе
			ТекстПредупреждения = НСтр("es='Para desellar los documentos, es necesario preliminarmente pasarlos. Es insuficientemente derecho para la realización de los documentos, la prensa es imposible.';ru='Для того чтобы распечатать документы, их необходимо предварительно провести. Недостаточно прав для проведения документов, печать невозможна.';en='In order to print documents, they must first be held. Not enough rights to hold documents, printing is not possible.'");
		КонецЕсли;
