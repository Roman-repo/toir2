...
Процедура АнализДоступностиФайловКлассификатораВКаталогеЗавершение(ИндексРегиона, ДополнительныеПараметры) Экспорт
	...
	Иначе // окончание цикла
		...
		Для каждого ОтсутствующийФайл Из ДополнительныеПараметры.ОтсутствующиеФайлы Цикл
			...
			СообщениеОбОшибке = НСтр("es='Para la región ""%1"" no es encontrado el fichero dado ""%2""';ru='Для региона ""%1"" не найден файл данных ""%2""';en=""Data file '%2' not found for region '%1'""") + Символы.ПС;
			СообщениеОбОшибке = СообщениеОбОшибке + НСтр("es='Se puede cargar las noticias actuales de domicilios a su destino http://its.1c.ru/download/fias2';ru='Актуальные адресные сведения можно загрузить по адресу http://its.1c.ru/download/fias2';en='Current address information can be downloaded at http://its.1c.ru/download/fias2'");
