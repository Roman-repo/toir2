...
Функция ПредставлениеТипа(ОбъектТип)
	...
	Если ОбъектТип = "DMFile" Тогда
		ПредставлениеТипа = НСтр("es='El fichero';ru='Файл';en='File'");
		
	ИначеЕсли ОбъектТип = "DMInternalDocument" Тогда
		ПредставлениеТипа =  НСтр("es='El documento interior';ru='Внутренний документ';en='Internal document'");
		
	ИначеЕсли ОбъектТип = "DMIncomingDocument" Тогда
		ПредставлениеТипа =  НСтр("es='El documento recibido';ru='Входящий документ';en='Incoming document'");
		
	ИначеЕсли ОбъектТип = "DMOutgoingDocument" Тогда
		ПредставлениеТипа =  НСтр("es='El documento saliente';ru='Исходящий документ';en='Outgoing document'");
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			ИнтеграцияС1СДокументооборот.ИмяСобытияЖурналаРегистрации(
				НСтр("es='No consiguió añadir el enlace';ru='Не удалось добавить связь';en='Failed to add link.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
...
Процедура УдалитьСвязь(ID, Тип, ИнтегрированныйОбъект) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			ИнтеграцияС1СДокументооборот.ИмяСобытияЖурналаРегистрации(
				НСтр("es='No consiguió quitar el enlace';ru='Не удалось удалить связь';en='Failed to unlink'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ПроверитьПодключение(Знач АдресСервиса = Неопределено, Знач ИмяПользователя, Знач Пароль,
	...
	Если Не ЗначениеЗаполнено(АдресСервиса) Тогда
		ТекстСообщенияОбОшибке = НСтр("es='1С:Документооборота.';ru='Не указан адрес веб-сервиса 1С:Документооборота.';en='The address of the 1C: Document management web service is not specified.'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ИмяПользователя)
		...
		ТекстСообщенияОбОшибке = НСтр("es='1С:Документооборота.';ru='Не указано имя пользователя 1С:Документооборота.';en='Not specified username 1C: Document management.'");
		Возврат Ложь;
	КонецЕсли;
		...
		Если Результат.documents.Количество() > 0 Тогда 
			...
			Если ДанныеВозврата.type = "DMInternalDocument" 
				...
				Если Объект.documentType = Неопределено Тогда
					ВызватьИсключение СтрШаблон(НСтр("es='Es imposible recibir los datos 1С:Документооборота: %1. "
"Se dirijan al administrador.';ru='Невозможно получить данные 1С:Документооборота: %1."
"Обратитесь к администратору.';en='Unable to retrieve 1C: Document management data: %1."
...
Функция НайтиСоздатьСвязанныйОбъектДО(ОбъектИС) Экспорт
	...
	Если Результат = Неопределено Тогда
		...
		Если Правила.Количество() = 0 Тогда
			Результат = НСтр("es='Es necesario ajustar "
"las reglas de la integración. Se dirijan al administrador.';ru='Необходимо настроить"
"правила интеграции. Обратитесь к администратору.';en='Required to configure"
"integration rules. Contact administrator. '");
...
Функция ВыполнитьСогласование(Знач Задачи, Знач Результат, Знач Комментарий, ТекстСообщенияОбОшибке) Экспорт
	...
	Для каждого ИдентификаторЗадачи из Задачи Цикл
		...
		Если Результат = "Согласовано" Тогда
			Ответ.name = НСтр("es='Es arreglado';ru='Согласовано';en='Agreed'");
		ИначеЕсли Результат = "СогласованоСЗамечаниями" Тогда
			Ответ.name = НСтр("es='Es arreglado';ru='Согласовано';en='Agreed'");
		ИначеЕсли Результат = "НеСогласовано" Тогда
			Ответ.name = НСтр("es='Es arreglado con las observaciones';ru='Согласовано с замечаниями';en='Agreed with comments'");
		КонецЕсли;
			...
			Иначе
				Если Метаданные.Справочники.Содержит(ОбъектМетаданных) Тогда
					Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El elemento de la guía %1';ru='Элемент справочника %1';en='Directory element %1'"),
...
Функция СтруктураИсполненияЗадачи(Знач ТипЗадачи, Знач ИдентификаторЗадачи, Знач ТипПроцесса, 
	...
	Если ТипПроцесса = "DMBusinessProcessApproval" Тогда
		Если ТочкаМаршрута = "Согласовать" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("es='Es arreglado';ru='Согласовано';en='Agreed'"));
			Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("es='Es arreglado con las observaciones';ru='Согласовано с замечаниями';en='Agreed with comments'"));
			Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи);
			Результат.Вставить("ВыполнитьЗадачуТретья", НСтр("es='Es arreglado con las observaciones';ru='Согласовано с замечаниями';en='Agreed with comments'"));
		ИначеЕсли ТочкаМаршрута = "Ознакомиться" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("es='No es arreglado';ru='Не согласовано';en='Not agreed'"));
			Если РезультатВыполнения = "Не согласовано" Тогда
				Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("es='Repetir la coordinación...';ru='Повторить согласование...';en='Repeat reconciliation ...'"));
	...
	ИначеЕсли ТипПроцесса = "DMBusinessProcessConfirmation" Тогда
		Если ТочкаМаршрута = "Утвердить" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("es='Es afirmado';ru='Утверждено';en='Approved by'"));
			Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("es='No es afirmado';ru='Не утверждено';en='Not approved'"));
			Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи);
		ИначеЕсли ТочкаМаршрута = "Ознакомиться" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("es='Se ha puesto al corriente';ru='Ознакомился';en='Got acquainted'"));
			Если РезультатВыполнения = "Не утверждено" Тогда
				Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("es='Repetir la afirmación...';ru='Повторить утверждение...';en='Repeat statement ...'"));
				Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи);
			КонецЕсли;
		КонецЕсли;
	// Рассмотрение.
	ИначеЕсли ТипПроцесса = "DMBusinessProcessConsideration" Тогда
		Если ТочкаМаршрута = "Рассмотреть" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("es='Es examinado';ru='Рассмотрено';en='Reviewed'"));
		ИначеЕсли ТочкаМаршрута = "Ознакомиться" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("es='Es examinado';ru='Рассмотрено';en='Reviewed'"));
		КонецЕсли;
	// Поручение.
	ИначеЕсли ТипПроцесса = "DMBusinessProcessOrder" Тогда
		Если ТочкаМаршрута = "Выполнить" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("es='Es cumplido';ru='Выполнено';en='Done'"));
		ИначеЕсли ТочкаМаршрута = "Проверить" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("es='Es cumplido';ru='Выполнено';en='Done'"));
			Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("es='Devolver a la elaboración definitiva';ru='Вернуть на доработку';en='Return for revision'"));
			Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи);
		ИначеЕсли ТочкаМаршрута = "Контролировать" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("es='Quitar del control';ru='Снять с контроля';en='Take out of control'"));
		КонецЕсли;
	// Исполнение.
	ИначеЕсли ТипПроцесса = "DMBusinessProcessPerformance" Тогда
		Если ТочкаМаршрута = "Исполнить" или ТочкаМаршрута = "ОтветственноеИсполнение" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("es='Es cumplido';ru='Исполнено';en='Executed'"));
		ИначеЕсли ТочкаМаршрута = "Контролировать" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("es='Es cumplido';ru='Исполнено';en='Executed'"));
		ИначеЕсли ТочкаМаршрута = "Проверить" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("es='Quitar del control';ru='Снять с контроля';en='Take out of control'"));
	...
	ИначеЕсли ТипПроцесса = "DMComplexBusinessProcess" Тогда
		Если ТочкаМаршрута = "Контролировать" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("es='Es controlado';ru='Проконтролировано';en='Controlled by'"));
		КонецЕсли;
	// Регистрация.
	ИначеЕсли ТипПроцесса = "DMBusinessProcessRegistration" Тогда
		Если ТочкаМаршрута = "Зарегистрировать" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("es='Es registrado';ru='Зарегистрировано';en='Registered'"));
		ИначеЕсли ТочкаМаршрута = "Ознакомиться" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("es='Es registrado';ru='Зарегистрировано';en='Registered'"));
		КонецЕсли;
	// Ознакомление.
	...
	ИначеЕсли ТипПроцесса = "DMBusinessProcessIssuesSolution" Тогда
		Если ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.2.7.3") Тогда
			...
			ИначеЕсли ТочкаМаршрута = "ОзнакомлениеСРезультатомРассмотрения" Тогда
				Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("es='Es examinado';ru='Рассмотрено';en='Reviewed'"));
				Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("es='Precisar';ru='Уточнить';en='Specify'"));
				Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи);
			КонецЕсли;
		КонецЕсли;
	// Приглашение.
	ИначеЕсли ТипПроцесса = "DMBusinessProcessInvitation" Тогда
		Если ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.2.7.3") Тогда
			Если ТочкаМаршрута = "Пригласить" Тогда
				Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("es='Aceptar la invitación';ru='Принять приглашение';en='Accept an invitation'"));
				Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("es='Declinar la invitación';ru='Отклонить приглашение';en='Reject invitation'"));
				Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи);
			ИначеЕсли ТочкаМаршрута = "Ознакомиться" Тогда
				...
				Если РезультатID = "ПринятоВсемиУчастниками" Тогда 
					Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("es='Confirmar las invitaciones';ru='Подтвердить приглашения';en='Confirm invitations'"));
				ИначеЕсли РезультатID = "ПринятоОбязательнымиУчастниками" Тогда 	
					Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("es='Confirmar las invitaciones';ru='Подтвердить приглашения';en='Confirm invitations'"));
					Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("es='Repetir la invitación...';ru='Повторить приглашение...';en='Repeat invitation ...'"));
					Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи);
				ИначеЕсли РезультатID = "НеПринятоОбязательнымиУчастниками"
					Или РезультатID = "НеПринятоВсемиУчастниками" Тогда 
					Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("es='Confirmar las invitaciones';ru='Подтвердить приглашения';en='Confirm invitations'"));
					Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("es='Repetir la invitación...';ru='Повторить приглашение...';en='Repeat invitation ...'"));
					Результат.Вставить("ВыполнитьЗадачуТретья", НСтр("es='Confirmar las invitaciones';ru='Подтвердить приглашения';en='Confirm invitations'"));
					Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи);
				КонецЕсли;
...
Процедура ОбновитьПредставлениеРезолюций(РезолюцииXDTO, ПредставлениеHTML, ЭлементФормы) Экспорт
	...
	Если РезолюцииXDTO.Количество() = 0 Тогда
		ПредставлениеHTML = "";
		ЭлементФормы.Заголовок = НСтр("es='Las resoluciones';ru='Резолюции';en='Resolutions'");
		Возврат;
	КонецЕсли;
	...
	ЭлементФормы.Заголовок = СтрШаблон(
		НСтр("es='Las resoluciones (%1)';ru='Резолюции (%1)';en='Resolutions (%1)'"),
...
Процедура ОбновитьВизыСогласования(ВизыXDTO, ВизыСогласования, ЭлементФормы) Экспорт
	...
	Если ВизыXDTO.Количество() = 0 Тогда
		ЭлементФормы.Заголовок = НСтр("es='Los visados';ru='Визы';en='Visas'");
		Возврат;
	КонецЕсли;
	...
	ЭлементФормы.Заголовок = СтрШаблон(
		НСтр("es='Los visados (%1)';ru='Визы (%1)';en='Visas (% 1)'"),
		ВизыXDTO.Количество());
...
Процедура ОбновитьСписокФайлов(ФайлыXDTO, Файлы, ЭлементДляОбновленияЗаголовка = Неопределено) Экспорт
	...
	Если ЭлементДляОбновленияЗаголовка <> Неопределено Тогда
		Если Файлы.Количество() = 0 Тогда 
			ФайлыЗаголовок = НСтр("es='Los ficheros';ru='Файлы';en='Files'");
		Иначе
			ФайлыЗаголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Los ficheros (%1)';ru='Файлы (%1)';en='Files (%1)'"),
				Файлы.Количество());
		КонецЕсли;
...
Функция ПолучитьФайлИПоместитьВХранилище(ID, УникальныйИдентификатор, ДатаЗаема = Неопределено,
	...
	Иначе // получение с захватом
		Если Не ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.4.9.1") Тогда
			ВызватьИсключение НСтр("es='El empréstito de los ficheros no es apoyado por la versión usada 1С:Документооборота';ru='Заем файлов не поддерживается используемой версией 1С:Документооборота';en='File lending is not supported by the used version 1C: Document management'");
		КонецЕсли;
...
Функция СделатьВерсиюАктивной(ИдентификаторФайла, ИдентификаторВерсии, ТекстСообщения = "") Экспорт
	...
	Если Не ДоступенФункционалВерсииСервиса("1.4.6.1") Тогда
		ТекстСообщения = 
			НСтр("es='Para el trabajo con las versiones es necesario 1С:Документооборот a la versión no más abajo 1.4.6.1';ru='Для работы с версиями нужен 1С:Документооборот версии не ниже 1.4.6.1';en='To work with versions you need 1C: Document flow of version no lower than 1.4.6.1'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если Размер > МаксРазмерФайла Тогда
		ВызватьИсключение
			   СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				 НСтр("es='La dimensión del fichero ""%1"" (%2 Mb) supera la dimensión máximamente admisible del fichero para la transmisión (%3 Mb).';ru='Размер файла ""%1"" (%2 Мб) превышает максимально допустимый размер файла для передачи (%3 Мб).';en='File size ""%1"" (%2 Mb) exceeds the maximum permissible file size for transfer (%3 Mb).'"),
...
Процедура ЗаполнитьПоляФайлаДаннымиВладельца(ID) Экспорт
	...
	Если Не ДоступенФункционалВерсииСервиса("2.0.15.1") Тогда
		ТекстСообщения = 
			НСтр("es='Para el relleno de los ficheros es necesario 1С:Документооборот a la versión no más abajo 2.0.15.1';ru='Для заполнения файлов нужен 1С:Документооборот версии не ниже 2.0.15.1';en='To fill in the files you need 1C: Document management version no lower than 2.0.15.1'");
...
Процедура ЗаполнитьСписокПодписейСервер(ПодписиXDTO, ФайлыXDTO, Форма, 
	...
	Если ЭлементДляИзмененияЗаголовка <> Неопределено Тогда
		ТекстЗаголовка = НСтр("es='ЭП';ru='ЭП';en='ES'");
...
Процедура ПроверитьОднуПодписьНаСервере(ДанныеСтроки, МенеджерКриптографии, УникальныйИдентификатор, 
	...
	Попытка
		...
		ДанныеСтроки.Статус = ?(ДанныеСтроки.Неверна,
			ДанныеСтроки.Статус = НСтр("es='No es justo';ru='Не верна';en='Incorrect'") + ": " + ОписаниеОшибки,
			НСтр("es='Es justa';ru='Верна';en='Verna'"));
	Исключение
		ДанныеСтроки.Статус = НСтр("es='No es justo: no es indicado el manager de la criptografía';ru='Не верна: не указан менеджер криптографии';en='Not true: cryptography manager not specified'");
