	...
	Если ПроверятьПараметрыАдминистрированияКластера Или ПроверятьПараметрыАдминистрированияИнформационнойБазы Тогда
		...
		Исключение
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='%1 "
""
"Si en el ordenador %2 no es puesto en marcha el servidor de la administración (ras), "
"debe ponerlo en marcha. "
""
"Por ejemplo: "
"""%3"" cluster - port = del %4 %5: el %6 "
""
"debe comprobar En caso contrario los parámetros de la conexión y la pantalla de red.';ru='%1"
""
...
""
"В противном случае следует проверить параметры подключения и сетевого экрана.';en='%1"
""
"If administration server (ras) is not running on computer %2,"
"should run it."
""
"For example:"
"""%3 "" cluster --port =%4%5:%6"
""
"Otherwise, check the connection and firewall settings.'"),
...
Процедура УдалитьСеансыИнформационнойБазы(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ, Знач Фильтр = Неопределено) Экспорт
	...
	Если Не ВсеСеансыУдалены Тогда
		...
		ВызватьИсключение НСтр("es='No consiguió quitar las sesiones.';ru='Не удалось удалить сеансы.';en='Failed to delete sessions.'");
...
Процедура РазорватьСоединенияСИнформационнойБазой(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ, Знач Фильтр = Неопределено) Экспорт
	...
	Если Не ВсеСоединенияРазорваны Тогда
		...
		ВызватьИсключение НСтр("es='No consiguió romper las uniones.';ru='Не удалось разорвать соединения.';en='Failed to disconnect.'");
...
Функция ПрофильБезопасности(Знач ПараметрыАдминистрированияКластера, Знач ИмяПрофиля, Знач ИдентификаторКластера = Неопределено) Экспорт
	...
	Если ПрофилиБезопасности.Количество() <> 1 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='En el clúster de los servidores %1 no es registrado el perfil de la seguridad %2';ru='В кластере серверов %1 не зарегистрирован профиль безопасности %2';en='Security cluster% 2 not registered in server cluster% 1'"), ИдентификаторКластера, ИмяПрофиля);
	КонецЕсли;
...
Процедура СоздатьПрофильБезопасности(Знач ПараметрыАдминистрированияКластера, Знач СвойстваПрофиляБезопасности) Экспорт
	...
	Если ПрофилиБезопасности.Количество() = 1 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='En el clúster de los servidores %1 es registrado ya el perfil de la seguridad %2';ru='В кластере серверов %1 уже зарегистрирован профиль безопасности %2';en='Security cluster% 2 already registered on server cluster% 1'"), ИдентификаторКластера, ИмяПрофиля);
	КонецЕсли;
...
Функция ИдентификаторИнформационнойБазы(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИнформационнойБазы) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='En el clúster de los servidores %1 no es registrada la base informativa %2';ru='В кластере серверов %1 не зарегистрирована информационная база %2';en='Infobase% 2 not registered in server cluster% 1'"), ИдентификаторКластера, ПараметрыАдминистрированияИнформационнойБазы.ИмяВКластере);
	КонецЕсли;
...
Функция ИдентификаторКластера(Знач ПараметрыАдминистрированияКластера) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1';ru='Не обнаружен кластер серверов с портом %1';en='Server cluster with port%1 not found'"), ПараметрыАдминистрированияКластера.ПортКластера);
	КонецЕсли;
...
Функция ПривестиЗначение(Знач Значение)
	...
	ИначеЕсли ТипЗнч(Значение) = Тип("Строка") Тогда
		...
		Кириллица = "АБВГДЕЖЗИЙКЛМНОПРСТУФХЦЧШЩЫЪЬЭЮЯабвгдежзийклмнопрстуфхцчшщыъьэюя"; // АПК:1036 алфовит не проверять орфографически.
...
Функция ПривестиЭлементВывода(ЭлементВывода, ТипЭлемента)
	...
	ИначеЕсли ТипЭлемента = Тип("Число") Тогда
		...
		Исключение
			ВызватьИсключение НСтр("es='El formato incorrecto.';ru='Неправильный формат.';en='Invalid format.'");
		КонецПопытки;
	...
	Иначе
		...
		ВызватьИсключение НСтр("es='El tipo equivocado del elemento.';ru='Неверный тип элемента.';en='Invalid item type.'");
...
Функция ЗапуститьКоманду(Команда, ПараметрыАдминистрированияКластера, Словарь = Неопределено, Фильтр = Неопределено, ТипыСвойств = Неопределено)
	...
	Если БезопасныйРежим() <> Ложь Тогда
		ВызватьИсключение НСтр("es='La administración del clúster es imposible en el régimen seguro';ru='Администрирование кластера невозможно в безопасном режиме';en='Cluster administration is not possible in safe mode'");
	КонецЕсли;
	
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ВызватьИсключение НСтр("es='En el modelo del servicio es inadmisible la ejecución por la base aplicada informativa de las funciones de la administración del clúster';ru='В модели сервиса недопустимо выполнение прикладной информационной базой функций администрирования кластера';en='In the service model, it is unacceptable that the application information base performs cluster administration functions'");
	КонецЕсли;
	...
	Если Не ФайлКлиента.Существует() Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es imposible cumplir la operación de la administración del clúster de los servidores de la causa: el fichero %1 no es encontrado Para la administración del clúster a través del servidor "
""
"de la administración (ras) es necesario establecer en "
"el ordenador dado el cliente del servidor de la administración (rac). "
"Para su instalación: "
"- los ordenadores con la SO Windows son necesarios переустановить la plataforma, habiendo establecido el componente ""el Servidor 1С:Предприятия""; "
"- los ordenadores de la SO Linux tienen que establecer el paquete 1c-enterprise83-server*.';ru='Невозможно выполнить операцию администрирования кластера серверов по причине: файл %1 не найден"
""
...
"- Для компьютеров с ОС Linux требуется установить пакет 1c-enterprise83-server*.';en='Unable to perform server cluster administration operation due to: file%1 not found"
""
"To administer a cluster through an administration server (ras), you need to install on this"
"Administration Server (rac) client computer"
"To install it:"
"- For computers with Windows OS, you need to reinstall the platform by installing the component """" Server 1C: Enterprise """";"
"- For computers running Linux, you need to install the 1c-enterprise83-server * package. '"),
...
Функция ПоддерживаемыеСвойстваПрофиля(ПараметрыАдминистрированияКластера)
	...
	ОписаниеПрофиля = НСтр("es='El perfil de servicio para la comprobación de las propiedades apoyadas.';ru='Служебный профиль для проверки поддерживаемых свойств.';en='A service profile for checking supported properties.'");
