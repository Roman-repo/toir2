	...
	Если Не ЗначениеЗаполнено(ИмяПользователя) Тогда // прочтем настройки из пользовательского хранилища.
		...
		Если Не ЗначениеЗаполнено(ИмяПользователя) Тогда
			...
			ЗаписьЖурналаРегистрации(
				...
				НСтр("es='Para el usuario de la tarea de reglamento del cambio no es indicado el nombre del usuario 1С:Документооборота';ru='Для пользователя регламентного задания обмена не указано имя пользователя 1С:Документооборота';en='For the user of the scheduled task of exchange, the user name 1C: Workflow is not specified'",
		...
		Если ПарольСохранен <> Истина Тогда
			...
			ЗаписьЖурналаРегистрации(
				ИнтеграцияС1СДокументооборот.ИмяСобытияЖурналаРегистрации(), 
				УровеньЖурналаРегистрации.Ошибка,,,
				НСтр("es='Para el usuario de la tarea de reglamento del cambio no es conservada la consigna 1С:Документооборота';ru='Для пользователя регламентного задания обмена не сохранен пароль 1С:Документооборота';en='The password of 1C: Document flow was not saved for the user of the scheduled task of exchange'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
...
Процедура ПодготовитьДанныеДляОтправки()
	...
	Для Каждого ИнтегрированныйОбъект Из ИнтегрированныеОбъекты Цикл
		...
		Исключение
			Инфо = ОписаниеОшибки();
			ЗаписьЖурналаРегистрации(
				ИнтеграцияС1СДокументооборот.ИмяСобытияЖурналаРегистрации(НСтр("es='La descarga XDTO en XML';ru='Выгрузка XDTO в XML';en='XDTO upload to XML'",
					ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
				УровеньЖурналаРегистрации.Ошибка,
				,
				ОбъектXDTO.Тип().Имя,
...
Процедура ОтправитьДанные()
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("es='La integración con 1С:Документооборотом. El envío de los datos';ru='Интеграция с 1С:Документооборотом.Отправка данных';en='Integration with 1C: Document management. Data sending'",
				ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
					...
					Если ЕстьИзменения Тогда
						...
						Иначе
							...
							Иначе
								...
								ЗаписьЖурналаРегистрации(
									ИнтеграцияС1СДокументооборот.ИмяСобытияЖурналаРегистрации(
										НСтр("es='La recepción de los datos';ru='Получение данных';en='Receiving data'",
											ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())),
									УровеньЖурналаРегистрации.Ошибка,
									Объект.Метаданные(),
									ОбъектСсылка,
