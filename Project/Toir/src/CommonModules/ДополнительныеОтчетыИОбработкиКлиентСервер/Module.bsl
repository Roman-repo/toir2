...
//       //         Функция возвращает этап сценария, который затем можно использовать в функции по заполнению параметров.
...
Функция ФильтрДиалоговВыбораИСохранения() Экспорт
	...
	Фильтр = НСтр("es='Los informes exteriores y los tratamientos (*. El %1, *. El %2) | *. El %1; *. El %2|Внешние los informes (*. El %1) | *. El %1|Внешние los tratamientos (*. El %2) | *. El %2';ru='Внешние отчеты и обработки (*.%1, *.%2)|*.%1;*.%2|Внешние отчеты (*.%1)|*.%1|Внешние обработки (*.%2)|*.%2';en='External reports and processing (*.%1, *.%2) | *.%1; *.%2 | External reports (*.%1) | *.%1 | External processing (*.%2) | * .%2'");
