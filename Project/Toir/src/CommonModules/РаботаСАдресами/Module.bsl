...
Функция РегионАдресаКонтактнойИнформации(Знач Адрес) Экспорт
	...
	ИначеЕсли ТипЗнч(Адрес) <> Тип("Структура") Тогда
		...
		ВызватьИсключение НСтр("es='Es imposible determinar el sujeto de la Federación Rusa, se espera la dirección.';ru='Невозможно определить субъекта РФ, ожидается адрес.';en='Unable to determine the subject of the Russian Federation, an address is expected.'");
...
Функция ГородАдресаКонтактнойИнформации(Знач Адрес) Экспорт
	...
	ИначеЕсли ТипЗнч(Адрес) <> Тип("Структура") Тогда
		...
		ВызватьИсключение НСтр("es='Es imposible determinar la ciudad, se espera la dirección.';ru='Невозможно определить город, ожидается адрес.';en='Unable to determine city, address expected.'");
...
Функция ПоляАдресаВJSON(ПоляАдреса) Экспорт
	...
	Если ПоляАдреса.ТипАдреса <> РаботаСАдресамиКлиентСервер.МуниципальныйАдрес()
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El tipo incorrecto de la (%1 dirección';ru='Некорректный тип адреса (%1)';en='Invalid address type (%1)'"),
...
Процедура ОбработкаЧастыхСокращенийВАдресах(Знач ДанныеАнализа) Экспорт
	...
	СубъектРФ.Вставить(НСтр("es='SANKT-PETERBURG';ru='САНКТ-ПЕТЕРБУРГ';en='ST. PETERSBURG'"), "Санкт-Петербург");
	СубъектРФ.Вставить(НСтр("es='MOSCÚ';ru='МОСКВА';en='MOSCOW'"), "Москва");
	СубъектРФ.Вставить(НСтр("es='HORA DE MOSCÚ';ru='МСК';en='MSC'"), "Москва");
	СубъектРФ.Вставить(НСтр("es='СПБ';ru='СПБ';en='SPB'"), "Санкт-Петербург");
...
Функция КодМуниципальногоРайона(Знач ТипМуниципальногоРайона)
	...
	Если СтрСравнить(ТипМуниципальногоРайона, НСтр("es='m.r-n';ru='м.р-н';en='м.р-н'")) = 0 Тогда
			КодМуниципальногоРайона = "1";
	ИначеЕсли СтрСравнить(ТипМуниципальногоРайона, НСтр("es='De la isla';ru='г.о.';en='г.о.'")) = 0 Тогда
			КодМуниципальногоРайона = "2";
	ИначеЕсли СтрСравнить(ТипМуниципальногоРайона, НСтр("es='вн.тер.';ru='вн.тер.';en='вн.тер.'")) = 0
		Или СтрСравнить(ТипМуниципальногоРайона, НСтр("es='вн.тер.';ru='вн.тер. г.';en='вн.тер. г.'")) = 0 Тогда
...
Функция КодПоселения(Знач ТипПоселения)
	...
	Если СтрСравнить(ТипПоселения, НСтр("es='De art.';ru='г. п.';en='г. п.'")) = 0 Тогда
		КодПоселения = "1";
	ИначеЕсли СтрСравнить(ТипПоселения, НСтр("es='с.п.';ru='с.п.';en='с.p.'")) = 0 Тогда
		КодПоселения = "2";
	ИначеЕсли СтрСравнить(ТипПоселения, НСтр("es='меж.тер.';ru='меж.тер.';en='inter.'")) = 0 Тогда
		КодПоселения = "3";
	ИначеЕсли СтрСравнить(ТипПоселения, НСтр("es='vn.r-n';ru='вн.р-н';en='вн.р-н'")) = 0 Тогда
		КодПоселения = "4";
	КонецЕсли;
