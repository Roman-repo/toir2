...
Функция ВыполнитьРассылкуОтчетов(Рассылка, ПараметрыЖурнала = Неопределено, ДополнительныеНастройки = Неопределено) Экспорт
	...
	Если Не ПараметрыЖурнала.Свойство("ИмяСобытия") Тогда
		ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("es='El envío de los informes. El lanzamiento a petición';ru='Рассылка отчетов. Запуск по требованию';en='Mailing reports. Demand launch'", ОбщегоНазначения.КодОсновногоЯзыка()));
	КонецЕсли;
	...
	Если Не Рассылка.Подготовлена
		...
		Если Не Рассылка.Подготовлена Тогда
			Причина = Причина + Символы.ПС + НСтр("es='El envío no es preparado';ru='Рассылка не подготовлена';en='Mailing not prepared'");
		КонецЕсли;
		Если Рассылка.ПометкаУдаления Тогда
			Причина = Причина + Символы.ПС + НСтр("es='El envío es marcado a la desaparición';ru='Рассылка помечена на удаление';en='Mailing marked for deletion'");
		КонецЕсли;
		
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
			НСтр("es='La terminación';ru='Завершение';en='Completion'"), СокрЛП(Причина));
		Возврат Ложь;
	...
	КонецЕсли;
	Если ФорматыПоУмолчанию.Количество() = 0 Тогда
		ВызватьИсключение НСтр("es='por reticencia.';ru='Не установлены форматы по умолчанию.';en='The default formats are not set.'");
	КонецЕсли;
	...
	Если Не ПараметрыДоставки.ИспользоватьПапку
		...
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
			НСтр("es='de la entrega.';ru='Не выбран способ доставки.';en='No delivery method selected.'"));
		Возврат Ложь;
	КонецЕсли;
...
Функция ВыполнитьРассылку(Отчеты, ПараметрыДоставки, НаименованиеРассылки = "", ПараметрыЖурнала = Неопределено) Экспорт
	...
	ЗаписьЖурнала(ПараметрыЖурнала, ,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Инициализирована el envío %1, el autor: %2';ru='Инициализирована рассылка %1, автор: %2';en='Mailing list %1 initialized by %2'"),
	...
	Для Каждого СтрокаОтчет Из Отчеты Цикл
		ТекстЖурнала = НСтр("es='El informe %1 se forma';ru='Отчет %1 формируется';en='Report %1 generated'");
		Если СтрокаОтчет.Настройки = Неопределено Тогда
			ТекстЖурнала = ТекстЖурнала + Символы.ПС + НСтр("es='(Los ajustes de usuario no son dados)';ru='(пользовательские настройки не заданы)';en='(no user settings)'");
		КонецЕсли;
		...
		Если ПараметрыДоставки.Персонализирована И НЕ ПараметрыОтчета.Персонализирован Тогда
			ПараметрыОтчета.Ошибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El informe ""%1"" no puede es formado, puesto que en sus ajustes no es indicada la selección por el destinatario del envío.';ru='Отчет ""%1"" не может сформирован, так как в его настройках не указан отбор по получателю рассылки.';en='Report ""%1"" cannot be generated, because its settings do not specify the selection by mailing recipient.'"),
		...
		Попытка
			...
			КонецЕсли;
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Примечание,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El informe %1 es con éxito formado';ru='Отчет %1 успешно сформирован';en='Report %1 successfully generated'"), "'"+ Строка(СтрокаОтчет.Отчет) +"'"));
			НомерОтчета = НомерОтчета + 1;
		Исключение
			ЗаписьЖурнала(ПараметрыЖурнала, ,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El informe %1 no es formado:';ru='Отчет %1 не сформирован:';en='Report %1 not generated:'"), "'"+ Строка(СтрокаОтчет.Отчет) +"'" ), ИнформацияОбОшибке());
		КонецПопытки;
	КонецЦикла;
	...
	Если ДеревоОтчетов.Строки.Найти(3, "Уровень", Истина) = Неопределено Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
			НСтр("es='El envío de los informes no es cumplido, puesto que los informes vacío o no son formados por las faltas.';ru='Рассылка отчетов не выполнена, так как отчеты пустые или не сформированы из-за ошибок.';en='Mailing reports failed because the reports are empty or not generated due to errors.'"));
	...
	Для Каждого СтрокаПолучатель Из ДеревоОтчетов.Строки Цикл
		...
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió enviar los informes al destinatario %1';ru='Не удалось отправить отчеты получателю %1';en='Failed to send reports to recipient %1'"), "'" + ПредставлениеПолучателя + "'");
	...
	Если РассылкаВыполнена Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, , НСтр("es='El envío es cumplido';ru='Рассылка выполнена';en='Mailing is done'"));
	Иначе
		ЗаписьЖурнала(ПараметрыЖурнала, , НСтр("es='El envío no es cumplido';ru='Рассылка не выполнена';en='Mailing failed'"));
	КонецЕсли;
...
Процедура ВыполнитьРассылкиВФоновомЗадании(ПараметрыВыполнения, АдресРезультата) Экспорт
	...
	Если Всего = 0 Тогда
		ТекстСообщения = НСтр("es='Los grupos escogidos no contienen los envíos de los informes.';ru='Выбранные группы не содержат рассылок отчетов.';en='Selected groups do not contain report mailings.'");
	ИначеЕсли Всего <= 5 Тогда
		ТекстСообщения = "";
		Для Каждого СтрокаТаблицы Из ТаблицаРассылок Цикл
			Если Не СтрокаТаблицы.Подготовлена Тогда
				ШаблонСообщения = НСтр("es='El envío ""%1"" no es preparado.';ru='Рассылка ""%1"" не подготовлена.';en='Mailing ""%1"" is not prepared.'");
			ИначеЕсли Не СтрокаТаблицы.Выполнена Тогда
				ШаблонСообщения = НСтр("es='El envío ""%1"" no es cumplido.';ru='Рассылка ""%1"" не выполнена.';en='Mailing ""%1"" failed.'");
			ИначеЕсли СтрокаТаблицы.СОшибками Тогда
				ШаблонСообщения = НСтр("es='El envío ""%1"" es cumplido con las faltas.';ru='Рассылка ""%1"" выполнена с ошибками.';en='Mailing ""%1"" failed.'");
			Иначе
				ШаблонСообщения = НСтр("es='El envío ""%1"" es cumplido.';ru='Рассылка ""%1"" выполнена.';en='Mailing ""%1"" completed.'");
			КонецЕсли;
	...
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es preparado los envíos: %1 de %2 "
"Es cumplido: %3 "
"Con las faltas: %4 "
"no es cumplido: %5';ru='Подготовлено рассылок: %1 из %2"
"Выполнено: %3"
"С ошибками: %4"
"Не выполнено: %5';en='Mailing list prepared:%1 of%2"
"%3 completed"
"With errors:%4"
"Failed:%5 '"),
...
Процедура ФормаОтчетаДобавитьКоманды(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	КомандаСоздать.Заголовок = НСтр("es='Crear el envío de los informes...';ru='Создать рассылку отчетов...';en='Create a newsletter of reports ...'");
	КомандаСоздать.Подсказка = НСтр("es='Crear un nuevo envío de los informes y añadir en ella el informe con los ajustes corrientes.';ru='Создать новую рассылку отчетов и добавить в нее отчет с текущими настройками.';en='Create a new report distribution and add a report with the current settings to it.'");
	...
	КомандаПрисоединить.Заголовок = НСтр("es='Incluir en el envío existente de los informes...';ru='Включить в существующую рассылку отчетов...';en='Include in existing report distribution ...'");
	КомандаПрисоединить.Подсказка = НСтр("es='Juntar el informe con los ajustes corrientes al envío existente de los informes.';ru='Присоединить отчет с текущими настройками к существующей рассылке отчетов.';en='Attach the report with the current settings to the existing report distribution.'");
	...
	Если КоличествоРассылокСОтчетом > 0 Тогда
		...
		КомандаРассылки.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Los envíos del informe (%1)';ru='Рассылки отчета (%1)';en='Report mailing (%1)'"), 
			КоличествоРассылокСОтчетом);
		КомандаРассылки.Подсказка = НСтр("es='Abrir la lista de los envíos, en que es incluido el informe.';ru='Открыть список рассылок, в которые включен отчет.';en='Open the list of mailing lists in which the report is included.'");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий     = НСтр("es='Desconecta las tareas no utilizadas de reglamento de los envíos de los informes.';ru='Отключает неиспользуемые регламентные задания рассылок отчетов.';en='Disables unused scheduled tasks for report distribution.'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("es='Los problemas con los envíos de los informes';ru='Проблемы с рассылками отчетов';en='Reports sending problems '");
...
Процедура АктуализироватьСписокРегламентныхЗаданий(Параметры) Экспорт
	...
	Пока РассылкиОтчетов.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió tratar la tarea de reglamento del envío de los informes: %1 con el tipo %2 por la causa: %3';ru='Не удалось обработать регламентное задание рассылки отчетов: %1 с типом %2 по причине: %3';en='Failed to process report distribution scheduled work: %1 with type %2 due to: %3'"),
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Al procedimiento de AktualizirovatSpisokReglamentnyhZadany no consiguió tratar algunas tareas de reglamento del envío de los informes (son dejados pasar): %1';ru='Процедуре АктуализироватьСписокРегламентныхЗаданий не удалось обработать некоторые регламентные задания рассылки отчетов (пропущены): %1';en='A procedure АктуализироватьСписокРегламентныхЗаданий failed to process some scheduled tasks for sending reports (skipped):%1'"), 
				ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			Метаданные.Справочники.РассылкиОтчетов,,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El procedimiento de AktualizirovatSpisokReglamentnyhZadany ha tratado la porción siguiente de envío de los informes: %1';ru='Процедура АктуализироватьСписокРегламентныхЗаданий обработала очередную порцию рассылки отчетов: %1';en='A procedure АктуализироватьСписокРегламентныхЗаданий processed the next batch of reports:%1'"),
					ОбъектовОбработано));
	КонецЕсли;
...
Процедура ЗаполнитьНаименованиеПредопределенногоЭлементаЛичныеРассылки() Экспорт
	...
	ЛичныеРассылки.Наименование = НСтр("es='Los envíos personales';ru='Личные рассылки';en='Personal newsletters'");
...
Процедура ВыполнитьРассылкуПоРасписанию(Рассылка) Экспорт
	...
	Если Не ПравоДоступа("Чтение", Метаданные.Справочники.РассылкиОтчетов) Тогда
		ВызватьИсключение
			НСтр("es='Al usuario corriente es insuficientemente derecho para la lectura de los envíos de los informes. "
"Se Recomienda desconectar todos los envíos de este usuario o cambiar al autor de sus envíos (en вкладке ""el Horario"").';ru='У текущего пользователя недостаточно прав для чтения рассылок отчетов."
"Рекомендуется отключить все рассылки этого пользователя или сменить автора его рассылок (на вкладке ""Расписание"").';en='The current user does not have sufficient permissions to read the newsletters."
"It is recommended to disable all mailings of this user or change the author of his mailings (on the ""Schedule"" tab).'");
	КонецЕсли;
	...
	Если Не Выборка.Следующий() Тогда
		ВызватьИсключение
			НСтр("es='Al usuario corriente es insuficientemente derecho para la lectura de este envío. "
"Se Recomienda cambiar al autor del envío (en вкладке ""el Horario"").';ru='У текущего пользователя недостаточно прав для чтения этой рассылки."
"Рекомендуется сменить автора рассылки (на вкладке ""Расписание"").';en='The current user does not have sufficient permissions to read this newsletter."
"It is recommended to change the author of the newsletter (on the ""Schedule"" tab).'");
	КонецЕсли;
	Если Не Выборка.ВыполнятьПоРасписанию Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Cerca del envío de los informes ""%1"" es desconectado el banderín ""Cumplir por el horario"" "
"se Recomienda desconectar la tarea correspondiente de reglamento o regrabar este envío.';ru='У рассылки отчетов ""%1"" отключен флажок ""Выполнять по расписанию"""
"Рекомендуется отключить соответствующее регламентное задание или перезаписать эту рассылку.';en='Report distribution ""%1"" the ""Run as scheduled"" checkbox is disabled"
"It is recommended that you disable the corresponding scheduled task or overwrite this newsletter.'"),
			Строка(Рассылка));
	КонецЕсли;
	
	// Параметры записи в журнал регистрации.
	ПараметрыЖурнала = Новый Структура("ИмяСобытия, Метаданные, Данные");
	ПараметрыЖурнала.ИмяСобытия = НСтр("es='El envío de los informes. El lanzamiento por el horario';ru='Рассылка отчетов. Запуск по расписанию';en='Mailing reports. Scheduled launch'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СформироватьСписокПолучателейРассылки(ПараметрыЖурнала, Рассылка) Экспорт
	...
	ТекстСообщенияОбОшибкеДляЖурналаРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='A la formación de la lista de los destinatarios ""%1"" ha surgido la falta:';ru='При формировании списка получателей ""%1"" возникла ошибка:';en='An error occurred while generating the list of recipients ""%1"":'"), Строка(ТипПолучателей));
	...
	Если СписокПолучателей.Количество() = 0 Тогда
		ТекстОшибок = НСтр("es='No consiguió formar la lista de los destinatarios de %1 envíos %2 por una de las causas posibles:"
" - a los destinatarios no es rellenada la %3 dirección postal;"
" - no es rellenada la lista de los destinatarios;"
" - Son escogidos los grupos vacíos de los destinatarios;"
" - los Destinatarios son marcados a la desaparición;"
" - Son excluidos todos los destinatarios (la excepción tiene la prioridad más alta; los participantes de los grupos excluidos se excluyen también de la lista);"
" - No Basta derechos de admisión a la guía;"
" - se Recomienda comprobar la corrección del ajuste de la anotación de descuento del correo electrónico, en nombre de que se cumple el envío.';ru='Не удалось сформировать список получателей %1 рассылки %2 по одной из возможных причин:"
...
" - Рекомендуется проверить корректность настройки учетной записи электронной почты, от имени которой выполняется рассылка.';en='Failed to generate recipient list%1 of mailing%2 for one of the possible reasons:"
"  - The recipients are not filled in the mailing address%3;"
"  - The list of recipients is not completed;"
"  - Selected empty recipient groups;"
"  - Recipients are marked for deletion;"
"  - All recipients are excluded (the exception has the highest priority; members of excluded groups are also excluded from the list);"
"  - Insufficient access rights to the directory;"
"  “It is recommended that you check the settings for the email account on whose behalf the mailing is performed.'");
...
Функция ИнициализироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ПерсонализацияВозможна, УникальныйИдентификаторФормы = Неопределено) Экспорт
	...
	Исключение
		ПараметрыОтчета.Ошибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió conectar y cargar los ajustes del informe ""%1"".';ru='Не удалось подключить и загрузить настройки отчета ""%1"".';en='Failed to connect and load report settings ""%1"".'"),
...
Функция СформироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, Получатель = Неопределено)
	...
	Если Не ПараметрыОтчета.Свойство("Инициализирован") Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, ,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es=""El informe ' %1 ' no инициализирован"";ru=""Отчет '%1' не инициализирован"";en=""Report '%1' not initialized"""), Строка(ПараметрыОтчета.Отчет)));
		Возврат Результат;
	КонецЕсли;
	...
	Если Не Формирование.Успех Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El informe ""%1"":';ru='Отчет ""%1"":';en='Report ""%1"":'"),
...
Функция ВыполнитьДоставку(ПараметрыЖурнала, ПараметрыДоставки, Вложения) Экспорт
	Результат = Ложь;
	ШаблонСообщенияОбОшибке = НСтр("es='La falta de la entrega de los informes';ru='Ошибка доставки отчетов';en='Report Delivery Failure'");
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		Если ПараметрыДоставки.ТолькоУведомить Тогда
			ШаблонСообщенияОбОшибке = НСтр("es='Es imposible enviar el aviso del envío por el correo electrónico:';ru='Невозможно отправить уведомление о рассылке по электронной почте:';en='Unable to send email notification:'");
			ВложенияПисьма = Новый Соответствие;
		Иначе
			ШаблонСообщенияОбОшибке = НСтр("es='Es imposible enviar el informe por el correo electrónico:';ru='Невозможно отправить отчет по электронной почте:';en='Unable to send report by email:'");
	...
	Если Рассылка.ИспользоватьСетевойКаталог Тогда
		Если ЗначениеЗаполнено(Рассылка.СетевойКаталогWindows) Тогда
			Элемент = МодульРаботаВБезопасномРежиме.РазрешениеНаИспользованиеКаталогаФайловойСистемы(
				...
				Истина,
				Истина,
				НСтр("es='El catálogo de red para la publicación de los informes del servidor Windows.';ru='Сетевой каталог для публикации отчетов с сервера Windows.';en='Network directory for publishing reports from a Windows server.'"));
		...
		Если ЗначениеЗаполнено(Рассылка.СетевойКаталогLinux) Тогда
			Элемент = МодульРаботаВБезопасномРежиме.РазрешениеНаИспользованиеКаталогаФайловойСистемы(
				...
				Истина,
				Истина,
				НСтр("es='El catálogo de red para la publicación de los informes del servidor Linux.';ru='Сетевой каталог для публикации отчетов с сервера Linux.';en='Network directory for publishing reports from a Linux server.'"));
			Разрешения.Добавить(Элемент);
		КонецЕсли;
	КонецЕсли;
	Если Рассылка.ИспользоватьFTPРесурс Тогда
		Если ЗначениеЗаполнено(Рассылка.FTPСервер) Тогда
			Элемент = МодульРаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
				...
				Рассылка.FTPПорт,
				НСтр("es='FTP el recurso para la publicación de los informes.';ru='FTP ресурс для публикации отчетов.';en='FTP resource for publishing reports.'"));
			Разрешения.Добавить(Элемент);
		КонецЕсли;
	КонецЕсли;
	Возврат Разрешения;
...
Процедура СформироватьИСохранитьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ДеревоОтчетов, ПараметрыДоставки, ПолучательСсылка)
	...
	Для Каждого Формат Из ПараметрыОтчета.Форматы Цикл
		...
		Если СтандартнаяОбработка = Истина Тогда
			ЗаголовокОшибки = НСтр("es='La falta de la anotación del informe %1 en el formato %2:';ru='Ошибка записи отчета %1 в формат %2:';en='Error writing report %1 to format %2:'");
			...
			Если ПараметрыФормата.ТипФайла = Неопределено Тогда
				ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
					...
					НСтр("es='El formato no es apoyado';ru='Формат не поддерживается';en='Format not supported'"));
				Продолжить;
			КонецЕсли;
		...
		Если Не ВременныйФайл.Существует() Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ЗаголовокОшибки + Символы.ПС + НСтр("es='El fichero %3 no es encontrado.';ru='Файл %3 не найден.';en='File%3 not found.'"),
				"'"+ СтрокаОтчет.Ключ +"'",
...
Функция ПроверитьИДозаполнитьПараметрыВыполнения(ТаблицаОтчетов, ПараметрыДоставки, НаименованиеРассылки, ПараметрыЖурнала)
	...
	Если Недоступные.Количество() > 0 Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='En el envío hay unos informes inaccesibles (%1: %2)';ru='В рассылке есть недоступные отчеты (%1):%2';en='There are inaccessible reports in the mailing list (% 1):% 2'"),
	...
	Если ПараметрыДоставки.ИспользоватьПапку Тогда
		Если Не ЗначениеЗаполнено(ПараметрыДоставки.Папка) Тогда
			...
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
				НСтр("es='La carpeta no es rellenada, la entrega en la carpeta es desconectada';ru='Папка не заполнена, доставка в папку отключена';en='The folder is empty, delivery to the folder is disabled'"));
		Иначе
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСФайлами") Тогда
				МодульРаботаСФайламиСлужебный = ОбщегоНазначения.ОбщийМодуль("РаботаСФайламиСлужебный");
			...
			Если Не ПравоДоступа Тогда
				...
				УстановитьПривилегированныйРежим(Ложь);
				ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Es insuficientemente derecho para la creación de los ficheros en la carpeta ""%1"".';ru='Недостаточно прав для создания файлов в папке ""%1"".';en='Not enough rights to create files in folder ""%1"".'"),
					ПредставлениеПапки));
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;
	...
	Если ПараметрыДоставки.ИспользоватьСетевойКаталог Тогда
		Если Не ЗначениеЗаполнено(ПараметрыДоставки.СетевойКаталогWindows) 
			...
			Если ЗначениеЗаполнено(ПараметрыДоставки.СетевойКаталогWindows) Тогда
				ЗначениеПодстановки = НСтр("es='Linux';ru='Linux';en='Linux'");
			ИначеЕсли ЗначениеЗаполнено(ПараметрыДоставки.СетевойКаталогLinux) Тогда
				ЗначениеПодстановки = НСтр("es='Windows';ru='Windows';en='Windows'");
			Иначе
				ЗначениеПодстановки = НСтр("es='Windows y Linux';ru='Windows и Linux';en='Windows and Linux'");
			КонецЕсли;
			
			ПараметрыДоставки.ИспользоватьСетевойКаталог = Ложь;
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El catálogo %1 de red no es escogido, la entrega en el catálogo de red es desconectada';ru='Сетевой каталог %1 не выбран, доставка в сетевой каталог отключена';en='Network directory%1 not selected, delivery to network directory disabled'"),
				ЗначениеПодстановки));
	...
	Если ПараметрыДоставки.ИспользоватьFTPРесурс И Не ЗначениеЗаполнено(ПараметрыДоставки.Сервер) Тогда
		...
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("es='FTP el servidor no es rellenado, la entrega en la carpeta en FTP el recurso es desconectada';ru='FTP сервер не заполнен, доставка в папку на FTP ресурс отключена';en='FTP server is not full, delivery to a folder on an FTP resource is disabled'"));
	КонецЕсли;
	
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту И Не ЗначениеЗаполнено(ПараметрыДоставки.УчетнаяЗапись) Тогда
		ПараметрыДоставки.ИспользоватьЭлектроннуюПочту = Ложь;
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("es='La anotación de descuento no es escogida, la entrega por el correo electrónico es desconectada';ru='Учетная запись не выбрана, доставка по электронной почте отключена';en='No account selected, email delivery disabled'"));
	КонецЕсли;
	...
	Если ПараметрыДоставки.Персонализирована Тогда
		Если Не ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				НСтр("es='El envío personalizado puede ser enviado solamente por el correo electrónico';ru='Персонализированная рассылка может быть отправлена только по электронной почте';en='Personalized newsletters can only be sent by email.'"));
			Возврат Ложь;
		КонецЕсли;
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		Если ПараметрыДоставки.ТолькоУведомить
			...
			И Не ПараметрыДоставки.ИспользоватьFTPРесурс Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
				НСтр("es='El uso de los avisos por el correo electrónico es posible solamente de otros modos de la entrega';ru='Использование уведомлений по электронной почте возможно только совместно с другими способами доставки';en='Using email notifications is only possible in conjunction with other delivery methods.'"));
			Возврат Ложь;
		КонецЕсли;
...
Функция ШаблонТемы() Экспорт
	Возврат НСтр("es=""[НаименованиеРассылки] de [DataVypolneniya (ДЛФ ='D ')]"";ru=""[НаименованиеРассылки] от [ДатаВыполнения(ДЛФ='D')]"";en=""[НаименованиеРассылки] от [ДатаВыполнения(ДЛФ='D')]""");
КонецФункции

// Возвращает шаблон тела сообщения по умолчанию для доставки по электронной почте.
Функция ШаблонТекста() Экспорт
	Возврат НСтр("es=""Son formados los informes: "
""
...
""
"[ЗаголовокСистемы] "
"[DataVypolneniya (ДЛФ ='DD ')]"";ru=""Сформированы отчеты:"
""
"[СформированныеОтчеты]"
""
"[СпособДоставки]"
""
"[ЗаголовокСистемы]"
"[ДатаВыполнения(ДЛФ='DD')]"";en=""Reports are generated:                   [СформированныеОтчеты]"
""
"[СпособДоставки]"
""
"[ЗаголовокСистемы]"
...
Функция ШаблонИмениАрхива() Экспорт
	...
	Возврат НСтр("es=""[НаименованиеРассылки] _ [DataVypolneniya (ДФ ='yyyy-MM-dd ')]"";ru=""[НаименованиеРассылки]_[ДатаВыполнения(ДФ='yyyy-MM-dd')]"";en=""[НаименованиеРассылки]_[ДатаВыполнения(ДФ='yyyy-MM-dd')]""");
КонецФункции
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		Если ПараметрыДоставки.ЗаполнитьСформированныеОтчетыВШаблонеСообщения Тогда
			ПараметрыДоставки.ПредставлениеОтчетовПолучателя = 
				...
				+ Символы.ПС 
				+ Символы.ПС
				+ НСтр("es='Los ficheros de los informes son embalados en el archivo';ru='Файлы отчетов запакованы в архив';en='Report files are archived'")
				+ " ";
		КонецЕсли;
...
Функция ПравоВывода(ПараметрыЖурнала)
	...
	Если Не ПравоВывода Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Al usuario %1 es insuficientemente derecho a la conclusión de la información';ru='У пользователя %1 недостаточно прав на вывод информации';en='The user %1 does not have sufficient rights to display information'"),
...
Функция СтрокаСообщенийПользователю(МассивОшибок = Неопределено, ДобавитьДежурнуюФразу = Истина) Экспорт
	...
	Если ВсеОшибки <> "" И ДобавитьДежурнуюФразу Тогда
		ВсеОшибки = ВсеОшибки + Отступ + "---" + Отступ + НСтр("es='Los detalles de cm. En la revista del registro.';ru='Подробности см. в журнале регистрации.';en='See the log for details.'");
	КонецЕсли;
...
Функция ТекстОшибкиПроверкиПраваДобавления() Экспорт
	Если Не ПравоДоступа("Вывод", Метаданные) Тогда
		Возврат НСтр("es='No hay derechos a la conclusión de la información.';ru='Нет прав на вывод информации.';en='No rights to display information.'");
	КонецЕсли;
	Если Не ПравоДоступа("Изменение", Метаданные.Справочники.РассылкиОтчетов) Тогда
		Возврат НСтр("es='No hay derechos a los envíos de los informes.';ru='Нет прав на рассылки отчетов.';en='No rights to send reports.'");
	КонецЕсли;
	Если Не РаботаСПочтовымиСообщениями.ДоступнаОтправкаПисем() Тогда
		Возврат НСтр("es='No hay derechos al envío de las cartas o no las anotaciones accesibles de descuento.';ru='Нет прав на отправку писем или нет доступных учетных записей.';en='There is no rights to send emails or no accounts available.'");
	КонецЕсли;
	Возврат "";
КонецФункции
