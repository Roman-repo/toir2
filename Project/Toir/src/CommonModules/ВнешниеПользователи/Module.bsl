	...
	Если ЗначениеЗаполнено(ОбъектАвторизации) Тогда
		Если ПользователиСлужебный.ОбъектАвторизацииИспользуется(ОбъектАвторизации, ВнешнийПользователь) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta en la base de datos: "
"el Objeto de la autorización ""%1"" (%2) "
"es establecido para algunos usuarios exteriores.';ru='Ошибка в базе данных:"
"Объект авторизации ""%1"" (%2)"
"установлен для нескольких внешних пользователей.';en='Error in database:"
"Authorization Object ""%1"" (%2)"
"Installed for multiple external users. '"),
				ОбъектАвторизации,
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta en la base de datos: "
"Para el usuario exterior ""%1"" no es dado el objeto de la autorización.';ru='Ошибка в базе данных:"
"Для внешнего пользователя ""%1"" не задан объект авторизации.';en='Error in database:"
"The external user ""%1 "" did not specify an authorization object. '"),
			ВнешнийПользователь);
	КонецЕсли;
