...
Процедура ИзменитьПрефиксИБ(Параметры, АдресРезультата = "") Экспорт
	...
	Исключение
		...
		ВызватьИсключение НСтр("es='No consiguió cambiar el prefijo.';ru='Не удалось изменить префикс.';en='Failed to change the prefix.'");
...
Функция ОписаниеМетаданныхИспользующихПрефиксы(РежимДиагностики = Ложь) Экспорт
	...
	ЕстьПодсистемаРаботаВМоделиСервиса = ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса");
			...
			Если Характеристики.ДлинаКода = 0 И Характеристики.ДлинаНомера = 0 Тогда
				...
				Если Не РежимДиагностики Тогда
					...
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La falta de la introducción del subsistema ""%1"" para el objeto de los metadatos ""%2"".';ru='Ошибка внедрения подсистемы ""%1"" для объекта метаданных ""%2"".';en='Implementation error of subsystem ""%1"" for metadata object ""%2"".'"),
...
Функция СобытиеЖурналаРегистрацииПерепрефиксацияОбъектов()
	...
	Возврат НСтр("es='La prefijación de los objetos. El cambio del prefijo de la base informativa';ru='Префиксация объектов.Изменение префикса информационной базы';en='Prefixing objects. Changing the infix prefix'",
