...
Процедура ПродолжитьВыполнениеКоманды(Контекст)
	...
	Если Контекст.ТребуетсяРаботаСФайлами Тогда
		...
		ТекстСообщения = НСтр("es='Para la continuación es necesario establecer la ampliación para el cliente web 1С:Предприятие.';ru='Для продолжения необходимо установить расширение для веб-клиента 1С:Предприятие.';en='To continue, you must install the extension for the 1C: Enterprise web client.'");
...
"La ejecución de la acción ""%1"" es posible solamente después del registro de datos. "
"Los datos serán anotados.';ru='Данные еще не записаны."
"Выполнение действия ""%1"" возможно только после записи данных."
"Данные будут записаны.';en='Data not yet recorded."
"The action """"%1 """" is only possible after writing data."
		...
		Если ИнформацияОДокументах.Непроведенные.Количество() > 0 Тогда
			Если ИнформацияОДокументах.ДоступноПравоПроведения Тогда
				Если ИнформацияОДокументах.Непроведенные.Количество() = 1 Тогда
					ТекстВопроса = НСтр("es='Para la ejecución de la orden es necesario preliminarmente pasar el documento. ¿Cumplir realización del documento y continuar?';ru='Для выполнения команды необходимо предварительно провести документ. Выполнить проведение документа и продолжить?';en='To execute the command, you must first post a document. Complete the document and continue?'");
				Иначе
					ТекстВопроса = НСтр("es='Para la ejecución de la orden es necesario preliminarmente pasar el documento. ¿Cumplir realización del documento y continuar?';ru='Для выполнения команды необходимо предварительно провести документ. Выполнить проведение документа и продолжить?';en='To execute the command, you must first post a document. Complete the document and continue?'");
				КонецЕсли;
			...
			Иначе
				Если ИнформацияОДокументах.Непроведенные.Количество() = 1 Тогда
					ТекстПредупреждения = НСтр("es='Para la ejecución de la orden es necesario preliminarmente pasar el documento. Es insuficientemente derecho para la realización del documento.';ru='Для выполнения команды необходимо предварительно провести документ. Недостаточно прав для проведения документа.';en='To execute the command, you must first post a document. Not enough rights to post a document.'");
				Иначе
					ТекстПредупреждения = НСтр("es='Para la ejecución de la orden es necesario preliminarmente pasar el documento. Es insuficientemente derecho para la realización del documento.';ru='Для выполнения команды необходимо предварительно провести документ. Недостаточно прав для проведения документа.';en='To execute the command, you must first post a document. Not enough rights to post a document.'");
				КонецЕсли;
...
Процедура ПродолжитьВыполнениеКомандыПослеПодтверждениеПроведения(Ответ, Контекст) Экспорт
	...
	ШаблонСообщения = НСтр("es='El documento %1 no es pasado: %2';ru='Документ %1 не проведен: %2';en='Document %1 failed: %2'");
	...
	Если НепроведенныеДокументы.Количество() > 0 Тогда
		...
		ТекстДиалога = НСтр("es='No consiguió pasar uno o plusieurs los documentos.';ru='Не удалось провести один или несколько документов.';en='Failed to post one or more documents.'");
		
		КнопкиДиалога = Новый СписокЗначений;
		Если Контекст.МассивСсылок.Количество() = 0 Тогда
			КнопкиДиалога.Добавить(КодВозвратаДиалога.Отмена, НСтр("es='ОК';ru='ОК';en='OK'"));
		Иначе
			ТекстДиалога = ТекстДиалога + " " + НСтр("es='¿Continuar?';ru='Продолжить?';en='Resume?'");
			КнопкиДиалога.Добавить(КодВозвратаДиалога.Пропустить, НСтр("en='Resume';es='Continuar';ru='Продолжить'"));
