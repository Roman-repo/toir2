...
Функция ИННСоответствуетТребованиям(Знач ИНН, ЭтоЮридическоеЛицо, ТекстСообщения) Экспорт
	...
	Если ЭтоЮридическоеЛицо = Неопределено Тогда
		ТекстСообщения = ТекстСообщения + НСтр("es='del INN.';ru='Не определен тип владельца ИНН.';en='Not defined owner type TIN.'");
		Возврат Ложь;
	КонецЕсли;
	
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ИНН) Тогда
		СоответствуетТребованиям = Ложь;
		ТекстСообщения = ТекстСообщения + НСтр("es='El INN debe consistir solamente de las cifras.';ru='ИНН должен состоять только из цифр.';en='TIN should consist only of numbers.'");
	КонецЕсли;

	Если  ЭтоЮридическоеЛицо И ДлинаИНН <> 10 Тогда
		СоответствуетТребованиям = Ложь;
		ТекстСообщения = ТекстСообщения + ?(ЗначениеЗаполнено(ТекстСообщения), Символы.ПС, "")
			+ НСтр("es='El INN de la persona jurídica debe consistir de 10 cifras.';ru='ИНН юридического лица должен состоять из 10 цифр.';en='The TIN of the legal entity should consist of 10 digits.'");
	ИначеЕсли НЕ ЭтоЮридическоеЛицо И ДлинаИНН <> 12 Тогда
		СоответствуетТребованиям = Ложь;
		ТекстСообщения = ТекстСообщения + ?(ЗначениеЗаполнено(ТекстСообщения), Символы.ПС, "")
			+ НСтр("es='El INN de la persona física debe consistir de 12 cifras.';ru='ИНН физического лица должен состоять из 12 цифр.';en='TIN of an individual should consist of 12 digits.'");
	КонецЕсли;
	...
	Если СоответствуетТребованиям Тогда
		...
		Если ЭтоЮридическоеЛицо Тогда
			...
			Если КонтрольныйРазряд <> Число(Сред(ИНН, 10, 1)) Тогда
				СоответствуетТребованиям = Ложь;
				ТекстСообщения = ТекстСообщения + ?(ЗначениеЗаполнено(ТекстСообщения), Символы.ПС, "")
				               + НСтр("es='El número de control para el INN no coincide con contado.';ru='Контрольное число для ИНН не совпадает с рассчитанным.';en='The control number for the TIN does not match the calculated one.'");
			КонецЕсли;
...
Функция КППСоответствуетТребованиям(Знач КПП, ТекстСообщения) Экспорт
	...
	Если СтрДлина(КПП) <> 9 Тогда
		Ошибки.Добавить(НСтр("es='КПП debe consistir de 9 símbolos.';ru='КПП должен состоять из 9 символов.';en='The PPC must be 9 characters long.'"));
	Иначе
		Если Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Лев(КПП, 4)) Тогда
			Ошибки.Добавить(НСтр("es='Los primeros 4 símbolos КПП (el código del órgano fiscal) deben ser las cifras.';ru='Первые 4 символа КПП (код налогового органа) должны быть цифрами.';en='The first 4 characters of the checkpoint (tax authority code) must be numbers.'"));
		КонецЕсли;
		...
		Для НомерСимвола = 5 По 6 Цикл
			...
			Если СтрНайти(ДопустимыеСимволы, Символ) = 0 Тогда
				Ошибки.Добавить(НСтр("es='Los símbolos 5-6 КПП (la causa del planteamiento al recuento) deben ser las cifras y/o las letras mayúsculas del alfabeto latino de A hasta Z.';ru='Символы 5-6 КПП (причина постановки на учет) должны быть цифрами и/или заглавными буквами латинского алфавита от A до Z.';en='Characters 5-6 of the checkpoint (reason for registration) must be numbers and / or capital letters of the Latin alphabet from A to Z.'"));
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		Если Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Прав(КПП, 3)) Тогда
			Ошибки.Добавить(НСтр("es='Los últimos 3 símbolos КПП (el número ordinal del planteamiento al recuento) deben ser las cifras.';ru='Последние 3 символа КПП (порядковый номер постановки на учет) должны быть цифрами.';en='The last 3 characters of the checkpoint (serial number of registration) should be digits.'"));
		КонецЕсли;
	КонецЕсли;
...
Функция ОГРНСоответствуетТребованиям(Знач ОГРН, ЭтоЮридическоеЛицо, ТекстСообщения) Экспорт
	...
	Если ЭтоЮридическоеЛицо = Неопределено Тогда
		ТекстСообщения = ТекстСообщения + НСтр("es='ОГРН.';ru='Не определен тип владельца ОГРН.';en='The type of OGRN owner is not defined.'");
		Возврат Ложь;
	КонецЕсли;

	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ОГРН) Тогда
		СоответствуетТребованиям = Ложь;
		ТекстСообщения = ТекстСообщения + НСтр("es='ОГРН debe consistir solamente de las cifras.';ru='ОГРН должен состоять только из цифр.';en='PSRN should consist only of numbers.'")
	КонецЕсли;

	Если ЭтоЮридическоеЛицо И ДлинаОГРН <> 13 Тогда
		СоответствуетТребованиям = Ложь;
		ТекстСообщения = ТекстСообщения + ?(ЗначениеЗаполнено(ТекстСообщения), Символы.ПС, "")
		               + НСтр("es='ОГРН de la persona jurídica debe consistir de 13 cifras.';ru='ОГРН юридического лица должен состоять из 13 цифр.';en='The PSRN of a legal entity should consist of 13 digits.'");
	ИначеЕсли НЕ ЭтоЮридическоеЛицо И ДлинаОГРН <> 15 Тогда
		СоответствуетТребованиям = Ложь;
		ТекстСообщения = ТекстСообщения + ?(ЗначениеЗаполнено(ТекстСообщения), Символы.ПС, "")
		               + НСтр("es='ОГРН de la persona física debe consistir de 15 cifras.';ru='ОГРН физического лица должен состоять из 15 цифр.';en='PSRN of an individual must consist of 15 digits.'");
	КонецЕсли;
	...
	Если СоответствуетТребованиям Тогда
		...
		Если ЭтоЮридическоеЛицо Тогда
			...
			Если КонтрольныйРазряд <> Прав(ОГРН, 1) Тогда
				СоответствуетТребованиям = Ложь;
				ТекстСообщения = ТекстСообщения + ?(ЗначениеЗаполнено(ТекстСообщения), Символы.ПС, "")
				               + НСтр("es='El número de control para ОГРН no coincide con contado.';ru='Контрольное число для ОГРН не совпадает с рассчитанным.';en='The control number for the OGRN does not coincide with the calculated one.'");
			КонецЕсли;
...
Функция КодПоОКПОСоответствуетТребованиям(Знач ПроверяемыйКод, ЭтоЮридическоеЛицо, ТекстСообщения = "") Экспорт
	...
	Если ЭтоЮридическоеЛицо = Неопределено Тогда
		ТекстСообщения = ТекстСообщения + НСтр("es='ОКПО.';ru='Не определен тип владельца кода ОКПО.';en='The type of owner of the OKPO code is not defined.'");
		Возврат Ложь;
	КонецЕсли;
	
	Если Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ПроверяемыйКод) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("es='El código ОКПО debe consistir solamente de las cifras.';ru='Код ОКПО должен состоять только из цифр.';en='The OKPO code should consist only of numbers.'") + Символы.ПС;
	КонецЕсли;

	Если ЭтоЮридическоеЛицо И ДлинаКода <> 8 Тогда
		ТекстСообщения = ТекстСообщения + НСтр("es='El código ОКПО de la persona jurídica debe consistir de 8 cifras.';ru='Код ОКПО юридического лица должен состоять из 8 цифр.';en='The OKPO code of a legal entity must consist of 8 digits.'") + Символы.ПС;
	ИначеЕсли Не ЭтоЮридическоеЛицо И ДлинаКода <> 10 Тогда
		ТекстСообщения = ТекстСообщения + НСтр("es='El código ОКПО de la persona física debe consistir de 10 cifras.';ru='Код ОКПО физического лица должен состоять из 10 цифр.';en='The OKPO code of an individual must consist of 10 digits.'") + Символы.ПС;
	КонецЕсли;
	...
	Если Не КодКлассификатораВерный(ПроверяемыйКод) Тогда
		ТекстСообщения = НСтр("es='El número de control para el código por ОКПО no coincide con contado.';ru='Контрольное число для кода по ОКПО не совпадает с рассчитанным.';en='The check number for the OKPO code does not match the calculated one.'");
		Возврат Ложь
	КонецЕсли;
...
Функция СтраховойНомерПФРСоответствуетТребованиям(Знач СтраховойНомер, ТекстСообщения) Экспорт
	...
	Если ПустаяСтрока(СтрокаЦифр) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("es='El número de seguros no es rellenado';ru='Страховой номер не заполнен';en='Insurance number not filled'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрДлина(СтрокаЦифр) < 11 Тогда
		ТекстСообщения = ТекстСообщения + НСтр("es='El número de seguros es dado no totalmente';ru='Страховой номер задан неполностью';en='Insurance number is incomplete'");
		Возврат Ложь;
	КонецЕсли;
	
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрокаЦифр) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("es='El número de seguros debe consistir solamente de las cifras.';ru='Страховой номер должен состоять только из цифр.';en='The insurance number should only consist of numbers.'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если Число(Лев(СтрокаЦифр, 9)) > 1001998 Тогда
		...
		Если Остаток <> КонтрольноеЧисло Тогда
			ТекстСообщения = ТекстСообщения + НСтр("es='El número de control para el número de seguros no coincide con contado.';ru='Контрольное число для страхового номера не совпадает с рассчитанным.';en='The control number for the insurance number does not match the calculated one.'");
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
