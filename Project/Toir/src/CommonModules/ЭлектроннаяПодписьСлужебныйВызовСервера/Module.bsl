...
Функция ДобавитьПодпись(СсылкаНаОбъект, СвойстваПодписи, ИдентификаторФормы, ВерсияОбъекта) Экспорт
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ПредставлениеОшибки = НСтр("es='A la anotación de la firma ha surgido la falta:';ru='При записи подписи возникла ошибка:';en='An error occurred while writing the signature:'")
			+ Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке);
	КонецПопытки;
...
Процедура СоздатьМенеджерКриптографииЦиклНачало(Контекст, ПараметрыВыполнения)
	...
	Исключение
		...
		ТекстОшибки = НСтр("es='el servidor.';ru='Не установлена на сервере.';en='Not installed on the server.'") + " " + Контекст.СвойстваОшибки.Ошибки[0].Описание;
