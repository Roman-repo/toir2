...
Функция ЗагрузитьЧастьФайла(ИдентификаторФайла, НомерЗагружаемойЧастиФайла, ЗагружаемаяЧастьФайла, СообщениеОбОшибке) Экспорт
	...
	Если Не ЗначениеЗаполнено(ИдентификаторФайла) Тогда
		СообщениеОбОшибке = НСтр("es='No es indicado el identificador del fichero cargado. La ejecución ulterior del método es imposible. "
"Es necesario fijar para el fichero cargado el identificador único.';ru='Не указан идентификатор загружаемого файла. Дальнейшее выполнение метода невозможно."
"Необходимо для загружаемого файла назначить уникальный идентификатор.';en='The identifier of the uploaded file is not specified. Further execution of the method is not possible."
"You must assign a unique identifier for the uploaded file.'");
		ВызватьИсключение(СообщениеОбОшибке);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ЗагружаемаяЧастьФайла)
		И ТипЗнч(ЗагружаемаяЧастьФайла) <> Тип("ДвоичныеДанные") Тогда
		СообщениеОбОшибке = НСтр("es='El método no puede ser cumplido, porque los datos entregados no corresponden al tipo para la recepción de los datos.';ru='Метод не может быть выполнен, т.к. переданные данные не соответствуют типу для получения данных.';en='The method cannot be executed because the transmitted data does not match the type for receiving data.'");
		ВызватьИсключение(СообщениеОбОшибке);
	КонецЕсли;
...
Функция ВыгрузитьЧастьФайла(ИдентификаторФайла, НомерВыгружаемойЧастиФайла, СообщениеОбОшибке) Экспорт
	...
	Иначе
		СообщениеОбОшибке = НСтр("es='La parte del fichero con el número indicado no es encontrada.';ru='Часть файла с указанным номером не найдена.';en='Part of the file with the specified number was not found.'");
	КонецЕсли;
...
Функция ПодготовитьФайлДляЗагрузки(ИдентификаторФайла, СообщениеОбОшибке) Экспорт
	...
	Иначе
		ШаблонСообщения = НСтр("es='No es encontrado ni un fragmento de la sesión de la transmisión con el identificador %1. "
"Es necesario persuadirse que en los ajustes del programa son dados los parámetros "
"""el Catálogo de los ficheros temporales para Linux"" y ""el Catálogo de los ficheros temporales para Windows"".';ru='Не найден ни один фрагмент сессии передачи с идентификатором %1."
"Необходимо убедиться, что в настройках программы заданы параметры"
"""Каталог временных файлов для Linux"" и ""Каталог временных файлов для Windows"".';en='No fragments of the transfer session with identifier %1 were found."
...
"""Directory of temporary files for Linux"" and ""Directory of temporary files for Windows"".'");
	...
	Если Разархиватор.Элементы.Количество() = 0 Тогда
		...
		СообщениеОбОшибке = НСтр("es='El fichero del archivo no contiene los datos.';ru='Файл архива не содержит данных.';en='Archive file does not contain data.'");
		ВызватьИсключение(СообщениеОбОшибке);
...
Процедура ЗагрузитьДанныеXDTOВИнформационнуюБазу(ПараметрыПроцедуры, АдресХранилища) Экспорт
	...
	Исключение
		...
		СообщениеОбОшибке = НСтр("es='La falta a la carga de los datos: %1';ru='Ошибка при загрузке данных: %1';en='Error loading data:%1'");
	...
	Исключение
		Информация = ИнформацияОбОшибке();
		СообщениеОбОшибке = НСтр("es='La falta a la desaparición de los objetos temporales creados por las referencias: %1';ru='Ошибка при удалении временных объектов, созданных по ссылкам: %1';en='Error deleting temporary objects created by links:%1'");
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			СообщениеОбОшибке,
			ПодробноеПредставлениеОшибки(Информация));
		ОбменДаннымиXDTOСервер.ЗаписатьВПротоколВыполнения(КомпонентыОбмена, СообщениеОбОшибке, , , , , Истина);
...
Процедура ПроверитьБлокировкуИнформационнойБазыДляОбновления() Экспорт
	...
	Если ЗначениеЗаполнено(ОбновлениеИнформационнойБазыСлужебный.ИнформационнаяБазаЗаблокированаДляОбновления()) Тогда
		...
		ВызватьИсключение НСтр("es='La sincronización de los datos es temporalmente inaccesible en relación a la renovación de la aplicación.';ru='Синхронизация данных временно недоступна в связи с обновлением приложения.';en='Data synchronization is temporarily unavailable due to application update.'");
...
Функция СформироватьКлючСообщенияЖР(НаправлениеОбмена, ПараметрыWEBСервиса)
	...
	Если НаправлениеОбмена = "Получение" Тогда
		ШаблонКлючаСообщения = НСтр("es='La carga de los datos a través del Web-servicio %1';ru='Загрузка данных через Web-сервис %1';en='Downloading data through the web service% 1'", ОбщегоНазначения.КодОсновногоЯзыка());
	Иначе
		ШаблонКлючаСообщения = НСтр("es='La descarga de los datos a través del Web-servicio %1';ru='Выгрузка данных через Web-сервис %1';en='Uploading data through the web service% 1'", ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
