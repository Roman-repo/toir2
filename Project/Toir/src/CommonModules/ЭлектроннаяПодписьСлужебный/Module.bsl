...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий =
		НСтр("es='El relleno de la guía del Programa de la firma electrónica y la cifración.';ru='Заполнение справочника Программы электронной подписи и шифрования.';en='Filling out the directory of the electronic signature and encryption program.'");
...
Функция МенеджерКриптографии(Операция, ПоказатьОшибку = Истина, ОписаниеОшибки = "", Программа = Неопределено) Экспорт
	...
	Если Операция = "Подписание" Тогда
		ЗаголовокОшибки = НСтр("es='No consiguió firmar los datos en el servidor %1 por la causa:';ru='Не удалось подписать данные на сервере %1 по причине:';en='Failed to sign data on server %1 due to:'");
		
	ИначеЕсли Операция = "ПроверкаПодписи" Тогда
		ЗаголовокОшибки = НСтр("es='No consiguió comprobar la firma en el servidor %1 por la causa:';ru='Не удалось проверить подпись на сервере %1 по причине:';en='Failed to verify the signature on server%1 due to:'");
		
	ИначеЕсли Операция = "Шифрование" Тогда
		ЗаголовокОшибки = НСтр("es='No consiguió cifrar los datos en el servidor %1 por la causa:';ru='Не удалось зашифровать данные на сервере %1 по причине:';en='Failed to encrypt data on server %1 due to:'");
		
	ИначеЕсли Операция = "Расшифровка" Тогда
		ЗаголовокОшибки = НСтр("es='No consiguió descifrar los datos en el servidor %1 por la causa:';ru='Не удалось расшифровать данные на сервере %1 по причине:';en='Failed to decrypt data on server%1 due to:'");
		
	ИначеЕсли Операция = "ПроверкаСертификата" Тогда
		ЗаголовокОшибки = НСтр("es='No consiguió comprobar el certificado en el servidor %1 por la causa:';ru='Не удалось проверить сертификат на сервере %1 по причине:';en='Failed to validate certificate on server%1 due to:'");
		
	ИначеЕсли Операция = "ПолучениеСертификатов" Тогда
		ЗаголовокОшибки = НСтр("es='No consiguió recibir los certificados en el servidor %1 por la causa:';ru='Не удалось получить сертификаты на сервере %1 по причине:';en='Failed to get certificates on server %1 due to:'");
...
"Неверное значение параметра Операция ""%1"".';en='Function error МенеджерКриптографии. Invalid value for Operation ""%1"".'"), Операция);
	...
	Иначе
		ЗаголовокОшибки = НСтр("es='No consiguió cumplir la operación en el servidor %1 por la causa:';ru='Не удалось выполнить операцию на сервере %1 по причине:';en='The operation on server  %1 failed due to:'");
	КонецЕсли;
	...
	ИначеЕсли Программа <> Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El programa ""%1"" no es accesible o no es establecido.';ru='Программа ""%1"" не доступна или не установлена.';en='The program ""%1"" is not available or not installed.'"), Программа);
	Иначе
		ТекстОшибки = НСтр("es='Ni un de los programas no es accesible o no es establecido.';ru='Ни одна из программ не доступна или не установлена.';en='None of the programs are available or installed.'");
	КонецЕсли;
...
Функция ПолучитьСертификатПоОтпечатку(Отпечаток, ТолькоВЛичномХранилище,
	...
	Если ЗначениеЗаполнено(ПредставлениеОшибки) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El certificado no es encontrado en el servidor por la causa: "
"%1';ru='Сертификат не найден на сервере по причине:"
"%1';en='Certificate was not found on the server due to:"
"%1'")
			+ Символы.ПС,
			ПредставлениеОшибки);
	Иначе
		ТекстОшибки = НСтр("es='El certificado no es encontrado en el servidor.';ru='Сертификат не найден на сервере.';en='Certificate not found on server.'");
	КонецЕсли;
		
	Если Не Пользователи.ЭтоПолноправныйПользователь(,, Ложь) Тогда
		ТекстОшибки = ТекстОшибки + Символы.ПС + НСтр("es='Se dirijan al administrador.';ru='Обратитесь к администратору.';en='Contact administrator.'")
	КонецЕсли;
...
Процедура ПроверитьУникальностьПредставления(Представление, СертификатСсылка, Поле, Отказ) Экспорт
	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		ТекстСообщения = НСтр("es='El certificado con tal representación existe ya.';ru='Сертификат с таким представлением уже существует.';en='A certificate with this view already exists.'");
...
Функция СведенияОПодписиДляЖурналаРегистрации(ДатаПодписи, СвойстваПодписи, ЭтоОшибкаПодписания = Ложь) Экспорт
	...
	Иначе
		СведенияОПодписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La fecha de la firma: %1';ru='Дата подписи: %1';en='Date signed:% 1'"), Формат(ДатаПодписи, "ДЛФ=DT")) + Символы.ПС;
	КонецЕсли;
	
	СведенияОПодписи = СведенияОПодписи + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El certificado: por %1 "
"Quién es dado: %2 "
"Propietario: %3 "
"Es válido: con %4 por %5';ru='Сертификат: %1"
"Кем выдан: %2"
"Владелец: %3"
"Действителен: с %4 по %5';en='Certificate:%1"
"Issued by:%2"
"Owner:%3"
"Valid:from %4 to %5 '"),
...
Процедура ЗарегистрироватьПодписаниеДанныхВЖурнале(ЭлементДанных, ОписаниеОшибки = "") Экспорт
	...
	Если ЭтоОшибкаПодписания Тогда
		ИмяСобытия = НСтр("es='La firma electrónica. La falta de la firma de los datos';ru='Электронная подпись.Ошибка подписания данных';en='Electronic signature. Data signing error'",
		...
		|" + ОписаниеОшибки;
	Иначе
		ИмяСобытия = НСтр("es='La firma electrónica. La firma de los datos';ru='Электронная подпись.Подписание данных';en='Electronic signature. Data signature'",
			ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
...
Функция СертификатИзДвоичныхДанных(ДанныеСертификата) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("es='La firma electrónica. La desaparición del fichero temporal';ru='Электронная подпись.Удаление временного файла';en='Electronic signature. Delete temporary file'",
				ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура НастроитьПояснениеВводаПароля(Форма, ИмяЭлементаУсиленнаяЗащитаЗакрытогоКлюча = "", ИмяЭлементаПояснениеУсиленногоПароля = "") Экспорт
	...
	Если ЗначениеЗаполнено(ИмяЭлементаУсиленнаяЗащитаЗакрытогоКлюча) Тогда
		...
		Элемент.Заголовок = НСтр("es='La introducción y la conservación de la consigna en el programa';ru='Ввод и сохранение пароля в программе';en='Entering and saving a password in the program'");
		Элемент.Подсказка =
			НСтр("es='- Se incluye el régimen interactivo del trabajo del programa de la firma electrónica,"
" a que pide la consigna y lo permite conservar. "
"- Es desconectada la interpelación de la consigna en forma 1С:Предприятия. "
""
"Obligatoriamente para las llaves cerradas de los certificados, para que en la SO es incluida la defensa intensiva.';ru='- Включается интерактивный режим работы программы электронной подписи,"
"  при котором она запрашивает пароль и позволяет его сохранить."
"- Отключается запрос пароля в форме 1С:Предприятия."
""
"Обязательно для закрытых ключей сертификатов, для которых в ОС включена усиленная защита.';en='- The interactive mode of the electronic signature program is activated,"
"  in which she asks for a password and allows you to save it."
"- The password request in the form 1C: Enterprise is disabled."
""
"Mandatory for certificate private keys for which enhanced security is enabled on the OS.'");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ИмяЭлементаПояснениеУсиленногоПароля) Тогда
		...
		Элемент.Подсказка =
			НСтр("es='La consigna es pedida por el programa de la firma electrónica, y no el programa 1С:Предприятие, "
"puesto que para el certificado escogido es incluido el régimen ""la Introducción y la conservación de la consigna en el programa"".';ru='Пароль запрашивает программа электронной подписи, а не программа 1С:Предприятие,"
"так как для выбранного сертификата включен режим ""Ввод и сохранение пароля в программе"".';en='The password is requested by the electronic signature program, not the 1C: Enterprise program,"
"because the mode ""Entering and saving the password in the program"" is enabled for the selected certificate.'");
	КонецЕсли;
...
Функция ПроверитьПодпись(Знач КонвертSOAP, ДанныеАлгоритмаПодписания, МенеджерКриптографии) Экспорт
	...
	Иначе
		...
		Если ПодписьВерна Тогда
			ВызватьИсключение НСтр("es='La firma no es justo (SignatureValue correctamente, se distingue DigestValue).';ru='Подпись не верна (SignatureValue корректно, отличается DigestValue).';en='Signature is incorrect (SignatureValue is correct, DigestValue differs).'")
		Иначе
			ВызватьИсключение НСтр("es='La firma no es justo (SignatureValue es incorrecto.';ru='Подпись не верна (SignatureValue некорректно.';en='Signature is incorrect (SignatureValue is incorrect.'");
		КонецЕсли;
...
Функция ОбъектВнешнейКомпонентыXMLDSig()
	...
	Если ОбъектКомпоненты = Неопределено Тогда
		ВызватьИсключение НСтр("es='No consiguió conectar el componente XMLDSig exterior.';ru='Не удалось подключить внешнюю компоненту XMLDSig.';en='Failed to connect an external XMLDSig component.'");
	КонецЕсли;
...
Функция СвойстваКриптопровайдера(МенеджерКриптографии)
	...
	Если ОписаниеПрограммы = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No consiguió determinar el tipo криптопровайдера %1';ru='Не удалось определить тип криптопровайдера %1';en='Could not determine type of cryptographic provider %1'"), ИмяКриптопровайдера);
	КонецЕсли;
...
Функция GetSignOIDFromCert(ОбъектКомпоненты, СертификатКриптографииBase64)
	...
	Исключение
		ВызватьИсключение НСтр("es='La falta de la llamada del método GetSignOIDFromCert de los componente XMLDSig.';ru='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.';en='Error calling  GetSignOIDFromCert method of the XMLDSig component.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если OIDАлгоритмаПодписи = Неопределено Тогда
		ВызватьИсключение НСтр("es='La falta de la llamada del método GetSignOIDFromCert de los componente XMLDSig.';ru='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.';en='Error calling  GetSignOIDFromCert method of the XMLDSig component.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
Функция C14N(ОбъектКомпоненты, КонвертSOAP, XPath)
	...
	Попытка
		КаноникализированныйТекстXML = ОбъектКомпоненты.C14N(
			...
			XPath);
	Исключение
		ВызватьИсключение НСтр("es='La falta de la llamada del método C14N de los componente XMLDSig.';ru='Ошибка вызова метода C14N компоненты XMLDSig.';en='Error invoking method C14N of XMLDSig component.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если КаноникализированныйТекстXML = Неопределено Тогда
		ВызватьИсключение НСтр("es='La falta de la llamada del método C14N de los componente XMLDSig.';ru='Ошибка вызова метода C14N компоненты XMLDSig.';en='Error invoking method C14N of XMLDSig component.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
Функция Hash(ОбъектКомпоненты, КаноникализированныйТекстXML, OIDАлгоритмаХеширования, ТипКриптопровайдера)
	...
	Попытка
		DigestValue = ОбъектКомпоненты.Hash(
			...
			ТипКриптопровайдера);
	Исключение
		ВызватьИсключение НСтр("es='La falta de la llamada del método Hash de los componente XMLDSig.';ru='Ошибка вызова метода Hash компоненты XMLDSig.';en='Error calling the Hash method of the XMLDSig component.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если DigestValue = Неопределено Тогда
		ВызватьИсключение НСтр("es='La falta de la llamada del método Hash de los componente XMLDSig.';ru='Ошибка вызова метода Hash компоненты XMLDSig.';en='Error calling the Hash method of the XMLDSig component.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
Функция Sign(ОбъектКомпоненты, КаноникализированныйТекстXMLSignedInfo, СертификатКриптографии, ПарольДоступаКЗакрытомуКлючу)
	...
	Исключение
		ВызватьИсключение НСтр("es='La falta de la llamada del método Sign de los componente XMLDSig.';ru='Ошибка вызова метода Sign компоненты XMLDSig.';en='Error calling the Sign method of the XMLDSig component.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если SignatureValue = Неопределено Тогда
		ВызватьИсключение НСтр("es='La falta de la llamada del método Sign de los componente XMLDSig.';ru='Ошибка вызова метода Sign компоненты XMLDSig.';en='Error calling the Sign method of the XMLDSig component.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
Функция VerifySign(ОбъектКомпоненты, КаноникализированныйТекстXMLSignedInfo, SignatureValue, СертификатКриптографииBase64, ТипКриптопровайдера)
	...
	Попытка
		ПодписьВерна = ОбъектКомпоненты.VerifySign(
			...
			ТипКриптопровайдера);
	Исключение
		ВызватьИсключение НСтр("es='La falta de la llamada del método VerifySign de los componente XMLDSig.';ru='Ошибка вызова метода VerifySign компоненты XMLDSig.';en='Error calling VerifySign method of XMLDSig component.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если ПодписьВерна = Неопределено Тогда
		ВызватьИсключение НСтр("es='La falta de la llamada del método VerifySign de los componente XMLDSig.';ru='Ошибка вызова метода VerifySign компоненты XMLDSig.';en='Error calling VerifySign method of XMLDSig component.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
Процедура ЗаполнитьДополнительныеСвойстваСертификата(Форма)
	...
	Исключение
		...
		Форма.Сертификат = Неопределено;
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='A la recepción de los datos del certificado ""%1"" "
"de la base informativa ha surgido la falta: "
"%2';ru='При получении данных сертификата ""%1"""
"из информационной базы возникла ошибка:"
"%2';en='Upon receipt of certificate data ""%1"" an error occurred from the infobase:"
"%2'"),
			Сертификат,
