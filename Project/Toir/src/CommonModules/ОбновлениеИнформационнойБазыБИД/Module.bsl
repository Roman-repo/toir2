...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("es='La comprobación de las reglas de la integración con 1С:Документооборотом';ru='Проверка правил интеграции с 1С:Документооборотом';en='Checking the rules of integration with 1C: Document Management'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "1.1.6.1";
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыБИД.ПерейтиНаВерсию_1_1_6_1";
	Обработчик.Комментарий = НСтр("es='La renovación de las reglas de la integración con 1С:Документооборотом';ru='Обновление правил интеграции с 1С:Документооборотом';en='Updating integration rules with 1C: Document management'");
	...
	Пока ВыборкаПравила.Следующий() Цикл
		...
		Если ОбъектМетаданных = Неопределено Тогда
			...
			ТекстСообщения = СтрШаблон(
				НСтр("es='Es marcado a la desaparición gobernaba para el objeto no existente %1';ru='Помечено на удаление правило для несуществующего объекта %1';en='Rule marked for deletion for nonexistent object  %1'",
			...
			Для Каждого Правило Из ПравилоОбъект.ПравилаЗаполненияРеквизитовПриВыгрузке Цикл
				...
				Если ОбъектМетаданных.Реквизиты.Найти(ИмяРеквизитаОбъектаПотребителя) = Неопределено
					...
					Если Не Найден Тогда
						...
						ТекстСообщения = СтрШаблон(НСтр("es='Es limpiada la regla de la descarga para el requisito no existente %1';ru='Очищено правило выгрузки для несуществующего реквизита %1';en='Cleared upload rule for nonexistent requisite %1'"),
			...
			Для Каждого Правило Из ПравилоОбъект.ПравилаЗаполненияРеквизитовПриЗагрузке Цикл
				...
				Если ОбъектМетаданных.Реквизиты.Найти(Правило.ИмяРеквизитаОбъектаПотребителя) = Неопределено
					...
					Если Не Найден Тогда
						...
						ТекстСообщения = СтрШаблон(НСтр("es='Es limpiada la regla de la carga para el requisito no existente %1';ru='Очищено правило загрузки для несуществующего реквизита %1';en='Cleared loading rule for nonexistent requisite %1'"),
							Правило.ИмяРеквизитаОбъектаПотребителя);
						ЗаписьЖурналаРегистрации(
							ИнтеграцияС1СДокументооборот.ИмяСобытияЖурналаРегистрации(),
							УровеньЖурналаРегистрации.Ошибка,,
