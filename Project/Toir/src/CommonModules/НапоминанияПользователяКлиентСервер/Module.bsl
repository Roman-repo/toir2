...
Функция ПредставлениеВремени(Знач Время, ПолноеПредставление = Истина, ВыводитьСекунды = Истина) Экспорт
	...
	ПредставлениеСекунд = НСтр("es='; %1 segundo;; %1 segundos; %1 segundos; %1 segundos';ru=';%1 секунду;;%1 секунды;%1 секунд;%1 секунды';en=';%1 second;;%1 seconds;%1 seconds;%1 seconds'");
...
Функция ЗаменитьЕдиницуИзмеренияНаМножитель(Знач Единица)
	...
	ДопустимыеСимволы = НСтр("es='абвгдеёжзийклмнопрстуфхцчшщъыьэюя';ru='абвгдеёжзийклмнопрстуфхцчшщъыьэюя';en='абвгдеёжзийклмнопрстуфхцчшщъыьэюя'");
	...
	СловоформыНедели = СтрРазделить(НСтр("es='нед, н';ru='нед,н';en='week, w'"), ",", Ложь);
	СловоформыДня = СтрРазделить(НСтр("es='ден, el fondo, el día, дн, д';ru='ден,дне,дня,дн,д';en='ден,дне,дня,дн,д'"), ",", Ложь);
	СловоформыЧаса = СтрРазделить(НСтр("es='La hora, h';ru='час,ч';en='hour, h'"), ",", Ложь);
	СловоформыМинуты = СтрРазделить(НСтр("es='De las minas, m';ru='мин,м';en='min, m'"), ",", Ложь);
	СловоформыСекунды = СтрРазделить(НСтр("es='сек, con';ru='сек,с';en='sec, from'"), ",", Ложь);
