...
Функция ПрофильБезопасности(Знач ПараметрыАдминистрированияКластера, Знач ИмяПрофиля) Экспорт
	...
	Если ПрофилиБезопасности.Количество() <> 1 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='En el clúster de los servidores %1 no es registrado el perfil de la seguridad %2.';ru='В кластере серверов %1 не зарегистрирован профиль безопасности %2.';en='Security cluster%2 is not registered in server cluster%1.'"), ИдентификаторКластера, ИмяПрофиля);
	КонецЕсли;
...
Процедура СоздатьПрофильБезопасности(Знач ПараметрыАдминистрированияКластера, Знач СвойстваПрофиляБезопасности) Экспорт
	...
	Если ПрофилиБезопасности.Количество() = 1 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='En el clúster de los servidores %1 es registrado ya el perfil de la seguridad %2.';ru='В кластере серверов %1 уже зарегистрирован профиль безопасности %2.';en='Server cluster%1 already has security profile%2 registered.'"), ИдентификаторКластера, ИмяПрофиля);
	КонецЕсли;
...
Функция ИдентификаторИнформационнойБазы(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИнформационнойБазы) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='En el clúster de los servidores %1 no es registrada la base informativa %2.';ru='В кластере серверов %1 не зарегистрирована информационная база %2.';en='Information cluster%2 is not registered in server cluster%1.'"), ИдентификаторКластера, ПараметрыАдминистрированияИнформационнойБазы.ИмяВКластере);
	КонецЕсли;
...
Функция ИдентификаторКластера(Знач ПараметрыАдминистрированияКластера) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1.';ru='Не обнаружен кластер серверов с портом %1.';en='A server cluster with port%1 was not found.'"), ПараметрыАдминистрированияКластера.ПортКластера);
	КонецЕсли;
...
Функция ВыполнитьКоманду(Знач Шаблон, Знач ПараметрыАдминистрированияКластера, Знач ЗначенияПараметров = Неопределено)
	...
	Если Не ФайлКлиента.Существует() Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es imposible cumplir la operación de la administración del clúster de los servidores de la causa: el fichero %1 no es encontrado. "
""
"Para la administración del clúster a través del servidor de la administración (ras) es necesario establecer en "
"el ordenador dado el cliente del servidor de la administración (rac). "
"Para su instalación: "
"- los ordenadores con la SO Windows son necesarios переустановить la plataforma, habiendo establecido el componente ""la Administración del servidor 1С:Предприятия""; "
...
"To install it:"
"- For computers with Windows OS, you need to reinstall the platform by installing the component """" Administration of the 1C: Enterprise server """";"
