...
Функция ДобавитьПодменюПросмотраСпискаНовостей(
	...
	Если (ТипЗнч(ЭлементКоманднаяПанель) = Тип("ГруппаФормы")) Тогда
		...
		Если ЭлементПодменюНовости = Неопределено Тогда // Еще подменю не создавали
			...
			ЭлементПодменюНовости.Заголовок = НСтр("es='Las novedades';ru='Новости';en='News'");
		...
		Если (ТипЗнч(ЭлементПодменюНовости) = Тип("ГруппаФормы")) Тогда
			...
			Если (ТипЗнч(ТаблицаНовостей) = Тип("ДанныеФормыКоллекция")
				...
				Если (ТаблицаНовостей.Количество() > 0) Тогда
					...
					ПодменюПостоянныхНовостей.Заголовок = НСтр("es='Las submenúes de la lista de las novedades constantes';ru='Подменю списка постоянных новостей';en='Permanent news submenu'");
					...
					ПодменюНовостей.Заголовок = НСтр("es='Las submenúes de la lista de las novedades';ru='Подменю списка новостей';en='News list submenu'");
					...
					ПунктМеню.Заголовок  = НСтр("es='Todas las novedades por este tema';ru='Все новости по этой теме';en='All news on this topic'");
...
Функция ДобавитьКнопкуПросмотраСпискаНовостей(
	...
	Если (ТипЗнч(ЭлементКоманднаяПанель) = Тип("ГруппаФормы")) Тогда
		...
		КомандаНовость.Подсказка   = НСтр("es='Presionen para la apertura de la lista de las novedades de contexto';ru='Нажмите для открытия списка контекстных новостей';en='Click to open the list of contextual news.'");

		КнопкаНовостей = Форма.Элементы.Добавить("КнопкаНовостей", Тип("КнопкаФормы"), ЭлементКоманднаяПанель);
		КнопкаНовостей.Заголовок             = НСтр("es='Las novedades';ru='Новости';en='News'");
	...
	Результат = 
		НРег(
			ЧислоПрописью(
				КоличествоНовостей,
				"НП=Истина; НД=Ложь;",
				НСтр("es='La novedad, la novedad, las novedades, 0';ru='новость,новости,новостей,ж,,,,,0';en='news, news, news, ,,,,, 0'")));
	...
	Если СтандартнаяОбработка <> Ложь Тогда
		...
		Если (СпособОтображенияПанелиКонтекстныхНовостей <> "Скрыть") Тогда
			...
			Иначе
				...
				МассивСтрок = Новый Массив;
				МассивСтрок.Добавить(НСтр("es='No hay novedades';ru='Нет новостей';en='No news'"));
				Форма.Элементы.ПанельКонтекстныхНовостей_СписокНовостей.Заголовок = Новый ФорматированнаяСтрока(МассивСтрок);
				Форма.Элементы.ПанельКонтекстныхНовостей.Видимость = Ложь;
				Если Форма.Новости.ЕстьКнопкаУправленияВидимостьюПанелиКонтекстныхНовостей = Истина Тогда
					Форма.Элементы.ФормаВидимостьПанелиКонтекстныхНовостей.Видимость = Истина;
