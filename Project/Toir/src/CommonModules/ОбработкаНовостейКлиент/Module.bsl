		...
		Если НайденноеДействие <> Неопределено Тогда
			...
			ИначеЕсли НайденноеДействие.Значение = "Предупреждение" Тогда
				...
				Если ТипЗнч(Текст) = Тип("ЭлементСпискаЗначений") Тогда
					...
					Иначе
						Заголовок = НСтр("es='La información';ru='Информация';en='Information'");
					КонецЕсли;
...
//  УправляемаяФорма, Неопределено - форма с новостью.
//
...
//
// Возвращаемое значение:
//  УправляемаяФорма, Неопределено - форма со списком новостей.
//
...
Процедура ИнтернетПоддержкаИСервисы_ВключитьРаботуСНовостямиПриИзменении(Форма, Элемент) Экспорт
	...
	Если НЕ Форма.БИПВключитьРаботуСНовостями Тогда
		...
		МассивСтрок.Добавить(НСтр("es='Con insistencia';ru='Настоятельно';en='Strongly'"));
		МассивСтрок.Добавить(" ");
		МассивСтрок.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("es='No es recomendable';ru='не рекомендуется';en='Not recommended'"),
				Новый Шрифт( , , Истина))); // Жирный
		МассивСтрок.Добавить(" ");
		МассивСтрок.Добавить(НСтр("es='Desconectar la opción general de la recepción de las novedades.';ru='отключать общую опцию получения новостей.';en='disable the general option to receive news.'"));
		МассивСтрок.Добавить(Символы.ПС);
		МассивСтрок.Добавить(НСтр("es='Esto permitirá rápidamente recibir la información importante del servicio del apoyo de la firma 1С.';ru='Это позволит оперативно получать важную информацию от службы поддержки фирмы 1С.';en='This will allow you to quickly receive important information from the 1C support service.'"));
		МассивСтрок.Добавить(Символы.ПС);
		МассивСтрок.Добавить(Символы.ПС);
		МассивСтрок.Добавить(НСтр("es='¿Desconectar la opción de la recepción de las novedades?';ru='Отключить опцию получения новостей?';en='Disable the option to receive news?'"));
		ТекстВопроса = Новый ФорматированнаяСтрока(МассивСтрок);
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("Форма", Форма);
		ДополнительныеПараметры.Вставить("Элемент", Элемент);
		СписокКнопок = Новый СписокЗначений;
		СписокКнопок.Добавить("Отключить", НСтр("es='Desconectar';ru='Отключить';en='Disable'"));
		СписокКнопок.Добавить("НеОтключать", НСтр("es='Dejar incluido';ru='Оставить включенной';en='Leave On'"));
		...
		ПоказатьВопрос(
			...
			НСтр("es='La prevención';ru='Предупреждение';en='Warning'"));
	Иначе
...
Процедура КонтекстныеНовости_ОбработкаОповещения(
	...
	ИначеЕсли ИмяСобытия = "Новости. Собрать информацию об открытых окнах" Тогда
		...
		КонецПопытки;
	ИначеЕсли ИмяСобытия = "Новости. Продолжить автолистание в Панели контекстных новостей" Тогда
		// Панель контекстных новостей.
		Если Параметр = Форма.УникальныйИдентификатор Тогда
			...
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли ИмяСобытия = "Новости. Выключить анимацию иконки в Панели контекстных новостей" Тогда
		// Панель контекстных новостей.
		Если Параметр = Форма.УникальныйИдентификатор Тогда
...
//
// Параметры:
//  Форма         - УправляемаяФорма, Структура - форма-владелец, в которой должен быть определен реквизит Новости произвольного типа (со структурой) с ключами:
...
//  ИдентификаторыСобытий - Строка, Массив - Имя события (имена событий), по которым надо отбирать новости;
//  НастройкиПолученияНовостей   - Структура, Неопределено - структура с ключами:
//   * ПолучатьКатегорииНовостей - Булево - если Истина, то в реквизите формы будут также сохраняться категории новостей.
//                                  Это может быть полезно для реализации особенного отбора новостей,
//                                    который невозможно реализовать с помощью регистров сведений ОтборыПоЛентамНовостей*.
...
Процедура КонтекстныеНовости_ПоказатьНовостиТребующиеПрочтенияПриПроизвольномСобытии(
	...
	Если лкИдентификаторыСобытий.Количество() = 0 Тогда
		ИнициаторОткрытияНовостиСобытие = "Отсутствует"; // Идентификатор.
	ИначеЕсли лкИдентификаторыСобытий.Количество() = 1 Тогда
		ИнициаторОткрытияНовостиСобытие = лкИдентификаторыСобытий[0];
	Иначе // > 1
		ИнициаторОткрытияНовостиСобытие = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"Массив(%1)",
			лкИдентификаторыСобытий.Количество());
	КонецЕсли;
					...
					Попытка
						...
						Если ИнтервалСекундЧислом < 2 Тогда
							ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La demostración de las novedades importantes y muy importantes será aplazada para 2 segundos "
"(el significado, entregado en el parámetro, %1 era aumentado hasta 2)';ru='Показ важных и очень важных новостей будет отложен на 2 секунды"
"(переданное в параметре значение %1 было увеличено до 2)';en='Showing important and very important news will be delayed for 2 seconds"
"(the value passed in the parameter%1 has been increased to 2)'"),
								ИнтервалСекундЧислом);
							ПоказатьОповещениеПользователя(
								НСтр("es='Es cambiado el tiempo de la primera demostración de las novedades';ru='Изменено время первого показа новостей';en='Changed the time of the first show of news'"),
								,
								ТекстСообщения);
							ИнтервалСекундЧислом = 2;
						ИначеЕсли ИнтервалСекундЧислом > 999 Тогда
							ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La demostración de las novedades importantes y muy importantes será aplazada para 999 segundos "
"(el significado, entregado en el parámetro, %1 era reducido hasta 999)';ru='Показ важных и очень важных новостей будет отложен на 999 секунд"
"(переданное в параметре значение %1 было уменьшено до 999)';en='Showing important and very important news will be delayed by 999 seconds'"),
								ИнтервалСекундЧислом);
							ПоказатьОповещениеПользователя(
								НСтр("es='Es cambiado el tiempo de la primera demostración de las novedades';ru='Изменено время первого показа новостей';en='Changed the time of the first show of news'"),
								,
						...
						Иначе
							ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La demostración de las novedades importantes y muy importantes será aplazada para %1 segundos';ru='Показ важных и очень важных новостей будет отложен на %1 секунд';en='Showing important and very important news will be delayed for%1 seconds'"),
								ИнтервалСекундЧислом);
							ПоказатьОповещениеПользователя(
								НСтр("es='Es cambiado el tiempo de la primera demostración de las novedades';ru='Изменено время первого показа новостей';en='Changed the time of the first show of news'"),
								,
					...
					Исключение
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='No consiguió transformar el parámetro del lanzamiento (el desplazamiento de la demostración de las novedades): %1. "
"La línea del parámetro: %2. "
"Es necesario indicar el tiempo en los segundos, de 2 hasta 999';ru='Не удалось преобразовать параметр запуска (сдвиг показа новостей): %1."
"Строка параметра: %2."
"Необходимо указать время в секундах, от 2 до 999';en='Failed to convert launch parameter (news feed shift):%1."
"Parameter string:%2."
"You must specify the time in seconds, from 2 to 999'"),
							ИнтервалСекундСтрокой,
							Сред(ПараметрЗапуска, ГдеНачало, ГдеКонец - ГдеНачало + 1));
						ПоказатьОповещениеПользователя(
							НСтр("es='La falta del cambio del tiempo de la primera demostración de las novedades';ru='Ошибка изменения времени первого показа новостей';en='Error changing the time of the first show of news'"),
							,
							ТекстСообщения);
					КонецПопытки;
				Иначе
					ТекстСообщения = НСтр("es='Para el desplazamiento de la demostración de las novedades importantes y muy importantes, "
"es necesario/C entregar en el parámetro del lanzamiento "
"OtlozhitPokazNovostey=HH; "
"donde ХХ - el número de los segundos de 2 hasta 999. "
"El parámetro debe acabar por el punto y coma.';ru='Для сдвига показа важных и очень важных новостей,"
"необходимо в параметре запуска /C передать"
"ОтложитьПоказНовостей=ХХ;"
"где ХХ - число секунд от 2 до 999."
"Параметр должен заканчиваться точкой с запятой.';en='To shift the display of important and very important news,"
"need to pass in the / C start parameter"
"\ОтложитьПоказНовостей=ХХ;;"
"where XX is the number of seconds from 2 to 999."
	...
	Если Действие = "Переход по навигационной ссылке" Тогда // Идентификатор.
		...
		Для каждого ТекущийПараметр Из СписокПараметров Цикл
			Если ВРег(СокрЛП(ТекущийПараметр.Представление))=ВРег("Навигационная ссылка") Тогда
				...
				Исключение
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Había una falta al tratamiento de la acción %1 con los parámetros %2. "
"La información detallada sobre la falta: "
"%3';ru='Произошла ошибка при обработке действия %1 с параметрами %2."
"Подробная информация об ошибке:"
"%3';en='An error occurred while processing action %1 with parameters %2."
...
"%3 '"),
					...
					ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("es='БИП:Новости. El cliente';ru='БИП:Новости.Клиент';en='BIP: News. Client'"), // ИмяСобытия
						НСтр("es='Las novedades. El cliente. ОбработкаНовостейКлиент. ОбработкаНавигационнойСсылки';ru='Новости. Клиент. ОбработкаНовостейКлиент. ОбработкаНавигационнойСсылки';en='News. Client. ОбработкаНовостейКлиент. ОбработкаНавигационнойСсылки'"), // ИдентификаторШага
...
"%3 '"), // ИдентификаторШага
	...
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La acción %1 con los parámetros %2 no es tratada.';ru='Действие %1 с параметрами %2 не обрабатывается.';en='The action% 1 with parameters% 2 is not processed.'"),
			Действие,
			СписокПараметров);
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. El cliente';ru='БИП:Новости.Клиент';en='BIP: News. Client'"), // ИмяСобытия
...
Процедура ОбработкаНажатияВТекстеНовости(Новости, ДанныеСобытия, СтандартнаяОбработкаПлатформой, Форма, ЭлементФормы) Экспорт
	...
	Если СтандартнаяОбработкаПодсистемой <> Ложь Тогда
		...
		Если (ДанныеСобытия.Anchor <> Неопределено) Тогда
			...
			// Если в href передать якорь вида "#ИмяЯкоря", то в некоторых браузерах он автоматически дополнится до такого вида:
			//  "e1c://.../#ИмяЯкоря" или "http(s)://.../ИмяЯкоря".
			// В браузере Safari под MacOS ДанныеСобытия.Href слева ничем не дополняется.
			Если Лев(НавигационнаяСсылка, 1) = "#" Тогда
				ТолькоСсылка = НавигационнаяСсылка;
			Иначе
				// Попробуем определить, это ссылка на якорь или нет.
				ТолькоСсылка = "";
				Попытка
					ТолькоСсылка = ДанныеСобытия.Anchor.nameProp; // Не все браузеры поддерживают это свойство.
					ИнформацияПереходаПоСсылке.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='ДанныеСобытия.Anchor.nameProp = %1';ru='ДанныеСобытия.Anchor.nameProp = %1';en='ДанныеСобытия.Anchor.nameProp = %1'"),
							ТолькоСсылка));
				Исключение
					ТолькоСсылка = "";
					...
					ИнформацияПереходаПоСсылке.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='La falta de la recepción de DannyeSobytiya.Anchor.nameProp por la causa: "
"%1';ru='Ошибка получения ДанныеСобытия.Anchor.nameProp по причине:"
"%1';en='Error receiving ДанныеСобытия.Anchor.nameProp due to: %1'"),
							ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)));
				КонецПопытки;
			КонецЕсли;

			// В некоторых браузерах нельзя получить содержимое атрибута href без адреса базы, поэтому будем пытаться выделить имя якоря.
			// Для этого проверим - есть ли в строке соединения с ИБ и в навигационной ссылке одна и та же подстрока (адрес базы).
			// Если такая подстрока есть, то оставим только то, что после символа "#".
			Если (ТолькоСсылка = Неопределено) ИЛИ ПустаяСтрока(ТолькоСсылка) Тогда
				СтрокаСоединенияСИБ = СтрокаСоединенияИнформационнойБазы();
				ГдеПротокол = СтрНайти(ВРег(СтрокаСоединенияСИБ), ВРег("http"));
				ИнформацияПереходаПоСсылке.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='СтрокаСоединенияИнформационнойБазы () = %1';ru='СтрокаСоединенияИнформационнойБазы() = %1';en='СтрокаСоединенияИнформационнойБазы() = %1'"),
						СтрокаСоединенияСИБ));
				Если ГдеПротокол > 0 Тогда
					// Выделение части адреса из строки соединения.
					ГдеРазделительВСтрокеСоединения2 = СтрНайти(СтрокаСоединенияСИБ, "/", , ГдеПротокол, 2); // http(s)://<...>/
					ГдеРазделительВСтрокеСоединения3 = СтрНайти(СтрокаСоединенияСИБ, "/", , ГдеПротокол, 3); // http(s)://<...>/
					Если ГдеРазделительВСтрокеСоединения3 > ГдеРазделительВСтрокеСоединения2 Тогда
						ВебАдресБазыВСтрокеСоединения = Сред(
							СтрокаСоединенияСИБ,
							ГдеРазделительВСтрокеСоединения2 + 1,
							ГдеРазделительВСтрокеСоединения3 - ГдеРазделительВСтрокеСоединения2 - 1);
						ИнформацияПереходаПоСсылке.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La dirección Web de la base en StrokaSoedineniyaInformatsionnoyBazy () = %1';ru='Веб-адрес базы в СтрокаСоединенияИнформационнойБазы() = %1';en='Web address of the database in СтрокаСоединенияИнформационнойБазы() = %1'"),
								ВебАдресБазыВСтрокеСоединения));
						// Выделение части адреса из ссылки.
						ГдеРазделительВСсылке2 = СтрНайти(НавигационнаяСсылка, "/", , , 2); // http(s)://<...>/
						ГдеРазделительВСсылке3 = СтрНайти(НавигационнаяСсылка, "/", , , 3); // http(s)://<...>/
						Если ГдеРазделительВСсылке3 > ГдеРазделительВСсылке2 Тогда
							ВебАдресБазыВСсылке = Сред(
								НавигационнаяСсылка,
								ГдеРазделительВСсылке2 + 1,
								ГдеРазделительВСсылке3 - ГдеРазделительВСсылке2 - 1);
							ИнформацияПереходаПоСсылке.Добавить(
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("es='La dirección Web de la base en la Referencia = %1';ru='Веб-адрес базы в Ссылке = %1';en='Base web address in Link =% 1'"),
									ВебАдресБазыВСсылке));
							Если ВРег(ВебАдресБазыВСтрокеСоединения) = ВРег(ВебАдресБазыВСсылке) Тогда
								ГдеРешетка = СтрНайти(НавигационнаяСсылка, "#", , ГдеРазделительВСсылке2);
								Если ГдеРешетка > 0 Тогда
									ТолькоСсылка = Прав(НавигационнаяСсылка, СтрДлина(НавигационнаяСсылка) - ГдеРешетка + 1);
								КонецЕсли
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
			...
			ИнформацияПереходаПоСсылке.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Solamente la referencia sin dirección de la base = %1';ru='Только ссылка без адреса базы = %1';en='Only link without base address =%1'"),
					ТолькоСсылка));

			// Возможно наличие "about:blank" в начале. Удалить "about:blank".
			СтрокаДляПоиска = "about:blank";
			Если СтрНачинаетсяС(НРег(ТолькоСсылка), СтрокаДляПоиска) Тогда
				ТолькоСсылка = Прав(ТолькоСсылка, СтрДлина(ТолькоСсылка) - СтрДлина(СтрокаДляПоиска));
				ИнформацияПереходаПоСсылке.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Solamente la referencia sin about:blank = %1';ru='Только ссылка без about:blank = %1';en='Only link without about: blank =%1'"),
						ТолькоСсылка));
			КонецЕсли;

			Если НЕ ПустаяСтрока(ТолькоСсылка) И (Лев(ТолькоСсылка, 1) = "#") Тогда
				ПродолжитьОбработку = Ложь;
				ИдентификаторЭлемента = Прав(ТолькоСсылка, СтрДлина(ТолькоСсылка) - 1);
				Попытка
					ИнформацияПереходаПоСсылке.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='La tentativa de la búsqueda del elemento con el identificador %1';ru='Попытка поиска элемента с идентификатором %1';en='Attempt to search for item ID%1'"),
				...
				Исключение
					...
					ОшибкиПереходаПоСсылке.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='La falta de la recepción del elemento del ancla con el identificador %1 por la causa: "
"%2';ru='Ошибка получения элемента якоря с идентификатором %1 по причине:"
"%2';en='Error retrieving anchor element with identifier%1 due to:"
"%2'"),
							ИдентификаторЭлемента,
							ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)));
				КонецПопытки;
				...
				Если ЭлементЯкорь <> Неопределено Тогда
					Попытка
						ИнформацияПереходаПоСсылке.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La tentativa del tránsito al elemento con el identificador %1';ru='Попытка перехода к элементу с идентификатором %1';en='Attempt to jump to item ID%1'"),
								ИдентификаторЭлемента));
					...
					Исключение
						...
						ОшибкиПереходаПоСсылке.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La falta del tránsito al ancla con el identificador %1 por la causa: "
"%2';ru='Ошибка перехода к якорю с идентификатором %1 по причине:"
"%2';en='Error navigating to anchor with identifier%1 due to:"
"%2 '"),
								ИдентификаторЭлемента,
								ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)));
					КонецПопытки;
				...
				Если ОшибкиПереходаПоСсылке.Количество() > 0 Тогда
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Había una falta durante el tránsito por la referencia %1. "
"La información detallada sobre la falta: "
""
""
"%2 %3';ru='Произошла ошибка при переходе по ссылке %1."
"Подробная информация об ошибке:"
"%2"
""
"%3';en='An error occurred while following the link%1."
"Details of the error:"
"%2"
""
"%3 '"),
					...
					ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("es='БИП:Новости. El cliente';ru='БИП:Новости.Клиент';en='BIP: News. Client'"), // ИмяСобытия
						НСтр("es='Las novedades. El cliente. ОбработкаНовостейКлиент. ПерейтиПоИнтернетСсылке';ru='Новости. Клиент. ОбработкаНовостейКлиент. ПерейтиПоИнтернетСсылке';en='News. Client. ОбработкаНовостейКлиент. ПерейтиПоИнтернетСсылке'"), // ИдентификаторШага
						"Ошибка", // УровеньЖурналаРегистрации строкой
						"", // ОбъектМетаданных строкой
						НавигационнаяСсылка, // Данные
						ТекстСообщения, // Комментарий
						ОбработкаНовостейКлиентПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
				КонецЕсли;
		...
		Если ПродолжитьОбработку = Истина Тогда
			...
			Иначе
				СтандартнаяОбработкаПлатформой = Ложь;
				ТекстСообщения = НСтр("es='La referencia desconocida: %НавигационнаяСсылка del %';ru='Неизвестная ссылка: %НавигационнаяСсылка%';en='Unknown link:%НавигационнаяСсылка%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НавигационнаяСсылка%", НавигационнаяСсылка);
				ПоказатьПредупреждение(
					, // ОписаниеОповещенияОЗавершении
					ТекстСообщения,
					0,
					НСтр("es='La falta';ru='Ошибка';en='Error '")); 
			КонецЕсли;
		КонецЕсли;
...
//  Форма      - УправляемаяФорма - форма, в которой произошло оповещение.
//
...
Процедура ПерейтиПоИнтернетСсылке(ИнтернетСсылка) Экспорт
	...
	Исключение
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Había una falta durante el tránsito por la referencia %1. "
"La información detallada sobre la falta: "
"%2';ru='Произошла ошибка при переходе по ссылке %1."
"Подробная информация об ошибке:"
"%2';en='An error occurred while following the link %1."
"Details of the error:"
"%2'"),
			ИнтернетСсылка,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
//
// Параметры:
//  Форма   - УправляемаяФорма - форма, в которой необходимо обработать событие Нажатие;
	...
	Иначе
		Для Каждого ТекущаяФорма Из ПараметрыАвтолистания Цикл
			Если ТекущаяФорма.Значение <= ТекущаяДатаСеансаДляПроверки Тогда
				Оповестить("Новости. Продолжить автолистание в Панели контекстных новостей", ТекущаяФорма.Ключ);
	...
	Иначе
		...
		Пока БылиУдаления = Истина Цикл
			...
			Для Каждого ТекущаяФорма Из ПараметрыАнимированныхИконок Цикл
				...
				Если ТекущаяФорма.Значение <= ТекущаяДатаСеансаДляПроверки Тогда
					Оповестить("Новости. Выключить анимацию иконки в Панели контекстных новостей", ТекущаяФорма.Ключ);
					БылиУдаления = Истина;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
...
//
// Параметры:
//  Форма     - УправляемаяФорма - форма, в которой необходимо изменить видимость панели контекстных новостей;
...
//
// Параметры:
//  Форма - УправляемаяФорма - форма, которую надо подключить / отключить от глобальных обработчиков.
//
Процедура ПанельКонтекстныхНовостей_УправлениеГлобальнымиОбработчиками(Форма)
	...
	Если (Форма.Новости.КоличествоНовостейДляПанелиКонтекстныхНовостей > 0)
