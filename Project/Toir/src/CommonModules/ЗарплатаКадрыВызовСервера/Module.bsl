...
Функция ВосстановитьНачальныеЗначения(ИменаОбъектовМетаданных, ИдентификаторФормы) Экспорт
	...
	Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		ИдентификаторФормы, "ЗарплатаКадры.УстановитьНачальныеЗначения", ПараметрыЗадания, НСтр("es='La instalación de los significados iniciales';ru='Установка начальных значений';en='Initial values setting'"));
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta a la anotación del empleado %1. Probablemente, los datos del empleado son redactados por otro usuario';ru='Ошибка при записи сотрудника %1. Возможно, данные сотрудника редактируются другим пользователем';en='Error while writing employee %1. The employee data may be being edited by another user.'"), Выборка.Наименование);
...
Процедура УдалитьРКПоОрганизации(Организация)
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta a la anotación de la subdivisión %1. Probablemente, las subdivisiones dadas son redactadas por otro usuario';ru='Ошибка при записи подразделения %1. Возможно, данные подразделения редактируются другим пользователем';en='Error while writing unit%1. Perhaps these units are being edited by another user.'"), Выборка.Наименование);
			ВызватьИсключение ТекстИсключения;
		КонецПопытки;
		Объект.Записать();
	КонецЦикла;
