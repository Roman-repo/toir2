...
Функция МожноЛиЗанятьФайл(ДанныеФайла, СтрокаОшибки = "") Экспорт
	...
	Если ДанныеФайла.ПометкаУдаления = Истина Тогда
		СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No es posible ocupar el fichero ""%1"", "
"porque él es marcado a la desaparición.';ru='Нельзя занять файл ""%1"","
"т.к. он помечен на удаление.';en='Can not occupy file ""%1"", because it is marked for deletion. '"),
	...
	Если Не Результат Тогда
		...
		ДатаЗаема = ?(ЗначениеЗаполнено(ДанныеФайла.ДатаЗаема), 
			" " + НСтр("es='Con';ru='с';en='from'") + " " + Формат(ДанныеФайла.ДатаЗаема, "ДЛФ=ДВ"), "");
		
		СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El fichero ""%1"" "
"es ocupado ya para la redacción por el usuario "
"""%2"" %3.';ru='Файл ""%1"""
"уже занят для редактирования пользователем"
"""%2""%3.';en='File ""%1"" "
"already inaccessible for editing by user"
"""%2""%3. '"),
