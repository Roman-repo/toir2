...
Функция НастройкиПоУмолчанию()
	...
	НастройкиПодсистемы.Вставить("РасположениеКомандыСнятияБлокировки",
		НСтр("es='El bloqueo posible es quitado también más tarde en la sección <b> por la Administración - el Servicio </b>.';ru='Блокировку также можно снять позднее в разделе <b>Администрирование - Обслуживание</b>.';en='You can also remove the lock later in the <b> Administration - Maintenance </b> section.'"));
...
Функция ВыполнитьРегламентноеЗаданиеВручную(Знач Задание) Экспорт
	...
	Если СвойстваПоследнегоФоновогоЗадания <> Неопределено
	   И СвойстваПоследнегоФоновогоЗадания.Состояние = СостояниеФоновогоЗадания.Активно Тогда
		...
		КонецЕсли;
	Иначе
		НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El lanzamiento a mano: %1';ru='Запуск вручную: %1';en='Manual launch:%1'"), ПредставлениеРегламентногоЗадания(Задание));
...
Функция ТекстНеОпределено() Экспорт
	...
	Возврат НСтр("es='<no es determinado>';ru='<не определено>';en='<не определено>'");
...
Процедура ОтменитьФоновоеЗадание(Идентификатор) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("es='La tarea de fondo no es encontrada en el servidor.';ru='Фоновое задание не найдено на сервере.';en='Background task not found on server.'");
	КонецЕсли;
	
	Если ФоновоеЗадание.Состояние <> СостояниеФоновогоЗадания.Активно Тогда
		ВызватьИсключение НСтр("es='La tarea no se cumple, de ello no es posible anular.';ru='Задание не выполняется, его нельзя отменить.';en='The task is not completed, it cannot be canceled.'");
	КонецЕсли;
