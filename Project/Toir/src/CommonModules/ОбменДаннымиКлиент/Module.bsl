...
Процедура ФормаНастройкиПередЗакрытием(Отказ, Форма, ЗавершениеРаботы) Экспорт
	...
	ТекстВопроса = НСтр("es='Los datos eran cambiados. ¿Cerrar la forma sin conservación de los cambios?';ru='Данные были изменены. Закрыть форму без сохранения изменений?';en='Data has been changed. Close form without saving changes?'");
...
Процедура УдалитьНастройкуСинхронизации(Знач УзелИнформационнойБазы) Экспорт
	...
	Если ОбменДаннымиВызовСервера.УзелЯвляетсяГлавным(УзелИнформационнойБазы) Тогда
		ТекстПредупреждения = НСтр("es='Se puede cumplir la desconexión de la base informativa del nudo principal "
"por medio del parámetro del lanzamiento конфигуратора/ResetMasterNode.';ru='Отключение информационной базы от главного узла можно выполнить"
"с помощью параметра запуска конфигуратора /ResetMasterNode.';en='Can disable the infobase from the main node"
"using the configurator launch option /ResetMasterNode. '");
...
Функция ЗаголовкиГиперссылокЗагрузкиДанных() Экспорт
	...
	Структура = Новый Структура;
	Структура.Вставить("Неопределено",               НСтр("es='La recepción de los datos no se cumplía';ru='Получение данных не выполнялось';en='Data retrieval failed'"));
	Структура.Вставить("Ошибка",                     НСтр("es='La recepción de los datos no se cumplía';ru='Получение данных не выполнялось';en='Data retrieval failed'"));
	Структура.Вставить("ВыполненоСПредупреждениями", НСтр("es='No consiguió recibir los datos';ru='Не удалось получить данные';en='Failed to get data.'"));
	Структура.Вставить("Успех",                      НСтр("es='Los datos son recibidos con las prevenciones';ru='Данные получены с предупреждениями';en='Data received with warnings'"));
	Структура.Вставить("Выполнение",                 НСтр("es='Los datos son con éxito recibidos';ru='Данные успешно получены';en='Data successfully received'"));
	
	Структура.Вставить("Предупреждение_СообщениеОбменаБылоРанееПринято", НСтр("es='No hay nuevos datos para la recepción';ru='Нет новых данных для получения';en='No new data to receive'"));
	Структура.Вставить("Ошибка_ТранспортСообщения",                      НСтр("es='No hay nuevos datos para la recepción';ru='Нет новых данных для получения';en='No new data to receive'"));
...
Функция ЗаголовкиГиперссылокВыгрузкиДанных() Экспорт
	...
	Структура = Новый Структура;
	Структура.Вставить("Неопределено", НСтр("es='El envío de los datos no se cumplía';ru='Отправка данных не выполнялась';en='Data was not sent'"));
	Структура.Вставить("Ошибка",       НСтр("es='El envío de los datos no se cumplía';ru='Отправка данных не выполнялась';en='Data was not sent'"));
	Структура.Вставить("Успех",        НСтр("es='No consiguió enviar los datos';ru='Не удалось отправить данные';en='Failed to send data.'"));
	Структура.Вставить("Выполнение",   НСтр("es='Los datos son con éxito enviados';ru='Данные успешно отправлены';en='Data sent successfully'"));
	
	Структура.Вставить("Предупреждение_СообщениеОбменаБылоРанееПринято", НСтр("es='Los datos son enviados con las prevenciones';ru='Данные отправлены с предупреждениями';en='Data sent with alerts'"));
	Структура.Вставить("ВыполненоСПредупреждениями",                     НСтр("es='Los datos son enviados con las prevenciones';ru='Данные отправлены с предупреждениями';en='Data sent with alerts'"));
	Структура.Вставить("Ошибка_ТранспортСообщения",                      НСтр("es='Los datos son enviados con las prevenciones';ru='Данные отправлены с предупреждениями';en='Data sent with alerts'"));
...
Процедура ОбработчикВыбораФайловогоКаталога(Объект, Знач ИмяСвойства, СтандартнаяОбработка = Ложь, Знач ПараметрыДиалога = Неопределено, ОповещениеЗавершения = Неопределено) Экспорт
	...
	УмолчанияДиалога.Вставить("Заголовок", НСтр("es='Indiquen el catálogo';ru='Укажите каталог';en='Specify a directory'") );
	
	УстановитьЗначенияСтруктурыПоУмолчанию(ПараметрыДиалога, УмолчанияДиалога);
	
	ТекстПредупреждения = НСтр("es='Para la operación dada es necesario establecer la ampliación para el cliente web 1С:Предприятие.';ru='Для данной операции необходимо установить расширение для веб-клиента 1С:Предприятие.';en='For this operation, you must install the extension for the 1C: Enterprise web client.'");
...
Процедура ОбработчикВыбораФайла(Объект, Знач ИмяСвойства, СтандартнаяОбработка = Ложь, Знач ПараметрыДиалога = Неопределено, ОповещениеЗавершения = Неопределено) Экспорт
	...
	УмолчанияДиалога.Вставить("Заголовок",                   НСтр("en='selected file';es='Escojan el fichero';ru='Выберите файл'"));
...
Процедура ВыбратьИСохранитьФайлНаКлиентеПослеПодключенияРасширенияРаботыСФайлами(Подключено, ДополнительныеПараметры) Экспорт
	...
	УмолчанияДиалога = Новый Структура;
	УмолчанияДиалога.Вставить("Заголовок",               НСтр("es='Escojan el fichero para la conservación';ru='Выберите файл для сохранения';en='Select a file to save'"));
	УмолчанияДиалога.Вставить("МножественныйВыбор",      Ложь);
	УмолчанияДиалога.Вставить("ПредварительныйПросмотр", Ложь);
...
Процедура ОткрытьИнструкциюКакИзменитьПарольСинхронизацииДанных(Знач АдресДляВосстановленияПароляУчетнойЗаписи) Экспорт
	...
	Если ПустаяСтрока(АдресДляВосстановленияПароляУчетнойЗаписи) Тогда
		...
		ПоказатьПредупреждение(, НСтр("es='La dirección de la instrucción para la reconstitución de la consigna de la anotación de descuento no es dada.';ru='Адрес инструкции для восстановления пароля учетной записи не задан.';en='The address for the account password reset instruction is not set.'"));
...
Функция ОткрытьФормуДополненияВыгрузкиВсеДокументы(Знач ДополнениеВыгрузки, Знач Владелец=Неопределено, Знач Уникальность=Неопределено, Знач Окно=Неопределено) Экспорт
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("es='La adición de los documentos para el envío';ru='Добавление документов для отправки';en='Adding documents to send'") );
