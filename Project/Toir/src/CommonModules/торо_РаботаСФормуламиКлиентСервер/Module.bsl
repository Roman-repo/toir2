...
Функция ПроверитьФормулу(Формула, Операнды, Знач Поле = "", Знач СообщениеОбОшибке = "", СтроковаяФормула = Ложь,
	...
	Если ЗначениеЗаполнено(Формула) Тогда
		...
		Попытка
			...
			Если СтроковаяФормула Тогда
				...
				Если ОтсутствиеРазделителей > 0 Тогда
					Если ВыводитьСообщения Тогда
						ТекстСообщения = НСтр("es='En la fórmula son descubiertas las faltas. Entre los operandos debe asistir el operador o разделитель';ru='В формуле обнаружены ошибки. Между операндами должен присутствовать оператор или разделитель';en='There are errors in the formula. There must be an operator or delimiter between operands'");
			...
			Если ДополнительныеПараметры <> Неопределено И ДополнительныеПараметры.Свойство("ТипРезультата") Тогда
				...
				Если Не ТипРезультатаРасчетаПравильный Тогда
					Если ВыводитьСообщения Тогда
						ШаблонСообщения = НСтр("es='En la fórmula son descubiertas las faltas. El resultado del cálculo debe ser el tipo %1';ru='В формуле обнаружены ошибки. Результат расчета должен быть типа %1';en='There are errors in the formula. The result of the calculation must be of type%1'");
		...
		Исключение
			...
			Если ВыводитьСообщения Тогда
				ТекстСообщения = ?(ЗначениеЗаполнено(СообщениеОбОшибке) ,СообщениеОбОшибке, НСтр("es='En la fórmula son descubiertas las faltas. Comprueben la fórmula. Las fórmulas deben componerse por las reglas de la escritura de las expresiones en la lengua empotrada 1С:Предприятия.';ru='В формуле обнаружены ошибки. Проверьте формулу. Формулы должны составляться по правилам написания выражений на встроенном языке 1С:Предприятия.';en='There are errors in the formula. Check the formula. Formulas should be prepared according to the rules for writing expressions in the embedded language 1C: Enterprise.'"));
				#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,, Поле, ПутьКДанным);
				#Иначе
					ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, Поле, ПутьКДанным);
