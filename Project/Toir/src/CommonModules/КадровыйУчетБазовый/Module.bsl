		...
		Если ЭлементМассиваПолей <> "ФизическоеЛицо" Тогда
			...
			ИначеЕсли РеквизитыСправочникаФизическиеЛица.Найти(ЭлементМассиваПолей) = Неопределено И ВРег(ЭлементМассиваПолей) <> "НАИМЕНОВАНИЕ"  И ВРег(ЭлементМассиваПолей) <> "КОД" Тогда
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("es='La guía ""las Personas físicas"" no contiene el requisito ""%1""';ru='Справочник ""Физические лица"" не содержит реквизита ""%1""';en='Directory ""Individuals"" does not contain the requisite ""%1""'"),
									ЭлементМассиваПолей);
	...
	Для Каждого ЭлементМассиваПолей Из КадровыеДанные Цикл
		...
		ИначеЕсли РеквизитыСправочникаСотрудники.Найти(ЭлементМассиваПолей) = Неопределено 
			...
			И ВРег(ЭлементМассиваПолей) <> "КОД" Тогда
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La guía ""los Empleados"" no contiene el requisito ""%1""';ru='Справочник ""Сотрудники"" не содержит реквизита ""%1""';en='Directory ""Employees"" does not contain the requisite ""%1""'"),
					ЭлементМассиваПолей);
		Иначе
...
Функция ОписанияСоставаНачисленийПоВременнойТаблице(МенеджерВременныхТаблиц, ИмяВТСотрудникиПериоды, ИмяПоляПериод, ИмяПоляСотрудник) Экспорт
	...
	Пока ВыборкаПоСотрудникам.Следующий() Цикл
		...
		Пока ВыборкаПоПериодам.Следующий() Цикл
			...
			ОписаниеСостава.ПоказательОкладТариф = НСтр("es='El sueldo';ru='Оклад';en='Salary'");
	...
	Пока ДанныеСотрудников.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ДанныеСотрудников.ДатаПриема)
			...
			Исключение
				...
				ШаблонСообщения = НСтр("es='A la creación de las órdenes profesionales no consiguió pasar la orden del despido del empleado %1, es necesario cumplir la realización en el régimen de mano.';ru='При создании кадровых приказов не удалось провести приказ об увольнении сотрудника %1, необходимо выполнить проведение в ручном режиме.';en='When creating personnel orders, the order to dismiss employee%1 failed, it is necessary to carry out the manual mode.'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ДанныеСотрудников.Сотрудник);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ДокументПриемНаРаботу.Ссылка);
		...
		Если ЗначениеЗаполнено(ДанныеСотрудников.ДатаУвольнения)
			...
			Исключение
				...
				ШаблонСообщения = НСтр("es='A la creación de las órdenes profesionales no consiguió pasar la orden de la recepción del empleado %1, es necesario cumplir la realización en el régimen de mano.';ru='При создании кадровых приказов не удалось провести приказ о приеме сотрудника %1, необходимо выполнить проведение в ручном режиме.';en='When creating personnel orders, it was not possible to carry out an order to accept employee%1, it is necessary to carry out the manual mode.'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ДанныеСотрудников.Сотрудник);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ДокументУвольнение.Ссылка);
