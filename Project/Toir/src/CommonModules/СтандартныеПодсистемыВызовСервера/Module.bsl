	...
	Если Не ВходВОбластьДанных
	 ...
	 Или ВходВОбластьДанных
	   И Не ПравоДоступа("АдминистрированиеДанных", Метаданные) Тогда
		...
		ВызватьИсключение НСтр("es='Es insuficientemente derecho para el trabajo con la lógica desconectada del trabajo del sistema.';ru='Недостаточно прав для работы с отключенной логикой работы системы.';en='Not enough rights to work with disabled system logic.'");
	КонецЕсли;
...
Функция ЗаписатьОшибкуВЖурналРегистрацииПриЗапускеИлиЗавершении(ПрекратитьРаботу, Знач Событие, Знач ТекстОшибки) Экспорт
	...
	Если Событие = "Запуск" Тогда
		ИмяСобытия = НСтр("es='El lanzamiento del programa';ru='Запуск программы';en='Program launch'", ОбщегоНазначения.КодОсновногоЯзыка());
		Если ПрекратитьРаботу Тогда
			НачалоОписанияОшибки = НСтр("es='Ha surgido la situación excepcional durante el lanzamiento del programa. El lanzamiento del programa es mal acabado.';ru='Возникла исключительная ситуация при запуске программы. Запуск программы аварийно завершен.';en='An exception occurred while starting the program. The program started abnormally.'");
		Иначе
			НачалоОписанияОшибки = НСтр("es='Ha surgido la situación excepcional durante el lanzamiento del programa.';ru='Возникла исключительная ситуация при запуске программы.';en='An exception occurred while starting the program.'");
		КонецЕсли;
	Иначе
		ИмяСобытия = НСтр("es='La terminación del programa';ru='Завершение программы';en='Program completing'", ОбщегоНазначения.КодОсновногоЯзыка());
		НачалоОписанияОшибки = НСтр("es='Ha surgido la situación excepcional a la terminación del programa.';ru='Возникла исключительная ситуация при завершении программы.';en='An exception occurred while completing the program.'");
	КонецЕсли;
