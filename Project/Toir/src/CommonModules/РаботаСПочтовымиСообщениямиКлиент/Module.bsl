...
Процедура СоздатьНовоеПисьмо(ПараметрыОтправкиПисьма = Неопределено, ОповещениеОЗакрытииФормы = Неопределено) Экспорт
	...
	Иначе
		...
		Иначе
			ТекстСообщения = НСтр("es='Para el envío de la carta es necesario el ajuste de la anotación de descuento del correo electrónico. "
"Se dirijan al administrador.';ru='Для отправки письма требуется настройка учетной записи электронной почты."
"Обратитесь к администратору.';en='Setting up an email account is required to send an email."
...
"%1';ru='Проверка параметров учетной записи завершилась с ошибками:"
"%1';en='Checking account settings failed with errors:"
"%1'"), СообщениеОбОшибке ),,
			НСтр("es='La comprobación de la anotación de descuento';ru='Проверка учетной записи';en='Account checking'"));
	Иначе
		ПоказатьПредупреждение(ОбработчикРезультата, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La comprobación de la anotación de descuento';ru='Проверка учетной записи';en='Account checking'"),
			ДополнительноеСообщение),,
