...
Процедура РазрешитьРаботуСНовостями(КонтекстВыполнения = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
			ТекстСообщения = НСтр("es='РазрешитьРаботуСНовостями: en la sesión dividida no es posible dirigir el trabajo con las novedades.';ru='РазрешитьРаботуСНовостями: в разделенном сеансе нельзя управлять работой с новостями.';en='РазрешитьРаботуСНовостями: in a split session, you can’t control how you work with news.'");
...
Процедура ЗапретитьРаботуСНовостями(КонтекстВыполнения = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		// Если вошли в область данных, то выдать исключение.
		Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
			ТекстСообщения = НСтр("es='ЗапретитьРаботуСНовостями: en la sesión dividida no es posible dirigir el trabajo con las novedades.';ru='ЗапретитьРаботуСНовостями: в разделенном сеансе нельзя управлять работой с новостями.';en='ЗапретитьРаботуСНовостями: in a split session, you can’t control how you work with news.'");
			ВызватьИсключение ТекстСообщения;
		КонецЕсли;
	КонецЕсли;
...
//
// Параметры:
//  ВыбранныеПользователи - Массив, СправочникСсылка.Пользователи, Неопределено - Массив пользователей или единственный пользователь,
//                          для которых необходимо установить состояния новостей;
...
Процедура УстановитьСостояниеНовостейДляПользователей(
	...
	ТекстСообщения = НСтр("es='УстановитьСостояниеНовостейДляПользователей, el comienzo del trabajo. "
"МассивПользователей:"
"	%1 "
"StrukturaOtborov:"
"	%2 "
"ZnacheniyaSostoyany:"
"	%3 "
"opción funcional Es permitida el trabajo con las novedades: %4';ru='УстановитьСостояниеНовостейДляПользователей, начало работы."
"МассивПользователей:"
"	%1"
"СтруктураОтборов:"
"	%2"
"ЗначенияСостояний:"
"	%3"
"Функциональная опция Разрешена работа с новостями: %4';en='УстановитьСостояниеНовостейДляПользователей, work start."
"МассивПользователей:"
"	%1"
"СтруктураОтборов:"
"	%2"
"ЗначенияСостояний:"
"	%3                                          Functional option                         Work with news enabled::             %4'")
		+ Символы.ПС;
	...
	Если (ТипЗнч(МассивПользователей) <> ТипМассив)
		...
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%1", НСтр("es='Por todos';ru='По всем';en='For all'"));
	Иначе
	...
	Иначе
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%2", НСтр("es='Por todos';ru='По всем';en='For all'"));
	КонецЕсли;
	...
	Иначе
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%3", НСтр("es='Es entregado los significados del tipo excelente de la Estructura';ru='Передано значения типа, отличного от Структуры';en='Transmitted values of a type other than Structure'"));
	КонецЕсли;
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
		НСтр("es='Las novedades. El servicio y el reglamento. УстановитьСостояниеНовостейДляПользователей';ru='Новости. Сервис и регламент. УстановитьСостояниеНовостейДляПользователей';en='News. Service and regulations. УстановитьСостояниеНовостейДляПользователей'"), // ИдентификаторШага
	...
	Если (ТипЗнч(ЗначенияСостояний) = ТипСтруктура)
		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			...
			Если НЕ ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() Тогда
				ТекстСообщения = НСтр("es='УстановитьСостояниеНовостейДляПользователей, la falta de la ejecución: en el trabajo en el modelo del servicio poner en marcha la recepción de las novedades se a mano puede solamente a incluido разделителях. "
"La línea de la conexión: %СтрокаСоединенияИнформационнойБазы del % "
"de TekuschiyRezhimZapuska: %ТекущийРежимЗапуска del % "
"Es la configuración dividida: %ЭтоРазделеннаяКонфигурация del % "
"la División es incluida: %РазделениеВключено del % "
"Es accesible el uso de los datos divididos: %ДоступноИспользованиеРазделенныхДанных del % "
"la Sesión es puesta en marcha sin разделителей: %СеансЗапущенБезРазделителей del % "
"la Configuración (metadatos): %Метаданные_Имя del %, %Метаданные_Версия del % "
"la Configuración (redefinición): %Переопределение_Имя del %, %Переопределение_Версия del % "
"la Plataforma: %Платформа_Версия del % "
"la opción Funcional Es permitida el trabajo con las novedades: %ОбработкаНовостейПовтИсп_РазрешенаРаботаСНовостями% "
"la opción Funcional Es permitida el trabajo con las novedades a través del Internet: %ОбработкаНовостейПовтИсп_РазрешенаРаботаСНовостямиЧерезИнтернет% "
"el usuario ИБ Corriente:"
"	El Nombre: %ТекущийПользователь_Имя del %"
"	УИН: %ТекущийПользователь_УникальныйИдентификатор%"
"	La División de los datos:"
"		%ТекущийПользователь_РазделениеДанных del % "
"el Usuario: %ОбработкаНовостейПовтИсп_ПолучитьИмяПользователяИБ%';ru='УстановитьСостояниеНовостейДляПользователей, ошибка выполнения: при работе в модели сервиса запускать получение новостей вручную можно только при включенных разделителях."
...
"Пользователь: %ОбработкаНовостейПовтИсп_ПолучитьИмяПользователяИБ%';en='УстановитьСостояниеНовостейДляПользователей, execution error: when working in the service model, it is possible to start receiving news manually only with separators enabled."
"Connection string: %СтрокаСоединенияИнформационнойБазы%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска%"
"This is a split configuration: %ЭтоРазделеннаяКонфигурация%"
"Separation included: %РазделениеВключено%"
"Use of shared data is available: %ДоступноИспользованиеРазделенныхДанных%"
"Session started without separators: %СеансЗапущенБезРазделителей%"
...
"Functional option Allowed to work with news : %ОбработкаНовостейПовтИсп_РазрешенаРаботаСНовостями%"
"Functional option Allowed to work with news via the Internet: %ОбработкаНовостейПовтИсп_РазрешенаРаботаСНовостямиЧерезИнтернет%"
"Current IB user:"
"	Name: %ТекущийПользователь_Имя%"
"	UIN: %ТекущийПользователь_УникальныйИдентификатор%"
"	Data separation:"
"		%ТекущийПользователь_РазделениеДанных%"
"User: %ОбработкаНовостейПовтИсп_ПолучитьИмяПользователяИБ%""'")
					+ Символы.ПС;
		...
		Если МассивПользователей.Количество() = 0 Тогда
			ТекстСообщения = НСтр("es='La falta de la ejecución: no hay a usuarios, para que sería necesario establecer los estados de las novedades. "
"Se puede establecer los estados de las novedades solamente para los usuarios con el papel de ChtenieNovostey o PolnyePrava';ru='Ошибка выполнения: нет пользователей, для которых бы следовало установить состояния новостей."
"Состояния новостей можно установить только для пользователей с ролью ЧтениеНовостей или ПолныеПрава';en='Runtime error: There are no users for whom to set the status of the news."
"News states can only be set for users with the ЧтениеНовостей or ПолныеПрава role'");
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
				НСтр("es='Las novedades. El servicio y el reglamento. УстановитьСостояниеНовостейДляПользователей';ru='Новости. Сервис и регламент. УстановитьСостояниеНовостейДляПользователей';en='News. Service and regulations. УстановитьСостояниеНовостейДляПользователей'") , // ИдентификаторШага
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='УстановитьСостояниеНовостейДляПользователей, la terminación. "
"Es tratado los usuarios: %1 "
"Es tratado las novedades: %2';ru='УстановитьСостояниеНовостейДляПользователей, завершение."
"Обработано пользователей: %1"
"Обработано новостей: %2';en='УстановитьСостояниеНовостейДляПользователей, completion."
"User processed:%1"
"Processed news:%2'")
				+ Символы.ПС,
	...
	Иначе
		...
		ТекстСообщения = НСтр("es='УстановитьСостояниеНовостейДляПользователей, la falta de la ejecución: en calidad de los significados del relleno es entregado el significado del tipo excelente de la Estructura, o no son rellenados los ajustes del relleno (no es indicado ni una llave de la estructura). "
"Los nombres posibles de las llaves coinciden con los nombres de los recursos del registro de las noticias de SostoyaniyaNovostey';ru='УстановитьСостояниеНовостейДляПользователей, ошибка выполнения: в качестве значений заполнения передано значение типа, отличного от Структура, либо не заполнены настройки заполнения (не указано ни одного ключа структуры)."
"Возможные имена ключей совпадают с наименованиями ресурсов регистра сведений СостоянияНовостей';en='УстановитьСостояниеНовостейДляПользователей, execution error: a type value other than Structure was passed as fill values, or fill settings were not filled (no structure key was specified)."
"Possible key names are the same as resource register information names СостоянияНовостей'");
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
			НСтр("es='Las novedades. El servicio y el reglamento. УстановитьСостояниеНовостейДляПользователей';ru='Новости. Сервис и регламент. УстановитьСостояниеНовостейДляПользователей';en='News. Service and regulations. УстановитьСостояниеНовостейДляПользователей'") , // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
...
Процедура ИзменитьПользовательскуюВидимостьЛентНовостей(ЛентыНовостей, Видимость = Ложь, Пользователь = Неопределено) Экспорт
	...
	Если ТипЗнч(ТекущийПользователь) <> ТипПользователь Тогда // Внешний пользователь, например.
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es entregado el parámetro el Usuario del tipo incorrecto: %1.';ru='Передан параметр Пользователь неправильного типа: %1.';en='Invalid type parameter User passed:%1.'"),
			ТипЗнч(Пользователь));
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	...
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es entregado el parámetro de LentyNovostey del tipo incorrecto: %1.';ru='Передан параметр ЛентыНовостей неправильного типа: %1.';en='Invalid type parameter ЛентыНовостей passed:%1.'"),
...
Процедура ИзменитьВидимостьЛентНовостейДляОбластиДанных(
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если НЕ ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
			...
			Иначе
				Если Видимость = Ложь Тогда
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='ИзменитьВидимостьЛентНовостейДляОбластиДанных: es entregado el parámetro De MassivOblasteyDannyh de la dimensión de cero o el tipo incorrecto: %1.';ru='ИзменитьВидимостьЛентНовостейДляОбластиДанных: передан параметр МассивОбластейДанных нулевого размера или неправильного типа: %1.';en='ИзменитьВидимостьЛентНовостейДляОбласти Data: parameter passed МассивОбластейДанных zero size or wrong type:%1.'"),
		...
		Иначе
			...
			Если (ТипЗнч(МассивОбластейДанных) = ТипМассив)
					И (МассивОбластейДанных.Количество() > 0) Тогда
				ТекстСообщения = НСтр("es='ИзменитьВидимостьЛентНовостейДляОбластиДанных: para la sesión dividida no es posible entregar el parámetro De MassivOblasteyDannyh.';ru='ИзменитьВидимостьЛентНовостейДляОбластиДанных: для разделенного сеанса нельзя передавать параметр МассивОбластейДанных.';en='ИзменитьВидимостьЛентНовостейДляОбласти Data: parameter cannot be passed for a split session МассивОбластейДанных.'");
				ВызватьИсключение ТекстСообщения;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es entregado el parámetro De MassivLentNovostey del tipo incorrecto: %1.';ru='Передан параметр МассивЛентНовостей неправильного типа: %1.';en='Invalid type parameter МассивЛентНовостей passed:%1.'"),
	...
	Если НЕ ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() Тогда
		...
		Если (Пользователи.ЭтоСеансВнешнегоПользователя()) Тогда
			...
			Если ПравоДоступа("Чтение", Метаданные.Справочники.Новости) Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No es recomendable dar la posibilidad de la lectura de las novedades al usuario exterior (%1) "
"Son conectados los papeles siguientes:"
"	-"
""
""
" %5 "
"Esto puede llevar ChtenieNovostey - %2	- RedaktirovanieNovostey - %3	- AdministratorSistemy - %4	- PolnyePrava - a las faltas en el trabajo con la configuración.';ru='Не рекомендуется давать возможность чтения новостей внешнему пользователю (%1)"
"Подключены следующие роли:"
"	- ЧтениеНовостей         - %2"
"	- РедактированиеНовостей - %3"
"	- АдминистраторСистемы   - %4"
"	- ПолныеПрава            - %5"
"Это может привести к ошибкам в работе с конфигурацией.';en='It is not recommended to give the ability to read news to an external user (%1)"
"The following roles are connected: - ЧтениеНовостей         - %2"
"	- РедактированиеНовостей - %3"
"	- АдминистраторСистемы   - %4"
"	- ПолныеПрава            - %5                                    This can lead to configuration errors.'"),
					...
					+ Символы.ПС;
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("es='БИП:Новости. Diferente';ru='БИП:Новости.Разное';en='BIP: News. Miscellaneous'"), // ИмяСобытия
					НСтр("es='Las novedades. Diferente. La falta del trabajo con los usuarios';ru='Новости. Разное. Ошибка работы с пользователями';en='News. Miscellaneous. Error working with users'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
Процедура ПриИзмененииЗначенийОтборовОбщих() Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
			ТекстСообщения = НСтр("es='Es imposible cambiar las selecciones generales en la sesión dividida.';ru='Невозможно изменить общие отборы в разделенном сеансе.';en='Unable to change the total selections in a split session.'");
			ВызватьИсключение ТекстСообщения;
		КонецЕсли;
	КонецЕсли;
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='Con el cambio de los significados de las selecciones generales';ru='При изменении значений общих отборов';en='When changing the values of the total selections'"));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
			НСтр("es='Las novedades. El servicio y el reglamento. ПриИзмененииЗначенийОтборов. Generales';ru='Новости. Сервис и регламент. ПриИзмененииЗначенийОтборов. Общие';en='News. Service and regulations. ПриИзмененииЗначенийОтборов. Общие'"), // ИдентификаторШага
			?(КодРезультата > 0,
...
Процедура ПриИзмененииЗначенийОтборовДляОбластиДанных() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='Con el cambio de los significados de las selecciones para la área de los datos';ru='При изменении значений отборов для области данных';en='When changing sampling values for a data area'"));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
			НСтр("es='Las novedades. El servicio y el reglamento. ПриИзмененииЗначенийОтборов. ДляОбластиДанных';ru='Новости. Сервис и регламент. ПриИзмененииЗначенийОтборов. ДляОбластиДанных';en='News. Service and regulations. ПриИзмененииЗначенийОтборов. ДляОбластиДанных'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ПриИзмененииЗначенийОтборовПользовательских(Пользователь = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='Con el cambio de los significados de las selecciones para el usuario';ru='При изменении значений отборов для пользователя';en='When changing selection values for the user'"));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
			НСтр("es='Las novedades. El servicio y el reglamento. ПриИзмененииЗначенийОтборов. De usuario';ru='Новости. Сервис и регламент. ПриИзмененииЗначенийОтборов. Пользовательские';en='News. Service and regulations. ПриИзмененииЗначенийОтборов. Пользовательские'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ДобавитьПрограммноеЗначениеОтбора_Общий(ЗначенияОтборов) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='En el modelo del servicio está permitido llamar el método [%1] solamente en el régimen no dividido.';ru='В модели сервиса метод [%1] разрешено вызывать только в неразделенном режиме.';en='In the service model the method [% 1] is allowed to be called only in undivided mode.'"),
...
Функция ПолучитьПрограммноеЗначениеОтбора_ДляОбластиДанных(
	...
	Если (ИспользоватьНомерОбластиДанных = Истина) Тогда
		...
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='En el modelo del servicio está permitido llamar el método [%1]:"
" - o en el régimen dividido"
" - o es necesario entregar el número positivo en el parámetro de NomerOblastiDannyh.';ru='В модели сервиса метод [%1] разрешено вызывать:"
" - или в разделенном режиме"
" - или необходимо передать положительное число в параметре НомерОбластиДанных.';en='In the service model, the method [% 1] is allowed to call:"
"  - or in split mode"
"  - or you need to pass a positive number in the НомерОбластиДанных  parameter.'"),
...
Процедура КонтекстныеНовости_ПриСозданииНаСервере(
	...
	ОписаниеНовостей.Вставить("СпособОтображенияПанелиКонтекстныхНовостей", "Скрывать");
...
Процедура УстановитьНастройкиПоказаНовостей(Настройки) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		// Если вошли в область данных, то выдать исключение.
		Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
			ТекстСообщения = НСтр("es='УстановитьНастройкиПоказаНовостей: en la sesión dividida no es posible establecer los ajustes de la demostración de las novedades.';ru='УстановитьНастройкиПоказаНовостей: в разделенном сеансе нельзя установить настройки показа новостей.';en='УстановитьНастройкиПоказаНовостей: in a split session, you cannot set news display preferences.'");
			ВызватьИсключение ТекстСообщения;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий         = НСтр("es='Es puesto en marcha durante el primer lanzamiento, en los datos generales. La instalación de las constantes, la carga de los significados estandartizados, el lanzamiento de los registros de las noticias (los datos generales).';ru='Запускается при первом запуске, в общих данных. Установка констант, загрузка стандартных значений, сброс регистров сведений (общих данных).';en='Launching at the first start, in the general data. Setting constants, loading standard values, resetting information registers (general data).'");
	...
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.Комментарий         = НСтр("es='Es puesto en marcha durante el primer lanzamiento, para cada área de los datos. La instalación de los ajustes de los usuarios (los datos divididos).';ru='Запускается при первом запуске, для каждой области данных. Установка настроек пользователей (разделенных данных).';en='Launching at the first start, for each data area. Setting user preferences (shared data).'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.Комментарий         = НСтр("es='Es puesto en marcha durante el tránsito a una nueva versión, en los datos generales. La instalación de las constantes, la carga de los significados estandartizados, el lanzamiento de los registros de las noticias (los datos generales).';ru='Запускается при переходе на новую версию, в общих данных. Установка констант, загрузка стандартных значений, сброс регистров сведений (общих данных).';en='Launching when switching to a new version, in general data. Setting constants, loading standard values, resetting information registers (general data).'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("es='Es puesto en marcha durante el tránsito a cualquier nueva versión, para cada área de los datos. La instalación de los ajustes de los usuarios (los datos divididos).';ru='Запускается при переходе на любую новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).';en='Launching when switching to any new version, for each data area. Setting user preferences (shared data).'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("es='Es puesto en marcha durante el tránsito a una nueva versión, para cada área de los datos. La instalación de los ajustes de los usuarios (los datos divididos).';ru='Запускается при переходе на новую версию, для каждой области данных. Установка настроек пользователей (разделенных данных).';en='Launching when switching to a new version, for each data area. Setting user preferences (shared data).'");
	Обработчик.РежимВыполнения     = "Оперативно";
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		"news.webits.1c.ru",
		443,
		НСтр("es='Los servicios Web de la recepción de las novedades';ru='Веб-сервисы получения новостей';en='News Web Services'"));
...
Функция СоединитьсяССерверомНовостей(ИмяСервера, Протокол, Логин, Пароль, ХттпСоединение, КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='La unión con el servidor de las novedades';ru='Соединение с сервером новостей';en='News server connection'"));
		...
		Если ((ВРег(Протокол) = ВРег("http")))
			...
			Пока (СоединениеУстановлено <> Истина) И (ПопыткаСоединения < НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокСоединения) Цикл
					...
					Исключение
						...
						ОписаниеРезультата = ОписаниеРезультата
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La tentativa %1 / %2: la Falta de la definición del puerto del nombre de servidor %3 (%4). La cinta de las novedades será dejada pasar.';ru='Попытка %1 / %2: Ошибка определения порта из имени сервера %3 (%4). Лента новостей будет пропущена.';en='Attempt %1 / %2: Error determining port from server name%3 (%4). The feed will be skipped.'"),
								ПопыткаСоединения,
				...
				Попытка
					...
					ОписаниеРезультата = ОписаниеРезультата
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='La tentativa %1 / %2: la Acta %3, el Nombre del servidor %4, el Puerto %5';ru='Попытка %1 / %2: Протокол %3, Имя сервера %4, Порт %5';en='Attempt %1 /%2: Protocol%3, Server name%4, Port%5'"),
							ПопыткаСоединения,
							НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокСоединения,
							Протокол,
							ИмяСервера_Сервер,
				...
				Исключение
					ОписаниеРезультата = ОписаниеРезультата
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='La tentativa %1 / %2: la Falta de la conexión a servidor %3 (%4) para la recepción de las novedades por la causa: "
"%5';ru='Попытка %1 / %2: Ошибка подключения к серверу %3 (%4) для получения новостей по причине:"
"%5';en='Attempt %1 /%2: Error connecting to server%3 (%4) to receive news due to:"
"%5'"),
							ПопыткаСоединения,
							НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокСоединения,
							ИмяСервера,
		...
		Иначе
			...
			ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La acta de la recepción de las novedades %1 se distingue de http y https. La unión con el servidor de las novedades no se establecía.';ru='Протокол получения новостей %1 отличается от http и https. Соединение с сервером новостей не устанавливалось.';en='The news feed protocol%1 is different from http and https. A connection to the news server was not established.'"),
				Протокол);
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. La carga de las novedades';ru='БИП:Новости.Загрузка новостей';en='BIP: News. Download News'"), // ИмяСобытия
			НСтр("es='Las novedades. La carga de las novedades. La unión con el servidor de las novedades';ru='Новости. Загрузка новостей. Соединение с сервером новостей';en='News. Download news. News Server Connection'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
#Область Шаблоны
	...
	Результат = Символы.ПС
		+ НСтр("es='# la INFORMACIÓN DE AJUSTE Sobre las NOVEDADES "
""
"la Fecha del informe: ** %ТекущаяУниверсальнаяДата del % ** "
""
"Es encontrado los elementos de la guía de la Novedad con UIN ** %УИННовости del % **: %КоличествоНайденныхНовостей del %"
""
"---------- "
"## los DATOS Sobre las NOVEDADES "
"%ДанныеОНовостях del %"
""
"----------"
""
"---------- "
"## los DATOS Sobre las CINTAS de las NOVEDADES "
"%ДанныеОЛентахНовостей del %"
""
"----------"
""
"---------- "
"## los DATOS Sobre las OPERACIONES DE REGLAMENTO "
"%ДанныеОРегламентныхОперациях del %"
""
"----------"
""
"---------- "
"## los DATOS Sobre el AMBIENTE DE PROGRAMA "
"%ДанныеОПрограммномОкружении del %"
""
"----------';ru='# ОТЛАДОЧНАЯ ИНФОРМАЦИЯ О НОВОСТЯХ"
""
...
""
"----------';en='# DEBUGGING NEWS INFORMATION"
""
"Report date: **%ТекущаяУниверсальнаяДата% **"
""
"Found Directory Elements News with UIN **%УИННовости%**: %КоличествоНайденныхНовостей%"
"----------"
"## NEWS DATA"
"% DataNews%"
""
...
"----------'")
	...
	ШаблонРезультатаНовости = Символы.ПС
		+ НСтр("es='### la Novedad: %НомерПоПорядку del % / %КоличествоНайденныхНовостей del % "
""
"#### el Elemento de la guía de la novedad "
"%ЭлементСправочникаНовости del % "
""
"#### los registros Auxiliares de las noticias "
"##### РегистрСведений.КатегорииНовостейИнтервалыВерсий "
"%РегистрСведений.КатегорииНовостейИнтервалыВерсий% "
""
"##### RegistrSvedeny. КатегорииНовостейПростые "
...
"%РегистрСведений.РассчитанныеОтборыПоНовостям_РедкоМеняющиеся% "
""
"##### РегистрСведений.РассчитанныеОтборыПоНовостям_Пользовательские (para el usuario corriente) "
"%РегистрСведений.РассчитанныеОтборыПоНовостям_Пользовательские% "
""
"##### RegistrSvedeny. ОтключенныеЛентыНовостей (para el usuario corriente) "
"%РегистрСведений.ОтключенныеЛентыНовостей% "
""
"##### РегистрСведений.ОтключенныеЛентыНовостейДляОбластиДанных (para la área de los datos) "
"%РегистрСведений.ОтключенныеЛентыНовостейДляОбластиДанных% "
""
"##### RegistrSvedeny. СостоянияНовостей "
"%РегистрСведений. СостоянияНовостей del % "
""
"#### los Datos en las maquetas básicas de los datos suministrados "
"%ПоставляемыеДанные del %';ru='### Новость: %НомерПоПорядку% / %КоличествоНайденныхНовостей%"
""
...
"%ПоставляемыеДанные%';en='### News: %НомерПоПорядку% / %КоличествоНайденныхНовостей%"
""
"#### News directory element"
"%ЭлементСправочникаНовости%"
""
...
"%ПоставляемыеДанные%'")
	...
	ШаблонРезультатаПоставляемыеДанные = Символы.ПС
		+ НСтр("es='##### los datos Suministrados, el identificador %ИдентификаторПоставляемыхДанных del % "
""
"el Texto, donde se encuentra UIN (+-200 de los símbolos): "
""
"%ТекстПоставляемыхДанных del %';ru='##### Поставляемые данные, идентификатор %ИдентификаторПоставляемыхДанных%"
""
"Текст, где встречается UIN (+-200 символов):"
""
"%ТекстПоставляемыхДанных%';en='##### Delivered data, identifier %ИдентификаторПоставляемыхДанных%"
""
"Text where UIN is encountered (+ -200 characters):"
""
"%ТекстПоставляемыхДанных%'")
	...
	ШаблонРезультатаЛентыНовостей = Символы.ПС
		+ НСтр("es='### la Cinta de las novedades: %НомерПоПорядку del % / %КоличествоНайденныхЛентНовостей del % "
""
"#### el Elemento de la guía de la Cinta de las novedades "
"%ЭлементСправочникаЛентыНовостей del %';ru='### Лента новостей: %НомерПоПорядку% / %КоличествоНайденныхЛентНовостей%"
""
"#### Элемент справочника Ленты новостей"
"%ЭлементСправочникаЛентыНовостей%';en='### Timeline: %НомерПоПорядку% / %КоличествоНайденныхЛентНовостей%"
""
"#### Timeline directory item"
"%ЭлементСправочникаЛентыНовостей%'")
			+ Символы.ПС
			+ Символы.ПС;

	ШаблонРезультатаРегламентныеОперации = Символы.ПС
		+ НСтр("es='### RegistrSvedeny. ВсеОбновленияНовостей "
"%РегистрСведений. ВсеОбновленияНовостей del % "
""
"### РегистрСведений.ДатыЗагрузкиПоследнихНовостей "
"%РегистрСведений.ДатыЗагрузкиПоследнихНовостей% "
""
"### РегистрСведений.ДатыОбновленияСтандартныхСписковНовостей "
"%РегистрСведений.ДатыОбновленияСтандартныхСписковНовостей%';ru='### РегистрСведений.ВсеОбновленияНовостей"
...
""
"### РегистрСведений.ДатыОбновленияСтандартныхСписковНовостей"
"%РегистрСведений.ДатыОбновленияСтандартныхСписковНовостей%';en='### РегистрСведений.ВсеОбновленияНовостей"
"%РегистрСведений.ВсеОбновленияНовостей%"
""
"### РегистрСведений.ДатыЗагрузкиПоследнихНовостей"
"%РегистрСведений.ДатыЗагрузкиПоследнихНовостей%"
...
"%РегистрСведений.ДатыОбновленияСтандартныхСписковНовостей%'")
	...
	ШаблонРезультатаПрограммноеОкружение = Символы.ПС
		+ НСтр("es='### las Constantes "
"%Константы del % "
""
"### Otros parámetros del ambiente de programa "
"%ДругиеПараметрыПрограммногоОкружения del % "
""
"### los Significados de las categorías predestinadas "
"%ЗначенияПредопределенныхКатегорий del % "
""
"### los Parámetros de la sesión "
"%ПараметрыСеанса del % "
""
"### los Datos sobre la configuración (de los metadatos) y el sistema de operaciones "
"%ДанныеОКонфигурацииИзМетаданных del % "
""
"### el Horario de la tarea de reglamento "
"%РасписаниеРегламентногоЗадания del %';ru='### Константы"
"%Константы%"
""
...
"%РасписаниеРегламентногоЗадания%';en='### Constants"
"%Константы%"
""
"### Other parameters of the software environment"
"%ДругиеПараметрыПрограммногоОкружения%"
""
...
"%ПараметрыСеанса%"
""
"### Configuration data (from metadata) and operating system"
"%ДанныеОКонфигурацииИзМетаданных%"
""
"### Regulatory work schedule % РасписаниеРегламентногоЗадания%'")
			+ Символы.ПС
			+ Символы.ПС;
...
#Область ЗаполнениеШаблонов_Новости_КатегорииНовостейИнтервалыВерсий
			...
			Иначе
				РезультатТекущейНовости = СтрЗаменить(
					РезультатТекущейНовости,
					"%РегистрСведений." + ИмяРегистраСведений + "%",
					Символы.Таб + НСтр("es='El juego de las anotaciones vacío.';ru='Набор записей пустой.';en='The recordset is empty.'"));
			КонецЕсли;
...
#Область ЗаполнениеШаблонов_Новости_РассчитанныеОтборыПоНовостям_Пользовательские
			...
			Иначе
				РезультатТекущейНовости = СтрЗаменить(
					РезультатТекущейНовости,
					"%РегистрСведений." + ИмяРегистраСведений + "%",
					НСтр("es='En el modelo del servicio para el administrador del sistema no es posible recibir los ajustes de usuario o las selecciones.';ru='В модели сервиса для администратора системы нельзя получить пользовательские настройки или отборы.';en='In the service model for the system administrator, you cannot get user settings or selections.'"));
			КонецЕсли;
...
#Область ЗаполнениеШаблонов_Новости_ПоставляемыеДанные
			...
			Иначе
				РезультатТекущейНовости = СтрЗаменить(РезультатТекущейНовости, "%ПоставляемыеДанные%", Символы.Таб + НСтр("es='No es encontrado en los datos suministrados.';ru='Не найдено в поставляемых данных.';en='Not found in the supplied data.'"));
			КонецЕсли;
...
#КонецОбласти
	...
	Иначе
		Результат = СтрЗаменить(Результат, "%ДанныеОНовостях%", НСтр("es='No es encontrado ni una novedad con tal UIN';ru='Не найдено ни одной новости с таким UIN';en='No news found with this UIN'"));
	КонецЕсли;
...
#Область ЗаполнениеШаблонов_ЛентыНовостей
	...
	Иначе
		Результат = СтрЗаменить(Результат, "%ДанныеОЛентахНовостей%", НСтр("es='No es encontrado ni una cinta de las novedades para la novedad con tal UIN';ru='Не найдено ни одной ленты новостей для новости с таким UIN';en='No news feeds found for news with such a UIN'"));
	КонецЕсли;
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_Константы
	...
	Иначе
		ПредставлениеНастройкиНовостей = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Los ajustes de las novedades del tipo %1 no tratado';ru='Настройки новостей необрабатываемого типа %1';en='Raw type news settings%1'"),
	...
	РезультатДанныеОПрограммномОкружении_Константы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La constante. НастройкиНовостей: "
"%1 "
"RazreshenaRabotaSNovostyami: %2 "
"RazreshenaRabotaSnovostyamiCherezInternet: %3';ru='Константа.НастройкиНовостей:"
"%1"
"РазрешенаРаботаСНовостями: %2"
"РазрешенаРаботаСНовостямиЧерезИнтернет: %3';en='Константа.НастройкиНовостей:"
"%1"
"РазрешенаРаботаСНовостями: %2"
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_ДругиеПараметрыПрограммногоОкружения
	...
	ВременныйТекст = НСтр("es='La línea de la conexión: %СтрокаСоединенияИнформационнойБазы del % "
"de TekuschiyRezhimZapuska: %ТекущийРежимЗапуска del % "
"Es la configuración dividida: %ЭтоРазделеннаяКонфигурация del % "
"la División es incluida: %РазделениеВключено del % "
"Es accesible el uso de los datos divididos: %ДоступноИспользованиеРазделенныхДанных del % "
...
"	La División de los datos:"
"		%ТекущийПользователь_РазделениеДанных del % "
"la Área de los datos (es entregada como el parámetro): %ОбластьДанных del % "
"la área de los datos Escogida (es usada para los cálculos): %ВыбраннаяОбластьДанных del % "
"el Parámetro del lanzamiento/C: %ПараметрЗапуска del %';ru='Строка подключения: %СтрокаСоединенияИнформационнойБазы%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация%"
"Разделение включено: %РазделениеВключено%"
"Доступно использование разделенных данных: %ДоступноИспользованиеРазделенныхДанных%"
...
"Параметр запуска /C: %ПараметрЗапуска%';en='Connection string:%СтрокаСоединенияИнформационнойБазы%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска%"
"This is a split configuration: %ЭтоРазделеннаяКонфигурация%"
"Separation included: %РазделениеВключено%"
"Use of shared data is available: %ДоступноИспользованиеРазделенныхДанных%"
"Session started without delimiters:%СеансЗапущенБезРазделителей%"
"Configuration (metadata): %Метаданные_Имя%, %Метаданные_Версия%"
"Configuration (override): %Переопределение_Имя%, %Переопределение_Версия%"
"Platform: %Платформа_Версия%"
"Functional option News work is allowed: %ОбработкаНовостейПовтИсп_РазрешенаРаботаСНовостями%"
"Функциональная опция Allowed to work with news via the Internet: %ОбработкаНовостейПовтИсп_РазрешенаРаботаСНовостямиЧерезИнтернет%"
"Current IB user:"
"	Name: %ТекущийПользователь_Имя%"
"	UIN: %ТекущийПользователь_УникальныйИдентификатор%"
"	Data sharing:"
"		%ТекущийПользователь_РазделениеДанных%"
"Data area (passed as parameter): %ОбластьДанных%"
...
"Параметр запуска /C: %ПараметрЗапуска%""'")
	...
	ВременныйТекст = СтрЗаменить(ВременныйТекст, "%ОбластьДанных%",
		?(ПрочиеПараметры.ОбластьДанных = Неопределено,
			НСтр("es='Неопределено';ru='Неопределено';en='Undefined'"),
...
"EtotProdukt = %7 "
"MetadannyeImya = %8 "
"MetadannyeVersiya = %9';ru='PlatformVersion = %1"
"ProductVersion = %2"
"Product = %3"
"ВерсияПлатформы = %4"
"ВерсияПродукта = %5"
...
"ЭтотПродукт = %7"
"МетаданныеИмя = %8"
"МетаданныеВерсия = %9';en='PlatformVersion = %1"
"ProductVersion = %2"
"Product = %3"
"ВерсияПлатформы = %4"
"ВерсияПродукта = %5"
...
"МетаданныеВерсия = %9'")
		...
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='DataSeparation = %1 "
"InfoBaseStorageType = %2';ru='DataSeparation = %1"
"InfoBaseStorageType = %2';en='DataSeparation = %1"
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_ПараметрыСеанса
	...
	Иначе
		ПредставлениеПараметрыОкруженияБИП_Новости = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Los parámetros del ambiente del subsistema de las novedades del tipo %1 no tratado';ru='Параметры окружения подсистемы новостей необрабатываемого типа %1';en='Environment parameters of the raw type news subsystem%1'"),
	...
	РезультатДанныеОПрограммномОкружении_ПараметрыСеанса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Los parámetros del ambiente del subsistema de las novedades: "
"%1';ru='Параметры окружения подсистемы новостей:"
"%1';en='Environment options for the news subsystem:"
"%1'"),
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_ДанныеОКонфигурацииИзМетаданных
	...
	РезультатДанныеОПрограммномОкружении_ДанныеОКонфигурацииИзМетаданных = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El nombre de la configuración: %1 "
"Versión de la configuración: %2 "
"Versión del subsistema БИП: %3 "
"Versión de la plataforma: %4 "
"sistema De operaciones: %5 "
"Programa de la presentación: %6 "
"Datos del ordenador: %7';ru='Имя конфигурации: %1"
"Версия конфигурации: %2"
"Версия подсистемы БИП: %3"
"Версия платформы: %4"
"Операционная система: %5"
"Программа просмотра: %6"
"Данные компьютера: %7';en='Configuration name:%1"
"Configuration version:%2"
"BIP subsystem Version:%3"
"Platform version:%4"
"Operating system:%5"
"Viewer:%6"
"Computer data:%7'"),
...
#Область ЗаполнениеШаблонов_ПрограммноеОкружение_РасписаниеРегламентногоЗадания
	...
	ТекущееРегламентноеЗадание = РегламентныеЗаданияСервер.Задание(Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей);
	Если ТекущееРегламентноеЗадание = Неопределено Тогда
		ПредставлениеРасписания = НСтр("es='La tarea de reglamento no es encontrada';ru='Регламентное задание не найдено';en='Regulatory task not found'");
		РегламентноеЗадание_Использование = Ложь;
		ПредставлениеРегламентноеЗадание_ПоследнееЗадание = НСтр("es='Se cumplía nunca';ru='Никогда не выполнялось';en='Never done'");
	Иначе
		...
		Если ТипЗнч(РегламентноеЗадание_ПоследнееЗадание) = ТипФоновоеЗадание Тогда
			...
			Иначе
				РегламентноеЗадание_СтруктураПоследнегоЗадания.Вставить("ПодробноеПредставлениеОшибки", НСтр("es='Es cumplido sin faltas';ru='Выполнено без ошибок';en='Done without errors'"));
			КонецЕсли;
		...
		Иначе
			ПредставлениеРегламентноеЗадание_ПоследнееЗадание = НСтр("es='No se cumplía';ru='Не выполнялось';en='Not performed'");
		КонецЕсли;
	КонецЕсли;
	...
	РезультатДанныеОПрограммномОкружении_РасписаниеРегламентногоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El horario de la tarea de reglamento: "
"%1 "
"Uso: %2 "
"Última tarea: "
"%3';ru='Расписание регламентного задания:"
"%1"
"Использование: %2"
"Последнее задание:"
"%3';en='Schedule of regulatory task:"
"%1"
"Usage: %2"
"Last quest:"
"%3 '"),
	...
	Если ПолнотекстовыйПоиск.ПолучитьРежимПолнотекстовогоПоиска() = РежимПолнотекстовогоПоиска.Разрешить Тогда
		Если НЕ ПустаяСтрока(лкСтрокаПоиска) Тогда
			...
			Исключение
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='ОбработкаНовостей. НайтиНовости: había una falta de la búsqueda de las novedades, la línea de la búsqueda [%1] por la causa: "
"%2';ru='ОбработкаНовостей.НайтиНовости: произошла ошибка поиска новостей, строка поиска [%1] по причине:"
"%2';en='ОбработкаНовостей.НайтиНовости: A news search error occurred, search string [%1] due to:"
"%2'"), 
					лкСтрокаПоиска,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("es='БИП:Новости. Diferente';ru='БИП:Новости.Разное';en='BIP: News. Miscellaneous'"), // ИмяСобытия
					НСтр("es='Las novedades. Diferente. La falta полнотекстового de la búsqueda';ru='Новости. Разное. Ошибка полнотекстового поиска';en='News. Miscellaneous. Full text error'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
	...
	Если Вариант = "Для форм, все поля" Тогда
		...
		Если НЕ Результат.Пустой() Тогда
			...
			Если ПолучатьКатегорииНовостей = Истина Тогда
				...
				КонецЕсли;
			КонецЕсли;
			Таблица.Колонки.Удалить("ИконкаНовостиДанные"); // Картинки уже получены их хранилища значений, эта колонка больше не нужна.
		Иначе
			Таблица = Новый ТаблицаЗначений;
		КонецЕсли;
...
Процедура ВсеОбновленияНовостей() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='La renovación de reglamento del subsistema de las novedades';ru='Регламентное обновление подсистемы новостей';en='Newsletter routine regulatory update'"));
	...
	Если МонопольныйРежим() Тогда
		...
		ЕстьОшибки = Истина;
		ОписаниеРезультата = НСтр("es='La terminación forzada de la ejecución de la tarea de reglamento: la base informativa trabaja en el régimen monopolizado.';ru='Принудительное завершение выполнения регламентного задания: информационная база работает в монопольном режиме.';en='Forced completion of the scheduled task: the information base works in exclusive mode.'");
	КонецЕсли;
	...
	Если ЕстьОшибки = Ложь Тогда
		...
		Если ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостями() Тогда
			...
			ОписаниеРезультата = НСтр("es='Es permitido el trabajo con las novedades. La ejecución de los pasos de la renovación.';ru='Разрешена работа с новостями. Выполнение шагов обновления.';en='News work enabled. Complete update steps.'");
...
#Область ВсеОбновленияНовостей_ЗагрузкаНовостей
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("es='1. La recepción y el tratamiento de las novedades de las cintas de las novedades (con el relleno obligatorio de la medida de LentaNovostey)';ru='1. Получение и обработка новостей из лент новостей (с обязательным заполнением измерения ЛентаНовостей)';en='1. Receiving and processing news from news feeds (with the mandatory filling of the ЛентаНовостей dimension)'"));
				...
				Если МассивЛентНовостей.Количество() > 0 Тогда
					ОписаниеРезультатаВложенный = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El macizo de las cintas de las novedades para la recepción de las novedades. La dimensión = %1. La composición: %2';ru='Массив лент новостей для получения новостей. Размер = %1. Состав: %2';en='An array of news feeds for receiving news. Size = %1. Composition: %2'"),
				...
				Иначе
					ОписаниеРезультатаВложенный = НСтр("es='No hay cintas de las novedades para la renovación de reglamento';ru='Нет лент новостей для регламентного обновления';en='There are no news feeds for regular updates.'");
				КонецЕсли;
...
#Область ВсеОбновленияНовостей_ОбновлениеКлассификаторов
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				КонтекстВыполненияВложенный,
				"ОбработкаНовостей." + НаименованиеПроцедурыФункции + "." + ВидОбновления, // Идентификатор.
				НСтр("es='2. La renovación de los clasificadores';ru='2. Обновление классификаторов';en='2. Updating classifiers'"));
				...
				Если РазрешенаРаботаСНовостямиЧерезИнтернет = Истина Тогда
					...
					ОписаниеРезультатаВложенный = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Es permitido el trabajo a través "
"del Internet TekuschayaDataDlyaRegistrov: %1 "
"DataSleduyuschegoObnovleniya: %2';ru='Разрешена работа через интернет"
"ТекущаяДатаДляРегистров: %1"
"ДатаСледующегоОбновления: %2';en='Work via the Internet enabled ТекущаяДатаДляРегистров: %1"
"ДатаСледующегоОбновления: %2'"),
					...
					Если ДатаСледующегоОбновления <= ТекущаяДатаДляРегистров Тогда
						ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
							+ Символы.ПС + НСтр("es='Ha comenzado el plazo de la ejecución.';ru='Наступил срок выполнения.';en='The deadline has come.'");
						ОбновитьСтандартныеСпискиДляНовостей_Ежедневно(КонтекстВыполненияВложенныйВложенный);
					Иначе
						ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
							+ Символы.ПС + НСтр("es='El plazo no ha comenzado.';ru='Срок не наступил.';en='The period has not come.'");
					КонецЕсли;

				Иначе
					ОписаниеРезультатаВложенный = НСтр("es='La renovación de los clasificadores a través del Internet es desconectada';ru='Обновление классификаторов через интернет отключено';en='Classifier update over the Internet is disabled'");
				КонецЕсли;
...
#Область ВсеОбновленияНовостей_АвтоудалениеНовостей
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				КонтекстВыполненияВложенный,
				"ОбработкаНовостей." + НаименованиеПроцедурыФункции + "." + ВидОбновления, // Идентификатор.
				НСтр("es='3. La autodesaparición de las novedades';ru='3. Автоудаление новостей';en='3. Auto delete news'"));
				...
				ОписаниеРезультатаВложенный = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='ТекущаяДатаДляРегистров: %1 "
"DataSleduyuschegoObnovleniya: %2';ru='ТекущаяДатаДляРегистров: %1"
"ДатаСледующегоОбновления: %2';en='ТекущаяДатаДляРегистров: %1"
"ДатаСледующегоОбновления: %2'"),
					ТекущаяДатаДляРегистров,
					ДатаСледующегоОбновления);
...
#Область ВсеОбновленияНовостей_ОбновлениеПериодическихСвойств
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				КонтекстВыполненияВложенный,
				"ОбработкаНовостей." + НаименованиеПроцедурыФункции + "." + ВидОбновления, // Идентификатор.
				НСтр("es='4. La renovación de las propiedades periódicas (la importancia, la actualidad)';ru='4. Обновление периодических свойств (важность, актуальность)';en='4. Updating periodic properties (importance, relevance)'"));
...
#Область ВсеОбновленияНовостей_ОбновлениеПривязокКМетаданным
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				КонтекстВыполненияВложенный,
				"ОбработкаНовостей." + НаименованиеПроцедурыФункции + "." + ВидОбновления, // Идентификатор.
				НСтр("es='5. La renovación de las ataduras a los metadatos (+ la importancia)';ru='5. Обновление привязок к метаданным (+ важность)';en='5. Updating metadata bindings (+ importance)'"));
...
#Область ВсеОбновленияНовостей_ПроверкаВерсииПлатформы
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				КонтекстВыполненияВложенный,
				"ОбработкаНовостей." + НаименованиеПроцедурыФункции + "." + ВидОбновления, // Идентификатор.
				НСтр("es='6. La comprobación de la versión de la plataforma';ru='6. Проверка версии платформы';en='6. Checking the platform version'"));
...
#КонецОбласти
		...
		Иначе
			ОписаниеРезультата = НСтр("es='Es prohibido el trabajo con las novedades.';ru='Запрещена работа с новостями.';en='It is prohibited to work with news.'");
		КонецЕсли;
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("es='БИП:Новости. Todas las renovaciones de las novedades';ru='БИП:Новости.Все обновления новостей';en='BIP: News. All news updates.'"), // ИмяСобытия
		НСтр("es='Las novedades. Todas las renovaciones de las novedades';ru='Новости. Все обновления новостей';en='News. All news updates'"), // ИдентификаторШага
		?(КодРезультата > 0,
			УровеньЖурналаРегистрации.Ошибка,
			УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
		...
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		КомментарийДляЖурналаРегистрации(
			НСтр("es='Todas las renovaciones de las novedades';ru='Все обновления новостей';en='All news updates'"),
			ШагВыполнения,
			КонтекстВыполнения,
...
Процедура УдалитьНовости_Ежедневно(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='La desaparición de las novedades (cada día)';ru='Удаление новостей (ежедневно)';en='Deleting news (daily)'"));

		Если ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостями() Тогда

			ОписаниеРезультата = НСтр("es='Es permitido el trabajo con las novedades';ru='Разрешена работа с новостями';en='News work enabled'");
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("es='La desaparición de las novedades (cada día) / УдалениеНовостей_ПолучитьСписокДанныхДляУдаления';ru='Удаление новостей (ежедневно) / УдалениеНовостей_ПолучитьСписокДанныхДляУдаления';en='Deleting news (daily) / УдалениеНовостей_ПолучитьСписокДанныхДляУдаления'"));
				...
				ОписаниеРезультатаВложенный = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Son recogidos los datos para la desaparición: "
"las Novedades (para la desaparición): %1 "
"Novedades (para la instalación de la marca de la desaparición): %2 "
"KategoriiNovosteyProstyeNeobrabotannye: %3 "
"КатегорииНовостейИнтервалыВерсийНеобработанные: %4';ru='Собраны данные для удаления:"
"Новостей (для удаления): %1"
"Новостей (для установки пометки удаления): %2"
"КатегорииНовостейПростыеНеобработанные: %3"
"КатегорииНовостейИнтервалыВерсийНеобработанные: %4';en='Data collected for deletion:"
"News (for deletion):%1"
"News (to set a deletion mark):%2 КатегорииНовостейПростыеНеобработанные: %3"
"КатегорииНовостейИнтервалыВерсийНеобработанные: %4'"),
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("es='La desaparición de las novedades (cada día) / УдалениеНовостей_ПометитьНаУдалениеДанные';ru='Удаление новостей (ежедневно) / УдалениеНовостей_ПометитьНаУдалениеДанные';en='Deleting news (daily) / УдалениеНовостей_ПометитьНаУдалениеДанные'"));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
			НСтр("es='Las novedades. El servicio y el reglamento. УдалитьНовости_Ежедневно';ru='Новости. Сервис и регламент. УдалитьНовости_Ежедневно';en='News. Service and regulations. УдалитьНовости_Ежедневно'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
...
Процедура ОбновлениеПериодическихСвойствНовостей(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='La renovación de las propiedades periódicas de las novedades';ru='Обновление периодических свойств новостей';en='Updating periodic news properties'"));
		...
		Если ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостями() Тогда
			...
			Если НЕ Результат.Пустой() Тогда
				...
				ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Es encontrado %1 novedades para la renovación de las propiedades periódicas';ru='Найдено %1 новостей для обновления периодических свойств';en='Found %1 news to update periodic properties'"),
					ВсегоСтрок);
				Пока Выборка.Следующий() Цикл
				...
				КонецЦикла;
			Иначе
				ОписаниеРезультата = НСтр("es='la renovación';ru='Не найдено данных для обновления';en='No update data found'");
			КонецЕсли;
		...
		Иначе
			ОписаниеРезультата = НСтр("es='El trabajo con las novedades es prohibido';ru='Работа с новостями запрещена';en='Work with news is prohibited'");
		КонецЕсли;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
			НСтр("es='Las novedades. El servicio y el reglamento. ОбновлениеПериодическихСвойствНовостей';ru='Новости. Сервис и регламент. ОбновлениеПериодическихСвойствНовостей';en='News. Service and regulations. ОбновлениеПериодическихСвойствНовостей'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
			КомментарийДляЖурналаРегистрации(
				НСтр("es='El tratamiento de los requisitos periódicos';ru='Обработка периодических реквизитов';en='Periodic account processing'"),
				ШагВыполнения,
				КонтекстВыполнения,
				"Расширенный"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
...
Процедура ОбновлениеПривязокКМетаданным(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		Если ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостями() Тогда
			...
			Если НЕ Результат.Пустой() Тогда
				...
				ВсегоСтрок = Выборка.Количество();
				ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Es encontrado %1 novedades para la renovación de las ataduras a los metadatos';ru='Найдено %1 новостей для обновления привязок к метаданным';en='Found %1 news to update metadata bindings'"),
					ВсегоСтрок);
				Пока Выборка.Следующий() Цикл
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
			НСтр("es='Las novedades. El servicio y el reglamento. ОбновлениеПривязокКМетаданным';ru='Новости. Сервис и регламент. ОбновлениеПривязокКМетаданным';en='News. Service and regulations. ОбновлениеПривязокКМетаданным'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
			КомментарийДляЖурналаРегистрации(
				НСтр("es='El tratamiento de las ataduras a los metadatos';ru='Обработка привязок к метаданным';en='Processing metadata bindings'"),
				ШагВыполнения,
				КонтекстВыполнения,
				"Расширенный"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
...
Процедура ПолучитьИОбработатьНовостиПоЛентамНовостей(МассивЛентНовостей, КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='La recepción y el tratamiento de las novedades por las cintas de las novedades';ru='Получение и обработка новостей по лентам новостей';en='Receiving and processing news on news feeds'"));
		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			// Зашли в конфигурацию под пользователем с разделением (с входом в область данных).
			Если НЕ ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() Тогда
				ОписаниеРезультата = НСтр("es='ПолучитьИОбработатьНовостиПоЛентамНовостей, en el trabajo en el modelo del servicio poner en marcha la recepción de las novedades se a mano puede solamente en la sesión no dividida.';ru='ПолучитьИОбработатьНовостиПоЛентамНовостей, при работе в модели сервиса запускать получение новостей вручную можно только в неразделенном сеансе.';en='ПолучитьИОбработатьНовостиПоЛентамНовостей, when working in the service model, you can only start receiving news manually in an unshared session.'");
				ЕстьОшибки = Истина;
			КонецЕсли;
		КонецЕсли;
		...
		Если ЕстьОшибки = Ложь Тогда
			...
			УстановитьПривилегированныйРежим(Истина);
				...
				РазрешенаРаботаСНовостямиЧерезИнтернет = ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостямиЧерезИнтернет();
				ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El macizo de las cintas de las novedades para el tratamiento: %1 "
"Es permitido el trabajo a través del Internet: %2';ru='Массив лент новостей для обработки: %1"
"Разрешена работа через интернет: %2';en='Array of feeds to process: %1"
"Internet work allowed: %2 '"),
					ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеЗначения(МассивЛентНовостей, ";"),
				...
				Если НЕ Результат.Пустой() Тогда
					...
					ОписаниеРезультата = ОписаниеРезультата
						+ Символы.ПС
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='La interpelación es cumplida. La cantidad de las cintas de las novedades, que serán tratados: %1';ru='Запрос выполнен. Количество лент новостей, которые будут обработаны: %1';en='Request completed. The number of news feeds to be processed:%1'"),
				...
				Иначе
					ОписаниеРезультата = ОписаниеРезультата
						+ Символы.ПС
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='ПолучитьИОбработатьНовостиПоЛентамНовостей, no consiguió determinar la lista de las cintas de las novedades "
"Es entregado el parámetro con la cantidad de los significados: %1 "
"Resultado de la ejecución de la interpelación: Vacío "
"es posible, la lista de las cintas de las novedades vacío, o cerca de todas las cintas de las novedades es establecido el indicio de LokalnayaLentaNovostey (que significa que por ella no es necesario cargar las novedades), o cerca de todas las cintas de las novedades es establecida la marca de la desaparición.';ru='ПолучитьИОбработатьНовостиПоЛентамНовостей, не удалось определить список лент новостей"
"Передан параметр с количеством значений: %1"
"Результат выполнения запроса: Пустой"
"Возможно, список лент новостей пустой, или у всех лент новостей установлен признак ЛокальнаяЛентаНовостей (что означает, что по ней не надо загружать новости), или у всех лент новостей установлена пометка удаления.';en='ПолучитьИОбработатьНовостиПоЛентамНовостей, could not determine the list of news feeds"
"Parameter passed with the number of values:%1"
"Query Result: Empty"
"Perhaps the list of news feeds is empty, or all news feeds have the ЛокальнаяЛентаНовостей flag set (which means that you don’t need to download news from it), or all news feeds have a deletion mark.'"),
							МассивЛентНовостей.Количество())
						+ Символы.ПС;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. Todas las renovaciones de las novedades';ru='БИП:Новости.Все обновления новостей';en='BIP: News. All news updates.'"), // ИмяСобытия
			НСтр("es='Las novedades. Todas las renovaciones de las novedades. La recepción y el tratamiento de las novedades por las cintas de las novedades';ru='Новости. Все обновления новостей. Получение и обработка новостей по лентам новостей';en='News. All news updates. Receiving and processing news on news feeds'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
...
Процедура ОбновитьСтандартныеСпискиДляНовостей_Ежедневно(КонтекстВыполнения = Неопределено)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='La renovación de los clasificadores';ru='Обновление классификаторов';en='Classifiers update'"));
		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			// Зашли в конфигурацию под пользователем с разделением (с входом в область данных).
			Если НЕ ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() Тогда
				ОписаниеРезультата = НСтр("es='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, en el trabajo en el modelo del servicio poner en marcha la recepción de los clasificadores se a mano puede solamente en la sesión no dividida.';ru='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, при работе в модели сервиса запускать получение классификаторов вручную можно только в неразделенном сеансе.';en='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, when working in a service model, you can start obtaining classifiers manually only in an unshared session.'");
				ЕстьОшибки = Истина;
			КонецЕсли;
		КонецЕсли;
		...
		Если ЕстьОшибки = Ложь Тогда
			...
			РазрешенаРаботаСНовостямиЧерезИнтернет = ОбработкаНовостейПовтИсп.РазрешенаРаботаСНовостямиЧерезИнтернет();
			ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='РазрешенаРаботаСНовостями: %1 "
"RazreshenaRabotaSnovostyamiCherezInternet: %2';ru='РазрешенаРаботаСНовостями: %1"
"РазрешенаРаботаСНовостямиЧерезИнтернет: %2';en='РазрешенаРаботаСНовостями: %1"
"РазрешенаРаботаСНовостямиЧерезИнтернет: %2'"),
			...
			Если РазрешенаРаботаСНовостями Тогда
				...
				Если РазрешенаРаботаСНовостямиЧерезИнтернет Тогда
					ОписаниеРезультата = ОписаниеРезультата
						+ Символы.ПС
						+ НСтр("es='La renovación de los clasificadores es puesta en marcha.';ru='Обновление классификаторов запущено.';en='Classifiers update launched.'");
					Результат = Обработки.УправлениеНовостями.ОбновитьВсеСтандартныеСпискиССервера(КонтекстВыполненияВложенный);
					ОписаниеРезультата = ОписаниеРезультата
						+ Символы.ПС
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='La renovación de los clasificadores es acabada. El resultado: %1';ru='Обновление классификаторов завершено. Результат: %1';en='Classifiers update completed. Result: %1'"),
							Результат);
				Иначе
					ОписаниеРезультата = ОписаниеРезультата
						+ Символы.ПС
						+ НСтр("es='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, es incluido el régimen [el Trabajo solamente con las novedades locales], "
"por eso ningunos clasificadores no se renuevan de los servidores 1С. "
"Se puede renovarlos de los metadatos (a la renovación de la configuración), o a mano.';ru='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, включен режим [Работа только с локальными новостями],"
"поэтому никакие классификаторы не обновляются с серверов 1С."
"Их можно обновлять из метаданных (при обновлении конфигурации), либо вручную.';en='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, [Работа только с локальными новостями] mode is enabled,"
"therefore, no classifiers are updated from 1C servers."
"They can be updated from metadata (when updating the configuration), or manually.'");
				КонецЕсли;
			...
			Иначе
				ОписаниеРезультата = ОписаниеРезультата
					+ Символы.ПС
					+ НСтр("es='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, es desconectado el régimen [Es permitido el trabajo con las novedades], "
"por eso todos los mecanismos del centro de noticias son desconectados.';ru='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно, отключен режим [Разрешена работа с новостями],"
"поэтому все механизмы новостного центра отключены.';en='ОбновитьСтандартныеСпискиДляНовостей_Ежедневно,  [Разрешена работа с новостями] mode is unabled, therefore all the mechanisms of the news center are disabled.'");
			КонецЕсли;
			...
			Если Результат = Истина Тогда // Успешно
				...
				ОписаниеРезультата = ОписаниеРезультата
					+ Символы.ПС
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Los clasificadores eran renovados sin faltas. "
"ТекущаяДатаДляРегистров: %1 "
"DataSleduyuschegoObnovleniya: %2';ru='Классификаторы были обновлены без ошибок."
"ТекущаяДатаДляРегистров: %1"
"ДатаСледующегоОбновления: %2';en='Classifiers were updated without errors. ТекущаяДатаДляРегистров: %1"
"ДатаСледующегоОбновления: %2'"),
						ТекущаяДатаДляРегистров,
			...
			Иначе // Неудача
				...
				ОписаниеРезультата = ОписаниеРезультата
					+ Символы.ПС
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Había unas faltas de la renovación de los clasificadores. El tiempo del lanzamiento siguiente es cambiado. "
"ТекущаяДатаДляРегистров: %1 "
"DataSleduyuschegoObnovleniya: %2';ru='Были ошибки обновления классификаторов. Время следующего запуска изменено."
"ТекущаяДатаДляРегистров: %1"
"ДатаСледующегоОбновления: %2';en='There were errors updating classifiers. Next start time changed. ТекущаяДатаДляРегистров: %1"
"ДатаСледующегоОбновления: %2'"),
						ТекущаяДатаДляРегистров,
						Запись.ДатаСледующегоОбновления);
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. Todas las renovaciones de las novedades';ru='БИП:Новости.Все обновления новостей';en='BIP: News. All news updates.'"), // ИмяСобытия
			НСтр("es='Las novedades. Todas las renovaciones de las novedades. La renovación de los clasificadores';ru='Новости. Все обновления новостей. Обновление классификаторов';en='News. All news updates. Classifier Update'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
...
Процедура ПересчитатьНеобработанныеКатегорииНовостей(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='El recuento de las categorías en bruto de las novedades';ru='Пересчет необработанных категорий новостей';en='Recalculation of raw news categories'"));
		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			// Зашли в конфигурацию под пользователем с разделением (с входом в область данных).
			Если НЕ ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() Тогда
				ОписаниеРезультата = НСтр("es='ПересчитатьНеобработанныеКатегорииНовостей, en el trabajo en el modelo del servicio poner en marcha la renovación de las listas estandartizadas se puede solamente a desconectado разделителях.';ru='ПересчитатьНеобработанныеКатегорииНовостей, при работе в модели сервиса запускать обновление стандартных списков можно только при отключенных разделителях.';en='ПересчитатьНеобработанныеКатегорииНовостей, when working in the service model, you can start updating standard lists only with separators disabled.'");
				ЕстьОшибки = Истина;
			КонецЕсли;
		КонецЕсли;
...
#Область ОбновлениеКатегорийИЗначений
				...
				ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
					...
					НСтр("es='El recuento de las categorías en bruto de las novedades';ru='Пересчет необработанных категорий новостей';en='Recalculation of raw news categories'") + " / " + ИдентификаторШага1);
					...
					Если НЕ Результаты[0].Пустой() Тогда
						...
						ОписаниеРезультатаВложенный.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='Los resultados [0] - no vacío. ТаблицаКатегорийИЗначенийПростые.Количество () = %1';ru='Результаты[0] - непустой. ТаблицаКатегорийИЗначенийПростые.Количество() = %1';en='Results [0]-nonempty. ТаблицаКатегорийИЗначенийПростые.Количество() = %1'"),
								ТаблицаКатегорийИЗначенийПростые.Количество()));
					Иначе
						ТаблицаКатегорийИЗначенийПростые = Новый ТаблицаЗначений;
						ОписаниеРезультатаВложенный.Добавить(
							НСтр("es='Los resultados [0] - vacío';ru='Результаты[0] - пустой';en='Results [0] - empty'"));
					КонецЕсли;
					Если НЕ Результаты[1].Пустой() Тогда
						...
						ОписаниеРезультатаВложенный.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='Los resultados [1] - no vacío. ТаблицаКатегорийИЗначенийИнтервалыВерсий.Количество () = %1';ru='Результаты[1] - непустой. ТаблицаКатегорийИЗначенийИнтервалыВерсий.Количество() = %1';en='Results [1]-nonempty. ТаблицаКатегорийИЗначенийИнтервалыВерсий.Количество() = %1'"),
								ТаблицаКатегорийИЗначенийИнтервалыВерсий.Количество()));
					Иначе
						ТаблицаКатегорийИЗначенийИнтервалыВерсий = Новый ТаблицаЗначений;
						ОписаниеРезультатаВложенный.Добавить(
							НСтр("es='Los resultados [1] - vacío';ru='Результаты[1] - пустой';en='Results [1] - empty'"));
					КонецЕсли;
					Если НЕ Результаты[2].Пустой() Тогда
						ТаблицаДоступныхКатегорий = Результаты[2].Выгрузить(ОбходРезультатаЗапроса.Прямой);
						ОписаниеРезультатаВложенный.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='Los resultados [2] - no vacío. ТаблицаДоступныхКатегорий. La cantidad () = %1';ru='Результаты[2] - непустой. ТаблицаДоступныхКатегорий.Количество() = %1';en='Results [2]-nonempty. ТаблицаДоступныхКатегорий.Количество() = %1'"),
								ТаблицаДоступныхКатегорий.Количество()));
					Иначе
						ТаблицаДоступныхКатегорий = Новый ТаблицаЗначений;
						ОписаниеРезультатаВложенный.Добавить(
							НСтр("es='Los resultados [2] - vacío';ru='Результаты[2] - пустой';en='Results [2] - empty'"));
					КонецЕсли;
					...
					Если ЕстьДанныеДляОбработки = Истина Тогда
						...
						ОписаниеРезультатаВложенный.Добавить(
							НСтр("es='Hay unos datos para el tratamiento.';ru='Есть данные для обработки.';en='There is data to process.'"));
...
#КонецОбласти
					...
					Иначе
						ОписаниеРезультатаВложенный.Добавить(
							НСтр("es='No hay datos para el tratamiento.';ru='Нет данных для обработки.';en='No data to process.'"));
					КонецЕсли;
					...
					ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
						...
						Для каждого ТекущаяСтрока Из ТаблицаКатегорийИЗначенийПростые Цикл
							Если ТекущаяСтрока.УдалитьЗапись = Истина Тогда
								...
								ОписаниеРезультатаВложенный.Добавить(
									СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("es='Es quitada la anotación del registro de las noticias de KategoriiNovosteyProstyeNeobrabotannye con las llaves:"
" - KategoriyaNovosteyStroka: %1"
" - ZnachenieKategoriiNovosteyStroka: %2"
" - la Condición de la selección: %3"
" - LentaNovostey: %4"
" - УИННовости: %5';ru='Удалена запись из регистра сведений КатегорииНовостейПростыеНеобработанные с ключами:"
" - КатегорияНовостейСтрока: %1"
" - ЗначениеКатегорииНовостейСтрока: %2"
" - Условие отбора: %3"
" - ЛентаНовостей: %4"
" - УИННовости: %5';en='Deleted a record from the information register КатегорииНовостейПростыеНеобработанные with keys:                                                      - КатегорияНовостейСтрока: %1"
"  - ЗначениеКатегорииНовостейСтрока: %2"
...
" - УИННовости: %5'"),
						...
						Для каждого ТекущаяСтрока Из ТаблицаКатегорийИЗначенийИнтервалыВерсий Цикл
							Если ТекущаяСтрока.УдалитьЗапись = Истина Тогда
								...
								ОписаниеРезультатаВложенный.Добавить(
									СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("es='Es quitada la anotación del registro de las noticias КатегорииНовостейИнтервалыВерсийНеобработанные con las llaves:"
" - KategoriyaNovosteyStroka: %1"
" - el Producto: %2"
" - VersiyaOT: %3"
" - VersiyaDO: %4"
" - LentaNovostey: %5"
" - УИННовости: %6';ru='Удалена запись из регистра сведений КатегорииНовостейИнтервалыВерсийНеобработанные с ключами:"
" - КатегорияНовостейСтрока: %1"
" - Продукт: %2"
" - ВерсияОТ: %3"
" - ВерсияДО: %4"
" - ЛентаНовостей: %5"
" - УИННовости: %6';en='Deleted a record from the information register КатегорииНовостейИнтервалыВерсийНеобработанные with keys:                                                      - КатегорияНовостейСтрока: %1"
" - Продукт: %2"
" - ВерсияОТ: %3"
" - ВерсияДО: %4"
" - ЛентаНовостей: %5"
				...
				Если ЕстьДанныеДляОбработки = Истина Тогда
					...
					ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
						...
						Если ТаблицаКатегорийИЗначенийПростые.Количество() > 0 Тогда
							...
							ОписаниеРезультатаВложенный.Добавить(
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("es='Es llevado %1 significados de las categorías simples';ru='Перенесено %1 значений простых категорий';en='Migrated%1 simple category values'"),
						...
						Если ТаблицаКатегорийИЗначенийИнтервалыВерсий.Количество() > 0 Тогда
							...
							ОписаниеРезультатаВложенный.Добавить(
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("es='Es llevado %1 significados de las categorías de los intervalos de las versiones';ru='Перенесено %1 значений категорий интервалов версий';en='Migrated%1 version interval category values'"),
									ТаблицаЗаполненныхКатегорийИЗначений.Количество()));
						КонецЕсли;
...
#Область ОчисткаМусора
				...
				Если ЕстьДанныеДляОбработки = Истина Тогда
					...
					ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
						КонтекстВыполненияВложенный,
						"ОбработкаНовостей." + НаименованиеПроцедурыФункции + "." + ИдентификаторШага1, // Идентификатор.
						НСтр("es='ПересчитатьНеобработанныеКатегорииНовостей, la desaparición de las anotaciones no atadas a las novedades.';ru='ПересчитатьНеобработанныеКатегорииНовостей, удаление записей, не привязанных к новостям.';en='ПересчитатьНеобработанныеКатегорииНовостей, removing non-news posts.'") + " / " + ИдентификаторШага1);
						...
						Если НЕ Результат.Пустой() Тогда
							ОписаниеРезультатаВложенный.Добавить(
								НСтр("es='Hay unas categorías en bruto simples no atadas a las novedades:';ru='Есть необработанные простые категории, не привязанные к новостям:';en='There are unprocessed simple categories that are not tied to news:'"));
							Выборка = Результат.Выбрать(ОбходРезультатаЗапроса.Прямой);
							Пока Выборка.Следующий() Цикл
								...
								ОписаниеРезультатаВложенный.Добавить(
									СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("es='УИН: %1';ru='УИН: %1';en='UIN:%1'"),
										Выборка.УИННовости));
							КонецЦикла;
						Иначе
							ОписаниеРезультатаВложенный.Добавить(
								НСтр("es='No hay categorías en bruto simples no atadas a las novedades.';ru='Нет необработанных простых категорий, не привязанных к новостям.';en='There are no unprocessed simple categories that are not tied to news.'"));
						КонецЕсли;
						ТКонец = ТекущаяУниверсальнаяДатаВМиллисекундах();
						ОписаниеРезультатаВложенный.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='El tiempo de la ejecución: %1 мс"
"-----';ru='Время выполнения: %1 мс"
"-----';en='""Lead time:% 1 ms"
"----- ""'"),
						...
						Если НЕ Результат.Пустой() Тогда
							ОписаниеРезультатаВложенный.Добавить(
								НСтр("es='Hay unas categorías en bruto de los intervalos de las versiones, no atado a las novedades:';ru='Есть необработанные категории интервалов версий, не привязанные к новостям:';en='There are raw categories of version intervals that are not tied to news:'"));
							Выборка = Результат.Выбрать(ОбходРезультатаЗапроса.Прямой);
							Пока Выборка.Следующий() Цикл
						...
						Иначе
							ОписаниеРезультатаВложенный.Добавить(
								НСтр("es='No hay categorías en bruto de los intervalos de las versiones no atadas a las novedades.';ru='Нет необработанных категорий интервалов версий, не привязанных к новостям.';en='There are no raw version interval categories that are not tied to news.'"));
						КонецЕсли;
						ТКонец = ТекущаяУниверсальнаяДатаВМиллисекундах();
						ОписаниеРезультатаВложенный.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
"----- ""'"),
						...
						Если НЕ Результат.Пустой() Тогда
							ОписаниеРезультатаВложенный.Добавить(
								НСтр("es='Hay unas categorías en bruto de los intervalos de las versiones, que son llevadas ya:';ru='Есть необработанные категории интервалов версий, которые уже перенесены:';en='There are raw categories of version ranges that have already been ported:'"));
							Выборка = Результат.Выбрать(ОбходРезультатаЗапроса.Прямой);
							Пока Выборка.Следующий() Цикл
								...
								ОписаниеРезультатаВложенный.Добавить(
									СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("es='УИН: %1, la Categoría %2, el Producto %3, %4. %5';ru='УИН: %1, Категория %2, Продукт %3, %4..%5';en='UIN:%1, Category%2, Product%3,%4 ..%5'"),
										Выборка.УИННовости,
						...
						Иначе
							ОписаниеРезультатаВложенный.Добавить(
								НСтр("es='No hay categorías en bruto de los intervalos de las versiones, que son llevadas ya.';ru='Нет необработанных категорий интервалов версий, которые уже перенесены.';en='There are no raw version interval categories that are already migrated.'"));
						КонецЕсли;
						ТКонец = ТекущаяУниверсальнаяДатаВМиллисекундах();
						ОписаниеРезультатаВложенный.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
			НСтр("es='Las novedades. El servicio y el reglamento. ПересчитатьНеобработанныеКатегорииНовостей';ru='Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей';en='News. Service and regulations. ПересчитатьНеобработанныеКатегорииНовостей'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
...
Процедура ПересчитатьОтборыПоНовостям_РедкоМеняющиеся(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='El recuento de las selecciones por las novedades (raramente que se cambian)';ru='Пересчет отборов по новостям (редко меняющихся)';en='Recalculation of news selections (rarely changing)'"));
		...
		Если ВыполнениеРазрешено = Истина Тогда
			...
			Если ПравоДоступа("Изменение", Метаданные.РегистрыСведений.РассчитанныеОтборыПоНовостям_РедкоМеняющиеся) = Истина Тогда
				...
				Если (КатегорияВерсияПродукта.Пустая())
					...
					// Возможно, список категорий еще не пришел с сервера.
					ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='ПересчитатьОтборыПоНовостям_РедкоМеняющиеся, no son rellenados los significados del plan de los tipos de las características de KategoriiNovostey: "
"КатегорияВерсияПродукта: %1 "
"KategoriyaVersiyaPlatformy: %2 "
"es posible, no se han renovado del servidor a la renovación de los clasificadores, también no eran rellenado en la maqueta de StandartnyeZnacheniya del plan de los tipos de las características de KategoriiNovostey';ru='ПересчитатьОтборыПоНовостям_РедкоМеняющиеся, не заполнены значения плана видов характеристик КатегорииНовостей:"
"КатегорияВерсияПродукта: %1"
"КатегорияВерсияПлатформы: %2"
"Возможно, они еще не обновились с сервера при обновлении классификаторов, а также не были заполнены в макете СтандартныеЗначения плана видов характеристик КатегорииНовостей';en='ПересчитатьОтборыПоНовостям_РедкоМеняющиеся, values of the plan of characteristic types are not filled КатегорииНовостей:"
"КатегорияВерсияПродукта: %1"
"КатегорияВерсияПлатформы: %2 Perhaps they have not yet been updated from the server when updating the classifiers, and also were not filled in the СтандартныеЗначения plan of the types of characteristics of the КатегорииНовостей'"),
			...
			Иначе
				ОписаниеРезультата = НСтр("es='Falta el derecho al cambio para РегистрыСведений.РассчитанныеОтборыПоНовостям_РедкоМеняющиеся.';ru='Отсутствует право на изменение для РегистрыСведений.РассчитанныеОтборыПоНовостям_РедкоМеняющиеся.';en='There is no right to change for РегистрыСведений.РассчитанныеОтборыПоНовостям_РедкоМеняющиеся.'");
			КонецЕсли;
		...
		Иначе
			...
			КодРезультата = 1101;
			ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La ejecución es prohíbida. "
"ОбщегоНазначения. РазделениеВключено () = %1 "
"ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей () = %2 "
"ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных () = %3';ru='Выполнение запрещено."
"ОбщегоНазначения.РазделениеВключено() = %1"
"ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() = %2"
"ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() = %3';en='""Execution is prohibited."
"ОбщегоНазначения.РазделениеВключено() = %1"
"ИнтернетПоддержкаПользователей.СеансЗапущенБезРазделителей() = %2"
"ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() = %3""'"),
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
			НСтр("es='Las novedades. El servicio y el reglamento. ПересчитатьОтборыПоНовостям. РедкоМеняющиеся';ru='Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. РедкоМеняющиеся';en='News. Service and regulations. ПересчитатьОтборыПоНовостям. РедкоМеняющиеся'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ПересчитатьОтборыПоНовостям_Общие(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='El recuento de las selecciones por las novedades de (generales)';ru='Пересчет отборов по новостям (общих)';en='Recalculation of news selections (general)'"));
...
#Область ЗаполнениеЗначенийАвтоматическихОтборов
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("es='El recuento de las selecciones por las novedades / Rellenar los significados de las selecciones automáticas';ru='Пересчет отборов по новостям / Заполнить значения автоматических отборов';en='Recalculation of news selections / Fill in the values of automatic selections'"));
...
#Область ПересчетОтборов_ВыполнениеЗапроса
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("es='El recuento de las selecciones por las novedades / la Ejecución de la interpelación';ru='Пересчет отборов по новостям / Выполнение запроса';en='Recalculation of news selections / Query completion'"));
		...
		ИнтернетПоддержкаПользователейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
			...
			0,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El resultado de la interpelación: %1';ru='Результат запроса: %1';en='Query result:%1'"),
				?(РезультатЗапроса.Пустой(),
					НСтр("es='Vacío';ru='пустой';en='empty'"),
					НСтр("es='Hay unos datos';ru='есть данные';en='there is data'"))),
			Неопределено);
...
#Область ПересчетОтборов_ЗаполнениеДанными
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("es='El recuento de las selecciones por las novedades / el Relleno por los datos';ru='Пересчет отборов по новостям / Заполнение данными';en='Recalculation of news selections / Filling with data'"));
		...
		ИнтернетПоддержкаПользователейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
			...
			0,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='НаборЗаписей. La cantidad (): %1';ru='НаборЗаписей.Количество(): %1';en='НаборЗаписей.Количество(): %1'"),
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
			НСтр("es='Las novedades. El servicio y el reglamento. ПересчитатьОтборыПоНовостям. Generales';ru='Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. Общие';en='News. Service and regulations. ПересчитатьОтборыПоНовостям. Общие'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
			КомментарийДляЖурналаРегистрации(
				НСтр("es='El recuento de las selecciones por las novedades (generales)';ru='Пересчет отборов по новостям (общие)';en='Recalculation of news selections (general)'"),
				ШагВыполнения,
				КонтекстВыполнения,
				"Расширенный"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
...
Процедура ПересчитатьОтборыПоНовостям_ДляОбластиДанных(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='El recuento de las selecciones por las novedades (generales para la área de los datos)';ru='Пересчет отборов по новостям (общих для области данных)';en='Recalculation of news selections (common to the data area)'"));
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
			НСтр("es='El recuento de las selecciones por las novedades / Rellenar los significados de las selecciones automáticas';ru='Пересчет отборов по новостям / Заполнить значения автоматических отборов';en='Recalculation of news selections / Fill in the values of automatic selections'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
			КомментарийДляЖурналаРегистрации(
				НСтр("es='El recuento de las selecciones por las novedades (generales en la región los datos)';ru='Пересчет отборов по новостям (общие по области данных)';en='Recalculation of news selections (general by area of data)'"),
				ШагВыполнения,
				КонтекстВыполнения,
				"Расширенный"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
...
Процедура ПересчитатьОтборыПоНовостям_Пользовательские(Пользователь = Неопределено, КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='El recuento de las selecciones por las novedades de (de usuario)';ru='Пересчет отборов по новостям (пользовательских)';en='Recalculation of news selections (custom)'"));
...
#Область ПолучениеСпискаПользователей
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("es='El recuento de las selecciones por las novedades / la Recepción de la lista de los usuarios';ru='Пересчет отборов по новостям / Получение списка пользователей';en='Recalculation of news selections / Getting a list of users'"));
		...
		ИнтернетПоддержкаПользователейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
			...
			0,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El resultado de la interpelación: %1 "
"Cantidad de los usuarios: %2 "
"PropustitPereschet: %3';ru='Результат запроса: %1"
"Количество пользователей: %2"
"ПропуститьПересчет: %3';en='Query result:%1"
"Number of users:%2"
...
#Область ОчисткаПредыдущегоРезультата
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("es='El recuento de las selecciones por las novedades / la Limpieza del resultado anterior';ru='Пересчет отборов по новостям / Очистка предыдущего результата';en='Recalculation of news selections / Clearing the previous result'"));
...
#Область ФормированиеСпискаНовостейИсключений_ВыполнениеЗапроса
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("es='El recuento de las selecciones por las novedades / la Formación de la lista de las novedades-excepciones / la Ejecución de la interpelación';ru='Пересчет отборов по новостям / Формирование списка новостей-исключений / Выполнение запроса';en='Recalculation of news selections / Formation of a list of news-exceptions / Request execution'"));
...
#Область ФормированиеСпискаНовостейИсключений_РаботаСДаннымиЗапроса
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("es='El recuento de las selecciones por las novedades / la Formación de la lista de las novedades-excepciones / el Trabajo con los datos de la interpelación';ru='Пересчет отборов по новостям / Формирование списка новостей-исключений / Работа с данными запроса';en='Recalculation of news selections / Creating a list of news exceptions / Working with query data'"));
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
			НСтр("es='Las novedades. El servicio y el reglamento. ПересчитатьОтборыПоНовостям. De usuario';ru='Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. Пользовательские';en='News. Service and regulations. ПересчитатьОтборыПоНовостям. Пользовательские'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
			КомментарийДляЖурналаРегистрации(
				НСтр("es='El recuento de las selecciones por las novedades (de usuario)';ru='Пересчет отборов по новостям (пользовательские)';en='Recalculation of news selections (custom)'"),
				ШагВыполнения,
				КонтекстВыполнения,
				"Расширенный"), // Комментарий
			ОбработкаНовостейПовтИсп.ВестиПодробныйЖурналРегистрации()); // ВестиПодробныйЖурналРегистрации
	...
	Если СтандартнаяОбработка <> Ложь Тогда
		...
		ИначеЕсли (ВРег(КодКатегорииНовостей) = ВРег("FnOptionOn"))
			...
			Исключение
				...
				ФОВключена = Ложь;
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La falta de la recepción del significado de la opción funcional %1 para la categoría %2. "
"Los detalles: "
"%3';ru='Ошибка получения значения функциональной опции %1 для категории %2."
"Подробности:"
"%3';en='Error getting value of functional option%1 for category%2."
"Details:"
"%3'"),
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
					НСтр("es='Las novedades. El servicio y el reglamento. РассчитатьОтборПоКатегории. Generales';ru='Новости. Сервис и регламент. РассчитатьОтборПоКатегории. Общие';en='News. Service and regulations. РассчитатьОтборПоКатегории. Общие'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
"%3'"),
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
					НСтр("es='Las novedades. El servicio y el reglamento. РассчитатьОтборПоКатегории. ДляОбластиДанных';ru='Новости. Сервис и регламент. РассчитатьОтборПоКатегории. ДляОбластиДанных';en='News. Service and regulations. РассчитатьОтборПоКатегории. ДляОбластиДанных'"), // ИдентификаторШага
					УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
...
Процедура ПроверитьИзменениеВерсииПлатформы(КонтекстВыполнения = Неопределено)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='La comprobación del cambio de la versión de la plataforma';ru='Проверка изменения версии платформы';en='Checking for platform version changes'"));
	...
	Если МонопольныйРежим() Тогда
		...
		//  то будут ошибки при всех попытках записи данных в базу.
		// Поэтому в монопольном режиме не следует выполнять никаких обновлений в подсистеме новостей.
		ОписаниеРезультата = НСтр("es='En el régimen monopolizado la comprobación de la versión de la plataforma no se realiza';ru='В монопольном режиме проверка версии платформы не осуществляется';en='In exclusive mode the platform version is not checked'");
		ЕстьОшибки = Истина;
	КонецЕсли;

	Если ЕстьОшибки = Ложь Тогда

		ОписаниеРезультата = НСтр("es='La versión conservada de la plataforma: %СохраненнаяВерсияПлатформы del % "
"la versión Corriente de la plataforma: %ТекущаяВерсияПлатформы del % "
"%ТребуетсяОбновлениеРедкоменяющихсяОтборов% "
""
"la Versión de la configuración en este tratamiento no es comprobada, y es contado forzadamente en el módulo de la renovación de la configuración, también después de cada recepción de las novedades.';ru='Сохраненная версия платформы: %СохраненнаяВерсияПлатформы%"
"Текущая версия платформы: %ТекущаяВерсияПлатформы%"
"%ТребуетсяОбновлениеРедкоменяющихсяОтборов%"
""
"Версия конфигурации в этой обработке не проверяется, а пересчитывается принудительно в модуле обновления конфигурации, а также после каждого получения новостей.';en='Saved platform version: %СохраненнаяВерсияПлатформы%"
"Current platform version: %ТекущаяВерсияПлатформы%"
"%ТребуетсяОбновлениеРедкоменяющихсяОтборов%                          The configuration version in this processing is not checked, but is forcefully recalculated in the configuration update module, as well as after each news.'");
		...
		УстановитьПривилегированныйРежим(Истина);
					...
					Если МассивВыполняемыхФоновыхЗаданий.Количество() = 0 Тогда
						...
						МассивФоновыхЗаданий.Добавить(
							ФоновыеЗадания.Выполнить(
								...
								"", // Ключ
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("es='ОбработкаНовостей.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся en relación al cambio de la versión de la plataforma con %1 en %2';ru='ОбработкаНовостей.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся в связи с изменением версии платформы с %1 на %2';en='ОбработкаНовостей.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся due to the change of platform version from%1 to%2'"),
									лкПоследняяВерсия,
									лкТекущаяВерсия)));
						// Последняя версия платформы сохранится в "ОбработкаНовостей.ПересчитатьОтборыПоНовостям_РедкоМеняющиеся".
						ОписаниеРезультата = СтрЗаменить(ОписаниеРезультата, "%ТребуетсяОбновлениеРедкоменяющихсяОтборов%", НСтр("es='Raramente selecciones que se cambian serán contadas (es puesta en marcha la tarea de fondo)';ru='Редко меняющиеся отборы будут пересчитаны (запущено фоновое задание)';en='Rarely changing selections will be recounted (background task launched)'"));
					Иначе
						ОписаниеРезультата = СтрЗаменить(ОписаниеРезультата, "%ТребуетсяОбновлениеРедкоменяющихсяОтборов%", НСтр("es='Raramente selecciones que se cambian son contadas ya (es puesta en marcha la tarea de fondo de otra sesión)';ru='Редко меняющиеся отборы уже пересчитываются (запущено фоновое задание из другого сеанса)';en='Rarely changing selections are already recounted (background task started from another session)'"));
					КонецЕсли;
				Иначе
					ОписаниеРезультата = СтрЗаменить(ОписаниеРезультата, "%ТребуетсяОбновлениеРедкоменяющихсяОтборов%", НСтр("es='Raramente selecciones que se cambian serán contadas (el lanzamiento evidente ПересчитатьОтборыПоНовостям_РедкоМеняющиеся ())';ru='Редко меняющиеся отборы будут пересчитаны (явный запуск ПересчитатьОтборыПоНовостям_РедкоМеняющиеся())';en='Rarely changing selections will be recounted (explicit launch ПересчитатьОтборыПоНовостям_РедкоМеняющиеся())'"));
			...
			Иначе
				ОписаниеРезультата = СтрЗаменить(ОписаниеРезультата, "%СохраненнаяВерсияПлатформы%", лкПоследняяВерсия);
				ОписаниеРезультата = СтрЗаменить(ОписаниеРезультата, "%ТекущаяВерсияПлатформы%", лкТекущаяВерсия);
				ОписаниеРезультата = СтрЗаменить(ОписаниеРезультата, "%ТребуетсяОбновлениеРедкоменяющихсяОтборов%", НСтр("es='Raramente selecciones que se cambian se quedarán como es';ru='Редко меняющиеся отборы останутся как есть';en='Rarely changing selections will remain as they are'"));
			КонецЕсли;
			...
			Попытка
				...
				ОписаниеРезультата = ОписаниеРезультата
					+ Символы.ПС
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La tentativa de fijar la transacción. "
"ТекущаяДатаДляРегистров: %1 "
"DataSleduyuschegoObnovleniya: %2';ru='Попытка зафиксировать транзакцию."
"ТекущаяДатаДляРегистров: %1"
"ДатаСледующегоОбновления: %2';en='Attempt to commit transaction."
"ТекущаяДатаДляРегистров: %1"
"ДатаСледующегоОбновления: %2'"),
						ТекущаяДатаДляРегистров,
						ДатаСледующегоОбновления);
			...
			Исключение
				...
				ОписаниеРезультата = ОписаниеРезультата
					+ Символы.ПС
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='No consiguió bloquear la anotación del registro de las noticias de VseObnovleniyaNovostey para el tipo de las renovaciones %1. "
"Probablemente, otro proceso en este momento conserva los datos. Se planeaba a anotar: "
"ТекущаяДатаДляРегистров: %2 "
"DataSleduyuschegoObnovleniya: %3';ru='Не удалось заблокировать запись регистра сведений ВсеОбновленияНовостей для вида обновлений %1."
"Возможно, другой процесс в этот момент сохраняет данные. Планировалось записать:"
"ТекущаяДатаДляРегистров: %2"
"ДатаСледующегоОбновления: %3';en='Failed to lock the All UpdatesNews information registry entry for update type%1."
"Perhaps another process is currently saving data. It was planned to record: ТекущаяДатаДляРегистров: %2"
"ДатаСледующегоОбновления: %3'"),
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. Todas las renovaciones de las novedades';ru='БИП:Новости.Все обновления новостей';en='BIP: News. All news updates.'"), // ИмяСобытия
			НСтр("es='Las novedades. Todas las renovaciones de las novedades. La comprobación de la versión de la plataforma';ru='Новости. Все обновления новостей. Проверка версии платформы';en='News. All news updates. Platform Version Check'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ОптимизироватьОтборыПоНовостям(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='La optimización de las selecciones por las novedades';ru='Оптимизация отборов по новостям';en='News screening Optimization'"));
...
#Область ВыполнениеЗапроса
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La optimización de las selecciones por las novedades / la Ejecución de la interpelación';ru='Оптимизация отборов по новостям / Выполнение запроса';en='News feed optimization / Query'"));
	...
	РезультатЗапроса1 = ?(РезультатыЗапроса[1].Пустой(), НСтр("es='Vacío';ru='пустой';en='empty'"), НСтр("es='Hay unos datos';ru='есть данные';en='there is data'"));
	РезультатЗапроса2 = ?(РезультатыЗапроса[2].Пустой(), НСтр("es='Vacío';ru='пустой';en='empty'"), НСтр("es='Hay unos datos';ru='есть данные';en='there is data'"));
	Если ОптимизироватьОтборыПоНовостямОбщие = Истина Тогда
		РезультатЗапроса3 = ?(РезультатыЗапроса[3].Пустой(), НСтр("es='Vacío';ru='пустой';en='empty'"), НСтр("es='Hay unos datos';ru='есть данные';en='there is data'"));
	Иначе
		РезультатЗапроса3 = НСтр("es='No se cumplía';ru='не выполнялся';en='not performed'");
	КонецЕсли;
	ИнтернетПоддержкаПользователейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполненияВложенный,
		0,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El resultado de la interpelación 1: %1 "
"Resultado de la interpelación 2: %2 "
"Resultado de la interpelación 3: %3';ru='Результат запроса 1: %1"
"Результат запроса 2: %2"
"Результат запроса 3: %3';en='Query result 1:%1"
"Query result 2:%2"
"Query result 3:%3'"),
			РезультатЗапроса1,
			РезультатЗапроса2,
...
#Область Оптимизация_УдалениеОтборовПоНесуществующимКатегориям_ДляОбластиДанных
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La optimización de las selecciones por las novedades / la Desaparición de las selecciones por las categorías no existentes / Para la área de los datos';ru='Оптимизация отборов по новостям / Удаление отборов по несуществующим категориям / Для области данных';en='Optimization of news selections / Deletion of selections by nonexistent categories / For data area'"));
...
#Область Оптимизация_УдалениеОтборовПоНесуществующимКатегориям_Пользовательские
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La optimización de las selecciones por las novedades / УдалениеОтборовПоНесуществующимКатегориям / De usuario';ru='Оптимизация отборов по новостям / УдалениеОтборовПоНесуществующимКатегориям / Пользовательские';en='Optimization of news selections / УдалениеОтборовПоНесуществующимКатегориям / Custom'"));
...
#Область Оптимизация_УдалениеОтборовПоНесуществующимКатегориям_Общие
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La optimización de las selecciones por las novedades / УдалениеОтборовПоНесуществующимКатегориям / Generales';ru='Оптимизация отборов по новостям / УдалениеОтборовПоНесуществующимКатегориям / Общие';en='Optimization of news selections / УдалениеОтборовПоНесуществующимКатегориям / General'"));
...
#Область Оптимизация_УдалениеЗначенийОтборов_ДляОбластиДанных
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La optimización de las selecciones por las novedades / la Desaparición de los significados de las selecciones / Generales para la área de los datos';ru='Оптимизация отборов по новостям / Удаление значений отборов / Общие для области данных';en='Optimization of news selections / Deleting selections / General for the data area'"));
...
#Область Оптимизация_УдалениеЗначенийОтборов_Пользовательские
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La optimización de las selecciones por las novedades / UdalenieZnacheniyOtborov / De usuario';ru='Оптимизация отборов по новостям / УдалениеЗначенийОтборов / Пользовательские';en='Optimization of News Screenings / УдалениеЗначенийОтборов / Custom'"));
...
#Область Оптимизация_ЛишниеКатегорииВНовостях
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La optimización de las selecciones por las novedades / la Desaparición de las categorías superfluas en las novedades';ru='Оптимизация отборов по новостям / Удаление лишних категорий в новостях';en='Optimization of news selections / Removing unnecessary categories in news'"));
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
			НСтр("es='Las novedades. El servicio y el reglamento. ОптимизацияОтборовПоНовостям';ru='Новости. Сервис и регламент. ОптимизацияОтборовПоНовостям';en='News. Service and regulations. ОптимизацияОтборовПоНовостям'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура НайтиИОбработатьНеактивныеЛентыНовостей(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='La búsqueda de las cintas inactivas de las novedades';ru='Поиск неактивных лент новостей';en='Search for inactive news feeds'"));
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Если НЕ Результат.Пустой() Тогда
				...
				ОписаниеРезультата = НСтр("es='La interpelación es cumplida.';ru='Запрос выполнен.';en='Request completed.'") + Символы.ПС;
				...
				Пока Выборка.Следующий() Цикл
					...
					Если Выборка.ЛентаНовостейЧастотаОбновления = 0 Тогда
						лкЧастотаОбновления = НСтр("es='A mano (0)';ru='вручную (0)';en='manually (0)'");
					ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 1 Тогда
						лкЧастотаОбновления = НСтр("es='Una vez al día (1)';ru='один раз в день (1)';en='once a day (1)'");
					ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 2 Тогда
						лкЧастотаОбновления = НСтр("es='Cada hora (2)';ru='каждый час (2)';en='every hour (2)'");
					ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 3 Тогда
						лкЧастотаОбновления = НСтр("es='Cada 15 minutos (3)';ru='каждые 15 минут (3)';en='every 15 minutes (3)'");
					ИначеЕсли Выборка.ЛентаНовостейЧастотаОбновления = 4 Тогда
						лкЧастотаОбновления = НСтр("es='Cada minuto (4)';ru='каждую минуту (4)';en='every minute (4)'");
					Иначе
						лкЧастотаОбновления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='неопределено (%1)';ru='неопределено (%1)';en='undefined (%1)'"),
							Строка(Выборка.ЛентаНовостейЧастотаОбновления));
					КонецЕсли;

					РезультатВыполнения_ЛентаНовостей = НСтр("es='La cinta de las novedades %ЛентаНовостей del %, la frecuencia de la renovación de la cinta de las novedades: %ЧастотаОбновления del %, la fecha de la última carga exitosa: %ДатаЗагрузкиПоследнихНовостей del %, los fracasos: %КоличествоНеудачПриЗагрузкеНовостей del %, la última tentativa de la carga: %ДатаПоследнейПопыткиЗагрузкиНовостей del %. %Действие del %';ru='Лента новостей %ЛентаНовостей%, частота обновления ленты новостей: %ЧастотаОбновления%, дата последней успешной загрузки: %ДатаЗагрузкиПоследнихНовостей%, неудач: %КоличествоНеудачПриЗагрузкеНовостей%, последняя попытка загрузки: %ДатаПоследнейПопыткиЗагрузкиНовостей%. %Действие%';en='News feed %ЛентаНовостей%, частота обновления ленты новостей: %ЧастотаОбновления%, дата последней успешной загрузки: %ДатаЗагрузкиПоследнихНовостей%, неудач: %КоличествоНеудачПриЗагрузкеНовостей%, last download attempt: %ДатаПоследнейПопыткиЗагрузкиНовостей%. %Действие%'");
					...
					РезультатВыполнения_ЛентаНовостей = СтрЗаменить(
						...
						?(Выборка.ДатаЗагрузкиПоследнихНовостей = '00010101',
							НСтр("es='(Nunca)';ru='(никогда)';en='(never)'"),
					...
					Если Выборка.ЛентаНовостейАктивна = Истина Тогда
						...
						Если СтруктураНовости.Ссылка.Пустая() Тогда
							...
							РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("es='La cinta de las novedades es activa, la novedad sobre неактивности no existen';ru='Лента новостей активна, новости о неактивности нет';en='News feed is active, there is no news about inactivity'"));
						ИначеЕсли СтруктураНовости.ПометкаУдаления = Истина Тогда
							...
							РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("es='La cinta de las novedades es activa, la novedad sobre неактивности era marcada ya a la desaparición';ru='Лента новостей активна, новость о неактивности уже была помечена на удаление';en='News feed is active, news of inactivity has already been flagged for deletion'"));
						Иначе
							// Пометить найденную новость на удаление.
							НовостьОбъект = СтруктураНовости.Ссылка.ПолучитьОбъект();
							НовостьОбъект.УстановитьПометкуУдаления(Истина);
							РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("es='La cinta de las novedades es activa, la novedad sobre неактивности será marcada a la desaparición';ru='Лента новостей активна, новость о неактивности будет помечена на удаление';en='News feed is active, news of inactivity will be marked for deletion'"));
						КонецЕсли;
					Иначе
						// Найти новость и обновить. Если ее нет - создать и заполнить.
						Если СтруктураНовости.Ссылка.Пустая() Тогда
							НовостьОбъект = Справочники.Новости.СоздатьЭлемент();
							РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("es='La cinta de las novedades es inactiva, la novedad sobre неактивности será creada';ru='Лента новостей НЕактивна, новость о неактивности будет создана';en='News feed is inactive, news about inactivity will be created'"));
						Иначе
							НовостьОбъект = СтруктураНовости.Ссылка.ПолучитьОбъект();
							РезультатВыполнения_ЛентаНовостей = СтрЗаменить(РезультатВыполнения_ЛентаНовостей, "%Действие%", НСтр("es='La cinta de las novedades es inactiva, la novedad sobre неактивности será renovada';ru='Лента новостей НЕактивна, новость о неактивности будет обновлена';en='News feed is inactive, the news of inactivity will be updated'"));
						КонецЕсли;
						...
						НовостьОбъект.Наименование                       = НСтр("es='La cinta de las novedades es inactiva';ru='Лента новостей неактивна';en='News feed inactive'");
						НовостьОбъект.Подзаголовок                       = НСтр("es='La cinta de las novedades es inactiva. Se Recomienda excluirla de la autorenovación';ru='Лента новостей неактивна. Рекомендуется исключить ее из автообновления';en='News feed is inactive. It is recommended to exclude it from auto-update.'");
						...
						Если Отказ <> Истина Тогда
							...
							Исключение
								ИнформацияОбОшибке = ИнформацияОбОшибке();
								ТекстСообщения = НСтр("es='Había una falta a la creación / la renovación de la novedad sobre неактивности las cintas de las novedades %ЛентаНовостей del % "
"Detalladamente: "
"%ИнформацияОбОшибке del %';ru='Произошла ошибка при создании / обновлении новости о неактивности ленты новостей %ЛентаНовостей%"
"Подробно:"
"%ИнформацияОбОшибке%';en='An error occurred while creating / updating news about inactive news feed %ЛентаНовостей% In detail:         %ИнформацияОбОшибке%'")
									+ Символы.ПС;
							...
							КонецПопытки;
						Иначе
							РезультатВыполнения_ЛентаНовостей = РезультатВыполнения_ЛентаНовостей + Символы.ПС + НСтр("es='Es anulada la creación / el cambio de la novedad sobre la ausencia de la actividad para la cinta de las novedades';ru='Отменено создание / изменение новости про отсутствие активности для ленты новостей';en='Created / changed news about the inactivity for the news feed is canceled'");
						КонецЕсли;
					КонецЕсли;
			...
			Иначе
				ОписаниеРезультата = НСтр("es='La interpelación vacío.';ru='Запрос пустой.';en='The request is empty.'") + Символы.ПС;
			КонецЕсли;

			ТекстСообщения = НСтр("es='La terminación de la comprobación de las cintas de las novedades a la actualidad. "
"La cinta de las novedades se considera NO actual, si"
" - por ella era más de 10 intentos fallados de la recepción de las novedades,"
" - la última tentativa acertada era más de día atrás,"
" es no la cinta obligatoria de las novedades,"
" es no cargado con los servidores 1С la cinta de las novedades. "
""
"El resultado de la ejecución: "
"%РезультатВыполнения del %';ru='Окончание проверки лент новостей на актуальность."
...
"%РезультатВыполнения%';en='The end of checking news feeds for relevance."
"A news feed is considered to be irrelevant if"
"  - on it there were more than 10 unsuccessful attempts to receive news,"
"  - the last successful attempt was more than a day ago,"
"  - This is not a required news feed,"
"  - this is not a news feed downloaded from 1C servers."
""
"Execution result: %РезультатВыполнения%'")
				+ Символы.ПС;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
			НСтр("es='Las novedades. El servicio y el reglamento. НайтиИОбработатьНеактивныеЛентыНовостей';ru='Новости. Сервис и регламент. НайтиИОбработатьНеактивныеЛентыНовостей';en='News. Service and regulations. НайтиИОбработатьНеактивныеЛентыНовостей'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ИзменитьИспользованиеРегламентныхЗаданий(Использовать) Экспорт
	...
	Исключение
		...
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No es encontrada la tarea predestinada de reglamento de VseObnovleniyaNovostey por la causa: "
"%1.';ru='Не найдено предопределенное регламентное задание ВсеОбновленияНовостей по причине:"
"%1.';en='The predetermined scheduled task of AllUpdates of News was not found due to:"
"%1.'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
	КонецПопытки;
	Если ЕстьОшибка = Ложь Тогда
		Если ТипЗнч(ТекущееРегламентноеЗадание) = ТипРегламентноеЗадание Тогда
			...
			Иначе
				...
				ЕстьОшибка = Истина;
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='ОбработкаНовостей.ИзменитьИспользованиеРегламентныхЗаданий: "
"en el parámetro Usar es entregado el significado del tipo incorrecto: %1';ru='ОбработкаНовостей.ИзменитьИспользованиеРегламентныхЗаданий:"
"в параметре Использовать передано значение неправильного типа: %1';en='ОбработкаНовостей.ИзменитьИспользованиеРегламентныхЗаданий: incorrect value passed in parameter Use: %1'"),
					ТипЗнч(Использовать));
			КонецЕсли;
			Если Записывать = Истина Тогда
				ТекущееРегламентноеЗадание.Наименование = НСтр("es='Todas las renovaciones de las novedades';ru='Все обновления новостей';en='All news updates'");
				ТекущееРегламентноеЗадание.Расписание   = ПолучитьРасписаниеОбновленийНовостей(Истина);
				ТекущееРегламентноеЗадание.Записать();
			КонецЕсли;
		Иначе
			ЕстьОшибка = Истина;
			ТекстСообщения = НСтр("es='No es encontrada la tarea predestinada de reglamento de VseObnovleniyaNovostey.';ru='Не найдено предопределенное регламентное задание ВсеОбновленияНовостей.';en='No predetermined scheduled task for ВсеОбновленияНовостей was found.'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ЕстьОшибка = Истина Тогда
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
			НСтр("es='Las novedades. El servicio y el reglamento. ИзменениеИспользованияРегламентныхЗаданий';ru='Новости. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий';en='News. Service and regulations. ИзменениеИспользованияРегламентныхЗаданий'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
Процедура УдалениеНовостей_ПометитьНаУдалениеДанные(ДанныеДляУдаления, КонтекстВыполнения = Неопределено)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='La recepción de la lista de las novedades para la desaparición';ru='Получение списка новостей для удаления';en='Getting a list of news to delete'"));
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Если (ДанныеДляУдаления.Свойство("НовостиДляУдаления"))
				...
				ТекстСообщения = НСтр("es='Hay unas novedades, marcadas a la desaparición, (%ДанныеДляУдаления.НовостиДляУдаления.Количество () el % de las novedades). Es necesario quitarlos a mano a través De todas las funciones / Estandartizado / la Desaparición de los objetos marcados.';ru='Есть помеченные на удаление новости (%ДанныеДляУдаления.НовостиДляУдаления.Количество()% новостей). Их надо удалить вручную через Все функции / Стандартные / Удаление помеченных объектов.';en='There are news marked for deletion (%ДанныеДляУдаления.НовостиДляУдаления.Количество()% новостей).They must be deleted manually through All functions / Accessories / Delete marked objects.'");
			...
			Иначе
				ТекстСообщения = НСтр("es='No hay novedades, marcadas a la desaparición';ru='Нет помеченных на удаление новостей';en='No news tagged to be deleted'");
			КонецЕсли;
			...
			Если (ДанныеДляУдаления.Свойство("НовостиДляУстановкиПометкиУдаления"))
				...
				Для Каждого ТекущаяНовость Из ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления Цикл
					...
					КонецЕсли;
				КонецЦикла;
				ТекстСообщения = НСтр("es='Todas las novedades que han caducado son con éxito marcadas a la desaparición (%ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество () el % de las novedades).';ru='Все устаревшие новости успешно помечены на удаление (%ДанныеДляУдаления.НовостиДляУстановкиПометкиУдаления.Количество()% новостей).';en='All outdated news was successfully flagged for deletion (% DataFor Deletion. News For Installing Marks For Deletion. Number ()% of news).'");
			...
			Иначе
				ТекстСообщения = НСтр("es='No hay novedades que han caducado, que es necesario marcar a la desaparición.';ru='Нет устаревших новостей, которые надо пометить на удаление.';en='There are no outdated news items that should be flagged for deletion.'");
			КонецЕсли;
			...
			Если (ДанныеДляУдаления.Свойство("КатегорииНовостейПростыеНеобработанные"))
				...
				КонецЦикла;
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Era quitado %1 categorías en bruto simples.';ru='Было удалено %1 необработанных простых категорий.';en='% 1 unprocessed simple categories have been removed.'"),
					ДанныеДляУдаления.КатегорииНовостейПростыеНеобработанные.Количество());
			Иначе
				ТекстСообщения = НСтр("es='De las categorías simples en bruto no era encontrado.';ru='Простых необработанных категорий не было найдено.';en='No simple raw categories were found.'");
			КонецЕсли;
			...
			Если (ДанныеДляУдаления.Свойство("КатегорииНовостейИнтервалыВерсийНеобработанные"))
				...
				КонецЦикла;
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Era quitado %1 categorías en bruto de los intervalos de las versiones.';ru='Было удалено %1 необработанных категорий интервалов версий.';en='% 1 unhandled version interval categories were deleted.'"),
					ДанныеДляУдаления.КатегорииНовостейИнтервалыВерсийНеобработанные.Количество());
			Иначе
				ТекстСообщения = НСтр("es='De las categorías en bruto de los intervalos de las versiones no era encontrado.';ru='Необработанных категорий интервалов версий не было найдено.';en='No raw version interval categories were found.'");
			КонецЕсли;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
			НСтр("es='Las novedades. El servicio y el reglamento. УдалитьНовости_Ежедневно. Después de la recogida de los datos para la desaparición';ru='Новости. Сервис и регламент. УдалитьНовости_Ежедневно. После сбора данных для удаления';en='News. Service and regulations. УдалитьНовости_Ежедневно.  After collecting data for deletion'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ЗагрузитьВсеСтандартныеЗначения(КонтекстВыполнения = Неопределено)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='La carga de los significados estandartizados';ru='Загрузка стандартных значений';en='Loading default values'"));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. El servicio y el reglamento';ru='БИП:Новости.Сервис и регламент';en='BIP: News. Service and regulations'"), // ИмяСобытия
			НСтр("es='Las novedades. El servicio y el reglamento. La carga de los significados estandartizados. Todos los significados estandartizados';ru='Новости. Сервис и регламент. Загрузка стандартных значений. Все стандартные значения';en='News. Service and regulations. Loading standard values. All default values'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
#Область ОсновныеДействия
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("es='Las acciones básicas';ru='Основные действия';en='Main actions'"));

				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("es='La comprobación de la corrección de la introducción del subsistema. El comienzo.';ru='Проверка правильности внедрения подсистемы. Начало.';en='Checking of the implementation of the subsystem. Start.'")
					+ Символы.ПС;
				Результат = ПроверитьПравильностьВнедренияПодсистемы("ТолькоПереопределения");
				Если НЕ ПустаяСтрока(Результат) Тогда
					ТекстСообщения = НСтр("es='Son descubiertas las faltas de la introducción del subsistema de InternetPodderzhkaPolzovateley. Las novedades. "
"Las faltas descubiertas de la introducción: "
"%Результат del %';ru='Обнаружены ошибки внедрения подсистемы ИнтернетПоддержкаПользователей.Новости."
"Обнаруженные ошибки внедрения:"
"%Результат%';en='Subsystem implementation errors detected ИнтернетПоддержкаПользователей.News."
"Detected implementation errors:"
"%Результат%'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Результат%", Результат);
					ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("es='БИП:Новости. La renovación ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: News. Information Security Update'"), // ИмяСобытия
						НСтр("es='Las novedades. La renovación ИБ. La comprobación de la introducción';ru='Новости. Обновление ИБ. Проверка внедрения';en='News. IB update. Verification of implementation'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
						, // ОбъектМетаданных
						, // Данные
						ТекстСообщения, // Комментарий
				...
				КонецЕсли;
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("es='La comprobación de la corrección de la introducción del subsistema. El fin.';ru='Проверка правильности внедрения подсистемы. Конец.';en='Checking of the implementation of the subsystem. The end.'")
					+ Символы.ПС;

				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("es='La instalación de las constantes básicas. El comienzo.';ru='Установка основных констант. Начало.';en='Setting basic constants. Start.'")
					+ Символы.ПС;
				...
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("es='La instalación de las constantes básicas. El fin.';ru='Установка основных констант. Конец.';en='Setting basic constants. The end.'")
					+ Символы.ПС;
				...
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("es='La limpieza del registro de DatyZagruzkiPoslednihNovostey. El comienzo.';ru='Очистка регистра ДатыЗагрузкиПоследнихНовостей. Начало.';en='Clearing register ДатыЗагрузкиПоследнихНовостей. Start.'")
					+ Символы.ПС;
				НаборЗаписей = РегистрыСведений.ДатыЗагрузкиПоследнихНовостей.СоздатьНаборЗаписей();
				НаборЗаписей.Записать(Истина);
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("es='La limpieza del registro de DatyZagruzkiPoslednihNovostey. El fin.';ru='Очистка регистра ДатыЗагрузкиПоследнихНовостей. Конец.';en='Clearing register ДатыЗагрузкиПоследнихНовостей. The end.'")
					+ Символы.ПС;
				...
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("es='La carga de los significados estandartizados. El comienzo.';ru='Загрузка стандартных значений. Начало.';en='Loading standard values. Start.'")
					+ Символы.ПС;
				ЗагрузитьВсеСтандартныеЗначения(КонтекстВыполненияВложенныйВложенный);
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("es='La carga de los significados estandartizados. El fin.';ru='Загрузка стандартных значений. Конец.';en='Loading standard values. End.'")
					+ Символы.ПС;
				...
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("es='La instalación de la fecha de la carga de las novedades en-3 del mes. El comienzo.';ru='Установка даты загрузки новостей на -3 месяца. Начало.';en='Setting the news upload date to -3 months. Start.'")
					+ Символы.ПС;
				Запрос = Новый Запрос;
				Запрос.Текст = "
					|ВЫБРАТЬ
				...
				КонецЕсли;
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("es='La instalación de la fecha de la carga de las novedades en-3 del mes. El fin.';ru='Установка даты загрузки новостей на -3 месяца. Конец.';en='Setting the news upload date to -3 months. The end.'")
					+ Символы.ПС;

				// Установить / переустановить значение расписания для регламентного задания ВсеОбновленияНовостей.
				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ НСтр("es='La instalación del horario de la tarea de reglamento. El comienzo.';ru='Установка расписания регламентного задания. Начало.';en='Setting a scheduled task. Start.'")
					+ Символы.ПС;
				ТекущееРегламентноеЗадание = РегламентныеЗаданияСервер.Задание(Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей);
				Если ТекущееРегламентноеЗадание <> Неопределено Тогда
					ТекущееРегламентноеЗадание.Наименование = НСтр("es='Todas las renovaciones de las novedades';ru='Все обновления новостей';en='All news updates'");
					...
					ТекущееРегламентноеЗадание.Записать();
					ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
						+ НСтр("es='La instalación del horario de la tarea de reglamento. El fin.';ru='Установка расписания регламентного задания. Конец.';en='Setting a scheduled task. The end.'")
						+ Символы.ПС;
				Иначе
					ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
						+ НСтр("es='La tarea de reglamento de VseObnovleniyaNovostey no es encontrada.';ru='Регламентное задание ВсеОбновленияНовостей не найдено.';en='Regulatory task ВсеОбновленияНовостей not found.'")
						+ Символы.ПС
						+ НСтр("es='La instalación del horario de la tarea de reglamento. El fin.';ru='Установка расписания регламентного задания. Конец.';en='Setting a scheduled task. The end.'")
						+ Символы.ПС;
				КонецЕсли;
...
#Область ПереопределяемыеДействия
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("es='Las acciones redeterminadas';ru='Переопределяемые действия';en='Override actions'"));
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. La renovación ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: News. Information Security Update'"), // ИмяСобытия
			НСтр("es='Las novedades. La renovación ИБ. ОбщиеДанные. ПервыйЗапуск';ru='Новости. Обновление ИБ. ОбщиеДанные. ПервыйЗапуск';en='News. IB update. ОбщиеДанные. ПервыйЗапуск'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ОбновлениеИнформационнойБазы_ОбластьДанных_ПервыйЗапуск() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		Если Пользователи.ЭтоСеансВнешнегоПользователя() Тогда
			...
			ТекстСообщения = НСтр("es='El trabajo con las novedades no es apoyado para el usuario exterior. El elaborador de la renovación es dejado pasar.';ru='Работа с новостями не поддерживается для внешнего пользователя. Обработчик обновления пропущен.';en='Work with news is not supported for an external user. The update handler is skipped.'");
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("es='БИП:Новости. La renovación ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: News. Information Security Update'"), // ИмяСобытия
				НСтр("es='Las novedades. La renovación ИБ. ОбластьДанных. ПервыйЗапуск. El fin';ru='Новости. Обновление ИБ. ОбластьДанных. ПервыйЗапуск. Конец';en='News. IB update. ОбластьДанных. ПервыйЗапуск. Конец'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
...
#Область ОсновныеДействия
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				Иначе
					ОписаниеРезультатаВложенный = НСтр("es='El trabajo con las novedades es desconectado. El elaborador es dejado pasar.';ru='Работа с новостями отключена. Обработчик пропущен.';en='Work with news is disabled. Handler skipped.'");
				КонецЕсли;
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. La renovación ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: News. Information Security Update'"), // ИмяСобытия
			НСтр("es='Las novedades. La renovación ИБ. ОбластьДанных. ПервыйЗапуск';ru='Новости. Обновление ИБ. ОбластьДанных. ПервыйЗапуск';en='News. IB update. ОбластьДанных. ПервыйЗапуск'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ОбновлениеИнформационнойБазы_ОбщиеДанные_ПерейтиНаВерсию() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El tránsito a la versión %1 (los datos generales)';ru='Переход на версию %1 (общие данные)';en='Going to version%1 (shared)'"),
			ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()));
...
"%Результат%'");
				...
				Иначе
					ОписаниеРезультата = НСтр("es='El trabajo con las novedades es desconectado.';ru='Работа с новостями отключена.';en='Work with news is disabled.'") + " ";
					// Записать об отключении подсистемы в журнал регистрации.
					ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("es='БИП:Новости. La renovación ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: News. Information Security Update'"), // ИмяСобытия
						НСтр("es='Las novedades. La renovación ИБ. ОбщиеДанные. ПерейтиНаВерсию';ru='Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию';en='News. IB update. ОбщиеДанные. ПерейтиНаВерсию'"), // ИдентификаторШага
						УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
						, // ОбъектМетаданных
						0, // Данные
						НСтр("es='El trabajo con las novedades es desconectado. Las novedades, incluso importante, no obrarán.';ru='Работа с новостями отключена. Новости, включая важные, не будут поступать.';en='Work with news is disabled. News, including important ones, will not be received.'"), // Комментарий
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. La renovación ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: News. Information Security Update'"), // ИмяСобытия
			НСтр("es='Las novedades. La renovación ИБ. ОбщиеДанные. ПерейтиНаВерсию_ХХХ';ru='Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию_ХХХ';en='News. IB update. ОбщиеДанные. ПерейтиНаВерсию_ХХХ'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
#Область ОсновныеДействия_ВключениеФОНовости
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("es='Las acciones básicas / la Instalación ФО Es permitida el trabajo con las novedades';ru='Основные действия / Установка ФО Разрешена работа с новостями';en='Main actions / Setting the FO News enabled'"));
...
#Область ОсновныеДействия_НастройкаПараметровДоступаКСерверамНовостей
			...
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				НСтр("es='Las acciones básicas / los Ajustes de las novedades';ru='Основные действия / Настройки новостей';en='Main actions / News settings'"));
...
Процедура ОбновлениеИнформационнойБазы_ОбластьДанных_ПерейтиНаВерсию() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		"ОбработкаНовостей." + НаименованиеПроцедурыФункции, // Идентификатор.
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El tránsito a la versión %1 (los datos divididos)';ru='Переход на версию %1 (разделенные данные)';en='Going to version%1 (shared data)'"),
			ИнтернетПоддержкаПользователейКлиентСервер.ВерсияБиблиотеки()));

		ЕстьОшибки = Ложь;

		Если Пользователи.ЭтоСеансВнешнегоПользователя() Тогда
			ОписаниеРезультата = НСтр("es='El trabajo con las novedades no es apoyado para el usuario exterior. El elaborador de la renovación es dejado pasar.';ru='Работа с новостями не поддерживается для внешнего пользователя. Обработчик обновления пропущен.';en='Work with news is not supported for an external user. The update handler is skipped.'");
			ЕстьОшибки = Истина;
		КонецЕсли;
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. La renovación ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: News. Information Security Update'"), // ИмяСобытия
			НСтр("es='Las novedades. La renovación ИБ. ОбластьДанных. ПерейтиНаВерсию';ru='Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию';en='News. IB update. ОбластьДанных. ПерейтиНаВерсию'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. La renovación ИБ';ru='БИП:Новости.Обновление ИБ';en='BIP: News. Information Security Update'"), // ИмяСобытия
			НСтр("es='Las novedades. La renovación ИБ. ОбластьДанных. ПерейтиНаВерсию_ХХХ';ru='Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию_ХХХ';en='News. IB update. ОбластьДанных. ПерейтиНаВерсию_ХХХ'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Функция ПроверитьПравильностьВнедренияПодсистемы(ВариантПроверки = "Все") Экспорт
	...
	Если ВариантПроверки = "Все" Тогда
		...
		Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
			Результат = Результат
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Las faltas del ajuste del requisito general De OblastDannyhOsnovnyeDannye: "
"%1';ru='Ошибки настройки общего реквизита ОбластьДанныхОсновныеДанные:"
"%1';en='Errors for setting general props ОбластьДанныхОсновныеДанные:"
"%1'")
						+ Символы.ПС
						+ Символы.ПС,
					РезультатЛокальный);
		...
		Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
			Результат = Результат
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Las faltas del ajuste del requisito general De OblastDannyhVspomogatelnyeDannye: "
"%1';ru='Ошибки настройки общего реквизита ОбластьДанныхВспомогательныеДанные:"
"%1';en='Errors for setting general props ОбластьДанныхВспомогательныеДанные:"
"%1'")
						+ Символы.ПС
						+ Символы.ПС,
					РезультатЛокальный);
	...
	Если ВариантПроверки = "Все" Тогда
		...
		Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
			Результат = Результат
				+ НСтр("es='Las faltas del ajuste de los derechos para el papel de AdministratorSistemy:';ru='Ошибки настройки прав для роли АдминистраторСистемы:';en='Errors in setting rights for a role АдминистраторСистемы:'")
		...
		Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
			Результат = Результат
				+ НСтр("es='Las faltas del ajuste de los derechos para el papel De PolnyePrava:';ru='Ошибки настройки прав для роли ПолныеПрава:';en='Errors in setting rights for a role ПолныеПрава:'")
				+ Символы.ПС + РезультатЛокальный
				+ Символы.ПС + Символы.ПС;
		КонецЕсли;
	...
	Если (ВариантПроверки = "Все") ИЛИ (ВариантПроверки = "ТолькоПереопределения") Тогда
		...
		Если ВРег(ИмяКонфигурации) = ВРег("Unknown") Тогда
			РезультатЛокальный = НСтр("es='de la configuración. "
"Su ajuste tiene que hacer los cambios en el módulo general "
"ОбработкаНовостейПереопределяемый.ЗначениеПредопределеннойКатегории, "
"para la línea de la condición ProductName. "
"En caso contrario las novedades para esta configuración no llegarán.';ru='Не настроен идентификатор конфигурации."
"Для его настройки необходимо внести изменения в общий модуль"
"ОбработкаНовостейПереопределяемый.ЗначениеПредопределеннойКатегории,"
"для строки условия ProductName."
"В противном случае новости для этой конфигурации приходить НЕ будут.';en='The configuration identifier is not configured."
"To configure it, you need to make changes to the general module"
"ОбработкаНовостейПереопределяемый.ЗначениеПредопределеннойКатегории,"
"for the condition line ProductName."
"Otherwise, the news for this configuration will NOT come.'")
				+ Символы.ПС;
		КонецЕсли;
		...
		Если СписокЗапрещенныхСимволов.Количество() > 0 Тогда
			...
			КонецЕсли;
			РезультатЛокальный = РезультатЛокальный  + НСтр("es='Es incorrectamente ajustado el identificador de la configuración. "
"Su ajuste tiene que hacer los cambios en el módulo general "
"ОбработкаНовостейПереопределяемый.ЗначениеПредопределеннойКатегории, "
"para la línea de la condición ProductName. "
"El identificador de la configuración debe consistir de las letras inglesas, las cifras, el signo menos y el signo del subrayado.';ru='Неправильно настроен идентификатор конфигурации."
"Для его настройки необходимо внести изменения в общий модуль"
"ОбработкаНовостейПереопределяемый.ЗначениеПредопределеннойКатегории,"
"для строки условия ProductName."
"Идентификатор конфигурации должен состоять из английских букв, цифр, знака минуса и знака подчеркивания.';en='Incorrect configuration id configured."
"To configure it, you need to make changes to the general module"
"ОбработкаНовостейПереопределяемый.ЗначениеПредопределеннойКатегории,"
"for the condition line ProductName."
"The configuration identifier must consist of English letters, numbers, a minus sign and an underscore.'")
				+ Символы.ПС;
		КонецЕсли;

		Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
			Результат = Результат
				+ НСтр("es='Las faltas del ajuste del código de la configuración:';ru='Ошибки настройки кода конфигурации:';en='Errors in setting the configuration code:'")
				+ Символы.ПС + РезультатЛокальный
				+ Символы.ПС + Символы.ПС;
		КонецЕсли;
	...
	Если (ВариантПроверки = "Все") ИЛИ (ВариантПроверки = "ТолькоПереопределения") Тогда
		...
		Если МассивСлов.Количество() <> 4 Тогда
			РезультатЛокальный = НСтр("es='Es incorrectamente ajustada la versión de la configuración (la cantidad de los números perfectamente de 4). "
"En los metadatos, en el campo la Versión deben ser los datos del tipo А.Б.В.Г (cuatro números divididos por el punto). "
"Si por unas causas la versión en los metadatos es dada en excelente de А.Б.В.Г el formato, "
"es necesario hacer los cambios en el módulo general "
"ОбработкаНовостейПереопределяемый.ЗначениеПредопределеннойКатегории, "
"para la línea de la condición ProductVersion.';ru='Неправильно настроена версия конфигурации (количество чисел отлично от 4)."
...
"ОбработкаНовостейПереопределяемый.ЗначениеПредопределеннойКатегории,"
"для строки условия ProductVersion.';en='The configuration version is incorrectly configured (the number of numbers is different from 4)."
"The metadata in the Version field should contain data of the form A.B.C.D. (four numbers separated by a period)."
"If for some reason the version in the metadata is specified in a format other than A.B.C.D.,"
"then you need to make changes to the general module"
"ОбработкаНовостейПереопределяемый.ЗначениеПредопределеннойКатегории,"
"for the condition line ProductVersion. '")
		Иначе
			...
			Если НайденныеНеправильныеСимволы.Количество() > 0 Тогда
				РезультатЛокальный = НСтр("es='Es incorrectamente ajustada la versión de la configuración (hay unos símbolos incorrectos). "
"En los metadatos, en el campo la Versión deben ser los datos del tipo А.Б.В.Г (cuatro números divididos por el punto). "
"Si por unas causas la versión en los metadatos es dada en excelente de А.Б.В.Г el formato, "
"es necesario hacer los cambios en el módulo general "
"ОбработкаНовостейПереопределяемый.ЗначениеПредопределеннойКатегории, "
"para la línea de la condición ProductVersion. "
"La lista de los símbolos incorrectos: %НайденныеНеправильныеСимволы del %';ru='Неправильно настроена версия конфигурации (есть неправильные символы)."
"В метаданных, в поле Версия должны быть данные вида А.Б.В.Г (четыре числа, разделенные точкой)."
"Если по каким-то причинам версия в метаданных задана в отличном от А.Б.В.Г формате,"
"то необходимо внести изменения в общий модуль"
"ОбработкаНовостейПереопределяемый.ЗначениеПредопределеннойКатегории,"
"для строки условия ProductVersion."
"Список неправильных символов: %НайденныеНеправильныеСимволы%';en='The configuration version is incorrectly configured (there are incorrect characters)."
"The metadata in the Version field should contain data of the form A.B.C.D. (four numbers separated by a period)."
"If for some reason the version in the metadata is specified in a format other than A.B.C.D.,"
"then you need to make changes to the general module"
"Processing News Redefinable. The value of the Predefined Category,"
"for the condition line ProductVersion."
"List of invalid characters:%НайденныеНеправильныеСимволы%'");
		...
		Если НЕ ПустаяСтрока(РезультатЛокальный) Тогда
			Результат = Результат
				+ НСтр("es='Las faltas del ajuste de la versión de la configuración:';ru='Ошибки настройки версии конфигурации:';en='Configuration version configuration errors:'")
				+ Символы.ПС + РезультатЛокальный
				+ Символы.ПС + Символы.ПС;
		КонецЕсли;
	...
	Если НайденныйЭлемент <> Неопределено Тогда
		...
		Иначе
			ТекущийРезультат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es incorrectamente establecido el uso para %1 en general el requisito %2. "
"Es establecido: %3, se espera: %4. "
"Es admisible También que a %1 es establecido el uso De Avto, pero entonces cerca del requisito general %2 debe ser establecido el Autouso en %4.';ru='Неправильно установлено использование для %1 в общем реквизите %2."
"Установлено: %3, ожидается: %4."
"Также допустимо, что у %1 установлено использование Авто, но тогда у общего реквизита %2 должно быть установлено Автоиспользование в %4.';en='The usage for%1 in the general props%2 is incorrectly set."
"Installed:%3, expected:%4."
"It’s also acceptable that%1 has Auto set to use, but then the general attribute%2 should have Auto use set to%4.'"),
		...
		КонецЕсли;
	Иначе
		ТекущийРезультат = НСтр("es='No es encontrado el objeto exigido de los metadatos: %ПолноеИмяМетаданных del %';ru='Не найден требуемый объект метаданных: %ПолноеИмяМетаданных%';en='The required metadata object was not found:%ПолноеИмяМетаданных%'");
...
Процедура Проверка_Права(
	...
	Если ТекущееЗначениеПрава <> ПравильноеЗначениеПрава Тогда
		ТекущийРезультат = НСтр("es='Es incorrectamente establecido el derecho %Право del % en el papel %Роль del % para %ПолноеИмяМетаданных del %. Es establecido: %ТекущееЗначениеПрава del %, se espera: %ПравильноеЗначениеПрава del %';ru='Неправильно установлено право %Право% в роли %Роль% для %ПолноеИмяМетаданных%. Установлено: %ТекущееЗначениеПрава%, ожидается: %ПравильноеЗначениеПрава%';en='Incorrectly set right %Право% in role %Роль% for %ПолноеИмяМетаданных%. Installed: %ТекущееЗначениеПрава%, expected: %ПравильноеЗначениеПрава%'");
		ТекущийРезультат = СтрЗаменить(
			ТекущийРезультат,
			"%ПолноеИмяМетаданных%",
			ОбщегоНазначения.ИмяБазовогоТипаПоОбъектуМетаданных(лкОбъектМетаданных) + "." + лкОбъектМетаданных.Имя);
...
Функция КомментарийДляЖурналаРегистрации(ОписаниеШага, ШагВыполнения, КонтекстВыполнения, Вариант = "Простой") Экспорт
	...
	Если Вариант = "Расширенный" Тогда
		...
		ТекстСообщения = НСтр("es='%ОписаниеШага del % "
"la Línea de la conexión: %СтрокаСоединенияИнформационнойБазы del % "
"de TekuschiyRezhimZapuska: %ТекущийРежимЗапуска del % "
"Es la configuración dividida: %ЭтоРазделеннаяКонфигурация del % "
"la División es incluida: %РазделениеВключено del % "
...
"	La División de los datos:"
"		%ТекущийПользователь_РазделениеДанных del % "
"el Tiempo del comienzo (мс): %ВремяНачала del % "
"el Tiempo de la terminación (мс): %ВремяОкончания del % "
"la Duración (мс): %Длительность del % "
"el Resultado por los pasos: "
"%РезультатПоШагам del %';ru='%ОписаниеШага%"
"Строка подключения: %СтрокаСоединенияИнформационнойБазы%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска%"
"Это разделенная конфигурация: %ЭтоРазделеннаяКонфигурация%"
"Разделение включено: %РазделениеВключено%"
...
"Результат по шагам:"
"%РезультатПоШагам%';en='%ОписаниеШага%"
"Connection string: %СтрокаСоединенияИнформационнойБазы%"
"ТекущийРежимЗапуска: %ТекущийРежимЗапуска%"
"This is a split configuration: %ЭтоРазделеннаяКонфигурация%"
"Separation included: %РазделениеВключено%"
"Using of shared data is available: %ДоступноИспользованиеРазделенныхДанных%"
"Session started without delimiters: %СеансЗапущенБезРазделителей%"
"Configuration (metadata): %Метаданные_Имя%, %Метаданные_Версия%"
"Configuration (override): %Переопределение_Имя%, %Переопределение_Версия%"
"Platform: %Платформа_Версия%"
"Functional option News operation is enabled: %ОбработкаНовостейПовтИсп_РазрешенаРаботаСНовостями%"
"Functional option Allowed to work with news via the Internet: %ОбработкаНовостейПовтИсп_РазрешенаРаботаСНовостямиЧерезИнтернет%"
"Current IB user:"
"	Name: %ТекущийПользователь_Имя%"
"	UIN: %ТекущийПользователь_УникальныйИдентификатор%"
