...
Процедура ОбновлениеИндексаППД() Экспорт
	...
	ОбновитьИндекс(НСтр("es='La renovación del índice ППД';ru='Обновление индекса ППД';en='PPD Index Update'"), Ложь, Истина);
...
Процедура СлияниеИндексаППД() Экспорт
	...
	ОбновитьИндекс(НСтр("es='La fusión del índice ППД';ru='Слияние индекса ППД';en='PPD index merger'"), Истина);
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Представление = НСтр("es='La falta del ajuste полнотекстового de la búsqueda';ru='Ошибка настройки полнотекстового поиска';en='Error setting full-text search'");
	Дело.Форма = "Обработка.ПолнотекстовыйПоискВДанных.Форма.УправлениеПолнотекстовымПоискомИИзвлечениемТекстов";
	Дело.Подсказка = 
		НСтр("es='Рассинхронизация los significados de la constante de IspolzovatPolnotekstovyyPoisk "
"y el régimen establecido полнотекстового de la búsqueda en la base informativa. "
"Prueben desconectar полнотекстовый la búsqueda y de nuevo incluir.';ru='Рассинхронизация значения константы ИспользоватьПолнотекстовыйПоиск"
"и установленного режима полнотекстового поиска в информационной базе."
"Попробуйте выключить полнотекстовый поиск и снова включить.';en='Constant value out of sync ИспользоватьПолнотекстовыйПоиск and the established full-text search mode in the infobase."
"Try turning off full-text search and turning it back on.'");
	...
	Если Состояние = "ТребуетсяОбновлениеИндекса" Тогда 
		...
		Если ДатаАктуальностиИндекса > ТекущаяДата Тогда
			Интервал = НСтр("es='Menos hace uno días';ru='менее одного дня назад';en='less than one day ago'");
		Иначе
			Интервал = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='%1 atrás';ru='%1 назад';en='% 1 back'"),
	...
	Иначе 
		Интервал = НСтр("es='Nunca';ru='Никогда';en='Never'");
		ЕстьДела = Ложь;
	КонецЕсли;
	...
	Дело.Представление = НСтр("es='El índice полнотекстового de la búsqueda ha caducado';ru='Индекс полнотекстового поиска устарел';en='Full text search index is outdated'");
	Дело.Форма = "Обработка.ПолнотекстовыйПоискВДанных.Форма.УправлениеПолнотекстовымПоискомИИзвлечениемТекстов";
	Дело.Подсказка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La última renovación %1';ru='Последнее обновление %1';en='Last update %1'"),
		Интервал);
	Дело.Владелец = Раздел;
...
Функция ИндексПолнотекстовогоПоискаАктуален()
	...
	Исключение
		ЗаписьЖурнала(
			...
			НСтр("es='No consiguió comprobar el estado del índice полнотекстового de la búsqueda';ru='Не удалось проверить состояние индекса полнотекстового поиска';en='Failed to check the status of the full-text search index'"),
			ИнформацияОбОшибке());
	КонецПопытки;
...
Функция ВыполнитьПолнотекстовыйПоиск(ПараметрыПоиска) Экспорт 
	...
	Попытка
		...
		Иначе 
			ВызватьИсключение НСтр("es='El parámetro de NapravleniePoiska es dado incorrectamente.';ru='Параметр НаправлениеПоиска задан неверно.';en='The НаправлениеПоиска parameter is invalid.'");
		КонецЕсли;
	Исключение
	...
	Если СписокПоиска.СлишкомМногоРезультатов() Тогда 
		ОписаниеОшибки = НСтр("es='Es demasidos resultados, precisen la interpelación';ru='Слишком много результатов, уточните запрос';en='Too many results, specify a query'");
	...
	Если ПолноеКоличество = 0 Тогда
		ОписаниеОшибки = НСтр("es='A petición de nada es encontrado';ru='По запросу ничего не найдено';en='Nothing found on request'");
...
Процедура ОбновитьИндекс(ПредставлениеПроцедуры, РазрешитьСлияние = Ложь, Порциями = Ложь)
	...
	ЗаписьЖурнала(
		Неопределено, 
		НСтр("es='El lanzamiento del procedimiento ""%1"".';ru='Запуск процедуры ""%1"".';en=""Launching procedure '%1'."""),,
	...
	Попытка
		...
		ЗаписьЖурнала(
			Неопределено, 
			НСтр("es='La terminación exitosa del procedimiento ""%1"".';ru='Успешное завершение процедуры ""%1"".';en=""Successful completion of procedure '%1'."""),, 
			ПредставлениеПроцедуры);
	Исключение
		ЗаписьЖурнала(
			УровеньЖурналаРегистрации.Предупреждение, 
			НСтр("es='No consiguió cumplir el procedimiento ""%1"":';ru='Не удалось выполнить процедуру ""%1"":';en=""Failed to execute procedure '%1':"""),
			ИнформацияОбОшибке(), 
			ПредставлениеПроцедуры);
	КонецПопытки;
...
Процедура ЗаписьЖурнала(
	...
	ЗаписьЖурналаРегистрации(
		НСтр("es='La indexación polnotekstovoe';ru='Полнотекстовое индексирование';en='Full text indexing'", ОбщегоНазначения.КодОсновногоЯзыка()), 
		УровеньЖурнала, , , 
		ТекстДляЖурнала);
