	...
	Для НомерСтроки = 1 По СтрЧислоСтрок(ОграниченияПрав) Цикл
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока) Тогда
			...
			Если СтрЧислоВхождений(ТекущаяСтрока, ".") <> 3 И СтрЧислоВхождений(ТекущаяСтрока, ".") <> 5 Тогда
				ПояснениеОшибки = НСтр("es='La línea debe ser en el formato ""<el nombre Completo de la tabla>. <el Nombre del derecho>. <el Nombre del tipo del acceso> [.Таблица del objeto]"".';ru='Строка должна быть в формате ""<Полное имя таблицы>.<Имя права>.<Имя вида доступа>[.Таблица объекта]"".';en='The line must be in the format ""<Полное имя таблицы>.<Имя права>.<Имя вида доступа>[.Таблица объекта]"".'");
			Иначе
				...
				Если Метаданные.НайтиПоПолномуИмени(Таблица) = Неопределено Тогда
					ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='""%1"".';ru='Не найдена таблица ""%1"".';en='Table ""%1"" not found'"), Таблица);
				
				ИначеЕсли Право <> "Чтение" И Право <> "Изменение" Тогда
					ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='No es encontrado el derecho ""%1"".';ru='Не найдено право ""%1"".';en='The right ""%1"" was not found.'"), Право);
				
				ИначеЕсли ВРег(ВидДоступа) = ВРег("Объект") Тогда
					Если Метаданные.НайтиПоПолномуИмени(ТаблицаОбъекта) = Неопределено Тогда
						ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='""%1"".';ru='Не найдена таблица объекта ""%1"".';en=""The table of object '%1' was not found."""),
							ТаблицаОбъекта);
					Иначе
				...
				ИначеЕсли ВРег(ВидДоступа) = ВРег("НастройкиПрав") Тогда
					Если Метаданные.НайтиПоПолномуИмени(ТаблицаОбъекта) = Неопределено Тогда
						ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='del propietario de los ajustes es derecho ""%1"".';ru='Не найдена таблица владельца настроек прав ""%1"".';en='The rights settings owner table ""%1"" could not be found.'"),
							ТаблицаОбъекта);
					Иначе
				...
				ИначеЕсли ВидыДоступаПоИменам.Получить(ВидДоступа) = Неопределено Тогда
					ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='""%1"".';ru='Не найден вид доступа ""%1"".';en=""Access type '%1' not found."""), ВидДоступа);
				Иначе
			...
			Если ЗначениеЗаполнено(ПояснениеОшибки) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La falta en la línea de la descripción del tipo de las restricciones del derecho del objeto de los metadatos: "
"""%1"".';ru='Ошибка в строке описания вида ограничений права объекта метаданных:"
"""%1"".';en='Error in the description line of the type of metadata object’s rights restrictions:"
"""%1"".'")
						+ Символы.ПС
						+ Символы.ПС,
						ТекущаяСтрока)
...
Функция РазрешенныйКлючДоступа() Экспорт
	...
	Если Не ЗначениеЗаполнено(СсылкаВБазеДанных) Тогда
		...
		РазрешенныйКлюч.Наименование = НСтр("es='La clave de acceso permitida';ru='Разрешенный ключ доступа';en='Permitted passkey'");
...
Функция РазрешенныйПустойНаборГруппДоступа() Экспорт
	...
	Если Не ЗначениеЗаполнено(СсылкаВБазеДанных) Тогда
		...
		РазрешенныйПустойНабор.Наименование = НСтр("es='El juego permitido vacío de los grupos del acceso';ru='Разрешенный пустой набор групп доступа';en='Permitted empty access group set'");
...
Функция РазмерностьКлючаДоступа() Экспорт
	...
	Если КоличествоПодобныхЭлементовВКоллекции(МетаданныеКлюча.Реквизиты, "Значение") <> 5 Тогда
		ВызватьИсключение
			НСтр("es='En la guía de KlyuchiDostupa deben ser 5 requisitos de Znachenie* y no más.';ru='В справочнике КлючиДоступа должно быть 5 реквизитов Значение* и не более.';en='In the Access Keys directory there should be 5 details Value * and no more.'");
	КонецЕсли;
	...
	Если МетаданныеКлюча.ТабличныеЧасти.Найти("Шапка") = Неопределено
	 Или КоличествоПодобныхЭлементовВКоллекции(МетаданныеКлюча.ТабличныеЧасти.Шапка.Реквизиты, "Значение", 6) <> 5 Тогда
		ВызватьИсключение
			НСтр("es='En la guía de KlyuchiDostupa debe ser la parte tabular el Gorro con 5 requisitos de Znachenie* y no más.';ru='В справочнике КлючиДоступа должна быть табличная часть Шапка с 5 реквизитами Значение* и не более.';en='In the  directory КлючиДоступа there should be a table part of the Cap with 5 details. Value * and no more.'");
	КонецЕсли;
	...
	Если КоличествоТабличныхЧастей < 1 Или КоличествоТабличныхЧастей > 12 Тогда
		ВызватьИсключение
			НСтр("es='En la guía de KlyuchiDostupa debe ser de 1 hasta 12 partes tabulares de TablichnayaChast*.';ru='В справочнике КлючиДоступа должно быть от 1 до 12 табличных частей ТабличнаяЧасть*.';en='Access Keys must contain from 1 to 12 tabular parts of the Table Part *.'");
	КонецЕсли;
	...
	Если КоличествоРеквизитовТабличнойЧасти = 0 Тогда
		ВызватьИсключение
			НСтр("es='En la guía de KlyuchiDostupa en las partes tabulares de TablichnayaChast* "
"es admisible solamente la cantidad igual de los requisitos de Znachenie* y no más 15.';ru='В справочнике КлючиДоступа в табличных частях ТабличнаяЧасть*"
"допустимо только одинаковое количество реквизитов Значение* и не более 15.';en='In the  directory КлючиДоступа in ТабличнаяЧасть of the Tabular Part *"
"only the same number of details is allowed. Value * and not more than 15. ""'");
	КонецЕсли;
...
Функция КоличествоОпорныхПолейРегистра(ИмяРегистра = "") Экспорт
	...
	Для Каждого Измерение Из МетаданныеРегистраКлючей.Измерения Цикл
		Если Измерение.Имя = "Регистр" Тогда
			...
			КонецЕсли;
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='En el registro separado de las noticias %1 "
"no debe ser la medida %2.';ru='В отдельном регистре сведений %1"
"не должно быть измерения %2.';en='In a separate information register% 1"
"there should not be a measurement of% 2.'"), ИмяРегистра, "Регистр");
		КонецЕсли;
...
Функция ОписаниеПредопределенногоИдентификатораОбъектаМетаданных(ПолноеИмяОбъектаМетаданных) Экспорт
	...
	Если ОбъектМетаданных = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir el nombre del identificador predestinado del objeto de los metadatos "
"puesto que no hay objeto indicado de los metadatos: "
"""%1"".';ru='Не удалось получить имя предопределенного идентификатора объекта метаданных"
"так как не существует указанный объект метаданных:"
"""%1"".';en='Failed to get the name of the predefined metadata object identifier"
"since the specified metadata object does not exist:"
"""%1""'"),
