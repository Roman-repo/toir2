...
//  Параметры - Структура - уточнения при заполнении из формы подбора, Владелец -- ссылка на документ, если он уже записан
Процедура ЗаполнитьДокументНаОсновании(Основание, ДокументОбъект, Параметры = Неопределено) Экспорт
	
	ТипЗначенияДокумента = ТипЗнч(ДокументОбъект.Ссылка);
	
	Если ТипЗначенияДокумента = Тип("ДокументСсылка.торо_АктПриемкиОборудования")  Тогда
		ЗаполнитьАктПриемкиОборудования(Основание, ДокументОбъект);
	ИначеЕсли ТипЗначенияДокумента = Тип("ДокументСсылка.торо_ОстановочныеРемонты")  Тогда
	...
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДанныеФормыСтруктура") Тогда
		...
		ЗаполнитьРемонтыОборудования(ВсеID, ДокументОбъект, Параметры);
		Если Параметры.Свойство("Владелец") Тогда
		    ЗаполнитьРемонтныеРаботы(ВсеID, ДокументОбъект, Параметры.Владелец);
		Иначе
	...
	Иначе
		...
		Для каждого СтрокаРО Из ДокументОбъект[ИмяТЧРемонтов] Цикл
			МассивРО.Добавить(СтрокаРО.id);
		КонецЦикла;
		
		ЗаполнитьЗатратыПоОснованию(ДокументОбъект, Основание, МассивРО);
...
Процедура ЗаполнитьРемонтыОборудования(Основание, ДокументОбъект, Параметры = Неопределено)
	...
	|			ID В
	|				(ВЫБРАТЬ
	|					ВТ_ТабДокументовОснований.ID КАК ID
	|				ИЗ
	|					ВТ_ТабДокументовОснований КАК ВТ_ТабДокументовОснований)) КАК торо_ПлановыеРемонтныеРаботыСрезПоследних
	|ГДЕ
	...
	|					ВТ_ТабДокументовОснований.ID КАК ID
	|				ИЗ
	|					ВТ_ТабДокументовОснований КАК ВТ_ТабДокументовОснований)) КАК торо_ПлановыеМероприятияСрезПоследних
	|ГДЕ
	|	торо_ПлановыеМероприятияСрезПоследних.Отменен
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	...
	|					ВТ_ТабДокументовОснований.ID КАК ID
	|				ИЗ
	|					ВТ_ТабДокументовОснований КАК ВТ_ТабДокументовОснований)) КАК торо_ОтмененныеРемонтыСрезПоследних
	|ГДЕ
	...
	|			ТОГДА ВЫБОР
	|					КОГДА торо_ВнешниеОснования.ID ЕСТЬ NULL
	|							ИЛИ торо_ВнешниеОснования.Регистратор ССЫЛКА Документ.торо_ВыявленныеДефекты
	...
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаПлановыхДат.Ссылка) = ТИП(Документ.торо_НарядНаВыполнениеРемонтныхРабот)
	|				ИЛИ ТИПЗНАЧЕНИЯ(ТаблицаПлановыхДат.Ссылка) = ТИП(Документ.торо_НарядНаРегламентноеМероприятие)
	|			ТОГДА торо_Ремонты.ДатаНачалаФакт
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаПлановыхДат.Ссылка) = ТИП(Документ.торо_ВыявленныеДефекты)
	|			ТОГДА ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|		ИНАЧЕ ТаблицаПлановыхДат.ДатаНачала
	...
	|		КОГДА ТИПЗНАЧЕНИЯ(ТаблицаПлановыхДат.Ссылка) = ТИП(Документ.торо_ВыявленныеДефекты)
	|			ТОГДА ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|		ИНАЧЕ ТаблицаПлановыхДат.ДатаОкончания
	...
	|	ЕСТЬNULL(торо_ВыявленныеДефектыСписокДефектов.НаправлениеНаУстранение, ЕСТЬNULL(торо_ВнешниеОснования.Описание, """")) КАК ОписаниеРемонта,
	...
	|				И торо_ЗавершенныеМероприятия.ID ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК Завершен
	|ПОМЕСТИТЬ Ремонты
	...
	|		ПО ТаблицаПлановыхДат.ID = ВТ_ОтмененныеРемонты.ID
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.торо_Ремонты КАК торо_Ремонты
	|		ПО ТаблицаПлановыхДат.ID = торо_Ремонты.ID
	|			И ТаблицаПлановыхДат.Ссылка = торо_Ремонты.Регистратор
	...
	|		ПО ТаблицаПлановыхДат.ID = торо_ЗавершенныеРемонтныеРаботы.ID
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.торо_ЗавершенныеМероприятия КАК торо_ЗавершенныеМероприятия
	|		ПО ТаблицаПлановыхДат.ID = торо_ЗавершенныеМероприятия.ID
	|ГДЕ
	...
	|	Ремонты.ДокументИсточник КАК ДокументОснование,
	|	Ремонты.ДокументИсточник.Организация КАК ДокументИсточникОрганизация,
	|	Ремонты.ДокументИсточник.Подразделение КАК ДокументИсточникПодразделение,
	|	ВЫРАЗИТЬ(Ремонты.ОписаниеРемонта КАК СТРОКА(1000)) КАК ОписаниеРемонта,
	|	Ремонты.КрайняяДопустимаяДатаЗавершенияРемонта КАК КрайняяДопустимаяДатаЗавершенияРемонта,
	|	Ремонты.КритичностьДефекта КАК КритичностьДефекта,
	|  Ремонты.Завершен КАК Завершен
	|ИЗ
	|	Ремонты КАК Ремонты
	|ГДЕ
	|	&УсловиеОтбора
	...
	Если Параметры <> Неопределено И Параметры.Свойство("ОрганизацияДляОтбора") И Параметры.Свойство("ПодразделениеДляОтбора") Тогда
	    Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеОтбора", 
			"Ремонты.ОбъектРемонта.Организация = &Организация И Ремонты.ОбъектРемонта.Подразделение = &Подразделение");
	...
	Для каждого Элемент Из Основание Цикл
		// Убрано соединение по ТаблицаДокументовОснований.Ссылка = торо_Ремонты.Регистратор, 
		// потому что корректировка не пишет в торо_Ремонты, соответственно при вводе на основании 
		// корректировки с 1 ремонтом этот ремонт не попадал в результат запроса -- и документ не заполнялся.
		Если ТипЗнч(Элемент.Значение) = Тип("ДокументСсылка.торо_ПланГрафикРемонта") 
			И Элемент.Значение.ВидОперации = Перечисления.торо_ВидыОперацийПланаГрафикаППР.Корректировка Тогда 
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ТаблицаПлановыхДат.Ссылка = торо_Ремонты.Регистратор", 
	...
	Пока Выборка.Следующий() Цикл
		НоваяСтрока = ДокументОбъект[ИмяТЧРемонтов].Добавить();
...
Процедура ЗаполнитьЗатратыПоОснованию(ДокументОбъект, Основания, СписокИДРемонтов, Параметры = Неопределено)
	...
	СоответствиеИзОснований = Новый Соответствие;
	СоответствиеИзНормативов = Новый Соответствие;
	
	Для каждого Документ Из Основания Цикл
		Если ТипЗнч(Документ.Значение.Ссылка) = Тип("ДокументСсылка.торо_ЗаявкаНаРемонт")
			ИЛИ ТипЗнч(Документ.Значение.Ссылка) = Тип("ДокументСсылка.торо_НарядНаРегламентноеМероприятие") 
			ИЛИ ТипЗнч(Документ.Значение.Ссылка) = Тип("ДокументСсылка.торо_НарядНаВыполнениеРемонтныхРабот") Тогда
			
			СоответствиеИзОснований.Вставить(Документ.Ключ, Документ.Значение);
		Иначе
			СоответствиеИзНормативов.Вставить(Документ.Ключ, Документ.Значение);
		КонецЕсли; 
	КонецЦикла; 	
	
	Если СоответствиеИзОснований.Количество() <> 0 Тогда
		ЗаполнитьЗатратыПоДокументуОснованию(ДокументОбъект, СоответствиеИзОснований, Параметры);	
	КонецЕсли; 
	
	Если СоответствиеИзНормативов.Количество() <> 0 Тогда
		ЗаполнитьЗатратыПоНормам(ДокументОбъект, СоответствиеИзНормативов, Параметры);	
	КонецЕсли; 
	
КонецПроцедуры

Процедура ЗаполнитьЗатратыПоДокументуОснованию(ДокументОбъект, Основания, Параметры)
	...
	Если ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.торо_АктОВыполненииЭтапаРабот") Тогда
		...
		ДанныеДляЗаполнения = торо_Ремонты.ПолучитьДанныеДляЗаполненияПоОснованиюИзДокументов(МассивОснований, 
			ДокументОбъект, СписокИДРемонтов, СписокИДОпераций, СписокИДТехКарт, "Материалы");
...
// процедура для версии платформы 8.3.12
Процедура ДополнитьЗапросРемонтнымиРаботамиИзНормативов(Запрос)
...
// При замене убрать экспорт -- добавлен только для Совместимо.
Процедура ДополнитьЗапросРемонтнымиРаботамиИзНормативов_8_3_13(Запрос) Экспорт
	...
	Запрос.Текст = Запрос.Текст + "
		 ...
		 |" + "ВЫБРАТЬ
		      ...
		      |ВЫБРАТЬ
		      |	ВТ_ВерсииТК.РемонтыОборудования_ID КАК РемонтыОборудования_ID,
		      |	ВТ_ВерсииТК.ТехКарта КАК РемонтнаяРабота,
		      |	ВТ_ВерсииТК.Ссылка КАК Ссылка,
		      |	ВТ_ВерсииТК.ID + 500 КАК ID,
		      |	ВТ_ВерсииТК.Родитель_ID КАК Родитель_ID,
		      |	ВТ_ВерсииТК.Количество КАК Количество,
		      |	ВТ_ВерсииТК.Картинка КАК Картинка
		      |ПОМЕСТИТЬ ВТ_ВерсияТК
		      ...
		      |			И (ВТ_МаксимумыПериодовТК.РемонтнаяРабота = ВТ_ВерсииТК.РемонтнаяРабота)
		      |;
		      |
		      |////////////////////////////////////////////////////////////////////////////////
		      |ВЫБРАТЬ
		      ...
		      |ИЗ
		      |	ВТ_ИерархияТК КАК ВТ_ИерархияТК
		      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.торо_ВерсииТехКарт.СрезПоследних(&Период, ) КАК торо_ВерсииТехКартСрезПоследних
		      |		ПО ВТ_ИерархияТК.РемонтнаяРабота = торо_ВерсииТехКартСрезПоследних.ТехКарта
		      |;
		      |
		      |////////////////////////////////////////////////////////////////////////////////
...
Процедура ЗаполнитьЗатратыПоНормам(ДокументОбъект, Основания, Параметры)
	
	ИмяТаблицыРемонтов = "РемонтыОборудования";
	Если ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.торо_НарядНаРегламентноеМероприятие") Тогда
		ИмяТаблицыРемонтов = "РегламентныеМероприятия";
	ИначеЕсли ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.торо_АктОВыполненииРегламентногоМероприятия") Тогда
		ИмяТаблицыРемонтов = "Мероприятия";
	КонецЕсли;
	
	ДанныеДляЗаполнения = ПолучитьНормыЗатрат(ДокументОбъект.РемонтныеРаботы, ДокументОбъект[ИмяТаблицыРемонтов], Основания);
	ЗаполнитьПоНормативам(ДокументОбъект, ДанныеДляЗаполнения);
...
Процедура ЗаполнитьПоНормативам(ДокументОбъект, ДанныеДляЗаполнения)
...
Функция ПолучитьНормыЗатрат(ТаблицаОпераций, ТаблицаРемонтов, Основания, Знач СтруктураТаблиц = Неопределено)
	...
	Для каждого ТекущийРемонт Из ТаблицаРемонтов Цикл
		...
		Если ТипЗнч(ДокОснование) = Тип("ДокументСсылка.торо_ВыявленныеДефекты") Тогда
			ДатаНачала = ДокОснование.ДатаОбнаружения;
		Иначе
			ДатаНачала = ТекущийРемонт.ДатаНачала;
		КонецЕсли;
		МассивДанныхЗаполнения = ПолучитьНормыЗатратИзВетвиДереваОпераций(ДеревоОпераций, ДатаНачала, ТекущийРемонт.ДатаОкончания, СтруктураТаблиц, , ДокОснование);
...
// -СтруктураТаблиц - структура с таблицами значений МатериальныеЗатраты, ТрудовыеЗатраты, ИнструментыИТехника,
//  соответствующими одной тех. операции.
Функция ПолучитьНормыЗатратИзВетвиДереваОпераций(ВеткаДереваОпераций, ДатаНачалаРемонта, ДатаОкончанияРемонта, Знач СтруктураТаблиц = Неопределено, ТехКартаРодитель = Неопределено, ДокОснование = Неопределено) Экспорт
	...
	Если ТипЗнч(ВеткаДереваОпераций) = Тип("ДанныеФормыДерево")
		...
		Для каждого Подветка Из ВложенныеВетки Цикл 
			МассивРезультатовПодветки = ПолучитьНормыЗатратИзВетвиДереваОпераций(Подветка, ДатаНачалаРемонта, ДатаОкончанияРемонта, СтруктураТаблиц, , ДокОснование);
	...
	ИначеЕсли ТипЗнч(ВеткаДереваОпераций.РемонтнаяРабота) = Тип("СправочникСсылка.торо_ИдентификаторыТехКарт") Тогда
		...
		Для каждого Подветка Из ВложенныеВетки Цикл 
			МассивРезультатовПодветки = ПолучитьНормыЗатратИзВетвиДереваОпераций(Подветка, ДатаНачалаРемонта, ДатаОкончанияРемонта, СтруктураТаблиц, ТехКарта, ДокОснование);
			Для каждого РезультатПодветки Из МассивРезультатовПодветки Цикл
				МассивРезультатов.Добавить(РезультатПодветки);
			КонецЦикла;
		КонецЦикла;
	...
	ИначеЕсли ТипЗнч(ВеткаДереваОпераций.РемонтнаяРабота) = Тип("СправочникСсылка.торо_ТехнологическиеОперации") Тогда
		...
		СтруктураРезультата.Вставить("ДокументОснование", ДокОснование);
