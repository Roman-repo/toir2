...
Функция ПолучитьПредоставленияОРДляПечати(ОбъектРемонта, Отказ=Ложь) Экспорт 
	Попытка	
		...
		ИначеЕсли ЗначениеКонстантыОР = Перечисления.торо_ПредставлениеОРДляПечати.ПолноеНаименованиеОР Тогда
			ОтказавшийОбъектРемонта = торо_РаботаСИерархией20.ПолноеНаименованиеОР(ОбъектРемонта);
		КонецЕсли;
...
Функция СформироватьСписокВыбораТипаДокументаОснования(Владелец) Экспорт
	...
	Если СписокВыбора.Количество() = 0 Тогда
		ТекстСообщения = НСтр("es='La introducción del documento dado a la razón es prohibida por los ajustes de la introducción a la razón. "
"El ajuste y la administración-> el Ajuste de los parámetros del subsistema ТОиР-> la Interfaz y la introducción de los documentos.';ru='Ввод данного документа на основании запрещен настройками ввода на основании."
"Настройка и администрирование -> Настройка параметров подсистемы ТОиР -> Интерфейс и ввод документов.';en='Entering this document on the basis is prohibited by the input settings on the basis."
...
Процедура ЗаполнитьСправочникДаннымиИзМакета(ИмяСправочника) Экспорт
	...
	Для СтрокаНомер = 1 По ОбластьДанные.ВысотаТаблицы Цикл
		...
		Для каждого КлючИЗНачение Из СтруктураЗначенийРеквизитов Цикл
			Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(СоздаваемыйОбъект[КлючИЗНачение.Ключ])) Тогда
				...
				Иначе
					...
					ШаблонСообщения = НСтр("es='No consiguió determinar el significado para el requisito %1 por el nombre <%2>';ru='Не удалось определить значение для реквизита %1 по наименованию <%2>';en='Could not determine value for props%1 by name <%2>'");
		...
		Если ОшибкиЗаполнения Тогда
			...
			ТекстСообщения = НСтр("es='No consiguió rellenar la guía por los significados suministrados.';ru='Не удалось заполнить справочник поставляемыми значениями.';en='Failed to populate the directory with the supplied values.'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
...
//
Процедура ЗаполнитьСтандартныеРеквизитыШапкиПоУмолчанию(ДокументОбъект) Экспорт
	...
	Для каждого СтрокаТаблицы Из ТабличнаяЧасть Цикл
		...
		Для каждого КлючЗначение Из СтруктураОбязательныхПолей Цикл
			...
			Если НЕ ЗначениеЗаполнено(Значение) Тогда
				...
				СтрокаНачалаСообщенияОбОшибке = НСтр("es='En la línea el número %1 partes tabulares ""%2"": ';ru='В строке номер %1 табличной части ""%2"": ';en='In line number% 1 of table part ""% 2"": '");
...
Функция ВернутьIDНеотмененногоРемонта(СсылкаНаДок, ТабЧасть) Экспорт
	...
	Если ТипЗнч(СсылкаНаДок) = Тип("ДокументСсылка.торо_ПланГрафикРемонта") Тогда
		...
		Если ЗначениеЗаполнено(Реквизиты.ПериодичностьДетализации) Тогда		
		...
		ВыборкаДанных = ПолучитьАктуальныеПлановыеДанныеРемонтов(ТабЧасть.ВыгрузитьКолонку("ID"), "Выборка");
...
Функция ПолучитьСписокСотрудниковСКвалификацией(Квалификация, ДатаРемонта, Текст = Неопределено) Экспорт 
	...
	Запрос.Текст =  "ВЫБРАТЬ
	                ...
	                |ИЗ
	                |	РегистрСведений.торо_КвалификацияРемонтногоПерсонала.СрезПоследних(&ДатаРемонта, Квалификация = &Квалификация) КАК торо_КвалификацияРемонтногоПерсоналаСрезПоследних
	                |ГДЕ
