	...
	Исключение
		...
		Если ЗначениеЗаполнено(СообщениеЖурналаРегистрации) Тогда
			ЗаписьЖурналаРегистрации(
				НСтр("es='La firma electrónica. La falta de la adición de la firma';ru='Электронная подпись.Ошибка добавления подписи';en='Electronic signature. Error adding signature'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура УдалитьПодпись(Объект, ПорядковыйНомер, ИдентификаторФормы = Неопределено,
	...
	Исключение
		...
		Если ЗначениеЗаполнено(СообщениеЖурналаРегистрации) Тогда
			ЗаписьЖурналаРегистрации(
				НСтр("es='La firma electrónica. La falta de la desaparición de la firma';ru='Электронная подпись.Ошибка удаления подписи';en='Electronic signature. Signature removal error'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				Объект.Метаданные(),
				Объект.Ссылка,
				СообщениеЖурналаРегистрации + "
...
Функция ШтампВизуализацииЭлектроннойПодписи(Сертификат, ДатаПодписи = Неопределено, ТекстОтметки = "", ЛоготипОрганизации = Неопределено) Экспорт
	...
	ПериодДействия = НСтр("es='Con %1 por %2';ru='с %1 по %2';en='from %1 to %2'");
...
Процедура ДобавитьСтрокуПодписи(ОбъектДанных, СвойстваПодписи, СообщениеЖурналаРегистрации)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("es='La firma electrónica. La adición de la firma';ru='Электронная подпись.Добавление подписи';en='Electronic signature adding a signature'", ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
...
Процедура УдалитьСтрокуПодписи(ПодписанныйОбъект, ПорядковыйНомер, СообщениеЖурналаРегистрации)
	...
	Если ВыборкаДетальныеЗаписи.Количество() = 0 Тогда
		ВызватьИсключение НСтр("es='La línea con la firma no es encontrada.';ru='Строка с подписью не найдена.';en='No signature line found.'");
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение НСтр("es='Es insuficientemente derecho a la desaparición de la firma.';ru='Недостаточно прав на удаление подписи.';en='Insufficient rights to delete signature.'");
	КонецЕсли;
	
	ЗаписьЖурналаРегистрации(
		НСтр("es='La firma electrónica. La desaparición de la firma';ru='Электронная подпись.Удаление подписи';en='Electronic signature. Delete signature'", ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
