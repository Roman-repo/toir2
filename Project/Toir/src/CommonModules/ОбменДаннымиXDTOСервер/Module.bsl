...
Функция ПКОПоИмени(КомпонентыОбмена, Имя) Экспорт
	...
	Если ПравилоКонвертации = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1';ru='Не найдено ПКО с именем %1';en='No FFP found with name%1'"), Имя);
...
Процедура ПроизвестиВыгрузкуДанных(КомпонентыОбмена) Экспорт
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La dirección: %1. "
"El elaborador: ПередКонвертацией. "
""
"La falta de la ejecución del elaborador. "
"%2.';ru='Направление: %1."
"Обработчик: ПередКонвертацией."
""
"Ошибка выполнения обработчика."
"%2.';en='Direction: %1."
"Handler: ПередКонвертацией."
""
...
"%2.'"),
	...
	Если КомпонентыОбмена.ФлагОшибки Тогда
		ВызватьИсключение НСтр("es='A la formación del mensaje del cambio para los datos había unas faltas. Más detalladamente cm. En la revista del registro.';ru='При формировании сообщения обмена данными произошли ошибки. Подробнее см. в журнале регистрации.';en='An error occurred while generating the data exchange message. See the log for details.'");
	КонецЕсли;
	
	Попытка
		КомпонентыОбмена.МенеджерОбмена.ПослеКонвертации(КомпонентыОбмена);
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El acontecimiento: %1. "
"El elaborador: ПослеКонвертации. "
""
"La falta de la ejecución del elaborador. "
"%2.';ru='Событие: %1."
...
"Ошибка выполнения обработчика."
"%2.';en='Event:%1."
"Handler: ПослеКонвертации.                                                Handler execution error."
"%2.'"),
			КомпонентыОбмена.НаправлениеОбмена,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Для Каждого Свойство Из ТипXDTO.Свойства Цикл
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El tipo desconocido de la propiedad <%1>. El tipo del objeto: %2';ru='Неизвестный тип свойства <%1>. Тип объекта: %2';en='Unknown property type <%1>. Object type:%2'"),
				Свойство.Имя,
				Строка(ТипXDTO));
		КонецЕсли;
		...
		Исключение
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta de la formación del objeto XDTO: el Tipo de la propiedad <%1>. El nombre de la propiedad: <%2>. "
""
"%3';ru='Ошибка формирования объекта XDTO: Тип свойства <%1>. Имя свойства: <%2>."
""
"%3';en='Error creating XDTO object: Property type <%1>. Property name: <%2>."
""
"%3'"),
				ТипСвойства,
				Свойство.Имя,
...
Функция ДанныеXDTOИзДанныхИБ(КомпонентыОбмена, Источник, Знач ПравилоКонвертации, СтекВыгрузки = Неопределено) Экспорт
	...
	Если ПравилоКонвертации.ЭтоКонстанта Тогда
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta del XML-esquema. Para el receptor debe ser dada una propiedad. "
"El tipo de la fuente: %1 "
"Tipo del receptor: %2';ru='Ошибка XML-схемы. Для приемника должно быть задано одно свойство."
"Тип источника: %1"
"Тип приемника: %2';en='XML schema error. One property must be set for the receiver."
"Source type:%1"
"Receiver type:%2 '"),
...
Процедура ОткрытьФайлВыгрузки(КомпонентыОбмена, ИмяФайлаОбмена = "") Экспорт
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		...
		Если ТранзакцияАктивна() Тогда
			ВызватьИсключение НСтр("es='El bloqueo al cambio para los datos no puede ser establecido en la transacción activa.';ru='Блокировка на обмен данными не может быть установлена в активной транзакции.';en='A data exchange lock cannot be set in an active transaction.'");
		КонецЕсли;
...
Процедура ОткрытьФайлЗагрузки(КомпонентыОбмена, ИмяФайлаОбмена) Экспорт
	...
	Пока Не ПрерватьЦикл Цикл
		...
		Исключение
			СтрокаСообщенияОбОшибке = НСтр("es='La falta a la carga de los datos: %1';ru='Ошибка при загрузке данных: %1';en='Error loading data:%1'");
		...
		Если (ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента
			...
			Если СообщениеОтНеобновленнойНастройки(ЧтениеXML) Тогда
				СтрокаСообщенияОбОшибке = НСтр("es='La recepción de los datos de la fuente, en que no es cumplida la renovación del ajuste de la sincronización "
"de los datos. Es necesario:';ru='Получение данных от источника, в котором не выполнено"
"обновление настройки синхронизации данных. Необходимо:';en='Receiving data from a source in which it is not executed"
"updating data synchronization settings. Necessary: '")
					+ Символы.ПС + НСтр("es='1) Cumplir la sincronización repetida de los datos dentro de un tiempo.';ru='1) Выполнить повторную синхронизацию данных через некоторое время.';en='1) Re-synchronize the data after a while.'")
					+ Символы.ПС + НСтр("es='2) Cumplir la sincronización de los datos a la parte de la fuente, después de esto "
"repetidamente cumplir la sincronización de los datos en esta base informativa.';ru='2) Выполнить синхронизацию данных на стороне источника, после этого "
"повторно выполнить синхронизацию данных в этой информационной базе.';en='2) Perform data synchronization on the source side, then"
"re-synchronize data in this infobase.'")
					+ Символы.ПС + НСтр("es='(1 - para el transporte A través del Internet, 2 - para el transporte Otro)';ru='(1 - для вида транспорта Через интернет, 2 - для вида транспорта Другое)';en='(1 - for the type of transport Via the Internet, 2 - for the type of transport Other)'");
				ЗаписатьВПротоколВыполнения(КомпонентыОбмена, СтрокаСообщенияОбОшибке);
			Иначе
			...
			Если Не КомпонентыОбмена.ТолькоНастройкиXDTO Тогда
				...
				Если ФорматПланаОбмена <> ФорматОбмена.БазовыйФормат Тогда
					СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El formato del mensaje del cambio ""%1"" no corresponde al formato del plan del cambio ""%2"".';ru='Формат сообщения обмена ""%1"" не соответствует формату плана обмена ""%2"".';en='The format of the exchange message ""%1"" does not match the format of the exchange plan ""%2"".'"),
				...
				Если ВерсииФорматаОбменаМассив(КомпонентыОбмена.УзелКорреспондента).Найти(ФорматОбмена.Версия) = Неопределено Тогда
					СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La versión ""%1"" formatos del mensaje del cambio ""%2"" no es apoyada.';ru='Версия ""%1"" формата сообщения обмена ""%2"" не поддерживается.';en='Version ""% 1"" of the message format ""% 2"" is not supported.'"),
				...
				Если Не КомпонентыОбмена.ОбменДаннымиСВнешнейСистемой Тогда
					...
					Если ОтправительИзСообщения = Неопределено
						...
						СтрокаСообщения = НСтр("es='de los datos. El plan del cambio: %1, el Identificador: %2';ru='Не найден узел обмена для загрузки данных. План обмена: %1, Идентификатор: %2';en='An exchange node was not found for loading data. Exchange plan:%1, Identifier: %2'");
...
Процедура ЗаписатьОбъектВИБ(КомпонентыОбмена, Объект, Тип, ЗаписатьОбъект = Ложь, Знач ОтправкаНазад = Ложь, УникальныйИдентификаторСтрокой = "") Экспорт
	...
	Если Не РазрешенаЗаписьОбъекта(Объект, КомпонентыОбмена) Тогда
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La tentativa del cambio de los datos no divididos (%1: %2) en el régimen dividido.';ru='Попытка изменения неразделенных данных (%1: %2) в разделенном режиме.';en='Attempt to change unshared data (%1:%2) in split mode.'"),
...
Функция МенеджерОбменаВерсииФормата(Знач ВерсияФормата, Знач УзелИнформационнойБазы = Неопределено) Экспорт
	...
	Если Результат = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='>.';ru='Не определен Менеджер конвертации для версии формата обмена <%1>.';en='The conversion manager is not defined for the exchange format version <%1>.'"),
			ВерсияФормата);
	КонецЕсли;
...
Процедура ВыполнитьВыгрузкуЗарегистрированныхДанных(КомпонентыОбмена, НомерСообщения)
	...
	Пока ВыборкаИзменений.Следующий() Цикл
		...
		Исключение
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El acontecimiento: %1. "
"El objeto: %2. "
""
"%3';ru='Событие: %1."
"Объект: %2."
""
"%3';en='Event:%1.                                       Object: %2.                                         %3'"),
				КомпонентыОбмена.НаправлениеОбмена,
				ПредставлениеДанных,
...
Функция НайтиПравилоКонвертацииДляЗначения(ПравилаКонвертацииПредопределенныхДанных, Знач Тип, Знач ТипXDTO = Неопределено)
	...
	Если ТипXDTO = Неопределено Тогда
		...
		ИначеЕсли НайденныеПравила.Количество() > 1 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta de las reglas de la conversión de los datos predestinados. "
"Es dado más de una regla de la conversión para el tipo de la fuente <%1>.';ru='Ошибка правил конвертации предопределенных данных."
"Задано более одного правила конвертации для типа источника <%1>.';en='Error converting rules for predefined data."
"More than one conversion rule is specified for source type <%1>. '"),
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta de las reglas de la conversión de los datos predestinados. "
"La regla de la conversión no es determinada para el tipo de la fuente <%1>.';ru='Ошибка правил конвертации предопределенных данных."
"Правило конвертации не определено для типа источника <%1>.';en='Error converting rules for predefined data."
"A conversion rule is not defined for source type <%1>.'"),
			Строка(Тип));
	...
	Иначе
		...
		ИначеЕсли НайденныеПравила.Количество() > 1 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta de las reglas de la conversión de los datos predestinados. "
"Es dado más de una regla de la conversión para el tipo de la fuente <%1> y como el receptor <%2>.';ru='Ошибка правил конвертации предопределенных данных."
"Задано более одного правила конвертации для типа источника <%1> и типа приемника <%2>.';en='Error converting rules for predefined data."
"More than one conversion rule is specified for source type <%1> and receiver type <%2>. '"),
				Строка(Тип),
				Строка(ТипXDTO));
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta de las reglas de la conversión de los datos predestinados. "
"La regla de la conversión no es determinada para el tipo de la fuente <%1> y como el receptor <%2>.';ru='Ошибка правил конвертации предопределенных данных."
"Правило конвертации не определено для типа источника <%1> и типа приемника <%2>.';en='Error converting rules for predefined data."
"A conversion rule is not defined for source type <%1> and receiver type <%2>. '"),
			Строка(Тип),
			Строка(ТипXDTO));
...
Функция ЗначениеПеречисленияXDTO(Знач КонвертацииЗначений, Знач Значение)
	...
	Если ЗначениеXDTO = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No es encontrada la regla de la conversión para el significado de los datos predestinados. "
"El tipo del significado de la fuente: <%1> "
"el Significado de la fuente: <%2>';ru='Не найдено правило конвертации для значения предопределенных данных."
"Тип значения источника: <%1>"
"Значение источника: <%2>';en='No conversion rule was found for the value of the predefined data."
"Source Value Type: <%1>"
"Source value: <%2>'"),
			ТипЗнч(Значение),
			Строка(Значение));
...
Процедура ПроверитьОбъектXDTOПоСхеме(ОбъектXDTO, ТипXDTO, Контекст, Отказ, ОписаниеОшибки)
	...
	НСтр("es='No consiguió cumplir la conversión en el objeto del formato "
"""%1: %2"" "
""
...
"El objeto: %6. "
""
"Más detalladamente cm. En la revista del registro.';ru='Не удалось выполнить конвертацию в объект формата ""%1"": "
"%2"
""
...
"Объект: %6."
""
"Подробнее см. в журнале регистрации.';en='Failed to convert to format object ""%1"":"
"%2"
""
	...
	Для Каждого Свойство Из ТипОбъектаXDTO.Свойства Цикл
		Если Не ОбъектXDTO.Установлено(Свойство) Тогда
			Если Свойство.НижняяГраница = 1
				...
				СообщениеОбОшибке = СообщениеОбОшибке + "->" + Свойство.Имя + " - " + НСтр("es='el campo obligatorio.';ru='не заполнено обязательное поле.';en='required field is not filled.'");
		...
		Иначе
			...
			Если ТипЗнч(ЗначениеСвойстваXDTO) = Тип("ЗначениеXDTO") Тогда
				...
				Исключение
					СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.СформироватьСтрокуСимволов("  ", Уровень);
					СообщениеОбОшибке = СообщениеОбОшибке + "->" + Свойство.Имя + " - " + НСтр("es='El significado del campo no corresponde a las restricciones dadas.';ru='значение поля не соответствует заданным ограничениям.';en='the field value does not meet the specified restrictions.'");
					СтекОшибок.Добавить(СообщениеОбОшибке);
				КонецПопытки;
...
Процедура КонвертацияСвойстваXDTOВЭлементСтруктуры(Источник, Свойство, Приемник, ИмяДляСвойстваСоставногоТипа = "")
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La falta de la lectura del objeto XDTO, el nombre de la propiedad: <%1>.';ru='Ошибка чтения объекта XDTO, имя свойства: <%1>.';en='Error reading XDTO object, property name: <%1>.'"), ИмяСвойства)
...
Процедура КонвертацияСвойствСтруктурыОбъектаXDTO(
	...
	Попытка
		...
		КонецЦикла;
	Исключение
		ТекстОшибки = Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El acontecimiento: %1. "
"El objeto: %2. "
""
"La falta de la conversión de las propiedades. "
"%3.';ru='Событие: %1."
"Объект: %2."
""
"Ошибка конвертации свойств."
"%3.';en='Event:%1.                                       Object: %2.                                         Error converting properties.            %3.'"),
			КомпонентыОбмена.НаправлениеОбмена,
...
Процедура КонвертацияСвойстваСтруктурыОбъектаXDTO(
	...
	Исключение
		ПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La falta de la conversión de la propiedad del objeto XDTO, el nombre de la propiedad: <%1>.';ru='Ошибка конвертации свойства объекта XDTO, имя свойства: <%1>.';en='Error converting property of an XDTO object, property name: <%1>.'"), ПКС.СвойствоКонфигурации)
			+ Символы.ПС + Символы.ПС + ПредставлениеОшибки;
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Процедура ОтложенноеЗаполнениеОбъектов(КомпонентыОбмена)
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La dirección: %1. "
"El elaborador: ПередОтложеннымЗаполнением. "
""
"La falta de la ejecución del elaborador. "
"%2.';ru='Направление: %1."
...
"Ошибка выполнения обработчика."
"%2.';en='Direction: %1."
"Handler: ПередОтложеннымЗаполнением."
""
"Handler execution error."
"%2.'"),
			КомпонентыОбмена.НаправлениеОбмена,
	...
	Для Каждого СтрокаТаблицы Из ЗагруженныеОбъекты Цикл
		...
		Исключение
			ТекстОшибки = Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El acontecimiento: %1. "
"El elaborador: ОтложенноеЗаполнениеОбъектов. "
"El objeto: %2. "
""
"La falta de la ejecución del elaborador. "
"%3.';ru='Событие: %1."
...
"Ошибка выполнения обработчика."
"%3.';en='Event:%1."
"Handler: ОтложенноеЗаполнениеОбъектов.                                                    Object:%2.                                           Handler execution error."
"%3.'"),
				КомпонентыОбмена.НаправлениеОбмена,
...
Функция РазложитьФорматОбмена(Знач ФорматОбмена)
	...
	Если ЭлементыФормата.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El nombre Nekanonichesky del formato del cambio <%1>';ru='Неканоническое имя формата обмена <%1>';en='Non-canonical exchange format name <%1>'"), ФорматОбмена);
	КонецЕсли;
	...
	Если Не РазрешенаЗаписьОбъекта(Объект, КомпонентыОбмена) Тогда
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			?(УдалитьНепосредственно,
				НСтр("es='La tentativa de la desaparición de los datos no divididos (%1: %2) en el régimen dividido.';ru='Попытка удаления неразделенных данных (%1: %2) в разделенном режиме.';en='Attempt to delete unshared data (%1:%2) in split mode.'"),
				НСтр("es='La tentativa de la marca a la desaparición de los datos no divididos (%1: %2) en el régimen dividido.';ru='Попытка пометки на удаление неразделенных данных (%1: %2) в разделенном режиме.';en='Attempt tagging to delete unshared data (%1:%2) in split mode.'")),
...
Процедура ОбновитьНастройкиXDTOКорреспондента(КомпонентыОбмена) Экспорт
	...
	Если МаксимальнаяОбщаяВерсия <> НомерВерсииКорреспондента Тогда
		...
		ЗаписатьВПротоколВыполнения(КомпонентыОбмена, 
			НСтр("es='Es cambiado el número de la versión del formato del cambio.';ru='Изменен номер версии формата обмена.';en='The version number of the exchange format has been changed.'"), , Ложь, , , Истина);
	КонецЕсли;
...
Функция ПОДПоТипуСсылкиXDTO(КомпонентыОбмена, ТипСсылкиXDTO, ВозвращатьПустоеЗначение = Ложь)
	...
	Если ПравилоОбработки = Неопределено Тогда
		...
		Иначе
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='XDTO. "
"El tipo de la referencia XDTO: %1 "
"Descripción de la falta: %2';ru='Не найдено ПОД для типа ссылки XDTO."
"Тип ссылки XDTO: %1"
"Описание ошибки: %2';en='No AML found for XDTO link type."
"XDTO Link Type:%1"
"Error Description:%2 '"),
...
Функция ПОДПоТипуОбъектаXDTO(КомпонентыОбмена, ТипОбъектаXDTO, ВозвращатьПустоеЗначение = Ложь)
	...
	Если ПравилоОбработки = Неопределено Тогда
		...
		Иначе
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='XDTO. "
"El tipo del objeto XDTO: %1 "
"Descripción de la falta: %2';ru='Не найдено ПОД для типа объекта XDTO."
"Тип объекта XDTO: %1"
"Описание ошибки: %2';en='No AML found for object type XDTO."
"XDTO Object Type:%1"
"Error Description:%2 '"),
...
Функция ПОДПоОбъектуМетаданных(КомпонентыОбмена, ОбъектМетаданных)
	...
	Если ПравилоОбработки = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='de los metadatos. "
"El objeto de los metadatos: %3.';ru='Не найдено ПОД для объекта метаданных."
"Объект метаданных: %3.';en='No AML found for metadata object."
"Metadata Object:%3. '"),
...
Функция ПОДПоИмени(КомпонентыОбмена, Имя)
	...
	Если ПравилоОбработки = Неопределено Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1';ru='Не найдено ПОД с именем %1';en='No POD found with name%1'"), Имя);
...
"Ошибка выполнения обработчика."
"%3.';en='Event:%1."
"Handler: ВыборкаДанных.            AML:%2.                                           Handler execution error."
"%3.'"),
				КомпонентыОбмена.НаправлениеОбмена,
				ПравилоОбработки.Имя,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ПриОтправкеДанных(ДанныеИБ, ДанныеXDTO, Знач ИмяОбработчика, КомпонентыОбмена, СтекВыгрузки)
	...
	Исключение
		ТекстОшибки = Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El acontecimiento: %1. "
"El elaborador: ПриОтправкеДанных. "
"El objeto: %2. "
""
"La falta de la ejecución del elaborador. "
"%3.';ru='Событие: %1."
...
"Ошибка выполнения обработчика."
"%3.';en='Event:%1."
"Handler: ПриОтправкеДанных.   Object: %2.                                             Handler execution error."
"%3.'"),
			КомпонентыОбмена.НаправлениеОбмена,
...
Процедура ПриКонвертацииДанныхXDTO(ДанныеXDTO, ПолученныеДанные, КомпонентыОбмена, Знач ИмяОбработчика)
	...
	Исключение
		ТекстОшибки = Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El acontecimiento: %1. "
"El elaborador: ПриКонвертацииДанныхXDTO. "
"El objeto: %2. "
""
"La falta de la ejecución del elaborador. "
"%3.';ru='Событие: %1."
...
"Ошибка выполнения обработчика."
"%3.';en='Event:%1."
"Handler: ПриКонвертацииДанныхXDTO.   Object: %2.                                             Handler execution error."
"%3.'"),
			КомпонентыОбмена.НаправлениеОбмена,
			ПредставлениеОбъектаДляПротокола(ПолученныеДанные),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ПередЗаписьюПолученныхДанных(ПолученныеДанные, ДанныеИБ, КомпонентыОбмена, ИмяОбработчика, КонвертацияСвойств)
	...
	Исключение
		ТекстОшибки = Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El acontecimiento: %1. "
"El elaborador: ПередЗаписьюПолученныхДанных. "
"El objeto: %2. "
""
"La falta de la ejecución del elaborador. "
"%3.';ru='Событие: %1."
...
"Ошибка выполнения обработчика."
"%3.';en='Event:%1."
"Handler: ПередЗаписьюПолученныхДанных.                                                  Object:%2.                                           Handler execution error."
"%3.'"),
			КомпонентыОбмена.НаправлениеОбмена,
...
Функция ПредставлениеОбъектаДляПротокола(Объект, МетаданныеДляПредставленияСтруктуры = Неопределено)
	...
	ПредставлениеОбъекта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1, %2 (%3)';ru='%1, %2 (%3)';en='% 1,% 2 (% 3)'"),
...
Функция ПредставлениеКоллекцииСвойствДляПротокола(КоллекцияСвойств)
	...
	Если КоллекцияСвойств.Свойство("Номер", ЗначениеСвойства)
		...
		Если КоллекцияСвойств.Свойство("Дата", ЗначениеСвойстваДата)
			...
			ЭлементыПредставления.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='№ %1 de %2';ru='№%1 от %2';en='%1 from %2'"), ЗначениеСвойства, Формат(ЗначениеСвойстваДата, "ДЛФ=D")));
		Иначе
			ЭлементыПредставления.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='№ %1';ru='№%1';en='№.% 1'"), ЗначениеСвойства));
		КонецЕсли;
	...
	ИначеЕсли КоллекцияСвойств.Свойство("Дата", ЗначениеСвойства)
		...
		ЭлементыПредставления.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='De %1';ru='от %1';en='from %1'"), ЗначениеСвойства));
...
Функция ОписаниеОшибкиПОД(НаправлениеОбмена, ИмяПОД, ПредставлениеОбъекта, Информация)
	...
	ШаблонОписанияОшибки = НСтр("es='El acontecimiento: %1. "
"El elaborador: ПриОбработкеПОД. "
"BAJO: %2. "
"El objeto: %3. "
""
"La falta de la ejecución del elaborador. "
...
"Ошибка выполнения обработчика."
"%4.';en='Event:%1."
"Handler: ПриОбработкеПОД. AML:%2.                                 Object: %3.                                             Handler execution error."
"%4.'");
...
Функция ОписаниеОшибкиПКО(НаправлениеОбмена, ИмяПОД, ИмяПКО, ПредставлениеОбъекта, Информация)
	...
	ШаблонОписанияОшибки = НСтр("es='La dirección: %1. "
"BAJO: %2. "
"ПКО: %3. "
"El objeto: %4. "
""
...
Функция ВерсииФорматаОбмена(Знач УзелИнформационнойБазы)
	...
	Если ВерсииФорматаОбмена.Количество() = 0 Тогда
		Если ЗначениеЗаполнено(УзелИнформационнойБазы) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='del cambio. "
"El nombre del plan del cambio: %1 "
"Procedimiento: ПолучитьВерсииФорматаОбмена (<VersiiFormataObmena>)';ru='Не заданы версии формата обмена."
"Имя плана обмена: %1"
"Процедура: ПолучитьВерсииФорматаОбмена(<ВерсииФорматаОбмена>)';en='No exchange format version specified."
"Exchange Plan Name:%1"
"Procedure: ПолучитьВерсииФорматаОбмена(<ВерсииФорматаОбмена>)'"),
				ИмяПланаОбмена);
		Иначе
			ВызватьИсключение НСтр("es='del cambio. "
"El procedimiento: ОбменДаннымиПереопределяемый.ПриПолученииДоступныхВерсийФормата (<VersiiFormataObmena>)';ru='Не заданы версии формата обмена."
"Процедура: ОбменДаннымиПереопределяемый.ПриПолученииДоступныхВерсийФормата(<ВерсииФорматаОбмена>)';en='No exchange format version specified."
"Procedure: ОбменДаннымиПереопределяемый.ПриПолученииДоступныхВерсийФормата(<ВерсииФорматаОбмена>)'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьВерсию(Знач Версия)
	...
	Если Версии.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La representación Nekanonichesky de la versión del formato del cambio: <%1>.';ru='Неканоническое представление версии формата обмена: <%1>.';en='Non-canonical representation of the exchange format version: <%1>.'"), Версия);
	КонецЕсли;
...
Функция СравнитьВерсии(Знач СтрокаВерсии1, Знач СтрокаВерсии2)
	...
	Если Версия1.Количество() < 2 ИЛИ Версия1.Количество() > 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El formato incorrecto del parámetro StrokaVersii1: %1';ru='Неправильный формат параметра СтрокаВерсии1: %1';en='Incorrect format for СтрокаВерсии1: %1'"), СтрокаВерсии1);
	КонецЕсли;
	Версия2 = СтрРазделить(Строка2, ".");
	Если Версия2.Количество() < 2 ИЛИ Версия2.Количество() > 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El formato incorrecto del parámetro StrokaVersii2: %1';ru='Неправильный формат параметра СтрокаВерсии2: %1';en='Incorrect format for СтрокаВерсии2: %1'"), СтрокаВерсии2);
	КонецЕсли;
