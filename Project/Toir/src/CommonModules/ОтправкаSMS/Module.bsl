...
Функция ОтправитьSMS(НомераПолучателей, Знач Текст, ИмяОтправителя = Неопределено, ПеревестиВТранслит = Ложь) Экспорт
	...
	Если Не НастройкаОтправкиSMSВыполнена() Тогда
		Результат.ОписаниеОшибки = НСтр("es='Son incorrectamente dados los ajustes del proveedor para el envío SMS.';ru='Неверно заданы настройки провайдера для отправки SMS.';en='Incorrect provider settings for sending SMS.'");
		Возврат Результат;
	КонецЕсли;
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(Результат.ОписаниеОшибки) И Не ЗначениеЗаполнено(Результат.ОтправленныеСообщения) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La falta a la salida del procedimiento de OtpravkaSMSPereopredelyaemyy. ОтправитьSMS: no son rellenados los parámetros de salida "
"de OpisanieOshibki y OtpravlennyeSoobscheniya (el proveedor: %1). "
"Se Espera el relleno por lo menos un de estos parámetros.';ru='Ошибка при выходе из процедуры ОтправкаSMSПереопределяемый.ОтправитьSMS:"
"Не заполнены выходные параметры ОписаниеОшибки и ОтправленныеСообщения (провайдер: %1)."
"Ожидается заполнение по меньшей мере одного из этих параметров.';en='Error exiting procedure ОтправкаSMSПереопределяемый.ОтправитьSMS: Output parameters are not filled ОписаниеОшибки and ОтправленныеСообщения (provider:%1)."
"At least one of these parameters is expected to be completed.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		Если Результат.ОтправленныеСообщения.Количество() > 0 Тогда
			ОбщегоНазначенияКлиентСервер.Проверить(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El tipo equivocado del significado en la colección el Resultado. ОтправленныеСообщения: "
"se espera el tipo ""la Estructura"", es entregado tipo ""%1""';ru='Неверный тип значения в коллекции Результат.ОтправленныеСообщения:"
"ожидается тип ""Структура"", передан тип ""%1""';en='Invalid collection value type Результат.ОтправленныеСообщения: expected type ""Structure"", passed type ""%1""'"),
...
Функция СпособыАвторизацииПровайдераПоУмолчанию()
	...
	ПоляАвторизации.Добавить("Логин", НСтр("es='El login';ru='Логин';en='Login'"));
	ПоляАвторизации.Добавить("Пароль", НСтр("es='La consigna';ru='Пароль';en='Password'"), Истина);
