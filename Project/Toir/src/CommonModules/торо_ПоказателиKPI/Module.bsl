...
Функция ПолучитьЗначениеПоказателяДляГИС(ОбъектРемонта, ВариантАнализа, НачалоПериода, КонецПериода) Экспорт
	...
	Если ТаблицаДанныхПоказателя <> Неопределено И ТаблицаДанныхПоказателя.Количество() > 0 Тогда
		...
		ПрименитьНормализациюКТаблице(СтруктураПараметровФормы, ТаблицаДанныхПоказателя, ВариантАнализа, ИмяКолонкиРесурса, Истина);
...
Процедура ПолучитьДанныеПоказателей(СписокВариантовАнализа, ЭтаФорма) Экспорт
	...
	Пока ВыборкаПоказателей.Следующий() Цикл
		...
		Исключение
			...
			ШаблонОшибки = НСтр("es='A la recepción de los datos del índice ""%1"" había una falta: %2';ru='При получении данных показателя ""%1"" произошла ошибка: %2';en=""There was an error retrieving metric data '%1':%2""");
...
Процедура ЗаполнитьТаблицыДат(ЭтаФорма)
	...
	Если ЭтаФорма.ИспользоватьДетализациюПоПериоду И ЗначениеЗаполнено(ДетализацияПоПериоду) Тогда
		...
		Иначе // Используем запрос к производственному календарю, т.к. нет функций НачалоДекады и НачалоПолугодия
			...
			ШаблонСообщения = НСтр("es='Comprueben el relleno del calendario de producción para el período con %1 por %2.';ru='Проверьте заполнение производственного календаря на период с %1 по %2.';en='Check the completion of the production calendar for the period from%1 to%2.'");
...
Функция ЕстьОбязательныеКолонки(ВариантАнализа, ТабДанных, ИмяКолонкиРесурса, МасАналитикДляПроверки)
	...
	ШаблонСообщенияНетРесурса = НСтр("es='En la tabla de los datos para la variante del análisis ""%1"" no es encontrado el campo ""%2"".';ru='В таблице данных для варианта анализа ""%1"" не найдено поле ""%2"".';en='The field ""%2"" was not found in the data table for analysis option ""%1"".'");
...
Функция ЕстьПериодИПериодичность(ЭтаФорма, ВариантАнализа, ТабДанных, ДляИнтегральнойОценки = Ложь)
	...
	ШаблонСообщенияНетОбязательногоПоля = НСтр("es='En la tabla de los datos para la variante del análisis ""%1"" falta el campo obligatorio ""%2"" (el Tipo del análisis ""%3"")';ru='В таблице данных для варианта анализа ""%1"" отсутствует обязательное поле ""%2"" (Тип анализа ""%3"")';en='The required field ""%2"" is missing in the data table for the analysis option ""%1"" (Analysis type ""%3"")'");
	ШаблонСообщенияНеЗаданаПериодичность = НСтр("es='En los ajustes del monitor KPI no es dada la periodicidad. Los datos de la variante del análisis ""%1"" no serán sacados (el Tipo del análisis ""%2"")';ru='В  настройках монитора KPI не задана периодичность. Данные по варианту анализа ""%1"" выведены не будут (Тип анализа ""%2"")';en='In the KPI monitor settings, the frequency is not set. Data for analysis option ""%1"" will not be displayed (Analysis type ""%2"")'");
	ШаблонСообщенияДляИнтегральнойОценки = НСтр("es='En los ajustes del monitor KPI no es dada la periodicidad. Los datos en la apreciación integral no serán sacados';ru='В  настройках монитора KPI не задана периодичность. Данные в интегральную оценку выведены не будут';en='In the KPI monitor settings, the frequency is not set. No data will be output to the integrated assessment'");
...
Процедура ЗаполнитьИнтегральнуюОценку(ИнтегральнаяДиаграмма, СвернутаяТабВариантовАнализа, ЭтаФорма) Экспорт
	...
	Для каждого ЭлементСписка из СвернутаяТабВариантовАнализа Цикл
		...
		Исключение
			ОписаниеОшибки = ОписаниеОшибки();
			ШаблонОшибки = НСтр("es='Al tratamiento de los datos del índice ""%1"" para la conclusión en la apreciación integral había una falta: %2';ru='При обработке данных показателя ""%1"" для вывода в интегральную оценку произошла ошибка: %2';en='An error occurred while processing the data of the indicator ""%1"" for output to the integral assessment:%2'");
		...
		Исключение
			ОписаниеОшибки = ОписаниеОшибки();
			ШаблонОшибки = НСтр("es='A la conclusión de los datos del índice ""%1"" en la apreciación integral había una falta: %2';ru='При выводе данных показателя ""%1"" в интегральную оценку произошла ошибка: %2';en='An error occurred while displaying the data of the indicator ""%1"" in the integrated assessment:%2'");
			ТекстСообщения = СтрШаблон(ШаблонОшибки, ВариантАнализа, ОписаниеОшибки);
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
		КонецПопытки;
...
Процедура ВывестиПоказательНаПечатьТаблица(ДокументРезультат, Макет, ВариантАнализа, ЭтаФорма, ТаблицаЗначений) Экспорт
	...
	Если ЕстьПревышениеОграничения Тогда
		...
		ЗаголовокОстальные = НСтр("es='Otro';ru='Остальные';en='Others'");
...
Процедура ВывестиЗаголовок(ДокументРезультат, Макет) Экспорт
	...
	СвойстваОбласти = Новый Структура("Заголовок", НСтр("es='El monitor KPI';ru='Монитор KPI';en='KPI Monitor'"));
	ЗаполнитьЗначенияСвойств(Область.Параметры, СвойстваОбласти); 
...
Процедура ВывестиДатуВремяФормирования(ДокументРезультат, Макет)  Экспорт
	...
	СвойстваОбласти = Новый Структура("ЗаголовокСформирован, ДатаВремяФормирования", 
		НСтр("es='Es formado';ru='Сформирован';en='Formed'")+ ":",
		ТекущаяДатаСеанса());
	ЗаполнитьЗначенияСвойств(Область.Параметры, СвойстваОбласти); 
...
Функция СформироватьСтруктуруРеквизитовФормыПоПараметрамГИС(ОбъектРемонта, ВариантАнализа, НачалоПериода, КонецПериода)
	...
	ЦелевыеЗначенияПоказателей = ПолучитьЦелевыеЗначенияПоказателей(СписокВариантовАнализа, СтруктураПараметров);
