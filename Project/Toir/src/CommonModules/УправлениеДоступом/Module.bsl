		 ...
		 ИЛИ ВРег(Строка.ВидДоступа) = ВРег("ПравоИзменения") Тогда
			...
			Если ВРег(Строка.ВидДоступа) = ВРег("ПравоИзменения") Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La falta como EstRol del módulo UpravlenieDostupom. "
"En el juego de los significados del acceso es indicado el tipo del acceso de PravoIzmeneniya "
"de la tabla con el identificador ""%1"". "
"En la restricción de la comprobación del papel (como el derecho adicional) "
"puede ser la dependencia solamente del derecho de la Lectura.';ru='Ошибка в функции ЕстьРоль модуля УправлениеДоступом."
...
"может быть зависимость только от права Чтения.';en='Function error ЕстьРоль of module УправлениеДоступом. The type of access is specified in the set of access values. ПравоИзменения tables with identifier ""%1""."
"In limiting role verification (as an additional right)"
"there may be dependence only on the right of Reading.'"),
		...
		ИначеЕсли ИменаВидовДоступа.Получить(Строка.ВидДоступа) <> Неопределено
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta como EstRol del módulo UpravlenieDostupom. "
"El juego de los significados del acceso contiene el tipo conocido del acceso ""%1"", "
"que no es necesario indicar. "
""
"Es necesario indicar solamente los tipos especiales del acceso "
"de ""PravoChteniya"", ""PravoIzmeneniya"", si se usan.';ru='Ошибка в функции ЕстьРоль модуля УправлениеДоступом."
...
"""ПравоЧтения"", ""ПравоИзменения"", если они используются.';en='Function error ЕстьРоль of module УправлениеДоступом. The access value set contains the known access type ""%1"","
"which is not required to be specified."
""
"Only special access types are required. ""ПравоЧтения"", ""ПравоИзменения"", if they are used.'"),
				Строка.ВидДоступа);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta como EstRol del módulo UpravlenieDostupom. "
"El juego de los significados del acceso contiene el tipo desconocido del acceso ""%1"".';ru='Ошибка в функции ЕстьРоль модуля УправлениеДоступом."
"Набор значений доступа содержит неизвестный вид доступа ""%1"".';en='Function error Function error ЕстьРоль of module УправлениеДоступом. The access value set contains the unknown access type ""%1"".'"),
				Строка.ВидДоступа);
		КонецЕсли;
...
Функция ЕстьПраво(Право, СсылкаНаОбъект, Пользователь = Неопределено) Экспорт
	...
	Если ОписаниеПрав = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='""%1""';ru='Не найдено описание возможных прав для таблицы ""%1""';en=""No description of possible permissions found for table '%1'"""),
	...
	Если ОписаниеПрава = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No es encontrada la descripción es derecha ""%1"" para tabla ""%2""';ru='Не найдено описание права ""%1"" для таблицы ""%2""';en='The description of the right ""%1"" for table ""%2"" was not found'"),
...
Процедура ПриСозданииФормыЗначенияДоступа(Форма,
	...
	ЗаголовокОшибки =
		НСтр("es='La falta en el procedimiento de PriSozdaniiFormyZnacheniyaDostupa "
"del módulo UpravlenieDostupom general.';ru='Ошибка в процедуре ПриСозданииФормыЗначенияДоступа"
"общего модуля УправлениеДоступом.';en='Error in procedure ПриСозданииФормыЗначенияДоступа of general module УправлениеДоступом.'");
	...
	Если ТипЗнч(СозданиеНового) <> Тип("Булево") Тогда
		...
		Исключение
			...
			ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El parámetro de SozdanieNovogo no es indicado, y el relleno automático "
"del requisito de la forma ""el Objeto. La referencia"" es inaccesible por la causa: "
"%1';ru='Параметр СозданиеНового не указан, а автоматическое заполнение"
"из реквизита формы ""Объект.Ссылка"" недоступно по причине:"
"%1';en='The parameter СозданиеНового not specified, but auto-complete"
"from the requisite of the ""Объект.Ссылка"" form is not available due to:"
"%1 '"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	...
	Если ТипЗнч(ТипЗначения) <> Тип("Тип") Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El parámetro de TipZnacheniya no es indicado, y el relleno automático "
"del requisito de la forma ""el Objeto. La referencia"" es inaccesible por la causa: "
"%1';ru='Параметр ТипЗначения не указан, а автоматическое заполнение"
"из реквизита формы ""Объект.Ссылка"" недоступно по причине:"
"%1';en='The parameter ТипЗначения is not specified, but auto-complete"
"from the props of the form ""Объект.Ссылка"" is not available due to:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		КонецПопытки;
	Иначе
	...
	Если Реквизит = Неопределено Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El parámetro el Requisito no es indicado, y el relleno automático "
"del requisito de la forma ""el Objeto. ГруппаДоступа"" es inaccesible por la causa: "
"%1';ru='Параметр Реквизит не указан, а автоматическое заполнение"
"из реквизита формы ""Объект.ГруппаДоступа"" недоступно по причине:"
"%1';en='The parameter Props is not specified, but automatic completion"
"from the props of the form ""Объект.ГруппаДоступа"" is not available due to:"
"%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		КонецПопытки;
	Иначе
		...
		Если ПозицияТочки = 0 Тогда
			...
			Исключение
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No consiguió recibir el significado del requisito de la forma ""%1"", indicado el parámetro el Requisito por la causa: "
"%2';ru='Не удалось получить значение реквизита формы ""%1"", указанного параметре Реквизит по причине:"
"%2';en='Failed to get the value of the form props ""%1"" specified by the Props parameter due to:           %2'"), Реквизит, КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
			КонецПопытки;
		Иначе
			Попытка
...
"%2';en='Failed to get the value of the form props ""%1"" specified by the Props parameter due to:           %2'"), Реквизит, КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	...
	Если ТипЗнч(ГруппаЗначенийДоступа) <> СвойстваГрупп.Тип Тогда
		ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Para los significados del acceso como ""%1"" "
"se usan el tipo del acceso ""%2"" con el tipo de los significados ""%3"" "
"dados en el módulo redeterminado. "
"Pero este tipo no coincide con tipo ""%4"" en forma del significado "
"del acceso cerca del requisito de GruppaDostupa.';ru='Для значений доступа типа ""%1"""
...
"доступа у реквизита ГруппаДоступа.';en='For access values of type"" ""%1"" """
"access type """"%2 """" with value type """"%3 """" is used,"
"defined in the redefined module."
"But this type does not match the type """"%4 """" in the form of value"
"access at the ГруппаДоступа attribute. '"),
	...
	Если ГруппыЗначенийДляИзменения.Количество() = 0
	   И СозданиеНового Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Para la adición son necesarios permitido ""%1"".';ru='Для добавления требуются разрешенные ""%1"".';en='To add, allowed ""% 1"" is required.'"),
...
Функция ГруппыЗначенийДоступаРазрешающиеИзменениеЗначенийДоступа(ТипЗначенийДоступа, ВозвращатьВсе = Ложь) Экспорт
	...
	ЗаголовокОшибки =
		НСтр("es='La falta en el procedimiento ГруппыЗначенийДоступаРазрешающиеИзменениеЗначенийДоступа "
"el módulo UpravlenieDostupom general.';ru='Ошибка в процедуре ГруппыЗначенийДоступаРазрешающиеИзменениеЗначенийДоступа"
"общего модуля УправлениеДоступом.';en='Error in procedure ГруппыЗначенийДоступаРазрешающиеИзменениеЗначенийДоступа of general module '");
...
Процедура НастроитьОтборыДинамическогоСписка(Список, ОписаниеОтборов) Экспорт
	...
	Если ТипЗнч(Список) <> Тип("ДинамическийСписок") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta de la llamada del procedimiento de NastroitOtboryDinamicheskogoSpiska del módulo UpravlenieDostupom general. "
"El significado del parámetro Lista ""%1"" no es la lista dinámica.';ru='Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом."
"Значение параметра Список ""%1"" не является динамическим списком.';en=""Procedure call error НастроитьОтборыДинамического General module list УправлениеДоступом. List parameter '%1' is not a dynamic list."""), Строка(Список));
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Список.ОсновнаяТаблица) Тогда
		ВызватьИсключение
			НСтр("es='La falta de la llamada del procedimiento de NastroitOtboryDinamicheskogoSpiska del módulo UpravlenieDostupom general. "
"Cerca de la lista entregada dinámica no es indicada la tabla básica.';ru='Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом."
"У переданного динамического списка не указана основная таблица.';en='Procedure call error НастроитьОтборыДинамического General module list УправлениеДоступом. The transferred dynamic list does not have a primary table.'");
	КонецЕсли;
	
	Если Не Список.ПроизвольныйЗапрос Тогда
		ВызватьИсключение
			НСтр("es='La falta de la llamada del procedimiento de NastroitOtboryDinamicheskogoSpiska del módulo UpravlenieDostupom general. "
"Cerca de la lista entregada dinámica no es establecido el banderín ""la interpelación Cualquiera"".';ru='Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом."
"У переданного динамического списка не установлен флажок ""Произвольный запрос"".';en='Procedure call error НастроитьОтборыДинамического General module list УправлениеДоступом. The submitted dynamic list does not have the ""Custom request"" checkbox selected.'");
	КонецЕсли;
	...
	Для Каждого ОписаниеОтбора Из ОписаниеОтборов Цикл
		...
		Если Не ЗначениеЗаполнено(Псевдоним) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta de la llamada del procedimiento de NastroitOtboryDinamicheskogoSpiska del módulo UpravlenieDostupom general. "
"Cerca de la lista entregada dinámica no consiguió encontrar el seudónimo de tabla básica "
"""%1"".';ru='Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом."
"У переданного динамического списка не удалось найти псевдоним основной таблицы"
"""%1"".';en='Procedure call error НастроитьОтборыДинамического General module list УправлениеДоступом. The transferred dynamic list could not find the alias of the main table"
...
Процедура ЗаполнитьНаборыЗначенийДоступа(Знач Объект, Таблица, Знач СсылкаНаПодчиненныйОбъект = Неопределено) Экспорт
	...
	Если НЕ НаборыЗаполняются Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Los parámetros equivocados. "
"El tipo del objeto ""%1"" "
"no es encontrado ni en un de подписок a los acontecimientos "
"""Anotar los juegos de los significados del acceso"", "
"""Anotar los juegos dependientes de los significados del acceso"".';ru='Неверные параметры."
"Тип объекта ""%1"""
"не найден ни в одной из подписок на события"
"""Записать наборы значений доступа"","
"""Записать зависимые наборы значений доступа"".';en='Invalid parameters."
"Object type ""%1"""
"not found in any of the event subscriptions"
"""Record access value sets"","
"""Record dependent sets of access values"".'"),
			ТипЗначенияОбъект);
	КонецЕсли;
	...
	Исключение
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='%1 ""%2"" "
"no ha formado el juego de los significados del acceso por la causa: "
"%3';ru='%1 ""%2"""
"не сформировал набор значений доступа по причине:"
"%3';en='%1 ""%2"""
"did not form a set of access values ​​due to:"
"%3'"),
	...
	Если Таблица.Количество() = 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='%1 ""%2"" "
"ha formado el juego vacío de los significados del acceso.';ru='%1 ""%2"""
"сформировал пустой набор значений доступа.';en='%1 ""%2"""
"formed an empty set of access values.'"),
			ТипЗнч(СсылкаНаОбъект),
			Объект);
	КонецЕсли;
...
//	ОписаниеПрофиля.Наименование  = НСтр("es='El usuario';ru='Пользователь';en='User'", Метаданные.ОсновнойЯзык.КодЯзыка);
...
Процедура ЗаменитьПраваВНастройкахПравОбъектов(ТаблицаПереименований) Экспорт
	...
	Попытка
		...
		Если ПовторяющиесяНовыеИмена.Количество() > 0 Тогда
			...
			КонецЦикла;
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es=""La falta en los parámetros del procedimiento ZamenitPravaVNastroykahPravOb'ektov "
"del módulo UpravlenieDostupom general. "
""
"Después de la renovación repetirán los ajustes de los nuevos nombres siguientes de los derechos: "
"%1."";ru='Ошибка в параметрах процедуры ЗаменитьПраваВНастройкахПравОбъектов"
"общего модуля УправлениеДоступом."
""
"После обновления будут повторяться настройки следующих новых имен прав:"
"%1.';en='Error in procedure parameters ЗаменитьПраваВНастройкахПравОбъектов general module УправлениеДоступом.                After the update, the settings of the following new rights names will be repeated:"
"%1.'"),
				ПовторяющиесяНовыеИменаПрав);
		КонецЕсли;
...
Процедура ОтключитьОбновлениеКлючейДоступа(Отключить, ПланироватьОбновление = Истина) Экспорт
	...
	Если Отключить И Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		ТекстОшибки =
			НСтр("es='La llamada incorrecta del procedimiento de OtklyuchitObnovlenieKlyucheyDostupa del módulo UpravlenieDostupom general. "
"La desconexión de la renovación de las claves de acceso es posible solamente el usuario con todos los derechos o "
"en el régimen privilegiado.';ru='Некорректный вызов процедуры ОтключитьОбновлениеКлючейДоступа общего модуля УправлениеДоступом."
"Отключение обновления ключей доступа возможно только для полноправного пользователя или"
"в привилегированном режиме.';en='Invalid call to the ОтключитьОбновлениеКлючейДоступа procedure of the general УправлениеДоступом module."
"Disabling access key updates is possible only for a full user or"
"in privileged mode.'");
...
Процедура ДобавитьОбработчикОбновленияДляВключенияУниверсальногоОграничения(Версия, Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("es='La inclusión de la restricción universal del acceso a nivel de las anotaciones.';ru='Включение универсального ограничения доступа на уровне записей.';en='Enabling universal record-level access restriction.'");
...
Процедура УточнитьНаборыЗначенийДоступа(СсылкаНаОбъект, Таблица)
	...
	Для каждого Строка Из Таблица Цикл
		...
		ИначеЕсли ИменаВидовДоступа.Получить(Строка.ВидДоступа) <> Неопределено
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El objeto ""%1"" ha formado el juego de los significados del acceso, "
"el tipo conocido que contiene del acceso ""%2"", que no es necesario indicar. "
""
"Es necesario indicar solamente los tipos especiales del acceso "
"de ""PravoChteniya"", ""PravoIzmeneniya"", si se usan.';ru='Объект ""%1"" сформировал набор значений доступа,"
"содержащий известный вид доступа ""%2"", который не требуется указывать."
""
"Указывать требуется только специальные виды доступа"
"""ПравоЧтения"", ""ПравоИзменения"", если они используются.';en='Object ""%1"" formed a set of access values,"
"containing the known access type ""%2 ""that does not need to be specified."
""
"Only special access types are required."
"""ПравоЧтения"", ""ПравоИзменения"", if used. '"),
				ТипЗнч(СсылкаНаОбъект),
				Строка.ВидДоступа);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El objeto ""%1"" ha formado el juego de los significados del acceso, "
"el tipo desconocido que contiene del acceso ""%2"".';ru='Объект ""%1"" сформировал набор значений доступа,"
"содержащий неизвестный вид доступа ""%2"".';en='Object ""%1"" formed a set of access values,"
"containing unknown access type ""%2"".'"),
				ТипЗнч(СсылкаНаОбъект),
				Строка.ВидДоступа);
		КонецЕсли;
...
Функция СвойстваГруппЗначенияДоступа(ТипЗначенияДоступа, ЗаголовокОшибки)
	...
	Если СвойстваВидаДоступа = Неопределено Тогда
		ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Para los significados del acceso como ""%1"" "
"no se usan los grupos de los significados del acceso.';ru='Для значений доступа типа ""%1"""
"не используются группы значений доступа.';en='For access values of type"" ""%1"" """
"groups of access values are not used. '"),
			Строка(ТипЗначенияДоступа));
	КонецЕсли;
...
Процедура ВключитьОтключитьПрофильПользователя(Пользователь, Профиль, Включить)
	...
	Если Не УправлениеДоступомСлужебный.УпрощенныйИнтерфейсНастройкиПравДоступа() Тогда
		ВызватьИсключение
			НСтр("es='La operación dada es posible solamente para "
"la interfaz agilizada del ajuste de los derechos de admisión.';ru='Данная операция возможна только для упрощенного"
"интерфейса настройки прав доступа.';en='This operation is only possible for simplified"
...
"Expected:%3; passed value:%4 (type%5). '"),
			...
			ОжидаемыеТипы, 
			?(ЗначениеПараметра <> Неопределено, ЗначениеПараметра, НСтр("es='Неопределено';ru='Неопределено';en='Undefined'")),
			...
			Если ПерсональнаяГруппаДоступа = Справочники.ГруппыДоступа.Администраторы Тогда
				...
				Если Не ОбщегоНазначения.РазделениеВключено() Тогда
					...
					Если ЗначениеЗаполнено(ОписаниеОшибки) Тогда
						ВызватьИсключение
							НСтр("es='El perfil el Administrador debe ser por lo menos a un usuario, "
"a que es permitida la entrada en el programa.';ru='Профиль Администратор должен быть хотя бы у одного пользователя,"
"которому разрешен вход в программу.';en='At least one user must have an Administrator profile,"
"who is allowed to enter the program.'");
					КонецЕсли;
				КонецЕсли;
