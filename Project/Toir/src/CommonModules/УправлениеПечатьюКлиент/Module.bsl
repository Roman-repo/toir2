...
#КонецЕсли
	...
	Если ПечатныеФормы.Отказ Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("es='No hay derechos para la conclusión de la forma de imprenta a la impresora, se dirijan al administrador.';ru='Нет прав для вывода печатной формы на принтер, обратитесь к администратору.';en='There is no right to print a print form to a printer; contact administrator.'"));
		Возврат;
	КонецЕсли;
...
Функция ОбластьМакета(Знач СсылкаНаМакет, Знач ОписаниеОбласти) Экспорт
	...
	ИначеЕсли СсылкаНаМакет.Тип = "ODT" Тогда
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El tipo de la esfera no es indicado o indicado es incorrecto: %1.';ru='Тип области не указан или указан некорректно: %1.';en='Area type not specified or specified incorrectly:%1.'"), ОписаниеОбласти.ИмяОбласти);
		КонецЕсли;
