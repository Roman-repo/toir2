...
Функция ДобавитьИзФайловойСистемыСРасширениемСинхронно(ПараметрыВыполнения) Экспорт
	...
	ФильтрДиалога = ?(ПараметрыВыполнения.Свойство("ФильтрДиалогаВыбора"),
		ПараметрыВыполнения.ФильтрДиалогаВыбора, НСтр("es='Todos los ficheros (*. *) |*.*';ru='Все файлы (*.*)|*.*';en='All files (*. *) | *. *'"));
	...
	Если Не ПараметрыВыполнения.Свойство("ПолноеИмяФайла") Тогда
		...
		ДиалогВыбораФайла.Заголовок = НСтр("es='La elección del fichero';ru='Выбор файла';en='File selection'");
	...
	Если ПараметрыВыполнения.Свойство("МаксимальныйРазмер")
		...
		ТекстОшибки = НСтр("es='La dimensión del fichero supera %1 Mb.';ru='Размер файла превышает %1 Мб.';en='File size exceeds %1 Mb.'");
...
#КонецЕсли
	...
	ПоказатьОповещениеПользователя(
		НСтр("es='La creación:';ru='Создание:';en='Creation:'"),
...
Процедура ДобавитьФайлыРасширениеПредложено(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Если РасширениеРаботыСФайламиПодключено Тогда
		...
		Если Не ДополнительныеПараметры.Свойство("ПолноеИмяФайла") Тогда
			...
			ВыборФайла.Заголовок = НСтр("es='La elección del fichero';ru='Выбор файла';en='File selection'");
			ВыборФайла.Фильтр = ?(ЗначениеЗаполнено(Фильтр), Фильтр, НСтр("es='Todos los ficheros';ru='Все файлы';en='All files'") + " (*.*)|*.*");
...
Процедура ПоказатьПредупреждениеОНеобходимостиРасширенияРаботыСФайлами(ОбработчикРезультата, ПредставлениеКоманды = "") Экспорт
	Если Не КлиентПоддерживаетСинхронныеВызовы() Тогда
		ТекстПредупреждения = НСтр("es='La ejecución de la orden ""%1"" en los navegadores "
"Google Chrome y Mozilla Firefox no es apoyada.';ru='Выполнение команды ""%1"" в браузерах"
"Google Chrome и Mozilla Firefox не поддерживается.';en='""Executing command"" ""%1"" ""in browsers"
"Google Chrome and Mozilla Firefox are not supported.'");
	Иначе
		ТекстПредупреждения = НСтр("es='Para la ejecución de la orden ""%1"" es necesario "
"establecer la ampliación para el cliente web 1С:Предприятие.';ru='Для выполнения команды ""%1"" необходимо"
"установить расширение для веб-клиента 1С:Предприятие.';en='To execute the command"" ""%1"" ""you must nstall the extension for the 1C: Enterprise web client. '");
	КонецЕсли;
...
Функция ИмпортируемыеФайлы() Экспорт
	...
	ДиалогОткрытияФайла.Заголовок          = НСтр("es='Todos los ficheros (*. *) |*.*';ru='Все файлы(*.*)|*.*';en='All files (*. *) | *. *'");
...
Процедура КорректноеИмяФайла(ИмяФайла, УдалятьНекорректныеСимволы = Ложь) Экспорт
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='En el nombre del fichero no deben ser los símbolos siguientes: %1';ru='В имени файла не должно быть следующих символов: %1';en='The file must not contain the following characters:% 1'"), СтрИсключения);
...
Процедура ПроверитьПодписи(Форма, СсылкаНаДвоичныеДанные, ВыделенныеСтроки = Неопределено) Экспорт
	...
	ОписаниеДанных.Вставить("Операция",              НСтр("es='El desciframiento del fichero';ru='Расшифровка файла';en='File decryption'"));
	ОписаниеДанных.Вставить("ЗаголовокДанных",       НСтр("es='El fichero';ru='Файл';en='File'"));
...
Процедура ПередЗавершениемРаботыСистемы(Отказ, Предупреждения) Экспорт
	...
	ПредупреждениеПользователю.ТекстГиперссылки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Abrir la lista de los ficheros redactados (%1)';ru='Открыть список редактируемых файлов (%1)';en='Open the list of editable files (%1)'"),
		Ответ.КоличествоЗанятыхФайлов);
	ПредупреждениеПользователю.ТекстПредупреждения = НСтр("es='Hay unos ficheros, ocupados para la redacción';ru='Имеются занятые для редактирования файлы';en='There are inaccessible editing files'");
	...
	Если РаботаСФайламиСлужебныйКлиентСервер.РасширениеФайлаВСписке(
		...
		Если ВызыватьИсключение Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La carga de los ficheros con la ampliación ""%1"" es prohibida. "
"Se dirijan al administrador.';ru='Загрузка файлов с расширением ""%1"" запрещена."
"Обратитесь к администратору.';en='Downloading files with the extension ""%1"" ""is prohibited. Contact administrator.'"),
				РасширениеФайла);
		Иначе
			Возврат Ложь;
		КонецЕсли;
...
Процедура ПолучитьРабочийКаталогДанныхПользователяПослеОшибкиПолучения(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	Результат.Вставить("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='No consiguió recibir el catálogo de trabajo de los datos del usuario por la causa: "
"%1';ru='Не удалось получить рабочий каталог данных пользователя по причине:"
"%1';en='Failed to get the working directory of user data due to:"
		...
		Если Параметры.РазмерВХранилищеФайлов <> 0
		   И Параметры.РазмерВХранилищеФайлов <> Параметры.РазмерВРабочемКаталоге Тогда
			...
			Параметры.Вставить("Заголовок",
				НСтр("es='La dimensión del fichero se distingue';ru='Размер файла отличается';en='File size is different'"));
			
			Параметры.Вставить("Сообщение",
				НСтр("es='La dimensión del fichero en el catálogo de trabajo en el ordenador y su copia en el programa se distingue. "
""
"¿Tomar el fichero del programa y sustituir por ello que existe en el ordenador o "
"abrir que existe sin renovación?';ru='Размер файла в рабочем каталоге на компьютере и его копии в программе отличается."
""
"Взять файл из программы и заменить им существующий на компьютере или"
"открыть существующий без обновления?';en='The size of the file in the working directory on the computer and its copy in the program is different."
""
"Take a file from the program and replace it with an existing one on the computer or"
"open an existing one without updating?'"));
		Иначе
	...
	ИначеЕсли Параметры.ДатаИзмененияУниверсальнаяВРабочемКаталоге
		...
		Если ДанныеФайла.ВРабочемКаталогеНаЧтение = Ложь Тогда
			...
			Параметры.Вставить("Заголовок", НСтр("es='En el programa una más nueva versión del fichero';ru='В программе более новая версия файла';en='The program has a newer version of the file'"));
			
			Параметры.Вставить("Сообщение",
				НСтр("es='El fichero en el programa, notado como ocupado para la redacción, "
"tiene una fecha más avanzada del cambio (новее), que su copia en el catálogo de trabajo en el ordenador. "
""
"¿Tomar el fichero del programa y sustituir por ello que existe en el ordenador o "
"abrir que existe?';ru='Файл в программе, отмеченный как занятый для редактирования,"
"имеет более позднюю дату изменения (новее), чем его копия в рабочем каталоге на компьютере."
""
"Взять файл из программы и заменить им существующий на компьютере или"
"открыть существующий?';en='A file in the program marked as inaccessible for editing,"
"has a later change date (newer) than a copy of it in the working directory on the computer."
""
"Take a file from the program and replace it with an existing one on the computer or"
"open existing? '"));
		Иначе
	...
	ИначеЕсли Параметры.ДатаИзмененияУниверсальнаяВРабочемКаталоге
		...
		Иначе
			...
			Параметры.Вставить("Заголовок", НСтр("es='En el ordenador una más nueva copia del fichero';ru='На компьютере более новая копия файла';en='On the computer, a newer copy of the file'"));
			
			Параметры.Вставить(
				"Сообщение",
				НСтр("es='La copia del fichero en el catálogo de trabajo en el ordenador tiene una fecha más avanzada del cambio (новее), que en el programa. Probablemente, esta copia era redactada. "
""
"¿Abrir el fichero existente en el ordenador o sustituirlo por el fichero "
"del programa c por la pérdida de los cambios y abrir?';ru='Копия файла в рабочем каталоге на компьютере имеет более позднюю дату изменения (новее), чем в программе. Возможно, эта копия была отредактирована."
""
"Открыть существующий файл на компьютере или заменить его на файл"
"из программы c потерей изменений и открыть?';en='A copy of the file in the working directory on the computer has a later date of change (newer) than in the program. Perhaps this copy has been edited."
""
"Open an existing file on a computer or replace it with a file"
"from the program with the loss of changes and open?'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ВывестиНапоминаниеПриРедактировании(ОбработчикРезультата)
	...
	Если ПерсональныеНастройки.ПоказыватьПодсказкиПриРедактированииФайлов = Истина Тогда
		Если НЕ РасширениеРаботыСФайламиПодключено() Тогда
			ТекстНапоминания = 
				НСтр("es='Será invitado a abrir o conservar ahora el fichero. "
""
"1. Presionen el botón ""Conservar"" (""Save""). "
""
"2. Escojan el catálogo para la conservación del fichero en el ordenador y lo recuerden "
"(el catálogo será necesario para la redacción y el local del fichero atrás en el programa). "
""
"3. Para la redacción del fichero pasen en el catálogo escogido antes, "
"encuentren allí el fichero conservado y lo abran.';ru='Сейчас будет предложено открыть или сохранить файл."
""
...
"найдите там сохраненный файл и откройте его.';en='Now it will be offered to open or save the file."
""
"1. Click on the “Save” button."
""
"2. Select the directory to save the file to the computer and remember it"
"(the directory will be needed for editing and putting the file back into the program)."
""
"3. To edit the file, go to the previously selected directory,"
"find the saved file there and open it.'");
			...
			Если СтрНайти(СистемнаяИнформация.ИнформацияПрограммыПросмотра, "Firefox") <> 0 Тогда
				...
				|
				|"
				+ НСтр("es='(Por reticencia el navegador Mozilla Firefox conserva automáticamente los ficheros en el catálogo ""Mis documentos"")';ru='(По умолчанию браузер Mozilla Firefox автоматически сохраняет файлы в каталоге ""Мои документы"")';en='(By default, the Mozilla Firefox browser automatically saves files in the ""My documents"" directory)'");
			КонецЕсли;
			...
			ПараметрыНапоминания.Вставить("ТекстФлажка",
				НСтр("es='Más mostrar este mensaje';ru='Больше не показывать это сообщение';en='Do not show this message again'"));
			ПараметрыНапоминания.Вставить("Заголовок",
				НСтр("es='La recepción del fichero para la presentación o la redacción';ru='Получение файла для просмотра или редактирования';en='Retrieving a file for viewing or editing'"));
		...
		Если НЕ ПоместитьФайлы(ПомещаемыеФайлы, ПомещенныеФайлы, , Ложь, ИдентификаторФормы) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La falta al local del fichero "
"""%1"" "
"en el programa.';ru='Ошибка при помещении файла"
"""%1"""
"в программу.';en='Error placing file"
"""%1"""
"into the program. '"),
					Файл.ПолноеИмя) );
			Продолжить;
...
Процедура ОбновитьСостояниеОСохраненииФайлов(Знач ВыбранныеФайлы,
	...
	Если ВыбранныеФайлы.Количество() > 1 Тогда
		Если ТекущаяПозиция = Неопределено Тогда
			ПоказатьОповещениеПользователя(НСтр("es='La conservación de los ficheros';ru='Сохранение файлов';en='Saving files'"),, НСтр("es='La conservación de los ficheros es con éxito acabada';ru='Сохранение файлов успешно завершено';en='Saving files completed successfully'"));
		КонецЕсли;
	Иначе
		Если ТекущаяПозиция = Неопределено Тогда
			ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El fichero ""%1"" (%2 Mb) es conservado.';ru='Файл ""%1"" (%2 Мб) сохранен.';en='File ""%1"" (%2 MB) saved.'"),
				СохраняемоеИмяФайла,
				РазмерВМб);
			ПоказатьОповещениеПользователя(НСтр("es='La conservación del fichero';ru='Сохранение файла';en='Saving file'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
		КонецЕсли;
	КонецЕсли;
...
Функция ВозможностьОсвободитьФайл(ОбъектСсылка,
	...
	ИначеЕсли Не ЗначениеЗаполнено(Редактирует) Тогда
		СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es imposible liberar el fichero ""%1"", "
"porque él por nadie es ocupado.';ru='Невозможно освободить файл ""%1"","
"т.к. он никем не занят.';en='Unable to free file ""%1"","
"because it is not taken by anyone.'"),
			Строка(ОбъектСсылка));
		Возврат Ложь;
	Иначе
		...
		СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es imposible liberar el fichero ""%1"", "
"porque él es ocupado por el usuario ""%2"".';ru='Невозможно освободить файл ""%1"","
"т.к. он занят пользователем ""%2"".';en='Unable to free file ""%1"","
"because it is taken by user ""%2"".'"),
			Строка(ОбъектСсылка),
			Строка(Редактирует));
		Возврат Ложь;
	КонецЕсли;
...
Процедура ОсвободитьФайлБезВопроса(ДанныеФайла, УникальныйИдентификатор = Неопределено)
	...
	ПоказатьОповещениеПользователя(НСтр("es='El fichero es liberado';ru='Файл освобожден';en='File released'"),
...
Процедура ПеренестиФайлыВПапку(ОбъектыСсылка, Папка) Экспорт
	...
	Для Каждого ДанныеФайла Из ДанныеФайлов Цикл
		...
		ПоказатьОповещениеПользователя(
			НСтр("es='El traslado del fichero';ru='Перенос файла';en='File transfer'"),
			ДанныеФайла.НавигационнаяСсылка,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El fichero ""%1"" "
"es llevado en la carpeta ""%2"".';ru='Файл ""%1"""
"перенесен в папку ""%2"".';en='File ""%1"" "
"moved to folder ""%2"". '"),
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("es='Los ficheros';ru='Файлы';en='Files'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
...
Функция ОшибкаСозданияНовогоФайла(ИнформацияОбОшибке)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La falta de la creación del nuevo fichero. "
""
"%1';ru='Ошибка создания нового файла."
""
"%1';en='Error creating new file."
""
"%1'"),
...
Функция ПроверитьВозможностьЗагрузкиФайла(Файл,
	...
	Если Файл.Размер() > ОбщиеНастройки.МаксимальныйРазмерФайла Тогда
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La dimensión del fichero ""%1"" (%2 Mb) "
"supera la dimensión máximamente admisible del fichero (%3 Mb).';ru='Размер файла ""%1"" (%2 Мб)"
"превышает максимально допустимый размер файла (%3 Мб).';en='File size ""%1"" ""(%2 Mb) exceeds the maximum permissible file size (%3 Mb).'"),
			Файл.Имя,
		...
		Если ПараметрыВыполнения.ПрименитьКоВсем = Ложь Тогда
			Если Не ПустаяСтрока(ПараметрыВыполнения.ПолныйПутьКФайлу) Тогда
				СтрокаПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No consiguió colocar el fichero "
"""%1"" (%2), "
"en el programa, porque él no existe en el catálogo de trabajo en el ordenador. "
""
"¿Liberar el fichero?';ru='Не удалось поместить файл"
"""%1"" (%2),"
"в программу, т.к. он не существует в рабочем каталоге на компьютере."
""
"Освободить файл?';en='Failed to put file"
"""%1""(%2),"
"into the program, because it does not exist in the working directory on the computer."
""
"Free file?'"),
			...
			Иначе
				СтрокаПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No consiguió colocar el fichero ""%1"", "
"en el programa, porque él no existe en el catálogo de trabajo en el ordenador. "
""
"¿Liberar el fichero?';ru='Не удалось поместить файл ""%1"","
"в программу, т.к. он не существует в рабочем каталоге на компьютере."
""
"Освободить файл?';en='Failed to put file ""%1"","
"into the program, because it does not exist in the working directory on the computer."
""
"Free file?'"),
					Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка));
	...
	Исключение
		ТекстОшибки = НСтр("es='No consiguió colocar el fichero ""%1"" en el catálogo de trabajo al ordenador, "
"puesto que es posible él es bloqueado por otro programa.';ru='Не удалось поместить файл ""%1"" в рабочий каталог на компьютер,"
"так как возможно он заблокирован другой программой.';en='Failed to put file ""%1"" into the working directory on the computer,"
"as it’s possibly blocked by another program.'");
	...
	Иначе
		...
		Исключение
			...
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió colocar el fichero del ordenador en el programa por la causa: "
"""%1"". "
""
"¿Repetir la operación?';ru='Не удалось поместить файл с компьютера в программу по причине:"
"""%1""."
""
"Повторить операцию?';en='Failed to put the file from the computer into the program because:"
"""%1""."
""
"Retry the operation?'"),
				КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура ЗакончитьРедактированиеСРасширениемПослеУдаленияФайлаИзРабочегоКаталога(Результат, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ПоказыватьОповещение Тогда
		Если ПараметрыВыполнения.ВерсияОбновлена Тогда
			ШаблонПояснения = НСтр("es='El fichero ""%1"" "
"es renovado y liberado.';ru='Файл ""%1"""
"обновлен и освобожден.';en='File ""%1"" "
"updated and freed.'");
		Иначе
			ШаблонПояснения = НСтр("es='El fichero ""%1"" "
"no se ha cambiado y es liberado.';ru='Файл ""%1"""
"не изменился и освобожден.';en='File ""%1"" "
"not changed and freed. '");
		КонецЕсли;
		
		ПоказатьОповещениеПользователя(
			НСтр("es='La redacción es acabada';ru='Редактирование закончено';en='Editing is complete'"),
...
Процедура ЗакончитьРедактированиеСРасширениемОбработкаИсключения(ИнформацияОбОшибке, ПараметрыВыполнения)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='No consiguió colocar el fichero ""%1"" "
"del disco en el ordenador en el programa por razón de "
"""%2"". "
""
"¿Repetir la operación?';ru='Не удалось поместить файл ""%1"""
"с диска на компьютере в программу по причине"
"""%2""."
""
"Повторить операцию?';en='Failed to put file ""%1"" "
"from the disk on the computer to the program for a reason"
"""%2""."
""
"Retry the operation?'"),
		Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка),
...
Процедура ЗакончитьРедактированиеБезРасширенияПослеЗагрузкиФайла(Помещен, Адрес, ВыбранноеИмяФайла, ПараметрыВыполнения) Экспорт
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция",            НСтр("es='La cifración del fichero';ru='Шифрование файла';en='File encryption'"));
	ОписаниеДанных.Вставить("ЗаголовокДанных",     НСтр("es='El fichero';ru='Файл';en='File'"));
...
Процедура ОбновитьИзФайлаНаДиске(
	...
	Диалог.Заголовок                   = НСтр("es='La elección del fichero';ru='Выбор файла';en='File selection'");
	...
	Иначе
		...
		Если ЗначениеЗаполнено(РасширениеДляЗашифрованныхФайлов) Тогда
			Фильтр = НСтр("es='El fichero (*. El %1) | *. El %1|Зашифрованный el fichero (*. El %2) | *. El %2|Все los ficheros (*. *) |*.*';ru='Файл (*.%1)|*.%1|Зашифрованный файл (*.%2)|*.%2|Все файлы (*.*)|*.*';en='File (*.%1) | *.%1 | Encrypted file (*.%2) | *.%2 | All files (*. *) | *. *'");
		...
		Иначе
			Фильтр = НСтр("es='El fichero (*. El %1) | *. El %1|Все los ficheros (*. *) |*.*';ru='Файл (*.%1)|*.%1|Все файлы (*.*)|*.*';en='File (*.%1) | *.%1 | All Files (*. *) | *. *'");
	...
	Если ПараметрыДобавленияФайла <> Неопределено
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La dimensión del fichero supera %1 Mb.';ru='Размер файла превышает %1 Мб.';en='File size exceeds %1 Mb.'"), ПараметрыДобавленияФайла.МаксимальныйРазмер);
...
Процедура ОбновитьИзФайлаНаДискеПродолжение(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.ВремяИзмененияВыбранное < ПараметрыВыполнения.ДатаФайлаВБазе Тогда // В хранилище более новый.
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El fichero ""%1"" "
"sobre el disco en el ordenador tiene una fecha más avanzada de cambio (%2), "
"que el fichero escogido (%3). "
""
"La operación es interrumpida.';ru='Файл ""%1"""
"на диске на компьютере имеет более позднюю дату изменения (%2),"
"чем выбранный файл (%3)."
""
"Операция прервана.';en='File ""%1"" "
"the disk on the computer has a later modification date (%2),"
	...
	Если ПараметрыВыполнения.ДанныеФайла.ФайлРедактируетТекущийПользователь Тогда // Файл уже был занят.
		...
		Если ФайлВРабочемКаталоге = Истина Тогда
			...
			Если ПараметрыВыполнения.ВремяИзмененияВыбранное < ВремяИзмененияВКэше Тогда // В рабочем каталоге более новый.
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El fichero ""%1"" "
"en el catálogo de trabajo tiene una fecha más avanzada de cambio (%2), "
"que el fichero escogido (%3). "
""
"La operación es interrumpida.';ru='Файл ""%1"""
...
"чем выбранный файл (%3)."
""
"Операция прервана.';en='File ""%1"" in the working directory has a later modification date (%2),"
"than the selected file (%3)."
""
"Operation aborted.'"),
					Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка),
			...
			#Если Не ВебКлиент И НЕ МобильныйКлиент Тогда
				...
				Исключение
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El fichero ""%1"" "
"en el catálogo de trabajo es abierto para la redacción. "
""
"Acaben la redacción ante la ejecución "
"de la renovación del fichero sobre el disco.';ru='Файл ""%1"""
"в рабочем каталоге открыт для редактирования."
""
"Закончите редактирование перед выполнением"
"обновления из файла на диске.';en='File ""%1"""
"in the working directory is open for editing."
""
"Finish editing before doing"
"updates from a file on disk.'"),
						ПолноеИмяФайла);
...
Процедура ЗанятьФайлПоСсылкеПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("es='La redacción del fichero';ru='Редактирование файла';en='File editing'"),
		ПараметрыВыполнения.ДанныеФайла.НавигационнаяСсылка,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El fichero ""%1"" "
"es ocupado para la redacción.';ru='Файл ""%1"""
"занят для редактирования.';en='File ""%1"""
"inaccessible for editing. '"), Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка)),
		БиблиотекаКартинок.Информация32);
...
Процедура ЗанятьФайлыПоСсылкамПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("es='Ocupar los ficheros';ru='Занять файлы';en='Borrow files'"),
		,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Los ficheros (%1 de %2) son ocupados para la redacción.';ru='Файлы (%1 из %2) заняты для редактирования.';en='Files (%1 of%2) are inaccessible for editing.'"),
			КоличествоЗанятых,
...
Процедура ОсвободитьФайлыПоСсылкамПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	ПоказатьВопрос(
		Обработчик,
		НСтр("es='La anulación de la redacción de los ficheros puede "
"llevar a la pérdida de sus cambios. "
""
"¿Continuar?';ru='Отмена редактирования файлов может"
"привести к потере Ваших изменений."
""
"Продолжить?';en='Cancel file editing may"
"result in the loss of your changes."
""
"Continue?'"),
...
Процедура ОсвободитьФайлыПоСсылкамПослеОтветаНаВопросОтменитьРедактирование(Ответ, ПараметрыВыполнения) Экспорт
	...
	ПоказатьОповещениеПользователя(
		НСтр("es='Anular la redacción de los ficheros';ru='Отменить редактирование файлов';en='Cancel file editing'"),,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es anulada la redacción de los ficheros (%1 de %2).';ru='Отменено редактирование файлов (%1 из %2).';en='Editing files canceled (%1 of%2).'"),
...
Процедура ОсвободитьФайлПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.НеЗадаватьВопрос = Ложь Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La anulación de la redacción del fichero "
"""%1"" "
"puede llevar a la pérdida de sus cambios. "
""
"¿Continuar?';ru='Отмена редактирования файла"
"""%1"""
"может привести к потере ваших изменений."
""
"Продолжить?';en='Cancel file editing"
"""%1"""
"may result in the loss of your changes."
""
"Continue?'"),
			Строка(ПараметрыВыполнения.ОбъектСсылка));
...
Процедура ОсвободитьФайлПослеОтветаНаВопросОтменаРедактирования(Ответ, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ПродолжитьРаботу Тогда
		...
		Если Не ПараметрыВыполнения.НеЗадаватьВопрос Тогда
			ПоказатьОповещениеПользователя(
				НСтр("es='El fichero es liberado';ru='Файл освобожден';en='File released'"),
				ПараметрыВыполнения.ДанныеФайла.НавигационнаяСсылка,
...
Процедура СохранитьИзмененияФайлаСРасширением(ПараметрыВыполнения)
	...
	Если Не ПараметрыВыполнения.ФайлНовойВерсии.Существует() Тогда
		Если Не ПустаяСтрока(ПараметрыВыполнения.ПолныйПутьКФайлу) Тогда
			СтрокаПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió colocar el fichero ""%1"" "
"en el programa, puesto que él no existe sobre el disco en el ordenador: "
"%2. "
""
"¿Liberar el fichero?';ru='Не удалось поместить файл ""%1"" "
...
""
"Освободить файл?';en='Failed to put file ""%1"" "
"to the program, since it does not exist on the disk on the computer:"
"%2."
""
"Free file?'"),
				Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка),
...
""
"Освободить файл?';en='Failed to put file ""%1"" "
"into the program because it does not exist on the disk on the computer."
""
"Free file? '"),
				Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка));
		КонецЕсли;
...
Процедура СохранитьИзмененияФайлаСРасширениемПослеПроверкиЗашифрован(Неопределен, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ПоказыватьОповещение Тогда
		Если ВерсияОбновлена Тогда
			ПоказатьОповещениеПользователя(
				НСтр("es='Una nueva versión es conservada';ru='Новая версия сохранена';en='New version saved'"),
				ПараметрыВыполнения.ДанныеФайла.НавигационнаяСсылка,
				ПараметрыВыполнения.ДанныеФайла.ПолноеНаименованиеВерсии,
				БиблиотекаКартинок.Информация32);
		Иначе
			ПоказатьОповещениеПользователя(
				НСтр("es='Una nueva versión no es conservada';ru='Новая версия не сохранена';en='New version not saved'"),,
				НСтр("es='El fichero no se ha cambiado';ru='Файл не изменился';en='File has not changed'"),
				БиблиотекаКартинок.Информация32);
...
Функция СертификатыНеУказаны(МассивСертификатов)
	...
	Если МассивСертификатов.Количество() = 0 Тогда
		ПоказатьПредупреждение(,
			НСтр("es='Cerca del fichero cifrado no son indicados los certificados. "
"Descifren el fichero y cifren de nuevo.';ru='У зашифрованного файла не указаны сертификаты."
"Расшифруйте файл и зашифруйте заново.';en='The encrypted file does not have certificates."
"Please decrypt the file and re-encrypt.'"));
...
Процедура ДобавитьФайлыЗавершение(ПрисоединенныйФайл, ДополнительныеПараметры) Экспорт
	...
	Если ОткрыватьКарточкуПослеСозданияИзФайла Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("es='La creación';ru='Создание';en='Creature'"),
			ПолучитьНавигационнуюСсылку(ПрисоединенныйФайл),
			ПрисоединенныйФайл,
			БиблиотекаКартинок.Информация32);
...
Процедура УдалитьФайл(ОбработчикРезультата, ПолноеИмяФайла, ЗадаватьВопрос = Неопределено, ШапкаВопроса = Неопределено)
	...
	Если ЗадаватьВопрос = Истина Тогда
		ТекстВопроса =
			НСтр("es='¿Quitar el fichero ""%1"" "
"del catálogo de trabajo?';ru='Удалить файл ""%1"""
...
Процедура ПолучитьФайлВерсииВЛокальныйКэшФайловЗапуск(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.ПолноеИмяФайла = "" Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='No consiguió recibir el fichero del programa en el catálogo de trabajo en el ordenador.';ru='Не удалось получить файл из программы в рабочий каталог на компьютере.';en='Failed to get the file from the program to the working directory on the computer.'"));
...
Процедура ПолучитьПрисоединенныйФайл(Оповещение, ПрисоединенныйФайл, ИдентификаторФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Контекст.Вставить("ЗаголовокОшибки",
		НСтр("es='No consiguió recibir el fichero al ordenador del programa por la causa:';ru='Не удалось получить файл на компьютер из программы по причине:';en='Failed to get the file to the computer from the program due to:'") + Символы.ПС);
	...
	Если Контекст.ДляРедактирования
	   И Контекст.ДанныеФайла.Редактирует <> ПользователиКлиент.АвторизованныйПользователь() Тогда
		...
		Результат.Вставить("ОписаниеОшибки", Контекст.ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El fichero es redactado ya por el usuario %1.';ru='Файл уже редактирует пользователь %1.';en='The file is already being edited by user%1.'"), Строка(Контекст.ДанныеФайла.Редактирует)));
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Результат);
		Возврат;
	КонецЕсли;
...
Процедура ПолучитьПрисоединенныйФайлПослеПодключенияРасширения(РасширениеПодключено, Контекст) Экспорт
	...
	Если Не РасширениеПодключено Тогда
		...
		Результат.Вставить("ОписаниеОшибки", Контекст.ЗаголовокОшибки
			+ НСтр("es='En el navegador no es establecida la ampliación para el trabajo con los ficheros.';ru='В браузере не установлено расширение для работы с файлами.';en='The browser does not have an extension for working with files.'"));
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Результат);
		Возврат;
	КонецЕсли;
...
Процедура ПолучитьПрисоединенныйФайлПослеПолученияРабочегоКаталога(Результат, Контекст) Экспорт
	...
	Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки
		+ НСтр("es='La creación del catálogo no es cumplida por la causa:';ru='Создание каталога не выполнено по причине:';en='Creating a directory failed due to:'"));
	...
	Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки
		+ НСтр("es='El cambio de la propiedad del fichero ""Solamente la presentación"" no es cumplido por la causa:';ru='Изменение свойства файла ""Только просмотр"" не выполнено по причине:';en='Change the property of the ""View only"" file failed due to:'"));
	ДействияСФайлом.Добавить(Действие);
	...
	Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки
		+ НСтр("es='La instalación de las propiedades del fichero no es cumplido por la causa:';ru='Установка свойств файла не выполнено по причине:';en='Setting file properties failed due to:'"));
	ДействияСФайлом.Добавить(Действие);
...
Процедура ПоместитьПрисоединенныйФайл(Оповещение, ПрисоединенныйФайл, ИдентификаторФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Контекст.Вставить("ЗаголовокОшибки",
		НСтр("es='No consiguió colocar el fichero del ordenador en el programa por la causa:';ru='Не удалось поместить файл с компьютера в программу по причине:';en='Failed to put the file from the computer to the program due to:'") + Символы.ПС);
...
Процедура ПоместитьПрисоединенныйФайлПослеПолученияРабочегоКаталога(Результат, Контекст) Экспорт
	...
	Если Контекст.ПолноеИмяФайла <> Контекст.ПолноеИмяПомещаемогоФайла Тогда
		...
		Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки
			+ НСтр("es='La copia del fichero no es cumplida por la causa:';ru='Копирование файла не выполнено по причине:';en='File copy failed due to:'"));
		ДействияСФайлом.Добавить(Действие);
	...
	Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки
		+ НСтр("es='La recepción de las propiedades del fichero no es cumplida por la causa:';ru='Получение свойств файла не выполнено по причине:';en='Getting file properties failed due to:'"));
	ДействияСФайлом.Добавить(Действие);
...
Процедура КаталогФайла(ОбработчикРезультата, ДанныеФайла) Экспорт
	...
	КнопкиВопроса.Добавить(КодВозвратаДиалога.Да, НСтр("es='Conservar y abrir el catálogo';ru='Сохранить и открыть каталог';en='Save and open directory'"));
	КнопкиВопроса.Добавить(КодВозвратаДиалога.Нет, НСтр("en='cancel';es='La anulación';ru='Отмена'"));
	ПоказатьВопрос(Обработчик,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El catálogo del fichero no existe. Probablemente, en el ordenador dado el fichero ""%1"" no se abría. "
"¿Conservar el fichero al ordenador y abrir su catálogo?';ru='Каталог файла не существует. Возможно, на данном компьютере файл ""%1"" еще не открывался."
"Сохранить файл на компьютер и открыть его каталог?';en='The file directory does not exist. The file ""%1"" may not yet open on this computer.'"),
			ИмяФайла),
		КнопкиВопроса);
...
Процедура ОсвободитьМестоВРабочемКаталоге(ОбработчикРезультата, РеквизитыВерсии)
	...
	#Если ВебКлиент Тогда
		...
		ВернутьРезультатПослеПоказаПредупреждения(
			ОбработчикРезультата,
			НСтр("es='La limpieza del catálogo de trabajo no es apoyada en el cliente web.';ru='Очистка рабочего каталога не поддерживается в веб-клиенте.';en='Clearing the working directory is not supported in the web client.'"),
			Неопределено);
		Возврат;
	#КонецЕсли
...
Процедура ОчиститьРабочийКаталог(ОбработчикРезультата, РазмерФайловВРабочемКаталоге, РазмерДобавляемогоФайла, ОчищатьВсе) Экспорт
	...
	#Если ВебКлиент Тогда
		ВернутьРезультатПослеПоказаПредупреждения(ОбработчикРезультата, НСтр("es='La limpieza del catálogo de trabajo no es apoyada en el cliente web.';ru='Очистка рабочего каталога не поддерживается в веб-клиенте.';en='Clearing the working directory is not supported in the web client.'"), Неопределено);
		Возврат;
	#КонецЕсли
	...
	Если ПараметрыВыполнения.НомерШага = 1 Тогда
		Если Не ПараметрыВыполнения.ДаДляВсех
			...
			Если ПараметрыВыполнения.ОчищатьВсе = Ложь Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Se cumple la limpieza del catálogo de trabajo en el ordenador. "
""
"En el programa falta el fichero "
"""%1"". "
""
"¿Quitar su copia del catálogo de trabajo en el ordenador?';ru='Выполняется очистка рабочего каталога на компьютере."
""
"В программе отсутствует файл"
"""%1""."
""
"Удалить его копию из рабочего каталога на компьютере?';en='The working directory on the computer is being cleaned."
""
"The program is missing a file"
"""%1""."
""
"Delete a copy of it from the working directory on the computer?'"),
					ПараметрыВыполнения.ИмяКаталога + ПараметрыВыполнения.Элемент.Путь);
			Иначе
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='En el programa falta el fichero "
"""%1"". "
""
"¿Quitar su copia del catálogo de trabajo en el ordenador?';ru='В программе отсутствует файл"
"""%1""."
""
"Удалить его копию из рабочего каталога на компьютере?';en='The program is missing a file"
"""%1""."
""
"Delete a copy of it from the working directory on the computer?'"),
					ПараметрыВыполнения.ИмяКаталога + ПараметрыВыполнения.Элемент.Путь);
			...
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить("Да",         НСтр("es='Sí';ru='Да';en='Yes'"));
			Кнопки.Добавить("ДаДляВсех",  НСтр("es='Sí para todos';ru='Да для всех';en='Yes for everyone'"));
			Кнопки.Добавить("Нет",        НСтр("es='No';ru='Нет';en='No'"));
			Кнопки.Добавить("НетДляВсех", НСтр("es='No existe para todos';ru='Нет для всех';en='No for everyone'"));
	...
	Если ПараметрыВыполнения.НомерШага = 2 Тогда
		...
		ФайлНаДиске.УстановитьТолькоЧтение(Ложь);
		Если ПараметрыВыполнения.ОчищатьВсе = Ложь Тогда
			ШапкаВопроса = НСтр("es='Se cumple la limpieza del catálogo de trabajo en el ordenador a la adición del fichero.';ru='Выполняется очистка рабочего каталога на компьютере при добавлении файла.';en='The working directory on the computer is cleaned when a file is added.'");
		Иначе
			ШапкаВопроса = НСтр("es='Se cumple la limpieza del catálogo de trabajo en el ordenador.';ru='Выполняется очистка рабочего каталога на компьютере.';en='The working directory on the computer is being cleaned.'");
		КонецЕсли;
...
#Если Не ВебКлиент Тогда
			Если СтрДлина(ПараметрыВыполнения.ПолноеИмяФайла) > ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути Тогда
				...
				Если СтрДлина(ПараметрыВыполнения.ИмяФайла) > МаксимальнаяДлинаИмениФайла Тогда
					ТекстСообщения =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La longitud de la vía al fichero (el catálogo de trabajo el más el nombre del fichero) supera %1 símbolos "
"%2';ru='Длина пути к файлу (рабочий каталог плюс имя файла) превышает %1 символов"
"%2';en='File path (working directory plus file name) exceeds%1 characters"
"%2 '"),
						ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути,
						ПараметрыВыполнения.ПолноеИмяФайла);
					
					ТекстСообщения = ТекстСообщения + Символы.ВК + Символы.ВК
						+ НСтр("es='Cambien el nombre del fichero en más corto.';ru='Измените имя файла на более короткое.';en='Change the file name to a shorter one.'");
...
Процедура ПолучитьССервераИЗарегистрироватьВЛокальномКэшеФайловПредложитьВыборКаталога(Ответ, ПараметрыВыполнения)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La longitud de la vía al fichero supera %1 símbolos: "
"¿%2 "
""
"Escoger otro catálogo básico de trabajo?';ru='Длина пути к файлу превышает %1 символов:"
"%2"
""
"Выбрать другой основной рабочий каталог?';en='File path exceeds%1 characters:"
"%2"
""
"Select another primary working directory?'"),
		ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути,
		ПараметрыВыполнения.ПолноеИмяФайла);
...
Процедура ПолучитьССервераИЗарегистрироватьВЛокальномКэшеФайловНачатьВыборКаталога(Ответ, ПараметрыВыполнения) Экспорт
	...
	Заголовок = НСтр("es='Escojan otro catálogo básico de trabajo';ru='Выберите другой основной рабочий каталог';en='Choose a different main working directory'");
	...
	Если ПараметрыВыполнения.ФайлЕстьВРегистре И Версия <> ПараметрыВыполнения.ДанныеФайла.ТекущаяВерсия Тогда
		...
		Если ПараметрыВыполнения.ДанныеФайла.Владелец = ВРегистреПапка Тогда // Одна и та же папка.
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='En el catálogo de trabajo en el ordenador hay ya un fichero "
"""%1"" "
"correspondiente a otro fichero en el programa. "
""
"El código del fichero en el programa: %2. "
"El código del fichero en el catálogo de trabajo: %3. "
""
"Se Recomienda cambiar de nombre a un de los ficheros en el programa.';ru='В рабочем каталоге на компьютере уже есть файл"
"""%1"","
...
""
"Рекомендуется переименовать один из файлов в программе.';en='There is already a file in the working directory on the computer"
"""""%1"""","
"corresponding to another file in the program."
""
"The file code in the program is% 2."
"The code for the file in the working directory is% 3."
""
"It is recommended that you rename one of the files in the program.'"),
				ПараметрыВыполнения.ПолноеИмяФайла,
...
"El código del fichero en el catálogo de trabajo: %3. "
""
"Se Recomienda indicar otro catálogo de trabajo para una de las carpetas en el programa. "
"(Dos carpetas no tener ser catálogo igual de trabajo).';ru='В рабочем каталоге на компьютере уже есть файл"
"""%1"","
"соответствующий другому файлу в программе."
""
"Код файла в программе: %2."
...
"(У двух папок не должно быть одинакового рабочего каталога).';en='There is already a file in the working directory on the computer"
"""""%1"""","
"corresponding to another file in the program."
""
"The file code in the program is% 2."
"The code for the file in the working directory is% 3."
""
"It is recommended to specify a different working directory for one of the folders in the program."
...
Процедура ПроверитьМаксимальнуюДлинуПолногоПутиВРабочемКаталогеЗапуск(ПараметрыВыполнения)
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La longitud de la vía completa al fichero (el catálogo de trabajo el más el nombre del fichero) supera %1 símbolos "
"""%2"".';ru='Длина полного пути к файлу (рабочий каталог плюс имя файла) превышает %1 символов"
"""%2"".';en='The full path to the file (working directory plus file name) exceeds%1 characters"
"""%2 "".'"),
		ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути,
		ПараметрыВыполнения.ПолноеИмяФайла);
	...
	Если СтрДлина(ПараметрыВыполнения.ДанныеФайла.РабочийКаталогВладельца) > ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути - 5 Тогда
		ТекстСообщения = ТекстСообщения + Символы.ВК + Символы.ВК
			+ НСтр("es='Cambien los nombres de las carpetas o lleven la carpeta corriente en otra carpeta.';ru='Измените имена папок или перенесите текущую папку в другую папку.';en='Change the folder names or move the current folder to another folder.'");
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, ТекстСообщения, Ложь);
		Возврат;
	КонецЕсли;
...
"""%2"". "
""
"¿Escoger otro catálogo básico de trabajo? "
"(El contenido del catálogo de trabajo será llevado en el catálogo escogido).';ru='Длина полного пути к файлу (рабочий каталог плюс имя файла) превышает %1 символов"
"""%2""."
""
"Выбрать другой основной рабочий каталог?"
"(Содержимое рабочего каталога будет перенесено в выбранный каталог).';en='The full path to the file (working directory plus file name) exceeds%1 characters"
"""""%2 """"."
""
"Choose a different main working directory?"
"(The contents of the working directory will be transferred to the selected directory).'"),
...
Процедура ПроверитьМаксимальнуюДлинуПолногоПутиВРабочемКаталогеНачатьВыборКаталога(Ответ, ПараметрыВыполнения) Экспорт
	...
	// Выбираем другой путь к рабочему каталогу.
	Заголовок = НСтр("es='Escojan otro catálogo de trabajo';ru='Выберите другой рабочий каталог';en='Choose a different working directory'");
...
Процедура СкопироватьСодержимоеКаталога(ОбработчикРезультата, Знач КаталогИсточника, Знач КаталогПриемника)
	...
	Если Результат.ВозниклаОшибка Тогда
		...
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió copiar el fichero "
"""%1"". "
"Probablemente él es ocupado por otra aplicación. "
""
"¿Repetir la operación?';ru='Не удалось скопировать файл"
"""%1""."
"Возможно он занят другим приложением."
""
"Повторить операцию?';en='Failed to copy file"
"""%1""."
"Perhaps it is taken by another application."
""
"Retry the operation?'"),
...
Процедура ПеренестиСодержимоеРабочегоКаталога(ОбработчикРезультата, КаталогИсточника, КаталогПриемника) Экспорт
	...
	Если СтрНайти(НРег(КаталогПриемника), НРег(КаталогИсточника)) <> 0 Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Catálogo escogido de trabajo "
"""%1"" "
"entra en catálogo viejo de trabajo "
"""%2"".';ru='Выбранный рабочий каталог"
"""%1"""
"входит в старый рабочий каталог"
"""%2"".';en='Selected working directory"
"""%1"""
"included in the old working directory"
"""% 2 "".'"),
			КаталогПриемника,
			КаталогИсточника);
...
Процедура ПеренестиСодержимоеРабочегоКаталогаПослеУспехаИОтменыОчистки(Результат, ПараметрыВыполнения) Экспорт
	...
	Если Результат.ВозниклаОшибка Тогда
		...
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió copiar atrás el contenido de catálogo "
"""%1"" "
"en catálogo "
"""%2"".';ru='Не удалось скопировать обратно содержимое каталога"
"""%1"""
"в каталог"
"""%2"".';en='Failed to copy directory contents back"
"""%1"""
"to the catalog"
"""%2"".'"),
			ПараметрыВыполнения.КаталогПриемника,
...
Процедура УдалитьСодержимоеКаталогаЗапуск(ПараметрыВыполнения)
	...
	Для Индекс = ПараметрыВыполнения.Индекс По ПараметрыВыполнения.ВГраница Цикл
		...
		Если Не ФайлУдален Тогда
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió quitar el fichero "
"""%1"". "
"Probablemente él es ocupado por otra aplicación. "
""
"¿Repetir la operación?';ru='Не удалось удалить файл"
"""%1""."
"Возможно он занят другим приложением."
""
"Повторить операцию?';en='Failed to delete file"
"""%1""."
"Perhaps it is taken by another application."
""
"Retry the operation?'"),
...
Процедура ИмпортФайловПослеПроверкиРазмеров(Результат, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.КоличествоСуммарное = 0 Тогда
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, НСтр("es='No hay ficheros para la adición';ru='Нет файлов для добавления';en='No files to add'"), Неопределено);
		Возврат;
	КонецЕсли;
	...
	Для Индекс = ПараметрыВыполнения.ВыбранныеФайлыИндекс По ПараметрыВыполнения.ВыбранныеФайлыВГраница Цикл
		...
		Если ВыбранКаталог Тогда
			...
			Если РаботаСФайламиСлужебныйВызовСервера.ЕстьПапкаСТакимИменем(
				...
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La carpeta ""%1"" existe ya. "
""
"¿Continuar la importación de la carpeta?';ru='Папка ""%1"" уже существует."
""
"Продолжить импорт папки?';en='The folder ""%1"" already exists."
""
"Continue importing the folder?'"),
					ПараметрыВыполнения.ИмяПапки);
...
Процедура ИмпортФайловПослеЦиклаПродолжение(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.МассивСтруктурВсехФайлов.Количество() > 1 Тогда
		ТекстСостояния = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La importación de los ficheros es acabada. Es importado los ficheros: %1';ru='Импорт файлов завершен. Импортировано файлов: %1';en='File import completed. Imported files:%1'"), Строка(ПараметрыВыполнения.МассивСтруктурВсехФайлов.Количество()) );
	...
	Если ПараметрыВыполнения.МассивСтруктурВсехФайлов.Количество() = 1 Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("es='El cambio:';ru='Изменение:';en='Change:'"),
			Ссылка,
			Элемент0.Файл,
			БиблиотекаКартинок.Информация32);
	КонецЕсли;
		...
		Если ФайлВРабочемКаталоге = Истина Тогда
			...
			Если ДатаФайлаНаДиске > ДатаФайлаВБазе Тогда // В рабочем каталоге более новый (изменен пользователем со стороны).
				...
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La fecha del cambio del fichero ""%1"" "
"en el catálogo de trabajo en el ordenador más avanzado (новее), que en el programa. "
"Probablemente, el fichero en el ordenador era redactado.';ru='Дата изменения файла ""%1"""
"в рабочем каталоге на компьютере более поздняя (новее), чем в программе."
"Возможно, файл на компьютере был отредактирован.';en='Date modified file"" ""%1"" """
"in the working directory on the computer is later (newer) than in the program."
"The file on your computer may have been edited. '"),
					Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка));
...
Процедура СохранитьКакСРасширениемПослеРасшифровки(ОписаниеДанных, ПараметрыВыполнения) Экспорт
	...
	Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Todos los ficheros (*. El %1) | *. El %1';ru='Все файлы (*.%1)|*.%1';en='All files (*.% 1) | *.% 1'"), Расширение);
	...
	ПоказатьОповещениеПользователя(НСтр("es='El fichero es con éxito conservado';ru='Файл успешно сохранен';en='File saved successfully'"), , ПолноеИмяФайла);
...
Процедура ПоказатьНапоминаниеПередПоместитьФайл(ОбработчикРезультата)
	...
	Если ПерсональныеНастройки.ПоказыватьПодсказкиПриРедактированииФайлов = Истина Тогда
		Если Не РасширениеРаботыСФайламиПодключено() Тогда
			ТекстНапоминания = 
				НСтр("es='Será invitado a escoger ahora el fichero "
"para colocarlo en el programa y acabar la redacción. "
""
"Encuentren el fichero necesario en aquel catálogo, que "
"era indicado antes al comienzo de la redacción.';ru='Сейчас будет предложено выбрать файл для того,"
"чтобы поместить его в программу и закончить редактирование."
""
"Найдите нужный файл в том каталоге, который был"
"указан ранее при начале редактирования.';en='Now it will be offered to select a file"
"to put it in the program and finish editing."
""
"Find the desired file in the directory that was"
"specified earlier when editing started.'");
			...
			ПараметрыНапоминания.Вставить("ТекстФлажка",
				НСтр("es='Más mostrar este mensaje';ru='Больше не показывать это сообщение';en='Do not show this message again'"));
			ПараметрыНапоминания.Вставить("Заголовок",
				НСтр("es='El local del fichero';ru='Помещение файла';en='File location'"));
			СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(
				ОбработчикРезультата, ТекстНапоминания, Кнопки, ПараметрыНапоминания);
			Возврат;
		КонецЕсли;
...
Процедура ПроверитьПредельныйРазмерФайлов(ОбработчикРезультата, ПараметрыПроверки)
	...
	Если МассивСлишкомБольшихФайлов.Количество() <> 0 Тогда 
		...
		Для Каждого Файл Из МассивСлишкомБольшихФайлов Цикл
			...
			ТекстСтроки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 (%2 MB)';ru='%1 (%2 МБ)';en='% 1 (% 2 MB)'"), Строка(Файл), Строка(РазмерФайлаВМб));
			ФайлыБольшие.Добавить(ТекстСтроки);
		КонецЦикла;
		
		Параметры.Вставить("ФайлыБольшие", ФайлыБольшие);
		Параметры.Вставить("Заголовок", НСтр("es='La prevención a la carga de los ficheros';ru='Предупреждение при загрузке файлов';en='File upload warning'"));
...
Процедура ПоказатьИнформациюФайлНеБылИзменен(ОбработчикРезультата)
	...
	Если ПерсональныеНастройки.ПоказыватьИнформациюЧтоФайлНеБылИзменен Тогда
		ТекстНапоминания = НСтр("es='La versión no era creada, porque el fichero no es cambiado. El comentario no es conservado.';ru='Версия не была создана, т.к. файл не изменен. Комментарий не сохранен.';en='The version was not created, because file not changed. Comment not saved.'");
		...
		ПараметрыНапоминания.Вставить("ТекстФлажка",
			НСтр("es='Más mostrar este mensaje';ru='Больше не показывать это сообщение';en='Do not show this message again'"));
		ПараметрыНапоминания.Вставить("Заголовок",
			НСтр("es='La información';ru='Информация';en='Information'"));
		СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(
			ОбработчикРезультата, ТекстНапоминания, Кнопки, ПараметрыНапоминания);
	Иначе
...
Процедура ЗакончитьРедактированиеПоСсылкамПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	ПоказатьОповещениеПользователя(
	НСтр("es='Acabar la redacción de los ficheros';ru='Завершить редактирование файлов';en='Complete file editing'"),,
	СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	НСтр("es='Es acabada la redacción de los ficheros (%1 de %2).';ru='Завершено редактирование файлов (%1 из %2).';en='Editing files completed (%1 of%2).'"),
	ПараметрыВыполнения.ДанныеФайлов.Количество(),
	ПараметрыВыполнения.МассивФайлов.Количество()),
	БиблиотекаКартинок.Информация32);
...
Процедура Зашифровать(ОбработчикРезультата, ДанныеФайла, УникальныйИдентификатор) Экспорт
	...
	Если ПараметрыВыполнения.ДанныеФайла.Зашифрован Тогда
		ТекстПредупреждения = НСтр("es='El fichero ""%1"" es cifrado ya.';ru='Файл ""%1"" уже зашифрован.';en='File ""%1"" is already encrypted.'");
	...
	Если ЗначениеЗаполнено(ПараметрыВыполнения.ДанныеФайла.Редактирует) Тогда
		ТекстПредупреждения = НСтр("es='No es posible cifrar el fichero ocupado.';ru='Нельзя зашифровать занятый файл.';en='Can not encrypt an inaccessible file.'");
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, ТекстПредупреждения, ПараметрыВыполнения);
		Возврат;
	КонецЕсли;
	...
	Если ПараметрыВыполнения.ДанныеФайла.КоличествоВерсий > 1 Тогда
		ПредставлениеФайла = ПредставлениеФайла + " (" + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='De las versiones: %1';ru='Версий: %1';en='Version:% 1'"), ПараметрыВыполнения.ДанныеФайла.КоличествоВерсий) + ")";
	КонецЕсли;
	...
	ОписаниеДанных.Вставить("ПредставлениеНабора", НСтр("es='Los ficheros (%1)';ru='Файлы (%1)';en='Files (%1)'"));
...
Процедура ДобавитьФайл(
	...
	Если ПараметрыДобавления = Неопределено
		...
		ПараметрыВыполнения.Вставить("ФильтрДиалогаВыбора",  НСтр("es='Todos los ficheros (*. *) |*.*';ru='Все файлы(*.*)|*.*';en='All files (*. *) | *. *'"));
...
Процедура ДобавитьИзФайловойСистемыБезРасширенияПослеЗагрузкиФайла(Помещен, Адрес, ВыбранноеИмяФайла, ПараметрыВыполнения) Экспорт
	...
	Если ПустаяСтрока(СтруктураПути.Расширение) Тогда
		ТекстВопроса = НСтр("es='Es necesario indicar el fichero con la ampliación.';ru='Необходимо указать файл с расширением.';en='Required to specify a file with the extension.'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Повторить, НСтр("es='Escoger otro fichero';ru='Выбрать другой файл';en='Choose another file'"));
...
Процедура ИнформироватьОШифровании(МассивФайловВРабочемКаталогеДляУдаления,
	...
	МодульЭлектроннаяПодписьКлиент.ИнформироватьОШифрованииОбъекта(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El fichero: %1';ru='Файл: %1';en='File:%1'"), ФайлСсылка));
...
Процедура ПодписатьФайл(ПрисоединенныйФайл, ДанныеФайла, ИдентификаторФормы,
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция",            НСтр("es='La firma del fichero';ru='Подписание файла';en='File signing'"));
	ОписаниеДанных.Вставить("ЗаголовокДанных",     НСтр("es='El fichero';ru='Файл';en='File'"));
...
Процедура ИмпортФайловРекурсивноБезДиалогов(Знач Владелец, Знач ВыбранныеФайлы, Знач ПараметрыВыполнения, Знач ЗадаватьВопросПапкаУжеСуществует)
	...
	Для Каждого ВыбранныйФайл Из ВыбранныеФайлы Цикл
		...
		Если Не ВыбранныйФайл.Существует() Тогда
			...
			Запись.Вставить("Ошибка", НСтр("es='El fichero falta sobre el disco.';ru='Файл отсутствует на диске.';en='File is missing from the disk.'"));
		...
		Попытка
			...
			НадписьПодробнее = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es tratado el fichero ""%1"" (%2 Mb)...';ru='Обрабатывается файл ""%1"" (%2 Мб)...';en='Processing file ""%1"" (%2 Mb) ...'"),
				ВыбранныйФайл.Имя, 
				РаботаСФайламиСлужебныйКлиентСервер.ПолучитьСтрокуСРазмеромФайла(РазмерВМб));
				
			ТекстСостояния = НСтр("es='La importación de los ficheros del disco...';ru='Импорт файлов с диска...';en='Import files from disk ...'");
			...
			Если Не ПоместитьФайлы(ПомещаемыеФайлы, ПомещенныеФайлы, , Ложь, ПараметрыВыполнения.ИдентификаторФормы) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La falta del local del fichero "
"""%1"" "
"en el depósito temporal.';ru='Ошибка помещения файла"
"""%1"""
"во временное хранилище.';en='Error placing file"
"""%1"""
...
Функция ПроверитьЗанятыеФайлыПриЗавершенииРаботы()
	...
	// ТОиР++
	Если ПерсональныеНастройкиРаботыСФайлами = Неопределено 
		ИЛИ НЕ ПерсональныеНастройкиРаботыСФайлами.Свойство("ПоказыватьЗанятыеФайлыПриЗавершенииРаботы") Тогда
		Возврат Неопределено;
	КонецЕсли;
	// ТОиР--
	...
	ПараметрыПрикладнойФормыПредупреждения.Вставить("СообщениеВопрос",      НСтр("es='¿Acabar el trabajo con el programa?';ru='Завершить работу с программой?';en='Complete the work with the program?'"));
	ПараметрыПрикладнойФормыПредупреждения.Вставить("СообщениеЗаголовок",   НСтр("es='Los ficheros siguientes son ocupados para la redacción:';ru='Следующие файлы заняты для редактирования:';en='The following files are inaccessible for editing:'"));
	ПараметрыПрикладнойФормыПредупреждения.Вставить("Заголовок",            НСтр("es='La terminación del trabajo';ru='Завершение работы';en='Completing of the work'"));
...
Функция ВыбратьПутьКРабочемуКаталогу(ИмяКаталога, Заголовок, РабочийКаталогВладельца) Экспорт
	...
	Если ДиалогОткрытияФайла.Выбрать() Тогда
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La vía equivocada o faltan los derechos a la anotación en catálogo "
"""%1"".';ru='Неверный путь или отсутствуют права на запись в каталог"
"""%1"".';en='Incorrect path or directory write permissions missing"
"""%1"".'"),
				ИмяКаталога);
...
#Если Не ВебКлиент Тогда
			...
			Если МассивФайловВКаталоге.Количество() <> 0 Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='En catálogo escogido de trabajo "
"""%1"" "
"hay ya unos ficheros. "
""
"Escojan otro catálogo.';ru='В выбранном рабочем каталоге"
"""%1"""
"уже есть файлы."
""
"Выберите другой каталог.';en='In the selected working directory"
"""%1"""
...
Процедура СравнитьФайлы(ИдентификаторФормы, ПервыйФайл, ВторойФайл, Расширение, ВладелецВерсии = Неопределено) Экспорт
	...
	Если Не РасширениеПоддерживается Тогда
		ТекстПредупреждения =
		НСтр("es='La comparación de los ficheros es apoyada solamente para los ficheros de los tipos siguientes:"
" el documento Textual (.txt)"
" el Documento del formato RTF (.rtf)"
" el Documento Microsoft Word (.doc.docx)"
" el Documento HTML (.html.htm)"
" el documento Tabular (.mxl)"
" el documento OpenDocument Textual (.odt)';ru='Сравнение файлов поддерживается только для файлов следующих типов: "
"   Текстовый документ (.txt)"
"   Документ формата RTF (.rtf) "
"   Документ Microsoft Word (.doc, .docx) "
"   Документ HTML (.html .htm) "
"   Табличный документ (.mxl) "
"   Текстовый документ OpenDocument (.odt)';en='File comparison is only supported for the following file types: "
" Text document.( txt)"
" RTF format document (.rtf) "
...
Процедура ЗаполнитьПараметрыСравнения(Результат, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ТекущийШаг = 5 Тогда
		Если ПараметрыВыполнения.Результат1 И ПараметрыВыполнения.Результат2 Тогда
			ШаблонЗаголовкаФайла = НСтр("es='El fichero № %1 (%2)';ru='Файл №%1 (%2)';en='File №%1 (%2)'");
...
#Если Не ВебКлиент И НЕ МобильныйКлиент Тогда
	...
	Исключение
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió comparar los ficheros por la causa: %1';ru='Не удалось сравнить файлы по причине: %1';en='Could not compare files due to: %1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("es='Los ficheros. La comparación de los ficheros';ru='Файлы. Сравнение файлов';en='Files. File comparison'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
...
Процедура ОбработатьФайлПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Если ТипЗнч(ПолученныеФайлы) <> Тип("Массив") Или ПолученныеФайлы.Количество() = 0 Тогда
		ОбработатьФайлПослеОшибки(НСтр("es='La recepción del fichero era anulada.';ru='Получение файла было отменено.';en='File receipt has been canceled.'"), Неопределено, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьФайлПослеПомещенияФайлов(ПомещенныеФайлы, Контекст) Экспорт
	...
	Если ТипЗнч(ПомещенныеФайлы) <> Тип("Массив") Или ПомещенныеФайлы.Количество() = 0 Тогда
		ОбработатьФайлПослеОшибки(НСтр("es='El local del fichero era anulado.';ru='Помещение файла было отменено.';en='The file location has been canceled.'"), Неопределено, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ПечатьФайлаСоШтампом(ТабличныйДокумент) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
		...
		ПечатнаяФорма.СинонимМакета = НСтр("es='El fichero con la estampilla';ru='Файл со штампом';en='Stamp file'");
...
Функция ПолучитьФайлВРабочийКаталог(Знач АдресДвоичныхДанныхФайла,
	...
	Исключение
		СообщениеОбОшибке = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		СообщениеОбОшибке = НСтр("es='La falta de la creación del catálogo sobre el disco:';ru='Ошибка создания каталога на диске:';en='Error creating directory on disk:'") + " " + СообщениеОбОшибке;
		ОбщегоНазначенияКлиент.СообщитьПользователю(СообщениеОбОшибке);
		Возврат Ложь;
	КонецПопытки;
