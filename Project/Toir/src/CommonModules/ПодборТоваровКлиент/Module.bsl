...
Процедура ВыполнениеРасширенногоПоискаВозможно(Форма, ОписаниеОповещенияПриУспехе = Неопределено) Экспорт
	...
	Если Форма.ИнформационнаяБазаФайловая И Не Форма.ИндексПолнотекстовогоПоискаАктуален Тогда
		...
		ПоказатьВопрос(Оповещение, НСтр("es='El índice полнотекстового de la búsqueda es no actual. ¿Renovar el índice?';ru='Индекс полнотекстового поиска неактуален. Обновить индекс?';en='The full-text search index is irrelevant. Refresh index?'"), РежимДиалогаВопрос.ДаНет);
	Иначе
		Если ОписаниеОповещенияПриУспехе <> Неопределено Тогда
			ВыполнитьОбработкуОповещения(ОписаниеОповещенияПриУспехе);
		КонецЕсли;
...
Функция ТекстПредупрежденияОшибкиРасширенногоПоиска(КодОшибки)
	...
	Если КодОшибки = "НичегоНеНайдено" Тогда
		ТекстПредупреждения = НСтр("es='De nada es encontrado, precisen la interpelación.';ru='Ничего не найдено, уточните запрос.';en='Nothing found, check the request.'");
	ИначеЕсли КодОшибки = "СлишкомМногоРезультатов" Тогда
		ТекстПредупреждения = НСтр("es='Es demasidos resultados de la búsqueda, precisen la interpelación.';ru='Слишком много результатов поиска, уточните запрос.';en='Too many search results, refine your query.'");
	ИначеЕсли КодОшибки = "ОшибкаПоиска" Тогда
		ТекстПредупреждения = НСтр("es='A la ejecución de la búsqueda había una falta, prueben cambiar la expresión de la búsqueda.';ru='При выполнении поиска произошла ошибка, попробуйте изменить выражение поиска.';en='An error occurred while performing a search, try changing the search expression.'");
	Иначе
		ТекстПредупреждения = "";
	КонецЕсли;
...
Процедура ОбновитьИндексПолнотекстовогоПоиска() Экспорт
	...
	Состояние(НСтр("es='Va la renovación del índice полнотекстового de la búsqueda...';ru='Идет обновление индекса полнотекстового поиска ...';en='Full-text search index is being updated ...'"));
	
	ПодборТоваровВызовСервера.ОбновитьИндексПолнотекстовогоПоиска();
	
	Состояние(НСтр("es='La renovación del índice полнотекстового de la búsqueda es acabada.';ru='Обновление индекса полнотекстового поиска завершено.';en='The full-text search index update is completed.'"));
...
Процедура ПриПереходеКСпискуХарактеристик(Форма, ПараметрыНоменклатуры)
	...
	ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Las características: ""%1""';ru='Характеристики: ""%1""';en='Characteristics: ""%1""'"), ПараметрыНоменклатуры.Номенклатура);
