	...
	Если ПустаяСтрока(Контекст.Идентификатор) Тогда 
		...
		Если КомпонентаСодержитЕдинственныйКлассОбъектов Тогда 
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es=""No consiguió conectar el componente exterior sobre el cliente "
"%1 "
"por la causa: "
"no es admisible al mismo tiempo no indicar y el Identificador e IdentifikatorySozdaniyaOb'ektov"";ru='Не удалось подключить внешнюю компоненту на клиенте"
"%1"
"по причине:"
"Не допустимо одновременно не указывать и Идентификатор и ИдентификаторыСозданияОбъектов';en='Failed to connect external component on client"
"%1"
"because of:"
"It is not permissible not to indicate both the Identifier and ИдентификаторыСозданияОбъектов'"), 
	...
	Если Не МестоположениеКомпонентыКорректно(Контекст.Местоположение) Тогда 
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió conectar el componente exterior ""%1"" sobre el cliente "
"%2 "
"por la causa: no es admisible conectar "
"los componentes de la posición indicada.';ru='Не удалось подключить внешнюю компоненту ""%1"" на клиенте"
"%2"
"по причине:"
"Не допустимо подключить компоненты из указанного местоположения.';en='Failed to connect the external component ""%1"" on the client"
"%2"
"due to:"
"It’s not permissible to connect components from the specified location.'"), 
...
Процедура УстановитьКомпоненту(Контекст) Экспорт
	...
	Если Не МестоположениеКомпонентыКорректно(Контекст.Местоположение) Тогда 
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió establecer el componente exterior ""%1"" sobre el cliente "
"%2 "
"por la causa: no es admisible establecer "
"los componentes de la posición indicada.';ru='Не удалось установить внешнюю компоненту ""%1"" на клиенте"
"%2"
"по причине:"
"Не допустимо устанавливать компоненты из указанного местоположения.';en='Failed to install the external component ""%1"" on the client"
"%2"
"due to:"
"It is not permissible to install components from a specified location. '"), 
			Контекст.Идентификатор,
			Контекст.Местоположение);
	КонецЕсли;
...
Процедура ПодтвердитьЗакрытиеФормы() Экспорт
	...
	Если ПустаяСтрока(Параметры.ТекстПредупреждения) Тогда
		ТекстВопроса = НСтр("es='Los datos eran cambiados. ¿Conservar los cambios?';ru='Данные были изменены. Сохранить изменения?';en='Data has been changed. Save changes?'");
	Иначе
...
"%2 "
"por la causa: "
"el Método de NachatPodklyuchenieVneshneyKomponenty ha devuelto la Mentira.';ru='Не удалось подключить внешнюю компоненту ""%1"" на клиенте"
"%2"
"по причине:"
"Метод НачатьПодключениеВнешнейКомпоненты вернул Ложь.';en='Failed to connect the external component"" ""%1"" ""on the client"
"%2"
"due to:                           The Method НачатьПодключениеВнешнейКомпоненты returned False.'"),
				Контекст.Идентификатор,
				Контекст.Местоположение);
...
"%2"
"по причине:"
"%3';en='Failed to connect the external component ""%1"" on the client"
"%2"
"due to:"
"%3'"),
		Контекст.Идентификатор,
...
Функция НовыйОбъектКомпоненты(Контекст)
	...
	Если КомпонентаСодержитЕдинственныйКлассОбъектов Тогда 
		...
		Если ПодключаемыйМодуль = Неопределено Тогда 
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió crear el objeto de los componente ""%1"" exteriores conectados sobre el cliente "
"%2, "
"por la causa: "
"%3';ru='Не удалось создать объект внешней компоненты ""%1"", подключенной на клиенте"
"%2,"
"по причине:"
"%3';en='Failed to create the external component object ""%1"" connected on the client"
"%2"
"due to:"
"%3'"),
				Контекст.Идентификатор,
	...
	Иначе 
		...
		Для каждого ИдентификаторОбъекта Из Контекст.ИдентификаторыСозданияОбъектов Цикл 
			...
			Если ПодключаемыйМодуль = Неопределено Тогда 
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No consiguió crear el objeto ""%1"" componentes ""%2"" exteriores, conectado en el cliente "
"%3, "
"por la causa: "
"%4';ru='Не удалось создать объект ""%1"" внешней компоненты ""%2"", подключенной на клиенте"
"%3,"
"по причине:"
"%4';en='Failed to create object ""%1"" of external component ""%2"" connected on client"
"%3"
"due to:"
"%4'"),
...
"%2"
"по причине:"
"%3';en='Failed to install the external component ""%1"" on the client"
"%2"
"due to:"
"%3 '"),
		Контекст.Идентификатор,
...
Процедура ЗарегистрироватьCOMСоединительПриПроверкеРегистрации(Результат, Контекст) Экспорт
	...
	Если ПриложениеЗапущено Тогда
		...
		Если ВыполнитьПерезагрузкуСеанса Тогда
			...
			ТекстВопроса = 
				НСтр("es='Para la terminación del nuevo registro de los componente comcntr es necesario перезапустить el programa. "
"¿Перезапустить es ahora?';ru='Для завершения перерегистрации компоненты comcntr необходимо перезапустить программу."
"Перезапустить сейчас?';en='To complete the re-registration of comcntr components, you must restart the program."
"Restart now? '");
	...
	Иначе 
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta al registro de los componente comcntr. "
"El código de la falta regsvr32: %1';ru='Ошибка при регистрации компоненты comcntr."
"Код ошибки regsvr32: %1';en='Error registering comcntr components."
"Error code regsvr32:%1'"),
			КодВозврата);
			
		Если КодВозврата = 5 Тогда
			ТекстСообщения = ТекстСообщения + " " + НСтр("es='No basta los derechos de admisión.';ru='Недостаточно прав доступа.';en='Not enough access rights.'");
		Иначе 
			ТекстСообщения = ТекстСообщения + Символы.ПС + ОписаниеОшибки;
		КонецЕсли;
		
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("es='El registro de los componente comcntr';ru='Регистрация компоненты comcntr';en='Register comcntr components'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), 
			"Ошибка", 
			ТекстСообщения,,
			Истина);
...
Процедура ПроверитьРасширениеРаботыСФайламиПодключеноЗавершение(РасширениеПодключено, ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(ТекстСообщения) Тогда
		ТекстСообщения = НСтр("es='La acción es inaccesible, puesto que no es establecida la ampliación para el cliente web 1С:Предприятие.';ru='Действие недоступно, так как не установлено расширение для веб-клиента 1С:Предприятие.';en='The action is not available because the extension for the 1C: Enterprise web client has not been installed.'")
	КонецЕсли;
