...
Процедура ВыполнитьДействиеОбменаДляУзлаИнформационнойБазы(
	...
	СтрокаСообщения = НСтр("es='El comienzo del proceso del cambio para los datos para el nudo %1';ru='Начало процесса обмена данными для узла %1';en='Start of data exchange for node%1'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СтруктураЗаголовкаГиперссылкиМонитораПроблем(Узлы = Неопределено) Экспорт
	...
	Если Количество > 0 Тогда
		...
		Заголовок = НСтр("es='Las prevenciones (%1)';ru='Предупреждения (%1)';en='Warnings (%1)'");
	...
	Иначе
		...
		Заголовок = НСтр("es='Las prevenciones no existen';ru='Предупреждений нет';en='No warnings'");
	...
	Для Каждого Элемент Из Таблицы Цикл
		...
		Если Реквизит <> Неопределено Тогда
			...
			Если ТипыРеквизита.Количество() <> 1 Тогда
				...
				СтрокаСообщения = НСтр("es='El tipo de datos componente para los significados no es apoyado por reticencia. "
"El requisito ""%1"".';ru='Составной тип данных для значений по умолчанию не поддерживается."
"Реквизит ""%1"".';en='The composite data type for default values is not supported."
"Props ""%1 "".'");
			...
			Если Не ОбщегоНазначения.ЭтоСправочник(ОбъектМетаданных) Тогда
				...
				СтрокаСообщения = НСтр("es='La elección de los significados es apoyada por reticencia solamente para las guías. "
"El requisito ""%1"".';ru='Выбор значений по умолчанию поддерживается только для справочников."
"Реквизит ""%1"".';en='The choice of default values is supported only for directories."
"Props """"% 1 """".'");
				СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, Реквизит.ПолноеИмя());
				ВызватьИсключение СтрокаСообщения;
			КонецЕсли;
...
Функция ПредставлениеДатыСинхронизации(Знач ДатаСинхронизации) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДатаСинхронизации) Тогда
		Возврат НСтр("es='La sincronización no se cumplía.';ru='Синхронизация не выполнялась.';en='Synchronization failed.'");
	КонецЕсли;
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La última sincronización: %1';ru='Последняя синхронизация: %1';en='Last sync:%1'"), ОтносительнаяДатаСинхронизации(ДатаСинхронизации));
КонецФункции
...
Функция ОтносительнаяДатаСинхронизации(Знач ДатаСинхронизации) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДатаСинхронизации) Тогда
		...
		Возврат НСтр("es='Nunca';ru='Никогда';en='Never'");
	...
	ИначеЕсли Интервал < 60 * 5 Тогда // 5 мин
		...
		Результат = НСтр("es='Es ahora';ru='Сейчас';en='Now'");
		
	ИначеЕсли Интервал < 60 * 15 Тогда // 15 мин
		
		Результат = НСтр("es='Hace 5 minutos';ru='5 минут назад';en='5 minutes ago'");
		
	ИначеЕсли Интервал < 60 * 30 Тогда // 30 мин
		
		Результат = НСтр("es='Hace 15 minutos';ru='15 минут назад';en='15 minutes ago'");
		
	ИначеЕсли Интервал < 60 * 60 * 1 Тогда // 1 час
		
		Результат = НСтр("es='Hace 30 minutos';ru='30 минут назад';en='30 minutes ago'");
		
	ИначеЕсли Интервал < 60 * 60 * 2 Тогда // 2 часа
		
		Результат = НСтр("es='Hace 1 hora';ru='1 час назад';en='1 hour ago'");
		
	ИначеЕсли Интервал < 60 * 60 * 3 Тогда // 3 часа
		
		Результат = НСтр("es='Hace 2 horas';ru='2 часа назад';en='2 hours ago'");
	...
	Иначе
		...
		Если КоличествоДнейРазницы = 0 Тогда // сегодня
			...
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Hoy, %1';ru='Сегодня, %1';en='Today,%1'"), Формат(ДатаСинхронизации, "ДЛФ=T"));
			
		ИначеЕсли КоличествоДнейРазницы = 1 Тогда // вчера
			
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Ayer, %1';ru='Вчера, %1';en='Yesterday,% 1'"), Формат(ДатаСинхронизации, "ДЛФ=T"));
			
		ИначеЕсли КоличествоДнейРазницы = 2 Тогда // позавчера
			
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Anteayer, %1';ru='Позавчера, %1';en='The day before yesterday, %1'"), Формат(ДатаСинхронизации, "ДЛФ=T"));
...
Процедура ПриСохраненииНастроекПодключенияВнешнейСистемы(Контекст, ПараметрыПодключения, Результат) Экспорт
	...
	Попытка
		Если Контекст.Режим = "НовоеПодключение" Тогда
			...
			ОбновитьЗаписьВРегистрСведений(СтруктураЗаписи, "НастройкиОбменаДаннымиXDTO");
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El régimen conveniente de la llamada de la función: %1';ru='Не подходящий режим вызова функции: %1';en='Inappropriate function call mode:%1'"), Контекст.Режим);
		КонецЕсли;
			...
			Если ОбщегоНазначения.РазделениеВключено() Тогда
				Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса.ОчередьЗаданий") Тогда
					...
					КлючЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='ОбменДаннымиСВнешнейСистемой (%1)';ru='ОбменДаннымиСВнешнейСистемой (%1)';en='ОбменДаннымиСВнешнейСистемой (%1)'"),
...
Функция НомерОбластиИзКодаУзлаПланаОбмена(Знач КодУзла) Экспорт
	...
	Если ТипЗнч(КодУзла) <> Тип("Строка") Тогда
		ВызватьИсключение НСтр("es='El tipo incorrecto del parámetro el número [1].';ru='Неправильный тип параметра номер [1].';en='Invalid parameter type number [1]. '");
	КонецЕсли;
...
Процедура ЗагрузитьПриоритетныеДанныеВПодчиненныйУзелРИБ(Отказ = Ложь) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("es='El cambio para los datos. La carga de los datos de prioridad';ru='Обмен данными.Загрузка приоритетных данных';en='Data exchange. Download priority data'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		
		ВызватьИсключение
			НСтр("es='La falta de la carga de los datos de prioridad del mensaje del cambio. "
"Véase los detalles en la revista del registro.';ru='Ошибка загрузки приоритетных данных из сообщения обмена."
"См. подробности в журнале регистрации.';en='Error loading priority data from exchange message."
"See details in the logbook.'");
	КонецПопытки;
	УстановитьПривилегированныйРежим(Истина);
	УстановитьРежимЗагрузкиСообщенияОбменаДаннымиПередЗапуском("ЗагрузкаРазрешена", Ложь);
	УстановитьПривилегированныйРежим(Ложь);
	...
	Если Отказ Тогда
		...
		Если КонфигурацияИзменена() Тогда
			ВызватьИсключение
				НСтр("es='Son cargados los cambios del programa recibido del nudo principal. "
"Acaben el trabajo del programa. Abran el programa en конфигураторе "
"y cumplan la orden ""Renovar la configuración de la base de datos (F7)"". "
""
"Después de esto pongan en marcha el programa.';ru='Загружены изменения программы, полученные из главного узла."
"Завершите работу программы. Откройте программу в конфигураторе"
"и выполните команду ""Обновить конфигурацию базы данных (F7)""."
""
"После этого запустите программу.';en='Uploaded program changes received from the main node."
"Exit the program. Open the program in the configurator"
"and run the command """" Update Database Configuration (F7) """"."
""
...
Процедура ЗаписатьДанныеДляОбновленияВФайл(Параметры, Данные, ВидДанных, ПолноеИмяОбъекта = "") Экспорт
	...
	Если Параметры.ЗаписьИзмененийДляПодчиненногоУзлаРИБСФильтрами = Неопределено Тогда
		ТекстИсключения = НСтр("es='En el elaborador es incorrectamente organizado el trabajo con los parámetros del registro de los datos al tratamiento.';ru='В обработчике неправильно организована работа с параметрами регистрации данных к обработке.';en='In the handler, the work with the parameters for registering data to processing is incorrectly organized.'");
...
Процедура РассчитатьПроцентВыгрузки(КоличествоВыгруженных, КоличествоОбъектовКВыгрузке) Экспорт
	...
	Если КоличествоОбъектовКВыгрузке = 0 Или КоличествоВыгруженных > КоличествоОбъектовКВыгрузке Тогда
		...
		Шаблон = НСтр("es='Es tratado: %1 objetos.';ru='Обработано: %1 объектов.';en='Processed: %1 objects.'");
	...
	Иначе
		...
		Шаблон = НСтр("es='Es tratado: %1 de %2 objetos.';ru='Обработано: %1 из %2 объектов.';en='Processed: %1 of %2 objects.'");
...
Процедура РассчитатьПроцентЗагрузки(КоличествоЗагруженных, КоличествоОбъектовКЗагрузке, РазмерФайлаСообщенияОбмена) Экспорт
	...
	Если РазмерФайлаСообщенияОбмена <> 0 Тогда
		Шаблон = НСтр("es='La dimensión del mensaje de %1 MB';ru='Размер сообщения %1 МБ';en='Message size%1 MB'");
...
Функция FTPИмяСервераИПуть(Знач СтрокаПодключения) Экспорт
	...
	Если (ВРег(Лев(СтрокаПодключения, 6)) <> "FTP://"
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La línea de la conexión al FTP-recurso no corresponde al formato: ""%1""';ru='Строка подключения к FTP-ресурсу не соответствует формату: ""%1""';en='The connection line to the FTP resource does not match the format: ""%1""'"), СтрокаПодключения);
	КонецЕсли;
	...
	Если ПараметрыПодключения.Количество() < 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='En la línea de la conexión al FTP-recurso no es indicado el nombre del servidor: ""%1""';ru='В строке подключения к FTP-ресурсу не указано имя сервера: ""%1""';en='Server name not specified in FTP connection string: ""% 1""'"), СтрокаПодключения);
	КонецЕсли;
...
Функция ВыбратьИзменения(Знач Узел, Знач НомерСообщения, Знач ФильтрВыборки = Неопределено) Экспорт
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение НСтр("es='El extracto de los cambios de los datos es prohibido en la transacción activa.';ru='Выборка изменений данных запрещена в активной транзакции.';en='Fetching data changes is prohibited in the active transaction.'");
	КонецЕсли;
...
Процедура ПроверитьВозможностьВыполненияОбменов(ПриложениеВИнтернете = Ложь) Экспорт
	...
	Если Не ПравоДоступа("Просмотр", Метаданные.ОбщиеКоманды.Синхронизировать) Тогда
		...
		Если ПриложениеВИнтернете Тогда
			ВызватьИсключение НСтр("es='Es insuficientemente derecho para la sincronización de los datos con la aplicación en el Internet.';ru='Недостаточно прав для синхронизации данных с приложением в Интернете.';en='Not enough rights to sync data with an application on the Internet.'");
		Иначе
			ВызватьИсключение НСтр("es='Es insuficientemente derecho para la sincronización de los datos.';ru='Недостаточно прав для синхронизации данных.';en='Not enough rights to sync data.'");
		КонецЕсли;
	...
	ИначеЕсли ОбновлениеИнформационнойБазы.НеобходимоОбновлениеИнформационнойБазы()
		...
		Если ПриложениеВИнтернете Тогда	
			ВызватьИсключение НСтр("es='La aplicación en el Internet se encuentra en el estado de la renovación.';ru='Приложение в Интернете находится в состоянии обновления.';en='The application on the Internet is in an update state.'");
		Иначе
			ВызватьИсключение НСтр("es='La base informativa se encuentra en el estado de la renovación.';ru='Информационная база находится в состоянии обновления.';en='The infobase is in an update state.'");
		КонецЕсли;
...
Процедура ПроверитьИспользованиеОбменаДанными(УстановитьИспользование = Ложь) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюДанных") Тогда
		...
		Иначе
			ТекстСообщения = НСтр("es='La sincronización de los datos es prohibida por el administrador.';ru='Синхронизация данных запрещена администратором.';en='Data synchronization is prohibited by the administrator.'");
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииОбменДанными(), УровеньЖурналаРегистрации.Ошибка,,,ТекстСообщения);
			ВызватьИсключение ТекстСообщения;
		КонецЕсли;
...
Процедура УдалитьДвиженияУДокумента(ДокументОбъект) Экспорт
	...
	Для Каждого СтрокаДвижения Из ТаблицаДвижений Цикл
		...
		Если Не ПравоДоступа("Изменение", Набор.Метаданные()) Тогда
			...
			ТекстИсключения = НСтр("es='La infracción de los derechos de admisión: %1';ru='Нарушение прав доступа: %1';en='Access violation:%1'");
	...
	Для Каждого СтрокаДвижения Из МассивОбрабатываемыхСтрокТаблицыДвижений Цикл		
		...
		Исключение
			...
			ТекстИсключения = НСтр("es='La operación no es cumplida: "
"%1 %2';ru='Операция не выполнена: %1"
"%2';en='Operation failed:%1"
"%2 '");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий =
		НСтр("es='El traslado de los ajustes de la conexión de los cambios para los datos en un nuevo registro ""los Ajustes del transporte del cambio para los datos"".';ru='Перенос настроек подключения обменов данными в новый регистр ""Настройки транспорта обмена данными"".';en='Transfer data exchange connection settings to the new register ""Data exchange transport settings"".'");
	...
	Обработчик.Комментарий =
		НСтр("es='El relleno de los ajustes auxiliares del cambio para los datos en el registro ""los ajustes Generales de los nudos de las bases informativas"".';ru='Заполнение вспомогательных настроек обмена данными в регистре ""Общие настройки узлов информационных баз"".';en='Filling in auxiliary settings for data exchange in the register ""General settings of nodes of infobases"".'");
	...
	Обработчик.Версия = "3.0.1.91";
	Обработчик.Комментарий =
		НСтр("es='El relleno inicial de los ajustes del cambio para los datos XDTO.';ru='Первоначальное заполнение настроек обмена данными XDTO.';en='Initial completion of XDTO data exchange settings.'");
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.0.2.207";
	Обработчик.Комментарий =
		НСтр("es='El traslado de los resultados de la ejecución de los cambios para los datos en un nuevo registro.';ru='Перенос результатов выполнения обменов данными в новый регистр.';en='Transferring the results of data exchanges to a new register.'");
...
Процедура ПриЗаполненииПоставляемыхПрофилейГруппДоступа(ОписанияПрофилей, ПараметрыОбновления) Экспорт
	...
	ОписаниеПрофиля.Наименование =
		НСтр("es='La sincronización de los datos con otros programas';ru='Синхронизация данных с другими программами';en='Synchronization data with other programs'", Метаданные.ОсновнойЯзык.КодЯзыка);
	ОписаниеПрофиля.Описание =
		НСтр("es='Es fijado complementariamente a aquellos usuarios, a que deben ser accesibles los medios "
"para el monitoring y la sincronización de los datos con otros programas.';ru='Дополнительно назначается тем пользователям, которым должны быть доступны средства"
"для мониторинга и синхронизации данных с другими программами.';en='Additionally assigned to those users for whom funds should be available"
"to monitor and synchronize data with other programs.'",
...
Процедура ПроверитьНедопустимыеСимволыВИмениПользователяWSПрокси(Знач ИмяПользователя)
	...
	Если СтрокаСодержитСимвол(ИмяПользователя, НедопустимыеСимволы) Тогда
		...
		СтрокаСообщения = НСтр("es='En el nombre del usuario %1 contienen los símbolos inadmisibles. "
"El nombre del usuario no debe contener los símbolos %2.';ru='В имени пользователя %1 содержатся недопустимые символы."
"Имя пользователя не должно содержать символы %2.';en='The username% 1 contains invalid characters."
"The username must not contain% 2 characters.'");
...
Процедура ПроверитьКорректностьФорматаАдресаWSПрокси(Знач АдресWSПрокси)
	...
	Если Не ЭтоИнтернетАдрес Тогда
		...
		СтрокаСообщения = НСтр("es='El formato equivocado de la ""%1 dirección. "
"La dirección debe comenzar del prefijo el Internet de la acta %2 (por ejemplo:"" http://myserver.com/service "").';ru='Неверный формат адреса ""%1""."
"Адрес должен начинаться с префикса Интернет протокола %2 (например: ""http://myserver.com/service"").';en='Invalid address format ""%1""."
"The address must begin with the Internet Protocol %2 prefix (for example: ""http://myserver.com/service"").'");
...
Процедура ОбновитьРегламентныеЗаданияСценариевОбменовДанными() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Отказ Тогда
			ВызватьИсключение НСтр("es='La falta a la renovación de la tarea de reglamento para el guión del cambio para los datos.';ru='Ошибка при обновлении регламентного задания для сценария обмена данными.';en='Error while updating the scheduled task for the data exchange scenario.'");
		КонецЕсли;
...
Процедура ВыполнитьОбменДаннымиПоСценариюОбменаДанными(Отказ, НастройкаВыполненияОбмена, НомерСтроки = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			СтрокаСообщения = НСтр("es='El comienzo del proceso del cambio para los datos del ajuste %1';ru='Начало процесса обмена данными по настройке %1';en='Beginning of the process of exchanging setup%1'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ВыполнитьОбменДаннымиПоРегламентномуЗаданию(КодСценарияОбмена) Экспорт
	...
	Если Не ЗначениеЗаполнено(КодСценарияОбмена) Тогда
		ВызватьИсключение НСтр("es='para los datos.';ru='Не задан сценарий обмена данными.';en='No communication scenario specified.'");
	КонецЕсли;
	...
	Иначе
		СтрокаСообщения = НСтр("es='El guión del cambio para los datos con el código %1 no es encontrado.';ru='Сценарий обмена данными с кодом %1 не найден.';en='Communication scenarios with code %1 not found.'");
...
Функция ПолучитьСообщениеОбменаВоВременныйКаталог(Отказ, УзелИнформационнойБазы, ВидТранспортаСообщенийОбмена, ВыводитьСообщения = Истина) Экспорт
	...
	Если СтруктураНастроекОбмена.Отказ Тогда
		...
		Если ВыводитьСообщения Тогда
			НСтрока = НСтр("es='A la inicialización del tratamiento del transporte de los mensajes del cambio han surgido las faltas.';ru='При инициализации обработки транспорта сообщений обмена возникли ошибки.';en='An error occurred while initializing the processing of the message transport.'");
	...
	Если СтруктураНастроекОбмена.РезультатВыполненияОбмена <> Неопределено Тогда
		...
		Если ВыводитьСообщения Тогда
			НСтрока = НСтр("es='A la recepción de los mensajes del cambio han surgido las faltas.';ru='При получении сообщений обмена возникли ошибки.';en='Errors occurred while receiving exchange messages.'");
			ОбщегоНазначения.СообщитьПользователю(НСтрока,,,, Отказ);
		КонецЕсли;
...
Функция ПолучитьСообщениеОбменаВоВременныйКаталогИзИнформационнойБазыКорреспондента(Отказ, УзелИнформационнойБазы, ВыводитьСообщения = Истина) Экспорт
	...
	Исключение
		Если ВыводитьСообщения Тогда
			Сообщение = НСтр("es='No consiguió hacer el cambio: %1';ru='Не удалось произвести обмен: %1';en='Failed to exchange:%1'");
	...
	Если Отказ Тогда
		...
		Если ВыводитьСообщения Тогда
			...
			Сообщение = НСтр("es='No consiguió descargar los datos: %1';ru='Не удалось выгрузить данные: %1';en='Failed to upload data: %1'");
...
Функция ПолучитьСообщениеОбменаВоВременныйКаталогИзИнформационнойБазыКорреспондентаЧерезВебСервис(
	...
	Исключение
		...
		Отказ = Истина;
		Сообщение = НСтр("es='A la descarga de los datos han surgido las faltas en la segunda base informativa: %1';ru='При выгрузке данных возникли ошибки во второй информационной базе: %1';en='When uploading data, errors occurred in the second infobase:%1'", ОбщегоНазначения.КодОсновногоЯзыка());
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Сообщение, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ДлительнаяОперация Тогда
		ЗаписьЖурналаРегистрацииОбменаДанными(НСтр("es='La espera de la recepción de los datos de la base-corresponsal...';ru='Ожидание получения данных от базы-корреспондента...';en='Waiting for data from the correspondent database ...'",
	...
	Исключение
		...
		Отказ = Истина;
		Сообщение = НСтр("es='Han surgido las faltas a la recepción del mensaje del cambio del servicio de la transmisión de los ficheros: %1';ru='Возникли ошибки при получении сообщения обмена из сервиса передачи файлов: %1';en='Errors occurred while receiving exchange message from file transfer service:% 1'", ОбщегоНазначения.КодОсновногоЯзыка());
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Сообщение, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ПолучитьСообщениеОбменаВоВременныйКаталогИзИнформационнойБазыКорреспондентаЧерезВебСервисЗавершениеДлительнойОперации(
	...
	Исключение
		Отказ = Истина;
		Сообщение = НСтр("es='A la recepción del mensaje del cambio han surgido las faltas: %1';ru='При получении сообщения обмена возникли ошибки: %1';en='Errors occurred while receiving exchange message:%1'", ОбщегоНазначения.КодОсновногоЯзыка());
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Сообщение, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		СтруктураНастроекОбмена = Новый Структура("КлючСообщенияЖурналаРегистрации");
		СтруктураНастроекОбмена.КлючСообщенияЖурналаРегистрации = КлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, Перечисления.ДействияПриОбмене.ЗагрузкаДанных);
		ЗаписьЖурналаРегистрацииОбменаДанными(Сообщение, СтруктураНастроекОбмена, Истина);
...
Процедура ВыполнитьОбменДаннымиДляУзлаИнформационнойБазыЧерезФайлИлиСтроку(ПараметрыОбмена) Экспорт
	...
	Если ПараметрыОбмена.УзелИнформационнойБазы = Неопределено Тогда
		...
		Если ПараметрыОбмена.УзелИнформационнойБазы.Пустая()
			...
			ИначеЕсли ОшибкаПерехода Тогда
				СтрокаСообщенияОбОшибке = НСтр("es='No consiguió cumplir el tránsito a la sincronización de los datos a través del formato universal.';ru='Не удалось выполнить переход на синхронизацию данных через универсальный формат.';en='Failed to switch to data synchronization through the universal format.'");
				ВызватьИсключение СтрокаСообщенияОбОшибке;
			КонецЕсли;
		КонецЕсли;
		
		Если ПараметрыОбмена.УзелИнформационнойБазы.Пустая() Тогда
			СтрокаСообщенияОбОшибке = НСтр("es='El nudo del plan del cambio %1 con el código %2 no es encontrado.';ru='Узел плана обмена %1 с кодом %2 не найден.';en='Exchange plan node %1 with code %2 not found.'");
	...
	Если Не НастройкаСинхронизацииЗавершена(ПараметрыОбмена.УзелИнформационнойБазы) Тогда
		...
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='En ""%1"" ajuste de la sincronización de los datos con ""%2"" (el identificador ""%3"") no es acabado.';ru='В ""%1"" настройка синхронизации данных с ""%2"" (идентификатор ""%3"") еще не завершена.';en='In ""% 1"", data synchronization with ""% 2"" (identifier ""% 3"") has not yet been completed.'"),
	...
	Если СтруктураНастроекОбмена.Отказ Тогда
		СтрокаСообщенияОбОшибке = НСтр("es='La falta a la inicialización del proceso del cambio para los datos.';ru='Ошибка при инициализации процесса обмена данными.';en='Error while initializing the data exchange process.'");
		ЗафиксироватьЗавершениеОбмена(СтруктураНастроекОбмена);
		ВызватьИсключение СтрокаСообщенияОбОшибке;
	КонецЕсли;
			...
			Если РезультатВыполненияОбменаВыполнено(СтруктураНастроекОбмена.РезультатВыполненияОбмена) И Не Отказ Тогда
				...
				Попытка
					...
					Иначе
						...
						Если ПараметрыОбмена.ДлительнаяОперация Тогда
							ЗаписьЖурналаРегистрацииОбменаДанными(НСтр("es='La espera de la carga de los datos en la base-corresponsal...';ru='Ожидание загрузки данных в базе-корреспонденте...';en='Waiting for data to load in the correspondent database ...'",
								ОбщегоНазначения.КодОсновногоЯзыка()), СтруктураНастроекОбмена);
							Возврат;
						КонецЕсли;
					КонецЕсли;
	...
	Если ЭтоПланОбменаXDTO Тогда
		...
		Если ПроверятьНаличиеУзлаВКорреспонденте Тогда
			...
			Если НЕ ЗначениеЗаполнено(ПланОбменаСсылка.Code) Тогда
				...
				ТекстСообщения = НСтр("es='Es necesario el tránsito a la sincronización de los datos a través del formato universal en la base-corresponsal.';ru='Необходим переход на синхронизацию данных через универсальный формат в базе-корреспонденте.';en='Required to switch to data synchronization through a universal format in the correspondent database.'");
				...
				Если СтруктураПараметров.Ошибка Тогда
					ЕстьОшибки = Истина;
					НСтрока = НСтр("es='La falta durante el tránsito a la sincronización de los datos a través del formato universal: %1. El cambio es anulado.';ru='Ошибка при переходе на синхронизацию данных через универсальный формат: %1. Обмен отменен.';en='Error while switching to data synchronization via the universal format:%1. Exchange canceled.'",
				...
				ИначеЕсли РезультатПерехода = Неопределено Тогда
					ЕстьОшибки = Истина;
					СтрокаСообщенияОбОшибке = НСтр("es='El tránsito a la sincronización de los datos a través del formato universal no es cumplido';ru='Переход на синхронизацию данных через универсальный формат не выполнен';en='Going to data synchronization through a universal format failed'");
				КонецЕсли;
				Если ЕстьОшибки Тогда
					...
					Возврат;
				Иначе
					Сообщение = НСтр("es='El tránsito a la sincronización de los datos a través del formato universal es acabado con éxito.';ru='Переход на синхронизацию данных через универсальный формат завершен успешно.';en='Going to data synchronization through a universal format was completed successfully.'");
	...
	Если СтруктураНастроекОбменаВнешнееСоединение.Свойство("НастройкаСинхронизацииДанныхЗавершена") Тогда
		Если Не СообщениеДляСопоставленияДанных
			...
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Para la continuación es necesario pasar en el programa ""%1"" y acabar en ella el ajuste de la sincronización. "
"La ejecución del cambio para los datos es anulada.';ru='Для продолжения необходимо перейти в программу ""%1"" и завершить в ней настройку синхронизации."
"Выполнение обмена данными отменено.';en='To continue, you must go to the program"" ""%1"" ""and complete the synchronization setting in it."
"Communication has been canceled. '"),
	...
	Если СтруктураНастроекОбменаВнешнееСоединение.Свойство("ПолученоСообщениеДляСопоставленияДанных") Тогда
		Если Не СообщениеДляСопоставленияДанных
			...
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Para la continuación es necesario pasar en el programa ""%1"" y cumplir la carga del mensaje para la comparación de los datos. "
"La ejecución del cambio para los datos es anulada.';ru='Для продолжения необходимо перейти в программу ""%1"" и выполнить загрузку сообщения для сопоставления данных."
"Выполнение обмена данными отменено.';en='To continue, you must go to the program ""%1"" ""and download the message to match the data. Communication has been canceled.'"),
				СтруктураНастроекОбменаВнешнееСоединение.УзелИнформационнойБазыНаименование);
			ЗаписьЖурналаРегистрацииОбменаДанными(СообщениеОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	Если СтруктураНастроекОбмена.ПроизводитьЗагрузкуДанных Тогда
		Если НЕ ЭтоПланОбменаXDTO Тогда
			...
			Если ПравилаКонвертацииОбъектов = Неопределено Тогда
				...
				НСтрока = НСтр("es='No son dadas las reglas de la conversión en la segunda base informativa para el plan del cambio %1. El cambio es anulado.';ru='Не заданы правила конвертации во второй информационной базе для плана обмена %1. Обмен отменен.';en='No conversion rules were specified in the second infobase for exchange plan%1. Exchange canceled.'",
					ОбщегоНазначения.КодОсновногоЯзыка());
		...
		Иначе
			...
			Исключение
				ЗаписьЖурналаРегистрацииОбменаДанными(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Ha surgido la falta en la segunda base informativa: %1';ru='Возникла ошибка во второй информационной базе: %1';en='An error occurred in the second infobase:% 1'"),
...
Процедура ПередЧтениемСообщенияОбмена(Знач Получатель, СообщениеОбмена, СтандартнаяОбработка)
	...
	Если ЭтоПодчиненныйУзелРИБ()
		...
		Если ТипЗнч(СохраненноеСообщениеОбмена) = Тип("Структура") Тогда
			...
			ЗаписатьСобытиеПолученияДанных(Получатель, НСтр("es='El mensaje del cambio es recibido del cache.';ru='Сообщение обмена получено из кэша.';en='The exchange message received from cache.'"));
...
Процедура ИнициализироватьЧтениеСообщенияДляСтандартнойЗагрузки(ПараметрыЗагрузки, РезультатВыполненияОбмена, СообщениеОбОшибке)
	...
	Если ЧтениеСообщения.Отправитель <> ПараметрыЗагрузки.УзелИнформационнойБазы Тогда
		...
		СообщениеОбОшибке = НСтр("es='El mensaje del cambio contiene los datos para otro nudo de la base informativa.';ru='Сообщение обмена содержит данные для другого узла информационной базы.';en='The exchange message contains data for another node of the infobase.'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		Возврат;
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииУстановкаПодключенияКWebСервису() Экспорт
	...
	Возврат НСтр("es='El cambio para los datos. La instalación de la conexión al web-servicio';ru='Обмен данными.Установка подключения к web-сервису';en='Data exchange. Establishing a connection to a web service'", ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииЗагрузкаПравилДляОбменаДанными() Экспорт
	
	Возврат НСтр("es='El cambio para los datos. La carga de las reglas';ru='Обмен данными.Загрузка правил';en='Data exchange. Download rules'", ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииСозданиеОбменаДанными() Экспорт
	
	Возврат НСтр("es='El cambio para los datos. La creación del cambio para los datos';ru='Обмен данными.Создание обмена данными';en='Data exchange. Create data exchange'", ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииУдалениеОбменаДанными() Экспорт
	
	Возврат НСтр("es='El cambio para los datos. La desaparición del cambio para los datos';ru='Обмен данными.Удаление обмена данными';en='Data exchange. Deleting data exchange'", ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииРегистрацияДанныхДляНачальнойВыгрузки() Экспорт
	
	Возврат НСтр("es='El cambio para los datos. El registro de los datos para la descarga inicial';ru='Обмен данными.Регистрация данных для начальной выгрузки';en='Data Exchange: Data logging for initial upload'", ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииВыгрузкаДанныхДляСопоставления() Экспорт
	
	Возврат НСтр("es='El cambio para los datos. La descarga de los datos para la comparación';ru='Обмен данными.Выгрузка данных для сопоставления';en='Data exchange. Upload data for matching'", ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииУдалениеВременногоФайла() Экспорт
	
	Возврат НСтр("es='El cambio para los datos. La desaparición del fichero temporal';ru='Обмен данными.Удаление временного файла';en='Data exchange. Delete temporary file'", ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииОбменДанными() Экспорт
	
	Возврат НСтр("es='El cambio para los datos';ru='Обмен данными';en='Data exchange'", ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииВыгрузкаДанныхВСервисПередачиФайлов() Экспорт
	
	Возврат НСтр("es='El cambio para los datos. El servicio de la transmisión de los ficheros. La descarga de los datos';ru='Обмен данными.Сервис передачи файлов.Выгрузка данных';en='Data exchange. File transfer service. Data upload'", ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииЗагрузкаДанныхИзСервисаПередачиФайлов() Экспорт
	
	Возврат НСтр("es='El cambio para los datos. El servicio de la transmisión de los ficheros. La carga de los datos';ru='Обмен данными.Сервис передачи файлов.Загрузка данных';en='Data exchange. File transfer service. Data upload.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПолучитьФайлИзХранилищаВСервисе(Знач ИдентификаторФайла, Знач УзелИнформационнойБазы, Знач РазмерЧасти = 1024, Знач ПараметрыАутентификации = Неопределено) Экспорт
	...
	Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El comienzo de la recepción del mensaje del cambio del Internet (la cantidad de las partes del fichero %1).';ru='Начало получения сообщения обмена из Интернета (количество частей файла %1).';en='Start receiving exchange message from the Internet (number of file parts%1).'"),
	...
	Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La terminación de la recepción del mensaje del cambio del Internet (la dimensión del mensaje apretado del cambio de %1 Mb).';ru='Окончание получения сообщения обмена из Интернета (размер сжатого сообщения обмена %1 Мб).';en='The end of receiving the exchange message from the Internet (the size of the compressed exchange message is%1 Mb).'"),
...
Процедура ПриПолученииФайлаИзХранилища(Знач ИдентификаторФайла, ИмяФайла)
	...
	Если РезультатЗапроса.Пустой() Тогда
		Описание = НСтр("es='El fichero con el identificador %1 no es descubierto.';ru='Файл с идентификатором %1 не обнаружен.';en='File with ID%1 not found.'");
...
Процедура ВыполнитьКонтрольЗаписиНеразделенныхДанных(Знач Данные) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		ТекстИсключения = НСтр("es='Es insuficientemente derecho para la ejecución de la acción.';ru='Недостаточно прав для выполнения действия.';en='Not enough rights to perform the action.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ВыполнитьИнициализациюСтруктурыНастроекОбмена(СтруктураНастроекОбмена, НастройкаВыполненияОбмена, НомерСтроки)
	...
	СтруктураНастроекОбмена.КлючСообщенияЖурналаРегистрации = НСтр("es='El cambio para los datos';ru='Обмен данными';en='Data exchange'");
...
Процедура ВыполнитьПроверкуОсновныхПолейСтруктурыНастроекОбмена(СтруктураНастроекОбмена)
	...
	Если НЕ ЗначениеЗаполнено(СтруктураНастроекОбмена.УзелИнформационнойБазы) Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("es='que es necesario hacer el cambio para la información. El cambio es anulado.';ru='Не задан узел информационной базы с которым нужно производить обмен информацией. Обмен отменен.';en='An infobase node has not been specified with which information is to be exchanged. Exchange canceled.'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
		ЗафиксироватьЗавершениеИнициализацииОбмена(СтруктураНастроекОбмена);
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(СтруктураНастроекОбмена.ВидТранспортаОбмена) Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("es='del cambio. El cambio es anulado.';ru='Не задан вид транспорта обмена. Обмен отменен.';en='The type of exchange transport is not specified. Exchange canceled.'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(СтруктураНастроекОбмена.ДействиеПриОбмене) Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("es='No es indicada la acción cumplida (la descarga / la carga). El cambio es anulado.';ru='Не указано выполняемое действие (выгрузка / загрузка). Обмен отменен.';en='The action to be performed is not specified (upload / download). Exchange canceled.'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
...
Процедура ВыполнитьПроверкуСтруктурыОбменаНаВалидность(СтруктураНастроекОбмена, ИспользоватьНастройкиТранспорта = Истина)
	...
	ИначеЕсли СтруктураНастроекОбмена.УзелИнформационнойБазы.ПометкаУдаления Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("es='El nudo de la base informativa es marcado a la desaparición. El cambio es anulado.';ru='Узел информационной базы помечен на удаление. Обмен отменен.';en='The infobase node is marked for deletion. Exchange canceled.'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли СтруктураНастроекОбмена.УзелИнформационнойБазы = СтруктураНастроекОбмена.ТекущийУзелПланаОбмена Тогда
		...
		СтрокаСообщенияОбОшибке = НСтр("es='No es posible organizar el cambio para los datos con el nudo corriente de la base informativa. El cambio es anulado.';ru='Нельзя организовать обмен данными с текущим узлом информационной базы. Обмен отменен.';en='Can not organize data exchange with the current node of the infobase. Exchange canceled.'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли ПустаяСтрока(СтруктураНастроекОбмена.УзелИнформационнойБазыКод)
		...
		СтрокаСообщенияОбОшибке = НСтр("es='Un de los nudos del cambio tiene el código vacío. El cambio es anulado.';ru='Один из узлов обмена имеет пустой код. Обмен отменен.';en='One of the exchange nodes has an empty code. Exchange canceled.'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
	...
	ИначеЕсли СтруктураНастроекОбмена.ОтладкаОбработчиковВыгрузки Тогда
		...
		Если Не ФайлОбработкиВыгрузки.Существует() Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("es='El fichero del tratamiento exterior para el ajuste de la descarga no existe. El cambio es anulado.';ru='Файл внешней обработки для отладки выгрузки не существует. Обмен отменен.';en='External processing file for debugging the upload does not exist. Exchange canceled.'",
				ОбщегоНазначения.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
...
Процедура УстановитьПравилаОбменаВыгрузкиДанных(ОбработкаОбменаДаннымиXML, СтруктураНастроекОбмена)
	...
	Если ПравилаКонвертацииОбъектов = Неопределено Тогда
		...
		// Правила обмена должны быть указаны.
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No son dadas las reglas de la conversión para el plan del cambio %1. La descarga de los datos es anulada.';ru='Не заданы правила конвертации для плана обмена %1. Выгрузка данных отменена.';en='No conversion rules have been set for the exchange plan%1. Data upload canceled.'", ОбщегоНазначения.КодОсновногоЯзыка()),
			СтруктураНастроекОбмена.ИмяПланаОбмена);
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
		ЗафиксироватьЗавершениеИнициализацииОбмена(СтруктураНастроекОбмена);
...
Процедура УстановитьПравилаОбменаЗагрузкиДанных(ОбработкаОбменаДаннымиXML, СтруктураНастроекОбмена)
	...
	Если ПравилаКонвертацииОбъектов = Неопределено Тогда
		...
		// Правила обмена должны быть указаны.
		НСтрока = НСтр("es='No son dadas las reglas de la conversión para el plan del cambio %1. La carga de los datos es anulada.';ru='Не заданы правила конвертации для плана обмена %1. Загрузка данных отменена.';en='No conversion rules have been set for the exchange plan%1. Data upload canceled.'",
			ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗарегистрироватьОшибкуПроведенияДокумента(
	...
	Если Не ПустаяСтрока(СокрЛП(ТекстСообщения)) Тогда
		...
		ПричинаОшибки = " " + НСтр("es='Por razón de %1.';ru='По причине %1.';en='Due to %1.'");
		ПричинаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПричинаОшибки, ТекстСообщения);
		
	КонецЕсли;
	
	СтрокаСообщения = НСтр("es='No consiguió pasar el documento %1 recibido de otra base informativa. El %2 no son rellenados "
"Probablemente todos los requisitos, obligatorio al relleno.';ru='Не удалось провести документ %1, полученный из другой информационной базы.%2"
"Возможно не заполнены все реквизиты, обязательные к заполнению.';en='Failed to post document %1 obtained from another infobase.%2"
"Perhaps all the required details are not filled.'",
		ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗарегистрироватьОшибкуЗаписиОбъекта(Объект, УзелОбмена, ТекстИсключения) Экспорт
	...
	СтрокаСообщения = НСтр("es='No consiguió anotar el objeto %1 recibido de otra base informativa. El %2 no son rellenados "
"Probablemente todos los requisitos, obligatorio al relleno.';ru='Не удалось записать объект %1, полученный из другой информационной базы.%2"
"Возможно не заполнены все реквизиты, обязательные к заполнению.';en='Failed to write to object%1 obtained from another infobase.%2"
"Perhaps all required details are not filled. '",
		ОбщегоНазначения.КодОсновногоЯзыка());
	СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, Строка(Объект), ПричинаОшибки);
...
Процедура ПроверитьВозможностьАдминистрированияОбменов() Экспорт
	...
	Если Не ЕстьПраваНаАдминистрированиеОбменов() Тогда
		...
		ВызватьИсключение НСтр("es='Es insuficientemente derecho para la administración de la sincronización de los datos.';ru='Недостаточно прав для администрирования синхронизации данных.';en='Not enough rights to administer data synchronization.'");
...
Процедура ПроверитьВозможностьВнешнегоСоединения()
	...
	Если ОбщегоНазначения.ЭтоLinuxСервер() Тогда
		...
		ВызватьИсключение НСтр("es='La sincronización de los datos a través de la conexión directa en el servidor bajo la dirección de la SO Linux es inaccesible. "
"Para la sincronización de los datos a través de la conexión directa es necesaria usar la SO Windows.';ru='Синхронизация данных через прямое подключение на сервере под управлением ОС Linux недоступно."
"Для синхронизации данных через прямое подключение требуется использовать ОС Windows.';en='Data synchronization through a direct connection on a server running Linux is not available."
"To synchronize data through a direct connection, you must use Windows.'");
...
Процедура ЗафиксироватьЗавершениеОбмена(СтруктураНастроекОбмена) Экспорт
	...
	Если СтруктураНастроекОбмена.ЭтоОбменВРИБ Тогда
		СтрокаСообщения = НСтр("es='%1, %2';ru='%1, %2';en='%1, %2'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Иначе
		СтрокаСообщения = НСтр("es='%1, %2; los Objetos es tratado: %3';ru='%1, %2; Объектов обработано: %3';en='%1, %2; Objects processed:% 3'", ОбщегоНазначения.КодОсновногоЯзыка());
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения,
							СтруктураНастроекОбмена.РезультатВыполненияОбмена,
							СтруктураНастроекОбмена.ДействиеПриОбмене,
...
Функция РаспаковатьZipФайл(Знач ПолноеИмяФайлаАрхива, Знач ПутьРаспаковкиФайлов, Знач ПарольАрхива = "") Экспорт
	...
	Исключение
		...
		СтрокаСообщения = НСтр("es='La falta al desempaquetamiento de los ficheros del archivo: %1 en el catálogo: %2';ru='Ошибка при распаковке файлов архива: %1 в каталог: %2';en='Error unpacking archive files:%1 to directory:%2'");
...
Функция ЗапаковатьВZipФайл(Знач ПолноеИмяФайлаАрхива, Знач МаскаУпаковкиФайлов, Знач ПарольАрхива = "") Экспорт
	...
	Исключение
		...
		СтрокаСообщения = НСтр("es='La falta a запаковке de los ficheros del archivo: %1 del catálogo: %2';ru='Ошибка при запаковке файлов архива: %1 из каталог: %2';en='Error while packing archive files:%1 from directory:%2'");
...
Функция КлючСообщенияЖурналаРегистрации(УзелИнформационнойБазы, ДействиеПриОбмене) Экспорт
	...
	КлючСообщения = НСтр("es='El cambio para los datos. [ИмяПланаОбмена]. [ДействиеПриОбмене]';ru='Обмен данными.[ИмяПланаОбмена].[ДействиеПриОбмене]';en='Data exchange. [ИмяПланаОбмена].[ДействиеПриОбмене]'",
		ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПроверитьОбязательныеРеквизитыФормы(Форма, Знач Реквизиты)
	...
	Если ОтсутствующиеРеквизиты.Количество() > 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Faltan los requisitos obligatorios de la forma del ajuste del nudo: %1';ru='Отсутствуют обязательные реквизиты формы настройки узла: %1';en='Missing required details of the site configuration form: %1'"),
...
Процедура ВнешнееСоединениеОбновитьНастройкиОбменаДанными(Знач ИмяПланаОбмена, Знач КодУзла, Знач ЗначенияПоУмолчаниюНаУзле) Экспорт
	...
	Если Не ЗначениеЗаполнено(УзелИнформационнойБазы) Тогда
		Сообщение = НСтр("es='del cambio; el nombre del plan del cambio %1; el código del nudo %2';ru='Не найден узел плана обмена; имя плана обмена %1; код узла %2';en='No exchange plan node found; exchange plan name %1; node code %2'");
...
Процедура ВыполнитьПроверкуНаличияПравилОбменаЗагруженныхИзФайла(ПравилаОбменаЗагруженныеИзФайла, ПравилаРегистрацииЗагруженныеИзФайла)
	...
	Если Не Результат.Пустой() Тогда
		...
		СтрокаСообщения = НСтр("es='Para los planes del cambio %1 se usan las reglas del cambio cargado del fichero. "
"Estas reglas pueden ser incompatibles con una nueva versión del programa. "
"Para la prevención del surgimiento posible de las faltas al trabajo con el programa se recomienda actualizar las reglas del cambio del fichero.';ru='Для планов обмена %1 используются правила обмена, загруженные из файла."
"Эти правила могут быть несовместимы с новой версией программы."
"Для предупреждения возможного возникновения ошибок при работе с программой рекомендуется актуализировать правила обмена из файла.';en='For sharing plans%1, sharing rules loaded from a file are used."
"These rules may not be compatible with the new version of the program."
"To prevent possible errors when working with the program, it is recommended to update the exchange rules from the file. '",
				ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПроверитьПодключениеОбработкиТранспортаСообщенийОбмена(Отказ,
	...
	Если Не ОбработкаОбъект.ПодключениеУстановлено() Тогда
		...
		СообщениеОбОшибке = ОбработкаОбъект.СтрокаСообщенияОбОшибке
			+ Символы.ПС + НСтр("es='La información técnica sobre la falta de cm. En la revista del registro.';ru='Техническую информацию об ошибке см. в журнале регистрации.';en='For technical error information, see the log.'");
		
		ЗаписьЖурналаРегистрации(НСтр("es='El transporte de los mensajes del cambio';ru='Транспорт сообщений обмена';en='Transport message exchange '", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция УстановитьВнешнееСоединениеСБазой(СтруктураНастроек) Экспорт
	...
	Если НетПолныхПрав Тогда
		Результат.ПодробноеОписаниеОшибки = НСтр("es='Al usuario indicado para la conexión a otro programa, deben ser fijados los papeles ""el Administrador del sistema"" y ""los Plenos derechos""';ru='Пользователю, указанному для подключения к другой программе, должны быть назначены роли ""Администратор системы"" и ""Полные права""';en='User specified to connect to another program must be assigned the roles ""System administrator"" and ""Full rights""'");
	...
	Иначе
		...
		Если СостояниеНеДопустимо Тогда
			Результат.ПодробноеОписаниеОшибки = НСтр("es='Otro programa se encuentra en el estado de la renovación.';ru='Другая программа находится в состоянии обновления.';en='Another program is in an update state.'");
			Результат.КраткоеОписаниеОшибки   = Результат.ПодробноеОписаниеОшибки;
			Результат.Соединение = Неопределено;
		КонецЕсли;
				...
				Для Каждого ВариантНастройки Из УстаревшиеВариантыНастроекОбмена(СтруктураНастроек.УзелИнформационнойБазы) Цикл
					...
					Если СостояниеНастройки.НастройкаСуществует Тогда
						...
						Иначе
							...
							СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='En el programa ""%1"" se cumple la renovación de los ajustes de la sincronización. "
"La carga de los datos es anulada. Es necesario poner en marcha la sincronización de los datos repetidamente.';ru='В программе ""%1"" выполняется обновление настроек синхронизации."
"Загрузка данных отменена. Необходимо запустить синхронизацию данных повторно.';en='The program ""% 1"" is updating the synchronization settings."
"Data upload canceled. Data synchronization again should be started again.'"),
		...
		Если Не СостояниеНастройки.НастройкаСуществует
			И СтруктураНастроек.ДействиеПриОбмене = Перечисления.ДействияПриОбмене.ВыгрузкаДанных Тогда
			// Проверка возможности выполнения перехода.
			Для Каждого ВариантНастройки Из УстаревшиеВариантыНастроекОбмена(СтруктураНастроек.УзелИнформационнойБазы) Цикл
				СостояниеНастройки = СостояниеНастройкиСинхронизацииВКорреспонденте(
					Прокси,
	...
	Если Не СостояниеНастройки.НастройкаСуществует Тогда
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='""%2"".';ru='Не найдена настройка синхронизации данных ""%1"" с идентификатором ""%2"".';en=""The data synchronization setting '%1' with identifier '%2' was not found."""),
			СтруктураНастроек.ИмяПланаОбмена,
			СтруктураНастроек.ТекущийУзелПланаОбменаКод);
		Отказ = Истина;
	КонецЕсли;
...
Функция WSПроксиДляУзлаИнформационнойБазы(УзелИнформационнойБазы,
	...
	Иначе
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El corresponsal no apoya la versión exigida ""%1"" interfaces ""ОбменДанными"".';ru='Корреспондент не поддерживает требуемую версию ""%1"" интерфейса ""ОбменДанными"".';en='The correspondent does not support version 3.0.1.1 of the ""ОбменДанными"" interface.""To configure the connection, you must update the configuration of the correspondent, or start the configuration from it.'"),
			МинимальнаяВерсия);
	КонецЕсли;
...
Функция ЕстьПодключениеККорреспонденту(Знач Корреспондент,
	...
	СтруктураНастроекОбмена.Вставить("КлючСообщенияЖурналаРегистрации",
		НСтр("es='El cambio para los datos. La comprobación de la conexión';ru='Обмен данными.Проверка подключения';en='Data Exchange: Connection test'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СтруктураНастроекОбмена.Вставить("ТекущийУзелПланаОбмена",
		ОбменДаннымиПовтИсп.ПолучитьЭтотУзелПланаОбмена(СтруктураНастроекОбмена.ИмяПланаОбмена));
	СтруктураНастроекОбмена.Вставить("ТекущийУзелПланаОбменаКод",
...
Процедура ВыполнитьОбновлениеВерсииТиповыхПравилДляОбменаДанными(ПравилаОбменаЗагруженныеИзФайла, ПравилаРегистрацииЗагруженныеИзФайла)
	...
	Для Каждого ЗаписьПланаОбмена Из Результат Цикл
		...
		Если ПравилаОбменаЗагруженныеИзФайла.Найти(ИмяПланаОбмена) = Неопределено
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Se cumple la renovación de las reglas de la conversión de los datos para el plan del cambio %1';ru='Выполняется обновление правил конвертации данных для плана обмена %1';en='Data conversion rules for exchange plan% 1 are being updated'"), ИмяПланаОбмена);
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииОбменДанными(),
		...
		Если ПравилаРегистрацииЗагруженныеИзФайла.Найти(ИмяПланаОбмена) = Неопределено
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Se cumple la renovación de las reglas del registro de los datos para el plan del cambio %1';ru='Выполняется обновление правил регистрации данных для плана обмена %1';en='Data registration rules for exchange plan% 1 are being updated'"), ИмяПланаОбмена);
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииОбменДанными(),
				УровеньЖурналаРегистрации.Информация,,, ТекстСообщения);
	...
	Если Отказ Тогда
		ВызватьИсключение НСтр("es='A la renovación de las reglas del cambio para los datos han surgido las faltas (cm. La revista del registro).';ru='При обновлении правил обмена данными возникли ошибки (см. Журнал регистрации).';en='Errors occurred while updating data exchange rules (see Log).'");
	КонецЕсли;
	...
	Если Файл.Существует() И Файл.ЭтоФайл() Тогда
		...
		Если МаксимальныйДопустимыйРазмерСообщения <> 0 Тогда
			...
			Если РазмерПакета > МаксимальныйДопустимыйРазмерСообщения Тогда
				...
				СтрокаСообщения = НСтр("es='La dimensión del paquete saliente ha compuesto %1 Kb que supera la restricción admisible de %2 Kb.';ru='Размер исходящего пакета составил %1 Кбайт, что превышает допустимое ограничение %2 Кбайт.';en='Size of the outgoing packet was%1 Kb, which exceeds the permissible limit of%2 Kb.'");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюДанных") Тогда
		...
		Если УзелИнформационнойБазы <> Неопределено Тогда
			...
			Попытка
				...
				Если Отказ Тогда
					ВызватьИсключение НСтр("es='La recepción de los datos del nudo principal se ha acabado con las faltas.';ru='Получение данных из главного узла завершилось с ошибками.';en='Retrieving data from the master node failed.'");
				КонецЕсли;
			Исключение
				УстановитьПривилегированныйРежим(Истина);
				УстановитьРежимЗагрузкиСообщенияОбменаДаннымиПередЗапуском("ЗагрузкаРазрешена", Ложь);
		...
		Если КонфигурацияИзменена() Тогда
			...
			Иначе
				...
				ЗаписатьСобытиеПолученияДанных(ГлавныйУзел(),
					НСтр("es='Es descubierto el regreso a la configuración de la base de datos. "
"La sincronización es anulada.';ru='Обнаружен возврат к конфигурации базы данных."
"Синхронизация отменена.';en='Detected return to database configuration."
"Sync canceled.'"));
				Возврат;
			КонецЕсли;
		КонецЕсли;
...
Функция РазличаютсяВерсииКорреспондента(ИмяПланаОбмена, КлючСообщенияЖурналаРегистрации, ВерсияВЭтойПрограмме,
	...
	Если ЗначениеЗаполнено(ВерсияВЭтойПрограмме) И ЗначениеЗаполнено(ВерсияВДругойПрограмме)
		...
		Если ВерсияВЭтойПрограммеБезНомераСборки <> ВерсияВДругойПрограммеБезНомераСборки Тогда
			...
			ШаблонСообщения = НСтр("es='La sincronización de los datos puede ser cumplida es incorrecto, porque la versión del programa ""%1"" (%2) en las reglas de la conversión de este programa se distingue de la versión %3 en las reglas de la conversión en otro programa. Se persuadan que son cargadas las reglas actuales convenientes para las versiones usadas de los dos programas.';ru='Синхронизация данных может быть выполнена некорректно, т.к. версия программы ""%1"" (%2) в правилах конвертации этой программы отличается от версии %3 в правилах конвертации в другой программе. Убедитесь, что загружены актуальные правила, подходящие для используемых версий обеих программ.';en='Data synchronization may not be performed correctly, as the version of the program ""%1"" (%2) in the conversion rules of this program differs from version%3 in the conversion rules in another program. Make sure that the current rules are loaded that are appropriate for the versions of both programs you are using.'");
...
Процедура ПриЗаполненииСпискаТекущихДелПредупрежденияСинхронизации(ТекущиеДела)
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("es='Las prevenciones a la sincronización';ru='Предупреждения при синхронизации';en='Synchronization warnings'");
...
Процедура ПриЗаполненииСпискаТекущихДелНеобходимоОбновление(ТекущиеДела)
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Важное         = Истина;
		Дело.Представление  = НСтр("es='Renovar la versión del programa';ru='Обновить версию программы';en='Update program version'");
		...
		Иначе
			...
			Дело.ПараметрыФормы = Новый Структура("Заголовок,ИмяМакета",
				НСтр("es='La instalación de la renovación';ru='Установка обновления';en='Install update'"), "ИнструкцияКакВыполнитьУстановкуОбновленияВручную");
		КонецЕсли;
		Дело.Владелец       = Раздел;
...
Процедура ПриЗаполненииСпискаТекущихДелПроверитьСовместимостьСТекущейВерсией(ТекущиеДела)
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("es='Las reglas del cambio';ru='Правила обмена';en='Exchange rules'");
		...
		Если ГруппаДела = Неопределено Тогда
			...
			ГруппаДела.Представление = НСтр("es='Comprobar la compatibilidad';ru='Проверить совместимость';en='Check compatibility'");
...
Функция ДеревоМетаданныхКонфигурации(Отбор = Неопределено) Экспорт
	...
	НоваяСтрокаКоллекцииОбъектовМетаданных("Константы",               НСтр("es='Las constantes';ru='Константы';en='Constants'"),                 БиблиотекаКартинок.Константа,              БиблиотекаКартинок.Константа,                    КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("Справочники",             НСтр("es='Las guías';ru='Справочники';en='Directories'"),               БиблиотекаКартинок.Справочник,             БиблиотекаКартинок.Справочник,                   КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("Документы",               НСтр("es='Los documentos';ru='Документы';en='Documents'"),                 БиблиотекаКартинок.Документ,               БиблиотекаКартинок.ДокументОбъект,               КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("ПланыВидовХарактеристик", НСтр("es='Los planes de los tipos de las características';ru='Планы видов характеристик';en='Features types plans'"), БиблиотекаКартинок.ПланВидовХарактеристик, БиблиотекаКартинок.ПланВидовХарактеристикОбъект, КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("ПланыСчетов",             НСтр("es='Los planes de las cuentas';ru='Планы счетов';en='Chart of accounts'"),              БиблиотекаКартинок.ПланСчетов,             БиблиотекаКартинок.ПланСчетовОбъект,             КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("ПланыВидовРасчета",       НСтр("es='Los planes de los tipos del cálculo';ru='Планы видов расчета';en='Calculation types plans'"),       БиблиотекаКартинок.ПланВидовРасчета,       БиблиотекаКартинок.ПланВидовРасчетаОбъект,       КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("РегистрыСведений",        НСтр("es='Los registros de las noticias';ru='Регистры сведений';en='Information registers'"),         БиблиотекаКартинок.РегистрСведений,        БиблиотекаКартинок.РегистрСведений,              КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("РегистрыНакопления",      НСтр("es='Los registros de la acumulación';ru='Регистры накопления';en='Accumulation registers'"),       БиблиотекаКартинок.РегистрНакопления,      БиблиотекаКартинок.РегистрНакопления,            КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("РегистрыБухгалтерии",     НСтр("es='Las tareas';ru='Задачи';en='Tasks'"),      БиблиотекаКартинок.РегистрБухгалтерии,     БиблиотекаКартинок.РегистрБухгалтерии,           КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("РегистрыРасчета",         НСтр("es='Los registros del cálculo';ru='Регистры расчета';en='Billing registers'"),          БиблиотекаКартинок.РегистрРасчета,         БиблиотекаКартинок.РегистрРасчета,               КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("БизнесПроцессы",          НСтр("es='Los procesos de negocios';ru='Бизнес-процессы';en='Business processes'"),           БиблиотекаКартинок.БизнесПроцесс,          БиблиотекаКартинок.БизнесПроцессОбъект,          КоллекцииОбъектовМетаданных);
	НоваяСтрокаКоллекцииОбъектовМетаданных("Задачи",                  НСтр("es='Las tareas';ru='Задачи';en='Tasks'"),                    БиблиотекаКартинок.Задача,                 БиблиотекаКартинок.ЗадачаОбъект,                 КоллекцииОбъектовМетаданных);
...
Функция ОписаниеВариантаНастройкиОбменаПоУмолчанию(ИмяПланаОбмена)
	...
	ЗаголовокФормыПомощника = НСтр("es='La sincronización de los datos con %Программа del % (el ajuste)';ru='Синхронизация данных с %Программа% (настройка)';en='Data synchronization with%Программа% (setting)'");
...
Процедура УстановитьСообщениеОбменаДаннымиИзГлавногоУзла(СообщениеОбмена, ГлавныйУзел) Экспорт
	...
	ЗаписатьСобытиеПолученияДанных(ГлавныйУзел, НСтр("es='El mensaje del cambio es anotado en el cache.';ru='Сообщение обмена записано в кэш.';en='The exchange message is written to the cache.'"));
...
Процедура ОчиститьСообщениеОбменаДаннымиИзГлавногоУзла() Экспорт
	...
	ЗаписатьСобытиеПолученияДанных(ГлавныйУзел(), НСтр("es='El mensaje del cambio es quitado del cache.';ru='Сообщение обмена удалено из кэша.';en='The exchange message has been removed from the cache.'"));
...
Функция СловарьКонтейнераВнешнегоМодуля() Экспорт
	...
	Результат.Вставить("Именительный", НСтр("es='El ajuste la sincronización de los datos';ru='Настройка синхронизация данных';en='Setting data synchronization'"));
	Результат.Вставить("Родительный", НСтр("es='Los ajustes de la sincronización de los datos';ru='Настройки синхронизации данных';en='Data Sync Settings'"));
...
Функция ИнтерактивноеИзменениеВыгрузки(Знач УзелИнформационнойБазы, Знач АдресХранилищаФормы, Знач ЕстьСценарийУзла=Неопределено) Экспорт
	...
	ПараметрыСценарияДополнения.ВариантБезДополнения.Вставить("Пояснение", 
		НСтр("es='Serán enviados solamente los datos conforme a los ajustes generales.';ru='Будут отправлены только данные согласно общим настройкам.';en='Only data will be sent according to the general settings.'")); 
	...
	ПараметрыСценарияДополнения.ВариантВсеДокументы.Вставить("Пояснение",
		НСтр("es='Serán enviados complementariamente todos los documentos por el período, que satisfacen a las condiciones de la selección.';ru='Дополнительно будут отправлены все документы за период, удовлетворяющие условиям отбора.';en='Additionally, all documents for a period that satisfy the selection conditions will be sent.'")); 
	...
	ПараметрыСценарияДополнения.ВариантПроизвольныйОтбор.Вставить("Пояснение",
		НСтр("es='Serán enviados complementariamente los datos conforme a la selección.';ru='Дополнительно будут отправлены данные согласно отбору.';en='Additionally, data will be sent according to the selection.'")); 
	...
	ПараметрыСценарияДополнения.ВариантДополнительно.Вставить("Пояснение",
		НСтр("es='Serán enviados los datos adicionales de los ajustes.';ru='Будут отправлены дополнительные данные по настройкам.';en='Additional settings data will be sent.'")); 
...
Функция ПредставлениеОтбораДополненияВыгрузки(Знач Период, Знач Отбор, Знач ОписаниеПустогоОтбора=Неопределено) Экспорт
	...
	Если ПустаяСтрока(ОтборСтрокой) Тогда
		Если ОписаниеПустогоОтбора=Неопределено Тогда
			ОтборСтрокой = НСтр("es='Todos los objetos';ru='Все объекты';en='All objects'");
		Иначе
...
Функция ПредставлениеДетальногоДополненияВыгрузки(Знач ДополнительнаяРегистрация, Знач ОписаниеПустогоОтбора=Неопределено) Экспорт
	...
	ИначеЕсли ОписаниеПустогоОтбора=Неопределено Тогда
		Возврат НСтр("es='Los datos adicionales no son escogidos';ru='Дополнительные данные не выбраны';en='No additional data selected'");
...
Функция ДополнениеВыгрузкиИмяАвтоСохраненияНастроек() Экспорт
	Возврат НСтр("es='El último envío (se conserva automáticamente)';ru='Последняя отправка (сохраняется автоматически)';en='Last Shipment (Saved Automatically)'");
КонецФункции
