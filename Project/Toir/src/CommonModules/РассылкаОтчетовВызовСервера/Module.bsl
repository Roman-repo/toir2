...
Функция СформироватьСписокПолучателейРассылки(Знач Параметры) Экспорт
	...
	ПараметрыЖурнала.ИмяСобытия   = НСтр("es='El envío de los informes. La formación de la lista de los destinatarios';ru='Рассылка отчетов. Формирование списка получателей';en='Mailing reports reuse. Recipient list compilation'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если РезультатВыполнения.БылиКритичныеОшибки Тогда
		РезультатВыполнения.Текст = НСтр("es='No consiguió formar la lista de los destinatarios';ru='Не удалось сформировать список получателей';en='Failed to generate recipient list'");
...
Функция ЗапуститьФоновоеЗадание(Знач ПараметрыМетода, Знач УникальныйИдентификатор) Экспорт
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("es='Los envíos de los informes: la Ejecución de los envíos en el fondo';ru='Рассылки отчетов: Выполнение рассылок в фоне';en='Reporting mailing: Running mailing in the background'");
