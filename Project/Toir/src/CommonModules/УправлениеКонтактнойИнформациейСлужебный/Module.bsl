...
Функция ПервоеЗначениеКонтактнойИнформацииОбъектаПоТипу(Ссылка, ТипКонтактнойИнформации, Дата) Экспорт
	...
	Если СтрНачинаетсяС(ПолноеИмя , НСтр("es='La guía';ru='Справочник';en='Directory'")) Тогда
		ИмяГруппыКонтактнойИнформации = НСтр("es='La guía';ru='Справочник';en='Directory'") + Ссылка.Метаданные().Имя;
	ИначеЕсли СтрНачинаетсяС(ПолноеИмя , НСтр("es='El documento';ru='Документ';en='Document'")) Тогда
		ИмяГруппыКонтактнойИнформации = НСтр("es='El documento';ru='Документ';en='Document'") + Ссылка.Метаданные().Имя;
	КонецЕсли;
...
Функция ПроверитьАдрес(Адрес, ПараметрыПроверкиАдреса = Неопределено) Экспорт
	...
	Если ТипЗнч(Адрес) <> Тип("Строка") Тогда
		...
		РезультатПроверки.СписокОшибок.Добавить("ФорматАдреса", НСтр("es='El formato incorrecto de la dirección';ru='Некорректный формат адреса';en='Invalid address format'"));
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("es='La renovación de las noticias de la información de contacto. "
"Antes de la terminación del tratamiento alguna información de contacto puede ser representada es incorrecto.';ru='Обновление сведений контактной информации."
"До завершения обработки некоторая контактная информация может отображаться некорректно.';en='Updating contact information."
"Until the processing is complete, some contact information may not be displayed correctly. '");
	...
	Обработчик.Комментарий = НСтр("es='La renovación de las noticias de la información de contacto que contienen las direcciones con la historia. "
"Antes de la terminación del tratamiento alguna información de contacto puede ser representada es incorrecto.';ru='Обновление сведений контактной информации содержащих адреса с историей."
"До завершения обработки некоторая контактная информация может отображаться некорректно.';en='Updating contact information containing history addresses."
"Until the processing is complete, some contact information may not be displayed correctly.'");
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.НачальноеЗаполнение = Ложь;
	...
	Обработчик.Комментарий = НСтр("es='El relleno del significado del campo ""Otro"" cerca de los tipos de la información de contacto. "
"Antes de la terminación del tratamiento del campo ""Otro"" de los documentos serán representados en algunos casos es incorrecto.';ru='Заполнение значения поля ""Другое"" у видов контактной информации."
"До завершения обработки поля ""Другое"" документов в ряде случаев будут отображаться некорректно.';en='Filling in the value of the"" Other ""field for the types of contact information."
"Until the processing of the ""Other"" field, the documents will in some cases be displayed incorrectly. '");
	...
	Обработчик.ПроцедураПроверки  = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Комментарий = НСтр("es='La renovación de las noticias sobre los países los mundos en concordancia con el clasificador de toda Rusia de los países del mundo. "
"Antes de la terminación del tratamiento el nombre de los países en los documentos será representado en algunos casos es incorrecto.';ru='Обновление сведений о странах мирах в соответствии с общероссийским классификатором стран мира."
"До завершения обработки наименование стран в документах в ряде случаев будет отображаться некорректно.';en='Updating information about the countries of the world in accordance with the all-Russian classifier of countries of the world."
"Until the processing is completed, the name of the countries in the documents will in some cases be displayed incorrectly.'");
...
Процедура ОбновитьСуществующиеСтраныМира() Экспорт
	...
	Для Каждого СтрокаКлассификатора Из МодульРаботаСАдресами.ТаблицаКлассификатора() Цикл
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La falta de la anotación del país del mundo %1 (el código %2) a la renovación del clasificador, %3';ru='Ошибка записи страны мира %1 (код %2) при обновлении классификатора, %3';en='Error writing country of the world%1 (code%2) when updating the classifier,%3'"),
...
Процедура ЗаполнитьНаименованияВидовКонтактнойИнформацииПользователя() Экспорт
	...
	ПараметрыГруппы.Наименование = НСтр("es='La información de contacto de la guía ""los Usuarios""';ru='Контактная информация справочника ""Пользователи""';en='Contact information of the ""Users"" directory'");
	...
	ПараметрыВида.Наименование = НСтр("es='El correo electrónico';ru='Электронная почта';en='Email'");
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("es='La información de contacto';ru='Контактная информация';en='Contact Information'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ОбновитьПредставлениеАдреса(Адрес, ВключатьСтрануВПредставление)
	...
	Если ТипЗнч(Адрес) <> Тип("Структура") Тогда
		ВызватьИсключение НСтр("es='Para la formación de la representación de la dirección es entregado el tipo incorrecto de la dirección';ru='Для формирования представления адреса передан некорректный тип адреса';en='Invalid address type passed to generate address representation'");
	КонецЕсли;
...
Функция ДесериализацияПрочейКонтактнойИнформации(ЗначенияПолей, Представление = "", ОжидаемыйТип = Неопределено)
	...
	ИначеЕсли ОжидаемыйТип <> Неопределено Тогда
		ВызватьИсключение НСтр("es='La falta десериализации de la información de contacto, se espera otro tipo';ru='Ошибка десериализации контактной информации, ожидается другой тип';en='Contact deserialization error, another type expected'");
...
Функция ПроверитьПараметрыВидаКонтактнойИнформации(ВидКонтактнойИнформации) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ВидКонтактнойИнформации.Тип) Тогда
		...
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='""%1"".';ru='Не заполнено обязательное поле Тип у вида контактной информации ""%1"".';en='The required field Type for the contact information type ""%1"" is not filled.'"),
	...
	Если НЕ ЗначениеЗаполнено(ВидКонтактнойИнформации.Наименование) Тогда
		Результат.ЕстьОшибки = Истина;
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='""%1"".';ru='Не заполнено обязательное поле Наименование у вида контактной информации ""%1"".';en='The required field Name for the contact information type ""%1"" is not filled.'"),
	...
	Если ВидКонтактнойИнформации.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
		...
		Если НЕ ВидКонтактнойИнформации.ТолькоНациональныйАдрес
			...
			ИЛИ ВидКонтактнойИнформации.СкрыватьНеактуальныеАдреса) Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Son rellenados los ajustes de la comprobación de la dirección cerca del tipo de la información de contacto %1."
" La comprobación de la corrección de la dirección es accesible solamente para las direcciones rusas';ru='Некорректно заполнены настройки проверки адреса у вида контактной информации %1."
" Проверка корректности адреса доступна только для российских адресов';en='Incorrectly configured settings for checking the address for the contact information type%1."
"  Verifying the address is available only for Russian addresses '"), Строка(ВидКонтактнойИнформации.Наименование));
					Разделитель = Символы.ПС;
			КонецЕсли;
			
		Если ВидКонтактнойИнформации.РазрешитьВводНесколькихЗначений
			И ВидКонтактнойИнформации.ХранитьИсториюИзменений Тогда
				Результат.ТекстОшибки = Результат.ТекстОшибки + Разделитель + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Son rellenados los ajustes de la dirección cerca del tipo de la información de contacto %1."
" No se permite la posibilidad de la introducción de algunos significados de la información de contacto a la historia incluida del almacenaje de los cambios.';ru='Некорректно заполнены настройки адреса у вида контактной информации %1."
" Не допускается возможность ввода нескольких значений контактной информации при включенной истории хранения изменений.';en='The address settings for the contact information type%1 are incorrectly filled."
"  It’s not allowed to enter multiple contact information values with the change storage history turned on. '"),
						Строка(ВидКонтактнойИнформации.Наименование));
		КонецЕсли;
	КонецЕсли;
...
Функция КонтактнаяИнформацияИзXML(Знач Текст, Знач ОжидаемыйВид = Неопределено, РезультатПреобразования = Неопределено, Знач Представление = "") Экспорт
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВXML(Текст) Тогда
		...
		Если НеобходимоВосстановитьКонтактнуюИнформацию Тогда
			ОписаниеПричиныОшибки = НСтр("es='Las noticias de la información de contacto eran restablecidas después de la intermitencia.';ru='Сведения контактной информации были восстановлены после сбоя.';en='Contact information was restored after a failure.'");
		...
		Если ТекстОшибки = Неопределено И ОжидаемыйТип <> Неопределено Тогда
			...
			Если Результат = Неопределено Тогда
				ТекстОшибки = СтрЗаменить(НСтр("es='Las noticias de la información de contacto %ОжидаемыйВид del % eran estropeadas o rellenadas.';ru='Сведения контактной информации %ОжидаемыйВид% были повреждены или некорректно заполнены.';en='Contact information  %ОжидаемыйВид% was damaged or incorrectly filled out.'"),
			...
			Иначе
				...
				ШаблонСообщения = СтрЗаменить(НСтр("es='Las noticias %1 información de contacto %ОжидаемыйВид del % eran estropeadas o rellenadas.';ru='Сведения %1 контактной информации %ОжидаемыйВид% были повреждены или некорректно заполнены.';en='Contact information %1 %ОжидаемыйВид% was damaged or incorrectly filled out.'"),
					"%ОжидаемыйВид%", Строка(ОжидаемыйВид));
				Если ОжидаемыйТип = ПеречислениеАдрес И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, НСтр("es='Sobre la dirección';ru='об адресе';en='about address'"));
				ИначеЕсли ОжидаемыйТип = ПеречислениеАдресЭлектроннойПочты И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "ЭлектроннаяПочта") Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, НСтр("es='Del correo electrónico';ru='электронной почты';en='Email'"));
				ИначеЕсли ОжидаемыйТип = ПеречислениеВебСтраница И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "ВебСайт") Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, НСтр("es='Las páginas web';ru='веб-страницы';en='web pages'"));
				...
				ИначеЕсли ОжидаемыйТип = ПеречислениеДругое И НайденТип <> ФабрикаXDTO.Тип(ПространствоИмен, "Прочее") Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, НСтр("es='Sobre el número de teléfono';ru='о номере телефона';en='about phone number'"));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	...
	Иначе
		ТекстОшибки = НСтр("es='Las noticias sobre el tipo de la información de contacto %1 eran estropeadas o rellenadas, "
"porque el campo obligatorio el tipo no es rellenado.';ru='Сведения о виде контактной информации %1 были повреждены или некорректно заполнены,"
"т.к. обязательное поле тип не заполнено.';en='Information about the type of contact information%1 was damaged or incorrectly filled out,"
"because required field type is not filled.'");
...
Функция ДесериализацияТелефонаФакса(ЗначенияПолей, Представление = "", ОжидаемыйТип = Неопределено)
	...
	Иначе
		ВызватьИсключение НСтр("es='La falta десериализации de la información de contacto, se espera teléfono o el fax';ru='Ошибка десериализации контактной информации, ожидается телефон или факс';en='Contact deserialization error, phone or fax expected'");
	КонецЕсли;
...
Функция РазличияКонтактнойИнформации(Знач Данные1, Знач Данные2) Экспорт
	...
	Если ТипКонтактнойИнформации <> ДанныеКИ2.ТипКонтактнойИнформации Тогда
		...
		СтрокаРезультата[Колонки.Добавить("Описание").Имя]  = НСтр("es='Los tipos distintos de la información de contacto';ru='Различные типы контактной информации';en='Different types of contact information'");
		Возврат Результат;
	КонецЕсли;
...
Функция ПривестиКонтактнуюИнформациюXML(Знач Данные) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("es='La falta de los parámetros de la transformación, no es determinado el tipo de la información de contacto';ru='Ошибка параметров преобразования, не определен тип контактной информации';en='Conversion parameter error, contact information type not defined'");
...
Функция КонтактнаяИнформацияИзJSONВXML(Знач КонтактнаяИнформация, ОжидаемыйТип = Неопределено) Экспорт
	...
	Если ОжидаемыйТип = Неопределено Тогда
		...
		Иначе
			ТекстОшибки = НСтр("es='La falta de la conversión de la información de contacto del formato JSON en XML.';ru='Ошибка конвертации контактной информации из формата JSON в XML.';en='Error converting contact information from JSON to XML format.'");
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), 
				УровеньЖурналаРегистрации.Ошибка,,,
				ТекстОшибки + Символы.ПС + Строка(КонтактнаяИнформация));
			ВызватьИсключение НСтр("es='No consiguió determinar el tipo de la información de contacto. Más detalladamente cm. En la revista del registro.';ru='Не удалось определить тип контактной информации. Подробнее см. в журнале регистрации.';en='Failed to determine the type of contact information. See the logbook for more details.'");
		КонецЕсли;
...
Функция КонвертироватьТелефонФаксИзJSONВXML(ЗначенияПолей, Представление = "", ОжидаемыйТип = Неопределено)
	...
	Иначе
		ВызватьИсключение НСтр("es='La falta de la transformación de la información de contacto, se espera teléfono o el fax';ru='Ошибка преобразования контактной информации, ожидается телефон или факс';en='Error converting contact information, phone or fax expected'");
	КонецЕсли;
