	...
	Если Программа <> Неопределено Тогда
		...
		Если ПрограммаНеНайдена Тогда
			МенеджерКриптографииДобавитьОшибку(Ошибки, Программа,
				НСтр("es='el uso.';ru='Не предусмотрена для использования.';en='Not intended for use.'"), Истина);
			Возврат Неопределено;
		КонецЕсли;
...
Функция МенеджерКриптографииСвойстваПрограммы(ОписаниеПрограммы, ЭтоLinux, Ошибки, ЭтоСервер,
	...
	Если Не ЗначениеЗаполнено(ОписаниеПрограммы.ИмяПрограммы) Тогда
		МенеджерКриптографииДобавитьОшибку(Ошибки, ОписаниеПрограммы.Ссылка,
			НСтр("es='del programa.';ru='Не указано имя программы.';en='The program name is not specified.'"), Истина);
		Возврат Неопределено;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ОписаниеПрограммы.ТипПрограммы) Тогда
		МенеджерКриптографииДобавитьОшибку(Ошибки, ОписаниеПрограммы.Ссылка,
			НСтр("es='del programa.';ru='Не указан тип программы.';en='Type of program not specified.'"), Истина);
		Возврат Неопределено;
	КонецЕсли;
	...
	Если ЭтоLinux Тогда
		...
		Если Не ЗначениеЗаполнено(ПутьКПрограмме) Тогда
			МенеджерКриптографииДобавитьОшибку(Ошибки, ОписаниеПрограммы.Ссылка,
				НСтр("es='el uso.';ru='Не предусмотрена для использования.';en='Not intended for use.'"), ЭтоСервер, , , Истина);
			Возврат Неопределено;
		КонецЕсли;
	Иначе
		ПутьКПрограмме = "";
...
Функция МенеджерКриптографииАлгоритмыУстановлены(ОписаниеПрограммы, Менеджер, Ошибки) Экспорт
	...
	Исключение
		...
		МенеджерКриптографииДобавитьОшибку(Ошибки, ОписаниеПрограммы.Ссылка, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es escogido el algoritmo desconocido de la firma ""%1"".';ru='Выбран неизвестный алгоритм подписи ""%1"".';en='An unknown signature algorithm ""%1"" was selected.'"), АлгоритмПодписи), Истина);
		Возврат Ложь;
	КонецПопытки;
	...
	Исключение
		...
		// Платформа использует обобщенное сообщение "Неизвестный алгоритм криптографии". Требуется более конкретное.
		МенеджерКриптографииДобавитьОшибку(Ошибки, ОписаниеПрограммы.Ссылка, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es escogido el algoritmo desconocido хеширования ""%1"".';ru='Выбран неизвестный алгоритм хеширования ""%1"".';en='An unknown hash algorithm ""%1"" was selected.'"), АлгоритмХеширования), Истина);
		Возврат Ложь;
	КонецПопытки;
	...
	Исключение
		...
		// Платформа использует обобщенное сообщение "Неизвестный алгоритм криптографии". Требуется более конкретное.
		МенеджерКриптографииДобавитьОшибку(Ошибки, ОписаниеПрограммы.Ссылка, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es escogido el algoritmo desconocido de la cifración ""%1"".';ru='Выбран неизвестный алгоритм шифрования ""%1"".';en=""Unknown encryption algorithm '%1' selected."""), АлгоритмШифрования), Истина);
		Возврат Ложь;
	КонецПопытки;
...
Процедура МенеджерКриптографииПрограммаНеНайдена(ОписаниеПрограммы, Ошибки, ЭтоСервер) Экспорт
	...
	МенеджерКриптографииДобавитьОшибку(Ошибки, ОписаниеПрограммы.Ссылка,
		НСтр("es='El programa no es encontrado en el ordenador.';ru='Программа не найдена на компьютере.';en='The program was not found on the computer.'"), ЭтоСервер, Истина);
...
Функция МенеджерКриптографииИмяПрограммыСовпадает(ОписаниеПрограммы, ИмяПрограммыПолученное, Ошибки, ЭтоСервер) Экспорт
	...
	Если ИмяПрограммыПолученное <> ОписаниеПрограммы.ИмяПрограммы Тогда
		МенеджерКриптографииДобавитьОшибку(Ошибки, ОписаниеПрограммы.Ссылка, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es recibido otro programa con el nombre ""%1"".';ru='Получена другая программа с именем ""%1"".';en='Received another program named ""%1"".'"), ИмяПрограммыПолученное), ЭтоСервер, Истина);
		Возврат Ложь;
	КонецЕсли;
...
Функция СертификатПросрочен(Сертификат, НаДату, ДобавкаВремени) Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='En %1 es caducado el certificado.';ru='На %1 просрочен сертификат.';en='%1 expired certificate.'"), Формат(НачалоДня(НаДату), "ДЛФ=D"));
...
Функция ЗаголовокОшибкиПолученияДанных(Операция) Экспорт
	...
	Если Операция = "Подписание" Тогда
		Возврат НСтр("es='A la recepción de los datos para la firma ha surgido la falta:';ru='При получении данных для подписания возникла ошибка:';en='An error occurred while getting the data for signing:'");
		
	ИначеЕсли Операция = "Шифрование" Тогда
		Возврат НСтр("es='A la recepción de los datos para la cifración ha surgido la falta:';ru='При получении данных для шифрования возникла ошибка:';en='An error occurred while getting data for encryption:'");
	Иначе
		Возврат НСтр("es='A la recepción de los datos para el desciframiento ha surgido la falta:';ru='При получении данных для расшифровки возникла ошибка:';en='An error occurred while getting data for decryption:'");
	КонецЕсли;
...
Функция ПустыеДанныеПодписи(ДанныеПодписи, ОписаниеОшибки) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДанныеПодписи) Тогда
		ОписаниеОшибки = НСтр("es='Es formada la firma vacía.';ru='Сформирована пустая подпись.';en='An empty signature has been generated.'");
		Возврат Истина;
	КонецЕсли;
...
Функция ПустыеЗашифрованныеДанные(ЗашифрованныеДанные, ОписаниеОшибки) Экспорт
	...
	Если Не ЗначениеЗаполнено(ЗашифрованныеДанные) Тогда
		ОписаниеОшибки = НСтр("es='Son formados los datos vacíos.';ru='Сформированы пустые данные.';en='Formed blank data.'");
		Возврат Истина;
	КонецЕсли;
...
Функция ПредставлениеСертификата(Сертификат, ДобавкаВремени, МодульЛокализации) Экспорт
	...
	Если ПустаяСтрока(Представление) Тогда
		ДатыСертификата = ДатыСертификата(Сертификат, ДобавкаВремени);
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1, hasta %2';ru='%1, до %2';en='%1, up to%2'"),
...
Процедура ЗаполнитьОписаниеДанныхСертификата(Таблица, СвойстваСертификата) Экспорт
	...
	Если СвойстваСертификата.Подписание И СвойстваСертификата.Шифрование Тогда
		Назначение = НСтр("es='La firma de los datos, la Cifración de los datos';ru='Подписание данных, Шифрование данных';en='Data signing, Data encryption'");
		
	ИначеЕсли СвойстваСертификата.Подписание Тогда
		Назначение = НСтр("es='La firma de los datos';ru='Подписание данных';en='Data signing'");
	Иначе
		Назначение = НСтр("es='La cifración de los datos';ru='Шифрование данных';en='Data encryption'");
	КонецЕсли;
	
	Таблица.Очистить();
	Строка = Таблица.Добавить();
	Строка.Свойство = НСтр("es='Quien es dado:';ru='Кому выдан:';en='To whom issued:'");
	Строка.Значение = СокрЛП(СвойстваСертификата.КомуВыдан);
	
	Строка = Таблица.Добавить();
	Строка.Свойство = НСтр("es='Por quién es dado:';ru='Кем выдан:';en='Issued by:'");
	Строка.Значение = СокрЛП(СвойстваСертификата.КемВыдан);
	
	Строка = Таблица.Добавить();
	Строка.Свойство = НСтр("es='Es válido hasta:';ru='Действителен до:';en='Valid until:'");
	Строка.Значение = Формат(СвойстваСертификата.ДействителенДо, "ДЛФ=D");
	
	Строка = Таблица.Добавить();
	Строка.Свойство = НСтр("es='El destino:';ru='Назначение:';en='Appointment:'");
