...
Функция ДвоичныеДанныеФайла(Знач ПрисоединенныйФайл, Знач ВызыватьИсключение = Истина) Экспорт
	...
	Если ФайлОбъект.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Los datos binarios del fichero faltan en el registro de DvoichnyeDannyeFayla "
""
"la Referencia al fichero: ""%1"".';ru='Двоичные данные файла отсутствуют в регистре ДвоичныеДанныеФайла"
""
"Ссылка на файл: ""%1"".';en='Binary file data is not in the register ДвоичныеДанныеФайла"
""
"File link: ""%1"".'"),
			ПолучитьНавигационнуюСсылку(ПрисоединенныйФайл));
		ЗаписьЖурналаРегистрации(НСтр("es='Los ficheros. La apertura del fichero';ru='Файлы.Открытие файла';en='Files. Open a file'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		ПредставлениеВладельцаФайла = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es juntado a %1: %2';ru='Присоединен к %1 : %2';en='Attached to %1 : %2'"),
	...
	Иначе
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("es='Los ficheros. La recepción del fichero del tomo';ru='Файлы.Получение файла из тома';en='Files. Retrieving a file from a volume'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.Справочники[ПрисоединенныйФайл.Метаданные().Имя],
				ПрисоединенныйФайл,
				СообщениеОбОшибке);
...
Функция ДанныеФайла(Знач ПрисоединенныйФайл,
	...
	Если ВызыватьИсключение Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(ФайлОбъект <> Неопределено, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No es encontrado el fichero juntado ""%1"" (%2)';ru='Не найден присоединенный файл ""%1"" (%2)';en='Attached file ""%1"" (%2) not found'"),
...
Функция НоваяСсылкаНаФайл(ВладелецФайлов, ИмяСправочника = Неопределено) Экспорт
	...
	ЗаголовокОшибки = НСтр("es='La falta a la recepción de la nueva referencia al fichero juntado.';ru='Ошибка при получении новой ссылки на присоединенный файл.';en='Error occurred while getting a new link to the attached file.'");
...
Функция ИмяФормыОбъектаФайловПоВладельцу(Знач ВладелецФайлов) Экспорт
	...
	ЗаголовокОшибки = НСтр("es='La falta a la recepción del nombre de la forma del fichero juntado.';ru='Ошибка при получении имени формы присоединенного файла.';en='Error getting form name of attached file.'");
	ОкончаниеОшибки = НСтр("es='En este caso la recepción de la forma es imposible.';ru='В этом случае получение формы невозможно.';en='In this case, obtaining a form is not possible.'");
...
Функция ДобавитьФайлСДиска(ВладелецФайлов, ПутьКФайлуНаДиске) Экспорт
	...
	Если Не ЗначениеЗаполнено(ВладелецФайлов) Тогда
		ВызватьИсключение НСтр("es='No el valor dado del parámetro VladeletsFaylov en RabotaSFaylami. ДобавитьФайлСДиска.';ru='Не задано значение параметра ВладелецФайлов в РаботаСФайлами.ДобавитьФайлСДиска.';en='Parameter value not set ВладелецФайлов в РаботаСФайлами.ДобавитьФайлСДиска.'");
	КонецЕсли;
...
Функция ДобавитьФайл(ПараметрыФайла,
	...
	Если Не ЗначениеЗаполнено(ВладелецФайлов) Тогда
		ВызватьИсключение НСтр("es='No el valor dado del parámetro VladeletsFaylov en RabotaSFaylami. ДобавитьФайл.';ru='Не задано значение параметра ВладелецФайлов в РаботаСФайлами.ДобавитьФайл.';en='Parameter value not set ВладелецФайлов в РаботаСФайлами.ДобавитьФайл.'");
	КонецЕсли;
	...
	ЗаголовокОшибки = НСтр("es='La falta a la adición del fichero juntado.';ru='Ошибка при добавлении присоединенного файла.';en='Error adding attachment.'");
	...
	Если НоваяСсылкаНаФайл = Неопределено Тогда
		ИмяСправочника = РаботаСФайламиСлужебный.ИмяСправочникаХраненияФайлов(ВладелецФайлов, "", ЗаголовокОшибки,
			НСтр("es='En este caso el parámetro ""NovayaSsylkaNaFayl"" debe ser indicado.';ru='В этом случае параметр ""НоваяСсылкаНаФайл"" должен быть указан.';en='In this case, the parameter ""NewReferenceFile"" must be specified.'"));
	...
	Иначе
		Если Не Справочники.ТипВсеСсылки().СодержитТип(ТипЗнч(НоваяСсылкаНаФайл))
			...
			ВызватьИсключение НСтр("es='La falta a la adición del fichero juntado. "
"La referencia a un nuevo fichero no es rellenada.';ru='Ошибка при добавлении присоединенного файла."
"Ссылка на новый файл не заполнена.';en='Error adding file attachment."
"The link to the new file is not filled.'");
		КонецЕсли;
	...
	Если ПрисоединенныйФайл.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда
		...
		Исключение
			...
			ШаблонСообщения = НСтр("es='La falta a la adición del fichero juntado "
"""%1: %2""';ru='Ошибка при добавлении присоединенного файла ""%1"":"
"%2';en='Error adding attachment"" ""%1"" "":"
"%2 '");
			...
			ЗаписьЖурналаРегистрации(
				НСтр("es='Los ficheros. La adición del fichero juntado';ru='Файлы.Добавление присоединенного файла';en='Files. Adding an Attached File'",
...
Функция СконвертироватьФайлыВПрисоединенные(Знач ВладелецФайлов, ИмяСправочника = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(ВладелецФайлов) Тогда
		ВызватьИсключение НСтр("es='No el valor dado del parámetro VladeletsFaylov en РаботаСФайлами.СконвертироватьФайлыВПрисоединенные.';ru='Не задано значение параметра ВладелецФайлов в РаботаСФайлами.СконвертироватьФайлыВПрисоединенные.';en='Parameter value not set ВладелецФайлов в РаботаСФайлами.СконвертироватьФайлыВПрисоединенные.'");
	КонецЕсли;
	
	Результат = Новый Соответствие;
	ЗаголовокОшибки = НСтр("es='La falta a la conversión de los ficheros juntados.';ru='Ошибка при конвертации присоединенных файлов.';en='Error while converting attached files.'");
	ИмяСправочника = РаботаСФайламиСлужебный.ИмяСправочникаХраненияФайлов(
		ВладелецФайлов, ИмяСправочника, ЗаголовокОшибки);
...
Процедура ИзменитьСправочникХраненияФайлов(Знач ВладелецФайлов, ИмяСправочника = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(ВладелецФайлов) Тогда
		ВызватьИсключение НСтр("es='No el valor dado del parámetro VladeletsFaylov en РаботаСФайлами.ИзменитьСправочникХраненияФайлов.';ru='Не задано значение параметра ВладелецФайлов в РаботаСФайлами.ИзменитьСправочникХраненияФайлов.';en='Parameter value not set ВладелецФайлов в РаботаСФайлами.ИзменитьСправочникХраненияФайлов.'");
	КонецЕсли;
...
Функция ГиперссылкаФайлов() Экспорт
	...
	ПараметрыГиперссылки.Вставить("Заголовок",                 НСтр("es='Los ficheros';ru='Файлы';en='Files'"));
...
Функция ПолеФайла() Экспорт
	...
	ПараметрыПоля.Вставить("ТекстНевыбраннойКартинки",  НСтр("es='Añadir la imagen';ru='Добавить изображение';en='Add image'"));
	...
	ПараметрыПоля.Вставить("ФильтрДиалогаВыбора",       НСтр("es='Todos los ficheros (*. *) |*.*';ru='Все файлы(*.*)|*.*';en='All files (*. *) | *. *'"));
...
Процедура ПометитьНаУдалениеПрисоединенныеФайлы(Знач Источник, ИмяСправочника = Неопределено)
	...
	Исключение
		...
		ВызватьИсключение НСтр("es='La falta a la marca a la desaparición de los ficheros juntados.';ru='Ошибка при пометке на удаление присоединенных файлов.';en='Error flagging for deletion of attached files.'")
			+ Символы.ПС
			+ ПредставлениеОшибки;
	КонецПопытки;
	...
	Для каждого ОписаниеИмениСправочника Из ИменаСправочников Цикл
		...
		Пока Выборка.Следующий() Цикл
			Если Источник.ПометкаУдаления И ЗначениеЗаполнено(Выборка.Редактирует) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='""%1"" no puede ser quitado, "
"porque contiene el fichero ""%2"" juntado "
"ocupado para la redacción.';ru='""%1"" не может быть удален,"
"т.к. содержит присоединенный файл ""%2"","
"занятый для редактирования.';en='""% 1"" can not be deleted,"
"as contains the attached file ""% 2"","
"busy for editing.'"),
...
Функция ТекстОшибкиПриПолученииФайла(Знач ИнформацияОбОшибке, Знач Файл)
	...
	Если Файл <> Неопределено Тогда
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='%1 "
""
"Referencia al fichero: ""%2"".';ru='%1"
""
"Ссылка на файл: ""%2"".';en='%1"
""
"File Link: ""%2 "".'"),
			СообщениеОбОшибке,
...
Процедура СоздатьГиперссылкуФайлов(Форма, ДобавляемыйЭлемент, ВладелецПрисоединенныхФайлов, ПараметрыГиперссылки)
	...
	Если ДобавляемыйЭлемент.ДобавлятьФайлы
		...
		ПодменюДобавления.Заголовок   = НСтр("es='Juntar los ficheros';ru='Присоединить файлы';en='Attach files'");
		ПодменюДобавления.Подсказка   = НСтр("es='Juntar los ficheros';ru='Присоединить файлы';en='Attach files'");
		...
		ЗагрузитьФайл.Подсказка = НСтр("es='Cargar el fichero del disco';ru='Загрузить файл с диска';en='Download file from disk'");
		ЗагрузитьФайл.Заголовок = НСтр("es='Cargar...';ru='Загрузить...';en='Download...'");
		...
		СоздатьПоШаблону.Заголовок = НСтр("es='Crear por el modelo...';ru='Создать по шаблону...';en='Create by template ...'");
		...
		Сканировать.Заголовок = НСтр("es='Escanear...';ru='Сканировать...';en='Scan...'");
...
Процедура СоздатьПолеФайла(Форма, ДобавляемыйЭлемент, ВладелецПрисоединенныхФайлов, ПараметрыПоляФайла)
	...
	СвойстваКнопкиЗагрузка.Вставить("Заголовок",            НСтр("es='Cargar...';ru='Загрузить...';en='Download...'"));
	...
	СвойстваКнопкиВыбор.Вставить("Заголовок",            НСтр("es='Escoger de juntado...';ru='Выбрать из присоединенных...';en='Choose from affiliated ...'"));
		...
		Если ЗначениеЗаполнено(РеквизитРазмещения)
			...
			Если ДанныеФайла <> Неопределено Тогда
				...
				Если ЗначениеДвоичныхДанных = Неопределено Тогда
					...
					ЭлементПредпросмотра.ТекстНевыбраннойКартинки = НСтр("es='La imagen falta';ru='Изображение отсутствует';en='No image'");
				КонецЕсли;
	...
	Если ДобавляемыйЭлемент.ВыводитьЗаголовокФайла Тогда
		...
		Если Не ЗначениеЗаполнено(РеквизитРазмещения) Тогда
			...
			ГиперссылкаЗаголовка.Заголовок = НСтр("es='Cargar';ru='загрузить';en='download'");
		ИначеЕсли ОбщегоНазначения.ЭтоСсылка(ТипЗнч(РеквизитРазмещения)) Тогда
			ЗаголовокФайла.Подсказка       = НСтр("es='Abrir el fichero';ru='Открыть файл';en='Open file'");
	...
	Если ДобавляемыйЭлемент.ПоказыватьКоманднуюПанель Тогда
		...
		ПодменюДобавления.Заголовок   = НСтр("es='Sustituir';ru='Заменить';en='Replace'");
		ПодменюДобавления.Отображение = ОтображениеКнопки.Картинка;
	...
	Если ДобавляемыйЭлемент.ВыбиратьФайл
		...
		ВыбратьФайл.Подсказка = НСтр("es='Escoger el fichero de juntado';ru='Выбрать файл из присоединенных';en='Select a file from the attached'");
	...
	Если ДобавляемыйЭлемент.ПросматриватьФайл Тогда
		...
		ПросмотретьФайл.Заголовок = НСтр("es='Ver';ru='Просмотреть';en='View'");
	...
	Если ДобавляемыйЭлемент.ОчищатьФайл
		...
		ОчиститьФайл.Заголовок = НСтр("es='Limpiar';ru='Очистить';en='Clear'");
	...
	Если ДобавляемыйЭлемент.РедактироватьФайл = "ВФорме" Тогда
		...
		РедактироватьФайл.Заголовок = НСтр("es='Abrir la tarjeta';ru='Открыть карточку';en='Open card'");
		РедактироватьФайл.Подсказка = НСтр("es='Abrir la tarjeta del fichero juntado';ru='Открыть карточку присоединенного файла';en='Open attached file card'");
	...
	ИначеЕсли ДобавляемыйЭлемент.РедактироватьФайл = "Непосредственно"
		...
		РедактироватьФайл.Заголовок = НСтр("es='Redactar';ru='Редактировать';en='Edit'");
		РедактироватьФайл.Подсказка = НСтр("es='Abrir el fichero para la redacción';ru='Открыть файл для редактирования';en='Open file for editing'");
		ЗаполнитьЗначенияСвойств(РедактироватьФайл, СвойстваКомандыФормыКартинка);
		...
		ПоместитьФайл.Заголовок = НСтр("es='Acabar la redacción';ru='Закончить редактирование';en='Finish editing'");
		ПоместитьФайл.Подсказка = НСтр("es='Conservar y liberar el fichero en la base informativa';ru='Сохранить и освободить файл в информационной базе';en='Save and free file in infobase'");
		...
		ОтменитьРедактирование.Заголовок = НСтр("es='Anular la redacción';ru='Отменить редактирование';en='Cancel editing'");
		ОтменитьРедактирование.Подсказка = НСтр("es='Liberar el fichero ocupado';ru='Освободить занятый файл';en='Free inaccessible file'");
