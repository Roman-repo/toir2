...
//      КомандаПечати.Представление = НСтр("es='La cuenta a la paga (a la impresora)';ru='Счет на оплату (на принтер)';en='Account for payment (to printer)'");
...
//  КомандаФормы.Заголовок = НСтр("es='Mi orden';ru='Моя команда';en='My team'");
//  
...
//   ТабличныйДокумент.Область("R1C1").Текст = НСтр("es='El ejemplo del uso del elaborador del servidor de la orden conectada.';ru='Пример использования серверного обработчика подключенной команды.';en='An example of using the server handler of a connected command.'");
//  
