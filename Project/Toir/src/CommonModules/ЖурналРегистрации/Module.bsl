...
Процедура ПрочитатьСобытияЖурналаРегистрации(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	Если ДатыОтбораУказаны И ДатаНачала > ДатаОкончания Тогда
		ВызватьИсключение НСтр("es='Son dadas las condiciones de la selección de la revista del registro. La fecha ha comenzado más de fechas de la terminación.';ru='Некорректно заданы условия отбора журнала регистрации. Дата начала больше даты окончания.';en='The conditions for selecting the logbook are incorrectly set. The start date is greater than the end date.'");
	КонецЕсли;
	...
	Для Каждого СобытиеЖурнала Из СобытияЖурнала Цикл
			...
			Если СобытиеЖурнала.Данные <> Неопределено Тогда
				Если СобытиеЖурнала.Данные.Свойство("Право") Тогда
					СобытиеЖурнала.Данные = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El derecho: %1';ru='Право: %1';en='Right:%1'"), 
						СобытиеЖурнала.Данные.Право);
				Иначе
					СобытиеЖурнала.Данные = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La acción: %1%2';ru='Действие: %1%2';en='%1%2 action'"), 
		...
		ИначеЕсли СобытиеЖурнала.Событие = "_$User$_.New"
			...
			КонецЕсли;
			СобытиеЖурнала.Данные = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El nombre: %1...';ru='Имя: %1, ...';en='Name: %1, …'"), ИмяПользователяИБ);
		...
		Если СобытиеЖурнала.Пользователь = Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000") Тогда
			СобытиеЖурнала.ИмяПользователя = НСтр("es='<Es indefinido>';ru='<Неопределен>';en='<Неопределен>'");
		...
		ИначеЕсли ПользователиИнформационнойБазы.НайтиПоУникальномуИдентификатору(СобытиеЖурнала.Пользователь) = Неопределено Тогда
			СобытиеЖурнала.ИмяПользователя = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 <Es quitado>';ru='%1 <Удален>';en='%1 <Удален>'"), СобытиеЖурнала.ИмяПользователя);
		КонецЕсли;
...
Процедура СформироватьПредставлениеОтбора(ПредставлениеОтбора, ОтборЖурналаРегистрации, 
	...
	Для Каждого ЭлементОтбора Из ОтборЖурналаРегистрации Цикл
		...
		Если ВРег(ИмяОграничения) = ВРег("ИмяПриложения") Тогда
			ИмяОграничения = НСтр("es='La aplicación';ru='Приложение';en='Application'");
		ИначеЕсли ВРег(ИмяОграничения) = ВРег("СтатусТранзакции") Тогда
			ИмяОграничения = НСтр("es='El estatus de la transacción';ru='Статус транзакции';en='Transaction status'");
		ИначеЕсли ВРег(ИмяОграничения) = ВРег("ПредставлениеДанных") Тогда
			ИмяОграничения = НСтр("es='La representación de los datos';ru='Представление данных';en='Data presentation'");
		ИначеЕсли ВРег(ИмяОграничения) = ВРег("РабочийСервер") Тогда
			ИмяОграничения = НСтр("es='El servidor de trabajo';ru='Рабочий сервер';en='Production server'");
		ИначеЕсли ВРег(ИмяОграничения) = ВРег("ОсновнойIPПорт") Тогда
			ИмяОграничения = НСтр("es='Básico IP el puerto';ru='Основной IP порт';en='Main IP port'");
		ИначеЕсли ВРег(ИмяОграничения) = ВРег("ВспомогательныйIPПорт") Тогда
			ИмяОграничения = НСтр("es='Auxiliar IP el puerto';ru='Вспомогательный IP порт';en='Auxiliary IP port'");
		ИначеЕсли ВРег(ИмяОграничения) = ВРег("РазделениеДанныхСеанса") Тогда
			ИмяОграничения = НСтр("es='La división de los datos de la sesión';ru='Разделение данных сеанса';en='Session sharing'");
		КонецЕсли;
	...
	Если ПустаяСтрока(ПредставлениеОтбора) Тогда
		ПредставлениеОтбора = НСтр("es='No es establecido';ru='Не установлен';en='Not installed'");
	КонецЕсли;
...
Процедура ДобавитьОграничениеВПредставлениеОтбора(ОтборЖурналаРегистрации, ПредставлениеОтбора, ИмяОграничения,
	...
	Если ИмяОграничения = "Событие" И СписокОграничений.Количество() > 5 Тогда
		...
		Ограничение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Los acontecimientos (%1)';ru='События (%1)';en='Events (%1)'"), СписокОграничений.Количество());
		
	ИначеЕсли ИмяОграничения = "Сеанс" И СписокОграничений.Количество() > 3 Тогда
		
		Ограничение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Las sesiones (%1)';ru='Сеансы (%1)';en='Sessions (%1)'"), СписокОграничений.Количество());
	...
	Иначе
		...
		Для Каждого ЭлементСписка Из СписокОграничений Цикл
			...
			И ПустаяСтрока(Ограничение) Тогда
				...
				Ограничение = НСтр("es='[ИмяОграничения]: [el Significado]';ru='[ИмяОграничения]: [Значение]';en='[ИмяОграничения]: [Значение]'");
