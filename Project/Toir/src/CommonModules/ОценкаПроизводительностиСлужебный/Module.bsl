			...
			Если Каталоги.Свойство("ВыполнятьЭкспортВЛокальныйКаталог") И Каталоги.ВыполнятьЭкспортВЛокальныйКаталог = Истина Тогда
				Если Каталоги.Свойство("ЛокальныйКаталогЭкспорта") И ЗначениеЗаполнено(Каталоги.ЛокальныйКаталогЭкспорта) Тогда
					Элемент = МодульРаботаВБезопасномРежиме.РазрешениеНаИспользованиеКаталогаФайловойСистемы(
						...
						Истина,
						Истина,
						НСтр("es='El catálogo de red para la exportación de los resultados de las mediciones de la productividad.';ru='Сетевой каталог для экспорта результатов замеров производительности.';en='Network directory for exporting performance measurements.'"));
			...
			Если Каталоги.Свойство("ВыполнятьЭкспортНаFTP") И Каталоги.ВыполнятьЭкспортНаFTP = Истина Тогда
				Если Каталоги.Свойство("FTPКаталогЭкспорта") И ЗначениеЗаполнено(Каталоги.FTPКаталогЭкспорта) Тогда
					Элемент = МодульРаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
						...
						НСтр("es='El FTP-recurso para la exportación de los resultados de las mediciones de la productividad.';ru='FTP-ресурс для экспорта результатов замеров производительности.';en='FTP resource for exporting performance measurements.'"));
					Разрешения.Добавить(Элемент);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Функция ОбщийМодуль(Имя) Экспорт
	...
	Иначе
		...
		Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
			СообщениеИсключения = НСтр("es='El módulo general ""%1"" no es encontrado.';ru='Общий модуль ""%1"" не найден.';en=""The general module '%1' was not found.""");
...
Функция СерверныйМодульМенеджера(Имя)
	...
	Если Не ОбъектНайден Тогда
		СообщениеИсключения = НСтр("es='El objeto de los metadatos ""%1"" no es encontrado, "
