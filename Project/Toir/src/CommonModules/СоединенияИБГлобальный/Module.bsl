...
Процедура КонтрольРежимаЗавершенияРаботыПользователей() Экспорт
	...
	Шаблон = НСтр("es='Se recomienda acabar el trabajo corriente y conservar todos los datos. El trabajo del programa será acabado %1 en %2. "
"%3';ru='Рекомендуется завершить текущую работу и сохранить все свои данные. Работа программы будет завершена %1 в %2. "
"%3';en='It is recommended to complete the current work and save all your data. The program will end%1 in%2."
"%3 '");
...
Процедура ЗавершитьРаботуПользователей() Экспорт
	...
	Если ТекущийМомент < ВремяНачалаБлокировки Тогда
		...
		ТекстСообщения = НСтр("es='El bloqueo del trabajo de los usuarios es planeado en %1.';ru='Блокировка работы пользователей запланирована на %1.';en='User blocking is planned at% 1.'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ВремяНачалаБлокировки);
		ПоказатьОповещениеПользователя(НСтр("es='La terminación del trabajo de los usuarios';ru='Завершение работы пользователей';en='User work completing'"), 
	...
	Исключение
		...
		#Если Не МобильныйКлиент Тогда
			ПоказатьОповещениеПользователя(НСтр("es='La terminación del trabajo de los usuarios';ru='Завершение работы пользователей';en='User work completing'"),
			"e1cib/app/Обработка.БлокировкаРаботыПользователей", 
			НСтр("es='La terminación de las sesiones no es cumplida. Los detalles de cm. En la revista del registro.';ru='Завершение сеансов не выполнено. Подробности см. в Журнале регистрации.';en='Session completing failed. See the log for details.'"), БиблиотекаКартинок.Предупреждение32);
		#КонецЕсли
	...
	ПоказатьОповещениеПользователя(НСтр("es='La terminación del trabajo de los usuarios';ru='Завершение работы пользователей';en='User work completing'"),
		"e1cib/app/Обработка.БлокировкаРаботыПользователей", 
		НСтр("es='La terminación de las sesiones es cumplida con éxito';ru='Завершение сеансов выполнено успешно';en='Session completing completed successfully'"), БиблиотекаКартинок.Информация32);
	#КонецЕсли
	Оповестить("ЗавершениеРаботыПользователей", Новый Структура("Статус,КоличествоСеансов", "Готово", КоличествоСеансов));
	СоединенияИБКлиент.ЗавершитьРаботуЭтогоСеанса();
