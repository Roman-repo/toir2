...
Процедура Подписать(ОписаниеДанных, Форма = Неопределено, ОбработкаРезультата = Неопределено) Экспорт
	...
	СерверныеПараметры.Вставить("Операция",            НСтр("es='La firma de los datos';ru='Подписание данных';en='Data signing'"));
...
Процедура Зашифровать(ОписаниеДанных, Форма = Неопределено, ОбработкаРезультата = Неопределено) Экспорт
	...
	СерверныеПараметры = Новый Структура;
	СерверныеПараметры.Вставить("Операция",            НСтр("es='La cifración de los datos';ru='Шифрование данных';en='Data encryption'"));
	СерверныеПараметры.Вставить("ЗаголовокДанных",     НСтр("es='Los datos';ru='Данные';en='Data'"));
...
Процедура Расшифровать(ОписаниеДанных, Форма = Неопределено, ОбработкаРезультата = Неопределено) Экспорт
	...
	СерверныеПараметры = Новый Структура;
	СерверныеПараметры.Вставить("Операция",            НСтр("es='El desciframiento de los datos';ru='Расшифровка данных';en='Data decryption'"));
	СерверныеПараметры.Вставить("ЗаголовокДанных",     НСтр("es='Los datos';ru='Данные';en='Data'"));
	СерверныеПараметры.Вставить("ОтборСертификатов");
...
Процедура ИнформироватьОПодписанииОбъекта(ПредставлениеДанных, МножествоДанных = Ложь, ИзФайла = Ложь) Экспорт
	...
	Если ИзФайла Тогда
		Если МножествоДанных Тогда
			ТекстСообщения = НСтр("es='Son añadidas las firmas de los ficheros:';ru='Добавлены подписи из файлов:';en='Added captions from files:'");
		Иначе
			ТекстСообщения = НСтр("es='Es añadida la firma del fichero:';ru='Добавлена подпись из файла:';en='Added signature from file:'");
		КонецЕсли;
	Иначе
		Если МножествоДанных Тогда
			ТекстСообщения = НСтр("es='Son establecidas las firmas:';ru='Установлены подписи:';en='Signatures set:'");
		Иначе
			ТекстСообщения = НСтр("es='Es establecida la firma:';ru='Установлена подпись:';en='Signed by:'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ИнформироватьОШифрованииОбъекта(ПредставлениеДанных, МножествоДанных = Ложь) Экспорт
	...
	ТекстСообщения = НСтр("es='Es cumplida la cifración:';ru='Выполнено шифрование:';en='Encryption completed:'");
...
Процедура ИнформироватьОРасшифровкеОбъекта(ПредставлениеДанных, МножествоДанных = Ложь) Экспорт
	...
	ТекстСообщения = НСтр("es='Es cumplido el desciframiento:';ru='Выполнена расшифровка:';en='Decryption completed:'");
