...
Процедура ПолучитьУникальныйИдентификаторАбонента(
	...
	Если ТипЗнч(СертификатЭП) <> Тип("СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования")
		...
		ВызватьИсключение НСтр("es='El significado incorrecto del parámetro ""SertifikatEP"".';ru='Некорректное значение параметра ""СертификатЭП"".';en='Invalid value of the ""Certificate EEP"" parameter.'");
	КонецЕсли;
...
Процедура ЗарегистрироватьСертификатАбонента(
	...
	ИначеЕсли ТипЗнч(ИдентификаторАбонента) <> Тип("Строка") Или ПустаяСтрока(ИдентификаторАбонента) Тогда
		ВызватьИсключение НСтр("es='El significado incorrecto del parámetro ""IdentifikatorAbonenta"".';ru='Некорректное значение параметра ""ИдентификаторАбонента"".';en='""ИдентификаторАбонента""  parameter invalid value.'");
	КонецЕсли;
...
Процедура ПриПолученииРезультатаСтатусаЗаявки(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("Структура") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El tipo inadmisible del significado del parámetro el Resultado (%1). "
"Se Espera el tipo la Estructura.';ru='Недопустимый тип значения параметра Результат (%1)."
"Ожидается тип Структура.';en='Invalid value type for the Result parameter (%1)."
"The expected type is Structure.'"),
	...
	ИначеЕсли Результат.Действие = "НоваяЗаявка" Тогда
		...
		Если ДополнительныеПараметры.СтатусЗаявки <> "Отклонена"
			...
			ТекстИсключения =
				НСтр("es='El significado inadmisible del parámetro la Acción. Una nueva demanda puede ser creada solamente "
"al estado corriente de la demanda ""Falta"" o ""Es declinada"".';ru='Недопустимое значение параметра Действие. Новая заявка может быть создана только"
"при текущем состоянии заявки ""Отсутствует"" или ""Отклонена"".';en='Invalid value for the Action parameter. A new ticket can only be created"
"in the current state of the application ""None"" or ""Rejected"".'");
	...
	ИначеЕсли Результат.Действие = "ПроверитьСтатусЗаявки" Тогда
		...
		Если ДополнительныеПараметры.СтатусЗаявки <> "ОжиданиеОбработки" Тогда
			...
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El significado inadmisible del parámetro la Acción. El estado de la ejecución de la demanda no puede ser comprobado, "
"porque la demanda se encuentra en el estado ""%1"".';ru='Недопустимое значение параметра Действие. Состояние выполнения заявки не может быть проверено,"
"т.к. заявка находится в состоянии ""%1"".';en='Invalid value for the parameter Action. The progress status of the application could not be verified,"
"because claim is in state ""%1"".'"),
	...
	Иначе
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El significado inadmisible del parámetro la Acción: ""%1"".';ru='Недопустимое значение параметра Действие: ""%1"".';en='Invalid value for the parameter Action: ""%1"".'"),
			Результат.Действие);
		ВызватьИсключение ТекстИсключения;
...
"The expected type is Structure.'"),
	...
	ИначеЕсли Результат.Действие = "ОтправитьЗаявку" Тогда
		...
		Если Не Результат.Свойство("ДанныеЗаявки") Тогда
			ВызватьИсключение НСтр("es='El significado inadmisible del parámetro el Resultado. Falta el campo de DannyeZayavki.';ru='Недопустимое значение параметра Результат. Отсутствует поле ДанныеЗаявки.';en='The value for the Результат parameter is not valid. Missing ДанныеЗаявки field.'");
		КонецЕсли;
		
		ДанныеЗаявки = Результат.ДанныеЗаявки;
		Если ТипЗнч(ДанныеЗаявки) <> Тип("Структура") Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El tipo inadmisible del significado del parámetro el Resultado. ДанныеЗаявки (%1). "
"Se Espera el tipo la Estructura.';ru='Недопустимый тип значения параметра Результат.ДанныеЗаявки (%1)."
"Ожидается тип Структура.';en='Invalid value type for Результат.ДанныеЗаявки (%1)."
"The expected type is Structure.'"),
	...
	Иначе
		...
		// Неизвестное действие.
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El significado inadmisible del parámetro ""la Acción"": ""%1"".';ru='Недопустимое значение параметра ""Действие"": ""%1"".';en='Invalid value for the Action parameter: ""%1"".'"),
			Результат.Действие);
		ВызватьИсключение ТекстИсключения;
...
Процедура РасшифроватьКонтрольныйМаркерЭПСервера(
	...
	СообщениеОбОшибкеДляПользователя = НСтр("es='La falta durante la comprobación de la consigna del certificado. "
"Más detalladamente cm. En la revista del registro.';ru='Ошибка при проверке пароля сертификата."
"Подробнее см. в журнале регистрации.';en='Error verifying certificate password."
"See the logbook for details.'");
	...
	Если ПараметрыДляРасшифрования.markerED = Неопределено Тогда
		...
		Подключение1СТакскомВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("es='La falta durante la comprobación de la autenticidad del propietario del certificado. Falta el marcador аутентификации (markerED)';ru='Ошибка при проверке подлинности владельца сертификата. Отсутствует маркер аутентификации (markerED)';en='Error verifying certificate owner authentication. Missing authentication token (markerED)'"));
	...
	Исключение
		...
		СообщениеДляЖурналаРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta durante la comprobación de la autenticidad del propietario del certificado. No consiguió recibir los datos binarios del marcador (markerED). "
"%1';ru='Ошибка при проверке подлинности владельца сертификата. Не удалось получить двоичные данные маркера (markerED)."
"%1';en='Error while authenticating certificate holder. Failed to get binary marker data (markerED)."
"%1'"),
	...
	Иначе
		...
		ЗавершитьБизнесПроцесс(КонтекстВзаимодействия);
		Подключение1СТакскомВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("es='La falta durante la comprobación de la autenticidad del propietario del certificado. No es indicado el certificado (IDCertificateED, IDCertificateED_Dop)';ru='Ошибка при проверке подлинности владельца сертификата. Не указан сертификат (IDCertificateED, IDCertificateED_Dop)';en='Error verifying certificate owner authentication. Certificate not specified (IDCertificateED, IDCertificateED_Dop)'"));
		ПоказатьПредупреждение(, СообщениеОбОшибкеДляПользователя);
		Возврат;
	...
	ОписаниеДанных.Вставить("Операция", НСтр("es='La comprobación de la consigna del certificado';ru='Проверка пароля сертификата';en='Certificate password checking'"));
...
Процедура ПоказатьПричинуОтклоненияЗаявкиЭДО(КонтекстВзаимодействия) Экспорт
	...
	Если НЕ ПустаяСтрока(ОписаниеПричины.КодОшибки) Тогда
		ТекстСообщения = НСтр("es='El código de la falta: %1';ru='Код ошибки: %1';en='Error code:%1'");
	...
	Если ПустаяСтрока(ОписаниеПричины.ТекстОшибки) Тогда
		ТекстСообщения = НСтр("es='La falta desconocida. Se dirijan en el servicio del apoyo técnico.';ru='Неизвестная ошибка. Обратитесь в службу техподдержки.';en='Unknown error. Contact technical support.'");
	КонецЕсли;
...
Функция ТекстТехническихПараметровЭДО(КонтекстВзаимодействия, Знач Сертификат = Неопределено) Экспорт
	...
	ТехническиеПараметры = НСтр("es='Los parámetros del participante del cambio ЭД: "
""
"- la huella del certificado: %1';ru='Параметры участника обмена ЭД:"
""
"- отпечаток сертификата: %1';en='Parameters of the participant sharing the ED:"
""
"- certificate fingerprint:%1'");
	...
	Если НЕ ПустаяСтрока(КодОшибки) Тогда
		...
		СтрДанныхОшибки = Символы.ПС + НСтр("es='- El código de la falta: %1, "
"- la descripción de la falta: %2';ru='- код ошибки: %1,"
"- описание ошибки: %2';en='- error code: %1,"
" - error description:%2'");
...
Процедура ВыполнитьСценарий(
	...
	Исключение
		...
		ПараметрыФормы.Вставить("ОписаниеОшибки",
			НСтр("es='La falta desconocida. Más detalladamente cm. En la revista del registro.';ru='Неизвестная ошибка. Подробнее см. в журнале регистрации.';en='Unknown error. See the log for more details.'"));
...
Процедура ОбработкаКомандСервиса(
	...
	Исключение
		...
		КонтекстОбработчика.ПолноеОписаниеОшибки = НСтр("es='Había una excepción no tratada:';ru='Произошло необрабатываемое исключение:';en='An unhandled exception occurred:'")
			+ " " + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	...
	Если КонтекстОбработчика.ПроизошлаОшибка Тогда
		...
		ИначеЕсли КонтекстОбработчика.ДействиеПриОшибкеДляКлиента = "ПоказатьСообщение"
			...
			ПоказатьПредупреждение(,
				КонтекстОбработчика.ПользовательскоеОписаниеОшибки,
				,
				НСтр("es='El apoyo de Internet de los usuarios';ru='Интернет-поддержка пользователей';en='Online user support'"));
			Возврат;
...
Процедура ОстановитьМеханизм(КонтекстВзаимодействия, СтруктураКоманды)
	...
	Если СокрЛП(Строка(КодОшибки)) <> "0" И КодОшибки <> Неопределено Тогда
		...
		ПараметрыФормы.Вставить("ПодробноеОписаниеОшибки",
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta del servicio (%1).';ru='Ошибка сервиса (%1).';en='Service error (%1).'"),
				Строка(КодОшибки)));
...
Процедура ОтобразитьСостояниеОбращениеКСервису()
	...
	Состояние(
		,
		,
		НСтр("es='La espera de la respuesta del servidor del Apoyo De Internet de los usuarios';ru='Ожидание ответа от сервера Интернет-поддержки пользователей';en='Waiting for a response from the Internet user support server'"),
		БиблиотекаКартинок.ИнтернетПоддержкаПользователей);
...
Функция ВопросПриВыходеАвторизованногоПользователя(КонтекстВзаимодействия)
	...
	ТекстВопроса = НСтр("es='La conexión del usuario con el login %1 al Apoyo De Internet "
"será cesada. Una nueva conexión tiene que de nuevo introducir el login y la consigna. "
"¿Continuar?';ru='Подключение пользователя с логином %1 к Интернет-поддержке будет"
"прекращено. Для нового подключения нужно заново ввести логин и пароль."
"Продолжить?';en='Connecting a user with login%1 to Internet support will be"
"discontinued. For a new connection, you must re-enter the username and password."
"Continue?'");
