	...
	Если ПустаяСтрока(ТекущийПользовательИБ.Имя) Тогда
		...
		Исключение
			...
			Возврат КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(ИнформацияОбОшибке,
				НСтр("es='No consiguió establecer el parámetro de la sesión de TekuschiyPolzovatel por la causa: "
"""%1"". "
""
"Se dirijan al administrador.';ru='Не удалось установить параметр сеанса ТекущийПользователь по причине:"
"""%1""."
""
"Обратитесь к администратору.';en='Failed to set the ТекущийПользователь session parameter due to:"
"""%1""."
""
"Contact administrator.'"),
	...
	Если ПользовательПоИдентификаторуСуществует(
		...
		Если ПриНачалеРаботыСистемы И РолиАдминистратораДоступны() Тогда
			ИнтеграцияПодсистемБСП.ПриСозданииАдминистратора(НайденныйПользователь,
				НСтр("es='A la autorización al usuario son encontrados los papeles del administrador.';ru='При авторизации у пользователя найдены роли администратора.';en='During authorization, the user has found administrator roles.'"));
		КонецЕсли;
		Возврат РезультатУстановкиПараметровСеанса(РегистрироватьВЖурнале);
	КонецЕсли;
	...
	Если Не РолиАдминистратораДоступны() Тогда
		Возврат КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(
			НСтр("es='El lanzamiento en nombre del usuario con el derecho la Administración es imposible, "
"puesto que él no es registrado en la lista de los usuarios. "
""
"A la gestión de la lista y el ajuste de los derechos de los usuarios es destinada la lista los Usuarios, "
"no debe usar el régimen de la configuración 1С:Предприятия para esto.';ru='Запуск от имени пользователя с правом Администрирование невозможен,"
...
"режим конфигурирования 1С:Предприятия для этого использовать не следует.';en='Launching as a user with the right Administration is not possible,"
"since it is not registered in the user list."
""
"To maintain a list and configure user rights, use the Users list,"
"1C: Enterprise configuration mode should not be used for this. '"),
			, РегистрироватьВЖурнале);
	КонецЕсли;
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		Возврат КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(ИнформацияОбОшибке,
			НСтр("es='No consiguió cumplir el registro automático del administrador en la lista por la causa: "
"""%1"". "
""
"A la gestión de la lista y el ajuste de los derechos de los usuarios es destinada la lista los Usuarios, "
"no debe usar el régimen de la configuración 1С:Предприятия para esto.';ru='Не удалось выполнить автоматическую регистрацию администратора в списке по причине:"
"""%1""."
""
"Для ведения списка и настройки прав пользователей предназначен список Пользователи,"
"режим конфигурирования 1С:Предприятия для этого использовать не следует.';en='Failed to automatically register the administrator in the list because:"
"""%1""."
""
"To maintain a list and configure user rights, use the Users list,"
"1C: Enterprise configuration mode should not be used for this.'"),
	...
	Комментарий =
		НСтр("es='Es cumplido el lanzamiento en nombre del usuario con el papel ""los Plenos derechos"", "
"que no es registrado en la lista de los usuarios. "
"Es cumplido el registro automático en la lista de los usuarios. "
""
"A la gestión de la lista y el ajuste de los derechos de los usuarios es destinada la lista los Usuarios, "
"no debe usar el régimen de la configuración 1С:Предприятия para esto.';ru='Выполнен запуск от имени пользователя с ролью ""Полные права"","
"который не зарегистрирован в списке пользователей."
"Выполнена автоматическая регистрация в списке пользователей."
""
"Для ведения списка и настройки прав пользователей предназначен список Пользователи,"
"режим конфигурирования 1С:Предприятия для этого использовать не следует.';en='Run as user with the role"" Full rights """","
"which is not registered in the user list."
"Automatically registered in the user list."
""
"To maintain a list and configure user rights, use the Users list,"
"1C: Enterprise configuration mode should not be used for this.'");
	...
	ЗаписьЖурналаРегистрации(
		НСтр("es='Los usuarios. El administrador es registrado en la guía los Usuarios';ru='Пользователи.Администратор зарегистрирован в справочнике Пользователи';en='Users. The administrator is registered in the Users directory'",
...
Функция ОшибкаНедостаточноПравДляВходаВПрограмму(РегистрироватьВЖурнале = Истина) Экспорт
	...
	Возврат КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(
		НСтр("es='Es insuficientemente derecho para la entrada en el programa. "
""
"Se dirijan al administrador.';ru='Недостаточно прав для входа в программу."
""
"Обратитесь к администратору.';en='Not enough rights to enter the program."
""
"Contact administrator. '"),
		, РегистрироватьВЖурнале);
...
Процедура ОбработатьИнтерфейсРолей(Действие, Параметры) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta en el procedimiento ПользователиСлужебный.ОбработатьИнтерфейсРолей () "
"el significado Equivocado del parámetro la Acción: ""%1"".';ru='Ошибка в процедуре ПользователиСлужебный.ОбработатьИнтерфейсРолей()"
"Неверное значение параметра Действие: ""%1"".';en='Error in procedure ПользователиСлужебный.ОбработатьИнтерфейсРолей()  Invalid value for the Action parameter: ""%1"".'"),
			Действие);
	КонецЕсли;
...
Функция СвойстваНеуказанногоПользователя() Экспорт
	...
	Свойства.Вставить("ПолноеИмяДляПоиска", НСтр("es='<no es indicado>';ru='<Не указан>';en='<Не указан>'"));
...
Процедура ПроверитьНазначениеРолей(НазначениеРолей = Неопределено, ПроверитьВсе = Ложь, СписокОшибок = Неопределено) Экспорт
	...
	ЗаголовокОшибки =
		НСтр("es='La falta en el procedimiento de PriOpredeleniiNaznacheniyaRoley del módulo PolzovateliPereopredelyaemyy general.';ru='Ошибка в процедуре ПриОпределенииНазначенияРолей общего модуля ПользователиПереопределяемый.';en='Error in procedure ПриОпределенииНазначенияРолей of general module ПользователиПереопределяемый.'");
		...
		Для Каждого КлючИЗначение Из ОписаниеНазначенияРолей.Значение Цикл
			...
			Если Роль = Неопределено Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='En los metadatos no es encontrado papel ""%1"" "
"indicado en el destino %2.';ru='В метаданных не найдена роль ""%1"","
"указанная в назначении %2.';en=""Role '% 1' not found in metadata,"
"specified in destination% 2."""),
			...
			Для Каждого ОписаниеНазначения Из Назначение Цикл
				...
				КонецЕсли;
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El papel ""%1"" es indicado más de en un destino: "
"%2, %3.';ru='Роль ""%1"" указана более чем в одном назначении:"
"%2, %3.';en='The role ""%1"" ""is indicated in more than one assignment:"
"%2,%3. '"),
	...
	ПроверитьСоставПравРолей(НедоступныеПрава, Назначение.ТолькоДляВнешнихПользователей, ТекстОшибки,
		НСтр("es='Durante la comprobación de los papeles solamente para los usuarios exteriores son encontradas las faltas:';ru='При проверке ролей только для внешних пользователей найдены ошибки:';en='Errors found while checking roles only for external users:'"), СписокОшибок);
	
	ПроверитьСоставПравРолей(НедоступныеПрава, Назначение.СовместноДляПользователейИВнешнихПользователей, ТекстОшибки,
		НСтр("es='Durante la comprobación de los papeles juntamente para los usuarios y los usuarios exteriores son encontradas las faltas:';ru='При проверке ролей совместно для пользователей и внешних пользователей найдены ошибки:';en='Errors were found when checking roles together for users and external users:'"), СписокОшибок);
	...
	Если ОбщегоНазначения.РазделениеВключено() Или ПроверитьВсе Тогда
		...
		ПроверитьСоставПравРолей(НедоступныеПрава, Роли, ТекстОшибки,
			НСтр("es='Durante la comprobación de los papeles para los usuarios de la aplicación son encontradas las faltas:';ru='При проверке ролей для пользователей приложения найдены ошибки:';en='Errors were found while checking roles for application users:'"), СписокОшибок, НеразделенныеДанные);
	КонецЕсли;
	Если Не ОбщегоНазначения.РазделениеВключено() Или ПроверитьВсе Тогда
		...
		ПроверитьСоставПравРолей(НедоступныеПрава, Роли, ТекстОшибки,
			НСтр("es='Durante la comprobación de los papeles para los usuarios son encontradas las faltas:';ru='При проверке ролей для пользователей найдены ошибки:';en='Errors found while checking roles for users:'"), СписокОшибок);
...
Процедура ПриЗаполненииВидовДоступа(ВидыДоступа) Экспорт
	...
	ВидДоступа.Представление          = НСтр("es='Los usuarios';ru='Пользователи';en='Users'");
	...
	ВидДоступа.Представление          = НСтр("es='Los usuarios exteriores';ru='Внешние пользователи';en='External users'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("es='Las noticias incorrectas sobre los usuarios';ru='Некорректные сведения о пользователях';en='Incorrect user information'");
...
Функция ВозможноИзменитьПароль(Пользователь, ДополнительныеПараметры = Неопределено) Экспорт
	   ...
	   И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса.ПользователиВМоделиСервиса") Тогда
		...
		Если Не ДействияСПользователемСервиса.ИзменениеПароля Тогда
			ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El servicio: es insuficientemente derecho para el cambio de la consigna del usuario ""%1"".';ru='Сервис: Недостаточно прав для изменения пароля пользователя ""%1"".';en='Service: Insufficient permissions to change user password ""%1"".'"), Пользователь));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если ДополнительныеПараметры.Свойство("ПроверятьДействительностьПользователя")
	   И РеквизитыПользователя.Недействителен <> Ложь Тогда
		...
		ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El usuario ""%1"" es no válido.';ru='Пользователь ""%1"" недействителен.';en=""User '%1' is invalid."""), Пользователь));
		Возврат Ложь;
	КонецЕсли;
	...
	Если ДополнительныеПараметры.Свойство("ПроверятьНаличиеПользователяИБ")
	   И ПользовательИБ = Неопределено Тогда
		...
		ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='""%1"".';ru='Не найдена учетная запись пользователя ""%1"".';en='User account ""%1"" could not be found.'"), Пользователь));
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не ДополнительныеПараметры.ЭтоТекущийПользовательИБ
	   И Не УровеньДоступа.НастройкиДляВхода Тогда
		...
		ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es insuficientemente derecho para el cambio de la consigna del usuario ""%1"".';ru='Недостаточно прав для изменения пароля пользователя ""%1"".';en='Not enough rights to change user password ""%1"".'"), Пользователь));
		Возврат Ложь;
	КонецЕсли;
	...
	Если ПользовательИБ <> Неопределено И ПользовательИБ.ЗапрещеноИзменятьПароль Тогда
		Если УровеньДоступа.НастройкиДляВхода Тогда
			Если ДополнительныеПараметры.Свойство("УчитыватьСвойствоЗапрещеноИзменятьПароль") Тогда
				ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Es establecida la prohibición del cambio de la consigna del usuario ""%1"".';ru='Установлен запрет изменения пароля пользователя ""%1"".';en='The prohibition of changing the password for user ""%1"" is set.'"), Пользователь));
				Возврат Ложь;
			КонецЕсли;
		Иначе
			ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es establecida la prohibición del cambio de la consigna del usuario ""%1"". "
"Se dirijan al administrador.';ru='Установлен запрет изменения пароля пользователя ""%1""."
"Обратитесь к администратору.';en='The prohibition of changing the password of the user ""%1"" ""is set.'"), Пользователь));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	ДополнительныеПараметры.Вставить("ТекстОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Se podrá cambiar la consigna solamente a través de %1.';ru='Пароль можно будет сменить только через %1.';en='The password can only be changed after %1.'"), ЧислоИПредмет));
...
Функция ОбработатьНовыйПароль(Параметры) Экспорт
	...
	Если ДополнительныеПараметры.ЭтоТекущийПользовательИБ
	   ...
	   И (ВызовИзФормыСменаПароля Или СтарыйПароль <> Неопределено) Тогда
		...
		Если Не Параметры.СтарыйПарольСовпадает Тогда
			Возврат НСтр("es='La consigna vieja es indicada incorrectamente.';ru='Старый пароль указан неверно.';en='The old password is incorrect.'");
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		...
		Если ПользователиСлужебныйПовтИсп.Настройки().ОбщиеНастройкиВхода
		   ...
		   И НовыйПарольИспользовалсяРанее(ИспользованныеПароли, СохраняемоеЗначениеПароля) Тогда
			...
			ТекстОшибки = НСтр("es='Una nueva consigna se usaba antes.';ru='Новый пароль использовался ранее.';en='The new password was used earlier.'");
	...
	Исключение
		...
		Если ВызовИзФормыСменаПароля Тогда
			ЗаписьЖурналаРегистрации(
				НСтр("es='Los usuarios. La falta del cambio de la consigna';ru='Пользователи.Ошибка смены пароля';en='Users. Password reset error'",
				     ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.НайтиПоТипу(ТипЗнч(Пользователь)),
				Пользователь,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No consiguió cambiar la consigna del usuario ""%1"" por la causa: "
"%2';ru='Не удалось сменить пароль пользователя ""%1"" по причине:"
"%2';en='Failed to change user password"" ""%1"" due to:"
"%2 '"),
...
Функция ПодсказкаДляНовогоПароля() Экспорт
	...
	Возврат
		НСтр("es='La consigna segura: "
"- tiene no menos 7 símbolos; "
"- contiene cualesquiera 3 de 4 tipos de los símbolos:"
" las letras mayúsculas, las letras minúsculas, la cifra, los símbolos especiales; "
"- no coincide con el nombre (para la entrada).';ru='Надежный пароль:"
"- имеет не менее 7 символов;"
"- содержит любые 3 из 4-х типов символов: заглавные"
"  буквы, строчные буквы, цифры, специальные символы;"
"- не совпадает с именем (для входа).';en='Strong password:"
"- has at least 7 characters;"
"- contains any 3 of 4 types of characters: uppercase"
"   letters, lowercase letters, numbers, special characters;"
"- does not match the name (for login).'");
...
Функция ПодсказкаПотребоватьСменуПароляПриВходе(ДляВнешнихПользователей) Экспорт
	...
	Если Не ЭтоПолноправныйПользователь Тогда
		Подсказка = НСтр("es='Las exigencias a la longitud y la complicación de la consigna son dadas separadamente. "
"Detrás de los detalles se dirijan al administrador.';ru='Требования к длине и сложности пароля задаются отдельно."
"За подробностями обратитесь к администратору.';en='Requirements for the length and complexity of the password are set separately."
"Contact your administrator for details.'");
	...
	Если Не ПользователиСлужебныйПовтИсп.Настройки().ОбщиеНастройкиВхода Тогда
		Подсказка = НСтр("es='Las exigencias a la longitud y la complicación de la consigna son dadas separadamente. "
"Véase los Parámetros de la base informativa en конфигураторе en el menú la Administración.';ru='Требования к длине и сложности пароля задаются отдельно."
"См. Параметры информационной базы в конфигураторе в меню Администрирование.';en='Requirements for the length and complexity of the password are set separately."
"See Infobase parameters in the configurator in the Administration menu.'");
		Возврат Новый ФорматированнаяСтрока(Подсказка);
	КонецЕсли;
	...
	Если ДляВнешнихПользователей Тогда
		Если ЕстьРазделАдминистрирование Тогда
			Подсказка = НСтр("es='Las exigencias a la longitud y la complicación de la consigna son dadas separadamente. "
"Véase <a href = ""NastroykiVhodaPolzovateley""> los Ajustes de la entrada de los usuarios exteriores </a> en la sección la Administración, "
"el punto del Ajuste de los usuarios y los derechos.';ru='Требования к длине и сложности пароля задаются отдельно."
"См. <a href = ""НастройкиВходаПользователей"">Настройки входа внешних пользователей</a> в разделе Администрирование,"
"пункт Настройки пользователей и прав.';en='Requirements for the length and complexity of the password are set separately."
"See <a href =                                ""НастройкиВходаПользователей"" > Login settings for external users </a> in the Administration section,"
"item User settings and rights.'");
		Иначе
			Подсказка = НСтр("es='Las exigencias a la longitud y la complicación de la consigna son dadas separadamente. "
"Véase <a href = ""NastroykiVhodaPolzovateley""> los Ajustes de la entrada de los usuarios exteriores </a>.';ru='Требования к длине и сложности пароля задаются отдельно."
"См. <a href = ""НастройкиВходаПользователей"">Настройки входа внешних пользователей</a>.';en='Requirements for the length and complexity of the password are set separately."
"See <a href =                                ""НастройкиВходаПользователей"" Login settings for external users </a>'");
		КонецЕсли;
	Иначе
		Если ЕстьРазделАдминистрирование Тогда
			Подсказка = НСтр("es='Las exigencias a la longitud y la complicación de la consigna son dadas separadamente. "
"Véase <a href = ""NastroykiVhodaPolzovateley""> los Ajustes de la entrada de los usuarios </a> en la sección la Administración, "
"el punto del Ajuste de los usuarios y los derechos.';ru='Требования к длине и сложности пароля задаются отдельно."
"См. <a href = ""НастройкиВходаПользователей"">Настройки входа пользователей</a> в разделе Администрирование,"
"пункт Настройки пользователей и прав.';en='Requirements for the length and complexity of the password are set separately."
"See <a href =                                ""НастройкиВходаПользователей"" > User Login Settings </a> in the Administration section,"
"item User settings and rights.'");
		Иначе
			Подсказка = НСтр("es='Las exigencias a la longitud y la complicación de la consigna son dadas separadamente. "
"Véase <a href = ""NastroykiVhodaPolzovateley""> los Ajustes de la entrada de los usuarios </a>.';ru='Требования к длине и сложности пароля задаются отдельно."
"См. <a href = ""НастройкиВходаПользователей"">Настройки входа пользователей</a>.';en='Requirements for the length and complexity of the password are set separately."
"See <a href =                                ""НастройкиВходаПользователей"" > User Login Settings </a>'");
		КонецЕсли;
	КонецЕсли;
...
Процедура НачатьОбработкуПользователяИБ(ПользовательОбъект,
	...
	ПараметрыОбработки.Вставить("ТекстСообщенияНедостаточноПрав",
		НСтр("es='Es insuficientemente derecho para el cambio del usuario de la base informativa.';ru='Недостаточно прав для изменения пользователя информационной базы.';en='Not enough rights to change the user of the infobase.'"));
	...
	Если НЕ ОписаниеПользователяИБ.Свойство("Действие") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la anotación del usuario ""%1"". "
"En el parámetro de OpisaniePolzovatelyaIB no es indicada la propiedad la Acción.';ru='Ошибка при записи пользователя ""%1""."
"В параметре ОписаниеПользователяИБ не указано свойство Действие.';en='Error while writing user ""%1""."
"In the parameter ОписаниеПользователяИБ is not set to the Action property.'"),
	...
	Если ОписаниеПользователяИБ.Действие <> "Записать"
	   И ОписаниеПользователяИБ.Действие <> "Удалить" Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la anotación del usuario ""%1"". "
"En el parámetro de OpisaniePolzovatelyaIB es indicado "
"el significado equivocado de ""%2"" propiedades la Acción.';ru='Ошибка при записи пользователя ""%1""."
"В параметре ОписаниеПользователяИБ указано"
"неверное значение ""%2"" свойства Действие.';en='Error while writing user ""%1""."
"In the parameter ОписаниеПользователяИБ indicated"
"invalid value for ""%2"" of the Action property.'"),
			ПользовательОбъект.Ссылка,
	...
	Если ОписаниеПользователяИБ.Действие = "Записать"
	   ...
	   И ОписаниеПользователяИБ.УникальныйИдентификатор
		...
		Если ПараметрыОбработки.СтарыйПользовательИБСуществует Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta a la anotación del usuario ""%1"". "
"No es posible confrontar al usuario ИБ con el usuario en la guía, "
"con que es confrontado ya por otro usuario ИБ.';ru='Ошибка при записи пользователя ""%1""."
"Нельзя сопоставить пользователя ИБ с пользователем в справочнике,"
"с которым уже сопоставлен другой пользователем ИБ.';en='Error while writing user ""%1""."
"You cannot map the IB user to the user in the directory,"
"with which another IB user is already mapped. '"),
		...
		Если ПользовательПоИдентификаторуСуществует(
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta a la anotación del usuario ""%1"". "
"No es posible confrontar al usuario ИБ con este usuario en la guía, "
"puesto que él es confrontado ya con otro usuario en la guía "
"""%2"".';ru='Ошибка при записи пользователя ""%1""."
...
"""%2"".';en='Error while writing user ""%1""."
"You cannot map the IB user to this user in the directory,"
"since it is already mapped to another user in the directory"
"""%2"".'"),
	   ...
	   И ПользователиИнформационнойБазы.ПолучитьПользователей().Количество() = 0 Тогда
		...
		Если ТипЗнч(ОписаниеПользователяИБ) = Тип("Структура") Тогда
			...
			Если ЗапретРедактированияРолей()
				...
				Текст =
					НСтр("es='En la lista de los usuarios del programa es añadido el primer usuario, por eso a ello "
"serán fijados automáticamente los papeles ""el Administrador del sistema"" y ""los Plenos derechos"". "
"¿Continuar?';ru='В список пользователей программы добавляется первый пользователь, поэтому ему"
"автоматически будут назначены роли ""Администратор системы"" и ""Полные права""."
"Продолжить?';en='The first user is added to the list of program users, so he"
"The roles System Administrator"" and ""Full Rights"" will be automatically assigned.'");
		...
		Иначе
			...
			Текст = НСтр("es='El primer usuario de la base informativa debe tener los plenos derechos. "
"El usuario exterior no puede ser con todos los derechos. "
"Creen primero al administrador en la guía los Usuarios.';ru='Первый пользователь информационной базы должен иметь полные права."
"Внешний пользователь не может быть полноправным."
"Сначала создайте администратора в справочнике Пользователи.';en='The first user of the infobase must have full rights."
"An external user cannot be full."
"First, create an administrator in the Users directory.'");
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Если ОписаниеПользователяИБ.Свойство("Имя") Тогда
		...
		Если ПустаяСтрока(Имя) Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("es='el Nombre (para la entrada).';ru='Не заполнено Имя (для входа).';en='Not filled Name (for login).'"),
				,
				"Имя",
				,
				Отказ);
		...
		ИначеЕсли СтрДлина(Имя) > 64 Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("es='El nombre (para la entrada) supera 64 símbolos.';ru='Имя (для входа) превышает 64 символа.';en='The name (for login) exceeds 64 characters.'"),
				,
				"Имя",
				,
				Отказ);
		...
		ИначеЕсли СтрНайти(Имя, ":") > 0 Тогда
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("es='El nombre (para la entrada) contiene el símbolo prohibido "":"".';ru='Имя (для входа) содержит запрещенный символ "":"".';en='The name (for login) contains the forbidden character "":"".'"),
				,
				"Имя",
				,
				Отказ);
		...
		Иначе
			   ...
			   И ПользовательИБ.УникальныйИдентификатор
				...
				Если НайденныйПользователь = Неопределено
				 ИЛИ НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
					...
					ТекстОшибки = НСтр("es='El nombre (para la entrada) es ocupado ya.';ru='Имя (для входа) уже занято.';en='The name (for login) is already taken.'");
				Иначе
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El nombre (para la entrada) es ocupado ya para el usuario ""%1"".';ru='Имя (для входа) уже занято для пользователя ""%1"".';en='The name (for login) is already taken for user ""%1"".'"),
			...
			ИначеЕсли Не ОписаниеПользователяИБ.Свойство("Пароль")
				...
				Если ЗначениеЗаполнено(НастройкиВхода.МинимальнаяДлинаПароля) Тогда
					...
					Если ПользовательИБ = Неопределено Тогда
						ОбщегоНазначения.СообщитьПользователю(
							НСтр("es='Es necesario establecer la consigna.';ru='Требуется установить пароль.';en='Password required.'"),, "СменитьПароль",, Отказ);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	...
	Если ОписаниеПользователяИБ.Свойство("ПользовательОС") Тогда
		...
		Если Не ПустаяСтрока(ОписаниеПользователяИБ.ПользовательОС)
		   И Не СтандартныеПодсистемыСервер.ЭтоУчебнаяПлатформа() Тогда
			...
			Исключение
				ОбщегоНазначения.СообщитьПользователю(
					НСтр("es='El usuario de la SO debe ser en el formato "
"""\\ImyaDomena\ImyaPolzovatelya"".';ru='Пользователь ОС должен быть в формате"
"""\\ИмяДомена\ИмяПользователя"".';en='OS user must be in format ""\\ИмяДомена\ИмяПользователя"".'"),
					,
					"ПользовательОС",
					,
					Отказ);
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='A la renovación de los papeles del usuario exterior "
"""%1"" "
"papel de ""%2"" "
"grupos de los usuarios exteriores ""%3"" no es encontrado en "
"los metadatos.';ru='При обновлении ролей внешнего пользователя"
"""%1"""
"роль ""%2"""
"группы внешних пользователей ""%3"""
"не найдена в метаданных.';en='When updating external user roles"
"""%1"""
"role ""%2"""
"external user groups ""%3"""
"not found in metadata. '"),
			...
			ЗаписьЖурналаРегистрации(
				НСтр("es='Los usuarios. El papel no es encontrado en los metadatos';ru='Пользователи.Роль не найдена в метаданных';en='Users. Role not found in metadata'",
				     ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
...
Функция ОбъектАвторизацииИспользуется(Знач СсылкаНаОбъектАвторизации,
	...
	Если Результат Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Hay ya usuario exterior vinculado al objeto ""%1"".';ru='Уже существует внешний пользователь, связанный с объектом ""%1"".';en=""An external user already exists associated with object '%1'"""),
...
Функция ПеремещениеПользователяВНовуюГруппу(МассивПользователей, ГруппаИсточник,
	...
	Если МассивПеремещенныхПользователей.Количество() = 0 И МассивНеПеремещенныхПользователей.Количество() = 0 Тогда
		Если МассивПользователей.Количество() = 1 Тогда
			ТекстСообщения = НСтр("es='El usuario ""%1"" es incluido ya en grupo ""%2"".';ru='Пользователь ""%1"" уже включен в группу ""%2"".';en='User ""%1"" is already included in group ""%2"".'");
			ИмяПеремещаемогоПользователя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(МассивПользователей[0], "Наименование");
		Иначе
			ТекстСообщения = НСтр("es='Todos los usuarios escogidos son incluidos ya en grupo ""%2"".';ru='Все выбранные пользователи уже включены в группу ""%2"".';en='All selected users are already included in group ""% 2"".'");
		...
		Если КоличествоНеПеремещенныхПользователей = 1 Тогда
			...
			Если ТипПользователяСовпадаетСГруппой Тогда
				СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El usuario ""%1"" no puede ser incluido en grupo ""%2"", "
"porque cerca del grupo hay un indicio ""los Todos usuarios del tipo dado"".';ru='Пользователь ""%1"" не может быть включен в группу ""%2"","
"т.к. у группы стоит признак ""Все пользователи заданного типа"".';en='User ""%1"" cannot be included in group ""%2"","
"because the group has the sign ""All users of the specified type"".'"),
					Предмет, НаименованиеГруппы) + Символы.ПС;
			Иначе
				СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El usuario ""%1"" no puede ser incluido en grupo ""%2"", "
"porque en la composición de sus participantes entran solamente %3.';ru='Пользователь ""%1"" не может быть включен в группу ""%2"","
"т.к. в состав ее участников входят только %3.';en='User ""%1"" cannot be included in group ""%2"","
"because only%3 are members. '"),
		...
		Иначе
			...
			СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No todos los usuarios pueden ser incluidos en grupo ""%1"", "
"porque en la composición de sus participantes entran solamente %2 "
"o cerca del grupo hay un indicio ""los Todos usuarios del tipo dado"".';ru='Не все пользователи могут быть включены в группу ""%1"","
"т.к. в состав ее участников входят только %2"
"или у группы стоит признак ""Все пользователи заданного типа"".';en='Not all users can be included in group ""%1"" "",because its members include only%2"
"or the group has the sign """" All users of the specified type """"'"),
				НаименованиеГруппы,
	...
	Если КоличествоПользователей = 1 Тогда
		...
		Если ГруппаПриемник = Справочники.ГруппыПользователей.ВсеПользователи
		 Или ГруппаПриемник = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
			...
			СтрокаДействие = НСтр("es='Es excluido del grupo';ru='исключен из группы';en='excluded from the group'");
			НаименованиеГруппы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ГруппаИсточник, "Наименование");
			
		ИначеЕсли Перемещение Тогда
			СтрокаДействие = НСтр("es='Es trasladado al grupo';ru='перемещен в группу';en='moved to group'");
		Иначе
			СтрокаДействие = НСтр("es='Es incluido en el grupo';ru='включен в группу';en='included in the group'");
		КонецЕсли;
		
		СообщениеПользователю = НСтр("es='""%1"" %2 ""%3""';ru='""%1"" %2 ""%3""';en='""%1"" %2 ""%3""'");
	...
	ИначеЕсли КоличествоПользователей > 1 Тогда
		...
		Если ГруппаПриемник = Справочники.ГруппыПользователей.ВсеПользователи Тогда
			СтрокаДействие = НСтр("es='Son excluidos del grupo';ru='исключены из группы';en='excluded from the group'");
			НаименованиеГруппы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ГруппаИсточник, "Наименование");
			
		ИначеЕсли Перемещение Тогда
			СтрокаДействие = НСтр("es='Son trasladados al grupo';ru='перемещены в группу';en='moved to group'");
		Иначе
			СтрокаДействие = НСтр("es='Son incluidos en el grupo';ru='включены в группу';en='included in the group'");
		КонецЕсли;
		СообщениеПользователю = НСтр("es='%1 %2 ""%3""';ru='%1 %2 ""%3""';en='% 1% 2 ""% 3""'");
	КонецЕсли;
...
Процедура ЗаполнитьНаименованиеПредопределенныхГруппПользователей() Экспорт
	...
	ГруппаПользователейОбъект.Наименование = НСтр("es='Todos los usuarios';ru='Все пользователи';en='All users'");
	...
	ГруппыВнешнихПользователейОбъект.Наименование = НСтр("es='Todos los usuarios exteriores';ru='Все внешние пользователи';en='All external users'");
...
Процедура ЗаполнитьСпискиНастроек(Параметры, АдресХранилища) Экспорт
	...
	Если Параметры.ПользовательИнформационнойБазы <> ИмяПользователя()
	   И Не ПравоДоступа("АдминистрированиеДанных", Метаданные) Тогда
		...
		ВызватьИсключение НСтр("es='Es insuficientemente derecho para la recepción de los ajustes del usuario.';ru='Недостаточно прав для получения настроек пользователя.';en='Not enough rights to get user settings.'");
	КонецЕсли;
...
Функция ШаблонТекстаОшибкиЗаписиСведенийОТекущемПользователе()
	...
	Возврат
		НСтр("es='No consiguió anotar las noticias sobre el usuario corriente por la causa: "
"%1 "
""
"se Dirijan al administrador.';ru='Не удалось записать сведения о текущем пользователе по причине:"
"%1"
""
"Обратитесь к администратору.';en='Failed to write current user information due to:"
"%one"
""
"Contact administrator. '");
...
Функция ЗаголовокСообщенияАвторизацияНеВыполненаСПереводомСтроки()
	...
	Возврат НСтр("es='La autorización no es cumplida. El trabajo del sistema será acabado.';ru='Авторизация не выполнена. Работа системы будет завершена.';en='Authorization failed. The system will be completed.'")
...
Функция ЗначенияПараметровСеансаТекущийПользователь()
	...
	ЗаголовокОшибки = НСтр("es='No consiguió establecer el parámetro de la sesión de TekuschiyPolzovatel.';ru='Не удалось установить параметр сеанса ТекущийПользователь.';en='Failed to set the ТекущийПользователь session parameter.'") + Символы.ПС;
	...
	Если СведенияОПользователе.ТекущийПользователь        = Неопределено
	 Или СведенияОПользователе.ТекущийВнешнийПользователь = Неопределено Тогда
		...
		Возврат ЗаголовокОшибки + ТекстСообщенияПользовательНеНайденВСправочнике(
			...
			+ НСтр("es='Ha surgido la falta interior durante la búsqueda del usuario.';ru='Возникла внутренняя ошибка при поиске пользователя.';en='An internal error occurred while searching for the user.'");
	КонецЕсли;
...
Функция ТекстСообщенияТекущийПользовательНедоступенВСеансеБезРазделителей()
	...
	Возврат
		НСтр("es='La recepción inadmisible del parámetro de la sesión de TekuschiyPolzovatel "
"en la sesión sin instrucción de todos разделителей.';ru='Недопустимое получение параметра сеанса ТекущийПользователь"
"в сеансе без указания всех разделителей.';en='Invalid retrieving session parameter CurrentUser"
"in a session without specifying all delimiters. '");
...
Функция НайтиТекущегоПользователяВСправочнике()
	...
	Если Выборка.Следующий() Тогда
		...
		Если Не ВнешниеПользователи.ИспользоватьВнешнихПользователей() Тогда
			Возврат НСтр("es='Los usuarios exteriores son desconectados.';ru='Внешние пользователи отключены.';en='External users are disconnected.'");
		КонецЕсли;
...
Функция ТекстСообщенияПользовательНеНайденВСправочнике(ИмяПользователя)
	...
	Если ВнешниеПользователи.ИспользоватьВнешнихПользователей() Тогда
		ШаблонСообщенияОбОшибке =
			НСтр("es='El usuario ""%1"" no es encontrado en las guías "
"""los Usuarios"" y ""los usuarios Exteriores"". "
""
"Se dirijan al administrador.';ru='Пользователь ""%1"" не найден в справочниках"
"""Пользователи"" и ""Внешние пользователи""."
""
"Обратитесь к администратору.';en='User"" ""%1"" not found in directories"
"""Users"" and ""External users""."
""
"Contact administrator. '");
	Иначе
		ШаблонСообщенияОбОшибке =
			НСтр("es='El usuario ""%1"" no es encontrado en la guía ""los Usuarios"". "
""
"Se dirijan al administrador.';ru='Пользователь ""%1"" не найден в справочнике ""Пользователи""."
""
"Обратитесь к администратору.';en='User"" ""%1"" not found in directory"
"""Users""."
""
"Contact administrator. '");
	КонецЕсли;
...
Функция КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(ИнформацияОбОшибке, ШаблонОшибки = "", РегистрироватьВЖурнале = Истина)
	...
	Если РегистрироватьВЖурнале Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("es='Los usuarios. La falta de la entrada en el programa';ru='Пользователи.Ошибка входа в программу';en='Users. Login error'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПроверитьПраваПользователя(ПользовательИБ, РежимПроверки, ЭтоВнешнийПользователь)
	...
	Для Каждого ОписаниеРоли Из РолиДляПроверки Цикл
		...
		Если НазначениеРолей.ТолькоДляАдминистраторовСистемы.Получить(ИмяРоли) <> Неопределено Тогда
			ТекстШаблона = НСтр("es='""%1"" (es destinada solamente a los administradores del sistema)';ru='""%1"" (предназначена только для администраторов системы)';en='""% 1"" (intended for system administrators only)'");
		
		ИначеЕсли РазделениеВключено
		        И НазначениеРолей.ТолькоДляПользователейСистемы.Получить(ИмяРоли) <> Неопределено Тогда
			
			ТекстШаблона = НСтр("es='""%1"" (es destinada solamente a los usuarios del sistema)';ru='""%1"" (предназначена только для пользователей системы)';en='""% 1"" (intended for users only)'");
			
		ИначеЕсли НазначениеРолей.ТолькоДляВнешнихПользователей.Получить(ИмяРоли) <> Неопределено Тогда
			ТекстШаблона = НСтр("es='""%1"" (es destinada solamente a los usuarios exteriores)';ru='""%1"" (предназначена только для внешних пользователей)';en='""% 1"" (intended for external users only)'");
			
		Иначе // Это внешний пользователь.
			ТекстШаблона = НСтр("es='""%1"" (es destinada solamente a los usuarios)';ru='""%1"" (предназначена только для пользователей)';en='""% 1"" (intended for users only)'");
		КонецЕсли;
	...
	ИмяСобытия = НСтр("es='Los usuarios. La falta a la instalación de los papeles al usuario ИБ';ru='Пользователи.Ошибка при установке ролей пользователю ИБ';en='Users.Error in setting roles for IB user'",
	     ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если РежимПроверки = "ПриЗапуске" Тогда
		Если СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) = 1 Тогда
			ТекстРегистрацииВхода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La tentativa de la entrada del usuario %1 con el papel inaccesible: "
"%2.';ru='Попытка входа пользователя %1 с недоступной ролью:"
"%2.';en='Attempt was made to log on user%1 with an unavailable role:"
"%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
		Иначе
			ТекстРегистрацииВхода = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La tentativa de la entrada del usuario %1 con los papeles inaccesibles: "
"%2.';ru='Попытка входа пользователя %1 с недоступными ролями:"
"%2.';en='Attempting to log in user%1 with unavailable roles:"
"%2. '"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
		КонецЕсли;
		...
		ТекстСообщенияВхода =
			НСтр("es='Es imposible cumplir la entrada por la presencia de los papeles inaccesibles. "
"Se dirijan al administrador.';ru='Невозможно выполнить вход из-за наличия недоступных ролей."
"Обратитесь к администратору.';en='Unable to sign in due to inaccessible roles."
"Contact administrator.'");
	...
	Если СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) = 1 И ЗначениеЗаполнено(ДобавляемыеНедоступныеРоли) Тогда
		ТекстРегистрацииДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La tentativa de fijar al usuario %1 papel inaccesible: "
"%2.';ru='Попытка назначить пользователю %1 недоступную роль:"
"%2.';en='Attempt to assign user%1 inaccessible role:"
"%2. '"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
			
	ИначеЕсли СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) > 1 Тогда
		ТекстРегистрацииДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La tentativa de fijar al usuario %1 papeles inaccesibles: "
"%2.';ru='Попытка назначить пользователю %1 недоступные роли:"
"%2.';en='Attempt to assign user%1 inaccessible roles:"
"%2. '"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
	Иначе
	...
	Если СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) = 1 И ЗначениеЗаполнено(ДобавляемыеНедоступныеРоли) Тогда
		ТекстСообщенияДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Al usuario ""%1"" es imposible fijar el papel inaccesible: "
"%2.';ru='Пользователю ""%1"" невозможно назначить недоступную роль:"
"%2.';en='User ""%1"" can not be assigned an inaccessible role:"
"%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
		
	ИначеЕсли СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) > 1 Тогда
		ТекстСообщенияДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Al usuario ""%1"" es imposible fijar los papeles inaccesibles: "
"%2.';ru='Пользователю ""%1"" невозможно назначить недоступные роли:"
"%2.';en='User ""%1"" can not be assigned an inaccessible roles:"
"%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
	Иначе
	...
	Для Каждого ОписаниеРоли Из ОписаниеРолей Цикл
		...
		Для Каждого НедоступноеПраво Из НедоступныеПрава Цикл
			Если ПравоДоступа(НедоступноеПраво, Метаданные, Роль) Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El papel ""%1"" contiene el derecho inaccesible %2.';ru='Роль ""%1"" содержит недоступное право %2.';en=""The role '%1' contains unavailable privilege%2."""),
		...
		Для Каждого СвойстваДанных Из НеразделенныеДанные Цикл
			...
			Если ПравоДоступа("Изменение", ОбъектМетаданных, Роль) Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El papel ""%1"" contiene el derecho el Cambio del objeto no dividido %2.';ru='Роль ""%1"" содержит право Изменение неразделенного объекта %2.';en='The role ""%1"" contains the right to Change undivided object%2.'"),
			...
			Если ПравоДоступа("Добавление", ОбъектМетаданных, Роль) Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El papel ""%1"" contiene el derecho la Adición del objeto no dividido %2.';ru='Роль ""%1"" содержит право Добавление неразделенного объекта %2.';en='The role ""%1"" contains the right to Add undivided object%2.'"),
					Роль, ОбъектМетаданных.ПолноеИмя());
				Если СписокОшибок = Неопределено Тогда
					ТекстОшибки = ТекстОшибки + Символы.ПС + ОписаниеОшибки;
				Иначе
			...
			Если ПравоДоступа("Удаление", ОбъектМетаданных, Роль) Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El papel ""%1"" contiene el derecho la Desaparición del objeto no dividido %2.';ru='Роль ""%1"" содержит право Удаление неразделенного объекта %2.';en='The role ""%1"" contains the right to Delete undivided object%2.'"),
					Роль, ОбъектМетаданных.ПолноеИмя());
				Если СписокОшибок = Неопределено Тогда
					ТекстОшибки = ТекстОшибки + Символы.ПС + ОписаниеОшибки;
				Иначе
...
Функция ТребуетсяСменитьПароль(ОписаниеОшибки = "", ПриЗапуске = Ложь, РегистрироватьВЖурнале = Истина)
	...
	Исключение
		...
		Если ПриЗапуске Тогда
			...
			Если РегистрироватьВЖурнале Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("es='Los usuarios. La falta de la entrada en el programa';ru='Пользователи.Ошибка входа в программу';en='Users. Login error'",
					     ОбщегоНазначения.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
		...
		Иначе
			Если РегистрироватьВЖурнале Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("es='Los usuarios. La falta de la renovación de la fecha de la última actividad';ru='Пользователи.Ошибка обновления даты последней активности';en='Users. Error updating the date of the last activity'",
					     ОбщегоНазначения.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					Метаданные.НайтиПоТипу(ТипЗнч(ТекущийПользователь)),
					ТекущийПользователь,
...
Функция ОшибкаДлиныИлиСложностиПароля(Пароль, Знач МинимальнаяДлинаПароля, СложныйПароль) Экспорт
	...
	Если СтрДлина(Пароль) < МинимальнаяДлинаПароля Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La cantidad de los símbolos de la nueva consigna debe ser no menos %1.';ru='Количество символов нового пароля должно быть не менее %1.';en='The number of characters for the new password must be at least %1.'"),
			Формат(МинимальнаяДлинаПароля, "ЧГ="));
	КонецЕсли;
	
	Если СложныйПароль И Не ПарольОтвечаетТребованиямСложности(Пароль) Тогда
		Возврат НСтр("es='La consigna no corresponde a las exigencias de la complicación.';ru='Пароль не отвечает требованиям сложности.';en='Password does not meet complexity requirements.'")
			+ Символы.ПС + Символы.ПС
...
Процедура ПроверитьИзмененияРеквизитовПользователя(ПользовательОбъект, ПараметрыОбработки)
	...
	Если ТипЗнч(ПользовательОбъект) = Тип("СправочникОбъект.Пользователи")
	   И БлокируемыеРеквизиты.Служебный <> ПользовательОбъект.Служебный Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la anotación del usuario ""%1"". "
"El requisito De servicio no se permite cambiar en подписках a los acontecimientos.';ru='Ошибка при записи пользователя ""%1""."
"Реквизит Служебный не допускается изменять в подписках на события.';en='Error while writing user ""%1""."
"The service props are not allowed to be changed in event subscriptions.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	...
	Если БлокируемыеРеквизиты.Подготовлен <> ПользовательОбъект.Подготовлен Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la anotación del usuario ""%1"". "
"El requisito Es preparado no se permite cambiar en подписках a los acontecimientos.';ru='Ошибка при записи пользователя ""%1""."
"Реквизит Подготовлен не допускается изменять в подписках на события.';en='Error while writing user ""%1""."
"Requisite Prepared is not allowed to change in event subscriptions. '"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	...
	Если АвтоРеквизиты.ИдентификаторПользователяИБ <> ПользовательОбъект.ИдентификаторПользователяИБ Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la anotación del usuario ""%1"". "
"El requisito de IdentifikatorPolzovatelyaIB no se permite cambiar. "
"La renovación del requisito se cumple automáticamente.';ru='Ошибка при записи пользователя ""%1""."
"Реквизит ИдентификаторПользователяИБ не допускается изменять."
"Обновление реквизита выполняется автоматически.';en='Error while writing user ""%1""."
"ИдентификаторПользователяИБ is not allowed to change."
"Props are updated automatically. '"),
	...
	Если Не ОбщегоНазначения.ДанныеСовпадают(АвтоРеквизиты.СвойстваПользователяИБ,
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la anotación del usuario ""%1"". "
"El requisito de SvoystvaPolzovatelyaIB no se permite cambiar. "
"La renovación del requisito se cumple automáticamente.';ru='Ошибка при записи пользователя ""%1""."
"Реквизит СвойстваПользователяИБ не допускается изменять."
"Обновление реквизита выполняется автоматически.';en='Error while writing user ""%1""."
"СвойстваПользователяИБ are not allowed to be changed."
"Props are updated automatically.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	...
	Если СтарыйПользователь.ПометкаУдаления = Ложь
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la anotación del usuario ""%1"". "
"No es posible marcar a la desaparición del usuario, a que es permitida la entrada en el programa.';ru='Ошибка при записи пользователя ""%1""."
"Нельзя помечать на удаление пользователя, которому разрешен вход в программу.';en='Error while writing user ""%1""."
"You can not mark the removal of a user who is allowed to enter the program.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	...
	Если СтарыйПользователь.Недействителен = Ложь
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la anotación del usuario ""%1"". "
"No es posible marcar no válida al usuario, a que es permitida la entrada en el programa.';ru='Ошибка при записи пользователя ""%1""."
"Нельзя пометить недействительным пользователя, которому разрешен вход в программу.';en='Error while writing user ""%1""."
"You can not invalidate a user who is allowed to enter the program. '"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	...
	Если СтарыйПользователь.Подготовлен = Ложь
	   ...
	   И Пользователи.ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la anotación del usuario ""%1"". "
"No es posible marcar preparado al usuario, a que es permitida la entrada en el programa.';ru='Ошибка при записи пользователя ""%1""."
"Нельзя пометить подготовленным пользователя, которому разрешен вход в программу.';en='Error while writing user ""%1""."
"You can not tag prepared users who are allowed to enter the program. '"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
		...
		Исключение
			...
			Если РегистрироватьВЖурнале Тогда
				Если Выборка.СрокДействияИстек Тогда
					ШаблонКомментария =
						НСтр("es='No consiguió quitar al usuario ""%1"" indicio "
"""la Entrada en el programa es permitido"" en relación a la terminación de la validez por la causa: "
"%2';ru='Не удалось снять пользователю ""%1"" признак"
"""Вход в программу разрешен"" в связи с окончанием срока действия по причине:"
"%2';en='Failed to remove user ""%1"" symptom"
"""Entrance to the program is allowed"" in connection with the expiration due to:"
"%2'");
				Иначе
					ШаблонКомментария =
						НСтр("es='No consiguió quitar al usuario ""%1"" indicio "
"""la Entrada en el programa es permitido"" en relación a la ausencia del trabajo "
"en el programa más de plazo fijado por la causa: "
"%2';ru='Не удалось снять пользователю ""%1"" признак"
"""Вход в программу разрешен"" в связи с отсутствием работы"
"в программе более установленного срока по причине:"
"%2';en='Failed to remove user ""%1"" symptom"
"""Logging into the program is allowed"" due to a lack of work"
"in the program for more than a specified period due to:"
"%2'");
				КонецЕсли;
				ЗаписьЖурналаРегистрации(
					НСтр("es='Los usuarios. La falta de la prohibición automática de la entrada en el programa';ru='Пользователи.Ошибка автоматического запрещения входа в программу';en='Users: Error automatically blocking entry to the program'",
					     ОбщегоНазначения.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					Метаданные.НайтиПоТипу(ТипЗнч(Пользователь)),
					Пользователь,
...
Процедура ПроверитьВходВПрограммуРазрешен(ОшибкаАвторизации)
	...
	ОшибкаАвторизации = ЗаголовокСообщенияАвторизацияНеВыполненаСПереводомСтроки()
		+ НСтр("es='Su anotación de descuento es desconectada. Se dirijan al administrador.';ru='Ваша учетная запись отключена. Обратитесь к администратору.';en='Your account has been disabled. Contact your administrator.'");
