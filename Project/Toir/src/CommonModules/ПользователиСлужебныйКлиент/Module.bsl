...
Процедура ОткрытьОтчетИлиФорму(ТекущийЭлемент, Пользователь, ТекущийПользователь, ИмяФормыПерсональныхНастроек) Экспорт
	...
	Если Пользователь <> ТекущийПользователь Тогда
		ТекстПредупреждения =
			НСтр("es='Para la presentación de los ajustes de otro usuario es necesario "
"poner en marcha el programa de su nombre y abrir el informe necesario o la forma.';ru='Для просмотра настроек другого пользователя необходимо"
"запустить программу от его имени и открыть нужный отчет или форму.';en='To view the settings of another user, you must"
"run the program on his behalf and open the desired report or form. '");
		...
		Для Каждого КлючОбъекта Из ЭлементДереваЗначений.ТекущиеДанные.Ключи Цикл
			...
			Иначе
				...
				Если ЭлементДереваЗначений.ТекущиеДанные.ТипСтроки = "НастройкиРабочегоСтола" Тогда
					ПоказатьПредупреждение(,
						НСтр("es='Para la presentación de los ajustes del escritorio pasen a la sección "
"""el Escritorio"" en la interfaz de mando del programa.';ru='Для просмотра настроек рабочего стола перейдите к разделу"
"""Рабочий стол"" в командном интерфейсе программы.';en='To view the desktop settings, go to"
""" Desktop "" in the command interface of the program.'"));
					Возврат;
				КонецЕсли;
				
				Если ЭлементДереваЗначений.ТекущиеДанные.ТипСтроки = "НастройкиКомандногоИнтерфейса" Тогда
					ПоказатьПредупреждение(,
						НСтр("es='Para la presentación de los ajustes de la interfaz de mando "
"escojan la sección necesaria de la interfaz de mando del programa.';ru='Для просмотра настроек командного интерфейса"
"выберите нужный раздел командного интерфейса программы.';en='To view command line settings"
"select the desired section of the command interface of the program. '"));
					Возврат;
				КонецЕсли;
				
				Если РодительЭлемента <> Неопределено Тогда
					ТекстПредупреждения =
						НСтр("es='Para la presentación del ajuste dado es necesario abrir ""%1"" "
"y luego pasar a la forma ""%2"".';ru='Для просмотра данной настройки необходимо открыть ""%1"" "
"и затем перейти к форме ""%2"".';en='To view this setting, open ""%1"" and then go to form ""%2"".'");
		...
		ПоказатьПредупреждение(,НСтр("es='El ajuste dado es imposible ver.';ru='Данную настройку невозможно просмотреть.';en='This setting cannot be viewed.'"));
		Возврат;
	...
	ПоказатьПредупреждение(,НСтр("es='Escojan el ajuste para la presentación.';ru='Выберите настройку для просмотра.';en='Select a setting to view.'"));
...
Функция ФормированиеПоясненияПриКопировании(ПредставлениеНастройки, КоличествоНастроек, ПояснениеКомуСкопированыНастройки) Экспорт
	...
	Если КоличествоНастроек = 1 Тогда
		...
		ПояснениеОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='""%1"" es copiada %2';ru='""%1"" скопирована %2';en=""'% 1' copied% 2"""),
	...
	Иначе
		...
		ПояснениеОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es copiado %1 %2';ru='Скопировано %1 %2';en='Copied%1%2'"),
			ПрописьПредмета,
			ПояснениеКомуСкопированыНастройки);
	КонецЕсли;
...
Функция ПояснениеПользователи(КоличествоПользователей, Пользователь) Экспорт
	...
	Если КоличествоПользователей = 1 Тогда
		ПояснениеКомуСкопированыНастройки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Al usuario ""%1""';ru='пользователю ""%1""';en='to user ""%1""'"), Пользователь);
	Иначе
		ПояснениеКомуСкопированыНастройки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='A %1 usuarios';ru='%1 пользователям';en='% 1 users'"), КоличествоПользователей);
	КонецЕсли;
