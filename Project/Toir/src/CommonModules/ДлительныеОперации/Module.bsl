...
Функция ВыполнитьВФоне(Знач ИмяПроцедуры, Знач ПараметрыПроцедуры, Знач ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ЗапуститьНеВФоне И ПараметрыВыполнения.ЗапуститьВФоне Тогда
		ВызватьИсключение НСтр("es='Los parámetros ""VsegdaNeVFone"" y ""VsegdaVFone"" "
"no pueden al mismo tiempo aceptar el significado la Verdad en DlitelnyeOperatsii. ВыполнитьВФоне.';ru='Параметры ""ВсегдаНеВФоне"" и ""ВсегдаВФоне"""
"не могут одновременно принимать значение Истина в ДлительныеОперации.ВыполнитьВФоне.';en='Parameters ""ВсегдаНеВФоне"" and ""ВсегдаВФоне"" can not simultaneously take on the value of True in'");
	КонецЕсли;
#Если ВнешнееСоединение Тогда
	ИнформационнаяБазаФайловая = ОбщегоНазначения.ИнформационнаяБазаФайловая();
	Если ПараметрыВыполнения.БезРасширений И ИнформационнаяБазаФайловая Тогда
		ВызватьИсключение НСтр("es='La tarea de fondo no puede ser puesta en marcha con el parámetro ""BezRasshireny"" "
"en la base de fichero informativa en DlitelnyeOperatsii. ВыполнитьВФоне.';ru='Фоновое задание не может быть запущено с параметром ""БезРасширений"""
"в файловой информационной базе в ДлительныеОперации.ВыполнитьВФоне.';en='The background task cannot be started with the parameter ""БезРасширений"""
"in the file infobase for ДлительныеОперации.ВыполнитьВФоне.'");
	КонецЕсли;
#КонецЕсли
...
#КонецЕсли
	...
	Если ВыполнитьБезФоновогоЗадания Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("es='La falta de la ejecución';ru='Ошибка выполнения';en='Error executing'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ОтменитьВыполнениеЗадания(Знач ИдентификаторЗадания) Экспорт 
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("es='Las operaciones largas. La anulación de la ejecución de la tarea de fondo';ru='Длительные операции.Отмена выполнения фонового задания';en='Long operations. Canceling a background operation'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ЗаданиеВыполнено(Знач ИдентификаторЗадания) Экспорт
	...
	Если Задание = Неопределено Тогда
		ЗаписьЖурналаРегистрации(НСтр("es='Las operaciones largas. La tarea de fondo no es encontrada';ru='Длительные операции.Фоновое задание не найдено';en='Long operations. Background job not found'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Иначе
		...
		ИначеЕсли Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
			ЗаписьЖурналаРегистрации(
				НСтр("es='Las operaciones largas. La tarea de fondo es anulada por el administrador';ru='Длительные операции.Фоновое задание отменено администратором';en='Long operations. Background operation canceled by admin'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				НСтр("es='La tarea se ha acabado con la falta desconocida.';ru='Задание завершилось с неизвестной ошибкой.';en='The task completed with an unknown error.'"));
		Иначе
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
	...
	ИначеЕсли ОперацияНеВыполнена Тогда
		ВызватьИсключение(НСтр("es='No consiguió cumplir la operación dada. "
"Los detalles de cm. En la revista del registro.';ru='Не удалось выполнить данную операцию. "
"Подробности см. в Журнале регистрации.';en='Failed to complete this operation."
...
Функция ОперацияВыполнена(Знач ИдентификаторЗадания, Знач ИсключениеПриОшибке = Ложь, Знач ВыводитьПрогрессВыполнения = Ложь, 
	...
	Задание = НайтиЗаданиеПоИдентификатору(ИдентификаторЗадания);
	Если Задание = Неопределено Тогда
		Пояснение = НСтр("es='La operación no es cumplida por la terminación de repuesto de la tarea de fondo. "
"La tarea de fondo no es encontrada';ru='Операция не выполнена из-за аварийного завершения фонового задания."
"Фоновое задание не найдено';en='Operation failed due to abnormal completion of background job."
"Background task not found """
"""Operation failed:%1'") + ": " + Строка(ИдентификаторЗадания);
		ЗаписьЖурналаРегистрации(НСтр("es='Las operaciones largas';ru='Длительные операции';en='Long operations'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , Пояснение);
		Если ИсключениеПриОшибке Тогда
			ВызватьИсключение(НСтр("es='No consiguió cumplir la operación dada.';ru='Не удалось выполнить данную операцию.';en='Failed to complete this operation.'"));
		КонецЕсли;
		Результат.Статус = "Ошибка";
		Результат.КраткоеПредставлениеОшибки = НСтр("es='La operación no es cumplida por la terminación de repuesto de la tarea de fondo.';ru='Операция не выполнена из-за аварийного завершения фонового задания.';en='The operation failed due to abnormal completion of the background task.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
		...
		Если ПараметрыСеанса.ОтмененныеДлительныеОперации.Найти(ИдентификаторЗадания) = Неопределено Тогда
			...
			Если Задание.ИнформацияОбОшибке <> Неопределено Тогда
				Результат.КраткоеПредставлениеОшибки   = НСтр("es='La operación es anulada por el administrador.';ru='Операция отменена администратором.';en='The operation was canceled by the administrator.'");
			...
			Если ИсключениеПриОшибке Тогда
				...
				Иначе
					ТекстСообщения = НСтр("es='No consiguió cumplir la operación dada.';ru='Не удалось выполнить данную операцию.';en='Failed to complete this operation.'");
				КонецЕсли;
...
Процедура ВыполнитьПроцедуру(ИмяПроцедуры, ПараметрыПроцедуры)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El formato equivocado del parámetro de ImyaProtsedury (el significado entregado: %1)';ru='Неверный формат параметра ИмяПроцедуры (переданное значение: %1)';en='Invalid format for the ИмяПроцедуры parameter (passed value: %1)'"), ИмяПроцедуры);
...
Функция ВыполнитьФоновоеЗадание(ПараметрыВыполнения, ИмяМетода, Параметры, Ключ, Наименование)
	...
	Если ТекущийРежимЗапуска() = Неопределено
		...
		Если ПараметрыВыполнения.ОжидатьЗавершение = Неопределено И Сеанс.ИмяПриложения = "BackgroundJob" Тогда
			ВызватьИсключение НСтр("es='En la base de fichero informativa es imposible al mismo tiempo cumplir más de una tarea de fondo';ru='В файловой информационной базе невозможно одновременно выполнять более одного фонового задания';en='In a file infobase, it is impossible to simultaneously execute more than one background job'");
		ИначеЕсли Сеанс.ИмяПриложения = "COMConnection" Тогда
			ВызватьИсключение НСтр("es='En la base de fichero informativa es posible poner en marcha la tarea de fondo solamente de la aplicación de clientes';ru='В файловой информационной базе можно запустить фоновое задание только из клиентского приложения';en='In the file infobase, you can run the background task only from the client application'");
		КонецЕсли;
