...
Процедура НастроитьОтчетВМодулеМенеджера(Настройки, ОтчетМетаданные) Экспорт
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El significado inadmisible del parámetro de KlyuchVarianta en el procedimiento ВариантыОтчетов.НастроитьОтчетВМодулеМенеджера. "
"No consiguió ajustar las variantes del informe del módulo del manager por la causa: "
"%1';ru='Недопустимое значение параметра КлючВарианта в процедуре ВариантыОтчетов.НастроитьОтчетВМодулеМенеджера."
"Не удалось настроить варианты отчета из модуля менеджера по причине:"
"%1';en='Invalid value for КлючВарианта parameter in the procedure ВариантыОтчетов.НастроитьОтчетВМодулеМенеджера.           Failed to configure report options from the manager module due to:"
"%1'"),
...
Функция ОписаниеОтчета(Настройки, Отчет) Экспорт
	...
	Если Результат.Количество() <> 1 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El significado inadmisible del parámetro el Informe en la función VariantyOtchetov. ОписаниеОтчета. "
"El informe ""%1"" no es conectado al subsistema ""%2"". Comprueben la propiedad ""el Depósito de las variantes"" en las propiedades del informe.';ru='Недопустимое значение параметра Отчет в функции ВариантыОтчетов.ОписаниеОтчета."
"Отчет ""%1"" не подключен к подсистеме ""%2"". Проверьте свойство ""Хранилище вариантов"" в свойствах отчета.';en='Invalid value for the Report parameter in the Report Variants function. Report Description."
"The report ""%1 "" is not connected to the subsystem ""%2 "". Check the Vault Options"" property in the report properties.'"),
...
Функция ОписаниеВарианта(Настройки, Отчет, КлючВарианта) Экспорт
	...
	Если Результат.Количество() <> 1 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El significado inadmisible del parámetro de KlyuchVarianta en la función VariantyOtchetov. ОписаниеВарианта: "
"variante ""%1"" falta en el informe ""%2"".';ru='Недопустимое значение параметра КлючВарианта в функции ВариантыОтчетов.ОписаниеВарианта:"
"вариант ""%1"" отсутствует в отчете ""%2"".';en='Invalid value for the КлючВарианта parameter in the function ВариантыОтчетов.ОписаниеВарианта:"
"option ""%1"" missing from report ""%2 ""'"),
...
Процедура СброситьПользовательскиеНастройки(Ключ, ТипыНастроек = Неопределено) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("es='El tipo incorrecto del parámetro ""el Informe""';ru='Некорректный тип параметра ""Отчет""';en='Invalid ""Report"" parameter type'");
	КонецЕсли;
		...
		Пока Истина Цикл
			...
			Исключение
				...
				ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка, 
					НСтр("es='Durante el extracto de los ajustes de usuario de los informes del depósito de sistema ha surgido la falta:';ru='В процессе выборки пользовательских настроек отчетов из системного хранилища возникла ошибка:';en='An error occurred while fetching custom report settings from the system store:'")
					+ Символы.ПС
					+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецПопытки;
...
Процедура ПеренестиПользовательскиеИзСтандартногоХранилища(ИменаОтчетов = "") Экспорт
	ПредставлениеПроцедуры = НСтр("es='La conversión directa de las variantes de los informes';ru='Прямая конвертация вариантов отчетов';en='Direct conversion of report options'");
	...
	Пока Истина Цикл
		...
		Исключение
			...
			ОшибокЧтенияПодряд = ОшибокЧтенияПодряд + 1;
			ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка,
				НСтр("es='Durante el extracto de las variantes de los informes del depósito estandartizado ha surgido la falta:';ru='В процессе выборки вариантов отчетов из стандартного хранилища возникла ошибка:';en='An error occurred while fetching report options from the standard repository:'")
				+ Символы.ПС
				+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
Процедура ЗагрузитьПользовательские(ПользовательскиеВарианты = Неопределено) Экспорт
	...
	ПредставлениеПроцедуры = НСтр("es='Acabar la conversión de las variantes de los informes';ru='Завершить конвертацию вариантов отчетов';en='Complete conversion of report options'");
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Пока ОписаниеВарианта.Следующий() Цикл 
		...
		Если Не ЗначениеЗаполнено(ОписаниеВарианта.Автор) Тогда 
			Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Variante ""%1"" informes ""%2"": no es encontrado el autor ""%3""';ru='Вариант ""%1"" отчета ""%2"": не найден автор ""%3""';en='Variant ""% 1"" of report ""% 2"": author ""% 3"" not found'"),
...
Функция ИспользуемыеРазделы() Экспорт
	...
	Для Каждого ЭлементСписка Из СписокРазделов Цикл
		...
		Иначе
			ШаблонЗаголовка = НСтр("es='Los informes de la sección ""%1""';ru='Отчеты раздела ""%1""';en='Reports in %1'");
		КонецЕсли;
		...
		Если РазделМетаданные = ИдентификаторНачальнойСтраницы Тогда
			...
			Строка.Представление = НСтр("es='La página inicial';ru='Начальная страница';en='start page'");
		Иначе
...
Функция ПодключитьОтчетИЗагрузитьНастройки(Параметры) Экспорт
		...
		Если ТипЗнч(Результат.СсылкаВарианта) <> Тип("СправочникСсылка.ВариантыОтчетов")
			...
			Если Результат.СсылкаВарианта = Неопределено Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='En el método ""%1"" no son indicados parámetros ""%2"".';ru='В методе ""%1"" не указаны параметры ""%2"".';en='Method ""% 1"" did not specify parameters ""% 2"".'"),
	...
	Если НадоПрочитатьНастройки Тогда
		...
		Если ВариантНастроекКД = Неопределено Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Variante ""%1"" (la llave ""%2"") no es encontrada en el esquema del informe ""%3"".';ru='Вариант ""%1"" (ключ ""%2"") не найден в схеме отчета ""%3"".';en='Option ""% 1"" (key ""% 2"") was not found in report schema ""% 3"".'"),
...
Процедура ПриЗаписиДополнительногоОтчета(ТекущийОбъект, Отказ, ВнешнийОбъект) Экспорт
	...
	Если Не ВариантыОтчетовПовтИсп.ПравоДобавления() Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No basta los derechos de admisión para la anotación de las variantes del informe adicional ""%1"".';ru='Недостаточно прав доступа для записи вариантов дополнительного отчета ""%1"".';en='There are not enough access rights to write options for the optional report ""%1"".'"),
...
Функция ПараметрыОткрытия(ВариантСсылка) Экспорт
	...
	Иначе
		...
		Если Не Выборка.Следующий() Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Es insuficientemente derecho para la apertura de variante ""%1"".';ru='Недостаточно прав для открытия варианта ""%1"".';en=""Not enough rights to open option '%1'."""), Строка(ВариантСсылка));
		КонецЕсли;
...
Процедура ПриПодключенииОтчета(ПараметрыОткрытия) Экспорт
	...
	Если ПараметрыОткрытия.ТипОтчета = "Внутренний"
		...
		Если ТипЗнч(МетаданныеОтчета) <> Тип("ОбъектМетаданных") Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió abrir el informe ""%1"". "
"Probablemente, era desconectada la ampliación de la configuración con este informe.';ru='Не удалось открыть отчет ""%1""."
"Возможно, было отключено расширение конфигурации с этим отчетом.';en='Failed to open report ""%1"". The configuration extension with this report may have been disabled. '"),
...
Функция СформироватьОтчет(Знач Параметры, Знач ПроверятьЗаполнение, Знач ПолучатьФлажокПустой) Экспорт
	...
	Если Не Подключение.Успех Тогда
		Результат.ТекстОшибки = НСтр("es='No consiguió formar el informe:';ru='Не удалось сформировать отчет:';en='Failed to generate a report:'") + Символы.ПС + Подключение.ТекстОшибки;
		Возврат Результат;
	КонецЕсли;
	...
	Если ПроверятьЗаполнение Тогда
		...
		Если Не ПроверкаПройдена Тогда
			Результат.ТекстОшибки = НСтр("es='El informe no ha pasado la comprobación del relleno:';ru='Отчет не прошел проверку заполнения:';en='Report did not pass the completion check:'");
	...
	Исключение
		ТекстОшибки = НСтр("es='No consiguió determinar las tablas usadas:';ru='Не удалось определить используемые таблицы:';en='Failed to determine which tables to use:'");
	...
	Если СсылкиВариантов.Количество() > 0 Тогда
		...
		Для Каждого Ссылка Из СсылкиВариантов Цикл
			...
			Если ОтчетЗначение = Неопределено Тогда
				СтрокаТаблицы.Представление = НСтр("es='<es insuficientemente derecho para el trabajo con la variante del informe>';ru='<Недостаточно прав для работы с вариантом отчета>';en='<Недостаточно прав для работы с вариантом отчета>'");
			Иначе
...
Функция СформироватьИнформациюОбОтчетеПоПолномуИмени(ОтчетПолноеИмя) Экспорт
		...
		Если Результат.ОтчетМетаданные = Неопределено Тогда
			...
			ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Предупреждение,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El informe ""%1"" no es encontrado en el programa, él figurará como exterior.';ru='Отчет ""%1"" не найден в программе, он будет значиться как внешний.';en=""Report '%1' was not found in the program; it will appear as external."""),
					ОтчетПолноеИмя));
		ИначеЕсли Не ПравоДоступа("Просмотр", Результат.ОтчетМетаданные) Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No basta los derechos de admisión al informe ""%1"".';ru='Недостаточно прав доступа к отчету ""%1"".';en=""Not enough access rights to report '%1'."""),
				ОтчетПолноеИмя);
		КонецЕсли;
	...
	Иначе
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Para el informe ""%1"" es imposible determinar el tipo (no es establecido el prefijo).';ru='Для отчета ""%1"" невозможно определить тип (не установлен префикс).';en=""It is not possible to determine the type for report '% 1' (no prefix is set)."""),
			ОтчетПолноеИмя);
		Возврат Результат;
	КонецЕсли;
...
Функция ХранилищеВариантовДополнительногоОтчетаКорректное(МетаданныеОтчета, ТекстПредупреждения = "") Экспорт 
	...
	ШаблонПредупреждения = НСтр("es='La propiedad del informe ""el Depósito de las variantes"" %1. "
"La conservación (elección) de las variantes del informe trabajará en el régimen limitado. "
"Se dirijan al elaborador del informe adicional (exterior).';ru='Свойство отчета ""Хранилище вариантов"" %1."
"Сохранение (выбор) вариантов отчета будет работать в ограниченном режиме."
"Обратитесь к разработчику дополнительного (внешнего) отчета.';en='Report property ""Vault"" %1."
"Saving (selecting) report options will work in a limited mode."
"Contact the developer of the additional (external) report.'");
	
	ПараметрПредупреждения = НСтр("es='Es rellenado no correctamente';ru='заполнено не корректно';en='not filled correctly'");
	Если МетаданныеОтчета.ХранилищеВариантов = Неопределено Тогда 
		ПараметрПредупреждения = НСтр("es='No es rellenado';ru='не заполнено';en='not completed'");
	КонецЕсли;
...
Функция ПредопределенныеВариантыОтчетов(ТипОтчетов = "Внутренний") Экспорт
	...
	Для Каждого ОтчетМетаданные Из Метаданные.Отчеты Цикл 
		...
		Если ОписаниеОтчета.ИспользуетСКД Тогда
			...
			Исключение
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No consiguió leer el esquema del informe: "
"%1';ru='Не удалось прочитать схему отчета:"
"%1';en='Failed to read report outline:"
"%1'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			...
			Если СхемаКД <> Неопределено Тогда
				...
				Исключение
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='No consiguió leer la lista de las variantes del informe: "
"%1';ru='Не удалось прочитать список вариантов отчета:"
"%1';en='Failed to read the list of report options:"
"%1'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
					ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Предупреждение, ТекстОшибки, ОтчетМетаданные);
				КонецПопытки;
			КонецЕсли;
			...
			Если ВариантыНастроек = Неопределено Тогда
				...
				Исключение
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='No consiguió leer la lista de las variantes del informe del módulo del manager: "
"%1';ru='Не удалось прочитать список вариантов отчета из модуля менеджера:"
"%1';en='Failed to read the list of report options from the manager module:"
"%1'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
					ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки, ОтчетМетаданные);
				КонецПопытки;
			КонецЕсли;
			...
			Если ВариантыНастроек <> Неопределено Тогда
				Для Каждого ВариантНастроекКД Из ВариантыНастроек Цикл
					...
					Если РазрешеноИндексирование И ТипЗнч(ВариантНастроекКД) = Тип("ВариантНастроекКомпоновкиДанных") Тогда
						...
						Исключение
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='No consiguió leer los ajustes de variante "
"""%1: %2""';ru='Не удалось прочитать настройки варианта ""%1"":"
"%2';en='Failed to read option settings"" ""%1"" "":"
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий     = НСтр("es='Reduce la cantidad de los ajustes rápidos en los informes de usuario hasta 2 piezas';ru='Уменьшает количество быстрых настроек в пользовательских отчетах до 2 шт.';en='Reduces the number of quick settings in user reports to 2 pcs.'");
	...
	Если РазрешеноИндексированиеНеразделенныхДанных() Тогда
		...
		Обработчик.Комментарий   = НСтр("es='La renovación del índice de la búsqueda de los informes previstos en el programa.';ru='Обновление индекса поиска отчетов, предусмотренных в программе.';en='Updating the search index of reports provided in the program.'");
	КонецЕсли;
	...
	Обработчик.Комментарий     = НСтр("es='La renovación del índice de la búsqueda de los informes conservados por los usuarios.';ru='Обновление индекса поиска отчетов, сохраненных пользователями.';en='Updating the search index of reports saved by users.'");
	...
	Обработчик.Комментарий = НСтр("es='La instalación del parámetro de IstochnikDannyh en los ajustes de las variantes del informe universal. "
"Después de la terminación del tratamiento, el cambio de nombre de los objetos de los metadatos no llevará a la pérdida de las variantes conservadas de los informes';ru='Установка параметра ИсточникДанных в настройках вариантов универсального отчета."
"После завершения обработки, переименование объектов метаданных не приведет к потере сохраненных вариантов отчетов';en='Setting the ИсточникДанных parameter in the settings of the universal report options."
"After processing is complete, renaming metadata objects will not result in the loss of saved report options'");
...
Процедура ПриОпределенииВидовПодключаемыхКоманд(ВидыПодключаемыхКоманд) Экспорт
	...
	Вид.Заголовок   = НСтр("es='Los informes';ru='Отчеты';en='Reports'");
	Вид.Порядок     = 50;
...
Функция НаименованиеПодсистемы(КодЯзыка)
	Возврат НСтр("es='Las variantes de los informes';ru='Варианты отчетов';en='Report options'", ?(КодЯзыка = Неопределено, ОбщегоНазначения.КодОсновногоЯзыка(), КодЯзыка));
КонецФункции
...
Функция ПодключитьОтчетОбъект(СсылкаОтчета, ПолучатьМетаданные)
	...
	Если СсылкаОтчета = Неопределено Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='En el método ""%1"" no es indicado parámetro ""%2"".';ru='В методе ""%1"" не указан параметр ""%2"".';en='The method ""% 1"" did not specify the parameter ""% 2"".'"),
	...
	Если ТипЗнч(Результат.Ссылка) = Тип("Строка") Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El informe ""%1"" es anotado como exterior y no puede ser conectado del programa';ru='Отчет ""%1"" записан как внешний и не может быть подключен из программы';en='Report ""%1"" is written as external and cannot be connected from the program'"),
			Результат.Ссылка);
		Возврат Результат;
	КонецЕсли;
	...
	Если ТипЗнч(Результат.Ссылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных")
	 Или ТипЗнч(Результат.Ссылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений") Тогда
		...
		Если ТипЗнч(Результат.Метаданные) <> Тип("ОбъектМетаданных") Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El informe ""%1"" no es encontrado en el programa';ru='Отчет ""%1"" не найден в программе';en=""Report '%1' not found in program"""),
				Результат.Имя);
			Возврат Результат;
		КонецЕсли;
		Результат.Имя = Результат.Метаданные.Имя;
		Если Не ПравоДоступа("Использование", Результат.Метаданные) Тогда
			Результат.ТекстОшибки = НСтр("es='No basta los derechos de admisión';ru='Недостаточно прав доступа';en='Not enough access rights'");
			Возврат Результат;
		КонецЕсли;
		Попытка
			...
			Результат.Успех = Истина;
		Исключение
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No consiguió conectar el informe %1:';ru='Не удалось подключить отчет %1:';en='Failed to connect report %1:'"),
...
Процедура УстановитьРежимВыводаОтчетаВПанеляхОтчетов(Настройки, Отчет, ГруппироватьПоОтчетам)
	...
	Иначе
		...
		Если ОписаниеОтчета.Количество() <> 1 Тогда
			ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Предупреждение, 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El informe ""%1"" no es conectado al subsistema.';ru='Отчет ""%1"" не подключен к подсистеме.';en=""Report '%1' is not connected to the subsystem."""), Отчет.Имя));
			Возврат;
		КонецЕсли;
...
Функция ИзмененияКлючей()
	...
	Для Каждого Изменение Из Изменения Цикл
		...
		Если Найденные.Количество() > 0 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta del registro de los cambios del nombre de la variante del informe ""%1"": "
"el nombre Actual de variante ""%2"" (el nombre viejo ""%3"") "
"figura también como el nombre viejo ""%4"" (el nombre actual ""%5"").';ru='Ошибка регистрации изменений имени варианта отчета ""%1"":"
"Актуальное имя варианта ""%2"" (старое имя ""%3"")"
"также числится как старое имя ""%4"" (актуальное имя ""%5"").';en='Error registering the name of the report variation ""%1"":"
"The current variant name is ""%2"" (old name is ""%3"")"
"also listed as the old name ""%4"" (actual name is ""%5"").'"),
		...
		Если Найденные.Количество() > 2 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta del registro de los cambios del nombre de la variante del informe ""%1"": "
"el nombre Viejo de variante ""%2"" (el nombre actual ""%3"") "
"figura también como el nombre viejo "
"de la variante del informe ""%4"" (el nombre actual ""%5"").';ru='Ошибка регистрации изменений имени варианта отчета ""%1"":"
"Старое имя варианта ""%2"" (актуальное имя ""%3"")"
"также числится как старое имя "
"варианта отчета ""%4"" (актуальное имя ""%5"").';en='Error registering the name of the report variation ""%1"":"
"The old variant name is ""%2"" (actual name is ""%3"")"
"also listed as old name"
"report option ""%4"" (actual name is ""%5"").'"),
				Строка(Изменение.Отчет),
				Изменение.СтароеИмяВарианта,
				Изменение.АктуальноеИмяВарианта,
...
Функция ГлобальныеНастройки() Экспорт
	...
	Результат.Поиск.Вставить("ПодсказкаВвода", НСтр("es='El nombre, el campo o el autor del informe';ru='Наименование, поле или автор отчета';en='Name, field or author of the report'"));
...
Процедура ЗаписатьВЖурналЗапускПроцедуры(ИмяПроцедуры)
	...
	ЖурналРегистрации.ДобавитьСообщениеДляЖурналаРегистрации(НаименованиеПодсистемы(Неопределено),
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El lanzamiento del procedimiento ""%1"".';ru='Запуск процедуры ""%1"".';en=""Launching procedure '%1'."""), ИмяПроцедуры)); 
...
Процедура ЗаписатьВЖурналЗавершениеПроцедуры(ИмяПроцедуры, ИзмененоОбъектов = Неопределено)
	...
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La terminación del procedimiento ""%1"".';ru='Завершение процедуры ""%1"".';en='Completing of the procedure ""% 1"".'"), ИмяПроцедуры);
	Если ИзмененоОбъектов <> Неопределено Тогда
		Текст = Текст + " " 
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Es cambiado %1 objetos.';ru='Изменено %1 объектов.';en='%1 objects changed.'"), ИзмененоОбъектов);
	КонецЕсли;
	ЖурналРегистрации.ДобавитьСообщениеДляЖурналаРегистрации(НаименованиеПодсистемы(Неопределено),
...
Процедура ПерейтиКРедакции21() Экспорт
	ПредставлениеПроцедуры = НСтр("es='Pasar a la redacción 2.1';ru='Перейти к редакции 2.1';en='Go to edition 2.1'");
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗаполнитьСсылкиПредопределенных() Экспорт
	ПредставлениеПроцедуры = НСтр("es='Rellenar las referencias de las variantes predestinadas de los informes';ru='Заполнить ссылки предопределенных вариантов отчетов';en='Fill in predefined report options links'");
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура СократитьКоличествоБыстрыхНастроек(ВходящиеПараметры = Неопределено) Экспорт
	ПредставлениеПроцедуры = НСтр("es='La reducción de la cantidad de los ajustes rápidos en los informes';ru='Сокращение количества быстрых настроек в отчетах';en='Reducing the number of quick settings in reports'");
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Для Каждого СтрокаТаблицы Из ТаблицаОтчетов Цикл
		...
		Если ИнформацияОбОшибке <> Неопределено Тогда // Возникла проблема.
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Variante ""%1"" informes ""%2"":';ru='Вариант ""%1"" отчета ""%2"":';en='Variant ""% 1"" of report ""% 2"":'")
				+ Символы.ПС + НСтр("es='A la reducción de la cantidad de los ajustes rápidos de usuario ha surgido la falta:';ru='При уменьшении количества быстрых настроек пользовательского возникла ошибка:';en='When reducing the number of quick user settings, an error occurred:'")
	...
	Если Ошибок > 0 Тогда
		ТекстОшибки = ПредставлениеПроцедуры + ":" + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió reducir la cantidad de los ajustes rápidos de %1 informes.';ru='Не удалось уменьшить количество быстрых настроек %1 отчетов.';en='Failed to reduce the number of quick settings for %1 reports.'"), Ошибок);
		ЗаписьЖурналаРегистрации(НаименованиеПодсистемы(Неопределено),
...
Функция ОбновитьИндексПоиска(Режим, ИндексироватьСхему)
	...
	Уточнение = НРег(ПредставлениеРежима(Режим)) + ", " + ?(ИндексироватьСхему, НСтр("es='Completo';ru='полное';en='complete'"), НСтр("es='Por los cambios';ru='по изменениям';en='by change'"));
	
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La renovación del índice de la búsqueda (%1)';ru='Обновление индекса поиска (%1)';en='Search index update (%1)'"), Уточнение);
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ОбщиеДанные Тогда
				...
				Если Найденные.Количество() = 0 Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Variante ""%1"" no es encontrada para el informe ""%2""';ru='Вариант ""%1"" не найден для отчета ""%2""';en=""Option '% 1' not found for report '% 2'"""), 
			...
			Исключение
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No consiguió reconstruir el índice de la búsqueda para variante ""%1"" informes ""%2"". Probablemente, el informe es defectuoso.';ru='Не удалось перестроить индекс поиска для варианта ""%1"" отчета ""%2"". Возможно, отчет неисправен.';en='Failed to rebuild the search index for option ""%1"" of report ""%2"". The report may be defective.'"), 
					ВариантОбъект.КлючВарианта, ВариантОбъект.Отчет);
...
Процедура АктуализироватьКлючиПредопределенных(Режим, Результат)
	...
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La renovación de las llaves de las variantes de los informes (%1)';ru='Обновление ключей вариантов отчетов (%1)';en='Update report keys (%1)'"), 
		?(Режим = "ОбщиеДанныеКонфигурации", НСтр("es='Las configuraciones metadadas';ru='метаданные конфигурации';en='configuration metadata'"), НСтр("es='Los metadatos de las ampliaciones';ru='метаданные расширений';en='extension metadata'")));
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ПометитьУдаленныеПредопределенные(Режим, Результат)
	...
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La renovación de los ajustes predestinado (%1)';ru='Обновление настроек предопределенных (%1)';en='Updating predefined settings (%1)'"), 
		?(Режим = "ОбщиеДанныеКонфигурации", НСтр("es='Las configuraciones metadadas';ru='метаданные конфигурации';en='configuration metadata'"), НСтр("es='Los metadatos de las ampliaciones';ru='метаданные расширений';en='extension metadata'")));
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура АктуализироватьВариантыОтчетовПоПредопределенным(Режим, Результат)
	...
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La renovación de las variantes de los informes (%1)';ru='Обновление вариантов отчетов (%1)';en='Update reporting options (%1)'"), 
...
Процедура ПометитьНаУдалениеВариантыУдаленныхОтчетов(Режим, Результат)
	...
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La desaparición de las variantes de los informes alejados (%1)';ru='Удаление вариантов удаленных отчетов (%1)';en='Deleting remote reporting options (%1)'"), 
		НРег(ПредставлениеРежима(Режим)));
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗарегистрироватьЗамерыВариантаКОбновлению(Знач СтарыйКлюч, Знач АктуальныйКлюч, Знач АктуальноеНаименование, Результат)
	...
	ОбновлениеЗамера.АктуальноеНаименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El informe ""%1"" (apertura)';ru='Отчет ""%1"" (открытие)';en='Report ""%1"" (open)'"), АктуальноеНаименование);
	
	ОбновлениеЗамера = Результат.ТаблицаЗамеров.Добавить();
	ОбновлениеЗамера.СтароеИмя     = СтарыйКлюч     + ".Формирование";
	ОбновлениеЗамера.АктуальноеИмя = АктуальныйКлюч + ".Формирование";
	ОбновлениеЗамера.АктуальноеНаименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El informe ""%1"" (formación)';ru='Отчет ""%1"" (формирование)';en='Report ""%1"" (generation)'"), АктуальноеНаименование);
	
	ОбновлениеЗамера = Результат.ТаблицаЗамеров.Добавить();
	ОбновлениеЗамера.СтароеИмя     = СтарыйКлюч     + ".Настройки";
	ОбновлениеЗамера.АктуальноеИмя = АктуальныйКлюч + ".Настройки";
	ОбновлениеЗамера.АктуальноеНаименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El informe de ""%1"" (ajustes)';ru='Отчет ""%1"" (настройки)';en=""Report '%1' (settings)"""), АктуальноеНаименование);
КонецПроцедуры
...
Процедура ЗаписатьТаблицуФункциональныхОпций(Режим, Результат)
	...
	КонецЕсли;
	ПредставлениеПроцедуры = НСтр("es='La anotación del cache no dividido en el registro';ru='Запись неразделенного кэша в регистр';en='Writing of non-shared cache to the register'");
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗафиксироватьТекущуюВерсиюРасширений()
	...
	ПредставлениеПроцедуры = НСтр("es='La anotación del registro de las versiones de las ampliaciones';ru='Запись регистра версий расширений';en='Extension version register writing'");
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗапланироватьЗаполнениеПредставлений(Знач КодыЯзыков = Неопределено, Знач ИндексЯзыка = 0)
	...
	ЗаполнениеПредставлений.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El relleno de las representaciones de las variantes predestinadas de los informes para la lengua %1';ru='Заполнение представлений предопределенных вариантов отчетов для языка %1';en='Filling views of predefined report options for language%1'"), Строка(КодЯзыка));
...
Функция СлужебныйПользователь(Знач КодЯзыка)
	...
	// Гкс+[ошибка в обновлении] Селимов от 21.11.2019. Добавлено:{
	НачатьТранзакцию(РежимУправленияБлокировкойДанных.Управляемый);
	// } Гкс-[ошибка в обновлении] Селимов от 21.11.2019.	
...
Функция ПредставлениеРежима(Режим)
	...
	Режимы.Вставить("ОбщиеДанныеКонфигурации", НСтр("es='Las configuraciones generales dadas';ru='Общие данные конфигурации';en='General configuration data'"));
	Режимы.Вставить("ОбщиеДанныеРасширений", НСтр("es='Los datos generales de las ampliaciones';ru='Общие данные расширений';en='General extension data'"));
	Режимы.Вставить("РазделенныеДанныеКонфигурации", НСтр("es='Las configuraciones divididas dadas';ru='Разделенные данные конфигурации';en='Shared configuration data'"));
	Режимы.Вставить("РазделенныеДанныеРасширений", НСтр("es='Los datos divididos de las ampliaciones';ru='Разделенные данные расширений';en='Separated extension data'"));
...
Функция ПредставлениеСмТакже() Экспорт
	Возврат НСтр("es='Véase también';ru='См. также';en='See also'");
КонецФункции 

// Представление группы важности.
Функция ПредставлениеВажный() Экспорт
	Возврат НСтр("es='Importante';ru='Важный';en='Important'");
КонецФункции
...
Функция ЗаполнитьПоляДляПоиска(ВариантОбъект, СведенияОбОтчете = Неопределено) Экспорт
	...
	Если СхемаКД = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Para variante ""%1"" informes ""%2"" no son rellenados los ajustes de la búsqueda: "
"los Nombres de los campos, los parámetros y las selecciones.';ru='Для варианта ""%1"" отчета ""%2"" не заполнены настройки поиска:"
"Наименования полей, параметров и отборов.';en='For option ""%1"" ""of report"" ""%2"" ""search settings are not filled out: Names of fields, parameters and selections.'"),
			ВариантОбъект.КлючВарианта, ВариантОбъект.Отчет);
		Если ЭтоПредопределенный Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС
				+ НСтр("es='Más detalladamente - cm. El procedimiento ""NastroitVariantyOtchetov"" del módulo ""ВариантыОтчетовПереопределяемый"".';ru='Подробнее - см. процедуру ""НастроитьВариантыОтчетов"" модуля ""ВариантыОтчетовПереопределяемый"".';en='For more details, see the procedure ""НастроитьВариантыОтчетов"" of module ""ВариантыОтчетовПереопределяемый"".'");
		КонецЕсли;
	...
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных")
		...
		Исключение
			ШаблонСообщения = НСтр("es='No consiguió leer los ajustes de la variante de usuario del informe: "
"%1';ru='Не удалось прочитать настройки пользовательского варианта отчета:"
"%1';en='Failed to read custom report option settings:"
"%1'");
	...
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		Если ТипЗнч(ВариантОбъект) = Тип("СправочникОбъект.ПредопределенныеВариантыОтчетов")
			...
			ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка, 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No consiguió leer los ajustes de la variante predestinada del informe ""%1"".';ru='Не удалось прочитать настройки предопределенного варианта отчета ""%1"".';en=""Failed to read the settings for the predefined report option '%1'."""), ВариантОбъект.КлючЗамеров),
...
Функция УменьшитьКоличествоБыстрыхНастроек(ВариантОбъект, ОтчетОбъект)
	...
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Son descubiertos los ajustes vacíos de variante de usuario ""%1"" informes ""%2"".';ru='Обнаружены пустые настройки пользовательского варианта ""%1"" отчета ""%2"".';en='Empty settings for user variant ""%1"" of report ""%2"" were found.'"), 
			ВариантОбъект.КлючВарианта, ВариантОбъект.Отчет);
		ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки, ВариантОбъект.Ссылка);
	...
	Если ОтчетОбъект = Неопределено Тогда
		...
		Иначе
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió recibir los ajustes del informe ""%1"":';ru='Не удалось получить настройки отчета ""%1"":';en=""Failed to get report settings '%1':""") + Символы.ПС + Подключение.ТекстОшибки,
				ОтчетСсылка);
...
Функция НайтиВариантыОтчетов(Знач ПараметрыПоиска, Знач ПолучатьИтоговуюТаблицу = Ложь, Знач ПолучатьПодсветку = Ложь) Экспорт
	...
	Запрос.УстановитьПараметр("НаименованиеРабочегоСтола",    НСтр("es='La página inicial';ru='Начальная страница';en='start page'"));
...
Функция СообщениеДанныеЕщеОбновляются() Экспорт
	Возврат НСтр("es='El informe puede contener los datos incorrectos, puesto que no es acabado el tránsito a una nueva versión del programa. Si el informe mucho tiempo es inaccesible, es necesario dirigirse al administrador.';ru='Отчет может содержать некорректные данные, так как не завершен переход на новую версию программы. Если отчет долгое время недоступен, необходимо обратиться к администратору.';en='he report may contain incorrect data, since the transition to the new version of the program has not been completed. If the report is unavailable for a long time, you must contact the administrator.'");
КонецФункции
		...
		Для Каждого СтрокаТаблицы Из Найденные Цикл
			...
			ИначеЕсли Не НайденВПрограмме Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='<el Informe ""%1"" no es encontrado en el programa>';ru='<Отчет ""%1"" не найден в программе>';en='<Отчет ""%1"" не найден в программе>'"),
					ИмяОтчета);
			ИначеЕсли Не ДоступенПоПравам Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='<es insuficientemente derecho para el trabajo con el informe ""%1"">';ru='<Недостаточно прав для работы с отчетом ""%1"">';en='<Недостаточно прав для работы с отчетом ""%1"">'"),
					ИмяОтчета);
			ИначеЕсли Не ДоступенПоОпциям Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='<el Informe ""%1"" es desconectado en los ajustes del programa>';ru='<Отчет ""%1"" отключен в настройках программы>';en='<Отчет ""%1"" отключен в настройках программы>'"),
					ИмяОтчета);
			Иначе
