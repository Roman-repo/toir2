...
#КонецЕсли
	...
	ЗаписатьПримечание(Ссылка, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La conexión, BezopasnyyRezhim = ""%1"".';ru='Подключение, БезопасныйРежим = ""%1"".';en='Connection, БезопасныйРежим = ""%1"".'"), БезопасныйРежим));
...
Функция ПодключенныеОбъектыМетаданных(Вид) Экспорт
	...
	Для Каждого ТипИлиМетаданные Из МассивТиповИлиМетаданных Цикл
		...
		Если ОбъектМетаданных = ДополнительныеОтчетыИОбработкиКлиентСервер.ИмяНачальнойСтраницы() Тогда
			...
			СтрокаТаблицы.Представление = НСтр("es='La página inicial';ru='Начальная страница';en='start page'");
		Иначе
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("es='Los informes adicionales y los tratamientos';ru='Дополнительные отчеты и обработки';en='Additional reports and processing'");
...
Процедура ПриЗаполненииВидовДоступа(ВидыДоступа) Экспорт
	...
	ВидДоступа.Представление = НСтр("es='Los informes adicionales y los tratamientos';ru='Дополнительные отчеты и обработки';en='Additional reports and processing'");
...
Процедура ПриПолученииПрочихНастроек(СведенияОПользователе, Настройки) Экспорт
	...
	НазваниеНастройки = НСтр("es='Los ajustes del acceso rápido a los informes adicionales y los tratamientos';ru='Настройки быстрого доступа к дополнительным отчетам и обработкам';en='Quick access settings for additional reports and processing'");
...
Процедура ПриОпределенииВидовПодключаемыхКоманд(ВидыПодключаемыхКоманд) Экспорт
	Если ВидыПодключаемыхКоманд.Найти("ЗаполнениеОбъектов", "Имя") = Неопределено Тогда
		...
		Вид.Заголовок   = НСтр("es='Rellenar';ru='Заполнить';en='Fill in'");
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	Если ТипыЗаполнениеОбъекта.Количество() > 0 Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЗаполнениеОбъектов") Тогда
			...
			Команда.Представление = НСтр("es='Los tratamientos adicionales del relleno...';ru='Дополнительные обработки заполнения...';en='Additional fill processing …'");
			Команда.Важность      = "СмТакже";
		Иначе
			Команда.Вид           = "КоманднаяПанель";
			Команда.Представление = НСтр("es='El relleno...';ru='Заполнение...';en='Filling ...'");
		КонецЕсли;
	...
	Если ТипыОтчетов.Количество() > 0 Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВариантыОтчетов") Тогда
			Команда.Вид           = "Отчеты";
			Команда.Важность      = "СмТакже";
			Команда.Представление = НСтр("es='Los informes adicionales...';ru='Дополнительные отчеты...';en='Additional reports ...'");
		Иначе
			Команда.Вид           = "КоманднаяПанель";
			Команда.Представление = НСтр("es='Los informes...';ru='Отчеты...';en='Reports ...'");
		КонецЕсли;
		Команда.Порядок            = 50;
		Команда.Обработчик         = "ДополнительныеОтчетыИОбработкиКлиент.ОткрытьСписокКоманд";
		Команда.РежимЗаписи        = "Записывать";
	...
	Если ТипыСозданияСвязанныхОбъектов.Количество() > 0 Тогда
		...
		Иначе
			...
			Команда.Представление      = НСтр("es='La creación de los objetos vinculados...';ru='Создание связанных объектов...';en='Creating related objects ...'");
...
Процедура ПриПодключенииДопОтчета(Ссылка, ПараметрыОтчета, Результат, ПолучатьМетаданные) Экспорт
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьДополнительныеОтчетыИОбработки") Тогда
		ПараметрыОтчета.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El elemento ""%1"" no es conectado, porque ""los informes Adicionales y los tratamientos"" son desconectados en los ajustes del programa.';ru='Элемент ""%1"" не подключен, потому что ""Дополнительные отчеты и обработки"" отключены в настройках программы.';en='Element ""%1"" is not connected because ""Additional reports and processing"" is disabled in the program settings.'"),
			"'" + Строка(Ссылка) + "'");
		Возврат;
	КонецЕсли;
	...
	Если Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.Отчет
		...
		Исключение
			ПараметрыОтчета.ТекстОшибки = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='A la conexión del informe adicional ""%1"" ha surgido la falta:';ru='При подключении дополнительного отчета ""%1"" возникла ошибка:';en='An error occurred while connecting the optional report ""%1"":'"), Строка(Ссылка))
	...
	Иначе
		...
		ПараметрыОтчета.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El elemento %1 no es el informe adicional';ru='Элемент %1 не является дополнительным отчетом';en='Element %1 is not an optional report'"),
			"'"+ Строка(Ссылка) +"'");
...
Процедура ПриПодключенииОтчета(Контекст) Экспорт
	...
	Если ТипЗнч(Ссылка) <> Тип("СправочникСсылка.ДополнительныеОтчетыИОбработки") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Al procedimiento ""%1"" no es entregado el informe';ru='В процедуру ""%1"" не передан отчет';en=""Report not sent to procedure '%1"""),
			"ДополнительныеОтчетыИОбработки.ПриПодключенииОтчета");
	КонецЕсли;
	
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьДополнительныеОтчетыИОбработки") Тогда
		ВызватьИсключение НСтр("es='""Los informes adicionales y los tratamientos"" es desconectada en los ajustes del programa.';ru='""Дополнительные отчеты и обработки"" отключена в настройках программы.';en='""Additional reports and processing"" is disabled in the program settings.'");
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El objeto ""%1"" no es el informe adicional';ru='Объект ""%1"" не является дополнительным отчетом';en=""Object '%1' is not an optional report"""), Строка(Ссылка));
	КонецЕсли;
		...
		Для Каждого СтрокаТаблицы Из Найденные Цикл
			Если Не ПодсистемаВключена Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='<el Informe ""%1"" es inaccesible, porque los informes adicionales y los tratamientos son desconectados en los ajustes del programa.';ru='<Отчет ""%1"" недоступен, т.к. дополнительные отчеты и обработки отключены в настройках программы.';en='<Отчет ""%1"" недоступен, т.к. дополнительные отчеты и обработки отключены в настройках программы.'"),
					СтрокаТаблицы.Представление);
			ИначеЕсли Не ЕстьПравоЧтения Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='<el Informe ""%1"" es inaccesible, porque falta el derecho de la lectura de los informes adicionales y los tratamientos.';ru='<Отчет ""%1"" недоступен, т.к. отсутствует право чтения дополнительных отчетов и обработок.';en='<Отчет ""%1"" недоступен, т.к. отсутствует право чтения дополнительных отчетов и обработок.'"),
					СтрокаТаблицы.Представление);
			ИначеЕсли Не ЭтоПоставляемаяОбработка(Отчет) Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='<el Informe ""%1"" es inaccesible en el modelo del servicio.';ru='<Отчет ""%1"" недоступен в модели сервиса.';en='<Отчет ""%1"" недоступен в модели сервиса.'"),
					СтрокаТаблицы.Представление);
			Иначе
			...
			Если Не Выборка.ЭтоГруппа И Не ЗначениеЗаполнено(Выборка.РежимСовместимостиРазрешений) Тогда
				...
				Исключение
					...
					ТекстОшибки = """" + Объект.Наименование + """:"
						+ Символы.ПС + НСтр("es='No consiguió determinar el régimen de la compatibilidad de los permisos por la causa:';ru='Не удалось определить режим совместимости разрешений по причине:';en='Failed to determine permission compatibility mode due to:'")
						+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
						+ Символы.ПС
						+ Символы.ПС + НСтр("es='El objeto es bloqueado en el régimen de la compatibilidad con la versión 2.1.3.';ru='Объект заблокирован в режиме совместимости с версией 2.1.3.';en='The object is locked in compatibility mode with version 2.1.3.'");
...
Процедура ВыполнитьОбработкуПоРегламентномуЗаданию(ВнешняяОбработка, ИдентификаторКоманды) Экспорт
	...
	ЗаписатьИнформацию(ВнешняяОбработка, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La orden %1: el Lanzamiento.';ru='Команда %1: Запуск.';en='Command%1: Launch.'"), ИдентификаторКоманды));
	...
	Исключение
		ЗаписатьОшибку(
			ВнешняяОбработка,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La orden %1: la Falta de la ejecución: "
"%2';ru='Команда %1: Ошибка выполнения:"
"%2';en='Command%1: Runtime Error:"
"%2 '"),
	...
	// Запись журнала регистрации
	ЗаписатьИнформацию(ВнешняяОбработка, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La orden %1: la Terminación.';ru='Команда %1: Завершение.';en='Command%1: Completion.'"), ИдентификаторКоманды));
...
Функция ПредставлениеРаздела(Раздел) Экспорт
	Если Раздел = ДополнительныеОтчетыИОбработкиКлиентСервер.ИмяНачальнойСтраницы()
		...
		Возврат НСтр("es='La página inicial';ru='Начальная страница';en='start page'");
	КонецЕсли;
...
Функция ПредставлениеОбъектаМетаданных(Объект) Экспорт
	Если ТипЗнч(Объект) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных") Тогда
		...
		Если ТипЗнч(ОбъектМетаданных) <> Тип("ОбъектМетаданных") Тогда
			Возврат НСтр("es='<no existe>';ru='<Не существует>';en='<Не существует>'");
		КонецЕсли;
...
Функция НаименованиеПодсистемы()
	Возврат НСтр("es='Los informes adicionales y los tratamientos';ru='Дополнительные отчеты и обработки';en='Additional reports and processing'", ОбщегоНазначения.КодОсновногоЯзыка());
КонецФункции
...
Функция ВыполнитьКомандуВнешнегоОбъекта(ВнешнийОбъект, ИдентификаторКоманды, ПараметрыКоманды, АдресРезультата)
	...
	Если ОписаниеКоманды = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La orden %1 no es descubierta.';ru='Команда %1 не обнаружена.';en='Command %1 not found.'"), ИдентификаторКоманды);
	КонецЕсли;
...
Процедура ВыгрузитьОтчетыИОбработкиВФайлы(ОтчетыИОбработки)
	...
	Если Не Выгрузка.Успех Тогда
		ТекстОшибки = СокрЛП(
			НСтр("es='No consiguió descargar los informes y los tratamientos de la configuración en los ficheros exteriores:';ru='Не удалось выгрузить отчеты и обработки конфигурации во внешние файлы:';en='Failed to upload reports and configuration processing to external files:'")
	...
	Пока ОбратныйИндекс > 0 Цикл
		...
		Иначе
			ЗаписатьОшибку(СтрокаТаблицы.Ссылка, 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El tipo no apoyado de los objetos de los metadatos: ""1""';ru='Неподдерживаемый вид объектов метаданных: ""1""';en='Unsupported view of metadata objects: ""1""'"), СтрокаТаблицы.Вид));
			ОтчетыИОбработки.Удалить(ОбратныйИндекс);
			Продолжить;
		КонецЕсли;
		...
		Если ТекстСхемы = Неопределено Тогда
			ЗаписатьОшибку(СтрокаТаблицы.Ссылка, 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='""%1"".';ru='Не обнаружен файл ""%1"".';en=""The file '%1' was not found."""), 
		...
		Если Не СозданиеОбработки.Успех Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió crear ""%1"" del fichero exterior "
""
"""%2: %3 %4""';ru='Не удалось создать ""%1"" из внешнего файла ""%2"":"
"%3"
"%4';en='Failed to create ""%1"" ""from external file ""%2"":"
"%3"
"%4'"),
...
Функция ПакетныйЗапускКонфигуратора(Параметры, ПереданныеКомандыЗапуска)
	...
	Если Не ЗначениеЗаполнено(Параметры.ПутьККонфигурации) Тогда
		...
		Если ОткрытКонфигуратор() Тогда
			...
			Иначе
				Результат.Кратко = НСтр("es='Para la descarga de los módulos es necesario cerrar конфигуратор.';ru='Для выгрузки модулей необходимо закрыть конфигуратор.';en='To unload modules, the configurator must be closed.'");
				Возврат Результат;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Результат.Кратко = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='No consiguió descargar la configuración en XML (el código de la falta ""%1"")';ru='Не удалось выгрузить конфигурацию в XML (код ошибки ""%1"")';en='Failed to unload configuration in XML (error code ""%1"")'"),
		КодВозврата);
		...
		Если Не Результат.Успех Тогда
			Если Результат.ИмяОбъектаЗанято Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El nombre ""%1"" es ocupado por los objetos ""%2""';ru='Имя ""%1"" занято объектами ""%2""';en='The name ""%1"" is occupied by objects ""%2""'"),
					ИмяОбъекта,
		...
		Для Каждого КлючИЗначение Из ПоискЗаданий Цикл
			...
			Исключение
				ЗаписьЖурналаРегистрации(
					...
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La falta a la desaparición de la tarea ""%1: %2""';ru='Ошибка при удалении задания ""%1"":%2';en=""Error deleting task '%1':%2"""),
...
Функция ЗарегистрироватьОбработку(Знач Объект, Знач ПараметрыРегистрации) Экспорт
	...
	Если РегистрационныеДанные.Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма
		...
		Результат.ТекстОшибки = НСтр("es='El trabajo con las formas de imprenta no es apoyado.';ru='Работа с печатными формами не поддерживается.';en='Work with printed forms is not supported.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если Не РегистрационныеДанные.БезопасныйРежим И Не Пользователи.ЭтоПолноправныйПользователь(, Истина) Тогда
		Результат.ТекстОшибки = НСтр("es='Para la conexión del tratamiento puesto en marcha en el régimen peligroso, son necesarios los derechos administrativos.';ru='Для подключения обработки, запускаемой в небезопасном режиме, требуются административные права.';en='Administrative rights are required to enable processing that runs in unsafe mode.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если НЕ Объект.ЭтоНовый() И РегистрационныеДанные.Вид <> Объект.Вид Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El tipo del objeto cargado (%1) no corresponde corriente (%2). "
"Para la carga del nuevo objeto presionen ""Crear"".';ru='Вид загружаемого объекта (%1) не соответствует текущему (%2)."
"Для загрузки нового объекта нажмите ""Создать"".';en='The view of the loaded object (% 1) does not match the current one (% 2)."
"To upload a new item, click ""Create"".'"),
			Строка(РегистрационныеДанные.Вид),
			Строка(Объект.Вид));
		Возврат Результат;
	ИначеЕсли ПараметрыРегистрации.ЭтоОтчет <> ЭтоВнешнийОтчет Тогда
		Результат.ТекстОшибки = НСтр("es='El tipo del tratamiento indicado en las noticias sobre el tratamiento exterior, no corresponde a su ampliación.';ru='Вид обработки, указанный в сведениях о внешней обработке, не соответствует ее расширению.';en='The type of processing specified in the information about external processing does not correspond to its extension.'");
		Возврат Результат;
	КонецЕсли;
	...
	Для Каждого КомандаОписание Из РегистрационныеДанные.Команды Цикл
		...
		Если НЕ ЗначениеЗаполнено(КомандаОписание.ВариантЗапуска) Тогда
			ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Para la orden ""%1"" no es determinado el modo del lanzamiento.';ru='Для команды ""%1"" не определен способ запуска.';en='The command ""%1"" does not specify a start method.'"), КомандаОписание.Представление));
		КонецЕсли;
	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		...
		Исключение
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta a la recepción de los datos de registro: "
"la Falta a la desaparición del fichero temporal "
"""%1: %2""';ru='Ошибка при получении регистрационных данных:"
"Ошибка при удалении временного файла ""%1"":"
"%2';en='Error getting registration data:"
"Error deleting temporary file ""%1 "":"
"%2'"),
				РезультатРегистрации.ИмяОбъекта,
	...
	Если ИнформацияОбОшибке <> Неопределено Тогда
		Если ПараметрыРегистрации.ЭтоОтчет Тогда
			ТекстОшибки = НСтр("es='Es imposible conectar el informe adicional del fichero. "
"Probablemente, él no se acerca para esta versión del programa.';ru='Невозможно подключить дополнительный отчет из файла."
"Возможно, он не подходит для этой версии программы.';en='Unable to include additional report from file."
"It may not be suitable for this version of the program.'");
		Иначе
			ТекстОшибки = НСтр("es='Es imposible conectar el tratamiento adicional del fichero. "
"Probablemente, no se acerca para esta versión del programa.';ru='Невозможно подключить дополнительную обработку из файла."
"Возможно, она не подходит для этой версии программы.';en='Unable to attach additional processing from file."
"It may not be suitable for this version of the program.'");
		КонецЕсли;
		ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС + НСтр("es='La información técnica:';ru='Техническая информация:';en='Technical information:'") + Символы.ПС;
