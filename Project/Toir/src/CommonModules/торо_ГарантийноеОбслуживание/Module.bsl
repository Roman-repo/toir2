...
Процедура СформироватьПредупрежденияОРекламационныхАктах(Знач ОбъектФормы, ИмяТЧ) Экспорт
	...
	Для каждого Строка Из ТаблицаГарантий Цикл
		...
		ШаблонСообщения = НСтр("es='La maquinaria <%1> se encuentra sobre el servicio de garantía cerca de la organización %2. Es necesario formalizar el acta de reclamación.';ru='Оборудование <%1> находится на гарантийном обслуживании у организации %2. Необходимо оформить рекламационный акт.';en='Equipment <%1> is under warranty with organization%2. It is necessary to issue a complaint.'");
...
Процедура ПроверитьНаличиеКонтрагентов(ДокОбъект, ИмяТЧ, ИмяРеквизита, Отказ) Экспорт
	...
	Если Отказ Тогда
		...
		ТекстСообщения = НСтр("es='La reparación de la maquinaria de garantía por propias fuerzas es prohibida. El contratista sale solamente para la tarjeta tecnológica. Añadan la tarjeta tecnológica y fijen a ella al contratista.';ru='Ремонт гарантийного оборудования собственными силами запрещён. Подрядчик выбирается только для технологической карты. Добавьте технологическую карту и назначьте ей подрядчика.';en='Repair of warranty equipment in-house is prohibited. The contractor is selected only for the routing. Add a routing and assign a contractor to it.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
...
Функция ПолучитьТаблицуГарантийССамимОР(ОбъектРемонта, ДатаОкончания = Неопределено) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |				ИЛИ ВТ_ТаблицаОбъектов.Родитель = торо_СрокиГарантииОбъектовРемонта.ОбъектРемонта)";
