...
//           например НСтр("es='El usuario no es escogido.';ru='Пользователь не выбран.';en='No user selected.'").
...
//           свойством ГруппаОшибок, например, НСтр("es='El usuario en la línea %1 no es escogido.';ru='Пользователь в строке %1 не выбран.';en='User on line %1 not selected.'").
...
Функция ТекстОшибкиЗаполнения(
	...
	Если ВРег(ВидПоля) = "ПОЛЕ" Тогда
		Если ВРег(ВидСообщения) = "ЗАПОЛНЕНИЕ" Тогда
			Шаблон =
				НСтр("es='Campo ""%1"" no es rellenado';ru='Поле ""%1"" не заполнено';en='Field ""%1"" is empty'");
		ИначеЕсли ВРег(ВидСообщения) = "КОРРЕКТНОСТЬ" Тогда
			Шаблон =
				НСтр("es='Campo ""%1"" es rellenado es incorrecto. "
"%4';ru='Поле ""%1"" заполнено некорректно."
"%4';en='Field ""%1"" is not filled out correctly.                                           %4'");
		КонецЕсли;
	ИначеЕсли ВРег(ВидПоля) = "КОЛОНКА" Тогда
		Если ВРег(ВидСообщения) = "ЗАПОЛНЕНИЕ" Тогда
			Шаблон = НСтр("es='""%3""';ru='Не заполнена колонка ""%1"" в строке %2 списка ""%3""';en='Column ""%1"" in line %2 of list ""%3"" is not filled'");
		ИначеЕсли ВРег(ВидСообщения) = "КОРРЕКТНОСТЬ" Тогда
			Шаблон = 
				НСтр("es='Es rellenada columna ""%1"" en la línea %2 listas ""%3"". "
"%4';ru='Некорректно заполнена колонка ""%1"" в строке %2 списка ""%3""."
"%4';en='The column ""%1"" in line %2 of list ""%3"" is incorrectly filled."
"%4'");
		КонецЕсли;
	ИначеЕсли ВРег(ВидПоля) = "СПИСОК" Тогда
		Если ВРег(ВидСообщения) = "ЗАПОЛНЕНИЕ" Тогда
			Шаблон = НСтр("es='No es introducido ni una línea en lista ""%3""';ru='Не введено ни одной строки в список ""%3""';en='No rows entered in list ""%3""'");
		ИначеЕсли ВРег(ВидСообщения) = "КОРРЕКТНОСТЬ" Тогда
			Шаблон =
				НСтр("es='Es rellenada lista ""%3"". "
"%4';ru='Некорректно заполнен список ""%3""."
"%4';en='List is incorrectly filled ""%3"".                           %4'");
		КонецЕсли;
	КонецЕсли;
...
Процедура Проверить(Знач Условие, Знач Сообщение = "", Знач КонтекстПроверки = "") Экспорт
	...
	Если Условие <> Истина Тогда
		...
		Если ПустаяСтрока(Сообщение) Тогда
			ТекстИсключения = НСтр("es='La operación inadmisible';ru='Недопустимая операция';en='Invalid operation'"); // Assertion failed
		Иначе
		...
		Если Не ПустаяСтрока(КонтекстПроверки) Тогда
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='%1 en %2';ru='%1 в %2';en='% 1 to% 2'"), 
				ТекстИсключения, 
				КонтекстПроверки);
		КонецЕсли;
...
Процедура ПроверитьПараметр(Знач ИмяПроцедурыИлиФункции, Знач ИмяПараметра, Знач ЗначениеПараметра, 
	...
	Проверить(
		...
		НСтр("es='El significado inadmisible del parámetro de ImyaProtseduryIliFunktsii';ru='Недопустимое значение параметра ИмяПроцедурыИлиФункции';en='Invalid value for ИмяПроцедурыИлиФункции parameter'"), 
		Контекст);
	
	Проверить(
		ТипЗнч(ИмяПараметра) = Тип("Строка"), 
		НСтр("es='El significado inadmisible del parámetro de ImyaParametra';ru='Недопустимое значение параметра ИмяПараметра';en='Invalid value for ИмяПараметра parameter'"), 
		Контекст);
	...
	Проверить(
		...
		НСтр("es='El significado inadmisible del parámetro de OzhidaemyeTipy';ru='Недопустимое значение параметра ОжидаемыеТипы';en='Invalid value for ОжидаемыеТипы parameter'"), 
		Контекст);
	...
	Если ТипЗнч(ЗначениеПараметра) = Тип("Структура") И ОжидаемыеТипыСвойств <> Неопределено Тогда
		...
		Для каждого Свойство Из ОжидаемыеТипыСвойств Цикл
			...
			Проверить(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El significado inadmisible del parámetro %1 (la Estructura) en %2. "
"En la estructura se espera la propiedad %3 (tipo %4).';ru='Недопустимое значение параметра %1 (Структура) в %2. "
"В структуре ожидалось свойство %3 (тип %4).';en='Invalid value for parameter%1 (Structure) in%2."
"Property%3 (type%4) was expected in the structure.'"), 
					ИмяПараметра, 
					ИмяПроцедурыИлиФункции, 
			...
			Проверить(
				ЭтоКорректныйТип, 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El significado inadmisible de la propiedad %1 en el parámetro %2 (la Estructura) en %3. "
"Se Espera: %4; es entregado el significado: %5 (tipo %6).';ru='Недопустимое значение свойства %1 в параметре %2 (Структура) в %3. "
"Ожидалось: %4; передано значение: %5 (тип %6).';en='Invalid value for property%1 in parameter%2 (Structure) in%3."
"Expected:%4; passed value:%5 (type%6). '"), 
					ОжидаемоеИмяСвойства,
					ИмяПараметра,
					ИмяПроцедурыИлиФункции,
					ПредставлениеТипов(ОжидаемыеТипы), 
	...
	Для Каждого Элемент Из Источник Цикл
		Если Заменять <> Истина И Приемник.Свойство(Элемент.Ключ) Тогда
			Если Заменять = Ложь Тогда
				Продолжить;
			Иначе
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La intersección de las llaves de la fuente y el receptor: ""%1"".';ru='Пересечение ключей источника и приемника: ""%1"".';en='Intersection of source and destination keys: ""%1"".'"), Элемент.Ключ);
			КонецЕсли
		КонецЕсли;
...
Функция СравнитьВерсии(Знач СтрокаВерсии1, Знач СтрокаВерсии2) Экспорт
	...
	Если Версия1.Количество() <> 4 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El formato incorrecto del parámetro StrokaVersii1: %1';ru='Неправильный формат параметра СтрокаВерсии1: %1';en='Incorrect format for СтрокаВерсии1: %1'"), СтрокаВерсии1);
	КонецЕсли;
	Версия2 = СтрРазделить(Строка2, ".");
	Если Версия2.Количество() <> 4 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	    	НСтр("es='El formato incorrecto del parámetro StrokaVersii2: %1';ru='Неправильный формат параметра СтрокаВерсии2: %1';en='Incorrect format for СтрокаВерсии2: %1'"), СтрокаВерсии2);
	КонецЕсли;
	...
	Для Каждого СтрокаАдреса Из МассивАдресов Цикл
		...
		Если Не АдресОпределен Тогда 
			Если ЕстьПризнакНаличияАдресаВСтроке Тогда 
				ОписаниеОшибки = НСтр("es='La dirección del correo electrónico contiene las faltas';ru='Адрес электронной почты содержит ошибки';en='Email address contains errors'");
			Иначе
				ОписаниеОшибки = НСтр("es='La línea no contiene la dirección del correo electrónico';ru='Строка не содержит адреса электронной почты';en='The line does not contain an email address'");
			КонецЕсли;
		КонецЕсли;
...
Функция РазобратьСтрокуСПочтовымиАдресами(Знач СтрокаПереданныхАдресов, ВызыватьИсключение = Истина) Экспорт
	...
	СообщениеНедопустимыеСимволы = НСтр("es='El símbolo inadmisible ""%1"" en la dirección del correo electrónico ""%2""';ru='Недопустимый символ ""%1"" в адресе электронной почты ""%2""';en=""Invalid character '%1' in email address '%2'""");
	СообщениеНеверныйФорматПочтовогоАдреса = НСтр("es='La dirección incorrecta del correo electrónico ""%1""';ru='Некорректный адрес электронной почты ""%1""';en=""Invalid email address '%1'""");
...
Процедура УстановитьСостояниеПоляТабличногоДокумента(ПолеТабличногоДокумента, Состояние = "НеИспользовать") Экспорт
	...
	Если ТипЗнч(ПолеТабличногоДокумента) = Тип("ПолеФормы") 
		...
		ИначеЕсли ВРег(Состояние) = "НЕАКТУАЛЬНОСТЬ" Тогда
			...
			ОтображениеСостояния.Картинка                       = Новый Картинка;
			ОтображениеСостояния.Текст                          = НСтр("es='El informe no es formado. Presionen ""Formar"" para la recepción del informe.';ru='Отчет не сформирован. Нажмите ""Сформировать"" для получения отчета.';en='The report is not generated. Click ""Generate"" to get a report.'");;
		ИначеЕсли ВРег(Состояние) = "ФОРМИРОВАНИЕОТЧЕТА" Тогда  
			ОтображениеСостояния.Видимость                      = Истина;
			ОтображениеСостояния.ДополнительныйРежимОтображения = ДополнительныйРежимОтображения.Неактуальность;
			ОтображениеСостояния.Картинка                       = БиблиотекаКартинок.ДлительнаяОперация48;
			ОтображениеСостояния.Текст                          = НСтр("es='El informe se forma...';ru='Отчет формируется...';en='Report is being formed ...'");
		Иначе
			ВызватьИсключение(НСтр("es=""El significado inadmisible del parámetro (el parámetro el número ' 2 ')"";ru=""Недопустимое значение параметра (параметр номер '2')"";en=""Invalid parameter value (parameter number '2')"""));
		КонецЕсли;
	Иначе
		ВызватьИсключение(НСтр("es=""El significado inadmisible del parámetro (el parámetro el número ' 1 ')"";ru=""Недопустимое значение параметра (параметр номер '1')"";en=""Invalid parameter value (parameter number '1')"""));
	КонецЕсли;
...
Функция УстановитьВнешнееСоединениеСБазой(Параметры) Экспорт
	...
	#Иначе
		...
		КраткоеОписаниеОшибки = НСтр("es='La conexión directa a la base informativa es accesible solamente sobre el cliente bajo la dirección la SO Windows.';ru='Прямое подключение к информационной базе доступно только на клиенте под управлением ОС Windows.';en='A direct connection to the infobase is available only on a client running Windows.'");
	#КонецЕсли
	...
	#Если Не МобильныйКлиент Тогда
		...
		Исключение
			...
			СтрокаСообщенияОбОшибке = НСтр("es='No consiguió conectarse a otro programa: %1';ru='Не удалось подключиться к другой программе: %1';en='Failed to connect to another program: %1'");
		...
		Если ФайловыйВариантРаботы Тогда
			...
			Если ПустаяСтрока(Параметры.КаталогИнформационнойБазы) Тогда
				СтрокаСообщенияОбОшибке = НСтр("es='de la base informativa.';ru='Не задано месторасположение каталога информационной базы.';en='The location of the infobase catalog is not specified.'");
		...
		Иначе
			...
			Если ПустаяСтрока(Параметры.ИмяСервера1СПредприятия) Или ПустаяСтрока(Параметры.ИмяИнформационнойБазыНаСервере1СПредприятия) Тогда
				СтрокаСообщенияОбОшибке = НСтр("es='de la conexión: ""el Nombre del servidor""; ""el Nombre de la base informativa en el servidor"".';ru='Не заданы обязательные параметры подключения: ""Имя сервера""; ""Имя информационной базы на сервере"".';en='Required connection parameters are not set: ""Server Name""; ""The name of the infobase on the server.'");
				ОшибкаПроверкиЗаполнения = Истина;
			КонецЕсли;
...
#Если ВебКлиент ИЛИ МобильныйКлиент Тогда
	ВызватьИсключение НСтр("es='El lanzamiento de los programas es inaccesible en el cliente web.';ru='Запуск программ недоступен в веб-клиенте.';en='Launching programs is not available in the web client.'");
#Иначе
...
#Если ВнешнееСоединение Тогда
		ВызватьИсключение 
			НСтр("es='El significado inadmisible del parámetro ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами. "
"La ahorcadura de los privilegios del sistema no es accesible de la unión exterior.';ru='Недопустимое значение параметра ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами."
"Повешение привилегий системы не доступно из внешнего соединения.';en='Invalid value of the parameter ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами. System privilege escalation is not available from an external connection.'");
#КонецЕсли
		
#Если Сервер Тогда
		ВызватьИсключение 
			НСтр("es='El significado inadmisible del parámetro ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами. "
"La ahorcadura de los privilegios del sistema no es accesible en el servidor.';ru='Недопустимое значение параметра ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами."
"Повешение привилегий системы не доступно на сервере.';en='Invalid value of the parameter ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами. System privilege escalation is not available on server.'");
#КонецЕсли
...
#Иначе
		...
		Если ВыполнитьСНаивысшимиПравами Тогда
			...
			Если ДождатьсяЗавершения Тогда
				ВызватьИсключение 
					НСтр("es='Es inadmisible al mismo tiempo establecer los parámetros"
" - ПараметрыЗапускаПрограммы.ДождатьсяЗавершения y"
" - ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами: "
"el sistema De operaciones no permite seguir en nombre del usuario los procesos "
"puestos en marcha por el administrador.';ru='Недопустимо одновременно устанавливать параметры "
...
"запущенные администратором.';en='Invalid to simultaneously set parameters"
"  - ПараметрыЗапускаПрограммы.ДождатьсяЗавершения and"
"  - ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами:"
"The operating system does not allow tracking processes on behalf of the user,"
...
#Если ВебКлиент Тогда
	ВызватьИсключение НСтр("es='La ejecución de la diagnosis de la unión es inaccesible en el cliente web.';ru='Выполнение диагностики соединения недоступно в веб-клиенте.';en='Performing connection diagnostics is not available in the web client.'");
#Иначе
	
	Описание = Новый Массив;
	Описание.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Al recurso por URL: %1';ru='При обращении по URL: %1';en='When accessed by URL:%1'"), 
		URL));
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		Описание.Добавить(
			НСтр("es='Se dirijan al administrador.';ru='Обратитесь к администратору.';en='Contact administrator.'"));
...
#КонецЕсли
	...
	Журнал = Новый Массив;
	Журнал.Добавить(
		НСтр("es='La revista de la diagnosis: "
"se Cumple la comprobación de la accesibilidad del servidor. "
"La descripción de la falta diagnosticada de cm. En el mensaje siguiente de la revista.';ru='Журнал диагностики:"
"Выполняется проверка доступности сервера."
"Описание диагностируемой ошибки см. в следующем сообщении журнала.';en='Diagnostic log:"
"A server availability check is in progress."
"See the next log message for a description of the diagnosed error. '"));
	Журнал.Добавить();
...
#КонецЕсли
	...
	Если СоединениеЧерезПрокси Тогда 
		...
		Описание.Добавить(
			НСтр("es='La diagnosis de la unión no es cumplida, porque es ajustado el proksi-servidor. "
"Se dirijan al administrador.';ru='Диагностика соединения не выполнена, т.к. настроен прокси-сервер."
"Обратитесь к администратору.';en='Connection diagnostics failed because a proxy server is configured."
"Contact the administrator.'"));
	...
	Иначе 
		...
		Если РезультатДоступностиРесурса.Доступен Тогда 
			...
			Описание.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es cumplido el recurso al recurso no existente en el servidor %1 "
"o han surgido los defectos en el servidor alejado.';ru='Выполнено обращение к несуществующему ресурсу на сервере %1"
"или возникли неполадки на удаленном сервере.';en='A call was made to a nonexistent resource on server% 1"
"or there is a problem on the remote server.'"),
				АдресСервераРесурса));
		...
		Иначе 
			...
			Если Не РезультатКонтрольнойПроверки.Доступен Тогда
				...
				Описание.Добавить(
					НСтр("es='Falta el acceso en la red el Internet por la causa: "
"- el ordenador no es conectado al Internet; "
"- el defecto al proveedor de Internet; "
"- la conexión al Internet es bloqueada межсетевой por la pantalla,"
" el programa antivírico u otro software.';ru='Отсутствует доступ в сеть интернет по причине:"
"- компьютер не подключен к интернету;"
"- неполадки у интернет-провайдера;"
"- подключение к интернету блокирует межсетевой экран, "
"  антивирусная программа или другое программное обеспечение.';en='There is no access to the Internet because:"
"- the computer is not connected to the Internet;"
"- problems with the Internet provider;"
"- an internet connection blocks the firewall,"
"   antivirus software or other software. '"));
				
			Иначе 
				
				Описание.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El servidor %1 no es accesible por la causa: "
"- el defecto al proveedor de Internet; "
"- la conexión al servidor es bloqueada межсетевой por la pantalla,"
" el programa antivírico u otro software; "
"- el servidor es desconectado o sobre el mantenimiento técnico.';ru='Сервер %1 не доступен по причине:"
"- неполадки у интернет-провайдера;"
"- подключение к серверу блокирует межсетевой экран, "
"  антивирусная программа или другое программное обеспечение;"
"- сервер отключен или на техническом обслуживании.';en='Server%1 is not available due to:"
"- problems with the Internet provider;"
"- connection to the server blocks the firewall,"
"   anti-virus program or other software;"
"- the server is down or on maintenance.'"),
					АдресСервераРесурса));
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	ЗаписьЖурналаРегистрации(
		НСтр("es='La diagnosis de la unión';ru='Диагностика соединения';en='Connection diagnostics'", КодОсновногоЯзыка()),
		...
		Для Каждого Элемент Из ОжидаемыеТипы Цикл
			...
			Если Индекс > 10 Тогда
				Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='%1... (Solamente %2 tipos)';ru='%1,... (всего %2 типов)';en='% 1, ... (total %2 types)'"), 
					Результат, 
...
Функция ПроверитьИПодготовитьПочтовыйАдрес(Знач ПолноеИмяАдресата, Знач ПочтовыйАдрес)
	...
	НедопустимыйСимволВИмениАдресата = НСтр("es='El símbolo inadmisible en el nombre del destinatario.';ru='Недопустимый символ в имени адресата.';en='Invalid character in destination name.'");
	НедопустимыйСимволВПочтовомАдресе = НСтр("es='El símbolo inadmisible en la dirección postal.';ru='Недопустимый символ в почтовом адресе.';en='Invalid character in the mailing address.'");
	СимволыОбрамления = "<>[]";
...
#Если Сервер Тогда
		ЗаписьЖурналаРегистрации(НСтр("es='La funcionalidad básica';ru='Базовая функциональность';en='Basic functionality'", КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Предупреждение,,, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió quitar el fichero temporal "
"%1 por la causa: %2';ru='Не удалось удалить временный файл"
"%1 по причине: %2';en='Failed to delete temporary file"
"%1 due to:%2'"), 
				ПолноеИмяФайла, 
...
#Иначе 
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La conexión es pasada en el ordenador (cliente) <%1>.';ru='Подключение проводится на компьютере (клиенте) <%1>.';en='Connection is made on the computer (client) <%1>.'"), ИмяКомпьютера());
#КонецЕсли
