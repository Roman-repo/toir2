...
Процедура УстановитьУсловноеОформлениеПоляДата(ЭтотОбъект, 
	...
	ЭлементОформления.Представление = НСтр("es='La representación de la fecha: ""10/06/2012""';ru='Представление даты: ""10.06.2012""';en='Date presentation: ""10/06/2012""'");
	...
	ЭлементОформления = УсловноеОформление.Элементы.Добавить();
	ЭлементОформления.Использование = Истина;
	ЭлементОформления.Представление = НСтр("es='La representación de la fecha hoy: ""09:46""';ru='Представление даты сегодня: ""09:46""';en='Presentation of the date today: ""09:46""'");
	...
	Если ВключитьВСостав.Количество() <> 0
		...
		Если ВключитьВСостав.Количество() <> 0 Тогда
			...
			ОписаниеИсключения1 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='En la composición del plan del cambio %1 deben entrar los objetos siguientes de los metadatos: %2';ru='В состав плана обмена %1 должны входить следующие объекты метаданных: %2';en='The%1 exchange plan must include the following metadata objects:%2'"),
				ИмяПланаОбмена,
		...
		Если ИсключитьИзСостава.Количество() <> 0 Тогда
			...
			ОписаниеИсключения2 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='En la composición del plan del cambio %1 no deben entrar los objetos siguientes de los metadatos: %2';ru='В состав плана обмена %1 НЕ должны входить следующие объекты метаданных: %2';en='The%1 sharing plan must NOT include the following metadata objects:%2'"),
				ИмяПланаОбмена,
		...
		Если ОтключитьАвторегистрацию.Количество() <> 0 Тогда
			...
			ОписаниеИсключения3 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Como parte del plan del cambio %1 no deben ser los objetos con el indicio establecido del autoregistro. "
"Es necesario prohibir el autoregistro para los objetos siguientes de los metadatos: %2';ru='В составе плана обмена %1 не должно быть объектов с установленным признаком авторегистрации."
"Требуется запретить авторегистрацию для следующих объектов метаданных: %2';en='The exchange plan%1 should not contain objects with the established sign of auto-registration."
"You want to disable auto-registration for the following metadata objects:%2'"),
				ИмяПланаОбмена,
...
Функция ПараметрыКлиентаНаСервере() Экспорт
	...
	Если ПараметрыКлиента.Количество() = 0
	   И ТекущийРежимЗапуска() <> Неопределено Тогда
		...
		ВызватьИсключение НСтр("es='el servidor.';ru='Не заполнены параметры клиента на сервере.';en='The client settings on the server are not populated.'");
	КонецЕсли;
...
Процедура ПотребоватьПерезапускСеансаПоПричинеДинамическогоОбновленияВерсииПрограммы() Экспорт
	...
	ТекстОшибки = НСтр("es='La versión del programa es renovada, es necesario перезапустить la sesión.';ru='Версия программы обновлена, требуется перезапустить сеанс.';en='The version of the program has been updated; a restart is required.'");
...
Процедура ВосстановитьПредопределенныеЭлементы() Экспорт
	...
	Если ПланыОбмена.ГлавныйУзел() <> Неопределено Тогда
		ВызватьИсключение НСтр("es='Debe cumplir la reconstitución de los elementos predestinados solamente en el nudo РИБ principal. "
"Luego cumplir la sincronización con los nudos subordinados.';ru='Восстановление предопределенных элементов следует выполнять только в главном узле РИБ."
"Затем выполнить синхронизацию с подчиненными узлами.';en='Restoring predefined items should only be done in the RIB main node."
"Then synchronize with the subordinate nodes.'");
	КонецЕсли;
...
Процедура ЗарегистрироватьОбъектНаВсехУзлах(Знач Объект, Знач ИмяПланаОбмена, Знач ВключаяГлавныйУзел = Истина) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
			ВызватьИсключение НСтр("es='El registro de los cambios de los datos no divididos en el régimen dividido.';ru='Регистрация изменений неразделенных данных в разделенном режиме.';en='Registration of changes in unshared data in split mode.'");
		КонецЕсли;
		...
		Если Не ЭтоРазделенныйПланОбмена Тогда
			ВызватьИсключение НСтр("es='El registro de los cambios para los planes no divididos del cambio no es apoyado.';ru='Регистрация изменений для неразделенных планов обмена не поддерживается.';en='Registration of changes for unshared exchange plans is not supported.'");
		КонецЕсли;
		...
		Если ЭтоРазделенныйОбъектМетаданных Тогда
				ВызватьИсключение НСтр("es='El registro de los cambios para los objetos divididos no es apoyado.';ru='Регистрация изменений для разделенных объектов не поддерживается.';en='Registration of changes for shared objects is not supported.'");
		КонецЕсли;
...
Функция УточнениеОшибкиПараметровРаботыПрограммыДляРазработчика() Экспорт
	...
	Возврат Символы.ПС + Символы.ПС 
		+ НСтр("es='Para el elaborador: es necesario renovar probablemente los datos auxiliares, "
"que influyen sobre el trabajo del programa. Para la ejecución de la renovación es posible: "
"- aprovecharse exterior por el tratamiento"
" ""los Instrumentos del elaborador: la Renovación de los datos auxiliares"", "
"- o poner en marcha el programa con el parámetro de la línea de mando 1С:Предприятия 8"
" ""/ con ZapustitObnovlenieInformatsionnoyBazy"", "
"- o aumentar el número de la versión de la configuración que durante el lanzamiento siguiente"
" se hayan cumplido los procedimientos de la renovación de los datos de la base informativa.';ru='Для разработчика: возможно требуется обновить вспомогательные данные,"
...
"  выполнились процедуры обновления данных информационной базы.';en='For the developer: it may be necessary to update supporting data,"
"which affect the operation of the program. To perform the update, you can:"
"- use external processing"
"   """" Developer Tools: Updating Support Data """","
"- either run the program with the command line parameter 1C: Enterprise 8"
"   ""/С ЗапуститьОбновлениеИнформационнойБазы"","
"- either increase the configuration version number so that at the next start"
"   the procedures for updating the data of the infobase were completed.'");
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешения.Добавить(МодульРаботаВБезопасномРежиме.РазрешениеНаИспользованиеКаталогаВременныхФайлов(Истина, Истина,
		НСтр("es='Para la posibilidad del trabajo del programa.';ru='Для возможности работы программы.';en='For the work possibility of the program.'")));
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Важное        = Истина;
	Дело.Представление = НСтр("es='La velocidad del trabajo es bajada';ru='Скорость работы снижена';en='Work speed ​​reduced'");
	Дело.Форма         = "Обработка.РекомендацияПоПовышениюСкоростиРаботы.Форма.Форма";
	Дело.Владелец      = НСтр("es='La velocidad del trabajo del programa';ru='Скорость работы программы';en='Program speed'");
...
Процедура ЗапретитьПометкуУдаленияПредопределенныхЭлементовПередЗаписью(Источник, Отказ) Экспорт
	...
	Если Источник.ЭтоНовый() Тогда
		ВызватьИсключение
			НСтр("es='Es inadmisible crear el elemento predestinado marcado en la desaparición.';ru='Недопустимо создавать предопределенный элемент помеченный на удаление.';en='Invalid to create a predefined item marked for deletion.'");
	Иначе
		ИмяРеквизита = "";
		...
		Если (СтарыеСвойства.ИмяПредопределенныхДанных <> "" Или ИмяРеквизита <> "" И СтарыеСвойства[ИмяРеквизита] <> "")
		   И СтарыеСвойства.ПометкаУдаления <> Истина Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es inadmisible marcar a la desaparición el elemento predestinado: "
"""%1"".';ru='Недопустимо помечать на удаление предопределенный элемент:"
"""%1"".';en='Invalid to flag a predefined item for deletion:"
"""%1"".'"),
...
Процедура ЗапретитьУдалениеПредопределенныхЭлементовПередУдалением(Источник, Отказ) Экспорт
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Es inadmisible quitar el elemento predestinado "
"""%1"".';ru='Недопустимо удалять предопределенный элемент"
"""%1"".';en='Invalid delete predefined item"
"""%1"".'"),
		Строка(Источник.Ссылка));
		...
		Если Счет.ВидыСубконто.Количество() > Индекс Тогда
			Если Счет.ВидыСубконто[Индекс].ВидСубконто <> Строка.ВидСубконто Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("es='El cambio para los datos. La desconexión del enlace con el nudo principal';ru='Обмен данными.Отключение связи с главным узлом';en='Data exchange. Disconnect from the host'",
						ОбщегоНазначения.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					Счет.Метаданные(),
					Счет,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Cerca de cuenta ""%1"" субконто № %2 ""%3"" no coincide con predestinado субконто ""%4"".';ru='У счета ""%1"" субконто №%2 ""%3"" не совпадает с предопределенным субконто ""%4"".';en='Account ""%1"", the sub-account №%2 ""%3"" does not match the predefined sub-account ""%4"".'"),
						Строка(Счет),
						Индекс + 1,
...
Процедура ПередЗапускомПрограммы()
	...
	Если Метаданные.ВариантВстроенногоЯзыка <> Метаданные.СвойстваОбъектов.ВариантВстроенногоЯзыка.Русский Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La variante de la lengua empotrada de la configuración ""%1"" no es apoyada. "
"Es necesario usar la variante de la lengua ""%2"".';ru='Вариант встроенного языка конфигурации ""%1"" не поддерживается."
"Необходимо использовать вариант языка ""%2"".';en='Embedded configuration language option ""% 1"" ""is not supported.'"),
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, МинимальнаяВерсияПлатформы) < 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La variante de la lengua empotrada de la configuración ""%1"" no es apoyada. "
"Es necesario usar la variante de la lengua ""%2"".';ru='Вариант встроенного языка конфигурации ""%1"" не поддерживается."
"Необходимо использовать вариант языка ""%2"".';en='Embedded configuration language option ""%1"" ""is not supported.'"), МинимальнаяВерсияПлатформы);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(НеПоддерживаемаяВерсияПлатформы) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El régimen de la compatibilidad de la configuración con 1С:Предприятием las versiones %1 no es apoyado. "
"Para el lanzamiento establezcan en la configuración el régimen de la compatibilidad ""no usar"" durante la elaboración en la versión %2 "
"(o ""la Versión %2"" durante la elaboración sobre unas versiones más mayores).';ru='Режим совместимости конфигурации с 1С:Предприятием версии %1 не поддерживается."
"Для запуска установите в конфигурации режим совместимости ""Не использовать"" при разработке на версии %2"
"(или ""Версия %2"" при разработке на более старших версиях).';en='Configuration compatibility mode with 1C: Enterprise version%1 is not supported."
"To start, set the ""Do not use"" compatibility mode in the configuration when developing on version %2"
"(or ""Version %2"" when developing on older versions).'"),
	...
	Если ПустаяСтрока(Метаданные.Версия) Тогда
		ВызватьИсключение НСтр("es='la Versión.';ru='Не заполнено свойство конфигурации Версия.';en='The configuration property Version is not populated.'");
	Иначе
		Попытка
			НулеваяВерсия = ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Метаданные.Версия, "0.0.0.0") = 0;
		Исключение
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='la Versión: ""%1"". "
"El formato correcto, por ejemplo: ""1.2.3.45"".';ru='Не правильно заполнено свойство конфигурации Версия: ""%1""."
"Правильный формат, например: ""1.2.3.45"".';en='Configuration property Version is not correctly populated:"" ""%1"" ""."
"The correct format, for example: """" 1.2.3.45 "".'"),
				Метаданные.Версия);
		КонецПопытки;
		Если НулеваяВерсия Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='la Versión: ""%1"". "
"La versión no puede ser de cero.';ru='Не правильно заполнено свойство конфигурации Версия: ""%1""."
"Версия не может быть нулевой.';en='Configuration property Version is not correctly populated:"" ""%1"" ""."
"Version cannot be zero. '"),
	...
	Если Не ТекущийПользовательИБ.АутентификацияСтандартная
	   ...
	   И Не ТекущийПользовательИБ.АутентификацияOpenID Тогда
		ВызватьИсключение НСтр("es='La entrada en el programa es prohibida. Se dirijan al administrador.';ru='Вход в программу запрещен. Обратитесь к администратору.';en='Entrance to the program is prohibited. Contact your administrator.'");
	КонецЕсли;
	...
	Если (Метаданные.ОсновныеРоли.Количество() <> 2 И Метаданные.ОсновныеРоли.Количество() <> 3)
	 ...
	 Или Не Метаданные.ОсновныеРоли.Содержит(Метаданные.Роли.ПолныеПрава) Тогда
		ВызватьИсключение
			НСтр("es='En la configuración en la propiedad de OsnovnyeRoli no son indicados los papeles estandartizados "
"de AdministratorSistemy y PolnyePrava o son indicados los papeles superfluos.';ru='В конфигурации в свойстве ОсновныеРоли не указаны стандартные роли"
"АдминистраторСистемы и ПолныеПрава или указаны лишние роли.';en='In the configuration, ОсновныеРоли property does not specify standard roles"
"АдминистраторСиситемы and ПолныеПрава or unnecessary roles specified.'");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПрофильИнформационнойБазы) Тогда
		...
		Если БезопасныйРежим() <> ПрофильИнформационнойБазы Тогда
			...
			Если Не ДоступенПривилегированныйРежим Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Es imposible establecer los parámetros de la sesión por la causa: el perfil de la seguridad %1 falta en el clúster de los servidores 1С:Предприятия, o para él es prohíbido el uso en calidad del perfil de la seguridad del régimen seguro. "
""
"La reconstitución de la capacidad de trabajo del programa tiene que desconectar el uso del perfil de la seguridad a través de la consola del clúster y de nuevo ajustar los perfiles de la seguridad por medio de la interfaz de la configuración (las órdenes correspondientes se encuentran en la sección de los ajustes del programa).';ru='Невозможно установить параметры сеанса по причине: профиль безопасности %1 отсутствует в кластере серверов 1С:Предприятия, или для него запрещено использование в качестве профиля безопасности безопасного режима."
""
"Для восстановления работоспособности программы требуется отключить использование профиля безопасности через консоль кластера и заново настроить профили безопасности с помощью интерфейса конфигурации (соответствующие команды находятся в разделе настроек программы).';en='Unable to set the session parameters due to: the security profile%1 is not in the server cluster 1C: Enterprise, or it is forbidden to use safe mode as the security profile."
""
"To restore the functionality of the program, you must disable the use of the security profile through the cluster console and re-configure the security profiles using the configuration interface (the corresponding commands are in the program settings section).'"),
					ПрофильИнформационнойБазы);
			КонецЕсли;
		...
		Если Не ДоступенПривилегированныйРежим Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es imposible establecer los parámetros de la sesión por la causa: el perfil de la seguridad %1 no contiene el permiso para la instalación del régimen privilegiado. Probablemente, él era redactado a través de la consola del clúster. "
""
"La reconstitución de la capacidad de trabajo del programa tiene que desconectar el uso del perfil de la seguridad a través de la consola del clúster y de nuevo ajustar los perfiles de la seguridad por medio de la interfaz de la configuración (las órdenes correspondientes se encuentran en la sección de los ajustes del programa).';ru='Невозможно установить параметры сеанса по причине: профиль безопасности %1 не содержит разрешения на установку привилегированного режима. Возможно, он был отредактирован через консоль кластера."
""
"Для восстановления работоспособности программы требуется отключить использование профиля безопасности через консоль кластера и заново настроить профили безопасности с помощью интерфейса конфигурации (соответствующие команды находятся в разделе настроек программы).';en='Unable to set session parameters due to: security profile%1 does not contain permission to set privileged mode. It may have been edited through the cluster console."
""
"To restore the functionality of the program, you must disable the use of the security profile through the cluster console and re-configure the security profiles using the configuration interface (the corresponding commands are in the program settings section).'"),
				ПрофильИнформационнойБазы);
	...
	Иначе
		...
		Исключение
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es imposible establecer los parámetros de la sesión por la causa: %1. "
""
"Probablemente, para la base informativa a través de la consola del clúster era establecido el perfil de la seguridad que no permite la ejecución los módulos exteriores sin instalación el régimen seguro. En este caso la reconstitución de la capacidad de trabajo del programa tiene que desconectar el uso del perfil de la seguridad a través de la consola del clúster y de nuevo ajustar los perfiles de la seguridad por medio de la interfaz de la configuración (las órdenes correspondientes se encuentran en la sección de los ajustes del programa). Además el programa será automáticamente correcto es ajustada al uso junto con los perfiles incluidos de la seguridad.';ru='Невозможно установить параметры сеанса по причине: %1."
""
"Возможно, для информационной базы через консоль кластера был установлен профиль безопасности, не допускающий выполнения внешних модулей без установки безопасного режима. В этом случае для восстановления работоспособности программы требуется отключить использование профиля безопасности через консоль кластера и заново настроить профили безопасности с помощью интерфейса конфигурации (соответствующие команды находятся в разделе настроек программы). При этом программа будет автоматически корректно настроена на использование совместно с включенными профилями безопасности.';en='Unable to set session parameters due to:%1."
""
"It is possible that a security profile has been set for the infobase through the cluster console, which does not allow the execution of external modules without setting the safe mode. In this case, to restore the program’s operability, you need to disable the use of the security profile through the cluster console and re-configure the security profiles using the configuration interface (the corresponding commands are in the program settings section). In this case, the program will be automatically correctly configured for use with the included security profiles'"),
