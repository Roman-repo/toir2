...
Функция ПараметрыРазбиенияСтроки() Экспорт
	...
	ПараметрыРазбиенияСтроки.Вставить("Заголовок", НСтр("es='Introduzcan la cantidad de la mercancía en una nueva línea';ru='Введите количество товара в новой строке';en='Enter the quantity of goods in a new line'"));
...
Процедура РазбитьСтрокуТЧ(ТЧ, ЭлементФормы, ПараметрыРазбиенияСтроки = Неопределено, ПотреблениеПоЗаказам = Ложь) Экспорт
	...
	Если ТекущаяСтрока = Неопределено Тогда
		ТекстСообщения = НСтр("es='Para la ejecución de la orden es necesario escoger la línea de la parte tabular.';ru='Для выполнения команды требуется выбрать строку табличной части.';en='To execute the command, you need to select the row of the tabular section.'");
	...
	ИначеЕсли ТекущаяСтрока[ПараметрыОбработки.ИмяПоляКоличество] = 0
		...
		ТекстСообщения = НСтр("es='Es imposible romper la línea con la cantidad de cero.';ru='Невозможно разбить строку с нулевым количеством.';en='Unable to split a row with a zero quantity.'");
		ПоказатьПредупреждение(,ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура РазбитьСтрокуТЧЗавершение(Число, ДопПараметры) Экспорт
	...
	Если Количество = 0
		И Не ПараметрыОбработки.РазрешитьНулевоеКоличество Тогда
		ТекстСообщения = НСтр("es='La cantidad en una nueva línea no puede ser igual al cero.';ru='Количество в новой строке не может быть равно нулю.';en='The number in the new line cannot be zero.'");
		
		ПоказатьПредупреждение(,ТекстСообщения);
		Отказ = Истина;	
	ИначеЕсли ТекущаяСтрока[ПараметрыОбработки.ИмяПоляКоличество] >= 0
		И Количество < 0 Тогда
		ТекстСообщения = НСтр("es='La cantidad en una nueva línea no puede ser negativa.';ru='Количество в новой строке не может быть отрицательным.';en='The number in the new line cannot be negative.'");
		ПоказатьПредупреждение(,ТекстСообщения);
		Отказ = Истина;	
	ИначеЕсли ТекущаяСтрока[ПараметрыОбработки.ИмяПоляКоличество] <= 0
		И Количество > 0 Тогда
		ТекстСообщения = НСтр("es='La cantidad en una nueva línea no puede ser positiva.';ru='Количество в новой строке не может быть положительным.';en='The number in the new line cannot be positive.'");
		ПоказатьПредупреждение(,ТекстСообщения);
		Отказ = Истина;	
	ИначеЕсли ТекущаяСтрока[ПараметрыОбработки.ИмяПоляКоличество] >= 0
		И Количество >  ТекущаяСтрока[ПараметрыОбработки.ИмяПоляКоличество] Тогда
		ТекстСообщения = НСтр("es='La cantidad en una nueva línea no puede ser más de cantidades en corriente.';ru='Количество в новой строке не может быть больше количества в текущей.';en='The number in the new line cannot be greater than the quantity in the current.'");
		ПоказатьПредупреждение(,ТекстСообщения);
		Отказ = Истина;	
	ИначеЕсли ТекущаяСтрока[ПараметрыОбработки.ИмяПоляКоличество] <= 0
		И Количество <  ТекущаяСтрока[ПараметрыОбработки.ИмяПоляКоличество] Тогда
		ТекстСообщения = НСтр("es='La cantidad en una nueva línea no puede ser menos de cantidades en corriente.';ru='Количество в новой строке не может быть меньше количества в текущей.';en='The number in the new line cannot be less than the quantity in the current.'");
		ПоказатьПредупреждение(,ТекстСообщения);
		Отказ = Истина;
	ИначеЕсли Количество =  ТекущаяСтрока[ПараметрыОбработки.ИмяПоляКоличество]
		И Не ПараметрыОбработки.РазрешитьНулевоеКоличество Тогда
		ТекстСообщения = НСтр("es='La cantidad en una nueva línea debe distinguirse de la cantidad en corriente.';ru='Количество в новой строке должно отличаться от количества в текущей.';en='The number in the new line must be different from the quantity in the current.'");
		ПоказатьПредупреждение(,ТекстСообщения);
		Отказ = Истина;
	КонецЕсли;
