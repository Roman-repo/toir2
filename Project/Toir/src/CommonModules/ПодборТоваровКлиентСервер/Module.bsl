...
Процедура УстановитьТекстИнформационнойНадписи(Форма) Экспорт
	...
	Количество = НРег(ЧислоПрописью(Корзина.Количество(), "Л = ru_RU; ДП = Ложь; НП = Истина; НД = Ложь;", НСтр("es='La posición, la posición, las posiciones, 0';ru='позиция,позиции,позиций,ж,,,,,0';en='position, position, position, w ,,,,, 0'")));
	
	ИнформационнаяНадпись = НСтр("es='Es recogida en total [la Cantidad] de las mercancías, a la suma [Suma] [Divisa] [СкрытьПоказать]';ru='Всего подобрано [Количество] товаров, на сумму [Сумма] [Валюта] [СкрытьПоказать]';en='Total selected [Количество] of goods, amounting to [Сумма] [Валюта] [СкрытьПоказать]'");
	...
	Если Не ЭтоФормаПомощникаПродаж Тогда
		Если Форма.РежимПодбораБезКоличественныхПараметров Или Форма.РежимПодбораБезСуммовыхПараметров Тогда
			ИнформационнаяНадпись = НСтр("es='Es recogida en total [la Cantidad] de las mercancías [СкрытьПоказать]';ru='Всего подобрано [Количество] товаров [СкрытьПоказать]';en='Total selected [Количество] items [СкрытьПоказать]'");
		КонецЕсли;
		
		СкрытьПоказать = ?(Форма.ПоказыватьПодобранныеТовары, НСтр("es='(Esconder)';ru='(скрыть)';en='(hide)'"), НСтр("es='(Mostrar)';ru='(показать)';en='(show)'"));
	КонецЕсли;
...
Процедура УстановитьТекстНадписиОтображатьОстатки(Форма) Экспорт
	...
	Форма.ОтображатьОстаткиНадпись = ?(Форма.Элементы.ОстаткиТоваров.Видимость, 
		НСтр("es='La información detallada sobre los restos de (esconder)';ru='Подробная информация об остатках (скрыть)';en='Details of balances (hide)'"), 
		НСтр("es='La información detallada sobre los restos de (mostrar)';ru='Подробная информация об остатках (показать)';en='Details of balances (show)'"));
...
Процедура СформироватьИнформационнуюНадписьПодобранныхТоваров(ИнформационнаяНадпись, КоличествоТоваров, СуммаТоваров, Валюта) Экспорт
	...
	ИнформационнаяНадпись = НСтр("es='Es recogido en total las mercancías [КоличествоТоваров] a la suma [СуммаТоваров] [la Divisa]';ru='Всего подобрано товаров [КоличествоТоваров] на сумму [СуммаТоваров] [Валюта]';en='Total selected products [КоличествоТоваров] for the amount of [СуммаТоваров] [Валюта]'");
...
Процедура УстановитьЗаголовокФормыПодбора(Форма) Экспорт
	...
	Иначе
		ТекстДокумент = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='%1 (nuevo)';ru='%1 (новый)';en='%1 (new)'"), Документ.Метаданные().Синоним);
...
Функция СформироватьЗаголовокФормыПодбора(Заголовок, Документ) Экспорт
	...
	Иначе
		...
		ТекстДокумент = НСтр("es='%1 (nuevo)';ru='%1 (новый)';en='%1 (new)'");
