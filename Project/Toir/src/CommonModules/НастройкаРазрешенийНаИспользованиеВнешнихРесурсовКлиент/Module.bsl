...
Процедура ПослеЗавершенияЗапросаРазрешенийНаИспользованиеВнешнихРесурсов(Результат, Состояние) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("es='El ajuste de los permisos';ru='Настройка разрешений';en='Permission settings'"),,
			НСтр("es='Son hechos los cambios en los ajustes de los perfiles de la seguridad en el clúster de los servidores.';ru='Внесены изменения в настройки профилей безопасности в кластере серверов.';en='Changes have been made to the security profile settings in the server cluster.'"));
