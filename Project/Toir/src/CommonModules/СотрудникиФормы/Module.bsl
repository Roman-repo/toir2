...
Процедура УстановитьПараметрыИнфоНадписиФормыФизЛица(Форма) Экспорт
	...
	Если Форма.ИмеютсяАктуальныеСотрудники  Тогда
		...
		Форма.ИнфоНадписьМестаРаботы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='%1 es a nuestro empleado. Son más abajo llevados los lugares del trabajo. Para el tránsito a la información completa sobre el lugar del trabajo usen la referencia correspondiente. Para el relleno de la información sobre un nuevo lugar del trabajo del empleado usen la referencia ""un Nuevo lugar del trabajo""';ru='%1 является нашим сотрудником. Ниже приведены места работы. Для перехода к полной информации о месте работы используйте соответствующую ссылку. Для заполнения информации о новом месте работы сотрудника используйте ссылку ""Новое место работы""';en='%1 is our employee. Below are the places of work. To go to full information about the place of work, use the appropriate link. To fill out information about the new employee’s place of work, use the link “New place of work”'"),
	...
	ИначеЕсли Форма.ИмеютсяАктуальныеСотрудникиНеОтображаемые Тогда
		...
		Форма.ИнфоНадписьМестаРаботы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='%1 es a nuestro empleado. Para el relleno de la información sobre el lugar del trabajo, el puesto y el sueldo del empleado usen la referencia ""un Nuevo lugar del trabajo""';ru='%1 является нашим сотрудником. Для заполнения информации о месте работы, должности и окладе сотрудника используйте ссылку ""Новое место работы""';en='%1 is our employee. To fill out information about the place of work, position and salary of an employee, use the link ""New place of work""'"),
			Форма.ФизическоеЛицоСсылка);
	...
	Иначе
		...
		Форма.ИнфоНадписьМестаРаботы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='%1 no es a nuestro empleado. Para el relleno de la información sobre el lugar del trabajo, el puesto y el sueldo del empleado usen la referencia ""un Nuevo lugar del trabajo""';ru='%1 не является нашим сотрудником. Для заполнения информации о месте работы, должности и окладе сотрудника используйте ссылку ""Новое место работы""';en='%1 is not our employee. To fill out information about the place of work, position and salary of an employee, use the link ""New place of work""'"),
			Форма.ФизическоеЛицоСсылка);
...
Процедура ЗаписатьФизическоеЛицоСотрудника(Форма) Экспорт
	...
	Если Форма.ФизическоеЛицоЗаблокировано ИЛИ Форма.ФизическоеЛицо.Ссылка.Пустая() Тогда
		...
		Если Форма.ФизическоеЛицо.Ссылка.Пустая() И НЕ ПравоНаДобавлениеИзменениеФизическихЛиц Тогда
			ВызватьИсключение НСТР("es='No hay derechos a la anotación de la nueva persona física. El empleado no es anotado.';ru='Нет прав на запись нового физического лица. Сотрудник не записан.';en='There is no right to record a new individual. The employee is not recorded.'");
		КонецЕсли;
...
Процедура ЗаполнитьФормуМестамиРаботы(Форма, ИмяГруппы, ИмяПоследнегоЭлементаГруппы = "", СотрудникИсключение = Неопределено, ВыводитьПодробнуюИнформацию = Ложь) Экспорт
	...
	МассивСтрокСПустойОрганизацией = ТаблицаПоРабочимМестам.НайтиСтроки(Новый Структура("Организация", Справочники.Организации.ПустаяСсылка()));
...
Процедура ДобавитьЭлементыФормыДляМестаРаботыПодробно(Форма, ДанныеСотрудника, РеквизитыОрганизации, НомерСотрудника, ИмяГруппы, ИмяПоследнегоЭлементаГруппы = "")
	...
	РеквизитИнфоНадписьОрганизация.Заголовок = НСтр("es='La organización';ru='Организация';en='Organization'");
	...
	РеквизитИнфоНадписьДолжность.Заголовок = НСтр("es='El puesto';ru='Должность';en='Position'");
	...
	РеквизитИнфоНадписьОклад.Заголовок =  НСтр("es='El sueldo';ru='Оклад';en='Salary'");
...
Процедура ДобавитьКомандуДругиеМестаРаботы(Форма, ИмяГруппы, ВыводитьПодробнуюИнформацию = Ложь)
	...
	Команда.Заголовок = НСтр("es='Otros lugares del trabajo';ru='Другие места работы';en='Other places of work'");
	Команда.Отображение  =  ОтображениеКнопки.Авто;	
...
Функция ТекстСообщенияОКонфликтеВидаЗанятости(ВидЗанятости, Организация, КонфликтныйСотрудникОрганизация = Неопределено, КонфликтныйСотрудникВидЗанятости = Неопределено, КонфликтныйСотрудникТабельныйНомер = "", КонфликтныйСотрудникДолжность = Неопределено)
	ТекстСообщения = "";
	Если ВидЗанятости = Перечисления.ВидыЗанятости.ОсновноеМестоРаботы Тогда
		ТекстСообщения =  НСтр("es='No es posible formalizar al empleado por el lugar básico del trabajo, puesto que él trabaja ya en %1 %2, Tab. El número: %3, %4.';ru='Нельзя оформить сотрудника по основному месту работы, так как он уже работает в %1 %2, Таб. номер: %3, %4.';en='The employee cannot be registered at the main place of work, since he already works in%1%2, Tab. number:%3,%4.'");
	ИначеЕсли ВидЗанятости = Перечисления.ВидыЗанятости.Совместительство Тогда
		ТекстСообщения =  НСтр("es='No es posible formalizar al empleado a la simultaneidad de cargos exterior, puesto que él trabaja ya en %1 %2, Tab. El número: %3, %4.';ru='Нельзя оформить сотрудника на внешнее совместительство, так как он уже работает в %1 %2, Таб. номер: %3, %4.';en='Can not apply for an employee for external part-time job, as he already works in%1%2, Tab. number:%3,%4.'");
	ИначеЕсли ВидЗанятости = Перечисления.ВидыЗанятости.ВнутреннееСовместительство Тогда
		ТекстСообщения = НСтр("es='No es posible formalizar al empleado a la simultaneidad de cargos interior, puesto que él no trabaja en %1.';ru='Нельзя оформить сотрудника на внутреннее совместительство, так как он не работает в %1.';en='The employee cannot be registered for internal part-time job, as he does not work in%1.'");
	КонецЕсли;
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ТекстСообщения,
		?(Организация <> КонфликтныйСотрудникОрганизация И КонфликтныйСотрудникОрганизация <> Неопределено, КонфликтныйСотрудникОрганизация, НСтр("es='De nuestra organización';ru='нашей организации';en='our organization'")),
		?(КонфликтныйСотрудникВидЗанятости = Перечисления.ВидыЗанятости.ОсновноеМестоРаботы, НСтр("es='Por el lugar básico del trabajo';ru='по основному месту работы';en='at the main place of work'"), НСтр("es='Como el pluralista exterior';ru='как внешний совместитель';en='as an external combiner'")),
