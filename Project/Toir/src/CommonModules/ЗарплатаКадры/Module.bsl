...
Функция ТекстЗапросаВТПериоды(Знач ОписаниеНачалаИнтервала, Знач ОписаниеОкончанияИнтервала, Периодичность = "МЕСЯЦ", ИмяПоляПериод = "Период", ИмяВТ = "ВТПериоды", ИспользоватьКонецПериода = Ложь) Экспорт
	...
	Если ВРег(Периодичность) <> "ГОД"
		...
		ВызватьИсключение НСтр("es='Es imposible formar la interpelación con los parámetros entregados.';ru='Невозможно сформировать запрос с переданными параметрами.';en='Unable to form a request with the passed parameters.'");
...
Процедура СоздатьВТПериоды(МенеджерВременныхТаблиц, НачалоИнтервала, ОкончаниеИнтервала, Периодичность = "МЕСЯЦ", ИмяПоляПериод = "Период", ИмяВТ = "ВТПериоды", ИспользоватьКонецПериода = Ложь) Экспорт
	...
	Если НачалоИнтервала > ОкончаниеИнтервала Тогда
		ВызватьИсключение НСтр("es='La fecha de la terminación no puede ser menos de fechas del comienzo.';ru='Дата окончания не может быть меньше даты начала.';en='The end date cannot be less than the start date.'")
	КонецЕсли;
...
Процедура ЗаписатьФайлВАрхив(ВладелецФайла, Знач ИмяФайла, ТекстФайла, РасширениеЗаглавнымиБуквами = Истина) Экспорт
	...
	Иначе
		...
		ПрисоединенныйФайл = РаботаСФайлами.ДобавитьФайл(ПараметрыФайла, АдресВременногоХранилищаФайла, "");
...
Функция ПолучитьДанныеФайла(СсылкаНаОбъект, УникальныйИдентификатор = Неопределено, ПолучатьСсылкуНаДвоичныеДанные = Истина) Экспорт
	...
	Возврат РаботаСФайлами.ДанныеФайла(СсылкаНаФайл, УникальныйИдентификатор, ПолучатьСсылкуНаДвоичныеДанные);	
КонецФункции
		...
		Если НЕ ОтключитьПроверкуЗанятости Тогда
				...
				Если СтрокаДанных.ОформленПриемНаРаботу Тогда
					...
					Если ЗначениеЗаполнено(СтрокаДанных.ДатаУвольнения) Тогда
						...
						ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='El empleado ""%1"" era admitido al trabajo con %2, es despedido %3';ru='Сотрудник ""%1"" был оформлен на работу с %2, уволен %3';en='Employee ""%1"" was hired from%2, fired%3'"),
							ДанныеЗаполнения,
					...
					Иначе
						...
						Если ЗначениеЗаполнено(СтрокаДанных.ДатаПриема) Тогда
							...
							ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='El empleado ""%1"" es admitido ya al trabajo con %2';ru='Сотрудник ""%1"" уже оформлен на работу с %2';en=""Employee '%1' is already hired from%2"""),
								ДанныеЗаполнения,
								Формат(СтрокаДанных.ДатаПриема, "ДЛФ=DD"));
						...
						Иначе
							...
							ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='El empleado ""%1"" es admitido ya al trabajo';ru='Сотрудник ""%1"" уже оформлен на работу';en=""Employee '%1' is already hired"""),
								ДанныеЗаполнения);
			...
			Иначе
				Если НЕ ЗначениеЗаполнено(СтрокаДанных.ГоловнаяОрганизация) Тогда
					...
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El empleado ""%1"" no es admitido al trabajo';ru='Сотрудник ""%1"" не оформлен на работу';en=""Employee '%1' not hired"""),
						ДанныеЗаполнения);
					
				ИначеЕсли ЗначениеЗаполнено(СтрокаДанных.ДатаУвольнения) Тогда
						
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El empleado ""%1"" es despedido %2';ru='Сотрудник ""%1"" уволен %2';en=""Employee '%1' fired%2"""),
						ДанныеЗаполнения,
						Формат(СтрокаДанных.ДатаУвольнения, "ДЛФ=DD"));
...
Функция АдресаОрганизаций(Организации) Экспорт
	...
	Пока ВыборкаОрганизаций.Следующий() Цикл
		...
		Пока ВыборкаПоВидам.Следующий() Цикл
			...
			АдресСтруктура = РаботаСАдресами.ПредыдущаяСтруктураКонтактнойИнформацииXML(
...
Процедура СформироватьОбластьТабличногоДокументаСОграниченнымНаборомСтрок(ТабличныйДокумент, ДанныеЗаполнения, ОграничениеСтрок,
	...
	Если КоличествоВыведенныхСтрок > ОграничениеСтрок Тогда
		...
		ТекстНадписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La continuación de la tabla de cm. En el complemento al documento № %1.';ru='Продолжение таблицы см. в дополнении к документу №%1.';en='For the continuation of the table, see the supplement to document №%1.'"),
	...
	Если СтруктураПроверяемыхДанных.Свойство("ИНН") Тогда 
		...
		Если Не ПустаяСтрока(СтруктураПроверяемыхДанных.ИНН) И НЕ РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(СтруктураПроверяемыхДанных.ИНН, Ложь, ТекстСообщения) Тогда
			Если ВыводитьСообщения Тогда
				ШаблонСообщения = Нстр("es='El empleado %1: %2';ru='Сотрудник %1: %2';en='Employee%1:%2'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ДанныеДляПроверки.СотрудникНаименование, ТекстСообщения);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ОткрываемыйОбъект, "ИНН", , Отказ);
			Иначе
				Отказ = Истина;
			КонецЕсли;	
		КонецЕсли;	
...
Процедура ПроверитьДатыРождения(Дата, СтруктураПроверяемыхДанных, СоответствиеРеквизитовОткрываемымОбъектам, ДанныеДляПроверки, ВыводитьСообщения = Истина, Отказ = Ложь)
	...
	Если СтруктураПроверяемыхДанных.Свойство("ДатаРожденияВСвидетельстве") Тогда
		...
		Если ЗначениеЗаполнено(СтруктураПроверяемыхДанных.ДатаРожденияВСвидетельстве) И СтруктураПроверяемыхДанных.ДатаРожденияВСвидетельстве > Дата Тогда
			Если ВыводитьСообщения Тогда
				ШаблонСообщения = НСтр("es='El empleado %1: ¡la fecha, indicada en el certificado, del nacimiento no puede ser más de fechas de la presentación de las noticias!';ru='Сотрудник %1: указанная в свидетельстве дата рождения не может быть больше даты подачи сведений!';en='Employee%1: the date of birth indicated in the certificate cannot be more than the date of submission of information!'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ДанныеДляПроверки.СотрудникНаименование);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ОткрываемыйОбъект, "ДатаРождения",,Отказ);
			Иначе
				Отказ = Истина;
			КонецЕсли;	
		КонецЕсли;	
	...
	Если СтруктураПроверяемыхДанных.Свойство("ДатаРождения") Тогда
		...
		Если ЗначениеЗаполнено(СтруктураПроверяемыхДанных.ДатаРождения) И СтруктураПроверяемыхДанных.ДатаРождения > Дата Тогда
			Если ВыводитьСообщения Тогда
				ШаблонСообщения = НСтр("es='El empleado %1: ¡la fecha del nacimiento no puede ser más de fechas de la presentación de las noticias!';ru='Сотрудник %1: дата рождения не может быть больше даты подачи сведений!';en='Employee%1: date of birth cannot be greater than the filing date!'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ДанныеДляПроверки.СотрудникНаименование);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ОткрываемыйОбъект, "ДатаРождения",,Отказ);
			Иначе
				Отказ = Истина;
	...
	Для Каждого КлючЗначение Из СтруктураПроверяемыхДанных Цикл
		...
		Если СоответствиеПутейИИменПроверяемыхРеквизитов.Свойство(КлючЗначение.Ключ) Тогда
			Если Не ЗначениеЗаполнено(КлючЗначение.Значение) Тогда
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El empleado %1: %2';ru='Сотрудник %1: %2';en='Employee%1:%2'"), ДанныеДляПроверки.СотрудникНаименование, ПредставленияПроверяемыхРеквизитов[КлючЗначение.Ключ]);
...
Процедура ПроверитьЗаполнениеСтраховыхНомеровПФР(СтруктураПроверяемыхДанных, СоответствиеРеквизитовОткрываемымОбъектам, ДанныеДляПроверки, ВыводитьСообщения = Истина, Отказ = Ложь)
	...
	Если СтруктураПроверяемыхДанных.Свойство("СтраховойНомерПФР") Тогда
		...
		Если ЗначениеЗаполнено(СтруктураПроверяемыхДанных.СтраховойНомерПФР) И (Не РегламентированныеДанныеКлиентСервер.СтраховойНомерПФРСоответствуетТребованиям(СтруктураПроверяемыхДанных.СтраховойНомерПФР, ТекстСообщения)) Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ДанныеДляПроверки.СотрудникНаименование, ТекстСообщения);		
			Если ВыводитьСообщения Тогда
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ОткрываемыйОбъект, "СтраховойНомерПФР",,Отказ);
			Иначе
				Отказ = Истина;
			КонецЕсли;	
		КонецЕсли;
	...
	Если СтруктураПроверяемыхДанных.Свойство("МестоРождения") Тогда
		...
		Если ЗначениеЗаполнено(ДанныеДляПроверки.МестоРождения) Тогда 
			Если СокрЛП(СтрЗаменить(ДанныеДляПроверки.МестоРождения,",","")) = "" 
				...
				ШаблонСообщения = НСтр("es='El empleado %1: ¡no es rellenado el lugar de nacimiento!';ru='Сотрудник %1: не заполнено место рождения!';en='Employee%1: birthplace not filled!'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ДанныеДляПроверки.СотрудникНаименование);
				Если ВыводитьСообщения Тогда
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ОткрываемыйОбъект, "ФизическоеЛицоМестоРождения", "ЭтаФорма",Отказ);
				Иначе
					Отказ = Истина;
				КонецЕсли;	
			КонецЕсли; 
	...
	Если ЗначениеЗаполнено(АдресРегистрации) Тогда 
		...
		Если МассивОшибок.Количество() <> 0 Тогда
			Если ВыводитьСообщения Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El empleado %1: - el campo ""la Dirección del registro"":';ru='Сотрудник %1: - поле ""Адрес регистрации"":';en='Employee%1: - field ""Registration address"":'"), ДанныеДляПроверки.СотрудникНаименование);
				ТекстСообщения = ТекстСообщения + Символы.ПС + СообщенияПроверки;
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,ОткрываемыйОбъект, ,,Отказ);
			Иначе
				Отказ = Истина;
			КонецЕсли;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(СтруктураПроверяемыхДанных.АдресФактический) Тогда 
			...
			Если МассивОшибок.Количество() <> 0 Тогда
				Если ВыводитьСообщения Тогда
					...
					КонецЦикла;
					СтроковыеФункцииКлиентСервер.УдалитьПоследнийСимволВСтроке(СообщенияПроверки, 1);
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El empleado %1: - el campo ""la dirección Real"":';ru='Сотрудник %1: - поле ""Фактический адрес"":';en='Employee%1: - field ""Actual address"":'"), ДанныеДляПроверки.СотрудникНаименование);
					ТекстСообщения = ТекстСообщения + Символы.ПС + СообщенияПроверки;
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ОткрываемыйОбъект, ,,Отказ);
				Иначе
					Отказ = Истина;
		...
		Если ЗначениеЗаполнено(СтруктураПроверяемыхДанных.АдресДляИнформирования) Тогда 
			...
			Если МассивОшибок.Количество() <> 0 Тогда
				Если ВыводитьСообщения Тогда
					...
					КонецЦикла;
					СтроковыеФункцииКлиентСервер.УдалитьПоследнийСимволВСтроке(СообщенияПроверки, 1);
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El empleado %1: - el campo ""la Dirección para la información"":';ru='Сотрудник %1: - поле ""Адрес для информирования"":';en='Employee%1: - field ""Information address"":'"), ДанныеДляПроверки.СотрудникНаименование);
					ТекстСообщения = ТекстСообщения + Символы.ПС + СообщенияПроверки;
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ОткрываемыйОбъект, ,,Отказ);
				Иначе
					Отказ = Истина;
...
Процедура ИзменитьНаименованиеФизическихЛиц(ДанныеФИО)
	...
	Пока ДанныеФИО.Следующий() Цикл
		...
		Исключение
			...
			НСтр("es='No consiguió cambiar el nombre de la persona física %1. "
"Probablemente, los datos del empleado son redactados por otro usuario';ru='Не удалось изменить имя физического лица %1."
"Возможно, данные сотрудника редактируются другим пользователем';en='Failed to change the name of the individual %1."
"The employee data may be being edited by another user.'"),
...
Процедура ИзменитьНаименованиеСотрудников(ДанныеФИО)
	...
	Пока ДанныеФИО.Следующий() Цикл
		...
		Исключение
			Отказ = Истина;
			ТекстИсключенияЗаписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió cambiar el nombre del empleado %1. "
"Probablemente, los datos del empleado son redactados por otro usuario';ru='Не удалось изменить имя сотрудника %1."
"Возможно, данные сотрудника редактируются другим пользователем';en='Failed to change employee name %1."
"The employee data may be being edited by another user.'"),
