...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			"webits.1c.ru",
			443,
			НСтр("es='El servicio 1S-Takskom';ru='Сервис 1С-Такском';en='Service 1C-Taxkom'"));
	...
	Если Результат = Неопределено Тогда
		...
		Если ПараметрыИнтернетПоддержки.РаботаВМоделиСервиса Тогда
			...
			Результат = Новый Структура("Действие,Сообщение",
				"ПоказатьСообщение",
				НСтр("es='El uso del Apoyo De Internet de los usuarios es inaccesible en el trabajo en el modelo del servicio.';ru='Использование Интернет-поддержки пользователей недоступно при работе в модели сервиса.';en='The use of online user support is not available when working in the service model.'"));

		ИначеЕсли НЕ ПараметрыИнтернетПоддержки.ЗапускРазрешен Тогда

			Результат = Новый Структура("Действие,Сообщение",
				"ПоказатьСообщение",
				НСтр("es='Es insuficientemente derecho para el lanzamiento del Apoyo De Internet de los usuarios. Se dirijan al administrador.';ru='Недостаточно прав для запуска Интернет-поддержки пользователей. Обратитесь к администратору.';en='Not enough rights to run online user support. Contact your administrator.'"));
...
Функция НовыйОписаниеСервисаИПП(МестоположениеWSDL = Неопределено, Таймаут = -1) Экспорт
	...
	Если НЕ ПустаяСтрока(ПроксиСервиса.КодОшибки) Тогда
		ТекстИсключения = НСтр("es='La falta a la conexión al servicio del Apoyo De Internet.';ru='Ошибка при подключении к сервису Интернет-поддержки.';en='Error connecting to online support service.'") + " "
...
Функция НовыйОпределенияСервиса(
	...
	Если НЕ ПустаяСтрока(ОписаниеWSDL.КодОшибки) Тогда
		...
		Результат.ИнформацияОбОшибке =
			НСтр("es='La falta a la recepción de la WSDL-descripción.';ru='Ошибка при получении WSDL-описания.';en='Error getting WSDL description.'") + " "
	...
	Исключение
		...
		Результат.СообщениеОбОшибке  = НСтр("es='La WSDL-descripción incorrecta del servicio.';ru='Некорректное WSDL-описание сервиса.';en='Invalid WSDL service description.'");
		Результат.ИнформацияОбОшибке =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta al tratamiento de la WSDL-descripción %1. %2';ru='Ошибка при обработке WSDL-описания %1. %2';en='Error while processing the WSDL description%1. %2'"),
				МестоположениеWSDL,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат Результат;
	...
	Если УзлыДирективыИмпорта.Количество() > 0 Тогда
		...
		Для каждого УзелИмпорта Из УзлыДирективыИмпорта Цикл
			...
			Если ПустаяСтрока(Location) Тогда
				...
				Результат.СообщениеОбОшибке  = НСтр("es='La WSDL-descripción incorrecta del servicio.';ru='Некорректное WSDL-описание сервиса.';en='Invalid WSDL service description.'");
				Результат.ИнформацияОбОшибке =
					НСтр("es='Falta location para la instrucción import.';ru='Отсутствует location для директивы import.';en='Missing location for import directive.'");
				Возврат Результат;
			КонецЕсли;
	...
	Для каждого УзелТипов Из УзлыОписанияТипов Цикл
		...
		Для каждого УзелСхемы Из УзлыСхем Цикл
			...
			Исключение
				...
				Результат.ИнформацияОбОшибке =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La falta al tratamiento de la WSDL-descripción %1. La falta de la construcción del esquema de los datos. %2';ru='Ошибка при обработке WSDL-описания %1. Ошибка построения схемы данных. %2';en='Error while processing the WSDL description%1. Error building data schema. %2'"),
						МестоположениеWSDL,
						ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				Возврат Результат;
	...
	Иначе
		...
		Иначе
			...
			Исключение
				...
				Результат.ИнформацияОбОшибке =
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La falta al tratamiento de la WSDL-descripción %1. La falta a la creación de la fábrica XDTO del servicio. %2';ru='Ошибка при обработке WSDL-описания %1. Ошибка при создании фабрики XDTO сервиса. %2';en='Error while processing the WSDL description%1. Error creating factory XDTO service. %2'"),
						МестоположениеWSDL,
						ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				Возврат Результат;
...
Функция НовыйПроксиВебСервиса(
	...
	Если НЕ ПустаяСтрока(ОпределениеСервиса.КодОшибки) Тогда
		...
		Результат.ИнформацияОбОшибке =
			НСтр("es='La falta a la creación de la descripción del servicio web.';ru='Ошибка при создании описания веб-сервиса.';en='Error creating web service description.'") + " "
	...
	Если НайденныйПорт = Неопределено Тогда
		...
		Результат.СообщениеОбОшибке = НСтр("es='del servicio.';ru='Не найдено определение сервиса.';en='No service definition found.'");
		Результат.ИнформацияОбОшибке =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta a la creación de la descripción del servicio web %1. La definición del servicio no es encontrada. "
"URI los espacios de los nombres: %2; "
"el Nombre del servicio: %3; "
"el Nombre del punto de la conexión: %4';ru='Ошибка при создании описания веб-сервиса %1. Определение сервиса не найдено."
"URI пространства имен: %2;"
"Имя сервиса: %3;"
"Имя точки подключения: %4';en='Error creating web service description%1. Service definition not found."
"Namespace URI:%2;"
"Service name:%3;"
...
Функция СервисИПП_isConfigurationSupported(ИмяКонфигурации, ОписаниеСервисаИПП)
	...
	Если ТипЗначенияСвойства = Неопределено Тогда
		ТекстИсключения = СтрЗаменить(НСтр("es='La falta con la llamada de la operación isConfigurationSupported del servicio (%1). "
"No consiguió determinar el tipo de la propiedad radical isConfigurationSupported.';ru='Ошибка при вызове операции isConfigurationSupported сервиса (%1)."
"Не удалось определить тип корневого свойства isConfigurationSupported.';en='Error while calling isConfigurationSupported service operation (%1)."
"Failed to determine root type of isConfigurationSupported property. '"),
			"%1",
	...
	Если НЕ ПустаяСтрока(ОписаниеОтветаSOAP.КодОшибки) Тогда
		ТекстИсключения = СтрЗаменить(НСтр("es='La falta con la llamada de la operación isConfigurationSupported del servicio (%1):';ru='Ошибка при вызове операции isConfigurationSupported сервиса (%1):';en='An error occurred while calling the operation isConfigurationSupported service (%1):'"),
			"%1",
			ОписаниеСервисаИПП.МестоположениеWSDL)
	...
	Если ТипОбъекта = Неопределено Тогда
		ТекстИсключения = СтрЗаменить(НСтр("es='La falta con la llamada de la operación isConfigurationSupported del servicio (%1). "
"No consiguió determinar el tipo de la propiedad radical isConfigurationSupportedResponse.';ru='Ошибка при вызове операции isConfigurationSupported сервиса (%1)."
"Не удалось определить тип корневого свойства isConfigurationSupportedResponse.';en='Error while calling isConfigurationSupported service operation (%1)."
"Failed to determine root type of isConfigurationSupportedResponse property.'"),
			"%1",
			ОписаниеСервисаИПП.МестоположениеWSDL);
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
...
Функция СервисИПП_process(ПараметрыЗапроса, ОписаниеСервисаИПП, Таймаут = -1)
	...
	Если НЕ ПустаяСтрока(ОписаниеОтветаSOAP.КодОшибки) Тогда
		ТекстИсключения = СтрЗаменить(НСтр("es='La falta con la llamada de la operación process del servicio (%1):';ru='Ошибка при вызове операции process сервиса (%1):';en='An error occurred while invoking the process service operation (%1):'"),
			"%1",
			ОписаниеСервисаИПП.МестоположениеWSDL)
			+ " " + ОписаниеОтветаSOAP.ИнформацияОбОшибке;
		ВызватьИсключение ТекстИсключения;
	...
	Если ТипОбъекта = Неопределено Тогда
		ТекстИсключения = СтрЗаменить(НСтр("es='La falta con la llamada de la operación process del servicio (%1). "
"No consiguió determinar el tipo de la propiedad radical processResponse.';ru='Ошибка при вызове операции process сервиса (%1)."
"Не удалось определить тип корневого свойства processResponse.';en='An error occurred while invoking the process service operation (%1)."
"Failed to determine root type of processResponse.'"),
			"%1",
			ОписаниеСервисаИПП.МестоположениеWSDL);
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
...
Функция ПрочитатьОтветВКонвертеSOAP(ТелоОтвета, ОписаниеСервисаИПП, ТипЗначения)
	...
	Исключение
		...
		Результат.СообщениеОбОшибке  = НСтр("es='El formato equivocado de la respuesta.';ru='Неверный формат ответа.';en='Invalid response format.'");
		Результат.ИнформацияОбОшибке =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El formato equivocado de la respuesta SOAP. %1';ru='Неверный формат ответа SOAP. %1';en='Invalid SOAP response format. %1'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат Результат;
	...
	Если ЧтениеОтвета.ТипУзла = ТипУзлаXML.НачалоЭлемента
		...
		Результат.Ошибка = Истина;
		Результат.СообщениеОбОшибке  = НСтр("es='La falta del SOAP-servidor.';ru='Ошибка SOAP-сервера.';en='SOAP server error.'");
		Результат.ИнформацияОбОшибке =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta del SOAP-servidor. %1';ru='Ошибка SOAP-сервера. %1';en='SOAP server error. %1'"),
...
Функция ОписаниеИсключенияSOAPВСтроку(ИсключениеSOAP)
	...
	Если НЕ ПустаяСтрока(ИсключениеSOAP.FaultActor) Тогда
		Результат = Результат + ?(ПустаяСтрока(Результат), "", Символы.ПС + НСтр("es='La fuente de la falta:';ru='Источник ошибки:';en='Source of error:'") + " ")
...
Функция СтруктурироватьОтветСервера(
	...
	Исключение
		...
		КонтекстОбработчика.ПользовательскоеОписаниеОшибки =
			НСтр("es='La falta desconocida. Véase los detalles en la revista del registro.';ru='Неизвестная ошибка. См. подробности в журнале регистрации.';en='Unknown error. See details in the logbook.'");
...
Функция СтруктурироватьСообщениеИлиВопросПользователю(КомандаСервера)
	...
	Если НЕ СтруктураКоманды.Свойство("Заголовок") ИЛИ ПустаяСтрока(СтруктураКоманды.Заголовок) Тогда
		СтруктураКоманды.Вставить("Заголовок", НСтр("es='El apoyo de Internet de los usuarios';ru='Интернет-поддержка пользователей';en='Online user support'"));
	КонецЕсли;
...
Функция ПараметрыВнутреннейФормы(ИндексФормы, МестоЗапуска)
	...
	ИначеЕсли ИндексФормы = "bh1" Тогда
		ПараметрыФормы.Вставить("Заголовок", НСтр("es='El apoyo de Internet de los usuarios';ru='Интернет-поддержка пользователей';en='Online user support'"));
...
Функция СтруктурироватьЗаписьАдресногоКлассификатора(КомандаСервера)
		...
		Для каждого Параметр Из ПараметрыСписокСтран Цикл
			...
			Если ИмяТекПараметра = "country" Тогда
				...
				Если ИдентификаторСтраны <> Неопределено Тогда
					...
					СписокРегионовСтраны.Вставить(0, "-1", НСтр("es='<no es escogido>';ru='<не выбран>';en='<не выбран>'"));
	...
	СписокСтран.Вставить(0, "-1", НСтр("es='<no es escogido>';ru='<не выбрана>';en='<не выбрана>'"));
...
Функция СтруктурироватьЗаписьОрганизацийПользователя(КомандаСервера)
	...
	СписокОрганизаций.Вставить(0, "-1", НСтр("es='<añadir la nueva organización>';ru='<добавить новую организацию>';en='<добавить новую организацию>'"));
...
Процедура ДобавитьКомандыСервиса(
		...
		Для каждого ПередаваемыйПараметр Из ПередаваемыеПараметрыЗапроса Цикл
			...
			Иначе
				...
				Исключение
					...
					ТекстИсключения = СтрЗаменить(НСтр("es='La falta a la transformación de los datos entregados. %1';ru='Ошибка при преобразовании передаваемых данных. %1';en='Error converting transmitted data. %1'"),
						"%1",
						ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
					ВызватьИсключение ТекстИсключения;
				КонецПопытки;
	...
	Если МассивСтруктурыКоманд = Неопределено ИЛИ МассивСтруктурыКоманд.Количество() = 0 Тогда
		ВызватьИсключение НСтр("es='La respuesta vacía del servidor.';ru='Пустой ответ сервера.';en='Empty server response.'");
	КонецЕсли;
...
Процедура ОпределитьВозможностьЗапуска(МестоЗапуска, ПараметрыИнтернетПоддержки, ОписаниеДействия)
	...
	Если НЕ ПараметрыИнтернетПоддержки.ИспользоватьПодключение1СТакском Тогда
		...
		ОписаниеДействия.Вставить("Сообщение",
			НСтр("es='El uso de la conexión 1S-Takskom es inaccesible en el régimen corriente del trabajo.';ru='Использование подключения 1С-Такском недоступно в текущем режиме работы.';en='Using 1C-Taxcom connection is not available in the current operating mode.'"));
		Возврат;
