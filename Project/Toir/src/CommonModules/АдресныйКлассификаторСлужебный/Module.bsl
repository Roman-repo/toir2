...
Функция ОписаниеПоследнейЗагрузки(ИдентификаторРегиона = Неопределено) Экспорт
	...
	Иначе
		...
		Результат.Представление        = НСтр("es='El clasificador de domicilios no era cargado.';ru='Адресный классификатор еще не загружался.';en='The address classifier has not yet loaded.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если ДнейРазницы = 0 Тогда
		Представление = НСтр("es='El clasificador de domicilios era cargado hoy.';ru='Адресный классификатор был загружен сегодня.';en='The address classifier was uploaded today.'");
		
	ИначеЕсли ДнейРазницы = 1 Тогда
		Представление = НСтр("es='El clasificador de domicilios era cargado ayer.';ru='Адресный классификатор был загружен вчера.';en='The address classifier was uploaded yesterday.'");
			
	ИначеЕсли ДнейРазницы = 2 Тогда
		Представление = НСтр("es='El clasificador de domicilios era cargado anteayer.';ru='Адресный классификатор был загружен позавчера.';en='The address classifier was loaded the day before yesterday.'");
			
	ИначеЕсли ДнейРазницы > 2 Тогда
		Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El clasificador de domicilios %1 era cargado atrás.';ru='Адресный классификатор был загружен %1 назад.';en='The address classifier was loaded%1 ago.'"),
...
Функция АдресаДляИнтерактивногоВыбора(Родитель, Уровень, ТипАдреса, ДополнительныеПараметры) Экспорт
	...
	Если Уровень = 1 Тогда
		Результат.Заголовок = НСтр("es='Escojan la región';ru='Выберите регион';en='Choose region'");
	ИначеЕсли Уровень = 2 Тогда
		Результат.Заголовок = НСтр("es='Escojan el distrito';ru='Выберите округ';en='Choose county'");
	ИначеЕсли Уровень = 3 Тогда
		Результат.Заголовок = НСтр("es='Escojan la región';ru='Выберите район';en='Choose area'");
	ИначеЕсли Уровень = 4 Тогда
		Результат.Заголовок = НСтр("es='Escojan la ciudad';ru='Выберите город';en='Choose a city'");
	ИначеЕсли Уровень = 5 Тогда
		Результат.Заголовок = НСтр("es='Escojan la región intramuros';ru='Выберите внутригородской район';en='Choose an intracity district'");
	ИначеЕсли Уровень = 6 Тогда
		Результат.Заголовок = НСтр("es='Escojan la localidad';ru='Выберите населенный пункт';en='Select location'");
	ИначеЕсли Уровень = 7 Тогда
		Результат.Заголовок = НСтр("es='Escojan la calle';ru='Выберите улицу';en='Choose a street'");
	ИначеЕсли Уровень = 65 Тогда
		Результат.Заголовок = НСтр("es='Escojan el territorio';ru='Выберите территорию';en='Choose territory'");
	ИначеЕсли Уровень = 41 Тогда
		Результат.Заголовок = НСтр("es='Escojan la colonia';ru='Выберите поселение';en='Choose a settlement'");
	ИначеЕсли Уровень = 31 Тогда
		Результат.Заголовок = НСтр("es='Escojan la región municipal';ru='Выберите муниципальный район';en='Choose a municipal area'");
	КонецЕсли;
...
Функция КодыАдреса(Знач Адрес, Источник = Неопределено) Экспорт
	...
	Иначе
		ОписаниеПричиныОшибки = НСтр("es='Es entregado el significado inadmisible de la dirección a la función de la recepción de los códigos de la dirección';ru='Передано недопустимое значение адреса в функцию получения кодов адреса';en='Invalid address value passed to the function of receiving address codes'");
	...
	Попытка
		...
		Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка,
					,, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No consiguió cargar las noticias de domicilios del sujeto de la Federación Rusa: %1 %2 "
"por la causa: %3';ru='Не удалось загрузить адресные сведения субъекта РФ: %1 %2"
"по причине: %3';en='Failed to load the address information of the subject of the Russian Federation:%1%2"
"due to:%3'"), СубъектРФ, НаименованиеРегиона, ОписаниеОшибки()));
				ВызватьИсключение;
			КонецПопытки;
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Описание = НСтр("es='La carga de las renovaciones del clasificador de domicilios.';ru='Загрузка обновлений адресного классификатора.';en='Download address classifier updates.'");
	...
	Описание = НСтр("es='El servicio Web del clasificador de domicilios.';ru='Веб-сервис адресного классификатора.';en='Address classifier web service.'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("es='El clasificador de domicilios ha caducado';ru='Адресный классификатор устарел';en='Address classifier is out of date'");
		Дело.Количество     = 0;
		Дело.Подсказка      = НСтр("es='La autoselección y la comprobación de la corrección de las direcciones son temporalmente inaccesibles.';ru='Автоподбор и проверка корректности адресов временно недоступны.';en='Auto-matching and address validation are temporarily unavailable.'");
	...
	Подсказка = Подсказка + Символы.ПС + НСтр("es='Es necesario comprobar la presencia de las renovaciones.';ru='Необходимо проверить наличие обновлений.';en='Required to heck for updates.'");
	Для Каждого Раздел Из Разделы Цикл
		Дело = ТекущиеДела.Добавить();
...
Функция РаспознатьАдресВебСервис(ЧастиАдреса, Представление, ВСтруктуру)
	...
	Иначе
		Если РезультатВебСервис.Данные.status = "PARTIALLY_RECOGNIZED" И ВСтруктуру Тогда
			Адрес = АдресДляСохраненияВБазе(РезультатВебСервис.Данные.value, Ложь);
			Адрес.comment = НСтр("es='La parte no discernida de la dirección';ru='Нераспознанная часть адреса';en='Unrecognized part of the address'") + ": " + РезультатВебСервис.Данные.unrecognizedPart;
		Иначе
			Адрес = ОписаниеНовойКонтактнойИнформации();
...
Процедура ЗаполнитьРезультатПроверкиАдресаПоКлассификаторуСервис1С(Результат, Знач АдресаДляПроверки)
	...
	Исключение
		...
		Результат.КраткоеПредставлениеОшибки = НСтр("es='A la recepción de los datos del servicio web han surgido los problemas.';ru='При получении данных из веб-сервиса возникли проблемы.';en='There was a problem retrieving data from the web service.'");
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка,,, Результат.ПодробноеПредставлениеОшибки);
		Возврат;
	КонецПопытки;
	...
	Иначе
		...
		Если РезультатПроверки.КодСостояния = 400 Тогда
			...
			ПредставлениеОшибки = НСтр("es='La comprobación de la dirección no es cumplida. Algunos campos de la dirección contienen las noticias incorrectas.';ru='Проверка адреса не выполнена. Некоторые поля адреса содержат некорректные сведения.';en='Address checking failed. Some address fields contain incorrect information.'");
			
		Иначе
			
			ПредставлениеОшибки = НСтр("es='La comprobación de la dirección no es accesible por la causa: "
"- no es conectado el apoyo de Internet de los usuarios; "
"- el defecto al proveedor de Internet; "
"- la conexión al servidor es bloqueada межсетевой por la pantalla,"
" el programa antivírico u otro software; "
"- el servicio web es desconectado o sobre el mantenimiento técnico.';ru='Проверка адреса не доступна по причине:"
...
"- веб-сервис отключен или на техническом обслуживании.';en='Address checking is not available due to:"
"- Internet user support is not connected;"
"- problems with the Internet provider;"
"- connection to the server blocks the firewall,"
"   anti-virus program or other software;"
"- The web service is disabled or on maintenance.'");
...
Процедура ОповеститьОПрогрессе(Знач НаименованиеРегиона, Знач СубъектРФ, Знач ОсталосьДляЗагрузки)
	...
	Если ОсталосьДляЗагрузки > 0 Тогда
		ШаблонСообщения = НСтр("es='La carga de la región ""%1 - %2""... (Se quedó %3)';ru='Загрузка региона ""%1 - %2""... (осталось %3)';en='Loading region ""% 1 -% 2"" ... (% 3 left)'");
		ТекстОсталосьЗагрузить = Формат(ОсталосьДляЗагрузки, "ЧН=");
	Иначе
		ШаблонСообщения = НСтр("es='La carga de la región ""%1 - %2""';ru='Загрузка региона ""%1 - %2""';en='Loading region ""% 1 -% 2""'") + "...";
		...
		Если ВРег(Прав(ПолноеИмяФайла, 4)) = ".ZIP" Тогда
			...
			Если ФайлыНовогоФИАС.Количество() = 0 Тогда
				...
				Если ФайлыСтарогоФИАС.Количество() > 0 Тогда
					...
					ТекстПричины = НСтр("es='No se permite la carga de las noticias de domicilios sin apoyo de las direcciones municipales.';ru='Не допускается загрузка адресных сведений без поддержки муниципальных адресов.';en='It is not allowed to download address information without the support of municipal addresses.'");
					
				Иначе
					
					ТекстПричины = НСтр("es='El fichero no contiene las noticias ФИАС de domicilios';ru='Файл не содержит адресных сведений ФИАС';en='File does not contain FIAS address information'");
					
				КонецЕсли;
				
				ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка,
				,, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No consiguió cargar las noticias de domicilios del fichero: %1 "
"por la causa: %2 "
"es necesario usar las noticias de domicilios situado a su destino https://its.1c.ru/download/fias2';ru='Не удалось загрузить адресные сведения из файла: %1"
"по причине: %2"
"Необходимо использовать адресные сведения расположенные по адресу https://its.1c.ru/download/fias2';en='Failed to load address information from file:%1"
"due to:%2"
"You must use the address information located at https://its.1c.ru/download/fias2'"), ПолноеИмяФайла, ТекстПричины));
...
Процедура ОпределитьИдентификаторыАдресаСервис1С(Результат, АдресДляПроверки)
	...
	Иначе
		ВызватьИсключение НСтр("es='Las noticias sobre la información de contacto eran estropeadas o rellenadas';ru='Сведения о контактной информации были повреждены или некорректно заполнены';en='Contact information was damaged or incorrectly filled'");
	КонецЕсли;
...
Функция АнализОдногоАдресаПоКлассификатору(Адрес)
	...
	Если СтрСравнить("ВСвободнойФорме", Адрес.AddressType) = 0 Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", 
			НСтр("es='La dirección no puede ser comprobada, puesto que él es introducido en la forma libre';ru='Адрес не может быть проверен, так как он введен в свободной форме';en='The address could not be verified, as it was entered in free form'") );
		Возврат Результат;
	КонецЕсли;
	...
	Если ПустаяСтрока(Адрес.Area) Тогда
		// Нет субъекта
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", НСтр("es='de la dirección';ru='Не указан регион адреса';en='Address region not specified'"));
		Возврат Результат;
	КонецЕсли;
	...
	Если РегионЗагруженВАдресныеСведения.КодСубъектаРФ = Неопределено Тогда 
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Es indicada la región no existente ""%1""';ru='Указан несуществующий регион ""%1""';en='Specified region ""%1"" does not exist'"), Адрес.Area + " " + Адрес.AreaType));
		Результат.АдресПроверен = Истина;
		Возврат Результат;
	ИначеЕсли Не РегионЗагруженВАдресныеСведения.Загружен Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ",
			НСтр("es='Faltan las noticias de domicilios sobre la región';ru='Отсутствуют адресные сведения по региону';en='Missing address information for the region'") + ": " + Адрес.Area + " " + Адрес.AreaType);
	...
	Для каждого АдресСтрока Из Адреса.Выгрузить() Цикл
		...
		Если КакиеУровниПроверять["Distict"] <> Неопределено И АдресСтрока["Наименование" + КакиеУровниПроверять["Distict"]] = NULL Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СвРайМО/Район",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La región ""%1"" falta en ФИАС';ru='Район ""%1"" отсутствует в ФИАС';en='Area ""%1"" is missing from FIAS'"), Адрес.District + " " + Адрес.DistrictType));
		КонецЕсли;
		
		Если НЕ ДанныеУровняКорректные(Адрес, "Area", НСтр("es='El sujeto de la Federación Rusa';ru='Субъект РФ';en='The subject of the Russian Federation'"), АдресСтрока, КакиеУровниПроверять, ОшибкиПроверки) Тогда
			Продолжить;
		КонецЕсли;
			
		Если НЕ ДанныеУровняКорректные(Адрес, "MunDistrict", НСтр("es='La región municipal';ru='Муниципальный район';en='Municipal district'"), АдресСтрока, КакиеУровниПроверять, ОшибкиПроверки) Тогда
			Продолжить;
		КонецЕсли;
		
		Если НЕ ДанныеУровняКорректные(Адрес, "Distict", НСтр("es='La región';ru='Район';en='Area'"), АдресСтрока, КакиеУровниПроверять, ОшибкиПроверки) Тогда
			Продолжить;
		КонецЕсли;
			
		Если НЕ ДанныеУровняКорректные(Адрес, "Settlement", НСтр("es='La colonia';ru='Поселение';en='Settlement'"), АдресСтрока, КакиеУровниПроверять, ОшибкиПроверки) Тогда
			Продолжить;
		КонецЕсли;
			
		Если НЕ ДанныеУровняКорректные(Адрес, "City", НСтр("es='La ciudad';ru='Город';en='City'"), АдресСтрока, КакиеУровниПроверять, ОшибкиПроверки) Тогда
			Продолжить;
		КонецЕсли;
		
		Если НЕ ДанныеУровняКорректные(Адрес, "CityDistrict", НСтр("es='La región intramuros';ru='Внутригородской район';en='Intracity district'"), АдресСтрока, КакиеУровниПроверять, ОшибкиПроверки) Тогда
			Продолжить;
		КонецЕсли;
		
		Если НЕ ДанныеУровняКорректные(Адрес, "Territory", НСтр("es='El territorio';ru='Территория';en='Territory'"), АдресСтрока, КакиеУровниПроверять, ОшибкиПроверки) Тогда
			Продолжить;
		КонецЕсли;
		
		Если НЕ ДанныеУровняКорректные(Адрес, "Locality", НСтр("es='La localidad';ru='Населенный пункт';en='Locality'"), АдресСтрока, КакиеУровниПроверять, ОшибкиПроверки) Тогда
			Продолжить;
		КонецЕсли;
		
		Если НЕ ДанныеУровняКорректные(Адрес, "Street", НСтр("es='La calle';ru='Улица';en='Street'"), АдресСтрока, КакиеУровниПроверять, ОшибкиПроверки) Тогда
			Продолжить;
		КонецЕсли;
	...
	Если СписокКорректныхАдресов.Количество() = 0 Тогда
		...
		Если ЗначениеЗаполнено(ИсторическийАдрес) Тогда
			...
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Представление",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La dirección incorrecto, porque él ha caducado "
"Actual: %1';ru='Адрес некорректный, т.к. он устарел"
"Актуальный: %1';en='The address is incorrect because it is out of date"
"Current:% 1'"), ИсторическийАдрес));
		КонецЕсли;
	...
	Если НЕ ДомаКорректный Тогда
		...
		Если ПохожиеВарианты.Количество() > 0 Тогда
			...
			Если ПохожиеВарианты.Количество() = 1 Тогда
				...
				ТекстПодсказки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Pero es %1';ru='но есть %1';en='but there is%1'"), ПервыйВариант);
				
			Иначе
				
				Если ПохожиеВарианты.Количество() = 2 Тогда
					
					ШаблонПодсказки = НСтр("es='Pero es %1 o %2';ru='но есть %1 или %2';en='but there is %1 or %2'");
					ВторойВариант = ПохожиеВарианты[1].Дом;
					
				Иначе
					
					ШаблонПодсказки = НСтр("es='Pero es %1 y otras variantes (%2)';ru='но есть %1 и другие варианты (%2)';en='but there are %1 and other options (%2)'");
		...
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, ПолеСОшибкой,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 falta en ФИАС%2';ru='%1 отсутствует в ФИАС%2';en='% 1 is missing from FIAS% 2'"),
	...
	Если СтрСравнить(ПочтовыйИндекс, Адрес.ZipCode) <> 0 Тогда
		ДополнительныйТекст = ?(ЗначениеЗаполнено(ПочтовыйИндекс), " " + НСтр("es='Probablemente, el índice correcto';ru='Возможно, правильный индекс';en='Perhaps the correct index'") + " "  + ПочтовыйИндекс, "");
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Индекс",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El índice ""%1"" no corresponde a la dirección. El %2';ru='Индекс ""%1"" не соответствует адресу.%2';en='Index ""%1"" does not match address.%2'"), Адрес.ZipCode, ДополнительныйТекст));
	КонецЕсли;
...
Функция ДанныеУровняКорректные(Знач Адрес, Знач ИмяУровня, Знач ПредставлениеУровня, Знач АдресСтрока, Знач КакиеУровниПроверять, Знач ОшибкиПроверки)
	...
	Если КакиеУровниПроверять[ИмяУровня] <> Неопределено И ПустаяСтрока(АдресСтрока["НаименованиеУровень" + КакиеУровниПроверять[ИмяУровня]]) Тогда
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 ""%2"" falta en ФИАС';ru='%1 ""%2"" отсутствует в ФИАС';en='% 1 ""% 2"" is missing from FIAS'"), ПредставлениеУровня,
...
Функция ПараметрыАутентификацииНаСайте() Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("es='El servicio del apoyo de Internet de los usuarios no es conectado.';ru='Сервис интернет-поддержки пользователей не подключен.';en='Online user support service is not connected.'");
	КонецЕсли;
...
Функция СтруктураОписанияОшибкиПоставщика(Описание = Неопределено, ИнформацияОбОшибке = Неопределено)
	...
	Если ИнформацияОбОшибке = Неопределено Тогда
		Описание.ПодробноеПредставлениеОшибки = НСтр("es='La información sobre la falta falta';ru='Информация об ошибке отсутствует';en='No error information'");
		Возврат Описание;
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("es='El clasificador de domicilios';ru='Адресный классификатор';en='Address classifier'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПредставлениеРегиона(Наименование, Сокращение)
	Если Сокращение = НСтр("es='Респ';ru='Респ';en='Republic'") Тогда
		Возврат НСтр("es='La república';ru='Республика';en='Republic'") + " " + Наименование;
	ИначеЕсли Сокращение = НСтр("es='обл';ru='обл';en='area'") Тогда
		Возврат Наименование + " " + НСтр("es='La esfera';ru='область';en='area'");
	ИначеЕсли Сокращение = НСтр("es='г';ru='г';en='г'") Тогда
		Возврат НСтр("ru='г.';en='г.'") + " " + Наименование;
	КонецЕсли;
...
Процедура ОчиститьКлассификаторАдресов(КодыСубъектовРФ)
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		НомерПоПорядку = НомерПоПорядку + 1;
		ДлительныеОперации.СообщитьПрогресс(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La limpieza de la región ""%1 - %2"" (se quedó %3)...';ru='Очистка региона ""%1 - %2"" (осталось %3) ...';en='Clearing region ""%1 -%2"" (%3 left) ...'"),
...
Процедура ЗагрузитьАдресныеОбъекты(КодСубъектаРФ, КаталогФайлов, ЗагружатьОднимНабором)
	...
	Если ФайлЗагрузки = Неопределено Тогда
		ВызватьИсключение НСтр("es='La carga de las noticias de domicilios es interrumpida. En los ficheros cargados no es encontrado el fichero con las noticias de domicilios.';ru='Загрузка адресных сведений прервана. В загружаемых файлах не найден файл с адресными сведениями.';en='Address information download aborted. The file with the address information was not found in the downloaded files.'");
		Возврат;
	КонецЕсли;
...
Функция ФайлЗагрузкиСубъектаРФ(КодСубъектаРФ, КаталогФайлов, КлючВида, ИмяОписывающегоУзла)
	...
	Если ТипXDTOЗаписи = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='En el fichero ""%1"" no es encontrada la lista de las anotaciones en el tipo %2';ru='В файле ""%1"" не найден список записей в типе  %2';en=""List of records in type% 2 not found in file '% 1'"""), Результат.ПолноеИмя, Результат.ТипXDTO);
	КонецЕсли;
	...
	Если НЕ КореньНайден Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='En el fichero ""%1"" no es encontrado el nudo radical %2';ru='В файле ""%1"" не найден корневой узел %2';en=""No root node% 2 found in file '% 1'"""), Результат.ПолноеИмя, ИмяОписывающегоУзла);
	КонецЕсли;
...
Процедура ОбработатьОшибкуОбращенияКВебСервису(Результат, Знач Ответ, ИнформацияОбОшибке, ТекстURL)
	...
	Если ПустаяСтрока(ПредставлениеОшибки) Тогда
		ПредставлениеОшибки = НСтр("es='El servicio Web para el trabajo con las direcciones no es accesible por la causa: "
"- no es conectado el apoyo de Internet de los usuarios; "
"- el defecto al proveedor de Internet; "
"- la conexión al servidor es bloqueada межсетевой por la pantalla,"
" el programa antivírico u otro software; "
"- el servicio es desconectado o sobre el mantenimiento técnico.';ru='Веб-сервис для работы с адресами не доступен по причине:"
"- не подключена интернет-поддержка пользователей;"
"- неполадки у интернет-провайдера;"
"- подключение к серверу блокирует межсетевой экран, "
"  антивирусная программа или другое программное обеспечение;"
"- сервис отключен или на техническом обслуживании.';en='The web service for working with addresses is not available due to:"
"- Internet user support is not connected;"
"- problems with the Internet provider;"
"- connection to the server blocks the firewall,"
"   anti-virus program or other software;"
"- the service is disabled or on maintenance.'");
	КонецЕсли;
...
Процедура ЗаписатьОшибкуЗапросаКВебСервисуВЖурналРегистрации(ОтветОтСервераСтрокой, Результат, ТекстURL, КодСостояния)
	...
	ШаблонТекстОтветаСервера = НСтр("es='%1 "
"información Técnica. A la interpelación: %2 "
"servidor ha devuelto HTTP el Código: %3 "
"con el texto: %4';ru='%1"
"Техническая информация. При запросе: %2"
"сервер вернул HTTP Код: %3"
"с текстом: %4';en='%1"
"Technical information. On request: %2"
...
Функция ТипыОбъектовАдресацииАдресаРФ()
	...
	// Уточняющие объекты
	Результат.Добавить(СтрокаОбъектаАдресации("10100000", НСтр("es='El índice postal';ru='Почтовый индекс';en='Postcode'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10200000", НСтр("es='El punto de domicilios';ru='Адресная точка';en='Address point'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10300000", НСтр("es='La sociedad de jardín';ru='Садовое товарищество';en='Garden partnership'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10400000", НСтр("es='El elemento de la red ulichno-de camino, la estructura de planificación del elemento adicional de domicilios';ru='Элемент улично-дорожной сети, планировочной структуры дополнительного адресного элемента';en='Element of the road network, planning structure of the additional address element'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10500000", НСтр("es='La zona industrial';ru='Промышленная зона';en='Industrial zone'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10600000", НСтр("es='La cooperativa garazhno-de construcción';ru='Гаражно-строительный кооператив';en='Garage building society'")));
	Результат.Добавить(СтрокаОбъектаАдресации("10700000", НСтр("es='El territorio';ru='Территория';en='Territory'")));
...
Функция КодСериализацииПочтовогоИндекса()
	...
	Возврат КодСериализацииОбъектаАдресации(НСтр("es='El índice postal';ru='Почтовый индекс';en='Postcode'"));
...
Функция КонтактнаяИнформацияВФорматеКлючЗначениеВJSON(Адрес)
	...
	ВызватьИсключение НСтр("es='La dirección entregada no puede ser transformada en las funciones АдресныйКлассификаторСлужебный.КодыАдреса.';ru='Переданный адрес не может быть преобразован в функции АдресныйКлассификаторСлужебный.КодыАдреса.';en='The transmitted address cannot be converted to functions АдресныйКлассификаторСлужебный.КодыАдреса.'");
