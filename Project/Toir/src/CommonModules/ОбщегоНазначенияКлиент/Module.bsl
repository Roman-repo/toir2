...
Функция ПроверитьТипПараметраКоманды(Знач Параметр, Знач ОжидаемыйТип) Экспорт
	...
	Если НЕ Результат Тогда
		ПоказатьПредупреждение(,НСтр("es='La acción no puede ser cumplida para el elemento escogido.';ru='Действие не может быть выполнено для выбранного элемента.';en='An action cannot be performed for the selected item.'"));
	КонецЕсли;
...
Процедура ПоказатьПодтверждениеЗакрытияФормы(
	...
	Если ЗавершениеРаботы Тогда
		Если ТекстПредупрежденияПриЗавершении = "" Тогда // передан параметр из ПередЗакрытием
			ТекстПредупрежденияПриЗавершении = НСтр("es='Los datos eran cambiados. Todos los cambios serán perdidos.';ru='Данные были изменены. Все изменения будут потеряны.';en='Data has been changed. All changes will be lost.'");
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
//
// Пример: 
//  ТекстПредупреждения = НСтр("es='¿Cerrar el ayudante?';ru='Закрыть помощник?';en='Close Assistant?'");
...
Процедура ПоказатьФормуРедактированияКомментария(
	...
	ЗаголовокФормы = ?(Заголовок <> Неопределено, Заголовок, НСтр("es='El comentario';ru='Комментарий';en='Comment'"));
