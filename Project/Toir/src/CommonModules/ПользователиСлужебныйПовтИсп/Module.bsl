...
Функция НедоступныеРоли(Назначение = "ДляПользователей", Сервис = Неопределено) Экспорт
	...
	ПроверитьНазначение(Назначение,
		НСтр("es='La falta como NedostupnyeRoli del módulo PolzovateliSluzhebnyyPovtIsp general.';ru='Ошибка в функции НедоступныеРоли общего модуля ПользователиСлужебныйПовтИсп.';en='Function error НедоступныеРоли of general module ПользователиСлужебныйПовтИсп.'"));
...
Функция ДеревоРолей(ПоПодсистемам = Истина, Назначение = "ДляПользователей") Экспорт
	...
	ПроверитьНазначение(Назначение,
		НСтр("es='La falta como DerevoRoley del módulo PolzovateliSluzhebnyyPovtIsp general.';ru='Ошибка в функции ДеревоРолей общего модуля ПользователиСлужебныйПовтИсп.';en='Function error ДеревоРолей of general module ПользователиСлужебныйПовтИсп.'"));
	...
	Если Назначение <> "ДляАдминистраторов"
	   ...
	   И Назначение <> "СовместноДляПользователейИВнешнихПользователей" Тогда
		...
		ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El parámetro el Destino ""%1"" es indicado es incorrecto. "
""
"Son admisibles solamente los significados siguientes: "
"- ""DlyaAdministratorov"", "
"- ""DlyaPolzovateley"", "
"- ""DlyaVneshnihPolzovateley"", "
"- ""СовместноДляПользователейИВнешнихПользователей"".';ru='Параметр Назначение ""%1"" указан некорректно."
""
"Допустимы только следующие значения:"
"- ""ДляАдминистраторов"","
"- ""ДляПользователей"","
"- ""ДляВнешнихПользователей"","
"- ""СовместноДляПользователейИВнешнихПользователей"".';en='The Purpose ""%1"" parameter is incorrect."
""
"Only the following values are valid:                                                     - ""ДляАдминистраторов"","
"- ""ДляПользователей"","
"- ""ДляВнешнихПользователей"","
"- ""СовместноДляПользователейИВнешнихПользователей"".'"),
			Назначение);
