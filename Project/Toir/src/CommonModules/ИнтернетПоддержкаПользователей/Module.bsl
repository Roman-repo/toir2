...
Функция ТикетАутентификацииНаПорталеПоддержки(ВладелецТикета) Экспорт
	...
	Если Не ЗначениеЗаполнено(ВладелецТикета) Тогда
		ВызватьИсключение НСтр("es='del parámetro ""VladeletsTiketa""';ru='Не заполнено значение параметра ""ВладелецТикета""';en='Parameter value ""ВладелецТикета"" not filled'");
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковУстановкиПараметровСеанса(Обработчики) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Новости") Тогда
		//МодульОбработкаНовостей = ОбщегоНазначения.ОбщийМодуль("ОбработкаНовостей");
		ОбработкаНовостей.ПриДобавленииОбработчиковУстановкиПараметровСеанса(Обработчики);
	КонецЕсли;
	// Конец Новости
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ОблачныйАрхив") Тогда
		...
		ОблачныйАрхив.ПриДобавленииОбработчиковУстановкиПараметровСеанса(Обработчики);
	КонецЕсли;
	// Конец ОблачныйАрхив
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			"login.1c.ru",
			443,
			НСтр("es='Los servicios аутентификации (la zona ru)';ru='Сервисы аутентификации (зона ru)';en='Authentication services (zone ru)'"));
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			"login.1c.eu",
			443,
			НСтр("es='Los servicios аутентификации (la zona eu)';ru='Сервисы аутентификации (зона eu)';en='Authentication services (eu zone)'"));
		НовыеРазрешения.Добавить(Разрешение);
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			443,
			НСтр("es='Los servicios del servicio del soporte técnico (la zona ru)';ru='Сервисы службы технической поддержки (зона ru)';en='Technical support services (zone ru)'"));
		НовыеРазрешения.Добавить(Разрешение);
		...
		Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
			...
			443,
			НСтр("es='Los servicios del servicio del soporte técnico (la zona eu)';ru='Сервисы службы технической поддержки (зона eu)';en='Technical support services (eu zone)'"));
		НовыеРазрешения.Добавить(Разрешение);
...
Функция ПоставщикУслугПортал1СИТСПриФормированииСпискаУслуг(ПоставщикиУслуг) Экспорт
	...
	ПоставщикПортал1СИТС.Вставить("Наименование" , НСтр("es='El portal 1С:ИТС';ru='Портал 1С:ИТС';en='Portal 1C: ITS'"));
...
Процедура СохранитьДанныеАутентификации(ДанныеАутентификации) Экспорт
	...
	Если ДанныеАутентификации = Неопределено Тогда
		...
		ЗаписатьИнформациюВЖурналРегистрации(НСтр("es='Son limpiados los datos аутентификации.';ru='Очищены данные аутентификации.';en='Authentication data cleared.'"));
	...
	Иначе
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Son anotados los datos аутентификации, el login ""%1"".';ru='Записаны данные аутентификации, логин ""%1"".';en='Authentication data written down, login ""%1"".'"),
				ДанныеАутентификации.Логин));
...
Функция КонтрольнаяСуммаФайла(Данные) Экспорт
	...
	Иначе
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El significado incorrecto del parámetro Dado (%1)';ru='Некорректное значение параметра Данные (%1)';en='Invalid value for the Data parameter (%1)'"),
			Строка(Данные));
...
Процедура ПроверитьURL(URL) Экспорт
	...
	Если ДоменХоста <> ".1c.ru" И ДоменХоста <> ".1c.eu" Тогда
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El huésped desconocido (%1)';ru='Неизвестный хост (%1)';en='Unknown host (%1)'"),
			СтруктураURI.Хост);
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	...
	Если ТипЗнч(ОбъектХДТО) = Тип("ОбъектXDTO") Тогда
		...
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta a la recepción de la xml-representación del objeto ХДТО: "
"%1';ru='Ошибка при получении xml-представления объекта ХДТО:"
"%1';en='Error getting xml representation of XTTO object:"
"%1'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("es='БИП:БазовыйФункционал. El ajuste';ru='БИП:БазовыйФункционал.Отладка';en='БИП:БазовыйФункционал.Отладка'"), // ИмяСобытия.
				НСтр("es='БИП:БазовыйФункционал. El ajuste';ru='БИП:БазовыйФункционал.Отладка';en='БИП:БазовыйФункционал.Отладка'"), // ИдентификаторШага.
...
Функция СодержимоеТекстовогоФайла(ПолноеИмяФайла, КоличествоСтрок = 10) Экспорт
	...
	Исключение
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='A la lectura del fichero con el nombre %1 había una falta: "
"%2';ru='При чтении файла с именем %1 произошла ошибка:"
"%2';en='An error occurred while reading a file named%1:"
"%2'"),
			ПолноеИмяФайла,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	...
	Иначе
		...
		Если ТребуетсяРазделение Тогда
			Если ЗначениеРазделителя = Неопределено Тогда
				ВызватьИсключение НСтр("es='del parámetro ""ZnachenieRazdelitelya"".';ru='Не заполнено значение параметра ""ЗначениеРазделителя"".';en='Parameter value ""ЗначениеРазделителя"" not filled'");
			КонецЕсли;
			МодульРаботаВМоделиСервиса = ОбщегоНазначения.ОбщийМодуль("РаботаВМоделиСервиса");
		...
		Если Не Результат Тогда
			ЗаписатьИнформациюВЖурналРегистрации(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El servicio con el identificador %1 no es conectado.';ru='Услуга с идентификатором %1 не подключена.';en='Service with ID%1 is not connected.'"),
					ИдентификаторУслуги));
		КонецЕсли;
	...
	Пока КоличествоПеренаправлений < МаксКолвоПеренаправлений Цикл
		...
		Исключение
			...
			ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió cargar el contenido (%1). %2';ru='Не удалось загрузить содержимое (%1). %2';en='Failed to load content (%1). %2'"),
				URL,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			...
			Попытка
				...
				ОписаниеРезультатаДиагностики = НСтр("es='Los resultados de la diagnosis de la unión:';ru='Результаты диагностики соединения:';en='Connection diagnostic results:'")
			...
			Исключение
				...
				ОписаниеРезультатаДиагностики = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No consiguió cumplir la diagnosis de la unión. %1';ru='Не удалось выполнить диагностику соединения. %1';en='Failed to perform connection diagnostics. %1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Если Ответ.КодСостояния = 301 // 301 Moved Permanently
			...
			Если КоличествоПеренаправлений > МаксКолвоПеренаправлений Тогда
				УстановитьОписаниеОшибки(
					...
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La falta del servidor a la recepción del fichero (%1). Es superada la cantidad de las redirecciones (%2).';ru='Ошибка сервера при получении файла (%1). Превышено количество перенаправлений (%2).';en='Server error while receiving file (%1). The number of redirects exceeded (%2).'"),
						URL,
			...
			Иначе
				...
				Если Location = Неопределено Тогда
					УстановитьОписаниеОшибки(
						Результат,
						"ServerError",
						НСтр("es='La redirección incorrecta.';ru='Некорректное перенаправление.';en='Invalid redirection.'"),
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='La falta de servidor (%1) a la recepción del fichero (%2). La redirección incorrecta, falta el HTTP-encabezamiento de la respuesta ""Location"".';ru='Ошибка сервера (%1) при получении файла (%2). Некорректное перенаправление, отсутствует HTTP-заголовок ответа ""Location"".';en='Server error (%1) while receiving file (%2). Invalid redirection, missing HTTP response header ""Location"".'"),
							Ответ.КодСостояния,
							URL),
						Перенаправления);
					Возврат Результат;
				Иначе
					...
					Если ПустаяСтрока(Location) Тогда
						УстановитьОписаниеОшибки(
							...
							НСтр("es='La redirección incorrecta.';ru='Некорректное перенаправление.';en='Invalid redirection.'"),
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La falta de servidor (%1) a la recepción del fichero (%2). La redirección incorrecta, el HTTP-encabezamiento vacío de la respuesta ""Location"".';ru='Ошибка сервера (%1) при получении файла (%2). Некорректное перенаправление, пустой HTTP-заголовок ответа ""Location"".';en='Server error (%1) while receiving file (%2). Invalid redirection, empty HTTP response header ""Location"".'"),
								Ответ.КодСостояния,
								URL),
							Перенаправления);
						Возврат Результат;
					...
					Если ВыполненныеПеренаправления.Получить(Location) <> Неопределено Тогда
						УстановитьОписаниеОшибки(
							Результат,
							"ServerError",
							НСтр("es='La redirección cíclica.';ru='Циклическое перенаправление.';en='Cyclic redirection.'"),
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La falta de servidor (%1) a la recepción del fichero (%2). La redirección cíclica (%3).';ru='Ошибка сервера (%1) при получении файла (%2). Циклическое перенаправление (%3).';en='Server error (%1) while receiving file (%2). Circular redirection (%3).'"),
								Ответ.КодСостояния,
								URL,
								Location),
							Перенаправления);
	...
	Если Ответ.КодСостояния < 200 Или Ответ.КодСостояния >= 300 Тогда
		...
		Если Ответ.КодСостояния = 407 Тогда
			...
			УстановитьОписаниеОшибки(
				Результат,
				"ConnectError",
				НСтр("es='La falta аутентификации en el proksi-servidor.';ru='Ошибка аутентификации на прокси-сервере.';en='Authentication error on proxy server.'"),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La falta del cliente (%1) a la ejecución de la interpelación al recurso (%2). "
"El cuerpo de la respuesta: %3';ru='Ошибка клиента (%1) при выполнении запроса к ресурсу (%2)."
"Тело ответа: %3';en='Client error (%1) while executing a request to the resource (%2)."
"Answer body:%3 '"),
					Ответ.КодСостояния,
					URL,
		...
		ИначеЕсли Ответ.КодСостояния < 200
			...
			УстановитьОписаниеОшибки(
				...
				"ServerError",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La respuesta incorrecta de servidor (%1).';ru='Некорректный ответ сервера (%1).';en='Invalid server response (%1).'"),
					Ответ.КодСостояния),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La respuesta incorrecta de servidor (%1).';ru='Некорректный ответ сервера (%1).';en='Invalid server response (%1).'"),
					URL,
		...
		ИначеЕсли Ответ.КодСостояния >= 400 И Ответ.КодСостояния < 500 Тогда
			...
			УстановитьОписаниеОшибки(
				...
				"ClientError",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La falta (%1) a la ejecución de la interpelación al recurso.';ru='Ошибка (%1) при выполнении запроса к ресурсу.';en='Error (%1) while executing a request to a resource.'"),
...
"Answer body:%3 '"),
		...
		Иначе
			...
			УстановитьОписаниеОшибки(
				...
				"ServerError",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El código de la falta: %1.';ru='Код ошибки: %1.';en='Error code:%1.'"),
					Строка(Ответ.КодСостояния)),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La falta de servidor (%1) al tratamiento de la interpelación al recurso (%2). "
"El cuerpo de la respuesta: %3';ru='Ошибка сервера (%1) при обработке запроса к ресурсу (%2)."
"Тело ответа: %3';en='Server error (%1) while processing request to resource (%2)."
"Answer body:%3'"),
					Ответ.КодСостояния,
					URL,
					Лев(Ответ.ПолучитьТелоКакСтроку(), 5120)),
...
Процедура ЗаписатьСообщениеВЖурналРегистрации(
	...
	Если ЛокальныйЗаписатьСообщениеВЖурналРегистрации = Истина Тогда
		...
		КомментарийЛокальный = НСтр("es='El identificador del paso: %ИдентификаторШага del % "
"%Комментарий del %';ru='Идентификатор шага: %ИдентификаторШага%"
"%Комментарий%';en='Step identifier: %ИдентификаторШага%"
"%Комментарий%'");
		...
		УстановитьПривилегированныйРежим(Истина);
				...
				Если ТипЗнч(ПараметрыФайлаВыгрузки) = ТипСтруктура Тогда
					Если (ПараметрыФайлаВыгрузки.Свойство("Архивировать"))
						...
						Архив = Новый ЗаписьZipФайла(
							...
							, // Без пароля
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La descarga de la revista de los registros de la biblioteca BIP en el periodo de %1... %2';ru='Выгрузка журнала регистраций библиотеки БИП за период %1...%2';en='Unloading the BIP library registration log for the period% 1 ...% 2'"),
			...
			Иначе
				Результат.Вставить("ТекстОшибки",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El fichero con la descarga de los acontecimientos de la revista del registro no existe. "
"Probablemente, faltan los acontecimientos por el período indicado (%1... %2).';ru='Файл с выгрузкой событий журнала регистрации не существует."
"Возможно, отсутствуют события за указанный период (%1...%2).';en='Log file of the logging event does not exist."
"There may be no events for the specified period (%1 ...%2). '"),
						Формат(ПараметрыОтбораПравильный.ДатаНачала, "ДЛФ=DT"),
						Формат(ПараметрыОтбораПравильный.ДатаОкончания, "ДЛФ=DT")));
			КонецЕсли;
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió exportar los acontecimientos de la revista del registro al fichero por la causa: "
"%1';ru='Не удалось экспортировать события журнала регистрации в файл по причине:"
"%1';en='Failed to export log events to file due to:"
"%1'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
"%1';en='Failed to export log events to file due to:"
"%1'"), // ИмяСобытия
			НСтр("es='БазовыйФункционал. El ajuste. La exportación de la revista del registro';ru='БазовыйФункционал. Отладка. Экспорт журнала регистрации';en='Базовый функционал. Debugging. Export дog иook'"), // ИдентификаторШага
...
Функция СобытиеЖурналаРегистрацииИнтернетПоддержкаПользователейОшибка()
	...
	Возврат НСтр("es='El apoyo de Internet de los usuarios. La falta';ru='Интернет-поддержка пользователей.Ошибка';en='Online user support. Error'",
...
Функция СобытиеЖурналаРегистрацииИнтернетПоддержкаПользователейИнформация()
	...
	Возврат НСтр("es='El apoyo de Internet de los usuarios. La información';ru='Интернет-поддержка пользователей.Информация';en='Online user support. Information'",
		ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СлужебнаяТикетАутентификации(
	...
	Если Не ЗначениеЗаполнено(Логин) Тогда
		...
		Результат.СообщениеОбОшибке  = НСтр("es='El login equivocado o la consigna.';ru='Неверный логин или пароль.';en='Wrong login or password.'");
	...
	Если РезультатОперации.КодСостояния = 200 Тогда
		...
		Исключение
			...
			Результат.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió recibir тикет аутентификации en el servicio %1. "
"La respuesta incorrecta del servicio. "
"La falta al tratamiento de la respuesta del servicio: "
"%2 "
"Código del estado: %3; "
"el Cuerpo de la respuesta: %4';ru='Не удалось получить тикет аутентификации в сервисе %1."
"Некорректный ответ сервиса."
"Ошибка при обработке ответа сервиса:"
"%2"
"Код состояния: %3;"
"Тело ответа: %4';en='Failed to get authentication ticket in service%1."
...
"Response body:%4 '"),
			...
			Результат.СообщениеОбОшибке =
				НСтр("es='La falta аутентификации. La respuesta incorrecta del servicio.';ru='Ошибка аутентификации. Некорректный ответ сервиса.';en='Authentication error. Invalid service response.'");
			Результат.СообщениеОбОшибке =
				НСтр("es='La falta аутентификации en el servicio. "
"Más detalladamente cm. En la revista del registro.';ru='Ошибка аутентификации в сервисе."
"Подробнее см. в журнале регистрации.';en='Authentication error in the service."
"See the log for details. '");
	...
	ИначеЕсли РезультатОперации.КодСостояния = 429 Тогда
		...
		Результат.СообщениеОбОшибке = НСтр("es='Es superada la cantidad de las tentativas аутентификации. "
"Repitan la tentativa más tarde.';ru='Превышено количество попыток аутентификации."
"Повторите попытку позже.';en='Exceeded number of authentication attempts."
"Please try again later. '");
	...
	ИначеЕсли РезультатОперации.КодСостояния = 500 Тогда
		...
		Результат.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir тикет аутентификации en el servicio %1. "
"La falta interior del servicio. "
"El código del estado: %2; "
"el Cuerpo de la respuesta: %3';ru='Не удалось получить тикет аутентификации в сервисе %1."
"Внутренняя ошибка сервиса."
"Код состояния: %2;"
"Тело ответа: %3';en='Failed to get authentication ticket in service%1."
...
"Response body:%3 '"),
		...
		Результат.СообщениеОбОшибке =
			НСтр("es='La falta аутентификации. La falta interior del servicio. "
"Más detalladamente cm. En la revista del registro.';ru='Ошибка аутентификации. Внутренняя ошибка сервиса."
"Подробнее см. в журнале регистрации.';en='Authentication error. Internal service error."
"See the logbook for details. '");
	...
	ИначеЕсли РезультатОперации.КодСостояния = 0 Тогда
		...
		Результат.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir тикет аутентификации en el servicio %1. "
"%2';ru='Не удалось получить тикет аутентификации в сервисе %1."
"%2';en='Failed to get authentication ticket in service %1."
"%2 '"),
			URLСервиса,
			РезультатОперации.ИнформацияОбОшибке);
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			Результат.ИнформацияОбОшибке);
		Результат.СообщениеОбОшибке = НСтр("es='La falta de la conexión al servicio. "
"Más detalladamente cm. En la revista del registro.';ru='Ошибка подключения к сервису."
"Подробнее см. в журнале регистрации.';en='Error connecting to the service."
"See the logbook for details.'");
	...
	Иначе
		...
		Результат.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir тикет аутентификации en el servicio %1. "
"El código desconocido del estado de la respuesta del servicio. "
"El código del estado: %2; "
"el Cuerpo de la respuesta: %3';ru='Не удалось получить тикет аутентификации в сервисе %1."
"Неизвестный код состояния ответа сервиса."
"Код состояния: %2;"
"Тело ответа: %3';en='Failed to get authentication ticket in service%1."
"Unknown service response status code."
"Status code:%2;"
"Response body:%3 '"),
			URLСервиса,
			РезультатОперации.КодСостояния,
		...
		Результат.СообщениеОбОшибке =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta аутентификации en el servicio (%1). "
"Más detalladamente cm. En la revista del registro.';ru='Ошибка аутентификации в сервисе (%1)."
"Подробнее см. в журнале регистрации.';en='Authentication error in the service (%1)."
"See the log for details. '"),
				РезультатОперации.КодСостояния);
...
Функция ПроверитьЛогинИПароль(Логин, Пароль) Экспорт
	...
	ИначеЕсли РезультатПолученияФайла.КодСостояния = 429 Тогда
		...
		Результат.КодОшибки = "ПревышеноКоличествоПопыток";
		Результат.СообщениеОбОшибке = НСтр("es='Es superada la cantidad de las tentativas de la introducción del login y la consigna. "
"Comprueben la corrección de los datos indicados y repitan "
"la tentativa en 30 minutos.';ru='Превышено количество попыток ввода логина и пароля."
"Проверьте правильность указанных данных и повторите"
"попытку через 30 минут.';en='Exceeded the number of attempts to enter login and password."
"Check that the data is correct and retry"
"try in 30 minutes.'");
	...
	Иначе
		...
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió comprobar el login y la consigna en el servicio %1. %2';ru='Не удалось проверить логин и пароль в сервисе %1. %2';en='Failed to verify the username and password in the service%1. %2'"),
				URLСервиса,
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий         = НСтр("es='Es puesto en marcha durante el primer lanzamiento, en los datos generales. La instalación de los datos generales.';ru='Запускается при первом запуске, в общих данных. Установка общих данных.';en='Launching at the first start, in the general data. Setting shared data.'");
	...
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.Комментарий         = НСтр("es='Es puesto en marcha durante el primer lanzamiento, para cada área de los datos. La instalación de los datos divididos.';ru='Запускается при первом запуске, для каждой области данных. Установка разделенных данных.';en='Launching at the first start, for each data area. Setting up shared data.'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.Комментарий         = НСтр("es='Es puesto en marcha durante el tránsito a una nueva versión, en los datos generales. La instalación de los datos generales.';ru='Запускается при переходе на новую версию, в общих данных. Установка общих данных.';en='Launching when switching to a new version, in general data. Setting shared data.'");
	Обработчик.РежимВыполнения     = "Оперативно";
	...
	Обработчик.ОбщиеДанные         = Ложь;
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("es='Es puesto en marcha durante el tránsito a cualquier nueva versión, para cada área de los datos. La instalación de los datos divididos.';ru='Запускается при переходе на любую новую версию, для каждой области данных. Установка разделенных данных.';en='Launching when switching to any new version, for each data area. Setting up shared data.'");
	Обработчик.РежимВыполнения     = "Оперативно";
...
Процедура ОбновлениеИнформационнойБазы_ОбщиеДанные_ПервыйЗапуск() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La ejecución ОбновлениеИнформационнойБазы_ОбщиеДанные_ПервыйЗапуск';ru='Выполнение ОбновлениеИнформационнойБазы_ОбщиеДанные_ПервыйЗапуск';en='Execution ОбновлениеИнформационнойБазы_ОбщиеДанные_ПервыйЗапуск'"));
...
""
"Otros parámetros:"
" Es la configuración dividida: %6"
" División es incluida: %7"
" Es accesible el uso de los datos divididos: %8"
" Sesión es puesta en marcha sin разделителей: %9';ru='%1, результат."
"Время начала (мс): %2"
...
"  Сеанс запущен без разделителей: %9';en='% 1, result."
"Start Time (ms):% 2"
"End Time (ms):% 3"
"Duration (ms):% 4"
"Execution Result:"
...
""
"Other parameters:"
"  This is a split configuration:% 6"
"  Split Enabled:% 7"
"  Available using shared data:% 8"
"  Session started without delimiters:% 9'")
			+ Символы.ПС,
	...
	ЗаписатьСообщениеВЖурналРегистрации(
		...
		НСтр("es='El servicio y el reglamento. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПервыйЗапуск';ru='Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПервыйЗапуск';en='Service and regulations. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПервыйЗапуск'"), // ИдентификаторШага
...
Процедура ОбновлениеИнформационнойБазы_ОбластьДанных_ПервыйЗапуск() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La ejecución ОбновлениеИнформационнойБазы_ОбластьДанных_ПервыйЗапуск';ru='Выполнение ОбновлениеИнформационнойБазы_ОбластьДанных_ПервыйЗапуск';en='Execution ОбновлениеИнформационнойБазы_ОбластьДанных_ПервыйЗапуск'"));
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La ejecución ОбновлениеИнформационнойБазы_ОбластьДанных_ПервыйЗапуск, la Redefinición';ru='Выполнение ОбновлениеИнформационнойБазы_ОбластьДанных_ПервыйЗапуск, Переопределение';en='Execution ОбновлениеИнформационнойБазы_ОбластьДанных_ПервыйЗапуск, Переопределение'"));
...
"  Session started without delimiters:% 9'")
	...
	ЗаписатьСообщениеВЖурналРегистрации(
		СобытиеЖурналаРегистрацииИнтернетПоддержкаПользователейИнформация(), // ИмяСобытия
		НСтр("es='El servicio y el reglamento. ОбновлениеИнформационнойБазы. ОбластьДанных. ПервыйЗапуск';ru='Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПервыйЗапуск';en='Service and regulations. ОбновлениеИнформационнойБазы. ОбластьДанных. ПервыйЗапуск'"), // ИдентификаторШага
		?(КодРезультатаОбщий = 0, УровеньЖурналаРегистрации.Информация, УровеньЖурналаРегистрации.Ошибка), // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполненияОбщий.ВремяОкончания - ШагВыполненияОбщий.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
Процедура ОбновлениеИнформационнойБазы_ОбщиеДанные_ПерейтиНаВерсию() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La ejecución ОбновлениеИнформационнойБазы_ОбщиеДанные_ПерейтиНаВерсию';ru='Выполнение ОбновлениеИнформационнойБазы_ОбщиеДанные_ПерейтиНаВерсию';en='Execution ОбновлениеИнформационнойБазы_ОбщиеДанные_ПерейтиНаВерсию'"));
...
"  Session started without delimiters:% 9'")
	...
	ЗаписатьСообщениеВЖурналРегистрации(
		СобытиеЖурналаРегистрацииИнтернетПоддержкаПользователейИнформация(), // ИмяСобытия
		НСтр("es='El servicio y el reglamento. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПерейтиНаВерсию';ru='Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПерейтиНаВерсию';en='Service and regulations. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПерейтиНаВерсию'"), // ИдентификаторШага
		?(КодРезультатаОбщий = 0, УровеньЖурналаРегистрации.Информация, УровеньЖурналаРегистрации.Ошибка), // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполненияОбщий.ВремяОкончания - ШагВыполненияОбщий.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
Процедура ОбновлениеИнформационнойБазы_ОбластьДанных_ПерейтиНаВерсию() Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La ejecución ОбновлениеИнформационнойБазы_ОбластьДанных_ПерейтиНаВерсию';ru='Выполнение ОбновлениеИнформационнойБазы_ОбластьДанных_ПерейтиНаВерсию';en='Execution ОбновлениеИнформационнойБазы_ОбластьДанных_ПерейтиНаВерсию'"));
...
"  Session started without delimiters:% 9'")
	...
	ЗаписатьСообщениеВЖурналРегистрации(
		СобытиеЖурналаРегистрацииИнтернетПоддержкаПользователейИнформация(), // ИмяСобытия
		НСтр("es='El servicio y el reglamento. ОбновлениеИнформационнойБазы. ОбластьДанных. ПерейтиНаВерсию';ru='Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПерейтиНаВерсию';en='Service and regulations. ОбновлениеИнформационнойБазы. ОбластьДанных. ПерейтиНаВерсию'"), // ИдентификаторШага
		?(КодРезультатаОбщий = 0, УровеньЖурналаРегистрации.Информация, УровеньЖурналаРегистрации.Ошибка), // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполненияОбщий.ВремяОкончания - ШагВыполненияОбщий.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
Функция ОтправитьДанныеСообщенияВТехПоддержку(
	...
	Если Не ПустаяСтрока(РезультатПроверки.ИмяОшибки) Тогда
		...
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió enviar los mensajes dados en el servicio del soporte técnico. "
"No consiguió comprobar la accesibilidad del servicio del envío de los mensajes. "
"%1';ru='Не удалось отправить данные сообщения в службу технической поддержки."
"Не удалось проверить доступность сервиса отправки сообщений."
"%1';en='Failed to send these messages to technical support."
"Failed to check the availability of the messaging service."
"%1'"),
				РезультатПроверки.ИнформацияОбОшибке));
		...
		Результат.СообщениеОбОшибке =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió enviar los mensajes dados en el servicio del soporte técnico. "
"No consiguió comprobar la accesibilidad del servicio. "
"%1';ru='Не удалось отправить данные сообщения в службу технической поддержки."
...
"%1'"),
	...
	Исключение
		...
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			...
			НСтр("es='No consiguió enviar los mensajes dados en el servicio del soporte técnico. "
"La falta al tratamiento de la respuesta del servicio. "
"La respuesta incorrecta del servicio del envío de los mensajes. "
"%1 "
"Cuerpo de la respuesta: %2';ru='Не удалось отправить данные сообщения в службу технической поддержки."
"Ошибка при обработке ответа сервиса."
"Некорректный ответ сервиса отправки сообщений."
"%1"
"Тело ответа: %2';en='Failed to send these messages to technical support."
"Error processing service response."
"Invalid response from the messaging service."
"%1"
"Response body: %2'"),
			ПодробноеПредставлениеОшибки(ИнфОшибка),
		...
		Результат.СообщениеОбОшибке =
			НСтр("es='No consiguió enviar los mensajes dados en el servicio del soporte técnico. "
"La respuesta incorrecta del servicio del envío de los mensajes.';ru='Не удалось отправить данные сообщения в службу технической поддержки."
"Некорректный ответ сервиса отправки сообщений.';en='Failed to send these messages to technical support."
"Invalid message sending service response.'");
	...
	Если РезультатВызова.success <> Истина Тогда
		...
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió enviar los mensajes dados en el servicio del soporte técnico. "
"La falta con la transmisión del mensaje en aquel. El apoyo. "
"El servicio ha informado sobre la falta. "
"La respuesta del servicio: %1';ru='Не удалось отправить данные сообщения в службу технической поддержки."
"Ошибка при передаче сообщения в тех. поддержку."
"Сервис сообщил об ошибке."
"Ответ сервиса: %1';en='Failed to send these messages to technical support."
"Error sending message to tech. support."
...
"Service response: %1'"),
		...
		Результат.СообщениеОбОшибке =
			НСтр("es='No consiguió enviar los mensajes dados en el servicio del soporte técnico. "
"La falta con la transmisión del mensaje en aquel. El apoyo. "
"Más detalladamente cm. En la revista del registro.';ru='Не удалось отправить данные сообщения в службу технической поддержки."
"Ошибка при передаче сообщения в тех. поддержку."
"Подробнее см. в журнале регистрации.';en='Failed to send these messages to technical support."
"Error sending message to tech. support."
"See the logbook for details. '");
...
Процедура УстановитьОписаниеОшибки(Результат, КодОшибки, СообщениеОбОшибке, ИнформацияОбОшибке, Перенаправления)
	...
	Если КодОшибки = "ConnectError" Тогда
		ДопСообщение = НСтр("es='La falta a la conexión al servidor.';ru='Ошибка при подключении к серверу.';en='Error connecting to server.'");

	ИначеЕсли КодОшибки = "ServerError" Тогда
		ДопСообщение = НСтр("es='En el servidor ha surgido la falta interior al tratamiento de la interpelación.';ru='На сервере возникла внутренняя ошибка при обработке запроса.';en='An internal error occurred while processing the request.'");

	ИначеЕсли КодОшибки = "ClientError" Тогда
		ДопСообщение = НСтр("es='La interpelación incorrecta.';ru='Некорректный запрос.';en='Invalid request.'");

	ИначеЕсли КодОшибки = "InternalError" Тогда
		ДопСообщение = НСтр("es='La falta interior.';ru='Внутренняя ошибка.';en='Internal error.'");

	ИначеЕсли КодОшибки = "LoginError" Тогда
		ДопСообщение = НСтр("es='La falta аутентификации en el servidor.';ru='Ошибка аутентификации на сервере.';en='Authentication error on server.'");
	...
	Если Перенаправления.Количество() > 0 Тогда
		Результат.ИнформацияОбОшибке = Результат.ИнформацияОбОшибке + Символы.ПС
			+ НСтр("es='Las redirecciones:';ru='Перенаправления:';en='Redirects:'") + Символы.ПС
...
Процедура СлужебнаяПроверитьURLДоступен(
	...
	Исключение
		ИмяОшибки = "Unknown";
		СообщениеОбОшибке = НСтр("es='La falta desconocida. Más detalladamente cm. En la revista del registro.';ru='Неизвестная ошибка. Подробнее см. в журнале регистрации.';en='Unknown error. See the log for more details.'");
		ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta desconocida durante la comprobación de la accesibilidad URL. "
"%1';ru='Неизвестная ошибка при проверке доступности URL."
"%1';en='Unknown error checking URL availability."
"%1'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
