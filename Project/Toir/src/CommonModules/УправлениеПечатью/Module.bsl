...
Процедура ЗаполнитьПрофильРедактированиеПечатныхФорм(ОписанияПрофилей) Экспорт
	...
	ОписаниеПрофиля.Наименование = НСтр("es='La redacción, el envío por correo, la conservación en el fichero de las formas de imprenta (complementariamente)';ru='Редактирование, отправка по почте, сохранение в файл печатных форм (дополнительно)';en='Editing, sending by mail, saving to a printed form file (optional)'",
		Метаданные.ОсновнойЯзык.КодЯзыка);
	ОписаниеПрофиля.Описание = НСтр("es='Es fijado complementariamente a los usuarios, a que debe ser accesible la posibilidad de la redacción, "
"ante la prensa, el envío por correo y la conservación en el fichero de las formas de imprenta formadas.';ru='Дополнительно назначается пользователям, которым должна быть доступна возможность редактирования,"
"перед печатью, отправка по почте и сохранение в файл сформированных печатных форм.';en='Additionally assigned to users who should be able to edit,"
"before printing, sending by mail and saving the generated printed forms to a file.'", Метаданные.ОсновнойЯзык.КодЯзыка);
...
Функция МакетПечатнойФормы(ПутьКМакету) Экспорт
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La maqueta ""%1"" no existe. La operación es interrumpida.';ru='Макет ""%1"" не существует. Операция прервана.';en='Layout ""%1"" does not exist. Operation aborted.'"), ПутьКМакету);
	...
	Для Каждого ИмяПоиска Из ИменаПоиска Цикл
		Если КоллекцияМакетов.Найти(ИмяПоиска) <> Неопределено Тогда
			...
			Иначе
				...
				// >> [Деснол] [Ушаков А.Д.] [16.09.2019]
				//Если ЛокализованныеФормы.Найти(ИмяПоиска) = Неопределено Тогда
				//	Макет = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ИмяОбъекта).ПолучитьМакет(ИмяПоиска);
				//Иначе
				//	КодЯзыка = ТекущийЯзык().КодЯзыка;
				//	ИмяПоискаССуффиксом = СтрШаблон("%1_%2", ИмяПоиска, КодЯзыка);
				//	Макет = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ИмяОбъекта).ПолучитьМакет(ИмяПоискаССуффиксом);
				//КонецЕсли; 
				//
				//Возврат Макет;
...
Процедура ОтключитьПользовательскийМакет(ПутьКМакету) Экспорт
	...
	Если ЧастиСтроки.Количество() <> 2 И ЧастиСтроки.Количество() <> 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La maqueta ""%1"" no es encontrada.';ru='Макет ""%1"" не найден.';en='Layout ""%1"" not found.'"), ПутьКМакету);
	КонецЕсли;
...
Функция ДанныеQRКода(QRСтрока, УровеньКоррекции, Размер) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("es='La formación del QR-código';ru='Формирование QR-кода';en='QR code formation'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ИнициализироватьПечатнуюФорму(Знач УдалитьТипДокумента, Знач УдалитьНастройкиСтраницыМакета = Неопределено, Макет = Неопределено) Экспорт
	...
	Если Макет = Неопределено Тогда
		ВызватьИсключение НСтр("es='Es necesario indicar el significado del parámetro ""la Maqueta""';ru='Необходимо указать значение параметра ""Макет""';en='Required to specify a value for the Layout parameter.'");
	КонецЕсли;
...
Функция ОбластьМакета(СсылкаНаМакет, ОписаниеОбласти) Экспорт
	...
	ИначеЕсли ОписаниеОбласти.ТипОбласти = "Список" Тогда
		Область = УправлениеПечатьюСлужебный.ПолучитьОбластьМакета(СсылкаНаМакет, ОписаниеОбласти.ИмяОбласти);
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El tipo de la esfera no es indicado o indicado es incorrecto: %1.';ru='Тип области не указан или указан некорректно: %1.';en='Area type not specified or specified incorrectly:%1.'"), ОписаниеОбласти.ТипОбласти);
	КонецЕсли;
...
Процедура ПрисоединитьОбласть(ПечатнаяФорма, ОбластьМакета, Знач ПереходНаСледующуюСтроку = Ложь) Экспорт
	...
	Исключение
		...
		СообщениеОбОшибке = СообщениеОбОшибке + " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La falta durante la tentativa de sacar la esfera ""%1"" de la maqueta.';ru='Ошибка при попытке вывести область ""%1"" из макета.';en='An error occurred while trying to display the region ""%1"" from the layout.'"),
...
Процедура ПриСозданииНаСервере(Форма, МестоРазмещенияКомандПоУмолчанию = Неопределено, ОбъектыПечати = Неопределено) Экспорт
	...
	Если ТипЗнч(МестоРазмещенияКомандПоУмолчанию) = Тип("ГруппаФормы") Тогда
		Если МестоРазмещенияКомандПоУмолчанию.Вид = ВидГруппыФормы.Подменю
			Или МестоРазмещенияКомандПоУмолчанию.Заголовок = НСтр("es='La prensa';ru='Печать';en='Printing'")
...
Процедура ПечатьПоВнешнемуИсточнику(ДополнительнаяОбработкаСсылка, ПараметрыИсточника, КоллекцияПечатныхФорм,
	...
	Если ВнешняяОбработкаОбъект = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El tratamiento exterior ""%1"" (tipo ""%2"") no es servido.';ru='Внешняя обработка ""%1"" (тип ""%2"") не обслуживается.';en='External processing ""% 1"" (type ""% 2"") is not serviced.'"),
	...
	Для Каждого ПечатнаяФорма Из КоллекцияПечатныхФорм Цикл
		Если ПечатнаяФорма.ТабличныйДокумент = Неопределено Тогда
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='En el elaborador de la prensa no era formado el documento tabular para: %1';ru='В обработчике печати не был сформирован табличный документ для: %1';en='A spreadsheet document was not generated in the print handler for:% 1'"),
...
Процедура ПриОпределенииВидовПодключаемыхКоманд(ВидыПодключаемыхКоманд) Экспорт
	...
	Вид.Заголовок   = НСтр("es='La prensa';ru='Печать';en='Printing'");
	Вид.Порядок     = 40;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("es='Limpia las maquetas de usuario, en que no hay cambios en comparación con las maquetas correspondientes suministradas. "
"Desconecta las maquetas de usuario, que no son compatible con la versión corriente de la configuración.';ru='Очищает пользовательские макеты, в которых нет изменений по сравнению с соответствующими поставляемыми макетами."
"Отключает пользовательские макеты, которые не совместимы с текущей версией конфигурации.';en='Clears custom layouts that have no changes compared to the corresponding supplied layouts."
"Disables custom layouts that are not compatible with the current version of the configuration.'");
	...
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Комментарий = НСтр("es='Determina, que maquetas suministradas de las formas de imprenta se han cambiado con relación a la versión anterior de la configuración.';ru='Определяет, какие поставляемые макеты печатных форм изменились по отношению к предыдущей версии конфигурации.';en='Defines which supplied print layouts have changed in relation to a previous version of the configuration.'");
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешения.Добавить(МодульРаботаВБезопасномРежиме.РазрешениеНаИспользованиеВнешнейКомпоненты(
		"ОбщийМакет.КомпонентаПечатиQRКода", НСтр("es='La prensa QR de los códigos.';ru='Печать QR кодов.';en='Printing QR codes.'")));
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("es='Las maquetas de las formas de imprenta';ru='Макеты печатных форм';en='Plate layouts'");
	...
	Для Каждого ИмяМакета Из ИменаМакетов Цикл
		...
		Для Каждого ОписаниеПечатнойФормы Из ВременнаяКоллекцияДляОднойПечатнойФормы Цикл
			ОбщегоНазначенияКлиентСервер.Проверить(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='""%1"".';ru='Не задано количество экземпляров для печатной формы ""%1"".';en='The number of copies for the printable ""%1"" is not specified.'"),
		...
		Если Отказ Тогда
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='A la formación de la forma de imprenta ""%1"" ha surgido la falta. Se dirijan al administrador.';ru='При формировании печатной формы ""%1"" возникла ошибка. Обратитесь к администратору.';en='An error occurred while generating the print form ""%1"". Contact your administrator.'"), ИмяМакета);
			ВызватьИсключение ТекстСообщенияОбОшибке;
		КонецЕсли;
...
Функция КомпонентаФормированияQRКода()
	...
	ТекстОшибки = НСтр("es='No consiguió conectar el componente exterior para la generación del QR-código. Los detalles en la revista del registro.';ru='Не удалось подключить внешнюю компоненту для генерации QR-кода. Подробности в журнале регистрации.';en='Failed to connect an external component to generate a QR code. Details are in the logbook.'");
...
Процедура СообщитьПечатнаяФормаНедоступна(Объект)
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La prensa %1 no es cumplida: la forma de imprenta escogida es inaccesible.';ru='Печать %1 не выполнена: выбранная печатная форма недоступна.';en='Printing%1 failed: Selected printing form is not available.'"), Объект);
...
Процедура ПроверитьРазметкуТабличногоДокументаПоОбъектамПечати(ТабличныйДокумент, ОбъектыПечати, Знач МенеджерПечати, Знач Идентификатор)
	...
	ТекстОшибкиРазметки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Falta la marca del documento tabular ""%1"" por los objetos de la prensa. "
"Es necesario usar el procedimiento УправлениеПечатью.ЗадатьОбластьПечатиДокумента () "
"a la formación del documento tabular';ru='Отсутствует разметка табличного документа ""%1"" по объектам печати."
"Необходимо использовать процедуру УправлениеПечатью.ЗадатьОбластьПечатиДокумента()"
"при формировании табличного документа';en='Missing layout of spreadsheet document ""%1"" for print objects."
"Must use procedure УправлениеПечатью.ЗадатьОбластьПечатиДокумента() when forming a spreadsheet document'"), Идентификатор);
...
Функция ТекстТипОбластиУказанНекорректно()
	Возврат НСтр("es='El tipo de la esfera no es indicado o indicado es incorrecto.';ru='Тип области не указан или указан некорректно.';en='The type of region is not specified or is specified incorrectly.'");
КонецФункции
...
Функция ИмяФайлаПечатнойФормыПоУмолчанию(ОбъектПечати, НазваниеПечатнойФормы)
	...
	Если ОбщегоНазначения.ЭтоДокумент(Метаданные.НайтиПоТипу(ТипЗнч(ОбъектПечати))) Тогда
		...
		Если ДокументСодержитНомер Тогда
			...
			Шаблон = НСтр("es='[НазваниеПечатнойФормы] № [el Número] de [la Fecha]';ru='[НазваниеПечатнойФормы] № [Номер] от [Дата]';en='[НазваниеПечатнойФормы] № [Номер] от [Дата]'");
		Иначе
			СписокРеквизитов = "Дата";
			Шаблон = НСтр("es='[НазваниеПечатнойФормы] de [la Fecha]';ru='[НазваниеПечатнойФормы] от [Дата]';en='[НазваниеПечатнойФормы] от [Дата]'");
		КонецЕсли;
	...
	Иначе
		...
		Шаблон = НСтр("es='[НазваниеПечатнойФормы] - [ПредставлениеОбъекта] - [ТекущаяДата]';ru='[НазваниеПечатнойФормы] - [ПредставлениеОбъекта] - [ТекущаяДата]';en='[НазваниеПечатнойФормы] - [ПредставлениеОбъекта] - [ТекущаяДата]'");
