...
Процедура ПроверитьПараметрыОжидатьЗавершение(Знач ДлительнаяОперация, Знач ОповещениеОЗавершении, Знач ПараметрыОжидания)
	...
	Если ПараметрыОжидания <> Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.Проверить(ПараметрыОжидания.Интервал = 0 Или ПараметрыОжидания.Интервал >= 1, 
			НСтр("es='El parámetro De ParametryOzhidaniya. El intervalo debe ser más o es igual 1';ru='Параметр ПараметрыОжидания.Интервал должен быть больше или равен 1';en='The parameter ПараметрыОжидания.Интервал must be greater than or equal to 1'"),
		...
		ОбщегоНазначенияКлиентСервер.Проверить(Не (ПараметрыОжидания.ОповещениеОПрогрессеВыполнения <> Неопределено И ПараметрыОжидания.ВыводитьОкноОжидания), 
			НСтр("es='Si el parámetro De ParametryOzhidaniya. ВыводитьОкноОжидания es establecido en la Verdad, el parámetro ПараметрыОжидания.ОповещениеОПрогрессеВыполнения no es apoyado';ru='Если параметр ПараметрыОжидания.ВыводитьОкноОжидания установлен в Истина, то параметр ПараметрыОжидания.ОповещениеОПрогрессеВыполнения не поддерживается';en='If the ПараметрыОжидания.ВыводитьОкноОжидания parameter is set to True, then the ПараметрыОжидания.ОповещениеОПрогрессеВыполнения parameter is not supported'"),
			"ДлительныеОперацииКлиент.ОжидатьЗавершение");
	КонецЕсли;
...
Процедура ПоказатьОповещение(ОповещениеПользователя) Экспорт
	...
	ПоказатьОповещениеПользователя(?(Оповещение.Текст <> Неопределено, Оповещение.Текст, НСтр("es='La acción es cumplida';ru='Действие выполнено';en='Action completed'")), 
