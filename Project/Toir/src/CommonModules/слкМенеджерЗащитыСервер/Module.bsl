...
Функция ПодключитьКомпонентуСЛК(Знач МестоположениеКомпонентыСЛК = Неопределено, ОписаниеОшибки = "")	Экспорт
	...
	Если НЕ ПодключитьВнешнююКомпоненту(АдресКомпонентыСЛК(МестоположениеКомпонентыСЛК), "Licence", ТипВнешнейКомпоненты.Native) Тогда
		...
		ОписаниеОшибки = НСтр("es='La falta de la conexión de los componente СЛК (';ru='Ошибка подключения компоненты СЛК(';en='Error connecting components of the SLC ('") + СисИнфо.ТипПлатформы + "):
			|" + ОписаниеОшибки();
	КонецЕсли;
...
Функция ПолучитьМенеджерОбъектовСерииЗащиты(Знач Серия)	Экспорт
	...
	// Создание объекта 
	МенеджерЛицензии = Новый("AddIn.Licence.LicenceExtension");
	...
	КонецЕсли;
	Если НЕ МенеджерЛицензии.Запуск(Серия) Тогда //, МенеджерСерииЗащиты.ТолькоНаличиеКлюча) Тогда
	...
	Исключение
		слкМенеджерЗащиты.ВызватьИсключениеСЛК(НСтр("es='La falta de la creación del manager de los objetos СЛК: ';ru='Ошибка создания менеджера объектов СЛК: ';en='Error creating the SLK object manager: '") + ОписаниеОшибки(), Серия);
	КонецПопытки;
	...
	Если Не флСтартНовойЗащиты Тогда 
		...
		Если флСтартСтаройЗащиты Тогда 
			слкМенеджерЗащиты.ВызватьИсключениеСЛК(НСтр("es='La falta de la creación del manager de los objetos СЛК: se usa la versión que ha caducado de los componente СЛК.';ru='Ошибка создания менеджера объектов СЛК: используется устаревшая версия компоненты СЛК.';en='Error creating the SLK object manager: using an outdated version of the SLK component.'"), Серия);
		Иначе
			слкМенеджерЗащиты.ВызватьИсключениеСЛК(НСтр("es='La falta de la creación del manager de los objetos СЛК: ';ru='Ошибка создания менеджера объектов СЛК: ';en='Error creating the SLK object manager: '") + ТекстОшибки, Серия);
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьМенеджерСерииЗащиты(Знач Серия)	Экспорт
	...
	Если ПравоДоступа("Установка", Метаданные.ПараметрыСеанса.слкМенеджерЗащиты,,) = Ложь Тогда
		слкМенеджерЗащиты.ВызватьИсключениеСЛК(НСтр("es='¡El acceso al sistema del licenciamiento es inaccesible para el usuario! "
"Se dirijan al administrador.';ru='Доступ к системе лицензирования недоступен для пользователя!"
"Обратитесь к администратору.';en='Access to the licensing system is not available to the user!"
"Contact administrator.'"), Серия);
	КонецЕсли;
	...
	Если МенеджерСерииЗащиты = Неопределено Тогда
		слкМенеджерЗащиты.ВызватьИсключениеСЛК(СтрШаблон(НСтр("es='Una serie de las llaves %1 es inaccesible para el usuario';ru='Серия ключей %1 недоступна для пользователя';en='Key series%1 not available to user'"), Серия), Серия);
	КонецЕсли;
...
Функция ПараметрыСвязи(Знач Host = "", Знач Port = "")
	...
	Если ЗначениеЗаполнено(Host) Тогда
		СтрокаПараметровСвязи = СтрокаПараметровСвязи + "ServerAddr=" + СокрЛП(Host);
	Иначе
		СтрокаПараметровСвязи = СтрокаПараметровСвязи + "ServerAddr=localhost";
	КонецЕсли;
	Если ЗначениеЗаполнено(Port) И Port <> "0" Тогда
		СтрокаПараметровСвязи = СтрокаПараметровСвязи + ?(ЗначениеЗаполнено(СтрокаПараметровСвязи), ";", "") + "ServerPort=" + СокрЛП(Port);
	Иначе
		СтрокаПараметровСвязи = СтрокаПараметровСвязи + ?(ЗначениеЗаполнено(СтрокаПараметровСвязи), ";", "") + "ServerPort=9099";
	КонецЕсли;
