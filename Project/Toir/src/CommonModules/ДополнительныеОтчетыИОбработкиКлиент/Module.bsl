...
//		ПараметрыКоманды.СопровождающийТекст = НСтр("es='Se cumple la orden...';ru='Выполняется команда...';en='The command is running ...'");
...
Процедура ВыполнитьКомандуВФоне(Знач ИдентификаторКоманды, Знач ПараметрыКоманды, Знач Обработчик) Экспорт
	...
	Если Не ЗначениеЗаполнено(СопровождающийТекст) Тогда
		СопровождающийТекст = НСтр("es='La orden se cumple.';ru='Команда выполняется.';en='The command is being executed.'");
	КонецЕсли;
...
Процедура ПоказатьОповещениеПриВыполненииКоманды(ВыполняемаяКоманда)
	Если ВыполняемаяКоманда.ПоказыватьОповещение Тогда
		ПоказатьОповещениеПользователя(НСтр("es='La orden se cumple...';ru='Команда выполняется...';en='The command is running ...'"), , ВыполняемаяКоманда.Представление);
	КонецЕсли;
КонецПроцедуры
	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		...
		Если ФормаОбработки = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Para el informe o el tratamiento ""%1"" no es fijada la forma básica, "
"o la forma básica no es destinada al lanzamiento a la aplicación regular. "
"La orden ""%2"" no puede ser cumplida.';ru='Для отчета или обработки ""%1"" не назначена основная форма,"
"или основная форма не предназначена для запуска в обычном приложении."
"Команда ""%2"" не может быть выполнена.';en='The report or processing"" ""%1"" ""has not been assigned a primary form,"
"or the main form is not intended to run in a regular application."
...
Процедура ВыгрузитьВФайл(ПараметрыВыгрузки) Экспорт
	...
	ПараметрыСохранения.ТекстПредложения = НСтр("es='Para la descarga del tratamiento (informe) exterior en el fichero se recomienda establecer la ampliación para el cliente web 1С:Предприятие.';ru='Для выгрузки внешней обработки (отчета) в файл рекомендуется установить расширение для веб-клиента 1С:Предприятие.';en='To upload external processing (report) to a file, it is recommended to install the extension for the 1C: Enterprise web client.'");
	ПараметрыСохранения.Диалог.Фильтр = ДополнительныеОтчетыИОбработкиКлиентСервер.ФильтрДиалоговВыбораИСохранения();
	ПараметрыСохранения.Диалог.Заголовок = НСтр("es='Indiquen el fichero';ru='Укажите файл';en='Specify file'");
