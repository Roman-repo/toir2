...
Процедура ВыбратьФайлВызовОнлайнПоддержки(ОповещениеОЗакрытии, ПутьКФайлу = "") Экспорт
	...
	Диалог.Заголовок = НСтр("es='Escojan el fichero cumplido de la aplicación';ru='Выберите исполняемый файл приложения';en='Select application executable'");
	Диалог.ПолноеИмяФайла = ПутьКФайлу;
	Диалог.Каталог = Каталог.Путь;
	Фильтр = НСтр("es='El fichero de la aplicación (*.exe) |*.exe';ru='Файл приложения (*.exe)|*.exe';en='Application file (* .exe) | * .exe'");
...
Процедура ВызватьОнлайнПоддержку() Экспорт
	...
	Если Не ОбщегоНазначенияКлиент.ЭтоWindowsКлиент() Тогда
		ПоказатьПредупреждение(,НСтр("es='Para el trabajo con la aplicación es necesario el sistema Microsoft Windows de operaciones.';ru='Для работы с приложением необходима операционная система Microsoft Windows.';en='To work with the application requires the operating system Microsoft Windows.'"));
		Возврат
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("ВызватьОнлайнПоддержкуПослеУстановкиРасширения", ЭтотОбъект);
	ТекстСообщения = НСтр("es='Para el lanzamiento de la aplicación es necesario establecer la ampliación del trabajo con los ficheros.';ru='Для запуска приложения необходимо установить расширение работы с файлами.';en='To run the application, you must install the file extension.'");
