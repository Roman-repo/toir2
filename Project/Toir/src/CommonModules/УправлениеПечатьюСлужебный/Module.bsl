...
Функция МакетИзДвоичныхДанных(ДвоичныеДанныеМакета) Экспорт
	...
	Если НЕ Расширение = "docx" Тогда
		ЗаписатьСобытияВЖурналРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", НСтр("es='La maqueta del modelo de documento de oficina tiene no el formato justo.';ru='Макет шаблона офисного документа имеет не верный формат.';en='The layout of the office document template is not in the correct format.'"));
		ВызватьИсключение(НСтр("es='La falta del análisis de la maqueta del modelo. La maqueta del modelo de documento de oficina tiene no el formato justo.';ru='Ошибка анализа макета шаблона. Макет шаблона офисного документа имеет не верный формат.';en='Error parsing layout template. The layout of the office document template is not in the correct format.'"));
	КонецЕсли;
...
Процедура ЗакрытьСоединение(ПечатнаяФорма) Экспорт
	...
	Исключение
		...
		ВызватьИсключение(НСтр("es='No consiguió quitar el catálogo temporal del modelo de la forma de imprenta por la causa:';ru='Не удалось удалить временный каталог шаблона печатной формы по причине:';en='Failed to delete the temporary directory of the printable template due to:'") + Символы.ПС 
...
Процедура РазобратьКонтейнерДокументDOCX(Знач ПолноеИмяФайла, Знач ПутьКСтруктуреФайла)
	...
	Исключение
		...
		ЗаписатьСобытияВЖурналРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение(НСтр("es='No consiguió abrir el fichero del modelo por la causa:';ru='Не удалось открыть файла шаблона по причине:';en='Failed to open the template file due to:'") + Символы.ПС 
			+ КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	...
	Исключение
		...
		УдалитьФайлы(ПолноеИмяФайла);
		ЗаписатьСобытияВЖурналРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение(НСтр("es='No consiguió cumplir la análisis del fichero del modelo por la causa:';ru='Не удалось выполнить разбор файла шаблона по причине:';en='Failed to parse the template file due to:'") + Символы.ПС 
			+ КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура СобратьКонтейнерДокументаDOCX(Знач ПолноеИмяФайла, Знач ПутьКСтруктуреФайла)
	...
	Исключение
		ЗаписатьСобытияВЖурналРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение(НСтр("es='No consiguió crear el fichero del documento por la causa:';ru='Не удалось создать файл документа по причине:';en='Failed to create document file due to:'") + Символы.ПС 
			+ КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	...
	Исключение
		ЗаписатьСобытияВЖурналРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение(НСтр("es='No consiguió formar el fichero del documento por la causa:';ru='Не удалось сформировать файл документа по причине:';en='Failed to generate document file due to:'") + Символы.ПС 
			+ КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("es='La prensa';ru='Печать';en='Printing'", ОбщегоНазначения.КодОсновногоЯзыка());
