...
Процедура СотрудникиОбработкаПроверкиЗаполненияНаСервере(Форма, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Форма.Сотрудник.ГоловнаяОрганизация) Тогда
		ТекстСообщения = НСтр("es='la Organización';ru='Не заполнена Организация';en='Not filled Organization'");
	...
	Если ДанныеСотрудника.Договорник = Истина Тогда
		...
		Иначе
			...
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСТр("es='%1 con %2';ru='%1 с %2';en='%1 s%2'"),
...
Функция БанковскийСчетИнформацияОПричинахНедоступности() Экспорт
	...
	Возврат НСтр("es='Para la introducción de la cuenta personal, es necesario formalizar el ingreso al trabajo';ru='Для ввода лицевого счета, необходимо оформить прием на работу';en='For entering a personal account, you must apply for a job'");
...
Процедура ПроверитьУникальностьФизическогоЛицаВФорме(Форма, Отказ)
	...
	Если НЕ РезультатПроверки.ФизическоеЛицоУникально 
		...
		Если НЕ РезультатПроверки.ДоступнаРольСохранениеДанныхЗадвоенныхФизическихЛиц Тогда
			ТекстСообщения = НСтр("es='¡La anotación es imposible! Para el permiso de los conflictos, se dirijan al administrador del sistema informativo.';ru='Запись невозможна! Для разрешения конфликтов, обратитесь к администратору информационной системы.';en='Writing is not possible! To resolve conflicts, contact the administrator of the information system.'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, , , , Отказ);
		ИначеЕсли Форма.Параметры.Ключ.Пустая() Тогда
			ТекстСообщения = НСтр("es='¡El registro de datos de la nueva persona es imposible!';ru='Запись данных нового человека невозможна!';en='Writing data of a new person is not possible!'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, , , , Отказ);
		КонецЕсли; 
