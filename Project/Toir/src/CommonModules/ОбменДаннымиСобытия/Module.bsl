...
Процедура ДополнитьПолучателей(Объект, Узлы) Экспорт
	...
	Для Каждого Элемент Из Узлы Цикл
		...
		Исключение
			...
			СтрокаСообщения = НСтр("es='Para la composición del plan del cambio [ИмяПланаОбмена] no es indicado el registro del objeto [»«½¡«Ñ¿¼n]';ru='Для состава плана обмена [ИмяПланаОбмена] не указана регистрация объекта [ПолноеИмя]';en='For the composition of the exchange plan [ИмяПланаОбмена], the registration of the object [ПолноеИмя] is not indicated'");
	...
	Иначе
		...
		Если РежимВыгрузкиОбъекта = Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПриНеобходимости Тогда
			...
			Если ЭтоНовыйОбъект Тогда
				ВызватьИсключение НСтр("es='El registro de los objetos no anotados descargados por la referencia, no es apoyado.';ru='Регистрация незаписанных объектов, выгружаемых по ссылке, не поддерживается.';en='Registration of unwritten objects that are unloaded by reference is not supported.'");
			КонецЕсли;
...
Процедура ПроверитьВозможностьИзмененияНастроекОбменаДанными(Источник, Отказ) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		ВызватьИсключение НСтр("es='El cambio del nombre y el código de la sincronización de los datos es inadmisible.';ru='Изменение наименования и кода синхронизации данных недопустимо.';en='Changing the name and data synchronization code is not allowed.'");
...
Процедура ЗарегистрироватьИзменениеОбъекта(ИмяПланаОбмена, Объект, Отказ, ДополнительныеПараметры = Неопределено)
		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			...
			Иначе
				...
				Если ЭтоРазделенныеДанные Тогда
					ВызватьИсключение НСтр("es='El registro de los cambios de los datos divididos en el régimen no dividido.';ru='Регистрация изменений разделенных данных в неразделенном режиме.';en='Registration of changes in shared data in non-shared mode.'");
				КонецЕсли;
	...
	Исключение
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió registrar los cambios sobre los nudos del plan del cambio %1 por la causa: %2';ru='Не удалось зарегистрировать изменения на узлах плана обмена %1 по причине: %2';en='Failed to register changes to exchange plan nodes %1 due to: %2'"),
			ИмяПланаОбмена,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		
		ЗаписьЖурналаРегистрации(НСтр("es='El cambio para los datos. Las reglas del registro de los objetos';ru='Обмен данными.Правила регистрации объектов';en='Data exchange. Object registration rules'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ОбъектМодифицированДляПланаОбмена(Источник, ОбъектМетаданных, ИмяПланаОбмена, РежимЗаписи, ЗарегистрироватьОбъектКВыгрузке)
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta de la definición модифицированности del objeto: %1';ru='Ошибка определения модифицированности объекта: %1';en='Error determining object modifiability: %1'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура ВыполнитьПравилаРегистрацииОбъектовДляПланаОбмена(МассивУзловРезультат, Объект, ИмяПланаОбмена, ДополнительныеПараметры)
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta de la ejecución de las reglas del registro de los objetos para el plan del cambio %1. "
"La descripción de la falta: "
"%2';ru='Ошибка выполнения правил регистрации объектов для плана обмена %1."
"Описание ошибки:"
"%2';en='Failed to execute object registration rules for exchange plan %1."
"Error description:"
"%2'"),
			ИмяПланаОбмена,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ЗначенияСвойствДляСсылки(Ссылка, СвойстваОбъекта, Знач СвойстваОбъектаСтрокой, Знач ОбъектМетаданныхИмя)
	...
	Исключение
		СтрокаСообщения = НСтр("es='La falta a la recepción de las propiedades de la referencia. La falta de la ejecución de la interpelación: [ОписаниеОшибки]';ru='Ошибка при получении свойств ссылки. Ошибка выполнения запроса: [ОписаниеОшибки]';en='Error getting link properties. Request execution error: [ОписаниеОшибки]'");
...
Функция МассивУзловПоЗначениямСвойств(ЗначенияСвойств, Знач ТекстЗапроса, Знач ИмяПланаОбмена, Знач ИмяРеквизитаФлага, Знач Выгрузка = Ложь) Экспорт
	...
	Исключение
		СтрокаСообщения = НСтр("es='La falta a la recepción de la lista de los nudos de los destinatarios. La falta de la ejecución de la interpelación: [ОписаниеОшибки]';ru='Ошибка при получении списка узлов получателей. Ошибка выполнения запроса: [ОписаниеОшибки]';en='Error occurred while getting the list of recipient nodes. Request execution error: [ОписаниеОшибки]'");
		СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОписаниеОшибки]", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение СтрокаСообщения;
	КонецПопытки;
...
Функция НаборЗаписейПоТипу(ОбъектМетаданных)
	...
	Иначе
		...
		СтрокаСообщения = НСтр("es='Para el objeto de los metadatos %1 no es previsto el juego de las anotaciones.';ru='Для объекта метаданных %1 не предусмотрено набора записей.';en='Metadata object% 1 does not have a recordset.'");
		...
		Иначе
			...
			Если СтрокаДерева.ВидЭлементаОтбора = ОбменДаннымиСервер.ЭлементОтбораСвойствоАлгоритмЗначения() Тогда
				...
				Исключение
					...
					СтрокаСообщения = НСтр("es='La falta del algoritmo del cálculo del significado de la constante: "
"el Plan del cambio: [ИмяПланаОбмена] "
"el Objeto de los metadatos: [ОбъектМетаданныхИмя] "
"la Descripción de la falta: [la Descripción"
"] el Algoritmo:"
"//{el Comienzo del algoritmo} "
"[ЗначениеКонстанты]"
"//{la Terminación del algoritmo}';ru='Ошибка алгоритма вычисления значения константы:"
...
"// {Начало алгоритма}"
"[ЗначениеКонстанты]"
"// {Окончание алгоритма}';en='Error algorithm for calculating the constant value:"
"Exchange plan: [ИмяПланаОбмена]"
"Объект метаданных: [ОбъектМетаданныхИмя]       Error description: [Описание]  Algorithm:                                          // {Начало алгоритма}"
"[ЗначениеКонстанты]"
"// {Окончание алгоритма}'");
...
Процедура ВыполнитьОбработчикПРОПередОбработкой(ПРО, Отказ, Объект, ОбъектМетаданных, Знач Выгрузка)
	...
	Если ПРО.ЕстьОбработчикПередОбработкой Тогда
		...
		Исключение
			СтрокаСообщения = НСтр("es='La falta a la ejecución del elaborador: ""[ИмяОбработчика]""; "
"el Plan del cambio: [ИмяПланаОбмена]; "
"el Objeto de los metadatos: [ОбъектМетаданныхИмя] "
"la Descripción de la falta: [la Descripción]';ru='Ошибка при выполнении обработчика: ""[ИмяОбработчика]"";"
"План обмена: [ИмяПланаОбмена];"
"Объект метаданных: [ОбъектМетаданныхИмя]"
"Описание ошибки: [Описание]';en='Error executing handler: ""[ИмяОбработчика]"";           Exchange plan: [ИмяПланаОбмена];                    Metadata object:                     [ОбъектМетаданныхИмя]            Error description: [Описание]'");
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяОбработчика]",      НСтр("es='Ante el tratamiento';ru='Перед обработкой';en='Before processing'"));
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяПланаОбмена]",      ПРО.ИмяПланаОбмена);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОбъектМетаданныхИмя]", ПРО.ОбъектМетаданныхИмя);
...
Процедура ВыполнитьОбработчикПРОПриОбработке(Отказ, ПРО, Объект, ДополнительныеПараметры)
	...
	Если ПРО.ЕстьОбработчикПриОбработке Тогда
		...
		Исключение
			СтрокаСообщения = НСтр("es='La falta a la ejecución del elaborador: ""[ИмяОбработчика]""; el Plan del cambio: [ИмяПланаОбмена]; el Objeto de los metadatos: [ОбъектМетаданныхИмя] "
"la Descripción de la falta: [la Descripción]';ru='Ошибка при выполнении обработчика: ""[ИмяОбработчика]""; План обмена: [ИмяПланаОбмена]; Объект метаданных: [ОбъектМетаданныхИмя]"
"Описание ошибки: [Описание]';en='Error executing handler: ""[ИмяОбработчика]"";           Exchange plan: [ИмяПланаОбмена];                    Metadata object:                     [ОбъектМетаданныхИмя]            Error description: [Описание]'");
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяОбработчика]",      НСтр("es='Al tratamiento';ru='При обработке';en='When processing'"));
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяПланаОбмена]",      ПРО.ИмяПланаОбмена);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОбъектМетаданныхИмя]", ПРО.ОбъектМетаданныхИмя);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[Описание]",            ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение СтрокаСообщения;
...
"la Descripción de la falta: [la Descripción]';ru='Ошибка при выполнении обработчика: ""[ИмяОбработчика]""; План обмена: [ИмяПланаОбмена]; Объект метаданных: [ОбъектМетаданныхИмя]"
"Описание ошибки: [Описание]';en='Error executing handler: ""[ИмяОбработчика]"";           Exchange plan: [ИмяПланаОбмена];                    Metadata object:                     [ОбъектМетаданныхИмя]            Error description: [Описание]'");
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяОбработчика]",      НСтр("es='Al tratamiento (adicional)';ru='При обработке (дополнительный)';en='During processing (optional)'"));
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяПланаОбмена]",      ПРО.ИмяПланаОбмена);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОбъектМетаданныхИмя]", ПРО.ОбъектМетаданныхИмя);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[Описание]",            ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение СтрокаСообщения;
...
"la Descripción de la falta: [la Descripción]';ru='Ошибка при выполнении обработчика: ""[ИмяОбработчика]""; План обмена: [ИмяПланаОбмена]; Объект метаданных: [ОбъектМетаданныхИмя]"
"Описание ошибки: [Описание]';en='Error executing handler: ""[ИмяОбработчика]"";           Exchange plan: [ИмяПланаОбмена];                    Metadata object:                     [ОбъектМетаданныхИмя]            Error description: [Описание]'");
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяОбработчика]",      НСтр("es='Después del tratamiento';ru='После обработки';en='After processing'"));
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ИмяПланаОбмена]",      ПРО.ИмяПланаОбмена);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[ОбъектМетаданныхИмя]", ПРО.ОбъектМетаданныхИмя);
			СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "[Описание]",            ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение СтрокаСообщения;
...
Функция ДанныеРазличаются(Данные1, Данные2, СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("es='Es dado el significado inadmisible de parámetro de [1] métodos de ObschegoNaznacheniya. ЗначенияСвойствИзменены.';ru='Задано недопустимое значение параметра [1] метода ОбщегоНазначения.ЗначенияСвойствИзменены.';en='Invalid method parameter [1] value specified ОбщегоНазначения.ЗначенияСвойствИзменены.'");
...
Процедура ВыполнитьОбработчикВПривилегированномРежиме(Значение, Знач СтрокаОбработчика)
	...
	Если ТекущийРежимЗапуска() = РежимЗапускаКлиентскогоПриложения.УправляемоеПриложение Тогда
		ВызватьИсключение НСтр("es='El método no es apoyado en el régimen de la aplicación dirigida.';ru='Метод не поддерживается в режиме управляемого приложения.';en='The method is not supported in managed application mode.'");
	КонецЕсли;
	
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		ВызватьИсключение НСтр("es='El método no es apoyado en el trabajo en el modelo del servicio.';ru='Метод не поддерживается при работе в модели сервиса.';en='The method is not supported when working in the service model.'");
	КонецЕсли;
		...
		Если ОбменДаннымиПовтИсп.ИспользуетсяВерсионирование(Отправитель) Тогда
			Если СсылкаСуществует Тогда
				...
				Если ЗаписатьОбъект Тогда
					Комментарий = НСтр("es='La versión anterior (el permiso automático del conflicto).';ru='Предыдущая версия (автоматическое разрешение конфликта).';en='Previous version (automatic conflict resolution).'");
				Иначе
					Комментарий = НСтр("es='La versión corriente (el permiso automático del conflicto).';ru='Текущая версия (автоматическое разрешение конфликта).';en='Current version (automatic conflict resolution).'");
				КонецЕсли;
			...
			Если ЗаписатьОбъект Тогда
				...
				СведенияОВерсииОбъекта.Вставить("Комментарий", НСтр("es='La versión corriente (el permiso automático del conflicto).';ru='Текущая версия (автоматическое разрешение конфликта).';en='Current version (automatic conflict resolution).'"));
			Иначе
				СведенияОВерсииОбъекта.Вставить("АвторВерсии", Отправитель);
				СведенияОВерсииОбъекта.Вставить("ТипВерсииОбъекта", "НепринятыеДанныеПоКоллизии");
				СведенияОВерсииОбъекта.Вставить("Комментарий", НСтр("es='La versión declinada (el permiso automático del conflicto).';ru='Отклоненная версия (автоматическое разрешение конфликта).';en='Rejected version (automatic conflict resolution).'"));
			КонецЕсли;
...
Процедура ЗарегистрироватьЗапретЗагрузкиДанныхПоДате(ЭлементДанных, Отправитель, СообщениеОбОшибке)
	...
	Если ОбменДаннымиПовтИсп.ИспользуетсяВерсионирование(Отправитель.Ссылка) И ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(ЭлементДанных.Метаданные()) Тогда
		...
		Если СсылкаСуществует Тогда
			...
			Комментарий = НСтр("es='La versión es creada a la sincronización de los datos.';ru='Версия создана при синхронизации данных.';en='Version created during data synchronization.'");
		...
		Иначе
			...
			СтрокаСообщенияОбОшибке = НСтр("es='%1 se prohíbe cargar en el período prohibido. El %2%2%3';ru='%1 запрещено загружать в запрещенный период.%2%2%3';en='% 1 is forbidden to load during the forbidden period.% 2% 2% 3'");
...
Процедура ЗарегистрироватьПредупреждениеОКоллизииВЖурналеРегистрации(Объект, МетаданныеОбъекта, ЗаписатьОбъект, ЭтоСсылочныйТип)
	...
	Если ЗаписатьОбъект Тогда
		...
		ТекстПредупрежденияЖР = НСтр("es='Ha surgido el conflicto de los cambios de los objetos. "
"El objeto de esta base informativa era sustituido por la versión del objeto de la segunda base informativa.';ru='Возник конфликт изменений объектов."
"Объект этой информационной базы был заменен версией объекта из второй информационной базы.';en='There is a conflict of object changes."
"The object of this infobase has been replaced by a version of the object from the second infobase.'");
		
	Иначе
		
		ТекстПредупрежденияЖР = НСтр("es='Ha surgido el conflicto de los cambios de los objetos. "
"El objeto de la segunda base informativa no es aceptado. El objeto de esta base informativa no es cambiado.';ru='Возник конфликт изменений объектов."
"Объект из второй информационной базы не принят. Объект этой информационной базы не изменен.';en='There is a conflict of object changes."
...
Функция ДанныеОбъектаСтрокойДоИзменения(Объект, МетаданныеОбъекта, ЭтоСсылочныйТип, СсылкаСуществует, ОбъектВБазе)
	...
	Если ЭтоСсылочныйТип Тогда
		...
		Иначе
			...
			СтрокаОбъекта = НСтр("es='El objeto es quitado';ru='Объект удален';en='Object deleted'");
