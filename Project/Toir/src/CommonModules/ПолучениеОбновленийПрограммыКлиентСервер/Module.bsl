...
Функция НовыйКонтекстПолученияИУстановкиОбновлений(Параметры) Экспорт
	...
	Если Параметры.УстановитьИсправления Тогда
		...
		// АПК:643-выкл часовой пояс не имеет значения.
		УдалитьУстаревшиеФайлы(
			Результат.КаталогДляРаботыСИсправлениями,
			ТекущаяДата() - 7776000); // 7776000 = 60 * 60 * 24 * 90. // Не локализуется
		// АПК:643-вкл
	...
	Результат.Вставить("ТекущееДействие",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La preparación para la recepción de la renovación...';ru='Подготовка к получению обновления...';en='Preparing to receive the update ...'"),
			КоличествоФайлов));
...
Процедура УдалитьУстаревшиеФайлы(Каталог, ДатаАктуальности)
	...
	Если ИменаФайловСОшибкамиУдаления.Количество() > 0 Тогда
		ПолучениеОбновленийПрограммыВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió quitar los ficheros que han caducado de las correcciones (патчей): "
""
"%1 %2';ru='Не удалось удалить устаревшие файлы исправлений (патчей):"
"%1"
"%2';en='Failed to delete obsolete corrections files (patch):"
"%1"
"%2 '"),
...
Процедура СоздатьКаталогиДляПолученияОбновления(Обновление, Контекст) Экспорт
	...
	Исключение
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta a la creación del catálogo para la conservación del distributivo de la configuración (%1).';ru='Ошибка при создании каталога для сохранения дистрибутива конфигурации (%1).';en='Error creating directory to save configuration distribution (%1).'"),
		...
		Контекст.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió crear el catálogo %1 para la conservación del distributivo de la configuración. %2';ru='Не удалось создать каталог %1 для сохранения дистрибутива конфигурации. %2';en='Failed to create directory%1 to save configuration distribution. %2'"),
			ВременныйКаталогОбновленийКонфигурации,
	...
	Исключение
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta a la creación de catálogo (%1).';ru='Ошибка при создании каталога (%1).';en='Error creating directory (%1).'"),
		...
		Контекст.ИнформацияОбОшибке = СообщениеЖурнала;
		Контекст.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió crear el catálogo %1. %2';ru='Не удалось создать каталог %1. %2';en='Failed to create directory%1. %2'"),
			КаталогИндексаФайлов,
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		Возврат;
	...
	Если Обновление.ФорматФайлаОбновления <> "zip" Тогда
		...
		Исключение
			...
			СообщениеЖурнала =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La falta a la creación del catálogo del distributivo de la configuración (%1).';ru='Ошибка при создании каталога дистрибутива конфигурации (%1).';en='An error occurred while creating the configuration distribution directory (%1).'"),
	...
	Если Обновление.ФорматФайлаОбновления = "zip" Тогда
		...
		Если Не ФайлСуществует(Обновление.ПолноеИмяCFUФайлаВКаталогеДистрибутивов, Ложь) Тогда
			...
			Контекст.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El fichero incorrecto del distributivo %1. Falta el fichero de la renovación de la configuración %2.';ru='Некорректный файл дистрибутива %1. Отсутствует файл обновления конфигурации %2.';en='Invalid distribution file%1. Missing configuration update file%2.'"),
			...
			Контекст.Сообщение = НСтр("es='El distributivo no contiene el fichero de la renovación de la configuración.';ru='Дистрибутив не содержит файл обновления конфигурации.';en='The distribution does not contain a configuration update file.'");
			Возврат;
...
Функция ИмяСобытияЖурналаРегистрации(КодОсновногоЯзыка) Экспорт
	...
	Возврат НСтр("es='La recepción de las renovaciones del programa';ru='Получение обновлений программы';en='Getting software updates'", КодОсновногоЯзыка);
