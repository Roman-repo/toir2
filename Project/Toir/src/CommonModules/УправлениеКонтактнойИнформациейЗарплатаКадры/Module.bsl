	...
	Если НастройкиОтображенияАдреса = Неопределено Тогда
		...
		Если Не ПустаяСтрока(Адрес) Тогда
			...
			Если Не АдресныйКлассификаторЗагружен Тогда
				...
				СообщенияПроверки = НСтр("es='El clasificador de domicilios no es cargado';ru='Адресный классификатор не загружен';en='Address classifier not loaded'");
			...
			Иначе
				...
				Если РезультатПроверки.Результат <> "Корректный" Тогда
					...
					СообщенияПроверки = НСтр("es='La dirección no corresponde al clasificador de domicilios"
"';ru='Адрес не соответствует адресному классификатору"
"';en='The address does not match the address classifier"
"'") + СообщенияПроверки;
			...
			СообщенияПроверки = ?(ПустаяСтрока(СообщенияПроверки), НСтр("es='La dirección es introducida correctamente - en concordancia con las exigencias';ru='Адрес введен правильно - в соответствии с требованиями';en='The address is entered correctly - in accordance with the requirements'"), СообщенияПроверки);
