...
Процедура ПриНачалеРаботыСистемы(Параметры) Экспорт
	...
	Если ФиксированныеПараметрыРезервногоКопированияИБ.ПроведеноВосстановление Тогда
		ТекстОповещения = НСтр("es='La reconstitución de los datos es pasada con éxito.';ru='Восстановление данных проведено успешно.';en='Data recovery was successful.'");
		ПоказатьОповещениеПользователя(НСтр("es='Los datos son restablecidos.';ru='Данные восстановлены.';en='Data recovered.'"), , ТекстОповещения);
	КонецЕсли;
...
Процедура ПередЗавершениемРаботыСистемы(Отказ, Предупреждения) Экспорт
	...
	ПараметрыПредупреждения.ТекстФлажка = НСтр("es='Cumplir la copia de reserva';ru='Выполнить резервное копирование';en='Back up'");
	ПараметрыПредупреждения.Приоритет = 50;
	ПараметрыПредупреждения.ТекстПредупреждения = НСтр("es='del trabajo.';ru='Не выполнено резервное копирование при завершении работы.';en='Backup failed at shutdown.'");
...
Процедура ПровестиРезервноеКопирование()
	...
	Кнопки.Добавить("Отложить", НСтр("es='Aplazar para 15 minutos';ru='Отложить на 15 минут';en='Set aside for 15 minutes'"));
	
	ОписаниеОповещение = Новый ОписаниеОповещения("ПровестиРезервноеКопированиеЗавершение", ЭтотОбъект);
	ПоказатьВопрос(ОписаниеОповещение, НСтр("es='Todo es preparado para la ejecución de la copia de reserva por el horario. "
"¿Cumplir la copia de reserva es ahora?';ru='Все готово для выполнения резервного копирования по расписанию."
"Выполнить резервное копирование сейчас?';en='Everything is ready for scheduled backups."
"Back up now?'"),
		Кнопки, 30, "Да", НСтр("es='La copia de reserva por el horario';ru='Резервное копирование по расписанию';en='Scheduled backup'"), "Да");
...
Процедура ПроверитьРезервноеКопированиеИБ(Параметры)
	...
	Иначе
		...
		ПоказатьОповещениеПользователя(НСтр("es='La copia de reserva';ru='Резервное копирование';en='Backup'"),
			"e1cib/command/ОбщаяКоманда.ПоказатьРезультатРезервногоКопирования",
			НСтр("es='La copia de reserva es pasada con éxito';ru='Резервное копирование проведено успешно';en='Successful backup'"), БиблиотекаКартинок.Информация32);
...
Процедура ОповеститьПользователяОРезервномКопировании(ВариантОповещения)
	...
	Если ВариантОповещения = "Просрочено" Тогда
		...
		ТекстПояснения = НСтр("es='La copia automática de reserva no era cumplida.';ru='Автоматическое резервное копирование не было выполнено.';en='Automatic backup failed.'"); 
		ПоказатьОповещениеПользователя(НСтр("es='La copia de reserva';ru='Резервное копирование';en='Backup'"),
	...
	ИначеЕсли ВариантОповещения = "ЕщеНеНастроено" Тогда
		...
		ТекстПояснения = НСтр("es='Se recomienda ajustar la copia de reserva de la base informativa.';ru='Рекомендуется настроить резервное копирование информационной базы.';en='It is recommended to set up backup of the infobase.'"); 
		ПоказатьОповещениеПользователя(НСтр("es='La copia de reserva';ru='Резервное копирование';en='Backup'"),
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("es='La copia de reserva de la base informativa';ru='Резервное копирование информационной базы';en='Backing up the infobase'",
	...
	Если ПараметрыУдаления.ТипОграничения <> "ХранитьВсе" И КаталогХранения <> Неопределено Тогда
		...
		Исключение
			...
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка",
				НСтр("es='No consiguió pasar la limpieza del catálogo con las copias de reserva.';ru='Не удалось провести очистку каталога с резервными копиями.';en='Failed to clean up the backup directory.'") + Символы.ПС 
...
Функция КлиентскиеПараметрыРезервногоКопирования() Экспорт
	...
	СтруктураПараметров.Вставить("СобытиеЖурналаРегистрации", НСтр("es='La copia ИБ de reserva';ru='Резервное копирование ИБ';en='IB backup'"));
