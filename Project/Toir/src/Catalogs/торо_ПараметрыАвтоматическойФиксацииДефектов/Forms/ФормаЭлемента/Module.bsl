...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	Если НЕ Объект.ЗаписыватьДокумент
		...
		ПоказатьПредупреждение(,НСтр("es='Para la anotación del objeto escojan la acción con el documento: abrir o anotar.';ru='Для записи объекта выберите действие с документом: открывать или записывать.';en='To record an object, select the action with the document: open or record.'"));
		Отказ = Истина;
	КонецЕсли;
	...
	Если НЕ Отказ И Объект.ЗаписыватьДокумент И Объект.ПроводитьДокумент Тогда
		...
		КонецЦикла;
		Если Отказ Тогда
			ПоказатьПредупреждение(,НСтр("es='Para la anotación del objeto rellenen los campos distinguido por el subrayado o quiten el banderín ""Pasar el documento"".';ru='Для записи объекта заполните поля выделенные подчеркиванием или снимите флажок ""Проводить документ"".';en='To record an object, fill in the fields underlined or uncheck the ""Post document"" box.'"));
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнениеШапкиШапкаЗначениеПоляНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТекДанные <> Неопределено И ТекДанные.ШапкаИмяПоля	= "Инициатор" Тогда
		...
		Иначе
			...
			СписокВыбораТипов.Добавить("Строка", 		НСтр("es='La cadena de';ru='Строка';en='Line'"));
			СписокВыбораТипов.Добавить("Подразделение",	НСтр("es='Unidad';ru='Подразделение';en='Subdivision'"));
			СписокВыбораТипов.Добавить("Контрагент", 	НСтр("es='La contraparte';ru='Контрагент';en='Counterparty'"));
			// << [Деснол] [Ушаков А.Д.] [23.10.2019] 
			СписокВыбораТипов.ПоказатьВыборЭлемента(Новый ОписаниеОповещения("ВыбратьИнициатора",ЭтотОбъект,ТекДанные),НСтр("es='Escojan el tipo';ru='Выберите тип';en='Choose type'"));
		КонецЕсли;
