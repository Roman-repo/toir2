...
Процедура УстановитьЗаголовок()
	...
	Иначе
		...
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 (el Significado de la propiedad %2)';ru='%1 (Значение свойства %2)';en='%1 (Property Value%2)'"),
				Объект.Наименование,
				ИмяСвойства);
		Иначе
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El significado de la propiedad %1 (Creación)';ru='Значение свойства %1 (Создание)';en='Property Value%1 (Creation)'"), ИмяСвойства);
		КонецЕсли;
	КонецЕсли;
