...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Los destinatarios del envío (%1)';ru='Получатели рассылки (%1)';en='Mailing recipients (%1)'"),
...
Процедура ОткрытьФормаДобавленияПолучателей(ЭтоПодбор)
	...
	Если ЭтоПодбор Тогда
		...
		ПараметрыФормыВыбора.Вставить("ЗаголовокФормыПодбора", НСтр("es='La selección de los destinatarios del envío';ru='Подбор получателей рассылки';en='Selection of mailing recipients'"));
...
Процедура ДобавитьПеретащитьПолучателя(ПолучательИлиНаборПолучателей)
	...
	Если МассивНовыхСтрок.Количество() > 0 Тогда
		Если МассивНовыхСтрок.Количество() = 1 Тогда
			ЗаголовокОповещения = НСтр("es='El destinatario es añadido en el envío';ru='Получатель добавлен в рассылку';en='Recipient added to newsletter'");
		Иначе
			ЗаголовокОповещения = НСтр("es='Los destinatarios son añadidos en el envío';ru='Получатели добавлены в рассылку';en='Recipients added to the newsletter'");
		КонецЕсли;
