...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.Автор) Тогда
			...
			ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("es='El envío de los informes. La apertura de la forma del elemento';ru='Рассылка отчетов. Открытие формы элемента';en='Mailing reports. Opening an item form'", ОбщегоНазначения.КодОсновногоЯзыка()));
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta del relleno del autor del envío: "
"el Usuario ""%1"" (%2) no puede ser el autor.';ru='Ошибка заполнения автора рассылки:"
"Пользователь ""%1"" (%2) не может быть автором.';en='Error populating the mailing list author:"
"User """"%1 """" (%2) cannot be an author. '"),
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		...
		Элементы.ТемаПисьмаКонтекстноеМенюПредпросмотрШаблона.Доступность = Ложь;
		Элементы.ТекстПисьмаФорматированныйДокументПредпросмотрШаблона.Доступность = Ложь;
		Элементы.ТекстПисьмаФорматированныйДокументКонтекстноеМенюПредпросмотрШаблона.Доступность = Ложь;
		Элементы.ПредпросмотрШаблона.Доступность = Ложь;
		Элементы.ТекстПисьмаКонтекстноеМенюПредпросмотрШаблона.Доступность = Ложь;
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(Объект.Наименование) Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("es='el nombre';ru='Не введено наименование';en='Name not entered'");
	...
	Если Объект.ИспользоватьЭлектроннуюПочту И Не Объект.Личная Тогда
		Если Не ЗначениеЗаполнено(ТипПолучателейРассылки) Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("es='de los destinatarios';ru='Не выбран тип получателей';en='No recipient type selected'");
	...
	Если Объект.Подготовлена Тогда
		Если Объект.Отчеты.Количество() = 0 Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("es='No es escogido ni un informe';ru='Не выбрано ни одного отчета';en='No reports selected'");
		...
		Если Не ЗначениеЗаполнено(Объект.ПериодичностьРасписания) Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("es='del lanzamiento';ru='Не выбрана периодичность запуска';en='No startup frequency selected'");
		...
		Если Объект.ИспользоватьFTPРесурс Тогда
			Если Не ЗначениеЗаполнено(Объект.FTPСервер)
				...
				Отказ = Истина;
				ТекстСообщения = НСтр("es='la dirección';ru='Не введен FTP адрес';en='FTP address not entered'");
		...
		Если Объект.ИспользоватьСетевойКаталог Тогда
			Если Не ЗначениеЗаполнено(Объект.СетевойКаталогWindows) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("es='Windows de red';ru='Не введен сетевой каталог Windows';en='Windows network directory not entered'");
			...
			Если Не ЗначениеЗаполнено(Объект.СетевойКаталогLinux) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("es='Linux de red';ru='Не введен сетевой каталог Linux';en='Linux network directory not entered'");
		...
		Если Объект.ИспользоватьПапку Тогда
			Если Не ЗначениеЗаполнено(Объект.Папка) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("es='la carpeta';ru='Не выбрана папка';en='No folder selected'");
		...
		Если Объект.ИспользоватьЭлектроннуюПочту Тогда
			Если Объект.Личная Тогда
				Если Не ЗначениеЗаполнено(Объект.ВидПочтовогоАдресаПолучателей) Тогда
					Отказ = Истина;
					ТекстСообщения = НСтр("es='la dirección postal';ru='Не выбран почтовый адрес';en='No email address selected'");
			...
			Иначе
				...
				Если Не ЗначениеЗаполнено(Объект.ВидПочтовогоАдресаПолучателей) Тогда
					Отказ = Истина;
					ТекстСообщения = НСтр("es='de los destinatarios';ru='Не выбран тип почтового адреса получателей';en='No recipient email address type selected'");
			...
			Если Не ЗначениеЗаполнено(Объект.УчетнаяЗапись) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("es='el envío';ru='Не выбрана учетная запись для отправки';en='No account selected to send'");
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Для Каждого СтрокаОтчеты Из Объект.Отчеты Цикл
		...
		Если СтрокаОтчеты.СКД Тогда
			...
			Если Не ЗаполненыВсеОбязательныеНастройки Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es=""Para el informe ' %1 ' son rellenados no todos los ajustes, obligatorio para el relleno. Es necesario rellenar todos los ajustes obligatorios o quitar el banderín ' Es preparada '."";ru=""Для отчета '%1' заполнены не все настройки, обязательные для заполнения. Необходимо заполнить все обязательные настройки или снять флажок 'Подготовлена'."";en=""For report '% 1', not all the required settings are filled out. You must complete all required settings or uncheck the 'Prepared' box."""),
		...
		Если РассылкаНеПерсонализирована И ОтчетПерсонализирован Тогда
			Отказ = Истина;
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es=""En los ajustes del informe ' %1 ' es dada la selección por el destinatario del envío. Es necesario desconectar esta selección o cambiar el tipo del envío en ' el informe para cada destinatario '."";ru=""В настройках отчета '%1' задан отбор по получателю рассылки. Необходимо отключить этот отбор или изменить вид рассылки на 'Свой отчет для каждого получателя'."";en=""In the report settings '% 1', the selection by mailing recipient is set. You must disable this selection or change the type of mailing to 'Own report for each recipient'."""),
				Строка(СтрокаОтчеты.Отчет));
			Поле = "Отчеты["+ Формат(ТекущийОбъект.Отчеты.Индекс(ОбъектСтрокаОтчеты), "ЧН=0; ЧГ=0") +"].Представление";
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ТекущийОбъект, Поле);
		КонецЕсли;
		...
		Если Объект.Персонализирована И	НЕ ОтчетПерсонализирован Тогда
			Отказ = Истина;
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='En los ajustes del informe ""%1"" no es indicada la selección por el destinatario del envío.';ru='В настройках отчета ""%1"" не указан отбор по получателю рассылки.';en='In the settings of the report ""% 1"", the selection by mailing recipient is not specified.'"),
				Строка(СтрокаОтчеты.Отчет));
			Поле = "Отчеты["+ Формат(ТекущийОбъект.Отчеты.Индекс(ОбъектСтрокаОтчеты), "ЧН=0; ЧГ=0") +"].Представление";
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ТекущийОбъект, Поле);
		КонецЕсли;
...
Процедура ТипПолучателейРассылкиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Объект.Получатели.Количество() > 0 Тогда
		...
		СтрокаВопроса = НСтр("es='Para la continuación es necesario limpiar la lista de los destinatarios.';ru='Для продолжения необходимо очистить список получателей.';en='To continue, you must clear the recipient list.'");
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("es='Limpiar';ru='Очистить';en='Clear'"));
...
Процедура FTPСерверИКаталогНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыПроизвольнойФормы.Вставить("Заголовок", НСтр("es='<Indiquen al destinatario>';ru='<Укажите получателя>';en='<Укажите получателя>'"));
...
Процедура ПапкаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если Не ПравоИзмененияПапкиИФайлов(ВыбранноеЗначение) Тогда
		СтандартнаяОбработка = Ложь;
		ТекстПредупреждения = НСтр("es='Es insuficientemente derecho para el cambio de los ficheros de la carpeta ""%1"".';ru='Недостаточно прав для изменения файлов папки ""%1"".';en='Not enough rights to modify files of folder ""%1"".'");
...
Процедура РодительОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если ВыбранноеЗначение = Кэш.ГруппаЛичныхРассылок Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьПредупреждение(, НСтр("es='El grupo escogido se usa solamente para los envíos personales por el correo electrónico';ru='Выбранная группа используется только для личных рассылок по электронной почте';en='The selected group is used only for personal e-mail newsletters.'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ОтчетыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если СтруктураВыбора.СОшибками.Количество > 0 Тогда
		...
		Если СтруктураВыбора.Выбрано.Количество = 1 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió incluir el informe en el envío por la causa: "
"%1';ru='Не удалось включить отчет в рассылку по причине:"
"%1';en='Failed to include the report in the newsletter due to:"
"%1'"), СтруктураВыбора.СОшибками.Текст);
		ИначеЕсли СтруктураВыбора.Успешно.Количество = 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió incluir los informes en el envío por la causa: "
"%1';ru='Не удалось включить отчеты в рассылку по причине:"
"%1';en='Failed to include reports in the newsletter due to:"
"%1'"), СтруктураВыбора.СОшибками.Текст);
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='En el envío es incluido los informes: %1 de %2 "
"Detalles: "
"%3';ru='В рассылку включено отчетов: %1 из %2"
"Подробности:"
"%3';en='Included reports in the newsletter:% 1 of% 2"
"Details:"
"% 3'"),
	...
	Иначе
		...
		Если СтруктураВыбора.Успешно.Количество = 0 Тогда
			...
			ТекстОповещения = НСтр("es='Todos los informes escogidos son incluidos ya en el envío';ru='Все выбранные отчеты уже включены в рассылку';en='All selected reports are already included in the newsletter.'");
		Иначе
			Если СтруктураВыбора.Выбрано.Количество = 1 Тогда
				ЗаголовокОповещения = НСтр("es='El informe es incluido en el envío';ru='Отчет включен в рассылку';en='Report is included in the newsletter'");
			Иначе
				ЗаголовокОповещения = НСтр("es='Los informes son incluidos en el envío';ru='Отчеты включены в рассылку';en='Reports included in the newsletter'");
			КонецЕсли;
...
Процедура ПолучателиРассылкиНажатие(Элемент, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.ТипПолучателейРассылки) Тогда
		ТекстОшибки = НСтр("es='Para la introducción de los destinatarios es necesario escoger su tipo';ru='Для ввода получателей необходимо выбрать их тип';en='For entering recipients, select their type'");
...
Процедура КомандаВыполнитьСейчас(Команда)
	Если Не Объект.Подготовлена Тогда
		ПоказатьПредупреждение(, НСтр("es='El envío no es preparado';ru='Рассылка не подготовлена';en='Mailing not prepared'"));
		Возврат;
	КонецЕсли;
	ПараметрыЗаписи = Новый Структура;
...
Процедура ПредварительныйПросмотрОтчета(Команда)
	...
	СтрокаОтчеты = Элементы.Отчеты.ТекущиеДанные;
	Если СтрокаОтчеты = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("es='Escojan el informe';ru='Выберите отчет';en='Select report'"));
		Возврат;
	КонецЕсли;
...
Процедура УказатьПолучателяРассылки(Команда)
	...
	Если Не Объект.Персонализирована Тогда
		...
		СообщениеТекст = НСтр("es='Usar al destinatario en los parámetros probablemente solamente para el tipo del envío ""%1""';ru='Использовать получателя в параметрах возможно только для вида рассылки ""%1""';en='Use of the recipient in the parameters is possible only for the distribution type ""%1""'");
	...
	Если КоличествоТипов <> 1 И КоличествоТипов <> 2 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("es='Campo ""los Destinatarios"" no es rellenado';ru='Поле ""Получатели"" не заполнено';en='""Recipients"" field is not filled'"), , "ТипПолучателейРассылки");
		Возврат;
	КонецЕсли;
	...
	Если НайденныеИдентификаторыОбъектовМетаданных.Количество() <> 1 Тогда
		ПоказатьПредупреждение(, НСтр("es='El tipo de los destinatarios no es encontrado';ru='Тип получателей не найден';en='Recipient type not found'"));
		Возврат;
	КонецЕсли;
	...
	Если Не Настройка.ОписаниеДоступныхТипов.СодержитТип(ОсновнойТипПолучателей) Тогда
		ТекстПредупреждения = НСтр("es='Tipo ""%1"" no se acerca por el tipo al ajuste escogido. "
"Es necesario escoger otro tipo de los destinatarios del envío u otro ajuste.';ru='Тип ""%1"" не подходит по типу к выбранной настройке."
"Необходимо выбрать другой тип получателей рассылки или другую настройку.';en='Type ""%1"" does not match the selected setting."
"You must select a different type of distribution recipient or a different setting. '");
...
Функция ОпределитьНастройку()
	...
	Если СКД Тогда
		...
		Если ИдентификаторНастройки = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("es='del informe.';ru='Не выбрана настройка отчета.';en='No report setting selected.'"));
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("es='Indicar al destinatario se puede solamente para los parámetros y las selecciones de los informes.';ru='Указывать получателя можно только для параметров и отборов отчетов.';en='You can only specify a recipient for report parameters and selections.'"));
			Возврат Неопределено;
		КонецЕсли;
		...
		Если ЭтоЭлементОтбора Тогда
			...
			Иначе
				...
				КонецЕсли;
				Если ПолеКД = Неопределено Тогда
					ПоказатьПредупреждение(, НСтр("es='Para el ajuste del informe no es encontrada la descripción del campo accesible';ru='Для настройки отчета не найдено описание доступного поля';en='Description of available field was not found for report setup'"));
					Возврат Неопределено;
				КонецЕсли;
...
Процедура ЗаполнитьРасписаниеПоШаблону(Команда)
	...
	СписокВариантов.ПоказатьВыборЭлемента(Обработчик, НСтр("es='Escojan el modelo del horario';ru='Выберите шаблон расписания';en='Choose a schedule template'"));
КонецПроцедуры
...
Процедура ДобавитьШаблонПолучателя(Команда)
	...
	Если НЕ Объект.Персонализирована Тогда
		СообщениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El uso del destinatario en el texto del modelo es posible solamente el tipo del envío ""%1""';ru='Использование получателя в тексте шаблона возможно только для вида рассылки ""%1""';en='Use of the recipient in the text of the template is possible only for the type of mailing ""%1""'"),
...
Процедура ДобавитьШаблонПоУмолчанию(Команда)
	...
	ИначеЕсли ЗначениеПредмета = ШаблонПоУмолчанию Тогда
		...
		Если ПерезаполняетсяТема Тогда
			ТекстПредупреждения = НСтр("es='El tema de la carta corresponde ya al modelo por reticencia.';ru='Тема письма уже соответствует шаблону по умолчанию.';en='The subject line already matches the default template.'");
		Иначе
			ТекстПредупреждения = НСтр("es='El texto de la carta corresponde ya al modelo por reticencia.';ru='Текст письма уже соответствует шаблону по умолчанию.';en='The text of the letter already matches the default template.'");
		КонецЕсли;
		ПоказатьПредупреждение(, ТекстПредупреждения);
	...
	Иначе
		...
		Если ПерезаполняетсяТема Тогда
			ЗаголовокВопроса = НСтр("es='Añadir en el tema de la carta el modelo por reticencia';ru='Добавить в тему письма шаблон по умолчанию';en='Add a default template to the email subject'");
			ТекстВопроса = НСтр("es='¿Sustituir el tema de la carta por el modelo por reticencia?';ru='Заменить тему письма на шаблон по умолчанию?';en='Replace email subject with default template?'");
		Иначе
			ЗаголовокВопроса = НСтр("es='Añadir en el texto de la carta el modelo por reticencia';ru='Добавить в текст письма шаблон по умолчанию';en='Add a default template to the message text'");
			ТекстВопроса = НСтр("es='¿Sustituir el texto de la carta por el modelo por reticencia?';ru='Заменить текст письма на шаблон по умолчанию?';en='Replace message text with default template?'");
		КонецЕсли;
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(1, НСтр("es='Sustituir';ru='Заменить';en='Replace'"));
		Кнопки.Добавить(2, НСтр("es='Añadir';ru='Добавить';en='Add'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
...
Процедура ПроверитьРассылкуПослеВыбораПолучателей(РезультатВыбора, ПараметрыДоставки) Экспорт
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		ПараметрыДоставки.Вставить("ШаблонТемы",    НСтр("es='El mensaje de test 1С:Предприятие';ru='Тестовое сообщение 1С:Предприятие';en='Test message 1C: Enterprise'"));
		ПараметрыДоставки.Вставить("ШаблонТекста",  НСтр("es='Este mensaje es enviado por el sistema de los envíos 1С:Предприятие.';ru='Это сообщение отправлено системой рассылок 1С:Предприятие.';en='This message was sent by the 1C: Enterprise mailing system.'") + Символы.ПС + Кэш.ЗаголовокСистемы);
	...
	СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(Неопределено, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 %2';ru='%1"
"%2';en='%1"
...
Процедура ВыбратьФормат(СсылкаНаОтчет, ОбработчикРезультата)
	...
	Если ЭтоФорматПоУмолчанию Тогда
		ЗаголовокДиалога = НСтр("es='Escojan los formatos por reticencia';ru='Выберите форматы по умолчанию';en='Choose default formats'");
	Иначе
		ЗаголовокДиалога = НСтр("es='Escojan los formatos para el informe ""%1""';ru='Выберите форматы для отчета ""%1""';en='Select formats for report ""% 1""'");
...
Процедура ПроверитьРассылку(ПараметрыДоставки)
	...
	Если Не Объект.Подготовлена Или Объект.Ссылка.Пустая() Тогда
		ЗаголовокВопроса = НСтр("es='La comprobación del modo de la entrega';ru='Проверка способа доставки';en='Check delivery method'");
		Если Не Объект.Подготовлена Тогда
			ТекстВопроса = НСтр("es='Ante la comprobación el envío debe ser preparado. "
"Presionen ""Continuar"" para incluir el banderín ""es preparada"" y anotar el envío.';ru='Перед проверкой рассылка должна быть подготовлена."
"Нажмите ""Продолжить"", чтобы включить флажок ""Подготовлена"" и записать рассылку.';en='A mailing list must be prepared before verification."
"Click Continue to enable the Prepared checkbox and record the newsletter.'");
		Иначе
			ТекстВопроса = НСтр("es='Ante la comprobación el envío debe ser anotado. "
"Presionen ""Continuar"" para anotar el envío.';ru='Перед проверкой рассылка должна быть записана."
"Нажмите ""Продолжить"", чтобы записать рассылку.';en='Before verification, the newsletter must be recorded."
"Click ""Continue"" to record the newsletter.'");
		КонецЕсли;
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(1, НСтр("en='Resume';es='Continuar';ru='Продолжить'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
...
Процедура ВидимостьДоступностьКорректность(Форма, Изменения = "")
	...
	Если Изменения = "" Или Изменения = "ВидРассылки" Тогда
		...
		Если Объект.Личная <> ИспользуетсяГруппаЛичныхРассылок Тогда
			УстановитьМодифицированностьФормы(Форма, "Родитель", , 
				НСтр("es='El grupo es establecido en concordancia con el tipo del envío';ru='Группа установлена в соответствии с видом рассылки';en='The group is set according to the type of distribution.'"));
	...
	Если Изменения = "" Или Изменения = "Отчеты" Тогда
		...
		Если КоличествоОтчетов > 0 Тогда
			Элементы.СтраницаОтчеты.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Los informes (%1)';ru='Отчеты (%1)';en='Reports (%1)'"), 
				Формат(КоличествоОтчетов, "ЧН=0; ЧГ="));
		Иначе
			Элементы.СтраницаОтчеты.Заголовок = НСтр("es='Los informes';ru='Отчеты';en='Reports'") ;
		КонецЕсли;
	КонецЕсли;
	...
	Если Изменения = ""
		...
		СпособыДоставки = "";
		Если Объект.ИспользоватьПапку Тогда
			СпособыДоставки = НСтр("es='La carpeta';ru='папка';en='folder'");
		КонецЕсли;
		Если Объект.ИспользоватьСетевойКаталог Тогда
			СпособыДоставки = НСтр("es='El catálogo de red';ru='сетевой каталог';en='network directory'");
		КонецЕсли;
		Если Объект.ИспользоватьFTPРесурс Тогда
			СпособыДоставки = НСтр("es='FTP';ru='FTP';en='FTP'");
		КонецЕсли;
		Если Объект.ИспользоватьЭлектроннуюПочту И Не Объект.ТолькоУведомить Тогда
			СпособыДоставки = СпособыДоставки + ?(СпособыДоставки = "", НСтр("es='эл. Los correos';ru='эл. почта';en='email '"), " "+ НСтр("es='Y эл. Los correos';ru='и эл. почта';en='and email'"));
		КонецЕсли;
		
		Элементы.СтраницаДоставка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La entrega (%1)';ru='Доставка (%1)';en='Delivery (%1)'"), СпособыДоставки);
	КонецЕсли;
	...
	Если Изменения = "" Или Изменения = "ВыполнятьПоРасписанию" Тогда
		Если Объект.ВыполнятьПоРасписанию Тогда
			Элементы.СтраницаРасписание.Заголовок = НСтр("es='El horario (activamente)';ru='Расписание (активно)';en='Schedule (active)'");
		Иначе
			Элементы.СтраницаРасписание.Заголовок = НСтр("es='El horario (no activamente)';ru='Расписание (не активно)';en='Schedule (not active)'");
		КонецЕсли;
	...
	Если Изменения = "" Или Изменения = "ПериодичностьРасписания" Тогда
		...
		ИначеЕсли ИмяПеречисления = "Ежемесячно" Тогда
			...
			Если Форма.Расписание.ДеньВМесяце >= 0 Тогда
				...
				Элементы.ГиперссылкаНачалоКонецМесяца.Заголовок = НСтр("es='Los comienzos';ru='начала';en='beginning'");
			Иначе
				Форма.ДеньВМесяце = -Форма.Расписание.ДеньВМесяце;
				Элементы.ГиперссылкаНачалоКонецМесяца.Заголовок = НСтр("es='Del fin';ru='конца';en='the end'");
			КонецЕсли;
...
Функция ПредставлениеПолучателей(Форма)
	...
	Если Представление.Всего = 0 Тогда
		Представление.Краткое = НСтр("es='<Indiquen a los destinatarios>';ru='<Укажите получателей>';en='<Укажите получателей>'");
	...
	Если ПредставлениеОтключенных.Всего > 0 Тогда
		ШаблонРазделения = НСтр("es='Excepto';ru='Кроме';en='Except'")+ ": ";
	...
	Если Представление.МаксимумПревышен
		...
		Если ПредставлениеОтключенных.Всего > 0 Тогда
			ШаблонОкончания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='(Solamente %1, excluyendo %2)';ru='(всего %1, исключая %2)';en='(total% 1 excluding %2)'"),
				Представление.Всего,
				ПредставлениеОтключенных.Всего);
		Иначе
			ШаблонОкончания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='(Solamente %1)';ru='(всего %1)';en='(total %1)'"),
				Представление.Всего);
		КонецЕсли;
...
Функция ПредставлениеФорматовПоУмолчанию()
	Возврат НСтр("es='Por reticencia';ru='по умолчанию';en='default'");
КонецФункции
...
Функция ПолучателиУказаны(Получатели)
	...
	ТекстСообщения = НСтр("es='No es escogido ni un destinatario';ru='Не выбрано ни одного получателя';en='No recipients selected'");
...
Функция ПроверитьСпособДоставки(Рассылка, Знач ПараметрыДоставки)
	...
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия",   НСтр("es='El envío de los informes. La comprobación del modo de la entrega';ru='Рассылка отчетов. Проверка способа доставки';en='Mailing reports. Check delivery method'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если Выполнена Тогда
		РезультатВыполнения.Текст = НСтр("es='El test del modo de la entrega es con éxito pasado.';ru='Тест способа доставки успешно пройден.';en='Delivery method test passed successfully.'");
	Иначе
		РезультатВыполнения.Текст = НСтр("es='El test del modo de la entrega no es pasado.';ru='Тест способа доставки не пройден.';en='Delivery method test failed.'");
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = НСтр("es='<el significado Vacío>';ru='<Пустое значение>';en='<Пустое значение>'");
	
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='[El destinatario]';ru='[Получатель]';en='[Получатель]'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
...
Процедура ЗаполнитьСведенияТаблицыОтчеты()
	...
	Для Каждого СтрокаОтчеты Из Объект.Отчеты Цикл
		...
		Если ИнформацияОбОтчете = Неопределено Тогда
			...
			СтрокаОтчеты.Представление = НСтр("es='<es insuficientemente derecho para el trabajo con el informe>';ru='<Недостаточно прав для работы с отчетом>';en='<Недостаточно прав для работы с отчетом>'");
		Иначе
...
Функция ИнициализироватьОтчет(СтрокаОтчеты, ДобавитьДежурнуюФразу, ПользовательскиеНастройки, Интерактивно = Истина)
	// Параметры журнала
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия",   НСтр("es='El envío de los informes. La inicialización del informe';ru='Рассылка отчетов. Инициализация отчета';en='Mailing reports. Report initialization'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ДобавитьНастройкиОтчетов(ПрисоединяемыеОтчеты)
	...
	Для Каждого СтрокаОтчетыПараметры Из ПрисоединяемыеОтчеты Цикл
		...
		Если Не ЭтоНовый Тогда
			Если Найденные.Количество() > 0 Тогда
				ШаблонСтрокиСообщения = НСтр("es='Para el informe ""%1"" son cargados los nuevos ajustes de usuario.';ru='Для отчета ""%1"" загружены новые пользовательские настройки.';en='For the report ""%1"" new user settings were loaded.'");
			Иначе
				ШаблонСтрокиСообщения = НСтр("es='Es añadido el informe ""%1"".';ru='Добавлен отчет ""%1"".';en='Addied report ""%1"".'");
			КонецЕсли;
...
Процедура ЗаписатьНаКлиенте(Результат, ПараметрыЗаписи) Экспорт
	...
	Если ПараметрыЗаписи.Шаг = 3 И ЖелательноОтключитьАрхивацию() Тогда
		...
		// Вопрос.
		ЗаголовокВопроса = НСтр("es='Desconectar la archivación';ru='Отключить архивацию';en='Disable archiving'");
		ТекстВопроса = НСтр("es='A la publicación de los informes en la carpeta se recomienda desconectar la archivación en ZIP.';ru='При публикации отчетов в папку рекомендуется отключать архивацию в ZIP.';en='When publishing reports to a folder, it is recommended to disable archiving in ZIP.'");
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("es='Desconectar la archivación en ZIP';ru='Отключить архивацию в ZIP';en='Disable ZIP archiving'"));
		Кнопки.Добавить(КодВозвратаДиалога.Пропустить, НСтр("en='Resume';es='Continuar';ru='Продолжить'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
...
Процедура СобытияРассылки()
	...
	Если ПараметрыФормыЖурналаРегистрации = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("es='El envío no se cumplía.';ru='Рассылка еще не выполнялась.';en='Mailing has not yet been performed.'"));
		Возврат;
	КонецЕсли;
