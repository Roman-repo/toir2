...
Процедура Заполнить(Команда)
	Если Сервер = "" Тогда
		ПолныйАдрес = НСтр("es='ftp://el login:parol@server:port/catálogo /';ru='ftp://логин:пароль@сервер:порт/каталог/';en='ftp://логин:пароль@сервер:порт/каталог/'");
	Иначе
		Если Логин = "" Тогда
	...
	ПоказатьВводСтроки(Обработчик, ПолныйАдрес, НСтр("es='Introduzcan completo ftp la dirección';ru='Введите полный ftp адрес';en='Enter full ftp address'"))
КонецПроцедуры
