...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		...
		Элементы.ФормаСоздатьСоСканера.Заголовок = НСтр("es='De la cámara del mecanismo...';ru='С камеры устройства...';en='From the device’s camera ...'");
	КонецЕсли;
...
Процедура ПеренестиВПапку(Команда)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Заголовок",    НСтр("es='La elección de la carpeta';ru='Выбор папки';en='Folder selection'"));
...
Процедура ИмпортФайловПослеУстановкиРасширения(Результат, ПараметрыВыполнения) Экспорт
	...
	ДиалогОткрытияФайла.Заголовок = НСтр("es='Escojan los ficheros';ru='Выберите файлы';en='Select files'");
		...
		ИначеЕсли ТипЗначения = Тип("СправочникСсылка.Файлы") Тогда
			...
			Если Действие = ДействиеПеретаскивания.Копирование Тогда
				...
				Если ЗначениеПеретаскивания.Количество() = 1 Тогда
					ОповещениеЗаголовок = НСтр("es='El fichero es copiado.';ru='Файл скопирован.';en='File is copied.'");
					ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El fichero ""%1"" "
"es copiado en la carpeta ""%2""';ru='Файл ""%1"""
"скопирован в папку ""%2""';en='File ""%1"" "
"copied to folder ""%2""'"),
						ЗначениеПеретаскивания[0],
						Строка(ПапкаДляДобавления));
				Иначе
					ОповещениеЗаголовок = НСтр("es='Los ficheros son copiados.';ru='Файлы скопированы.';en='Files copied.'");
					ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Los ficheros (%1 piezas) son copiados en la carpeta ""%2""';ru='Файлы (%1 шт.) скопированы в папку ""%2""';en='Files (%1 pc.) copied to folder ""%2""'"),
			...
			Иначе
				...
				Если ЗначениеПеретаскивания.Количество() = 1 Тогда
					ОповещениеЗаголовок = НСтр("es='El fichero es llevado.';ru='Файл перенесен.';en='File transferred.'");
					ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El fichero ""%1"" "
...
"moved to folder ""%2""'"),
						...
						Строка(ПапкаДляДобавления));
				Иначе
					ОповещениеЗаголовок = НСтр("es='Los ficheros son llevados.';ru='Файлы перенесены.';en='Files transferred.'");
					ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Los ficheros (%1 piezas) son llevados en la carpeta ""%2""';ru='Файлы (%1 шт.) перенесены в папку ""%2""';en='Files (%1 pcs.) moved to the folder ""%2""'"),
		...
		ИначеЕсли ТипЗначения = Тип("СправочникСсылка.ПапкиФайлов") Тогда
			...
			Если РодительСменен <> Истина Тогда
				Если НайденоЗацикливание = Истина Тогда
					Если ЗначениеПеретаскивания.Количество() = 1 Тогда
						ТекстСообщения = НСтр("es='El traslado es imposible. "
"La carpeta ""%1"" es filial para la carpeta trasladada ""%2"".';ru='Перемещение невозможно."
"Папка ""%1"" является дочерней для перемещаемой папки ""%2"".';en='Moving is not possible."
"The folder ""%1"" is a child of the moved folder ""%2"".'");
					Иначе
						ТекстСообщения = НСтр("es='El traslado es imposible. "
"La carpeta ""%1"" es filial para una de las carpetas trasladadas.';ru='Перемещение невозможно."
"Папка ""%1"" является дочерней для одной из перемещаемых папок.';en='Moving is not possible."
"The folder ""%1"" is a child of one of the moved folders.'");
					КонецЕсли;
			...
			Если ЗначениеПеретаскивания.Количество() = 1 Тогда
				...
				ОповещениеЗаголовок = НСтр("es='La carpeta es llevada.';ru='Папка перенесена.';en='Folder migrated.'");
				ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La carpeta ""%1"" "
"es llevada en la carpeta ""%2""';ru='Папка ""%1"""
"перенесена в папку ""%2""';en='Folder ""%1"""
"moved to folder ""%2 ""'"),
					Строка(ЗначениеПеретаскивания[0]),
					Строка(ПапкаДляДобавления));
			Иначе
				ОповещениеЗаголовок = НСтр("es='Las carpetas son llevadas.';ru='Папки перенесены.';en='Folders migrated.'");
				ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Las carpetas (%1 piezas) son llevadas en la carpeta ""%2""';ru='Папки (%1 шт.) перенесены в папку ""%2""';en='Folders (%1 pcs.) moved to the folder ""%2""'"),
					Строка(ЗначениеПеретаскивания.Количество()),
					Строка(ПапкаДляДобавления));
			КонецЕсли;
			ПоказатьОповещениеПользователя(ОповещениеЗаголовок, , ОповещениеТекст, БиблиотекаКартинок.Информация32);
...
Процедура УстановитьВидимостьПоясненияСинхронизацииФайлов()
	...
	Иначе
		...
		Если Элементы.Папки.ТекущиеДанные.ПапкаСинхронизируется Тогда
			...
			ЧастиСтроки.Добавить(НСтр("es='El trabajo con los ficheros de esta carpeta tiene lugar en el servicio nuboso';ru='Работа с файлами этой папки ведется в облачном сервисе';en='Work with the files of this folder is in the cloud service'"));
