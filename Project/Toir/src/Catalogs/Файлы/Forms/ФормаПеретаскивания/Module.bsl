...
#Если ВебКлиент Тогда
	ТекстПредупреждения =
		НСтр("es='En el cliente web la importación de los ficheros es inaccesible. Usen la orden ""Añadir"" en la lista de los ficheros.';ru='В веб-клиенте импорт файлов недоступен. Используйте команду ""Добавить"" в списке файлов.';en='File import is not available in the web client. Use the Add command in the file list.'");
...
#КонецЕсли
	...
	Если ТолькоКаталоги Тогда
		Заголовок = НСтр("es='La carga de las carpetas';ru='Загрузка папок';en='Download folders'");
	КонецЕсли;
...
Процедура ЗагрузитьФайлы()
	...
	Если ВыбранныеФайлы.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='No hay ficheros para la adición.';ru='Нет файлов для добавления.';en='There are no files to add.'"), , "ВыбранныеФайлы");
		ПоляНеЗаполнены = Истина;
	КонецЕсли;
	
	Если ПапкаДляДобавления.Пустая() Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='Indiquen la carpeta.';ru='Укажите папку.';en='Specify a folder.'"), , "ПапкаДляДобавления");
		ПоляНеЗаполнены = Истина;
	КонецЕсли;
