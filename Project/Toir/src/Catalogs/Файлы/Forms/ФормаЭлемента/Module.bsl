...
Процедура ОбновитьЗаголовок()
	...
	Если ТипЗнч(Объект.ВладелецФайла) = Тип("СправочникСсылка.ПапкиФайлов") Тогда
		ТипФайла = НСтр("es='El fichero';ru='Файл';en='File'");
	Иначе
		ТипФайла = НСтр("es='El fichero juntado';ru='Присоединенный файл';en='Attached file'");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ЭтотОбъект.Объект.Ссылка) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='%1 (%2)';ru='%1 (%2)';en='%1(%2)'"), Строка(ЭтотОбъект.Объект.Ссылка), ТипФайла);
	Иначе
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='%1 (Creación)';ru='%1 (Создание)';en='% 1 (Create)'"), ТипФайла);
	КонецЕсли;
