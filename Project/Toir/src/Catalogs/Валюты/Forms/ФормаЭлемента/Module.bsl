...
Процедура ПараметрыПрописиВалютыНажатие(Элемент)
	...
	Иначе
		ПоказатьВводСтроки(ОписаниеОповещения, Объект.ПараметрыПрописи, НСтр("es='Los parámetros del modelo de escritura de la divisa';ru='Параметры прописи валюты';en='Currency prescription parameters'"));
	КонецЕсли;
