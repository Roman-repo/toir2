...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(ДатаОкончания) И ДатаОкончания < ДатаНачала Тогда
		ТекстСообщения = НСтр("es='La fecha de la terminación es más pequeña que la fecha del comienzo. Lo más probable, la fecha de la terminación es rellenada incorrectamente.';ru='Дата окончания меньше даты начала. Скорее всего, дата окончания заполнена неверно.';en='The end date is less than the start date. Most likely, the end date is incorrect.'");
...
Процедура торо_ПроверитьПересечениеПереходящегоВремени(Отказ)
	...
	Если Не Результат.Пустой() Тогда
		...
		МассивТекстов.Добавить(НСтр("es='Son descubiertas las intersecciones de los períodos entre los días del gráfico:';ru='Обнаружены пересечения периодов между днями графика:';en='Intersections of periods between the days of the chart were found:'"));
