	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ПроизводственныйКалендарь) Тогда
			Если Не ЗначениеЗаполнено(Выборка.ДатаЗаполненностиПроизводственногоКалендаря) Тогда
				ВозможнаяПричина = НСтр("es='El calendario de producción no es rellenado';ru='Производственный календарь не заполнен';en='Production calendar is empty'");
			Иначе
				Если Выборка.ДатаЗаполненностиПроизводственногоКалендаря < ДатаНеобходимойЗаполненности Тогда
					ВозможнаяПричина = НСтр("es='El calendario de producción no es rellenado para el año civil siguiente';ru='Производственный календарь не заполнен на очередной календарный год';en='Production calendar is not filled for the next calendar year'");
				КонецЕсли;
			КонецЕсли;
		Иначе
			Если Не ЗначениеЗаполнено(Выборка.ДатаОкончания) Тогда
				ВозможнаяПричина = НСтр("es='El gráfico no era rellenado para el año civil siguiente';ru='График не был заполнен на очередной календарный год';en='The schedule was not completed for the next calendar year.'");
			Иначе
				Если Выборка.ДатаОкончания < ДатаНеобходимойЗаполненности Тогда
					ВозможнаяПричина = НСтр("es='El período del relleno del gráfico es limitado (cm. El campo «la Fecha de la terminación»)';ru='Период заполнения графика ограничен (см. поле «Дата окончания»)';en='The period for filling out the schedule is limited (see the field ""End date"")'")
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
