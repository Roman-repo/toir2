...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.ШаблонЗаполнения.НайтиСтроки(Новый Структура("ДеньВключенВГрафик", Истина)).Количество() = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("es='los días incluidos en el gráfico del trabajo';ru='Не отмечены дни, включаемые в график работы';en='The days included in the work schedule are not marked'"), , "Объект.ШаблонЗаполнения", , Отказ);
	КонецЕсли;
...
Процедура НомерТекущегоГодаПриИзменении(Элемент)
	...
	Если МодифицированностьРезультата Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='¿Anotar los datos cambiados en %1 año?';ru='Записать измененные данные за %1 год?';en='Write modified data for%1 year?'"), Формат(НомерПредыдущегоГода, "ЧГ=0"));
...
Процедура СформироватьШаблонЗаполнения(СпособЗаполнения, ШаблонЗаполнения, Знач ДлинаЦикла, Знач ДатаОтсчета = Неопределено)
	...
	Если СпособЗаполнения = ПредопределенноеЗначение("Перечисление.СпособыЗаполненияГрафикаРаботы.ПоНеделям") Тогда
		ШаблонЗаполнения[0].ПредставлениеДня = НСтр("es='El lunes';ru='Понедельник';en='Monday'");
		ШаблонЗаполнения[1].ПредставлениеДня = НСтр("es='El martes';ru='Вторник';en='Tuesday'");
		ШаблонЗаполнения[2].ПредставлениеДня = НСтр("es='El ambiente';ru='Среда';en='Wednesday'");
		ШаблонЗаполнения[3].ПредставлениеДня = НСтр("es='El jueves';ru='Четверг';en='Thursday'");
		ШаблонЗаполнения[4].ПредставлениеДня = НСтр("es='El viernes';ru='Пятница';en='Friday'");
		ШаблонЗаполнения[5].ПредставлениеДня = НСтр("es='El sábado';ru='Суббота';en='Saturday'");
		ШаблонЗаполнения[6].ПредставлениеДня = НСтр("es='El domingo';ru='Воскресенье';en='Sunday'");
	Иначе
		ДатаДня = ДатаОтсчета;
...
Функция ПредставлениеРасписанияДня(Форма, НомерДня)
	...
	Для Каждого СтрокаРасписания Из Форма.Объект.РасписаниеРаботы Цикл
		...
		Если СтрокаРасписания.торо_ПереходящееВремяНаСледующийДень Тогда
			ПредставлениеИнтервала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1-%2(перех.), ", Формат(СтрокаРасписания.ВремяНачала, "ДФ=ЧЧ:мм; ДП="), Формат(СтрокаРасписания.ВремяОкончания, "ДФ=ЧЧ:мм; ДП="));
		Иначе
			ПредставлениеИнтервала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1-%2, ", Формат(СтрокаРасписания.ВремяНачала, "ДФ=ЧЧ:мм; ДП="), Формат(СтрокаРасписания.ВремяОкончания, "ДФ=ЧЧ:мм; ДП="));
		КонецЕсли;
		// ТОИР--
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 h. (%2)';ru='%1 ч. (%2)';en='%1 hour (%2)'"), Часов, ПредставлениеИнтервалов);
...
Функция ПредставлениеПустогоРасписания()
	...
	Возврат НСтр("es='Rellenar el horario';ru='Заполнить расписание';en='Fill in the schedule'");
...
Процедура УточнитьЗаполненностьГрафика(Форма)
	...
	Если Не ЗначениеЗаполнено(Форма.ДатаЗаполненности) Тогда
		Форма.ЗаполненностьИнформационныйТекст = НСтр("es='El gráfico del trabajo no es rellenado';ru='График работы не заполнен';en='Schedule is not filled'");
		Форма.ТребуетЗаполнения = Истина;
	Иначе	
		Если Не ЗначениеЗаполнено(Форма.Объект.ГоризонтПланирования) Тогда
			ИнформационныйТекст = НСтр("es='El gráfico del trabajo es rellenado hasta %1';ru='График работы заполнен до %1';en='Schedule filled up to%1'");
		...
		Иначе											
			...
			ИнформационныйТекст = НСтр("es='El gráfico del trabajo es rellenado hasta %1, tomando en cuenta el horizonte de la planificación el gráfico debe ser rellenado hasta %2';ru='График работы заполнен до %1, с учетом горизонта планирования график должен быть заполнен до %2';en='The work schedule is filled up to%1, taking into account the planning horizon, the schedule should be filled up to%2'");
...
Процедура ЗаписатьДанныеГрафикаРаботыНаГод(НомерГода)
	...
	РегистрыСведений.КалендарныеГрафики.ЗаписатьДанныеГрафикаВРегистр(
		Объект.Ссылка, ДниГрафика, Дата(НомерГода, 1, 1), Дата(НомерГода, 12, 31), Истина);
...
Процедура ЗаполнитьИнформационныйТекстРезультатаЗаполнения(Форма)
	...
	Если Форма.РучноеРедактирование Тогда
		ИнформационныйТекст = НСтр("es='El gráfico del trabajo es cambiado al año en curso a mano. "
"Presionen «Rellenar por el modelo» para volver al relleno automático.';ru='График работы на текущий год изменен вручную. "
"Нажмите «Заполнить по шаблону» чтобы вернуться к автоматическому заполнению.';en='The work schedule for the current year has been manually changed."
"Click Fill by template"" to return to automatic completion.'");
	...
	Иначе
		Если Форма.РезультатЗаполненПоШаблону Тогда
			Если ЗначениеЗаполнено(Форма.Объект.ПроизводственныйКалендарь) Тогда
				ИнформационныйТекст = НСтр("es='El gráfico del trabajo se renueva automáticamente con el cambio del calendario de producción por el año en curso.';ru='График работы автоматически обновляется при изменении производственного календаря за текущий год.';en='The work schedule is automatically updated when the production calendar for the current year changes.'");
				ИнформационнаяКартинка = БиблиотекаКартинок.Информация;
			КонецЕсли;
		Иначе
			ИнформационныйТекст = НСтр("es='El resultado representado no corresponde al ajuste del modelo. "
"Presionen «Rellenar por el modelo» para ver como se ve el gráfico del trabajo tomando en cuenta los cambios del modelo.';ru='Отображаемый результат не соответствует настройке шаблона. "
"Нажмите «Заполнить по шаблону», чтобы увидеть как выглядит график работы с учетом изменений шаблона.';en='The displayed result does not match the template setting."
"Click Fill in the template"" to see how the work schedule looks like taking into account template changes. '");
			ИнформационнаяКартинка = БиблиотекаКартинок.Предупреждение;
			ДоступноЗаполнениеПоШаблону = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьИнформационныйТекстРучногоРедактирования(Форма)
	...
	Если Форма.РучноеРедактирование Тогда
		ИнформационнаяКартинка = БиблиотекаКартинок.Предупреждение;
		ИнформационныйТекст = НСтр("es='El gráfico del trabajo es cambiado al año en curso a mano. Los cambios son distinguidos en los resultados del relleno.';ru='График работы на текущий год изменен вручную. Изменения выделены в результатах заполнения.';en='Schedule for the current year is changed manually. Changes are highlighted in the results of filling.'");
	КонецЕсли;
