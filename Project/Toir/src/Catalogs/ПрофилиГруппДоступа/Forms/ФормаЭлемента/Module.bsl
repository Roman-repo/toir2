	...
	Если Не Элементы.Роли.ТолькоПросмотр Тогда
		...
		Для Каждого Строка Из ЭлементыДерева Цикл
			...
			Если Строка.ЭтоНедоступнаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El papel ""%1"" es inaccesible para el destino del perfil.';ru='Роль ""%1"" недоступна для назначения профиля.';en=""The role '%1' is not available for profile assignment."""), Строка.Синоним),
					"Роли",
					ЭлементыДерева.Индекс(Строка),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El papel ""%1"" en la línea %%1 es inaccesible para el destino del perfil.';ru='Роль ""%1"" в строке %%1 недоступна для назначения профиля.';en='The role ""%1"" in line %%1 is not available for profile assignment.'"), Строка.Синоним));
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ВосстановитьПоНачальномуЗаполнению(Команда)
	...
	ПоказатьВопрос(
		...
		НСтр("es='¿Restablecer el perfil por el contenido del relleno inicial?';ru='Восстановить профиль по содержимому начального заполнения?';en='Recover profile from initial fill content?'"),
...
Процедура ВыбратьНазначение(Команда)
	...
	ПользователиСлужебныйКлиент.ВыбратьНазначение(ЭтотОбъект, НСтр("es='La elección del destino del perfil de los grupos del acceso';ru='Выбор назначения профиля групп доступа';en='Choosing an Access Group Profile Assignment'"),,, ОписаниеОповещения);
КонецПроцедуры
...
Процедура ВосстановитьПоНачальномуЗаполнениюЗавершение(Ответ, Контекст) Экспорт
	...
	Если ОбновитьГруппыДоступа Тогда
		Текст =
			НСтр("es='El perfil ""%1"" es restablecido por el contenido del relleno inicial, "
"los grupos del acceso del perfil son renovados.';ru='Профиль ""%1"" восстановлен по содержимому начального заполнения,"
"группы доступа профиля обновлены.';en='Profile ""%1"" restored to initial fill content, profile access groups are updated. '");
	Иначе
		Текст =
			НСтр("es='El perfil ""%1"" es restablecido por el contenido del relleno inicial, "
"los grupos del acceso del perfil no son renovados.';ru='Профиль ""%1"" восстановлен по содержимому начального заполнения,"
"группы доступа профиля не обновлены.';en='Profile ""%1"" restored to initial fill content, profile access groups are not updated. '");
	КонецЕсли;
	
	ПоказатьОповещениеПользователя(НСтр("es='El perfil es restablecido';ru='Профиль восстановлен';en='Profile restored'"),
...
Функция ТекстВопросаОбновитьГруппыДоступаПрофиля()
	...
	Возврат
		НСтр("es='¿Renovar los grupos del acceso que usan este perfil? "
""
"Serán quitados los tipos superfluos del acceso con "
"los significados, dados para ellos, del acceso y son añadidos los tipos que faltan del acceso.';ru='Обновить группы доступа, использующие этот профиль?"
""
"Будут удалены лишние виды доступа с заданными для них"
"значениями доступа и добавлены недостающие виды доступа.';en='Update access groups using this profile?"
""
"Extra access types set for them will be deleted"
"access values and missing access types added.'");
