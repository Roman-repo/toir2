...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.РежимВыбора Тогда
		...
		Если Параметры.ЗакрыватьПриВыборе = Ложь Тогда
			...
			Заголовок = НСтр("es='La selección de los perfiles de los grupos del acceso';ru='Подбор профилей групп доступа';en='Selection of access group profiles'");
		Иначе
			Заголовок = НСтр("es='La elección del perfil de los grupos del acceso';ru='Выбор профиля групп доступа';en='Choosing an Access Group Profile'");
		КонецЕсли;
	Иначе
...
Процедура ВидПользователейНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПользователиСлужебныйКлиент.ВыбратьНазначение(ЭтотОбъект,
		НСтр("es='La elección del destino de los perfiles';ru='Выбор назначения профилей';en='Select Profile Assignment'"), Истина, Истина, ОписаниеОповещения);
