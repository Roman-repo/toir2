...
Процедура ПроверитьОднозначностьПоставляемыхДанных(ПроверкаЗаполнения = Ложь, Отказ = Ложь)
	...
	Если Выборка.Количество() > 1 Тогда
		...
		КраткоеПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la anotación del perfil ""%1"". "
"El perfil suministrado existe ya:';ru='Ошибка при записи профиля ""%1""."
"Поставляемый профиль уже существует:';en='Error while writing profile ""%1""."
"The supplied profile already exists:'"),
			Наименование);
		
		ПодробноеПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la anotación del perfil ""%1"". "
"El identificador de los datos suministrados ""%2"" se usa ya en el perfil:';ru='Ошибка при записи профиля ""%1""."
"Идентификатор поставляемых данных ""%2"" уже используется в профиле:';en='Error while writing profile ""%1""."
"The supplied data identifier ""%2"" is already used in the profile: '"),
			Наименование,
		...
		Иначе
			ЗаписьЖурналаРегистрации(
				НСтр("es='La dirección del acceso. La infracción de la no ambigüedad del perfil suministrado';ru='Управление доступом.Нарушение однозначности поставляемого профиля';en='Access control. Violation of the ambiguity of the supplied profile'",
