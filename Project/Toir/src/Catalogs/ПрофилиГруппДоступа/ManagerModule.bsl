...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление = НСтр("es='No son compatible con la versión corriente';ru='Не совместимы с текущей версией';en='Not compatible with current version'");
		...
		Дело.ЕстьДела      = КоличествоНесовместимыхПрофилейГруппДоступа > 0;
		Дело.Важное        = Истина;
		Дело.Представление = НСтр("es='Los perfiles de los grupos del acceso';ru='Профили групп доступа';en='Access group profiles'");
		Дело.Количество    = КоличествоНесовместимыхПрофилейГруппДоступа;
...
Функция ПоставляемыйПрофильПоИдентификатору(Идентификатор, ВызыватьИсключениеЕслиНетВБазеДанных = Ложь) Экспорт
	...
	Если СвойстваПрофиля = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El perfil c por el identificador ""%1"" "
"no es suministrado en el programa.';ru='Профиль c идентификатором ""%1"""
"не поставляется в программе.';en='Profile with id ""%1"" "
"not included in the program. '"),
	...
	Если ВызыватьИсключениеЕслиНетВБазеДанных Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El perfil suministrado con el identificador ""%1"" no es encontrado en "
"la base informativa.';ru='Поставляемый профиль с идентификатором ""%1"""
"не найден в информационной базе.';en='Delivered profile with id ""%1"" "
"not found in the infobase.'"),
			Строка(Идентификатор));
	КонецЕсли;
...
Функция ПоставляемыйПрофильИзменен(Профиль) Экспорт
	...
	Для каждого Роль Из ОписаниеРолейПрофиля Цикл
		...
		Если МетаданныеРоли = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Durante la comprobación del perfil suministrado ""%1"" "
"papel ""%2"" no es encontrado en los metadatos.';ru='При проверке поставляемого профиля ""%1"""
"роль ""%2"" не найдена в метаданных.';en='When checking the supplied profile ""%1"" "
"role ""%2"" not found in metadata.'"),
...
Функция ПоставляемыеПрофили()
	...
	ЗаголовокОшибки =
		НСтр("es='Son dados los significados inadmisibles en el procedimiento ПриЗаполненииПоставляемыхПрофилейГруппДоступа "
"el módulo UpravlenieDostupomPereopredelyaemyy general.';ru='Заданы недопустимые значения в процедуре ПриЗаполненииПоставляемыхПрофилейГруппДоступа"
"общего модуля УправлениеДоступомПереопределяемый.';en='Invalid values specified in procedure ПриЗаполненииПоставляемыхПрофилейГруппДоступа"
	...
	Если ПараметрыОбновления.ЗапретитьИзменениеПрофилей
	   И НЕ ПараметрыОбновления.ОбновлятьИзмененныеПрофили Тогда
		...
		ВызватьИсключение ЗаголовокОшибки
			+ НСтр("es='Cuando en el parámetro De ParametryObnovleniya la propiedad "
"de ObnovlyatIzmenennyeProfili es establecida la Mentira, "
"entonces la propiedad de ZapretitIzmenenieProfiley "
"debe ser establecida también la Mentira.';ru='Когда в параметре ПараметрыОбновления свойство"
"ОбновлятьИзмененныеПрофили установлено Ложь,"
"тогда свойство ЗапретитьИзменениеПрофилей тоже"
"должно быть установлено Ложь.';en='When in the ПараметрыОбновления property"
"ОбновлятьИзмененныеПрофили set to False,"
"then the ЗапретитьИзменениеПрофилей too"
"must be set to False. '");
	...
	ОписаниеПрофиляАдминистратор.Наименование  = НСтр("es='El administrador';ru='Администратор';en='Administrator'");
...
"- el ajuste de los parámetros del trabajo y el servicio del sistema informativo, "
"- el ajuste de los derechos de admisión de otros usuarios, "
"- la desaparición de los objetos marcados, "
"- rara vez para la inserción de los cambios en la configuración. "
""
"Se Recomienda no usar para el trabajo ""regular"" en el sistema informativo.';ru='Предназначен для:"
...
""
"Рекомендуется не использовать для ""обычной"" работы в информационной системе.';en='Created for:"
"- settings for the operation and maintenance of the information system,"
"- settings of access rights of other users,"
"- delete marked objects,"
"- in rare cases, to make configuration changes."
""
"It is recommended not to use it for ""normal"" work in the information system.'")
		+ Символы.ПС;
	...
	Для Каждого ОписаниеПрофиля Из ОписанияПрофилей Цикл
		...
		Если Не ЗначениеЗаполнено(ОписаниеПрофиля.Идентификатор) Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='En la descripción del perfil ""%1"" no es rellenada la propiedad el Identificador.';ru='В описании профиля ""%1"" не заполнено свойство Идентификатор.';en='In the profile description ""% 1"", the Identifier property is not filled.'"),
		...
		ИначеЕсли Не СтроковыеФункцииКлиентСервер.ЭтоУникальныйИдентификатор(ОписаниеПрофиля.Идентификатор) Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='En la descripción del perfil ""%1"" es indicado el identificador incorrecto: ""%2"".';ru='В описании профиля ""%1"" указан некорректный идентификатор: ""%2"".';en='The profile description ""% 1"" has an invalid identifier: ""% 2"".'"),
				?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя), ОписаниеПрофиля.Имя, ОписаниеПрофиля.Наименование),
				...
				Если ТипЗнч(Тип) <> Тип("Тип")
				 ...
				 Или Тип <> Тип("СправочникСсылка.Пользователи")
				   И Не Метаданные.ОпределяемыеТипы.ВнешнийПользователь.Тип.СодержитТип(Тип) Тогда
					ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='En la descripción del perfil ""%1"" "
"es indicado el destino inadmisible ""%2 (%3)"". "
"Se Espera el destino, como el significado como ""el Tipo"" para la referencia, "
"indicado en el tipo determinado el Usuario y "
"el VneshniyPolzovatel, indicado en el tipo determinado, "
"(excepto el tipo de SpravochnikSsylka. Los usuarios).';ru='В описании профиля ""%1"""
...
"(кроме типа СправочникСсылка.Пользователи).';en='In profile description ""% 1"""
"Invalid destination ''% 2 (% 3)'' specified."
"Assignment is expected as a type value for the link,"
"specified in the defined type User and"
"specified in the type being defined ВнешнийПользователь (except for type СправочникСсылка.Пользователи).'"),
		...
		Для Каждого Роль Из ОписаниеПрофиля.Роли Цикл
			...
			Если ВсеРоли.Получить(Роль) = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='En la descripción del perfil ""%1 (%2)"" "
"el papel ""%3"" no es encontrado en los metadatos.';ru='В описании профиля ""%1 (%2)"""
"роль ""%3"" не найдена в метаданных.';en='In profile description ""% 1 (% 2)"""
"role ''% 3'' not found in metadata.'"),
					ОписаниеПрофиля.Имя,
					ОписаниеПрофиля.Идентификатор,
					Роль);
			КонецЕсли;
			...
			Если НедоступныеРоли.Получить(Роль) <> Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='En la descripción del perfil ""%1 (%2)"" "
"es indicado el papel ""%3"", "
"que no corresponde al destino del perfil: "
"""%4"".';ru='В описании профиля ""%1 (%2)"""
"указана роль ""%3"", "
"которая не соответствует назначению профиля:"
"""%4"".';en='In profile description ""% 1 (% 2)"""
"the role ""% 3"" is specified,"
"which does not match the purpose of the profile:"
"""%4"".'"),
					ОписаниеПрофиля.Имя,
					ОписаниеПрофиля.Идентификатор,
					Роль,
		...
		Если СвойстваПрофилей.Получить(ОписаниеПрофиля.Идентификатор) <> Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El perfil con el identificador ""%1"" existe ya.';ru='Профиль с идентификатором ""%1"" уже существует.';en='Profile with id ""%1"" already exists.'"),
				ОписаниеПрофиля.Идентификатор);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ОписаниеПрофиля.Имя) Тогда
			Если СвойстваПрофилей.Получить(ОписаниеПрофиля.Имя) <> Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El perfil con el nombre ""%1"" existe ya.';ru='Профиль с именем ""%1"" уже существует.';en='A profile with the name ""%1"" already exists.'"),
					ОписаниеПрофиля.Имя);
			КонецЕсли;
		...
		Для Каждого ЭлементСписка Из ОписаниеПрофиля.ВидыДоступа Цикл
			...
			Если СвойстваВидовДоступа.ПоИменам.Получить(ИмяВидаДоступа) = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='En la descripción del perfil ""%1"" "
"es indicado el tipo no existente del acceso ""%2"".';ru='В описании профиля ""%1"""
"указан несуществующий вид доступа ""%2"".';en='In profile description ""% 1"""
"specified non-existent access type ''% 2''.'"),
					?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
					  ОписаниеПрофиля.Имя,
					  ОписаниеПрофиля.Идентификатор),
					ИмяВидаДоступа);
			...
			Если Не ВидДоступаСоответствуетНазначениюПрофиля Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='En la descripción del perfil ""%1"" "
"es indicado el tipo del acceso ""%2"" "
"que no corresponde al destino del perfil: "
"""%3"".';ru='В описании профиля ""%1"""
"указан вид доступа ""%2"""
"который не соответствует назначению профиля:"
"""%3"".';en='In profile description ""% 1"""
"Access type ''% 2'' specified"
"which does not match the purpose of the profile:"
"""% 3"".'"),
					?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
					  ОписаниеПрофиля.Имя,
			...
			Если УточнениеВидаДоступа <> ""
			   ...
			   И УточнениеВидаДоступа <> "ВначалеВсеРазрешены" Тогда
				...
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='En la descripción del perfil ""%1"" "
"para el tipo del acceso ""%2"" es indicada la especificación desconocida ""%3"". "
""
"Son admisibles solamente las especificaciones siguientes: "
"- ""VnachaleVseZaprescheny"" o """", "
"- ""VnachaleVseRazresheny"", "
"- ""Preestablecido"".';ru='В описании профиля ""%1"""
"для вида доступа ""%2"" указано неизвестное уточнение ""%3""."
""
"Допустимы только следующие уточнения:"
"- ""ВначалеВсеЗапрещены"" или """","
"- ""ВначалеВсеРазрешены"","
"- ""Предустановленный"".';en='In profile description ""% 1"""
"access type ""% 2"" specified an unknown qualification ""% 3""."
""
"Only the following clarifications are allowed:                              - ""ВначалеВсеЗапрещены"" or """",                               - ""ВначалеВсеРазрешены"","
"- ""Предустановленный"".'"),
					?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
					  ОписаниеПрофиля.Имя,
					  ОписаниеПрофиля.Идентификатор),
		...
		Для Каждого ЭлементСписка Из ОписаниеПрофиля.ЗначенияДоступа Цикл
			...
			Если СвойстваВидаДоступа = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='En la descripción del perfil ""%1"" "
"es indicado el tipo no existente del acceso ""%2"" "
"para el significado del acceso "
"""%3"".';ru='В описании профиля ""%1"""
...
"""% 3"".'"),
			...
			Если ОбъектМетаданных = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='En la descripción del perfil ""%1"" "
"para el tipo del acceso ""%2"" "
"no hay tipo del significado indicado del acceso "
"""%3"".';ru='В описании профиля ""%1"""
"для вида доступа ""%2"""
"не существует тип указанного значения доступа"
"""%3"".';en='In profile description ""% 1"""
"for access type ""% 2"""
"the type of the specified access value does not exist"
"""% 3"".'"),
					?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
					  ОписаниеПрофиля.Имя,
					  ОписаниеПрофиля.Идентификатор),
			...
			Если ПустаяСсылкаЗначенияДоступа = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='En la descripción del perfil ""%1"" "
"para el tipo del acceso ""%2"" "
"es indicado no el tipo de referencia del significado del acceso "
"""%3"".';ru='В описании профиля ""%1"""
"для вида доступа ""%2"""
"указан не ссылочный тип значения доступа"
"""%3"".';en='In profile description ""% 1"""
"for access type ""% 2"""
"non-reference access value type specified"
"""% 3"".'"),
					?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
					  ОписаниеПрофиля.Имя,
					  ОписаниеПрофиля.Идентификатор),
			...
			Если СвойстваВидаДоступаПоТипу = Неопределено
			 ИЛИ СвойстваВидаДоступаПоТипу.Имя <> ВидДоступа Тогда
				...
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='En la descripción del perfil ""%1"" "
"es indicado el significado del acceso ""%3"" "
"tipos, que no es indicado en las propiedades del tipo del acceso ""%2"".';ru='В описании профиля ""%1"""
"указано значение доступа ""%3"""
"типа, который не указан в свойствах вида доступа ""%2"".';en='In profile description ""% 1"""
"specified access value ""% 3"""
"type that is not specified in the properties of access type ""% 2"".'"),
					?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
					  ОписаниеПрофиля.Имя,
					  ОписаниеПрофиля.Идентификатор),
					ВидДоступа,
			...
			Если ТаблицаЗначенийДоступа.НайтиСтроки(Отбор).Количество() > 0 Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='En la descripción del perfil ""%1"" "
"para el tipo del acceso ""%2"" "
"es repetidamente indicado el significado del acceso "
"""%3"".';ru='В описании профиля ""%1"""
"для вида доступа ""%2"""
"повторно указано значение доступа"
"""%3"".';en='In profile description ""% 1"""
"for access type ""% 2"""
"reassigned access value"
"""% 3"".'"),
					?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя),
					  ОписаниеПрофиля.Имя,
					  ОписаниеПрофиля.Идентификатор),
...
Функция ПредставлениеНазначенияПрофиля(НазначениеПрофиля)
	...
	Если НазначениеПрофиля = "СовместноДляПользователейИВнешнихПользователей" Тогда
		Возврат НСтр("es='Juntamente para los usuarios y los usuarios exteriores';ru='Совместно для пользователей и внешних пользователей';en='Jointly for users and external users'");
		
	ИначеЕсли НазначениеПрофиля = "ДляВнешнихПользователей" Тогда
		Возврат НСтр("es='Para los usuarios exteriores';ru='Для внешних пользователей';en='For external users'");
	КонецЕсли;
	
	Возврат НСтр("es='Para los usuarios';ru='Для пользователей';en='For users'");
...
Функция ОбновитьПрофильГруппДоступа(СвойстваПрофиля, НеОбновлятьРолиПользователей = Ложь)
	...
	Если ПрофильИзменен Тогда
		...
		Для каждого Роль Из ОписаниеРолейПрофиля(СвойстваПрофиля) Цикл
			...
			Если МетаданныеРоли = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='A la renovación del perfil suministrado ""%1"" "
"papel ""%2"" no es encontrado en los metadatos.';ru='При обновлении поставляемого профиля ""%1"""
"роль ""%2"" не найдена в метаданных.';en='When updating the supplied profile ""%1"""
"role ""%2"" not found in metadata.'"),
					СвойстваПрофиля.Наименование,
					Роль);
			КонецЕсли;
