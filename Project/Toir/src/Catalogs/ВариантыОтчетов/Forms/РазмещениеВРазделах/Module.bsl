...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СмешаннаяВажность = НСтр("es='Distinto';ru='Различная';en='Different'");
...
"Подробности:"
"%2';en='%1"
"Details:                             %2'"), СообщенияОбОшибках.Текст, СообщенияОбОшибках.Подробно), РежимДиалогаВопрос.ОК);
	КонецЕсли;
КонецПроцедуры
...
Процедура Разместить(Команда)
	ЗаписатьНаСервере();
	ТекстОповещения = НСтр("es='Son cambiados los ajustes de las variantes de los informes (%1 piezas).';ru='Изменены настройки вариантов отчетов (%1 шт.).';en='Report option settings changed (%1 pc.).'");
...
Процедура ЗаполнитьРазделы()
	...
	Если КоличествоОшибок > 0 Тогда
		...
		Для Каждого СтрокаТаблицы Из ОтфильтрованныеВарианты Цикл
			Если ТекущаяПричина <> СтрокаТаблицы.Причина Тогда
				...
				Если ТекущаяПричина = 1 Тогда
					СообщенияОбОшибках.Подробно = СообщенияОбОшибках.Подробно + НСтр("es='Marcado a la desaparición:';ru='Помеченные на удаление:';en='Marked for deletion:'");
				ИначеЕсли ТекущаяПричина = 2 Тогда
					СообщенияОбОшибках.Подробно = СообщенияОбОшибках.Подробно + НСтр("es='Marcado a la desaparición:';ru='Помеченные на удаление:';en='Marked for deletion:'");
				ИначеЕсли ТекущаяПричина = 3 Тогда
					СообщенияОбОшибках.Подробно = СообщенияОбОшибках.Подробно + НСтр("es='Es insuficientemente derecho para el cambio:';ru='Недостаточно прав для изменения:';en='Not enough rights to change:'");
				ИначеЕсли ТекущаяПричина = 4 Тогда
					СообщенияОбОшибках.Подробно = СообщенияОбОшибках.Подробно + НСтр("es='El informe es desconectado o inaccesible por los derechos:';ru='Отчет отключен или недоступен по правам:';en='Report disabled or unavailable by rights:'");
				КонецЕсли;
			КонецЕсли;
		...
		Если КоличествоВариантов = 0 Тогда
			СообщенияОбОшибках.Текст = НСтр("es='Es insuficientemente derecho para la instalación en las secciones de las variantes escogidas de los informes.';ru='Недостаточно прав для размещения в разделах выбранных вариантов отчетов.';en='Not enough rights to place selected report options in sections.'");
		Иначе
			СообщенияОбОшибках.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es insuficientemente derecho para la instalación en las secciones de algunas variantes de los informes (%1).';ru='Недостаточно прав для размещения в разделах некоторых вариантов отчетов (%1).';en='Not enough rights to place in sections of some report options (%1).'"),
