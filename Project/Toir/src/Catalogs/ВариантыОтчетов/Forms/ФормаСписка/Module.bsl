...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокВыбора.Добавить(1, НСтр("es='Todo, excepto exterior';ru='Все, кроме внешних';en='All but external'"));
	СписокВыбора.Добавить(Перечисления.ТипыОтчетов.Внутренний,     НСтр("es='Interior';ru='Внутренние';en='Domestic'"));
	СписокВыбора.Добавить(Перечисления.ТипыОтчетов.Расширение,     НСтр("es='Las ampliaciones';ru='Расширения';en='Extensions'"));
	СписокВыбора.Добавить(Перечисления.ТипыОтчетов.Дополнительный, НСтр("es='Adicional';ru='Дополнительные';en='Additional'"));
	СписокВыбора.Добавить(Перечисления.ТипыОтчетов.Внешний,        НСтр("es='Exterior';ru='Внешние';en='External'"));
...
Процедура ДеревоПодсистемПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если ПараметрыРазмещения.Источник.Ссылка = ПараметрыРазмещения.Приемник.Ссылка Тогда
		ПоказатьПредупреждение(, НСтр("es='Las variantes escogidas de los informes ya en sección dada.';ru='Выбранные варианты отчетов уже в данном разделе.';en='Selected report options are already in this section.'"));
		Возврат;
	КонецЕсли;
	
	Если ПараметрыРазмещения.Варианты.Всего = 1 Тогда
		Если ПараметрыРазмещения.Действие = "Копирование" Тогда
			ШаблонВопроса = НСтр("es='¿Instalar ""%1"" en ""%4""?';ru='Разместить ""%1"" в ""%4""?';en='Place ""%1"" in ""%4""?'");
		Иначе
			ШаблонВопроса = НСтр("es='¿Trasladar ""%1"" de ""%3"" en ""%4""?';ru='Переместить ""%1"" из ""%3"" в ""%4""?';en='Move ""%1"" from ""%3"" to ""%4""?'");
		КонецЕсли;
	...
	Иначе
		...
		КонецЦикла;
		Если ПараметрыРазмещения.Действие = "Копирование" Тогда
			ШаблонВопроса = НСтр("es='Instalar las variantes de los informes ""%1"" (¿%2 piezas) en ""%4""?';ru='Разместить варианты отчетов ""%1"" (%2 шт.) в ""%4""?';en='Place report options ""%1"" (%2 pcs.) In ""%4""?'");
		Иначе
			ШаблонВопроса = НСтр("es='Trasladar las variantes de los informes ""%1"" (¿%2 piezas) de ""%3"" en ""%4""?';ru='Переместить варианты отчетов ""%1"" (%2 шт.) из ""%3"" в ""%4""?';en='Move report options ""%1"" (%2 pcs.) From ""%3"" to ""%4""?'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ПараметрыРазмещения.Варианты.Всего = РезультатВыполнения.Размещено Тогда
		Если ПараметрыРазмещения.Варианты.Всего = 1 Тогда
			Если ПараметрыРазмещения.Действие = "Перемещение" Тогда
				Шаблон = НСтр("es='Es con éxito trasladado en ""%1"".';ru='Успешно перемещен в ""%1"".';en='Successfully moved to ""%1"".'");
			Иначе
				Шаблон = НСтр("es='Es con éxito instalado en ""%1"".';ru='Успешно размещен в ""%1"".';en=""Successfully posted to '%1'.""");
			КонецЕсли;
			Текст = ПараметрыРазмещения.Варианты.Представление;
			Ссылка = ПолучитьНавигационнуюСсылку(ПараметрыРазмещения.Варианты.Массив[0]);
		Иначе
			Если ПараметрыРазмещения.Действие = "Перемещение" Тогда
				Шаблон = НСтр("es='Son con éxito trasladados en ""%1"".';ru='Успешно перемещены в ""%1"".';en=""Successfully moved to '%1'.""");
			Иначе
				Шаблон = НСтр("es='Son con éxito instalados en ""%1"".';ru='Успешно размещены в ""%1"".';en=""Successfully posted to '%1'.""");
			КонецЕсли;
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Las variantes de los informes (%1).';ru='Варианты отчетов (%1).';en='Reporting options (%1).'"), Формат(ПараметрыРазмещения.Варианты.Всего, "ЧН=0; ЧГ=0"));
			Ссылка = Неопределено;
		КонецЕсли;
	...
	Иначе
		...
		Если Не ПустаяСтрока(РезультатВыполнения.НеМогутРазмещаться) Тогда
			ТекстОшибок = ?(ТекстОшибок = "", "", ТекстОшибок + Символы.ПС + Символы.ПС)
				+ НСтр("es='No pueden instalarse en la interfaz de mando:';ru='Не могут размещаться в командном интерфейсе:';en='Cannot be placed in the command interface:'")
				+ Символы.ПС
				+ РезультатВыполнения.НеМогутРазмещаться;
		КонецЕсли;
		Если Не ПустаяСтрока(РезультатВыполнения.УжеРазмещены) Тогда
			ТекстОшибок = ?(ТекстОшибок = "", "", ТекстОшибок + Символы.ПС + Символы.ПС)
				+ НСтр("es='Son instalados ya en esta sección:';ru='Уже размещены в этом разделе:';en='Already posted in this section:'")
				+ Символы.ПС
				+ РезультатВыполнения.УжеРазмещены;
		КонецЕсли;
		
		Если ПараметрыРазмещения.Действие = "Перемещение" Тогда
			Шаблон = НСтр("es='Es trasladado las variantes de los informes: %1 de %2. "
"Los detalles: "
"%3';ru='Перемещено вариантов отчетов: %1 из %2."
"Подробности:"
"%3';en='Moved report options: %1 of %2."
"Details:"
"%3'");
		Иначе
			Шаблон = НСтр("es='Es instalado las variantes de los informes: %1 de %2. "
"Los detalles: "
"%3';ru='Размещено вариантов отчетов: %1 из %2."
"Подробности:"
"%3';en='Posted report options:%1 of%2."
"Details:"
"%3'");
		КонецЕсли;
...
Процедура УстановитьЗаголовокДереваПодсистем()
	Элементы.ГруппаРазделы.Заголовок = ?(Элементы.ДеревоПодсистем.ТекущиеДанные = Неопределено,
		НСтр("es='Todas las secciones';ru='Все разделы';en='All sections'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
		...
		Для Каждого ВариантСсылка Из ПараметрыРазмещения.Варианты.Массив Цикл
			Если ВариантСсылка.ТипОтчета = Перечисления.ТипыОтчетов.Внешний Тогда
				НеМогутРазмещаться = ?(НеМогутРазмещаться = "", "", НеМогутРазмещаться + Символы.ПС)
					...
					+ Строка(ВариантСсылка)
					+ " ("
					+ НСтр("es='Exterior';ru='внешний';en='external'")
					+ ")";
				Продолжить;
			ИначеЕсли ВариантСсылка.ПометкаУдаления Тогда
				НеМогутРазмещаться = ?(НеМогутРазмещаться = "", "", НеМогутРазмещаться + Символы.ПС)
					...
					+ Строка(ВариантСсылка)
					+ " ("
					+ НСтр("es='Es marcado a la desaparición';ru='помечен на удаление';en='marked for deletion'")
					+ ")";
				Продолжить;
			КонецЕсли;
...
Функция НачатьЗамер(Событие)
	...
	Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
		Комментарий = Комментарий
			...
			+ "; " + НСтр("es='Incluso subordinado:';ru='Включая подчиненные:';en='Including subordinates:'") + " " + Строка(ВключаяПодчиненные);
	Иначе
