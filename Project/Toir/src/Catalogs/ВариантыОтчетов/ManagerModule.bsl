...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если ВидФормы = "ФормаОбъекта" Тогда
		...
		Если Не ЗначениеЗаполнено(СсылкаВарианта) Тогда
			ВызватьИсключение НСтр("es='Se puede crear una nueva variante del informe solamente de la forma del informe';ru='Новый вариант отчета можно создать только из формы отчета';en='A new report variant can only be created from the report form.'");
		КонецЕсли;
		...
		ИначеЕсли ПараметрыОткрытия.ТипОтчета = "Дополнительный" Тогда
			...
			Если Не ПараметрыОткрытия.Подключен Тогда
				ВызватьИсключение НСтр("es='Se puede abrir la variante del informe exterior solamente de la forma del informe.';ru='Вариант внешнего отчета можно открыть только из формы отчета.';en='An external report variant can only be opened from the report form.'");
			КонецЕсли;
		Иначе
			ВызватьИсключение НСтр("es='Se puede abrir la variante del informe exterior solamente de la forma del informe.';ru='Вариант внешнего отчета можно открыть только из формы отчета.';en='An external report variant can only be opened from the report form.'");
		КонецЕсли;
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт 
	...
	Пока Вариант.Следующий() Цикл 
		...
		Исключение
			...
			ШаблонКомментария = НСтр("es='No consiguió establecer la fuente de los datos de la variante del informe %1. "
"Probablemente él es estropeado y no está sujeto a la reconstitución. "
""
"La información técnica sobre el problema: %2';ru='Не удалось установить источник данных варианта отчета %1."
"Возможно он поврежден и не подлежит восстановлению."
""
"Техническая информация о проблеме: %2';en='Failed to set data source for report option%1."
"It may be damaged and cannot be repaired."
""
"Technical issue:%2'");
	...
	Если Обработано = 0 И Отказано <> 0 Тогда
		ШаблонСообщения = НСтр("es='Al procedimiento de UstanovitIstochnikDannyhVarianta no consiguió tratar algunas variantes de los informes: %1';ru='Процедуре УстановитьИсточникДанныхВарианта не удалось обработать некоторые варианты отчетов: %1';en='A procedure УстановитьИсточникДанныхВарианта failed to process some report options:%1'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Отказано);
		
		ВызватьИсключение ТекстСообщения;
	Иначе
		ШаблонКомментария = НСтр("es='El procedimiento de UstanovitIstochnikDannyhVarianta ha tratado el paquete siguiente de las variantes de los informes: %1';ru='Процедура УстановитьИсточникДанныхВарианта обработала очередной пакет вариантов отчетов: %1';en='A procedure УстановитьИсточникДанныхВарианта processed the next batch of report options:%1'");
