...
Процедура ОбновитьПредставление()
	...
	Если ЗначениеЗаполнено(ТипОбъектаДокументооборота) Тогда 
		...
		Если ТипОбъектаДокументооборота = "DMIncomingDocument" Тогда
			ПредставлениеОбъектаДокументооборота = НСтр("es='El documento recibido';ru='Входящий документ';en='Incoming document'");
		ИначеЕсли ТипОбъектаДокументооборота = "DMInternalDocument" Тогда
			ПредставлениеОбъектаДокументооборота = НСтр("es='El documento interior';ru='Внутренний документ';en='Internal document'");
		ИначеЕсли ТипОбъектаДокументооборота = "DMOutgoingDocument" Тогда
			ПредставлениеОбъектаДокументооборота = НСтр("es='El documento saliente';ru='Исходящий документ';en='Outgoing document'");
		ИначеЕсли ТипОбъектаДокументооборота = "DMCorrespondent" Тогда
			Если ИнтеграцияС1СДокументооборотПовтИсп.ИспользоватьТерминКорреспонденты() Тогда
				ПредставлениеОбъектаДокументооборота = НСтр("es='El corresponsal';ru='Корреспондент';en='Correspondent'");
			Иначе
				ПредставлениеОбъектаДокументооборота = НСтр("es='El contratante';ru='Контрагент';en='Counterparty'");
			КонецЕсли;
		Иначе
