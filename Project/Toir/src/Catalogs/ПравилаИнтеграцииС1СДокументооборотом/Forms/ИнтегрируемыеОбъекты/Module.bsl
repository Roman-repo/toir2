...
Процедура УстановитьДоступностьКоманд(ТекущиеДанные)
	...
	ИначеЕсли ТекущиеДанные.ЭтоГруппа Тогда
		...
		Если ТекущиеДанные.Автонастройка = Истина Тогда
			Подсказка = НСтр("es='Los objetos del subsistema apoyan el ajuste automático de la integración.';ru='Объекты подсистемы поддерживают автоматическую настройку интеграции.';en='Subsystem objects support automatic integration configuration.'");
		ИначеЕсли ТекущиеДанные.Автонастройка = Ложь Тогда
			Подсказка = НСтр("es='La integración por los objetos del subsistema se dispone a mano.';ru='Интеграция по объектам подсистемы настраивается вручную.';en='Integration by subsystem objects is configured manually.'");
		Иначе
			Подсказка = НСтр("es='Algunos objetos del subsistema apoyan el ajuste automático de la integración.';ru='Некоторые объекты подсистемы поддерживают автоматическую настройку интеграции.';en='Some objects of the subsystem support automatic configuration of integration.'");
		КонецЕсли;
...
Функция ОписаниеВыполняемыхДействийПоУмолчанию()
	...
	Возврат НСтр("es='Será automáticamente creada la regla de la integración. "
"En 1С:Документообороте será creado el tipo correspondiente del documento.';ru='Будет автоматически создано правило интеграции."
"В 1С:Документообороте будет создан соответствующий вид документа.';en='An integration rule will be automatically created."
"In 1C: Document Management, an appropriate type of document will be created.'");
...
Процедура НастроитьАвтоматически(ТекущиеДанные)
	...
	ТекстВопроса = ОписаниеВыполняемыхДействий
		...
		+ Символы.ПС
		+ НСтр("es='¿Continuar?';ru='Продолжить?';en='Resume?'");
