...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗаполненВШаблоне Тогда
		...
		СписокВариантов.Добавить(ИзШаблона, НСтр("es='Del modelo';ru='Из шаблона';en='From the template'"));
		...
		Если Параметры.ШаблонЗапрещаетИзменение = Истина
			...
			Если ИмяРеквизитаОбъектаДокументооборота = "documentType" Тогда
				ИнформационнаяНадпись = 
					НСтр("es='El tipo del documento es escogido en el modelo y no puede ser cambiado.';ru='Вид документа выбран в шаблоне и не может быть изменен.';en='The type of document is selected in the template and cannot be changed.'");
			Иначе
				ИнформационнаяНадпись = 
					НСтр("es='El modelo prohíbe el cambio de los requisitos, dados en ello.';ru='Шаблон запрещает изменение заданных в нем реквизитов.';en='The template prohibits changing the details specified in it.'");
			КонецЕсли;
