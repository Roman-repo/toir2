...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПустаяСтрока(Объект.Сервис) Тогда
		Если Объект.Сервис = "https://webdav.yandex.com"
			...
			Сервис = НСтр("es='El Yandex. El disco';ru='Яндекс.Диск';en='Yandex.Disk'");
	...
	Если АвтоНаименование Тогда
		Если ЗначениеЗаполнено(Объект.Сервис) Тогда
			Объект.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La sincronización con %1';ru='Синхронизация с %1';en='Synchronization with%1'"), 
...
Процедура ПроверитьНастройки(Команда)
	...
	Если Объект.Ссылка.Пустая() Или Модифицированность Тогда
		...
		ТекстВопроса = НСтр("es='Para la comprobación de los ajustes es necesario anotar los datos de la anotación de descuento. ¿Continuar?';ru='Для проверки настроек необходимо записать данные учетной записи. Продолжить?';en='To check the settings, you need to write account information. Continue?'");
...
Процедура ПроверитьВозможностьСинхронизацииСОблачнымСервисом()
	...
	Если СтруктураРезультата.Отказ Тогда
		...
		ИначеЕсли СтруктураРезультата.КодОшибки = 404 Тогда
			ТекстОшибки = НСтр("es='Comprueben la corrección de la carpeta indicada radical.';ru='Проверьте правильность указанной корневой папки.';en='Check that the specified root folder is correct.'");
		ИначеЕсли СтруктураРезультата.КодОшибки = 401 Тогда
			ТекстОшибки = НСтр("es='Comprueben la corrección del login/consigna introducido.';ru='Проверьте правильность введенных логина/пароля.';en='Check the correct username / password.'");
		Иначе
			ТекстОшибки = НСтр("es='Comprueben la corrección de los datos introducidos.';ru='Проверьте правильность введенных данных.';en='Check that the data entered is correct.'");
		КонецЕсли;
		
		ПоказатьПредупреждение(Неопределено, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La comprobación de los parámetros de la anotación de descuento se ha acabado con las faltas. "
"%1 "
""
"detalles Técnicos: "
"%2';ru='Проверка параметров учетной записи завершилась с ошибками."
"%1"
""
"Технические подробности:"
"%2';en='Checking account settings failed."
"%1                                          Technical details:"
"%2'"),
					   ТекстОшибки,
					   ТекстПротокола));
	Иначе
		ПоказатьПредупреждение(Неопределено, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La comprobación de los parámetros de la anotación de descuento se ha acabado con éxito. "
"%1';ru='Проверка параметров учетной записи завершилась успешно. "
"%1';en='Verification of account settings completed successfully."
"%1'"),
			РезультатТекст));
	КонецЕсли;
...
Функция ПроверитьСоединение(Сервис, ТекстПротокола)
	...
	Иначе
		...
		Возврат Новый Структура("ОписаниеОшибки, ЖурналДиагностики",
			НСтр("es='Comprueben la unión con la red el Internet.';ru='Проверьте соединение с сетью Интернет.';en='Check your internet connection.'"), ТекстПротокола);
