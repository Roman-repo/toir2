...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЕстьБазовыйКалендарь И Не ЗначениеЗаполнено(Объект.БазовыйКалендарь) Тогда
		ТекстСообщения = НСтр("es='El calendario federal no es rellenado.';ru='Федеральный календарь не заполнен.';en='The federal calendar not filled.'");
...
Процедура ПеренестиДень(Команда)
	...
	ПараметрыВыбораДаты.Заголовок = НСтр("es='La elección de la fecha del traslado';ru='Выбор даты переноса';en='Transfer Date Selection'");
	
	ТекстСообщения = НСтр("es='Escojan la fecha, a que será realizado el traslado del día %1 (%2)';ru='Выберите дату, на которую будет осуществлен перенос дня %1 (%2)';en='Select the date to which day%1 (%2) will be carried out'");
...
Процедура Печать(Команда)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		ПоказатьВопрос(
			Обработчик,
			НСтр("es='Los datos del calendario de producción no son anotados. "
"La prensa es posible solamente después del registro de datos. "
""
"¿Anotar?';ru='Данные производственного календаря еще не записаны."
"Печать возможна только после записи данных."
""
"Записать?';en='Production calendar data has not yet been recorded."
"Printing is possible only after recording data."
""
"Record?'"),
...
Функция ПредставлениеПереноса(ДатаИсточник, ДатаПриемник, ВидДняИсточник, ВидДняПриемник)
	...
	ШаблонМР = НСтр("es='%1 %3 es llevado en %2 %4';ru='%1 %3 перенесен на %2 %4';en='% 1% 3 carried over to% 2% 4'");
	ШаблонСР = НСтр("es='%1 %3 es llevado en %2 %4';ru='%1 %3 перенесено на %2 %4';en='% 1% 3 carried over to% 2% 4'");
	ШаблонЖР = НСтр("es='%1 %3 es llevada en %2 %4';ru='%1 %3 перенесена на %2 %4';en='% 1% 3 carried over to% 2% 4'");
	
	ДниЖР = Новый Соответствие;
	ДниЖР.Вставить(НСтр("es='El ambiente';ru='среда';en='Wednesday'"), НСтр("es='El ambiente';ru='среду';en='Wednesday'"));
	ДниЖР.Вставить(НСтр("es='El viernes';ru='пятница';en='Friday'"), НСтр("es='El viernes';ru='пятницу';en='friday'"));
	ДниЖР.Вставить(НСтр("es='El sábado';ru='суббота';en='Saturday'"), НСтр("es='El sábado';ru='субботу';en='Saturday'"));
	
	ДниСР = Новый Соответствие;
	ДниСР.Вставить(НСтр("es='El sábado';ru='суббота';en='Saturday'"), Истина);
...
Функция ЗагрузитьСписокПоддерживаемыхПроизводственныхКалендарей()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='El relleno de la lista de los calendarios apoyados';ru='Заполнение списка поддерживаемых календарей';en='Fill in the list of supported calendars'");
