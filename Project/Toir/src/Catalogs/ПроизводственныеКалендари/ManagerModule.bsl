...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
		...
		МодульУправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
				...
				"ПроизводственныйКалендарь", НСтр("es='El calendario de producción';ru='Производственный календарь';en='Production calendar'"),
...
Функция ПроизводственныеКалендариПоУмолчанию()
	...
	Исключение
		ИмяСобытия = НСтр("es='Los gráficos de calendario. La recepción de los calendarios del clasificador';ru='Календарные графики.Получение календарей из классификатора';en='Calendar charts. Getting calendars from the classifier'", ОбщегоНазначения.КодОсновногоЯзыка());
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir la lista de los calendarios de producción del clasificador. "
"La lista de los calendarios es recibida de la maqueta suministrada. "
"%1';ru='Не удалось получить список производственных календарей из классификатора."
"Список календарей получен из поставляемого макета."
"%1';en='Failed to get the list of production calendars from the classifier."
"The list of calendars is obtained from the supplied layout."
"%1'"), 
...
Функция ПечатнаяФормаПроизводственногоКалендаря(ПараметрыПодготовкиПечатнойФормы)
	...
	Пока ВыборкаПоГоду.Следующий() Цикл
		...
		Пока ВыборкаПоКварталу.Следующий() Цикл
			...
			НомерКвартала.Параметры.НомерКвартала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 trimestre';ru='%1 квартал';en='% 1 quarter'"), ВыборкаПоКварталу.КварталКалендаря);
			...
			КолонкаМесяца.Параметры.ИмяМесяца 		= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 trimestre';ru='%1 квартал';en='% 1 quarter'"), ВыборкаПоКварталу.КварталКалендаря);
			ТабличныйДокумент.Присоединить(КолонкаМесяца);
			...
			Если ВыборкаПоКварталу.КварталКалендаря = 2 
				...
				КолонкаМесяца.Параметры.ИмяМесяца 		= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 medio año';ru='%1 полугодие';en='% 1 half year'"), ВыборкаПоКварталу.КварталКалендаря / 2);
				ТабличныйДокумент.Присоединить(КолонкаМесяца);
			КонецЕсли;
		...
		КолонкаМесяца.Параметры.ИмяМесяца 		= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 año';ru='%1 год';en='%1 year'"), Формат(ВыборкаПоГоду.ГодКалендаря, "ЧГ="));
		ТабличныйДокумент.Присоединить(КолонкаМесяца);
	...
	КолонкаМесяца.Параметры.РабочееВремя36 	= РабочееВремя36Год;
	КолонкаМесяца.Параметры.РабочееВремя24 	= РабочееВремя24Год;
	КолонкаМесяца.Параметры.ИмяМесяца 		= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 año';ru='%1 год';en='%1 year'"), Формат(НомерГода, "ЧГ="));
	...
	КолонкаМесяца.Параметры.ИмяМесяца 		= НСтр("es='La cantidad media mensual';ru='Среднемесячное количество';en='Average monthly amount'");
	ТабличныйДокумент.Присоединить(КолонкаМесяца);
