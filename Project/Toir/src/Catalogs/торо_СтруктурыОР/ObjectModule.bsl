...
Процедура ПриЗаписи(Отказ)
	...
	Если Не ИзменяетсяДокументами Тогда
		...
		Если Выборка.Следующий() Тогда
			ТекстСообщения = НСтр("es='Por la estructura dada hay unos movimientos de los documentos, debe ser установлени el indicio ""Es cambiado por los documentos""';ru='По данной структуре существуют движения документов, должен быть установлени признак ""Изменяется документами""';en='According to this structure, there are document movements, the sign ""Changed by documents"" must be established'");
	...
	Если СтроитсяАвтоматически И ИзменилсяРеквизит Тогда
		...
		Если Не ТаблицаИерархии = Неопределено Тогда
			...
			Исключение
				ТекстСообщения = НСтр("en='Failed to write data to the information register ""EI Order by Hierarchy""';es='No consiguió anotar los datos en el registro de las noticias ""el Orden ОР por la jerarquía""';ru='Не удалось записать данные в регистр сведений ""Порядок ОР по иерархии""'");
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,,,, Отказ);
			КонецПопытки;
