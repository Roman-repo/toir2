...
Процедура ПроверкаИспользования(ОбъектыРасширений = Ложь) Экспорт
	...
	Если СтандартныеПодсистемыПовтИсп.ОтключитьИдентификаторыОбъектовМетаданных() Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La guía ""%1"" no se usa.';ru='Справочник ""%1"" не используется.';en=""Directory '%1' is not used."""), НазваниеСправочника(ОбъектыРасширений));
	КонецЕсли;
	...
	Если ПланыОбмена.ГлавныйУзел() = Неопределено
	   И ЗначениеЗаполнено(ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени("Константа.ГлавныйУзел").Получить()) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La guía ""%1"" no puede usarse "
"en la base informativa con la anulación no confirmada del nudo principal. "
""
"Para la reconstitución del enlace con el nudo principal pongan en marcha 1С:Предприятие y "
"presionen el botón Restablecer o establezcan respecto a programas el nudo principal "
"conservado en la constante el nudo Principal. "
""
"Para la confirmación de la anulación del enlace con el nudo principal pongan en marcha 1С:Предприятие y "
"presionen el botón Desconectar o limpien respecto a programas la constante el nudo Principal.';ru='Справочник ""%1"" не может использоваться"
...
"нажмите кнопку Отключить или программно очистите константу Главный узел.';en='Directory ""%1"" cannot be used"
"in the infobase with unconfirmed cancellation of the main node."
""
"To restore communication with the main node, run 1C: Enterprise and"
"click the Restore button or programmatically install the main node,"
"saved in the constant Main node."
""
"To confirm the cancellation of communication with the main node, run 1C: Enterprise and"
"click the Disable button or programmatically clear the Main node constant. '"),
		...
		Для каждого Объект Из Объекты Цикл
			...
			Иначе
				...
				Если Не ЗначениеЗаполнено(СвойстваЗагружаемого.Ссылка) Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La falta de la carga de los identificadores de los objetos de los metadatos. "
"Es imposible cargar un nuevo elemento a que no es indicada la referencia nuevo: "
"""%1"".';ru='Ошибка загрузки идентификаторов объектов метаданных."
"Невозможно загрузить новый элемент у которого не указана ссылка нового:"
"""%1"".';en='Error loading metadata object identifiers."
"Unable to load a new item that does not have a new link specified:"
"""%1"".'"),
						Объект.ПолноеИмя);
				КонецЕсли;
			КонецЕсли;
		...
		Для каждого СвойстваЗагружаемого Из ТаблицаЗагружаемых Цикл
			...
			Если ПолныеИменаЗагружаемых.Получить(НРег(Объект.ПолноеИмя)) <> Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La falta de la carga de los identificadores de los objetos de los metadatos. "
"Es imposible cargar dos elementos a que coincide el nombre completo: "
"""%1"".';ru='Ошибка загрузки идентификаторов объектов метаданных."
"Невозможно загрузить два элемента у которых совпадает полное имя:"
"""%1"".';en='Error loading metadata object identifiers."
"It is not possible to load two elements with the same full name:"
"""%1"".'"),
					Объект.ПолноеИмя);
			КонецЕсли;
			...
			Если ТипЗнч(КлючОбъектаМетаданных) = Тип("Тип")
			   И КлючОбъектаМетаданных <> Тип("Неопределено") Тогда
				...
				Если КлючиЗагружаемых.Получить(КлючОбъектаМетаданных) <> Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La falta de la carga de los identificadores de los objetos de los metadatos. "
"Es imposible cargar dos elementos a que coincide la llave del objeto de los metadatos: "
"""%1"".';ru='Ошибка загрузки идентификаторов объектов метаданных."
"Невозможно загрузить два элемента у которых совпадает ключ объекта метаданных:"
"""%1"".';en='Error loading metadata object identifiers."
"It is not possible to load two elements with the same metadata object key:"
"""%1"".'"),
				...
				Если СвойстваЗагружаемого.ОбъектМетаданныхПоКлючу <> СвойстваЗагружаемого.ОбъектМетаданныхПоПолномуИмени Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La falta de la carga de los identificadores de los objetos de los metadatos. "
"Es imposible cargar el elemento a que llave del objeto de los metadatos "
"""%1"" no corresponde al nombre completo ""%2"".';ru='Ошибка загрузки идентификаторов объектов метаданных."
"Невозможно загрузить элемент у которого ключ объекта метаданных"
"""%1"" не соответствует полному имени ""%2"".';en='Error loading metadata object identifiers."
"Unable to load item whose metadata object key"
"""%1"" does not match the full name ""%2"".'"),
	...
	Для Каждого ОписаниеОбъектаМетаданных Из ОписаниеОбъектовМетаданных Цикл
		...
		Если ТипОписанияОбъектаМетаданных = Тип("Тип") Тогда
			...
			Если ОбъектМетаданных = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El significado incorrecto del parámetro de OpisanieOb''ektovMetadannyh de la función ОбщегоНазначения.ИдентификаторыОбъектовМетаданных: "
"Es indicado el objeto no existente de los metadatos: ""%1"".';ru='Некорректное значение параметра ОписаниеОбъектовМетаданных функции ОбщегоНазначения.ИдентификаторыОбъектовМетаданных:"
"Указан несуществующий объект метаданных: ""%1"".';en='Invalid parameter value ОписаниеОбъектаМетаданных в функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных.        The nonexistent metadata object specified is: ""%1"".'"),
		...
		Иначе	
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El tipo incorrecto del parámetro de OpisanieOb''ektovMetadannyh de la función ОбщегоНазначения.ИдентификаторыОбъектовМетаданных: "
"""%1""';ru='Некорректный тип параметра ОписаниеОбъектовМетаданных функции ОбщегоНазначения.ИдентификаторыОбъектовМетаданных:"
"""%1""';en='Invalid parameter type ОписаниеОбъектаМетаданных of function ОбщегоНазначения.ИдентификаторыОбъектовМетаданных:"
"""%1"".'"),
...
Функция ИдентификаторОбъектаМетаданных(ОписаниеОбъектаМетаданных) Экспорт
	...
	Если ТипОписанияОбъектаМетаданных = Тип("Тип") Тогда
		...
		Если ОбъектМетаданных = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El significado incorrecto del parámetro de OpisanieOb''ektaMetadannyh en la función ОбщегоНазначения.ИдентификаторОбъектаМетаданных. "
"Es indicado el objeto no existente de los metadatos: ""%1"".';ru='Некорректное значение параметра ОписаниеОбъектаМетаданных в функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных."
"Указан несуществующий объект метаданных: ""%1"".';en='Invalid parameter value ОписаниеОбъектаМетаданных в функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных.        The nonexistent metadata object specified is: ""%1"".'"),
				ОписаниеОбъектаМетаданных);
		Иначе
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El tipo incorrecto del parámetro de OpisanieOb''ektaMetadannyh en la función ОбщегоНазначения.ИдентификаторОбъектаМетаданных: "
"""%1"".';ru='Некорректный тип параметра ОписаниеОбъектаМетаданных в функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных:"
"""%1"".';en='Invalid parameter type ОписаниеОбъектаМетаданных in function ОбщегоНазначения.ИдентификаторОбъектаМетаданных:"
"""%1"".'"),
			ТипОписанияОбъектаМетаданных);
	КонецЕсли;
...
Процедура ДобавитьПереименование(Итог, ВерсияИБ, СтароеПолноеИмя, НовоеПолноеИмя, ИдентификаторБиблиотеки = "") Экспорт
	...
	ЗаголовокОшибки =
		НСтр("es='La falta en el procedimiento ПриДобавленииПереименованийОбъектовМетаданных el módulo ObschegoNaznacheniyaPereopredelyaemyy general.';ru='Ошибка в процедуре ПриДобавленииПереименованийОбъектовМетаданных общего модуля ОбщегоНазначенияПереопределяемый.';en='Error in procedure ПриДобавленииПереименованийОбъектовМетаданных of general module ОбщегоНазначенияПереопределяемый.'");
	...
	Если СтароеИмяКоллекции <> НовоеИмяКоллекции Тогда
		ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No coinciden los nombres de los tipos del objeto cambiado de nombre de los metadatos. "
"El tipo anterior: ""%1"", "
"un nuevo tipo: ""%2"".';ru='Не совпадают имена типов переименованного объекта метаданных."
"Прежний тип: ""%1"","
"новый тип: ""%2"".';en='Type names of the renamed metadata object do not match."
"Previous type: ""%1 "","
"new type: ""%2 "".'"),
			СтароеПолноеИмя,
			НовоеПолноеИмя);
...
Функция СвойстваКоллекцийОбъектовМетаданных(ОбъектыРасширений = Ложь) Экспорт
	...
	Строка.Синоним         = НСтр("es='Las constantes';ru='Константы';en='Constants'");
	Строка.ИмяВЕдЧисле     = "Константа";
	Строка.СинонимВЕдЧисле = НСтр("es='La constante';ru='Константа';en='Constant'");
	
	// Подсистемы
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("cdf5ac50-08e8-46af-9a80-4e63fd4a88ff");
	Строка.Имя             = "Подсистемы";
	Строка.Синоним         = НСтр("es='Los subsistemas';ru='Подсистемы';en='Subsystems'");
	Строка.ИмяВЕдЧисле     = "Подсистема";
	Строка.СинонимВЕдЧисле = НСтр("es='El subsistema';ru='Подсистема';en='Subsystem'");
	...
	Строка.Имя             = "Роли";
	Строка.Синоним         = НСтр("es='Los papeles';ru='Роли';en='Roles'");
	Строка.ИмяВЕдЧисле     = "Роль";
	Строка.СинонимВЕдЧисле = НСтр("es='El papel';ru='Роль';en='Role'");
	Строка.БезДанных       = Истина;
	Строка.БезКлючаОбъектаМетаданных = Истина;
	Строка.ОбъектыРасширений = Истина;
	...
	Строка.Синоним         = НСтр("es='Los planes del cambio';ru='Планы обмена';en='Sharing plans'");
	Строка.ИмяВЕдЧисле     = "ПланОбмена";
	Строка.СинонимВЕдЧисле = НСтр("es='El plan del cambio';ru='План обмена';en='Exchange plan'");
	Строка.ОбъектыРасширений = Истина;
	...
	Строка.Синоним         = НСтр("es='Las guías';ru='Справочники';en='Directories'");
	Строка.ИмяВЕдЧисле     = "Справочник";
	Строка.СинонимВЕдЧисле = НСтр("es='La guía';ru='Справочник';en='Directory'");
	Строка.ОбъектыРасширений = Истина;
	...
	Строка.Синоним         = НСтр("es='Los documentos';ru='Документы';en='Documents'");
	Строка.ИмяВЕдЧисле     = "Документ";
	Строка.СинонимВЕдЧисле = НСтр("es='El documento';ru='Документ';en='Document'");
	Строка.ОбъектыРасширений = Истина;
	...
	Строка.Синоним         = НСтр("es='Las revistas de los documentos';ru='Журналы документов';en='Document logs'");
	Строка.ИмяВЕдЧисле     = "ЖурналДокументов";
	Строка.СинонимВЕдЧисле = НСтр("es='La revista de los documentos';ru='Журнал документов';en='Document log'");
	Строка.БезДанных       = Истина;
	...
	Строка.Имя             = "Отчеты";
	Строка.Синоним         = НСтр("es='Los informes';ru='Отчеты';en='Reports'");
	Строка.ИмяВЕдЧисле     = "Отчет";
	Строка.СинонимВЕдЧисле = НСтр("es='El informe';ru='Отчет';en='Report'");
	Строка.БезДанных       = Истина;
	Строка.ОбъектыРасширений = Истина;
	...
	Строка.Синоним         = НСтр("es='El documento';ru='Документ';en='Document'");
	Строка.ИмяВЕдЧисле     = "Обработка";
	Строка.СинонимВЕдЧисле = НСтр("es='El tratamiento';ru='Обработка';en='Processing'");
	Строка.БезДанных       = Истина;
	Строка.ОбъектыРасширений = Истина;
	...
	Строка.Синоним         = НСтр("es='Los planes de los tipos de las características';ru='Планы видов характеристик';en='Features types plans'");
	Строка.ИмяВЕдЧисле     = "ПланВидовХарактеристик";
	Строка.СинонимВЕдЧисле = НСтр("es='El plan de los tipos de las características';ru='План видов характеристик';en='Feature type plan'");
	
	// ПланыСчетов
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("4295af27-543f-4373-bcfc-c0ace9b7620c");
	Строка.Имя             = "ПланыСчетов";
	Строка.Синоним         = НСтр("es='El informe';ru='Отчет';en='Report'");
	Строка.ИмяВЕдЧисле     = "ПланСчетов";
	Строка.СинонимВЕдЧисле = НСтр("es='El plan de las cuentas';ru='План счетов';en='Chart of accounts'");
	
	// ПланыВидовРасчета
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("fca3e7e1-1bf1-49c8-9921-aafb4e787c75");
	Строка.Имя             = "ПланыВидовРасчета";
	Строка.Синоним         = НСтр("es='Los tratamientos';ru='Обработки';en='Processing'");
	...
	Строка.Синоним         = НСтр("es='Los registros de las noticias';ru='Регистры сведений';en='Information registers'");
	Строка.ИмяВЕдЧисле     = "РегистрСведений";
	Строка.СинонимВЕдЧисле = НСтр("es='Los planes de los tipos de las características';ru='Планы видов характеристик';en='Features types plans'");
	Строка.ОбъектыРасширений = Истина;
	...
	Строка.Синоним         = НСтр("es='Los registros de la acumulación';ru='Регистры накопления';en='Accumulation registers'");
	Строка.ИмяВЕдЧисле     = "РегистрНакопления";
	Строка.СинонимВЕдЧисле = НСтр("es='El registro de la acumulación';ru='Регистр накопления';en='Accumulation register'");
	
	// РегистрыБухгалтерии
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("9a0d75ff-0eda-454e-b2b7-d2412ffdff18");
	Строка.Имя             = "РегистрыБухгалтерии";
	Строка.Синоним         = НСтр("es='Los registros de la contabilidad';ru='Регистры бухгалтерии';en='Accounting registers'");
	Строка.ИмяВЕдЧисле     = "РегистрБухгалтерии";
	Строка.СинонимВЕдЧисле = НСтр("es='El registro de la contabilidad';ru='Регистр бухгалтерии';en='Accounting register'");
	
	// РегистрыРасчета
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("f330686a-0acf-4e26-9cda-108f1404687d");
	Строка.Имя             = "РегистрыРасчета";
	Строка.Синоним         = НСтр("es='Los registros del cálculo';ru='Регистры расчета';en='Billing registers'");
	Строка.ИмяВЕдЧисле     = "РегистрРасчета";
	Строка.СинонимВЕдЧисле = НСтр("es='El registro del cálculo';ru='Регистр расчета';en='Calculation register'");
	
	// БизнесПроцессы
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("a8cdd0e0-c27f-4bf0-9718-10ec054dc468");
	Строка.Имя             = "БизнесПроцессы";
	Строка.Синоним         = НСтр("es='Los procesos de negocios';ru='Бизнес-процессы';en='Business processes'");
	Строка.ИмяВЕдЧисле     = "БизнесПроцесс";
	Строка.СинонимВЕдЧисле = НСтр("es='Los registros de la acumulación';ru='Регистры накопления';en='Accumulation registers'");
	
	// Задачи
	Строка = СвойстваКоллекцийОбъектовМетаданных.Добавить();
	Строка.Идентификатор   = Новый УникальныйИдентификатор("8d9153ad-7cea-4e25-9542-a557ee59fd16");
	Строка.Имя             = "Задачи";
	Строка.Синоним         = НСтр("es='Las tareas';ru='Задачи';en='Tasks'");
	Строка.ИмяВЕдЧисле     = "Задача";
	Строка.СинонимВЕдЧисле = НСтр("es='Los registros de la contabilidad';ru='Регистры бухгалтерии';en='Accounting registers'");
...
Процедура ПередУдалениемОбъекта(Объект) Экспорт
	...
	Если Не Объект.ПометкаУдаления Тогда
		ВызватьИсключениеПоОшибке(ОбъектыРасширений,
			НСтр("es='La desaparición de los identificadores de los objetos, cerca de que el significado "
"del requisito ""la Marca de la desaparición"" es establecido la Mentira es inadmisible.';ru='Удаление идентификаторов объектов, у которых значение"
"реквизита ""Пометка удаления"" установлено Ложь недопустимо.';en='Deleting identifiers of objects whose value is"
"props "" Deletion flag "" set False unacceptable. '"));
	КонецЕсли;
...
Процедура ОбновитьДанные(ЕстьИзменения, ЕстьУдаленные, ТолькоПроверка,
	...
	Попытка
		...
		Если ЗначениеЗаполнено(СписокПереименованийОбъектовМетаданных) Тогда
			СписокКритичныхИзменений = НСтр("es='El cambio de nombre de los identificadores de los objetos De StaroePolnoeImya metadados-> NovoePolnoeImya:';ru='Переименование идентификаторов объектов метаданных СтароеПолноеИмя -> НовоеПолноеИмя:';en='Renaming metadata object identifiers СтароеПолноеИмя -> НовоеПолноеИмя:'")
		...
		Если ЗначениеЗаполнено(СписокНовыхОбъектовМетаданных) Тогда
			СписокКритичныхИзменений = СписокКритичныхИзменений
				+ НСтр("es='La adición de los nuevos identificadores de los objetos de los metadatos:';ru='Добавление новых идентификаторов объектов метаданных:';en='Adding new metadata object identifiers:'")
		...
		Если Не ТолькоПроверка
		   ...
		   И ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Тогда
			...
			ВызватьИсключениеПоОшибке(ОбъектыРасширений,
				НСтр("es='Los cambios críticos pueden ser cumplidos solamente "
"en el nudo principal de la base distribuida informativa. "
"La composición de los cambios exigidos de cm. En la revista del registro.';ru='Критичные изменения могут быть выполнены только"
"в главном узле распределенной информационной базы."
"Состав требуемых изменений см. в журнале регистрации.';en='Critical changes can only be made"
"in the main node of the distributed infobase."
"For the composition of the required changes, see the log. '"));
		КонецЕсли;
		...
		Если Не ТолькоПроверка Тогда
			Если ЗначениеЗаполнено(СписокКритичныхИзменений) Тогда
				ЖурналРегистрации.ДобавитьСообщениеДляЖурналаРегистрации(?(ОбъектыРасширений,
						НСтр("es='Los identificadores de los objetos de las ampliaciones. Son cumplidos los cambios críticos';ru='Идентификаторы объектов расширений.Выполнены критичные изменения';en='Identifiers of extension objects. Critical changes implemented'",
							ОбщегоНазначения.КодОсновногоЯзыка()),
						НСтр("es='Los identificadores de los objetos de los metadatos. Son cumplidos los cambios críticos';ru='Идентификаторы объектов метаданных.Выполнены критичные изменения';en='Metadata object Identifiers. Critical changes made'",
							ОбщегоНазначения.КодОсновногоЯзыка())),
...
Функция МетаданныеНайтиПоПолномуИмени(ПолноеИмя)
	...
	Если ВРег(ОбъектМетаданных.ПолноеИмя()) <> ВРег(ПолноеИмя) Тогда
		...
		Если СтрЧислоВхождений(ВРег(ПолноеИмя), ВРег("Подсистема.")) > 1 Тогда
			...
			КонецЕсли;
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta durante la búsqueda del subsistema filial por el nombre completo (durante la búsqueda ""%1"" era encontrada ""%2""). "
"No debe llamar los subsistemas igualmente, o usar una más nueva versión de la plataforma.';ru='Ошибка при поиске дочерней подсистемы по полному имени (при поиске ""%1"" была найдена ""%2"")."
"Не следует называть подсистемы одинаково, либо использовать более новую версию платформы.';en='An error occurred while searching for a child subsystem by its full name (when searching for ""%1"",""%2"" was found)."
"Do not call the subsystem the same, or use a newer version of the platform. '"),
				ПолноеИмя,
				ОбъектМетаданных.ПолноеИмя());
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta durante la búsqueda del objeto de los metadatos por el nombre completo (durante la búsqueda ""%1"" era encontrado ""%2"").';ru='Ошибка при поиске объекта метаданных по полному имени (при поиске ""%1"" был найден ""%2"").';en='An error occurred while searching for a metadata object by its full name (""%2"" was found while searching for ""%1"").'"),
				ПолноеИмя,
				ОбъектМетаданных.ПолноеИмя());
		КонецЕсли;
	КонецЕсли;
...
Функция НазваниеСправочника(ОбъектыРасширений)
	...
	Если ОбъектыРасширений Тогда
		НазваниеСправочника = НСтр("es='Los identificadores de los objetos de las ampliaciones';ru='Идентификаторы объектов расширений';en='Extension object identifiers'");
	Иначе
		НазваниеСправочника = НСтр("es='Los identificadores de los objetos de los metadatos';ru='Идентификаторы объектов метаданных';en='Metadata object identifiers'");
	КонецЕсли;
...
Процедура ВызватьИсключениеПоОшибке(ОбъектыРасширений, ТекстОшибки)
	...
	ЗаголовокОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La falta en el trabajo con la guía ""%1"".';ru='Ошибка при работе со справочником ""%1"".';en='Error occurred while working with the directory ""%1"".'"),
		НазваниеСправочника(ОбъектыРасширений));
...
Процедура ПроверитьОбъектПередЗаписью(Объект, АвтоматическоеОбновление = Ложь)
	...
	Если НЕ АвтоматическоеОбновление Тогда
		...
		Если Объект.ЭтоНовый() Тогда
			...
			ВызватьИсключениеПоОшибке(ОбъектыРасширений,
				НСтр("es='La creación del nuevo identificador del objeto "
"probablemente es automática solamente a la renovación de los datos de la guía.';ru='Создание нового идентификатора объекта"
"возможно только автоматически при обновлении данных справочника.';en='Creating a new object identifier"
"possible only automatically when updating the directory data. '"));
		...
		ИначеЕсли ЗапрещеноИзменятьПолноеИмя(Объект) Тогда
			...
			ВызватьИсключениеПоОшибке(ОбъектыРасширений, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Con el cambio del identificador del objeto es indicado "
"el nombre ""%1"" completo, que puede "
"ser establecido solamente automáticamente a la renovación de los datos de la guía.';ru='При изменении идентификатора объекта указано"
"полное имя ""%1"", которое может быть"
"установлено только автоматически при обновлении данных справочника.';en='When changing the identifier of the object is indicated"
"full name ""%1"" which may be"
"set only automatically when updating the directory data.'"),
				Объект.ПолноеИмя));
		Иначе
			...
			Если ЗначениеЗаполнено(НазваниеСправочника) Тогда
				ВызватьИсключениеПоОшибке(ОбъектыРасширений, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Con el cambio del identificador del objeto es indicado "
"el nombre ""%1"" completo, "
"que se usa ya en la guía ""%2"".';ru='При изменении идентификатора объекта указано"
"полное имя ""%1"","
"которое уже используется в справочнике ""%2"".';en='When changing the identifier of the object is indicated"
"full name ""%1"","
"which is already used in the directory ""%2"".'"),
	...
	Если Не ОбъектыРасширений И ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Тогда
		...
		Если Объект.ЭтоНовый()
		   И Не ЭтоКоллекция(Объект.ПолучитьСсылкуНового(), ЭтоОбъектРасширений(Объект)) Тогда
			...
			ВызватьИсключениеПоОшибке(ОбъектыРасширений,
				НСтр("es='La adición de los nuevos elementos puede ser cumplida "
"solamente en el nudo principal de la base distribuida informativa.';ru='Добавление новых элементов может быть выполнено"
"только в главном узле распределенной информационной базы.';en='Adding new items can be done"
		...
		Если Не Объект.ПометкаУдаления
		   И Не ЭтоКоллекция(Объект.Ссылка, ЭтоОбъектРасширений(Объект)) Тогда
			...
			Если ВРег(Объект.ПолноеИмя) <> ВРег(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "ПолноеИмя")) Тогда
				ВызватьИсключениеПоОшибке(ОбъектыРасширений,
					НСтр("es='El cambio del requisito ""el nombre Completo"" puede ser cumplido "
"solamente en el nudo principal de la base distribuida informativa.';ru='Изменение реквизита ""Полное имя"" может быть выполнено"
"только в главном узле распределенной информационной базы.';en='Change props ""Full name"" ""can be performed only in the main node of the distributed infobase.'"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ОписаниеОшибкиИдентификаторыОбъектовРасширенийНедоступныВНеразделенномРежиме()
	...
	Возврат
		НСтр("es='La guía ""los Identificadores de los objetos de las ampliaciones"" "
"no puede usarse en el régimen no dividido.';ru='Справочник ""Идентификаторы объектов расширений"""
"не может использоваться в неразделенном режиме.';en='Directory ""Extension Object Identifiers"" "
"cannot be used in unshared mode.'");
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			Если ОбъектМетаданных = Неопределено Тогда
				...
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El objeto de los metadatos no es encontrado por el nombre completo: "
"""%1"".';ru='Объект метаданных не найден по полному имени:"
"""%1"".';en='Metadata object not found by full name:"
"""%1"".'"),
			...
			Если Не Метаданные.Роли.Содержит(ОбъектМетаданных)
			   ...
			   И Не ЭтоПодсистема(ОбъектМетаданных) Тогда
				...
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El objeto de los metadatos no es apoyado: "
"""%1"". "
""
"Son admisibles solamente los tipos de los objetos de los metadatos enumerado en los comentarios de la función.';ru='Объект метаданных не поддерживается:"
"""%1""."
""
"Допустимы только типы объектов метаданных перечисленные в комментарии к функции.';en='Metadata object is not supported:"
"""%1""."
""
"Only metadata object types listed in the function comment are allowed.'"),
					ПолноеИмяОбъектаМетаданных);
				Ошибки.Добавить(ОписаниеОшибки);
				Продолжить;
			КонецЕсли;
			...
			Если Расширение <> Неопределено
			   И Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
				...
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Los identificadores de los objetos de las ampliaciones metadadas no son apoyados en el régimen no dividido. "
"Es imposible devolver el identificador del objeto de los metadatos de ""%1"" "
"ampliaciones de ""%2"" versiones ""%3"".';ru='Идентификаторы объектов метаданных расширений не поддерживаются в неразделенном режиме."
"Невозможно вернуть идентификатор объекта метаданных ""%1"""
"расширения ""%2"" версии ""%3"".';en='Extension metadata object identifiers are not supported in unshared mode."
"Unable to return metadata object id ""%1 """
"extension ""%2 "" version ""%3 "". '"),
			...
			ШаблонОшибки = ?(Расширение <> Неопределено,
				НСтр("es='Para el objeto de los metadatos ""%1"" no es encontrado el identificador en el registro de las noticias ""los Identificadores de los objetos de las versiones de las ampliaciones"
""".';ru='Для объекта метаданных ""%1"""
"не найден идентификатор в регистре сведений ""Идентификаторы объектов версий расширений"".';en='For metadata object"" ""%1"""
"the identifier was not found in the information register """" Identifiers of extensions version objects ""'"),
				НСтр("es='Para el objeto de los metadatos ""%1"" no es encontrado el identificador en la guía ""los Identificadores de los objetos de los metadatos"
""".';ru='Для объекта метаданных ""%1"""
"не найден идентификатор в справочнике ""Идентификаторы объектов метаданных"".';en='For metadata object"" ""%1"" "
"the identifier was not found in the reference """" Metadata object identifiers ""'"));
		...
		ИначеЕсли НайденныеСтроки.Количество() > 1 Тогда
			...
			ШаблонОшибки = ?(ДоступныИдентификаторыОбъектовРасширений,
				НСтр("es='Para el objeto de los metadatos ""%1"" "
"es encontrado algunos identificadores en la guía ""los Identificadores de los objetos de los metadatos"" y "
"el registro de las noticias ""los Identificadores de los objetos de las versiones de las ampliaciones"".';ru='Для объекта метаданных ""%1"""
"найдено несколько идентификаторов в справочнике ""Идентификаторы объектов метаданных"" и"
"регистре сведений ""Идентификаторы объектов версий расширений"".';en='For metadata object"" ""%1"" "
"several identifiers were found in the ""Metadata Object Identifiers"" and"
"information register "" Identifiers of extensions version objects"".'"),
				НСтр("es='Para el objeto de los metadatos ""%1"" "
"es encontrado algunos identificadores en la guía ""los Identificadores de los objetos de los metadatos"".';ru='Для объекта метаданных ""%1"""
"найдено несколько идентификаторов в справочнике ""Идентификаторы объектов метаданных"".';en='For metadata object"" ""%1"""
"Multiple identifiers found in the reference ""Metadata Object Identifiers"".'"));
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, ПолноеИмяОбъектаМетаданных);
			ДобавитьУточнениеОшибкиПараметровРаботыПрограммыДляРазработчика = Истина;
			Ошибки.Добавить(ОписаниеОшибки);
			Продолжить;
		...
		Если РезультатПроверки.НеСоответствует Тогда
			...
			Если РезультатПроверки.ОбъектМетаданных = Неопределено Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Para el objeto de los metadatos ""%1"" "
"es encontrado el identificador en la guía ""%2"", "
"a que corresponde el objeto alejado de los metadatos.';ru='Для объекта метаданных ""%1"""
"найден идентификатор в справочнике ""%2"","
"которому соответствует удаленный объект метаданных.';en='For metadata object"" ""%1"" "
"the identifier was found in the directory """"%2"","
"which corresponds to the remote metadata object.'"),
					ПолноеИмяОбъектаМетаданных, НазваниеСправочника);
			Иначе
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Para el objeto de los metadatos ""%1"" "
"es encontrado el identificador en la guía ""%2"", "
"que corresponde a otro objeto de los metadatos ""%3"".';ru='Для объекта метаданных ""%1"""
"найден идентификатор в справочнике ""%2"","
"который соответствует другому объекту метаданных ""%3"".';en='For metadata object"" ""%1"""
"the identifier was found in the directory """"%2 "","
"which corresponds to another metadata object ""%3 ""'"),
	...
	Если КоличествоОшибок > 0 Тогда
		...
		Если ОдинЭлемент Тогда
			ЗаголовокОшибки = НСтр("es='La falta a la ejecución de la función ОбщегоНазначения.ИдентификаторОбъектаМетаданных ().';ru='Ошибка при выполнении функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных().';en='Error executing function ОбщегоНазначения.ИдентификаторОбъектаМетаданных().'");
			
		ИначеЕсли КоличествоОшибок = 1 Тогда
			ЗаголовокОшибки = НСтр("es='La falta a la ejecución de la función ОбщегоНазначения.ИдентификаторыОбъектаМетаданных ().';ru='Ошибка при выполнении функции ОбщегоНазначения.ИдентификаторыОбъектаМетаданных().';en='Error executing function ОбщегоНазначения.ИдентификаторыОбъектаМетаданных().'");
		Иначе
			ЗаголовокОшибки = НСтр("es='Las faltas a la ejecución de la función ОбщегоНазначения.ИдентификаторыОбъектаМетаданных ().';ru='Ошибки при выполнении функции ОбщегоНазначения.ИдентификаторыОбъектаМетаданных().';en='Errors while executing a function ОбщегоНазначения.ИдентификаторыОбъектаМетаданных().'");
		КонецЕсли;
		...
		Для Каждого ОписаниеОшибки Из Ошибки Цикл
			...
			Если НомерОшибки = 3 И КоличествоОшибок > 5 Тогда
				ПредставлениеКоличества = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
					НСтр("es='; %1 falta;; %1 faltas; %1 faltas; %1 faltas';ru=';%1 ошибка;; %1 ошибки; %1 ошибок; %1 ошибки';en=';%1 error;; %1 errors; %1 errors; %1 errors'"), (КоличествоОшибок - НомерОшибки));
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='... Y todavía %1';ru='... И еще %1';en='... And more %1'"),
...
Функция ОбъектыМетаданныхПоИдентификаторамСПопыткойПовтора(Идентификаторы, ВызыватьИсключение)
	...
	Для Каждого ТекущийИдентификатор Из Идентификаторы Цикл
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta a la ejecución de la función ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору (). "
""
"El tipo equivocado del identificador del objeto de los metadatos: "
"""%1"".';ru='Ошибка при выполнении функции ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору()."
""
"Неверный тип идентификатора объекта метаданных:"
"""%1"".';en='Error executing function ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору().    Invalid metadata object identifier type:"
"""%1"".'"),
...
Функция ОбъектыМетаданныхПоИдентификаторамБезПопыткиПовтора(Идентификаторы,
	...
	Выгрузка = Запрос.Выполнить().Выгрузить();
	ЗаголовокОшибки = НСтр("es='La falta a la ejecución de la función ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору ().';ru='Ошибка при выполнении функции ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору().';en='Error executing function ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору().'");
	...
	Если Выгрузка.Количество() < ВсегоИдентификаторов Тогда
		...
		Если ВызыватьИсключение Тогда
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El identificador ""%1"" no es encontrado en la guía "
"""%2"".';ru='Идентификатор ""%1"""
"не найден в справочнике ""%2"".';en='Identifier ""%1"""
"not found in directory ""%2 "".'"),
				...
				Если Свойства.ОбъектРасширений И Не Свойства.ПометкаУдаления Тогда
					...
					Если ВызыватьИсключение Тогда
						...
						Если РасширенияКонфигурации.Получить(Отбор, ИсточникРасширенийКонфигурации.СеансОтключенные).Количество() > 0 Тогда
							ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La ampliación de la configuración %1 es establecida, pero es desconectado durante el lanzamiento de la sesión. "
"Habitualmente, esto significa que a la conexión de la ampliación de la configuración ha surgido la falta.';ru='Расширение конфигурации %1 установлено, но отключено при запуске сеанса."
"Обычно, это значит, что при подключении расширения конфигурации возникла ошибка.';en='The configuration extension%1 is installed but disabled at session start."
"Typically, this means that an error occurred while connecting the configuration extension.'"),
								Свойства.ИмяРасширения);
						Иначе
							ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La ampliación de la configuración %1 es establecida, pero no es conectado. "
"Es necesario перезапустить la sesión.';ru='Расширение конфигурации %1 установлено, но не подключено."
"Требуется перезапустить сеанс.';en='The configuration extension%1 is installed but not connected."
"You must restart the session. '"),
				...
				ИначеЕсли РезультатПроверки.КлючОбъектаМетаданных = Неопределено Тогда
					ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Al identificador ""%1"" "
"encontrado en la guía ""%2"", "
"le corresponde el objeto no existente de los metadatos "
"""%3"".';ru='Идентификатору ""%1"""
"найденному в справочнике ""%2"","
"соответствует несуществующий объект метаданных"
"""%3"".';en='Identifier ""%1"""
"found in the directory ""%2"","
"matches a nonexistent metadata object"
"""%3 "".'"),
				...
				Иначе
					ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Al identificador ""%1"" "
"encontrado en la guía ""%2"", "
"le corresponde el objeto alejado de los metadatos.';ru='Идентификатору ""%1"""
"найденному в справочнике ""%2"","
"соответствует удаленный объект метаданных.';en='Identifier ""%1"""
"found in the directory ""%2"","
"matches the remote metadata object.'"),
						Строка(Свойства.Ссылка),
						НазваниеСправочника(Свойства.ОбъектРасширений));
				КонецЕсли;
			...
			Иначе
				ОписаниеОшибки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Al identificador ""%1"" "
"encontrado en la guía ""%2"", "
"le corresponde el objeto de los metadatos ""%3"", "
"que nombre completo se distingue de dado en el identificador.';ru='Идентификатору ""%1"""
"найденному в справочнике ""%2"","
"соответствует объект метаданных ""%3"","
"полное имя которого отличается от заданного в идентификаторе.';en='Identifier ""%1"" "
"found in the directory ""%2"","
"matches metadata object ""%3"" , full name of which is different from that specified in the identifier. '"),
					Строка(Свойства.Ссылка),
					НазваниеСправочника(Свойства.ОбъектРасширений),
		...
		Если Не Свойства.ОбъектРасширений И Свойства.ПометкаУдаления И Не ВерсияПрограммыОбновленаДинамически Тогда
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El identificador ""%1"" "
"es encontrado en la guía ""%2"", "
"pero el significado del requisito ""la Marca de la desaparición"" es establecido la Verdad.';ru='Идентификатор ""%1"""
"найден в справочнике ""%2"","
"но значение реквизита ""Пометка удаления"" установлено Истина.';en='Identifier ""%1"" found in directory ""%2"","
"but the value of the props ""Deletion mark"" is set to True.'"),
				Строка(Свойства.Ссылка),
				НазваниеСправочника(Свойства.ОбъектРасширений));
...
Процедура ЗаменитьДублиПодчиненногоУзлаНайденныеПриЗагрузке(ТолькоПроверка, ЕстьИзменения)
	...
	Пока Истина Цикл
		...
		Если ТекущаяПопытка > 10 Тогда
			ВызватьИсключение
				НСтр("es='No consiguió cumplir la sustitución de las tomas de los identificadores de los objetos de los metadatos. "
"Después de 10 tentativas hay todavía unos datos, que exigen la sustitución. "
"Cumplan la acción en el régimen monopolizado.';ru='Не удалось выполнить замену дублей идентификаторов объектов метаданных."
"После 10 попыток все еще имеются данные, которые требуют замены."
"Выполните действие в монопольном режиме.';en='Failed to replace duplicate metadata object identifiers."
"After 10 attempts, there is still data that needs to be replaced."
"Perform the action in exclusive mode. '");
		КонецЕсли;
		...
		Если Не БезОшибок Тогда
			ВызватьИсключение
				НСтр("es='No consiguió cumplir la sustitución de las tomas de los identificadores de los objetos de los metadatos. "
"Más detalladamente cm. Las faltas a la sustitución del identificador en la revista del registro.';ru='Не удалось выполнить замену дублей идентификаторов объектов метаданных."
"Подробнее см. ошибки при замене идентификатора в журнале регистрации.';en='Failed to replace duplicate metadata object identifiers."
"For more details, see errors when replacing the identifier in the log. '");
		КонецЕсли;
				...
				Если ОбрабатываемаяСсылка <> Неопределено И Параметры.Объект <> Неопределено Тогда
					...
					Исключение
						...
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='A la anotación del objeto ""%1"" ha surgido la falta: "
"%2';ru='При записи объекта ""%1"" возникла ошибка:"
"%2';en='An error occurred while writing to object ""%1"":"
"%2'"),
				...
				Для Каждого Движение Из СтрокаТаблицы.Метаданные.Движения Цикл
					...
					Если НадоЗаписывать Тогда
						...
						Исключение
							...
							ИнформацияОбОшибке = ИнформацияОбОшибке();
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='A la anotación de los movimientos del objeto ""%1"" en ""%2"" ha surgido la falta: "
"%3';ru='При записи движений объекта ""%1"" в ""%2"" возникла ошибка:"
"%3';en='An error occurred while recording the movements of the object ""%1"" in ""%2"":"
"%3'"),
								ПолучитьНавигационнуюСсылку(Параметры.Объект.Ссылка),
						...
						Если НаборЗаписи.Количество() > 0 Тогда
							...
							Если НадоЗаписывать Тогда
								...
								Исключение
									...
									ИнформацияОбОшибке = ИнформацияОбОшибке();
									ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("es='A la anotación por el registrador ""%1"" en ""%2"" ha surgido la falta: "
"%3';ru='При записи по регистратору ""%1"" в ""%2"" возникла ошибка:"
"%3';en='An error occurred while writing to the registrar ""%1"" in ""%2"":"
"%3 '"),
			...
			ИначеЕсли Метаданные.РегистрыСведений.Содержит(СтрокаТаблицы.Метаданные) Тогда
				...
				Попытка
					...
					Исключение
						ИнформацияОбОшибке = ИнформацияОбОшибке();
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='A la desaparición de la anotación ""%1"" ha surgido la falta: "
"%2';ru='При удалении записи ""%1"" возникла ошибка:"
"%2';en='An error occurred while deleting entry ""%1"":                                      %2 '"),
					...
					Исключение
						ИнформацияОбОшибке = ИнформацияОбОшибке();
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='A la adición de la anotación ""%1"" ha surgido la falta: "
"%2';ru='При добавлении записи ""%1"" возникла ошибка:"
"%2';en='An error occurred while adding record ""%1"":"
"%2'"),
							ПолучитьНавигационнуюСсылку(МенеджерРегистра.СоздатьКлючЗаписи(СтруктураИзмерений)),
							ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
						ВызватьИсключение;
				...
				КонецПопытки;
			Иначе
				СообщитьОбОшибке(НСтр("es='Los significados no son sustituidos en los datos del tipo';ru='Значения не заменяются в данных типа';en='Values are not replaced in data type'")
...
Процедура СообщитьОбОшибке(Знач Описание, ОбъектыРасширений)
	...
	ЗаписьЖурналаРегистрации(
		?(ОбъектыРасширений,
			НСтр("es='Los identificadores de los objetos de las ampliaciones. La sustitución del identificador';ru='Идентификаторы объектов расширений.Замена идентификатора';en='Identifiers of extension objects. Change of identifier'",
				ОбщегоНазначения.КодОсновногоЯзыка()),
			НСтр("es='Los identificadores de los objetos de los metadatos. La sustitución del identificador';ru='Идентификаторы объектов метаданных.Замена идентификатора';en='Identifiers of metadata objects. Replacement of identifier'",
				ОбщегоНазначения.КодОсновногоЯзыка())),
