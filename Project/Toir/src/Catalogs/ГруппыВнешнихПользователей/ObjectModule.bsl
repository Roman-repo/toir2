	...
	Для каждого ТекущаяСтрока Из Состав Цикл
		...
		Если НЕ ЗначениеЗаполнено(ТекущаяСтрока.ВнешнийПользователь) Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				...
				НСтр("es='El usuario exterior no es escogido.';ru='Внешний пользователь не выбран.';en='No external user selected.'"),
				"Объект.Состав",
				НомерСтроки,
				НСтр("es='El usuario exterior en la línea %1 no es escogido.';ru='Внешний пользователь в строке %1 не выбран.';en='The external user in row%1 is not selected.'"));
			Продолжить;
		КонецЕсли;
		...
		Если НайденныеЗначения.Количество() > 1 Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				"Объект.Состав[%1].ВнешнийПользователь",
				НСтр("es='El usuario exterior repete.';ru='Внешний пользователь повторяется.';en='The external user is repeating.'"),
				"Объект.Состав",
				НомерСтроки,
				НСтр("es='El usuario exterior en la línea %1 repete.';ru='Внешний пользователь в строке %1 повторяется.';en='The external user at line%1 is repeated.'"));
		КонецЕсли;
	КонецЦикла;
...
Процедура ПередЗаписью(Отказ)
	...
	Если Ссылка = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
		Если Состав.Количество() > 0 Тогда
			ВызватьИсключение
				НСтр("es='La adición de los participantes en el grupo predestinado ""Todos los usuarios exteriores"" es prohíbida.';ru='Добавление участников в предопределенную группу ""Все внешние пользователи"" запрещено.';en='Adding members to the predefined group ""All external users"" is prohibited.'");
		КонецЕсли;
	Иначе
		ТекстОшибки = ТекстОшибкиПроверкиНазначения();
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
...
Функция ТекстОшибкиПроверкиРодителя()
	...
	Если Родитель = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
		Возврат
			НСтр("es='El grupo predestinado ""Todos los usuarios exteriores"" no puede ser los padres.';ru='Предопределенная группа ""Все внешние пользователи"" не может быть родителем.';en='Predefined group ""All external users"" cannot be a parent.'");
	КонецЕсли;
	
	Если Ссылка = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
		Если Не Родитель.Пустая() Тогда
			Возврат
				НСтр("es='El grupo predestinado ""Todos los usuarios exteriores"" no puede ser trasladado.';ru='Предопределенная группа ""Все внешние пользователи"" не может быть перемещена.';en='Predefined group ""All external users"" cannot be moved.'");
		КонецЕсли;
	Иначе
		Если Родитель = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
			Возврат
				НСтр("es='Es imposible añadir el subgrupo al grupo predestinado ""Todos los usuarios exteriores"".';ru='Невозможно добавить подгруппу к предопределенной группе ""Все внешние пользователи"".';en='Unable to add a subgroup to the predefined group ""All external users"".'");
			
		ИначеЕсли Родитель.ВсеОбъектыАвторизации Тогда
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es imposible añadir el subgrupo a grupo ""%1"", "
"puesto que en el número de sus participantes entran todos los usuarios.';ru='Невозможно добавить подгруппу к группе ""%1"","
"так как в число ее участников входят все пользователи.';en='Unable to add subgroup to group ""%1"","
"because its members include all users. '"), Родитель);
		КонецЕсли;
		
		Если ВсеОбъектыАвторизации И ЗначениеЗаполнено(Родитель) Тогда
			Возврат
				НСтр("es='Es imposible trasladar el grupo, en que número de los participantes entran todos los usuarios.';ru='Невозможно переместить группу, в число участников которой входят все пользователи.';en='Unable to move a group whose members include all users.'");
		КонецЕсли;
	КонецЕсли;
...
Функция ТекстОшибкиПроверкиНазначения()
	...
	Если Назначение.Количество() = 0 Тогда
		Возврат НСтр("es='del grupo.';ru='Не указан вид участников группы.';en='The type of group members is not specified.'");
	КонецЕсли;
	...
	Если ВсеОбъектыАвторизации Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.СпискиЗначенийИдентичны(НазначениеВсеВнешниеПользователи, МассивНазначения) Тогда
			Возврат
				НСтр("es='Es imposible crear el grupo que coincide a destino "
"con el grupo predestinado ""Todos los usuarios exteriores"".';ru='Невозможно создать группу, совпадающую по назначению"
"с предопределенной группой ""Все внешние пользователи"".';en='Unable to create a matching group"
"with the predefined group ""All external users"".'");
		КонецЕсли;
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Hay ya grupo ""%1"", "
"en que número de los participantes entran todos los usuarios de los tipos indicados.';ru='Уже существует группа ""%1"","
"в число участников которой входят все пользователи указанных видов.';en='Group ""%1"" already exists,"
"which includes all users of these types.'"),
	...
	Если ЗначениеЗаполнено(Родитель) Тогда
		...
		Для Каждого ТипПользователя Из ТипПользователей Цикл
			Если ТипПользователейРодителя.Найти(ТипПользователя) = Неопределено Тогда
				Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El tipo de los participantes del grupo debe ser como cerca "
"del grupo superior de los usuarios exteriores ""%1"".';ru='Вид участников группы должен быть как у вышестоящей"
"группы внешних пользователей ""%1"".';en='The appearance of the group members should be like that of the parent"
"external user groups ""% 1 "".'"), Родитель);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Если ВсеОбъектыАвторизации
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			Возврат
				НСтр("es='Es imposible cambiar el tipo de los participantes del grupo, "
"puesto que ella tiene unos subgrupos.';ru='Невозможно изменить вид участников группы,"
"так как у нее имеются подгруппы.';en='Unable to change the view of the group’s members,"
"because she has subgroups.'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Ссылка) Тогда
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es imposible cambiar el tipo de los participantes del grupo, "
"puesto que ella tiene un subgrupo ""%1"" con otro destino de los participantes.';ru='Невозможно изменить вид участников группы,"
"так как у нее имеется подгруппа ""%1"" с другим назначением участников.';en='Unable to change the view of the group’s members,"
"because she has a subgroup ""%1"" with a different assignment of participants.'"),
				Выборка.СсылкаПредставление);
		КонецЕсли;
	КонецЕсли;
