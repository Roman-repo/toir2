...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПользователиСлужебный.ВнешниеПользователиВнедрены() Тогда
		ВызватьИсключение НСтр("es='Los usuarios exteriores no son previstos en la versión dada del programa.';ru='Внешние пользователи не предусмотрены в данной версии программы.';en='External users are not provided in this version of the program.'");
	КонецЕсли;
...
Процедура СоставПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если СообщениеПользователю <> Неопределено Тогда
		ПоказатьОповещениеПользователя(
			НСтр("es='El traslado de los usuarios';ru='Перемещение пользователей';en='User movement'"), , СообщениеПользователю, БиблиотекаКартинок.Информация32);
	КонецЕсли;
...
Процедура ВыбратьНазначение(Команда)
	...
	ПользователиСлужебныйКлиент.ВыбратьНазначение(ЭтотОбъект, НСтр("es='La elección del tipo de los usuarios';ru='Выбор вида пользователей';en='User type selection'"), Ложь, Ложь, ОписаниеОповещения);
...
Функция ПараметрыРасширеннойФормыПодбора()
	...
	ЗаголовокФормыПодбора = НСтр("es='La selección de los participantes del grupo de los usuarios exteriores';ru='Подбор участников группы внешних пользователей';en='Selection of members of the external user group'");
