...
Процедура ПроверитьПриоритет(Отказ)
	...
	Если Выборка.Следующий() Тогда
		ТекстСообщения = НСтр("es='La operación clave con la prioridad ""%1"" existe ya (%2).';ru='Ключевая операция с приоритетом ""%1"" уже существует (%2).';en=""Key operation with priority '%1' already exists (%2).""");
