...
Процедура ДекорацияТребуетсяОбновлениеССервераОбработкаНавигационнойСсылки(
	...
	Если ВРег(НавигационнаяСсылкаФорматированнойСтроки) = ВРег("Update") Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("es='La renovación es acabada';ru='Обновление завершено';en='Update completed'"),
			,
			НСтр("es='La renovación de los datos, también la optimización ulterior de los datos cargados son acabadas';ru='Обновление данных, а также последующая оптимизация загруженных данных завершены';en='Updating data, and subsequent optimization of the downloaded data is completed'"));
...
Процедура ДекорацияТребуетсяОбновлениеССервераОбработкаНавигационнойСсылкиСервер()
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La renovación de los clasificadores (a mano)';ru='Обновление классификаторов (вручную)';en='Classifiers update (manually)'"));
