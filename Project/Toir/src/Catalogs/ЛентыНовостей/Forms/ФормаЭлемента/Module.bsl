...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Ключ.Пустая() Тогда
		...
		Если (ЭтотОбъект.РазделениеВключено) И (ЭтотОбъект.ДоступныРазделенныеДанные) Тогда
			...
			Если Запись.Выбран() Тогда
				...
				Если ЭтотОбъект.РазделениеВключено Тогда
					ТекстЗаголовка = НСтр("es='La cinta de las novedades es respecto a programas escondida de todos los usuarios de esta área de los datos';ru='Лента новостей программно скрыта от всех пользователей этой области данных';en='News feed is software hidden from all users of this data area.'");
				Иначе
					ТекстЗаголовка = НСтр("es='La cinta de las novedades es respecto a programas escondida de todos los usuarios';ru='Лента новостей программно скрыта от всех пользователей';en='News feed is software hidden from all users'");
				КонецЕсли;
					...
					Если ЭтотОбъект.РазделениеВключено Тогда
						ТекстЗаголовка = ТекстЗаголовка
							...
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='Por la causa: %1.';ru='по причине: %1.';en='due to: %1.'"),
								ПричинаОтключения);
					Иначе
						ТекстЗаголовка = ТекстЗаголовка
							+ " "
	...
	Иначе
		...
		Если Объект.ЗагруженоССервера = Ложь Тогда
			...
			Для Каждого ТекущийРедактор Из МассивРедакторовЛентыНовостей Цикл
				...
				НоваяСтрока.ПользовательПредставление = СтрЗаменить(НоваяСтрока.ПользовательПредставление, НСтр("es='(Los plenos derechos)';ru='(полные права)';en='(full rights)'"), НСтр("es='(Los plenos derechos, puede redactar las novedades en todas las cintas de las novedades)';ru='(полные права, может редактировать новости во всех лентах новостей)';en='(full rights, can edit news in all news feeds)'"));
			КонецЦикла;
	...
	Если (Объект.ЗагруженоССервера = Истина) Тогда
		...
		Иначе
			Элементы.ГруппаФайлНовостей.Заголовок = НСтр("es='La recepción del fichero de las novedades (estos campos перезаполняются es automático con los cambios en el servidor 1С)';ru='Получение файла новостей (эти поля перезаполняются автоматически при изменениях на сервере 1С)';en='Receiving a news file (these fields are automatically replenished upon changes on the 1C server)'");
		КонецЕсли;
	...
	Если (Объект.ЗагруженоССервера = Ложь)
		...
		Элементы.ЧастотаОбновления.СписокВыбора.Добавить(0, НСтр("es='Renovar solamente a mano';ru='Обновлять только вручную';en='Update manually only'"));
	Иначе
		...
		КонецЕсли;
	КонецЕсли;
	Элементы.ЧастотаОбновления.СписокВыбора.Добавить(1, НСтр("es='Raramente (una vez al día)';ru='Редко (один раз в день)';en='Rare (once a day)'"));
	Элементы.ЧастотаОбновления.СписокВыбора.Добавить(2, НСтр("es='A menudo (cada hora)';ru='Часто (каждый час)';en='Often (every hour)'"));
	Элементы.ЧастотаОбновления.СписокВыбора.Добавить(3, НСтр("es='Muy a menudo (cada 15 minutos)';ru='Очень часто (каждые 15 минут)';en='Very often (every 15 minutes)'"));
	Элементы.ЧастотаОбновления.СписокВыбора.Добавить(4, НСтр("es='Cada minuto';ru='Каждую минуту';en='Every minute'"));
...
Процедура ИмяФайлаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ВРег(Объект.Протокол) = ВРег("file") Тогда
		...
		Диалог.Заголовок = НСтр("es='Escojan el fichero de las novedades en el formato Atom o RSS';ru='Выберите файл новостей в формате Atom или RSS';en='Select a news file in Atom or RSS format'");
...
Процедура ДоступныеКатегорииНовостейПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Объект.ЗагруженоССервера = Истина Тогда
		...
		Сообщение.Текст = НСтр("es='No es posible añadir las categorías para la cinta de las novedades cargadas del servidor.';ru='Нельзя добавлять категории для ленты новостей, загруженной с сервера.';en='Can not add categories to feeds downloaded from the server.'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура ДоступныеКатегорииНовостейПередУдалением(Элемент, Отказ)
	...
	Если Объект.ЗагруженоССервера = Истина Тогда
		Отказ = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("es='No es posible quitar las categorías para la cinta de las novedades cargadas del servidor.';ru='Нельзя удалять категории для ленты новостей, загруженной с сервера.';en='Can not delete categories for a feed downloaded from the server.'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура КомандаПодборКатегорий(Команда)
	...
	Если Объект.ЗагруженоССервера = Истина Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("es='No es posible cambiar las categorías para la cinta de las novedades cargadas del servidor.';ru='Нельзя изменять категории для ленты новостей, загруженной с сервера.';en='Can not change categories for feeds downloaded from the server.'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьИнформационныеСтроки()
	...
	Если ЭтотОбъект.РазделениеВключено Тогда
		...
		Элементы.ДекорацияВидимостьПоУмолчанию.Заголовок   = НСтр("es='Sin excepciones. En el modelo del servicio no es posible ajustar la visibilidad de la cinta de las novedades por los usuarios.';ru='без исключений. В модели сервиса нельзя настроить видимость ленты новостей по пользователям.';en='with no exceptions. In the service model, you cannot configure the visibility of the news feed by users.'");
		Элементы.ДекорацияВидимостьПоУмолчанию.Гиперссылка = Ложь;
	Иначе
		Элементы.ДекорацияВидимостьПоУмолчанию.Гиперссылка = Истина;
		Если Объект.ИсключенияВидимости.Количество() = 0 Тогда
			Если Объект.ВидимостьПоУмолчанию = Истина Тогда
				Элементы.ДекорацияВидимостьПоУмолчанию.Заголовок = НСтр("es='Sin excepciones';ru='без исключений';en='with no exceptions'");
			Иначе
				Элементы.ДекорацияВидимостьПоУмолчанию.Заголовок = НСтр("es='Sin excepciones. Nadie puede escoger esta cinta de las novedades para la recepción de las novedades.';ru='без исключений. Никто не сможет выбрать эту ленту новостей для получения новостей.';en='with no exceptions. No one will be able to select this news feed to receive news.'");
		...
		ИначеЕсли Объект.ИсключенияВидимости.Количество() <= 3 Тогда
			СписокИсключений = НСтр("es='Excepto:';ru='кроме:';en='except:'") + " ";
		...
		Иначе // > 3
			...
			ТекстСообщения   = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='%1 y todavía %2 usuarios';ru='%1 и еще %2 пользователей';en='%1 and%2 more users'"),
				СписокИсключений,
	...
	Если Объект.Ссылка.Пустая() Тогда
		Элементы.ДекорацияСтатистика.Заголовок = НСтр("es='Es necesario al principio anotar la cinta de las novedades para recibir por ella la estadística.';ru='Необходимо вначале записать ленту новостей, чтобы получить по ней статистику.';en='Required first to record a news feed in order to get statistics on it.'");
	Иначе
		...
		Если Объект.ЛокальнаяЛентаНовостей = Истина Тогда
			Если КоличествоНовостейВсего = 0 Тогда
				ТекстСтатистики = НСтр("es='No existe ni una novedad';ru='Нет ни одной новости';en='No news'")
					+ Символы.ПС;
			Иначе
				ТекстСтатистики = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='En total novedades en esta cinta de las novedades: %1';ru='Всего новостей в этой ленте новостей: %1';en='Total news in this feed:%1'"),
					КоличествоНовостейВсего);
				Если КоличествоНовостейПомеченныхНаУдаление = 0 Тогда
					ТекстСтатистики = ТекстСтатистики + " " + НСтр("es='Y entre ellos no existe ni uno marcado en la desaparición';ru='и среди них нет ни одной помеченной на удаление';en='and among them there is not one marked for deletion'")
						+ Символы.ПС;
				ИначеЕсли КоличествоНовостейПомеченныхНаУдаление = КоличествоНовостейВсего Тогда // Все помечены на удаление
					ТекстСтатистики = ТекстСтатистики + ", " + НСтр("es='Todos ellos son marcados a la desaparición (entonces no son visible en la lista de las novedades)';ru='все они помечены на удаление (а значит не видны в списке новостей)';en='they are all marked for deletion (which means they are not visible in the news list)'")
						+ Символы.ПС;
				Иначе
					ТекстСтатистики = ТекстСтатистики
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es=', De ellos es marcado a la desaparición %1';ru=', из них помечено на удаление %1';en=', of whick are marked for deleting %1'")
								+ Символы.ПС,
		...
		Иначе
			Если КоличествоНовостейВсего = 0 Тогда
				ТекстСтатистики = НСтр("es='No existe ni una novedad cargada';ru='Нет ни одной загруженной новости';en='No news uploaded'")
					+ Символы.ПС;
			Иначе
				ТекстСтатистики = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='En total novedades en esta cinta de las novedades: %1';ru='Всего новостей в этой ленте новостей: %1';en='Total news in this feed:%1'"),
				...
				Иначе
					ТекстСтатистики = ТекстСтатистики + ", "
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='De ellos es marcado a la desaparición %1';ru='из них помечено на удаление %1';en='of them marked for deletion %1'")
								+ Символы.ПС,
							КоличествоНовостейПомеченныхНаУдаление);
				КонецЕсли;
			КонецЕсли;
			Если ДатаПоследнейУспешнойЗагрузки = '00010101' Тогда
				Если КоличествоНеудачПриЗагрузкеНовостей = 0 Тогда
					ТекстСтатистики = ТекстСтатистики + НСтр("es='Las novedades por esta cinta de las novedades no eran cargadas';ru='Новости по этой ленте новостей еще не загружались';en='News on this feed has not yet been downloaded.'");
				Иначе
					ТекстСтатистики = ТекстСтатистики
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='Las novedades por esta cinta de las novedades todavía eran con éxito cargadas nunca (había %1 intentos fallados de la carga de las novedades)';ru='Новости по этой ленте новостей еще ни разу не были успешно загружены (было %1 неудачных попыток загрузки новостей)';en='News on this feed has never been successfully downloaded (there were%1 unsuccessful attempts to download news)'"),
			...
			Иначе
				Если КоличествоНеудачПриЗагрузкеНовостей = 0 Тогда
					ТекстСтатистики = ТекстСтатистики
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='Las novedades por esta cinta de las novedades la última vez eran con éxito cargadas %1 (hora local)';ru='Новости по этой ленте новостей последний раз были успешно загружены %1 (по местному времени)';en='The news on this feed was last successfully downloaded%1 (local time)'"),
							Формат(МестноеВремя(ДатаПоследнейУспешнойЗагрузки), "ДЛФ=DT"));
				Иначе
					ТекстСтатистики = ТекстСтатистики
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='Las novedades por esta cinta de las novedades la última vez eran con éxito cargadas %1 (hora local), después de esto había %2 intentos fallados de la carga de las novedades.';ru='Новости по этой ленте новостей последний раз были успешно загружены %1 (по местному времени), после этого было %2 неудачных попыток загрузки новостей.';en='The news on this news feed was last successfully downloaded%1 (local time), after which there were%2 unsuccessful attempts to download news.'"),
							Формат(МестноеВремя(ДатаПоследнейУспешнойЗагрузки), "ДЛФ=DT"),
							КоличествоНеудачПриЗагрузкеНовостей);
				КонецЕсли;
			КонецЕсли;
