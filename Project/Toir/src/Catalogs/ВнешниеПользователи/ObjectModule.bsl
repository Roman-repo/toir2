...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ОбъектАвторизации) Тогда
		ВызватьИсключение НСтр("es='Al usuario exterior no es dado el objeto de la autorización.';ru='У внешнего пользователя не задан объект авторизации.';en='The external user does not have an authorization object.'");
	Иначе
		ТекстОшибки = "";
	...
	Иначе
		...
		Если ЗначениеЗаполнено(СтарыйОбъектАвторизации)
		   И СтарыйОбъектАвторизации <> ОбъектАвторизации Тогда
			...
			ВызватьИсключение НСтр("es='Es imposible cambiar antes el objeto indicado de la autorización.';ru='Невозможно изменить ранее указанный объект авторизации.';en='Unable to modify the previously specified authorization object.'");
		КонецЕсли;
	КонецЕсли;
