...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Пользователи.ЭтоПолноправныйПользователь() Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
			...
			НСтр("es='Preparado responsable de la lista';ru='Подготовленные ответственным за список';en='Prepared by the list manager'"), Ложь,
...
Процедура ВидПользователейНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПользователиСлужебныйКлиент.ВыбратьНазначение(ЭтотОбъект, НСтр("es='La elección del tipo de los usuarios';ru='Выбор вида пользователей';en='User type selection'"), Ложь, Истина, ОписаниеОповещения);
...
Процедура ГруппыВнешнихПользователейПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если ВыбиратьИерархически Тогда
		ПоказатьПредупреждение(,
			НСтр("es='Para перетаскивания el usuario en los grupos es necesario desconectar "
"el banderín ""Mostrar a los usuarios de los grupos inferiores"".';ru='Для перетаскивания пользователя в группы необходимо отключить"
"флажок ""Показывать пользователей нижестоящих групп"".';en='To drag a user into groups, you must disable"
"checkbox  ""Show users of downstream groups ""'"));
		Возврат;
	КонецЕсли;
	...
	Если Строка = ХранимыеПараметры.ГруппаВсеПользователи
		...
		СообщениеПользователю = Новый Структура("Сообщение, ЕстьОшибки, Пользователи",
			НСтр("es='De los grupos con el tipo de los participantes ""los Todos usuarios del tipo dado"" la excepción de los usuarios es imposible.';ru='Из групп с типом участников ""Все пользователи заданного типа"" исключение пользователей невозможно.';en='It is not possible to exclude users from groups with the type of members ""All users of a given type"".'"),
			Истина,
			Неопределено);
	Иначе
		...
		ДействиеСПользователем = 
			...
			НСтр("es='Incluir';ru='Включить';en='Enable'"),
			?(Перемещение, НСтр("es='Trasladar';ru='Переместить';en='Move'"), НСтр("es='Copiar';ru='Скопировать';en='Copy'")));
		
		Если ГруппаПомеченаНаУдаление Тогда
			ШаблонДействия = ?(Перемещение, НСтр("es='Grupo ""%1"" es marcado a la desaparición. %2';ru='Группа ""%1"" помечена на удаление. %2';en=""Group '% 1' is marked for deletion. % 2"""), 
				НСтр("es='Grupo ""%1"" es marcado a la desaparición. %2';ru='Группа ""%1"" помечена на удаление. %2';en=""Group '% 1' is marked for deletion. % 2"""));
		...
		Если КоличествоПользователей = 1 Тогда
			Если ДействиеИсключитьПользователя Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='¿Excluir al usuario ""%1"" de grupo ""%2""?';ru='Исключить пользователя ""%1"" из группы ""%2""?';en='Exclude user ""%1"" from group ""%2""?'"),
			...
			ИначеЕсли Не ГруппаПомеченаНаУдаление Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='¿%1 usuarios ""%2"" en grupo ""%3""?';ru='%1 пользователя ""%2"" в группу ""%3""?';en='% 1 of user ""% 2"" in group ""% 3""?'"),
					ДействиеСПользователем,
					Строка(ПараметрыПеретаскивания.Значение[0]),
					Строка(Строка));
			Иначе
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='¿%1 usuarios ""%2"" en este grupo?';ru='%1 пользователя ""%2"" в эту группу?';en='% 1 of user ""% 2"" in this group?'"),
					ДействиеСПользователем,
					Строка(ПараметрыПеретаскивания.Значение[0]));
			КонецЕсли;
		Иначе
			Если ДействиеИсключитьПользователя Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='¿Excluir a los usuarios (%1) de grupo ""%2""?';ru='Исключить пользователей (%1) из группы ""%2""?';en='Exclude users (%1) from group ""%2""?'"),
					КоличествоПользователей,
					Строка(Элементы.ГруппыВнешнихПользователей.ТекущаяСтрока));
				
			ИначеЕсли Не ГруппаПомеченаНаУдаление Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='¿%1 usuarios (%2) en grupo ""%3""?';ru='%1 пользователей (%2) в группу ""%3""?';en=""% 1 users (% 2) in group '% 3'?"""),
					ДействиеСПользователем,
					КоличествоПользователей,
					Строка(Строка));
			Иначе
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='¿%1 usuarios (%2) en este grupo?';ru='%1 пользователей (%2) в эту группу?';en='% 1 users (% 2) in this group?'"),
					ДействиеСПользователем,
					КоличествоПользователей);
			КонецЕсли;
		КонецЕсли;
...
Процедура СоздатьГруппуВнешнихПользователей(Команда)
	...
	Если ТекущиеДанные.ВсеОбъектыАвторизации Тогда
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es imposible añadir el subgrupo a grupo ""%1"", "
"puesto que en el número de sus participantes entran todos los usuarios de los tipos escogidos.';ru='Невозможно добавить подгруппу к группе ""%1"", "
"так как в число ее участников входят все пользователи выбранных видов.';en='Unable to add subgroup to group ""%1"","
"as its members include all users of the selected species.'"),
...
Процедура ИзменитьПараметрыРасширеннойФормыПодбора()
	...
	ИначеЕсли ИспользоватьГруппыПользователей Тогда
		...
		Элементы.ВнешниеПользователиСписок.Заголовок             = НСтр("es='Los usuarios en el grupo';ru='Пользователи в группе';en='Users in the group'");
...
Процедура ОбновитьЗаголовокСпискаВыбранныхПользователейИГрупп()
	...
	Если ХранимыеПараметры.ИспользоватьГруппы Тогда
		ЗаголовокВыбранныеПользователиИГруппы = НСтр("es='Los usuarios escogidos y grupos (%1)';ru='Выбранные пользователи и группы (%1)';en='Selected Users and Groups (% 1)'");
	Иначе
		ЗаголовокВыбранныеПользователиИГруппы = НСтр("es='Los usuarios escogidos y grupos (%1)';ru='Выбранные пользователи и группы (%1)';en='Selected Users and Groups (%1)'");
	КонецЕсли;
	...
	Иначе
		...
		Если ХранимыеПараметры.ИспользоватьГруппы Тогда
			Элементы.СписокВыбранныхПользователейИГрупп.Заголовок = НСтр("es='Los usuarios escogidos y los grupos';ru='Выбранные пользователи и группы';en='Selected Users and Groups'");
		Иначе
			Элементы.СписокВыбранныхПользователейИГрупп.Заголовок = НСтр("es='Los usuarios escogidos y los grupos';ru='Выбранные пользователи и группы';en='Selected Users and Groups'");
		КонецЕсли;
	...
	Если Параметры.РежимВыбора Тогда
		...
		Если Параметры.ЗакрыватьПриВыборе = Ложь Тогда
			...
			Если ВыборГруппВнешнихПользователей Тогда
				...
				Иначе
					Заголовок = НСтр("es='La selección de los usuarios exteriores y los grupos';ru='Подбор внешних пользователей и групп';en='Selection of external users and groups'");
				КонецЕсли;
				
				ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
					"ВыбратьВнешнегоПользователя", "Заголовок", НСтр("es='Escoger a los usuarios exteriores';ru='Выбрать внешних пользователей';en='Select external users'"));
				
				ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
					"ВыбратьГруппуВнешнихПользователей", "Заголовок", НСтр("es='Escoger los grupos';ru='Выбрать группы';en='Choose groups'"));
			Иначе
				Если ХранимыеПараметры.РасширенныйПодбор Тогда
					Заголовок = ХранимыеПараметры.ЗаголовокФормыПодбора;
				Иначе
					Заголовок = НСтр("es='La selección de los usuarios exteriores';ru='Подбор внешних пользователей';en='Selection of external users'");
				КонецЕсли;
			КонецЕсли;
		Иначе
			// Режим выбора.
			Если ВыборГруппВнешнихПользователей Тогда
				Заголовок = НСтр("es='La elección del usuario exterior o el grupo';ru='Выбор внешнего пользователя или группы';en='Select external user or group'");
				
				ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
					"ВыбратьВнешнегоПользователя", "Заголовок", НСтр("es='Escoger al usuario exterior';ru='Выбрать внешнего пользователя';en='Select external user'"));
			Иначе
				Заголовок = НСтр("es='La elección del usuario exterior';ru='Выбор внешнего пользователя';en='External user selection'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ГруппыВнешнихПользователейПеретаскиваниеЗавершение(СообщениеПользователю)
	...
	Если СообщениеПользователю.ЕстьОшибки = Ложь Тогда
		ПоказатьОповещениеПользователя(
			НСтр("es='El traslado de los usuarios';ru='Перемещение пользователей';en='User movement'"), , СообщениеПользователю.Сообщение, БиблиотекаКартинок.Информация32);
	Иначе
...
"usuarios Siguientes no eran incluidos en el grupo escogido: "
"%2';ru='%1"
"Следующие пользователи не были включены в выбранную группу:"
"%2';en='%1                                 The following users were not included in the selected group:"
