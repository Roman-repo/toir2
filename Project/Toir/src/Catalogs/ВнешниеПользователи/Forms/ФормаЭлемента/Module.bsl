	   ...
	   И ПользовательИБРоли.Количество() = 0 Тогда
		...
		Если НЕ ПараметрыЗаписи.Свойство("СПустымСпискомРолей") Тогда
			...
			ПоказатьВопрос(
				...
				,
				,
				НСтр("es='La anotación del usuario de la base informativa';ru='Запись пользователя информационной базы';en='Information base user writing'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если Не Элементы.Роли.ТолькоПросмотр Тогда
		...
		Для Каждого Строка Из ЭлементыДерева Цикл
			...
			Если Строка.ЭтоНедоступнаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El papel ""%1"" es inaccesible para los usuarios exteriores.';ru='Роль ""%1"" недоступна для внешних пользователей.';en='The role ""%1"" is not available to external users.'"), Строка.Синоним),
					"Роли",
					ЭлементыДерева.Индекс(Строка),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El papel ""%1"" en la línea %%1 es inaccesible para los usuarios exteriores.';ru='Роль ""%1"" в строке %%1 недоступна для внешних пользователей.';en='The role ""%1"" in line %%1 is not available to external users.'"), Строка.Синоним));
			КонецЕсли;
		КонецЦикла;
...
"la marca a la desaparición de este usuario exterior.';ru='Чтобы разрешить вход в программу, требуется снять"
"пометку на удаление с этого внешнего пользователя.';en='To allow entry into the program, you must remove"
"a note to remove from this external user.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОпределитьНесоответствияПользователяСПользователемИБ(ПараметрыЗаписи = Неопределено)
	...
	ИначеЕсли УровеньДоступа.УправлениеСписком Тогда
		...
		Если ПользовательИБАутентификацияОС <> Ложь Тогда
			УточнениеСвойств.Добавить(НСтр("es='Аутентификация la SO (es incluida)';ru='Аутентификация ОС (включена)';en='OS Authentication (Enabled)'"));
		КонецЕсли;
		...
		Если ПользовательИБПользовательОС <> "" Тогда
			УточнениеСвойств.Добавить(НСтр("es='El usuario de la SO (es indicado)';ru='Пользователь ОС (указан)';en='OS user (specified)'"));
		КонецЕсли;
		
		Если ПользовательИБПоказыватьВСпискеВыбора Тогда
			ЕстьРазличияУстранимыеБезАдминистратора = Истина;
			УточнениеСвойств.Добавить(НСтр("es='Mostrar en la lista de la elección (es incluido)';ru='Показывать в списке выбора (включено)';en='Show in selection list (included)'"));
		КонецЕсли;
		
		Если ПользовательИБРежимЗапуска <> "Авто" Тогда
			ЕстьРазличияУстранимыеБезАдминистратора = Истина;
			УточнениеСвойств.Добавить(НСтр("es='El régimen del lanzamiento (no Avto)';ru='Режим запуска (не Авто)';en='Startup mode (not Auto)'"));
		КонецЕсли;
