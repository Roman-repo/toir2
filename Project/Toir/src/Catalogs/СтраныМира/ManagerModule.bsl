...
Процедура ЗаполнитьСписокСтранКОбработке(Параметры) Экспорт
	...
	НоваяСтрока.Код                = "203";
	НоваяСтрока.Наименование       = НСтр("es='LA REPÚBLICA CHECA';ru='ЧЕШСКАЯ РЕСПУБЛИКА';en='CZECH REPUBLIC'");
	НоваяСтрока.КодАльфа2          = "CZ";
	НоваяСтрока.КодАльфа3          = "CZE";
	
	НоваяСтрока                    = ЗначенияОКСМ.Добавить();
	НоваяСтрока.Код                = "270";
	НоваяСтрока.Наименование       = НСтр("es='LA GAMBIA';ru='ГАМБИЯ';en='GAMBIA'");
	НоваяСтрока.КодАльфа2          = "GM";
	НоваяСтрока.КодАльфа3          = "GMB";
	НоваяСтрока.НаименованиеПолное = НСтр("es='La república Gambia';ru='Республика Гамбия';en='Republic of The Gambia '");
...
Процедура ОбновитьСтраныМираПоОКСМ(Параметры) Экспорт
	...
	Пока СтранаМираСсылка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió tratar el país: %1 por la causa: %2';ru='Не удалось обработать страну: %1 по причине: %2';en='Failed to process country: %1 due to: %2'"),
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Al procedimiento de ObnovitStranyMiraPoOKSM no consiguió tratar algunos países del mundo (son dejados pasar): %1';ru='Процедуре ОбновитьСтраныМираПоОКСМ не удалось обработать некоторые страны мира(пропущены): %1';en='A procedure ОбновитьСтраныМираПоОКСМ не удалось обработать некоторые страны мира(пропущены): %1'"), 
				ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			Метаданные.Справочники.СтраныМира,,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El procedimiento de ObnovitStranyMiraPoOKSM ha tratado la porción siguiente de países del mundo: %1';ru='Процедура ОбновитьСтраныМираПоОКСМ обработала очередную порцию стран мира: %1';en='A procedure ОбновитьСтраныМираПоОКСМ processed the next batch of world countries:%1'"),
					ОбъектовОбработано));
	КонецЕсли;
