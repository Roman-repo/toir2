...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("es='La tarjeta de la nomenclatura';ru='Карточка номенклатуры';en='Item card'");
	КомандаПечати.ЗаголовокФормы = НСтр("es='La tarjeta de la nomenclatura';ru='Карточка номенклатуры';en='Item card'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КарточкаНоменклатуры") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("es='La tarjeta de la nomenclatura';ru='Карточка номенклатуры';en='Item card'"), 
...
Процедура ЗаполнитьТабличныйДокументКарточкаНоменклатуры(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ДанныеПечати.Код) Тогда
			...
			СтруктураПараметровМакета.Вставить("ИмяПараметра", НСтр("es='El código:';ru='Код:';en='Code:'"));
		...
		Если ЗначениеЗаполнено(ДанныеПечати.Артикул) Тогда
			...
			ОбластьМакета = ?(ПорядокПараметра%2 = 0, ОбластьПараметровТемная, ОбластьПараметровСветлая);
			СтруктураПараметровМакета = Новый Структура;
			СтруктураПараметровМакета.Вставить("ИмяПараметра", НСтр("es='El artículo:';ru='Артикул:';en='Vendor code:'"));
		...
		Если ЗначениеЗаполнено(ДанныеПечати.ТипНоменклатуры) Тогда
			Если ДанныеПечати.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Услуга 
				...
				ОбластьМакета = ?(ПорядокПараметра%2 = 0, ОбластьПараметровТемная, ОбластьПараметровСветлая);
				СтруктураПараметровМакета = Новый Структура;
				СтруктураПараметровМакета.Вставить("ИмяПараметра", НСтр("es='El tipo:';ru='Тип:';en='Type:'"));
				СтруктураПараметровМакета.Вставить("ЗначениеПараметра", НСтр("es='El servicio';ru='Услуга';en='Service'"));
				ОбластьМакета.Параметры.Заполнить(СтруктураПараметровМакета);
				ТабличныйДокумент.Вывести(ОбластьМакета);
				ПорядокПараметра = ПорядокПараметра + 1;
		...
		Если ЗначениеЗаполнено(ДанныеПечати.Производитель) Тогда
			...
			ОбластьМакета = ?(ПорядокПараметра%2 = 0, ОбластьПараметровТемная, ОбластьПараметровСветлая);
			СтруктураПараметровМакета = Новый Структура;
			СтруктураПараметровМакета.Вставить("ИмяПараметра", НСтр("es='El productor:';ru='Производитель:';en='Manufacturer:'"));
		...
		Если ЗначениеЗаполнено(ДанныеПечати.ЕдиницаИзмерения) Тогда
			...
			ОбластьМакета = ?(ПорядокПараметра%2 = 0, ОбластьПараметровТемная, ОбластьПараметровСветлая);
			СтруктураПараметровМакета = Новый Структура;
			СтруктураПараметровМакета.Вставить("ИмяПараметра", НСтр("es='La unidad de medida:';ru='Единица измерения:';en='Unit of measurement:'"));
