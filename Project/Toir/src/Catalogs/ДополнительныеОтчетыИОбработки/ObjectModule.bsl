	...
	Если НЕ ДополнительныеОтчетыИОбработки.ПроверитьГлобальнаяОбработка(Вид) Тогда
		Если НЕ ИспользоватьДляФормыОбъекта И НЕ ИспользоватьДляФормыСписка 
			...
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("es='Es necesario desconectar la publicación o escoger para el uso como mínimo una de las formas';ru='Необходимо отключить публикацию или выбрать для использования как минимум одну из форм';en='Required to disable the publication or select at least one of the forms to use.'")
				,
				,
				,
		...
		Если Конфликтующие.Количество() > 0 Тогда
			...
			Если ПроверкаЭлемента Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El nombre ""%1"" usado por el informe (tratamiento) dado, es ocupado ya por otro informe (tratamiento) publicado adicional. "
""
"La continuación tiene que cambiar el tipo de la Publicación con ""%2"" en ""%3"" o ""%4"".';ru='Имя ""%1"", используемое данным отчетом (обработкой), уже занято другим опубликованным дополнительным отчетом (обработкой). "
""
"Для продолжения необходимо изменить вид Публикации с ""%2"" на ""%3"" или ""%4"".';en='Name ""%1"" ""used by this report (processing) is already taken by another published additional report (processing).'"),
					ИмяОбъекта,
			...
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El nombre ""%1"" usado por el informe (tratamiento) ""%2"", es ocupado ya por otro informe (tratamiento) publicado adicional.';ru='Имя ""%1"", используемое отчетом (обработкой) ""%2"", уже занято другим опубликованным дополнительным отчетом (обработкой).';en='The name ""%1"" used by the report (processing) ""%2"" is already taken by another published additional report (processing).'"),
					ИмяОбъекта,
...
Процедура ПередЗаписью(Отказ)
	...
	ИнтеграцияСТехнологиейСервиса.ПередЗаписьюДополнительнойОбработки(ЭтотОбъект, Отказ);
	
	Если ЭтоНовый() И НЕ ДополнительныеОтчетыИОбработки.ПравоДобавления(ЭтотОбъект) Тогда
		ВызватьИсключение НСтр("es='Es insuficientemente derecho para la adición de los informes adicionales o los tratamientos.';ru='Недостаточно прав для добавления дополнительных отчетов или обработок.';en='Not enough permissions to add additional reports or processings.'");
	КонецЕсли;
	
	// Предварительные проверки
	Если НЕ ЭтоНовый() И Вид <> ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "Вид") Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("es='Es imposible cambiar el tipo del informe existente adicional o el tratamiento.';ru='Невозможно сменить вид существующего дополнительного отчета или обработки.';en='Unable to change the appearance of an existing subreport or processing.'"),,,,
			Отказ);
		Возврат;
	КонецЕсли;
...
Процедура ПередУдалением(Отказ)
	...
	ИнтеграцияСТехнологиейСервиса.ПередУдалениемДополнительнойОбработки(ЭтотОбъект, Отказ);
...
Функция ПредставлениеЗадания(Команда)
	...
	Возврат (
		...
		+ СокрЛП(Наименование)
		+ " / "
		+ НСтр("es='La orden';ru='Команда';en='Command'")
		+ ": "
...
Процедура ПриЗаписиОтчета(Отказ)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВариантыОтчетов") Тогда
		...
		Попытка
			...
			КонецЕсли;
		Исключение
			ТекстОшибки = НСтр("es='La falta de la conexión:';ru='Ошибка подключения:';en='Connection error:'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
