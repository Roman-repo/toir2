...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
		ВызватьИсключение НСтр("es='La creación del nuevo elemento es prohíbida la copia.';ru='Создание нового элемента копированием запрещено.';en='Creating a new item by copying is prohibited.'");
	КонецЕсли;
	...
	Если Объект.Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма
		...
		Отказ = Истина;
		ОбщегоНазначения.СообщитьПользователю(НСтр("es='El trabajo con las formas de imprenta no es apoyado.';ru='Работа с печатными формами не поддерживается.';en='Work with printed forms is not supported.'"));
		Возврат;
	КонецЕсли;
	...
	Если Не ПравоДобавления Тогда
		Если ЭтоНовый Тогда
			ВызватьИсключение НСтр("es='No basta los derechos de admisión para la adición de los informes adicionales o los tratamientos.';ru='Недостаточно прав доступа для добавления дополнительных отчетов или обработок.';en='Not enough access rights to add additional reports or processings.'");
		Иначе
	...
	Если ПоказатьДиалогЗагрузкиИзФайлаПриОткрытии И Не Элементы.ЗагрузитьИзФайла.Видимость Тогда
		ВызватьИсключение НСтр("es='Es insuficientemente derecho para la carga de los informes adicionales y los tratamientos';ru='Недостаточно прав для загрузки дополнительных отчетов и обработок';en='Not enough rights to download additional reports and processings'");
	КонецЕсли;
...
Процедура ВариантыДополнительногоОтчетаПередУдалением(Элемент, Отказ)
	...
	Если НЕ Вариант.Пользовательский Тогда
		ПоказатьПредупреждение(, НСтр("es='La marca a la desaparición de la variante predestinada del informe es prohibida.';ru='Пометка на удаление предопределенного варианта отчета запрещена.';en='Mark to delete a predefined report variant is prohibited.'"));
		Возврат;
	КонецЕсли;
	
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='¿Marcar ""%1"" a la desaparición?';ru='Пометить ""%1"" на удаление?';en='Mark ""%1"" for deletion?'"), Вариант.Наименование);
...
Процедура ВариантыДополнительногоОтчетаОткрыть(Команда)
	...
	Если Вариант = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("es='Escojan la variante del informe.';ru='Выберите вариант отчета.';en='Select a report option.'"));
		Возврат;
	КонецЕсли;
...
Процедура НастроитьВидимость(Команда)
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("es='Para el ajuste de la visibilidad de las órdenes de la prensa el tratamiento es necesario anotar. ¿Continuar?';ru='Для настройки видимости команд печати обработку необходимо записать. Продолжить?';en='To adjust the visibility of print commands, the processing must be recorded. Continue?'");
...
Процедура ВыполнитьКоманду(Команда)
	...
	Если Объект.Ссылка.Пустая() Или Модифицированность Тогда
		ТекстВопроса = НСтр("es='Para la ejecución de la orden es necesario anotar los datos.';ru='Для выполнения команды необходимо записать данные.';en='To execute the command, you must write data.'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("ЗаписатьИПродолжить", НСтр("es='Anotar y continuar';ru='Записать и продолжить';en='Write and continue'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
...
Процедура ПродолжитьЗаписьНаКлиенте(Результат, ЗакрытьПослеЗаписи)  Экспорт
	...
	Если ПараметрыЗаписи.РегистрацияОбработки Тогда
		ОбновитьПовторноИспользуемыеЗначения();
		ТекстОповещения = НСтр("es='Para la aplicación de los cambios en las ventanas abiertas los es necesario cerrar y abrir de nuevo.';ru='Для применения изменений в открытых окнах необходимо их закрыть и открыть заново.';en='To apply changes in open windows, you must close and open them again.'");
...
Процедура ОбновитьИзФайлаПослеПодтверждения(Ответ, ПараметрыРегистрации) Экспорт
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		ПараметрыЗагрузки.Диалог.Заголовок = НСтр("es='Escojan el fichero del informe exterior o el tratamiento';ru='Выберите файл внешнего отчета или обработки';en='Select an external report or processing file'");
	ИначеЕсли Объект.Вид = ВидДополнительныйОтчет Или Объект.Вид = ВидОтчет Тогда
		ПараметрыЗагрузки.Диалог.ИндексФильтра = 1;
		ПараметрыЗагрузки.Диалог.Заголовок = НСтр("es='Escojan el fichero del informe exterior';ru='Выберите файл внешнего отчета';en='Select External Report File'");
	Иначе
		ПараметрыЗагрузки.Диалог.ИндексФильтра = 2;
		ПараметрыЗагрузки.Диалог.Заголовок = НСтр("es='Escojan el fichero del tratamiento exterior';ru='Выберите файл внешней обработки';en='Select external processing file'");
	КонецЕсли;
...
Процедура ОбновитьИзФайлаПослеВыбораФайла(ОписаниеФайла, ПараметрыРегистрации) Экспорт
	...
	Иначе
		...
		ТекстПредупреждения = НСтр("es='La ampliación del fichero no corresponde a la ampliación del informe exterior (ERF) o el tratamiento (EPF).';ru='Расширение файла не соответствует расширению внешнего отчета (ERF) или обработки (EPF).';en='The file extension does not match the external report extension (ERF) or processing (EPF).'");
...
Процедура ОбновитьИзФайлаИСообщить(ПараметрыРегистрации)
	...
	Если ПараметрыРегистрации.Успех Тогда
		ОповещениеЗаголовок = ?(ПараметрыРегистрации.ЭтоОтчет, НСтр("es='El fichero del informe exterior es cargado';ru='Файл внешнего отчета загружен';en='External report file uploaded'"), НСтр("es='El fichero del tratamiento exterior es cargado';ru='Файл внешней обработки загружен';en='External processing file uploaded'"));
...
Процедура ПоказатьКонфликты(ПараметрыРегистрации)
	...
	Если ПараметрыРегистрации.КоличествоКонфликтующих > 1 Тогда
		Если ПараметрыРегистрации.ЭтоОтчет Тогда
			ЗаголовокВопроса = НСтр("es='Los conflictos a la carga del informe exterior';ru='Конфликты при загрузке внешнего отчета';en='Conflicts when loading an external report'");
			ТекстВопроса = НСтр("es='El nombre interior del informe ""[Name]"" "
"es ocupado ya por los informes existentes adicionales ([Count]): "
"[List]. "
""
"Escojan: "
"1. ""[Continue]"" - cargar un nuevo informe en el régimen del ajuste. "
"2. ""[Disable]"" - cargar un nuevo informe, habiendo desconectado la publicación de todos los informes que chocan. "
"3. ""[Open]"" - anular la carga y mostrar la lista de los informes que chocan.';ru='Внутреннее имя отчета ""[Name]"""
...
"3. ""[Open]"" - отменить загрузку и показать список конфликтующих отчетов.';en='Internal report name"" ""[Name]"" """
"already taken by existing additional reports ([Count]):"
"[List]."
""
"Choose:"
"1. """" [Continue] """" - load a new report in debug mode."
"2. """" [Disable] """" - load a new report, disabling the publication of all conflicting reports."
"3. """" [Open] """" - cancel the download and show a list of conflicting reports.'");
		Иначе
			ЗаголовокВопроса = НСтр("es='Los conflictos a la carga del tratamiento exterior';ru='Конфликты при загрузке внешней обработки';en='Conflicts when loading external processing'");
			ТекстВопроса = НСтр("es='El nombre interior del tratamiento ""[Name]"" "
"es ocupado ya por los tratamientos existentes adicionales ([Count]): "
"[List]. "
""
"Escojan: "
"1. ""[Continue]"" - cargar un nuevo tratamiento en el régimen del ajuste. "
"2. ""[Disable]"" - cargar un nuevo tratamiento, habiendo desconectado la publicación de todos los tratamientos que chocan. "
"3. ""[Open]"" - anular la carga y mostrar la lista de los tratamientos que chocan.';ru='Внутреннее имя обработки ""[Name]"""
...
"3. ""[Open]"" - отменить загрузку и показать список конфликтующих обработок.';en='Internal name of processing"" ""[Name]"" """
"already taken by existing additional treatments ([Count]):"
"[List]."
""
"Choose:"
"1. """" [Continue] """" - load the new processing in debug mode."
"2. """" [Disable] """" - load a new processing, disabling the publication of all conflicting treatments."
"3. """" [Open] """" - cancel the download and show a list of conflicting processing.'");
		КонецЕсли;
		ПредставлениеКнопкиОтключить = НСтр("es='Desconectar que chocan';ru='Отключить конфликтующие';en='Disable conflicting'");
		ПредставлениеКнопкиОткрыть = НСтр("es='Anular y mostrar la lista';ru='Отменить и показать список';en='Cancel and show list'");
	Иначе
		Если ПараметрыРегистрации.ЭтоОтчет Тогда
			ЗаголовокВопроса = НСтр("es='El conflicto a la carga del informe exterior';ru='Конфликт при загрузке внешнего отчета';en='Conflict while loading an external report'");
			ТекстВопроса = НСтр("es='El nombre interior del informe ""[Name]"" "
"es ocupado ya por el informe existente adicional [List]. "
""
"Escojan: "
"1. ""[Continue]"" - cargar un nuevo informe en el régimen del ajuste. "
"2. ""[Disable]"" - cargar un nuevo informe, habiendo desconectado la publicación del informe que choca. "
"3. ""[Open]"" - abrir la tarjeta del informe que choca.';ru='Внутреннее имя отчета ""[Name]"""
...
"3. ""[Open]"" - открыть карточку конфликтующего отчета.';en='Internal report name"" ""[Name]"" """
"already taken by existing sub report [List]."
""
"Choose:"
"1. """" [Continue] """" - load a new report in debug mode."
"2. """" [Disable] """" - load a new report by disabling the publication of the conflicting report."
"3. """" [Open] """" - open the card of the conflicting report.'");
			ПредставлениеКнопкиОтключить = НСтр("es='Desconectar otro informe';ru='Отключить другой отчет';en='Disable another report'");
		Иначе
			ЗаголовокВопроса = НСтр("es='El conflicto a la carga del tratamiento exterior';ru='Конфликт при загрузке внешней обработки';en='Conflict while loading external processing'");
			ТекстВопроса = НСтр("es='El nombre interior del tratamiento ""[Name]"" "
"es ocupado ya por el tratamiento existente adicional [List]. "
""
"Escojan: "
"1. ""[Continue]"" - cargar un nuevo tratamiento en el régimen del ajuste. "
"2. ""[Disable]"" - cargar un nuevo tratamiento, habiendo desconectado la publicación del tratamiento que choca. "
"3. ""[Open]"" - abrir la tarjeta del tratamiento que choca.';ru='Внутреннее имя обработки ""[Name]"""
...
"3. ""[Open]"" - открыть карточку конфликтующей обработки.';en='Internal name of processing"" ""[Name]"" """
"already taken by existing additional processing [List]."
""
"Choose:"
"1. """" [Continue] """" - load the new processing in debug mode."
"2. """" [Disable] """" - load a new processing by disabling the publication of conflicting processing."
"3. """" [Open] """" - open the card of conflicting processing.'");
			ПредставлениеКнопкиОтключить = НСтр("es='Desconectar otro tratamiento';ru='Отключить другую обработку';en='Disable other processing'");
		КонецЕсли;
		ПредставлениеКнопкиОткрыть = НСтр("es='Anular y abrir';ru='Отменить и открыть';en='Cancel and open'");
	КонецЕсли;
	ПредставлениеКнопкиПродолжить = НСтр("es='En el régimen del ajuste';ru='В режиме отладки';en='In debug mode'");
...
Процедура ОбновитьИзФайлаРазрешениеКонфликтов(Ответ, ПараметрыРегистрации) Экспорт
	...
	ИначеЕсли Ответ = "ОтменитьИОткрыть" Тогда
		...
		Если ПоказатьСписок Тогда // Форма списка с отбором по конфликтующим.
			...
			ФормаЗаголовок = НСтр("es='Los informes adicionales y los tratamientos con interior por el nombre ""%1""';ru='Дополнительные отчеты и обработки с внутреннем именем ""%1""';en='Additional reports and processes with internal name ""%1""'");
...
Процедура ОткрытьВариант()
	...
	Если НЕ ЗначениеЗаполнено(Вариант.Ссылка) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La variante del informe ""%1"" no es registrada.';ru='Вариант отчета ""%1"" не зарегистрирован.';en=""The report variant '% 1' is not registered."""), Вариант.Наименование);
...
Процедура ИзменитьРегламентноеЗадание(РежимВыбора = Ложь, ИзмененФлажок = Ложь)
	...
	Если ЭлементКоманда.ВариантЗапуска <> ПредопределенноеЗначение("Перечисление.СпособыВызоваДополнительныхОбработок.ВызовСерверногоМетода")
		...
		ТекстОшибки = НСтр("es='La orden con la variante del lanzamiento ""%1"" "
"no puede usarse en las tareas de reglamento.';ru='Команда с вариантом запуска ""%1"""
"не может использоваться в регламентных заданиях.';en='Command with run option ""%1"" "
...
Процедура ПослеОкончанияРедактированияРасписания(Расписание, Контекст) Экспорт
	...
	Иначе
		...
		Иначе
			ЭлементКоманда.РегламентноеЗаданиеПредставление = НСтр("es='No es rellenado';ru='Не заполнено';en='Not completed'");
			Если ЭлементКоманда.РегламентноеЗаданиеИспользование Тогда
				ЭлементКоманда.РегламентноеЗаданиеИспользование = Ложь;
				ПоказатьОповещениеПользователя(
					НСтр("es='El lanzamiento por el horario es desconectado';ru='Запуск по расписанию отключен';en='Scheduled launch disabled'"),
					,
					НСтр("es='El horario no es rellenado';ru='Расписание не заполнено';en='Schedule is not filled'"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПослеЗавершенияВыполненияСервернойКомандыВФоне(Задание, ВыполняемаяКоманда) Экспорт
	Если Задание.Статус = "Ошибка" Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió cumplir la orden por la causa: "
"%1.';ru='Не удалось выполнить команду по причине:"
"%1.';en='The command could not be completed because:"
...
Функция ПредставлениеБыстрогоДоступаПользователей(КоличествоПользователей)
	...
	Если КоличествоПользователей = 0 Тогда
		Возврат НСтр("es='No';ru='Нет';en='No'");
	КонецЕсли;
	
	БыстрыйДоступПредставление = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
		НСтр("es='; %1 usuario;; %1 usuarios; %1 usuarios; %1 usuarios';ru=';%1 пользователь;;%1 пользователя;%1 пользователей;%1 пользователя';en=';%1 user;;%1 user;%1 usesr;%1 user'"), КоличествоПользователей);
...
Функция НачатьВыполнениеСервернойКомандыВФоне(ВыполняемаяКоманда, УникальныйИдентификатор)
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("es='Los informes adicionales y los tratamientos: la Ejecución del método del servidor del tratamiento';ru='Дополнительные отчеты и обработки: Выполнение серверного метода обработки';en='Additional reports and processing: Executing the server processing method'");
...
Функция ПодготовитьПараметрыФормыВыборОбъектовМетаданных()
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("es='El destino del tratamiento adicional';ru='Назначение дополнительной обработки';en='Purpose of additional processing'"));
...
Процедура УстановитьВидимостьДоступность(Регистрация = Ложь)
	...
	Если Объект.Вид = ВидДополнительныйОтчет И Объект.ИспользуетХранилищеВариантов Тогда
		...
		Если Регистрация ИЛИ КоличествоВариантов = 0 Тогда
			...
			Элементы.СтраницаВарианты.Заголовок = НСтр("es='Las variantes del informe';ru='Варианты отчета';en='Report options'");
		Иначе
			...
			Элементы.СтраницаВарианты.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Las variantes del informe (%1)';ru='Варианты отчета (%1)';en='Report options (% 1)'"),
	...
	Иначе
		...
		Элементы.СтраницаРазрешения.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Los permisos (%1)';ru='Разрешения (%1)';en='Permissions (%1)'"),
		...
		ИначеЕсли ТипЗнч(БезопасныйРежим) = Тип("Строка") Тогда
			...
			Элементы.ДекорацияПерсональныйПрофильБезопасностиНадпись.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El informe adicional o el tratamiento se conectará al programa con el uso "
"del perfil ""personal"" de la seguridad %1, en que serán permitidas solamente las operaciones siguientes:';ru='Дополнительный отчет или обработка будет подключаться к программе с использованием ""персонального"""
"профиля безопасности %1, в котором будут разрешены только следующие операции:';en='An additional report or processing will be connected to the program using"" ""personal"" """
"security profile%1, in which only the following operations will be allowed:'"),
				БезопасныйРежим);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='%1 no es el régimen correcto de la conexión para los informes adicionales y los tratamientos "
"exigentes los permisos para el uso los perfiles la seguridad.';ru='%1 не является корректным режимом подключения для дополнительных отчетов и обработок,"
"требующих разрешений на использование профилей безопасности.';en='% 1 is not a valid connection mode for additional reports and processing,"
"requiring permissions to use security profiles.'"),
				БезопасныйРежим);
		КонецЕсли;
	...
	Если ПредставлениеНазначения = "" Тогда
		ПредставлениеНазначения = НСтр("es='No es determinado';ru='Не определено';en='Undefined'");
	КонецЕсли;
	...
	Если ЭтоНовый Тогда
		Заголовок = ?(ЭтоОтчет, НСтр("es='El informe (creación) adicional';ru='Дополнительный отчет (создание)';en='Additional report (creation)'"), НСтр("es='El tratamiento (creación) adicional';ru='Дополнительная обработка (создание)';en='Additional processing (creation)'"));
	Иначе
		Заголовок = Объект.Наименование + " " + ?(ЭтоОтчет, НСтр("es='(El informe adicional)';ru='(Дополнительный отчет)';en='(Supplementary report)'"), НСтр("es='(El tratamiento adicional)';ru='(Дополнительная обработка)';en='(Additional processing)'"));
	КонецЕсли;
	...
	Для Каждого ЭлементКоманда Из Объект.Команды Цикл
			...
			Если Не ПустаяСтрока(ЭлементКоманда.ЗаменяемыеКоманды) И КомандыПечатиОбъекта <> Неопределено Тогда
				...
				Если КоличествоЗаменяемых > 0 Тогда
					Если КоличествоЗаменяемых = 1 Тогда
						ШаблонКомментария = НСтр("es='Sustituye la orden estandartizada de la prensa %1';ru='Заменяет стандартную команду печати %1';en='Replaces the standard print command%1'");
					Иначе
						ШаблонКомментария = НСтр("es='Sustituye las órdenes estandartizadas de la prensa: %1';ru='Заменяет стандартные команды печати: %1';en='Replaces standard print commands:%1'");
					КонецЕсли;
		...
		Иначе
			ЭлементКоманда.РегламентноеЗаданиеПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es inaplicable para las órdenes con la variante del lanzamiento ""%1""';ru='Неприменимо для команд с вариантом запуска ""%1""';en='Not applicable for commands with run option ""%1""'"),
...
Процедура ВариантыДополнительногоОтчетаЗаполнить()
	...
	Исключение
		ТекстОшибки = НСтр("es='No consiguió recibir la lista de las variantes del informe por la falta que ha surgido a la conexión de este informe:';ru='Не удалось получить список вариантов отчета из-за ошибки, возникшей при подключении этого отчета:';en='Failed to get a list of report options due to an error that occurred while connecting this report:'");
...
Функция НазваниеСтраницыКоманд()
	Если Объект.Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма Тогда
		Возврат НСтр("es='Las órdenes de la prensa';ru='Команды печати';en='Print commands'");
	Иначе
		Возврат НСтр("es='Las órdenes';ru='Команды';en='Commands'");
	КонецЕсли;
КонецФункции
