...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВсеПубликацииКромеНеиспользующихся.Количество() > 1 Тогда
		ПредставлениеМассива = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='%1 o %2';ru='%1 или %2';en='%1 or 2'"),
	...
	СписокВыбора.Добавить(1, НСтр("es='Solamente los informes';ru='Только отчеты';en='Reports only'"));
	СписокВыбора.Добавить(2, НСтр("es='Solamente los tratamientos';ru='Только обработки';en='Processing only'"));
...
Функция ВыбранЭлемент(ДанныеСтроки)
	Если ТипЗнч(ДанныеСтроки.Ссылка) <> Тип("СправочникСсылка.ДополнительныеОтчетыИОбработки") Тогда
		ПоказатьПредупреждение(, НСтр("es='La orden no puede ser cumplida para el objeto indicado. "
"Escojan el informe adicional o el tratamiento.';ru='Команда не может быть выполнена для указанного объекта."
"Выберите дополнительный отчет или обработку.';en='The command could not be executed for the specified object."
"Select an optional report or processing. '"));
		Возврат Ложь;
	КонецЕсли;
	Если ДанныеСтроки.ЭтоГруппа Тогда
		ПоказатьПредупреждение(, НСтр("es='La orden no puede ser cumplida para el grupo. "
"Escojan el informe adicional o el tratamiento.';ru='Команда не может быть выполнена для группы."
"Выберите дополнительный отчет или обработку.';en='The command could not be executed for the group."
"Select an optional report or processing. '"));
		Возврат Ложь;
	КонецЕсли;
	Возврат Истина;
...
Процедура ИзменитьПубликацию(ВариантПубликации)
	...
	Если КоличествоСтрок = 0 Тогда
		ПоказатьПредупреждение(, НСтр("es='No es escogido ni un informe (tratamiento) adicional';ru='Не выбран ни один дополнительный отчет (обработка)';en='No additional reports selected (processing)'"));
		Возврат;
	КонецЕсли;
	...
	Если КоличествоСтрок = 1 Тогда
		ТекстСообщения = НСтр("es='Es cambiada la publicación del informe (tratamiento) adicional ""%1""';ru='Изменена публикация дополнительного отчета (обработки) ""%1""';en='The publication of the additional report (processing) ""%1"" was changed'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Строка(ВыделенныеСтроки[0]));
	Иначе
		ТекстСообщения = НСтр("es='Es cambiada la publicación cerca de los informes (tratamientos) adicionales: %1';ru='Изменена публикация у дополнительных отчетов (обработок): %1';en='The publication of additional reports (processings) was changed:%1'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, КоличествоСтрок);
	КонецЕсли;
	
	ПоказатьОповещениеПользователя(НСтр("es='Es cambiada la publicación';ru='Изменена публикация';en='Publication changed'"),, ТекстСообщения);
