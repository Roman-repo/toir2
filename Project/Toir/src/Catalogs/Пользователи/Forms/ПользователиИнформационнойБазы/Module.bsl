...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Пользователи.ЭтоПолноправныйПользователь(, Не РазделениеВключено) Тогда
		ВызватьИсключение НСтр("es='Es insuficientemente derecho para la apertura de la lista de los usuarios de la base informativa.';ru='Недостаточно прав для открытия списка пользователей информационной базы.';en='Not enough rights to open the list of users of the infobase.'");
	КонецЕсли;
...
Процедура ОтменитьСопоставление(Команда)
	...
	Кнопки.Добавить("ОтменитьСопоставление", НСтр("es='Anular la comparación';ru='Отменить сопоставление';en='Cancel match'"));
	Кнопки.Добавить("ОставитьСопоставление", НСтр("es='Dejar la comparación';ru='Оставить сопоставление';en='Leave a match'"));
	
	ПоказатьВопрос(
		Новый ОписаниеОповещения("ОтменитьСопоставлениеПродолжение", ЭтотОбъект),
		НСтр("es='La anulación de la comparación del usuario de la base informativa con el usuario en la guía. "
""
"La anulación de la comparación es necesaria extremadamente raramente - solamente si la comparación era cumplida es incorrecto, por ejemplo, "
"a la renovación de la base informativa, por eso no es recomendable anular la comparación por cualquier otra causa.';ru='Отмена сопоставления пользователя информационной базы с пользователем в справочнике."
""
"Отмена сопоставления требуется крайне редко - только если сопоставление было выполнено некорректно, например,"
"при обновлении информационной базы, поэтому не рекомендуется отменять сопоставление по любой другой причине.';en='Canceling the mapping of the user of the infobase to the user in the directory."
""
"Cancellation of a match is extremely rare - only if the match was not performed correctly, for example,"
"when updating the infobase, therefore, it is not recommended to cancel the mapping for any other reason.'"),
		Кнопки,
		,
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='<no Hay dado>';ru='<Нет данных>';en='<Нет данных>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("Формат", НСтр("es='БЛ=Нет; БИ=Да';ru='БЛ=Нет; БИ=Да';en='БЛ = No; БИ = Yes'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Формат", НСтр("es='БЛ =; БИ=Да';ru='БЛ=; БИ=Да';en='БЛ=; БИ=Yes'"));
...
Процедура УдалитьТекущегоПользователяИБ(УдалитьСтроку = Ложь)
	...
	ПоказатьВопрос(
		...
		НСтр("es='¿Quitar al usuario de la base informativa?';ru='Удалить пользователя информационной базы?';en='Delete user infobase?'"),
