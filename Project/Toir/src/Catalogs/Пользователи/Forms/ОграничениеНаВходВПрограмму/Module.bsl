	...
	Если ВидСрока = "ДоДаты" Тогда
		Если Не ЗначениеЗаполнено(Срок) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='La fecha no es indicada.';ru='Дата не указана.';en='No date specified.'"),, "Срок");
			Возврат;
			
		ИначеЕсли Срок <= НачалоДня(ОбщегоНазначенияКлиент.ДатаСеанса()) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='La restricción debe ser hasta mañana o más.';ru='Ограничение должно быть до завтра или более.';en='The limit must be before tomorrow or more.'"),, "Срок");
			Возврат;
		КонецЕсли;
	КонецЕсли;
