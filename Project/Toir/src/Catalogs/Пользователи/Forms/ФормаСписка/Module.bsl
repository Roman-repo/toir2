...
Процедура ИзменитьПараметрыРасширеннойФормыПодбора()
	...
	ИначеЕсли ПолучитьФункциональнуюОпцию("ИспользоватьГруппыПользователей") Тогда
		...
		Элементы.ПользователиСписок.Заголовок                    = НСтр("es='Los usuarios en el grupo';ru='Пользователи в группе';en='Users in the group'");
...
Процедура ОбновитьЗаголовокСпискаВыбранныхПользователейИГрупп()
	...
	Иначе
		ЗаголовокВыбранныеПользователиИГруппы = НСтр("es='Los usuarios escogidos (%1)';ru='Выбранные пользователи (%1)';en='Selected Users (% 1)'");
	КонецЕсли;
	...
	Иначе
		...
		Иначе
			Элементы.СписокВыбранныхПользователейИГрупп.Заголовок = НСтр("es='Los usuarios escogidos';ru='Выбранные пользователи';en='Selected Users'");
		КонецЕсли;
	КонецЕсли;
	...
	Если Параметры.РежимВыбора Тогда
		...
		Если Параметры.ЗакрыватьПриВыборе = Ложь Тогда
			...
			Если ВыборГруппПользователей Тогда
				...
				Иначе
					Заголовок = НСтр("es='La selección de los usuarios y los grupos';ru='Подбор пользователей и групп';en='Selection of users and groups'");
				КонецЕсли;
				
				ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
					"ВыбратьПользователя", "Заголовок", НСтр("es='Escoger a los usuarios';ru='Выбрать пользователей';en='Select Users'"));
				
				ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
					"ВыбратьГруппуПользователей", "Заголовок", НСтр("es='Escoger los grupos';ru='Выбрать группы';en='Choose groups'"));
			Иначе
				
				Если ХранимыеПараметры.РасширенныйПодбор Тогда
					Заголовок = ХранимыеПараметры.ЗаголовокФормыПодбора;
				Иначе
					Заголовок = НСтр("es='La selección de los usuarios';ru='Подбор пользователей';en='User selection'");
				КонецЕсли;
		...
		Иначе
			// Режим выбора.
			Если ВыборГруппПользователей Тогда
				Заголовок = НСтр("es='La elección del usuario o el grupo';ru='Выбор пользователя или группы';en='Select user or group'");
				
				ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
					"ВыбратьПользователя", "Заголовок", НСтр("es='Escoger al usuario';ru='Выбрать пользователя';en='Select user'"));
			Иначе
				Заголовок = НСтр("es='La elección del usuario';ru='Выбор пользователя';en='User Choice'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
