	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Если НЕ ДоступноИзменениеПользователей Тогда
			Если Объект.Ссылка.Пустая() Тогда
				ВызватьИсключение
					НСтр("es='En el régimen de exposición no es apoyada "
"la creación de los nuevos usuarios.';ru='В демонстрационном режиме не поддерживается"
"создание новых пользователей.';en='Not available in demo mode"
"creating new users.'");
			КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	ОчиститьСообщения();
	ЗаголовокВопросов = НСтр("es='La anotación del usuario de la base informativa';ru='Запись пользователя информационной базы';en='Information base user writing'");
	   ...
	   И (Не ПараметрыЗаписи.Свойство("НеКопироватьПраваПользователя")
		...
		ПоказатьВопрос(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='¿Copiar también los derechos del usuario ""%1""?';ru='Скопировать также права пользователя ""%1""?';en='Copy also user rights ""%1""?'"), Строка(ЗначениеКопирования)),
	...
	Если Не ПараметрыЗаписи.Свойство("СПустымСпискомРолей")
	   ...
	   И ПользовательИБРоли.Количество() = 0 Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("es='Al usuario de la base informativa no le es establecido ni un papel. ¿Continuar?';ru='Пользователю информационной базы не установлено ни одной роли. Продолжить?';en='Infobase user has not been assigned a single role. Continue?'"),
			РежимДиалогаВопрос.ДаНет,
			,
			,
			ЗаголовокВопросов);
	...
	Если ТребуетсяЗаписьПользователяИБ(ЭтотОбъект) Тогда
		...
		Если ПараметрыЗаписи.Свойство("ССозданиемПервогоАдминистратора") Тогда
			ТекущийОбъект.ДополнительныеСвойства.Вставить("СозданиеАдминистратора",
				НСтр("es='El primer usuario de la base informativa es fijado el administrador.';ru='Первый пользователь информационной базы назначается администратором.';en='The first user of the infobase is appointed by the administrator.'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если ВходВПрограммуРазрешен
	   ...
	   И СрокДействия <= НачалоДня(ТекущаяДатаСеанса()) Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("es='La restricción debe ser hasta mañana o más.';ru='Ограничение должно быть до завтра или более.';en='The limit must be before tomorrow or more.'"),, "ВходВПрограммуРазрешен",, Отказ);
	КонецЕсли;
		...
		Для Каждого Строка Из ЭлементыДерева Цикл
			...
			Если Строка.ЭтоНесуществующаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El papel ""%1"" no es encontrado en los metadatos.';ru='Роль ""%1"" не найдена в метаданных.';en=""The role '%1' not found in metadata."""), Строка.Синоним),
					"Роли",
					ЭлементыДерева.Индекс(Строка),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El papel ""%1"" en la línea %%1 no es encontrado en los metadatos.';ru='Роль ""%1"" в строке %%1 не найдена в метаданных.';en=""The role '%1' in line %%1 not found in metadata."""), Строка.Синоним));
			КонецЕсли;
			Если Строка.ЭтоНедоступнаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El papel ""%1"" es inaccesible para los usuarios.';ru='Роль ""%1"" недоступна для пользователей.';en='The role ""%1"" is not accessible to users.'"), Строка.Синоним),
					"Роли",
					ЭлементыДерева.Индекс(Строка),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El papel ""%1"" en la línea %%1 es inaccesible para los usuarios.';ru='Роль ""%1"" в строке %%1 недоступна для пользователей.';en='The role ""%1"" in line %%1 is not accessible to users.'"), Строка.Синоним));
			КонецЕсли;
		КонецЦикла;
...
Процедура ВходВПрограммуРазрешенПриИзменении(Элемент)
	...
	Если Объект.ПометкаУдаления И ВходВПрограммуРазрешен Тогда
		...
		ПоказатьПредупреждение(,
			НСтр("es='Para permitir la entrada en el programa, es necesario quitar "
"la marca a la desaparición de este usuario.';ru='Чтобы разрешить вход в программу, требуется снять"
"пометку на удаление с этого пользователя.';en='To allow entry into the program, you must remove"
"mark to remove from this user.'"));
		Возврат;
	КонецЕсли;
	...
	Если Не УровеньДоступа.ИзменениеРазрешенияНаВход
	   И Не ВходВПрограммуРазрешен Тогда
		...
		ПоказатьПредупреждение(,
			НСтр("es='Después de la anotación la entrada en el programa puede ser permitida solamente por el administrador.';ru='После записи вход в программу сможет разрешить только администратор.';en='After recording, only the administrator will be allowed to enter the program.'"));
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ПравоеЗначение = НСтр("es='ПолныеПрава';ru='ПолныеПрава';en='ПолныеПрава'");
...
Процедура ПроверитьПервогоАдминистратора() 
	...
	ЗаголовокВопроса = НСтр("es='La anotación del usuario de la base informativa';ru='Запись пользователя информационной базы';en='Information base user writing'");
	ПоказатьВопрос(
...
Процедура ПроверитьЗаполнениеEmail(Отказ)
	...
	ОбщегоНазначения.СообщитьПользователю(
		НСтр("es='del correo electrónico';ru='Не заполнен адрес электронной почты';en='Email address is not filled'"),,
...
Процедура ОпределитьНесоответствияПользователяСПользователемИБ(ПараметрыЗаписи = Неопределено)
	...
	ИначеЕсли УровеньДоступа.УправлениеСписком Тогда
		...
		Если ПользовательИБПолноеИмя <> Объект.Наименование Тогда
			...
			УточнениеСвойств.Вставить(0, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El nombre completo ""%1""';ru='Полное имя ""%1""';en='Full name ""%1""'"),
		...
		Если УточнениеСвойств.Количество() > 0 Тогда
			...
			Если ПоказатьКомандыУстраненияРазличий Тогда
				Рекомендация = Символы.ПС
					+ НСтр("es='Presionen ""Anotar"" para eliminar las distinciones y no sacar esta prevención.';ru='Нажмите ""Записать"", чтобы устранить различия и не выводить это предупреждение.';en='Click ""Record"" to resolve the differences and not display this warning.'");
			
			ИначеЕсли Не Пользователи.ЭтоПолноправныйПользователь() Тогда
				Рекомендация = Символы.ПС
					+ НСтр("es='Se dirijan al administrador para eliminar las distinciones.';ru='Обратитесь к администратору, чтобы устранить различия.';en='Contactadministrator to resolve the differences.'");
			Иначе
				Рекомендация = "";
			КонецЕсли;
			Элементы.СвойстваПояснениеНесоответствия.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Las propiedades siguientes del usuario de la base informativa se distinguen de indicado en esta forma: "
"%1.';ru='Следующие свойства пользователя информационной базы отличаются от указанных в этой форме:"
"%1.';en='The following properties of the user of the infobase differ from those specified in this form:"
"%1.'"),
	...
	Если ДействияВФорме.СвойстваЭлемента = "Редактирование" Тогда
		Рекомендация = Символы.ПС
			+ НСтр("es='Presionen ""Anotar"" para eliminar el problema y no sacar esta prevención.';ru='Нажмите ""Записать"", чтобы устранить проблему и не выводить это предупреждение.';en='Click ""Record"" to fix the problem and not display this warning.'");
	...
	Элементы.СопоставлениеПояснениеНесоответствия.Заголовок =
		НСтр("es='El usuario de la base informativa no es encontrado.';ru='Пользователь информационной базы не найден.';en='Information base user not found.'") + Рекомендация;
