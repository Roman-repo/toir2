...
Процедура Выбрать(Команда)
	...
	Если Элементы.ТаблицаДоменов.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("es='Escojan el dominio.';ru='Выберите домен.';en='Select a domain.'"));
		Возврат;
	КонецЕсли;
	
	Если Элементы.ТаблицаПользователейДомена.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("es='Escojan al usuario del dominio.';ru='Выберите пользователя домена.';en='Select a domain user.'"));
		Возврат;
	КонецЕсли;
