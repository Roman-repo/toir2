...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("es='La renovación de las anotaciones personales de descuento';ru='Обновление персональных учетных записей';en='Updating personal accounts'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ПроверитьПодключениеКСерверуВходящейПочты(Профиль, Протокол)
	...
	Если Протокол = ПротоколИнтернетПочты.POP3 Тогда
		ТекстДляЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1:%2%3 (%4)" + Символы.ПС + "%5",
			...
			?(ПустаяСтрока(ТекстОшибки), НСтр("es='OK';ru='OK';en='OK'"), ТекстОшибки));
	Иначе
		ТекстДляЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1:%2%3 (%4)" + Символы.ПС + "%5",
	...
	ЗаписьЖурналаРегистрации(НСтр("es='La comprobación de la conexión al servidor postal';ru='Проверка подключения к почтовому серверу';en='Checking the connection to the mail server'", ОбщегоНазначения.КодОсновногоЯзыка()), 
...
Функция ПроверитьПодключениеКСерверуИсходящейПочты(Профиль, АдресЭлектроннойПочты)
	...
	Тело = НСтр("es='Este mensaje es enviado por el subsistema del correo electrónico 1С:Предприятие';ru='Это сообщение отправлено подсистемой электронной почты 1С:Предприятие';en='This message was sent by the 1C: Enterprise email subsystem.'");
	ИмяОтправителяПисем = НСтр("es='1С:Предприятие';ru='1С:Предприятие';en='1C: Enterprise'");
