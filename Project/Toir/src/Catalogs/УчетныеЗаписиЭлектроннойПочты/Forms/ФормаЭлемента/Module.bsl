...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ДоступноПолучениеПисем Тогда
		Элементы.ДляОтправки.Заголовок = НСтр("es='Usar para el envío de las cartas';ru='Использовать для отправки писем';en='Use to send letters'");
	КонецЕсли;
	
	Элементы.СерверВходящейПочты.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El servidor %1';ru='Сервер %1';en='Server%1'"), Объект.ПротоколВходящейПочты);
	...
	// ТОИР++
	Элементы.торо_МаксимальноеКоличествоОтправляемых.Видимость = (Объект.Ссылка = Справочники.УчетныеЗаписиЭлектроннойПочты.СистемнаяУчетнаяЗаписьЭлектроннойПочты);
	// ТОИР—
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	Если ВидУчетнойЗаписи = "Персональная" И Не ЗначениеЗаполнено(Объект.ВладелецУчетнойЗаписи) Тогда 
		Отказ = Истина;
		ТекстСообщения = НСтр("es='de la anotación de descuento.';ru='Не выбран владелец учетной записи.';en='No account owner selected.'");
...
Процедура УстановитьВидГруппыТребуетсяАвторизация(Форма, ИспользуетсяПротоколPOP)
	...
	Если ИспользуетсяПротоколPOP Тогда
		Форма.Элементы.ТребуетсяАвторизацияПриОтправкеПисем.Заголовок = НСтр("es='Al envío de las cartas es necesaria la autorización';ru='При отправке писем требуется авторизация';en='When sending letters authorization is required'");
	Иначе
		Форма.Элементы.ТребуетсяАвторизацияПриОтправкеПисем.Заголовок = НСтр("es='Al envío de las cartas es necesaria la autorización en el servidor del correo saliente (SMTP)';ru='При отправке писем требуется авторизация на сервере исходящей почты (SMTP)';en='When sending letters, authorization on the outgoing mail server (SMTP) is required'");
	КонецЕсли;
