...
Процедура УстановитьЗаголовок()
	...
	Если НЕ ПустаяСтрока(ИмяСвойства) Тогда
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 (%2)';ru='%1 (%2)';en='%1(%2)'"),
				Объект.Наименование,
				ИмяСвойства);
		Иначе
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 (Creación)';ru='%1 (Создание)';en='%1 (Create)'"), ИмяСвойства);
		КонецЕсли;
	Иначе
		ИмяСвойства = Строка(ЗначенияРеквизитов.Заголовок);
		
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 (el Grupo de los significados de la propiedad %2)';ru='%1 (Группа значений свойства %2)';en='%1 (Property Value Group %2)'"),
				Объект.Наименование,
				ИмяСвойства);
		Иначе
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El grupo de los significados de la propiedad %1 (Creación)';ru='Группа значений свойства %1 (Создание)';en='Property value group %1 (Creation)'"), ИмяСвойства);
		КонецЕсли;
	КонецЕсли;
