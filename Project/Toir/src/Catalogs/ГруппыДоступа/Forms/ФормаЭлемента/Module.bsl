	...
	Если ОбщегоНазначения.РазделениеВключено()
	   ...
	   И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса.ПользователиВМоделиСервиса") Тогда
		...
		Если НЕ ДействияСПользователемСервиса.ИзменениеАдминистративногоДоступа Тогда
			ВызватьИсключение
				НСтр("es='No basta los derechos de admisión para el cambio de la composición de los administradores.';ru='Не достаточно прав доступа для изменения состава администраторов.';en='Not enough access rights to change the composition of administrators.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	ИначеЕсли Не ТолькоПросмотр
		...
		ПоказатьВопрос(
			...
			НСтр("es='Cerca del grupo del acceso los Administradores ser el perfil. "
""
"Establecer Administrador el perfil en el grupo del acceso (¿no existe - abrir solamente para la presentación)?';ru='У группы доступа Администраторы должен быть профиль Администратор."
""
"Установить профиль в группе доступа (нет - открыть только для просмотра)?';en='The Administrators access group must have an Administrator profile."
""
"Set profile in access group (no - open for viewing only)?'"),
	...
	Иначе
		...
		ИначеЕсли НЕ ТолькоПросмотр И ИзменилсяСоставВидовДоступаПриЧтении Тогда
			...
			ПоказатьВопрос(
				...
				НСтр("es='Se ha cambiado la composición de los tipos del acceso del perfil de este grupo del acceso. "
""
"Renovar los tipos del acceso en el grupo del acceso (¿no existe - abrir solamente para la presentación)?';ru='Изменился состав видов доступа профиля этой группы доступа."
""
"Обновить виды доступа в группе доступа (нет - открыть только для просмотра)?';en='The composition of the access types of the profile of this access group has changed."
""
"Update access types in an access group (no - open for viewing only)? '"),
				РежимДиалогаВопрос.ДаНет,
				,
				КодВозвратаДиалога.Нет);
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("ПредупредитьЧтоПрофильПомеченНаУдаление") Тогда
		...
		ПоказатьПредупреждение(
			...
			НСтр("es='El grupo del acceso no influye sobre los derechos de los participantes "
"puesto que su perfil es marcado a la desaparición.';ru='Группа доступа не влияет на права участников"
"так как ее профиль помечен на удаление.';en='Access group does not affect member rights"
"because her profile is marked for deletion. '"));
	Иначе
	...
	Для Каждого ТекущаяСтрока Из ДеревоПользователейСтроки Цикл
		...
		Если НЕ ЗначениеЗаполнено(Участник) Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				...
				УточнитьСообщение(НСтр("es='El usuario no es escogido.';ru='Пользователь не выбран.';en='No user selected.'"), Участник),
				"ПользователиГруппы",
				НомерСтроки,
				УточнитьСообщение(НСтр("es='El usuario en la línea %1 no es escogido.';ru='Пользователь в строке %1 не выбран.';en='User on line %1 not selected.'"), Участник));
			Продолжить;
		КонецЕсли;
		...
		Если НайденныеЗначения.Количество() > 1 Тогда
			...
			Если ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("es='El usuario ""%2"" repete.';ru='Пользователь ""%2"" повторяется.';en='User ""%2"" is repeating.'");
				ТекстДляНесколькихОшибок = НСтр("es='El usuario ""%2"" en la línea %1 repete.';ru='Пользователь ""%2"" в строке %1 повторяется.';en='User ""%2"" on line%1 is repeated.'");
				
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("es='El usuario exterior ""%2"" repete.';ru='Внешний пользователь ""%2"" повторяется.';en='External user ""% 2"" repeats.'");
				ТекстДляНесколькихОшибок = НСтр("es='El usuario ""%2"" repete.';ru='Пользователь ""%2"" повторяется.';en='User ""%2"" is repeating.'");
				
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ТекстДляОднойОшибки      = НСтр("es='El grupo de los usuarios ""%2"" repete.';ru='Группа пользователей ""%2"" повторяется.';en=""User group '% 2' repeats.""");
				ТекстДляНесколькихОшибок = НСтр("es='El usuario exterior ""%2"" repete.';ru='Внешний пользователь ""%2"" повторяется.';en='External user ""%2"" repeats.'");
			Иначе
				ТекстДляОднойОшибки      = НСтр("es='El grupo de los usuarios exteriores ""%2"" repete.';ru='Группа внешних пользователей ""%2"" повторяется.';en=""The external user group '% 2' is repeated.""");
				ТекстДляНесколькихОшибок = НСтр("es='El grupo de los usuarios exteriores ""%2"" en la línea %1 repete.';ru='Группа внешних пользователей ""%2"" в строке %1 повторяется.';en='The external user group ""% 2"" in line% 1 is repeated.'");
			КонецЕсли;
		...
		Если Объект.Ссылка = Справочники.ГруппыДоступа.Администраторы
		   И ТипЗнч(ТекущаяСтрока.Пользователь) <> Тип("СправочникСсылка.Пользователи") Тогда
			...
			Если ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("es='El usuario exterior ""%2"" es inadmisible en el grupo predestinado del acceso los Administradores.';ru='Внешний пользователь ""%2"" недопустим в предопределенной группе доступа Администраторы.';en='External user ""% 2"" is not allowed in the Administrators predefined access group.'");
				ТекстДляНесколькихОшибок = НСтр("es='El usuario exterior ""%2"" en la línea %1 es inadmisible en el grupo predestinado del acceso los Administradores.';ru='Внешний пользователь ""%2"" в строке %1 недопустим в предопределенной группе доступа Администраторы.';en='External user ""% 2"" on line% 1 is not valid in the Administrators predefined access group.'");
				
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ТекстДляОднойОшибки      = НСтр("es='El grupo de los usuarios ""%2"" es inadmisible en el grupo predestinado del acceso los Administradores.';ru='Группа пользователей ""%2"" недопустима в предопределенной группе доступа Администраторы.';en=""User group '% 2' is not valid in the Administrators predefined access group.""");
				ТекстДляНесколькихОшибок = НСтр("es='El usuario exterior ""%2"" es inadmisible en el grupo predestinado del acceso los Administradores.';ru='Внешний пользователь ""%2"" недопустим в предопределенной группе доступа Администраторы.';en='External user ""%2"" is not allowed in the Administrators predefined access group.'");
			Иначе
				ТекстДляОднойОшибки      = НСтр("es='El grupo de los usuarios exteriores ""%2"" es inadmisible en el grupo predestinado del acceso los Administradores.';ru='Группа внешних пользователей ""%2"" недопустима в предопределенной группе доступа Администраторы.';en=""The external user group '% 2' is not valid in the Administrators predefined access group.""");
				ТекстДляНесколькихОшибок = НСтр("es='El grupo de los usuarios exteriores ""%2"" en la línea %1 es inadmisible en el grupo predestinado del acceso los Administradores.';ru='Группа внешних пользователей ""%2"" в строке %1 недопустима в предопределенной группе доступа Администраторы.';en=""The external user group '% 2' in line% 1 is not valid in the Administrators predefined access group.""");
			КонецЕсли;
		...
		Если НедопустимыеПользователи.Найти(ТекущаяСтрока.Пользователь) <> Неопределено Тогда
			...
			Если ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("es='El usuario ""%2"" es inadmisible al tipo indicado de los participantes.';ru='Пользователь ""%2"" недопустим для указанного типа участников.';en='User ""%2"" is not valid for the specified member type.'");
				ТекстДляНесколькихОшибок = НСтр("es='El usuario ""%2"" en la línea %1 es inadmisible al tipo indicado de los participantes.';ru='Пользователь ""%2"" в строке %1 недопустим для указанного типа участников.';en='User ""%2"" on line%1 is not valid for the specified type of participants.'");
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ТекстДляОднойОшибки      = НСтр("es='El grupo de los usuarios ""%2"" es inadmisible al tipo indicado de los participantes.';ru='Группа пользователей ""%2"" недопустима для указанного типа участников.';en=""User group '% 2' is not valid for the specified member type.""");
				ТекстДляНесколькихОшибок = НСтр("es='El grupo de los usuarios ""%2"" en la línea %1 es inadmisible al tipo indicado de los participantes.';ru='Группа пользователей ""%2"" в строке %1 недопустима для указанного типа участников.';en=""User group '% 2' in line% 1 is not valid for the specified member type.""");
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("es='El usuario ""%2"" en la línea %1 es inadmisible al tipo indicado de los participantes.';ru='Пользователь ""%2"" в строке %1 недопустим для указанного типа участников.';en='User ""%2"" on line%1 is not valid for the specified type of participants.'");
				ТекстДляНесколькихОшибок = НСтр("es='El usuario exterior ""%2"" en la línea %1 es inadmisible al tipo indicado de los participantes.';ru='Внешний пользователь ""%2"" в строке %1 недопустим для указанного типа участников.';en='The external user ""% 2"" in line% 1 is not valid for the specified type of participants.'");
			Иначе // Группа внешних пользователей.
				ТекстДляОднойОшибки      = НСтр("es='El grupo de los usuarios ""%2"" en la línea %1 es inadmisible al tipo indicado de los participantes.';ru='Группа пользователей ""%2"" в строке %1 недопустима для указанного типа участников.';en=""User group '%2' in line%1 is not valid for the specified member type.""");
				ТекстДляНесколькихОшибок = НСтр("es='El grupo de los usuarios exteriores ""%2"" en la línea %1 es inadmisible al tipo indicado de los participantes.';ru='Группа внешних пользователей ""%2"" в строке %1 недопустима для указанного типа участников.';en=""The external user group '% 2' in line% 1 is not valid for the specified member type.""");
			КонецЕсли;
...
Процедура ПользователиПередУдалением(Элемент, Отказ)
	...
	Если СтрокаРодителя <> Неопределено Тогда
		...
		Если ТипЗнч(СтрокаРодителя.Пользователь) =
			...
			ПоказатьПредупреждение(,
				НСтр("es='Los usuarios de los grupos son representados para la noticia "
"que reciben el acceso de los grupos de los usuarios. "
"De ellos no es posible quitar en esta lista.';ru='Пользователи групп отображаются для сведения,"
"что они получают доступ групп пользователей."
"Их нельзя удалить в этом списке.';en='Group users are displayed for information,"
"that they are accessing user groups."
"They cannot be removed from this list. '"));
		Иначе
			ПоказатьПредупреждение(,
				НСтр("es='Los usuarios exteriores de los grupos son representados para la noticia "
"que reciben el acceso de los grupos de los usuarios exteriores. "
"De ellos no es posible quitar en esta lista.';ru='Внешние пользователи групп отображаются для сведения,"
"что они получают доступ групп внешних пользователей."
"Их нельзя удалить в этом списке.';en='External group users are displayed for information,"
"that they gain access to groups of external users."
"They cannot be removed from this list.'"));
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		Если ЗначениеЗаполнено(ТекущийОбъект.Пользователь) Тогда
			...
			Заголовок = УправлениеДоступомСлужебныйКлиентСервер.ПредставлениеГруппыДоступа(ТекущийОбъект)
				+ " " + НСтр("es='(El grupo del acceso)';ru='(Группа доступа)';en='(Access Group)'");
...
Процедура ОбновитьНазначение()
	...
	Для Каждого Участник Из Объект.Профиль.Назначение Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Элементы.Пользователи.Подсказка = НСтр("es='Los participantes admisibles:';ru='Допустимые участники:';en='Allowed participants:'") + " " + НазначениеПредставление;
...
Процедура ВыбратьПодобратьПользователейЗавершение(ВыборИПодборВнешнихПользователей, Подобрать) Экспорт
	...
	Если ВыборИПодборВнешнихПользователей Тогда
		...
		Если Не ИспользоватьВнешнихПользователей Тогда
			ПоказатьПредупреждение(, НСтр("es='La gestión de los usuarios exteriores es desconectada en los ajustes del programa.';ru='Ведение внешних пользователей отключено в настройках программы.';en='External user management is disabled in the program settings.'"));
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("es='Es insuficientemente derecho para la elección de los usuarios exteriores.';ru='Недостаточно прав для выбора внешних пользователей.';en='Not enough rights to select external users.'"));
		КонецЕсли;
	Иначе
...
Функция ПараметрыРасширеннойФормыПодбора()
	...
	ЗаголовокФормыПодбора = НСтр("es='La selección de los participantes del grupo del acceso';ru='Подбор участников группы доступа';en='Selection of access group members'");
