...
Функция РодительПерсональныхГруппДоступа(Знач НеСоздавать = Ложь, НаименованиеГруппыЭлементов = Неопределено) Экспорт
	...
	НаименованиеГруппыЭлементов = НСтр("es='Los grupos personales del acceso';ru='Персональные группы доступа';en='Personal access groups'");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ШаблонОшибкиОбработкиГруппыДоступа =
		НСтр("es='No consiguió tratar el grupo del acceso ""%1"" por la causa: "
"%2';ru='Не удалось обработать группу доступа ""%1"" по причине:"
"%2';en='Failed to process access group ""%1"" due to:"
"%2'");
	ШаблонОшибкиОбновленияТаблицГруппДоступа =
		НСтр("es='No consiguió renovar las tablas del grupo del acceso ""%1"" por la causa: "
"%2';ru='Не удалось обновить таблицы группы доступа ""%1"" по причине:"
"%2';en='Failed to update access group tables ""%1"" due to:"
"%2 '");
	ШаблонОшибкиОбновленияЗначенийГруппДоступа =
		НСтр("es='No consiguió renovar los significados del acceso del grupo del acceso ""%1"" por la causa: "
"%2';ru='Не удалось обновить значения доступа группы доступа ""%1"" по причине:"
"%2';en='Failed to update access group access values ""%1"" due to:"
"%2 '");
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Al procedimiento Справочники.ГруппыДоступа.ОбработатьДанныеДляПереходаНаНовуюВерсию no consiguió "
"renovar los datos auxiliares de algunos grupos del acceso (son dejados pasar): %1';ru='Процедуре Справочники.ГруппыДоступа.ОбработатьДанныеДляПереходаНаНовуюВерсию не удалось"
"обновить вспомогательные данные некоторых групп доступа (пропущены): %1';en='A procedure Справочники.ГруппыДоступа.ОбработатьДанныеДляПереходаНаНовуюВерсию failed to"
"update auxiliary data of some access groups (skipped):%1'"), 
			ПроблемныхОбъектов);
	...
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			...
			НСтр("es='El procedimiento Справочники.ГруппыДоступа.ОбработатьДанныеДляПереходаНаНовуюВерсию "
"ha tratado la porción siguiente de grupos del acceso: %1';ru='Процедура Справочники.ГруппыДоступа.ОбработатьДанныеДляПереходаНаНовуюВерсию"
"обработала очередную порцию групп доступа: %1';en='A procedure Справочники.ГруппыДоступа.ОбработатьДанныеДляПереходаНаНовуюВерсию processed the next batch of access groups:%1'"),
		ОбъектовОбработано));
	КонецЕсли;
...
Функция СвойстваВидаСсылок(ВидСсылок)
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El significado inadmisible de ""%1"" parámetros de VidSsylok de la función de SvoystvaVidaSsylok.';ru='Недопустимое значение ""%1"" параметра ВидСсылок функции СвойстваВидаСсылок.';en='Invalid value ""%1"" for the ВидСсылок parameter of the СвойстваВидаСсылок. function.'"),
			ВидСсылок);
	КонецЕсли;
