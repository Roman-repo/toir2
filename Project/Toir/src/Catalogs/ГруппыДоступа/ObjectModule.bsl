	...
	Если Ссылка = Справочники.ГруппыДоступа.Администраторы Тогда
		...
		Если НЕ ПривилегированныйРежим()
		   И НЕ УправлениеДоступом.ЕстьРоль("ПолныеПрава") Тогда
			...
			ВызватьИсключение
				НСтр("es='Se puede cambiar el grupo predestinado del acceso los Administradores, o en el régimen privilegiado, "
"o en existencia del papel ""los Plenos derechos"".';ru='Предопределенную группу доступа Администраторы"
"можно изменять, либо в привилегированном режиме,"
"либо при наличии роли ""Полные права"".';en='Administrators predefined access group"
"can be changed, either in privileged mode,"
"or with the role ""Full Rights"".'");
		КонецЕсли;
		...
		Для каждого ТекущаяСтрока Из Пользователи Цикл
			Если ТипЗнч(ТекущаяСтрока.Пользователь) <> Тип("СправочникСсылка.Пользователи") Тогда
				ВызватьИсключение
					НСтр("es='El grupo predestinado del acceso los Administradores "
"puede contener solamente a los usuarios. "
""
"Los grupos de los usuarios, los usuarios exteriores y "
"los grupos de los usuarios exteriores son inadmisibles.';ru='Предопределенная группа доступа Администраторы"
"может содержать только пользователей."
""
"Группы пользователей, внешние пользователи и"
"группы внешних пользователей недопустимы.';en='Administrators predefined access group"
...
"External user groups are not allowed.'");
	...
	ИначеЕсли Профиль = Справочники.ПрофилиГруппДоступа.Администратор Тогда
		ВызватьИсключение
			НСтр("es='El perfil predestinado el Administrador puede ser solamente "
"cerca del grupo predestinado del acceso los Administradores.';ru='Предопределенный профиль Администратор может быть только"
"у предопределенной группы доступа Администраторы.';en='Predefined administrator profile can only be"
"The Administrators predefined access group. '");
	КонецЕсли;
