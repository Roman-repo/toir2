...
Процедура ПередЗаписью(Отказ)
	...
	ИначеЕсли (СокрЛП(ВРег(УИННовости)) = ВРег("313f4928-a71c-4cef-96ee-a4b6106c1275"))
		...
		ТекстСообщения = НСтр("es='Ante la anotación de la novedad sobre la falta de sistema a ella era establecida la propiedad de SkryvatVObschemSpiskeNovostey que no sea representado al usuario. "
"Las novedades dadas: "
"УИН: %УИННовости del % "
"el Texto de la novedad: %ТекстНовости del %';ru='Перед записью новости про системную ошибку у нее было установлено свойство СкрыватьВОбщемСпискеНовостей, чтобы она не отображалась пользователю."
"Данные новости:"
"УИН: %УИННовости%"
"Текст новости: %ТекстНовости%';en='Before recording news about a system error, the Hide property in the General List of News was set for it so that it would not be displayed to the user."
"News data:"
"UIN:%УИННовости%"
"News text:%ТекстНовости%'");
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. El cambio de los datos';ru='БИП:Новости.Изменение данных';en='BIP: News Data Changes'"), // ИмяСобытия
			НСтр("es='Las novedades. El cambio de los datos. La novedad. Ante la anotación';ru='Новости. Изменение данных. Новость. Перед записью';en='News. Data change. News. Before recording'"), // ИдентификаторШага
	...
	Иначе
		...
		Если ВыполнениеРазрешено = Ложь Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр("es='En el régimen dividido está permitido redactar las novedades solamente en la sesión no dividida.';ru='В разделенном режиме разрешено редактировать новости только в неразделенном сеансе.';en='In split mode, you can edit news only in an unshared session.'");
				...
				Иначе
					Если ЭтоРедакторНовостей = Истина Тогда
						...
						Если ЛентаНовостей.ЗагруженоССервера = Истина Тогда
							Отказ = Истина;
							ТекстСообщения = НСтр("es='Se prohíbe anotar la novedad en la cinta de las novedades cargadas del servidor.';ru='Запрещено записывать новость в ленту новостей, загруженную с сервера.';en='It is prohibited to write news in the news feed downloaded from the server.'");
							ВызватьИсключение ТекстСообщения;
						ИначеЕсли ЛентаНовостей.ЛокальнаяЛентаНовостей = Ложь Тогда
							Отказ = Истина;
							ТекстСообщения = НСтр("es='Se prohíbe anotar la novedad en la cinta de las novedades, que no es local.';ru='Запрещено записывать новость в ленту новостей, которая не является локальной.';en='It is prohibbited to write news in a news feed that is not local.'");
							ВызватьИсключение ТекстСообщения;
						Иначе
							Запрос = Новый Запрос;
							Запрос.Текст = "
							...
							Если РезультатЗапроса.Пустой() Тогда
								Отказ = Истина;
								ТекстСообщения = НСтр("es='El usuario corriente (%1) "
"no tiene derechos a la anotación de la novedad en la cinta de las novedades %2.';ru='У текущего пользователя (%1)"
"нет прав на запись новости в ленту новостей %2.';en='The current user (%1)"
"no rights to write news to%2 feed.'");
					...
					Иначе
						Отказ = Истина;
						ТекстСообщения = НСтр("es='No hay derechos para la anotación de la novedad. "
"La anotación es accesible:"
" - para el papel de AdministratorSistemy (sin restricciones);"
" - para el papel de RedaktorNovostey (con las restricciones).';ru='Нет прав для записи новости."
"Запись доступна:"
" - для роли АдминистраторСистемы (без ограничений);"
" - для роли РедакторНовостей (с ограничениями).';en='No rights to record news."
"Record available:"
"  - for the role of the АдминистраторСистемы (without restrictions);"
"  - for the role of РедакторНовостей (with restrictions).'");
						ВызватьИсключение ТекстСообщения;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
