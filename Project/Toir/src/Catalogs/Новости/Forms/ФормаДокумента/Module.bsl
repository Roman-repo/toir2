...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='(Todo el tiempo es indicado para el huso horario %1 (%2))';ru='(все время указано для часового пояса %1 (%2))';en='(all time indicated for time zone %1 (%2))'"),
		ЧасовойПояс(),
	...
	Если (ЭтотОбъект.БезАвторизации = Истина)
		...
		Элементы.ДекорацияВозможностьИзмененияНовости.Заголовок =
			НСтр("es='Para la redacción de esta novedad es necesario ser autorizado en el centro de noticias.';ru='Для редактирования этой новости необходимо авторизоваться в новостном центре.';en='To edit this news, you need to log in to the news center.'");
	...
	ИначеЕсли (ЭтотОбъект.БезАвторизации = Ложь)
		...
		Элементы.ДекорацияВозможностьИзмененияНовости.Заголовок =
			НСтр("es='La novedad era con éxito publicada. Para cambiarla, es necesario al principio anular la publicación.';ru='Новость была успешно опубликована. Чтобы изменить ее, необходимо вначале отменить публикацию.';en='The news was successfully published. To change it, you must first cancel the publication.'");
		Элементы.ДекорацияВозможностьИзмененияНовости.Видимость = Истина;
		ЭтотОбъект.ТолькоПросмотр = Истина;
	ИначеЕсли (ЭтотОбъект.БезАвторизации = Ложь)
			И (ЭтотОбъект.СтатусНовостиНаСервере = 3) Тогда // Есть на сервере, опубликована, нельзя повторно публиковать.
		Элементы.ДекорацияВозможностьИзмененияНовости.Заголовок =
			НСтр("es='La novedad era con éxito publicada. Puesto que la novedad era publicada en los canales, por que es prohibida la republicación (емейл, смс), cambiarla ya no es posible, pero es posible copiar esta novedad y gobernar ya de ella.';ru='Новость была успешно опубликована. Так как новость была опубликована в каналах, по которым запрещена перепубликация (емейл, смс), то изменить ее уже нельзя, но можно скопировать эту новость и править уже ее.';en='The news was successfully published. Since the news was published in channels through which republishing (email, SMS) is prohibited, it is no longer possible to change it, but you can copy this news and edit it already.'");
		Элементы.ДекорацияВозможностьИзмененияНовости.Видимость = Истина;
		ЭтотОбъект.ТолькоПросмотр = Истина;
	КонецЕсли;
	...
	Если ЭтотОбъект.БезАвторизации = Истина Тогда
		...
		Если ЭтотОбъект.РазрешенныеДляРедактированияЛентыНовостей.Количество() = 0 Тогда
			...
			Иначе
				...
				ТекстСообщения = НСтр("es='No hay derechos a la redacción de las novedades. "
"Deben ser incluidos los papeles de AdministratorSistemy y PolnyePrava (entonces es posible redactar todas las cintas locales de las novedades) "
"o el papel de RedaktirovanieNovostey (junto con el ajuste de las cintas de las novedades, en que es posible redactar las novedades).';ru='Нет прав на редактирование новостей."
"Должны быть включены роли АдминистраторСистемы и ПолныеПрава (тогда можно редактировать все локальные ленты новостей)"
"или роль РедактированиеНовостей (вместе с настройкой лент новостей, в которых можно редактировать новости).';en='No rights to edit news."
"The roles of АдминистраторСистемы and ПолныеПрава should be included (then you can edit all local news feeds)"
"or the РедактированиеНовостей role (along with setting up news feeds where you can edit news).'");
		...
		Если МассивРазрешенныхДляРедактированияЛентНовостей.Количество() = 0 Тогда
			ЭтотОбъект.ТолькоПросмотр = Истина;
			ТекстСообщения = НСтр("es='No es encontrado ni una cinta, permitida para la redacción, de las novedades. "
"Es imposible redactar la novedad.';ru='Не найдено ни одной разрешенной для редактирования ленты новостей."
"Невозможно отредактировать новость.';en='No news feeds allowed for editing were found."
"Unable to edit news.'");
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = ТекстСообщения;
			Сообщение.УстановитьДанные(Объект);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для С=0 По Объект.Действия.Количество()-1 Цикл
		...
		Если НайденныеПовторения.Количество() > 1 Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("es='El identificador de la acción %УИНДействия del % repete %КоличествоПовторений del %';ru='Идентификатор действия %УИНДействия% повторяется %КоличествоПовторений%';en='Actions identifier %УИНДействия% repeated %КоличествоПовторений%'");
		...
		Если Лев(ВРег(ТекущаяСтрока.УИНДействия), "3") <> ВРег("1C:") Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("es='El identificador de la acción debe comenzar con 1C:';ru='Идентификатор действия должен начинаться с 1C:';en='The action identifier must begin with 1C:'");
			Сообщение.Поле  = "Объект.Действия[" + С + "].УИНДействия";
			Сообщение.Сообщить();
		КонецЕсли;
		...
		Для ТекущийСимвол=4 По СтрДлина(ТекущаяСтрока.УИНДействия) Цикл
			...
			Если (СтрНайти(РазрешенныеСимволы, Символ) = 0) Тогда
				Отказ = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("es='El identificador de la acción debe comenzar con 1C: y consistir solamente de los símbolos ingleses, las cifras y el signo del subrayado';ru='Идентификатор действия должен начинаться с 1C: и состоять только из английских символов, цифр и знака подчеркивания';en='The action identifier must begin with 1C: and consist only of English characters, numbers and the underscore'");
				Сообщение.Поле  = "Объект.Действия[" + С + "].УИНДействия";
				Сообщение.Сообщить();
			КонецЕсли;
		КонецЦикла;
		...
		Если ПустаяСтрока(ТекущаяСтрока.Действие) Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("es='La acción vacía';ru='Пустое действие';en='Empty action'");
		...
		Иначе
			...
			Иначе
				Отказ = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("es='La acción incorrecta. Escojan el significado de la acción de la lista';ru='Неправильное действие. Выберите значение действия из списка';en='Wrong action. Select an action value from the list'");
				Сообщение.Поле  = "Объект.Действия[" + С + "].Действие";
				Сообщение.Сообщить();
			КонецЕсли;
		КонецЕсли;
	...
	Для С=0 По Объект.ПараметрыДействий.Количество()-1 Цикл
		...
		Если НайденныеВладельцы.Количество() = 0 Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("es='El parámetro no es atado ni a una acción (debe ser por lo menos una acción con el mismo identificador [%УИНДействия del %]). Se Recomienda quitar la línea.';ru='Параметр не привязан ни к одному действию (должно быть хотя бы одно действие с таким же идентификатором [%УИНДействия%]). Рекомендуется удалить строку.';en='The parameter is not associated with any action (there must be at least one action with the same identifier [%УИНДействия%]). It is recommended to delete the line.'");
			Сообщение.Текст = СтрЗаменить(Сообщение.Текст, "%УИНДействия%", ТекущаяСтрока.УИНДействия);
		...
		ИначеЕсли НайденныеВладельцы.Количество() > 1 Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("es='El parámetro tener ser solamente un propietario (una acción)';ru='У параметра должен быть только один владелец (одно действие)';en='The parameter must have only one owner (one action)'");
			Сообщение.Поле  = "Объект.ПараметрыДействий[" + С + "].УИНДействия";
			Сообщение.Сообщить();
		Иначе // =1
			...
			Если ТекущийВладелец.Действие = "Переход по навигационной ссылке" Тогда
				...
				Иначе
					Отказ = Истина;
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = НСтр("es='Cerca de la acción [%Действие del %] el parámetro [%Параметр del %] incorrecto. Escojan el significado de la lista';ru='У действия [%Действие%] параметр [%Параметр%] неправильный. Выберите значение из списка';en='The action [%Действие%] has the parameter [%Параметр%] incorrect. Please select a value from the list'");
			...
			ИначеЕсли ТекущийВладелец.Действие = "Показать картинку" Тогда
				Если ТекущаяСтрока.Параметр = "УИН" Тогда
					...
					Если НайденныеБинарныеДанные.Количество() = 0 Тогда
						Отказ = Истина;
						Сообщение = Новый СообщениеПользователю;
						Сообщение.Текст = НСтр("es='Cerca de la acción [%Действие del %] el parámetro [%Параметр del %] indica a los datos no existentes binarios. Escojan el significado correcto de los datos binarios.';ru='У действия [%Действие%] параметр [%Параметр%] указывает на несуществующие бинарные данные. Выберите правильное значение бинарных данных.';en='The action [%Действие%], the parameter [%Параметр%] indicates non-existent binary data. Please select the correct binary data value.'");
						Сообщение.Текст = СтрЗаменить(Сообщение.Текст, "%Действие%", ТекущийВладелец.Действие);
						Сообщение.Текст = СтрЗаменить(Сообщение.Текст, "%Параметр%", ТекущаяСтрока.Параметр);
						Сообщение.Поле  = "Объект.ПараметрыДействий[" + С + "].Параметр";
						Сообщение.Сообщить();
	...
	Если ОтменаРедактирования <> Истина Тогда
		...
		Если ТекущиеДанные <> Неопределено Тогда
			...
			Если ТекущиеДанные.ВерсияОТ > ТекущиеДанные.ВерсияДО Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("es='La versión DE [ВерсияОТ] no debe ser más que la Versión HASTA [ВерсияДО].';ru='Версия ОТ [ВерсияОТ] не должна быть больше, чем Версия ДО [ВерсияДО].';en='The version FROM [ВерсияOT] should not be larger than the Version BEFORE [ВерсияДO].'");
...
Процедура ДействияПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если НЕ ОтменаРедактирования Тогда
		...
		Если Отказ = Истина Тогда
			ТекстОшибки = НСтр("es='El identificador de la hiperreferencia debe comenzar con 1C: (donde C - la letra inglesa), "
"y consistir solamente de las letras inglesas, las cifras y los signos del subrayado. "
""
"Y en el campo la Acción debe ser el significado escogido de la lista.';ru='Идентификатор гиперссылки должен начинаться с 1C: (где C - английская буква),"
"и состоять только из английских букв, цифр и знаков подчеркивания."
""
"А в поле Действие должно быть значение, выбранное из списка.';en='The hyperlink identifier must begin with 1C: (where C is the English letter)"
"and consist only of English letters, numbers and underscores."
""
"And in the Action field should be a value selected from the list.'");
			ПоказатьПредупреждение(
				, // ОписаниеоповещенияОЗавершении
				ТекстОшибки, // ТекстПредупреждения
				0, // Таймаут
				НСтр("es='La falta';ru='Ошибка';en='Error '")); // Заголовок
		КонецЕсли;
	КонецЕсли;
...
Процедура ПараметрыДействийЗначениеПараметраНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли лкДействие = "Открытие новости" Тогда
			...
			КонецЕсли;
			Если лкПараметр = "ИдентификаторМеста" Тогда
...
Процедура КомандаЗагрузитьФайлКакБинарныеДанные(Команда)
	...
	Иначе
		ТекстОшибки = НСтр("es='Escojan la línea, donde colocar los datos';ru='Выберите строку, куда помещать данные';en='Choose the line where to put the data'");
		ПоказатьПредупреждение(
			, // ОписаниеоповещенияОЗавершении
			ТекстОшибки, // ТекстПредупреждения
			0, // Таймаут
...
Процедура Подключаемый_КомандаБуферОбмена_Копировать(Команда)
	...
	Если ВРег(Команда.Имя) = ВРег("КомандаБуферОбмена_Копировать_КатегорииПростые")
		...
		Если ЭлементФормы.ВыделенныеСтроки.Количество() > 0 Тогда
			...
			КонецЦикла;
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es copiado las líneas: %1';ru='Скопировано строк: %1';en='Lines copied:%1'"),
		...
		Иначе
			ПоказатьОповещениеПользователя(НСтр("es='Distingan por lo menos una línea';ru='Выделите хотя бы одну строку';en='Select at least one line'"), , , БиблиотекаКартинок.НовостиВнимание);
		КонецЕсли;
...
Процедура Подключаемый_КомандаБуферОбмена_Вставить(Команда)
	...
	Если ВРег(Команда.Имя) = ВРег("КомандаБуферОбмена_Вставить_ТекущийБуферОбмена_КатегорииПростые")
		...
		Если ДанныеБуфераОбмена.Количество() > 0 Тогда
			...
			КонецЦикла;
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es puesto las líneas: %1';ru='Вставлено строк: %1';en='Rows inserted:% 1'"),
				ДанныеБуфераОбмена.Количество());
			ПоказатьОповещениеПользователя(ТекстСообщения, , , БиблиотекаКартинок.Информация);
		Иначе
			ПоказатьОповещениеПользователя(
				НСтр("es='No hay datos en el portapapeles del cambio.';ru='Нет данных в буфере обмена.';en='No data on clipboard.'"),
				,
				НСтр("es='Al principio copien las líneas.';ru='Вначале скопируйте строки.';en='Copy the lines first.'"),
...
Процедура КомандаОткрытьФормуКонтекстнойНовости(Команда)
	...
	Если Элементы.ПривязкаКМетаданным.ТекущиеДанные <> Неопределено Тогда
		...
		Исключение
			ТекстОшибки = НСтр("es='La forma %ИмяФормыПроверкиНовости del % no es descubierta. "
"Probablemente, para la descripción de la forma son usados no los nombres en los metadatos, y otro texto,"
"	O son incorrectamente introducidos los identificadores de los metadatos y / o la forma. "
"La comprobación de la novedad tiene que abrir la forma a mano o corregir los significados en las columnas.';ru='Форма %ИмяФормыПроверкиНовости% не обнаружена."
...
"Для проверки новости необходимо открыть форму вручную или исправить значения в колонках.';en='Form %ИмяФормыПроверкиНовости% not found."
"It’s possible that not the names in the metadata were used to describe the form, but different text,"
"or metadata and / or form identifiers entered incorrectly."
"To check the news, you must open the form manually or correct the values in the columns.'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ИмяФормыПроверкиНовости%", ИмяФормыПроверкиНовости);
			ПоказатьПредупреждение(
				, // ОписаниеоповещенияОЗавершении
				ТекстОшибки, // ТекстПредупреждения
				0, // Таймаут
				НСтр("es='La prevención';ru='Предупреждение';en='Warning'")); // Заголовок
		КонецПопытки;
	...
	Если СтрНайти(ВРег(Команда.Имя), ВРег(ШаблонИмениКоманды)) = 1 Тогда
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			...
			Если ПустаяСтрока(ПолеТекстовогоДокумента.ВыделенныйТекст) Тогда
				ТекстГиперссылки = НСтр("es='Presionen aquí';ru='Нажмите здесь';en='Click here'");
			Иначе
...
Процедура КомандаИмпортЭкспортXML(Команда)
	...
	ИначеЕсли Команда.Имя = "КомандаИмпортИзXML" Тогда
		Если Объект.ЛентаНовостей.Пустая() Тогда
			ТекстСообщения = НСтр("es='Es necesario rellenar la cinta de las novedades.';ru='Необходимо заполнить ленту новостей.';en='Required to fill out the news feed.'");
			Сообщение = Новый СообщениеПользователю();
			Сообщение.Текст = ТекстСообщения;
...
Процедура ОбновитьИнформационныеСтроки()
	...
	лкТекст = НСтр("es='La dimensión de todos los datos binarios: %РазмерБинарныхДанных del % del byte "
"la dimensión Aproximada de la novedad (incluso el texto de la novedad): %ОбщийРазмер del % del byte "
"%Предупреждение del %';ru='Размер всех бинарных данных: %РазмерБинарныхДанных% байт"
"Примерный размер новости (включая текст новости): %ОбщийРазмер% байт"
"%Предупреждение%';en='Size of all binary data: %РазмерБинарныхДанных% bytes"
"The approximate size of the news (including the text of the news): %ОбщийРазмер% bytes"
"%Предупреждение%'");
	...
	Если Объект.БинарныеДанные.Итог("ДанныеРазмер") > 50000 Тогда
		лкТекст = СтрЗаменить(лкТекст, "%Предупреждение%", НСтр("es='Las novedades de la dimensión grande pueden disminuir el trabajo del programa en la plataforma 1С';ru='Новости большого размера могут замедлить работу программы на платформе 1С';en='Large news can slow down the program on the 1C platform'"));
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц(СписокТабличныхЧастей = Неопределено)
	...
	Если МассивТабличныхЧастей.Найти("БинарныеДанные") <> Неопределено Тогда
		...
		Иначе
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("es='No consiguió cargar los datos binarios (no coincide la cantidad de las líneas en la forma y en el objeto). A la conservación serán perdidos. Se Recomienda cerrar / abrir la forma de la novedad sin conservación';ru='Не удалось загрузить бинарные данные (не совпадает количество строк в форме и в объекте). При сохранении они будут утеряны. Рекомендуется закрыть / открыть форму новости без сохранения';en='Failed to load binary data (the number of lines in the form and in the object does not match). When saved, they will be lost. It is recommended to close / open the news form without saving'");
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьДоступныеКатегорииДляЛентыНовостейСервер()
	...
	Если МассивКатегорий_Простые.Количество() = 0 Тогда
		...
		Элементы.ГруппаКатегорииПростые.Заголовок = НСтр("es='Las categorías simples (es inaccesible, puesto que para esta cinta de las novedades no son incluidas las selecciones por las categorías simples)';ru='Простые категории (недоступно, так как для этой ленты новостей не включены отборы по простым категориям)';en='Simple categories (not available, since selections for simple categories are not included for this feed)'");
	Иначе
		Элементы.КатегорииПростые.ТолькоПросмотр = Ложь;
		Элементы.ГруппаКатегорииПростые.Доступность = Истина;
		Элементы.ГруппаКатегорииПростые.Заголовок = НСтр("es='Las categorías simples';ru='Простые категории';en='Simple categories'");
	КонецЕсли;
	...
	Если МассивКатегорий_ИнтервалыВерсий.Количество() = 0 Тогда
		...
		Элементы.ГруппаКатегорииИнтервалыВерсий.Заголовок = НСтр("es='La atadura a las versiones (es inaccesible, puesto que para esta cinta de las novedades no son incluidas las selecciones según las versiones)';ru='Привязка к версиям (недоступна, так как для этой ленты новостей не включены отборы по версиям)';en='Linking to versions (not available, since version selections are not included for this news feed)'");
	Иначе
		Элементы.КатегорииИнтервалыВерсий.ТолькоПросмотр = Ложь;
		Элементы.ГруппаКатегорииИнтервалыВерсий.Доступность = Истина;
		Элементы.ГруппаКатегорииИнтервалыВерсий.Заголовок = НСтр("es='La atadura a las versiones';ru='Привязка к версиям';en='Version binding'");
	КонецЕсли;
	...
	Если ТипЗнч(РезультатЗакрытия) = ТипСтруктура Тогда
		...
		Если ВРег(РезультатЗакрытия.Действие) = ВРег("Переход по интернет ссылке") Тогда
			...
			Если ПустаяСтрока(лкВыделенныйТекст) Тогда
				лкВыделенныйТекст = НСтр("es='Presionen aquí';ru='Нажмите здесь';en='Click here'");
			КонецЕсли;
				...
				#Если НЕ ВебКлиент Тогда
					...
					Попытка
						...
						Если НЕ ПустаяСтрока(УИНМаркера) Тогда
							ЭлементХТМЛМаркер = ДокументХТМЛ.ПолучитьЭлементПоИдентификатору(УИНМаркера);
							...
							Иначе
								...
								ЭлементХТМЛМаркер.УстановитьАтрибут("id", УИНМаркера);
								ЭлементХТМЛМаркер.УстановитьАтрибутИдентификатор("id", Истина);
							КонецЕсли;
							ЭлементХТМЛМаркер.ТекстовоеСодержимое = МаркерСвернуто;
							ЭлементХТМЛГиперссылка.ДобавитьДочерний(ЭлементХТМЛМаркер);
						КонецЕсли;
						ЭлементХТМЛТекст = ДокументХТМЛ.СоздатьТекстовыйУзел(лкВыделенныйТекст);
						...
						Если НЕ ПустаяСтрока(УИНБлока) Тогда
							ЭлементХТМЛБлок = ДокументХТМЛ.ПолучитьЭлементПоИдентификатору(УИНБлока);
							...
							Иначе
								...
								ЭлементХТМЛБлок.УстановитьАтрибут("id", УИНБлока);
							...
							Для С=1 По 5 Цикл
								ЭлементХТМЛТекст = ДокументХТМЛ.СоздатьТекстовыйУзел(НСтр("es='La línea';ru='Строка';en='Line'") + " " + С + Символы.ПС);
							...
							ЭлементХТМЛДополнительнаяГиперссылка.ТекстовоеСодержимое = НСтр("es='Doblar';ru='свернуть';en='collapse'");
...
#Область ФорматированиеHTML_БлокиИРазрывы
	...
	НоваяКоманда.Заголовок                 = НСтр("es='La ruptura de la línea <br/>';ru='Разрыв строки <br/>';en='Line break <br/>'");
	...
	НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("es='La línea horizontal <hr/>';ru='Горизонтальная линия <hr/>';en='Horizontal line <hr />'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("es='El párrafo <p> </p>';ru='Абзац <p></p>';en='Paragraph <p> </p>'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("es='El bloque para el párrafo <div> </div>';ru='Блок для абзаца <div></div>';en='Paragraph box <div> </div>'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("es='El bloque para el texto dentro del párrafo <span> </span>';ru='Блок для текста внутри абзаца <span></span>';en='Block for text inside paragraph <span> </span>'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_ФорматШрифта
	...
	НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("es='Los caracteres gruesos (<b> </b>)';ru='Жирный шрифт (<b></b>)';en='Bold font (<b> </b>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("es='La letra cursiva (<i> </i>)';ru='Курсивный шрифт (<i></i>)';en='Italic font (<i> </i>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("es='La letra subrayada (<u> </u>)';ru='Подчеркнутый шрифт (<u></u>)';en='Underline font (<u> </u>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("es='La letra tachada (<s> </s>)';ru='Зачеркнутый шрифт (<s></s>)';en='Strikethrough font (<s> </s>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("es='El índice superior (<sup> </sup>)';ru='Верхний индекс (<sup></sup>)';en='Superscript (<sup> </sup>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("es='El índice inferior (<sub> </sub>)';ru='Нижний индекс (<sub></sub>)';en='Subscript (<sub> </sub>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_ФорматШрифта_Заголовки
	...
	Для С=1 По 6 Цикл
		...
		НоваяКоманда.Заголовок                 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El encabezamiento %1 del nivel (<h%1> </h%1>)';ru='Заголовок %1 уровня (<h%1></h%1>)';en='Heading % 1 level (<h% 1> </ h% 1>)'"),
			С);
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_ФорматАбзаца
	...
	НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("es='La alineación a la izquierda';ru='Выравнивание влево';en='Left alignment'"); // <p style="text-align: left;"></p>
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("es='La alineación a la derecha';ru='Выравнивание вправо';en='Right alignment'"); // <p style="text-align: right;"></p>
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("es='La alineación por el centro';ru='Выравнивание по центру';en='Center alignment'"); // <p style="text-align: center;"></p>
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("es='La alineación por la anchura';ru='Выравнивание по ширине';en='Justified alignment'"); // <p style="text-align: justify;"></p>
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_СтилиТекста
	...
	НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("es='La cita larga (<blockquote> </blockquote>)';ru='Длинная цитата (<blockquote></blockquote>)';en='Long quote (<blockquote></blockquote>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("es='La cita corta (<q> </q>)';ru='Короткая цитата (<q></q>)';en='Short Quote (<q> </q>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("es='La fuente de la cita (<cite> </cite>)';ru='Источник цитаты (<cite></cite>)';en='Source of quote (<cite> </cite>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("es='La introducción del teclado (<kbd> </kbd>)';ru='Ввод с клавиатуры (<kbd></kbd>)';en='Keyboard Input (<kbd> </kbd>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("es='El código de programa (<code> </code>)';ru='Программный код (<code></code>)';en='Program code (<code> </code>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
	...
	НоваяКоманда.Действие                  = "Подключаемый_КомандаВставитьТеги";
	НоваяКоманда.Заголовок                 = НСтр("es='El texto subrayado (<em> </em>)';ru='Акцентированный текст (<em></em>)';en='Accented text (<em> </em>)'");
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.Отображение               = ОтображениеКнопки.КартинкаИТекст;
	НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
#Область ФорматированиеHTML_БлокиИРазрывы
	...
	ГруппаКоманд.Заголовок = НСтр("es='Los bloques y las rupturas';ru='Блоки и разрывы';en='Blocks and breaks'");
...
#Область ФорматированиеHTML_ФорматШрифта
	...
	ГруппаКоманд.Вид = ВидГруппыФормы.ГруппаКнопок;
	ГруппаКоманд.Заголовок = НСтр("es='El formato de la letra';ru='Формат шрифта';en='Font format'");
...
#Область ФорматированиеHTML_ФорматШрифта_Заголовки
	...
	ГруппаКоманд.Заголовок = НСтр("es='Los encabezamientos';ru='Заголовки';en='Headings'");
...
#Область ФорматированиеHTML_СтилиТекста
	...
	ГруппаКоманд.Вид = ВидГруппыФормы.ГруппаКнопок;
	ГруппаКоманд.Заголовок = НСтр("es='Los estilos del texto';ru='Стили текста';en='Text styles'");
...
Процедура СоздатьКомандыБуфераОбмена(МассивБуферовОбмена)
	...
	Для каждого ТекущийБуферОбмена Из МассивБуферовОбмена Цикл
		...
		НоваяКоманда.Заголовок                 = НСтр("es='Copiar';ru='Копировать';en='Copy'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Ложь;
		НоваяКоманда.Отображение               = ОтображениеКнопки.Картинка;
		НоваяКоманда.Подсказка                 = НСтр("es='Copiar las líneas distinguidas en el portapapeles del cambio';ru='Копировать выделенные строки в буфер обмена';en='Copy selected lines to clipboard'");
		...
		НоваяКоманда.Заголовок                 = НСтр("es='La dirección de los portapapeles conservados del cambio';ru='Управление сохраненными буферами обмена';en='Control of saved clipboards'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Ложь;
		НоваяКоманда.Отображение               = ОтображениеКнопки.Картинка;
		НоваяКоманда.Подсказка                 = НСтр("es='La dirección de los portapapeles conservados antes del cambio';ru='Управление сохраненными раньше буферами обмена';en='Control of previously saved clipboards'");
		...
		НоваяКоманда.Заголовок                 = НСтр("es='Poner el contenido del portapapeles del cambio';ru='Вставить содержимое буфера обмена';en='Paste clipboard contents'");
		НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
		НоваяКоманда.Отображение               = ОтображениеКнопки.Картинка;
		НоваяКоманда.Подсказка                 = НоваяКоманда.Заголовок;
...
Процедура СоздатьКомандыБуфераОбмена_СохраненныеБуферыОбмена(МассивБуферовОбмена)
	...
	Для каждого ТекущийБуферОбмена Из МассивБуферовОбмена Цикл
		...
		Для каждого ТекущийЭлементСписка Из ЭтотОбъект["СохраненныеБуферыОбмена_" + ТекущийБуферОбмена] Цикл
			...
			НоваяКоманда.Действие                  = "Подключаемый_КомандаБуферОбмена_Вставить";
			НоваяКоманда.Заголовок                 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Poner de %1';ru='Вставить из %1';en='Paste from% 1'"),
...
Процедура СоздатьПодменюБуфераОбмена(Подменю, БуферОбмена)
	...
	ГруппаКомандВставить.Заголовок   = "Вставить";
		...
		ГруппаКомандВставитьСохраненный.Заголовок = НСтр("es='Poner conservado';ru='Вставить сохраненный';en='Paste Saved'");
...
Процедура УстановитьУсловноеОформление()
	...
	// 2. Заменить в поле Объект.Важность число (0, 1, 2) на текст ("Обычная", "Очень важная", "Важная").
		УсловияОтбора = Новый Соответствие;
		УсловияОтбора.Вставить(0, НСтр("es='Regular';ru='Обычная';en='Usual'"));
		УсловияОтбора.Вставить(1, НСтр("es='Muy importante';ru='Очень важная';en='Very important'"));
		УсловияОтбора.Вставить(2, НСтр("es='Importante';ru='Важная';en='Important'"));
	...
	// 8. Поле "Единственный параметр".
		...
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='Algunos parámetros se disponen en la tabla [los Parámetros]';ru='Несколько параметров настраиваются в таблице [Параметры]';en='Several parameters are configured in the [Параметры] table'"));
		Элемент.Оформление.УстановитьЗначениеПараметра("Доступность", Ложь);
...
Процедура ВывестиСтатистику() Экспорт
	...
	ТекстПояснения = НСтр("es='El texto para las fuentes exteriores:"
" - 10 = %ТекстВИ_10_Э del % / %ТекстВИ_10_О del %,"
" - 13 = %ТекстВИ_13_Э del % / %ТекстВИ_13_О del %, "
"el Texto para 1С:"
" - 10 = %Текст1С_10_Э del % / %Текст1С_10_О del %,"
" - 13 = %Текст1С_13_Э del % / %Текст1С_13_О del %,';ru='Текст для внешних источников:"
" - 10 = %ТекстВИ_10_Э% / %ТекстВИ_10_О%,"
" - 13 = %ТекстВИ_13_Э% / %ТекстВИ_13_О%,"
"Текст для 1С:"
" - 10 = %Текст1С_10_Э% / %Текст1С_10_О%,"
" - 13 = %Текст1С_13_Э% / %Текст1С_13_О%,';en='Text for external sources:                                                - 10 = %ТекстВИ_10_Э% / %ТекстВИ_10_О%,"
" - 13 = %ТекстВИ_13_Э% / %ТекстВИ_13_О%,"
"Текст для 1С:"
" - 10 = %Текст1С_10_Э% / %Текст1С_10_О%,"
" - 13 = %Текст1С_13_Э% / %Текст1С_13_О%,'")
	...
	ПоказатьОповещениеПользователя(
		НСтр("es='La estadística por las traducciones de las líneas:';ru='Статистика по переводам строк:';en='Line feed statistics:'"),
		,
		ТекстПояснения,
			...
			Если СвойствоСписокКатегорийПростых <> Неопределено Тогда
				...
				Для каждого ТекущаяКатегорияПростаяХДТО Из СписокКатегорийПростыхХДТО Цикл
					...
					Если НайденнаяКатегория.Пустая() Тогда
						ТекстСообщения = НСтр("es='La categoría (simple) con УИН %УИН del % y el código %Код del % no es encontrada.';ru='Категория (простая) с УИН %УИН% и кодом %Код% не найдена.';en='Category (simple) with UIN %УИН% and code %Код% not found.'");
		...
		Если СвойствоКатегорииИнтервалыВерсий <> Неопределено Тогда
			...
			Если СвойствоСписокКатегорийИнтервалыВерсий <> Неопределено Тогда
				...
				Для каждого ТекущаяКатегорияИнтервалыВерсийХДТО Из СписокКатегорийПростыхХДТО Цикл
					...
					НайденнаяКатегория = ПланыВидовХарактеристик.КатегорииНовостей.НайтиПоКоду(КодКатегории);
					Если НайденнаяКатегория.Пустая() Тогда
						ТекстСообщения = НСтр("es='La categoría (el intervalo de las versiones) con УИН %УИН del % y el código %Код del % no es encontrada.';ru='Категория (интервал версий) с УИН %УИН% и кодом %Код% не найдена.';en='Category (version range) with UIN %УИН% and code %Код% not found.'");
...
Функция ПолучитьПредставлениеНовостиXML()
	...
	Если ПустаяСтрока(ЗаголовокНовостиЛокальный) Тогда
		ЗаголовокНовостиЛокальный = НСтр("es='El encabezamiento de la novedad';ru='Заголовок новости';en='News heading'");
	КонецЕсли;
