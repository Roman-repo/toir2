...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Сообщение.Текст = НСтр("es='En la guía de las novedades no es posible crear las novedades a mano. "
"Usen la carga de las novedades de las cintas de las novedades.';ru='В справочнике новостей нельзя создавать новости вручную."
"Используйте загрузку новостей из лент новостей.';en='You cannot manually create news in the news directory."
"Use news feeds from feeds.'");
		Сообщение.Сообщить();
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура УправлениеФормой()
	...
	Если Пометка = 0 Тогда
		...
		Элементы.Пометка.Подсказка = НСтр("es='La novedad no es notada';ru='Новость не отмечена';en='No news marked'");
	Иначе
		Элементы.Пометка.Рамка = Новый Рамка(ТипРамкиЭлементаУправления.Вдавленная);
		Элементы.Пометка.Подсказка = НСтр("es='La novedad es notada';ru='Новость отмечена';en='News marked'");
	КонецЕсли;
