...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПолнотекстовыйПоиск.ПолучитьРежимПолнотекстовогоПоиска() = РежимПолнотекстовогоПоиска.Разрешить Тогда
		Элементы.СтрокаПоиска.Подсказка = НСтр("es='La búsqueda polnotekstovyy es incluida. "
"¿Se puede usar los símbolos de la substitución * y?';ru='Полнотекстовый поиск включен."
"Можно использовать символы подстановки * и ?';en='Full text search enabled."
"Can use wildcard characters * and? '");
	Иначе
		Элементы.СтрокаПоиска.Подсказка = НСтр("es='La búsqueda polnotekstovyy es desconectada. "
"Es accesible la búsqueda solamente las frases exactas';ru='Полнотекстовый поиск отключен."
"Доступен поиск только точных фраз';en='Full text search disabled."
"Search only for exact phrases '");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПустаяСтрока(ЭтотОбъект.СтрокаПоиска) Тогда
		...
		Сообщение.Текст = НСтр("es='La línea de la búsqueda vacío';ru='Строка поиска пустая';en='Search line is empty'");
	...
	Если (ЭтотОбъект.ПоискДатаОТ <> '00010101') И (ЭтотОбъект.ПоискДатаДО <> '00010101') Тогда
		Если (ЭтотОбъект.ПоискДатаОТ > ЭтотОбъект.ПоискДатаДО) Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("es='Es incorrectamente dado el intervalo de la búsqueda por las fechas: la fecha ha comenzado después de la fecha de la terminación';ru='Неверно задан интервал поиска по датам: дата начала позже даты окончания';en='Date search interval set incorrectly: start date later than end date'");
