...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтотОбъект.ВремяПереносаПоказатьПозжеМинут > 0 Тогда
		Если ЭтотОбъект.ТаблицаНовостей.Количество() = 1 Тогда
			ТекстПодсказки = НСтр("es='No recordar sobre esta novedad %ВремяПереносаПоказатьПозжеМинут el % de los minutos';ru='Не напоминать про эту новость %ВремяПереносаПоказатьПозжеМинут% минут';en='Do not remind about this news %ВремяПереносаПоказатьПозжеМинут% minutes'");
		Иначе
			ТекстПодсказки = НСтр("es='No recordar sobre estas novedades %ВремяПереносаПоказатьПозжеМинут el % de los minutos';ru='Не напоминать про эти новости %ВремяПереносаПоказатьПозжеМинут% минут';en='Do not remind about this news %ВремяПереносаПоказатьПозжеМинут% minutes'");
		КонецЕсли;
