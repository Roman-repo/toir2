...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы = Истина Тогда
		...
		Если ЭтотОбъект.НеобходимПересчетСлужебныхДанных = Истина Тогда
			Отказ = Истина;
			ТекстПредупреждения = НСтр("es='Es necesario el recuento de los datos de servicio. "
"Presionen el botón [el Recuento de los datos de servicio] "
"o cierren la ventana a mano, hasta la salida del programa.';ru='Необходим пересчет служебных данных."
"Нажмите кнопку [Пересчет служебных данных]"
"или закройте окно вручную, до выхода из программы.';en='Service data recount required Click the [Пересчет служебных данных] button"
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если ЭтотОбъект.РазрешенныеДляРедактированияЛентыНовостей.Количество()=0 Тогда
		ТекстСообщения = НСтр("es='No es posible añadir las novedades, si no existe ni una cinta local de las novedades, donde está permitido la redacción. "
"Al usuario con los papeles de AdministratorSistemy y PolnyePrava es posible redactar todas las cintas locales de las novedades. "
"Al usuario con el papel de RedaktirovanieNovostey es posible redactar las cintas locales, permitidas por el Administrador, de las novedades.';ru='Нельзя добавлять новости, если нет ни одной локальной ленты новостей, где разрешено редактирование."
"Пользователю с ролями АдминистраторСистемы и ПолныеПрава можно редактировать все локальные ленты новостей."
"Пользователю с ролью РедактированиеНовостей можно редактировать разрешенные Администратором локальные ленты новостей.';en='Can not add news if there is not a single local news feed where editing is allowed."
"A user with the roles АдминистраторСистемы and ПолныеПрава can edit all local news feeds."
"A user with the РедактированиеНовостей role can edit the local news feeds allowed by the Administrator. '");
...
Процедура СписокПередНачаломИзменения(Элемент, Отказ)
	...
	Если ЭтотОбъект.РазрешенныеДляРедактированияЛентыНовостей.НайтиПоЗначению(Элемент.ТекущиеДанные.ЛентаНовостей) = Неопределено Тогда
		ТекстСообщения = НСтр("es='Se puede cambiar las novedades solamente en la cinta local de las novedades, donde está permitido la redacción. "
"Al usuario con los papeles de AdministratorSistemy y PolnyePrava es posible redactar todas las cintas locales de las novedades. "
"Al usuario con el papel de RedaktirovanieNovostey es posible redactar las cintas locales, permitidas por el Administrador, de las novedades.';ru='Можно изменять новости только в локальной ленте новостей, где разрешено редактирование."
"Пользователю с ролями АдминистраторСистемы и ПолныеПрава можно редактировать все локальные ленты новостей."
"Пользователю с ролью РедактированиеНовостей можно редактировать разрешенные Администратором локальные ленты новостей.';en='You can change news only in the local news feed, where editing is allowed."
"User with roles АдминистраторСистемы and  ПолныеПрава You can edit all local news feeds."
"Users with the Edit News role can edit local news feeds allowed by the Administrator.'");
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = ТекстСообщения;
		Сообщение.Сообщить();
...
Процедура ПересчетСлужебныхДанныхНаСервере()
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='El recuento de los datos de servicio por las novedades';ru='Пересчет служебных данных по новостям';en='Recalculation of news service data'"));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El recuento de todos los datos de servicio por las novedades "
"el Tiempo del comienzo (мс): %1 "
"Tiempo de la terminación (мс): %2 "
"Duración (мс): %3 "
""
"Por los pasos: "
"%4';ru='Пересчет всех служебных данных по новостям"
"Время начала (мс): %1"
"Время окончания (мс): %2"
"Длительность (мс): %3"
""
"По шагам:"
"%4';en='Recalculation of all news service data"
"Start time (ms):%1"
"End time (ms):%2"
"Duration (ms):%3"
""
...
"%4'")
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("es='Las novedades. El servicio y el reglamento. ПересчетВсехСлужебныхДанныхПоНовостям';ru='Новости. Сервис и регламент. ПересчетВсехСлужебныхДанныхПоНовостям';en='News. Service and regulations. ПересчетВсехСлужебныхДанныхПоНовостям'"), // ИдентификаторШага
