...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПустаяСтрока(ЭтотОбъект.Действие) Тогда
		...
		Сообщение.Текст = НСтр("es='la Acción';ru='Не заполнено поле Действие';en='Action field is not filled'");
	...
	Если ПустаяСтрока(ЭтотОбъект.УИНДействия) Тогда
		Отказ = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("es='de la hiperreferencia (debe llenarse automáticamente después de la elección de la Acción)';ru='Не заполнен идентификатор гиперссылки (должен заполниться автоматически после выбора Действия)';en='Hyperlink identifier is not filled (should be filled in automatically after selecting Actions)'");
	...
	Если ЭтотОбъект.Действие = "Переход по навигационной ссылке" Тогда
		Если ПустаяСтрока(ЭтотОбъект.ПараметрДействия_НавигационнаяСсылка) Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("es='la referencia de navegación';ru='Не заполнена навигационная ссылка';en='Navigation link is not filled'");
	...
	ИначеЕсли ЭтотОбъект.Действие = "Открытие раздела справки" Тогда
		Если ПустаяСтрока(ЭтотОбъект.ПараметрДействия_РазделСправки) Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("es='No es rellenada la sección de la información';ru='Не заполнен раздел справки';en='Help section is not filled'");
	...
	ИначеЕсли ЭтотОбъект.Действие = "Переход по интернет ссылке" Тогда
		Если ПустаяСтрока(ЭтотОбъект.ПараметрДействия_ИнтернетСсылка) Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("es='la referencia de Internet';ru='Не заполнена интернет-ссылка';en='Internet link is not filled'");
	...
	ИначеЕсли ЭтотОбъект.Действие = "Показать картинку" Тогда
		Если ПустаяСтрока(ЭтотОбъект.КартинкаЗаголовок) Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("es='de la estampa';ru='Не заполнен заголовок картинки';en='The title of the picture is not filled'");
		...
		Если ПустаяСтрока(ЭтотОбъект.КартинкаДанные) Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("es='la estampa. Presionen <Escoger la estampa del fichero>';ru='Не загружена картинка. Нажмите <Выбрать картинку из файла>';en='Not uploaded picture. Click <Select image from file>'");
			Сообщение.Поле  = "КартинкаЗаголовок";
			Сообщение.Сообщить();
		КонецЕсли;
...
Процедура ПараметрДействия_Оповещение_ПараметрыЗначениеПараметраНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТекущиеДанные.ИмяПараметра = "МаркерРазвернуто" Тогда // Идентификатор.
			СтандартнаяОбработка = Ложь;
			ДанныеВыбора = Новый СписокЗначений;
			ДанныеВыбора.Добавить(НСтр("es='Es desenvuelto';ru='развернуто';en='expanded'"));
			ДанныеВыбора.Добавить(НСтр("es='Doblar';ru='свернуть';en='collapse'"));
			ДанныеВыбора.Добавить("[-]");
		ИначеЕсли ТекущиеДанные.ИмяПараметра = "МаркерСвернуто" Тогда // Идентификатор.
			СтандартнаяОбработка = Ложь;
			ДанныеВыбора = Новый СписокЗначений;
			ДанныеВыбора.Добавить(НСтр("es='Es doblado';ru='свернуто';en='collapsed'"));
			ДанныеВыбора.Добавить(НСтр("es='Desenvolver';ru='развернуть';en='expand'"));
			ДанныеВыбора.Добавить("[+]");
		КонецЕсли;
	КонецЕсли;
