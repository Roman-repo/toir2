...
Процедура КомандаПоиск(Команда)
	...
	Для каждого ТекущаяПапка Из ЭтотОбъект.Папки.ПолучитьЭлементы() Цикл
		...
		Если ТекущаяПапка.ВариантОтбора = 2 Тогда // Результаты поиска
			...
			Если ПустаяСтрока(ЭтотОбъект.СтрокаПоиска) Тогда
				ТекущаяПапка.Представление = НСтр("es='Los resultados de la búsqueda';ru='Результаты поиска';en='Search results'");
			Иначе
				ТекущаяПапка.Представление = НСтр("es='Los resultados de la búsqueda';ru='Результаты поиска';en='Search results'") + " (" + СокрЛП(ЭтотОбъект.СтрокаПоиска) + ")";
			КонецЕсли;
...
Процедура ОбновитьИнформационныеСтрокиКлиент()
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La última renovación de la lista de las novedades era "
"las %1 "
"listas, que Sigue la renovación, de las novedades pasará "
""
"%2 %3';ru='Последнее обновление списка новостей было"
"%1"
"Следующее обновление списка новостей произойдет"
"%2"
"%3';en='The latest news list update was"
"%one"
"The next news list update will happen."
"%2"
"%3 '"),
...
Процедура ОбновитьИнформационныеСтрокиСервер()
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='%1, la última novedad de %2, la última carga %3';ru='%1, последняя новость от %2, последняя загрузка %3';en='% 1, latest news from% 2, last download% 3'"),
	...
	Если СтрДлина(ЭтотОбъект.ДатыПоследнихЗагрузокНовостей) > 0 Тогда
		ЭтотОбъект.ДатыПоследнихЗагрузокНовостей =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				Символы.ПС
					+ НСтр("es='El estado de la carga de las novedades: %1';ru='Состояние загрузки новостей: %1';en='News download status:%1'"),
				ЭтотОбъект.ДатыПоследнихЗагрузокНовостей);
	КонецЕсли;
...
Процедура УправлениеФормой(Знач Форма)
	...
	Если ПолнотекстовыйПоиск.ПолучитьРежимПолнотекстовогоПоиска() = РежимПолнотекстовогоПоиска.Разрешить Тогда
		ТекстПодсказки = НСтр("es='La búsqueda polnotekstovyy es incluida.';ru='Полнотекстовый поиск включен.';en='Full text search enabled.'");
	Иначе
		ТекстПодсказки = НСтр("es='La búsqueda polnotekstovyy es desconectada.';ru='Полнотекстовый поиск отключен.';en='Full text search disabled.'");
	КонецЕсли;
	ТекстПодсказки = ТекстПодсказки + Символы.ПС + Символы.ПС
		+ НСтр("es='Es accesible el uso подстановочных de los signos: "
"el % - cualquier consecuencia de los símbolos; "
"_ (подчерк) - cualquier símbolo; "
"[а-я] - los símbolos de y hasta mí; "
"[^a-yo] - los símbolos, excluyendo de y hasta mí; "
"\- el símbolo especial, преобразующий подстановочный el signo en el símbolo simple de la búsqueda.';ru='Доступно использование подстановочных знаков:"
"% - любая последовательность символов;"
"_ (подчерк) - любой символ;"
"[а-я] - символы от а до я;"
"[^а-я] - символы, исключая от а до я;"
"\ - спецсимвол, преобразующий подстановочный знак в простой символ поиска.';en='Use of wildcards is available:"
"% - any sequence of characters;"
"_ (underline) - any character;"
"[а-я] - characters from a to z;"
"[^ а-я] - characters excluding a to z;"
"\ is a special character that converts a wildcard to a simple search character.'");
...
Процедура ОбновитьСписокПапокОтбора()
	...
	НоваяСтрока.Наименование           = НСтр("es='Todas las novedades';ru='Все новости';en='All news'");
	...
	НоваяСтрока.ЗначениеОтбора         = Неопределено;
	Если ПустаяСтрока(ЭтотОбъект.СтрокаПоиска) Тогда
		НоваяСтрока.Наименование = НСтр("es='Los resultados de la búsqueda';ru='Результаты поиска';en='Search results'");
	Иначе
		НоваяСтрока.Наименование =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Los resultados de la búsqueda (%1)';ru='Результаты поиска (%1)';en='Search results (%1)'"),
	...
	НоваяСтрока.ЗначениеОтбора         = Неопределено;
	НоваяСтрока.Наименование           = НСтр("es='Las novedades marcadas';ru='Помеченные новости';en='Starred news'");
	НоваяСтрока.КоличествоНепрочтенных = 0; //// ... Всегда = 0, т.к. расчет количества непрочтенных для этой ситуации будет ресурсоемким
	НоваяСтрока.ВыделеннаяСтрока       = Ложь;
	...
	НоваяСтрока.ЗначениеОтбора         = Неопределено;
	НоваяСтрока.Наименование           = НСтр("es='Las novedades importantes';ru='Важные новости';en='Important news'");
	НоваяСтрока.КоличествоНепрочтенных = 0; //// ... Всегда = 0, т.к. расчет количества непрочтенных для этой ситуации будет ресурсоемким
	НоваяСтрока.ВыделеннаяСтрока       = Ложь;
	...
	НоваяСтрока.ЗначениеОтбора         = Неопределено;
	НоваяСтрока.Наименование           = НСтр("es='La cesta';ru='Корзина';en='Basket'");
...
Процедура УстановитьУсловноеОформление()
	...
	// 5. Представление даты: сегодня.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='Hoy';ru='Сегодня';en='Today'"));
	...
	// 6. Представление даты: вчера.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='Ayer';ru='Вчера';en='Yesterday'"));
