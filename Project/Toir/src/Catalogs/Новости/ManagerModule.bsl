...
Процедура ЗагрузитьСтандартныеЗначения(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИдентификаторШага = НСтр("es='Las novedades. El servicio y el reglamento. La carga de los significados estandartizados. %ИмяСвойства del %. El comienzo';ru='Новости. Сервис и регламент. Загрузка стандартных значений. %ИмяСвойства%. Начало';en='News. Service and regulations. Loading standard values. %ИмяСвойства%. Start'");
		...
		Если ВыполнениеРазрешено = Истина Тогда
			...
			Иначе
				ЛогШаговВыполнения.Добавить(
					НСтр("es='La falta de la carga de los significados estandartizados de las novedades de las maquetas. La guía de las cintas de las novedades vacío.';ru='Ошибка загрузки стандартных значений новостей из макетов. Справочник лент новостей пустой.';en='Error loading standard news values from layouts. The news feed directory is empty.'"));
						...
						Иначе
							ЛогШаговВыполнения.Добавить(
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("es='La falta de la carga de los significados estandartizados de las novedades de la maqueta %1. No es encontrada la cinta de las novedades con tal "
"identificador.';ru='Ошибка загрузки стандартных значений новостей из макета %1."
"Не найдена лента новостей с таким идентификатором.';en='Error loading default news value from layout%1."
"No feed found with this identifier.'"),
									ТекущийМакет.Имя));
						КонецЕсли;
					КонецЕсли;
				КонецЦикла;
							...
							Если (ТипЗнч(СтрокаСтандартныхЗначений.Значение) = ТипСтрока)
								...
								Исключение
									...
									ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("es='La falta de la carga de los significados adicionales estandartizados de las novedades por la causa: "
"%1 "
"Primeros 2000 símbolos del contenido adicional: "
"%2';ru='Ошибка загрузки дополнительных стандартных значений новостей по причине:"
"%1"
"Первые 2000 символов дополнительного содержимого:"
"%2';en='Error loading additional standard news values due to:"
"%1                                                     First 2000 characters of additional content:"
"%2'"),
