...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДобавлениеВСписок Тогда
		...
		Элементы.Выбрать.Заголовок = НСтр("es='Añadir';ru='Добавить';en='Add'");
		
		Элементы.ПояснениеУсиленногоПароля.Заголовок =
			НСтр("es='Presionen Añadir para pasar a la introducción de la consigna.';ru='Нажмите Добавить, чтобы перейти к вводу пароля.';en='Click Add to proceed to enter the password.'");
		
		ЛичныйСписокПриДобавлении = Параметры.ЛичныйСписокПриДобавлении;
		Элементы.ПоказыватьВсе.Подсказка =
			НСтр("es='Mostrar todos los certificados sin selección (por ejemplo, incluso añadido y caducado)';ru='Показать все сертификаты без отбора (например, включая добавленные и просроченные)';en='Show all certificates without selection (for example, including added and expired)'");
	КонецЕсли;
	...
	Если ДляШифрованияИРасшифровки = Истина Тогда
		Если Параметры.ДобавлениеВСписок Тогда
			Заголовок = НСтр("es='La adición del certificado para la cifración y el desciframiento de los datos';ru='Добавление сертификата для шифрования и расшифровки данных';en='Adding a certificate to encrypt and decrypt data'");
		Иначе
			Заголовок = НСтр("es='La elección del certificado para la cifración y el desciframiento de los datos';ru='Выбор сертификата для шифрования и расшифровки данных';en='Choosing a certificate for encrypting and decrypting data'");
		КонецЕсли;
	ИначеЕсли ДляШифрованияИРасшифровки = Ложь Тогда
		Если Параметры.ДобавлениеВСписок Тогда
			Заголовок = НСтр("es='La adición del certificado para la firma de los datos';ru='Добавление сертификата для подписания данных';en='Adding a certificate for signing data'");
		КонецЕсли;
	ИначеЕсли ЭлектроннаяПодпись.ИспользоватьШифрование() Тогда
		Заголовок = НСтр("es='La adición del certificado para la firma y la cifración de los datos';ru='Добавление сертификата для подписания и шифрования данных';en='Adding a certificate for signing and encrypting data'");
	Иначе
		Заголовок = НСтр("es='La adición del certificado para la firma de los datos';ru='Добавление сертификата для подписания данных';en='Adding a certificate for signing data'");
	КонецЕсли;
	...
	Если ЭлектроннаяПодпись.СоздаватьЭлектронныеПодписиНаСервере() Тогда
		Элементы.ГруппаСертификаты.Заголовок =
			НСтр("es='Los certificados personales en el ordenador y el servidor';ru='Личные сертификаты на компьютере и сервере';en='Personal certificates on the computer and server'");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Элементы.СертификатОрганизация.Видимость
	   ...
	   И Не ЗначениеЗаполнено(СертификатОрганизация) Тогда
		...
		ТекстСообщения = НСтр("es='Campo Organizatsiya no es rellenado.';ru='Поле Организация не заполнено.';en='The Organization field is not filled.'");
...
Процедура СертификатыНедоступныНаКлиентеНадписьНажатие(Элемент)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("es='Los certificados en el ordenador';ru='Сертификаты на компьютере';en='Computer certificates'"), "", ОшибкаПолученияСертификатовНаКлиенте, Новый Структура);
...
Процедура СертификатыНедоступныНаСервереНадписьНажатие(Элемент)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("es='Los certificados en el servidor';ru='Сертификаты на сервере';en='Certificates on server'"), "", ОшибкаПолученияСертификатовНаСервере, Новый Структура);
...
Процедура ВыбратьПослеПроверкиСертификата(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Если ДляШифрованияИРасшифровки = Истина Тогда
			ЗаголовокФормы = НСтр("es='La comprobación de la cifración y el desciframiento';ru='Проверка шифрования и расшифровки';en='Encryption and decryption check'");
		Иначе
			ЗаголовокФормы = НСтр("es='La comprobación de la instalación de la firma electrónica';ru='Проверка установки электронной подписи';en='Checking of installation of electronic signature'");
		КонецЕсли;
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
...
Процедура ПерейтиКВыборуТекущегоСертификата(Оповещение)
	...
	Если Элементы.Сертификаты.ТекущиеДанные = Неопределено Тогда
		Результат.ОписаниеОшибки = НСтр("es='Distingan el certificado, que se usará.';ru='Выделите сертификат, который будет использоваться.';en='Highlight the certificate to be used.'");
	...
	Если ТекущиеДанные.ЭтоЗаявление Тогда
		...
		Результат.ОписаниеОшибки =
			НСтр("es='Para este certificado la declaración a la salida no es cumplida. "
"Abran la declaración a la salida del certificado y cumplan los pasos exigidos.';ru='Для этого сертификата заявление на выпуск еще не исполнено."
"Откройте заявление на выпуск сертификата и выполните требуемые шаги.';en='For this certificate, the application for issue has not yet been executed."
"Open the certificate application and follow the required steps.'");
		ВыполнитьОбработкуОповещения(Оповещение, Результат);
		Возврат;
	КонецЕсли;
	...
	Если СертификатНаСервере Тогда
		...
		Иначе
			Результат.ОписаниеОшибки = НСтр("es='El certificado no es encontrado en el servidor (es quitado probablemente).';ru='Сертификат не найден на сервере (возможно удален).';en='Certificate not found on server (possibly deleted).'");
			Результат.ОбновитьСписокСертификатов = Истина;
			ВыполнитьОбработкуОповещения(Оповещение, Результат);
		КонецЕсли;
		Возврат;
...
Процедура ПерейтиКВыборуТекущегоСертификатаПослеПоискаСертификата(РезультатПоиска, Контекст) Экспорт
	...
	Если ТипЗнч(РезультатПоиска) <> Тип("СертификатКриптографии") Тогда
		Если РезультатПоиска.Свойство("СертификатНеНайден") Тогда
			Контекст.Результат.ОписаниеОшибки = НСтр("es='El certificado no es encontrado en el ordenador (es quitado probablemente).';ru='Сертификат не найден на компьютере (возможно удален).';en='Certificate not found on computer (possibly deleted).'");
		Иначе
...
Процедура ПерейтиКВыборуТекущегоСертификатаПослеПоискаСертификатаВОблачномСервисе(РезультатПоиска, Контекст) Экспорт
	...
	Если Не РезультатПоиска.Выполнено
		...
		Контекст.Результат.ОписаниеОшибки = ?(РезультатПоиска.Выполнено,
			НСтр("es='El certificado no es encontrado en el servicio nuboso.';ru='Сертификат не найден в облачном сервисе.';en='Certificate not found in cloud service.'"), РезультатПоиска.ОписаниеОшибки.Описание);
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Контекст.Результат);
		Возврат;
...
Процедура ПерейтиКВыборуТекущегоСертификатаПослеЗаполненияСвойствСертификата(Контекст)
	...
	Если ДобавлениеВСписок Тогда
		Строка = ?(ЗначениеЗаполнено(Сертификат), НСтр("es='Renovar';ru='Обновить';en='Update'"), НСтр("es='Añadir';ru='Добавить';en='Add'"));
...
Процедура ПроверитьСертификатПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("МенеджерКриптографии")
		...
		Если ДляШифрованияИРасшифровки = Истина Тогда
			Результат.Вставить("ЗаголовокОшибки", НСтр("es='No consiguió pasar la comprobación de la cifración por la causa:';ru='Не удалось пройти проверку шифрования по причине:';en='Failed to pass encryption verification due to:'"));
		Иначе
			Результат.Вставить("ЗаголовокОшибки", НСтр("es='No consiguió pasar la comprobación de la firma por la causa:';ru='Не удалось пройти проверку подписания по причине:';en='Failed to complete signing verification due to:'"));
		КонецЕсли;
...
Функция ПроверитьСертификатИЗаписатьВСправочник(Знач ЗначениеПароля, ОшибкаНаСервере)
	...
	Если ЭлектроннаяПодписьСлужебный.МенеджерКриптографии("", Ложь, ОшибкаНаСервере) = Неопределено
	   И ОшибкаНаСервере.Общая Тогда
		...
		Если ДляШифрованияИРасшифровки = Истина Тогда
			ОшибкаНаСервере.Вставить("ЗаголовокОшибки", НСтр("es='No consiguió pasar la comprobación de la cifración por la causa:';ru='Не удалось пройти проверку шифрования по причине:';en='Failed to pass encryption verification due to:'"));
		Иначе
			ОшибкаНаСервере.Вставить("ЗаголовокОшибки", НСтр("es='No consiguió pasar la comprobación de la firma por la causa:';ru='Не удалось пройти проверку подписания по причине:';en='Failed to complete signing verification due to:'"));
		КонецЕсли;
		Возврат Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьОшибкуШифрования(Ошибка, ОписаниеОшибки, ОписаниеПрограммы, ПредставлениеОшибки)
	...
	ОписаниеОшибки = ОписаниеОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='No consiguió pasar la comprobación de la cifración por medio del programa %1 por la causa: "
"%2';ru='Не удалось пройти проверку шифрования с помощью программы %1 по причине:"
"%2';en='Failed to pass encryption check using program %1 due to:"
"%2 '"),
...
Процедура ЗаполнитьОшибкуРасшифровки(Ошибка, ОписаниеОшибки, ОписаниеПрограммы, ПредставлениеОшибки)
	...
	ОписаниеОшибки = ОписаниеОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='No consiguió pasar la comprobación del desciframiento por medio del programa %1 por la causa: "
"%2';ru='Не удалось пройти проверку расшифровки с помощью программы %1 по причине:"
"%2';en='Failed to pass decryption check using program %1 due to:"
"%2 '"),
		ОписаниеПрограммы.Наименование,
		ПредставлениеОшибки);
...
Процедура ЗаполнитьОшибкуПодписания(Ошибка, ОписаниеОшибки, ОписаниеПрограммы, ПредставлениеОшибки, ПустыеДанные)
	...
	ОписаниеОшибки = ОписаниеОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='No consiguió pasar la comprobación de la firma por medio del programa %1 por la causa: "
"%2';ru='Не удалось пройти проверку подписания с помощью программы %1 по причине:"
"%2';en='Failed to complete signing verification using program%1 due to:"
"%2 '"),
		ОписаниеПрограммы.Наименование,
		ПредставлениеОшибки);
