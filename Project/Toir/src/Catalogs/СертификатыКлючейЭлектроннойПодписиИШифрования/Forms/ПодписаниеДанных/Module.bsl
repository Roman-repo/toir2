...
Процедура ПодписатьДанные(Оповещение)
	...
	Если СертификатДействителенДо < ОбщегоНазначенияКлиент.ДатаСеанса() Тогда
		Контекст.ОшибкаНаКлиенте.Вставить("ОписаниеОшибки",
			НСтр("es='Cerca del certificado escogido ha cumplido la validez. "
"Escojan otro certificado.';ru='У выбранного сертификата истек срок действия."
"Выберите другой сертификат.';en='The selected certificate has expired."
...
"Escojan otro certificado.';ru='У выбранного сертификата не указана программа для закрытого ключа."
"Выберите другой сертификат.';en='The selected certificate does not have a private key program."
"Please select a different certificate.'"));
		ПоказатьОшибку(Контекст.ОшибкаНаКлиенте, Контекст.ОшибкаНаСервере);
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Ложь);
		Возврат;
...
Процедура ПоказатьОшибку(ОшибкаНаКлиенте, ОшибкаНаСервере, НеподписанныеДанные = Неопределено)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("es='No consiguió firmar los datos';ru='Не удалось подписать данные';en='Failed to sign data'"), "", 
