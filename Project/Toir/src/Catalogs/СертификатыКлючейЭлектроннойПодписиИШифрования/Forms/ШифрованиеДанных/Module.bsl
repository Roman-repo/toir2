		...
		Для Каждого Строка Из Строки Цикл
			...
			Если ТипЗнч(ДанныеСертификата) <> Тип("ДвоичныеДанные") Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Los datos del certificado ""%1"" no son encontrados en la guía';ru='Данные сертификата ""%1"" не найдены в справочнике';en='Certificate data ""%1"" not found in the directory'"), Выборка.Представление);
			КонецЕсли;
			Попытка
			...
			Исключение
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Los datos del certificado ""%1"" en la guía no son correcto por la causa: "
"%2';ru='Данные сертификата ""%1"" в справочнике не корректны по причине:"
"%2';en='Certificate data ""%1"" ""in the directory is not correct due to: %2'"),
	...
	Если ЗначениеЗаполнено(Сертификат) Тогда
		Если СертификатДействителенДо < ОбщегоНазначенияКлиент.ДатаСеанса() Тогда
			Контекст.ОшибкаНаКлиенте.Вставить("ОписаниеОшибки",
				НСтр("es='Cerca del certificado escogido personal ha cumplido la validez. "
"Escojan otro certificado.';ru='У выбранного личного сертификата истек срок действия."
"Выберите другой сертификат.';en='The selected personal certificate has expired."
"Please select a different certificate.'"));
		...
		Если Не ЗначениеЗаполнено(СертификатПрограмма) Тогда
			Контекст.ОшибкаНаКлиенте.Вставить("ОписаниеОшибки",
				НСтр("es='Cerca del certificado escogido personal no es indicado el programa para la llave cerrada. "
"Escojan otro certificado.';ru='У выбранного личного сертификата не указана программа для закрытого ключа."
"Выберите другой сертификат.';en='The selected personal certificate does not have a private key program."
"Please select a different certificate. '"));
			ПоказатьОшибку(Контекст.ОшибкаНаКлиенте, Контекст.ОшибкаНаСервере);
			ВыполнитьОбработкуОповещения(Контекст.Оповещение, Ложь);
			Возврат;
...
Функция СвойстваСертификатов(Знач Ссылки, Знач ИдентификаторФормы)
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеСертификата = Выборка.ДанныеСертификата.Получить();
		Если ТипЗнч(ДанныеСертификата) <> Тип("ДвоичныеДанные") Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Los datos del certificado ""%1"" no son encontrados en la guía';ru='Данные сертификата ""%1"" не найдены в справочнике';en='Certificate data ""% 1"" not found in the directory'"),
...
Процедура ЗаписатьСертификатыШифрованияНаСервере(ОписаниеОбъектов, АдресСертификатов, ИдентификаторФормы, Ошибка)
	...
	Исключение
		ОтменитьТранзакцию();
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		Ошибка.Вставить("ОписаниеОшибки", НСтр("es='A la anotación de los certificados de la cifración ha surgido la falta:';ru='При записи сертификатов шифрования возникла ошибка:';en='An error occurred while writing encryption certificates:'")
...
Процедура ПоказатьОшибку(ОшибкаНаКлиенте, ОшибкаНаСервере)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("es='No consiguió cifrar los datos';ru='Не удалось зашифровать данные';en='Failed to encrypt data'"), "",
