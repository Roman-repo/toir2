...
// АПК:78-вкл: для безопасной передачи данных на клиенте между формами, не отправляя их на сервер.
	...
	Если СтрНайти(ВидыФорм, "," + ВидФормы + ",") = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta en el procedimiento de OtkrytNovuyuFormu. ВидФормы ""%1"" no es apoyado.';ru='Ошибка в процедуре ОткрытьНовуюФорму. ВидФормы ""%1"" не поддерживается.';en='Error in procedure ОткрытьНовуюФорму. ВидФормы ""%1 not supported.'"),
			ВидФормы);
	КонецЕсли;
