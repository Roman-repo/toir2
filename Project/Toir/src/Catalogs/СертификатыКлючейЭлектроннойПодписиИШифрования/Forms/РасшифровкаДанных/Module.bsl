...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоАутентификация Тогда
		Элементы.ФормаРасшифровать.Заголовок = НСтр("es='ОК';ru='ОК';en='OK'");
		Элементы.ПояснениеУсиленногоПароля.Заголовок = НСтр("es='Presionen ОК para pasar a la introducción de la consigna.';ru='Нажмите ОК, чтобы перейти к вводу пароля.';en='Click OK to proceed to password entry.'");
	КонецЕсли;
...
Процедура РасшифроватьДанные(Оповещение)
	...
	Если Не ЗначениеЗаполнено(СертификатПрограмма) Тогда
		Контекст.ОшибкаНаКлиенте.Вставить("ОписаниеОшибки",
			НСтр("es='Cerca del certificado escogido no es indicado el programa para la llave cerrada. "
"Escojan el certificado repetidamente de la lista completa o "
"abran el certificado e indiquen el programa a mano.';ru='У выбранного сертификата не указана программа для закрытого ключа."
"Выберите сертификат повторно из полного списка или"
"откройте сертификат и укажите программу вручную.';en='The selected certificate does not have a private key program."
"Please select the certificate again from the full list or"
"open the certificate and specify the program manually.'"));
...
Процедура ЗаписатьСертификатыШифрованияНаСервере(ОписаниеОбъектов, ИдентификаторФормы, Ошибка)
	...
	Исключение
		...
		Ошибка.Вставить("ОписаниеОшибки", НСтр("es='A la limpieza de los certificados de la cifración ha surgido la falta:';ru='При очистке сертификатов шифрования возникла ошибка:';en='An error occurred while cleaning the encryption certificates:'")
...
Процедура ПоказатьОшибку(ОшибкаНаКлиенте, ОшибкаНаСервере)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("es='No consiguió descifrar los datos';ru='Не удалось расшифровать данные';en='Failed to decrypt data'"), "",
