...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПроверкаПриВыборе Тогда
		Элементы.ФормаПроверить.Заголовок = НСтр("es='Comprobar y continuar';ru='Проверить и продолжить';en='Check and continue'");
	...
	Если Не СтандартныеПроверки Тогда
		Если ДополнительныеПроверки.Количество() = 0 Тогда
			ВызватьИсключение
				НСтр("es='Para la comprobación del certificado son desconectadas las comprobaciones estandartizadas, "
"además las comprobaciones adicionales no es indicado.';ru='Для проверки сертификата отключены стандартные проверки,"
"при этом дополнительных проверок не указано.';en='Standard checks are disabled for certificate verification,"
"there’s no further verification.'");
		КонецЕсли;
...
Процедура ПоказатьПредупреждениеОНевозможностиПродолжения()
	...
	ПоказатьПредупреждение(,
		НСтр("es='No consiguió continuar, porque son pasadas no todas las comprobaciones exigidas.';ru='Не удалось продолжить, т.к. пройдены не все требуемые проверки.';en='Failed to continue because Not all required checks passed.'"));
...
Процедура ПроверитьНаСторонеКлиентаПослеИнициализацииСертификата(СертификатКриптографии, Контекст) Экспорт
	...
	Если Не Элементы.ГруппаЗаконныйСертификат.Видимость
	 Или Контекст.СертификатКриптографии.Субъект.Свойство("SN") Тогда
		...
		ОписаниеОшибки = "";
	Иначе
		ОписаниеОшибки = НСтр("es='En la descripción del sujeto del certificado no es encontrado el campo ""SN"".';ru='В описании субъекта сертификата не найдено поле ""SN"".';en='The ""SN"" field was not found in the description of the certificate subject.'");
	КонецЕсли;
...
Процедура ПроверитьНаСторонеКлиентаПослеПоискаСертификата(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("СертификатКриптографии") Тогда
		ОписаниеОшибки = Результат.ОписаниеОшибки + Символы.ПС + Символы.ПС
			+ НСтр("es='La comprobación de la firma, la firma creada y el desciframiento no puede ser cumplida.';ru='Проверка подписания, созданной подписи и расшифровки не могут быть выполнены.';en='Checking of signatures, created signatures and decryption cannot be performed.'");
	Иначе
		ОписаниеОшибки = "";
	КонецЕсли;
...
Процедура ПроверитьНаСторонеКлиентаПослеПоискаСертификатаВМоделиСервиса(Результат, Контекст) Экспорт
	...
	ИначеЕсли Результат.Сертификат = Неопределено Тогда
		ОписаниеОшибки = НСтр("es='El certificado no es encontrado en el depósito de los certificados.';ru='Сертификат не найден в хранилище сертификатов.';en='Certificate not found in certificate store.'") + Символы.ПС
			+ НСтр("es='La comprobación de la firma, la firma creada y el desciframiento no puede ser cumplida.';ru='Проверка подписания, созданной подписи и расшифровки не могут быть выполнены.';en='Checking of signatures, created signatures and decryption cannot be performed.'");
	Иначе
		ОписаниеОшибки = "";
	КонецЕсли;
...
Процедура ПроверитьНаСторонеКлиентаПослеПроверкиСертификата(Результат, Контекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("es='El programa para el uso de la llave cerrada no es indicado en el certificado.';ru='Программа для использования закрытого ключа не указана в сертификате.';en='The program for using the private key is not indicated in the certificate.'");
...
Процедура ПроверитьНаСторонеКлиентаПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = Результат + Символы.ПС + Символы.ПС
			+ НСтр("es='La comprobación de la firma, la firma creada, la cifración y "
"el desciframiento no puede ser cumplida.';ru='Проверка подписания, созданной подписи, шифрования и"
"расшифровки не могут быть выполнены.';en='Checking of signature, created signature, encryption and"
...
Процедура УстановитьЭлемент(Форма, НачалоЭлемента, НаСервере, ОписаниеОшибки = Неопределено, ЭтоОшибка = Ложь)
	...
	Если ОписаниеОшибки = Неопределено Тогда
		...
		ЭлементКартинка.Подсказка   = НСтр("es='La comprobación no se cumplía.';ru='Проверка не выполнялась.';en='Checking was not performed.'");
	...
	Иначе
		...
		ЭлементКартинка.Подсказка   = НСтр("es='La comprobación es cumplida con éxito.';ru='Проверка выполнена успешно.';en='Checking completed successfully.'");;
