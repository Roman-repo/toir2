...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого КлючИЗначение Из ПараметрыРеквизитов Цикл
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Campo %1 no es rellenado.';ru='Поле %1 не заполнено.';en='Field %1 is empty.'"),
...
Процедура ПроверитьСертификат(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ПоказатьПредупреждение(, НСтр("es='El certificado no es anotado.';ru='Сертификат еще не записан.';en='The certificate has not yet been recorded.'"));
		Возврат;
	КонецЕсли;
...
Процедура СертификатОтозван(Команда)
	...
	Если Объект.Отозван Тогда
		ПоказатьПредупреждение(, НСтр("es='Después de la anotación anular la revocación será imposible.';ru='После записи отменить отзыв будет невозможно.';en='After recording, it will be impossible to cancel the review.'"));
	КонецЕсли;
