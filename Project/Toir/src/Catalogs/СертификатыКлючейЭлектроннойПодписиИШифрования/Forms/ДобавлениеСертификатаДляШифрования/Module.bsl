...
Процедура Далее(Команда)
	...
	Если Элементы.Сертификаты.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("es='Distingan los certificados, que es necesario añadir.';ru='Выделите сертификаты, которые требуется добавить.';en='Highlight the certificates you want to add.'"));
		Возврат;
	КонецЕсли;
...
Процедура ДалееПослеПоискаСертификата(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("СертификатНеНайден") Тогда
		Контекст.Вставить("ОписаниеОшибки", НСтр("es='El certificado no es encontrado en el ordenador (es quitado probablemente).';ru='Сертификат не найден на компьютере (возможно удален).';en='Certificate not found on computer (possibly deleted).'"));
	Иначе
...
Процедура ДалееПослеПоискаСертификатаВОблачномСервисе(Результат, Контекст) Экспорт
	...
	Если Не Результат.Выполнено Тогда
		Контекст.Вставить("ОписаниеОшибки", НСтр("es='El certificado no es encontrado en el servicio nuboso (es quitado probablemente).';ru='Сертификат не найден в облачном сервисе (возможно удален).';en='Certificate not found in cloud service (possibly deleted).'"));
	Иначе
		Контекст.Вставить("ОписаниеОшибки", Результат.ОписаниеОшибки);
	КонецЕсли;
...
Процедура ПоказатьСтраницуУточнениеСвойствСертификата(Форма, СертификатКриптографии, ДанныеСертификата, СвойстваСертификата)
	...
	Строка = ?(ЗначениеЗаполнено(Форма.Сертификат), НСтр("es='Renovar';ru='Обновить';en='Update'"), НСтр("es='Añadir';ru='Добавить';en='Add'"));
