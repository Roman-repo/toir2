...
Процедура УдалитьЛишниеФайлы(Команда)
	...
	Если КоличествоЛишнихФайлов = 0 Тогда
		ПоказатьПредупреждение(, НСтр("es='No existe ni un fichero superfluo sobre el disco';ru='Нет ни одного лишнего файла на диске';en='There is no extra file on disk'"));
		Возврат;
	КонецЕсли;
...
Процедура ПодключениеРасширенияРаботыСФайламиЗавершение(РасширениеПодключено, ДополнительныеПараметры) Экспорт
	...
	Если Не РасширениеПодключено Тогда
		ПоказатьПредупреждение(, НСтр("en='The file extension is not installed. You cannot work with files with an unspecified extension in the web client.';es='La ampliación del trabajo con los ficheros no es establecida. El trabajo con los ficheros con la ampliación no establecida en веб el cliente es imposible.';ru='Расширение работы с файлами не установлено. Работа с файлами с неустановленным расширением в веб клиенте невозможна.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПроверкаСуществованияКаталогаЗавершение(Существует, ДополнительныеПараметры) Экспорт
	...
	Если Не Существует Тогда
		ПоказатьПредупреждение(, НСтр("es='La vía al catálogo de la copia es incorrecta.';ru='Путь к каталогу копирования некорректен.';en='The path to the copy directory is invalid.'"));
	Иначе
...
Процедура ПослеОбработкиФайлов(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЧислоУдаленныхФайлов <> 0 Тогда
		ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es quitado los ficheros: %1';ru='Удалено файлов: %1';en='Deleted files:%1'"),
			ЧислоУдаленныхФайлов);
		ПоказатьОповещениеПользователя(
			НСтр("es='Es acabada la desaparición de los ficheros superfluos.';ru='Завершено удаление лишних файлов.';en='Completed deletion of unnecessary files.'"),
			,
			ТекстОповещения,
...
Процедура ЗаполнитьТаблицуЛишнихФайлов()
	...
	Для Каждого Файл Из МассивФайлов Цикл
		...
		НоваяСтрока.СтатусПроверки   = НСтр("es='Los ficheros superfluos (es sobre el disco, pero las noticias sobre ellos faltan)';ru='Лишние файлы (есть на диске, но сведения о них отсутствуют)';en='Extra files (there are on the disk, but there is no information about them)'");
	...
	МассивЛишнихФайлов = ТаблицаФайловНаДиске.НайтиСтроки(
		Новый Структура("СтатусПроверки", НСтр("es='Los ficheros superfluos (es sobre el disco, pero las noticias sobre ellos faltan)';ru='Лишние файлы (есть на диске, но сведения о них отсутствуют)';en='Extra files (there are on the disk, but there is no information about them)'")));
...
Процедура ОбработатьОшибкуПравДоступа(ИнформацияОбОшибке, ИсходноеОповещение)
	...
	ШаблонОшибки = НСтр("es='La vía del catálogo para la copia es incorrecta. "
"Probablemente la anotación de descuento, de parte de que trabaja "
"el servidor 1С:Предприятия, no tiene los derechos de admisión al catálogo indicado. "
""
"%1';ru='Путь каталога для копирования некорректен."
"Возможно учетная запись, от лица которой работает"
"сервер 1С:Предприятия, не имеет прав доступа к указанному каталогу."
""
"%1';en='The directory path for the copy is invalid."
"Perhaps the account on behalf of which it works"
"1C: Enterprise server, does not have access rights to the specified directory."
""
"%1'");
...
Процедура ОбработатьСообщениеОбОшибке(ИмяФайла, ИнформацияОбОшибке)
	...
	ЗаписьЖурналаРегистрации(НСтр("es='Los ficheros. La falta de la desaparición de los ficheros superfluos';ru='Файлы.Ошибка удаления лишних файлов';en='Files. Error deleting unnecessary files'", ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,,,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='A la desaparición del fichero de disco "
"""%1"" "
"ha surgido la falta: "
"""%2"".';ru='При удалении файла с диска"
"""%1"""
"возникла ошибка:"
"""%2"".';en='When deleting a file from disk"
"""%1"""
"an error occurred:"
"""%2"".'"),
			ИмяФайла,
...
Процедура СформироватьОтчетОбОшибках(ОтчетОбОшибках, МассивФайловСОшибками)
	...
	ОбластьЗаголовок.Параметры.Описание = НСтр("es='Los ficheros con las faltas:';ru='Файлы с ошибками:';en='Files with errors:'");
