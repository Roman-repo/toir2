...
Процедура ПередЗаписью(Отказ)
	...
	Если Владелец.ТекущаяВерсия = Ссылка Тогда
		Если ПометкаУдаления = Истина И Владелец.ПометкаУдаления <> Истина Тогда
			ВызватьИсключение НСтр("es='La versión activa no es posible quitar.';ru='Активную версию нельзя удалить.';en='The active version cannot be deleted.'");
		КонецЕсли;
	ИначеЕсли РодительскаяВерсия.Пустая() Тогда
		Если ПометкаУдаления = Истина И Владелец.ПометкаУдаления <> Истина Тогда
			ВызватьИсключение НСтр("es='La primera versión no es posible quitar.';ru='Первую версию нельзя удалить.';en='The first version cannot be removed.'");
		КонецЕсли;
	...
	Если ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
		Если НЕ Том.Пустая() И ОбщегоНазначения.СсылкаСуществует(Том) Тогда
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(НСтр("es='Los ficheros. La falta de la desaparición del fichero.';ru='Файлы.Ошибка удаления файла.';en='Files. Error deleting file.'", 
