...
Процедура ЗаполнитьВидыКонтактнойИнформации(Параметры) Экспорт
	...
	Пока ВидКонтактнойИнформацииСсылка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió tratar el tipo de la información de contacto: %1 por la causa: %2';ru='Не удалось обработать вид контактной информации: %1 по причине: %2';en='Failed to process contact information view: %1 due to: %2'"),
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Al procedimiento de ZapolnitVidyKontaktnoyInformatsii no consiguió tratar algunos tipos de la información de contacto (son dejados pasar): %1';ru='Процедуре ЗаполнитьВидыКонтактнойИнформации не удалось обработать некоторые виды контактной информации (пропущены): %1';en='A procedure ЗаполнитьВидыКонтактнойИнформации failed to process some types of contact information (skipped):%1'"), 
				ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			Метаданные.Справочники.ВидыКонтактнойИнформации,,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El procedimiento de ZapolnitVidyKontaktnoyInformatsii ha tratado la porción siguiente de tipos de la información de contacto: %1';ru='Процедура ЗаполнитьВидыКонтактнойИнформации обработала очередную порцию видов контактной информации: %1';en='A procedure ЗаполнитьВидыКонтактнойИнформации processed the next batch of types of contact information:%1'"),
					ОбъектовОбработано));
	КонецЕсли;
