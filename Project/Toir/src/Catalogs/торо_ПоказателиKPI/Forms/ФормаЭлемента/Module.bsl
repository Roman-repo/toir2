...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(ТекущийОбъект.Отчет) Тогда
		...
		Если обМетаданных = Неопределено Тогда
			ТекстСообщения = НСтр("es='¡El informe indicado no es encontrado!';ru='Указанный отчёт не найден!';en='The specified report was not found!'");
	...
	Если 	ПроверятьУникальностьИдентификатора Тогда
		Если ЗначениеЗаполнено(ТекущийОбъект.ИдентификаторДляФормул) Тогда
			...
			ШаблонСообщения = НСтр("es='El identificador para las fórmulas ""%1"" es usado ya en el índice ""%2"".';ru='Идентификатор для формул ""%1"" уже использован в показателе ""%2"".';en='The identifier for the formulas ""%1"" is already used in the metric ""%2"".'");
...
Процедура РедактироватьНастройкиСКД(Команда)
	...
	#Иначе
		ТекстСообщения = НСтр("es='Para redactar el esquema de la composición, es necesario poner en marcha la configuración en el régimen del cliente gordo.';ru='Для того чтобы редактировать схему компоновки, необходимо запустить конфигурацию в режиме толстого клиента.';en='In order to edit the layout, you must run the configuration in the thick client mode.'");
