...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЦикла;
	Элементы.Наименование.СписокВыбора.Добавить("", НСтр("es='<Otro programa>';ru='<Другая программа>';en='<Другая программа>'"));
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("es='El programa con el nombre indicado y el tipo es añadido ya en la lista.';ru='Программа с указанным именем и типом уже добавлена в список.';en='The program with the specified name and type has already been added to the list.'"),
			,
			"Объект.ИмяПрограммы");
	КонецЕсли;
...
Процедура УстановитьПометкуУдаления(Команда)
	...
	ПоказатьВопрос(
		...
		НСтр("es='Para la instalación de la marca de la desaparición es necesario anotar los cambios hechos. "
"¿Anotar datos?';ru='Для установки отметки удаления необходимо записать сделанные изменения."
"Записать данные?';en='To set the deletion mark, the changes made must be recorded."
"Record data? '"), РежимДиалогаВопрос.ДаНет);
