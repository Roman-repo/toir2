...
Процедура ДобавитьНастройкиMicrosoftEnhancedCSP(Настройки) Экспорт
	...
	Настройка.Представление       = НСтр("es='Microsoft Enhanced CSP';ru='Microsoft Enhanced CSP';en='Microsoft Enhanced CSP'");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		...
		НСтр("es='Al procedimiento Справочники.ОбработатьДанныеДляПереходаНаНовуюВерсию.ОбработатьДанныеДляПереходаНаНовуюВерсию no consiguió tratar algunos programas de la firma electrónica (son dejados pasar): %1';ru='Процедуре Справочники.ОбработатьДанныеДляПереходаНаНовуюВерсию.ОбработатьДанныеДляПереходаНаНовуюВерсию не удалось обработать некоторые программы электронной подписи (пропущены): %1';en='A procedure Справочники.ОбработатьДанныеДляПереходаНаНовуюВерсию.ОбработатьДанныеДляПереходаНаНовуюВерсию failed to process some e-signature programs (skipped):%1'"), 
		ПроблемныхОбъектов);
	...
	Иначе
		...
		НСтр("es='El procedimiento Справочники.ОбработатьДанныеДляПереходаНаНовуюВерсию.ОбработатьДанныеДляПереходаНаНовуюВерсию ha tratado la porción siguiente de programas de la firma electrónica: %1';ru='Процедура Справочники.ОбработатьДанныеДляПереходаНаНовуюВерсию.ОбработатьДанныеДляПереходаНаНовуюВерсию обработала очередную порцию программ электронной подписи: %1';en='A procedure Справочники.ОбработатьДанныеДляПереходаНаНовуюВерсию.ОбработатьДанныеДляПереходаНаНовуюВерсию processed the next batch of electronic signature programs:%1'"),
		ОбъектовОбработано));
	КонецЕсли;
...
Процедура ОбновитьПредставлениеПрограммыОтложенно(Программа, ОбъектовОбработано, ПроблемныхОбъектов, ПоставляемыеНастройки)
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió renovar el programa ""%1"" por la causa: "
"%2';ru='Не удалось обновить программу ""%1"" по причине:"
"%2';en='Failed to update program ""%1"" due to:"
...
Процедура ДозаполнитьНачальныеНастройкиОтложенно(ОбъектовОбработано, ПроблемныхОбъектов)
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió дозаполнить los ajustes iniciales de los programas por la causa: "
"%1';ru='Не удалось дозаполнить начальные настройки программ по причине:"
"%1';en='Failed to replenish the initial program settings due to:"
"%one'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
