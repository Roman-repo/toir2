...
Процедура ПередЗаписью(Отказ)
	...
	Если ЭтоНовый() Или ТекущаяПапка.Родитель <> Родитель Тогда
		...
		Если НЕ РаботаСФайламиСлужебный.ЕстьПраво("ИзменениеПапок", ТекущаяПапка.Родитель) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es insuficientemente derecho para el traslado de la carpeta de los ficheros ""%1"".';ru='Недостаточно прав для перемещения из папки файлов ""%1"".';en='Not enough rights to move from file folder ""%1"".'"),
				Строка(?(ЗначениеЗаполнено(ТекущаяПапка.Родитель), ТекущаяПапка.Родитель, НСтр("es='Las carpetas';ru='Папки';en='Folders'"))));
		КонецЕсли;
		// Проверка права на папку назначения.
		Если НЕ РаботаСФайламиСлужебный.ЕстьПраво("ИзменениеПапок", Родитель) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es insuficientemente derecho para la adición подпапок en la carpeta de los ficheros ""%1"".';ru='Недостаточно прав для добавления подпапок в папку файлов ""%1"".';en='Not enough rights to add subfolders to the file folder ""%1"".'"),
				Строка(?(ЗначениеЗаполнено(Родитель), Родитель, НСтр("es='Las carpetas';ru='Папки';en='Folders'"))));
		КонецЕсли;
	КонецЕсли;
	...
	Если ПометкаУдаления И ТекущаяПапка.ПометкаУдаления <> Истина Тогда
		...
		Если НЕ РаботаСФайламиСлужебный.ЕстьПраво("ИзменениеПапок", Ссылка) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es insuficientemente derecho para el cambio de la carpeta de los ficheros ""%1"".';ru='Недостаточно прав для изменения папки файлов ""%1"".';en='Not enough rights to change file folder ""%1"".'"),
				Строка(Ссылка));
		КонецЕсли;
	КонецЕсли;
	...
	Если ПометкаУдаления <> ТекущаяПапка.ПометкаУдаления И Не Ссылка.Пустая() Тогда
		...
		Пока Выборка.Следующий() Цикл
			Если ЗначениеЗаполнено(Выборка.Редактирует) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				                     НСтр("es='La carpeta %1 no es posible quitar, porque contiene el fichero ""%2"" ocupado para la redacción.';ru='Папку %1 нельзя удалить, т.к. она содержит файл ""%2"", занятый для редактирования.';en='Folder%1 cannot be deleted because it contains the file ""%2"" used for editing.'"),
				                     Строка(Ссылка),
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если МассивНайденныхНедопустимыхСимволов.Количество() <> 0 Тогда
		...
		Текст = НСтр("es='El nombre de la carpeta contiene los símbolos prohibidos (\/: ¿*? ""<> |.)';ru='Наименование папки содержит запрещенные символы ( \ / : * ? "" < > | .. )';en='The folder name contains forbidden characters (\ /: *? ""<> | ..)'");
