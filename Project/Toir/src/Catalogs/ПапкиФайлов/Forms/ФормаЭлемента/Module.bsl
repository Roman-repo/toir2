...
Процедура РаботыСФайламиПодключеноРабочийКаталогВладельцаНачалоВыбораПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Фильтр = НСтр("es='Todos los ficheros (*. *) |*.*';ru='Все файлы(*.*)|*.*';en='All files (*. *) | *. *'");
	...
	Если ДиалогОткрытияФайла.Выбрать() Тогда
		...
		Исключение
			...
			ТекстОшибки =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La vía equivocada o faltan los derechos a la anotación en catálogo ""%1""';ru='Неверный путь или отсутствуют права на запись в каталог ""%1""';en=""Incorrect path or missing write permissions to directory '%1'."""), ИмяКаталога);
