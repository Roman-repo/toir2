...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Родитель = Справочники.ГруппыПользователей.ВсеПользователи Тогда
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
			"Объект.Родитель",
			НСтр("es='El grupo predestinado ""Todos los usuarios"" no puede ser los padres.';ru='Предопределенная группа ""Все пользователи"" не может быть родителем.';en='Predefined group ""All users"" cannot be a parent.'"),
			"");
	КонецЕсли;
	...
	Для каждого ТекущаяСтрока Из Состав Цикл;
		...
		Если НЕ ЗначениеЗаполнено(ТекущаяСтрока.Пользователь) Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				...
				НСтр("es='El usuario no es escogido.';ru='Пользователь не выбран.';en='No user selected.'"),
				"Объект.Состав",
				НомерСтроки,
				НСтр("es='El usuario en la línea %1 no es escogido.';ru='Пользователь в строке %1 не выбран.';en='User on line %1 not selected.'"));
			Продолжить;
		КонецЕсли;
		...
		Если НайденныеЗначения.Количество() > 1 Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				"Объект.Состав[%1].Пользователь",
				НСтр("es='El usuario repete.';ru='Пользователь повторяется.';en='User repeats.'"),
				"Объект.Состав",
				НомерСтроки,
				НСтр("es='El usuario en la línea %1 repete.';ru='Пользователь в строке %1 повторяется.';en='User on line %1 is repeated.'"));
		КонецЕсли;
	КонецЦикла;
...
Процедура ПередЗаписью(Отказ)
	...
	Если Ссылка = Справочники.ГруппыПользователей.ВсеПользователи Тогда
		Если НЕ Родитель.Пустая() Тогда
			ВызватьИсключение
				НСтр("es='El grupo predestinado ""Todos los usuarios"" "
"puede ser solamente radicalmente.';ru='Предопределенная группа ""Все пользователи"""
"может быть только в корне.';en='Predefined group"" ""All users"" """
"can only be at the root.'");
		КонецЕсли;
		Если Состав.Количество() > 0 Тогда
			ВызватьИсключение
				НСтр("es='La adición de los usuarios en el grupo "
"""Todos los usuarios"" no es apoyada.';ru='Добавление пользователей в группу"
"""Все пользователи"" не поддерживается.';en='Adding users to a group"
""" All users "" is not supported.'");
		КонецЕсли;
	Иначе
		Если Родитель = Справочники.ГруппыПользователей.ВсеПользователи Тогда
			ВызватьИсключение
				НСтр("es='El grupo predestinado ""Todos los usuarios"" "
"no puede ser los padres.';ru='Предопределенная группа ""Все пользователи"""
"не может быть родителем.';en='Predefined group"" ""All users"" """
"cannot be a parent.'");
		КонецЕсли;
...
Процедура ПроверитьПравоИзмененияСостава(ИзмененияСостава)
	...
	ТекстОшибки =
		НСтр("es='No basta los derechos de admisión. "
""
"En la composición de los participantes de los grupos de los usuarios es posible añadir y quitar solamente "
"a los nuevos usuarios (añadidos), cerca de que es incluido el indicio Es preparado. "
""
"Se prohíbe añadir y quitar a los usuarios existentes:';ru='Недостаточно прав доступа."
""
...
""
"Запрещено добавлять и удалять существующих пользователей:';en='Not enough access rights."
""
"Only members can be added or removed to members of user groups."
"new (added) users who have the Ready flag enabled."
""
"It is forbidden to add and delete existing users:'");
