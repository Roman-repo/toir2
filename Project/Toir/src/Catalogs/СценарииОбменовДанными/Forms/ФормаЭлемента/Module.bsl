...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый
		...
		Наименование = НСтр("es='El guión de la sincronización para %1';ru='Сценарий синхронизации для %1';en='Sync scenarios for %1'");
...
Процедура НастройкиОбменаУзелИнформационнойБазыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Тип") И СписокУзловОбмена.НайтиПоЗначению(ВыбранноеЗначение) = Неопределено Тогда
		ТекстСообщения = НСтр("es='Los datos del tipo escogido no pueden ser usados en esta forma. "
"Escojan otro tipo de datos.';ru='Данные выбранного типа не могут быть использованы в этой форме."
"Выберите другой тип данных.';en='The data of the selected type cannot be used in this form."
"Please select a different data type.'");
...
Процедура ОбновитьПредставлениеРасписания()
	...
	Если ПредставлениеРасписания = Строка(Новый РасписаниеРегламентногоЗадания) Тогда
		...
		ПредставлениеРасписания = НСтр("es='El horario no es dado';ru='Расписание не задано';en='Schedule not set'");
...
Процедура ВыполнитьОбменДаннымиНаКлиенте()
	...
	Если НомерОбрабатываемойСтроки > КоличествоСтрок Тогда // выход из рекурсии
		...
		Состояние(НСтр("es='Los datos son sincronizados.';ru='Данные синхронизированы.';en='Data is synchronized.'"), ?(ВыводитьСостояние, 100, Неопределено));
		Возврат; // выходим
	КонецЕсли;
	...
	СтрокаСообщения = НСтр("es='Se cumple %1 para %2';ru='Выполняется %1 для %2';en='Running% 1 for% 2'");
