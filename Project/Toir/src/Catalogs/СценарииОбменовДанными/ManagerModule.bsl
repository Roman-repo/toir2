...
Процедура СоздатьСценарий(
	...
	Наименование = НСтр("es='La sincronización automática de los datos con %1';ru='Автоматическая синхронизация данных с %1';en='Automatic data synchronization with%1'");
...
Процедура УстановитьПараметрыРегламентногоЗадания(РегламентноеЗаданиеОбъект, РасписаниеРегламентногоЗадания, ТекущийОбъект)
	...
	НаименованиеРегламентногоЗадания = НСтр("es='La ejecución del cambio por el guión: %1';ru='Выполнение обмена по сценарию: %1';en='Script exchange completed:%1'");
...
Процедура ЗаписатьРегламентноеЗадание(Отказ, РегламентноеЗаданиеОбъект)
	...
	Исключение
		...
		СтрокаСообщения = НСтр("es='Había una falta a la conservación del horario de la ejecución de los cambios. La descripción minuciosa de la falta: %1';ru='Произошла ошибка при сохранении расписания выполнения обменов. Подробное описание ошибки: %1';en='An error occurred while saving the exchange execution schedule. Error details:%1'");
