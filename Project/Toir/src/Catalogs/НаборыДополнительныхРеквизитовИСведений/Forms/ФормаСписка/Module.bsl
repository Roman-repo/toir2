...
Процедура ОформитьНаборыИСвойства()
	...
	ЭлементЦветаОформления.Значение = НСтр("es='Los juegos';ru='Наборы';en='Sets'");
...
Процедура НастроитьОтображениеНаборов()
	...
	Если ЭтоНаборыДополнительныхСведений Тогда
		Заголовок = НСтр("es='Las noticias adicionales';ru='Дополнительные сведения';en='additional information'");
		
		КомандаСоздать.Подсказка          = НСтр("es='Crear la noticia única';ru='Создать уникальное сведение';en='Create a unique information'");
		КомандаСоздать.Заголовок          = НСтр("en='New';es='Nuevo';ru='Новое'");
		КомандаСоздать.Подсказка          = НСтр("es='Crear la noticia única';ru='Создать уникальное сведение';en='Create a unique information'");
		
		КомандаСкопировать.Подсказка        = НСтр("es='Crear la nueva noticia por la copia corriente';ru='Создать новое сведение копированием текущего';en='Create a new mix by copying the current'");
		КомандаИзменить.Подсказка           = НСтр("es='Cambiar (o abrir) la noticia corriente';ru='Изменить (или открыть) текущее сведение';en='Change (or open) the current information'");
		КомандаПометитьНаУдаление.Подсказка = НСтр("es='Marcar la noticia corriente a la desaparición (Del)';ru='Пометить текущее сведение на удаление (Del)';en='Mark current deletion information (Del)'");
		КомандаПереместитьВверх.Подсказка   = НСтр("es='Trasladar la noticia corriente hacia arriba';ru='Переместить текущее сведение вверх';en='Move current roll up'");
		КомандаПереместитьВниз.Подсказка    = НСтр("es='Trasladar la noticia corriente hacia abajo';ru='Переместить текущее сведение вниз';en='Move current roll down'");
		...
		Элементы.СвойстваТипЗначения.Подсказка =
			НСтр("es='Los tipos del significado, que se puede introducir al relleno de la noticia.';ru='Типы значения, которое можно ввести при заполнении сведения.';en='The types of value that you can enter when filling out information.'");
		
		Элементы.СвойстваОбщиеЗначения.Подсказка =
			НСтр("es='La noticia usa la lista de los significados de la noticia-modelo.';ru='Сведение использует список значений сведения-образца.';en='Information uses a list of sample information values.'");
		
		Элементы.СвойстваОбщее.Заголовок = НСтр("es='General';ru='Общее';en='General'");
		Элементы.СвойстваОбщее.Подсказка = НСтр("es='La noticia general adicional, que se usa en "
"algunos juegos de las noticias adicionales.';ru='Общее дополнительное сведение, которое используется в"
"нескольких наборах дополнительных сведений.';en='General supplemental information used in"
"multiple sets of additional information.'");
	Иначе
		Заголовок = НСтр("es='Los requisitos adicionales';ru='Дополнительные реквизиты';en='Additional details'");
		КомандаСоздать.Заголовок          = НСтр("es='Nuevo';ru='Новый';en='New'");
		КомандаСоздать.Подсказка          = НСтр("es='Nuevo';ru='Новый';en='New'");
		
		КомандаСкопировать.Подсказка        = НСтр("es='Crear un nuevo requisito por la copia corriente';ru='Создать новый реквизит копированием текущего';en='Create a new attribute by copying the current'");
		КомандаИзменить.Подсказка           = НСтр("es='Crear un nuevo requisito por la copia corriente';ru='Создать новый реквизит копированием текущего';en='Create a new attribute by copying the current'");
		КомандаПометитьНаУдаление.Подсказка = НСтр("es='Cambiar (o abrir) el requisito corriente';ru='Изменить (или открыть) текущий реквизит';en='Change (or open) the current props'");
		КомандаПереместитьВверх.Подсказка   = НСтр("es='Marcar el requisito corriente a la desaparición (Del)';ru='Пометить текущий реквизит на удаление (Del)';en='Mark current deletion attribute (Del)'");
		КомандаПереместитьВниз.Подсказка    = НСтр("es='Trasladar el requisito corriente hacia arriba';ru='Переместить текущий реквизит вверх';en='Move current attribute up'");
		...
		Элементы.СвойстваТипЗначения.Подсказка =
			НСтр("es='Los tipos del significado, que se puede introducir al relleno del requisito.';ru='Типы значения, которое можно ввести при заполнении реквизита.';en='The types of value that can be entered when filling out the requisite.'");
		
		Элементы.СвойстваОбщиеЗначения.Подсказка =
			НСтр("es='El requisito usa la lista de los significados del requisito-modelo.';ru='Реквизит использует список значений реквизита-образца.';en='Props uses a list of sample attribute values.'");
		
		Элементы.СвойстваОбщее.Заголовок = НСтр("es='General';ru='Общий';en='General'");
		Элементы.СвойстваОбщее.Подсказка = НСтр("es='General';ru='Общий';en='General'");
	КонецЕсли;
	...
	Если Элементы.Свойства.ТекущиеДанные <> Неопределено Тогда
		...
		Если ЭтоНаборыДополнительныхСведений Тогда
			Если Элементы.Свойства.ТекущиеДанные.Общее Тогда
				ТекстВопроса = НСтр("es='¿Excluir corriente general las la noticias del juego?';ru='Исключить текущее общее сведения из набора?';en='Exclude current general information from the set?'");
				
			ИначеЕсли Элементы.Свойства.ТекущиеДанные.ПометкаУдаления Тогда
				ТекстВопроса = НСтр("es='¿Quitar de la noticia corriente la marca a la desaparición?';ru='Снять с текущего сведения пометку на удаление?';en='Unmark deletion from current information?'");
			Иначе
				ТекстВопроса = НСтр("es='¿Quitar de la noticia corriente la marca a la desaparición?';ru='Снять с текущего сведения пометку на удаление?';en='Unmark deletion from current information?'");
			КонецЕсли;
		Иначе
			Если Элементы.Свойства.ТекущиеДанные.Общее Тогда
				ТекстВопроса = НСтр("es='¿Excluir el requisito corriente general del juego?';ru='Исключить текущий общий реквизит из набора?';en='Exclude current general attribute from the set?'");
				
			ИначеЕсли Элементы.Свойства.ТекущиеДанные.ПометкаУдаления Тогда
				ТекстВопроса = НСтр("es='¿Quitar del requisito corriente la marca a la desaparición?';ru='Снять с текущего реквизита пометку на удаление?';en='Unmark deletion from current props?'");
			Иначе
				ТекстВопроса = НСтр("es='¿Quitar del requisito corriente la marca a la desaparición?';ru='Снять с текущего реквизита пометку на удаление?';en='Unmark deletion from current props?'");
			КонецЕсли;
		КонецЕсли;
	...
	Попытка
		...
		Попытка
			...
			Если ТекущийНаборОбъект.ВерсияДанных <> ТекущийНаборВерсияДанных Тогда
				...
				Если ЭтоНаборыДополнительныхСведений Тогда
					ВызватьИсключение
						НСтр("es='La acción no es cumplida, puesto que la composición de las noticias adicionales "
"era cambiada por otro usuario. "
"Una nueva composición de las noticias adicionales es leída. "
""
"Repitan la acción, si es necesario.';ru='Действие не выполнено, так как состав дополнительных сведений"
"был изменен другим пользователем."
"Новый состав дополнительных сведений прочитан."
""
"Повторите действие, если требуется.';en='The action has not been completed because the composition of the additional information"
"was changed by another user."
"A new set of additional information has been read."
""
"Repeat if necessary.'");
				Иначе
					ВызватьИсключение
						НСтр("es='La acción no es cumplida, puesto que la composición de los requisitos adicionales "
"era cambiada por otro usuario. "
"Una nueva composición de los requisitos adicionales es leída. "
""
"Repitan la acción, si es necesario.';ru='Действие не выполнено, так как состав дополнительных реквизитов"
"был изменен другим пользователем."
"Новый состав дополнительных реквизитов прочитан."
""
"Повторите действие, если требуется.';en='The action has not been completed because the composition of the additional details"
"was changed by another user."
"The new composition of the additional details has been read."
""
"Repeat if necessary.'");
				КонецЕсли;
			КонецЕсли;
