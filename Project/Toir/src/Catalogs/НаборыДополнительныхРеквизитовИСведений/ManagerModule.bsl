	...
	Для Каждого ОбновляемыйНабор Из Результат Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió tratar el juego de las propiedades: %1 por la causa: "
"%2';ru='Не удалось обработать набор свойств: %1 по причине:"
"%2';en='Failed to process property set: %1 due to:"
"%2'"), 
	...
	Если ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = НСтр("es='El procedimiento ОбработатьНаборыСвойствДляПереходаНаНовуюВерсию se ha acabado con la falta. No consiguió renovar todos los juegos de las propiedades.';ru='Процедура ОбработатьНаборыСвойствДляПереходаНаНовуюВерсию завершилась с ошибкой. Не все наборы свойств удалось обновить.';en='A procedure ОбработатьНаборыСвойствДляПереходаНаНовуюВерсию ended with an error. Not all property sets have been updated.'");
...
Функция СвойстваНабора(НаборыСвойств, Набор, Родитель = Неопределено, Наименования = Неопределено)
	...
	ЗаголовокОшибки =
		НСтр("es='La falta en el procedimiento ПриСозданииПредопределенныхНаборовСвойств "
"el módulo UpravlenieSvoystvamiPereopredelyaemyy general.';ru='Ошибка в процедуре ПриСозданииПредопределенныхНаборовСвойств"
"общего модуля УправлениеСвойствамиПереопределяемый.';en='Error in procedure ПриСозданииПредопределенныхНаборовСвойств of general module УправлениеСвойствамиПереопределяемый.'")
		+ Символы.ПС
		+ Символы.ПС;
	
	Если Не ЗначениеЗаполнено(Набор.Имя) Тогда
		ВызватьИсключение ЗаголовокОшибки + НСтр("es='El nombre del juego de las propiedades no es rellenado.';ru='Имя набора свойств не заполнено.';en='The name of the property set is empty.'");
	КонецЕсли;
	...
	Если НаборыСвойств.Получить(Набор.Имя) <> Неопределено Тогда
		ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El nombre del juego de las propiedades ""%1"" es determinado ya.';ru='Имя набора свойств ""%1"" уже определено.';en=""The name of the property set '%1' is already defined."""),
			Набор.Имя);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Набор.Идентификатор) Тогда
		ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El identificador del juego de las propiedades ""%1"" no es rellenado.';ru='Идентификатор набора свойств ""%1"" не заполнен.';en='Property set identifier ""%1"" is not filled.'"),
			Набор.Имя);
	КонецЕсли;
	...
	Если НаборыСвойств.Получить(НаборСсылка) <> Неопределено Тогда
		ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El identificador ""%1"" juegos de las propiedades "
"""%2"" se usa ya para el juego ""%3"".';ru='Идентификатор ""%1"" набора свойств"
"""%2"" уже используется для набора ""%3"".';en='Property set Identifier ""%1"" "
"""%2"" is already in use for dialing ""%3"".'"),
