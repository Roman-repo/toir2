...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Исключение
		ТекстСообщения = НСтр("es='La fecha incorrecta del comienzo de la temporada.';ru='Некорректная дата начала сезона.';en='Invalid start date for the season.'");
	...
	Исключение
		ТекстСообщения = НСтр("es='La fecha incorrecta de la terminación de la temporada.';ru='Некорректная дата окончания сезона.';en='Incorrect season end date.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,,,Отказ);
	КонецПопытки;
	...
	Если ПарамДатаНачалаСезона > ПарамДатаОкончанияСезона Тогда
		Если Не ПроцессЗаписи Тогда
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("ПередЗаписьюЗавершение",ЭтотОбъект,
							...
							НСтр("es='A la terminación del año la temporada no será acabada. ¿La temporada debe continuar en el año siguiente?';ru='При окончании года сезон не будет завершен. Сезон должен продолжаться в следующем году?';en='At the end of the year, the season will not be completed. Should the season continue next year?'"),
