...
Функция УзелПоОчереди(Очередь) Экспорт
	...
	Если ТипЗнч(Очередь) <> Тип("Число") Или Очередь = 0 Тогда
		ВызватьИсключение НСтр("es='Es imposible recibir el nudo del plan del cambio de ObnovlenieInformatsionnoyBazy, porque no es entregado el número del turno.';ru='Невозможно получить узел плана обмена ОбновлениеИнформационнойБазы, т.к. не передан номер очереди.';en='Unable to get the exchange plan node Update of the Information Base because The queue number has not been transmitted.'");
	КонецЕсли;
