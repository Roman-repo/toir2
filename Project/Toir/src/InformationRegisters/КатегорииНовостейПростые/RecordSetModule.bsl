	...
	Для каждого ТекущаяЗапись Из ЭтотОбъект Цикл
		...
		Если ВРег(ТекущаяЗапись.КатегорияНовостей.Код) = ВРег("ProductVersion")
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='En las categorías simples no debe ser la categoría con el código %1 - debe ser en las categorías de los intervalos de las versiones.';ru='В простых категориях не должно быть категории с кодом %1 - она должна быть в категориях интервалов версий.';en='In simple categories, there should not be a category with the code%1 - it should be in the categories of version intervals.'"),
