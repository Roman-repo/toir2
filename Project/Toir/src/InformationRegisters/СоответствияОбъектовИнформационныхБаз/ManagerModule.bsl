...
Процедура ДобавитьЗапись(СтруктураЗаписи, Загрузка = Ложь) Экспорт
	...
	Для Каждого ПроверяемыйРеквизит Из ПроверяемыеРеквизиты Цикл
		Если СтруктураЗаписи.Свойство(ПроверяемыйРеквизит)
			...
			ОписаниеСобытия = НСтр("es='La adición de la anotación del registro de las noticias ""la Conformidad de los objetos de las bases informativas""';ru='Добавление записи регистра сведений ""Соответствия объектов информационных баз""';en='Adding an information register record ""Compliance of objects of infobases""'",
				ОбщегоНазначения.КодОсновногоЯзыка());
			Комментарий     = НСтр("es='%1. La creación de la anotación del registro es imposible.';ru='Не заполнен реквизит %1. Создание записи регистра невозможно.';en='Requisite%1 is not filled. Creating a register entry is not possible.'");
