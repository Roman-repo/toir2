...
Процедура ОбработатьПользовательскиеМакеты(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Метаданные.НайтиПоПолномуИмени(ИмяОбъектаМетаданныхМакета) = Неопределено Тогда
			ИмяСобытия = НСтр("es='La prensa';ru='Печать';en='Printing'", ОбщегоНазначения.КодОсновногоЯзыка());
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es descubierta la maqueta de usuario que falta en los metadatos de la configuración: "
"""%1"".';ru='Обнаружен пользовательский макет, отсутствующий в метаданных конфигурации:"
"""%1"".';en='A custom layout was found that is missing from the configuration metadata:"
