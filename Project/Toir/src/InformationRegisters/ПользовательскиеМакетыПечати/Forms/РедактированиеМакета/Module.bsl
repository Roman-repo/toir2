...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ТолькоОткрытие Тогда
		Заголовок = НСтр("es='La apertura de la maqueta de la forma de imprenta';ru='Открытие макета печатной формы';en='Opening a print layout'");
	КонецЕсли;
	...
	Если Не ОбщегоНазначения.ЭтоВебКлиент() И Не ОбщегоНазначения.ЭтоМобильныйКлиент() И ТипМакета = "MXL" Тогда
		Элементы.НадписьЗавершениеИзмененияНеВебКлиент.Заголовок = НСтр("es='Después de la inserción de los cambios necesarios en la maqueta presionen el botón ""Acabar el cambio""';ru='После внесения необходимых изменений в макет нажмите на кнопку ""Завершить изменение""';en='After making the necessary changes to the layout, click on the ""Complete Change"" button.'");
	КонецЕсли;
...
Процедура УстановитьНазваниеПрограммыДляОткрытияМакета()
	...
	Если ТипФайла = "mxl" Тогда
		НазваниеПрограммыДляОткрытияМакета = НСтр("es='""1С:Предприятие - el Trabajo con los ficheros""';ru='""1С:Предприятие - Работа с файлами""';en='""1C:Enterprise - Work with files""'");
		АдресПрограммыДляОткрытияМакета = "http://v8.1c.ru/metod/fileworkshop.htm";
	ИначеЕсли ТипФайла = "doc" Тогда
		НазваниеПрограммыДляОткрытияМакета = НСтр("es='""Microsoft Word""';ru='""Microsoft Word""';en='""Microsoft Word""'");
		АдресПрограммыДляОткрытияМакета = "http://office.microsoft.com/ru-ru/word";
	ИначеЕсли ТипФайла = "odt" Тогда
		НазваниеПрограммыДляОткрытияМакета = НСтр("es='""OpenOffice Writer""';ru='""OpenOffice Writer""';en='""OpenOffice Writer""'");
		АдресПрограммыДляОткрытияМакета = "http://www.openoffice.org/product/writer.html";
	ИначеЕсли ТипФайла = "docx" Тогда
		НазваниеПрограммыДляОткрытияМакета = НСтр("es='Un de paquetes de oficina o el redactor de los documentos del formato Office Open XML';ru='один из офисных пакетов или редактор документов формата Office Open XML';en='one of the office suites or Office Open XML format document editor'");
	...
	СведенияДляЗаполнения.Вставить("ОписаниеДействия", ?(Параметры.ТолькоОткрытие, НСтр("es='Las aperturas';ru='открытия';en='opening'"), НСтр("ru='внесения изменений';en='changes mading';es='De la inserción de los cambios'")));
