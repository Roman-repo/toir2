...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.СпособУстановкиВремениНапоминания = ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.ОтносительноТекущегоВремени") Тогда
		СпособУстановкиВремениНапоминания = НСтр("es='A través de';ru='через';en='across'") + " " + НапоминанияПользователяКлиентСервер.ПредставлениеВремени(Объект.ИнтервалВремениНапоминания);
	Иначе
...
Процедура СпособУстановкиВремениНапоминанияПриИзменении(Элемент)
	...
	Если ИнтервалВремени > 0 Тогда
		...
		СпособУстановкиВремениНапоминания = НСтр("es='A través de';ru='через';en='across'") + " " + ИнтервалВремениСтрокой;
	Иначе
		Если Элементы.СпособУстановкиВремениНапоминания.СписокВыбора.НайтиПоЗначению(СпособУстановкиВремениНапоминания) = Неопределено Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("es='El intervalo del tiempo no es determinado.';ru='Интервал времени не определен.';en='The time interval is not defined.'"), , "СпособУстановкиВремениНапоминания");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриИзмененииИнтервалаВремени(Элемент)
	...
	Если Объект.ИнтервалВремениНапоминания > 0 Тогда
		ИнтервалВремениСтрокой = НапоминанияПользователяКлиентСервер.ПредставлениеВремени(Объект.ИнтервалВремениНапоминания);
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("es='El intervalo del tiempo no es determinado';ru='Интервал времени не определен';en='The time interval is not defined'"), , "ИнтервалВремениСтрокой");
	КонецЕсли;
КонецПроцедуры
...
Процедура Удалить(Команда)
	...
	КнопкиДиалога.Добавить(КодВозвратаДиалога.Да, НСтр("es='Quitar';ru='Удалить';en='Delete'"));
	КнопкиДиалога.Добавить(КодВозвратаДиалога.Отмена, НСтр("es='No quitar';ru='Не удалять';en='Do not delete'"));
	
	ОписаниеОповещения = Новый ОписаниеОповещения("УдалитьНапоминание", ЭтотОбъект);
	ПоказатьВопрос(ОписаниеОповещения, НСтр("es='¿Quitar el recordatorio?';ru='Удалить напоминание?';en='Delete reminder?'"), КнопкиДиалога);
...
Функция ПолучитьПредопределенныеСпособыОповещения()
	...
	Результат.Вставить(НСтр("es='Acerca del objeto';ru='относительно предмета';en='regarding the subject'"), ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.ОтносительноВремениПредмета"));
	Результат.Вставить(НСтр("es='En el momento indicado';ru='в указанное время';en='at the indicated time'"), ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.ВУказанноеВремя"));
	Результат.Вставить(НСтр("es='Periódicamente';ru='периодически';en='periodically'"), ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.Периодически"));
	Возврат Результат;
КонецФункции
...
Процедура ЗаполнитьСпособыОповещения()
	...
	Для Каждого Интервал Из ИнтервалыВремени Цикл
		СпособыОповещения.Добавить(НСтр("es='A través de';ru='через';en='across'") + " " + Интервал);
...
Процедура ЗаполнитьВариантыПериодичности()
	...
	Элементы.ВариантПериодичности.СписокВыбора.Добавить("", НСтр("es='Por el horario dado...';ru='по заданному расписанию...';en='according to a given schedule ...'"));	
КонецПроцедуры
...
Процедура ОткрытьДиалогНастройкиРасписанияЗавершение(ВыбранноеРасписание, ДополнительныеПараметры) Экспорт
	...
	Если Не РасписаниеСоответствуетТребованиям(Расписание) Тогда 
		ПоказатьПредупреждение(, НСтр("es='La periodicidad entre día no es apoyada, los ajustes correspondientes son limpiados.';ru='Периодичность в течение дня не поддерживается, соответствующие настройки очищены.';en='The frequency during the day is not supported, the corresponding settings are cleared.'"));
	КонецЕсли;
