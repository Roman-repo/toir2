...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		...
		Элементы.КнопкаОтложить.Заголовок = НСтр("es='Aplazar';ru='Отложить';en='Set aside'");
...
Процедура ОбновитьВремяВТаблицеНапоминаний()
	...
	Для Каждого СтрокаТаблицы Из Напоминания Цикл
		ПредставлениеВремени = НСтр("es='El plazo no es determinado';ru='срок не определен';en='no period'");
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.ВремяСобытия) Тогда
			...
			Если СтрокаТаблицы.ВремяСобытия - НачалоДня(СтрокаТаблицы.ВремяСобытия) < 60 // События "на весь день".
				И НачалоДня(СтрокаТаблицы.ВремяСобытия) = НачалоДня(ТекущаяДата) Тогда
					ПредставлениеВремени = НСтр("es='Hoy';ru='сегодня';en='Today'");
			Иначе
...
Функция ПредставлениеИнтервалаВремени(Знач КоличествоВремени)
	...
	ПредставлениеНедель = НСтр("es='; %1 semana;; %1 semanas; %1 semanas; %1 semanas';ru=';%1 неделю;;%1 недели;%1 недель;%1 недели';en=';%1 week;;%1 week;%1 weeks;%1 weeks'");
	ПредставлениеДней   = НСтр("es='; %1 día;; %1 días; %1 días; %1 días';ru=';%1 день;;%1 дня;%1 дней;%1 дня';en=';%1 day;;%1 day;%1 days;%1 day'");
	ПредставлениеЧасов  = НСтр("es='; %1 hora;; %1 horas; %1 horas; %1 horas';ru=';%1 час;;%1 часа;%1 часов;%1 часа';en=';%1 hour;;%1 hour;%1 hours;%1 hour'");
	ПредставлениеМинут  = НСтр("es='; %1 minuto;; %1 minutos; %1 minutos; %1 minutos';ru=';%1 минуту;;%1 минуты;%1 минут;%1 минуты';en=';%1 minute;;%1 minute;%1 minutes;%1 minute'");
	...
	ШаблонПредставления = НСтр("es='%1 atrás';ru='%1 назад';en='%1 back'");
	Если КоличествоВремени < 0 Тогда
		ШаблонПредставления = НСтр("es='A través de %1';ru='через %1';en='in %1'");
	...
	Если КоличествоНедель > 4 Тогда
		Если СобытиеНаступило Тогда
			Возврат НСтр("es='Hace muchísimo';ru='очень давно';en='a long time ago'");
		Иначе
			Возврат НСтр("es='Todavía no pronto';ru='еще не скоро';en='not soon'");
		КонецЕсли;
	...
	ИначеЕсли КоличествоНедель > 0 Тогда
		Результат = НСтр("es='La semana';ru='неделю';en='week'");
		
	ИначеЕсли КоличествоДней > 1 Тогда
		Если НачалоДня(ТекущаяДата) - НачалоДня(ТекущаяДата - КоличествоВремени) = 60*60*24 * 2 Тогда
			Если СобытиеНаступило Тогда
				Возврат НСтр("es='Anteayer';ru='позавчера';en='the day before yesterday'");
			Иначе
				Возврат НСтр("es='Pasado mañana';ru='послезавтра';en='day after tomorrow'");
			КонецЕсли;
		Иначе
	...
	ИначеЕсли КоличествоЧасов + КоличествоДней * 24 > 3 И Не СобытиеСегодня Тогда
			Если СобытиеНаступило Тогда
				Возврат НСтр("es='Ayer';ru='вчера';en='yesterday'");
			Иначе
				Возврат НСтр("es='Mañana';ru='завтра';en='tomorrow'");
			КонецЕсли;
	ИначеЕсли КоличествоДней > 0 Тогда
		Результат = НСтр("es='El día';ru='день';en='day'");
	...
	ИначеЕсли КоличествоЧасов > 0 Тогда
		Результат = НСтр("es='La hora';ru='час';en='hour'");
	...
	ИначеЕсли КоличествоМинут > 0 Тогда
		Результат = НСтр("es='El minuto';ru='минуту';en='a minute'");
		
	Иначе
		Возврат НСтр("es='Es ahora';ru='сейчас';en='now'");
	КонецЕсли;
