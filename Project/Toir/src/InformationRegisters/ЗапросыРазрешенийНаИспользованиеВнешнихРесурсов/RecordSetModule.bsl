		...
		Для Каждого Запись Из ЭтотОбъект Цикл
			...
			Если Запись.БезопасныйРежим <> ТекущийБезопасныйРежим Тогда
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El régimen seguro %1 se distingue de corriente %2';ru='Безопасный режим %1 отличается от текущего %2';en='Safe mode%1 is different from current%2'"),
			...
			Если ПрограммныйМодуль <> Справочники.ИдентификаторыОбъектовМетаданных.ПустаяСсылка() Тогда
				...
				Если Запись.БезопасныйРежим <> БезопасныйРежимПрограммногоМодуля Тогда
					...
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Para el módulo de programa %1 no puede ser cumplida la interpelación de los permisos del régimen seguro %2';ru='Для программного модуля %1 не может быть выполнен запрос разрешений из безопасного режима %2';en='For program module%1, permission request cannot be made from safe mode%2'"),
