...
Процедура ЗаполнитьВсеПараметрыРаботыРасширенийФоновоеЗадание(Параметры) Экспорт
	...
	Если Параметры.ИмяКонфигурации    <> Метаданные.Имя
	 Или Параметры.ВерсияКонфигурации <> Метаданные.Версия Тогда
		ТекстОшибки =
			НСтр("es='No consiguió renovar todos los parámetros del trabajo de las ampliaciones, puesto que "
"se ha cambiado el nombre o la versión de la configuración - es necesario el relanzamiento de la sesión.';ru='Не удалось обновить все параметры работы расширений, так как"
"изменилось имя или версия конфигурации - требуется перезапуск сеанса.';en='Failed to update all extension settings because"
"the name or version of the configuration has changed - a session restart is required.'");
	КонецЕсли;
	...
	Если Параметры.УстановленныеРасширения.Основные    <> ПараметрыСеанса.УстановленныеРасширения.Основные
	 Или Параметры.УстановленныеРасширения.Исправления <> ПараметрыСеанса.УстановленныеРасширения.Исправления Тогда
		ТекстОшибки =
			НСтр("es='No consiguió renovar todos los parámetros del trabajo de las ampliaciones, "
"puesto que la composición indicada de las ampliaciones se distingue de corriente.';ru='Не удалось обновить все параметры работы расширений,"
"так как указанный состав расширений отличается от текущего.';en='Failed to update all extension settings,"
"because the specified composition of the extensions is different from the current one.'");
	КонецЕсли;
...
Процедура ОбновитьПараметрыРаботыРасширений(ПроверяемыеРасширения = Неопределено, НеподключенныеРасширения = "", ТекстДляАсинхронногоВызова = "") Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("Структура") Тогда
		ВызватьИсключение НСтр("es='La tarea de fondo de la preparación de las ampliaciones no ha devuelto el resultado.';ru='Фоновое задание подготовки расширений не вернуло результат.';en='The extension preparation background task did not return a result.'");
	КонецЕсли;
