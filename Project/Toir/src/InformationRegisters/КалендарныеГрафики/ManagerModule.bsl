...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПредставлениеОтбора   = НСтр("es='El calendario = ""%1""';ru='Календарь = ""%1""';en='Calendar = ""%1""'");
...
"%3'"), 
	...
	Если Обработано = 0 И Проблемных <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Al procedimiento %1 no consiguió tratar algunas anotaciones (son dejados pasar): %2';ru='Процедуре %1 не удалось обработать некоторые записи (пропущены): %2';en='A procedure %1 failed to process some records (skipped):%2'"), 
			ИмяПроцедуры,
			Проблемных);
	...
	ЗаписьЖурналаРегистрации(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El procedimiento %1 ha tratado la porción siguiente de anotaciones: %2';ru='Процедура %1 обработала очередную порцию записей: %2';en='Procedure %1 processed the next batch of records: %2'"),
			ИмяПроцедуры,
			Обработано));
