	...
	Для каждого ЗаписьВерсии Из ЭтотОбъект Цикл
		Если Не ПустаяСтрока(ЗаписьВерсии.Версия) И Не ЭтоПолныйНомерВерсии(ЗаписьВерсии.Версия) Тогда
			ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El número incorrecto de la versión: %1 (se espera el significado en el tipo ""1.2.3.4"")';ru='Некорректный номер версии: %1 (ожидалось значение в виде ""1.2.3.4"")';en='Invalid version number:%1 (expected value in the form of ""1.2.3.4"")'"), ЗаписьВерсии.Версия));
			Отказ = Истина; 
