...
Функция НастройкиТранспортаWS(Корреспондент, ПараметрыАутентификации = Неопределено) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("es='La falta a la extracción de la consigna del depósito seguro.';ru='Ошибка при извлечении пароля из безопасного хранилища.';en='Error while retrieving password from secure store.'");
	КонецЕсли;
