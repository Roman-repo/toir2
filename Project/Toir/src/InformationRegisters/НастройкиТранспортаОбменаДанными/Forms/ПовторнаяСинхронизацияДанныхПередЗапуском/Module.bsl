...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НадписьИмяУзла = НСтр("es='No consiguió establecer la renovación de programa recibido de "
"""%1"". "
"La información técnica de cm. En <a href = ""ZhurnalRegistratsii""> la Revista del registro </a>.';ru='Не удалось установить обновление программы, полученное из"
"""%1""."
"Техническую информацию см. в <a href = ""ЖурналРегистрации"">Журнале регистрации</a>.';en='Failed to install the software update obtained from"
"""%1"".                               For technical information, see the <a href = ""ЖурналРегистрации""> Logbook </a>. '");
...
Процедура СинхронизироватьИПродолжить(Команда)
	...
	ИначеЕсли СтатусОбновления = "ОбновлениеКонфигурации" Тогда
		...
		ТекстПредупреждения = НСтр("es='Del nudo principal son recibidos los cambios, que no son aplicados. "
"Es necesario abrir конфигуратор y renovar la configuración de la base de datos.';ru='Из главного узла получены изменения, которые еще не применены."
"Требуется открыть конфигуратор и обновить конфигурацию базы данных.';en='Changes that have not yet been applied are received from the main node."
"It is required to open the configurator and update the database configuration.'");
...
Процедура СинхронизироватьИПродолжитьБезОбновленияИБЗавершение()
	...
	ИначеЕсли КонфигурацияИзменена() Тогда
		...
		КонецЕсли;
		ТекстПредупреждения = НСтр("es='Del nudo principal son recibidos los cambios, que es necesario aplicar. "
"Es necesario abrir конфигуратор y renovar la configuración de la base de datos.';ru='Из главного узла получены изменения, которые нужно применить."
"Требуется открыть конфигуратор и обновить конфигурацию базы данных.';en='Changes that need to be applied are received from the main node."
"It is required to open the configurator and update the database configuration.'");
	Иначе
		...
		ТекстПредупреждения = НСтр("es='La recepción de los datos del nudo principal se ha acabado con las faltas. "
"Los detalles de cm. En la revista del registro.';ru='Получение данных из главного узла завершилось с ошибками."
"Подробности см. в журнале регистрации.';en='Retrieving data from the master node failed."
