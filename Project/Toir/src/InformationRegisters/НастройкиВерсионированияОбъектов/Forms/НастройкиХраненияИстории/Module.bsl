...
Процедура УстановитьВариантВерсионированияПриПроведении(Команда)
	...
	Если ВыбранТипДокументовБезВозможностиПроведения() Тогда
		ПоказатьПредупреждение(, НСтр("es='A los documentos, que no pueden ser pasados, establecidos el régimen версионирования ""a Versionirovat a la anotación"".';ru='Документам, которые не могут быть проведены, установлен режим версионирования ""Версионировать при записи"".';en='For documents that cannot be posted, the versioning mode is ""Version on write"".'"));
	КонецЕсли;
...
Процедура ЗаполнитьТипыОбъектовВДеревеЗначений()
	...
	Для Каждого Тип Из МассивТипов Цикл
		...
		ИначеЕсли ПланыСчетов.ТипВсеСсылки().СодержитТип(Тип) Тогда
			...
			ПредставлениеГруппы = НСтр("es='Los planes de las cuentas';ru='Планы счетов';en='Chart of accounts'");
		...
		ИначеЕсли ПланыВидовХарактеристик.ТипВсеСсылки().СодержитТип(Тип) Тогда
			...
			ПредставлениеГруппы = НСтр("es='Los planes de los tipos de las características';ru='Планы видов характеристик';en='Features types plans'");
...
Процедура ПриЗавершенииОперацииПоискаУстаревшихВерсий(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Ошибка" Тогда
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(НСтр("es='La búsqueda de las versiones que han caducado de los objetos';ru='Поиск устаревших версий объектов';en='Search for obsolete versions of objects'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
...
Функция ТекстСостоянияПодсчет()
	Возврат НСтр("es='La búsqueda de las versiones que han caducado...';ru='Поиск устаревших версий...';en='Search for obsolete versions ...'");
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ТекстСостоянияОчистка()
	Возврат НСтр("es='Se cumple la limpieza de las versiones que han caducado...';ru='Выполняется очистка устаревших версий...';en='Outdated versions are being cleaned ...'");
КонецФункции
...
Функция ВыполнитьПоискУстаревшихВерсий()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='La búsqueda de las versiones que han caducado de los objetos';ru='Поиск устаревших версий объектов';en='Search for obsolete versions of objects'");
...
Процедура ВывестиИнформациюОбУстаревшихВерсиях()
	...
	Если ИнформацияОбУстаревшихВерсиях.РазмерДанных > 0 Тогда
		Элементы.ИнформацияОбУстаревшихВерсиях.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='En total versiones que han caducado: %1 (%2)';ru='Всего устаревших версий: %1 (%2)';en='Total outdated versions:%1 (%2)'"),
			ИнформацияОбУстаревшихВерсиях.КоличествоВерсий,
			ИнформацияОбУстаревшихВерсиях.РазмерДанныхСтрокой);
	Иначе
		Элементы.ИнформацияОбУстаревшихВерсиях.Заголовок = НСтр("es='En total versiones que han caducado: no';ru='Всего устаревших версий: нет';en='Total outdated versions: no'");
	КонецЕсли;
