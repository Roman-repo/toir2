...
Процедура ОбновитьКонтрольнуюСуммуМакетов(Параметры) Экспорт
	...
	Для Каждого ОписаниеМакета Из ОбрабатываемыеМакеты Цикл
		...
		Исключение
			...
			ТекстОшибки = НСтр("es='No consiguió anotar las noticias sobre la maqueta';ru='Не удалось записать сведения о макете';en='Failed to write layout information.'") + Символы.ПС
				+ Макет.ПолноеИмя() + Символы.ПС
				+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
			
			ЗаписьЖурналаРегистрации(НСтр("es='El control del cambio de las maquetas suministradas';ru='Контроль изменения поставляемых макетов';en='Control of changes in supplied layouts'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Если ЗначениеЗаполнено(МакетыТребующиеОбновленияКонтрольнойСуммы) Тогда
		СписокОшибок.Вставить(0, НСтр("es='No consiguió anotar las noticias sobre las maquetas de las formas de imprenta:';ru='Не удалось записать сведения о макетах печатных форм:';en='Failed to write plate layout information:'"));
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		ПечатнаяФорма.СинонимМакета = НСтр("es='Como hacer la firma facsímil y la prensa';ru='Как сделать факсимильную подпись и печать';en='How to make a facsimile signature and stamp'");
