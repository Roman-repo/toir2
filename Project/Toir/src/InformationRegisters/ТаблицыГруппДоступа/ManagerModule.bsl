...
Процедура ОбновитьДанныеРегистра(ГруппыДоступа = Неопределено,
	...
	Если Справочники.ВерсииРасширений.РасширенияИзмененыДинамически() Тогда
		ВызватьИсключение НСтр("es='Las ampliaciones de la configuración son renovadas, es necesario перезапустить la sesión.';ru='Расширения конфигурации обновлены, требуется перезапустить сеанс.';en='The configuration extensions have been updated; you must restart the session.'");
	КонецЕсли;
