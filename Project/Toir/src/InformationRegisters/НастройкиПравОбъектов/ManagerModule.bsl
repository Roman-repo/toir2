...
Функция Прочитать(Знач СсылкаНаОбъект) Экспорт
	...
	Если ОписаниеПрав = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta en el procedimiento РегистрыСведений.НастройкиПравОбъектов.Прочитать () "
""
"el significado Equivocado del parámetro de SsylkaNaOb''ekt ""%1"". "
"Para los objetos de tabla de ""%2"" derechos no se disponen.';ru='Ошибка в процедуре РегистрыСведений.НастройкиПравОбъектов.Прочитать()"
""
"Неверное значение параметра СсылкаНаОбъект ""%1""."
"Для объектов таблицы ""%2"" права не настраиваются.';en='Error in procedure РегистрыСведений.НастройкиПравОбъектов.Прочитать()           Invalid parameter value СсылкаНаОбъект ""%1"".             For objects of table ""%2"", rights are not configured.'"),
	...
	Для каждого Строка Из Таблица Цикл
		...
		Если Настройки.Колонки.Найти(Строка.Право) = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta en el procedimiento РегистрыСведений.НастройкиПравОбъектов.Прочитать () "
""
"Para los objetos de tabla ""%1"" "
"es derecho ""%2"" no se dispone, sin embargo es anotado "
"en el registro de las noticias NastroykiPravOb''ektov para "
"el objeto ""%3"". "
""
"Probablemente, la renovación de la base informativa no es cumplida o cumplida con "
"la falta. "
"Es necesario corregir los datos del registro.';ru='Ошибка в процедуре РегистрыСведений.НастройкиПравОбъектов.Прочитать()"
""
...
"Требуется исправить данные регистра.';en='Error in procedure РегистрыСведений.НастройкиПравОбъектов.Прочитать()            For table objects ""%1"""
"the right ""%2"" is not configurable, but it is recorded"
"in the information register НастройкиПравОбъектов for"
"object ''%3''."
""
"Perhaps updating the infobase"
"failed or failed."
"It is required to correct the register data.'"),
				СсылкаНаОбъект.Метаданные().ПолноеИмя(),
				Строка.Право,
				Строка(СсылкаНаОбъект));
		КонецЕсли;
...
Процедура ОбновитьВспомогательныеДанныеРегистра(ЕстьИзменения = Неопределено) Экспорт
	...
	Для каждого КлючИЗначение Из ВозможныеПрава.ПоТипамСсылок Цикл
		...
		Если ПустыеСсылкиВладелецПрав.Получить(ТипВладельцаПрав) = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta en el procedimiento de ObnovitVspomogatelnyeDannyeRegistra "
"del módulo del manager del registro de las noticias NastroykiPravOb''ektov. "
""
"El tipo de los propietarios es derecho ""%1"" no es indicado en la medida el Objeto.';ru='Ошибка в процедуре ОбновитьВспомогательныеДанныеРегистра"
"модуля менеджера регистра сведений НастройкиПравОбъектов."
""
"Тип владельцев прав ""%1"" не указан в измерении Объект.';en='Error in procedure ОбновитьВспомогательныеДанныеРегистра information register manager module НастройкиПравОбъектов.              The type of rights owners ""%1"" is not specified in the Object dimension.'"),
				ТипВладельцаПрав);
		КонецЕсли;
...
Функция ВозможныеПрава()
	...
	ЗаголовокОшибки =
		НСтр("es='La falta en el procedimiento ПриЗаполненииВозможныхПравДляНастройкиПравОбъектов "
"el módulo UpravlenieDostupomPereopredelyaemyy general.';ru='Ошибка в процедуре ПриЗаполненииВозможныхПравДляНастройкиПравОбъектов"
"общего модуля УправлениеДоступомПереопределяемый.';en='Error in procedure ПриЗаполненииВозможныхПравДляНастройкиПравОбъектов of general module  УправлениеДоступомПереопределяемый.'")
		+ Символы.ПС
		+ Символы.ПС;
	...
	Для каждого ВозможноеПраво Из ВозможныеПрава Цикл
		...
		Если ОбъектМетаданныхВладельца = Неопределено Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No es encontrado el propietario es derecho ""%1"".';ru='Не найден владелец прав ""%1"".';en=""The owner of the rights '%1' was not found."""),
		...
		Если ПраваВладельца = Неопределено Тогда
			...
			Если ОпределяемыйТипВладельцевПрав.Получить(ТипСсылки) = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El tipo del propietario es derecho ""%1"" "
"no es indicado en el tipo determinado ""el Propietario de los ajustes de los derechos"".';ru='Тип владельца прав ""%1"""
"не указан в определяемом типе ""Владелец настроек прав"".';en='Rights owner type ""%1"" "
"not specified in the type defined ""The owner of the rights settings"".'"),
					Строка(ТипСсылки));
			КонецЕсли;
			...
			Если (ТипыПодпискиЗаписатьЗависимыеНаборыЗначенийДоступа.Получить(ТипОбъекта) <> Неопределено
				...
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El tipo del propietario es derecho ""%1"" "
"no es indicado en el tipo determinado ""el Significado del acceso"", "
"pero se usa para el relleno de los juegos de los significados del acceso, "
"porque es indicado en un de подписок al acontecimiento: "
"- ZapisatZavisimyeNaboryZnacheniyDostupa *, "
"- ZapisatNaboryZnacheniyDostupa*. "
"Es necesario indicar el tipo en el tipo determinado ""el Significado del acceso"" "
"para el relleno correcto del registro de NaboryZnacheniyDostupa.';ru='Тип владельца прав ""%1"""
...
"для корректного заполнения регистра НаборыЗначенийДоступа.';en='Rights owner type ""%1"""
"not specified in the defined type ""Access Value"","
"but used to populate sets of access values,"
"because indicated in one of the event subscriptions:                                           - ЗаписатьЗависимыеНаборыЗначенийДоступа*,"
"- ЗаписатьНаборыЗначенийДоступа*.                                                      It is required to specify the type in the defined type ""Access Value"""
"to fill in the register correctly            НаборыЗначенийДоступа.'"),
					Строка(ТипСсылки));
			КонецЕсли;
...
"to fill in the register correctly            НаборыЗначенийДоступа.'"),
			...
			Если СвойстваВидовДоступа.ПоТипамГруппИЗначений.Получить(ТипСсылки) <> Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El tipo del propietario es derecho ""%1"" "
"no puede usarse, como el tipo de los grupos de los significados del acceso, "
"pero es descubierto en la descripción del tipo del acceso ""%2"".';ru='Тип владельца прав ""%1"""
"не может использоваться, как тип групп значений доступа,"
"но обнаружен в описании вида доступа ""%2"".';en='Rights owner type ""%1"" cannot be used as a type of access value groups,"
"but found in access type description ""%2"".'"),
					Строка(ТипСсылки),
					СвойстваВидовДоступа.ПоТипамЗначений.Получить(ТипСсылки).Имя);
			КонецЕсли;
			...
			Если ТипыПодпискиОбновитьГруппыВладельцевНастроекПрав.Получить(ТипОбъекта) = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El tipo del propietario es derecho ""%1"" "
"no es indicado en el tipo determinado ""el Propietario de los ajustes de los derechos el objeto"".';ru='Тип владельца прав ""%1"""
"не указан в определяемом типе ""Владелец настроек прав объект"".';en='Rights owner type ""%1"" "
"not specified in the defined type ""The owner of the rights settings object"". '"),
					Строка(ТипОбъекта));
			КонецЕсли;
		...
		Если ПраваВладельца.Получить(ВозможноеПраво.Имя) <> Неопределено Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Para el propietario es derecho ""%1"" "
"es repetidamente determinado el derecho ""%2"".';ru='Для владельца прав ""%1"""
"повторно определено право ""%2"".';en='For rights holder"" ""%1"""
"redefined the right """"%2 "".'"),
				ВозможноеПраво.ВладелецПрав,
				ВозможноеПраво.Имя);
		КонецЕсли;
		...
		Если Значение = "*" Тогда
			Если ВозможноеПраво[Свойство].Количество() <> 1 Тогда
				...
				Если Свойство = "ЧтениеВТаблицах" Тогда
					ОписаниеОшибки =
						НСтр("es='Para el propietario es derecho ""%1"" "
"para el derecho ""%2"" en las tablas para la lectura es indicado el símbolo ""*"". "
"En este caso las tablas separadas no es necesario indicar.';ru='Для владельца прав ""%1"""
"для права ""%2"" в таблицах для чтения указан символ ""*""."
"В этом случае отдельных таблиц указывать не нужно.';en='For rights holder"" ""%1"" """
"for the right """"%2 """", the symbol """" * """" is specified in the reading tables."
"In this case, no separate tables are required. '")
				Иначе
					ОписаниеОшибки =
						НСтр("es='Para el propietario es derecho ""%1"" "
"para el derecho ""%2"" en las tablas para el cambio es indicado el símbolo ""*"". "
"En este caso las tablas separadas no es necesario indicar.';ru='Для владельца прав ""%1"""
"для права ""%2"" в таблицах для изменения указан символ ""*""."
"В этом случае отдельных таблиц указывать не нужно.';en='For rights holder"" ""%1"" """
"for the right """"%2 """", the symbol """" * """" is indicated in the tables for the change."
"In this case, no separate tables are required.'")
				КонецЕсли;
			...
			Если ЗначениеЗаполнено(ОбщиеПрава[Свойство]) Тогда
				...
				Если Свойство = "ЧтениеВТаблицах" Тогда
					ОписаниеОшибки =
						НСтр("es='Para el propietario es derecho ""%1"" "
"para el derecho ""%2"" en las tablas para la lectura es indicado el símbolo ""*"". "
"Sin embargo el símbolo ""*"" es indicado ya en las tablas para la lectura para el derecho ""%3"".';ru='Для владельца прав ""%1"""
"для права ""%2"" в таблицах для чтения указан символ ""*""."
"Однако символ ""*"" уже указан в таблицах для чтения для права ""%3"".';en='For rights holder"" ""%1"" """
"for the right """"%2 """", the symbol """" * """" is specified in the reading tables."
"However, the symbol """" * """" is already specified in the reading tables for the right """"%3 "". '")
				Иначе
					ОписаниеОшибки =
						НСтр("es='Para el propietario es derecho ""%1"" "
"para el derecho ""%2"" en las tablas para el cambio es indicado el símbolo ""*"". "
"Sin embargo el símbolo ""*"" es indicado ya en las tablas para el cambio para el derecho ""%3"".';ru='Для владельца прав ""%1"""
"для права ""%2"" в таблицах для изменения указан символ ""*""."
"Однако символ ""*"" уже указан в таблицах для изменения для права ""%3"".';en='For rights holder"" ""%1"" """
"for the right """"%2 """", the symbol """" * """" is indicated in the tables for the change."
"However, the symbol """" * """" is already indicated in the tables for the change for the right """"%3"".'")
				КонецЕсли;
		...
		ИначеЕсли Свойство = "ЧтениеВТаблицах" Тогда
			ОписаниеОшибки =
				НСтр("es='Para el propietario es derecho ""%1"" "
"para el derecho ""%2"" es indicada la tabla concreta para la lectura ""%3"". "
"Sin embargo esto no vale la pena, porque es derecha la Lectura puede depender solamente del derecho la Lectura. "
"Vale la pena usar solamente el símbolo ""*"".';ru='Для владельца прав ""%1"""
...
"Имеет смысл использовать только символ ""*"".';en='For rights holder"" ""%1"" "
"for the right ""%2 "", a specific table is specified for reading """"%3 ""."
"However, this does not make sense, because The right of Reading can depend only on the right of Reading."
"It makes sense to use only the symbol ""*"". '");
		...
		ИначеЕсли Метаданные.НайтиПоПолномуИмени(Значение) = Неопределено Тогда
			...
			Если Свойство = "ЧтениеВТаблицах" Тогда
				ОписаниеОшибки = НСтр("es='Para el propietario es derecho ""%1"" "
"para el derecho ""%2"" no es encontrada la tabla para la lectura ""%3"".';ru='Для владельца прав ""%1"""
"для права ""%2"" не найдена таблица для чтения ""%3"".';en='For rights holder"" ""%1"" "
"for the right """"%2 "" no table was found for reading """"%3"".'")
			Иначе
				ОписаниеОшибки = НСтр("es='Para el propietario es derecho ""%1"" "
"para el derecho ""%2"" no es encontrada la tabla para cambio ""%3"".';ru='Для владельца прав ""%1"""
"для права ""%2"" не найдена таблица для изменения ""%3"".';en='For rights holder"" ""%1"" """
"for the right """"%2 """" no table was found for the change """"%3 "".'")
			КонецЕсли;
