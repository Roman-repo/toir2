...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.РазделениеВключено() И ИспользуетсяВерсионирование Тогда
		Элементы.КоллизииАвторДругойВерсии.Заголовок = НСтр("es='La versión es recibida de la aplicación';ru='Версия получена из приложения';en='Version obtained from application'");
	КонецЕсли;
...
Процедура КоллизииПриАктивизацииСтроки(Элемент)
	...
	Если Элемент.ТекущиеДанные <> Неопределено Тогда
		...
		Если Элемент.ТекущиеДанные.ДругаяВерсияПринята Тогда
			...
			ПричинаКонфликта = НСтр("es='El conflicto era permitido automáticamente en beneficio del programa ""%1"". "
"La versión en este programa era sustituida por la versión de otro programa.';ru='Конфликт был разрешен автоматически в пользу программы ""%1""."
"Версия в этой программе была заменена на версию из другой программы.';en='The conflict was resolved automatically in favor of the program ""%1"". The version in this program has been replaced with a version from another program. '");
		...
		Иначе
			...
			ПричинаКонфликта =НСтр("es='El conflicto era permitido automáticamente en beneficio de este programa. "
"La versión en este programa era conservada, la versión de otro programa era declinada.';ru='Конфликт был разрешен автоматически в пользу этой программы."
"Версия в этой программе была сохранена, версия из другой программы была отклонена.';en='The conflict was resolved automatically in favor of this program."
"The version in this program was saved, the version from another program was rejected.'");
...
Процедура ПринятьВерсиюНепринятые(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("es='¿Aceptar la versión, a pesar de la prohibición de la carga?';ru='Принять версию, несмотря на запрет загрузки?';en='Accept version despite ban on download?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Процедура ИзменитьРезультатКонфликта(Команда)
	...
	Если Элементы.Коллизии.ТекущиеДанные <> Неопределено Тогда
		...
		Если Элементы.Коллизии.ТекущиеДанные.ДругаяВерсияПринята Тогда
			...
			ТекстВопроса = НСтр("es='¿Sustituir la versión recibida de otro programa, por la versión de este programa?';ru='Заменить версию, полученную из другой программы, на версию из этой программы?';en='Replace the version obtained from another program with the version from this program?'");
			
		Иначе
			
			ТекстВопроса = НСтр("es='¿Sustituir la versión de este programa por la versión recibida de otro programa?';ru='Заменить версию этой программы на версию, полученную из другой программы?';en='Replace the version of this program with the version obtained from another program?'");
...
Процедура ЗаполнитьСписокУзлов()
	...
	Если ОтсутствуетОбменПоПравилам И ОтсутствуетОбменXDTO И ОтсутствуетСтандартныйОбмен Тогда
		...
		Заголовок = НСтр("es='Los conflictos a la sincronización de los datos';ru='Конфликты при синхронизации данных';en='Data sync conflicts'");
...
Процедура ОткрытьОбъект(Элемент)
	...
	Если Элемент.ТекущаяСтрока = Неопределено Или ТипЗнч(Элемент.ТекущаяСтрока) = Тип("СтрокаГруппировкиДинамическогоСписка") Тогда
		ПоказатьПредупреждение(, НСтр("es='La orden no puede ser cumplida para el objeto indicado.';ru='Команда не может быть выполнена для указанного объекта.';en='The command could not be executed for the specified object.'"));
		Возврат;
	Иначе
...
Процедура ПоказатьОтличия(Элемент)
	...
	Если СравниваемыеВерсии.Количество() <> 2 Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("es='No hay versión para la comparación.';ru='Нет версии для сравнения.';en='No version to compare.'"));
		Возврат;
...
Процедура ОбновитьЗаголовкиСтраниц()
	...
	УстановитьЗаголовокСтраницы(Элементы.СтраницаНепроведенныеДокументы, НСтр("es='Los documentos no pasados';ru='Непроведенные документы';en='Missed documents'"), КоличествоНепроведенныхДокументов());
	УстановитьЗаголовокСтраницы(Элементы.СтраницаНезаполненныеРеквизиты, НСтр("es='Los requisitos no rellenados';ru='Незаполненные реквизиты';en='Unfilled prop'"), КоличествоНезаполненныхРеквизитов());
	УстановитьЗаголовокСтраницы(Элементы.ОшибкиОбъектаXDTO,              НСтр("es='Los documentos no pasados';ru='Непроведенные документы';en='Missed documents'"), КоличествоОшибокXDTO());	
	
	Если ИспользуетсяВерсионирование Тогда
		УстановитьЗаголовокСтраницы(Элементы.СтраницаКоллизии, НСтр("es='Los conflictos';ru='Конфликты';en='Conflicts'"), КоличествоКоллизий());
		УстановитьЗаголовокСтраницы(Элементы.СтраницаНепринятыеПоДатеЗапрета, НСтр("es='No aceptado por la fecha de la prohibición';ru='Непринятые по дате запрета';en='Missed by ban date'"), КоличествоНепринятых());
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='Es quitada';ru='Удалена';en='Deleted'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='Faltan';ru='Отсутствуют';en='Missing'"));
