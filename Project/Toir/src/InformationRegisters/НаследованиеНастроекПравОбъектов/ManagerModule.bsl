...
Процедура ОбновитьРодителейВладельца(ВладелецНастроекПрав, ЕстьИзменения = Ложь, ОбновитьИерархию = Ложь, ОбъектыСИзменениями = Неопределено) Экспорт
	...
	ЗаголовокОшибки =
		НСтр("es='La falta a la renovación de la jerarquía de los propietarios de los derechos por los significados del acceso.';ru='Ошибка при обновлении иерархии владельцев прав по значениям доступа.';en='Error while updating the hierarchy of rights owners by access values.'")
		+ Символы.ПС
		+ Символы.ПС;
	...
	Если ВозможныеПрава.ПоТипам.Получить(ТипВладельца) = Неопределено Тогда
		ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Para el tipo ""%1"" no es ajustado el uso de los ajustes de los derechos "
"de los objetos.';ru='Для типа ""%1"""
"не настроено использование настроек прав объектов.';en='For type""%1"""
"not configured to use object rights settings.'"),
			Строка(ТипВладельца));
	КонецЕсли;
