...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Текст = НСтр("es='¿Interrumpir la limpieza del clasificador de domicilios?';ru='Прервать очистку адресного классификатора?';en='Abort address classifier cleanup?'");
...
Процедура ОбновитьИнтерфейсПоКоличествуОчищаемых()
	...
	// Страница загрузки
	ТекстОписанияОчистки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Se limpian los datos de las regiones escogidas (%1)';ru='Очищаются данные выбранных регионов (%1)';en='The data of the selected regions is cleared (%1)'"), ВыбраноРегионовДляОчистки);
...
Процедура ОчиститьКлассификатор()
	...
	ТекстСостоянияОчистки = НСтр("es='La limpieza del clasificador de domicilios...';ru='Очистка адресного классификатора ...';en='Clearing the address classifier ...'");
...
Процедура ПослеФоновойОчистки(Задание, ДополнительныеПараметры) Экспорт
	...
	Элементы.ШагиОчистки.ТекущаяСтраница = Элементы.УспешноеЗавершение;
		ТекстОписанияОчистки = НСтр("es='El clasificador de domicilios es con éxito limpiado.';ru='Адресный классификатор успешно очищен.';en='The address classifier has been cleared successfully.'");
	...
	ИначеЕсли Задание.Статус = "Ошибка" Тогда
		...
		ТекстОшибки = НСтр("es='No consigue limpiar el clasificador de domicilios.';ru='Не удается очистить адресный классификатор.';en='Unable to clear the address classifier.'");
		ТекстОшибки = ТекстОшибки + Символы.ПС + НСтр("es='La información técnica:';ru='Техническая информация:';en='Technical information:'") + Задание.ПодробноеПредставлениеОшибки;
...
Функция ЗапуститьФоновуюОчисткуНаСервере(Знач КодыРегионов, Знач УникальныйИдентификатор)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='La limpieza del clasificador de domicilios';ru='Очистка адресного классификатора';en='Clear address classifier'");
