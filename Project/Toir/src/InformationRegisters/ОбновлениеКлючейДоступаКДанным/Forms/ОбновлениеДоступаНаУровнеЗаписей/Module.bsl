...
Процедура ПоследнееЗавершениеОбновленияДоступаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	#Иначе
		...
		ТекстовыйДокумент.Показать(НСтр("es='La falta de la renovación del acceso';ru='Ошибка обновления доступа';en='Access update error'"));
	#КонецЕсли
	КонецЕсли;
...
Процедура ОтменитьОбновлениеПрогресса(Команда)
	...
	Если ЗначениеЗаполнено(ИдентификаторЗаданияОбновленияПрогресса) Тогда
		...
		Если АвтообновлениеПрогресса Тогда
			...
			Пояснение = НСтр("es='La autorenovación del progreso es desconectada';ru='Автообновление прогресса отключено';en='Auto update progress disabled'");
		Иначе
			Пояснение = "";
		КонецЕсли;
		ПоказатьОповещениеПользователя(НСтр("es='La renovación del progreso es anulada';ru='Обновление прогресса отменено';en='Progress update canceled'"),,
			Пояснение);
	КонецЕсли;
	...
	Элементы.ГруппаКоличествоПотоковОбновленияДоступа1.Заголовок =
		Формат(КоличествоПотоковОбновленияДоступа, "ЧГ=") + " "
			+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоПотоковОбновленияДоступа,
				"", НСтр("es='El flujo, el flujo, los flujos, 0';ru='поток,потока,потоков,,,,,,0';en='stream, stream, streams ,,,,,, 0'"));
...
Процедура ОбновитьСостояниеЗаданияОбновленияДоступа(Состояние = Неопределено)
	...
	Если ЗначениеЗаполнено(Состояние.ПоследнееЗавершениеОбновленияДоступа) Тогда
		...
		Если Состояние.ОбновлениеОтмененоНештатно Тогда
			Если Состояние.ПоследнееЗавершениеСегодня Тогда
				Шаблон = НСтр("es='<1> es anulado нештатно </1> después del lanzamiento en %1';ru='<1>отменено нештатно</1> после запуска в %1';en='<1> canceled abnormally </1> after starting in%1'");
			Иначе
				Шаблон = НСтр("es='<1> es anulado нештатно </1> después del lanzamiento %1';ru='<1>отменено нештатно</1> после запуска %1';en='<1> abnormally canceled </1> after running%1'");
			КонецЕсли;
		...
		ИначеЕсли ЗначениеЗаполнено(ТекстОшибкиОбновленияДоступа) Тогда
			Если Состояние.ОбновлениеОтменено Тогда
				Если Состояние.ПоследнееЗавершениеСегодня Тогда
					Шаблон = НСтр("es='<1> es anulado </1> <2> con la falta </2> en %1 a través de %2';ru='<1>отменено</1> <2>с ошибкой</2> в %1 через %2';en='<1> canceled </1> <2> with error </2> in%1 through%2'");
				Иначе
					Шаблон = НСтр("es='<1> es anulado </1> <2> con la falta </2> %1 a través de %2';ru='<1>отменено</1> <2>с ошибкой</2> %1 через %2';en='<1> canceled </1> <2> with error </2>%1 through%2'");
				КонецЕсли;
			Иначе
				Если Состояние.ПоследнееЗавершениеСегодня Тогда
					Шаблон = НСтр("es='<1> es acabado </1> <2> con la falta </2> en %1 a través de %2';ru='<1>завершено</1> <2>с ошибкой</2> в %1 через %2';en='<1> completed </1> <2> with error </2> in%1 through%2'");
				Иначе
					Шаблон = НСтр("es='<1> es acabado </1> <2> con la falta </2> %1 a través de %2';ru='<1>завершено</1> <2>с ошибкой</2> %1 через %2';en='<1> completed </1> <2> with error </2>%1 through%2'");
				КонецЕсли;
			КонецЕсли;
			ФорматЧастей.Вставить(1, Новый Структура("Шрифт, ЦветТекста", ЖирныйШрифт, КрасныйЦвет));
		...
		Иначе
			Если Состояние.ОбновлениеОтменено Тогда
				Если Состояние.ПоследнееЗавершениеСегодня Тогда
					Шаблон = НСтр("es='Es anulado en %1 a través de %2';ru='отменено в %1 через %2';en='canceled in %1 through %2'");
				Иначе
					Шаблон = НСтр("es='Es anulado %1 a través de %2';ru='отменено %1 через %2';en='canceled %1 through %2'");
				КонецЕсли;
			Иначе
				Если Состояние.ПоследнееЗавершениеСегодня Тогда
					Шаблон = НСтр("es='Es acabado en %1 por %2';ru='завершено в %1 за %2';en='completed in %1 for %2'");
				Иначе
					Шаблон = НСтр("es='Es acabado %1 por %2';ru='завершено %1 за %2';en='%1 completed for%2'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		Если Состояние.ПоследнееЗавершениеСегодня Тогда
	...
	Иначе
		ПоследнееЗавершение = "(" + ?(Состояние.ОбновлениеДоступаВыполняется,
			НСтр("es='No terminaba';ru='не завершалось';en='did not end'"), НСтр("es='No era puesto en marcha';ru='не запускалось';en='did not start'")) + ")";
	КонецЕсли;
	...
	ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Se cumple %1';ru='Выполняется %1';en='%1 in progress'"),
...
Функция ВремяВыполненияСтрокой(ВремяВСекундах)
	...
	Если ВсегоЧасов > 0 Тогда
		Шаблон = НСтр("es='%3 h de %2 minas %1 сек';ru='%3 ч %2 мин %1 сек';en='%3 hours %2 minutes %1 seconds'");
		
	ИначеЕсли Минут > 0 Тогда
		Шаблон = НСтр("es='%2 minas %1 сек';ru='%2 мин %1 сек';en='%2 min %1 sec'");
	Иначе
		Шаблон = НСтр("es='%1 сек';ru='%1 сек';en='%1 sec'");
	КонецЕсли;
...
Процедура ПоказатьОшибкуОбновленияПрогрессаОбработчикОжидания()
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='No consiguió renovar el progreso por la causa: "
""
"%1';ru='Не удалось обновить прогресс по причине:"
""
"%1';en='Failed to update progress due to:"
""
...
Функция НачатьОбновлениеПрогрессаНаСервере(Контекст, АдресРезультата, АдресХранимыхДанных,
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания =
		НСтр("es='La dirección del acceso: la Recepción del progreso de la renovación del acceso';ru='Управление доступом: Получение прогресса обновления доступа';en='Access Control: Getting access update progress'");
...
Процедура СортироватьСписок(ПоУбыванию = Ложь)
	...
	Если ТекущаяКолонка = Неопределено
	 Или Не СтрНачинаетсяС(ТекущаяКолонка.Имя, "Списки") Тогда
		...
		ПоказатьПредупреждение(,
			НСтр("es='Escojan la columna para la selección';ru='Выберите колонку для сортировки';en='Select a column to sort'"));
		Возврат;
	КонецЕсли;
	...
	ПоказатьОповещениеПользователя(
		?(ПоУбыванию, НСтр("es='La selección la merma';ru='Сортировка по убыванию';en='Sort descending'"),
			НСтр("es='La selección el acrecentamiento';ru='Сортировка по возрастанию';en='Sort ascending'")),,
