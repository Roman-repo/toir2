...
Процедура ПоИдентификаторуПриИзменении(Элемент)
	...
	Исключение
		ПоказатьПредупреждение(, НСтр("es='El identificador es equivocado.';ru='Идентификатор неверен.';en='The identifier is invalid.'"));
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список,
			"Идентификатор",
			Неопределено,
			ВидСравненияКомпоновкиДанных.Равно,,
...
Процедура ОткрытьСообщение()
	...
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Состояние(НСтр("es='El mensaje se abre. Por favor, esperen...';ru='Сообщение открывается. Пожалуйста, подождите...';en='The message opens. Please, wait...'"));
