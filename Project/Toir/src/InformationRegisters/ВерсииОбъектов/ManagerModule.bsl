	...
	Для Каждого текСтрока Из ТаблицаИзмененийТабличныхДокументов Цикл
		...
		Для Индекс = 0 По ВГраница Цикл
			...
			Если ТекСтрока[ИмяКолонки] = "И" Тогда
				...
				ШаблонТекста = НСтр("es='Comparar la versión № %1 con la versión № %2';ru='сравнить версию №%1 с версией №%2';en='compare version number%1 with version number%2'");
			...
			ИначеЕсли текСтрока[ИмяКолонки] = "У" Тогда
				...
				Область.Текст = НСтр("es='La conservación de los cambios del documento tabular es desconectada';ru='Сохранение изменений табличного документа отключено';en='Saving spreadsheet document changes is disabled'");
				
			ИначеЕсли текСтрока[ИмяКолонки] = "Д" Тогда
				Область = ТЧОтчета.Присоединить(ШаблонЯчейкаДобавлена);
				Область.Текст = НСтр("es='Abrir';ru='открыть';en='open'");
...
Процедура ВывестиШапку(ТЧОтчета, СписокВерсий, ЧислоВерсий, ОбщийШаблон, СсылкаНаОбъект)
	...
	ОбластьШапка.Параметры.НаименованиеОтчета = НСтр("es='El informe de los cambios de las versiones del objeto';ru='Отчет по изменениям версий объекта';en='Object version change report'");
...
Функция ПолучитьОписаниеПоВерсии(СсылкаНаОбъект, НомерВерсии)
	...
	Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='№ %1 / (%2) / %3';ru='№ %1 / (%2) / %3';en='№ %1 / (%2) / %3'"), НомерВерсии.Представление, 
...
Процедура ДобавитьНомераСтрокВТабличныеЧасти(ТабличныеЧасти)
	...
	Для Каждого Соответствие Из ТабличныеЧасти Цикл
		...
		Если Таблица.Колонки.Найти("НомерСтроки") <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Таблица.Колонки.Вставить(0, "НомерСтроки",,НСтр("es='№ las líneas';ru='№ строки';en='Line №'"));
