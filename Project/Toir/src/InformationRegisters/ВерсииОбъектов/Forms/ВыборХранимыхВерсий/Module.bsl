...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.НетВерсий.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Las versiones anteriores faltan: ""%1"".';ru='Предыдущие версии отсутствуют: ""%1"".';en='No previous versions: ""%1"".'"), Строка(Ссылка));
	...
	Реквизиты = НСтр("es='Todo';ru='Все';en='All'")
...
Процедура СформироватьОтчетПоИзменениям(Команда)
	...
	Если СравниваемыеВерсии.Количество() < 2 Тогда
		ПоказатьПредупреждение(, НСтр("es='Para la formación del informe por los cambios es necesario escoger por lo menos dos versiones.';ru='Для формирования отчета по изменениям необходимо выбрать хотя бы две версии.';en='To generate a report on changes, you must select at least two versions.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьПереходНаВыбраннуюВерсию(ОтменятьПроведение = Ложь)
	...
	ИначеЕсли Результат = "ОшибкаПроведения" Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El tránsito a la versión no era cumplido por la causa: "
"¿%1 "
"Pasar a la versión escogida con la anulación de la realización?';ru='Переход на версию не был выполнен по причине:"
"%1"
"Перейти на выбранную версию с отменой проведения?';en='The transition was not completed due to:"
"%1"
"Switch to the selected version with cancellation?'"),
			ТекстСообщенияОбОшибке);
			
		ОписаниеОповещения = Новый ОписаниеОповещения("ВыполнитьПереходНаВыбраннуюВерсиюВопросЗадан", ЭтотОбъект);
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("Перейти", НСтр("es='Pasar';ru='Перейти';en='Go to'"));
	...
	Иначе //Результат = "ВосстановлениеВыполнено"
		...
		ПоказатьОповещениеПользователя(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Es restablecida la versión № %1.';ru='Восстановлена версия № %1.';en='Restored version number% 1.'"), ПредставлениеНомераВерсии),
...
Функция ИмяЭтойИнформационнойБазы()
	Возврат НСтр("es='Este programa';ru='Эта программа';en='This program'");
КонецФункции
