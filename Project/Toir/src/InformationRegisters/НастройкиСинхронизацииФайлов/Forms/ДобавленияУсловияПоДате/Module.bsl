...
Процедура ОК(Команда)
	...
	Если ИнтервалИсключение = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("es='La cantidad de los días no puede ser igual 0.';ru='Количество дней не может быть равно 0.';en='The number of days cannot be equal to 0.'"),,, "ИнтервалИсключение");
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(РеквизитСТипомДата) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("es='Es necesario rellenar las condiciones de la limpieza de los ficheros.';ru='Необходимо заполнить условия очистки файлов.';en='Required to fill out the conditions for cleaning files.'"),,, "РеквизитСТипомДата");
		Возврат;
	КонецЕсли;
