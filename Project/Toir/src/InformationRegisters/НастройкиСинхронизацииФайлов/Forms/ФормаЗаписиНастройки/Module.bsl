...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = НСтр("es='El ajuste de la sincronización de los ficheros:';ru='Настройка синхронизации файлов:';en='Setting file synchronization:'") + " " + ПредставлениеВладельцаДляЗаголовка;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ОбъектСинхронизации = "ТолькоФайлыЭлемента" И Не ЗначениеЗаполнено(ЭлементСправочника) Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='el objeto con los ficheros juntados.';ru='Не заполнен объект с присоединенными файлами.';en='The object with the attached files is not filled.'"),
			,
			"ЭлементСправочника");
	КонецЕсли;
...
Процедура ДобавитьВОтборИнтервалИсключение(ВыбранноеЗначение)
	...
	ТекстПредставления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Limpiar después %1 días acerca de la fecha (%2)';ru='Очищать спустя %1 дней относительно даты (%2)';en='Clear after %1 days relative to date (%2)'"), 
