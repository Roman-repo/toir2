...
Процедура ДеревоОбъектовМетаданныхПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса = НСтр("es='La desaparición del ajuste llevará al cese de la sincronización "
"por las reglas, dadas en ella. ¿Continuar?';ru='Удаление настройки приведет к прекращению синхронизации "
"по заданным в ней правилам. Продолжить?';en='Deleting a setting will stop syncing"
"according to the rules specified in it. Continue?'");
...
Процедура СинхронизацияЭлемента(Команда)
	...
	Если Не СтрокаДерева.ВозможностьДетализации Тогда
		ТекстСообщения = НСтр("es='La adición del ajuste es posible solamente las guías jerárquicas';ru='Добавление настройки возможно только для иерархических справочников';en='Adding a setting is possible only for hierarchical directories'");
	...
	ПараметрыФормыВыбора.Вставить("ЗаголовокФормыПодбора", НСтр("es='La selección de los elementos de los ajustes';ru='Подбор элементов настроек';en='Selection of settings items'"));
	...
	Для Каждого СведенияОВладельце Из ТаблицаВладельцевФайлов Цикл
		...
		Если ВсеСправочники.СодержитТип(СведенияОВладельце.ВладелецФайла) Тогда
			Если УзелСправочники = Неопределено Тогда
				...
				УзелСправочники.СинонимНаименованияОбъекта = НСтр("es='Las guías';ru='Справочники';en='Directories'");
			КонецЕсли;
		...
		ИначеЕсли ВсеДокументы.СодержитТип(СведенияОВладельце.ВладелецФайла) Тогда
			Если УзелДокументы = Неопределено Тогда
				...
				УзелДокументы.СинонимНаименованияОбъекта = НСтр("es='Los documentos';ru='Документы';en='Documents'");
			КонецЕсли;
		...
		ИначеЕсли БизнесПроцессы.ТипВсеСсылки().СодержитТип(СведенияОВладельце.ВладелецФайла) Тогда
			Если УзелБизнесПроцессы = Неопределено Тогда
				...
				УзелБизнесПроцессы.СинонимНаименованияОбъекта = НСтр("es='Los procesos de negocios';ru='Бизнес-процессы';en='Business processes'");
			КонецЕсли;
		...
		Если ДетализированныеНастройки.Количество() > 0 Тогда
			Для Каждого Настройка Из ДетализированныеНастройки Цикл
				...
				ДетализированнаяНастройка.ПравилоОтбора    =
					?(ЕстьПравилоОтбора, НСтр("es='Los ficheros escogidos';ru='Выбранные файлы';en='Selected files'"), НСтр("es='Todos los ficheros';ru='Все файлы';en='All files'"));
		...
		Если НайденныеНастройки.Количество() > 0 Тогда
			...
			Если ПравилоОтбора <> Неопределено И ПравилоОтбора.Отбор.Элементы.Количество() > 0 Тогда
				НоваяСтрокаТаблицы.ПравилоОтбора = ?(ПустаяСтрока(НайденныеНастройки[0].Наименование), 
					НСтр("es='Los ficheros escogidos';ru='Выбранные файлы';en='Selected files'"), НайденныеНастройки[0].Наименование);
			Иначе
				НоваяСтрокаТаблицы.ПравилоОтбора = НСтр("es='Todos los ficheros';ru='Все файлы';en='All files'");
			КонецЕсли;
...
Процедура УстановитьНастройкиОтбора(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранноеЗначение.ЕстьПравилоОтбора Тогда
		ОбновляемаяСтрока.ПравилоОтбора =
			?( ЗначениеЗаполнено(ВыбранноеЗначение.Наименование), ВыбранноеЗначение.Наименование, НСтр("es='Los ficheros escogidos';ru='Выбранные файлы';en='Selected files'"));
	Иначе
		ОбновляемаяСтрока.ПравилоОтбора = НСтр("es='Todos los ficheros';ru='Все файлы';en='All files'");
	КонецЕсли;
...
Функция ОчиститьДанныеНастройки()
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("es='El subsistema el Trabajo con los ficheros: la Desconexión de la sincronización de los ficheros con el servicio nuboso';ru='Подсистема Работа с файлам: Отключение синхронизации файлов с облачным сервисом';en='Subsystem Working with files: Disabling file synchronization with the cloud service'");
...
Процедура ЗапуститьРегламентноеЗадание()
	...
	Иначе
		...
		ПараметрыЗадания.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El lanzamiento a mano: %1';ru='Запуск вручную: %1';en='Manual launch:%1'"), РегламентноеЗаданиеМетаданные.Синоним);
