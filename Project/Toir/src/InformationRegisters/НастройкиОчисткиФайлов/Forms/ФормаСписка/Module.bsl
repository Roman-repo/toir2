...
Процедура ДеревоОбъектовМетаданныхПередУдалением(Элемент, Отказ)
	...
	Если НастройкаДляУдаления <> Неопределено Тогда
		...
		Если НастройкаДляУдаленияРодителя <> Неопределено И НастройкаДляУдаленияРодителя.ВозможностьДетализации Тогда
			...
			ТекстВопроса = НСтр("es='La desaparición del ajuste llevará al cese de la limpieza de los ficheros "
"por las reglas, dadas en ella. ¿Continuar?';ru='Удаление настройки приведет к прекращению очистки файлов"
"по заданным в ней правилам. Продолжить?';en='Deleting a setting will stop cleaning files"
"according to the rules specified in it. Continue?'");
	...
	ТекстСообщения = НСтр("es='El ajuste extendido de la limpieza de los ficheros no es previsto para este objeto.';ru='Расширенная настройка очистки файлов не предусмотрена для этого объекта.';en='Extended settings for cleaning files are not provided for this object.'");
...
Функция МожноОчищатьВерсии()
	Если Элементы.ДеревоОбъектовМетаданных.ВыделенныеСтроки.Количество() = 1 Тогда
		...
		Если Не ТекущиеДанные.ЭтоФайл
			...
			ПоказатьПредупреждение(, НСтр("es='Para el objeto dado de la versión de los ficheros no están.';ru='Для данного объекта версии файлов не хранятся.';en='No file versions are stored for this object.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьНастройкиПоВладельцу(ВыбранноеЗначение)
	...
	Для Каждого Настройка Из ВыбранноеЗначение Цикл
		...
		ДетализированнаяНастройка.ПравилоОтбора = НСтр("es='Cambiar la regla';ru='Изменить правило';en='Change rule'");
	КонецЦикла;
