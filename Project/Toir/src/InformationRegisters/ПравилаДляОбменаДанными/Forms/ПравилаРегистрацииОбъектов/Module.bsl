...
Процедура ЗагрузитьПравила(Команда)
	...
	ПараметрыДиалога.Вставить("Заголовок", НСтр("es='Indiquen, de que fichero cargar las reglas';ru='Укажите, из какого файла загрузить правила';en='Specify which file to load the rules from.'"));
	ПараметрыДиалога.Вставить("Фильтр",
		  НСтр("es='Los ficheros de las reglas del registro (*.xml)';ru='Файлы правил регистрации (*.xml)';en='Registration rules files (* .xml)'") + "|*.xml|"
		+ НСтр("es='Los archivos ZIP (*.zip)';ru='Архивы ZIP (*.zip)';en='ZIP archives (* .zip)'")   + "|*.zip");
...
Процедура ВыгрузитьПравила(Команда)
	...
	ФильтрИмени = НСтр("es='Los ficheros de las reglas (*.xml)';ru='Файлы правил (*.xml)';en='Rule files (* .xml)'") + "|*.xml";
	...
	Если ПустаяСтрока(ЧастиИмени.ИмяБезРасширения) Тогда
		ПолноеИмяФайла = НСтр("es='Las reglas del registro';ru='Правила регистрации';en='Registration rules'");
	Иначе
...
Процедура ЗагрузитьПравилаЗавершение(Знач РезультатПомещенияФайлов, Знач ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(ТекстОшибки) И ПустаяСтрока(АдресПомещенногоФайла) Тогда
		ТекстОшибки = НСтр("es='La falta de la transmisión del fichero al servidor';ru='Ошибка передачи файла на сервер';en='Error transferring file to server'");
	КонецЕсли;
