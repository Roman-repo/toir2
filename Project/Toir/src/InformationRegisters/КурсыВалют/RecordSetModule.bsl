...
Функция КурсВалютыПоФормуле(Знач Валюта, Знач Формула, Знач Период)
	...
	Исключение
		...
		Если ОшибкаРасчетаКурсаПоФормуле[Валюта] = Неопределено Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El cálculo de la cotización ""%1"" por la fórmula ""%2"" no es cumplido:';ru='Расчет курса валюты ""%1"" по формуле ""%2"" не выполнен:';en='Currency rate calculation ""%1"" using the formula ""%2"" failed:'",
			...
			ЗаписьЖурналаРегистрации(НСтр("es='Las divisas. La carga de las cotizaciones';ru='Валюты.Загрузка курсов валют';en='Currencies. Download currency rates'", ОбщегоНазначения.КодОсновногоЯзыка()),
