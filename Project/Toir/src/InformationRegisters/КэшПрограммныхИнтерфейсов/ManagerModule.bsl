...
Функция ДанныеКэшаВерсий(Знач Идентификатор, Знач ТипДанных, Знач ПараметрыПолучения, Знач ИспользоватьУстаревшиеДанные = Истина) Экспорт
	...
	Если ТребуетсяВыполнитьОбновление Тогда
		...
		Иначе
			...
			НаименованиеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La renovación del cache de las versiones. El identificador de la anotación %1. El Tipo de datos %2.';ru='Обновление кэша версий. Идентификатор записи %1. Тип данных %2.';en='Version cache update. Record ID%1. Data type%2.'"),
				Идентификатор,
				ТипДанных);
		...
		Если ТребуетсяПеречитатьДанные Тогда
			...
			Если Результат.Пустой() Тогда
				ШаблонСообщения = НСтр("es='La falta a la renovación del cache de las versiones. Los datos no son recibidos. "
"El identificador de la anotación: %1 "
"Tipo de datos: %2';ru='Ошибка при обновлении кэша версий. Данные не получены."
"Идентификатор записи: %1"
"Тип данных: %2';en='Error while updating version cache. Data not received."
"Record identifier:%1"
"Data type:%2'");
...
Функция ПодготовитьДанныеКэшаВерсий(Знач ТипДанных, Знач ПараметрыПолучения) Экспорт
	...
	Иначе
		ШаблонТекста = НСтр("es='El tipo de datos desconocido del cache de las versiones: %1';ru='Неизвестный тип данных кэша версий: %1';en='Unknown version cache data type:%1'");
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПолучениеФайловИзИнтернета") Тогда
		...
		Исключение
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió recibir las WS-definiciones a dirección de "
"%1 "
"por la causa: "
"%2';ru='Не удалось получить WS-определения по адресу "
"%1"
"по причине:"
"%2';en='Failed to get WS definitions at"
"%1"
"due to:"
"%2'"),
				АдресWSDL,
...
"%2'"),
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir las WS-definiciones del cache "
"por la causa: "
"%1';ru='Не удалось получить WS-определения из кэша "
"по причине:"
"%1';en='Failed to get WS definitions from cache"
"due  to:"
"%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение ТекстОшибки;
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("es='La recepción WSDL';ru='Получение WSDL';en='Getting WSDL'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ПолучитьWSDL(Знач Адрес, Знач ИмяПользователя, Знач Пароль, Знач Таймаут, Знач ЗащищенноеСоединение = Неопределено)
	...
	Иначе
		ВызватьИсключение 
			НСтр("es='El subsistema ""la Recepción de los ficheros del Internet"" no es accesible.';ru='Подсистема ""Получение файлов из интернета"" не доступна.';en='Subsystem ""Receiving files from the Internet"" is not available.'");
	КонецЕсли;
	
	Если Не ОписаниеФайла.Статус Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir el fichero de la descripción del web-servicio "
"por la causa: "
"%1';ru='Не удалось получить файл описания web-сервиса"
"по причине:"
"%1';en='Failed to get web service description file             due to:"
"%1'"),
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir el fichero de la descripción del web-servicio "
"por la causa: "
"no consiguió recibir las WS-definiciones para la conservación en el cache: "
"%1';ru='Не удалось получить файл описания web-сервиса"
"по причине:"
"Не удалось получить WS-определения для сохранения в кэше:"
"%1';en='Failed to get web service description file"
"due to:"
"Failed to get WS definitions to cache:"
"%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	...
	Если Определения.Сервисы.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir el fichero de la descripción del web-servicio "
"por la causa: "
"En el fichero recibido no contiene ni una descripción del servicio. "
""
"Probablemente, la dirección del fichero de la descripción es indicada incorrectamente: "
"%1';ru='Не удалось получить файл описания web-сервиса"
"по причине:"
...
""
"The description file address may be incorrect:"
"%1'"),
			Адрес);
	КонецЕсли;
