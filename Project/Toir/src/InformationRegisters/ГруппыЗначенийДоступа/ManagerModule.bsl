...
Процедура ОбновитьГруппировкиПользователей(Параметры = Неопределено, ЕстьИзменения = Неопределено) Экспорт
	...
	Иначе
		ВызватьИсключение
			НСтр("es='La falta en el procedimiento de ObnovitGruppirovkiPolzovateley "
"del módulo del manager del registro de las noticias del Grupo de los significados del acceso. "
""
"Son indicados los parámetros equivocados.';ru='Ошибка в процедуре ОбновитьГруппировкиПользователей"
"модуля менеджера регистра сведений Группы значений доступа."
""
"Указаны неверные параметры.';en='Error in procedure ОбновитьГруппировкиПользователей module of the register of information register of the group of access values"
""
...
Процедура ОбновитьГруппыЗначенияДоступа(ЗначениеДоступа, ЕстьИзменения, ЗначенияСИзменениямиПоТипам)
	...
	ЗаголовокОшибки =
		НСтр("es='La falta a la renovación de los grupos de los significados del acceso.';ru='Ошибка при обновлении групп значений доступа.';en='Error while updating access value groups.'")
		+ Символы.ПС
		+ Символы.ПС;
	...
	Если СвойстваВидаДоступа = Неопределено Тогда
		ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Para el tipo ""%1"" no es ajustado el uso de los grupos de los significados "
"del acceso.';ru='Для типа ""%1"""
"не настроено использование групп значений доступа.';en='For type ""%1"""
"not configured to use access value groups.'"),
	...
	Исключение
		...
		Если СвойстваВидаДоступа.ТипГруппЗначений = Тип("Неопределено") Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Cerca del significado del acceso ""%1"" "
"como ""%2"" "
"no consiguió leer el requisito la Referencia por la causa: "
"%3';ru='У значения доступа ""%1"""
"типа ""%2"""
"не удалось прочитать реквизит Ссылка по причине:"
"%3';en='Access value ""%1"" "
"type ""%2"""
"Failed to read props Link due to:"
"%3'"),
		...
		ИначеЕсли СвойстваВидаДоступа.НесколькоГруппЗначений Тогда
			...
			Если ТабличнаяЧастьМетаданные = Неопределено
			 ИЛИ ТабличнаяЧастьМетаданные.Реквизиты.Найти("ГруппаДоступа") = Неопределено Тогда
				...
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Cerca del tipo de los significados del acceso ""%1"" no es creada la parte especial tabular de GruppyDostupa con el requisito especial "
""
"de GruppaDostupa.';ru='У типа значений доступа ""%1"""
"не создана специальная табличная часть ГруппыДоступа"
"со специальным реквизитом ГруппаДоступа.';en='The type of access value is ""%1"""
"no special tabular part of ГруппыДоступа created"
"with the special attribute ГруппаДоступа.'"),
					Строка(ТипЗначенияДоступа));
			Иначе
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Cerca del significado del acceso ""%1"" "
"como ""%2"" "
"no consiguió leer la parte tabular de GruppaDostupa "
"con el requisito de GruppaDostupa por la causa: "
"%3';ru='У значения доступа ""%1"""
"типа ""%2"""
...
"%3';en='Access value ""%1"" "
"type ""%2"""
"failed to read the tabular part ГруппаДоступа"
"with the requisite ГруппаДоступа due to:"
"%3 '"),
					Строка(ЗначениеДоступа),
					Строка(ТипЗначенияДоступа),
					КраткоеПредставлениеОшибки(Ошибка));
		...
		Иначе
			Если МетаданныеТипа.Реквизиты.Найти("ГруппаДоступа") = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Cerca del tipo de los significados del acceso ""%1"" no es creado el requisito especial "
"de GruppaDostupa.';ru='У типа значений доступа ""%1"""
"не создан специальный реквизит ГруппаДоступа.';en='The type of access value is ""% """
"the special attribute ГруппаДоступа has not been created.'"),
					Строка(ТипЗначенияДоступа));
			Иначе
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Cerca del significado del acceso ""%1"" "
"como ""%2"" "
"no consiguió leer el requisito de GruppaDostupa por la causa: "
"%3';ru='У значения доступа ""%1"""
"типа ""%2"""
"не удалось прочитать реквизит ГруппаДоступа по причине:"
"%3';en='Access value ""%1"" "
"type ""%2"""
"Failed to read the ГруппаДоступа attribute due to:"
"%3 '"),
					Строка(ЗначениеДоступа),
					Строка(ТипЗначенияДоступа),
					КраткоеПредставлениеОшибки(Ошибка));
...
Процедура ОбновитьГруппыИсполнителей(ГруппыИсполнителей = Неопределено,
	...
	Иначе
		ВызватьИсключение
			НСтр("es='La falta en el procedimiento de ObnovitGruppyIspolniteley "
"del módulo del manager del registro de las noticias de GruppyZnacheniyDostupa. "
""
"Son indicados los parámetros equivocados.';ru='Ошибка в процедуре ОбновитьГруппыИсполнителей"
"модуля менеджера регистра сведений ГруппыЗначенийДоступа."
""
"Указаны неверные параметры.';en='Error in procedure ОбновитьГруппыИсполнителей information register manager module ГруппыЗначенийДоступа.       Invalid parameters specified.'");
	КонецЕсли;
