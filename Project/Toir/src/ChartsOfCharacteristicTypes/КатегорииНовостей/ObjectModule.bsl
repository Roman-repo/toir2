...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если СписокЗапрещенныхСимволов.Количество() > 0 Тогда
		...
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='En el identificador hay unos símbolos prohibidos: %1. "
"Está permitido usar las cifras, las letras inglesas, el subrayado y el menos.';ru='В идентификаторе присутствуют запрещенные символы: %1."
"Разрешено использовать цифры, английские буквы, подчеркивание и минус.';en='The identifier contains illegal characters:%1."
"Allowed to use numbers, English letters, underscore and minus'"),
	...
	Если ТипЗначения.Типы().Количество() > 1 Тогда
		...
		Сообщение.ПутьКДанным = "Объект";
		Сообщение.Текст = НСтр("es='Los tipos componentes no son apoyados. Escojan solamente un tipo del significado.';ru='Составные типы не поддерживаются. Выберите только один тип значения.';en='Compound types are not supported. Select only one value type.'");
		Сообщение.Сообщить();
	КонецЕсли;
	...
	Если ТипЗначения.Типы().Количество() = 0 Тогда
		...
		Сообщение.Поле = "ТипЗначения";
		Сообщение.ПутьКДанным = "Объект";
		Сообщение.Текст = НСтр("es='Los tipos vacíos no son apoyados. Escojan un cualquier tipo del significado.';ru='Пустые типы не поддерживаются. Выберите один любой тип значения.';en='Empty types are not supported. Select one of any value type.'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ)
	...
	Если ЗагруженоССервера = Истина Тогда
		...
		Если (ПометкаУдаления = Истина)
			...
			ТекстСообщения = НСтр("es='No es posible marcar a la desaparición las categorías cargadas automáticamente del servidor de las novedades.';ru='Нельзя помечать на удаление категории, загруженные автоматически с сервера новостей.';en='Categories that are automatically deleted from the news server can not be marked for deletion.'");
			Сообщение = Новый СообщениеПользователю();
