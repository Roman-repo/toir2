		...
		Для каждого КлючЗначение Из СтруктураДействий Цикл
			...
			ИначеЕсли КлючЗначение.Ключ = "ОбновляетсяССервера" Тогда
				...
				Если Объект.ОбновляетсяССервера = Истина Тогда
					...
					Элементы.ЗначенияКатегорийНовостейСправочник.Заголовок = НСтр("es='La lista de los significados accesibles (se renueva del servidor, la redacción es prohíbida)';ru='Список доступных значений (обновляется с сервера, редактирование запрещено)';en='List of available values ​​(updated from the server, editing is prohibited)'");
				Иначе
					...
					Элементы.ЗначенияКатегорийНовостейСправочник.Заголовок = НСтр("es='La lista de los significados accesibles';ru='Список доступных значений';en='List of available values'");
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция УправлениеФормойСервер(Знач СтруктураПараметров)
	...
	Если СтруктураПараметров.ЗаполняетсяАвтоматически = Истина Тогда
		...
		Если ПустаяСтрока(Значение) Тогда
			СтруктураДействий.Вставить("ФиксированноеЗначение", НСтр("es='Es imposible determinar el significado para esta categoría';ru='Невозможно определить значение для этой категории';en='Unable to determine value for this category.'"));
		...
		Иначе
			СтруктураДействий.Вставить(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Para esta configuración el significado de esta categoría en el momento actual es igual %1';ru='Для этой конфигурации значение этой категории на текущий момент равно %1';en='For this configuration, the value of this category is currently%1'"),
					Значение));
		КонецЕсли;
	Иначе
		...
		Если Объект.ОбновляетсяССервера Тогда
			...
			Если Запись.Выбран() Тогда
				Если Запись.ТекущаяВерсияНаКлиенте >= Запись.ТекущаяВерсияНаСервере Тогда
					ТекстНадписи = НСтр("es='Los datos son actuales y corresponden a los datos del servidor de %ТекущаяВерсияНаСервере del %.';ru='Данные актуальны и соответствуют данным с сервера от %ТекущаяВерсияНаСервере%.';en='The data is relevant and corresponds to the data from the server from  %ТекущаяВерсияНаСервере%.'");
...
"y en el servidor ya la versión de %ТекущаяВерсияНаСервере del %.';ru='Данные никогда не обновлялись с сервера,"
"а на сервере уже версия от %ТекущаяВерсияНаСервере%.';en='Data has never been updated from the server,"
"and the server already has a version from%ТекущаяВерсияНаСервере%.'");
					Иначе
						ТекстНадписи = НСтр("es='La última vez los datos se renovaban del servidor %ТекущаяВерсияНаКлиенте del %, "
"y en el servidor ya la versión de %ТекущаяВерсияНаСервере del %.';ru='Последний раз данные обновлялись с сервера %ТекущаяВерсияНаКлиенте%,"
"а на сервере уже версия от %ТекущаяВерсияНаСервере%.';en='Last time the data was updated from server %ТекущаяВерсияНаКлиенте%,"
"and the server already has a version from %ТекущаяВерсияНаСервере%.'");
					КонецЕсли;
		...
		Иначе
			Элементы.ДекорацияТребуетсяОбновлениеССервера.Заголовок  = НСтр("es='Los datos son introducidos a mano y la renovación del servidor no es necesario.';ru='Данные вводятся вручную и обновление с сервера не требуется.';en='Data is entered manually and updating from the server is not required.'");
			Элементы.ДекорацияТребуетсяОбновлениеССервера.ЦветТекста = ЦветаСтиля.ЦветМикротекста;
		КонецЕсли;
