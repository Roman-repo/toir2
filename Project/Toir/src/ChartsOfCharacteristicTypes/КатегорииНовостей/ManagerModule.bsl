...
Процедура ЗагрузитьСтандартныеЗначения(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИдентификаторШага = НСтр("es='Las novedades. El servicio y el reglamento. La carga de los significados estandartizados. %ИмяСвойства del %. El fin';ru='Новости. Сервис и регламент. Загрузка стандартных значений. %ИмяСвойства%. Конец';en='News. Service and regulations. Loading standard values. %ИмяСвойства%. The end'");
					...
					Попытка
						ОписаниеРезультата = ОписаниеРезультата
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La tentativa de la carga de los significados estandartizados de la línea (los primeros 2000 símbolos): "
"%1';ru='Попытка загрузки стандартных значений из строки (первые 2000 символов):"
"%1';en='Attempting to load standard values from a string (first 2000 characters):"
"%1'")
									+ Символы.ПС,
					...
					Исключение
						...
						ОписаниеРезультата = ОписаниеРезультата
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La falta de la carga de los significados estandartizados de la línea por la causa: "
"%1';ru='Ошибка загрузки стандартных значений из строки по причине:"
"%1';en='Error loading standard values from string due to:"
"%1'"),
...
Процедура ЗагрузитьСтандартноеЗначение(ОбъектХДТО, ОбъектМетаданных, ИмяСвойства, ЛогЗагрузки)
	...
	Попытка
		...
		Если ПустаяСтрока(НайденныйЭлемент.ВерсияДанных) Тогда // Новый объект
			...
			Если ОбъектХДТО.DeletionMark = Истина Тогда
				...
				СтрокаЛогаЗагрузки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La admisión (no es encontrado marcado en la desaparición): %1';ru='Пропуск (не найден помеченный на удаление): %1';en='Skip (not found marked for deletion):%1'"),
					СокрЛП(ОбъектХДТО.Code));
				Записывать = Ложь;
			Иначе
				// Нет ранее созданных элементов с таким же кодом - оставить как есть.
				СтрокаЛогаЗагрузки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La creación: %1';ru='Создание: %1';en='Creation:%1'"),
					СокрЛП(ОбъектХДТО.Code));
			КонецЕсли;
		Иначе // Объект есть в базе данных
			...
			СтрокаЛогаЗагрузки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El cambio (por УИН): %1, %2';ru='Изменение (по УИН): %1, %2';en='Change (by UIN):%1,%2'"),
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ИдентификаторШага = НСтр("es='Las novedades. El servicio y el reglamento. La carga de los significados estandartizados. %ИмяСвойства del %. La falta';ru='Новости. Сервис и регламент. Загрузка стандартных значений. %ИмяСвойства%. Ошибка';en='News. Service and regulations. Loading standard values. %ИмяСвойства%. Error'");
		ИдентификаторШага = СтрЗаменить(ИдентификаторШага, "%ИмяСвойства%", ИмяСвойства);
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió anotar el objeto de los metadatos de la causa: "
"%1';ru='Не удалось записать объект метаданных по причине:"
"%1';en='Failed to write metadata object due to:"
"%1'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		...
		СтрокаЛогаЗагрузки = СтрокаЛогаЗагрузки + ". "
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Había una falta de la anotación: %1';ru='Произошла ошибка записи: %1';en='A write error occurred: %1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
	КонецПопытки;
