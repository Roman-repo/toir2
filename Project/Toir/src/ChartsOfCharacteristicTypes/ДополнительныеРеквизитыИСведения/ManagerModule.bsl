	...
	Попытка
		...
		Если МетаданныеВладельца = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("en='Mistake at change of control of property %1. "
"A set of properties %2 is not connected with one owner of values of properties.';es='La falta con el cambio del ajuste de la propiedad %1. "
"El juego de las propiedades %2 no es vinculado ni a un propietario de los significados de las propiedades.';ru='Ошибка при изменении настройки свойства %1."
"Набор свойств %2 не связан ни с одним владельцем значений свойств.'"),
		...
		ИначеЕсли Свойство = СвойствоОбъект.Ссылка Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("en='Mistake at change of control of property %1. "
"The type of value does not contain additional values.';es='La falta con el cambio del ajuste de la propiedad %1. "
"El tipo del significado no contiene los significados adicionales.';ru='Ошибка при изменении настройки свойства %1."
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("en='It was not succeeded to process an additional requisite (data): %1 for the reason: "
"%2';es='No consiguió tratar el requisito (noticia) adicional: %1 por la causa: "
...
"%2'"), 
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("en='It was not succeeded to process some additional details or data (are passed): %1';es='No consiguió tratar algunos requisitos adicionales o las noticias (son dejadas pasar): %1';ru='Не удалось обработать некоторые дополнительные реквизиты или сведения (пропущены): %1'"), 
				ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			Метаданные.ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения,,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("en='The next portion of additional details (data) is processed: %1';es='Es tratada la porción siguiente de requisitos (noticias) adicionales: %1';ru='Обработана очередная порция дополнительных реквизитов (сведений): %1'"),
					ОбъектовОбработано));
	КонецЕсли;
