...
Процедура РассчитатьПрименениеЗапросов() Экспорт
	...
	Если ТребуетсяПрименениеРазрешенийВКластереСерверов() Тогда
		...
		Исключение
			ВызватьИсключение
				НСтр("es='La falta del acceso competitivo al ajuste de los permisos al uso de los recursos exteriores. "
"Prueben cumplir la operación más tarde.';ru='Ошибка конкурентного доступа к настройке разрешений на использование внешних ресурсов."
"Попробуйте выполнить операцию позже.';en='Error concurrent access to setting permissions to use external resources."
...
Функция НовоеОписаниеПрофиляБезопасности(Знач ТипПрограммногоМодуля, Знач ИдентификаторПрограммногоМодуля)
	...
	Шаблон = НСтр("es='[ИБ %1] %2 ""%3""';ru='[ИБ %1] %2 ""%3""';en='[ИБ %1] %2 ""%3""'");
	...
	Если ПустаяСтрока(ИмяИБ) Тогда
		ВызватьИсключение НСтр("es='La línea de la unión de la base informativa debe contener la base informativa';ru='Строка соединения информационной базы должна содержать информационной базы';en='The infobase connection line must contain the infobase'");
	КонецЕсли;
	
	Если ТипПрограммногоМодуля = Справочники.ИдентификаторыОбъектовМетаданных.ПустаяСсылка() Тогда
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, ИмяИБ,
			НСтр("es='El perfil de la seguridad para la base informativa';ru='Профиль безопасности для информационной базы';en='Security profile for infobase'"), СтрокаСоединенияИнформационнойБазы());
	Иначе
