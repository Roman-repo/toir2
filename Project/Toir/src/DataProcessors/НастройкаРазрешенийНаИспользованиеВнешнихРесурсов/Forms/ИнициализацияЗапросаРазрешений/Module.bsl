...
Функция НачатьОбработкуЗапросов(Знач Запросы, Знач РежимВключения, РежимОтключения, Знач РежимВосстановления, Знач РежимПроверкиПрименения)
	...
	Задание = ФоновыеЗадания.Выполнить("ОбщегоНазначения.ВыполнитьМетодКонфигурации",
			...
			,
			НСтр("es='El tratamiento de las interpelaciones por el uso de los recursos exteriores...';ru='Обработка запросов на использование внешних ресурсов...';en='Processing requests for using external resources ...'"));
...
Функция ЗапросыОбработаны(Знач ИдентификаторЗадания)
	...
	Если Задание = Неопределено Тогда
		ВызватьИсключение(НСтр("es='Al tratamiento de las interpelaciones había una falta - no es encontrada la tarea del tratamiento de las interpelaciones.';ru='При обработке запросов произошла ошибка - не найдено задание обработки запросов.';en='An error occurred while processing requests - the request processing task was not found.'"));
	КонецЕсли;
	...
	Если Задание.Состояние = СостояниеФоновогоЗадания.ЗавершеноАварийно Тогда
		...
		Иначе
			ВызватьИсключение(НСтр("es='Al tratamiento de las interpelaciones había una falta - la tarea del tratamiento de las interpelaciones se ha acabado con la falta desconocida.';ru='При обработке запросов произошла ошибка - задание обработки запросов завершилось с неизвестной ошибкой.';en='An error occurred while processing requests - the request processing task completed with an unknown error.'"));
		КонецЕсли;
	ИначеЕсли Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
		ВызватьИсключение(НСтр("es='Al tratamiento de las interpelaciones había una falta - la tarea del tratamiento de las interpelaciones era anulada por el administrador.';ru='При обработке запросов произошла ошибка - задание обработки запросов было отменено администратором.';en='An error occurred while processing requests - the request processing task was canceled by the administrator.'"));
	Иначе
...
Процедура ОтменитьОбработкуЗапросов(Знач ИдентификаторЗадания)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("es='El ajuste de los permisos al uso de los recursos exteriores. La anulación de la ejecución de la tarea de fondo';ru='Настройка разрешений на использование внешних ресурсов.Отмена выполнения фонового задания';en='Set permissions to use external resources. Cancel a background job'", ОбщегоНазначения.КодОсновногоЯзыка()),
