...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ФормаРазрешить.Заголовок = НСтр("es='Más>';ru='Далее >';en='Next >'");
...
Процедура ПриОткрытии(Отказ)
	...
	#Если МобильныйКлиент Тогда
		ПоказатьПредупреждение(, НСтр("es='Para el trabajo correcto es necesario el régimen del cliente delgado o gordo.';ru='Для корректной работы необходим режим тонкого или толстого клиента.';en='For correct operation, a thin or thick client mode is required.'"));
		Отказ = Истина;
		Возврат;
	#КонецЕсли
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ТребуютсяПараметрыАдминистрированияИнформационнойБазы Тогда
		...
		Если ПользовательИБ = Неопределено Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("es='El usuario indicado no tiene el acceso a la base informativa.';ru='Указанный пользователь не имеет доступа к информационной базе.';en='The specified user does not have access to the infobase.'"),,
				ИмяПоля,,Отказ);
			Возврат;
		КонецЕсли;
		
		Если Не Пользователи.ЭтоПолноправныйПользователь(ПользовательИБ, Истина) Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("es='El usuario no tiene derechos administrativos.';ru='У пользователя нет административных прав.';en='The user does not have administrative rights.'"),,
				ИмяПоля,,Отказ);
			Возврат;
		КонецЕсли;
...
Процедура Далее(Команда)
	...
	Если Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаРазрешения Тогда
		...
		Элементы.ФормаРазрешить.Заголовок = НСтр("es='Ajustar los permisos en el clúster de los servidores';ru='Настроить разрешения в кластере серверов';en='Configure permissions in a server cluster'");
	...
	Для Каждого ЭлементСценария Из СценарийПримененияРазрешений Цикл
		...
		Если ЭлементСценария.Операция = ВидыОпераций.Создание Тогда
			...
			Если АдминистрированиеКластера.ПрофильБезопасностиСуществует(ПараметрыАдминистрированияКластера, ЭлементСценария.Профиль) Тогда
				...
				ОбщегоНазначения.СообщитьПользователю(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El perfil de la seguridad %1 asiste ya en el clúster de los servidores. Los ajustes en el perfil de la seguridad serán sustituidos...';ru='Профиль безопасности %1 уже присутствует в кластере серверов. Настройки в профиле безопасности будут замещены...';en='The security profile%1 is already present in the server cluster. Settings in the security profile will be replaced …'"), ЭлементСценария.Профиль));
		...
		ИначеЕсли ЭлементСценария.Операция = ВидыОпераций.Удаление Тогда
			...
			Иначе
				...
				ОбщегоНазначения.СообщитьПользователю(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El perfil de la seguridad %1 falta en el clúster de los servidores. Probablemente, el perfil de la seguridad era quitado antes...';ru='Профиль безопасности %1 отсутствует в кластере серверов. Возможно, профиль безопасности был удален ранее...';en='Security profile%1 is not in the server cluster. The security profile may have been deleted earlier …'"), ЭлементСценария.Профиль));
