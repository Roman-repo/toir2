...
Функция ВыполнитьОбработкуЗапросовПроверкиПрименения() Экспорт
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение НСтр("es='La transacción es activa';ru='Транзакция активна';en='Transaction active'");
	КонецЕсли;
...
Процедура ЗаблокироватьРегистрыПредоставленныхРазрешений(Знач ПрограммныйМодуль = Неопределено, Знач ЗаблокироватьРежимыПодключения = Истина) Экспорт
	...
	Если Не ТранзакцияАктивна() Тогда
		ВызватьИсключение НСтр("es='Debe ser la transacción activa';ru='Должна быть активная транзакция';en='There must be an active transaction'");
	КонецЕсли;
...
Функция СтрокаТаблицыРазрешений(Знач ТаблицаРазрешений, Знач Отбор, Знач ДобавитьПриОтсутствии = Истина) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La infracción del carácter unical de las líneas en la tabla de los permisos por la selección %1';ru='Нарушение уникальности строк в таблице разрешений по отбору %1';en='Uniqueness of rows in the table of permissions for selection%1'"),
