...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(УзелИнформационнойБазы) Тогда
		...
		Иначе
			ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("es='de la forma. La forma no puede ser abierta.';ru='Не заданы параметры формы. Форма не может быть открыта.';en='No form parameters set. The form cannot be opened.'"), Отказ);
			Возврат;
		КонецЕсли;
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La sincronización de los datos con ""%1""';ru='Синхронизация данных с ""%1""';en='Data synchronization with ""%1""'"), НаименованиеКорреспондента);
...
Процедура ПроверитьПодключениеНаСервере(ЕстьПодключение, ПараметрыАутентификации)
	...
	Если Не ЕстьПодключение Тогда
		СообщениеОбОшибке = НСтр("es='No consiguió conectarse a la aplicación en el Internet, por razón de ""%1"". "
"Se persuadan que: "
"- es introducida la consigna correcta; "
"- es indicada la dirección correcta para la conexión; "
"- la aplicación es accesible a la dirección, indicada en los ajustes; "
"- el ajuste de la sincronización no era quitado en la aplicación en el Internet. "
"Repitan la tentativa de la sincronización.';ru='Не удалось подключиться к приложению в Интернете, по причине ""%1""."
"Убедитесь, что:"
...
"Повторите попытку синхронизации.';en='Failed to connect to the application on the Internet due to ""%1""."
"Make sure that:"
"- The correct password is entered;"
"- The correct address for the connection is specified;"
"- the application is available at the address specified in the settings;"
"- The synchronization setting has not been deleted in the application on the Internet."
"Please try syncing again. '");
	...
	ИначеЕсли Не НастройкаЗавершена Тогда
		СообщениеОбОшибкеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Para la continuación es necesario pasar en el programa ""%1"" y acabar en ella el ajuste de la sincronización. La ejecución del cambio para los datos es anulada.';ru='Для продолжения необходимо перейти в программу ""%1"" и завершить в ней настройку синхронизации. Выполнение обмена данными отменено.';en='To continue, you must go to the program ""% 1"" and complete the synchronization setting in it. Communication has been canceled.'"),
			НаименованиеКорреспондента);
		СинхронизацияДанныхОтключена = Истина;
	ИначеЕсли ПолученыДанныеДляСопоставления Тогда
		СообщениеОбОшибкеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Para la continuación es necesario pasar en el programa ""%1"" y cumplir la carga del mensaje para la comparación de los datos. La ejecución del cambio para los datos es anulada.';ru='Для продолжения необходимо перейти в программу ""%1"" и выполнить загрузку сообщения для сопоставления данных. Выполнение обмена данными отменено.';en='To continue, you must go to the program ""% 1"" and download the message to match the data. Communication has been canceled.'"),
			НаименованиеКорреспондента);
		СинхронизацияДанныхОтключена = Истина;
	КонецЕсли;
...
Функция Подключаемый_ЗапросПароляПользователя_ПриПереходеДалее(Отказ)
	...
	Если Не ПарольСинхронизацииЗадан
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("es='la consigna.';ru='Не указан пароль.';en='No password specified.'"), , "WSПароль", , Отказ);
		Возврат Неопределено;
	КонецЕсли;
...
Функция ФоновоеЗаданиеЗапуститьНаСервере(ПараметрыЗадания, ОшибкаРасхожденияВерсийПриПолученииДанных, ПроверятьРасхождениеВерсий)
	...
	Если ПараметрыЗадания.ВыполнятьЗагрузку Тогда
		...
		ШаблонНаименования = НСтр("es='Se cumple la carga de los datos de %1';ru='Выполняется загрузка данных из %1';en='Loading data from% 1'");
		
	Иначе
		ШаблонНаименования = НСтр("es='Se cumple la descarga de los datos en %1';ru='Выполняется выгрузка данных в %1';en='Unloading data in% 1'");
	КонецЕсли;
...
Процедура ФоновоеЗаданиеПолучитьРезультатНаСервере()
	...
	ТиповоеПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='No consiguió cumplir %1. Los detalles de cm. En la revista del registro';ru='Не удалось выполнить %1. Подробности см. в журнале регистрации';en='Failed to execute%1. See the log for details.'"),
		?(ФоновоеЗаданиеВыполняемоеДействие = 1, НСтр("es='La recepción de los datos';ru='получение данных';en='receiving data'"), НСтр("es='El envío de los datos';ru='отправку данных';en='sending data'")));
