...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		ТекстОшибки = НСтр("es='Es insuficientemente derecho para la ejecución de la operación.';ru='Недостаточно прав для выполнения операции.';en='Not enough rights to perform the operation.'");
		Возврат; // Отказ устанавливается в ПриОткрытии.
	КонецЕсли;
	
	Если ОбщегоНазначения.РазделениеВключено()
		И Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		ТекстОшибки = НСтр("es='Para la desaparición marcado es necesario entrar en la área de los datos.';ru='Для удаления помеченных необходимо войти в область данных.';en='To delete the marked ones, you must enter the data area.'");
		Возврат; // Отказ устанавливается в ПриОткрытии.
	КонецЕсли;
	...
	Если Не ВидимостьРасписания Тогда 
		Элементы.УдалениеПомеченныхИспользование.Заголовок = НСтр("es='Automáticamente quitar los objetos marcados';ru='Автоматически удалять помеченные объекты';en='Automatically delete marked objects'");
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстВопроса = НСтр("es='La desaparición marcado se cumple todavía. "
"¿Interrumpir?';ru='Удаление помеченных еще выполняется."
"Прервать?';en='Deleting marked ones is still in progress."
"Abort? '");
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(КодВозвратаДиалога.Прервать);
	Кнопки.Добавить(КодВозвратаДиалога.Пропустить, НСтр("es='No interrumpir';ru='Не прерывать';en=""Don't interrupt"""));
...
Процедура НеУдаленныеЗаменитьНа(Команда)
	...
	Если МассивСсылок.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("es='Escojan los objetos';ru='Выберите объекты';en='Select objects'"));
		Возврат;
	КонецЕсли;
...
Процедура ПоказатьСвязиНеУдаленныхНаСервере()
	...
	Если СтрокаДерева = Неопределено Или СтрокаДерева.НомерКартинки < 1 Тогда
		...
		НеУдаленныеПодсказка = НСтр("es='Escojan el objeto para conocer la causa, "
"por que no consiguió quitarlo.';ru='Выберите объект, чтобы узнать причину,"
"по которой его не удалось удалить.';en='Select an item to find out the reason"
"which failed to remove it.'");
	Иначе
	...
	Иначе
		...
		Шаблон = НСтр("es='Los lugares del uso del objeto ""%1"" (%2):';ru='Места использования объекта ""%1"" (%2):';en='Places of use of the object ""%1"" (%2):'");
...
Процедура ПометитьВыделенныеОбъектыТаблицыНаУдаление(ТаблицаЭлемент)
	...
	Если КоличествоМожноУдалить = 0 Тогда
		ТекстОшибки = НСтр("es='Escojan el objeto.';ru='Выберите объект.';en='Select an object.'");
		Если КоличествоВыделено = 1 Тогда
			Если ЕстьЗаписиРегистров Тогда
				ТекстОшибки = НСтр("es='La desaparición de la anotación del registro se cumple de su tarjeta.';ru='Удаление записи регистра выполняется из ее карточки.';en='Deleting a register entry is performed from its card.'");
			ИначеЕсли ЕстьКонстанты Тогда
				ТекстОшибки = НСтр("es='La desaparición de la anotación del registro se cumple de su tarjeta.';ru='Удаление записи регистра выполняется из ее карточки.';en='Deleting a register entry is performed from its card.'");
			КонецЕсли;
		Иначе
			Если ЕстьЗаписиРегистров Или ЕстьКонстанты Тогда
				ТекстОшибки = НСтр("es='La desaparición de las anotaciones de los registros o la limpieza de los significados de las constantes se cumple de sus tarjetas.';ru='Удаление записей регистров или очистка значений констант выполняется из их карточек.';en='Deleting register entries or clearing constant values is performed from their cards.'");
			КонецЕсли;
		КонецЕсли;
		ПоказатьПредупреждение(, ТекстОшибки);
		Возврат;
	...
	Иначе
		Если УстанавливатьПометку Тогда
			ТекстВопроса = НСтр("es='¿Marcar los objetos distinguidos (%1) a la desaparición?';ru='Пометить выделенные объекты (%1) на удаление?';en='Mark selected objects (%1) for deletion?'");
		Иначе
			ТекстВопроса = НСтр("es='¿Marcar los objetos distinguidos (%1) a la desaparición?';ru='Пометить выделенные объекты (%1) на удаление?';en='Mark selected objects (%1) for deletion?'");
		КонецЕсли;
...
Процедура ПометитьВыделенныеОбъектыТаблицыНаУдалениеЗавершение(Ответ, ПараметрыВыполнения) Экспорт
	...
	Если КоличествоОбъектов = 0 Тогда
		ОповещениеЗаголовок = НСтр("es='El objeto no es encontrado';ru='Объект не найден';en='Object not found'");
	Иначе
		Если ПараметрыВыполнения.УстанавливатьПометку Тогда
			ОповещениеЗаголовок = НСтр("es='La marca de la desaparición es establecida';ru='Пометка удаления установлена';en='Delete mark set'");
		Иначе
			ОповещениеЗаголовок = НСтр("es='La marca de la desaparición es establecida';ru='Пометка удаления установлена';en='Delete mark set'");
		КонецЕсли;
...
Процедура ВидимостьДоступность(Форма)
	...
	// >> [Деснол] [Ушаков А.Д.] [05.11.2019] 
	Элементы.КнопкаДалее.Заголовок = НСтр("es='Quitar';ru='Удалить';en='Delete'");
	Элементы.КнопкаЗакрыть.Заголовок = НСтр("es='Cerrar';ru='Закрыть';en='Close'");
	Элементы.КнопкаНазад.Заголовок = НСтр("es='< En el comienzo de la';ru='< В начало';en='< In the beginning'");
	...
	Если ТекущаяСтраница = Элементы.СтраницаВыборРежимаУдаления Тогда
		...
		Элементы.КнопкаЗакрыть.Заголовок = НСтр("en='cancel';es='La anulación';ru='Отмена'");
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаПомеченныеНаУдаление Тогда
		Элементы.КнопкаНазад.Видимость = Истина;
		Элементы.КнопкаНазад.Заголовок = НСтр("es='< Atrás';ru='< Назад';en='< Back'");
		Элементы.КнопкаДалее.Видимость = Истина;
		Элементы.КнопкаДалее.Заголовок = НСтр("es='Quitar';ru='Удалить';en='Delete'");
		Элементы.КнопкаДалее.КнопкаПоУмолчанию = Истина;
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаДлительнаяОперация Тогда
		...
		Элементы.КнопкаЗакрыть.Заголовок = НСтр("es='Interrumpir y cerrar';ru='Прервать и закрыть';en='Abort and close'");
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаПричиныНевозможностиУдаления Тогда
		Элементы.КнопкаНазад.Видимость = Истина;
		Элементы.КнопкаДалее.Видимость = Истина;
		Элементы.КнопкаДалее.Заголовок = НСтр("es='Repetir la desaparición';ru='Повторить удаление';en='Repeat deleting >'");
		Элементы.КнопкаДалее.КнопкаПоУмолчанию = Истина;
		// << [Деснол] [Ушаков А.Д.] [05.11.2019] 
...
Процедура УстановитьДоступность()
	...
	Если Элементы.УдалениеПомеченныхНастроитьРасписание.Видимость Тогда
		...
		Иначе
			Представление = НСтр("es='<Es desconectado>';ru='<Отключено>';en='<Отключено>'");
		КонецЕсли;
	...
	Для Каждого Причина Из ПрепятствующиеУдалению Цикл
		...
		Иначе
			...
			Если Причина.ОбнаруженныйСсылка = Неопределено Тогда // Константа
				...
				СтрокаПрепятствующегоУдалению.Представление = ОбнаруженныйИнформация.ПредставлениеЭлемента + " (" + НСтр("es='La constante';ru='Константа';en='Constant'") + ")";
			Иначе
	...
	Если ОбнаруженыРегистры И Не ОбнаруженыСсылочныеНеОтмеченные Тогда
		Если Не ПодписьДобавлена Тогда
			Элементы.НадписьВПодвале.Заголовок = ЗаголовокПодвала + " "
				+ НСтр("es='Para la eliminación de las dependencias difíciles se recomienda repetir la desaparición con el banderín incluido ""Bloquear todo el trabajo en el programa y acelerar la desaparición"" (presionen el botón ""<En el comienzo"").';ru='Для устранения сложных зависимостей рекомендуется повторить удаление с включенным флажком ""Заблокировать всю работу в программе и ускорить удаление"" (нажмите кнопку ""< В начало"").';en='To eliminate complex dependencies, it is recommended to repeat the deletion with the ""Block all work in the program and accelerate deletion"" checkbox turned on (click the ""<Home"") button.'");
		КонецЕсли;
	Иначе
...
Процедура ЗапуститьФоновоеЗаданиеКлиент(Режим)
	...
	Если ПараметрыМетода.ИскатьПомеченные Тогда
		Текст = НСтр("es='La búsqueda de los objetos, marcados a la desaparición...';ru='Поиск помеченных на удаление объектов...';en='Search for objects marked for deletion ...'");
	Иначе
		Текст = НСтр("es='La búsqueda de los objetos, marcados a la desaparición...';ru='Поиск помеченных на удаление объектов...';en='Search for objects marked for deletion ...'");
	КонецЕсли;
	...
	ИначеЕсли Задание.ОшибкаПриУстановкеМонопольногоРежима Тогда
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió bloquear el trabajo en el programa por la causa: "
"%1';ru='Не удалось заблокировать работу в программе по причине: "
"%1';en='Failed to block work in the program due to:"
"%1'"),
...
Функция ЗапуститьФоновоеЗадание(Знач ПараметрыМетода)
	...
	Если ПоказыватьБублик Тогда
		Элементы.НадписьДлительнаяОперация.Заголовок = НСтр("es='Por favor, esperen...';ru='Пожалуйста, подождите...';en='Please, wait...'");
	...
	ИначеЕсли ПараметрыМетода.УдалятьПомеченные Тогда
		Элементы.НадписьДлительнаяОперация.Заголовок = НСтр("es='Se alejan los objetos marcados a la desaparición...';ru='Удаляются объекты, помеченные на удаление...';en='Objects marked for deletion are deleted ...'");
	...
	Иначе
		Элементы.НадписьДлительнаяОперация.Заголовок = НСтр("es='La búsqueda de los objetos marcados a la desaparición...';ru='Поиск объектов, помеченных на удаление...';en='Search for objects marked for deletion ...'");
		Элементы.НадписьДлительнаяОперация.ГоризонтальноеПоложениеВГруппе = ГоризонтальноеПоложениеЭлемента.Центр;
		Элементы.ФоновоеЗаданиеСостояние.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Центр;
	КонецЕсли;
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("es='La desaparición de los objetos marcados (interactivo)';ru='Удаление помеченных объектов (интерактивное)';en='Deleting marked objects (interactive)'");
...
Процедура ПослеЗавершенияФоновогоЗадания(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Выполнено" Тогда
		...
		Если Результат = Неопределено Тогда
			ВывестиОшибку(НСтр("es='No consiguió leer el resultado de la desaparición marcado';ru='Не удалось прочитать результат удаления помеченных';en='Failed to read the result of deleting the marked'"), "");
			Возврат;
		КонецЕсли;
		...
		Если Не ПустаяСтрока(Результат.ОповещениеТекст) Тогда
			ПоказатьОповещениеПользователя(
				НСтр("es='La desaparición marcado';ru='Удаление помеченных';en='Deleting marked'"),
				НавигационнаяСсылка,
	...
	Если РезультатВыполненияВФоне.УдалятьПомеченные Тогда
		...
		ИначеЕсли НеУдаленныеКоличество = 0 Тогда
			...
			Результат.ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es quitado los objetos: %1.';ru='Удалено объектов: %1.';en='Deleted objects:%1.'"),
		...
		Иначе
			...
			Результат.ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es quitado: %1, no es quitado: %2.';ru='Удалено: %1, не удалено: %2.';en='Deleted:%1, not deleted:%2.'"),
				Формат(УдаленныеКоличество, "ЧН=0; ЧГ="),
			...
			Если УдаленныеКоличество = 0 Тогда
				Элементы.НадписьИтогиЧастичногоУдаления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No ha resultado quitar objetos marcados a la desaparición (%1):';ru='Не получилось удалить объекты, помеченные на удаление (%1):';en='Failed to delete objects marked for deletion (%1):'"),
					Формат(НеУдаленныеКоличество, "ЧН=0; ЧГ="));
			Иначе
				Элементы.НадписьИтогиЧастичногоУдаления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Es con éxito quitado: %1 de %2, los otros objetos no son quitados (%3):';ru='Успешно удалено: %1 из %2, остальные объекты не удалены (%3):';en='Successfully deleted:%1 of%2, other objects not deleted (%3):'"),
					Формат(УдаленныеКоличество, "ЧН=0; ЧГ="),
...
Процедура ВывестиОшибку(КраткоеПредставлениеОшибки, ПодробноеПредставлениеОшибки);
	Кратко = НСтр("es='La desaparición no es cumplida por la causa:';ru='Удаление не выполнено по причине:';en='Deleting failed due to:'") + Символы.ПС + КраткоеПредставлениеОшибки;
