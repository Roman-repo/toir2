...
Процедура ПопробоватьУдалитьСсылку(Ссылка, ПараметрыВыполнения, Информация, Результат)
	...
	Если Объект.ПометкаУдаления <> Истина Тогда
		Результат.Успех = Ложь;
		Результат.ИнформацияОбОшибке = НСтр("es='El objeto no es marcado a la desaparición.';ru='Объект не помечен на удаление.';en='Object not marked for deletion.'");
		Возврат;
	КонецЕсли;
	...
	Если Не Результат.Успех Тогда
		Результат.ИнформацияОбОшибке = НСтр("es='El objeto se usa en otros objetos del programa.';ru='Объект используется в других объектах программы.';en='The object is used in other program objects.'");
...
Процедура УдалитьПомеченныеОбъектыМонопольно(ПараметрыВыполнения)
	...
	Пока УдаляемыеОбъекты.Количество() > 0 Цикл
		...
		Если ПрепятствующиеУдалению.Колонки.Количество() < 3 Тогда
			ВызватьИсключение НСтр("es='No consiguió cumplir la desaparición de los objetos.';ru='Не удалось выполнить удаление объектов.';en='Failed to delete objects.'");
		КонецЕсли;
...
Процедура ЗаписатьПредупреждение(Ссылка, ИнформацияОбОшибке)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("es='La desaparición marcado';ru='Удаление помеченных';en='Deleting marked'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Если СтрокаТаблицы.ОбнаруженныйСсылка = Неопределено Тогда
		...
		Иначе
			Причина.ОбнаруженныйСсылка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Son descubiertas las referencias insolubles (%1)';ru='Обнаружены неразрешимые ссылки (%1)';en='Insoluble links found (%1)'"),
...
Процедура ОтметитьПрогрессОбходаКоллекции(ПараметрыВыполнения, ИмяКоллекции)
	...
	Если ИмяКоллекции = "ПередПоискомПомеченныхНаУдаление" Тогда
		...
		Текст = НСтр("es='La preparación para la búsqueda de los objetos marcados a la desaparición.';ru='Подготовка к поиску объектов, помеченных на удаление.';en='Preparing to search for objects marked for deletion.'");
		
	ИначеЕсли ИмяКоллекции = "НайтиПомеченныеНаУдаление" Тогда
		
		Текст = НСтр("es='La búsqueda de los objetos marcados a la desaparición.';ru='Поиск объектов, помеченных на удаление.';en='Search for objects marked for deletion.'");
		
	ИначеЕсли ИмяКоллекции = "ВсеПомеченныеНаУдаление" Тогда
		
		Текст = НСтр("es='El análisis marcado a la desaparición.';ru='Анализ помеченных на удаление.';en='Analysis labeled for deletion.'");
		
	ИначеЕсли ИмяКоллекции = "ТехнологическиеОбъекты" Тогда
		
		Текст = НСтр("es='La preparación para la desaparición.';ru='Подготовка к удалению.';en='Preparing for removal.'");
		
	ИначеЕсли ИмяКоллекции = "МонопольноеУдаление" Тогда
		
		Текст = НСтр("es='Se cumple la desaparición de los objetos.';ru='Выполняется удаление объектов.';en='Object deletion is in progress.'");
	...
	ИначеЕсли ИмяКоллекции = "ПользовательскиеОбъекты" Тогда
		...
		Если НеУдалено = 0 Тогда // Переход на СтрШаблон невозможен.
			Текст = НСтр("es='Es quitado: %1 de %2 objetos.';ru='Удалено: %1 из %2 объектов.';en='Deleted:%1 of%2 objects.'");
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Текст, ПредставлениеНомер, ПредставлениеВсего);
		Иначе
			Текст = НСтр("es='Es tratado: %1 de %2 objetos, de ellos no es quitado: %3.';ru='Обработано: %1 из %2 объектов, из них не удалено: %3.';en='Processed: %1 of %2 objects, of which not deleted: %3.'");
	...
	ИначеЕсли ИмяКоллекции = "ПовторноУдаляемые" Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La comprobación repetida de los objetos alejados: %1 de %2.';ru='Повторная проверка не удаленных объектов: %1 из %2.';en='Re-checking non-deleted objects:%1 of%2.'"),
	...
	ИначеЕсли ИмяКоллекции = "ПрепятствующиеУдалению" Тогда
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El análisis de los objetos que obstaculizan a la desaparición: %1 de %2.';ru='Анализ объектов, препятствующих удалению: %1 из %2.';en='Analysis of objects that prevent deletion:%1 of%2.'"),
			Формат(ПараметрыВыполнения.Номер, "ЧН=0; ЧГ="),
			Формат(ПараметрыВыполнения.Всего, "ЧН=0; ЧГ="));
