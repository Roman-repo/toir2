...
&НаСервере
Процедура ЗаполнитьТЗСоответствиеВероятностиИменамКолонок(ВыборкаКатегорииВероятности)
...
&НаСервере
Процедура ЗаполнитьТаблицыДаннымиДляВыбора(ВыборкаКатегорииТяжести, МассивВыбранныхЗначений)
...
&НаСервере
Процедура ЗаполнитьТаблицуВыбранныхЗначений(ИмяТаблицы, ИмяВероятности,ТекущиеДанные, ТекущийОбъектВоздействияСтроки, ВероятностьПоследствий, ОписаниеВероятности)
...
Процедура ВосстановитьНастройкиВыбранныеЗначения(МассивВыбранныхЗначений)
	...
	Для каждого Строка Из МассивВыбранныхЗначений Цикл
		...
		НужнаяСтрокаСоответствия = ТЗСоответствиеОбъектовВоздействияИменамТаблиц.НайтиСтроки(Новый Структура("ОбъектВоздействия", Строка.ОбъектВоздействия));
		
		Если Не НужнаяСтрокаСоответствия = Неопределено Тогда
			
			ИмяТаблицы = НужнаяСтрокаСоответствия[0].ИмяТаблицы;
			
			КатегорияВероятности = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Строка.КатегорияРиска, "КатегорияВероятности");
			ИмяКолонкиСоответствие = ТЗСоответствиеВероятностиИменамКолонок.НайтиСтроки(Новый Структура("ВероятностьПоследствий", КатегорияВероятности));
			
			Если ИмяКолонкиСоответствие.Количество() > 0 Тогда
				НайдСтрока = ЭтаФорма[ИмяТаблицы].НайтиСтроки(Новый Структура(ИмяКолонкиСоответствие[0].ИмяКолонки,Строка.КатегорияРиска));
				Если НайдСтрока.Количество() > 0 Тогда
					Для каждого КолонкаИмя ИЗ ТЗСоответствиеВероятностиИменамКолонок  Цикл
						
						Если НайдСтрока[0][КолонкаИмя.ИмяКолонки] = Строка.КатегорияРиска Тогда
							ИмяВероятности = СтрЗаменить(КолонкаИмя.ИмяКолонки, "Колонка_", "");
							НайдСтрока[0][КолонкаИмя.ИмяВыбран] = Истина;	
							ОписаниеВероятности = КолонкаИмя.Описание;
							ЗаполнитьТаблицуВыбранныхЗначений(ИмяТаблицы, ИмяВероятности, НайдСтрока[0], НужнаяСтрокаСоответствия, КатегорияВероятности, ОписаниеВероятности); 
							Прервать;
						КонецЕсли; 
					КонецЦикла; 
				КонецЕсли; 
