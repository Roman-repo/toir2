...
Процедура УстановитьРежимВставкиИзБуфераОбмена(ШаблонСДанными, ИнформацияПоКолонкам, ОписаниеТипов) Экспорт
	...
	Для каждого Тип Из ОписаниеТипов.Типы() Цикл
		...
		ЗаголовокКолонки = НСтр("es='Los datos introducidos';ru='Введенные данные';en='Data entered'");
...
Процедура ИнформациюПоКолонкамИзРеквизитовСправочника(ПараметрыЗагрузки, ИнформацияПоКолонкам)
	...
	Для каждого Реквизит Из МетаданныеСправочника.Реквизиты Цикл
		...
		Если Реквизит.Тип.СодержитТип(Тип("Булево")) Тогда 
			ОписаниеТипаКолонки = НСтр("es='La bandera, Sí o 1 / No existe o 0';ru='Флаг, Да или 1 / Нет или 0';en='Flag, Yes or 1 / No or 0'");
		ИначеЕсли Реквизит.Тип.СодержитТип(Тип("Число")) Тогда 
			ОписаниеТипаКолонки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El número, la Longitud: %1, la Exactitud: %2';ru='Число, Длина: %1, Точность: %2';en='Number, Length:%1, Accuracy:%2'"),
		...
		ИначеЕсли Реквизит.Тип.СодержитТип(Тип("Строка")) Тогда
			Если Реквизит.Тип.КвалификаторыСтроки.Длина > 0 Тогда
				...
				ОписаниеТипаКолонки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La línea, макс. La cantidad de los símbolos: %1';ru='Строка, макс. количество символов: %1';en='Line, max number of characters:%1'"), ДлинаСтроки);
			Иначе
				ОписаниеТипаКолонки = НСтр("es='La línea de la longitud ilimitada';ru='Строка неограниченной длины';en='Unlimited line'");
			КонецЕсли;
		...
		ИначеЕсли Реквизит.Тип.СодержитТип(Тип("УникальныйИдентификатор")) Тогда
			ОписаниеТипаКолонки = НСтр("es='УникальныйИдентификатор';ru='УникальныйИдентификатор';en='УникальныйИдентификатор'");
		КонецЕсли;
...
Процедура СоздатьКолонкуСтандартныеРеквизиты(ИнформацияПоКолонкам, МетаданныеСправочника, ИмяКолонки, Позиция)
	...
	Если ТипДанных = Тип("Строка") Тогда 
		ПредставлениеТипа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La línea (no más %1 símbolos)';ru='Строка (не более %1 символов)';en='Line (max %1 characters)'"), ОписаниеТипа.КвалификаторыСтроки.Длина);
	...
	ИначеЕсли ТипДанных = Тип("Число") Тогда
		ПредставлениеТипа = НСтр("es='El número';ru='Число';en='Number'");
	Иначе
...
Процедура ОпределитьИнформацияПоКолонкамТабличнаяЧасть(Знач ИнформацияПоКолонкам, Макет, ОбластьЗаголовокТаблицы, Знач ПараметрыЗагрузки)
	...
	Если ОбъектМетаданных = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La carga de los datos del fichero en la parte tabular no es apoyada para los objetos del tipo: %1';ru='Загрузка данных из файла в табличную часть не поддерживается для объектов типа: %1';en='Loading data from a file into a tabular section is not supported for objects of type:%1'"),
	...
	Иначе
		ВызватьИсключение НСтр("es='del fichero';ru='Не найден макет для загрузки данных из файла';en='No layout found for loading data from file'");
	КонецЕсли;
...
Процедура СоздатьИнформациюПоКолонкамНаОснованиеШаблона(ОбластьЗаголовокТаблицы, ПараметрыЗагрузкиИзФайла, ИнформацияПоКолонкам, ИменаДобавляемыхКолонок = Неопределено)
	...
	Для НомерКолонки = 1 По ОбластьЗаголовокТаблицы.ШиринаТаблицы Цикл
		...
		Если ПредставлениеРеквизита = НСтр("es='<los requisitos Adicionales>';ru='<Дополнительные реквизиты>';en='<Дополнительные реквизиты>'") Тогда
			МетаданныеСправочника = Метаданные.НайтиПоПолномуИмени(ПараметрыЗагрузкиИзФайла.ПолноеИмяОбъекта);
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Свойства") Тогда
				МодульУправлениеСвойствамиСлужебный = ОбщегоНазначения.ОбщийМодуль("УправлениеСвойствамиСлужебный");
				МодульУправлениеСвойствамиСлужебный.КолонкиДляЗагрузкиДанных(МетаданныеСправочника, ИнформацияПоКолонкам);
			КонецЕсли;
		ИначеЕсли ПредставлениеРеквизита = НСтр("es='<la información De contacto>';ru='<Контактная информация>';en='<Контактная информация>'") Тогда
			МетаданныеСправочника = Метаданные.НайтиПоПолномуИмени(ПараметрыЗагрузкиИзФайла.ПолноеИмяОбъекта);
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
				МодульУправлениеКонтактнойИнформацией = ОбщегоНазначения.ОбщийМодуль("УправлениеКонтактнойИнформацией");
				МодульУправлениеКонтактнойИнформацией.КолонкиДляЗагрузкиДанных(МетаданныеСправочника, ИнформацияПоКолонкам);
...
Процедура ЗагрузитьCSVФайлВТаблицу(ИмяФайла, ШаблонСДанными, ИнформацияПоКолонкам)
	...
	Если Строка = Неопределено Тогда 
		ТекстСообщения = НСтр("es='No ha resultado cargar los datos de este fichero. Se persuadan en las correcciones de los datos en el fichero.';ru='Не получилось загрузить данные из этого файла. Убедитесь в корректности данных в файле.';en='Failed to load data from this file. Make sure the data in the file is correct.'");
...
Процедура ЗаписатьСопоставленныеДанные(ПараметрыВыгрузки, АдресХранилища) Экспорт
	...
	Для каждого СтрокаТаблицы Из СопоставленныеДанные Цикл
		...
		Попытка
			Если ЗначениеЗаполнено(СтрокаТаблицы.ОбъектСопоставления) Тогда
				...
				Если ЭлементСправочника = Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La nomenclatura con el artículo %1 no existe.';ru='Номенклатура с артикулом %1 не существует.';en='The nomenclature with article number%1 does not exist.'"),
		...
		Исключение
			...
			НСтр("es='No consiguió anotar el elemento de la guía %1 por la causa: "
"%2';ru='Не удалось записать элемент справочника %1 по причине:"
"%2';en='Failed to write directory item %1 due to:"
"%2 '"), 
...
Процедура СформироватьМакетОтчета(ТаблицаОтчет, ШаблонСДанными)
	...
	ЗаполнитьЯчейкуЗаголовкаМакета(Ячейка, НСтр("es='El resultado';ru='Результат';en='Result'"), 12, НСтр("es='El resultado de la carga de los datos';ru='Результат загрузки данных';en='Data upload result'"), Истина);
...
Функция СобытиеЖурналаРегистрации() 
	...
	Возврат НСтр("es='La carga de los datos del fichero';ru='Загрузка данных из файла';en='Loading data from a file'", ОбщегоНазначения.КодОсновногоЯзыка());
