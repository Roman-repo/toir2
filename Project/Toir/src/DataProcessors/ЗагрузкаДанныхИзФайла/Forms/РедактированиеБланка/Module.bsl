...
Процедура СброситьНастройки(Команда)
	...
	ПоказатьВопрос(Оповещение, НСтр("es='¿Establecer los ajustes de las columnas en el estado inicial?';ru='Установить настройки колонок в первоначальное состояние?';en='Set column settings in original condition?'"), РежимДиалогаВопрос.ДаНет);
КонецПроцедуры
...
Процедура УстановитьОформлениеДанных()
	...
	ЭлементОтбора.Использование = Истина;
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='El nombre estandartizado';ru='Стандартное наименование';en='Standard name'"));
