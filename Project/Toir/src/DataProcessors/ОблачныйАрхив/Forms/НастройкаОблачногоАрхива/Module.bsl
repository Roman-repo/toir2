...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		ЭтотОбъект.ПарольАдминистратораОС = " ";
		Элементы.ПарольАдминистратораОС.ПодсказкаВвода = НСтр("es='Introduzcan la consigna';ru='Введите пароль';en='enter password'");
	КонецЕсли;
	...
	МассивПодстрок.Добавить(
		Новый ФорматированнаяСтрока(
			НСтр("es='La revista del trabajo';ru='Журнал работы';en='Work log'"),,ЦветаСтиля.ЦветГиперссылки,,"backup1C:BackupAgentEventLog"));
	МассивПодстрок.Добавить(" ");
	МассивПодстрок.Добавить(НСтр("es='Del agente de la copia de reserva.';ru='Агента резервного копирования.';en='Backup agent.'"));
	МассивПодстрок.Добавить(Символы.ПС);
	МассивПодстрок.Добавить(Символы.ПС);
	Если ПроверкаНеобходимостиОбновленияАгента.ТребуетсяУстановка = Истина Тогда
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("es='En este ordenador no es establecido el Agente de la copia de reserva.';ru='На этом компьютере не установлен Агент резервного копирования.';en='Backup Agent is not installed on this computer.'"),
				Новый Шрифт(Элементы.ДекорацияЖурналРаботыРезервногоКопированияИВерсияАгента.Шрифт,,,Истина),
				ЦветаСтиля.ЦветОсобогоТекста));
		МассивПодстрок.Добавить(Символы.ПС);
	Иначе
		МассивПодстрок.Добавить(НСтр("es='La versión establecida del Agente de la copia de reserva:';ru='Установленная версия Агента резервного копирования:';en='Installed version of Backup Agent:'"));
		МассивПодстрок.Добавить(" ");
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
		...
		МассивПодстрок.Добавить(Символы.ПС);
	КонецЕсли;
	МассивПодстрок.Добавить(НСтр("es='La versión establecida del subsistema el archivo Nuboso:';ru='Установленная версия подсистемы Облачный архив:';en='Installed version of the Cloud Archive subsystem:'"));
	МассивПодстрок.Добавить(" ");
	МассивПодстрок.Добавить(
		Новый ФорматированнаяСтрока(
			ИнтернетПоддержкаПользователейКлиентСервер.ПользовательскоеПредставлениеНомераВерсии(
	...
	ИначеЕсли ПроверкаНеобходимостиОбновленияАгента.ТребуетсяОбновление = 1 Тогда // Доступно обновление.
		МассивПодстрок.Добавить(НСтр("es='Es accesible una nueva versión del Agente de la copia de reserva:';ru='Доступна новая версия Агента резервного копирования:';en='A new version of Backup Agent is available:'"));
		МассивПодстрок.Добавить(" ");
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
				ИнтернетПоддержкаПользователейКлиентСервер.ПользовательскоеПредставлениеНомераВерсии(
		...
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("es='Renovar';ru='Обновить';en='Update'"),,ЦветаСтиля.ЦветГиперссылки,,"backup1C:UpdateAgentOnThisPC"));
		МассивПодстрок.Добавить(".");
		МассивПодстрок.Добавить(Символы.ПС);
		МассивПодстрок.Добавить(Символы.ПС);
	ИначеЕсли ПроверкаНеобходимостиОбновленияАгента.ТребуетсяОбновление = 2 Тогда // Рекомендуется обновление.
		МассивПодстрок.Добавить(НСтр("es='Se recomienda renovar al Agente de la copia de reserva a la versión:';ru='Рекомендуется обновить Агента резервного копирования на версию:';en='It is recommended to upgrade the Backup Agent to the version:'"));
		МассивПодстрок.Добавить(" ");
	...
	ИначеЕсли ПроверкаНеобходимостиОбновленияАгента.ТребуетсяОбновление = 3 Тогда // Необходимо обновление.
		МассивПодстрок.Добавить(НСтр("es='Es necesario renovar al Agente de la copia de reserva a la versión:';ru='Необходимо обновить Агента резервного копирования на версию:';en='Required to upgrade the Backup Agent to the version:'"));
		МассивПодстрок.Добавить(" ");
		МассивПодстрок.Добавить(Новый ФорматированнаяСтрока(
			ИнтернетПоддержкаПользователейКлиентСервер.ПользовательскоеПредставлениеНомераВерсии(
				ПроверкаНеобходимостиОбновленияАгента.ВерсияНовейшегоАгентаКопирования,
			...
			Если (НЕ ПустаяСтрока(ЭтотОбъект.ЛогинАдминистратораОС)) Тогда
				Если (НЕ ПустаяСтрока(ЭтотОбъект.ПарольАдминистратораОС)) Тогда
					...
					Если КодВозврата <> 0 Тогда
						ТекстСообщения = НСтр("es='Es incorrectamente indicado el login o la consigna';ru='Неверно указан логин или пароль';en='Invalid username or password'");
				...
				Иначе
					...
					ТекстСообщения = НСтр("es='la consigna';ru='Не заполнен пароль';en='Password is not filled'");
					Сообщение = Новый СообщениеПользователю();
					Сообщение.Текст = ТекстСообщения;
...
Процедура ИнициализироватьРасписание()
	...
	Если ЭтотОбъект.РасписаниеВключено = Ложь Тогда
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("es='La inicialización del nuevo horario para la utilidad de la copia de reserva / la Desconexión del horario';ru='Инициализация нового расписания для утилиты резервного копирования / Отключение расписания';en='Initializing a new schedule for the backup utility / Disabling a schedule'"));
		...
		ИнтернетПоддержкаПользователейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El horario es desconectado. "
"La línea de la orden: %1 "
"Catálogo de la instalación del agente de la copia: %2 "
"Código del regreso: %3 "
"Flujo de la conclusión: "
"%4 "
"Flujo de las faltas: "
"%5';ru='Расписание отключено."
"Строка команды: %1"
...
"%4"
"Поток ошибок:"
"%5';en='Schedule is disabled."
"Command line:%1"
"Copy Agent installation Directory:%2"
"Return code:%3"
"Output stream:"
"%4                                               Error stream:"
"%5'"),
	...
	Иначе
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("es='La inicialización del nuevo horario para la utilidad de la copia de reserva / el Cambio / la Creación del fichero de los ajustes';ru='Инициализация нового расписания для утилиты резервного копирования / Изменение / Создание файла настроек';en='Initializing a new schedule for the backup utility / Modify / Create a settings file'"));
		...
		ИнтернетПоддержкаПользователейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
			...
			0,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El fichero del horario es formado. "
"El nombre del fichero: %1';ru='Файл расписания сформирован."
"Имя файла: %1';en='Schedule file generated."
"File name:%1 '"),
				ИмяВременногоФайлаНастроекИБ),
			Неопределено);
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("es='La inicialización del nuevo horario para la utilidad de la copia de reserva / el Cambio / el Lanzamiento de la utilidad';ru='Инициализация нового расписания для утилиты резервного копирования / Изменение / Запуск утилиты';en='Initialize a new schedule for the backup utility / Change / Launch the utility'"));
		...
		ИнтернетПоддержкаПользователейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
			...
			0,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El lanzamiento de la utilidad. "
"La línea de la orden: %1 "
"Catálogo de la instalación del agente de la copia: %2 "
"Código del regreso: %3';ru='Запуск утилиты."
"Строка команды: %1"
"Каталог установки агента копирования: %2"
"Код возврата: %3';en='Launch the utility."
"Command line:%1"
"Copy Agent Installation Directory:%2"
"Return Code:%3 '"),
				СтрокаКоманды,
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			НСтр("es='La inicialización del nuevo horario para la utilidad de la copia de reserva / el Cambio / la Desaparición de los ficheros temporales';ru='Инициализация нового расписания для утилиты резервного копирования / Изменение / Удаление временных файлов';en='Initialize a new schedule for the backup utility / Modify / Delete temporary files'"));

			Попытка
				УдалитьФайлы(ИмяВременногоФайлаНастроекИБ);
				ПодробноеСообщение = НСтр("es='La desaparición del fichero de los ajustes ha pasado con éxito';ru='Удаление файла настроек прошло успешно';en='Deleting of settings file was successful'");
			Исключение
...
"El nombre del fichero de los ajustes: %1 "
"Detalles: "
"%2';ru='Удаление временных файлов."
"Имя файла настроек: %1"
"Подробности:"
"%2';en='Deleting temporary files."
"Settings file name:%1"
"Details:"
"%2'"),
				ИмяВременногоФайлаНастроекИБ,
				ПодробноеСообщение),
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La inicialización del nuevo horario para la utilidad de la copia de reserva / el Recuento del horario';ru='Инициализация нового расписания для утилиты резервного копирования / Пересчет расписания';en='Initializing a new schedule for backup utility / Schedule recalculation'"));
	...
	ИнтернетПоддержкаПользователейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
		...
		0,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El recuento del horario. "
"La fecha de la sesión: %1';ru='Пересчет расписания."
"Дата сеанса: %1';en='ecalculation of the schedule."
"Session date:%1'"),
			ТекущаяДатаСеанса),
		Неопределено);
	...
	ИнтернетПоддержкаПользователейКлиентСервер.ЗарегистрироватьРезультатВыполненияОперации(
		...
		НСтр("es='La inicialización del nuevo horario para la utilidad de la copia de reserva / el Lanzamiento al cliente web';ru='Инициализация нового расписания для утилиты резервного копирования / Запуск в веб-клиенте';en='Initialize a new schedule for the backup utility / Launch in the web client'"),
		0,
	...
	ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
		...
		НСтр("es='El archivo nuboso. El trabajo con las utilidades exteriores. El lanzamiento del Agente de la copia de reserva';ru='Облачный архив. Работа с внешними утилитами. Запуск Агента резервного копирования';en='Cloud Archive. Work with external utilities. Starting Backup agent'"), // ИдентификаторШага
...
Процедура УправлениеФормой(Форма)
	...
	Если Форма.РасписаниеВключено = Истина Тогда
		МассивПодстрок = Новый Массив;
		МассивПодстрок.Добавить(НСтр("es='Cumplir la copia automática de reserva';ru='Выполнять автоматическое резервное копирование';en='Perform automatic backup'"));
		МассивПодстрок.Добавить(" ");
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
	...
	Иначе
		...
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("es='Por el horario';ru='по расписанию';en='As scheduled'"), // Содержимое
				, // Шрифт
				Форма.ЦветГиперссылки, // ЦветТекста
				, // ЦветФона
				"backup1C:EditBackupScheduler")); // Ссылка
	...
	Если ОблачныйАрхивКлиентСервер.ЕстьЗапрещенныеСимволыДляКоманднойСтроки(Форма.ЛогинАдминистратораОС) Тогда
		Элементы.ДекорацияЛогинАдминистратораОС.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es descubierto un de los símbolos prohibidos: %1. "
"Esto puede llevar a la falta del lanzamiento del Agente de la copia de reserva.';ru='Обнаружен один из запрещенных символов: %1."
"Это может привести к ошибке запуска Агента резервного копирования.';en='One of the forbidden characters was found:%1."
"This may result in an error starting the Backup Agent. '"),
