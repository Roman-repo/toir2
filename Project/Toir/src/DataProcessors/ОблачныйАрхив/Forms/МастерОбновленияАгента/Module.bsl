...
#Область ДекорацияИнформацияОНовомАгентеРезервногоКопированияТекст
	...
	МассивПодстрок.Добавить(НСтр("es='La versión del distributivo:';ru='Версия дистрибутива:';en='Distribution version:'"));
	...
	МассивПодстрок.Добавить(НСтр("es='La dimensión del distributivo:';ru='Размер дистрибутива:';en='Distribution size:'"));
	МассивПодстрок.Добавить(" ");
	МассивПодстрок.Добавить(
		Новый ФорматированнаяСтрока(
	...
	Если НЕ ПустаяСтрока(ЭтотОбъект.ТекстЧтоНовогоВВерсии) Тогда
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
					НСтр("es='Nuevo en las versiones.';ru='Новое в версии.';en='New in version.'"),
					,
	...
	Если НЕ ПустаяСтрока(ЭтотОбъект.ТекстПорядокОбновления) Тогда
		МассивПодстрок.Добавить(НСтр("es='Ante la instalación de la renovación es necesario ponerse al corriente con';ru='Перед установкой обновления необходимо ознакомиться с';en='Before installing the update, you must familiarize yourself with'"));
		МассивПодстрок.Добавить(" ");
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
					НСтр("es='Por los rasgos del tránsito';ru='особенностями перехода';en='transition features'"),
					,
					ЦветаСтиля.ЦветГиперссылки,
					,
					"backup1C:BackupAgentUpdate_HowToUpdate")); // Идентификатор.
		МассивПодстрок.Добавить(" ");
		МассивПодстрок.Добавить(НСтр("es='A esta versión del Agente de la copia de reserva.';ru='на эту версию Агента резервного копирования.';en='to this version of Backup Agent.'"));
		МассивПодстрок.Добавить(Символы.ПС);
		МассивПодстрок.Добавить(Символы.ПС);
	КонецЕсли;

	Если ЭтотОбъект.АгентКопированияУстановлен Тогда
		МассивПодстрок.Добавить(НСтр("es='Es establecido el Agente de la copia de reserva de la versión:';ru='Установлен Агент резервного копирования версии:';en='Installed Backup Agent version:'"));
		МассивПодстрок.Добавить(" ");
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
					ИнтернетПоддержкаПользователейКлиентСервер.ПользовательскоеПредставлениеНомераВерсии(
		...
		МассивПодстрок.Добавить(Символы.ПС);
	Иначе
		МассивПодстрок.Добавить(НСтр("es='El agente de la copia de reserva no es establecido';ru='Агент резервного копирования не установлен';en='Backup agent is not installed'"));
		МассивПодстрок.Добавить(Символы.ПС);
		МассивПодстрок.Добавить(Символы.ПС);
	КонецЕсли;
	...
	Иначе
		...
		Иначе
			Элементы.ДекорацияУстановкаАгентаРезервногоКопированияЗавершенаСОшибкамиТекст.Заголовок =
				Новый ФорматированнаяСтрока(
					НСтр("es='Para la renovación del Agente de la copia de reserva es necesario poseer los derechos del administrador Windows. "
""
"Durante la renovación había unas faltas. "
"La información sobre las faltas es conservada en la revista del registro.';ru='Для обновления Агента резервного копирования необходимо обладать правами администратора Windows."
""
"Во время обновления произошли ошибки."
"Информация об ошибках сохранена в журнал регистрации.';en='To upgrade the Backup Agent, you must have Windows administrator rights."
""
"Errors occurred during update."
"Error information is saved to the log.'")
						+ Символы.ПС,
					Новый ФорматированнаяСтрока(
						НСтр("es='La revista del registro.';ru='Журнал регистрации.';en='Registration log.'"),
						,
						ЦветаСтиля.ЦветГиперссылки,
						,
	...
	ИначеЕсли ИмяТекущейСтраницы = "СтраницаОбновлениеАгентаРезервногоКопирования" Тогда
		Если НаправлениеПерехода = "Далее" Тогда
			...
			Элементы.ДекорацияДлительнаяОперацияТекст.Заголовок =
				НСтр("es='La carga y la renovación del Agente de la copia de reserva.';ru='Загрузка и обновление Агента резервного копирования.';en='Download and update Backup Agent.'");
...
#Область Окончание
		...
		Если ЕстьОшибки = Истина Тогда
			ОбновитьНадписиНаСервере(НСтр("es='Hay unas faltas';ru='Есть ошибки';en='There are mistakes'"));
