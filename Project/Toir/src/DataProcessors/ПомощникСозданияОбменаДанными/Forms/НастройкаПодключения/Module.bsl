...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("es='¿Interrumpir el ajuste de los parámetros de la conexión para la sincronización de los datos?';ru='Прервать настройку параметров подключения для синхронизации данных?';en='Abort connection settings for data synchronization?'");
...
Процедура ИмяФайлаНастроекПодключенияДляЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НастройкиДиалога.Вставить("Заголовок", НСтр("es='Escojan el fichero con los ajustes de la conexión';ru='Выберите файл с настройками подключения';en='Select a file with connection settings'"));
	НастройкиДиалога.Вставить("Фильтр",    НСтр("es='El fichero de los ajustes de la conexión (*.xml)';ru='Файл настроек подключения (*.xml)';en='Connection settings File (* .xml)'") + "|*.xml");
...
Процедура ИмяФайлаНастроекXDTOКорреспондентаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НастройкиДиалога = Новый Структура;
	НастройкиДиалога.Вставить("Заголовок", НСтр("es='Escojan el fichero con los ajustes del corresponsal';ru='Выберите файл с настройками корреспондента';en='Select a file with the correspondent settings'"));
	НастройкиДиалога.Вставить("Фильтр",    НСтр("es='El fichero de los ajustes del corresponsal (*.xml)';ru='Файл настроек корреспондента (*.xml)';en='Correspondent settings file (* .xml)'") + "|*.xml");
...
Процедура ИмяФайлаНастроекПодключенияДляВыгрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НастройкиДиалога.Вставить("Заголовок", НСтр("es='Indiquen el fichero para la conservación de los ajustes de la conexión';ru='Укажите файл для сохранения настроек подключения';en='Specify a file to save connection settings'"));
	НастройкиДиалога.Вставить("Фильтр",    НСтр("es='El fichero de los ajustes de la conexión (*.xml)';ru='Файл настроек подключения (*.xml)';en='Connection settings File (* .xml)'") + "|*.xml");
	...
	Иначе
		...
		Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='No consiguió conectarse al programa. Comprueben los ajustes de la conexión.';ru='Не удалось подключиться к программе. Проверьте настройки подключения.';en='Failed to connect to the program. Check your connection settings.'"));
		КонецЕсли;
	КонецЕсли;
		...
		Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No consiguió conectarse al programa. Comprueben los ajustes de la conexión %1.';ru='Не удалось подключиться к программе. Проверьте настройки подключения %1.';en='Failed to connect to the program. Check connection settings%1.'"), ПроверяемыйВидТранспорта));
		КонецЕсли;
...
Процедура ПолучениеФайлаНастроекПодключенияЗавершение(ПолученныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ПолученныеФайлы = Неопределено Тогда
		...
		Результат.СообщениеОбОшибке = НСтр("es='No consiguió conservar los ajustes de la conexión en el fichero.';ru='Не удалось сохранить настройки подключения в файл.';en='Failed to save connection settings to file.'");
	КонецЕсли;
...
Процедура СохранениеНастроекПодключенияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Если ПустаяСтрока(ТекстСообщения) Тогда
			ТекстСообщения = НСтр("es='No consiguió conservar los ajustes de la conexión.';ru='Не удалось сохранить настройки подключения.';en='Failed to save connection settings.'");
		КонецЕсли;
...
Процедура ПриЗавершенииПомещенияФайлаНастроекПодключенияДляЗагрузки(РезультатВыбора, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Отказ Тогда
		Если ПустаяСтрока(Результат.СообщениеОбОшибке) Тогда
			Результат.СообщениеОбОшибке = НСтр("es='No consiguió cargar el fichero con los ajustes de la conexión.';ru='Не удалось загрузить файл с настройками подключения.';en='Failed to load connection settings file.'");
		КонецЕсли;
	Иначе
...
Процедура ПриЗавершенииПомещенияФайлаНастроекXDTOКорреспондента(РезультатВыбора, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Отказ Тогда
		Если ПустаяСтрока(Результат.СообщениеОбОшибке) Тогда
			Результат.СообщениеОбОшибке = НСтр("es='No consiguió cargar el fichero con los ajustes del corresponsal.';ru='Не удалось загрузить файл с настройками корреспондента.';en='Failed to load correspondent settings file.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьВозможностьИспользованияФормы(Отказ = Ложь)
	...
	Если Не Параметры.Свойство("ИмяПланаОбмена")
		...
		ТекстСообщения = НСтр("es='La forma no es destinada al uso directo.';ru='Форма не предназначена для непосредственного использования.';en='The form is not intended for direct use.'");
...
Процедура ИнициализироватьСвойстваФормы()
	...
	Если НастройкаРИБ Тогда
		Заголовок = НСтр("es='El ajuste de la base distribuida informativa';ru='Настройка распределенной информационной базы';en='Setting up a distributed infobase'");
	Иначе
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El ajuste de la conexión a ""%1""';ru='Настройка подключения к ""%1""';en='Setting connection to ""%1""'"),
...
Функция Подключаемый_СтраницаСпособНастройкиПодключения_ПриПереходеДалее(Отказ)
	...
	Если СпособНастройкиПодключения = "ПриложениеИзСписка" Тогда
		...
		Если ТекущиеДанные = Неопределено Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='Escojan la aplicación de la lista para la continuación del ajuste de la conexión.';ru='Выберите приложение из списка для продолжения настройки подключения.';en='Select an application from the list to continue setting up the connection.'"),
				, "ПриложенияВСервисе", , Отказ);
			Возврат Неопределено;
		ИначеЕсли ТекущиеДанные.НастройкаСинхронизацииВМенеджереСервиса Тогда
			ПоказатьПредупреждение(, НСтр("es='Para el ajuste de la sincronización de los datos con la aplicación escogida pasen en el manager del servicio. "
"En el manager del servicio se aprovechen de la orden ""la Sincronización de los datos"".';ru='Для настройки синхронизации данных с выбранным приложением перейдите в менеджер сервиса."
"В менеджере сервиса воспользуйтесь командой ""Синхронизация данных"".';en='To configure data synchronization with the selected application, go to the service manager."
"In the service manager, use the command ""Data sync""'"));
			Отказ = Истина;
			Возврат Неопределено;
		Иначе
	...
	ИначеЕсли СпособНастройкиПодключения = "НастроитьВручную" Тогда
		...
		Если ВидПодключения = "ОбычныеКаналыСвязи"
			...
			Если ПустаяСтрока(ИмяФайлаНастроекПодключенияДляЗагрузки) Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(
					НСтр("es='Escojan el fichero con los ajustes de la conexión.';ru='Выберите файл с настройками подключения.';en='Select a file with connection settings.'"),
		...
		ИначеЕсли ВидПодключения = "ПассивныйРежим" Тогда
			Если ПустаяСтрока(ИмяФайлаНастроекXDTOКорреспондента) Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(
					НСтр("es='Escojan el fichero con los ajustes del programa-corresponsal.';ru='Выберите файл с настройками программы-корреспондента.';en='Select the file with the settings of the correspondent program.'"),
...
Функция Подключаемый_СтраницаПараметрыПодключенияИнтернет_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(ИнтернетВебАдрес) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='Indiquen la dirección del programa en el Internet.';ru='Укажите адрес программы в Интернет.';en='Specify the address of the program on the Internet.'"),
...
Функция Подключаемый_СтраницаПараметрыПодключенияВнешнееСоединение_ПриПереходеДалее(Отказ)
	...
	Если ВнешнееСоединениеВариантРаботыИнформационнойБазы = "Файловый" Тогда
		...
		Если ПустаяСтрока(ВнешнееСоединениеКаталогИнформационнойБазы) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='Escojan el catálogo de la disposición de la base informativa.';ru='Выберите каталог расположения информационной базы.';en='Select an infobase location directory.'"),
	...
	ИначеЕсли ВнешнееСоединениеВариантРаботыИнформационнойБазы = "КлиентСерверный" Тогда
		...
		Если ПустаяСтрока(ВнешнееСоединениеКластерСерверов) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='Indiquen el nombre del clúster de los servidores 1С:Предприятия.';ru='Укажите имя кластера серверов 1С:Предприятия.';en='Specify the name of the 1C: Enterprise server cluster.'"),
		...
		Если ПустаяСтрока(ВнешнееСоединениеИмяИнформационнойБазы) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='Indiquen el nombre de la base informativa en el clúster de los servidores 1С:Предприятия.';ru='Укажите имя информационной базы в кластере серверов 1С:Предприятия.';en='Specify the name of the infobase in the 1C: Enterprise server cluster.'"),
...
Функция Подключаемый_СтраницаПараметрыПодключенияОбычныеКаналыСвязи_ПриПереходеДалее(Отказ)
	...
	Если Не ОбычныеКаналыСвязиFILEИспользование
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='Escojan por lo menos un modo de la transmisión de los ficheros con los datos.';ru='Выберите хотя бы один способ передачи файлов с данными.';en='Please select at least one data file transfer method.'"),
	...
	Если ОбычныеКаналыСвязиАрхивироватьФайлы
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='Indiquen la consigna para la archivación de los ficheros.';ru='Укажите пароль для архивации файлов.';en='Specify a password for archiving files.'"),
	...
	Если ОбычныеКаналыСвязиFILEИспользование Тогда
		...
		Если ПустаяСтрока(ОбычныеКаналыСвязиFILEКаталог) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='Escojan el catálogo para la transmisión de los ficheros con los datos.';ru='Выберите каталог для передачи файлов с данными.';en='Select a directory for transferring data files.'"),
	...
	Если ОбычныеКаналыСвязиFTPИспользование Тогда
		...
		Если ПустаяСтрока(ОбычныеКаналыСвязиFTPПуть) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='Indiquen la dirección del catálogo para la transmisión de los ficheros con los datos.';ru='Укажите адрес каталога для передачи файлов с данными.';en='Specify the directory address for transferring data files.'"),
		...
		Если ОбычныеКаналыСвязиFTPИспользоватьОграничениеРазмераФайла
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='Indiquen la dimensión máxima admisible de los ficheros con los datos.';ru='Укажите максимальный допустимый размер файлов с данными.';en='Specify the maximum size for data files.'"),
	...
	Если ОбычныеКаналыСвязиEMAILИспользование Тогда
		...
		Если Не ЗначениеЗаполнено(ОбычныеКаналыСвязиEMAILУчетнаяЗапись) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='Escojan la anotación de descuento del correo electrónico para el envío de los mensajes con los datos.';ru='Выберите учетную запись электронной почты для отправки сообщений с данными.';en='Select an email account to send data messages.'"),
		...
		Если ОбычныеКаналыСвязиEMAILИспользоватьОграничениеРазмераВложения
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='Indiquen la dimensión máxima admisible de la inversión postal.';ru='Укажите максимальный допустимый размер почтового вложения.';en='Specify the maximum allowed mail attachment size.'"),
...
Функция Подключаемый_СтраницаОбщиеНастройкиСинхронизации_ПриПереходеДалее(Отказ)
	...
	Если Не ВидПодключения = "ПассивныйРежим" Тогда
		...
		Если ДоступноИзменениеНаименования
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='Indiquen el nombre de este programa.';ru='Укажите наименование этой программы.';en='Specify the name of this program.'"),
				, "Наименование", , Отказ);
		КонецЕсли;
		...
		Если ДоступноИзменениеПрефикса
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='Indiquen el prefijo de este programa.';ru='Укажите префикс этой программы.';en='Specify the prefix for this program.'"),
				, "Префикс", , Отказ);
		КонецЕсли;
	...
	Если ДоступноИзменениеНаименованияКорреспондента
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='Indiquen el nombre del programa-corresponsal.';ru='Укажите наименование программы-корреспондента.';en='Specify the name of the correspondent program.'"),
	...
	Если Не ВидПодключения = "ПассивныйРежим" Тогда
		...
		Если ДоступноИзменениеПрефиксаКорреспондента
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='Indiquen el prefijo del programa-corresponsal.';ru='Укажите префикс программы-корреспондента.';en='Specify the prefix for the correspondent program.'"),
	...
	Если СохранитьПараметрыПодключенияВФайл
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='Indiquen la vía al fichero para la conservación de los ajustes de la conexión.';ru='Укажите путь к файлу для сохранения настроек подключения.';en='Specify the path to the file to save the connection settings.'"),
