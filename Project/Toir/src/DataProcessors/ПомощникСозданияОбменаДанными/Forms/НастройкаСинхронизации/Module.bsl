...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(УзелОбмена)
		...
		ТекстПредупреждения = НСтр("es='El ajuste de la sincronización de los datos no es acabado. "
"¿Acabar el trabajo con el ayudante? Se podrá continuar el ajuste más tarde.';ru='Настройка синхронизации данных еще не завершена."
"Завершить работу с помощником? Настройку можно будет продолжить позже.';en='Setting up data synchronization is not yet complete."
"Finish work with assistant? You can continue the setup later.'");
...
Процедура НастроитьПараметрыПодключения(Команда)
	...
	Если ЭтоОбменСПриложениемВСервисе
		...
		СтрокаПредупреждение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El ajuste de la conexión a ""%1"" es cumplido ya. "
"La redacción de los parámetros de la conexión no es prevista.';ru='Настройка подключения к ""%1"" уже выполнена."
"Редактирование параметров подключения не предусмотрено.';en='Connection to ""%1"" has already been configured.'"), УзелОбмена);
...
Процедура ПолучитьПодтверждениеПодключения(Команда)
	...
	Если Не ОбменДаннымиСВнешнейСистемой
		...
		ПоказатьПредупреждение(, НСтр("es='La conexión es confirmada.';ru='Подключение подтверждено.';en='Connection confirmed.'"));
		Возврат;
	КонецЕсли;
...
Процедура НастроитьПравилаОтправкиИПолученияДанныхПродолжение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если НастройкаВыполнена Тогда
		ПоказатьПредупреждение(, НСтр("es='El ajuste de las reglas del envío y la recepción de los datos es cumplido.';ru='Настройка правил отправки и получения данных выполнена.';en='The rules for sending and receiving data have been configured.'"));
	...
	Если ПустаяСтрока(Заголовок) Тогда
		...
		Иначе
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El ajuste de la sincronización de los datos con ""%1""';ru='Настройка синхронизации данных с ""%1""';en='Setting data synchronization with ""%1""'"),
...
Процедура УстановитьНачальноеОтображениеЭлементовФормы()
	...
	Если РезервноеКопирование Тогда
		...
		ЧастиНадписиРезервноеКопирование.Добавить(НСтр("es='Ante el comienzo del ajuste de la nueva sincronización de los datos se recomienda';ru='Перед началом настройки новой синхронизации данных рекомендуется';en='Before you start setting up a new data synchronization, it is recommended'"));
		ЧастиНадписиРезервноеКопирование.Добавить(" ");
		ЧастиНадписиРезервноеКопирование.Добавить(
			Новый ФорматированнаяСтрока(НСтр("es='Crear la copia de reserva de los datos';ru='создать резервную копию данных';en='Create back up data'"), , , , НавигационнаяСсылкаОбработкиРезервногоКопирования));
		ЧастиНадписиРезервноеКопирование.Добавить(".");
