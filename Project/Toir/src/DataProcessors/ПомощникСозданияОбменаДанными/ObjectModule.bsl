...
Процедура ВнешнееСоединениеВыполнитьДействияПоНастройкеНовогоОбменаДанными(Отказ, 
	...
	Попытка
		...
		Если Отказ Тогда
			ВызватьИсключение(НСтр("es='A la creación del ajuste de la sincronización de los datos han surgido las faltas.';ru='При создании настройки синхронизации данных возникли ошибки.';en='Errors occurred while creating data synchronization settings.'"));
		КонецЕсли;
...
"Perhaps the infobase is not a subordinate node in the RIB.'");
	...
	Иначе
		...
		Иначе
			ВызватьИсключение НСтр("es='El significado del prefijo de la primera base informativa no es único. "
"En el sistema hay ya una sincronización de los datos para la base (programa) informativa con el prefijo indicado.';ru='Значение префикса первой информационной базы не уникально."
"В системе уже существует синхронизация данных для информационной базы (программы) с указанным префиксом.';en='The value of the prefix of the first infobase is not unique."
"The system already has data synchronization for the infobase (program) with the specified prefix.'");
		КонецЕсли;
...
Процедура ВыполнитьЗагрузкуПараметровМастера(Отказ, СтрокаXML) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		ПолеСтрокаСообщенияОбОшибке = НСтр("es='La sincronización de los datos con este programa en el régimen del servicio no es prevista.';ru='Синхронизация данных с этой программой в режиме сервиса не предусмотрена.';en='Data synchronization with this program in service mode is not provided.'");
