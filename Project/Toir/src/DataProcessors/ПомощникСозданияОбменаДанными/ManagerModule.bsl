...
Процедура ПриНачалеПроверкиПодключения(НастройкиПодключения, ПараметрыОбработчика, ПродолжитьОжидание = Истина) Экспорт
	...
	КлючФоновогоЗадания = КлючФоновогоЗадания(НастройкиПодключения.ИмяПланаОбмена,			
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La comprobación de la conexión %1';ru='Проверка подключения %1';en='Connection check %1'"), НастройкиПодключения.ВидТранспортаСообщенийОбмена));

	Если ЕстьАктивныеФоновыеЗадания(КлючФоновогоЗадания) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La comprobación de la conexión %1 se cumple ya.';ru='Проверка подключения %1 уже выполняется.';en='Connection check %1 is already in progress.'"), НастройкиПодключения.ВидТранспортаСообщенийОбмена);
	КонецЕсли;
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La comprobación de la conexión al corresponsal: %1.';ru='Проверка подключения к корреспонденту: %1.';en='Checking correspondent connection:%1.'"), НастройкиПодключения.ВидТранспортаСообщенийОбмена);
...
Процедура ПриНачалеСохраненияНастроекПодключения(НастройкиПодключения, ПараметрыОбработчика, ПродолжитьОжидание = Истина) Экспорт
	...
	КлючФоновогоЗадания = КлючФоновогоЗадания(НастройкиПодключения.ИмяПланаОбмена,
		НСтр("es='La conservación de los ajustes de la conexión';ru='Сохранение настроек подключения';en='Saving connection settings'"));

	Если ЕстьАктивныеФоновыеЗадания(КлючФоновогоЗадания) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La conservación de los ajustes de la conexión para ""%1"" se cumple ya.';ru='Сохранение настроек подключения для ""%1"" уже выполняется.';en='Saving connection settings for ""%1"" is already in progress.'"), НастройкиПодключения.ИмяПланаОбмена);
	КонецЕсли;
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(Новый УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La conservación de los ajustes de la conexión: %1.';ru='Сохранение настроек подключения: %1.';en='Saving connection settings:%1.'"), НастройкиПодключения.ИмяПланаОбмена);
	ПараметрыВыполнения.КлючФоновогоЗадания = КлючФоновогоЗадания;
	ПараметрыВыполнения.ЗапуститьНеВФоне    = Ложь;
	ПараметрыВыполнения.ЗапуститьВФоне      = Истина;
...
Процедура ПриНачалеСохраненияНастроекСинхронизации(НастройкиСинхронизации, ПараметрыОбработчика, ПродолжитьОжидание = Истина) Экспорт
	...
	КлючФоновогоЗадания = КлючФоновогоЗадания(ИмяПланаОбмена,
		НСтр("es='La conservación de los ajustes de la sincronización de los datos';ru='Сохранение настроек синхронизации данных';en='Saving data synchronization settings'"));

	Если ЕстьАктивныеФоновыеЗадания(КлючФоновогоЗадания) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La conservación de los ajustes de la sincronización de los datos para ""%1"" se cumple ya.';ru='Сохранение настроек синхронизации данных для ""%1"" уже выполняется.';en='Saving data synchronization settings for ""%1"" is already in progress.'"), ИмяПланаОбмена);
	КонецЕсли;
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(Новый УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La conservación de los ajustes de la sincronización de los datos: %1.';ru='Сохранение настроек синхронизации данных: %1.';en='Saving data synchronization settings:%1.'"), ИмяПланаОбмена);
	ПараметрыВыполнения.КлючФоновогоЗадания = КлючФоновогоЗадания;
	ПараметрыВыполнения.ЗапуститьНеВФоне    = Ложь;
	ПараметрыВыполнения.ЗапуститьВФоне      = Истина;
...
Процедура ПриНачалеУдаленияНастройкиСинхронизации(НастройкиУдаления, ПараметрыОбработчика, ПродолжитьОжидание = Истина) Экспорт
	...
	КлючФоновогоЗадания = КлючФоновогоЗадания(ИмяПланаОбмена,
		НСтр("es='La desaparición del ajuste de la sincronización de los datos';ru='Удаление настройки синхронизации данных';en='Deleting data sync settings'"));

	Если ЕстьАктивныеФоновыеЗадания(КлючФоновогоЗадания) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La desaparición del ajuste de la sincronización de los datos para ""%1"" se cumple ya.';ru='Удаление настройки синхронизации данных для ""%1"" уже выполняется.';en='Deleting data synchronization setting for ""%1"" is already in progress.'"), ИмяПланаОбмена);
	КонецЕсли;
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(Новый УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La desaparición del ajuste de la sincronización de los datos: %1.';ru='Удаление настройки синхронизации данных: %1.';en='Deleting data synchronization settings: %1.'"), ИмяПланаОбмена);
	ПараметрыВыполнения.КлючФоновогоЗадания = КлючФоновогоЗадания;
	ПараметрыВыполнения.ЗапуститьНеВФоне    = Ложь;
	ПараметрыВыполнения.ЗапуститьВФоне      = Истина;
...
Процедура ПриНачалеРегистрацииДанныхДляНачальнойВыгрузки(НастройкиРегистрации, ПараметрыОбработчика, ПродолжитьОжидание = Истина) Экспорт
	...
	КлючФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El registro de los datos para la descarga inicial (%1)';ru='Регистрация данных для начальной выгрузки (%1)';en='Logging data for initial upload (%1)'"),
		НастройкиРегистрации.УзелОбмена);

	Если ЕстьАктивныеФоновыеЗадания(КлючФоновогоЗадания) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El registro de los datos para la descarga inicial para ""%1"" se cumple ya.';ru='Регистрация данных для начальной выгрузки для ""%1"" уже выполняется.';en='Logging data for initial upload for ""%1"" is already in progress.'"),
			НастройкиРегистрации.УзелОбмена);
	КонецЕсли;
...
Процедура ПриНачалеЗагрузкиНастроекXDTO(НастройкиЗагрузки, ПараметрыОбработчика, ПродолжитьОжидание = Истина) Экспорт
	...
	КлючФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La carga de los ajustes XDTO (%1)';ru='Загрузка настроек XDTO (%1)';en='Download XDTO Settings (%1)'"),
		НастройкиЗагрузки.УзелОбмена);

	Если ЕстьАктивныеФоновыеЗадания(КлючФоновогоЗадания) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La carga de los ajustes XDTO para ""%1"" se cumple ya.';ru='Загрузка настроек XDTO для ""%1"" уже выполняется.';en='The XDTO settings for ""%1"" are already loading.'"),
			НастройкиЗагрузки.УзелОбмена);
	КонецЕсли;
...
Процедура ПриНачалеПолученияВариантовНастроекОбменаДанными(УникальныйИдентификатор, ПараметрыОбработчика, ПродолжитьОжидание) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ОбменДаннымиСВнешнимиСистемами") Тогда
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='La recepción de las variantes accesibles de los ajustes del cambio para los datos con los sistemas exteriores.';ru='Получение доступных вариантов настроек обмена данными с внешними системами.';en='Getting available options for setting data exchange with external systems.'");
...
Функция КлючФоновогоЗадания(ИмяПланаОбмена, Действие)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='ПланОбмена: el %1 Acción: el %2';ru='ПланОбмена:%1 Действие:%2';en='ПланОбмена:%1 Action:%2'"), ИмяПланаОбмена, Действие);
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(ОбменДаннымиСервер.СобытиеЖурналаРегистрацииСозданиеОбменаДанными(),
			УровеньЖурналаРегистрации.Ошибка, , , СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='A la ejecución del elaborador PriPodklyucheniiKkorrespondentu había una falta: el %1%2';ru='При выполнении обработчика ПриПодключенииККорреспонденту произошла ошибка:%1%2';en='An error occurred while executing the handler ПриПодключенииККорреспонденту: %1%2'"),
		...
		Если РезультатПроверки.ВерсииИнтерфейса.Найти("3.0.1.1") <> Неопределено Тогда
			...
			Если Не ПараметрыКорреспондента.ПланОбменаСуществует Тогда
				РезультатПроверки.СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='En el corresponsal no es encontrado el plan del cambio ""%1"". "
"Se persuadan que"
" - es escogido el tipo correcto de la aplicación para el ajuste del cambio;"
" - es correctamente indicada la disposición del programa, a que se cumple la conexión.';ru='В корреспонденте не найден план обмена ""%1""."
"Убедитесь, что"
" - выбран правильный вид приложения для настройки обмена;"
" - корректно указано расположение программы, к которой выполняется подключение.';en='No correspondence plan ""%1"" was found in the correspondent."
"Make sure that"
"  - the correct type of application is selected for exchange settings;"
"  - The location of the program to which the connection is made is correctly indicated.'"),
		...
		Иначе
			...
			РезультатПроверки.СообщениеОбОшибке = НСтр("es='El corresponsal no apoya la versión 3.0.1.1 interfaces ""ОбменДанными"". "
"El ajuste de la conexión tiene que renovar la configuración del corresponsal, o comenzar el ajuste de él.';ru='Корреспондент не поддерживает версию 3.0.1.1 интерфейса ""ОбменДанными""."
"Для настройки подключения необходимо обновить конфигурацию корреспондента, или начать настройку из него.';en='The correspondent does not support version 3.0.1.1 of the ""ОбменДанными"" interface."
"To configure the connection, you must update the configuration of the correspondent, or start the configuration from it.'");
...
"Se persuadan que"
" - es escogido el tipo correcto de la aplicación para el ajuste del cambio;"
" - es indicada la dirección correcta de la aplicación en el Internet.';ru='В корреспонденте не найден план обмена ""%1""."
"Убедитесь, что"
" - выбран правильный вид приложения для настройки обмена;"
" - указан правильный адрес приложения в Интернете.';en='No correspondence plan ""% 1"" was found in the correspondent."
"Make sure that"
"  - the correct type of application is selected for exchange settings;"
"  - The correct address of the application on the Internet is indicated.'"),
					НастройкиПодключения.ИмяПланаОбмена);
...
Процедура ЗагрузитьНастройкиXDTOКорреспондента(Параметры, АдресРезультата) Экспорт
	...
	Если Отказ Тогда
		...
		Если ПустаяСтрока(Результат.СообщениеОбОшибке) Тогда
			Результат.СообщениеОбОшибке = НСтр("es='No consiguió recibir los parámetros del corresponsal.';ru='Не удалось получить параметры корреспондента.';en='Failed to get correspondent settings.'");
		КонецЕсли;
	Иначе
...
Процедура УдалитьНастройкуСинхронизации(Параметры, АдресРезультата) Экспорт
	...
	Если НастройкиУдаления.УдалитьНастройкуВКорреспонденте Тогда
		...
		Если Не Результат.НастройкаУдаленаВКорреспонденте Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='A la desaparición del ajuste de la sincronización en el programa ""%1"" ha surgido la falta: %2. "
""
"Prueben cumplir la desaparición más tarde, o quiten el banderín ""Quitar el ajuste también en el programa..."".';ru='При удалении настройки синхронизации в программе ""%1"" возникла ошибка: %2."
""
"Попробуйте выполнить удаление позже, или снимите флажок ""Удалить настройку также в программе..."".';en='An error occurred while deleting the synchronization settings in the program ""%1"" :%2."
""
"Try uninstalling later, or uncheck ""Uninstall the setting also in the program ... "".'"),
...
Процедура УдалитьНастройкуСинхронизацииВКорреспонденте(НастройкиУдаления, Результат)
	...
	Если ВидТранспорта = Перечисления.ВидыТранспортаСообщенийОбмена.COM Тогда
		...
		Если УзелКорреспондента = Неопределено Тогда
			Результат.СообщениеОбОшибкеВКорреспонденте = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='En el programa-corresponsal no es encontrado el nudo del plan del cambio ""%1"" por el código ""%2"".';ru='В программе-корреспонденте не найден узел плана обмена ""%1"" по коду ""%2"".';en='No exchange plan node ""% 1"" for code ""% 2"" was found in the correspondent program.'"),
	...
	Иначе
		...
		Результат.СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Para el tipo de la conexión ""%1"" no es apoyada la desaparición del ajuste de la sincronización a la parte del programa-corresponsal.';ru='Для вида подключения ""%1"" не поддерживается удаление настройки синхронизации на стороне программы-корреспондента.';en='For connection type ""%1"", deleting synchronization settings on the side of the correspondent program is not supported.'"),
			ВидТранспорта);
	КонецЕсли;
...
Процедура СоздатьОбновитьУзлыПланаОбмена(НастройкиПодключения)
	...
	Если ЭтоПланОбменаРИБ
		...
		Если ГлавныйУзел = Неопределено Тогда
			...
			ВызватьИсключение НСтр("es='El nudo principal para la base corriente informativa no es determinado. "
"Probablemente, la base informativa no es el nudo subordinado en РИБ.';ru='Главный узел для текущей информационной базы не определен."
"Возможно, информационная база не является подчиненным узлом в РИБ.';en='The main node for the current infobase is not defined."
"Perhaps the infobase is not a subordinate node in the RIB.'");
	...
	Иначе
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El significado del prefijo del programa ""%1"" no es único (""%2""). Hay Ya un ajuste de la sincronización con el mismo prefijo. "
"La continuación tiene que fijar en el programa ""%1"" prefijo único de la base informativa excelente de corriente.';ru='Значение префикса программы ""%1"" не уникально (""%2""). Уже существует настройка синхронизации с таким же префиксом."
"Для продолжения необходимо назначить в программе ""%1"" уникальный префикс информационной базы, отличный от текущего.';en='The value of the program prefix ""%1"" is not unique (""%2""). There is already a synchronization setting with the same prefix."
"To continue, you must assign a unique infobase prefix in the program ""%1 "" that is different from the current one.'"),
...
Процедура ЗаполнитьНастройкиПодключенияИзXML(НастройкиПодключения,
	...
	Если Не КорректныйФайлНастроек Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El fichero no contiene los ajustes de la conexión para el cambio escogido por los datos. "
"Es escogido el cambio ""%1"". "
"En el fichero son indicados los ajustes para el cambio ""%2"".';ru='Файл не содержит настроек подключения для выбранного обмена данными."
"Выбран обмен ""%1""."
"В файле указаны настройки для обмена ""%2"".';en='File does not contain connection settings for the selected data exchange."
"The exchange ""%1"" has been selected."
"The file contains settings for the exchange ""%2"". '"),
	...
	Если НастройкиПодключения.ВариантРаботыМастера = "ПродолжитьНастройкуОбменаДанными"
		...
		Если Не ПустаяСтрока(ПрефиксИБ)
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El prefijo del programa indicado al ajuste (""%1""), no corresponde al prefijo en este programa (""%2""). "
"La continuación tiene que comenzar el ajuste de nuevo de otro programa, e indicar el prefijo correcto (""%2"").';ru='Префикс программы, указанный при настройке (""%1""), не соответствует префиксу в этой программе (""%2"")."
"Для продолжения необходимо начать настройку заново из другой программы, и указать корректный префикс (""%2"").';en='The program prefix specified during configuration (""%1"") does not match the prefix in this program (""%2""). To continue, you must start the setup again from another program, and specify the correct prefix (""%2"").'"),
	...
	Если НастройкиПодключения.ВерсияФорматаФайлаНастроекОбменаДанными = "1.0" Тогда
		...
		НастройкиПодключения.НаименованиеЭтойБазы    = НСтр("es='Esta base informativa';ru='Эта информационная база';en='This infobase'");
...
Процедура СчитатьДанныеВСтруктуру(СтруктураНастроек, ЧтениеXML)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента Тогда
		ВызватьИсключение НСтр("es='La falta de la análisis XML.';ru='Ошибка разбора XML.';en='XML parsing error.'");
	КонецЕсли;
...
Функция НастройкиXDTOКорреспондентаИзXML(ИмяФайлаСтрокаXML, ЭтоФайл, УзелОбмена) Экспорт
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El identificador del remitente <%1> en el fichero con los ajustes EnterpriseData no corresponde al formato GUID esperado.';ru='Идентификатор отправителя <%1> в файле с настройками EnterpriseData не соответствует ожидаемому формату GUID.';en='Sender identifier <%1> in the EnterpriseData settings file does not match the expected GUID format.'"),
			УИД);
	КонецПопытки;
