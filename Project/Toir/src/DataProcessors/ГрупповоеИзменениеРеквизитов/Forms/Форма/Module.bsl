...
Процедура ВидОперацииПриИзменении(Элемент)
	...
	Если Объект.ТипОперации = "ВыполнитьАлгоритм" Тогда
		...
		Элементы.ФормаИзменить.Заголовок = НСтр("es='Cumplir';ru='Выполнить';en='Run'");
	...
	Иначе
		...
		Элементы.ФормаИзменить.Заголовок = НСтр("es='Cambiar los requisitos';ru='Изменить реквизиты';en='Change details'");
...
Процедура Изменить(Команда)
	...
	Если НазначениеКнопки = "Изменить" Тогда
		Если Не ЕстьВыбранныеОбъекты() Тогда
			ПоказатьПредупреждение(, НСтр("es='el cambio';ru='Не указаны элементы для изменения';en='Items to change is not specified'"));
			Возврат;
		КонецЕсли;
		...
		Иначе
			ТекстВопроса = НСтр("es='La selección no es dada. ¿Cambiar todos los elementos?';ru='Отбор не задан. Изменить все элементы?';en='No selection specified. Change all items?'");
			ОписаниеОповещения = Новый ОписаниеОповещения("ВыполнитьИзменениеПроверкаОтбораВыполнена", ЭтотОбъект);
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, , , НСтр("es='El cambio de los elementos';ru='Изменение элементов';en='Change items'"));
		КонецЕсли;
...
Процедура ВыполнитьИзменениеПроверкаОтбораВыполнена(РезультатВопроса = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ЕстьНастроенныеИзменения() И Объект.ТипОперации = "ВвестиЗначения" Тогда
		ТекстВопроса = НСтр("es='Los cambios no son ajustados. ¿Cumplir перезапись de los elementos sin cambios?';ru='Изменения не настроены. Выполнить перезапись элементов без изменений?';en='Changes are not configured. Overwrite items unchanged?'");
...
Процедура ВыполнитьДействияПриКонтекстномОткрытии()
	...
	ШаблонЗаголовка = НСтр("es='El cambio de los elementos distinguidos ""%1"" (%2)';ru='Изменение выделенных элементов ""%1"" (%2)';en=""Change selected items '%1' (%2)""");
	...
	Если ТипыОбъектов.Количество() > 1 Тогда
		ПредставлениеТипа = "";
		ШаблонЗаголовка = НСтр("es='El cambio de los elementos distinguidos (%2)';ru='Изменение выделенных элементов (%2)';en='Change selected items (%2)'");
	КонецЕсли;
...
Процедура ВопросПерейтиКРазблокированиюРеквизитов(ВыбранныйРеквизит)
	...
	Если КонтекстныйВызов Тогда
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("es='Pasar';ru='Перейти';en='Go to'"));
		ТекстВопроса = НСтр("es='¿El requisito es bloqueado, pasar al desbloqueo de los requisitos?';ru='Реквизит заблокирован, перейти к разблокированию реквизитов?';en='Props blocked, go to unlock details?'");
	Иначе
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("es='Desbloquear';ru='Разблокировать';en='Unlock'"));
		ТекстВопроса = ПодставитьПараметрыВСтроку(
			НСтр("es='Para no permitir la disonancia de los datos en el programa, "
"el requisito ""%1"" no es accesible para la redacción. "
""
"Antes de que permitir su redacción, se recomienda estimar las consecuencias, "
"habiendo comprobado todos los lugares del uso de los elementos escogidos en el programa. "
""
"¿Desbloquear el requisito ""%1"" para el cambio?';ru='Для того чтобы не допустить рассогласования данных в программе,"
...
""
"Разблокировать реквизит ""%1"" для изменения?';en='In order to prevent data inconsistencies in the program,"
"props """"%1 """" is not editable."
""
"Before allowing its editing, it is recommended to evaluate the consequences,"
"having checked all places of use of the selected elements in the program."
""
"Unlock props ""%1"" for change?'"),
	...
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки, , КодВозвратаДиалога.Да, НСтр("es='El requisito es bloqueado';ru='Реквизит заблокирован';en='Props blocked'"));
...
Процедура УстановитьКнопкиНаВремяИзменения(НачалоИзменения)
	...
	Если НачалоИзменения Тогда
		Элементы.ФормаИзменить.Заголовок = НСтр("es='Interrumpir';ru='Прервать';en='Abort'");
	Иначе
		Если ОбъектыКоторыеНеУдалосьИзменить.Количество() > 0 Тогда
			Элементы.ФормаИзменить.Заголовок = НСтр("es='Repetir el cambio';ru='Повторить изменение';en='Repeat change'");
		Иначе
			Элементы.ФормаИзменить.Заголовок = НСтр("es='Cerrar';ru='Закрыть';en='Close'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ИзменитьОбъекты()
	...
	Если Объект.ИзменятьВТранзакции Тогда
		...
		Если ТОНастройкаПорции = 1 Тогда // обработка одним вызовом
			...
			ПоказатьОповещениеПользователя(НСтр("es='El cambio de los elementos distinguidos';ru='Изменение выделенных элементов';en='Change selected items'"), ,НСтр("es='Esperen por favor, el tratamiento puede ocupar un tiempo...';ru='Пожалуйста подождите, обработка может занять некоторое время...';en='Please wait, processing may take some time ...'"));
	...
	Иначе
		...
		Состояние(НСтр("es='Son tratados los elementos...';ru='Обрабатываются элементы...';en='Elements are being processed ...'"), 0, НСтр("es='El cambio de los elementos distinguidos';ru='Изменение выделенных элементов';en='Change selected items'"));
...
Процедура ОбработатьРезультатИзменения(РезультатИзменения = Неопределено, ПродолжитьОбработку = Неопределено)
	...
	Пока ПродолжитьОбработку Цикл
		...
		ТекстВопроса = НСтр("es='Con el cambio de los elementos (el grupo de los elementos) han surgido las faltas. "
"¿Interrumpir el cambio de los elementos y pasar a la presentación de las faltas?';ru='При изменении элементов (группы элементов) возникли ошибки."
"Прервать изменение элементов и перейти к просмотру ошибок?';en='Errors occurred while changing items (group of items)."
"Abort item changes and go to error viewing?'");
		Кнопки = Новый СписокЗначений;
		...
		Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("es='Más preguntar';ru='Больше не спрашивать';en=""Don't ask again"""));
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ОбработатьРезультатИзмененияОтветПолучен", ЭтотОбъект, РезультатИзменения);
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки, , КодВозвратаДиалога.Прервать, НСтр("es='Las faltas con el cambio de los elementos';ru='Ошибки при изменении элементов';en='Errors when changing items'"));
		Возврат;
	КонецЦикла;
	...
	Если ТекущееСостояниеИзменения.ПоказыватьПроцентОбработанных Тогда
		...
		Состояние(НСтр("es='Son tratados los elementos...';ru='Обрабатываются элементы...';en='Elements are being processed ...'"), ТекущийПроцент, НСтр("es='El cambio de los elementos distinguidos';ru='Изменение выделенных элементов';en='Change selected items'"));
	КонецЕсли;
...
Процедура ЗавершитьИзменениеОбъектов()
	...
	Если ОбработкаЗавершена Тогда
		ПоказатьОповещениеПользователя(НСтр("es='El cambio de los requisitos de los elementos';ru='Изменение реквизитов элементов';en='Change item details'"), , 
			ПодставитьПараметрыВСтроку(НСтр("es='Son cambiados los elementos (%1).';ru='Изменены элементы (%1).';en='Items changed (%1).'"), ТекущееСостояниеИзменения.КоличествоИзмененных));
	...
	Если ОбработкаЗавершена Тогда
		ШаблонСообщения = НСтр("es='Los cambios son cumplidos en todos los elementos escogidos (%2).';ru='Изменения выполнены во всех выбранных элементах (%2).';en='Changes were made to all selected items (%2).'");
	Иначе
		Если Объект.ИзменятьВТранзакции Или ТекущееСостояниеИзменения.КоличествоИзмененных = 0 Тогда
			ШаблонСообщения = НСтр("es='Los cambios no son cumplidos.';ru='Изменения не выполнены.';en='Changes not implemented.'");
		Иначе
			ШаблонСообщения = НСтр("es='Los cambios son cumplidos parcialmente. "
"Es cambiado: %1; no consiguió cambiar: %3';ru='Изменения выполнены частично."
"Изменено: %1; Не удалось изменить: %3';en='Partial changes."
"Changed:%1; Failed to change:%3'");
		КонецЕсли;
	КонецЕсли;
	...
	Если Объект.ИзменятьВТранзакции И Не ОбработкаЗавершена Тогда
		...
		Если КоличествоПропущенных > 0 И Не ТекущееСостояниеИзменения.ПрерватьИзменение Тогда
			...
			СтрокаТаблицы.Объект = ПодставитьПараметрыВСтроку(НСтр("es='... Y otros elementos (%1)';ru='... и другие элементы (%1)';en='... and other elements (% 1)'"), КоличествоПропущенных);
			СтрокаТаблицы.Причина = НСтр("es='Son dejados pasar, puesto que no eran cambiado unos o más elementos.';ru='Пропущены, так как не были изменены один или более элементов.';en='Skipped because one or more items have not been changed.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура НазадСервер()
	...
	Если Объект.ТипОперации = "ВыполнитьАлгоритм" Тогда
		Элементы.ФормаИзменить.Заголовок = НСтр("es='Cumplir';ru='Выполнить';en='Run'");
		Элементы.ФормаИзменить.РасширеннаяПодсказка.Заголовок = НСтр("es='Cumplir el algoritmo';ru='Выполнить алгоритм';en='Run algorithm'");
	Иначе
		Элементы.ФормаИзменить.Заголовок = НСтр("es='Cambiar los requisitos';ru='Изменить реквизиты';en='Change details'");
	КонецЕсли;
...
Процедура ПерейтиНаСтраницуВсеГотово()
	...
	Элементы.НадписьУспешноЗавершено.Заголовок = ПодставитьПараметрыВСтроку(
		НСтр("es='Los requisitos de los elementos escogidos eran cambiados. "
"Es cambiado en total los elementos: %1';ru='Реквизиты выбранных элементов были изменены."
"Всего изменено элементов: %1';en='Props of selected items have been changed."
"Total items changed:%1 '"), ТекущееСостояниеИзменения.КоличествоИзмененных);
	Элементы.ФормаИзменить.Заголовок = НСтр("es='Es preparado';ru='Готово';en='Done'");
	Элементы.ФормаНазад.Видимость = Истина;
...
Процедура СформироватьПояснениеПоАвтонумерации()
	...
	ИначеЕсли Автонумерация Тогда
		ПояснениеПоАвтонумерации = НСтр("es='<Establecer es automático>';ru='<Установить автоматически>';en='<Установить автоматически>'");
	Иначе
		ПояснениеПоАвтонумерации = НСтр("es='<Limpiar>';ru='<Очистить>';en='<Очистить>'");
	КонецЕсли;
...
Функция ОписанияКолонокТаблицыРеквизитов()
	...
	ОписаниеКолонки.Тип = Новый ОписаниеТипов("Строка");
	ОписаниеКолонки.Представление = НСтр("es='El requisito';ru='Реквизит';en='Props'");
	...
	ОписаниеКолонки.Представление = НСтр("es='Un nuevo significado';ru='Новое значение';en='New value'");
	ОписаниеКолонки.ВидПоля = ВидПоляФормы.ПолеВвода;
...
Процедура СформироватьПояснениеОНастроенныхИзменениях()
	...
	Если ИзменяемыеРеквизиты.Количество() = 1 Тогда
		Пояснение = НСтр("es='El requisito';ru='реквизит';en='props'") + " " + Пояснение;
	ИначеЕсли ИзменяемыеРеквизиты.Количество() > 1 Тогда
		Пояснение = НСтр("es='Los requisitos';ru='реквизиты';en='props'") + " " + Пояснение;
	КонецЕсли;
	
	Если Не ПустаяСтрока(Пояснение) Тогда
		Пояснение = Пояснение + " " + НСтр("es='En los elementos escogidos';ru='в выбранных элементах';en='in selected items'");
	КонецЕсли;
	...
	Для Каждого ТабличнаяЧасть Из ИзменяемыеТабличныеЧасти Цикл
		...
		Если ИзменяемыеРеквизиты.Количество() > 3 Тогда
			Если Не ПустаяСтрока(Пояснение) Тогда
				Пояснение = Пояснение + ", ";
			КонецЕсли;
			Пояснение = Пояснение + ПодставитьПараметрыВСтроку(НСтр("es='Los requisitos (%1)';ru='реквизиты (%1)';en='props (%1)'"), ИзменяемыеРеквизиты.Количество());
		Иначе
			Для Каждого Реквизит Из ИзменяемыеРеквизиты Цикл
				Если Не ПустаяСтрока(Пояснение) Тогда
					Пояснение = Пояснение + ", ";
				...
				Если ИзменяемыеРеквизиты.Найти(Реквизит) = 0 Тогда
					Если ИзменяемыеРеквизиты.Количество() = 1 Тогда
						Пояснение = Пояснение + НСтр("es='El requisito';ru='реквизит';en='props'") + " ";
					ИначеЕсли ИзменяемыеРеквизиты.Количество() > 1 Тогда
						Пояснение =  Пояснение + НСтр("es='Los requisitos';ru='реквизиты';en='props'") + " ";
					КонецЕсли;
				КонецЕсли;
				Пояснение = Пояснение + """" + Реквизит + """";
			КонецЦикла;
		КонецЕсли;
		Пояснение = Пояснение + " " 
			+ ПодставитьПараметрыВСтроку(НСтр("es='En la parte tabular ""%1""';ru='в табличной части ""%1""';en=""in tabular portion '% 1'"""), ТабличнаяЧасть.Ключ);
	КонецЦикла;
	
	Если Не ПустаяСтрока(Пояснение) Тогда
		Если ИзменяемыеТабличныеЧасти.Количество() > 0 Тогда
			Если ЕстьОтборПоСтрокам Тогда 
				Пояснение = Пояснение + " " + НСтр("es='En aquellas líneas de los elementos escogidos, que satisfacen a las condiciones de la selección';ru='в тех строках выбранных элементов, которые удовлетворяют условиям отбора';en='in those rows of selected items that satisfy the selection conditions'")
			Иначе
				Пояснение = Пояснение + " " + НСтр("es='<b> en todas las líneas </b> los elementos escogidos';ru='<b>во всех строках</b> выбранных элементов';en='<b> in all rows </b> of selected items'")
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЕстьВыбранныеОбъекты() Тогда
		Если Не ПустаяСтрока(Пояснение) Тогда
			Пояснение = НСтр("es='Cambiar';ru='Изменить';en='Change'") + " " + Пояснение + ".";
		Иначе
			Пояснение = НСтр("es='Cumplir <b> перезапись </b> los elementos escogidos.';ru='Выполнить <b>перезапись</b> выбранных элементов.';en='Perform <b> overwrite </b> of the selected items.'");
		КонецЕсли;
	Иначе
		Пояснение = НСтр("es='los elementos, que requisitos es necesario cambiar.';ru='Не выбраны элементы, реквизиты которых необходимо изменить.';en='No items were selected whose details you need to change.'");
	КонецЕсли;
...
Процедура ОбновитьНадписьКоличествоВыбранных()
	...
	Иначе
		ТекстНадписи = НСтр("es='Todos los elementos';ru='Все элементы';en='All items'");
	КонецЕсли;
...
Процедура УстановитьНастройкуИзменения(Знач Настройка)
	...
	Если ЕстьЗаблокированные Тогда
		ПоказатьПредупреждение(, НСтр("es='Algunos requisitos son bloqueados para el cambio, los cambios no son establecidos.';ru='Некоторые реквизиты заблокированы для изменения, изменения не установлены.';en='Some details are blocked for change, changes are not installed.'"));
	КонецЕсли;
...
Функция ПредставлениеСвязейПараметровВыбора(СвязиПараметровВыбора, ОбъектМетаданных)
		...
		Если ВладелецРеквизитов <> Неопределено Тогда
			...
			Если Реквизит <> Неопределено Тогда
				...
				Если Не ПустаяСтрока(ПредставлениеТабличнойЧасти) Тогда
					ПредставлениеРеквизита = ПредставлениеРеквизита + " (" + НСтр("es='La tabla';ru='таблица';en='table'") + " " 
						+ ПредставлениеТабличнойЧасти + ")";
				КонецЕсли;
	...
	Если СвязанныеРеквизиты.Количество() > 0 Тогда
		ШаблонПредставленияСвязей = НСтр("es='Depende de los requisitos: %1.';ru='Зависит от реквизитов: %1.';en='Depends on the props :% 1.'");
		Если СвязанныеРеквизиты.Количество() = 1 Тогда
			ШаблонПредставленияСвязей = НСтр("es='Depende del requisito %1.';ru='Зависит от реквизита %1.';en='Depends on the props %1.'");
		КонецЕсли;
...
Функция ПараметрыКонструктора(Формула)
	...
	Результат.Вставить("ОперандыЗаголовок", НСтр("es='Los requisitos accesibles';ru='Доступные реквизиты';en='Available details'"));
...
Функция МенеджерОбъектаПоПолномуИмени(ПолноеИмя)
	...
	ИначеЕсли ВРег(КлассОМ) = "РЕГИСТРРАСЧЕТА" Тогда
		...
		Иначе
			...
			Иначе
				ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("es='El tipo desconocido del objeto metadado ""%1""';ru='Неизвестный тип объекта метаданных ""%1""';en=""Unknown metadata object type '%1'"""), ПолноеИмя);
			КонецЕсли;
		КонецЕсли;
...
Функция ВидОбъектаПоТипу(Тип)
	...
	Иначе
		ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("es='El tipo equivocado del significado de parámetro (%1)';ru='Неверный тип значения параметра (%1)';en='Invalid parameter value type (%1)'"), Строка(Тип));
...
Функция ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты)
	...
	ИначеЕсли ТипЗнч(Реквизиты) = Тип("Массив") Или ТипЗнч(Реквизиты) = Тип("ФиксированныйМассив") Тогда
		...
		КонецЦикла;
	Иначе
		ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("es='El tipo equivocado del segundo parámetro los Requisitos: %1';ru='Неверный тип второго параметра Реквизиты: %1';en='Invalid type of the second parameter Requisites:%1'"), Строка(ТипЗнч(Реквизиты)));
	КонецЕсли;
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
		ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("es='El módulo general ""%1"" no es encontrado.';ru='Общий модуль ""%1"" не найден.';en=""The general module '%1' was not found."""), Имя);
	КонецЕсли;
#Иначе
	Модуль = Вычислить(Имя);
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
...
Процедура ПроверитьВерсиюИРежимСовместимостиПлатформы()
	...
	Если Не (Лев(Информация.ВерсияПриложения, 3) = "8.3"
		...
		ВызватьИсключение НСтр("es='El tratamiento es destinado al lanzamiento en la versión de la plataforma "
"1С:Предприятие 8.3 con el régimen desconectado de la compatibilidad o es más alto';ru='Обработка предназначена для запуска на версии платформы"
"1С:Предприятие 8.3 с отключенным режимом совместимости или выше';en='Processing designed to run on platform version"
"1C: Enterprise 8.3 with disabled compatibility mode or higher'");
