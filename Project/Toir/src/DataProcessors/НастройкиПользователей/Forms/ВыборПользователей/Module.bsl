...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОчисткаНастроек Тогда
		Заголовок =
			НСтр("es='La elección de los usuarios para la limpieza de los ajustes';ru='Выбор пользователей для очистки настроек';en='Select users to clear settings'");
		Элементы.Надпись.Заголовок =
			НСтр("es='Escojan a los usuarios, a que tienen que limpiar los ajustes';ru='Выберите пользователей, которым необходимо очистить настройки';en='Select the users who need to clear the settings.'");
	КонецЕсли;
...
Процедура Выбрать(Команда)
	...
	Если ПользователиПриемник.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("es='Es necesario notar unos o plusieurs usuarios.';ru='Необходимо отметить одного или несколько пользователей.';en='Required to mark one or more users.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьЗаголовокГруппы(Форма, ГруппаПользователей, СтрокаСпискаПользователей, ЗначениеПометки)
	...
	Если ПомеченныйПользователь <> Неопределено И ЗначениеПометки <> СтрокаСпискаПользователей.Пометка Тогда
		...
		ГруппаПользователей.НаименованиеГруппыИПомеченоПользователей = 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 (%2)';ru='%1 (%2)';en='%1(%2)'"), Строка(ГруппаПользователей.Группа), ГруппаПользователей.ПомеченоПользователей);
	КонецЕсли;
