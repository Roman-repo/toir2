...
Процедура ПереключательОчищаемыеНастройкиПриИзменении(Элемент)
	...
	Если ПереключательКомуОчищатьНастройки = "ВыбраннымПользователям"
		...
		ПоказатьПредупреждение(,НСтр("es='La limpieza de los ajustes separados es accesible solamente en la elección de un usuario.';ru='Очистка отдельных настроек доступна только при выборе одного пользователя.';en='Clearing individual settings is available only when a single user is selected.'"));
...
Функция ОчисткаНастроек()
	...
	Если ПереключательКомуОчищатьНастройки = "ВыбраннымПользователям"
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='Escojan al usuario o los usuarios, "
"a que tienen que limpiar los ajustes.';ru='Выберите пользователя или пользователей,"
"которым необходимо очистить настройки.';en='Select a user or users,"
"who need to clear the settings.'"), , "Источник");
		Возврат Ложь;
	КонецЕсли;
	...
	Если ПереключательКомуОчищатьНастройки = "ВыбраннымПользователям" Тогда
		...
		Если КоличествоПользователей = 1 Тогда
			ПояснениеУКогоОчищеныНастройки = НСтр("es='Del usuario ""%1""';ru='пользователя ""%1""';en=""user '%1'""");
		...
		Иначе
			ПояснениеУКогоОчищеныНастройки = НСтр("es='A %1 usuarios';ru='%1 пользователям';en='% 1 users'");
	...
	Иначе
		ПояснениеУКогоОчищеныНастройки = НСтр("es='A todos los usuarios';ru='всем пользователям';en='to all users'");
	КонецЕсли;
	
	Если ПереключательОчищаемыеНастройки = "ОтдельныеНастройки"
		И КоличествоНастроек = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='Escojan los ajustes, que es necesario limpiar.';ru='Выберите настройки, которые необходимо очистить.';en='Select the settings you want to clear.'"), , "ПереключательОчищаемыеНастройки");
		Возврат Ложь;
	КонецЕсли;
	...
	Если ПереключательОчищаемыеНастройки = "ОтдельныеНастройки" Тогда
		...
		Если КоличествоНастроек = 1 Тогда
			...
			ТекстПояснения = НСтр("es='""%1"" es limpiada a %2';ru='""%1"" очищена у %2';en='% 1 cleared at %2'");
		...
		Иначе
			...
			ТекстПояснения = НСтр("es='Es limpiado %1 a %2';ru='Очищено %1 у %2';en='Cleared %1 at %2'");
	...
	ИначеЕсли ПереключательОчищаемыеНастройки = "ОчиститьВсе" Тогда
		...
		ТекстПояснения = НСтр("es='Son limpiados todos los ajustes %1';ru='Очищены все настройки %1';en='All settings%1 cleared'");
