...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Пользователь <> Неопределено Тогда
		...
		Заголовок = НСтр("es='Los ajustes del usuario';ru='Настройки пользователя';en='User Settings'");
	Иначе
...
Процедура НастройкиПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса = НСтр("es='¿Limpiar los ajustes escogidos?';ru='Очистить выбранные настройки?';en='Clear selected settings?'");
...
Процедура ПользовательСсылкаПриИзменении(Элемент)
	...
	Если РезультатПолученияНастроек() = "НетПользователяИБ" Тогда
		...
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Al usuario escogido ""%1"" no es indicado el usuario "
"de la base informativa, ver sus ajustes es imposible. "
"Eliminar problema se puede en la tarjeta del usuario dado.';ru='У выбранного пользователя ""%1"" не указан"
"пользователь информационной базы, просмотреть его настройки невозможно."
"Устранить проблему можно в карточке данного пользователя.';en='The selected user ""%1"" is not specified"
"user of the infobase, it is impossible to view its settings."
"You can fix the problem on the user’s card.'"),
			ПользовательСсылка));
		Возврат;
	КонецЕсли;
...
Процедура СкопироватьВсеНастройки(Команда)
	...
	КопируемыеНастройки.Добавить("НастройкиОтчетов",      НСтр("es='Los ajustes de los informes';ru='Настройки отчетов';en='Report Settings'"));
	КопируемыеНастройки.Добавить("НастройкиВнешнегоВида", НСтр("es='Los ajustes del aspecto exterior';ru='Настройки внешнего вида';en='Appearance Settings'"));
	КопируемыеНастройки.Добавить("ДанныеФорм",            НСтр("es='Los ajustes de los informes';ru='Настройки отчетов';en='Report Settings'"));
	КопируемыеНастройки.Добавить("ПерсональныеНастройки", НСтр("es='Los ajustes del aspecto exterior';ru='Настройки внешнего вида';en='Appearance Settings'"));
	КопируемыеНастройки.Добавить("Избранное",             НСтр("es='Los datos de las formas';ru='Данные форм';en='Form Data'"));
	КопируемыеНастройки.Добавить("НастройкиПечати",       НСтр("es='Los ajustes personales';ru='Персональные настройки';en='Personal settings'"));
	КопируемыеНастройки.Добавить("ПрочиеПользовательскиеНастройки",
		НСтр("es='Los ajustes de los informes adicionales y los tratamientos';ru='Настройки дополнительных отчетов и обработок';en='Additional reporting and processing settings'"));
...
Процедура Очистить(Команда)
	...
	Если ДеревоНастроек.ВыделенныеСтроки.Количество() = 0 Тогда
		...
		ПоказатьПредупреждение(,НСтр("es='Es necesario escoger los ajustes, que es necesario quitar.';ru='Необходимо выбрать настройки, которые требуется удалить.';en='Required to select the settings that you want to delete.'"));
		Возврат;
		
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("ОчиститьЗавершение", ЭтотОбъект, ДеревоНастроек);
	ТекстВопроса = НСтр("es='¿Limpiar los ajustes distinguidos?';ru='Очистить выделенные настройки?';en='Clear selected settings?'");
...
Процедура ОчиститьНастройкиВыбраннымПользователям(Команда)
	...
	ТекстВопроса =
		НСтр("es='¿Limpiar los ajustes distinguidos? "
"Se abrirá la ventana de la elección de los usuarios, a que tiene que limpiar los ajustes.';ru='Очистить выделенные настройки?"
"Откроется окно выбора пользователей, которым необходимо очистить настройки.';en='Clear selected settings?"
"A tab will open for selecting users who need to clear the settings. '");
...
Процедура ОчиститьВсеНастройки(Команда)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='¿Limpiar todos los ajustes al usuario ""%1""?';ru='Очистить все настройки у пользователя ""%1""?';en='Clear all settings for user ""%1""?'"), Строка(ПользовательСсылка));
	
	КнопкиВопроса = Новый СписокЗначений;
	КнопкиВопроса.Добавить("Очистить", НСтр("es='Limpiar';ru='Очистить';en='Clear'"));
	КнопкиВопроса.Добавить("Отмена",   НСтр("en='cancel';es='La anulación';ru='Отмена'"));
...
Процедура ОчиститьНастройкиОтчетовИВнешнегоВида(Команда)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='¿Limpiar todos los ajustes de los informes y el aspecto exterior al usuario ""%1""?';ru='Очистить все настройки отчетов и внешнего вида у пользователя ""%1""?';en='Clear all report and appearance settings for user ""%1""?'"),
...
Процедура ОчиститьНастройкиВсехПользователей(Команда)
	...
	ТекстВопроса =
		НСтр("es='Serán limpiados ahora todos los ajustes de todos los usuarios. "
"¿Continuar?';ru='Сейчас будут очищены все настройки всех пользователей."
"Продолжить?';en='Now all settings of all users will be cleared."
"Continue?'");
	
	КнопкиВопроса = Новый СписокЗначений;
	КнопкиВопроса.Добавить("ОчиститьВсе", НСтр("es='Limpiar todo';ru='Очистить все';en='Clear all'"));
	КнопкиВопроса.Добавить("Отмена",      НСтр("en='cancel';es='La anulación';ru='Отмена'"));
...
Функция ОбновлениеСпискаНастроек()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='La renovación de los ajustes de los usuarios';ru='Обновление настроек пользователей';en='Updating user settings'");
...
Процедура РассчитатьКоличествоНастроек()
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.НастройкиОтчетовСтраница.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Los ajustes de los informes (%1)';ru='Настройки отчетов (%1)';en='Report Settings (%1)'"), КоличествоНастроек);
	Иначе
		Элементы.НастройкиОтчетовСтраница.Заголовок = НСтр("es='Los ajustes de los informes';ru='Настройки отчетов';en='Report Settings'");
	КонецЕсли;
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.ВнешнийВидСтраница.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El aspecto exterior (%1)';ru='Внешний вид (%1)';en='Appearance (% 1)'"), КоличествоНастроек);
	Иначе
		Элементы.ВнешнийВидСтраница.Заголовок = НСтр("es='El aspecto exterior';ru='Внешний вид';en='Appearance'");
	КонецЕсли;
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.ПрочиеНастройкиСтраница.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Otros ajustes (%1)';ru='Прочие настройки (%1)';en='Other settings (%1)'"), КоличествоНастроек);
	Иначе
		Элементы.ПрочиеНастройкиСтраница.Заголовок = НСтр("es='Otros ajustes';ru='Прочие настройки';en='Other settings'");
	КонецЕсли;
...
Процедура Подключаемый_ВыполнитьОбработкуОповещения()
	...
	Если Параметр.КопироватьВсе Тогда
		...
		Иначе
			ПояснениеОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='%1 son copiados %2';ru='%1 скопированы %2';en='% 1 copied% 2'"), ИменаНастроек, ПояснениеКомуСкопированыНастройки);
		КонецЕсли;
...
Процедура ОчиститьНастройкиВсехПользователейЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("es='La limpieza de los ajustes';ru='Очистка настроек';en='Clearing settings'"), ,
		НСтр("es='Son limpiados todos los ajustes de todos el usuario';ru='Очищены все настройки всех пользователя';en='All settings of all user are cleared'"), БиблиотекаКартинок.Информация32);
...
Процедура ОчиститьВсеНастройкиЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ТекстПояснения = НСтр("es='Son limpiados todos los ajustes del usuario ""%1""';ru='Очищены все настройки пользователя ""%1""';en='All settings for user ""%1"" are cleared'");
	ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстПояснения, ПользовательСсылка);
	ПоказатьОповещениеПользователя(НСтр("es='La limpieza de los ajustes';ru='Очистка настроек';en='Clearing settings'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
...
Процедура ОчиститьНастройкиОтчетовИВнешнегоВидаЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ТекстПояснения = НСтр("es='Son limpiados todos los ajustes de los informes y el aspecto exterior al usuario ""%1""';ru='Очищены все настройки отчетов и внешнего вида у пользователя ""%1""';en='Cleared all report and appearance settings for user ""%1""'");
...
Процедура ОповеститьОбУдалении(КоличествоНастроек, НазваниеНастройки = Неопределено, КоличествоПользователей = Неопределено)
	...
	Если КоличествоНастроек = 1
	   И КоличествоПользователей = Неопределено Тогда
		...
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='""%1"" es limpiada al usuario ""%2""';ru='""%1"" очищена пользователю ""%2""';en='% 1 cleared by user ""% 2""'"), НазваниеНастройки, Строка(ПользовательСсылка));
		
	ИначеЕсли КоличествоПользователей = Неопределено Тогда
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='""%1"" es limpiada al usuario ""%2""';ru='""%1"" очищена пользователю ""%2""';en='%1 cleared by user ""%2""'"), ПрописьПредмета, Строка(ПользовательСсылка));
	КонецЕсли;
	...
	Если КоличествоПользователей <> Неопределено Тогда
		...
		Если КоличествоНастроек = 1 Тогда
			ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='""%1"" es limpiada %2';ru='""%1"" очищена %2';en='%1 cleared %2'"), НазваниеНастройки, ПояснениеКомуОчищатьНастройки);
		Иначе
			ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es limpiado %1 %2';ru='Очищено %1 %2';en='Cleared %1 %2'"), ПрописьПредмета, ПояснениеКомуОчищатьНастройки);
		КонецЕсли;
...
Процедура СкопироватьНастройки()
	...
	Если ДеревоНастроек.ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(,
			НСтр("es='Es necesario escoger los ajustes, que es necesario copiar.';ru='Необходимо выбрать настройки, которые требуется скопировать.';en='Required to select the settings that you want to copy.'"));
		Возврат;
	ИначеЕсли ДеревоНастроек.ВыделенныеСтроки.Количество() = 1 Тогда
		
		Если ДеревоНастроек.ТекущиеДанные.Тип = "ВариантЛичный" Тогда
			ПоказатьПредупреждение(,
				НСтр("es='Es imposible copiar la variante personal del informe. "
"Para que la variante personal del informe se haga accesible a otros usuarios, lo es necesario "
"пересохранить con la marca quitada ""Solamente para el autor"".';ru='Невозможно скопировать личный вариант отчета."
"Для того чтобы личный вариант отчета стал доступен другим пользователям, необходимо"
"его пересохранить со снятой пометкой ""Только для автора"".';en='Unable to copy personal version of report."
"In order for the personal version of the report to be available to other users, it is necessary"
"re-save it with unchecked ""Only for the author"".'"));
			Возврат;
		ИначеЕсли ДеревоНастроек.ТекущиеДанные.Тип = "НастройкаЛичная" Тогда
			ПоказатьПредупреждение(,
				НСтр("es='Es imposible copiar el ajuste de la variante personal del informe. "
"La copia de los ajustes de las variantes personales de los informes no es prevista.';ru='Невозможно скопировать настройку личного варианта отчета."
"Копирование настроек личных вариантов отчетов не предусмотрено.';en='Unable to copy custom report option setting."
"Copying the settings for personal reporting options is not provided. '"));
			Возврат;
		КонецЕсли;
