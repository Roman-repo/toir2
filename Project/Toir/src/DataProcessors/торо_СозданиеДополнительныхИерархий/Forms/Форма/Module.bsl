...
Процедура СоздатьГруппу(Команда)
	...
	Если НЕ ЗначениеЗаполнено(НоваяИерархия) Тогда
		ТекстСообщения = НСтр("es='¡Para la ejecución de la acción es necesario escoger una nueva jerarquía!';ru='Для выполнения действия необходимо выбрать новую иерархию!';en='To perform an action, you must select a new hierarchy!'");
...
Процедура ПеренестиОР(Команда)
	...
	Если НЕ ЗначениеЗаполнено(НоваяИерархия) Тогда
		ТекстСообщения = НСтр("es='Es necesario escoger una nueva jerarquía.';ru='Необходимо выбрать новую иерархию.';en='Required to select a new hierarchy.'");
	...
	Если ТекДанные = Неопределено Тогда
		ТекстСообщения = НСтр("es='jerarquía! Activen la línea necesaria.';ru='Не выбрано место переноса в новой иерархии! Активизируйте нужную строку.';en='No transfer location in the new hierarchy! Activate the desired line.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
