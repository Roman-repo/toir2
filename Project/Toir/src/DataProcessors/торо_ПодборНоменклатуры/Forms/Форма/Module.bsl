...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ВопросПриЗакрытии Тогда
		...
		Если Не ЗавершениеРаботы Тогда
			...
			ПоказатьВопрос(ОписаниеОповещения, НСтр("es='La nomenclatura recogida no es llevada. ¿Llevar?';ru='Подобранная номенклатура не перенесена. Перенести?';en='The selected item is not transferred. Move?'"), РежимДиалогаВопрос.ДаНетОтмена);
		КонецЕсли;
	КонецЕсли;
...
Процедура СписокНоменклатураПриАктивизацииСтроки(Элемент)
	...
	Если текДанные <> Неопределено Тогда
		ЗаполнитьОстаткиТоваров(Объект.Склад, текДанные.Номенклатура, текДанные.Характеристика);
	Иначе
		ЗаполнитьОстаткиТоваров(Объект.Склад, Неопределено, Неопределено);
	КонецЕсли;
...
Процедура СписокНоменклатураВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТекДанные <> Неопределено Тогда
		...
		Иначе
			ВыбЗнач = 0;
			ПоказатьВводЗначения(Новый ОписаниеОповещения("СписокНоменклатураВыборЗавершение", ЭтотОбъект, Новый Структура("ТекДанные", ТекДанные)), ВыбЗнач, НСтр("es='Introduzcan la cantidad';ru='Введите количество';en='Enter quantity'"), Новый ОписаниеТипов("Число",,,новый КвалификаторыЧисла(12,3)));
		КонецЕсли;		
	КонецЕсли;
...
Процедура ЗаполнитьОстаткиТоваров(Склад, Номенклатура, Характеристика = Неопределено)
	...
	МассивОстатков = ПолучитьОстаткиТоваров(Склад, Номенклатура, Характеристика);
...
&НаСервереБезКонтекста
Функция ПолучитьОстаткиТоваров(Склад, Номенклатура, Характеристика)
...
Процедура дс_УстановитьЗаголовкиЭлементов()
	...
	Заголовки.Вставить("СписокНоменклатураВНаличии", НСтр("es='En presencia de';ru='В наличии';en='In stock'"));
