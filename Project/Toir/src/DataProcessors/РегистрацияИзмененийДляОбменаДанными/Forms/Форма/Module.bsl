...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Иначе
			ВызватьИсключение СтрЗаменить(
				НСтр("es='Los parámetros incorrectos de los objetos del destino de la apertura de la orden ""%1""';ru='Некорректные параметры объектов назначения открытия команды ""%1""';en='Invalid parameters of destination objects for opening command ""%1""'"),
	...
	Если Не ПроконтролироватьНастройки() И ОткрытиеСУзломПараметром Тогда
		...
		ТекстСообщения = СтрЗаменить(
			НСтр("es='Para ""%1"" redacción del registro de los objetos es inaccesible.';ru='Для ""%1"" редактирование регистрации объектов недоступно.';en='For ""%1"" editing registration of objects is not available.'"),
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ДлительнаяОперацияЗапущена Тогда
		...
		ПоказатьВопрос(Оповещение, НСтр("es='¿Interrumpir la ejecución del registro de los datos?';ru='Прервать выполнение регистрации данных?';en='Abort data logging?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) <> Тип("Структура") 
		...
		Ошибка = НСтр("es='El resultado inesperado de la elección de la consola de las interpelaciones';ru='Неожиданный результат выбора из консоли запросов';en='Unexpected result from query console selection'");
	Иначе
	...
	Если ВыбранноеЗначение.ДействиеВыбора Тогда
		Текст = НСтр("es='¿Registrar el resultado de la interpelación "
"sobre el nudo ""%1""?';ru='Зарегистрировать результат запроса"
"на узле ""%1""?';en='Register query result"
"on node ""%1""? '"); 
	Иначе
		Текст = НСтр("es='¿Anular el registro del resultado de la interpelación "
"sobre el nudo ""%1""?';ru='Отменить регистрацию результата запроса"
"на узле ""%1""?';en='Cancel request result register"
"on node ""%1""? '");
	КонецЕсли;
	Текст = СтрЗаменить(Текст, "%1", Строка(УзелОбменаСсылка));
					 
	ЗаголовокВопроса = НСтр("es='La confirmación';ru='Подтверждение';en='Confirmation'");
...
Процедура ПоказатьРезультатВыгрузки(Команда)
	...
	Если Сериализация.Количество() > 0 Тогда
		...
		ЗаголовокТекста = НСтр("es='El resultado de la descarga estandartizada (РИБ)';ru='Результат стандартной выгрузки (РИБ)';en='Result of standard discharge (RIB)'");
...
Процедура ЗарегистрироватьИОМИПредопределенные(Команда)
	...
	ТекстВопроса     = СтрЗаменить( 
		НСтр("es='¿Registrar los datos para la reconstitución del nudo РИБ subordinado "
"sobre el nudo ""%1""?';ru='Зарегистрировать данные для восстановления подчиненного узла РИБ"
"на узле ""%1""?';en='Register data for recovery of the slave RIB node"
"on node ""%1""? '"),
		"%1", УзелОбменаСсылка);
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='No descargaba';ru='Не выгружалось';en='Not unloaded'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='No hay cambios';ru='Нет изменений';en='No changes'"));
...
Процедура РедактироватьНомерСообщенияКонстанты()
	...
	Подсказка = НСтр("es='El número enviado';ru='Номер отправленного';en='Sent number'"); 
...
Процедура УстановитьЗаголовокНомеровСообщений()
	...
	Текст = НСтр("es='№ enviado %1, № aceptado %2';ru='№ отправленного %1, № принятого %2';en='№. Sent %1, № received% 2'");
...
Процедура СообщитьОРезультатахРегистрации(Результаты)
	...
	Если ТипЗнч(Команда) = Тип("Булево") Тогда
		Если Команда Тогда
			ЗаголовокПредупреждения = НСтр("es='El registro de los cambios:';ru='Регистрация изменений:';en='Registration of changes:'");
			ТекстПредупреждения = НСтр("es='Es registrado %1 cambios de %2 "
"sobre el nudo ""%0""';ru='Зарегистрировано %1 изменений из %2"
"на узле ""%0""';en='%1 changes registered from %2"
"on node ""%0"" '");
		Иначе
			ЗаголовокПредупреждения = НСтр("es='La anulación del registro:';ru='Отмена регистрации:';en='Unregistration:'");
			ТекстПредупреждения = НСтр("es='Es anulado el registro de %1 cambios "
"sobre el nudo ""%0"".';ru='Отменена регистрация %1 изменений "
"на узле ""%0"".';en='Canceled %1 changes"
"on node ""%0"". '");
		КонецЕсли;
	Иначе
		ЗаголовокПредупреждения = НСтр("es='El cambio del número del mensaje:';ru='Изменение номера сообщения:';en='Change message number:'");
		ТекстПредупреждения = НСтр("es='El número del mensaje es cambiado en %3 "
"cerca de %1 objetos (ов)';ru='Номер сообщения изменен на %3"
"у %1 объекта(ов)';en='Message number changed to%3"
"%1 item (s) '");
	КонецЕсли;
...
Процедура УдалитьРегистрациюКонстантыВСписке()
	...
	ИначеЕсли Количество = 1 Тогда
		Текст = НСтр("es='¿Anular el registro ""%2"" "
"sobre el nudo ""%1""?';ru='Отменить регистрацию ""%2"""
"на узле ""%1""?';en='Cancel registration ""%2"""
"on node ""%1?'"); 
	Иначе
		Текст = НСтр("es='¿Anular el registro de las constantes escogidas "
"sobre el nudo ""%1""?';ru='Отменить регистрацию выбранных констант"
"на узле ""%1""?';en='Cancel registration of selected constants"
"on node ""%1""? '"); 
	КонецЕсли;
...
"on node ""%1?'"); 
	Иначе
		Текст = НСтр("es='¿Anular el registro de los objetos escogidos "
"sobre el nudo ""%1""?';ru='Отменить регистрацию выбранных объектов"
"на узле ""%1""?';en='Cancel registration of selected objects"
"on node ""%1""? '"); 
	КонецЕсли;
	Текст = СтрЗаменить(Текст, "%1", УзелОбменаСсылка);
...
Процедура ДобавитьРегистрациюВыделенныхОбъектов(БезУчетаАвторегистрации = Истина)
	...
	Текст = НСтр("es='¿Registrar %1 para la descarga sobre el nudo ""%2""? "
""
"El cambio del registro de la gran cantidad de los objetos puede ocupar el largo tiempo.';ru='Зарегистрировать %1 для выгрузки на узле ""%2""?"
""
"Изменение регистрации большого количества объектов может занять продолжительное время.';en='Register%1 for upload to host ""%2""?"
""
...
Процедура УдалитьРегистрациюВыделенныхОбъектов(БезУчетаАвторегистрации = Истина)
	...
	Текст = НСтр("es='¿Anular el registro %1 para la descarga sobre el nudo ""%2""? "
""
"El cambio del registro de la gran cantidad de los objetos puede ocupar el largo tiempo.';ru='Отменить регистрацию %1 для выгрузки на узле ""%2""?"
""
"Изменение регистрации большого количества объектов может занять продолжительное время.';en='Cancel registration %1"
"for unloading on the node ""%2""? Changing the registration of a large number of objects can take a long time.'");
...
Процедура ФоновоеЗаданиеЗапуститьНаКлиенте(ПараметрыФоновогоЗадания)
	...
	Если Результат = Неопределено Тогда
		...
		ТекстПредупреждения = НСтр("es='Durante el lanzamiento de la tarea de fondo con el fin del cambio del registro había una falta.';ru='При запуске фонового задания с целью изменения регистрации произошла ошибка.';en='An error occurred while starting the background job to change the registration.'");
...
Процедура СтраницаДлительнойОперации()
	...
	Если ВидДлительнойОперации Тогда
		СтатусОперации = НСтр("es='Se cumple el registro de los cambios. Por favor, esperen.';ru='Выполняется регистрация изменений. Пожалуйста, подождите.';en='Registration of changes is in progress. Please, wait.'");
	Иначе
		СтатусОперации = НСтр("es='Se cumple la anulación del registro de los cambios. Por favor, esperen.';ru='Выполняется отмена регистрации изменений. Пожалуйста, подождите.';en='Unregister changes. Wait, please.'");
	КонецЕсли;
...
Процедура ФоновоеЗаданиеПолучитьРезультатНаСервере()
	...
	Если ФоновоеЗаданиеРезультатВыполнения <> Неопределено Тогда
		...
		СообщениеОбОшибке = "";
		ТиповоеПредставлениеОшибки = НСтр("es='Con el cambio del registro había una falta. Los detalles de cm. En la revista del registro';ru='При изменении регистрации произошла ошибка. Подробности см. в журнале регистрации';en='An error occurred while changing the registration. See the log for details.'");
...
Функция ОбщийМодульДлительныеОперацииКлиент()
	...
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
		ВызватьИсключение НСтр("es='El módulo ""ДлительныеОперацииКлиент"" general no es encontrado.';ru='Общий модуль ""ДлительныеОперацииКлиент"" не найден.';en='The general module ""ДлительныеОперацииКлиент"" was not found.'");
	КонецЕсли;
...
Функция ОбщийМодульДлительныеОперации()
	...
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
		ВызватьИсключение НСтр("es='El módulo ""ДлительныеОперации"" general no es encontrado.';ru='Общий модуль ""ДлительныеОперации"" не найден.';en='The general module ""ДлительныеОперации"" was not found.'");
	КонецЕсли;
...
Процедура ОперацияСРезультатамиЗапроса(КомандаОперации)
	...
	Если ТекИмяФормы <> Неопределено Тогда
		...
		Если КомандаОперации Тогда
			Текст = НСтр("es='El registro de los cambios del resultado de la interpelación';ru='Регистрация изменений результата запроса';en='Registration of changes in query result'");
		Иначе
			Текст = НСтр("es='La anulación del registro de los cambios del resultado de la interpelación';ru='Отмена регистрации изменений результата запроса';en='Unregister request result changes'");
		КонецЕсли;
	...
	Текст = НСтр("es='En los ajustes no es indicado el tratamiento para la ejecución de las interpelaciones. "
"¿Ajustar es ahora?';ru='В настройках не указана обработка для выполнения запросов."
"Настроить сейчас?';en='The settings do not specify the processing for query execution."
"Customize now?'");
	
	ЗаголовокВопроса = НСтр("es='Los ajustes';ru='Настройки';en='Settings'");
...
Функция КонтрольСсылокДляВыбораЗапросом(Адрес)
	...
	Если ТипЗнч(Результат) = Тип("Массив") Тогда 
		...
		Если Результат.Колонки.Найти("Ссылка") = Неопределено Тогда
			Возврат НСтр("es='En el último resultado de la interpelación falta la columna ""la Referencia""';ru='В последнем результате запроса отсутствует колонка ""Ссылка""';en='The last result of the request is missing the ""Link"" column'");
		КонецЕсли;
	Иначе		
		Возврат НСтр("es='La falta de la recepción de los datos del resultado de la interpelación';ru='Ошибка получения данных результата запроса';en='Error retrieving query result data'");
	КонецЕсли;
...
Процедура НастроитьПустуюСтраницу(Наименование, ИмяТаблицы = Неопределено)
	...
	Иначе
		...
		Если Строка <> Неопределено Тогда
			ТекстКоличеств = НСтр("es='Es registrado los objetos: %1 "
"Es descargado los objetos: %2 no es descargado "
"los objetos: %3';ru='Зарегистрировано объектов: %1"
"Выгружено объектов: %2"
"Не выгружено объектов: %3';en='%1 registered objects"
"Unloaded objects:%2"
"Unloaded objects:%3 '");
...
""
"%2 "
"Para el registro o la anulación del registro del cambio para los datos sobre el nudo "
"""%3"" "
"escojan el tipo del objeto a la izquierda en el árbol de los metadatos y se aprovechen "
"de las órdenes ""Registrar"" o ""Anular el registro""';ru='%1."
""
"%2"
...
""
"% 2"
"To register or unregister data exchange on a node"
"""% 3 """
"select the type of object on the left in the metadata tree and use"
...
Функция ПолучитьОписаниеВыбранныхМетаданных(БезУчетаАвторегистрации, МетаИмяГруппа = Неопределено, МетаИмяУзел = Неопределено)
	...
	Если МетаИмяГруппа = Неопределено И МетаИмяУзел = Неопределено Тогда
		// Не указано ничего
		Текст = НСтр("es='Todos los objetos %1 por la jerarquía escogida del tipo';ru='все объекты %1 по выбранной иерархии вида';en='all objects% 1 in the selected view hierarchy'");
	...
	Иначе
		...
		Текст = НСтр("es='Todos los objetos como ""%3"" %1';ru='все объекты типа ""%3"" %1';en=""all objects of type '% 3'% 1""");
	...
	Если БезУчетаАвторегистрации Тогда
		ТекстФлага = "";
	Иначе
		ТекстФлага = НСтр("es='Con el indicio del autoregistro';ru='с признаком авторегистрации';en='with a sign of auto registration'");
	КонецЕсли;
...
Процедура ОбработатьЗапретИзмененияУзла()
	...
	Если ОперацииРазрешены Тогда
		...
		Заголовок = НСтр("es='El registro de los cambios para el cambio para los datos';ru='Регистрация изменений для обмена данными';en='Registration of changes for data exchange'");
	Иначе
		Элементы.УзелОбменаСсылка.Видимость = Ложь;
		Заголовок = СтрЗаменить(НСтр("es='El registro de los cambios para el cambio con ""%1""';ru='Регистрация изменений для обмена с  ""%1""';en='Registration of changes for exchange with ""%1""'"), "%1", Строка(УзелОбменаСсылка));
	КонецЕсли;
...
"1С:Предприятие 8.3.5 con el régimen desconectado de la compatibilidad o es más alto';ru='Обработка предназначена для запуска на версии платформы"
"1С:Предприятие 8.3.5 с отключенным режимом совместимости или выше';en='Processing designed to run on platform version"
"1C: Enterprise 8.3.5 with disabled compatibility mode or higher'");
