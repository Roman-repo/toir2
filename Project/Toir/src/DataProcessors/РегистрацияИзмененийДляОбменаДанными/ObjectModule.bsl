...
// Параметры:
	...
	ТекНомерСтроки = 1;
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 1,  2,  Ложь,   "Константы",               НСтр("es='Las constantes';ru='Константы';en='Constants'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 3,  4,  Истина, "Справочники",             НСтр("es='Las guías';ru='Справочники';en='Directories'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 5,  6,  Истина, "Последовательности",      НСтр("es='Las consecuencias';ru='Последовательности';en='Sequences'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 7,  8,  Истина, "Документы",               НСтр("es='Los documentos';ru='Документы';en='Documents'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 9,  10, Истина, "ПланыВидовХарактеристик", НСтр("es='Los planes de los tipos de las características';ru='Планы видов характеристик';en='Features types plans'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 11, 12, Истина, "ПланыСчетов",             НСтр("es='Los planes de las cuentas';ru='Планы счетов';en='Chart of accounts'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 13, 14, Истина, "ПланыВидовРасчета",       НСтр("es='Los planes de los tipos del cálculo';ru='Планы видов расчета';en='Calculation types plans'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 15, 16, Истина, "РегистрыСведений",        НСтр("es='Los registros de las noticias';ru='Регистры сведений';en='Information registers'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 17, 18, Истина, "РегистрыНакопления",      НСтр("es='Los registros de la acumulación';ru='Регистры накопления';en='Accumulation registers'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 19, 20, Истина, "РегистрыБухгалтерии",     НСтр("es='Los registros de la contabilidad';ru='Регистры бухгалтерии';en='Accounting registers'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 21, 22, Истина, "РегистрыРасчета",         НСтр("es='Los registros del cálculo';ru='Регистры расчета';en='Billing registers'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 23, 24, Истина, "БизнесПроцессы",          НСтр("es='Los procesos de negocios';ru='Бизнес-процессы';en='Business processes'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 25, 26, Истина, "Задачи",                  НСтр("es='Las tareas';ru='Задачи';en='Tasks'"));
...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	Инфо.Вставить("Наименование", НСтр("es='El registro de los cambios para el cambio para los datos';ru='Регистрация изменений для обмена данными';en='Registration of changes for data exchange'"));
	Инфо.Вставить("Версия",       "1.0");
	Инфо.Вставить("ВерсияБСП",    "1.2.1.4");
	Инфо.Вставить("Информация",    НСтр("es='El tratamiento para la dirección del registro de los objetos sobre los nudos del cambio antes de la formación de la descarga. En el trabajo como parte de la configuración con БСП las versiones 2.1.2.0 y hace mayor el control de las restricciones de la migración de los datos para los nudos del cambio.';ru='Обработка для управления регистрацией объектов на узлах обмена до формирования выгрузки. При работе в составе конфигурации с БСП версии 2.1.2.0 и старше производит контроль ограничений миграции данных для узлов обмена.';en='Processing for managing the registration of objects at exchange nodes before forming the upload. When working as part of a configuration with BSP version 2.1.2.0 and later, it controls data migration restrictions for exchange nodes.'"));
	...
	Команда.Представление = НСтр("es='La redacción del registro de los cambios del objeto';ru='Редактирование регистрации изменений объекта';en='Editing object changes registration'");
...
Функция ИзмеренияНабораЗаписей(ИмяТаблицы, ВсеИзмерения = Ложь) Экспорт
	...
	Если Метаданные.РегистрыНакопления.Содержит(Мета)
	 ...
	 Или Метаданные.Последовательности.Содержит(Мета) Тогда
		...
		Строка.Заголовок   = НСтр("es='El registrador';ru='Регистратор';en='Registrar'");
	КонецЕсли;
	...
	Если ЭтоРегистрСведений И Мета.ОсновнойОтборПоПериоду Тогда
		...
		Строка.Заголовок   = НСтр("es='El período';ru='Период';en='Period'");
	КонецЕсли;
	...
	Если Метаданные.РегистрыРасчета.Содержит(Мета.Родитель()) Тогда
		...
		Строка.ТипЗначения = Документы.ТипВсеСсылки();
		Строка.Заголовок   = НСтр("es='El objeto del recuento';ru='Объект перерасчета';en='Allocation object'");
	КонецЕсли;
...
Функция ПредставлениеСсылки(ОбъектПредставления) Экспорт
	...
	Если ПустаяСтрока(Результат) Тогда
		Результат = НСтр("es='No es dado';ru='не задано';en='not set'");
	КонецЕсли;
...
Функция ПроверитьКорректностьНастроек(КлючНастройки = "") Экспорт
	...
	Иначе
		...
		Если Метаданные.Обработки.Найти(НастройкаАдресВнешнейОбработкиЗапросов) = Неопределено Тогда
			Текст = НСтр("es='El tratamiento ""%1"" no es encontrado como parte de la configuración';ru='Обработка ""%1"" не найдена в составе конфигурации';en=""Processing '%1' not found in configuration""");
