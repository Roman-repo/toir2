...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ЭтаФорма.Параметры.Свойство("СсылкаНаАкт") И ЭтаФорма.Параметры.СсылкаНаАкт <> Документы.торо_АктОВыполненииЭтапаРабот.ПустаяСсылка() Тогда
		...
		ЭтаФорма.Заголовок = НСтр("es='La elección de la reparación del acta';ru='Выбор ремонта из акта';en='The choice of repair from the act'");
	Иначе	
...
Процедура РемонтыИЗППРВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если НайдСтроки.Количество() > 0 Тогда
		...
		ТекстСообщения = НСтр("es='El tipo dado de la reparación ya добален.';ru='Данный вид ремонта уже добален.';en='This type of repair is already completed.'");
