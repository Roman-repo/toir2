...
Процедура ВыполнитьЗагрузкуДанныхВИнформационнуюБазу(Отказ, ТаблицыДляЗагрузки) Экспорт
	...
	Если ОбработкаОбменаДанными.ФлагОшибки() Тогда
		НСтрока = НСтр("es='A la carga del mensaje del cambio han surgido las faltas: %1';ru='При загрузке сообщения обмена возникли ошибки: %1';en='Errors occurred while loading the exchange message:%1'");
	...
	Для каждого Элемент Из СписокПолейТаблицы Цикл
		...
		Если Реквизит = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Para el objeto de los metadatos ""%1"" no es determinado el requisito con el nombre ""%2""';ru='Для объекта метаданных ""%1"" не определен реквизит с именем ""%2""';en=""No metadata object '%1' defined with name '%2'"""),
