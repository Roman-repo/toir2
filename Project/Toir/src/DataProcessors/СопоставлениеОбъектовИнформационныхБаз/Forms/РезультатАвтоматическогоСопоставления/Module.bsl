...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("ИмяФайлаСообщенияОбмена") Тогда
		...
		НСтрока = НСтр("es='La forma no puede ser abierta es interactivo.';ru='Форма не может быть открыта интерактивно.';en='The form cannot be opened interactively.'");
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы Тогда
		Возврат;
	КонецЕсли;
	ПоказатьПредупреждение(, НСтр("es='La forma contiene los datos de la comparación automática. La acción es anulada.';ru='Форма содержит данные автоматического сопоставления. Действие отменено.';en='The form contains auto-matching data. The action is canceled.'"));
...
Функция ФоновоеЗаданиеЗапуститьНаСервере(Отказ)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='La comparación automática de los objetos';ru='Автоматическое сопоставление объектов';en='Auto match objects'");
...
Процедура ЗафиксироватьОшибку(ПодробноеПредставлениеОшибки)
	ЗаписьЖурналаРегистрации(
		НСтр("es='El ayudante de la comparación de los objetos. La comparación automática';ru='Помощник сопоставления объектов.Автоматическое сопоставление';en='Object mapping assistant. auto mapping'", ОбщегоНазначения.КодОсновногоЯзыка()),
