...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НовыйЭлементСписка = СписокВыбора.Добавить("ВсеОбъекты", НСтр("es='Todos los datos';ru='Все данные';en='All data'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, Новый ФиксированнаяСтруктура);
	
	НовыйЭлементСписка = СписокВыбора.Добавить("СопоставленныеОбъектыНеутвержденные", НСтр("es='Los cambios';ru='Изменения';en='Changes'"));
	...
	НовыйЭлементСписка = СписокВыбора.Добавить("СопоставленныеОбъекты", НСтр("es='Los datos confrontados';ru='Сопоставленные данные';en='Matching data'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
						Новый ФиксированнаяСтруктура("СтатусСопоставленияДополнительный", 0));
	
	НовыйЭлементСписка = СписокВыбора.Добавить("НесопоставленныеОбъекты", НСтр("es='Los datos no confrontados';ru='Несопоставленные данные';en='Unmatched data'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
						Новый ФиксированнаяСтруктура("СтатусСопоставленияДополнительный", 1));
	
	НовыйЭлементСписка = СписокВыбора.Добавить("НесопоставленныеОбъектыПриемника", НСтр("es='Los datos no confrontados de esta base';ru='Несопоставленные данные этой базы';en='Unmatched data for this database'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
						Новый ФиксированнаяСтруктура("СтатусСопоставления",  1));
	
	НовыйЭлементСписка = СписокВыбора.Добавить("НесопоставленныеОбъектыИсточника", НСтр("es='Los datos no confrontados de la segunda base';ru='Несопоставленные данные второй базы';en='Unmatched second base data'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
						Новый ФиксированнаяСтруктура("СтатусСопоставления", -1));
	...
	КонецЕсли;
	Заголовок = НСтр("es='La comparación de los datos ""[ПредставлениеДанных]""';ru='Сопоставление данных ""[ПредставлениеДанных]""';en='Data mapping ""[ПредставлениеДанных]""'");
	...
	НаименованиеЭтойПрограммы = ?(ПустаяСтрока(НаименованиеЭтойПрограммы), НСтр("es='Este programa';ru='Эта программа';en='This program'"), НаименованиеЭтойПрограммы);
	
	НаименованиеВторойПрограммы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.УзелИнформационнойБазы, "Наименование");
	НаименованиеВторойПрограммы = ?(ПустаяСтрока(НаименованиеВторойПрограммы), НСтр("es='El segundo programa';ru='Вторая программа';en='Second program'"), НаименованиеВторойПрограммы);
	...
	Элементы.Пояснение.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Para la comparación de los datos ""%1"" "
"con los datos ""%2"" se aprovechen de la orden ""Confrontar automáticamente..."". "
"Se puede vincular los datos que se han quedado no confrontados uno con otro a mano.';ru='Для сопоставления данных ""%1"""
"с данными ""%2"" воспользуйтесь командой ""Сопоставить автоматически...""."
"Оставшиеся несопоставленные данные можно связать друг с другом вручную.';en='To match data"" ""%1"" """
"with the data """"%2 """" use the command """" Match automatically ... """"."
"The remaining unmatched data can be linked to each other manually.'"),
...
Процедура ВыполнитьЗагрузкуДанных(Команда)
	НСтрока = НСтр("es='¿Recibir los datos en la base informativa?';ru='Получить данные в информационную базу?';en='Get data in the infobase?'");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='No hay conformidad, el objeto será copiado';ru='Нет соответствия, объект будет скопирован';en='No match, the object will be copied'"));
...
Процедура ВыполнитьЗагрузкуДанныхПослеВопросаОПодтвержденииПолученияДанных(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	...
	Если Объект.ДанныеУспешноЗагружены Тогда
		НСтрока = НСтр("es='Los datos eran recibidos ya. ¿Cumplir la recepción de los datos es repetido?';ru='Данные уже были получены. Выполнить получение данных повторно?';en='Data has already been received. Retrieve data again?'");
...
Процедура ВыполнитьЗагрузкуДанныхПослеПодтвержденияПолученияДанных()
	...
	Если Отказ Тогда
		НСтрока = НСтр("es='A la recepción de los datos han surgido las faltas. "
"¿Pasar en la revista del registro?';ru='При получении данных возникли ошибки."
"Перейти в журнал регистрации?';en='Errors occurred while receiving data."
"Go to the logbook? '");
...
Процедура ВыполнитьПроверкуЗаданияПользовательскихПолей(Отказ, ПользовательскиеПоля)
	...
	Если ПользовательскиеПоля.Количество() = 0 Тогда
		...
		НСтрока = НСтр("es='Debe indicar por lo menos un campo para la representación';ru='Следует указать хотя бы одно поле для отображения';en='At least one field for displaying should be specified'");
	...
	ИначеЕсли ПользовательскиеПоля.Количество() > МаксимальноеКоличествоПользовательскихПолей() Тогда
		...
		СтрокаСообщения = НСтр("es='Reduzcan la cantidad de los campos (se puede escoger no más %1 campos)';ru='Уменьшите количество полей (можно выбирать не более %1 полей)';en='Reduce the number of fields (you can select no more than%1 fields)'");
...
Процедура УстановитьСвязьИнтерактивно()
	...
	Если Не (ТекущиеДанные.СтатусСопоставления=-1 Или ТекущиеДанные.СтатусСопоставления=+1) Тогда
		...
		ПоказатьПредупреждение(, НСтр("es='Los objetos son confrontados ya';ru='Объекты уже сопоставлены';en='Objects are already mapped'"), 2);
	...
	Кнопки.Добавить(КодВозвратаДиалога.Да,     НСтр("es='Establecer';ru='Установить';en='Install'"));
	...
	ТекстВопроса = НСтр("es='¿Establecer la conformidad entre los objetos escogidos?';ru='Установить соответствие между выбранными объектами?';en='Match between selected objects?'");
...
Функция ПолучитьОбъектДляСопоставления(Данные)
	...
	Если Результат.Количество() = 0 Тогда
		...
		Результат.Добавить(НСтр("es='<no es dado>';ru='<не задан>';en='<не задан>'"));
...
Процедура ЗавершениеФоновогоЗадания(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Неопределено Тогда
		...
		ЗафиксироватьОшибку(НСтр("es='La tarea de fondo ha sida anulada mal.';ru='Фоновое задание аварийно отменилось.';en='The background task is abnormally canceled.'"));
		ПерейтиНазад();
...
Функция ФоновоеЗаданиеЗапуститьНаСервере(Отказ)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='La comparación de los objetos';ru='Сопоставление объектов';en='Object mapping'");
...
Процедура ЗафиксироватьОшибку(ПодробноеПредставлениеОшибки)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("es='El ayudante de la comparación de los objetos. El análisis de los datos';ru='Помощник сопоставления объектов.Анализ данных';en='Object mapping helper data analysis'", ОбщегоНазначения.КодОсновногоЯзыка()),
