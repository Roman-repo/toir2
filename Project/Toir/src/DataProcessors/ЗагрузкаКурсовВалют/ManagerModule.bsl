...
Процедура ЗагрузитьАктуальныйКурс(ПараметрыЗагрузки = Неопределено, АдресРезультата = Неопределено) Экспорт
	...
	ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Информация, , ,
		НСтр("es='Es comenzada la carga de reglamento de las cotizaciones';ru='Начата регламентная загрузка курсов валют';en='Started routine loading of exchange rates'"));
	...
	Если ПриЗагрузкеВозниклиОшибки Тогда
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка,,,
			НСтр("es='Durante la carga de reglamento de las cotizaciones han surgido las faltas';ru='Во время регламентной загрузки курсов валют возникли ошибки';en='Errors occurred during routine loading of exchange rates'"));
		Если ПараметрыЗагрузки = Неопределено Тогда
			ВызватьИсключение НСтр("es='La carga de los cursos no es cumplida.';ru='Загрузка курсов не выполнена.';en='Course upload failed.'");
		КонецЕсли;
	Иначе
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Информация,,,
			НСтр("es='Es acabada la carga de reglamento de las cotizaciones.';ru='Завершена регламентная загрузка курсов валют.';en='The routine loading of exchange rates has been completed.'"));
	КонецЕсли;
...
Процедура ДобавитьРазрешения(Разрешения) Экспорт
	...
	Если ИспользоватьАльтернативныйСервер Тогда
		...
		Порт = Неопределено;
		Описание = НСтр("es='La carga de las cotizaciones del sitio la RBC.';ru='Загрузка курсов валют с сайта РБК.';en='Download exchange rates from the RBC website.'");
		Разрешения.Добавить( 
	...
	Иначе
		...
		Порт = Неопределено;
		Описание = НСтр("es='La carga de las cotizaciones del sitio 1С.';ru='Загрузка курсов валют с сайта 1С.';en='Download currency rates from the site 1C.'");
		Разрешения.Добавить( 
			МодульРаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(Протокол, Адрес, Порт, Описание));
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("es='Desconecta la carga erróneamente incluida de las cotizaciones ""el rublo Ruso (643)"" del Internet.';ru='Отключает ошибочно включенную загрузку курсов валюты ""Российский рубль (643)"" из интернета.';en='Disables the erroneously included download of currency rates ""Russian Ruble (643)"" from the Internet.'");
	...
	Для Каждого Валюта Из Валюты Цикл
		Если ВалютыЗагружаемыеИзИнтернета.Найти(Валюта.Валюта) = Неопределено Тогда
			...
			ПоясняющееСообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es imposible recibir el fichero de los datos con las cotizaciones %2 (el código %1): "
"los Cursos de la divisa dada no son concedidos.';ru='Невозможно получить файл данных с курсами валюты %2 (код %1):"
"Курсы данной валюты не предоставляются.';en='Unable to get data file with exchange rates%2 (code%1):"
		...
		Иначе
			...
			Иначе
				ПоясняющееСообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Es imposible recibir el fichero de los datos con las cotizaciones %2 (el código %1): "
"%3 "
"es posible, no hay acceso a веб al sitio con las cotizaciones, o es indicada la divisa no existente.';ru='Невозможно получить файл данных с курсами валюты %2 (код %1):"
"%3"
"Возможно, нет доступа к веб сайту с курсами валют, либо указана несуществующая валюта.';en='Unable to get data file with exchange rates%2 (code%1):"
"%3"
"There may be no access to the website with exchange rates, or a non-existent currency is indicated. '"),
					Валюта.КодВалюты,
					Валюта.Валюта,
...
Функция ЗагрузитьКурсВалютыИзФайла(Знач Валюта, Знач ПутьКФайлу, Знач НачалоПериодаЗагрузки, Знач ОкончаниеПериодаЗагрузки)
	...
	Если ЧислоЗагружаемыхДнейВсего <> ЧислоЗагруженныхДней Тогда
		Если ЧислоЗагруженныхДней = 0 Тогда
			ПояснениеОЗагрузке = НСтр("es='Las cotizaciones %1 (%2) no son cargadas. "
"No existe la noticia sobre el curso por el período indicado.';ru='Курсы валюты %1 (%2) не загружены."
"Нет сведение о курсе за указанный период.';en='Currency rates%1 (%2) not loaded."
"There is no course information for the specified period.'");
		Иначе
			ПояснениеОЗагрузке = НСтр("es='Son cargados no todos los cursos por la divisa %1 (%2).';ru='Загружены не все курсы по валюте %1 (%2).';en='Not all currency rates are downloaded % 1 (% 2).'");
		КонецЕсли;
	КонецЕсли;
	...
	Если ДатаЗапрета <> Неопределено Тогда
		ПояснениеОЗагрузке = ПояснениеОЗагрузке + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La carga de las cotizaciones %1 (%2) es limitada la fecha de la prohibición de cambios %3. "
"Los cursos del período prohibido eran dejados pasar a la carga.';ru='Загрузка курсов валюты %1(%2) ограничена датой запрета изменений %3."
"Курсы запрещенного периода были пропущены при загрузке.';en='Loading of currency rates%1 (%2) is limited by the date of the prohibition of changes%3."
"Forbidden courses were skipped at boot time. '"), Валюта.Наименование, Валюта.Код, Формат(ДатаЗапрета, "ДЛФ=D"));
	КонецЕсли;
...
Функция КурсИзСтроки(Знач Строка)
	...
	Если Строка = "" Или ЧастиСтроки.Количество() > 2 Тогда
		ВызватьИсключение НСтр("es='La transformación del significado al tipo el Número no puede ser cumplida.';ru='Преобразование значения к типу Число не может быть выполнено.';en='Converting a value to the type Number cannot be performed.'");
	КонецЕсли;
...
Функция СуммаПрописью(СуммаЧислом, ПараметрыПрописиНаРусскомЯзыке, ВыводитьСуммуБезКопеек = Ложь) Экспорт
	...
	Результат = ЧислоПрописью(Сумма, "ДП=Ложь", ПараметрыПрописиНаРусскомЯзыке); // всегда на русском языке
	// << [Деснол] [Ушаков А.Д.] [26.09.2019] 
...
Функция ИмяСобытияЖурналаРегистрации()
	Возврат НСтр("es='Las divisas. La carga de las cotizaciones';ru='Валюты.Загрузка курсов валют';en='Currencies. Download currency rates'", ОбщегоНазначения.КодОсновногоЯзыка());
КонецФункции
