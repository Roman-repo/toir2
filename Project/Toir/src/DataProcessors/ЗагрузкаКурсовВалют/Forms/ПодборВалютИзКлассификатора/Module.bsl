...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		...
		Элементы.СтраныИТерритории.Заголовок = НСтр("es='Los países';ru='Страны';en='Countries'");
	КонецЕсли;
...
Процедура ОбработатьВыборВСпискеВалют(СтандартнаяОбработка = Неопределено)
	...
	ПоказатьОповещениеПользователя(
		НСтр("es='Las divisas son añadidas.';ru='Валюты добавлены.';en='Currencies added.'"), ,
		?(СтандартныеПодсистемыКлиент.ПараметрыРаботыКлиента().РазделениеВключено И ЕстьКурсы, 
			НСтр("es='Los cursos serán cargados automáticamente a través del tiempo corto.';ru='Курсы будут загружены автоматически через непродолжительное время.';en='Rates will be downloaded automatically after a short time.'"), ""),
