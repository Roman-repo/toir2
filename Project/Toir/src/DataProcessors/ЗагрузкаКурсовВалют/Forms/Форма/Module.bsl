...
Процедура ПроверитьСписокЗагружаемыхВалют()
	Если Объект.СписокВалют.Количество() = 0 Тогда
		...
		ТекстПредупреждения = НСтр("es='En la guía de las divisas faltan las divisas, que cursos es posible cargar de la red el Internet.';ru='В справочнике валют отсутствуют валюты, курсы которых можно загружать из сети Интернет.';en='There are no currencies in the currency directory whose rates can be downloaded from the Internet.'");
...
Функция ВыполнитьЗагрузкуКурсов()
	...
	НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El lanzamiento a mano: %1';ru='Запуск вручную: %1';en='Manual launch:%1'"),
...
Процедура ПриЗавершенииЗагрузкиКурсов(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Ошибка" Тогда
		ВызватьИсключение Результат.КраткоеПредставлениеОшибки + Символы.ПС + НСтр("es='Los detalles de cm. En la revista del registro.';ru='Подробности см. в журнале регистрации.';en='See the log for details.'");
	КонецЕсли;
...
Процедура ОбработатьРезультатЗагрузки(РезультатЗагрузки)
	...
	Иначе
		...
		Если КоличествоОшибок > 1 Тогда
			...
			Кнопки.Добавить("Подробнее", НСтр("es='Más detalladamente...';ru='Подробнее...';en='More details …'"));
			Кнопки.Добавить("Продолжить", НСтр("en='Resume';es='Continuar';ru='Продолжить'"));
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No consiguió cargar las cotizaciones (%1).';ru='Не удалось загрузить курсы валют (%1).';en='Failed to load exchange rates (%1).'"), КоличествоОшибок);
...
Процедура НачатьЗагрузкуКурсовВалют()
	Если ТребуетсяНастройкаАвторизацияИнтернетПоддержки() Тогда
		ТекстВопроса = НСтр("es='Para la carga de las cotizaciones del Internet "
"es necesario conectarse. "
"¿Conectarse al Apoyo De Internet de los usuarios es ahora?';ru='Для загрузки курсов валют из Интернета"
"необходимо подключиться к Интернет-поддержке пользователей."
"Подключиться сейчас?';en='To download exchange rates from the Internet"
"You must connect to online user support."
"Connect now?'");
		ОписаниеОповещения = Новый ОписаниеОповещения("ПриПодключенииИнтернетПоддержки", ЭтотОбъект);
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("es='Conectarse';ru='Подключиться';en='Connect'"));
...
Процедура ПродолжитьЗагрузкуКурсовВалют()
	...
	Если Не ЗначениеЗаполнено(Объект.НачалоПериодаЗагрузки) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='de la carga.';ru='Не задана дата начала периода загрузки.';en='The start date of the download period is not set.'"),
			,
	...
	Если Не ЗначениеЗаполнено(Объект.ОкончаниеПериодаЗагрузки) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='de la carga.';ru='Не задана дата окончания периода загрузки.';en='The end date of the download period is not set.'"),
			,
