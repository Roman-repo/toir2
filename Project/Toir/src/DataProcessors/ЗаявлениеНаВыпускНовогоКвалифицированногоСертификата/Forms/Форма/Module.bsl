...
Процедура ЮридическийАдресНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПредставлениеАдресаНачалоВыбора(ЭтотОбъект, Элемент, ДанныеВыбора, СтандартнаяОбработка,
		"ЮридическийАдрес", НСтр("es='La dirección jurídica de la organización';ru='Юридический адрес организации';en='Legal address of the organization'"));
...
Процедура ФактическийАдресНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПредставлениеАдресаНачалоВыбора(ЭтотОбъект, Элемент, ДанныеВыбора, СтандартнаяОбработка,
		"ФактическийАдрес", НСтр("es='La dirección real de la organización';ru='Фактический адрес организации';en='Organization actual address'"));
...
Процедура ТелефонНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПредставлениеТелефонаНачалоВыбора(ЭтотОбъект, Элемент, ДанныеВыбора, СтандартнаяОбработка,
		"Телефон", НСтр("es='Teléfono de la organización';ru='Телефон организации';en='Organization phone'"));
	...
	Если Не ЗначениеЗаполнено(ГражданствоОКСМКодАльфа3) Тогда
		Если ЗначениеЗаполнено(Гражданство) Тогда
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Cerca del país ""%1"" no es indicado трехзначный el código de letras 3 alfa por ОКСМ.';ru='У страны ""%1"" не указан трехзначный буквенный код альфа-3 по ОКСМ.';en='Country ""%1"" does not have a three-digit alpha-3 alphanumeric code for ACSM.'"),
				Гражданство));
		КонецЕсли;
...
Процедура ПрограммаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Строка") Тогда
		...
		Если Не ЭтоПолноправныйПользователь Тогда
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El programa %1 no es añadido en la lista de los programas usados. "
"Se dirijan al administrador.';ru='Программа %1 еще не добавлена в список используемых программ."
"Обратитесь к администратору.';en='Program %1 has not yet been added to the list of used programs."
"Contact your administrator. '"),
		...
		ИначеЕсли ЭтоПодчиненныйУзелРИБ Тогда
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El programa %1 no es añadido en la lista de los programas usados. "
"Cumplan la adición en el nudo principal de la base informativa.';ru='Программа %1 еще не добавлена в список используемых программ."
"Выполните добавление в главном узле информационной базы.';en='The program %1 has not yet been added to the list of used programs."
"Add in the main node of the infobase.'"),
					Представление));
		Иначе
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить("Добавить",    НСтр("es='Añadir';ru='Добавить';en='Add'"));
			Кнопки.Добавить("НеДобавлять", НСтр("es='No añadir';ru='Не добавлять';en='Do not add'"));
			ПоказатьВопрос(
				Новый ОписаниеОповещения("ПрограммаОбработкаВыбораПродолжение", ЭтотОбъект, ВыбранноеЗначение),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El programa %1 no es añadido en la lista de los programas usados. "
"¿Añadir?';ru='Программа %1 еще не добавлена в список используемых программ."
"Добавить?';en='The program %1 has not yet been added to the list of used programs."
"Add?'"),
					Представление),
				Кнопки,, "НеДобавлять");
		КонецЕсли;
...
Процедура ДекорацияСозданиеКлючаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "КлючЭлектроннойПодписи" Тогда
		ПоказатьПредупреждение(,
			НСтр("es='La llave de la firma electrónica es una información secreta, que se conserva "
"al ordenador, флешку, el disquete u otro portador de la información y "
"se usa en lo sucesivo para la creación de las firmas electrónicas.';ru='Ключ электронной подписи - это секретная информация, которая сохраняется"
"на компьютер, флешку, дискету или другой носитель информации и"
"используется в дальнейшем для создания электронных подписей.';en='An electronic signature key is sensitive information that is stored"
"to a computer, flash drive, floppy disk or other storage medium and"
"used in the future to create electronic signatures. '"));
		
	ИначеЕсли НавигационнаяСсылка = "ЗапросНаСертификат" Тогда
		ПоказатьПредупреждение(,
			НСтр("es='La interpelación por el certificado es no la información secreta, que se crea "
"en base a la llave de la firma electrónica, sale junto con la declaración "
"a la salida del certificado y es necesario para la salida del certificado.';ru='Запрос на сертификат - это не секретная информация, которая создается"
"на основе ключа электронной подписи, отправляется вместе с заявлением"
"на выпуск сертификата и требуется для выпуска сертификата.';en='Certificate request is not classified information that is created"
"based on the electronic signature key, sent with the application"
"to issue a certificate and is required to issue a certificate.'"));
	КонецЕсли;
...
Процедура ИнструкцияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если ЭтоИндивидуальныйПредприниматель Тогда
		ТекстПредупреждения =
			НСтр("es='El juego de los documentos: "
"1. La declaración a la salida del certificado (preparado sobre el paso anterior). "
"2. La copia del certificado del planteamiento al recuento en el órgano fiscal (el INN). "
"3. La copia del certificado del registro estatal del empresario individual (ОГРН). "
"4. La copia del documento que certifica la persona del propietario del certificado. "
"5. La copia del certificado de seguros de la seguridad obligatoria de pensión (СНИЛС) del propietario del certificado.';ru='Комплект документов:"
...
"5. Копия страхового свидетельства обязательного пенсионного страхования (СНИЛС) владельца сертификата.';en='Set of documents:"
"1. Application for the issuance of a certificate (prepared in the previous step)."
"2. A copy of the certificate of registration with the tax authority (TIN)."
"3. A copy of the certificate of state registration of an individual entrepreneur (PSRN)."
"4. A copy of the identity document of the certificate holder."
"5. Copy of the certificate of compulsory pension insurance (SNILS) of the certificate holder. '");
	Иначе
		ТекстПредупреждения =
			НСтр("es='El juego de los documentos: "
"1. La declaración a la salida del certificado (preparado sobre el paso anterior). "
"2. Копия* los certificados del planteamiento al recuento en el órgano fiscal (el INN). "
"3. Копия* los certificados del registro estatal de la persona jurídica (ОГРН). "
"4. Копия* del documento que certifica la persona del representante de la organización, el certificado, indicado para la salida. "
"5. Копия* del certificado de seguros de la seguridad obligatoria de pensión (СНИЛС) del representante de la organización"
" indicada para la salida del certificado. "
"6. Копия* del documento que confirma a los poderes del dirigente, que ha firmado la declaración (la acta de la reunión"
" de los fundadores, la decisión del propietario, el estatuto) o el extracto actual de ЕГРЮЛ, asegurado por el órgano fiscal. "
""
"* las copias de los documentos son aseguradas por la firma del dirigente y la prensa de la organización.';ru='Комплект документов:"
"1. Заявление на выпуск сертификата (подготовленное на предыдущем шаге)."
"2. Копия* свидетельства о постановке на учет в налоговом органе (ИНН)."
...
""
"* Копии документов заверяются подписью руководителя и печатью организации.';en='Set of documents:"
"1. Application for the issuance of a certificate (prepared in the previous step)."
"2. Copy * certificate of registration with the tax authority (TIN)."
"3. Copy * certificate of state registration of legal entity (PSRN)."
"4. A copy * of the identity document of the representative of the organization indicated for the issuance of the certificate."
"5. Copy * of the certificate of compulsory pension insurance (SNILS) of the organization’s representative,"
"   specified for issuing the certificate."
"6. Copy * of the document confirming the authority of the head who signed the statement (minutes of the meeting"
"   founders, owner’s decision, charter) or current extract from the register, certified by the tax authority."
""
"* Copies of documents are certified by the signature of the head and seal of the organization.'");
	КонецЕсли;
...
Процедура ВыгрузитьКорневойСертификат(Команда)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.СохранитьСертификат(Неопределено,
		АдресКорневогоСертификата, НСтр("es='El certificado radical de SA NPTS 1С';ru='Корневой сертификат ООО НПЦ 1С';en='Root certificate of LLC SPC 1C'"));
...
Процедура НапечататьДокументы(Команда)
	...
	НазваниеПечатнойФормы = НСтр("es='La declaración a la salida del certificado';ru='Заявление на выпуск сертификата';en='Certificate application'");
...
Процедура ПерейтиНаСтраницуСоглашение()
	...
	Элементы.ФормаЗакрыть.Заголовок = НСтр("en='cancel';es='La anulación';ru='Отмена'");
...
Процедура ПерейтиНаСтраницуПодготовкаЗапросаНаСертификатНаСервереПриЗагрузке()
	...
	Элементы.ФормаНазад.Видимость = Истина;
	Элементы.ФормаЗакрыть.Заголовок = НСтр("es='Cerrar';ru='Закрыть';en='Close'");
...
Процедура ПерейтиНаСтраницуОтправкаЗаявления()
	...
	Если Не ЗначениеЗаполнено(Объект.Программа) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='Campo ""el Programa de la firma electrónica"" no es rellenado';ru='Поле ""Программа электронной подписи"" не заполнено';en='Field ""Electronic signature program"" is not filled'"), , "Программа");
...
Процедура ПерейтиНаСтраницуОтправкаЗаявленияПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("МенеджерКриптографии") Тогда
		...
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("es='La creación de la llave de la firma electrónica';ru='Создание ключа электронной подписи';en='Creating an electronic signature key'"), "", Результат,
...
Процедура ПерейтиНаСтраницуОжиданиеОбработкиЗаявления()
	...
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить("Отправить",   НСтр("es='Enviar';ru='Отправить';en='Submit'"));
	Кнопки.Добавить("НеОтправить", НСтр("es='No enviar';ru='Не отправлять';en='Do not send'"));
	
	Текст = "";
	Если Не ДокументыПечатались Тогда
		Текст =
			НСтр("es='Los documentos no eran imprimidos. "
"La declaración no será aceptado no serán recibidos los documentos tipográficos.';ru='Документы еще не печатались."
"Заявление не будет принято пока не будут получены печатные документы.';en='Documents have not been printed yet."
"The application will not be accepted until printed documents are received.'");
	КонецЕсли;
	
	Если ДокументыПартнерЭтоИП Тогда
		Если Не ЗначениеЗаполнено(ДокументыПартнерИНН) Тогда
			Текст = Текст + Символы.ПС  + Символы.ПС
				+ НСтр("es='No es indicado al INN de la organización que sirve. "
"La declaración puede ser tratada más largamente regular.';ru='Не указан ИНН обслуживающей организации."
"Заявление может обрабатываться дольше обычного.';en='The TIN of the service organization is not specified."
"The application may take longer than usual. '");
		КонецЕсли;
	...
	ИначеЕсли Не ЗначениеЗаполнено(ДокументыПартнерИНН)
		...
		Текст = Текст + Символы.ПС  + Символы.ПС
			+ НСтр("es='al INN o КПП a la organización que sirve. "
"La declaración puede ser tratada más largamente regular.';ru='Не указан ИНН или КПП обслуживающей организации."
"Заявление может обрабатываться дольше обычного.';en='The TIN or PPC of the service organization is not specified."
"The application may take longer than usual. '");
	КонецЕсли;
...
Процедура ПерейтиНаСтраницуОжиданиеОбработкиЗаявленияПродолжение(Ответ, Контекст) Экспорт
	...
	Если Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей") Тогда
		ПоказатьПредупреждение(,
			НСтр("es='Es imposible conectarse al portal del apoyo de Internet por la causa: "
"la Biblioteca el Internet del apoyo de los usuarios no es introducida en la configuración.';ru='Невозможно подключиться к порталу интернет-поддержки по причине:"
"Библиотека интернет поддержки пользователей не внедрена в конфигурацию.';en='Unable to connect to the online support portal because:"
"The online customer support library is not implemented in the configuration.'"));
	КонецЕсли;
...
Функция ПерейтиНаСтраницуОжиданиеОбработкиЗаявленияНаСервере(ОписаниеОшибки)
	...
	СостояниеОбработкиЗаявления = НСтр("es='La declaración es aceptada para el tratamiento.';ru='Заявление принято для обработки.';en='Application accepted for processing.'");
...
Процедура ОбновитьДатаОбновленияСостоянияВЗаголовке(Форма)
	...
	Форма.Элементы.СостояниеОбработкиЗаявления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El estado del tratamiento de la declaración (en %1)';ru='Состояние обработки заявления (на %1)';en='Application processing status (%1)'"),
...
Процедура ПерейтиНаСтраницуУстановкаСертификата()
	...
	Если Результат <> Истина Тогда
		ПослеЗаписи(НСтр("es='La declaración es declinada.';ru='Заявление отклонено.';en='Application rejected.'"));
		Закрыть();
		Возврат;
	КонецЕсли;
...
Процедура ПерейтиНаСтраницуУстановкаСертификатаПослеУстановки(Установлен, Контекст) Экспорт
	...
	Если Установлен Тогда
		...
		ПослеЗаписи(НСтр("es='La declaración es cumplida.';ru='Заявление исполнено.';en='Application completed.'"));
		Закрыть();
	Иначе
		ПослеЗаписи();
		ЗаписатьЗаявление();
...
Процедура ОбновитьДатаУстановкиСертификатаВЗаголовке(Форма)
	...
	Форма.Элементы.ОшибкаУстановкиСертификата.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La falta de la instalación del certificado (en %1)';ru='Ошибка установки сертификата (на %1)';en='Error installing certificate (%1)'"),
...
Процедура ЗакрытьПомощникПослеУстановкиСертификатаЗавершение(Установлен, Контекст) Экспорт
	...
	ПослеЗаписи(НСтр("es='El certificado es establecido.';ru='Сертификат установлен.';en='The certificate is installed.'"));
	Закрыть();
...
Процедура ПриИзмененииОрганизацииНаСервере(Загрузка = Ложь)
	...
	Если ЭтоИндивидуальныйПредприниматель Тогда
		...
		Элементы.ОГРН.Заголовок = НСтр("es='ОГРНИП';ru='ОГРНИП';en='PSRNIP'");
	Иначе
		Элементы.КПП.Видимость = Истина;
		Элементы.ОГРН.Заголовок = НСтр("es='ОГРН';ru='ОГРН';en='PSRN '");
	КонецЕсли;
	...
	Попытка
		...
		Если ПодробныйИтог.Результат <> "Корректный" Тогда
			...
			Если Не ЗначениеЗаполнено(Сообщение) Тогда
				Сообщение = НСтр("es='La dirección no es rellenada';ru='Адрес не заполнен';en='Address is empty'");
			КонецЕсли;
		КонецЕсли;
	Исключение
...
Функция РасчетныйСчетСоответствуетТребованиям(РасчетныйСчет, ТекстСообщения)
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Значение) Тогда
		ТекстСообщения = НСтр("es='La cuenta contable debe consistir solamente de las cifras.';ru='Расчетный счет должен состоять только из цифр.';en='The settlement account should consist only of numbers.'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрДлина(Значение) <> 20 Тогда
		ТекстСообщения = НСтр("es='La cuenta contable debe consistir de 20 cifras.';ru='Расчетный счет должен состоять из 20 цифр.';en='The settlement acount must consist of 20 digits.'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не РегламентированныеДанныеКлиентСервер.КонтрольныйКлючЛицевогоСчетаСоответствуетТребованиям(Значение, БИК, ЭтоБанк) Тогда
		ТекстСообщения = НСтр("es='El número de control de la cuenta no coincide con contado tomando en cuenta БИК';ru='Контрольное число счета не совпадает с рассчитанным с учетом БИК';en='The control number of the account does not coincide with the calculated taking into account the BIC'");
		Возврат Ложь;
	КонецЕсли;
...
Функция БИКСоответствуетТребованиям(БИК, ТекстСообщения)
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Значение) Тогда
		ТекстСообщения = НСтр("es='БИК debe consistir solamente de las cifras.';ru='БИК должен состоять только из цифр.';en='BIC should consist only of numbers.'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрДлина(Значение) <> 9 Тогда
		ТекстСообщения = НСтр("es='БИК debe consistir de 9 cifras.';ru='БИК должен состоять из 9 цифр.';en='BIC should consist of 9 digits.'");
		Возврат Ложь;
	КонецЕсли;
...
Функция КорреспондентскийСчетСоответствуетТребованиям(КорреспондентскийСчет, ТекстСообщения)
	...
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Значение) Тогда
		ТекстСообщения = НСтр("es='La cuenta de correspondencia debe consistir solamente de las cifras.';ru='Корреспондентский счет должен состоять только из цифр.';en='Correspondent account should consist only of numbers.'");
		Возврат Ложь;
	КонецЕсли;

	Если СтрДлина(Значение) <> 20 Тогда
		ТекстСообщения = НСтр("es='La cuenta de correspondencia debe consistir de 20 cifras.';ru='Корреспондентский счет должен состоять из 20 цифр.';en='Correspondent account must consist of 20 digits.'");
		Возврат Ложь;
	КонецЕсли;
...
Функция АдресСоответствуетТребованиям(Знач АдресXML, ТекстСообщения)
	...
	Если Не АдресСтруктура.Свойство("КодРегиона") Тогда
		ТекстСообщения = ТекстСообщения + НСтр("es='Esto no la dirección rusa';ru='Это не российский адрес';en='This is not a Russian address'");
		Возврат Ложь;
	КонецЕсли;
	
	// Проверка, что указан регион.
	Если Не АдресСтруктура.Свойство("Регион") Или Не ЗначениеЗаполнено(АдресСтруктура.Регион) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("es='la región';ru='Не указан регион';en='Region not specified'");
		Возврат Ложь;
	КонецЕсли;
	
	// Проверка, что регион указан правильно - код региона определен.
	Если Не ЗначениеЗаполнено(АдресСтруктура.КодРегиона) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("es='La región incorrecta (el código de la región no es determinado)';ru='Некорректный регион (код региона не определен)';en='Invalid region (region code not defined)'");
		Возврат Ложь; 
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(НаселенныйПунктПолностью) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("es='la localidad';ru='Не указан населенный пункт';en='No location specified'");
		Возврат Ложь;
	КонецЕсли;
...
Функция ТелефонСоответствуетТребованиям(Знач ТелефонXML, ТекстСообщения)
	...
	Если СтрЗаменить(ТелефонСтруктура.КодСтраны, "+", "") <> "7" Тогда
		ТекстСообщения = ТекстСообщения + НСтр("es='El código del país no ruso (debe ser ""7"")';ru='Код страны не российский (должен быть ""7"")';en='The country code is not Russian (must be ""7"")'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(НомерТелефонаБезКодаСтраны) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("es='de teléfono';ru='Не заполнен номер телефона';en='Phone number is not filled'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрДлина(ТолькоЦифры(НомерТелефонаБезКодаСтраны)) <> 10 Тогда
		ТекстСообщения = ТекстСообщения + НСтр("es='El número de teléfono con el código de la ciudad debe consistir de 10 cifras';ru='Номер телефона с кодом города должен состоять из 10-и цифр';en='Phone number with area code must be 10 digits'");
		Возврат Ложь;
	КонецЕсли;
		...
		Если Не ЭтоИндивидуальныйПредприниматель Тогда
			Если ЗначениеЗаполнено(Реквизиты.Директор) Тогда
				Элементы.Сотрудник.СписокВыбора.Добавить(Реквизиты.Директор, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El director: %1';ru='Директор: %1';en='Director:% 1'"), Реквизиты.Директор));
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(Реквизиты.ГлавныйБухгалтер) Тогда
				Элементы.Сотрудник.СписокВыбора.Добавить(Реквизиты.ГлавныйБухгалтер, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El jefe de contabilidad: %1';ru='Главный бухгалтер: %1';en='Chief Accountant:% 1'"), Реквизиты.ГлавныйБухгалтер));
			КонецЕсли;
		КонецЕсли;
		
		Если ЭтоИндивидуальныйПредприниматель Тогда
			Элементы.Сотрудник.Заголовок = НСтр("es='El empresario individual';ru='Индивидуальный предприниматель';en='Individual entrepreneur'");
		Иначе
			Элементы.Сотрудник.Заголовок = НСтр("es='El empleado';ru='Сотрудник';en='Employee'");
		КонецЕсли;
...
Процедура ПриИзмененииВидаДокументаНаСервере()
	...
	Если ДокументВид = "21" Тогда
		Элементы.ДокументНомер.Заголовок = НСтр("es='Una serie y el número';ru='Серия и номер';en='Series and number'");
		Элементы.ДокументНомер.Маска = "99 99 999999";
		Элементы.ДокументКодПодразделения.Видимость = Истина;
	Иначе
		Элементы.ДокументНомер.Заголовок = НСтр("es='El número';ru='Номер';en='Number'");
...
Функция НомерПаспортаРФСоответствуетТребованиям(Знач НомерПаспортаРФ, ТекстСообщения)
	...
	Если ПустаяСтрока(СтрокаЦифр) Тогда
		ТекстСообщения = ТекстСообщения + НСтр("es='El número del pasaporte no es rellenado';ru='Номер паспорта не заполнен';en='Passport number is empty'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрДлина(СтрокаЦифр) < 10 Тогда
		ТекстСообщения  =  ТекстСообщения + НСтр("es='El número del pasaporte es dado no totalmente';ru='Номер паспорта задан неполностью';en='Passport number is incomplete'");
		Возврат Ложь;
	КонецЕсли;
	
	Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрокаЦифр) Тогда
		Результат = Ложь;
		ТекстСообщения = ТекстСообщения + НСтр("es='El número del pasaporte debe consistir solamente de las cifras.';ru='Номер паспорта должен состоять только из цифр.';en='The passport number must consist of numbers only.'");
	КонецЕсли;
...
Функция КодПодразделенияВыдачиПаспортаРФСоответствуетТребованиям(КодПодразделения, ТекстСообщения)
	...
	Если СтрДлина(СтрокаЦифр) < 6 Тогда
		ТекстСообщения  = НСтр("es='El código de la subdivisión es dado no totalmente';ru='Код подразделения задан неполностью';en='Unit code is incomplete'");
		Возврат Ложь;
	КонецЕсли;
	
	Если Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрокаЦифр) Тогда
		ТекстСообщения = НСтр("es='El código de la subdivisión debe consistir solamente de las cifras.';ru='Код подразделения должен состоять только из цифр.';en='Unit code must consist of numbers only.'");
		Возврат Ложь;
	КонецЕсли;
	
	Если СтрокаЦифр = "000000" Тогда
		ТекстСообщения = НСтр("es='El código de la subdivisión no puede ser de cero.';ru='Код подразделения не может быть нулевым.';en='Unit code cannot be null.'");
		Возврат Истина;
	КонецЕсли;
...
Функция ПроверитьРеквизит(Отказ, ИмяРеквизита)
	...
	Если Не ЗначениеЗаполнено(ЭтотОбъект[ИмяРеквизита]) Тогда
		ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Campo ""%1"" no es rellenado';ru='Поле ""%1"" не заполнено';en='Field ""%1"" is empty'"), ЗаголовокПоля), , ИмяРеквизита, , Отказ);
...
Функция ПодготовленныйДокумент()
	...
	Макет.Параметры.УдостоверениеЛичности = ДокументВидПредставление + " " + ДокументНомерПредставление + " "
		+ НСтр("es='De';ru='от';en='from'") + " " + ДокументДатаВыдачиПредставление + " "
		+ НСтр("es='Dado';ru='выданный';en='issued by'") + " " + ДокументКемВыдан
		+ ?(ЗначениеЗаполнено(ДокументКодПодразделенияПредставление), ", " + НСтр("es='El código de la subdivisión';ru='код подразделения';en='unit code'")
...
Функция ОтправитьЗаявление(ОписаниеОшибки)
	...
	ШаблонОписанияОшибки =
		НСтр("es='No consiguió enviar la declaración por la causa: "
"%1';ru='Не удалось отправить заявление по причине:"
"%1';en='Failed to send the application due to:"
"%1'");
...
"The online customer support library is not implemented in the configuration.'");
	...
	Если ЗначениеЗаполнено(Результат.КодОшибки) Тогда
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió conectarse al portal del apoyo de Internet por la causa: "
"%1';ru='Не удалось подключиться к порталу интернет-поддержки по причине:"
"%1';en='Failed to connect to the online support portal due to:"
"%1'"),
	...
	Если Не ЗначениеЗаполнено(Ответ) Тогда
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОписанияОшибки,
			НСтр("es='El servidor ha devuelto la respuesta vacía.';ru='Сервер вернул пустой ответ.';en='Server returned an empty response.'"));
		Возврат Ложь;
	КонецЕсли;
...
Функция ТекстФайлаЗапросаНаСертификат()
	...
	ТекстОшибки =
		НСтр("es='No consiguió leer antes la interpelación creada por el certificado. "
"La declaración no puede ser enviada. "
"Lo quiten y creen nuevo.';ru='Не удалось прочитать ранее созданный запрос на сертификат."
"Заявление не может быть отправлено."
"Удалите его и создайте новое.';en='Failed to read the previously created certificate request."
...
Функция ПолучитьСертификат()
	...
	ШаблонОписанияОшибки =
		НСтр("es='No consiguió renovar el estado por la causa: "
"%1';ru='Не удалось обновить состояние по причине:"
"%1';en='Failed to update state due to:"
"%1'");
	...
	Если КодРезультата = "0" Тогда
		...
		Иначе
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОписанияОшибки,
				НСтр("es='El servidor ha devuelto los datos vacíos.';ru='Сервер вернул пустые данные.';en='Server returned empty data.'"));
		КонецЕсли;
		
	ИначеЕсли КодРезультата = "1" Тогда
		ОписаниеОшибки = НСтр("es='La declaración no es tratada, prueben más tarde.';ru='Заявление еще не обработано, попробуйте позже.';en='The application has not yet been processed, please try again later.'");
	Иначе
		УзелDOM = ПостроительDOM.ПолучитьЭлементыПоИмени("errorMessage");
		Если УзелDOM.Количество() > 0 Тогда
			ОписаниеОшибки = УзелDOM[0].ТекстовоеСодержимое;
		Иначе
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El servidor ha devuelto el código de la falta: %1';ru='Сервер вернул код ошибки: %1';en='Server returned error code:%1'"),
	...
	Попытка
		...
		Если Не ФайлНайден Тогда
			ВызватьИсключение НСтр("es='El formato equivocado de los datos.';ru='Неверный формат данных.';en='Invalid data format.'");
		КонецЕсли;
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió desmontar la respuesta del servidor por la causa: "
"%1';ru='Не удалось разобрать ответ сервера по причине:"
"%1';en='Failed to parse server response due to:"
"%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	КонецПопытки;
...
Процедура УстановитьСертификатПослеСозданияМенеджераКриптографии(Менеджер, Контекст) Экспорт
	...
	Если ТипЗнч(Менеджер) <> Тип("МенеджерКриптографии") Тогда
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("es='La instalación del certificado al ordenador';ru='Установка сертификата на компьютер';en='Installing a certificate on a computer'"), "", Менеджер,
			Новый Структура, Новый Структура("ПоказатьИнструкцию", Истина));
	...
	Иначе
		СоздатьОбъектКриптографии(Новый ОписаниеОповещения(
			...
			НСтр("es='Para la instalación del certificado al ordenador es necesario "
"establecer la ampliación para el cliente web 1С:Предприятия.';ru='Для установки сертификата на компьютер требуется"
"установить расширение для веб-клиента 1С:Предприятия.';en='To install a certificate on a computer,"
"install the extension for the 1C: Enterprise web client. '"),
			НСтр("es='Para la instalación del certificado al ordenador es necesario "
"establecer el componente exterior.';ru='Для установки сертификата на компьютер требуется"
"установить внешнюю компоненту.';en='To install a certificate on a computer,"
...
Процедура УстановитьСертификатПриОшибкеСозданияОбъектаКриптографииПослеПодключенияРасширенияРаботыСФайлами(Подключено, Контекст) Экспорт
	...
	Если Не Подключено Тогда
		ОшибкаУстановкиСертификата = НСтр("es='los ficheros.';ru='Не установлено расширение для работы с файлами.';en='The extension for working with files is not installed.'");
	Иначе
...
Процедура УстановитьСертификатПриОшибкеСозданияОбъектаКриптографииПослеПодключенияРасширенияРаботыСКриптографией(Подключено, Контекст) Экспорт
	...
	Если Не Подключено Тогда
		ОшибкаУстановкиСертификата = НСтр("es='la criptografía.';ru='Не установлено расширение для работы с криптографией.';en='The extension for working with cryptography is not installed.'");
	Иначе
		ОшибкаУстановкиСертификата = НСтр("es='el componente exterior.';ru='Не установлена внешняя компонента.';en='An external component is not installed.'");
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПолученияРазрешений(РазрешенияПолучены, Контекст) Экспорт
	...
	Если Не РазрешенияПолучены Тогда
		ОшибкаУстановкиСертификата =
			НСтр("es='La conservación de los certificados en la carpeta temporal es anulada por el usuario.';ru='Сохранение сертификатов во временную папку отменено пользователем.';en='Saving certificates to a temporary folder was canceled by the user.'");
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Ложь);
		Возврат;
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Если ПолученныеФайлы = Неопределено
	 Или ПолученныеФайлы.Количество() <> 2 Тогда
		...
		ОшибкаУстановкиСертификата = НСтр("es='Los certificados no eran conservados en la carpeta temporal.';ru='Сертификаты не были сохранены во временную папку.';en='Certificates were not saved to a temporary folder.'");
...
Процедура УстановитьСертификатПослеПроверкиСуществованияКонтейнераКлючей(Существует, Контекст) Экспорт
	...
	Если Не Существует Тогда
		ОшибкаУстановкиСертификата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió encontrar el contenedor de la llave en el ordenador. "
"El nombre del contenedor: ""%1"". "
"La vía al contenedor: ""%2"".';ru='Не удалось найти контейнер ключа на компьютере."
"Имя контейнера: ""%1""."
"Путь к контейнеру: ""%2"".';en='Failed to find the key container on the computer."
"Container name: ""%1""."
"Container path: ""%2"". '"),
			КонтейнерКлючаИмя,
			КонтейнерКлючаПуть);
...
Процедура УстановитьСертификатПослеОшибкиВызоваУстановитьСертификатВКонтейнерИХранилище(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	ОшибкаУстановкиСертификата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='No consiguió establecer el certificado personal en el contenedor de las llaves y el depósito de la SO por la causa: "
"%1';ru='Не удалось установить личный сертификат в контейнер ключей и хранилище ОС по причине:"
"%1';en='Failed to install personal certificate in key container and OS storage due to:"
"%1'"),
...
Процедура УстановитьСертификатПослеВызоваУстановитьСертификатВКонтейнерИХранилищеПослеПолученияСписокОшибок(СписокОшибок, Контекст) Экспорт
	...
	Если Результат <> Истина И Не ЗначениеЗаполнено(ОшибкаУстановкиСертификата) Тогда
		ОшибкаУстановкиСертификата = НСтр("es='No consiguió establecer el certificado personal en el contenedor de las llaves y el depósito de la SO.';ru='Не удалось установить личный сертификат в контейнер ключей и хранилище ОС.';en='Failed to install personal certificate in key container and OS storage.'");
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПоискаСертификата(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("СертификатКриптографии") Тогда
		Если Результат.Свойство("СертификатНеНайден") Тогда
			ОшибкаПоиска =
				НСтр("es='No consiguió encontrar el certificado personal establecido al ordenador.';ru='Не удалось найти личный сертификат, установленный на компьютер.';en='Failed to find a personal certificate installed on the computer.'");
		Иначе
			ОшибкаПоиска = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió encontrar el certificado personal establecido al ordenador por la causa: "
"%1';ru='Не удалось найти личный сертификат, установленный на компьютер по причине:"
"%1';en='Failed to find a personal certificate installed on the computer due to:"
"%one'"),
...
Процедура УстановитьСертификатПослеОшибкиВызоваИмпортироватьСертификат(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	ОшибкаУстановкиСертификата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='No consiguió establecer el certificado radical en el depósito de la SO por la causa: "
"%1';ru='Не удалось установить корневой сертификат в хранилище ОС по причине:"
"%1';en='Failed to install the root certificate in the OS repository due to:"
"%1'"),
		ПредставлениеОшибкиДляПользователя(ИнформацияОбОшибке));
...
Процедура УстановитьСертификатПослеВызоваИмпортироватьСертификатПослеПолученияСписокОшибок(СписокОшибок, Контекст) Экспорт
	...
	Если Результат <> Истина И Не ЗначениеЗаполнено(ОшибкаУстановкиСертификата) Тогда
		ОшибкаУстановкиСертификата = НСтр("es='No consiguió establecer el certificado radical en el depósito de la SO.';ru='Не удалось установить корневой сертификат в хранилище ОС.';en='Failed to install root certificate in OS repository.'");
	КонецЕсли;
...
Процедура УстановитьСертификатПослеПоискаКорневогоСертификата(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("СертификатКриптографии") Тогда
		Если Результат.Свойство("СертификатНеНайден") Тогда
			ОшибкаПоиска =
				НСтр("es='No consiguió encontrar el certificado radical establecido al ordenador.';ru='Не удалось найти корневой сертификат, установленный на компьютер.';en='Failed to find the root certificate installed on the computer.'");
		Иначе
			ОшибкаПоиска = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió encontrar el certificado radical establecido al ordenador por la causa: "
"%1';ru='Не удалось найти корневой сертификат, установленный на компьютер по причине:"
"%1';en='Failed to find the root certificate installed on the computer due to:"
"%1'"),
				Результат.ОписаниеОшибки);
		КонецЕсли;
		ОшибкаУстановкиСертификата = СокрЛ(ОшибкаУстановкиСертификата + Символы.ПС + Символы.ПС) + ОшибкаПоиска;
...
Функция ИмяРегионаРФПоРекомендациямДляСКПЭП(КодРегиона)
	...
	Если Не СтрНачинаетсяС(Строка, КодРегиона + " ") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La región de la Federación Rusa con el código ""%1"" no existe.';ru='Регион РФ с кодом ""%1"" не существует.';en='The region of the Russian Federation with the code ""%1"" does not exist.'"),
			КодРегиона);
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Строка) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Para la región de la Federación Rusa con el código ""%1"" nombre recomendado para СКПЭП, no es fijado.';ru='Для региона РФ с кодом ""%1"" имя, рекомендованное для СКПЭП, еще не назначено.';en='For the region of the Russian Federation with the code ""% 1"", the name recommended for SKEPEP has not yet been assigned.'"),
			КодРегиона);
	КонецЕсли;
...
Процедура ЗагрузитьЗаявление()
	...
	ИначеЕсли ТекущийОбъект.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.Отклонено Тогда
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("es='La declaración, por que no consiguió recibir el certificado';ru='Заявление, по которому не удалось получить сертификат';en='Certificate failed application'");
		Элементы.ФормаДалее.Видимость = Ложь;
		Элементы.ФормаЗакрыть.Заголовок = НСтр("es='Cerrar';ru='Закрыть';en='Close'");
	...
	ИначеЕсли ТекущийОбъект.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.Исполнено Тогда
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("es='La declaración, por que era recibido el certificado';ru='Заявление, по которому был получен сертификат';en='Application for which the certificate was received'");
...
Процедура ЗаполнитьРеквизитыЗаявления(Все = Ложь)
	...
	Если Все Тогда
		...
		Строка.Реквизит = НСтр("es='El programa de la firma electrónica';ru='Программа электронной подписи';en='Electronic signature program'");
		Строка.Значение = ПредставлениеПрограммы;
	КонецЕсли;
	
	// Заполнение сведений об организации.
	ЗаполнитьРеквизит(НСтр("es='Las noticias sobre la organización';ru='Сведения об организации';en='Organization information'"),, Истина);
	...
	ЗаполнитьРеквизит(НСтр("es='Las noticias sobre el propietario del certificado';ru='Сведения о владельце сертификата';en='Certificate holder information'"),, Истина);
	...
	Строка = РеквизитыЗаявления.Добавить();
	Строка.Реквизит = НСтр("es='El documento que certifica la persona';ru='Документ, удостоверяющий личность';en='Identity document'");
	Строка.Значение = Элементы.ДокументВид.СписокВыбора.НайтиПоЗначению(ДокументВид).Представление;
	
	Строка = РеквизитыЗаявления.Добавить();
	Если ДокументВид = "21" Тогда
		Строка.Реквизит = НСтр("es='Una serie y el número';ru='Серия и номер';en='Series and number'");
		Строка.Значение = ПредставлениеРеквизитаНомерПаспортаРФ(ДокументНомер);
	Иначе
		Строка.Реквизит = НСтр("es='El número';ru='Номер';en='Number'");
	...
	Если Не ЭтоИндивидуальныйПредприниматель Тогда
		...
		ЗаполнитьРеквизит(НСтр("es='Las noticias sobre el dirigente';ru='Сведения о руководителе';en='Head information'"),, Истина);
		
		Строка = РеквизитыЗаявления.Добавить();
		Строка.Реквизит = НСтр("es='ФИО del dirigente';ru='ФИО руководителя';en='Full name of the head'");
		Строка.Значение = ДокументыРуководитель;
		
		Строка = РеквизитыЗаявления.Добавить();
		Строка.Реквизит = НСтр("es='El puesto del dirigente';ru='Должность руководителя';en='Head position'");
		Строка.Значение = ДокументыРуководительДолжность;
		
		Строка = РеквизитыЗаявления.Добавить();
		Строка.Реквизит = НСтр("es='La razón de las acciones del dirigente';ru='Основание действий руководителя';en='The basis for the actions of the leader'");
		Строка.Значение = ДокументыРуководительОснование;
	КонецЕсли;
	
	// Заполнение сведений о печатных документах.
	ЗаполнитьРеквизит(НСтр("es='Las noticias sobre la organización que sirve';ru='Сведения об обслуживающей организации';en='Information about the service organization'"),, Истина);
	
	Строка = РеквизитыЗаявления.Добавить();
	Строка.Реквизит = НСтр("es='El nombre del socio';ru='Наименование партнера';en='Name of partner'");
	Строка.Значение = ДокументыПартнер;
	
	Строка = РеквизитыЗаявления.Добавить();
	Строка.Реквизит = НСтр("es='El INN del socio';ru='ИНН партнера';en='Partner TIN'");
	Строка.Значение = ДокументыПартнерИНН;
	
	Если Не ДокументыПартнерЭтоИП Тогда
		Строка = РеквизитыЗаявления.Добавить();
		Строка.Реквизит = НСтр("es='КПП del socio';ru='КПП партнера';en='Partner PPC'");
...
Функция ПредставлениеРеквизитаПол(Пол)
	...
	Если Пол = "Мужской" Тогда
		Возврат НСтр("es='De hombre';ru='Мужской';en='Male'");
		
	ИначеЕсли Пол = "Женский" Тогда
		Возврат НСтр("es='Femenino';ru='Женский';en='Female'");
	КонецЕсли;
...
Процедура УстановитьСоглашение()
	...
	ТекстHTML = СтрЗаменить(ТекстHTML, "%1",
		...
		НСтр("es='SA «centro De investigación y de producción"" 1С ""»';ru='ООО «Научно-производственный центр ""1С""»';en='LLC ""Scientific-Production Center"" 1C'"));
	
	ТекстHTML = СтрЗаменить(ТекстHTML, "%2",
		// Адрес регламента удостоверяющего центра.
		НСтр("es='http://ca.1c.ru/reglament.pdf';ru='http://ca.1c.ru/reglament.pdf';en='http://ca.1c.ru/reglament.pdf'"));
	
	ТекстHTML = СтрЗаменить(ТекстHTML, "%3",
		// Сеть доверенных удостоверяющих центров.
		НСтр("es='Del órgano autorizado federal en el campo del uso de la firma electrónica';ru='уполномоченного федерального органа в области использования электронной подписи';en='authorized federal body in the use of electronic signatures'"));
...
Функция ВариантАлгоритмов()
	...
	Попытка
		...
		Если СвойстваФункции = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='En el servicio web ""%1"" no es determinada la función obligatoria ""GetCertificateSettings"".';ru='В веб-сервисе ""%1"" не определена обязательная функция ""GetCertificateSettings"".';en='Web service ""% 1"" does not define the required function ""GetCertificateSettings"".'"),
		...
		Если ВариантАлгоритмов <> "2001" И ВариантАлгоритмов <> "2012" Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La respuesta del servidor no corresponde esperado: "
"%1';ru='Ответ сервера не соответствует ожидаемому:"
"%1';en='Server response is not as expected:"
"%1'"), Ответ);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir la variante exigida de los algoritmos del servidor 1С:Подпись por la causa: "
"%1';ru='Не удалось получить требуемый вариант алгоритмов от сервера 1С:Подпись по причине:"
"%1';en='Failed to get the required version of the algorithms from the server 1C: Signature due to:"
...
Процедура ПослеВыбораПрограммыПослеСозданияМенеджераКриптографии(Менеджер, Контекст) Экспорт
	...
	Если ТипЗнч(Менеджер) <> Тип("МенеджерКриптографии") Тогда
		...
		Если Контекст.ПоказатьОшибку Тогда
			ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
				НСтр("es='La elección del programa de la firma electrónica';ru='Выбор программы электронной подписи';en='Choosing an Electronic Signature Program'"), "", Менеджер,
				Новый Структура, Новый Структура("ПоказатьИнструкцию", Истина));
		КонецЕсли;
...
Процедура ЗаписатьЗаявление()
	...
	Если ТекущийОбъект.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.НеПодготовлено Тогда
		...
		ТекущийОбъект.Наименование = СокрП(Фамилия + " " + Имя + " " + Отчество)
			+ " (" + НСтр("es='La declaración a un nuevo certificado';ru='Заявление на новый сертификат';en='Application for a new certificate'") + ")";
		// Организация.
	...
	ИначеЕсли ТекущийОбъект.СостояниеЗаявления = Перечисления.СостоянияЗаявленияНаВыпускСертификата.Отклонено Тогда
		Если ЭтоНовоеСостояние Тогда
			ОбновитьЗначение(Содержание, "ДатаОбновленияСостояния");
			ОбновитьЗначение(Содержание, "СостояниеОбработкиЗаявления");
			НовоеНаименование = СокрП(Фамилия + " " + Имя + " " + Отчество) + " (" + НСтр("es='Quitar';ru='Удалить';en='Delete'") + ")";
...
Процедура ПослеЗаписи(ТекстОповещения = "")
	...
	Если Не ЗначениеЗаполнено(ТекстОповещения) Тогда
		ТекстОповещения = НСтр("es='La declaración es conservada.';ru='Заявление сохранено.';en='Application saved.'");
	КонецЕсли;
...
Процедура СоздатьКлючИЗапросНаСертификат(Оповещение)
	...
	Иначе
		СоздатьОбъектКриптографии(Новый ОписаниеОповещения(
			...
			НСтр("es='Para la creación de la llave de la firma electrónica y la interpelación por el certificado "
"es necesario establecer la ampliación para el cliente web 1С:Предприятия.';ru='Для создания ключа электронной подписи и запроса на сертификат"
"требуется установить расширение для веб-клиента 1С:Предприятия.';en='To create an electronic signature key and certificate request"
"it is required to install the extension for the 1C: Enterprise web client.'"),
			НСтр("es='Para la creación de la llave de la firma electrónica y la interpelación por el certificado "
"es necesario establecer el componente exterior.';ru='Для создания ключа электронной подписи и запроса на сертификат"
"требуется установить внешнюю компоненту.';en='To create an electronic signature key and certificate request"
"an external component is required. '"));
	КонецЕсли;
...
Процедура СоздатьКлючИЗапросНаСертификатПослеПроверкиСуществованияКонтейнераКлючей(Существует, Контекст) Экспорт
	...
	Если Существует Тогда
		ПоказатьОповещениеПользователя(НСтр("es='Se usa antes el contenedor creado de la llave:';ru='Используется ранее созданный контейнер ключа:';en='The previously created key container is used:'"),,
			КонтейнерКлючаПуть);
...
Процедура СоздатьКлючИЗапросНаСертификатПослеОшибкиВызоваСоздатьКонтейнер(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	Ошибка.Вставить("ОписаниеОшибки", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='No consiguió cumplir la operación de la causa: "
"%1';ru='Не удалось выполнить операцию по причине:"
"%1';en='The operation failed due to:"
"%1'"),
		ПредставлениеОшибкиДляПользователя(ИнформацияОбОшибке)));
	
	ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
		НСтр("es='La creación de la llave de la firma electrónica';ru='Создание ключа электронной подписи';en='Creating an electronic signature key'"), "", Ошибка,
		Новый Структура, Новый Структура("ПоказатьИнструкцию", Истина));
...
Процедура СоздатьКлючИЗапросНаСертификатПослеВызоваСоздатьКонтейнерПослеПолученияСписокОшибок(СписокОшибок, Контекст) Экспорт
	...
	Если Не ЗначениеЗаполнено(Путь) Тогда
		...
		Если СтрНачинаетсяС(Строки[0].Идентификатор, "CryptoPro") Тогда
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La llave de la firma electrónica no es creada. "
""
"Debe tomar en consideración que para la creación de la llave por medio del programa %1, "
"son necesarios los derechos del administrador del sistema de operaciones.';ru='Ключ электронной подписи не создан."
""
"Следует учитывать, что для создания ключа с помощью программы %1,"
"требуются права администратора операционной системы.';en='An electronic signature key has not been created."
""
"Please note that to create a key using the program %1,"
"administrator privileges are required.'"),
				Строки[0].Представление));
		Иначе
			ПоказатьПредупреждение(,
				НСтр("es='La llave de la firma electrónica no es creada.';ru='Ключ электронной подписи не создан.';en='An electronic signature key has not been created.'"));
		КонецЕсли;
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Ложь);
		Возврат;
	КонецЕсли;
	...
	ПоказатьОповещениеПользователя(НСтр("es='Es creado un nuevo contenedor de la llave:';ru='Создан новый контейнер ключа:';en='A new key container has been created:'"),, КонтейнерКлючаПуть);
...
Процедура СоздатьКлючИЗапросНаСертификатПослеОшибкиВызоваСоздатьЗапросНаСертификат(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке,
		НСтр("es='No consiguió cumplir la operación de la causa:';ru='Не удалось выполнить операцию по причине:';en='The operation failed due to:'"));
...
Процедура СоздатьКлючИЗапросНаСертификатПослеОшибкиВызоваПолучитьОткрытыйКлюч(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке,
		НСтр("es='No consiguió recibir la parte abierta de la llave por la causa:';ru='Не удалось получить открытую часть ключа по причине:';en='Failed to get the public part of the key due to:'"));
...
Процедура СоздатьКлючИЗапросНаСертификатПослеОшибкиПомещенияФайлов(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке,
		НСтр("es='No consiguió entregar el fichero de la interpelación por el certificado al servidor por la causa:';ru='Не удалось передать файл запроса на сертификат на сервер по причине:';en='Failed to transfer certificate request file to server due to:'"));
...
Процедура СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке = Неопределено, ЗаголовокОшибки = "")
	...
	Если ИнформацияОбОшибке <> Неопределено Тогда
		...
		ЭлектроннаяПодписьСлужебныйКлиент.ПоказатьОшибкуОбращенияКПрограмме(
			НСтр("es='La creación de la interpelación por el certificado';ru='Создание запроса на сертификат';en='Creating a certificate request'"), "", Ошибка,
			Новый Структура, Новый Структура("ПоказатьИнструкцию", Истина));
		
	ИначеЕсли Не Контекст.ЗапросСоздан Тогда
		ПоказатьПредупреждение(, НСтр("es='La interpelación por el certificado no es creada.';ru='Запрос на сертификат не создан.';en='Certificate request not created.'"));
	КонецЕсли;
...
Процедура СоздатьКлючИЗапросНаСертификатПослеОшибкиУдаленияФайлов(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	СоздатьКлючИЗапросНаСертификатЗавершение(Контекст, ИнформацияОбОшибке,
		НСтр("es='No consiguió quitar el fichero de la interpelación por el certificado en el ordenador por la causa:';ru='Не удалось удалить файл запроса на сертификат на компьютере по причине:';en='Failed to delete certificate request file on computer due to:'"));
...
Процедура ПолучитьИмяНовогоКонтейнераКлюча(Оповещение)
	...
	Если ЭтоИндивидуальныйПредприниматель Тогда
		...
		Субъект = ПодготовитьСтрокуДляИмениКонтейнера(Фамилия) + " "
			...
			+ ПодготовитьСтрокуДляИмениКонтейнера(НСтр("es='El empresario individual';ru='Индивидуальный предприниматель';en='Individual entrepreneur'"));
	Иначе
...
Процедура ПолучитьИменаКонтейнеровКлючейЦиклПослеОшибкиВызоваПолучитьСледующийКонтейнерКлючей(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	ПоказатьПредупреждение(,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió recibir el nombre del contenedor siguiente de las llaves por la causa: "
"%1';ru='Не удалось получить имя следующего контейнера ключей по причине:"
"%1';en='Failed to get the name of the following key container due to:            %1'"),
			ПредставлениеОшибкиДляПользователя(ИнформацияОбОшибке)));
...
Процедура СоздатьОбъектКриптографииПослеОшибкиВызоваСоздатьМенеджераКриптографии(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	ПоказатьПредупреждение(Новый ОписаниеОповещения(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El programa de la firma electrónica no es accesible a través del componente exterior por la causa: "
"%1';ru='Программа электронной подписи не доступна через внешнюю компоненту по причине:"
"%1';en='The electronic signature program is not available through an external component due to:"
"%1'"),
			ПредставлениеОшибкиДляПользователя(ИнформацияОбОшибке)));
...
Функция ВозможноПродолжениеПослеОперацииКомпоненты()
	...
	ПоказатьПредупреждение(,
		НСтр("es='La declaración a la salida del certificado era cerrada, la operación es anulada.';ru='Заявление на выпуск сертификата было закрыто, операция отменена.';en='The certificate issuance application was closed, the operation was canceled.'"));
