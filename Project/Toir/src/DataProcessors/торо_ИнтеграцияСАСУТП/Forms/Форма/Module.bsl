...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	нз = РегистрыСведений.торо_СписокОповещаемыхЛиц.СоздатьНаборЗаписей();
...
Процедура ЗагрузитьСоответствияИзМакета(Команда)
	...
	Для номСтроки = 2 по ВсегоСтрок Цикл
		струк = Новый Структура("НаименованиеОР, Код, Тег, Подключение, ОбъектРемонта, ТипСигнала, Показатель, ПараметрНаработки, Состояние, Отключить, Ответственный, Подразделение, Организация, ПричинаПростоя, ВидДефекта, ПоказательKPI");
	...
	Иначе
		ТекстСообщения = НСтр("es='Son cargadas 0 anotaciones.';ru='Загружено 0 записей.';en='Downloaded 0 records.'");
...
Процедура ЗагрузитьСоответствияИзМакетаНаСервере(МасСтрок)
	...
	Для каждого текСтрока из тзДанные Цикл
		...
		Если текСтрока.ТипСигнала = Перечисления.торо_ТипыСигналовАСУТП.Показатель Тогда
			Если не ЗначениеЗаполнено(текСтрока.Показатель) или не ЗначениеЗаполнено(текСтрока.ВидДефекта) Тогда
				Ошибка = истина;
			КонецЕсли;
	...
	ШаблонСообщения = НСтр("es='Es cargado %1 anotaciones.';ru='Загружено %1 записей.';en='%1 records downloaded.'");
...
Процедура ДЗвБД()
	...
	торо_ПолучениеДанныхАСУТП.ГенераторЗаписей(КоличествоЗаписей, Истина);
