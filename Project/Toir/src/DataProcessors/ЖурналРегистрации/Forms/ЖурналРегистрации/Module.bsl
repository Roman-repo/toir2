...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Критичность = дс_Критичность_ВсеСобытия(); // идентификатор.
...
Процедура КритичностьПриИзменении(Элемент)
	...
	// >> [Деснол] [Ушаков А.Д.] [24.10.2019] 
	Если Критичность = дс_Критичность_Ошибка() Тогда
	...
	// << [Деснол] [Ушаков А.Д.] [24.10.2019] 
		...
		ОбновитьТекущийСписок();
	// >> [Деснол] [Ушаков А.Д.] [24.10.2019] 
	ИначеЕсли Критичность = дс_Критичность_Предупреждение() Тогда
...
Процедура ВыгрузитьЖурналДляПередачиВТехподдержку(Команда)
	...
	ПараметрыСохраненияФайла.Диалог.Фильтр = НСтр("es='Los datos de la revista del registro';ru='Данные журнала регистрации';en='Logbook data'") + "(*.xml)|*.xml";
...
Функция ПрочитатьЖурнал()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='La renovación de la revista del registro';ru='Обновление журнала регистрации';en='Logbook update'");
...
Процедура дс_УстановитьЗаголовкиЭлементов()
	...
	Критичности.Добавить(дс_Критичность_Ошибка());
	Критичности.Добавить(дс_Критичность_Предупреждение());
	Критичности.Добавить(дс_Критичность_ВсеСобытия());
	
	Элементы.Критичность.СписокВыбора.ЗагрузитьЗначения(Критичности);
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция дс_Критичность_Ошибка()
	Возврат НСтр("es='Error';ru='Ошибка';en='Error '");
КонецФункции 
 
&НаКлиентеНаСервереБезКонтекста
Функция дс_Критичность_Предупреждение()
	Возврат НСтр("es='Advertencia';ru='Предупреждение';en='Warning'");
КонецФункции 
 
&НаКлиентеНаСервереБезКонтекста
Функция дс_Критичность_ВсеСобытия()
	Возврат НСтр("es='Todos los eventos';ru='Все события';en='All events'");
КонецФункции 
