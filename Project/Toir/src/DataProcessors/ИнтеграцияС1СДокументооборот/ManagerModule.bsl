...
Процедура УстановитьЗаголовокДокумента(Форма) Экспорт
	...
	Если Не ЗначениеЗаполнено(Форма.ID) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='%1 (creación)';ru='%1 (создание)';en='%1 (create)'"), ПредставлениеТипа);
	Иначе
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='%1 (%2)';ru='%1 (%2)';en='%1(%2)'"), ПредставлениеДокумента, ПредставлениеТипа);
	КонецЕсли;
...
Процедура УстановитьСвойстваЭлементовХронометража(
	...
	Если ВключенХронометраж Тогда // хронометраж включен
		КомандаПереключитьХронометраж.Подсказка = НСтр("es='Acabar el cronometraje';ru='Закончить хронометраж';en='Finish timing'");
		ЭлементПереключитьХронометраж.Пометка = Истина;
	Иначе
		КомандаПереключитьХронометраж.Подсказка = НСтр("es='Incluir el cronometraje';ru='Включить хронометраж';en='Enable timing'");
...
Функция ЗаполнитьДеревоСвязейИНачатьПолучениеФайлов(Форма, ОбъектXDTO, 
	...
	Если ОбъектXDTO.Свойства().Получить("relations") = Неопределено
		...
		Если ЭлементДляОбновленияЗаголовка <> Неопределено Тогда
			ЭлементДляОбновленияЗаголовка.Заголовок = НСтр("es='Los enlaces';ru='Связи';en='Links'");
		КонецЕсли;
	...
	Для Каждого Связь Из ОбъектXDTO.relations Цикл
		...
		Если ЗначениеЗаполнено(Связь.relatedDocument.regNumber)
			...
			СтрокаДокумент.Заголовок = СтроковыеФункцииКлиентСервер.
				ПодставитьПараметрыВСтроку(НСтр("es='%1 (№ %2 de %3)';ru='%1 (№ %2 от %3)';en='%1 (№.%2 of%3)'"),
	...
	Если ЭлементДляОбновленияЗаголовка <> Неопределено Тогда
		Если ОбъектXDTO.relations.Количество() = 0 Тогда 
			ТекстЗаголовка = НСтр("es='Los enlaces';ru='Связи';en='Links'");
		Иначе
			ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Los enlaces (%1)';ru='Связи (%1)';en='Links (%1)'"),
	...
	Для Каждого ПодходящийОбъект Из ПодходящиеОбъекты Цикл
		...
		Исключение
			ТекстСообщения = СтрШаблон(
				НСтр("es='No consiguió recibir el objeto vinculado de la Circulación de documentos a la creación automática de los enlaces: %1';ru='Не удалось получить связанный объект Документооборота при автоматическом создании связей: %1';en='Failed to get the associated Document management object while automatically creating links: %1'"),
		...
		Иначе // предопределенный
			ТипСвязи = Новый Структура("ID, Тип, Заголовок",
				"",
				"DMRelationType",
				НСтр("es='Contiene la referencia en';ru='Содержит ссылку на';en='Contains a reference to'"));
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВидДокументаID) И ЗначениеЗаполнено(ВидДокументаТип) Тогда 
		...
		Исключение
			ВызватьИсключение СтрШаблон(
				НСтр("es='El tipo del documento ""%1"" no es encontrado en 1С:Документообороте';ru='Вид документа ""%1"" не найден в 1С:Документообороте';en='The type of document ""% 1"" was not found in 1C: Document Management'"),
				Форма.ВидДокумента);
		КонецПопытки;
...
Процедура ОбработатьФормуПриНедоступностиФункционалаВерсииСервиса(Форма) Экспорт
	...
	Форма.Элементы.ДекорацияФункционалНеПоддерживается.Заголовок = 
		НСтр("es='Функционал no es apoyado en la versión dada 1С:Документооборота.';ru='Функционал не поддерживается в данной версии 1С:Документооборота.';en='Functionality is not supported in this version of 1C: Workflow.'");
...
Процедура ЗаполнитьСтандартнуюШапкуБизнесПроцесса(Форма, ОбъектXDTO) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Форма.ID) Тогда
		Форма.Заголовок = СтрШаблон(НСтр("es='%1 (Creación)';ru='%1 (Создание)';en='% 1 (Create)'"), Форма.Заголовок);
...
Процедура ВыполнитьПоискПоРеквизитам(ТипОбъекта, РеквизитыПоиска, АдресВоВременномХранилище, 
	...
	Если Представление = "" Тогда
		Представление = НСтр("es='Las condiciones no son dadas.';ru='Условия не заданы.';en='No conditions specified.'");
	Иначе
		Представление = Представление + ".";
	КонецЕсли;
	...
	Если ?(Результат.Установлено("tooManyObjects"), Результат.tooManyObjects, Ложь) Тогда
		Представление = Представление
			+ " " + НСтр("es='Arrastren aquí cualquier requisito para precisar las condiciones.';ru='Перетащите сюда любой реквизит, чтобы уточнить условия.';en='Drag here any props to clarify the conditions.'");
		;
	Иначе
...
Функция ПодготовитьЛистСогласования(ПредметыСогласования)
	...
	Если Не ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.4.8.1") Тогда
		ВызватьИсключение НСтр("es='Para la conclusión de la hoja de la coordinación es necesario 1С:Документооборот a la versión no más abajo 1.4.8.1';ru='Для вывода листа согласования нужен 1С:Документооборот версии не ниже 1.4.8.1';en='To display the reconciliation sheet, 1C is needed: Document management version no lower than 1.4.8.1'");
	КонецЕсли;
