	...
	Если Элементы.Файлы.ВыделенныеСтроки.Количество() = 1 Тогда
		Если ТекущиеДанные.ПометкаУдаления Тогда
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='¿Quitar con ""%1"" marca a la desaparición?';ru='Снять с ""%1"" пометку на удаление?';en='Unmark deletion from ""%1""?'"), ТекущиеДанные.Наименование);
		Иначе
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='¿Marcar ""%1"" a la desaparición?';ru='Пометить ""%1"" на удаление?';en='Mark ""%1"" for deletion?'"), ТекущиеДанные.Наименование);
		КонецЕсли;
	Иначе
		Если ТекущиеДанные.ПометкаУдаления Тогда
			ТекстВопроса = НСтр("es='¿Quitar de los ficheros distinguidos la marca a la desaparición?';ru='Снять с выделенных файлов пометку на удаление?';en='Unmark deletion files?'");
		Иначе
...
Процедура Печать(Команда)
	...
	#Если ВебКлиент Тогда 
		ПоказатьПредупреждение(, НСтр("es='En el cliente Web la prensa de los ficheros no es apoyada.';ru='В Веб-клиенте печать файлов не поддерживается.';en='Web printing does not support file printing.'"));
		Возврат;
	#КонецЕсли
...
Процедура ДобавитьПечатнуюФормуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ФормаДобавления = Неопределено Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='%1 no tiene las formas de imprenta.';ru='%1 не имеет печатных форм.';en='%1 has no printing forms.'"), Строка(Владелец));
...
Процедура ОбновитьПредпросмотр(ТекущаяСтрока)
	...
	Иначе
		АдресДанныхФайла = "";
		Элементы.КартинкаПредпросмотра.ТекстНевыбраннойКартинки = НСтр("es='La prepresentación de los ficheros de este tipo no es apoyada.';ru='Предпросмотр файлов этого типа не поддерживается.';en='Previewing files of this type is not supported.'");
	КонецЕсли;
...
Функция ПараметрыИсходящегоПисьма(Знач Владелец, Знач Вложения, Знач УникальныйИдентификатор)
	...
	Тема = СтрШаблон(НСтр("es='%1, %2 al documento %3 %4 de %5';ru='%1, %2 к документу %3 %4 от %5';en='%1,%2 to document %3%4 from%5'"),
		?(ОдноВложение, НСтр("es='El fichero';ru='Файл';en='File'"), НСтр("es='Los ficheros';ru='Файлы';en='Files'")),
		?(ОдноВложение, НСтр("es='Juntado';ru='присоединенный';en='attached'"), НСтр("es='Juntado';ru='присоединенные';en='attached'")),
		ТипЗнч(Владелец),
		НомерДокумента,
		ДатаДокумента);
	...
	// Текст.
	Текст = СтрШаблон(НСтр("es='A la carta %1 %2:';ru='К письму %1 %2:';en='To the letter %1 %2:'"),
		?(ОдноВложение, НСтр("es='Es juntado';ru='присоединен';en='attached'"), НСтр("es='Son juntados';ru='присоединены';en='attached'")),
		?(ОдноВложение, НСтр("es='El fichero';ru='файл';en='file'"), НСтр("es='Los ficheros';ru='файлы';en='files'")));
	...
	Для Каждого Вложение Из Вложения Цикл
		...
		Текст = Текст + Символы.ПС + СтрШаблон(НСтр("es='- %1';ru='- %1';en='-%1'"), Вложение.Представление);
	КонецЦикла;
