...
Процедура НачалоДняПриИзменении(Элемент)
	...
	Если НачалоДня > ОкончаниеДня И ЗначениеЗаполнено(ОкончаниеДня) Тогда 
		ПоказатьПредупреждение(, НСтр("es='¡El tiempo indicado del comienzo del día es más grande que el tiempo de la terminación!';ru='Указанное время начала дня больше времени окончания!';en='The indicated start time is longer than the end time!'"));
	КонецЕсли;
...
Процедура ОкончаниеДняПриИзменении(Элемент)
	...
	Если ОкончаниеДня < НачалоДня И ЗначениеЗаполнено(ОкончаниеДня) Тогда 
		ПоказатьПредупреждение(, НСтр("es='El tiempo indicado de la terminación del día es más pequeño que el tiempo del comienzo';ru='Указанное время окончания дня меньше времени начала';en='The indicated end time is less than the start time'"));	
	КонецЕсли;
	...
	Если (СпособУказанияВремениID = ЗначениеПеречисленияВремяНачала) И (Работы.Количество() > 0) И ЗначениеЗаполнено(ОкончаниеДня) Тогда 
		Если ОкончаниеДня < Работы[Работы.Количество()-1].Начало Тогда
			ПоказатьПредупреждение(, НСтр("es='¡El tiempo indicado de la terminación del día es más pequeño que el tiempo del comienzo de un de los trabajos!';ru='Указанное время окончания дня меньше времени начала одной из работ!';en='The indicated end time is less than the start time of one of the jobs!'"));
...
Процедура ЗаполнитьСписокТиповИсточника(СписокТипов) 
	...
	ТипыИсточника.Вставить("DMInternalDocument",НСтр("es='El documento interior';ru='Внутренний документ';en='Internal document'"));
	ТипыИсточника.Вставить("DMIncomingEMail", НСтр("es='La carta entrante';ru='Входящее письмо';en='Incoming mail'"));
	ТипыИсточника.Вставить("DMIncomingDocument", НСтр("es='El documento recibido';ru='Входящий документ';en='Incoming document'"));
	ТипыИсточника.Вставить("DMDailyReport", НСтр("es='El informe diario';ru='Ежедневный отчет';en='Daily report'"));
	ТипыИсточника.Вставить("DMBusinessProcessTask", НСтр("es='La tarea';ru='Задача';en='Task'"));
	ТипыИсточника.Вставить("DMOutgoingEMail", НСтр("es='La carta saliente';ru='Исходящее письмо';en='Outgoing mail'"));
	ТипыИсточника.Вставить("DMOutgoingDocument", НСтр("es='El documento saliente';ru='Исходящий документ';en='Outgoing document'"));
	ТипыИсточника.Вставить("DMActivity", НСтр("es='La medida';ru='Мероприятие';en='Event'"));
	ТипыИсточника.Вставить("DMProject", НСтр("es='El proyecto';ru='Проект';en='Project'"));
	ТипыИсточника.Вставить("DMProjectTask", НСтр("es='La tarea de proyectos';ru='Проектная задача';en='Design task'"));
	ТипыИсточника.Вставить("DMFile", НСтр("es='El fichero';ru='Файл';en='File'"));
...
Процедура СформироватьЗаголовокФормы()
    ...
    Если Не ЗначениеЗаполнено(ID) Тогда
		Заголовок = НСтр("es='El informe (creación) diario';ru='Ежедневный отчет (создание)';en='Daily report (creation)'");
	Иначе
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El informe diario por %1';ru='Ежедневный отчет за %1';en='%1 daily report'"), Формат(Дата, "ДЛФ=D"));
	КонецЕсли;
...
Процедура ОбновитьПодвал()
	...
	ДлительностьРабот = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='En total en un día: %1';ru='Всего за день: %1';en='Total per day:%1'"), ИнтеграцияС1СДокументооборотКлиентСервер.ЧислоВСтроку(ДлительностьРаботСек, "0"));
	...
	ИначеЕсли ДлительностьРаботСек > ПродолжительностьДняСек Тогда 
		СообщениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='(Es superado en %1)';ru='(превышено на %1)';en='(exceeded by%1)'"),
			ИнтеграцияС1СДокументооборотКлиентСервер.ЧислоВСтроку(ДлительностьРаботСек - ПродолжительностьДняСек));
		
	ИначеЕсли ДлительностьРаботСек < ПродолжительностьДняСек Тогда 
		СообщениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='(Se quedó %1)';ru='(осталось %1)';en='(%1 left)'"),
	...
	Для Каждого Строка Из Работы Цикл
		...
		Если Не ЗначениеЗаполнено(Строка.Работа) Тогда 
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='del ""Trabajo"".';ru='Не заполнена колонка ""Содержание работ"" в строке %1 списка ""Работы"".';en='The column ""Work Content"" in row%1 of the ""Jobs"" list is not filled.'"),
				ИндексСтроки+1);
		...
		Если Не ЗначениеЗаполнено(Строка.ВидРабот) Тогда 
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='del ""Trabajo"".';ru='Не заполнена колонка ""Вид работ"" в строке %1 списка ""Работы"".';en='The column ""Type of work"" in line%1 of the list of ""Work"" is not filled.'"),
				ИндексСтроки+1);
		...
		Если СпособУказанияВремениID = ЗначениеПеречисленияДлительность Тогда
			...
			Если Не ЗначениеЗаполнено(Длительность) Тогда 
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='del ""Trabajo"".';ru='Не заполнена колонка ""Время"" в строке %1 списка ""Работы"".';en='The ""Time"" column in row%1 of the ""Works"" list is not filled.'"),
					ИндексСтроки+1);
		...
		Иначе
			Если Не ЗначениеЗаполнено(Строка.Начало) Тогда 
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='la columna ""el Comienzo de los trabajos"" en la línea %1 listas del ""Trabajo"".';ru='Не заполнена колонка ""Начало работ"" в строке %1 списка ""Работы"".';en='The column ""Start of work"" in row%1 of the list of ""Jobs"" is not filled.'"),
					ИндексСтроки+1);
			...
			Если ДоступенФункционалХронометраж Тогда
				Если Не ЗначениеЗаполнено(Строка.Окончание) Тогда 
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='del ""Trabajo"".';ru='Не заполнена колонка ""Окончание работ"" в строке %1 списка ""Работы"".';en='The column “Completion of work” is not filled in line%1 of the list of “Jobs”.'"),
						ИндексСтроки+1);
				...
				Если Строка.Начало > Строка.Окончание Тогда 
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El tiempo del comienzo de los trabajos más que el tiempo de la terminación de los trabajos en la línea %1 listas del ""Trabajo"".';ru='Время начала работ больше, чем время окончания работ в строке %1 списка ""Работы"".';en='The start time of the work is longer than the end time of the work in line%1 of the ""Jobs"" list.'"),
						ИндексСтроки+1);
			...
			Для Инд1 = 0 По Работы.Количество()-2 Цикл
				...
				Для Инд2 = Инд1+1 По Работы.Количество()-1 Цикл
					...
					Если (Строка2.Начало >= Строка1.Начало И Строка2.Начало < Строка1.Окончание)
						...
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='Son encontrados los intervalos que se cruzan del tiempo en las líneas %1 y %2 listas del ""Trabajo"".';ru='Найдены пересекающиеся интервалы времени в строках %1 и %2 списка ""Работы"".';en='Crossing time intervals found in lines%1 and%2 of the ""Works"" list.'"),
							Инд1+1,
							Инд2+1);
