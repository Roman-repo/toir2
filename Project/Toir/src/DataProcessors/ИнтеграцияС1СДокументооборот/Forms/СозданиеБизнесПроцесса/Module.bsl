...
Процедура УстановитьЗаголовокПредметаПоТипуОбъекта()
	...
	Если ПредметТип = "DMInternalDocument" Тогда
		Элементы.Предмет.Заголовок = НСтр("es='El documento interior';ru='Внутренний документ';en='Internal document'");
		
	ИначеЕсли ПредметТип = "DMIncomingDocument" Тогда
		Элементы.Предмет.Заголовок = НСтр("es='El documento recibido';ru='Входящий документ';en='Incoming document'");
		
	ИначеЕсли ПредметТип = "DMOutgoingDocument" Тогда
		Элементы.Предмет.Заголовок = НСтр("es='El documento saliente';ru='Исходящий документ';en='Outgoing document'");
		
	ИначеЕсли ПредметТип = "DMFile" Тогда
		Элементы.Предмет.Заголовок = НСтр("es='El fichero';ru='Файл';en='File'");
		
	ИначеЕсли ПредметТип = "DMActivity" Тогда
		Элементы.Предмет.Заголовок = НСтр("es='La medida';ru='Мероприятие';en='Event'");
		
	ИначеЕсли ПредметТип = "DMIncomingEMail" Тогда
		Элементы.Предмет.Заголовок = НСтр("es='La carta entrante';ru='Входящее письмо';en='Incoming mail'");
		
	ИначеЕсли ПредметТип = "DMOutgoingEMail" Тогда
		Элементы.Предмет.Заголовок = НСтр("es='La carta saliente';ru='Исходящее письмо';en='Outgoing mail'");
		
	ИначеЕсли ПредметТип = "DMProject" Тогда
		Элементы.Предмет.Заголовок = НСтр("es='El proyecto';ru='Проект';en='Project'");
		
	ИначеЕсли ПредметТип = "DMProjectTask" Тогда
		Элементы.Предмет.Заголовок = НСтр("es='La tarea de proyectos';ru='Проектная задача';en='Design task'");
		
	ИначеЕсли ПредметТип = "DMDiscussionMessage" Тогда
		Элементы.Предмет.Заголовок = НСтр("es='El mensaje';ru='Сообщение';en='Message'");
		...
		Если ЗначениеЗаполнено(Данные.ШаблонID) Тогда 
			...
			Если ЗаполнитьСтартоватьПроцесс(Данные.ТипПроцесса, ПараметрыФормы) Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Стартован el proceso de negocios del modelo ""%1"".';ru='Стартован бизнес-процесс по шаблону ""%1"".';en='Started business process using template ""%1"".'"), Данные.Наименование);
				ТекстЗаголовка = НСтр("es='Стартован el proceso';ru='Стартован процесс';en='Process started'");
