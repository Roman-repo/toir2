...
Процедура ВыполнитьВыбор(НайтиСразу)
	...
	Если ВидСравнения = "Между" Тогда
		Если Значение2 < Значение Тогда
			ПоказатьПредупреждение(, НСтр("es='Comprueben los significados introducidos: la fecha izquierda después de derecha.';ru='Проверьте введенные значения: левая дата позже правой.';en='Check the entered values: the left date is later than the right.'"));
			Возврат;
		КонецЕсли;
