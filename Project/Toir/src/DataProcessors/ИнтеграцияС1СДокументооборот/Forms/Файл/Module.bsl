...
Процедура УдалитьПодпись(Команда)
	...
	ТекстВопроса = НСтр("es='¿Quitar las firmas distinguidas?';ru='Удалить выделенные подписи?';en='Delete selected captions?'");
	ИнтеграцияС1СДокументооборотКлиент.ПоказатьВопросДаНет(Оповещение, ТекстВопроса,
		НСтр("es='Quitar';ru='Удалить';en='Delete'"), НСтр("es='No quitar';ru='Не удалять';en='Do not delete'"));
...
Процедура ФайловыеОперацииЕще(Команда)
	...
	Если ДоступенЗахватИРедактирование Тогда
		Подменю.Добавить("ОткрытьКаталогФайла", НСтр("es='Abrir el catálogo del fichero';ru='Открыть каталог файла';en='Open file directory'"),, 
			БиблиотекаКартинок.Папка);
	КонецЕсли;
	Если (Не Редактируется Или РедактируетсяТекущимПользователем)
		И РазрешеноРедактирование Тогда
		Подменю.Добавить("ОбновитьИзФайлаНаДиске", НСтр("es='Renovar del fichero sobre el disco';ru='Обновить из файла на диске';en='Update from file on disk'"),, 
	...
	Если ДоступенЗахватИРедактирование И РедактируетсяТекущимПользователем Тогда
		Подменю.Добавить("ОтменитьРедактирование", НСтр("es='Anular la redacción';ru='Отменить редактирование';en='Cancel editing'"),,
			БиблиотекаКартинок.ОсвободитьФайл);
		Если РазрешеноРедактирование Тогда
			Подменю.Добавить("СохранитьИзменения", НСтр("es='Conservar los cambios';ru='Сохранить изменения';en='Save changes'"),,
...
Процедура ЗаполнитьФорму(ОписаниеФайла)
	...
	Иначе
		...
		Элементы.ДекорацияРедактируется.Заголовок = ?(Редактируется, НСтр("es='Es redactado';ru='Редактируется';en='Editable'"), "");
	КонецЕсли;
...
Процедура ПолучитьОписаниеФайлаИЗаполнитьФорму()
	...
	Иначе
		ВызватьИсключение НСтр("es='del fichero.';ru='Не указан идентификатор файла.';en='No file identifier specified.'");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ID) Тогда
		Если ЗначениеЗаполнено(ВладелецФайла) Тогда
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='%1 (el fichero Juntado)';ru='%1 (Присоединенный файл)';en='%1 (Attached file)'"), Наименование);
		Иначе
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='%1 (Fichero)';ru='%1 (Файл)';en='%1 (File)'"), Наименование);
		КонецЕсли;
	Иначе // создание
		Если ЗначениеЗаполнено(ВладелецФайла) Тогда
			Заголовок = НСтр("es='El fichero (creación) juntado';ru='Присоединенный файл (создание)';en='Attached file (creation)'");
		Иначе
			Заголовок = НСтр("es='El fichero (creación)';ru='Файл (создание)';en='File (creation)'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаписатьВыполнитьКлиент(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(Наименование) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(,, НСтр("en='Name';es='El nombre';ru='Наименование'")),, 
			"Наименование");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
