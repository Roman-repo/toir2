		...
		Если ДетальныеСведенияЗадачи.ВключенХронометраж Тогда
			...
			Если ДетальныеСведенияЗадачи.ВключенХронометраж Тогда
				Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Es incluido el cronometraje por ""%1""';ru='Включен хронометраж по ""%1""';en='Timing by ""%1"" is enabled'"), Строка(СтрокаЗадачи.Задача));
				Состояние(Текст);
			КонецЕсли;
...
Процедура ВыполнитьЗадачуПервая(Команда)
	...
	Если СтрокаЗадачи.ПроцессТип = "DMBusinessProcessInvitation" Тогда 
		...
		Если РезультатID = "ПринятоОбязательнымиУчастниками" Тогда 
			...
			ТекстВопроса = Нстр("es='Hay unas invitaciones que no han aceptado los participantes. "
"A la confirmación serán excluidos de la lista de los participantes. ¿Continuar?';ru='Есть не принявшие приглашения участники."
"При подтверждении они будут исключены из списка участников. Продолжить?';en='There are non-inviting members."
"Upon confirmation, they will be excluded from the list of participants. Continue?'");
...
Процедура ОбработатьФормуСогласноВерсииСервиса()
	...
	Заголовок = НСтр("es='La circulación de documentos: las tareas a mí';ru='Документооборот: задачи мне';en='Document management: tasks to me'");
	...
	Иначе
		...
		Если ВерсияСервиса = "0.0.0.0" Тогда
			...
			Элементы.ДекорацияФункционалНеПоддерживается.Заголовок = 
				НСтр("es='No hay acceso a 1С:Документообороту.';ru='Нет доступа к 1С:Документообороту.';en='No access to 1C: Document management.'");
...
Процедура ЗаполнитьПоляЧислаЗадач()
	...
	Заголовок =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La circulación de documentos: las tareas a mí (Nuevo: %1 / Caducado: %2 / En total %3)';ru='Документооборот: задачи мне (Новых: %1 / Просроченных: %2 / Всего: %3)';en='Document management: tasks to me (New:% 1 / Expired:% 2 / Total:% 3)'"),
...
Функция ПолучитьДетальныеСведенияНаСервере(РеквизитыЗадачи, НеобходимоОбновлениеФормы)
				...
				Исключение
					...
					ЗаписьЖурналаРегистрации(
						ИнтеграцияС1СДокументооборот.ИмяСобытияЖурналаРегистрации(
							НСтр("es='La falta a la recepción de la representación del objeto vinculado';ru='Ошибка при получении представления связанного объекта';en='Error getting view of related object'",
	...
	Если РеквизитыЗадачи.НомерИтерации <> 0 Тогда
		ИтерацияЗадачи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСТР("es='El ciclo: %1';ru='Цикл: %1';en='Cycle:%1'"),
	...
	Иначе
		ТекстHTML = ТекстHTML + "<p><FONT color=""#C0C0C0"">"
			+ НСтр("es='La tarea no tiene descripción.';ru='У задачи нет описания.';en='The task has no description.'") + "</FONT></p>";
	КонецЕсли;
...
Процедура ПереключитьХронометражЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	Если ДетальныеСведенияЗадачи.ВключенХронометраж Тогда
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Es incluido el cronometraje por ""%1""';ru='Включен хронометраж по ""%1""';en='Timing by ""% 1"" is enabled'"), Строка(ПараметрыОповещения.Задача));
		Состояние(Текст);
	КонецЕсли;
