...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ЗначениеЗаполнено(ТипСоздаваемогоОбъекта) Тогда
		...
		Если ТипСоздаваемогоОбъекта = "DMIncomingDocument" Тогда 
			Элементы.ДекорацияОбъектЗаголовок.Заголовок = НСтр("es='El documento recibido:';ru='Входящий документ:';en='Incoming document:'");
		ИначеЕсли ТипСоздаваемогоОбъекта = "DMOutgoingDocument" Тогда 
			Элементы.ДекорацияОбъектЗаголовок.Заголовок = НСтр("es='El documento saliente:';ru='Исходящий документ:';en='Outgoing document:'");
		ИначеЕсли ТипСоздаваемогоОбъекта = "DMInternalDocument" Тогда
			Элементы.ДекорацияОбъектЗаголовок.Заголовок = НСтр("es='El documento interior:';ru='Внутренний документ:';en='Internal document:'");
		ИначеЕсли ТипСоздаваемогоОбъекта = "DMCorrespondent" Тогда 
			Если ИнтеграцияС1СДокументооборотПовтИсп.ИспользоватьТерминКорреспонденты() Тогда
				Элементы.ДекорацияОбъектЗаголовок.Заголовок = НСтр("es='El corresponsal:';ru='Корреспондент:';en='Correspondent:'");
			Иначе
				Элементы.ДекорацияОбъектЗаголовок.Заголовок = НСтр("es='El contratante:';ru='Контрагент:';en='Counterparty:'");
			КонецЕсли;
		КонецЕсли;
		Элементы.ГруппаИнфо.Видимость = Ложь;
	...
	Иначе // нет подходящих правил
		...
		Элементы.ДекорацияОбъектЗаголовок.Заголовок = НСтр("es='La conformidad en 1С:Документообороте';ru='Соответствие в 1С:Документообороте';en='Compliance in 1C: Document management'");
		Элементы.ДекорацияИнфо.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='""%1""';ru='Не настроено заполнение документа 1С:Документооборота из ""%1""';en='Not configured to fill out document 1C: Workflow from ""%1""'"), ИмяПотребителя);
...
Процедура ДекорацияОбъектВыбратьНажатие(Элемент)
	...
	Иначе // тип неизвестен, предложим выбор пользователю
		...
		ПараметрыФормы.Вставить("Заголовок", НСтр("es='Escoger en 1С:Документообороте';ru='Выбрать в 1С:Документообороте';en='Select in 1C: Document Management'"));
		ПараметрыФормы.Вставить("ЗаголовокКоманды", НСтр("es='Escoger';ru='Выбрать';en='Choose'"));
...
"%1?'"),Строка(Параметры.ВнешнийОбъект));
	...
	ИнтеграцияС1СДокументооборотКлиент.ПоказатьВопросДаНет(Оповещение, ТекстВопроса, 
		НСтр("es='Limpiar';ru='Очистить';en='Clear'"), НСтр("es='No limpiar';ru='Не очищать';en='Do not clean'"), КодВозвратаДиалога.Нет);
...
Процедура СписокФайловПередУдалением(Элемент, Отказ)
	...
	Если Элементы.СписокФайлов.ВыделенныеСтроки.Количество() = 1 Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='¿Marcar ""%1"" a la desaparición?';ru='Пометить ""%1"" на удаление?';en='Mark ""%1"" for deletion?'"),
			Элементы.СписокФайлов.ТекущиеДанные.Наименование);
	Иначе
		ТекстВопроса = НСтр("es='¿Marcar los elementos distinguidos a la desaparición?';ru='Пометить выделенные элементы на удаление?';en='Mark selected items for deletion?'");
	КонецЕсли;
...
Процедура ОбновитьДекорацииДокумента()
	...
	Если ДокументТип = "DMCorrespondent" Тогда
		...
		Элементы.ДеревоПисемПисьмо.Заголовок = НСтр("es='El documento';ru='Документ';en='Document'");
	Иначе
		...
		Элементы.ДеревоПисемПисьмо.Заголовок = НСтр("es='La carta';ru='Письмо';en='Letter'");
	КонецЕсли;
		...
		Иначе
			...
			Иначе // нет подходящих правил
				Элементы.ДекорацияОбъектЗаголовок.Заголовок = 
					НСтр("es='La conformidad en 1С:Документообороте:';ru='Соответствие в 1С:Документообороте:';en='Compliance in 1C: Document management:'");
			КонецЕсли;
		КонецЕсли;
...
Процедура СоздатьФайлСДиска()
	...
	Если Не ЗначениеЗаполнено(ИдентификаторКорневойПапки1СДокументооборот) Тогда
		ПоказатьПредупреждение(, 
			НСтр("es='1С:Документооборота.';ru='Не указана корневая папка для хранения файлов 1С:Документооборота.';en='No root folder for storing 1C: Document management files.'"));
		Возврат;
	КонецЕсли;
...
Процедура СоздатьФайлСДискаПослеПолученияУниверсальногоВремениИзменения(ВремяИзмененияУниверсальное, 
	...
	ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Va la conservación del fichero ""%1"" (%2 Mb). "
"Por favor, esperen...';ru='Идет сохранение файла ""%1"" (%2 Мб)."
"Пожалуйста, подождите...';en='The file ""%1""(%2 Mb) is being saved. Please, wait…'"),
		ПараметрыОповещения.ПолноеИмяФайла,
		РаботаСФайламиСлужебныйКлиентСервер.ПолучитьСтрокуСРазмеромФайла(РазмерВМб));
...
Процедура ОбновитьДеревоСвязей(Знач ОбъектXDTO = Неопределено, ДобавленныйДокумент = Неопределено)
	...
	Если Не ЗначениеЗаполнено(ДокументID) Тогда
		...
		Элементы.ГруппаСвязанныеДокументы.Заголовок = НСтр("es='Los enlaces';ru='Связи';en='Links'");
		Возврат;
	КонецЕсли;
