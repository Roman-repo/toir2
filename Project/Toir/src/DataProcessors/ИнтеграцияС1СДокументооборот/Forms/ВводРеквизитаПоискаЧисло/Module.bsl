...
Процедура ВыполнитьВыбор(НайтиСразу)
	...
	Если ВидСравнения = "Между" Тогда
		Если Значение2 < Значение Тогда
			ПоказатьПредупреждение(, НСтр("es='Comprueben los significados introducidos: el número izquierdo es más pequeño que derecho.';ru='Проверьте введенные значения: левое число меньше правого.';en='Check the entered values: the left number is less than the right.'"));
			Возврат;
		КонецЕсли;
