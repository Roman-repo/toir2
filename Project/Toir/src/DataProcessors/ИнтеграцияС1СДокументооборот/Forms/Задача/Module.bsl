...
Процедура ВыполнитьЗадачуПервая(Команда)
	...
	ИначеЕсли Тип = "DMBusinessProcessIssuesSolutionTaskQuestion" Тогда
		Если ВидВопросаID = "Иное" И НЕ ЗначениеЗаполнено(РезультатВыполнения) Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(,, НСтр("es='El comentario';ru='Комментарий';en='Comment'")),,
				"РезультатВыполнения");
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ПроцессТип = "DMBusinessProcessInvitation" И РезультатID = "ПринятоОбязательнымиУчастниками" Тогда 
		...
		ТекстВопроса = Нстр("es='Hay unas invitaciones que no han aceptado los participantes. A la confirmación serán excluidos de la lista de los participantes. ¿Continuar?';ru='Есть не принявшие приглашения участники. При подтверждении они будут исключены из списка участников. Продолжить?';en='There are members who do not accept invitations. Upon confirmation, they will be excluded from the list of participants. Continue?'");
...
Процедура ВыполнитьЗадачуВторая(Команда)
	...
	ИначеЕсли ПроцессТип = "DMBusinessProcessConfirmation" И ЭтоТочкаМаршрута("Утвердить") Тогда
		Если Не ЗначениеЗаполнено(РезультатВыполнения) Тогда 
			ОчиститьСообщения();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("es='Indiquen la causa de la desviación del documento';ru='Укажите причину отклонения документа';en='Specify the reason for rejecting the document'"),, 
				"РезультатВыполнения");
			Возврат;
		КонецЕсли;
...
Процедура ПринятьКИсполнению(Команда)
	...
	Если ПринятаКИсполнению Тогда
		Состояние(НСтр("es='¡La tarea es aceptada a la realización!';ru='Задача принята к исполнению!';en='The task has been accepted!'"));
		...
		Если ВключенХронометраж Тогда
			...
			Если ВключенХронометраж Тогда
				Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Es incluido el cronometraje por ""%1""';ru='Включен хронометраж по ""%1""';en='Timing by ""%1"" is enabled'"), Строка(Представление));
				Состояние(Текст);
			КонецЕсли;
		Иначе 
...
Процедура ЗаполнитьФормуЗадачи(Параметры)
	...
	Если Элементы.Найти("ПолноеОписаниеЗадачи") <> Неопределено Тогда
		...
		Если ПроцессОбъект.Свойства().Получить("state") <> Неопределено
			...
			Если ПроцессОбъект.state.name = "Остановлен" Тогда
				...
				КонецЕсли;
				ПолноеОписаниеЗадачи = ПолноеОписаниеЗадачи + НСтр("es='La tarea es parada';ru='Задача остановлена';en='Task stopped'") + Символы.ПС;
			ИначеЕсли ПроцессОбъект.state.name = "Прерван" Тогда
				Если Не ПустаяСтрока(ПолноеОписаниеЗадачи) Тогда
					ПолноеОписаниеЗадачи = ПолноеОписаниеЗадачи + Символы.ПС;
				КонецЕсли;
				ПолноеОписаниеЗадачи = ПолноеОписаниеЗадачи + НСтр("es='La tarea es interrumpida';ru='Задача прервана';en='Task aborted'") + Символы.ПС;
			КонецЕсли;
		...
		ПолноеОписаниеЗадачи = ПолноеОписаниеЗадачи
			+ СтрШаблон(НСтр("es='La importancia: %1';ru='Важность: %1';en='Importance:% 1'"), ОбъектXDTO.importance.objectID.id)
			+ Символы.ПС;
		...
		ПолноеОписаниеЗадачи = ПолноеОписаниеЗадачи + СтрШаблон(
			НСтр("es='A quien: %1';ru='Кому: %1';en='To:%1'"),
			ПредставлениеИсполнителя);
		...
		Если ЗначениеЗаполнено(ОбъектXDTO.dueDate) Тогда
			...
			КонецЕсли;
			ПолноеОписаниеЗадачи = ПолноеОписаниеЗадачи
				+ СтрШаблон(НСтр("es='El plazo: %1';ru='Срок: %1';en='Period:%1'"), Формат(ОбъектXDTO.dueDate, "dd.MM.yyyy HH:mm"));
		КонецЕсли;
		...
		Если ОбъектXDTO.Установлено("author") Тогда
			...
			КонецЕсли;
			ПолноеОписаниеЗадачи = ПолноеОписаниеЗадачи
				+ СтрШаблон(НСтр("es='El autor: %1';ru='Автор: %1';en='Author:% 1'"), ОбъектXDTO.author.name);
		КонецЕсли;
		...
		Если ПроцессОбъект.Свойства().Получить("executionComment") <> Неопределено
			...
			КонецЕсли;
			ПолноеОписаниеЗадачи = ПолноеОписаниеЗадачи 
			+ Символы.ПС + НСтр("es='La historia de la ejecución:';ru='История выполнения:';en='Execution history:'")
	...
	Если Не РазрешеноИзменение Или Не ДоступнаМультипредметность Тогда // уберем гиперссылку "Добавить..."
		Элементы.ДекорацияПредметыИФайлы.Заголовок = НСтр("es='La tarea no tiene objetos.';ru='У задачи нет предметов.';en='The task has no items.'");
	КонецЕсли;
...
Процедура ЗаполнитьДанныеБПСогласование(ЗадачаОбъект)
	...
	Если ЭтоТочкаМаршрута("Согласовать") Тогда
		...
		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Es arreglado';ru='Согласовано';en='Agreed'");
		Элементы.ВыполнитьЗадачуВторая.Заголовок = НСтр("es='Es arreglado "
"con las observaciones';ru='Согласовано "
"с замечаниями';en='Agreed"
"with comments '");
		Элементы.ВыполнитьЗадачуВторая.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
		Элементы.ВыполнитьЗадачуТретья.Заголовок = НСтр("es='No es arreglado';ru='Не согласовано';en='Not agreed'");
	...
	ИначеЕсли ЭтоТочкаМаршрута("Ознакомиться") Тогда
		...
		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Se ha puesto al corriente';ru='Ознакомился';en='Got acquainted'");
		Элементы.ВыполнитьЗадачуВторая.Заголовок = НСтр("es='Repetir "
"la coordinación...';ru='Повторить"
"согласование...';en='Repeat"
"matching...'");
		...
		Элементы.ИсполнителиИсполнитель.Заголовок = НСтр("es='La persona soglasuyuschee';ru='Согласующее лицо';en='Matching person'");
		...
		Если РезультатID = "НеСогласовано" Тогда 
			Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Acabar "
"la coordinación';ru='Завершить "
"согласование';en='Complete"
"matching'");
		Иначе	
		...
		Если ОтправленоНаПовторноеСогласование <> Неопределено Тогда
			Если ОтправленоНаПовторноеСогласование Тогда
				Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("es='Es enviado a la coordinación repetida.';ru='Отправлено на повторное согласование.';en='Sent for re-approval.'");
				Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
			Иначе	
				Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("es='Se ha puesto al corriente.';ru='Ознакомился.';en='Got acquainted.'");
				Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаполнитьДанныеБПУтверждение(ЗадачаОбъект)
	...
	Если ЭтоТочкаМаршрута("Утвердить") Тогда
		...
		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Es afirmado';ru='Утверждено';en='Approved by'");
		Элементы.ВыполнитьЗадачуВторая.Заголовок = НСтр("es='No es afirmado';ru='Не утверждено';en='Not approved'");
		Элементы.ВыполнитьЗадачуВторая.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
		Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
	...
	ИначеЕсли ЭтоТочкаМаршрута("Ознакомиться") Тогда
		...
		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Se ha puesto al corriente';ru='Ознакомился';en='Got acquainted'");
		Элементы.ВыполнитьЗадачуВторая.Заголовок = НСтр("es='Repetir "
"la afirmación...';ru='Повторить "
"утверждение...';en='Repeat"
"statement...'");
		Элементы.ВыполнитьЗадачуВторая.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
		Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
		...
		Если РезультатID = "НеУтверждено" Тогда 
			Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Acabar "
"la afirmación';ru='Завершить"
"утверждение';en='Complete"
"statement'");
		Иначе	
			Элементы.ВыполнитьЗадачуВторая.Видимость = Ложь;
		КонецЕсли;
		...
		Если ОтправленоНаПовторноеУтверждение <> Неопределено Тогда
			Если ОтправленоНаПовторноеУтверждение Тогда
				Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("es='Es enviado a la afirmación repetida.';ru='Отправлено на повторное утверждение.';en='Sent for reapproval.'");
				Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
			Иначе	
				Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("es='Se ha puesto al corriente.';ru='Ознакомился.';en='Got acquainted.'");
				Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
...
Процедура ЗаполнитьДанныеБППоручение(Прокси, ЗадачаОбъект)
	...
	Если ЭтоТочкаМаршрута("Выполнить") Тогда
		...
		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Es cumplido';ru='Выполнено';en='Done'");
		Элементы.ВыполнитьЗадачуВторая.Видимость = Ложь;
		Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
		
		Если Выполнена Тогда
			Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("es='Es cumplido.';ru='Выполнено.';en='Done.'");
			Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
		КонецЕсли;
		
	ИначеЕсли ЭтоТочкаМаршрута("Проверить") Тогда
		
		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Acabar el encargo';ru='Завершить поручение';en='Complete assignment'");
		Элементы.ВыполнитьЗадачуВторая.Заголовок = НСтр("es='Devolver a la elaboración definitiva';ru='Вернуть на доработку';en='Return for revision'");
		Элементы.ВыполнитьЗадачуВторая.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
		Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
		...
		Если Выполнена Тогда
			Если ВозвращеноНаДоработку Тогда
				Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("es='Es devuelto a la elaboración definitiva.';ru='Возвращено на доработку.';en='Returned for revision.'");
				Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
			Иначе
				Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("es='Se ha puesto al corriente.';ru='Ознакомился.';en='Got acquainted.'");
				Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
	...
	ИначеЕсли ЭтоТочкаМаршрута("Контролировать") Тогда
		...
		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Quitar del control';ru='Снять с контроля';en='Take out of control'");
		Элементы.ВыполнитьЗадачуВторая.Видимость = Ложь;
		Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
		
		Если Выполнена Тогда
			Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("es='Es quitado del control.';ru='Снято с контроля.';en='Discontinued.'");
			Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
		КонецЕсли;
...
Процедура ЗаполнитьДанныеБПИсполнение(Прокси, ЗадачаОбъект)
	...
	Если ЭтоТочкаМаршрута("Исполнить") Тогда
		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Es cumplido';ru='Исполнено';en='Executed'");
		Элементы.ВыполнитьЗадачуВторая.Видимость = Ложь;
		Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
		Если Выполнена Тогда
			Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("es='Es cumplido.';ru='Исполнено.';en='Executed.'");
			Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
		КонецЕсли;
	...
	ИначеЕсли ЭтоТочкаМаршрута("ОтветственноеИсполнение") Тогда
		...
		Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
		Если Выполнена Тогда
			Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("es='Es cumplido y comprobado.';ru='Исполнено и проверено.';en='Executed and verified.'");
			Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
		КонецЕсли;
	...
	ИначеЕсли ЭтоТочкаМаршрута("Контролировать") Тогда
		...
		Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
		Если Выполнена Тогда
			Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("es='Es controlado.';ru='Проконтролировано.';en='Controlled by.'");
			Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
		КонецЕсли;
		
		Элементы.Исполнители.Видимость = Истина;
		Элементы.Исполнители.Заголовок = НСтр("es='El curso de la realización';ru='Ход исполнения';en='Execution progress'");
	...
	ИначеЕсли ЭтоТочкаМаршрута("Проверить") Тогда
		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Es comprobado';ru='Проверено';en='Checked.'");
		Элементы.ВыполнитьЗадачуВторая.Видимость = Ложь;
		Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
		Если Выполнена Тогда
			Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("es='Es comprobado.';ru='Проверено.';en='Checked.'");
			Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
		...
		Элементы.Исполнители.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
		Элементы.ИсполнителиКомментарий.Заголовок = НСтр("es='El resultado de la ejecución';ru='Результат выполнения';en='Execution result'");
		Элементы.ИсполнителиОтправленоНаДоработку.Видимость = Истина;
		Элементы.ИсполнителиОтправленоНаДоработку.Заголовок = НСтр("es='Devolver a la elaboración definitiva';ru='Вернуть на доработку';en='Return for revision'");
...
Процедура ЗаполнитьДанныеБПРегистрация(ЗадачаОбъект)
	...
	Если Не ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.3.1.3") Тогда
		...
		Если ЭтоТочкаМаршрута("Зарегистрировать") Тогда
			Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Es registrado';ru='Зарегистрировано';en='Registered'");
			Элементы.ВыполнитьЗадачуВторая.Видимость = Ложь;
			Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
			Если Выполнена Тогда
				Элементы.ТекстРезультатаВыполнения.Заголовок = НСТР("es='Es registrado y enviado.';ru='Зарегистрировано и отправлено.';en='Registered and sent.'");
				Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
			КонецЕсли;
		ИначеЕсли ЭтоТочкаМаршрута("Ознакомиться") Тогда
			Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Se ha puesto al corriente';ru='Ознакомился';en='Got acquainted'");
	...
	Иначе
		...
		Если ЭтоТочкаМаршрута("Зарегистрировать") Тогда
			...
			Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Es registrado';ru='Зарегистрировано';en='Registered'");
			Элементы.ВыполнитьЗадачуВторая.Заголовок = НСтр("es='No es registrado';ru='Не зарегистрировано';en='Not registered'");
			Элементы.ВыполнитьЗадачуВторая.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
			Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
		...
		ИначеЕсли ЭтоТочкаМаршрута("Ознакомиться") Тогда
			...
			Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Se ha puesto al corriente';ru='Ознакомился';en='Got acquainted'");
			Элементы.ВыполнитьЗадачуВторая.Заголовок = НСтр("es='Repetir el registro...';ru='Повторить регистрацию...';en='Repeat registration ...'");
			Элементы.ВыполнитьЗадачуВторая.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
			Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
			...
			Если РезультатID = "НеЗарегистрировано" Тогда 
				Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Acabar el registro';ru='Завершить регистрацию';en='Complete registration'");
			Иначе	
				Элементы.ВыполнитьЗадачуВторая.Видимость = Ложь;
			КонецЕсли;
			...
			Если ОтправленоНаПовторнуюРегистрацию <> Неопределено Тогда
				Если ОтправленоНаПовторнуюРегистрацию Тогда
					Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("es='Es enviado al registro repetido.';ru='Отправлено на повторную регистрацию.';en='Sent for re-registration.'");
					Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
				Иначе	
					Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("es='Se ha puesto al corriente.';ru='Ознакомился.';en='Got acquainted.'");
					Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
...
Процедура ЗаполнитьДанныеБПРассмотрение(ЗадачаОбъект)
	...
	Если ЭтоТочкаМаршрута("Рассмотреть") Тогда
		...
		Если Выполнена Тогда
			Элементы.ТекстРезультатаВыполненияРассмотрение.Заголовок = НСтр("es='Es examinado.';ru='Рассмотрено.';en='Reviewed.'");
	...
	ИначеЕсли ЭтоТочкаМаршрута("Ознакомиться") Тогда
		Элементы.ГруппаКомандыВыполнения.Видимость = ДоступноВыполнение;
		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Es tratado';ru='Обработано';en='Processed'");
		Элементы.ВыполнитьЗадачуВторая.Видимость = Ложь;
		Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
		Если Выполнена Тогда
			Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("es='Es tratado.';ru='Обработано.';en='Processed.'");
			Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
		...
		Элементы.СтраницаОсновныхЗадач.Заголовок = НСтр("es='Que tratar';ru='Что обработать';en='What to process'");
		Элементы.СтраницаРезультатОбработки.Видимость = Истина;
...
Процедура ЗаполнитьДанныеБПКомплексныйПроцесс(Прокси, ПроцессОбъект)
	...
	Если ЭтоТочкаМаршрута("Контролировать") Тогда
		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Es controlado';ru='Проконтролировано';en='Controlled by'");
		Элементы.ВыполнитьЗадачуВторая.Видимость = Ложь;
		Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
		Если Выполнена Тогда
			Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("es='Es quitado del control.';ru='Снято с контроля.';en='Discontinued.'");
...
Процедура ЗаполнитьДанныеБПРешениеВопросов(ЗадачаОбъект)
	...
	НомерИтерации = ЗадачаОбъект.iterationNumber;
	Элементы.Наименование.Заголовок = НСтр("en='Name';es='El nombre';ru='Наименование'");
	Элементы.ПолноеОписаниеЗадачиЗаголовок.Заголовок = 
	СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La historia de la pregunta (el ciclo: %1)';ru='История вопроса (цикл: %1)';en='Issue history (cycle:%1)'"),НомерИтерации);
	
	Если ЭтоТочкаМаршрута("РассмотрениеИнициатором") И ВидВопросаID = "Иное" Тогда
		Элементы.ГруппаКомандыВыполнения.Видимость = ДоступноВыполнение;
		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Es examinado';ru='Рассмотрено';en='Reviewed'");
		Элементы.ВыполнитьЗадачуВторая.Видимость = Ложь;
		Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
		Если Выполнена Тогда
			Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("es='Es examinado.';ru='Рассмотрено.';en='Reviewed.'");
			Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
		КонецЕсли;
	...
	ИначеЕсли ЭтоТочкаМаршрута("ОзнакомлениеСРезультатомРассмотрения") Тогда
		Элементы.ГруппаКомандыВыполнения.Видимость = ДоступноВыполнение;
		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Cerrar la pregunta';ru='Закрыть вопрос';en='Close the question'");
		Элементы.ВыполнитьЗадачуВторая.Заголовок = НСтр("es='Precisar';ru='Уточнить';en='Specify'");
		Элементы.ВыполнитьЗадачуВторая.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
		Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
		Если ЗадачаОбъект.Установлено("returned") Тогда
			ВозвращеноНаДоработку = ЗадачаОбъект.returned;
		...
		Если ВидВопросаID = "ПереносСрока" Тогда
			...
			Если ЗадачаОбъект.postponingAmount = 0 Тогда
				Элементы.ДекорацияСтатусРассмотрения.Заголовок = НСтр("es='El traslado del plazo no es arreglado.';ru='Перенос срока не согласован.';en='The postponement is not agreed.'");
				Элементы.ДекорацияСтатусРассмотрения.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
			Иначе
				Элементы.ДекорацияСтатусРассмотрения.Заголовок = НСтр("es='El autor de la tarea ha llevado el plazo.';ru='Автор задачи перенес срок.';en='The author of the task rescheduled.'");
			КонецЕсли;
		КонецЕсли;
		Если Выполнена Тогда
			Если ВозвращеноНаДоработку Тогда
				Элементы.ТекстРезультатаВыполнения.Заголовок = НСТР("es='Es enviado a la especificación.';ru='Отправлено на уточнение.';en='Sent for clarification.'");
				Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
			Иначе
				Элементы.ТекстРезультатаВыполнения.Заголовок = НСТР("es='Es cumplido.';ru='Выполнено.';en='Done.'");
				Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
...
Процедура ЗаполнитьДанныеБППриглашение(ЗадачаОбъект)
	...
	Если ЭтоТочкаМаршрута("Пригласить") Тогда
		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Aceptar "
"la invitación';ru='Принять "
"приглашение';en='To accept"
"invitation'");
		Элементы.ВыполнитьЗадачуВторая.Заголовок = НСтр("es='Declinar "
"la invitación';ru='Отклонить "
"приглашение';en='Reject"
"invitation'");
		Элементы.ВыполнитьЗадачуВторая.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
		Элементы.ВыполнитьЗадачуТретья.Видимость = Ложь;
	...
	ИначеЕсли ЭтоТочкаМаршрута("Ознакомиться") Тогда
		Элементы.ВыполнитьЗадачуПервая.Заголовок = НСтр("es='Confirmar "
"las invitaciones';ru='Подтвердить "
"приглашения';en='Confirm"
"invitations'");
		Элементы.ВыполнитьЗадачуВторая.Заголовок = НСтр("es='Repetir "
"la invitación...';ru='Повторить "
"приглашение...';en='Repeat"
"invitation...'");
		Элементы.ВыполнитьЗадачуТретья.Заголовок = НСтр("es='Anular "
"la invitación';ru='Отменить "
"приглашение';en='Cancel"
"invitation'");
		Если Выполнена Тогда
			Если ЗадачаОбъект.invitationReturned Тогда
				Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("es='Es enviado a la invitación repetida.';ru='Отправлено на повторное приглашение.';en='Sent to re-invitation.'");
			Иначе	
				Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("es='La invitación es acabada.';ru='Приглашение завершено.';en='Invitation completed.'");
			КонецЕсли;
		КонецЕсли;
		...
		Элементы.Исполнители.Видимость = Истина;
		Элементы.Исполнители.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
		Элементы.ИсполнителиИсполнитель.Заголовок = НСтр("es='El participante';ru='Участник';en='Member'");
		Элементы.ИсполнителиРезультат.Видимость = Истина;
	...
	ИначеЕсли ЭтоТочкаМаршрута("Оповестить") Тогда
		...
		Если РезультатID = "ПринятоВсемиУчастниками"
			...
			Результат =  НСтр("es='La invitación es confirmada con el lugar indicado y el tiempo';ru='Приглашение подтверждено с указанным местом и временем';en='The invitation is confirmed with the specified place and time.'");
			Элементы.РезультатПриглашения.ЦветТекста = ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи;
		Иначе
			Результат =  НСтр("es='La invitación es anulada con el lugar indicado y el tiempo';ru='Приглашение отменено с указанным местом и временем';en='The invitation has been canceled with the specified place and time.'");
			Элементы.РезультатПриглашения.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
		КонецЕсли;
...
Процедура ВывестиИнформациюОНедоступностиВыполненияЗадачи()
	...
	Элементы.ТекстРезультатаВыполнения.Заголовок = НСтр("es='La ejecución de las tareas de tal tipo no es apoyada en la versión dada 1С:Документооборота.';ru='Выполнение задач такого типа не поддерживается в данной версии 1С:Документооборота.';en='Performing tasks of this type is not supported in this version of 1C: Workflow.'");
	Элементы.ТекстРезультатаВыполнения.ЦветТекста = ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи;
...
Процедура ДополнитьИмяШаблонаПоТипуШаблона(СтрокаЭтапа)
	...
	Дополнение = "";
	Если СтрокаЭтапа.ШаблонБизнесПроцессаТип = "DMBusinessProcessOrderTemplate"	Тогда Дополнение = НСтр("es='El encargo';ru='Поручение';en='Commission'");
	ИначеЕсли СтрокаЭтапа.ШаблонБизнесПроцессаТип = "DMBusinessProcessConsiderationTemplate" Тогда Дополнение = НСтр("es='El examen';ru='Рассмотрение';en='Review'");
	ИначеЕсли СтрокаЭтапа.ШаблонБизнесПроцессаТип = "DMBusinessProcessRegistrationTemplate" Тогда Дополнение = НСтр("es='El registro';ru='Регистрация';en='registration'");
	ИначеЕсли СтрокаЭтапа.ШаблонБизнесПроцессаТип = "DMBusinessProcessApprovalTemplate" 	Тогда Дополнение = НСтр("es='La coordinación';ru='Согласование';en='Approval'");
	ИначеЕсли СтрокаЭтапа.ШаблонБизнесПроцессаТип = "DMBusinessProcessConfirmationTemplate" Тогда Дополнение = НСтр("es='La afirmación';ru='Утверждение';en='Approval  '");
	ИначеЕсли СтрокаЭтапа.ШаблонБизнесПроцессаТип = "DMBusinessProcessPerformanceTemplate" 	Тогда Дополнение = НСтр("es='La realización';ru='Исполнение';en='Execution'");
	ИначеЕсли СтрокаЭтапа.ШаблонБизнесПроцессаТип = "DMBusinessProcessAcquaintanceTemplate" Тогда Дополнение = НСтр("es='El conocimiento';ru='Ознакомление';en='Acquaintance'");
	ИначеЕсли СтрокаЭтапа.ШаблонБизнесПроцессаТип = "DMCompoundBusinessProcessTemplate" 	Тогда Дополнение = НСтр("es='El proceso componente';ru='Составной процесс';en='Compound process'");
	ИначеЕсли СтрокаЭтапа.ШаблонБизнесПроцессаТип = "DMComplexBusinessProcessTemplate" 		Тогда Дополнение = НСтр("es='El proceso complejo';ru='Комплексный процесс';en='Complex process'");
	КонецЕсли;
...
Процедура ДополнитьИмяПредметаПоТипуОбъекта(ИмяПредмета, ТипОбъекта)
	...
	Если ТипОбъекта = "DMInternalDocument" Тогда
		ИмяТипаПредмета = НСтр("es='El documento interior';ru='Внутренний документ';en='Internal document'");
	ИначеЕсли ТипОбъекта = "DMIncomingDocument" Тогда
		ИмяТипаПредмета = НСтр("es='El documento recibido';ru='Входящий документ';en='Incoming document'");
	ИначеЕсли ТипОбъекта = "DMOutgoingDocument" Тогда
		ИмяТипаПредмета = НСтр("es='El documento saliente';ru='Исходящий документ';en='Outgoing document'");
	ИначеЕсли ТипОбъекта = "DMFile" Тогда
		ИмяТипаПредмета = НСтр("es='El fichero';ru='Файл';en='File'");
	ИначеЕсли ТипОбъекта = "DMActivity" Тогда
		ИмяТипаПредмета = НСтр("es='La medida';ru='Мероприятие';en='Event'");
	ИначеЕсли ТипОбъекта = "DMIncomingEMail" Тогда
		ИмяТипаПредмета = НСтр("es='La carta entrante';ru='Входящее письмо';en='Incoming mail'");
...
Процедура Редактировать(Команда)
	...
	Если ТекущиеДанные.Редактируется И Не ТекущиеДанные.РедактируетсяТекущимПользователем Тогда
		ПоказатьПредупреждение(, НСтр("es='Este fichero es redactado ahora por otro usuario.';ru='Этот файл сейчас редактируется другим пользователем.';en='This file is currently being edited by another user.'"));
		Возврат;
	КонецЕсли;
