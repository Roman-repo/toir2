...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.2.8.1.CORP") Тогда
		ВызватьИсключение(НСтр("es='Функционал no es apoyado en la versión dada 1С:Документооборота.';ru='Функционал не поддерживается в данной версии 1С:Документооборота.';en='Functionality is not supported in this version of 1C: Workflow.'"));
	КонецЕсли;
...
Процедура АдресатыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Не ЗначениеЗаполнено(СтрокаДанных.ТипАдреса) Тогда
		Если КоличествоАдресатовДоДобавления = 1 Тогда
			СтрокаДанных.ТипАдреса = НСтр("es='A quien:';ru='Кому:';en='To:'");
		Иначе
			СтрокаДанных.ТипАдреса = НСтр("es='La copia:';ru='Копия:';en='Copy:'");
		КонецЕсли;
	КонецЕсли;
...
Процедура Отправить(Команда)
	...
	Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La carta ""%1"" es enviada.';ru='Письмо ""%1"" отправлено.';en=""Email '%1' sent."""), Тема));
...
Процедура ЗаполнитьФормуИзОбъектаXDTO(ПисьмоXDTO, НовоеПисьмо = Ложь)
	...
	Если ЗначениеЗаполнено(ПисьмоXDTO.objectId.id) Тогда
		ДобавитьАдресатов(ПисьмоXDTO.senderAddress, НСтр("es='De:';ru='От:';en='From:'"));
	Иначе
		Отправитель = ПисьмоXDTO.senderAddress;
	КонецЕсли;
	
	ДобавитьАдресатов(ПисьмоXDTO.recipients, НСтр("es='A quien:';ru='Кому:';en='To:'"));
	ДобавитьАдресатов(ПисьмоXDTO.courtesyCopyRecipients, НСтр("es='La copia:';ru='Копия:';en='Copy:'"));
	ДобавитьАдресатов(ПисьмоXDTO.blindCourtesyCopyRecipients, НСтр("es='СК:';ru='СК:';en='SK:'"));
	...
	Если ЗначениеЗаполнено(ПисьмоXDTO.subject) Тогда
		Заголовок = СтрШаблон(НСтр("es='%1 (la carta Saliente)';ru='%1 (Исходящее письмо)';en='%1 (Outgoing mail)'"), ПисьмоXDTO.subject);
	Иначе
...
Процедура ОтобразитьКоличествоФайловСервер()
	...
	Иначе
		Элементы.ДеревоПриложенийСсылка.Заголовок = НСтр("es='Los ficheros';ru='Файлы';en='Files'");
	КонецЕсли;
...
Процедура НачатьДобавлениеФайлаСДискаПослеПодключенияРасширения(Подключено, ПараметрыОповещения) Экспорт	
	...
	Иначе 
		...
		ПоказатьПредупреждение(, НСтр("es='Es necesario conectar la ampliación para el trabajo en los ficheros.';ru='Необходимо подключить расширение для работы файлами.';en='Required to connect the extension to work with files.'"));
...
Функция ПроверитьПисьмоПередОтправкой()
	...
	Если СписокАдресов.Количество() = 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("es='No es posible enviar la carta. Es necesario indicar por lo menos a un destinatario.';ru='Нельзя отправить письмо. Необходимо указать хотя бы одного получателя.';en='Can not send a letter. At least one recipient must be specified.'"),,
				"Адресаты");
	...
	Если ПустаяСтрока(Тема) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("es='No es posible enviar la carta. No es rellenado el tema.';ru='Нельзя отправить письмо. Не заполнена тема.';en='Can not send a letter. The topic is not filled.'"),,
			"Тема");
