...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИнтеграцияС1СДокументооборотПовтИсп.ИспользоватьТерминКорреспонденты() Тогда
		Элементы.ГруппаКонтрагент.Заголовок = НСтр("es='El corresponsal';ru='Корреспондент';en='Correspondent'");
		Элементы.Контрагент.Заголовок = НСтр("es='El corresponsal';ru='Корреспондент';en='Correspondent'");
	КонецЕсли;
...
Процедура СвязьОчиститьНажатие(Элемент)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='¿Queréis limpiar realmente la conformidad para "
"%1?';ru='Вы действительно хотите очистить соответствие для"
"%1?';en='Are you sure you want to clear the match for"
"%1?'"), Строка(ВнешнийОбъект));
	
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("es='Limpiar';ru='Очистить';en='Clear'"));
	Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("es='No limpiar';ru='Не очищать';en='Do not clean'"));
...
Процедура КонтактноеЛицоНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе	
		ТекстСообщения = НСтр("es='""el Contratante""';ru='Не заполнено поле ""Контрагент""';en='The Counterparty field is not filled'");
...
Процедура ФайлыПередУдалением(Элемент, Отказ)
	...
	Если Элементы.Файлы.ВыделенныеСтроки.Количество() = 1 Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='¿Marcar ""%1"" a la desaparición?';ru='Пометить ""%1"" на удаление?';en='Mark ""%1"" for deletion?'"), Элементы.Файлы.ТекущиеДанные.Наименование);
	Иначе
		ТекстВопроса = НСтр("es='¿Marcar los ficheros distinguidos a la desaparición?';ru='Пометить выделенные файлы на удаление?';en='Mark selected files for deletion?'");
	КонецЕсли;
...
Процедура Зарегистрировать(Команда)
	...
	Если ОграничиватьДоступностьПолей Тогда 
		...
		ТекстВопроса = НСтр("es='Después del registro el documento será inaccesible para el cambio. "
"¿Continuar?';ru='После регистрации документ станет недоступным для изменения."
"Продолжить?';en='After registration, the document will become unavailable for change."
"Continue?'");
...
"¿%1 "
"Desconectar el cronometraje y fijar трудозатраты "
"ante la inclusión del nuevo cronometraje?';ru='Включен хронометраж по другим объектам:"
"%1"
"Отключить хронометраж и зафиксировать трудозатраты"
"перед включением нового хронометража?';en='Timekeeping for other objects is included:"
"%1"
"Disable timing and record labor"
"before starting a new timing?'"),
						СтрокаОбъектовХронометража);
...
Процедура ДобавитьПечатнуюФорму(Команда)
	...
	Если Не ЗначениеЗаполнено(ВнешнийОбъект) Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Ante la adición de la forma de imprenta es necesario vincular "
"el documento interior al objeto %1';ru='Перед добавлением печатной формы нужно связать"
"внутренний документ с объектом %1';en='Before adding a printing plate, you need to bind"
"internal document with object %1 '"), СокращенноеНаименованиеКонфигурации);
	...
	Если ФормаДобавления = Неопределено Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='%1 no tiene las formas de imprenta.';ru='%1 не имеет печатных форм.';en='% 1 has no printing forms.'"), Строка(ВнешнийОбъект));
		ПоказатьПредупреждение(, ТекстПредупреждения);
	КонецЕсли;
...
Процедура ЗаполнитьФайлыКопированием(Команда)
	...
	Режим.Добавить(КодВозвратаДиалога.Да, НСтр("es='Rellenar';ru='Заполнить';en='Fill in'"));
	Режим.Добавить(КодВозвратаДиалога.Нет, НСтр("es='No rellenar';ru='Не заполнять';en='Do not fill'"));
	
	Оповещение = Новый ОписаниеОповещения("ЗаполнитьФайлыКопированиемЗавершение", ЭтаФорма);
	ТекстВопроса =  НСтр("es='¿Rellenar los ficheros por la copia de los ficheros juntados?';ru='Заполнить файлы копированием из присоединенных файлов?';en='Fill files with copy from attached files?'");
...
Процедура ЗаполнитьФайлДаннымиДокумента(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Текст = НСтр("es='Campos en el fichero son renovados por los datos del propietario.';ru='Поля в файле обновлены данными владельца.';en='Fields in the file are updated with owner data.'");
...
Процедура УдалитьПодпись(Команда)
	...
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("es='Quitar';ru='Удалить';en='Delete'"));
	Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("es='No quitar';ru='Не удалять';en='Do not delete'"));
	
	Оповещение = Новый ОписаниеОповещения("УдалитьПодписьЗавершение", ЭтаФорма);
	ПоказатьВопрос(Оповещение, НСтр("es='¿Quitar las firmas distinguidas?';ru='Удалить выделенные подписи?';en='Delete selected captions?'"), Кнопки);
...
Процедура ЗаписатьИВозможноЗакрыть(ЗакрытьПослеЗаписи, ОповещениеПослеЗаписи = Неопределено)
	...
	Если ОграничиватьДоступностьПолей 
		...
		ТекстВопроса = НСтр("es='A la anotación el documento será registrado y será inaccesible para el cambio. "
"¿Continuar?';ru='При записи документ будет зарегистрирован и станет недоступным для изменения."
"Продолжить?';en='When you record, the document will be registered and will be unavailable for change."
"Continue?'");
		ИнтеграцияС1СДокументооборотКлиент.ПоказатьВопросДаНет(
			Оповещение, ТекстВопроса,,, КодВозвратаДиалога.Да);
	...
	Если Не ДокументЗаполнен Тогда
		...
		Если ПравилаЗаполнения.Количество() = 1 Тогда
			ТекстЗаголовкаСоздать = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Crear %1';ru='создать %1';en='create%1'"), НРег(ПравилаЗаполнения[0].Представление));
		ИначеЕсли ПравилаЗаполнения.Количество() = 0 Тогда
			Элементы.ГруппаСвязь.Видимость = Ложь;
			Возврат;
		Иначе
			ТекстЗаголовкаСоздать = НСтр("es='Crear...';ru='создать...';en='create ...'");
		КонецЕсли;
		
		Элементы.СвязьСоздать.Заголовок = ТекстЗаголовкаСоздать;
		
		Элементы.СтраницаСвязи.Заголовок = НСтр("es='Los enlaces';ru='Связи';en='Links'");
...
Процедура ЗаполнитьФайлыКопированиемЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	Состояние(НСтр("es='Va el relleno de los ficheros por la copia de los ficheros juntados...';ru='Идет заполнение файлов копированием из присоединенных файлов...';en='Files are being copied by copying from attached files ...'"));
