...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИнтеграцияС1СДокументооборотПовтИсп.ИспользоватьТерминКорреспонденты() Тогда
		Элементы.ОткрытьКонтрагенты.Заголовок = НСтр("es='Los corresponsales';ru='Корреспонденты';en='Correspondents'");
	КонецЕсли;
...
Процедура ЗаполнитьАдреснуюКнигу(КомандаЗаполнения, ИндексЭлемента = Неопределено)
	...
	Если ТипАдресатов = "СотрудникиПоПодразделениям" Тогда
		Элементы.ЗаголовокАдресатов.Заголовок = НСтр("es='Los empleados por las subdivisiones:';ru='Сотрудники по подразделениям:';en='Department employees:'");
	...
	ИначеЕсли ТипАдресатов = "СотрудникиСписком" Тогда
		Элементы.ЗаголовокАдресатов.Заголовок = НСтр("es='Los empleados por la lista:';ru='Сотрудники списком:';en='Employees list:'");
	...
	ИначеЕсли ТипАдресатов = "СотрудникиПоГруппам" Тогда
		Элементы.ЗаголовокАдресатов.Заголовок = НСтр("es='Los empleados por los grupos:';ru='Сотрудники по группам:';en='Group employees:'");
		Запрос.addressBookType = "usersInFolders";
		Элементы.Адресаты.Отображение = ОтображениеТаблицы.Дерево;
		Элементы.Адресаты.НачальноеОтображениеДерева = НачальноеОтображениеДерева.НеРаскрывать;
	ИначеЕсли ТипАдресатов = "ЛичныеАдресаты" Тогда
		Элементы.ЗаголовокАдресатов.Заголовок = НСтр("es='Los destinatarios personales:';ru='Личные адресаты:';en='Personal recipients:'");
		Запрос.addressBookType = "personalRecipients";
		Элементы.Адресаты.Отображение = ОтображениеТаблицы.Дерево;
		Элементы.Адресаты.НачальноеОтображениеДерева = НачальноеОтображениеДерева.НеРаскрывать;
	ИначеЕсли ТипАдресатов = "Контрагенты" Тогда
		Если ИнтеграцияС1СДокументооборотПовтИсп.ИспользоватьТерминКорреспонденты() Тогда
			Элементы.ЗаголовокАдресатов.Заголовок = НСтр("es='Los corresponsales:';ru='Корреспонденты:';en='Correspondents:'");
		Иначе
			Элементы.ЗаголовокАдресатов.Заголовок = НСтр("es='Los contratantes:';ru='Контрагенты:';en='Counterparties:'");
		КонецЕсли;
