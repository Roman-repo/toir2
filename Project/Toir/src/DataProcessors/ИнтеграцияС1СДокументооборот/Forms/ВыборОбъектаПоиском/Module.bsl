...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипОбъектаВыбора = "DMIncomingDocument" Тогда
		Заголовок = НСтр("es='Las condiciones de la búsqueda del documento recibido';ru='Условия поиска входящего документа';en='Incoming document search terms'");
	ИначеЕсли ТипОбъектаВыбора = "DMInternalDocument" Тогда 
		Заголовок = НСтр("es='Las condiciones de la búsqueda del documento interior';ru='Условия поиска внутреннего документа';en='Internal document search terms'");
	ИначеЕсли ТипОбъектаВыбора = "DMOutgoingDocument" Тогда
		Заголовок = НСтр("es='Las condiciones de la búsqueda del documento saliente';ru='Условия поиска исходящего документа';en='Outbound search terms'");
	ИначеЕсли ТипОбъектаВыбора = "DMCorrespondent" Тогда
		Если ИнтеграцияС1СДокументооборотПовтИсп.ИспользоватьТерминКорреспонденты() Тогда
			Заголовок = НСтр("es='Las condiciones de la búsqueda del corresponsal';ru='Условия поиска корреспондента';en='Correspondent search terms'");
		Иначе
			Заголовок = НСтр("es='Las condiciones de la búsqueda del contratante';ru='Условия поиска контрагента';en='Counterparty search terms'");
		КонецЕсли;
	КонецЕсли;
...
Процедура Искать(Команда)
	...
	ИначеЕсли ПредельноеКоличествоРезультатов <> 0 Тогда
		...
		Кнопки.Добавить(Истина, СтрШаблон(
			НСтр("es='Sí, mostrar primero %1';ru='Да, показать первые %1';en='Yes, show the first%1'"),
				Формат(ПредельноеКоличествоРезультатов, "ЧГ=0")));
		Кнопки.Добавить(Ложь, НСтр("es='No, precisar las condiciones';ru='Нет, уточнить условия';en='No, specify the conditions'"));
		ОписаниеОповещения = Новый ОписаниеОповещения("ВыполнитьПоискЗавершение", ЭтаФорма);
		ПоказатьВопрос(ОписаниеОповещения,
...
Функция СтруктураОтбора()
	...
	Если ЗначениеЗаполнено(СокрЛП(Наименование)) Тогда
		...
		Значение.Вставить("Представление", НСтр("en='Name';es='El nombre';ru='Наименование'"));
		...
		Значение.Вставить("ПредставлениеУсловия", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Contiene ""%1""';ru='содержит ""%1""';en='contains ""%1""'"), СокрЛП(Наименование)));
		Отбор.Вставить("name", Значение);
	КонецЕсли;
	Если ЗначениеЗаполнено(ВидДокументаID) Тогда
		Значение = Новый Структура;
		Значение.Вставить("Представление", НСтр("es='El tipo del documento';ru='Вид документа';en='Document type'"));
	...
	Если ЗначениеЗаполнено(Сумма) Тогда
		Значение = Новый Структура;
		Значение.Вставить("Представление", НСтр("es='La suma';ru='Сумма';en='Amount'"));
	...
	Если ЗначениеЗаполнено(СокрЛП(РегистрационныйНомер)) Тогда
		Значение = Новый Структура;
		Значение.Вставить("Представление", НСтр("es='Рег. №';ru='Рег. №';en='Reg. №'"));
	...
	Если ЗначениеЗаполнено(ЛюбаяДата) Тогда
		Значение = Новый Структура;
		Значение.Вставить("Представление", НСтр("es='A partir de';ru='Начиная с';en='Beginning with'"));
	...
	Если ЗначениеЗаполнено(ОрганизацияID) Тогда
		Значение = Новый Структура;
		Значение.Вставить("Представление", НСтр("es='La organización';ru='Организация';en='Organization'"));
	...
	Если ЗначениеЗаполнено(КонтрагентID) Тогда
		Значение = Новый Структура;
		Если ИнтеграцияС1СДокументооборотПовтИсп.ИспользоватьТерминКорреспонденты() Тогда
			Значение.Вставить("Представление", НСтр("es='El corresponsal';ru='Корреспондент';en='Correspondent'"));
		Иначе
			Значение.Вставить("Представление", НСтр("es='El contratante';ru='Контрагент';en='Counterparty'"));
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПапкаID) Тогда
		Значение = Новый Структура;
		Значение.Вставить("Представление", НСтр("es='La carpeta';ru='Папка';en='Folder'"));
	...
	Если ЗначениеЗаполнено(СокрЛП(ИНН)) Тогда
		Значение = Новый Структура;
		Значение.Вставить("Представление", НСтр("es='EL INN';ru='ИНН';en='TIN'"));
	...
	Если ЗначениеЗаполнено(СокрЛП(КПП)) Тогда
		Значение = Новый Структура;
		Значение.Вставить("Представление", НСтр("es='КПП';ru='КПП';en='PPC:'"));
