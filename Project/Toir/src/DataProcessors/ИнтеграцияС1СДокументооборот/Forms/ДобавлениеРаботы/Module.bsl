...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(ОписаниеРаботы) Тогда
		...
		Если ЗначениеЗаполнено(ВнешнийОбъект) Тогда
			...
			ОписаниеРаботы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El trabajo sobre ""%1""';ru='Работа над ""%1""';en=""Work on '%1'"""), СтрокаПодстановки);
		Иначе
			ОписаниеРаботы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='El trabajo sobre ""%1""';ru='Работа над ""%1""';en=""Work on '%1'"""), Источник);
		КонецЕсли;
	...
	Если Не ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.3.2.3.CORP") Тогда
		...
		Элементы.ДатаДобавления.Заголовок = НСтр("es='Añadir en el informe por';ru='Добавить в отчет за';en='Add to report for'");
		Элементы.Начало.Заголовок = НСтр("es='El tiempo del comienzo del trabajo';ru='Время начала работы';en='Start time'");
	Иначе
	...
	Если СпособУказанияВремениID = ЗначениеПеречисленияДлительность Тогда 
		Если Не ЗначениеЗаполнено(ДлительностьРаботы) Или ДлительностьРаботы = "00:00" Тогда 
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(,, НСтр("es='La duración';ru='Длительность';en='Duration'")),,
				"ДлительностьРаботы",, 
				Отказ);
		КонецЕсли;
	Иначе
		Если Не ЗначениеЗаполнено(Начало) Тогда 
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(,, НСтр("es='El comienzo';ru='Начало';en='Start'")),,
				"Начало",, 
				Отказ);
		КонецЕсли;
		...
		Если ДоступенФункционалХронометраж Тогда
			Если Не ЗначениеЗаполнено(Окончание) Тогда 
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(,, НСтр("es='La terminación';ru='Окончание';en='The ending'")),,
					"Окончание",, 
					Отказ);
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(Начало) И ЗначениеЗаполнено(Окончание) И Начало > Окончание Тогда 
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("es='El tiempo de la terminación menos que el tiempo del comienzo';ru='Время окончания меньше, чем время начала';en='End time is less than start time'"),,
					"Окончание",, 
					Отказ);
			КонецЕсли;
		КонецЕсли;
