...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИнтеграцияС1СДокументооборотПовтИсп.ИспользоватьТерминКорреспонденты() Тогда
		Элементы.ДекорацияЮрФизЛицо.Заголовок = НСтр("es='El tipo del corresponsal';ru='Вид корреспондента';en='Type of correspondent'");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ID) И ЗначениеЗаполнено(Тип) Тогда 
		...
		Если ИнтеграцияС1СДокументооборотПовтИсп.ИспользоватьТерминКорреспонденты() Тогда
			Заголовок = СтрШаблон(НСтр("es='%1 (Corresponsal)';ru='%1 (Корреспондент)';en='%1 (Correspondent)'"), ОбъектXDTO.name);
		Иначе
			Заголовок = СтрШаблон(НСтр("es='%1 (Contratante)';ru='%1 (Контрагент)';en='%1 (Counterparty)'"), ОбъектXDTO.name);
		КонецЕсли;
	...
	Иначе
		...
		Если ИнтеграцияС1СДокументооборотПовтИсп.ИспользоватьТерминКорреспонденты() Тогда
			Заголовок = НСтр("es='El corresponsal (creación)';ru='Корреспондент (создание)';en='Correspondent (creation)'");
		Иначе
			Заголовок = НСтр("es='El contratante (creación)';ru='Контрагент (создание)';en='Counterparty (creation)'");
		КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ID)
		...
		Если ЮрФизЛицо = НСтр("es='La persona física';ru='Физическое лицо';en='Individual'") Тогда
			ЮрФизЛицоID = "ФизЛицо";
			ЮрФизЛицоТип = "DMLegalPrivatePerson";
		ИначеЕсли ЮрФизЛицо = НСтр("es='La persona jurídica';ru='Юридическое лицо';en='Entity'") Тогда
			ЮрФизЛицоID = "ЮрЛицо";
			ЮрФизЛицоТип = "DMLegalPrivatePerson";
		ИначеЕсли ЮрФизЛицо = НСтр("es='El empresario individual';ru='Индивидуальный предприниматель';en='Individual entrepreneur'") Тогда
			ЮрФизЛицоID =  "ИндивидуальныйПредприниматель";
			ЮрФизЛицоТип = "DMLegalPrivatePerson";
		ИначеЕсли ЮрФизЛицо = НСтр("es='No el residente';ru='Не резидент';en='Non resident'")   Тогда
...
"La creación del proceso de negocios es posible solamente después del registro de datos. "
...
"Creating a business process is possible only after writing data."
