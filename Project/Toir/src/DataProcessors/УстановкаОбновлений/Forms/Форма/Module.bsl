...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбновлениеКонфигурации.ЕстьПраваНаУстановкуОбновления() Тогда
		ВызватьИсключение НСтр("es='Es insuficientemente derecho para la renovación de la configuración. Se dirijan al administrador.';ru='Недостаточно прав для обновления конфигурации. Обратитесь к администратору.';en='Not enough rights to update configuration. Contact your administrator.'");
	ИначеЕсли Пользователи.ЭтоСеансВнешнегоПользователя() Тогда
		ВызватьИсключение НСтр("es='La operación dada no es accesible al usuario exterior del sistema.';ru='Данная операция не доступна внешнему пользователю системы.';en='This operation is not available to an external user of the system.'");
	КонецЕсли;
	...
	Если ЭтоБазоваяВерсия Тогда
		ЗаголовокВариантаВыбора = "&" + НСтр("es='Indiquen el fichero de la renovación';ru='Укажите файл обновления';en='Specify update file'");
...
Процедура ПолеФайлОбновленияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЭтоБазоваяВерсия Тогда
		Диалог.Фильтр = НСтр("es='Todos los ficheros del suministro (*.cf *;*.cfu) | *.cf *;*.cfu|Файлы los suministros de la configuración (*.cf) | *.cf|Файлы los suministros de la renovación de la configuración (*.cfu) | *.cfu';ru='Все файлы поставки (*.cf*;*.cfu)|*.cf*;*.cfu|Файлы поставки конфигурации (*.cf)|*.cf|Файлы поставки обновления конфигурации(*.cfu)|*.cfu';en='All delivery files (* .cf *; *. Cfu) | * .cf *; *. Cfu | Configuration delivery files (* .cf) | * .cf | Configuration update delivery files (*. Cfu) | * .cfu'");
	Иначе
		Диалог.МножественныйВыбор = Истина;
		Диалог.Фильтр = НСтр("es='Todos los ficheros (*.cf*;*.cfu;*.cfe;*.zip) | *.cf *;*.cfu; *.cfe; *.zip|Файлы los suministros de la configuración (*.cf) | *.cf|Файлы los suministros de la renovación de la configuración (*.cfu) | *.cfu|Файлы de las correcciones (*.cfe*;*.zip) |*.cfe*;*.zip';ru='Все файлы (*.cf*;*.cfu;*.cfe;*.zip)|*.cf*;*.cfu;*.cfe;*.zip|Файлы поставки конфигурации (*.cf)|*.cf|Файлы поставки обновления конфигурации (*.cfu)|*.cfu|Файлы исправлений (*.cfe*;*.zip)|*.cfe*;*.zip';en='All files (* .cf *; *. Cfu; *. Cfe; *. Zip) | * .cf *; *. Cfu; *. Cfe; *. Zip | Configuration delivery files (* .cf) | * .cf | Configuration update delivery files (* .cfu) | * .cfu | Patch files (* .cfe *; *. Zip) | * .cfe *; *. Zip'");
	КонецЕсли;
	Диалог.Заголовок = НСтр("es='La elección del suministro de la renovación de la configuración';ru='Выбор поставки обновления конфигурации';en='Choosing a Configuration Update Delivery'");
...
Процедура НадписьОшибкаУстановкиИсправленийОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ОтборЖурнала.Вставить("СобытиеЖурналаРегистрации", НСтр("es='Las correcciones. La instalación';ru='Исправления.Установка';en='Corrections: Installation'"));
...
Процедура ПередОткрытиемСтраницы(НоваяТекущаяСтраница = Неопределено)
	...
	КнопкаДалее.Заголовок = ?(ФункцияКнопкиДалее, НСтр("es='Más>';ru='Далее >';en='Next >'"), НСтр("es='Es preparado';ru='Готово';en='Done'"));
	КнопкаЗакрыть.Заголовок = ?(ФункцияКнопкиЗакрыть, НСтр("en='cancel';es='La anulación';ru='Отмена'"), НСтр("es='Cerrar';ru='Закрыть';en='Close'"));
	...
	Для Каждого ПомещенныйФайл Из ПомещенныеФайлы Цикл
		...
		Попытка
			Если СтрЗаканчиваетсяНа(ПомещенныйФайл.Имя, ".zip") Тогда
				...
				Иначе
					ВызватьИсключение НСтр("es='El fichero no es la corrección.';ru='Файл не является исправлением.';en='File is not a fix.'");
				КонецЕсли;
				ЧтениеZip.Закрыть();
			...
			Если Не ОбновлениеКонфигурации.ЭтоИсправление(УстановленноеРасширение) Тогда
				Расширение.Удалить();
				ВызватьИсключение НСтр("es='La ampliación no es патчем.';ru='Расширение не является патчем.';en='The extension is not a patch.'");
			КонецЕсли;
		...
		Исключение
			...
			ТекстОшибки = НСтр("es='A la instalación de la corrección %1 ha surgido la falta: "
"%2';ru='При установке исправления %1 возникла ошибка:"
"%2';en='An error occurred while installing patch %1:"
"%2 '");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
				ИмяФайлаИсправления,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			ЗаписьЖурналаРегистрации(НСтр("es='Las correcciones. La instalación';ru='Исправления.Установка';en='Corrections: Installation'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Если ЕстьОшибки Тогда
		...
		Если УстановленоИсправлений > 0 Тогда
			ТекстНадписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Es establecido las correcciones: %1 de %2, los cambios entrarán en vigor después del relanzamiento del programa';ru='Установлено исправлений: %1 из %2, изменения вступят в силу после перезапуска программы';en='Patches installed:%1 of%2, changes will take effect after restarting the program'"),
	...
	Если Результат = "Продолжить" Тогда
		...
		Для Каждого ИмяФайла Из ИменаВыбранныхФайлов Цикл
			...
			Иначе // Это файл обновления.
				...
				Иначе
					ВызватьИсключение НСтр("es='Es admisible escoger solamente un fichero de la renovación.';ru='Допустимо выбирать только один файл обновления.';en='Only one update file can be selected.'");
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		...
		Если НуженФайлОбновления = 1 Тогда
			Если Не ЗначениеЗаполнено(ВыбранныеФайлы) Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("es='Indiquen el fichero del suministro de la renovación de la configuración.';ru='Укажите файл поставки обновления конфигурации.';en='Specify the configuration update delivery file.'"),,"Объект.ИмяФайлаОбновления");
			...
			Если Не ПустаяСтрока(Объект.ИмяФайлаОбновления) Тогда
				...
				Если Не Файл.Существует() Или Не Файл.ЭтоФайл() Тогда
					ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("es='El fichero del suministro de la renovación de la configuración no es encontrado.';ru='Файл поставки обновления конфигурации не найден.';en='Configuration update delivery file not found.'"),,"Объект.ИмяФайлаОбновления");
					ТекущийЭлемент = Элементы.ПолеФайлОбновления;
					Возврат;
				КонецЕсли;
			КонецЕсли;
