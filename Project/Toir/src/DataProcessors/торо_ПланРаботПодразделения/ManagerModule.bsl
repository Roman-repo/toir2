...
Функция ПечатьПланРабот(МассивОбъектов, ПараметрыПечати)
	...
	ПодразделениеВладелец = ПараметрыПечати.Параметры.ПодразделениеВладелец;
	ПодразделениеИсполнитель = ПараметрыПечати.Параметры.ПодразделениеИсполнитель;
	...
	|	торо_ПлановыеРемонтныеРаботыСрезПоследних.Регистратор КАК ДокументППР,
	...
	|ГДЕ
	|	&ПоПланГрафикамППР
	//|	И торо_ПлановыеРемонтныеРаботыСрезПоследних.Регистратор ССЫЛКА Документ.торо_ПланГрафикРемонта
	|	И НЕ торо_ПлановыеРемонтныеРаботыСрезПоследних.Отменен
	|	И НЕ торо_ПлановыеРемонтныеРаботыСрезПоследних.Замещен
	|	И (&НетПодразделений
	|			ИЛИ &ПодразделенияУказаны = ИСТИНА
	|				И (торо_ПлановыеРемонтныеРаботыСрезПоследних.ОбъектРемонтныхРабот.Подразделение = &ПодразделениеВладелец
	|					И торо_ПлановыеРемонтныеРаботыСрезПоследних.Исполнитель = &ПодразделениеИсполнитель)
	|			ИЛИ &ПодразделенияУказаны = ЛОЖЬ
	|				И (торо_ПлановыеРемонтныеРаботыСрезПоследних.ОбъектРемонтныхРабот.Подразделение = &ПодразделениеВладелец
	|					ИЛИ торо_ПлановыеРемонтныеРаботыСрезПоследних.Исполнитель = &ПодразделениеИсполнитель))
	|	И торо_ПлановыеРемонтныеРаботыСрезПоследних.ДатаНачалаРемонтныхРабот МЕЖДУ &ДатаНачала И &ДатаОкончания
	|	И торо_ПлановыеРемонтныеРаботыСрезПоследних.Отменен = ЛОЖЬ
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И (&НетПодразделений
	|			ИЛИ торо_ПлановыеРемонтныеРаботы.Исполнитель = &ПодразделениеИсполнитель)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	(&НетПодразделений
	|			ИЛИ РемонтЗаявка.ОбъектРемонта.Подразделение = &ПодразделениеВладелец)
	...
	|	ТаблицаНачало.Подразделение КАК Подразделение,
	|	ТаблицаНачало.ДокументППР КАК ДокументППР,
	|	ТаблицаНачало.ИсточникДанных КАК ИсточникДанных
	|ИЗ
		...
		Если СтрокаОтбора.Использование Тогда
			...
			Если ЗначениеЗаполнено(СтрокаОтбора.Имя) Тогда
				...
				Если первыйОтбор Тогда
					первыйОтбор = Ложь;
					Запрос.Текст = Запрос.Текст + " Где ТаблицаНачало." + СтрокаОтбора.Имя + " " + ВидСравненияВЗапросе.ВидСравненияНачало + " &Параметр" + Сч + ВидСравненияВЗапросе.ВидСравненияКонец;
				Иначе
	...
	Запрос.УстановитьПараметр("ПодразделениеВладелец", ПодразделениеВладелец);
	Запрос.УстановитьПараметр("ПодразделениеИсполнитель", ПодразделениеИсполнитель);
	Запрос.УстановитьПараметр("НетПодразделений", НЕ ЗначениеЗаполнено(ПодразделениеВладелец) И НЕ ЗначениеЗаполнено(ПодразделениеИсполнитель));
	Запрос.УстановитьПараметр("ПодразделенияУказаны", ЗначениеЗаполнено(ПодразделениеВладелец) И ЗначениеЗаполнено(ПодразделениеИсполнитель));
	...
	МакетТаблицы =  УправлениеПечатью.МакетПечатнойФормы("Обработка.торо_ПланРаботПодразделения.ПФ_MXL_МакетПланаРемонтовНаПериод");
	...
	Если ЗначениеЗаполнено(ПодразделениеИсполнитель) Тогда
		...
		Область.Параметры.ПодразделениеНомер = ПодразделениеИсполнитель;
		
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПодразделениеВладелец) И ЗначениеЗаполнено(ПодразделениеИсполнитель) Тогда
		
		Область.Параметры.Подразделение = "Владелец - " + ПодразделениеВладелец + ", Исполнитель - " + ПодразделениеИсполнитель;
		
	Иначе
		
		Если ЗначениеЗаполнено(ПодразделениеВладелец) Тогда
			
			Область.Параметры.Подразделение = ПодразделениеВладелец;
			
		ИначеЕсли ЗначениеЗаполнено(ПодразделениеИсполнитель) Тогда
			
			Область.Параметры.Подразделение = ПодразделениеИсполнитель;
