...
Процедура Сформировать(Команда)
	...
	Если Не ЗначениеЗаполнено(Сотрудник) Тогда
		ТекстСообщения = НСтр("es='Campo ""el Empleado"" no es rellenado.';ru='Поле ""Сотрудник"" не заполнено.';en='Field ""Employee"" is not filled.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,"Сотрудник");
		Возврат;
	КонецЕсли;
	
	Если ДатаОкончания - ДатаНачала > 60 * 60 * 24 * 3 Тогда
		ТекстСообщения = НСтр("es='El período dado supera recomendado (3 días).';ru='Заданный период превышает рекомендуемый (3 дня).';en='The set period exceeds the recommended (3 days).'");
	...
	Если HTML = "" Тогда
		ТекстСообщения = НСтр("es='Las coordenadas en el intervalo dado no existe.';ru='Координат в заданном интервале не существует.';en='The coordinates in the specified interval do not exist.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
