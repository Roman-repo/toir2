...
Процедура Инициализация() Экспорт
	...
	ТелоСообщенияПростое	= НСтр("es='El mensaje del cambio para los datos';ru='Сообщение обмена данными';en='Data exchange message'");
	ТелоСообщенияСжатое	= НСтр("es='El mensaje apretado del cambio para los datos';ru='Сжатое сообщение обмена данными';en='Compressed data exchange message'");
	ТелоСообщенияПакетное	= НСтр("es='El mensaje de paquete del cambio para los datos';ru='Пакетное сообщение обмена данными';en='Batch сommunication'");
...
Функция ПолучитьСообщениеОбмена(ПроверкаНаличия)
	...
	Если ТаблицаСообщенийОбмена.Количество() = 0 Тогда
		...
		Если Не ПроверкаНаличия Тогда
			...
			СтрокаСообщения = НСтр("es='el encabezamiento: ""%1""';ru='Не обнаружены письма с заголовком: ""%1""';en='No messages were found with the header: ""%1""'");
...
Процедура ИнициализацияСообщенийОшибок()
	...
	// Общие коды ошибок
	СообщенияОшибок.Вставить(001, НСтр("es='del cambio.';ru='Не обнаружены сообщения обмена.';en='No exchange messages found.'"));
	...
	СообщенияОшибок.Вставить(006, НСтр("es='El mensaje del cambio no es enviado: es superada la dimensión admisible del mensaje.';ru='Сообщение обмена не отправлено: превышен допустимый размер сообщения.';en='The exchange message not sent: message size exceeded.'"));
	
	// Коды ошибок, зависящие от вида транспорта.
	СообщенияОшибок.Вставить(101, НСтр("es='La falta de la inicialización: no es indicada la anotación de descuento del correo electrónico del transporte de los mensajes del cambio.';ru='Ошибка инициализации: не указана учетная запись электронной почты транспорта сообщений обмена.';en='Initialization Error: No messaging transport email account specified.'"));
	СообщенияОшибок.Вставить(102, НСтр("es='La falta al envío del mensaje del correo electrónico.';ru='Ошибка при отправке сообщения электронной почты.';en='Error sending email.'"));
	СообщенияОшибок.Вставить(103, НСтр("es='La falta a la recepción de los encabezamientos de los mensajes del servidor del correo electrónico.';ru='Ошибка при получении заголовков сообщений с сервера электронной почты.';en='Error occurred while receiving message headers from the email server.'"));
	СообщенияОшибок.Вставить(104, НСтр("es='el servidor postal.';ru='Не обнаружены сообщения обмена на почтовом сервере.';en='No exchange messages were detected on the mail server.'"));
	СообщенияОшибок.Вставить(105, НСтр("es='La falta a la recepción del mensaje del servidor del correo electrónico.';ru='Ошибка при получении сообщения с сервера электронной почты.';en='Error receiving message from email server.'"));
	СообщенияОшибок.Вставить(106, НСтр("es='La falta al registro del fichero del mensaje del cambio por el disco.';ru='Ошибка при записи файла сообщения обмена на диск.';en='Error while writing the exchange message file to disk.'"));
	СообщенияОшибок.Вставить(107, НСтр("es='La comprobación de los parámetros de la anotación de descuento se ha acabado con las faltas.';ru='Проверка параметров учетной записи завершилась с ошибками.';en='Checking account settings failed.'"));
	...
	СообщенияОшибок.Вставить(109, НСтр("es='La falta: en el mensaje postal no es encontrado el fichero con el mensaje.';ru='Ошибка: в почтовом сообщении не найден файл с сообщением.';en='Error: message file was not found in the mail message.'"));
