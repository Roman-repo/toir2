...
Процедура ПутьККаталогуАрхивовНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогОткрытияФайла.Заголовок= НСтр("es='Escojan el catálogo para la conservación de las copias de reserva';ru='Выберите каталог для сохранения резервных копий';en='Select a directory to save backups'");
...
Процедура ПерейтиСоСтраницыНастройкиПослеПроверкиДоступаКИнформационнойБазе(РезультатПодключения, Контекст) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("es='La copia de reserva';ru='Резервное копирование';en='Backup'"), ИмяФормыНастроек,
		НСтр("es='La copia de reserva es ajustada.';ru='Резервное копирование настроено.';en='Backup is configured.'"));
...
#Если ВебКлиент ИЛИ МобильныйКлиент Тогда
	ТекстСообщения = НСтр("es='Para el trabajo correcto es necesario el régimen del cliente delgado o gordo.';ru='Для корректной работы необходим режим тонкого или толстого клиента.';en='For correct operation, a thin or thick client mode is required.'");
...
#Иначе
	...
	Иначе
		...
		Исключение
			ТекстСообщения = НСтр("es='No hay acceso al catálogo con las copias de reserva.';ru='Нет доступа к каталогу с резервными копиями.';en='No access to backup directory.'");
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "Объект.КаталогСРезервнымиКопиями");
			РеквизитыЗаполнены = Ложь;
		КонецПопытки;
