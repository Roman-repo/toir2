...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаголовокФормы = НСтр("es='El cambio universal para los datos en el formato XML (%ВерсияОбработки%)';ru='Универсальный обмен данными в формате XML (%ВерсияОбработки%)';en='Universal XML data exchange (%ВерсияОбработки%)'");
	//ЗаголовокФормы = НСтр("es='El cambio universal para los datos en el formato XML (%ВерсияОбработки del %)';ru='Универсальный обмен данными в формате XML (%ВерсияОбработки%)';en='Universal XML data exchange (%ВерсияОбработки%)'");
	// << [Деснол] [Ушаков А.Д.] [24.10.2019] 
...
Процедура ПриОткрытии(Отказ)
	...
	Если СохраненныйРежимЗагрузки
		...
		Если Объект.НастройкаАвтоматическойЗагрузкиДанных = 1 Тогда
			...
			ПоказатьВопрос(ОписаниеОповещения, НСтр("es='¿Cumplir la carga de los datos del fichero del cambio?';ru='Выполнить загрузку данных из файла обмена?';en='Download data from an exchange file?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура КаталогИнформационнойБазыДляПодключенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбораФайла.Заголовок = НСтр("es='Escojan el catálogo de la base informativa';ru='Выберите каталог информационной базы';en='Select infobase directory'");
...
Процедура ИмяФайлаПравилПослеПроверкиСуществования(Существует, ДополнительныеПараметры) Экспорт
	...
	Если Не Существует Тогда
		СообщитьПользователю(НСтр("es='No es encontrado el fichero gobernaba el cambio';ru='Не найден файл правил обмена';en='Exchange rules file not found'"), "ИмяФайлаПравил");
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("es='¿Cargar las reglas del cambio para los datos?';ru='Загрузить правила обмена данными?';en='Download data exchange rules?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура ПолучитьИнформациюОФайлеОбмена(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайла,НСтр("es='El fichero del cambio';ru='Файл обмена';en='Sharing file'"),, УникальныйИдентификатор);
...
Процедура ПолучитьИнформациюОФайлеОбменаЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Исключение
			...
			СообщитьПользователю(НСтр("es='No consiguió leer el fichero del cambio.';ru='Не удалось прочитать файл обмена.';en='Failed to read the exchange file.'"));
			ОчиститьДанныеОФайлеДляЗагрузкиДанных();
...
Процедура УдалениеУдалить(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("es='¿Quitar los datos escogidos en la base informativa?';ru='Удалить выбранные данные в информационной базе?';en='Delete selected data in the infobase?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Процедура НастройкаОтладкиЗагрузки(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайлаОбменаВХранилище,НСтр("es='El fichero del cambio';ru='Файл обмена';en='Sharing file'"),, УникальныйИдентификатор);
...
Процедура ПрочитатьПравилаОбмена(Команда)
	...
	Если Не ЭтоWindowsКлиент() И ПрямаяВыгрузка = 1 Тогда
		ПоказатьПредупреждение(,НСтр("es='La conexión directa a la base informativa es apoyada solamente en el cliente bajo la dirección la SO Windows.';ru='Прямое подключение к информационной базе поддерживается только в клиенте под управлением ОС Windows.';en='Direct connection to the infobase is supported only in a client running Windows.'"));
		Возврат;
	КонецЕсли;
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайлаПравилВХранилище,НСтр("es='El fichero gobernaba el cambio';ru='Файл правил обмена';en='Exchange Rules File'"),, УникальныйИдентификатор);
...
Функция ИменаФайловПравилИОбменаСовпадают()
	...
	Если ВРег(СокрЛП(ИмяФайлаПравил)) = ВРег(СокрЛП(ИмяФайлаДанных)) Тогда
		...
		СообщитьПользователю(НСтр("es='El fichero gobernaba el cambio no puede coincidir con el fichero de los datos. "
"Escojan otro fichero para la descarga de los datos.';ru='Файл правил обмена не может совпадать с файлом данных."
"Выберите другой файл для выгрузки данных.';en='Exchange rule file cannot match the data file."
"Select another file to upload data. '"));
		Возврат Истина;
...
Процедура ЗаполнитьСписокТиповДоступныхДляУдаления()
	...
	СтрокаДерева.Представление = НСтр("es='Las guías';ru='Справочники';en='Directories'");
	...
	Для каждого ОбъектМД Из Метаданные.Справочники Цикл
		...
		// >> [Деснол] [Ушаков А.Д.] [24.10.2019] 
		СтрокаМД.Представление = ОбъектМД.Синоним;
	...
	СтрокаДерева = ДеревоДанных.Строки.Добавить();
	СтрокаДерева.Представление = НСтр("es='Los planes de los tipos de las características';ru='Планы видов характеристик';en='Features types plans'");
	...
	СтрокаДерева = ДеревоДанных.Строки.Добавить();
	СтрокаДерева.Представление = НСтр("es='Los documentos';ru='Документы';en='Documents'");
...
Процедура ВыполнитьПодключениеКИБПриемникуНаСервере()
	...
	Если РезультатПодключения <> Неопределено Тогда
		...
		СообщитьПользователю(НСтр("es='La conexión es con éxito establecida.';ru='Подключение успешно установлено.';en='Connection successfully established.'"));
...
Процедура ОткрытьФайлЗагрузкиНаСервере(АдресФайла)
	...
	Иначе
		...
		Если Не ФайлНаСервере.Существует() Тогда
			...
			СообщитьПользователю(НСтр("es='el servidor.';ru='Не найден файл обмена на сервере.';en='The exchange file was not found on the server.'"), "ИмяФайлаОбмена");
			Возврат;
...
Процедура ВыполнитьЗагрузкуНаСервере(АдресФайла, ИмяФайлаДляРасширения)
	...
	Если Объект.БезопаснаяЗагрузка Тогда
		...
		Иначе
			СообщитьПользователю(НСтр("es='No es indicado el fichero gobernaba para la carga de los datos.';ru='Не указан файл правил для загрузки данных.';en='No rules file for data loading specified.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("es='El cambio universal para los datos en el formato XML';ru='Универсальный обмен данными в формате XML';en='Universal XML data exchange'", ОбъектДляСервера.КодОсновногоЯзыка()),
...
Функция ИмяФайлаНаСервереИлиКлиенте(ИмяРеквизита ,Знач АдресФайла, Знач ИмяФайлаДляРасширения = ".xml",
	...
	Иначе
		...
		Если Не ФайлНаСервере.Существует() И ПроверятьСуществование Тогда
			...
			СообщитьПользователю(НСтр("es='El fichero indicado no existe.';ru='Указанный файл не существует.';en='The specified file does not exist.'"));
...
Процедура ВыполнитьВыгрузкуИзФормы()
	...
	Если ЭтоКлиент И Не ПрямаяВыгрузка И Не Объект.ФлагОшибки Тогда
		...
		ИмяСохраняемогоФайла = ?(Объект.АрхивироватьФайл, НСтр("es='El fichero выгрузки.zip';ru='Файл выгрузки.zip';en='Upload file.zip'"),НСтр("es='El fichero выгрузки.xml';ru='Файл выгрузки.xml';en='Upload file.xml'"));
...
Функция ВыполнитьВыгрузкуНаСервере()
	...
	Если Не ПрямаяВыгрузка Тогда
		...
		Если ИмяВременногоФайлаДанных = Неопределено Тогда
			...
			СообщитьПользователю(НСтр("es='de los datos';ru='Не определен файл данных';en='Data file not defined'"));
	...
	Если ОбъектДляСервера.ФлагРежимОтладкиОбработчиков Тогда
		...
		Если Не Файл.Существует() Тогда
			...
			СообщитьПользователю(НСтр("es='El fichero del tratamiento exterior de los ajustadores de los acontecimientos no existe en el servidor';ru='Файл внешней обработки отладчиков событий не существует на сервере';en='File for external processing of event debuggers does not exist on the server'"));
			Возврат Неопределено;
		...
		Если Отказ Тогда
			...
			СообщитьПользователю(НСтр("es='No consiguió descargar los elaboradores de los acontecimientos';ru='Не удалось выгрузить обработчики событий';en='Failed to unload event handlers'"));
			Возврат "";
...
Процедура ИзменитьРежимОбработки(РежимРаботы)
	...
	?(РежимРаботы, НСтр("es='El régimen del trabajo (sobre el cliente)';ru='Режим работы (на клиенте)';en='Operating mode (on the client)'"), НСтр("es='El régimen del trabajo (en el servidor)';ru='Режим работы (на сервере)';en='Operating mode (on the server)'"));
...
Функция ПустоеЗначениеРеквизита(Реквизит, ПутьКДанным, Заголовок)
	...
	Если ПустаяСтрока(Реквизит) Тогда
		...
		ТекстСообщения = НСтр("es='Campo ""%1"" no es rellenado';ru='Поле ""%1"" не заполнено';en='Field ""%1"" is empty'");
...
Процедура ПоместитьФайлПравилЗагрузкиВХранилище()
	...
	НачатьПомещениеФайла(ОписаниеОповещения, АдресФайла,
		?(ЧерезХранилище, НСтр("es='El fichero del cambio';ru='Файл обмена';en='Sharing file'"), ИмяФайлаПравилЗагрузки), ЧерезХранилище, УникальныйИдентификатор);
