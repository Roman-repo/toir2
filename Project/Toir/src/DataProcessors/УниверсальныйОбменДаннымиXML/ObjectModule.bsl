...
Функция одТипЗначенияСтрокой(ЗначениеИлиТип) Экспорт
	...
	Иначе
		...
		Если Менеджер = Неопределено Тогда
			...
			Текст= НСтр("es='El tipo desconocido:';ru='Неизвестный тип:';en='Unknown type:'") + Строка(ТипЗнч(ТипЗначения));
...
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаПользователю = ?(ТолькоПолучитьУзелСсылки, НСтр("es='La conversión de la referencia al objeto: %1';ru='Конвертация ссылки на объект: %1';en='Object link conversion: %1'"), НСтр("es='La conversión del objeto: %1';ru='Конвертация объекта: %1';en='Object conversion: %1'"));
...
Процедура ПередатьИнформациюОЗаписиВПриемник(ИнформацияДляЗаписиВФайл, СтрокаОшибкиВБазеПриемнике = "") Экспорт
	...
	Если Не ПустаяСтрока(СтрокаОшибкиВБазеПриемнике) Тогда
		...
		СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("es='La carga en el receptor: %1';ru='Загрузка в приемнике: %1';en='Receiver download:%1'"), СтрокаОшибкиВБазеПриемнике);
...
Процедура УдалитьВременныеФайлы(ИмяВременногоФайла) Экспорт
	...
	Если Не ПустаяСтрока(ИмяВременногоФайла) Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("es='El cambio universal para los datos en el formato XML';ru='Универсальный обмен данными в формате XML';en='Universal XML data exchange'", КодОсновногоЯзыка()),
...
Функция ЗаписьТекстаВоВременныйФайл(СписокВременныхФайлов)
	...
	Исключение
		ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(1000,
			ОписаниеОшибки(),
			НСтр("es='La falta a la creación del fichero temporal para la descarga de los datos';ru='Ошибка при создании временного файла для выгрузки данных';en='Error creating temporary file to upload data'"));
		ВызватьИсключение;
	КонецПопытки;
...
Функция ЧтениеТекстаИзВременногоФайла(ИмяВременногоФайла)
	...
	Исключение
		ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(1000,
			ОписаниеОшибки(),
			НСтр("es='La falta a la apertura del fichero temporal para el traslado de los datos al fichero del cambio';ru='Ошибка при открытии временного файла для переноса данных в файл обмена';en='Error while opening temporary file to transfer data to exchange file'"));
		ВызватьИсключение;
	КонецПопытки;
...
Функция КодировкаФайлаПротоколаОбмена()
	...
	Если Не ПустаяСтрока(КодировкаФайлаПротоколаОбмена) Тогда
		Если СтрНачинаетсяС(ПредставлениеКодировки, "КодировкаТекста.") Тогда
			...
			Исключение
				ТекстОшибки = ПодставитьПараметрыВСтроку(НСтр("es='La codificación desconocida del fichero de la acta del cambio: %1. "
"Se usa ANSI.';ru='Неизвестная кодировка файла протокола обмена: %1."
"Используется ANSI.';en='Unknown exchange protocol file encoding:%1."
"Used by ANSI.'"), ПредставлениеКодировки);
				ЗаписьЖурналаРегистрации(НСтр("es='El cambio universal para los datos en el formato XML';ru='Универсальный обмен данными в формате XML';en='Universal XML data exchange'", КодОсновногоЯзыка()),
...
Процедура ВыгрузитьОбработчикиСобытий(Отказ) Экспорт
	...
	Если ЭтоИнтерактивныйРежим Тогда
		...
		Если ФлагОшибки Тогда
			...
			СообщитьПользователю(НСтр("es='A la descarga de los elaboradores eran descubiertas las faltas.';ru='При выгрузке обработчиков были обнаружены ошибки.';en='When unloading handlers, errors were detected.'"));
			
		Иначе
			
			СообщитьПользователю(НСтр("es='Los elaboradores son con éxito descargados.';ru='Обработчики успешно выгружены.';en='Handlers successfully unloaded.'"));
...
Функция ЗаменитьВызовыАлгоритмовКодомЭтихАлгоритмовВОбработчике(Обработчик, АлгоритмВладелец, МассивЗапрошенныхЭлементов, Знач СтрокаПрефикса = "")
	...
	ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, АлгоритмВладелец, СтрокаПрефикса, НСтр("es='{EL COMIENZO DEL ALGORITMO}';ru='{НАЧАЛО АЛГОРИТМА}';en='{НАЧАЛО АЛГОРИТМА}'"));
		...
		Если АлгоритмыОбработчика.Количество() <> 0 Тогда // В этой строке есть вызовы алгоритмов.
			...
			Для Каждого Алгоритм Из АлгоритмыОбработчика Цикл
				...
				Если МассивЗапрошенныхЭлементов.Найти(ВРег(Алгоритм)) <> Неопределено Тогда // Рекурсивный вызов алгоритма.
					...
					ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, Алгоритм, СтрокаПрефиксаДляВложенногоКода, НСтр("es='{РЕКУРСИВНЫЙ LA LLAMADA DEL ALGORITMO}';ru='{РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА}';en='{РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА}'"));
					
					СтрокаОператора = НСтр("es='ВызватьИсключение ""РЕКУРСИВНЫЙ la LLAMADA del ALGORITMO: %1"";';ru='ВызватьИсключение ""РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА: %1"";';en='ВызватьИсключение""RECURSIVE ALGORITHM CALL:% 1"";'");
	...
	ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, АлгоритмВладелец, СтрокаПрефикса, НСтр("es='{EL FIN DEL ALGORITMO}';ru='{КОНЕЦ АЛГОРИТМА}';en='{КОНЕЦ АЛГОРИТМА}'"));
...
Процедура КомментарииКЗагрузкеОбъекта(НПП, ИмяПравила, Источник, ТипОбъекта, ГНПП = 0)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		Если НПП <> 0 Тогда
			СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("es='La carga del objeto № %1';ru='Загрузка объекта № %1';en='Loading object No.% 1'"), НПП);
		Иначе
			СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("es='La carga del objeto № %1';ru='Загрузка объекта № %1';en='Loading object No.% 1'"), ГНПП);
		КонецЕсли;
...
Процедура ВыгрузкаОбъектаВыборки(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, ВыборкаДляВыгрузкиДанных = Неопределено)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("es='La descarga del objeto: %1';ru='Выгрузка объекта: %1';en='Unloading object:% 1'"), ПрОбъекта);
...
Процедура ВыгрузитьДанныеПоПравилу(Правило)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("es='La regla de la descarga de los datos: %1 (%2)';ru='Правило выгрузки данных: %1 (%2)';en='Data upload rule:%1 (%2)'"), СокрЛП(Правило.Имя), СокрЛП(Правило.Наименование));
			...
			Если ПоследнийОбъектМетаданных <> ТекущийОбъектМетаданных Тогда
				...
				Если ТекущаяСтрокаПравилаВыгрузки <> Неопределено Тогда
					...
					Если ФлагКомментироватьОбработкуОбъектов Тогда
						...
						СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("es='La regla de la descarga de los datos: %1 (%2)';ru='Правило выгрузки данных: %1 (%2)';en='Data upload rule:%1 (%2)'"),
...
Процедура ЗагрузитьЗначенияПараметровОбменаДанными()
	...
	Если СобытияПослеЗагрузкиПараметров.Свойство(Имя, АлгоритмПослеЗагрузкиПараметра)
		...
		Если ФлагРежимОтладкиОбработчиков Тогда
			...
			ВызватьИсключение НСтр("es='El ajuste del elaborador ""Después de la carga del parámetro"" no es apoyado.';ru='Отладка обработчика ""После загрузки параметра"" не поддерживается.';en='Debugging the ""After loading the parameter"" handler is not supported.'");
	...
	Если Конвертация.Свойство("ПослеЗагрузкиПравилОбмена", ТекстСобытияПослеЗагрузкиПравилОбмена)
		...
		Попытка
			...
			Если ФлагРежимОтладкиОбработчиков Тогда
				...
				ВызватьИсключение НСтр("es='El ajuste del elaborador ""Después de la carga de las reglas del cambio"" no es apoyado.';ru='Отладка обработчика ""После загрузки правил обмена"" не поддерживается.';en='Debugging the ""After loading exchange rules"" handler is not supported.'");
		...
		Исключение
			...
			Текст = НСтр("es='El elaborador: ""ПослеЗагрузкиПравилОбмена"": %1';ru='Обработчик: ""ПослеЗагрузкиПравилОбмена"": %1';en='Processor: ""ПослеЗагрузкиПравилОбмена"": %1'");
...
Процедура ПроизвестиЧтениеДанных(РезультирующаяСтрокаСИнформациейОбОшибке = "") Экспорт
				...
				Если Не ПустаяСтрока(ТекстАлгоритма) Тогда
					...
					Попытка
						...
						Если ФлагРежимОтладкиОбработчиков Тогда
							...
							ВызватьИсключение НСтр("es='El ajuste del elaborador ""Después de la carga de los parámetros"" no es apoyado.';ru='Отладка обработчика ""После загрузки параметров"" не поддерживается.';en='Debugging the ""After loading the parameters"" handler is not supported.'");
						...
						Если Отказ = Истина Тогда
							...
							Если Не ПустаяСтрока(ПричинаОтказа) Тогда
								СтрокаИсключения = ПодставитьПараметрыВСтроку(НСтр("es='La carga de los datos es anulada por la causa: %1';ru='Загрузка данных отменена по причине: %1';en='Data upload canceled due to:% 1'"), ПричинаОтказа);
				...
				Если Не ПустаяСтрока(ТекстАлгоритма) Тогда
					...
					Попытка
						...
						Если ФлагРежимОтладкиОбработчиков Тогда
							...
							ВызватьИсключение НСтр("es='El ajuste del algoritmo global no es apoyado.';ru='Отладка глобального алгоритма не поддерживается.';en='Debugging a global algorithm is not supported.'");
	...
	Исключение
		...
		СтрокаОшибки = ПодставитьПараметрыВСтроку(НСтр("es='La falta a la carga de los datos: %1';ru='Ошибка при загрузке данных: %1';en='Error loading data:%1'"), ОписаниеОшибки());
...
Функция ВыполнитьДействияПередЧтениемДанных(СтрокаДанных = "") Экспорт
	...
	СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("es='El comienzo de la carga: %1';ru='Начало загрузки: %1';en='Start loading:%1'"), ТекущаяДатаСеанса());
...
Процедура ВыполнитьДействияПослеЗавершенияЧтенияДанных() Экспорт
	...
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("es='La terminación de la carga: %1';ru='Окончание загрузки: %1';en='End of download:%1'"), ТекущаяДатаСеанса()), , Ложь, , , Истина);
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("es='Es cargado los objetos: %1';ru='Загружено объектов: %1';en='% 1 items downloaded'"), мСчетчикЗагруженныхОбъектов), , Ложь, , , Истина);
	
	ЗавершитьВедениеПротоколаОбмена();
	
	Если ЭтоИнтерактивныйРежим Тогда
		СообщитьПользователю(НСтр("es='La carga de los datos es acabada.';ru='Загрузка данных завершена.';en='Data loading completed.'"));
	КонецЕсли;
...
Процедура СжатьРезультирующийФайлОбмена()
	...
	Попытка
		...
		Архиватор = Новый ЗаписьZipФайла(ИмяФайлаОбмена, ПарольДляСжатияФайлаОбмена, НСтр("es='El fichero del cambio para los datos';ru='Файл обмена данными';en='Data exchange file'"));
...
Процедура ВыполнитьВыгрузку() Экспорт
	...
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("es='El comienzo de la descarga: %1';ru='Начало выгрузки: %1';en='Start of upload:%1'"), ТекущаяДатаСеанса()), , Ложь, , , Истина);
	...
	Исключение
		...
		ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
			НСтр("es='La falta a la descarga de los datos: %1';ru='Ошибка при выгрузке данных: %1';en='Error while uploading data:%1'"), СтрокаОшибки), Неопределено, Истина, , , Истина);
	...
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("es='La terminación de la descarga: %1';ru='Окончание выгрузки: %1';en='End of upload:%1'"), ТекущаяДатаСеанса()), , Ложь, , ,Истина);
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("es='Es descargado los objetos: %1';ru='Выгружено объектов: %1';en='Unloaded objects:% 1'"), мСчетчикВыгруженныхОбъектов), , Ложь, , , Истина);
	...
	Если ЭтоИнтерактивныйРежим Тогда
		СообщитьПользователю(НСтр("es='La descarga de los datos es acabada.';ru='Выгрузка данных завершена.';en='Data upload completed.'"));
	КонецЕсли;
...
Процедура ИнициализацияСообщений()
	...
	одСообщения.Вставить(2,  НСтр("es='La falta del desempaquetamiento del fichero del cambio. El fichero es bloqueado';ru='Ошибка распаковки файла обмена. Файл заблокирован';en='Error unpacking file sharing. File locked'"));
	одСообщения.Вставить(3,  НСтр("es='La falta del desempaquetamiento del fichero del cambio. El fichero es bloqueado';ru='Ошибка распаковки файла обмена. Файл заблокирован';en='Error unpacking file sharing. File locked'"));
	одСообщения.Вставить(4,  НСтр("es='El fichero indicado gobernaba el cambio no existe';ru='Указанный файл правил обмена не существует';en='The specified exchange rule file does not exist.'"));
	одСообщения.Вставить(5,  НСтр("es='La falta a la creación del COM-objeto Msxml2.DOMDocument';ru='Ошибка при создании COM-объекта Msxml2.DOMDocument';en='Error creating COM object Msxml2.DOMDocument'"));
	одСообщения.Вставить(6,  НСтр("es='La falta de la apertura del fichero del cambio';ru='Ошибка открытия файла обмена';en='Error opening file sharing'"));
	одСообщения.Вставить(7,  НСтр("es='La falta a la carga de las reglas del cambio';ru='Ошибка при загрузке правил обмена';en='Error loading exchange rules'"));
	одСообщения.Вставить(8,  НСтр("es='La falta del formato de las reglas del cambio';ru='Ошибка формата правил обмена';en='Exchange rule format error'"));
	одСообщения.Вставить(9,  НСтр("es='Es indicado el nombre del fichero para la descarga de los datos';ru='Некорректно указано имя файла для выгрузки данных';en='Incorrect file name for uploading data'"));
	одСообщения.Вставить(10, НСтр("es='La falta del formato del fichero del cambio';ru='Ошибка формата файла обмена';en='Exchange file format error'"));
	одСообщения.Вставить(11, НСтр("es='dado (el Nombre del fichero de los datos)';ru='Не указано имя файла для выгрузки данных (Имя файла данных)';en='Missing file name specified (Data file name)'"));
	одСообщения.Вставить(12, НСтр("es='La referencia al objeto no existente de los metadatos en las reglas del cambio';ru='Ссылка на несуществующий объект метаданных в правилах обмена';en='Reference to a nonexistent metadata object in exchange rules'"));
	
	одСообщения.Вставить(13, НСтр("es='La falta de la recepción del significado de la propiedad del objeto (por nombre de las propiedades de la fuente)';ru='Ошибка получения значения свойства объекта (по имени свойства источника)';en='Error getting the property value of the object (by the name of the source property)'"));
	одСообщения.Вставить(14, НСтр("es='La falta de la recepción del significado de la propiedad del objeto (por nombre de las propiedades de la fuente)';ru='Ошибка получения значения свойства объекта (по имени свойства источника)';en='Error getting the property value of the object (by the name of the source property)'"));
	
	одСообщения.Вставить(15, НСтр("es='dado (el Nombre del fichero para la carga)';ru='Не указано имя файла для загрузки данных (Имя файла для загрузки)';en='File name to upload data (File name to upload) is not specified. '"));
	
	одСообщения.Вставить(16, НСтр("es='La falta de la recepción del significado de la propiedad del objeto subordinado (por nombre de las propiedades de la fuente)';ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства источника)';en='Error getting the value of the property of the subordinate object (by the name of the source property)'"));
	одСообщения.Вставить(17, НСтр("es='La falta de la recepción del significado de la propiedad del objeto subordinado (por nombre de las propiedades de la fuente)';ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства источника)';en='Error getting the value of the property of the subordinate object (by the name of the source property)'"));
	
	одСообщения.Вставить(19, НСтр("es=""La falta en el elaborador del acontecimiento De PeredZagruzkoyOb'ekta"";ru='Ошибка в обработчике события ПередЗагрузкойОбъекта';en='Error in the ПередЗагрузкойОбъекта event handler'"));
	одСообщения.Вставить(20, НСтр("es=""La falta en el elaborador del acontecimiento De PeredZagruzkoyOb'ekta"";ru='Ошибка в обработчике события ПередЗагрузкойОбъекта';en='Error in the ПередЗагрузкойОбъекта event handler'"));
	одСообщения.Вставить(21, НСтр("es=""La falta en el elaborador del acontecimiento de PriZagruzkeOb'ekta"";ru='Ошибка в обработчике события ПриЗагрузкеОбъекта';en='Error in the event handler ПриЗагрузкеОбъекта'"));
	одСообщения.Вставить(22, НСтр("es=""La falta en el elaborador del acontecimiento De PosleZagruzkiOb'ekta"";ru='Ошибка в обработчике события ПослеЗагрузкиОбъекта';en='Error in the event handler ПослеЗагрузкиОбъекта'"));
	одСообщения.Вставить(23, НСтр("es='La falta en el elaborador del acontecimiento PeredZagruzkoyDannyh (conversión)';ru='Ошибка в обработчике события ПередЗагрузкойДанных (конвертация)';en='Error in the ПередЗагрузкойДанных event handler (conversion)'"));
	одСообщения.Вставить(24, НСтр("es='La falta en el elaborador del acontecimiento PosleZagruzkiDannyh (conversión)';ru='Ошибка в обработчике события ПослеЗагрузкиДанных (конвертация)';en='Error in the event handler ПослеЗагрузкиДанных (conversion)'"));
	одСообщения.Вставить(25, НСтр("es='La falta a la desaparición del objeto';ru='Ошибка при удалении объекта';en='Error deleting object'"));
	одСообщения.Вставить(26, НСтр("es='La falta a la anotación del documento';ru='Ошибка при записи документа';en='Error while writing document'"));
	одСообщения.Вставить(27, НСтр("es='La falta de la anotación del objeto';ru='Ошибка записи объекта';en='Error writing object'"));
	одСообщения.Вставить(28, НСтр("es='La falta en el elaborador del acontecimiento De PeredObrabotkoyPravilaOchistki';ru='Ошибка в обработчике события ПередОбработкойПравилаОчистки';en='Error in the ПередОбработкойПравилаОчистки event handler'"));
	одСообщения.Вставить(29, НСтр("es='La falta en el elaborador del acontecimiento De PosleObrabotkiPravilaOchistki';ru='Ошибка в обработчике события ПослеОбработкиПравилаОчистки';en='Error in the event handler ПослеОбработкиПравилаОчистки'"));
	
	одСообщения.Вставить(31, НСтр("es='La falta en el elaborador del acontecimiento De PeredObrabotkoyPravilaVygruzki';ru='Ошибка в обработчике события ПередОбработкойПравилаВыгрузки';en='Error in the ПередОбработкойПравилаВыгрузки event handler'"));
	одСообщения.Вставить(32, НСтр("es='La falta en el elaborador del acontecimiento De PeredObrabotkoyPravilaVygruzki';ru='Ошибка в обработчике события ПередОбработкойПравилаВыгрузки';en='Error in the ПередОбработкойПравилаВыгрузки event handler'"));
	одСообщения.Вставить(33, НСтр("es='La falta en el elaborador del acontecimiento De PosleObrabotkiPravilaVygruzki';ru='Ошибка в обработчике события ПослеОбработкиПравилаВыгрузки';en='Error in the event handler ПослеОбработкиПравилаВыгрузки'"));
	одСообщения.Вставить(34, НСтр("es=""La falta en el elaborador del acontecimiento De PeredVygruzkoyOb'ekta"";ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';en='Error in the ПередВыгрузкойОбъекта event handler'"));
	
	одСообщения.Вставить(39, НСтр("es='La falta a la ejecución del algoritmo que contiene en el fichero del cambio';ru='Ошибка при выполнении алгоритма, содержащегося в файле обмена';en='An error occurred while executing the algorithm contained in the exchange file'"));
	
	одСообщения.Вставить(41, НСтр("es=""La falta en el elaborador del acontecimiento De PeredVygruzkoyOb'ekta"";ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';en='Error in the ПередВыгрузкойОбъекта event handler'"));
	одСообщения.Вставить(42, НСтр("es=""La falta en el elaborador del acontecimiento De PeredVygruzkoyOb'ekta"";ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';en='Error in the ПередВыгрузкойОбъекта event handler'"));
	одСообщения.Вставить(43, НСтр("es=""La falta en el elaborador del acontecimiento de PriVygruzkeOb'ekta"";ru='Ошибка в обработчике события ПриВыгрузкеОбъекта';en='Error in the event handler ПриВыгрузкеОбъекта'"));
	
	одСообщения.Вставить(45, НСтр("es='No es encontrada la regla de la conversión de los objetos';ru='Не найдено правило конвертации объектов';en='Object conversion rule not found'"));
	
	одСообщения.Вставить(48, НСтр("es='La falta en el elaborador del acontecimiento De PeredObrabotkoyVygruzki del grupo de las propiedades';ru='Ошибка в обработчике события ПередОбработкойВыгрузки группы свойств';en='Error in the ПередОбработкойВыгрузки event handler properties group'"));
	одСообщения.Вставить(49, НСтр("es='La falta en el elaborador del acontecimiento De PeredObrabotkoyVygruzki del grupo de las propiedades';ru='Ошибка в обработчике события ПередОбработкойВыгрузки группы свойств';en='Error in the ПередОбработкойВыгрузки event handler properties group'"));
	одСообщения.Вставить(50, НСтр("es='La falta en el elaborador del acontecimiento De PosleObrabotkiVygruzki del grupo de las propiedades';ru='Ошибка в обработчике события ПослеОбработкиВыгрузки группы свойств';en='Error in the event handler ПослеОбработкиВыгрузки properties group'"));
	одСообщения.Вставить(51, НСтр("es='La falta en el elaborador del acontecimiento PeredVygruzkoy (el objeto de la colección)';ru='Ошибка в обработчике события ПередВыгрузкой (объекта коллекции)';en='Error in the ПередВыгрузкой event handler (collection object)'"));
	одСообщения.Вставить(52, НСтр("es='La falta en el elaborador del acontecimiento de PriVygruzke (el objeto de la colección)';ru='Ошибка в обработчике события ПриВыгрузке (объекта коллекции)';en='Error in the event handler ПриВыгрузке (collection object)'"));
	одСообщения.Вставить(53, НСтр("es='La falta en el elaborador del acontecimiento De PosleVygruzki (el objeto de la colección)';ru='Ошибка в обработчике события ПослеВыгрузки (объекта коллекции)';en='Error in the event handler ПослеВыгрузки (collection object)'"));
	одСообщения.Вставить(54, НСтр("es=""La falta en el elaborador global del acontecimiento PeredZagruzkoyOb'ekta (conversión)"";ru='Ошибка в глобальном обработчике события ПередЗагрузкойОбъекта (конвертация)';en='Error in the global event handler ПередЗагрузкойОбъекта (conversion)'"));
	одСообщения.Вставить(55, НСтр("es=""La falta en el elaborador global del acontecimiento PosleZagruzkiOb'ekta (conversión)"";ru='Ошибка в глобальном обработчике события ПослеЗагрузкиОбъекта (конвертация)';en='Error in the global event handler ПослеЗагрузкиОбъекта (conversion)'"));
	одСообщения.Вставить(56, НСтр("es='La falta en el elaborador del acontecimiento PeredVygruzkoy (propiedades)';ru='Ошибка в обработчике события ПередВыгрузкой (свойства)';en='Error in the ПередВыгрузкой event handler (properties)'"));
	одСообщения.Вставить(57, НСтр("es='La falta en el elaborador del acontecimiento de PriVygruzke (propiedad)';ru='Ошибка в обработчике события ПриВыгрузке (свойства)';en='Error in the event handler ПриВыгрузке (properties)'"));
	
	одСообщения.Вставить(62, НСтр("es='La falta en el elaborador del acontecimiento PeredVygruzkoyDannyh (conversión)';ru='Ошибка в обработчике события ПередВыгрузкойДанных (конвертация)';en='Error in the ПередВыгрузкойДанных event handler (conversion)'"));
	одСообщения.Вставить(63, НСтр("es='La falta en el elaborador del acontecimiento PeredVygruzkoyDannyh (conversión)';ru='Ошибка в обработчике события ПередВыгрузкойДанных (конвертация)';en='Error in the ПередВыгрузкойДанных event handler (conversion)'"));
	одСообщения.Вставить(64, НСтр("es='La falta en el elaborador del acontecimiento PosleVygruzkiDannyh (conversión)';ru='Ошибка в обработчике события ПослеВыгрузкиДанных (конвертация)';en='Error in the event handler ПослеВыгрузкиДанных (conversion)'"));
	одСообщения.Вставить(65, НСтр("es=""La falta en el elaborador global del acontecimiento PeredKonvertatsieyOb'ekta (conversión)"";ru='Ошибка в глобальном обработчике события ПередКонвертациейОбъекта (конвертация)';en='Error in the global event handler ПередКонвертациейОбъекта (conversion)'"));
	одСообщения.Вставить(66, НСтр("es=""La falta en el elaborador global del acontecimiento PeredVygruzkoyOb'ekta (conversión)"";ru='Ошибка в глобальном обработчике события ПередВыгрузкойОбъекта (конвертация)';en='Error in the global event handler ПередВыгрузкойОбъекта  (conversion)'"));
	одСообщения.Вставить(67, НСтр("es='La falta de la recepción de la colección de los objetos subordinados de los datos entrantes';ru='Ошибка получения коллекции подчиненных объектов из входящих данных';en='Error retrieving a collection of subordinate objects from incoming data'"));
	одСообщения.Вставить(68, НСтр("es='La falta de la recepción de la propiedad del objeto subordinado de los datos entrantes';ru='Ошибка получения свойства подчиненного объекта из входящих данных';en='Error getting property of a slave object from incoming data'"));
	
	одСообщения.Вставить(69, НСтр("es=""La falta en el elaborador global del acontecimiento PosleVygruzkiOb'ekta (conversión)"";ru='Ошибка в глобальном обработчике события ПослеВыгрузкиОбъекта (конвертация)';en='Error in the global event handler ПослеВыгрузкиОбъекта (conversion)'"));
	
	одСообщения.Вставить(71, НСтр("es='de la Fuente';ru='Не найдено соответствие для значения Источника';en='No match found for Source value'"));
	
	одСообщения.Вставить(72, НСтр("es='La falta a la descarga de los datos para el nudo del plan del cambio';ru='Ошибка при выгрузке данных для узла плана обмена';en='Error while uploading data for exchange plan node'"));
	
	одСообщения.Вставить(73, НСтр("es='La falta en el elaborador del acontecimiento De PosledovatelnostPoleyPoiska';ru='Ошибка в обработчике события ПоследовательностьПолейПоиска';en='Error in the event handler ПоследовательностьПолейПоиска'"));
	
	одСообщения.Вставить(74, НСтр("es='Es necesario recargar las reglas del cambio para la descarga de los datos';ru='Необходимо перезагрузить правила обмена для выгрузки данных';en='Required to reload exchange rules to upload data'"));
	
	одСообщения.Вставить(75, НСтр("es='La falta a la ejecución del algoritmo después de la carga de los significados de los parámetros';ru='Ошибка при выполнении алгоритма после загрузки значений параметров';en='An error occurred while executing the algorithm after loading parameter values'"));
	
	одСообщения.Вставить(76, НСтр("es=""La falta en el elaborador del acontecimiento De PosleVygruzkiOb'ektaVFayl"";ru='Ошибка в обработчике события ПослеВыгрузкиОбъектаВФайл';en='Error in the event handler ПослеВыгрузкиОбъектаВФайл'"));
	
	одСообщения.Вставить(77, НСтр("es='del tratamiento exterior con los procedimientos conectados de los elaboradores de los acontecimientos';ru='Не указан файл внешней обработки с подключаемыми процедурами обработчиков событий';en='No external processing file specified with event handler hook procedures'"));
	
	одСообщения.Вставить(78, НСтр("es='La falta de la creación del tratamiento exterior del fichero con los procedimientos de los elaboradores de los acontecimientos';ru='Ошибка создания внешней обработки из файла с процедурами обработчиков событий';en='Error creating external processing from a file with event handler procedures'"));
	
	одСообщения.Вставить(79, НСтр("es='El código de los algoritmos no puede ser integrado en el elaborador por descubierto рекурсивного de la llamada de los algoritmos. "
"Si durante el ajuste no hay necesidad de ajustar el código de los algoritmos, indiquen el régimen ""no ajustar los algoritmos"" "
"Si es necesario cumplir el ajuste de los algoritmos con рекурсивным por la llamada, indiquen el régimen ""los algoritmos ajustar como los procedimientos"" "
"y repitan la descarga.';ru='Код алгоритмов не может быть интегрирован в обработчик из-за обнаруженного рекурсивного вызова алгоритмов. "
...
"и повторите выгрузку.';en='Algorithm code cannot be integrated into the handler due to a detected recursive call of algorithms."
"If during the debugging process there is no need to debug the algorithm code, then specify the mode ""do not debug algorithms"""
"If you need to debug algorithms with a recursive call, specify the mode ""to debug algorithms as procedures"""
"and retry uploading. '"));
	
	одСообщения.Вставить(80, НСтр("es='Se puede pasar el cambio para los datos solamente bajo los plenos derechos';ru='Обмен данными можно проводить только под полными правами';en='Data exchange can only be done under full rights.'"));
	
	одСообщения.Вставить(1000, НСтр("es='La falta a la creación del fichero temporal de la descarga de los datos';ru='Ошибка при создании временного файла выгрузки данных';en='Error creating temporary data upload file'"));
...
Функция ОпределитьДостаточностьПараметровДляПодключенияКИнформационнойБазе(СтруктураПодключения, СтрокаПодключения = "", СтрокаСообщенияОбОшибке = "")
	...
	Если СтруктураПодключения.ФайловыйРежим  Тогда
		...
		Если ПустаяСтрока(СтруктураПодключения.КаталогИБ) Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("es='de la base-receptor informativa';ru='Не задан каталог информационной базы-приемника';en='The destination infobase directory is not defined'");
	...
	Иначе
		...
		Если ПустаяСтрока(СтруктураПодключения.ИмяСервера) Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("es='de la base-receptor informativa';ru='Не задано имя сервера 1С:Предприятия информационной базы-приемника';en='The server name 1C: Enterprise information base-receiver is not set'");
		...
		Если ПустаяСтрока(СтруктураПодключения.ИмяИБНаСервере) Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("es='1С:Предприятия';ru='Не задано имя информационной базы-приемника на сервере 1С:Предприятия';en='The name of the information base-receiver on the server 1C: Enterprise is not set'");
...
Функция ПодключитсяКИнформационнойБазе(СтруктураПодключения, СтрокаСообщенияОбОшибке = "")
	...
	Исключение
		...
		СтрокаСообщенияОбОшибке = НСтр("es='Durante la tentativa de la unión con el COM-servidor había una falta siguiente: "
"%1';ru='При попытке соединения с COM-сервером произошла следующая ошибка:"
"%1';en='Following error occurred while trying to connect to the COM server:"
"%1'");
...
Функция ВыполнитьПодключениеКИБПриемнику() Экспорт
	...
	Исключение
		...
		Текст = НСтр("es='Durante la tentativa de la creación del tratamiento de UniversalnyyObmenDannymiXML había una falta: %1';ru='При попытке создания обработки УниверсальныйОбменДаннымиXML произошла ошибка: %1';en='An error occurred while trying to create processing УниверсальныйОбменДаннымиXML: %1'");
		Текст = ПодставитьПараметрыВСтроку(Текст, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		СообщитьПользователю(Текст);
		РезультатПодключения = Неопределено;
	КонецПопытки;
...
//
// Пример:
//  ПодставитьПараметрыВСтроку(НСтр("es='%1 ha ido en %2';ru='%1 пошел в %2';en='% 1 went to% 2'"), "Вася", "Зоопарк") = "Вася пошел в Зоопарк".
//
