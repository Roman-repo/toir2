...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_ОтключеноОбновлениеЧерезИнтернет", НСтр("es='(Es desconectada la renovación a través del Internet)';ru='(отключено обновление через интернет)';en='(internet update disabled)'"));
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_СоздаютсяНапрямуюВСправочнике", НСтр("es='Se crean directamente en la guía';ru='Создаются напрямую в справочнике';en='Created directly in the directory'"));
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_ЗагружаютсяИзФайла", НСтр("es='Son cargados del fichero';ru='Загружаются из файла';en='Download from file'"));
	ЭтотОбъект.ЛентыНовостей.Параметры.УстановитьЗначениеПараметра("Надпись_ЗагружаютсяЧерезИнтернетПоПротоколу", НСтр("es='Son cargados a través del Internet por la acta';ru='Загружаются через интернет по протоколу';en='Download via the Internet Protocol'"));

	ЭтотОбъект.СписокОбновляемыхДанных.Параметры.УстановитьЗначениеПараметра("Надпись_СписокЗначенийКлассификатора", НСтр("es='La lista de los significados del clasificador';ru='Список значений классификатора';en='List of Classifier Values'"));
	ЭтотОбъект.СписокОбновляемыхДанных.Параметры.УстановитьЗначениеПараметра("Надпись_ТребуетсяОбновление", НСтр("es='Es necesaria la renovación';ru='Требуется обновление';en='Update required'"));
	ЭтотОбъект.СписокОбновляемыхДанных.Параметры.УстановитьЗначениеПараметра("Надпись_ДанныеОтсутствуют", НСтр("es='Los datos no exigen la renovación';ru='Данные не требуют обновления';en='Data does not require updating'"));
	ЭтотОбъект.СписокОбновляемыхДанных.Параметры.УстановитьЗначениеПараметра("Надпись_ДанныеАктуальны", НСтр("es='Los datos son actuales';ru='Данные актуальны';en='Data relevant'"));
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы = Истина Тогда
		...
		Если ЭтотОбъект.Модифицированность Тогда
			Отказ = Истина;
			ТекстПредупреждения = НСтр("es='Los datos no son conservados. "
"Presionen el botón [Anotar y cerrar] "
"o cierren la ventana a mano, hasta la salida del programa.';ru='Данные не сохранены."
"Нажмите кнопку [Записать и закрыть]"
"или закройте окно вручную, до выхода из программы.';en='Data not saved."
"Click the [Записать и закрыть] button"
"or close the tab manually before exiting the program.'");
		КонецЕсли;
	Иначе
	КонецЕсли;
...
Процедура ДекорацияУдалитьНовостиОбработкаНавигационнойСсылки(Элемент, лкНавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если ВРег(лкНавигационнаяСсылка) = ВРег("DeleteNow") Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("es='¿Quitar las novedades que han caducado?';ru='Удалить устаревшие новости?';en='Delete outdated news?'"),
			РежимДиалогаВопрос.ДаНет,
			0,
			КодВозвратаДиалога.Нет,
			НСтр("es='La desaparición de las novedades que han caducado a mano';ru='Удаление устаревших новостей вручную';en='Manual deleting of obsolete news'"));
	КонецЕсли;
...
Процедура КомандаЗагрузитьНовости(Команда)
	...
	ФормаСостоянияОперации = ОткрытьФорму(
		...
		Новый Структура("Заголовок, СостояниеОперации", 
			НСтр("es='La carga de las novedades';ru='Загрузка новостей';en='Download News'"),
			НСтр("es='Hay una carga de las novedades "
"Esto puede ocupar un tiempo...';ru='Происходит загрузка новостей"
"Это может занять некоторое время...';en='News is loading"
...
Процедура КомандаЗагрузитьСтандартныеСписки(Команда)
	...
	ФормаСостоянияОперации = ОткрытьФорму(
		"Обработка.УправлениеНовостями.Форма.ФормаСостоянияОперации",
		Новый Структура("Заголовок, СостояниеОперации", 
			НСтр("es='La renovación de los clasificadores del servidor';ru='Обновление классификаторов с сервера';en='Updating classifiers from the server'"),
			НСтр("es='Hay una renovación de los clasificadores del servidor "
"Esto puede ocupar un tiempo...';ru='Происходит обновление классификаторов с сервера"
"Это может занять некоторое время...';en='Classifiers are being updated from the server"
"This may take some time...'")),
		ЭтотОбъект,
	...
	Оповестить(
		...
		НСтр("es='Hay una renovación de los clasificadores del servidor "
"Esto puede ocupar un tiempo... "
""
"La Renovación de las fechas de la actualidad de los clasificadores';ru='Происходит обновление классификаторов с сервера"
"Это может занять некоторое время..."
""
"Обновление дат актуальности классификаторов';en='Classifiers are being updated from the server"
"This may take some time..."
""
"Updating dates of relevance of classifiers '"),
		ИдентификаторФормыСостоянияОперации);
...
"Esto puede ocupar un tiempo... "
""
"La Renovación de los clasificadores';ru='Происходит обновление классификаторов с сервера"
"Это может занять некоторое время..."
""
"Обновление классификаторов';en='Classifiers are being updated from the server"
"This may take some time..."
""
"Classifier update '"),
		ИдентификаторФормыСостоянияОперации);
...
"La renovación de los clasificadores sobre el cliente...';ru='Происходит обновление классификаторов с сервера"
"Это может занять некоторое время..."
""
"Данные получены."
"Обновление классификаторов на клиенте...';en='Classifiers are being updated from the server"
"This may take some time..."
""
"Data received."
"Updating classifiers on the client …'"),
		ИдентификаторФормыСостоянияОперации);
...
""
"Los Datos son recibidos. "
"La renovación de las cintas de las novedades sobre el cliente...';ru='Происходит обновление классификаторов с сервера"
"Это может занять некоторое время..."
""
"Данные получены."
"Обновление лент новостей на клиенте...';en='Classifiers are being updated from the server"
"This may take some time..."
""
"Data received."
"Updating news feeds on the client …'"),
		ИдентификаторФормыСостоянияОперации);
	Элементы.ЛентыНовостей.Обновить();
...
Процедура КомандаПолучитьОтладочнуюИнформациюОНовости(Команда)
	...
	Если НЕ ПустаяСтрока(ЭтотОбъект.УИННовости) Тогда
		...
		#Если ВебКлиент Тогда
			...
			ТекстовыйДокумент.Показать(НСтр("es='La información de ajuste sobre la novedad';ru='Отладочная информация о новости';en='News debugging information'"));
		...
		#Иначе
			...
			ТекстовыйДокумент = Новый ТекстовыйДокумент;
			ТекстовыйДокумент.УстановитьТекст(ИнформацияОНовости);
			ТекстовыйДокумент.Показать(НСтр("es='La información de ajuste sobre la novedad';ru='Отладочная информация о новости';en='News debugging information'"), ИмяВременногоФайла);
...
Процедура КомандаДиагностикаПолученияНовостей(Команда)
	...
	РезультатДиагностикиПолученияНовостей = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='"
"# la COMPROBACIÓN de la RECEPCIÓN de las NOVEDADES "
""
"la Fecha del informe: ** %1 **';ru='"
"# ПРОВЕРКА ПОЛУЧЕНИЯ НОВОСТЕЙ"
""
"Дата отчета: **%1**';en='"
"# NEWS         RECEIVING  CHECKING"
""
"Report date: **%1**'")
			+ Символы.ПС
			+ Символы.ПС,
	...
	Для Каждого ТекущийШаг Из СписокШагов Цикл
		Состояние(
			НСтр("es='La ejecución de la comprobación';ru='Выполнение проверки';en='Check execution'"),
...
"%2'")
	...
	#Если ВебКлиент Тогда
		...
		ТекстовыйДокумент.Показать(НСтр("es='El resultado de la diagnosis de la recepción de las novedades';ru='Результат диагностики получения новостей';en='News diagnostic result'"));
	...
	#Иначе
		...
		ТекстовыйДокумент = Новый ТекстовыйДокумент;
		ТекстовыйДокумент.УстановитьТекст(РезультатДиагностикиПолученияНовостей);
		ТекстовыйДокумент.Показать(НСтр("es='El resultado de la diagnosis de la recepción de las novedades';ru='Результат диагностики получения новостей';en='News diagnostic result'"), ИмяВременногоФайла);
...
Процедура УправлениеФормой(Форма)
	...
	Если лкРазрешенаРаботаСНовостямиЧерезИнтернет = Истина Тогда
		Элементы.РазрешенаРаботаСНовостямиЧерезИнтернет.Подсказка = НСтр("es='Será comprobada regularmente la presencia de las nuevas novedades.';ru='Будет регулярно проверяться наличие новых новостей.';en='New news will be regularly checked.'");
		Элементы.РазрешенаРаботаСНовостямиЧерезИнтернет.РасширеннаяПодсказка.ЦветТекста = Форма.ЦветМикротекста; // По-умолчанию.
	Иначе
		Элементы.РазрешенаРаботаСНовостямиЧерезИнтернет.Подсказка = НСтр("es='La comprobación de las nuevas novedades a través del Internet es desconectada. "
"Las novedades, incluso importante, no obrarán.';ru='Проверка новых новостей через интернет отключена."
"Новости, включая важные, не будут поступать.';en='Checking for new news over the Internet is disabled."
"News, including important ones, will not be received. '");
...
Процедура УстановитьУсловноеОформление()
	...
	// 1. Частота обновления = 0 (обновлять вручную), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='Renovar solamente a mano';ru='Обновлять только вручную';en='Update manually only'"));
	...
	// 2. Частота обновления = 1 (Ежедневно), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='Cada día';ru='Ежедневно';en='Daily'"));
	...
	// 3. Частота обновления = 2 (Ежечасно), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='Cada hora';ru='Ежечасно';en='Hourly'"));
	...
	// 4. Частота обновления = 3 (Каждые 15 минут), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='Cada 15 minutos';ru='Каждые 15 минут';en='Every 15 minutes'"));
	...
	// 5. Частота обновления = 4 (Каждую минуту), и это НЕ локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='Cada minuto';ru='Каждую минуту';en='Every minute'"));
	...
	// 6. Частота обновления = не требуется, т.к. это локальная лента новостей.
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='No es necesario';ru='Не требуется';en='Not required'"));
	...
	// 7. Дата последнего обновления = пустая, то вывести текст "никогда не обновлялся".
		...
		// Оформление
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='Se renovaba nunca';ru='никогда не обновлялся';en='never updated'"));
	...
	// 12. "Требуется обновление" - красным, "Данные актуальны", "Данные не требуют обновления" - серым;
		...
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = НСтр("es='Los datos no exigen la renovación';ru='Данные не требуют обновления';en='Data does not require updating'");
...
Процедура ПослеОтветаУдалениеНовостей(РезультатВопроса, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El resultado de la ejecución: "
"%РезультатВыполнения del %';ru='Результат выполнения:"
"%РезультатВыполнения%';en='Execution result:"
"%РезультатВыполнения% '"),
			РезультатВыполнения);
...
Функция ВыполнитьШагПроверкиНаСервере(Шаг, ПрочиеПараметры)
	...
	ИначеЕсли Шаг = 5 Тогда
		...
		Если ПустаяСтрока(Результат) Тогда
			Результат = Символы.Таб + НСтр("es='El subsistema de la Novedad es introducido correctamente';ru='Подсистема Новости внедрена правильно';en='News subsystem implemented correctly'")
				+ Символы.ПС;
		КонецЕсли;
...
Функция ВыполнитьШагПроверки_ЗаполненностьОбъектовМетаданных()
	...
	Результат = Результат
		+ "  " + НСтр("es='Las categorías de las novedades (los primeros 20 significados):';ru='Категории новостей (первые 20 значений):';en='News categories (first 20 values):'")
			+ Символы.ПС;
	...
	Иначе
		Результат = Результат
			+ "    " + НСтр("es='No hay datos';ru='Нет данных';en='No data'")
				+ Символы.ПС;
	КонецЕсли;

	Результат = Результат + Символы.ПС;

	Результат = Результат + "  " + НСтр("es='Los significados de las categorías de las novedades (los primeros 20 significados):';ru='Значения категорий новостей (первые 20 значений):';en='News category values (first 20 values):'")
		+ Символы.ПС;
	Если НЕ РезультатЗапроса[1].Пустой() Тогда
		...
		КонецЦикла;
	Иначе
		Результат = Результат + "    " + НСтр("es='No hay datos';ru='Нет данных';en='No data'")
			+ Символы.ПС;
	КонецЕсли;

	Результат = Результат + Символы.ПС;

	Результат = Результат + "  " + НСтр("es='Las cintas de las novedades (los primeros 20 significados):';ru='Ленты новостей (первые 20 значений):';en='News feeds (first 20 values):'")
		+ Символы.ПС;
...
#Область АдминистративныеОтборы
	...
	Результат = Результат
		+ "###" + НСтр("es='Las selecciones ajustadas por el administrador (los primeros 40 significados):';ru='Отборы, настроенные администратором (первые 40 значений):';en='Selections configured by the administrator (first 40 values):'")
		+ Символы.ПС
		+ Список
		+ Символы.ПС;
...
#Область ОтборыДляОбластиДанных
	...
	Результат = Результат
		+ "###" + НСтр("es='Las selecciones ajustadas para las áreas de los datos (los primeros 40 significados):';ru='Отборы, настроенные для областей данных (первые 40 значений):';en='Samples configured for data areas (first 40 values):'")
		+ Символы.ПС
		+ Список
		+ Символы.ПС;
...
#Область ПользовательскиеОтборы
	...
	Результат = Результат
		+ "###" + НСтр("es='Las selecciones ajustadas por los usuarios (los primeros 40 significados):';ru='Отборы, настроенные пользователями (первые 40 значений):';en='User-configured selections (first 40 values):'")
		+ Символы.ПС
		+ Список
		+ Символы.ПС;
...
"%1 "
"Área de los datos (es entregada como el parámetro): %2 "
"área de los datos Escogida (es usada para los cálculos): %3 "
"Parámetro del lanzamiento/C: %4';ru='Параметры окружения подсистемы новостей:"
"%1"
"Область данных (передана как параметр): %2"
"Выбранная область данных (использована для расчетов): %3"
"Параметр запуска /C: %4';en='Environment options for the news subsystem:"
"%1"
"Data area (passed as parameter):%2"
"Selected data area (used for calculations):%3"
"Startup option / C:%4 '"),
...
Функция СформироватьПолныйАдресЛентыНовостей(лкСсылка, лкПротокол, лкСайт, лкИмяФайла)
	...
	Если СтрНайти(ВРег(лкПротокол), ВРег("http")) = 1 Тогда // http, https
		Если ПустаяСтрока(лкСайт) Тогда
			Результат = НСтр("es='el Sitio';ru='Не заполнено поле Сайт';en='The Site field is not filled'");
		ИначеЕсли ПустаяСтрока(лкИмяФайла) Тогда
			Результат = НСтр("es='del fichero';ru='Не заполнено поле Имя файла';en='File Name field is not filled'");
		...
		КонецЕсли;
	Иначе
		Результат = НСтр("es='Se puede recibir la dirección de la cinta de las novedades solamente para las actas http y https';ru='Адрес ленты новостей можно получить только для протоколов http и https';en='The feed address can only be obtained for the http and https protocols'");
	КонецЕсли;
...
#Область ОченьВажныеНовости
	...
	Если (ТипЗнч(ОченьВажныеНовостиСВключеннымиНапоминаниями) = ТипМассив)
		...
		КонецЦикла;
	Иначе
		Список = НСтр("es='Vacío.';ru='Пустой.';en='Empty.'")
			+ Символы.ПС;
	КонецЕсли;
	...
	Результат = Результат
		+ "###" + НСтр("es='La lista de las novedades muy importantes:';ru='Список очень важных новостей:';en='List of very important news:'")
		+ Символы.ПС
		+ Список;
...
#Область ВажныеНовости
	...
	Результат = Результат
		+ "###" + НСтр("es='La lista de las novedades importantes:';ru='Список важных новостей:';en='List of important news:'")
		+ Символы.ПС
		+ Список;
...
Функция ВыполнитьШагПроверки_ПроверкаХТТПСоединения()
	...
	Результат = Результат
		+ НСтр("es='La recepción de las novedades por primero algunas cintas de las novedades:';ru='Получение новостей по первым нескольким лентам новостей:';en='Receive news on the first few news feeds:'")
		+ Символы.ПС;
				...
				Если ВременныйФайл.Существует() Тогда
					...
					РезультатШага = РезультатШага
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='%1Успешное la recepción del fichero de las novedades. "
"%1Код HTTP de la respuesta: %2 "
"%1Заголовки HTTP de la respuesta: "
"%3 "
"%1Имя del fichero temporal: %4 "
"%1Размер del fichero (el byte): %5 "
"%1Первые las líneas del fichero: "
"%6';ru='%1Успешное получение файла новостей."
"%1Код HTTP ответа: %2"
...
"%6';en='%1 Successfully retrieving a news file."
"%1 HTTP response code:% 2"
"%1 HTTP response headers:"
"%3"
"%1 Temporary file name:% 4"
"%1File size (bytes):% 5"
"% 1First lines of a file:"
"%6'")
								+ Символы.ПС
								+ Символы.ПС
								+ Символы.ПС,
				...
				Иначе
					РезультатШага = РезультатШага
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='%1Неудачная la tentativa de la recepción del fichero de las novedades. "
"%1Код HTTP de la respuesta: %2 "
"%1Заголовки HTTP de la respuesta: "
"%3 "
"%1Имя del fichero temporal: %4';ru='%1Неудачная попытка получения файла новостей."
"%1Код HTTP ответа: %2"
"%1Заголовки HTTP ответа:"
"%3"
"%1Имя временного файла: %4';en='%1 Failed to retrieve the news file."
"%1 HTTP response code:% 2"
"%1 HTTP response headers:"
"%3"
"%1 Temporary file name:% 4'")
			...
			Исключение
				РезультатШага = РезультатШага
					+ "    "
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La FALTA a la recepción de las novedades: "
"%1';ru='ОШИБКА при получении новостей:"
"%1';en='ERROR while receiving news:"
"%1'")
							+ Символы.ПС,
		...
		КонецЦикла;
	Иначе
		Результат = Результат + "    " + НСтр("es='No hay cintas convenientes de las novedades';ru='Нет подходящих лент новостей';en='No matching feeds'")
			+ Символы.ПС;
	КонецЕсли;
...
Функция ВыполнитьШагПроверки_ПроверкаСвободногоМестаВоВременномКаталоге()
	...
	Результат = Результат
		+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La comprobación automática no es realizada. "
"Es necesario a mano comprobar, si basta el lugar para el almacenaje de los ficheros por la vía siguiente: "
"%1 "
"es necesario recordar También que en caso del uso del clúster de los servidores, la recepción de las novedades puede realizarse"
" sobre cualquier de los servidores, por eso comprobar el lugar libre es necesario en todos los servidores.';ru='Автоматическая проверка НЕ реализована."
...
" на любом из серверов, поэтому проверять свободное место необходимо на всех серверах.';en='Automatic verification is NOT implemented."
"you must manually check if there is enough space to store the files in the following way:"
"%1"
"It is also necessary to remember that in the case of using a server cluster, news can be received"
"  on any of the servers, so you need to check the free space on all servers.'"),
			ИмяВременногоФайла);
...
Функция ВыполнитьШагПроверки_НастройкиОткрытыхФорм()
	...
	Для Каждого ТекущаяИнформация Из ЭтотОбъект.ИнформацияОбОткрытыхОкнах Цикл
		Если ТипЗнч(ТекущаяИнформация.Значение) = ТипСтрока Тогда // Ошибка
			Результат = Результат
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El nombre de la forma: %1 "
"Falta de la recepción de los datos del requisito de la Novedad: "
"%2';ru='Имя формы: %1"
"Ошибка получения данных реквизита Новости:"
"%2';en='Form name:%1"
"Error receiving data details News:"
"%2 '")
						+ Символы.ПС,
		...
		ИначеЕсли ТипЗнч(ТекущаяИнформация.Значение) = ТипСтруктура Тогда // Новости
			...
			Если ТекущаяИнформация.Значение.Свойство("Новости") Тогда
				Если ТипЗнч(ТекущаяИнформация.Значение.Новости)= ТипМассив Тогда
					Если ТекущаяИнформация.Значение.Новости.Количество() > 0 Тогда
						...
						Для Каждого ТекущаяНовость Из ТекущаяИнформация.Значение.Новости Цикл
							...
							СписокПервыхНовостей = СписокПервыхНовостей
								...
								+ ?(ТекущаяНовость.Прочтена, НСтр("es='Es leída';ru='Прочтена';en='Read'"), НСтр("es='No es leído';ru='НЕ прочтена';en='NOT read'")) + "; "
								+ Символы.ПС;
							С = С + 1;
							Если С >= 5 Тогда
								Прервать;
					...
					Иначе
						СписокПервыхНовостей = НСтр("es='La lista vacía de las novedades de contexto.';ru='Пустой список контекстных новостей.';en='An empty list of contextual news.'");
					КонецЕсли;
				Иначе
					СписокПервыхНовостей = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='En el requisito de la Novedad la llave del macizo de las novedades de contexto de las Novedades del tipo incorrecto: %1.';ru='В реквизите Новости ключ массива контекстных новостей Новостей неправильного типа: %1.';en='In the requisite News, the key to the array of contextual News news of the wrong type:% 1.'"),
						ТипЗнч(ТекущаяИнформация.Значение.Новости));
				КонецЕсли;
			Иначе
				СписокПервыхНовостей = НСтр("es='En el requisito de la Novedad falta la llave del macizo de las novedades de contexto de las Novedades.';ru='В реквизите Новости отсутствует ключ массива контекстных новостей Новостей.';en='In the requisite News there is no key for the array of contextual News news.'");
			КонецЕсли;
			Результат = Результат
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El nombre de la forma: %1"
"	Los Metadatos: %2"
"	La Forma: %3"
"	ZagolovokFormyKontekstnyhNovostey: %4"
"	Todo de las novedades de contexto: %5"
"	Algunas novedades casuales de contexto: "
"%6"
"	PoluchatNovostiNaServere: %7"
"	HranitMassivNovosteyTolkoNaServere: %8';ru='Имя формы: %1"
"	Метаданные: %2"
"	Форма: %3"
...
"	ХранитьМассивНовостейТолькоНаСервере: %8';en='Form name: %1"
"Metadata: %2"
"Form: %3 ЗаголовокФормыКонтекстныхНовостей: %4 Total contextual news: %5"
"Some random contextual news:"
"%6 ПолучатьНовостиНаСервере: %7"
"	ХранитьМассивНовостейТолькоНаСервере: %8'")
		...
		Иначе // Неизвестные данные
			Результат = Результат
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El nombre de la forma: %1 "
"Falta de la recepción de los datos del requisito de la Novedad. El tipo de datos incorrecto: %2';ru='Имя формы: %1"
"Ошибка получения данных реквизита Новости. Неправильный тип данных: %2';en='Form name:%1"
"Error receiving data requisites News. Invalid data type:%2'")
						+ Символы.ПС,
					ТекущаяИнформация.Представление,
