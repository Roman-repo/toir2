...
Процедура ЗагрузитьФайлыНовостейССервера(ЛентыНовостей, КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("es='La carga de los ficheros de las novedades';ru='Загрузка файлов новостей';en='Downloading news files'"));
			...
			Если НЕ ПустаяСтрока(Результат) Тогда
				...
				ЛогШаговВыполненияВложенный.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Las faltas de la introducción del subsistema de la Novedad: "
"%1';ru='Ошибки внедрения подсистемы Новости:"
"%1';en='Errors in the implementation of the News subsystem:"
"%1'"),
						Результат));
			КонецЕсли;
			...
			Если ЕстьОшибкиВнедрения =Истина Тогда
				Для Каждого ТекущаяЛентаНовостей Из ЛентыНовостей Цикл
					Если СтрНайти(ВРег(ТекущаяЛентаНовостей.ИмяФайла), ВРег("[config]")) > 0 Тогда
						...
						ЛогШаговВыполненияВложенный.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La cinta de las novedades [%1] es excluida de la carga por las faltas de la introducción. El nombre del fichero en el servidor: %2';ru='Лента новостей [%1] исключена из загрузки из-за ошибок внедрения. Имя файла на сервере: %2';en='The feed [%1] was excluded from download due to deployment errors. Server file name:%2'"),
...
#Область ЗагрузкаФайлов
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			Для каждого ТекущееХТТПСоединение Из ТаблицаХТТПСоединений Цикл
				...
				ЛогШаговВыполненияВложенный.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El tratamiento: %1, %2';ru='Обработка: %1, %2';en='Processing:%1,%2'"),
								...
								Если МассивСлов.Количество() = 5 Тогда
									...
									Исключение
										...
										ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("es='La falta de la recepción de las novedades de los metadatos %1 para la cinta de las novedades %2 por razón de "
"%3';ru='Ошибка получения новостей из метаданных %1 для ленты новостей %2 по причине"
"%3';en='Error retrieving news from metadata%1 for news feed%2 due to"
"%3'"),
											ТекущаяЛентаНовостей.ИмяФайла,
											ТекущаяЛентаНовостей.ЛентаНовостей,
										...
										Попытка
											Если ТипЗнч(ОбъектМетаданныхМакет) = ТипОбъектМетаданных Тогда
												...
												Иначе
													Макет = Неопределено;
													ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
														НСтр("es='La falta de la recepción de las novedades de la maqueta %1 para la cinta de las novedades %2 - estos metadatos no son tratados';ru='Ошибка получения новостей из макета %1 для ленты новостей %2 - эти метаданные не обрабатываются';en='Error retrieving news from layout %1 for news feed %2 - this metadata is not processed'"),
														ТекущаяЛентаНовостей.ИмяФайла,
														ТекущаяЛентаНовостей.ЛентаНовостей);
												...
												Если Макет <> Неопределено Тогда
													Если ОбъектМетаданныхМакет.ТипМакета = Метаданные.СвойстваОбъектов.ТипМакета.ТекстовыйДокумент Тогда
														...
														Исключение
															...
															ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
																НСтр("es='ЗагрузитьФайлыНовостейССервера (es recibido de la maqueta): no consiguió quitar el fichero temporal %1 para la cinta de las novedades %2 por la causa: "
"%3 "
""
"se Recomienda a mano quitar este fichero temporal.';ru='ЗагрузитьФайлыНовостейССервера (получено из макета): Не удалось удалить временный файл %1 для ленты новостей %2 по причине:"
"%3"
""
"Рекомендуется вручную удалить этот временный файл.';en='ЗагрузитьФайлыНовостейССервера (obtained from the layout): Failed to delete temporary file%1 for news feed%2 due to:"
"%3"
""
"It is recommended that you manually delete this temporary file.'"),
													...
													Иначе
														ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
															НСтр("es='La falta de la recepción de las novedades de la maqueta %1 para la cinta de las novedades %2: es posible la carga solamente de la maqueta textual';ru='Ошибка получения новостей из макета %1 для ленты новостей %2: возможна загрузка только из текстового макета';en='Error retrieving news from layout%1 for news feed%2: loading only from a text layout is possible'"),
															ТекущаяЛентаНовостей.ИмяФайла,
															ТекущаяЛентаНовостей.ЛентаНовостей);
										...
										Исключение
											ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("es='La falta de la recepción de las novedades de la maqueta %1 para la cinta de las novedades %2 por la causa: "
"%3';ru='Ошибка получения новостей из макета %1 для ленты новостей %2 по причине:"
"%3';en='Error retrieving news from layout%1 for news feed%2 due to:"
"%3'"),
												ТекущаяЛентаНовостей.ИмяФайла,
												ТекущаяЛентаНовостей.ЛентаНовостей,
												ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
						...
						Исключение
							ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La falta de la recepción del fichero %1 para la cinta de las novedades %2 por la causa: "
"%3';ru='Ошибка получения файла %1 для ленты новостей %2 по причине:"
"%3';en='Error getting file%1 for news feed%2 due to:"
"%3'"),
								ТекущаяЛентаНовостей.ИмяФайла,
								ТекущаяЛентаНовостей.ЛентаНовостей,
								ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
						...
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='La carga: la carga del fichero de las novedades de la cinta de las novedades %1 (la acta %2, el sitio %3, el nombre del fichero %4). "
"¿Con éxito? %5 "
"Tiempo de la ejecución (мс): %6';ru='Загрузка: загрузка файла новостей из ленты новостей %1 (протокол %2, сайт %3, имя файла %4)."
"Успешно? %5"
"Время выполнения (мс): %6';en='Downloading: downloading the news file from the news feed%1 (protocol%2, site%3, file name%4)."
"Successfully? %5"
"Execution time (ms):%6'"),
							ТекущаяЛентаНовостей.ЛентаНовостей,
...
#Область ЗагрузкаФайлов_ХТТП
					...
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La carga: la instalación http/https las uniones (acta %1). "
"¿Con éxito? %2';ru='Загрузка: установка http/https соединения (протокол %1)."
"Успешно? %2';en='Download: Establish an http / https connection (protocol%1)."
"Successfully? %2'"),
						ТекущееХТТПСоединение.Протокол,
						(ЕстьОшибки = Ложь));
					...
					Если ЕстьОшибки = Ложь Тогда
						...
						Для каждого ТекущаяЛентаНовостей Из МассивОбрабатываемыхЛентНовостей Цикл
							...
							ПричинаРегламентныхРабот   = НСтр("es='La causa es desconocida';ru='Причина неизвестна';en='Reason unknown'");
							...
							Пока (ФайлПолучен <> Истина) И (ПопыткаПолученияФайла < НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокПолученияФайла) Цикл
								Попытка
									...
									// Непосредственно загрузка файла, начало.
										...
										ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("es='El comienzo de la tentativa %1 / %2 recepciones del fichero %3 del servidor %4 para la cinta de las novedades %5 en el fichero temporal %6. "
"(URL %7).';ru='Начало попытки %1 / %2 получения файла %3 с сервера %4 для ленты новостей %5 во временный файл %6."
"(URL %7).';en='Start of attempt%1 /%2 to get file%3 from server%4 for news feed%5 to temporary file%6."
"(URL%7). '"), 
										...
										КонецЦикла;
										ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("es='Durante la tentativa %1 / %2 recepciones del fichero %3 del servidor %4 para la cinta de las novedades %5 en el fichero temporal es recibida la respuesta del servidor con el código %6, los encabezamientos: "
"%7 "
""
"(URL %8) "
"el Tiempo de la carga (сек): %9';ru='При попытке %1 / %2 получения файла %3 с сервера %4 для ленты новостей %5 во временный файл получен ответ сервера с кодом %6, заголовки:"
"%7"
""
"(URL %8)"
"Время загрузки (сек): %9';en='When trying to%1 /%2 to receive file%3 from server%4 for news feed%5, a server response with code%6 was received in a temporary file, headers:"
"%7"
""
"(URL%8)"
"Download time (s):%9 '"),
										...
										Если ВременныйФайл.Существует() Тогда
											ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("es='La terminación exitosa de la tentativa %1 / %2 recepciones del fichero %3 del servidor %4 para la cinta de las novedades %5 en el fichero temporal %6. "
"Es recibido el fichero de la dimensión de %7 bytes. "
"(URL %8) "
""
"las Primeras líneas del fichero: "
"%9';ru='Успешное завершение попытки %1 / %2 получения файла %3 с сервера %4 для ленты новостей %5 во временный файл %6."
"Получен файл размером %7 байт."
"(URL %8)"
""
"Первые строки файла:"
"%9';en='Successfully attempting%1 /%2 to retrieve file%3 from server%4 for feed%5 to temporary file%6."
"Received file size of%7 bytes."
"(URL%8)"
""
...
"%9'"),
											...
											Если (ХттпОтвет.КодСостояния >= 500) И (ХттпОтвет.КодСостояния <= 599) Тогда
												...
												Исключение
													...
													ТекстХТМЛРегламентныхРабот = "";
													ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
														НСтр("es='ЗагрузитьФайлыНовостейССервера (es recibido del Internet): no consiguió recibir el texto de la falta del fichero temporal %1 para la cinta de las novedades %2 por la causa: "
"%3';ru='ЗагрузитьФайлыНовостейССервера (получено из интернет): Не удалось получить текст ошибки из временного файла %1 для ленты новостей %2 по причине:"
"%3';en='ЗагрузитьФайлыНовостейССервера (obtained from the Internet): Failed to get error text from temporary file%1 for news feed%2 due to:"
"%3'"),
														СокрЛП(ИмяВременногоФайла),
														ТекущаяЛентаНовостей.ЛентаНовостей,
														ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
													...
													ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
														...
														НСтр("es='Las novedades. La carga de las novedades. La carga de los ficheros de las novedades. La falta de la recepción del texto de la falta del servidor';ru='Новости. Загрузка новостей. Загрузка файлов новостей. Ошибка получения текста ошибки сервера';en='News. Download news. Download news files. Error getting server error text'"), // ИдентификаторШага
											...
											Исключение
												ИнформацияОбОшибке = ИнформацияОбОшибке();
												ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
													НСтр("es='ЗагрузитьФайлыНовостейССервера (es recibido del Internet): no consiguió quitar el fichero temporal %1 para la cinta de las novedades %2 por la causa: "
"%3 "
""
"se Recomienda a mano quitar este fichero temporal.';ru='ЗагрузитьФайлыНовостейССервера (получено из интернет): Не удалось удалить временный файл %1 для ленты новостей %2 по причине:"
"%3"
""
"Рекомендуется вручную удалить этот временный файл.';en='ЗагрузитьФайлыНовостейССервера (obtained from the Internet):Failed to delete temporary file%1 for feed%2 due to:"
"%3"
""
"It is recommended that you manually delete this temporary file.'"),
													СокрЛП(ИмяВременногоФайла),
										...
										Иначе
											ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("es='La terminación desgraciada de la tentativa %1 / %2 recepciones del fichero %3 del servidor %4 para la cinta de las novedades %5 en el fichero temporal %6 - después de la tentativa de la carga, el fichero no existe todavía. "
"(URL %7).';ru='Неудачное завершение попытки %1 / %2 получения файла %3 с сервера %4 для ленты новостей %5 во временный файл %6 - после попытки загрузки, файла все еще не существует."
"(URL %7).';en='Failed to try%1 /%2 to retrieve file%3 from server%4 for news feed%5 to temporary file%6 - after trying to download, the file still does not exist."
"(URL%7).'"),
												ПопыткаПолученияФайла,
												НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокПолученияФайла,
												ТекущаяЛентаНовостей.ИмяФайла,
									...
									ИначеЕсли (ХттпОтвет.КодСостояния >= 500) И (ХттпОтвет.КодСостояния <= 599) Тогда
										...
										Если НЕ ПустаяСтрока(ТекстХТМЛРегламентныхРабот) Тогда
											...
											Исключение
												ИнформацияОбОшибке = ИнформацияОбОшибке();
												ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
													НСтр("es='ЗагрузитьФайлыНовостейССервера (es recibido del Internet): no consiguió распарсить el texto de la falta para la cinta de las novedades %1 por la causa: "
"%2 "
""
"Texto de la falta: "
"%3';ru='ЗагрузитьФайлыНовостейССервера (получено из интернет): Не удалось распарсить текст ошибки для ленты новостей %1 по причине:"
"%2"
""
"Текст ошибки:"
"%3';en='ЗагрузитьФайлыНовостейССервера (obtained from the Internet):Failed to parse error text for news feed%1 due to:"
"%2"
""
"Error text:"
"%3'"),
												...
												ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
													НСтр("es='БИП:Новости. La carga de las novedades';ru='БИП:Новости.Загрузка новостей';en='BIP: News. Download News'"), // ИмяСобытия
													НСтр("es='Las novedades. La carga de las novedades. La carga de los ficheros de las novedades. La falta del desciframiento del texto de la falta del servidor';ru='Новости. Загрузка новостей. Загрузка файлов новостей. Ошибка расшифровки текста ошибки сервера';en='News. Download news. Download news files. Error decrypting server error text'"), // ИдентификаторШага
													УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
													, // ОбъектМетаданных
													, // Данные
													ТекстСообщения, // Комментарий
											...
											ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("es='La carga: la falta de la carga del fichero de las novedades de la cinta de las novedades %1. "
"El código del regreso: %2 "
"Causa: %3 "
"Fecha del comienzo de los trabajos de reglamento: %4 "
"Fecha de la terminación de los trabajos de reglamento: %5 "
"Texto del regreso: "
"%6';ru='Загрузка: ошибка загрузки файла новостей из ленты новостей %1."
"Код возврата: %2"
"Причина: %3"
"Дата начала регламентных работ: %4"
"Дата завершения регламентных работ: %5"
"Текст возврата:"
"%6';en='Downloading: error loading news file from news feed%1."
"Return Code:%2"
"Reason:%3"
"Maintenance Start Date:%4"
"Date of completion of scheduled maintenance:%5"
"Return text:"
"%6'"),
												ТекущаяЛентаНовостей.ЛентаНовостей,
												ХттпОтвет.КодСостояния,
								...
								Исключение
									ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("es='La tentativa %1 / %2. La falta de la recepción del fichero %3 del servidor %4 para la cinta de las novedades %5 en el fichero temporal por la causa: "
"%6';ru='Попытка %1 / %2. Ошибка получения файла %3 с сервера %4 для ленты новостей %5 во временный файл по причине:"
"%6';en='Attempt %1 /%2. Error getting file%3 from server%4 for news feed%5 to a temporary file due to:"
"%6'"),
										ПопыткаПолученияФайла,
										НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокПолученияФайла,
										ТекущаяЛентаНовостей.ИмяФайла,
							...
							ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La carga: la carga del fichero de las novedades de la cinta de las novedades %1 (la acta %2, el sitio %3, el nombre del fichero %4, el nombre del fichero en el servidor %5). "
"¿Con éxito? %6 "
"Tiempo de la ejecución (мс): %7';ru='Загрузка: загрузка файла новостей из ленты новостей %1 (протокол %2, сайт %3, имя файла %4, имя файла на сервере %5)."
"Успешно? %6"
"Время выполнения (мс): %7';en='Downloading: downloading news file from news feed%1 (protocol%2, site%3, file name%4, file name on server%5)."
"Successfully? %6"
"Execution time (ms):%7'"),
								ТекущаяЛентаНовостей.ЛентаНовостей,
								ТекущаяЛентаНовостей.Протокол,
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. La carga de las novedades';ru='БИП:Новости.Загрузка новостей';en='BIP: News. Download News'"), // ИмяСобытия
			НСтр("es='Las novedades. La carga de las novedades. La carga de los ficheros de las novedades';ru='Новости. Загрузка новостей. Загрузка файлов новостей';en='News. Download news. Download news files'"), // ИдентификаторШага
			?(КодРезультата > 0,
...
Процедура ПарсингХМЛФайловНовостей(ЛентыНовостей, КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='Парсинг de los ficheros de las novedades';ru='Парсинг файлов новостей';en='Parsing news files'"));
...
#Область ОсновныеОбработки
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
				+ НСтр("es='La conservación de la fecha de las últimas cargas';ru='Сохранение даты последних загрузок';en='Save last upload date'")
				+ Символы.ПС;
			...
			// Прочитать файлы
			ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
				+ НСтр("es='El tratamiento de las cintas de las novedades:';ru='Обработка лент новостей:';en='Processing news feeds:'")
				+ Символы.ПС;
			Для каждого ТекущаяСтрока Из ЛентыНовостей Цикл

				ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La cinta: %1, obligatorio: %2, la marca: %3';ru='Лента: %1, обязательная: %2, пометка: %3';en='Feed:%1, required:%2, tagged:%3'"),
				...
				Если (ТекущаяСтрока.Пометка) ИЛИ (ТекущаяСтрока.ОбязательныйКанал) Тогда
					...
					ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
						+ НСтр("es='Es tratado...';ru='Обрабатывается...';en='Processing ...'")
						+ Символы.ПС;
					...
					Если НЕ ПустаяСтрока(ТекущаяСтрока.АдресВХранилище) Тогда
						...
						Если Файл.Существует() Тогда
							...
							Исключение
								...
								ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("es='En el fichero cargado para la cinta de las novedades %1 se encuentra el texto no en el formato xml. La cinta de las novedades será dejada pasar. "
""
"La representación detallada de la falta: "
"%2 "
""
"Primeras líneas del fichero: "
"%3';ru='В загруженном файле для ленты новостей %1 находится текст не в формате xml. Лента новостей будет пропущена."
""
"Подробное представление ошибки:"
"%2"
""
"Первые строки файла:"
"%3';en='The downloaded file for news feed% 1 contains non-xml text. The feed will be skipped."
""
"Detailed error view:"
"% 2"
""
							...
							Если ЕстьОшибки = Ложь Тогда
								...
								ИначеЕсли (НРег(ХМЛТип.ИмяТипа) = "rss") Тогда // И (ВРег(ХМЛТип.URIПространстваИмен)=ВРег("http://www.w3.org/2005/RSS_2_0")).
									...
									Если (ОбъектХДТО.Свойства().Получить("version") = Неопределено) Тогда
										ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("es='Es imposible determinar la versión rss en la cinta de las novedades %1 - falta la propiedad version. Son necesarias las versiones, dadas en el formato, 2.0.';ru='Невозможно определить версию rss в ленте новостей %1 - отсутствует свойство version. Необходимы данные в формате версии 2.0.';en='Unable to determine rss version in news feed%1 - version property is missing. Data is required in version 2.0 format.'"),
											ТекущаяСтрока.ЛентаНовостей);
									...
									Иначе
										...
										Если (Число(лкВерсия) <> 2.0) Тогда
											ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("es='La versión no apoyada de las novedades RSS %1 en la cinta de las novedades %2';ru='Неподдерживаемая версия новостей RSS %1 в ленте новостей %2';en='Unsupported RSS feed version%1 in feed%2'"),
												ОбъектХДТО.version,
												ТекущаяСтрока.ЛентаНовостей);
								...
								Иначе
									...
									ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("es='El tipo no apoyado de las novedades %1 en el fichero %2 para la cinta de las novedades %3 "
""
"Primeras líneas del fichero: "
"%4';ru='Неподдерживаемый тип новостей %1 в файле %2 для ленты новостей %3"
""
"Первые строки файла:"
"%4';en='Unsupported news type%1 in file%2 for news feed%3"
""
"The first lines of the file:"
"%4'"),
							...
							Исключение
								ИнформацияОбОшибке = ИнформацияОбОшибке();
								ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("es='ПарсингХМЛФайловНовостей: no consiguió quitar el fichero temporal %1 para la cinta de las novedades %2 por la causa: "
"%3 "
""
"se Recomienda a mano quitar este fichero temporal.';ru='ПарсингХМЛФайловНовостей: Не удалось удалить временный файл %1 для ленты новостей %2 по причине:"
"%3"
""
"Рекомендуется вручную удалить этот временный файл.';en='ПарсингХМЛФайловНовостей:   Failed to delete temporary file%1 for feed%2 due to:"
"%3"
""
"It is recommended that you manually delete this temporary file.'"),
									СокрЛП(ИмяВременногоФайла),
						...
						Иначе
							ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='Falta el fichero cargado %1 para la cinta de las novedades %2. Probablemente, no basta lugar sobre el disco, no hay derechos para la anotación en este catálogo o había una otra falta.';ru='Отсутствует загруженный файл %1 для ленты новостей %2. Возможно, недостаточно места на диске, нет прав для записи в этот каталог или произошла какая-то другая ошибка.';en='Missing file %1 for feed %2 is missing. There may be insufficient disk space, no permissions to write to this directory, or some other error has occurred.'"),
								СокрЛП(ИмяВременногоФайла),
								ТекущаяСтрока.ЛентаНовостей);
					...
					Иначе
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='%1 (la dirección vacía en el depósito después de la carga del fichero [%2] del sitio [%3] por la acta [%4]). La cinta de las novedades no es tratada.';ru='Не загружен файл новостей для ленты новостей %1 (пустой адрес в хранилище после загрузки файла [%2] с сайта [%3] по протоколу [%4]). Лента новостей не обработана.';en='The news file for news feed%1 was not loaded (an empty address in the repository after downloading the file [%2] from the site [%3] using the protocol [%4]). The feed has not been processed.'"),
							ТекущаяСтрока.ЛентаНовостей,
				...
				Иначе
					ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
						+ НСтр("es='el tratamiento';ru='Не помечена для обработки';en='Not marked for processing'")
						+ Символы.ПС;
				КонецЕсли;

				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Парсинг: la cinta de las novedades %1. "
"¿Con éxito? %2';ru='Парсинг: лента новостей %1."
"Успешно? %2';en='Parsing: feed %1."
"Successfully? %2'"),
					ТекущаяСтрока.ЛентаНовостей,
					(ЕстьОшибки = Ложь));
...
#Область ВспомогательныеОбработкиОсновные_ПересчитатьОтборыПоНовостям_ДляОбластиДанных
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			Если ОбрабатыватьОбластиДанных = Истина Тогда
				ОписаниеРезультатаВложенный = НСтр("es='La ejecución del recuento de las selecciones por las novedades para la área de los datos.';ru='Выполнение пересчета отборов по новостям для области данных.';en='Recounting news feeds for a data area.'");
				ОбработкаНовостей.ПересчитатьОтборыПоНовостям_ДляОбластиДанных(КонтекстВыполненияВложенныйВложенный);
			Иначе
				ОписаниеРезультатаВложенный = НСтр("es='La admisión del recuento de las selecciones por las novedades para la área de los datos.';ru='Пропуск пересчета отборов по новостям для области данных.';en='Skip recounting news feeds for the data area.'")
			КонецЕсли;
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. La carga de las novedades';ru='БИП:Новости.Загрузка новостей';en='BIP: News. Download News'"), // ИмяСобытия
			НСтр("es='Las novedades. La carga de las novedades. Парсинг de los ficheros de las novedades';ru='Новости. Загрузка новостей. Парсинг файлов новостей';en='News. Download news. Parsing News Files'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Для каждого ТекущаяСтрока Из ТаблицаОбновляемыхДанных Цикл
			Если ТипЗнч(ТекущаяСтрока.Список) = Тип("ПланВидовХарактеристикСсылка.КатегорииНовостей") Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La carga de los significados de la categoría %1 (%2)';ru='Загрузка значений категории %1 (%2)';en='Loading category values%1 (%2)'"),
			...
			Иначе
				Если ТекущаяСтрока.Список = "Список лент новостей" Тогда // Идентификатор.
					ТекстСообщения = НСтр("es='La carga de la lista de las cintas de las novedades';ru='Загрузка списка лент новостей';en='Download a list of news feeds'");
					ТекущаяСтрока.ПредставлениеДляЗагрузки = ТекстСообщения;
				ИначеЕсли ТекущаяСтрока.Список = "Список категорий новостей" Тогда // Идентификатор.
					ТекстСообщения = НСтр("es='La carga de la lista de las categorías de las novedades';ru='Загрузка списка категорий новостей';en='Download a list of news categories'");
					ТекущаяСтрока.ПредставлениеДляЗагрузки = ТекстСообщения;
				ИначеЕсли СтрНайти(ВРег(ТекущаяСтрока.Список), ВРег("Значения категории новостей:")) = 1 Тогда // Идентификатор.
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La carga de los significados de la categoría con el código %1';ru='Загрузка значений категории с кодом %1';en='Loading category values ​​with code%1'"),
...
Функция ОбновитьСтандартныйСписокССервера(Список, ТекущаяВерсияНаКлиенте, Прокси = Неопределено, КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='La renovación del clasificador';ru='Обновление классификатора';en='Classifier update'"));
			...
			Если ТипЗнч(Список) = Тип("ПланВидовХарактеристикСсылка.КатегорииНовостей") Тогда
				Если НЕ ПустаяСтрока(СокрЛП(Список.Код)) Тогда
					...
					Исключение
						ИнформацияОбОшибке = ИнформацияОбОшибке();
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='La falta de la renovación de la lista de los significados de la categoría de las novedades (Прокси.getUpdatedCategoryValuesByCode): "
"sCategoryCode = %1 "
"errCode = %2 "
"errDescription = "
"%3 %4';ru='Ошибка обновления списка значений категории новостей (Прокси.getUpdatedCategoryValuesByCode):"
"sCategoryCode = %1"
"errCode = %2"
"errDescription = %3"
"%4';en='Error updating the list of news category values (Proxy.getUpdatedCategoryValuesByCode):"
"sCategoryCode = %1"
"errCode = %2"
"errDescription = %3"
"%4'"),
					...
					Если ТипЗнч(СписокОбновленныхДанных) = ТипОбъектXDTO Тогда
						...
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='La información sobre la renovación de los significados de las categorías de las novedades (Прокси.getUpdatedCategoryValuesByCode): "
"configName = %1 "
"configVersion = %2 "
"Código de la categoría: %3 "
"lastSynchronizationDateTime = %4 "
"errCode = %5 "
"errDescription = %6 "
"datos Recibidos: "
"%7';ru='Информация об обновлении значений категорий новостей (Прокси.getUpdatedCategoryValuesByCode):"
"configName = %1"
"configVersion = %2"
"Код категории: %3"
"lastSynchronizationDateTime = %4"
...
"errDescription = %6"
"Полученные данные:"
"%7';en='News category update information (Прокси.getUpdatedCategoryValuesByCode):"
"configName = %1"
"configVersion = %2      Category code: %3"
"lastSynchronizationDateTime = %4"
"errCode = %5"
"errDescription = %6 Received data:"
"%7'"),
							configName,
							configVersion,
							СокрЛП(Список.Код),
								...
								Для каждого ТекущаяСтрокаХДТО Из СписокОбновленныхДанных.categoryValue Цикл
									...
									Иначе
										...
										Исключение
											ИнформацияОбОшибке = ИнформацияОбОшибке();
											ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("es='La falta de la anotación del elemento (%1) con el identificador único "
"%2: %3';ru='Ошибка записи элемента (%1) с уникальным идентификатором %2:"
"%3';en='Error writing item (%1) with unique identifier%2:"
"%3'"),
												Режим,
			...
			ИначеЕсли Список = "Список лент новостей" Тогда // Идентификатор.
				...
				Исключение
					ИнформацияОбОшибке = ИнформацияОбОшибке();
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La falta de la renovación de la lista de las cintas de las novedades (Прокси.getUpdatedChannels): "
"errCode = %1 "
"errDescription = %2 "
"representación Detallada de la falta: "
"%3';ru='Ошибка обновления списка лент новостей (Прокси.getUpdatedChannels):"
"errCode = %1"
"errDescription = %2"
"Подробное представление ошибки:"
"%3';en='Error updating the list of news category values (ProxygetUpdatedChannels):"
"errCode = %1"
"errDescription = %2                        Detailed error view:"
"%3'"),
						errCode,
						errDescription,
						ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				...
				Если ТипЗнч(СписокОбновленныхДанных) = ТипОбъектXDTO Тогда
					...
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La información sobre la renovación de la lista de las cintas de las novedades (Прокси.getUpdatedChannels): "
"configName = %1 "
"configVersion = %2 "
...
"errDescription = %5 "
"datos Recibidos: "
"%6';ru='Информация об обновлении списка лент новостей (Прокси.getUpdatedChannels):"
"configName = %1"
"configVersion = %2"
"lastSynchronizationDateTime = %3"
"errCode = %4"
"errDescription = %5"
"Полученные данные:"
"%6';en='News feed update information (Прокси.getUpdatedChannels):"
"configName = %1"
"configVersion = %2"
"lastSynchronizationDateTime = %3"
"errCode = %4"
"errDescription = %5 Received data:"
"%6'"),
						configName,
						configVersion,
						lastSynchronizationDateTime,
							...
							Для каждого ТекущаяСтрокаХДТО Из СписокОбновленныхДанных.channel Цикл
								...
								Иначе
									...
									Исключение
										ИнформацияОбОшибке = ИнформацияОбОшибке();
										ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("es='La falta de la anotación del elemento (%1) con el identificador único %2 por la causa: "
"%3';ru='Ошибка записи элемента (%1) с уникальным идентификатором %2 по причине:"
"%3';en='Error writing item (%1) with unique identifier%2 because:"
"%3 '"),
											Режим,
											ТекущаяСтрокаХДТО.guid,
											ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				...
				Исключение
					ИнформацияОбОшибке = ИнформацияОбОшибке();
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La falta de la renovación de la lista de las cintas de las novedades (Прокси.getAllChannels): "
"errCode = %1 "
"errDescription = %2 "
"representación Detallada de la falta: "
"%3';ru='Ошибка обновления списка лент новостей (Прокси.getAllChannels):"
"errCode = %1"
"errDescription = %2"
"Подробное представление ошибки:"
"%3';en='Error updating the list of news category values (Proxy.getAllChannels):"
"errCode = %1"
"errDescription = %2               Detailed error view:"
"%3'"),
						errCode,
				...
				Если ТипЗнч(СписокВсехДанных) = ТипОбъектXDTO Тогда
					...
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La información sobre la renovación de la lista de las cintas de las novedades (Прокси.getAllChannels): "
"configName = %1 "
"configVersion = %2 "
"errCode = %3 "
"errDescription = %4 "
"datos Recibidos: "
"%5';ru='Информация об обновлении списка лент новостей (Прокси.getAllChannels):"
"configName = %1"
"configVersion = %2"
"errCode = %3"
"errDescription = %4"
"Полученные данные:"
"%5';en='News feed update information (Прокси.getAllChannels):"
"configName = %1"
"configVersion = %2"
"errCode = %3"
"errDescription = %4 Received data:"
"%5'"),
						configName,
						configVersion,
						errCode,
						...
						Если ТипЗнч(СписокВсехДанных.channel) = ТипСписокXDTO Тогда // Правильно сработает, даже если передали только одно значение.
							...
							Для Каждого ТекущийЭлементСписка Из СписокВсехЗагруженныхСерверныхЛентНовостей Цикл
								Если ТекущийЭлементСписка.Пометка = Ложь Тогда
									...
									ТекстСообщения = НСтр("es='La cinta de las novedades %ЛентаНовостейНаименование del % es marcada a la desaparición, porque era desconectada en el servidor 1С';ru='Лента новостей %ЛентаНовостейНаименование% помечена на удаление, т.к. она была отключена на сервере 1С';en='News feed  %ЛентаНовостейНаименование% marked for deletion, as it was disabled on the 1C server'");
			...
			ИначеЕсли Список = "Список категорий новостей" Тогда // Идентификатор.
				...
				Исключение
					ИнформацияОбОшибке = ИнформацияОбОшибке();
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La falta de la renovación de la lista de las categorías de las novedades (Прокси.getUpdatedCategories): "
"errCode = %1 "
"errDescription = %2 "
"representación Detallada de la falta: "
"%3';ru='Ошибка обновления списка категорий новостей (Прокси.getUpdatedCategories):"
"errCode = %1"
"errDescription = %2"
"Подробное представление ошибки:"
"%3';en='Error updating the list of news category values (Proxy .getUpdatedCategories):"
"errCode = %1"
"errDescription = %2           Detailed error view:"
"%3'"),
						errCode,
				...
				Если ТипЗнч(СписокОбновленныхДанных) = ТипОбъектXDTO Тогда
					...
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='La información sobre la renovación de la lista de las categorías de las novedades (Прокси.getUpdatedCategories): "
"configName = %1 "
"configVersion = %2 "
"lastSynchronizationDateTime = %3 "
"errCode = %4 "
"errDescription = %5 "
"datos Recibidos: "
"%6';ru='Информация об обновлении списка категорий новостей (Прокси.getUpdatedCategories):"
"configName = %1"
"configVersion = %2"
"lastSynchronizationDateTime = %3"
"errCode = %4"
"errDescription = %5"
"Полученные данные:"
"%6';en='News category list update information (Прокси.getUpdatedCategoryValuesByCode):"
"configName = %1"
"configVersion = %2       "
"lastSynchronizationDateTime = %3"
"errCode = %4"
				...
				Если НЕ ПустаяСтрока(СписокУИН) Тогда
					...
					Если НЕ ПустаяСтрока(Владелец.ВерсияДанных) Тогда
						...
						Исключение
							ИнформацияОбОшибке = ИнформацияОбОшибке();
							ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La falta de la renovación de la lista de los significados de la categoría de las novedades (Прокси.getUpdatedCategoryValuesByGuid): "
"sCategoryGuid = %1 "
"errCode = %2 "
"errDescription = %3 "
"representación Detallada de la falta: "
"%4';ru='Ошибка обновления списка значений категории новостей (Прокси.getUpdatedCategoryValuesByGuid):"
"sCategoryGuid = %1"
"errCode = %2"
"errDescription = %3"
"Подробное представление ошибки:"
"%4';en='Error updating the list of news category values (ProxygetUpdatedCategoryValuesByGuid):"
"sCategoryGuid = %1"
"errCode = %2"
"errDescription = %3              Detailed error view:"
"%4'"),
								СокрЛП(СписокУИН),
								errCode,
								errDescription,
						...
						Если ТипЗнч(СписокОбновленныхДанных) = ТипОбъектXDTO Тогда
							...
							ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='La información sobre la renovación de los significados de las categorías de las novedades (Прокси.getUpdatedCategoryValuesByGuid): "
"configName = %1 "
"configVersion = %2 "
"УИН las categorías: %3 "
"lastSynchronizationDateTime = %4 "
"errCode = %5 "
"errDescription = %6 "
"datos Recibidos: "
"%7';ru='Информация об обновлении значений категорий новостей (Прокси.getUpdatedCategoryValuesByGuid):"
"configName = %1"
"configVersion = %2"
"УИН категории: %3"
"lastSynchronizationDateTime = %4"
...
"%7';en='News category update information (Прокси.getUpdatedCategoryValuesByCode):"
"configName = %1"
"configVersion = %2       Category UIN: %3"
"lastSynchronizationDateTime = %4"
"errCode = %5"
"errDescription = %6 Received data:"
"%7'"),
...
"%3 '"),
		...
		Иначе
			БылиОшибки = Истина;
			ОписаниеРезультата = НСтр("es='No consiguió conectarse a los servicios web.';ru='Не удалось подключиться к веб-сервисам.';en='Failed to connect to web services.'");
		КонецЕсли;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. La carga de los clasificadores';ru='БИП:Новости.Загрузка классификаторов';en='BIP: News.Loading classifiers'"), // ИмяСобытия
			НСтр("es='Las novedades. La carga de los clasificadores';ru='Новости. Загрузка классификаторов';en='News. Classifier loading'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Функция ОбновитьВсеСтандартныеСпискиССервера(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		Если РазрешенаРаботаСНовостямиЧерезИнтернет = Истина Тогда
			...
			Если ТипЗнч(Прокси) <> ТипWSПрокси Тогда
				ЕстьОшибки = Истина;
				ОписаниеРезультата = НСтр("es='No consiguió conectarse a прокси de los servicios web de la renovación de los clasificadores';ru='Не удалось подключиться к прокси веб-сервисов обновления классификаторов';en='Failed to connect to the classifier update web services proxy'");
			КонецЕсли;
		...
		Иначе
			КодРезультата = 2;
			ОписаниеРезультата = НСтр("es='Обработка.УправлениеНовостями.ОбновитьВсеСтандартныеСпискиССервера: Es desconectado el trabajo con las novedades a través del Internet. La renovación de los clasificadores no se cumplía.';ru='Обработка.УправлениеНовостями.ОбновитьВсеСтандартныеСпискиССервера: Отключена работа с новостями через интернет. Обновление классификаторов не выполнялось.';en='Обработка.УправлениеНовостями.ОбновитьВсеСтандартныеСпискиССервера: Disabled work with news via the Internet. Classifiers were not updated.'");
		КонецЕсли;
...
Процедура ОбновитьСписокСправочниковИВерсииССервера(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		ИнтернетПоддержкаПользователейКлиентСервер.ЗарегистрироватьРезультатВыполненияОперации(
			...
			НСтр("es='Los parámetros de la ejecución';ru='Параметры выполнения';en='Execution options'"),
			0,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='configName = %1 "
"configVersion = %2';ru='configName = %1"
"configVersion = %2';en='configName =% 1"
"configVersion =% 2'"),
					configName,
					configVersion),
			Неопределено);
		...
		Если ТипЗнч(Прокси) = ТипWSПрокси Тогда
			...
			ИнтернетПоддержкаПользователейКлиентСервер.ЗарегистрироватьРезультатВыполненияОперации(
				...
				0,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La recepción de la lista de las últimas renovaciones, la llamada getLastUpdatesList "
"configName = %1 "
"configVersion = %2 "
"errCode = %3 "
"errDescription = %4 "
"Resultado: "
"%5';ru='Получение списка последних обновлений, вызов getLastUpdatesList"
"configName = %1"
"configVersion = %2"
"errCode = %3"
"errDescription = %4"
"Результат:"
"%5';en='Getting a list of recent updates, call getLastUpdatesList"
"configName =%1"
"configVersion =%2"
"errCode =%3"
"errDescription =%4"
...
"%5'"),
					...
					?(ТипЗнч(ДатыОбновленияСписков) = ТипОбъектXDTO,
						...
						НСтр("es=""Es recibido el significado con el tipo del significado excelente de Ob'ektXDTO"";ru='Получено значение с типом значения, отличным от ОбъектXDTO';en='Received value with value type other than XDTOObject'"))),
				Неопределено);
		...
		Иначе
			ОписаниеРезультата = НСтр("es='No consiguió conectarse al servicio web de la renovación de los clasificadores.';ru='Не удалось подключиться к веб-сервису обновления классификаторов.';en='Failed to connect to the classifier update web service.'");
		КонецЕсли;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. La carga de los clasificadores';ru='БИП:Новости.Загрузка классификаторов';en='BIP: News.Loading classifiers'"), // ИмяСобытия
			НСтр("es='Las novedades. La carga de los clasificadores. La lista de las renovaciones de los clasificadores';ru='Новости. Загрузка классификаторов. Список обновлений классификаторов';en='News. Classifiers loading. List of Classifier Updates'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ПарсингХМЛФайлаНовостей(ОбъектХДТО, ФорматНовостей, ЛентаНовостей, Прокси, КонтекстВыполнения = Неопределено)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='Парсинг del fichero de las novedades';ru='Парсинг файла новостей';en='Parsing a news file'"));
		...
		Если НРег(ФорматНовостей) = "rss" Тогда
			...
			ОписаниеРезультата = ОписаниеРезультата
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Парсинг del fichero de las novedades para la cinta de las novedades %1 "
"Es determinado el formato de la cinta de las novedades: %2';ru='Парсинг файла новостей для ленты новостей %1"
"Определен формат ленты новостей: %2';en='Parsing the news file for feed%1"
"Feed format defined:%2'"),
					ЛентаНовостей,
					НРег(ФорматНовостей))
				+ Символы.ПС;
			...
			Если ТипЗнч(ОбъектХДТО.Свойства().Получить("channel")) = ТипСвойствоXDTO
				...
				Иначе
					ОписаниеРезультата = ОписаниеРезультата
						+ НСтр("es='Обработка.УправлениеНовостями.ПарсингХМЛФайлаНовостей (rss): Es desconectado el trabajo con las novedades a través del Internet. La renovación de los clasificadores no se cumplía.';ru='Обработка.УправлениеНовостями.ПарсингХМЛФайлаНовостей(rss): Отключена работа с новостями через интернет. Обновление классификаторов не выполнялось.';en='Обработка.УправлениеНовостями.ПарсингХМЛФайлаНовостей(rss): Disabled work with news via the Internet. Classifiers were not updated.'")
						+ Символы.ПС;
				КонецЕсли;
				...
				Если ОбъектХДТО.Свойства().Получить("channel") <> Неопределено Тогда
					Если ОбъектХДТО.channel.Свойства().Получить("lastBuildDate") <> Неопределено Тогда
						...
						Если ДатаЗагрузкиПоследнихНовостейНаКлиенте >= ДатаЗагрузкиПоследнихНовостейВФайле Тогда
							...
							ОписаниеРезультата = ОписаниеРезультата
								+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("es='ДатаЗагрузкиПоследнихНовостейНаКлиенте: %1 "
"DataZagruzkiPoslednihNovosteyVFayle: %2 "
"Novedades en el programa no exigen la renovación.';ru='ДатаЗагрузкиПоследнихНовостейНаКлиенте: %1"
"ДатаЗагрузкиПоследнихНовостейВФайле: %2"
"Новости в программе не требуют обновления.';en='ДатаЗагрузкиПоследнихНовостейНаКлиенте: %1"
"ДатаЗагрузкиПоследнихНовостейВФайле: %2"
"Новости в программе не требуют обновления.'"),
					...
					Иначе
						...
						ОписаниеРезультата = ОписаниеРезультата
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='En el fichero recibido para la cinta de las novedades %1 falta el campo channel.lastBuildDate. Todas las novedades serán leídas y cargadas, pero esto ocupará más de tiempo.';ru='В файле полученном для ленты новостей %1 отсутствует поле channel.lastBuildDate. Все новости будут прочитаны и загружены, но это займет больше времени.';en='The file received for news feed% 1 is missing the channel.lastBuildDate field. All news will be read and downloaded, but it will take more time.'"),
								ЛентаНовостей)
							+ Символы.ПС;
					КонецЕсли;
				Иначе
					ОписаниеРезультата = ОписаниеРезультата
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("es='En el fichero recibido para la cinta de las novedades %1 falta el campo obligatorio channel. El fichero no era tratado.';ru='В файле полученном для ленты новостей %1 отсутствует обязательное поле channel. Файл не был обработан.';en='The file received for news feed% 1 is missing the required channel field. The file was not processed.'"),
							ЛентаНовостей)
						+ Символы.ПС;
					НеобходимоОбработатьФайл = Ложь;
				КонецЕсли;
				...
				Если НеобходимоОбработатьФайл = Истина Тогда
					...
					Если ЕстьЗначимыеНовости = Истина Тогда
						Если ДатаЗагрузкиПоследнихНовостейНаКлиенте < ДатаЗагрузкиПоследнихНовостейВФайле Тогда
							...
							ОписаниеРезультата = ОписаниеРезультата
								+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("es='ДатаЗагрузкиПоследнихНовостейВФайле: %1 "
"DataPosledneyPopytkiZagruzkiNovostey: %2';ru='ДатаЗагрузкиПоследнихНовостейВФайле: %1"
"ДатаПоследнейПопыткиЗагрузкиНовостей: %2';en='ДатаЗагрузкиПоследнихНовостейВФайле: %1"
...
"Feed format defined:%2'"),
			...
			Иначе
				ОписаниеРезультата = ОписаниеРезультата
					+ НСтр("es='Обработка.УправлениеНовостями.ПарсингХМЛФайлаНовостей (atom): Es desconectado el trabajo con las novedades a través del Internet. La renovación de los clasificadores no se cumplía.';ru='Обработка.УправлениеНовостями.ПарсингХМЛФайлаНовостей(atom): Отключена работа с новостями через интернет. Обновление классификаторов не выполнялось.';en='Обработка.УправлениеНовостями.ПарсингХМЛФайлаНовостей(atom): Disabled work with news via the Internet. Classifiers were not updated.'")
					+ Символы.ПС;
			КонецЕсли;
...
"Новости в программе не требуют обновления.'"),
			...
			Иначе // updated - обязательное поле, наверное формат некорректный
				...
				ОписаниеРезультата = ОписаниеРезультата
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='En el fichero recibido para la cinta de las novedades %1 falta el campo feed.updated. Todas las novedades serán leídas y cargadas, pero esto ocupará más de tiempo.';ru='В файле полученном для ленты новостей %1 отсутствует поле feed.updated. Все новости будут прочитаны и загружены, но это займет больше времени.';en='The feed.updated field is missing in the file received for news feed% 1. All news will be read and downloaded, but it will take more time.'"),
						ЛентаНовостей)
					+ Символы.ПС;
			КонецЕсли;
...
"ДатаПоследнейПопыткиЗагрузкиНовостей: %2'"),
		...
		Иначе
			ОписаниеРезультата = ОписаниеРезультата
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Las novedades no son cargadas, porque es entregado el formato no tratado de la cinta de las novedades: %1';ru='Новости не загружены, т.к. передан необрабатываемый формат ленты новостей: %1';en='News not uploaded Unreadable feed format passed:%1'"),
					НРег(ФорматНовостей))
				+ Символы.ПС;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. La carga de las novedades';ru='БИП:Новости.Загрузка новостей';en='BIP: News. Download News'"), // ИмяСобытия
			НСтр("es='Las novedades. La carga de las novedades. Парсинг de los ficheros de las novedades. Парсинг del fichero de las novedades';ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей';en='News. Download news. Parsing news files. Parsing a news file'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ЗагрузитьНовость(
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='La carga de la novedad';ru='Загрузка новости';en='Download News'"));
			...
			Если ЗаписыватьНовость = Истина Тогда
				...
				ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
					...
					Попытка
						...
						ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
							+ НСтр("es='El comienzo de la tentativa de la anotación de la novedad';ru='Начало попытки записи новости';en='Beginning of an attempt to record news'")
							+ Символы.ПС;
						...
						ОписаниеРезультатаВложенный = ОписаниеРезультатаВложенный
							+ НСтр("es='La tentativa exitosa de la anotación de la novedad';ru='Успешная попытка записи новости';en='Successful attempt to record news'")
							+ Символы.ПС;

					Исключение

						ТекстСообщения = НСтр("es='El intento fallado de la anotación de la novedad por la causa: "
"%ИнформацияОбОшибке del %';ru='Неудачная попытка записи новости по причине:"
"%ИнформацияОбОшибке%';en='Unsuccessful attempt to record news due to:"
"%ИнформацияОбОшибке%'");
						ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИнформацияОбОшибке%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
						ОписаниеРезультата = ОписаниеРезультата
							+ ТекстСообщения
			...
			Иначе
				...
				ОписаниеРезультата = ОписаниеРезультата
					+ НСтр("es='La novedad es dejada pasar';ru='Новость пропущена';en='News skipped'")
					+ Символы.ПС;
			...
			РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%ЗаписыватьНовость%", ?(ЗаписыватьНовость = Истина, НСтр("es='Será anotada';ru='Будет записана';en='Will be recorded'"), НСтр("es='Será dejada pasar';ru='Будет пропущена';en='Will be skipped'")));
			РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%СуществуетВБазе%", ?(СтруктураНовости.Ссылка.Пустая(), НСтр("es='En la base no es creado';ru='В базе еще не создана';en='Not yet created in the database'"), НСтр("es='Existe ya en la base';ru='Уже существует в базе';en='Already exists in base'")));
			РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%ДатаПубликацииИБ%", ?(СтруктураНовости.Ссылка.Пустая(), " " + НСтр("es='(En la base no era)';ru='(в базе не было)';en='(not in the database)'"), " (" + Формат(ДатаПубликацииИБ, "ДЛФ=DT") + ")"));
			РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%ЗначениеПометкаУдаленияИБ%", ?(СтруктураНовости.Ссылка.Пустая(), " " + НСтр("es='(En la base no era)';ru='(в базе не было)';en='(not in the database)'"), " (" + ЗначениеПометкаУдаленияИБ + ")"));
			РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%УИН_ИБ%", ?(СтруктураНовости.Ссылка.Пустая(), НСтр("es='(No es creado)';ru='(еще не создана)';en='(not yet created)'"), СтруктураНовости.УИН_ИБ));
			РезультатЗагрузкиНовости = СтрЗаменить(РезультатЗагрузкиНовости, "%НавигационнаяСсылка%", ?(СтруктураНовости.Ссылка.Пустая(), НСтр("es='(No es creado)';ru='(еще не создана)';en='(not yet created)'"), ПолучитьНавигационнуюСсылку(СтруктураНовости.Ссылка)));
...
"%ИнформацияОбОшибке%'");
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. La carga de las novedades';ru='БИП:Новости.Загрузка новостей';en='BIP: News. Download News'"), // ИмяСобытия
			НСтр("es='Las novedades. La carga de las novedades. La carga de la novedad';ru='Новости. Загрузка новостей. Загрузка новости';en='News. Download news. Download news'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
	...
	Если ТипЗнч(ДанныеХДТО) = ТипОбъектXDTO Тогда
		...
		Если ТипЗнч(СвойствоДвоичныеДанные) = ТипСвойствоXDTO Тогда
			...
			Исключение
				...
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='No consiguió cargar los datos binarios para la novedad con УИН %1 por la causa: "
"%2';ru='Не удалось загрузить бинарные данные для новости с УИН %1 по причине: "
"%2';en='Failed to load binary data for news from UIN%1 due to:"
"%2'"),
					ОбъектНовость.УИННовости,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("es='БИП:Новости. La carga de las novedades';ru='БИП:Новости.Загрузка новостей';en='BIP: News. Download News'"), // ИмяСобытия
					НСтр("es='Las novedades. La carga de las novedades. Парсинг de los ficheros de las novedades. Парсинг del fichero de las novedades. Парсинг las novedades. La lectura de la novedad. El bloque de los datos binarios';ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Парсинг новости. Чтение новости. Блок бинарных данных';en='News. Download news. Parsing news files. Parsing a news file. Parsing news. Reading the news. Binary data block'"), // ИдентификаторШага
...
Процедура ЗагрузитьДатуПоследнегоОбновленияСтандартногоСписка(Обновление, КонтекстВыполнения = Неопределено)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='La carga de la fecha de la última renovación de la lista estandartizada';ru='Загрузка даты последнего обновления стандартного списка';en='Download the date of the last update of the standard list'"));
		...
		Если ТипЗнч(Обновление) = ТипОбъектXDTO Тогда
			...
			Исключение
				ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La falta de la carga de la línea de la renovación de las listas por la causa: "
"%1';ru='Ошибка загрузки строки обновления списков по причине:"
"%1';en='Error loading the list update line due to:"
"%1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ЗаполнитьОбъектЛентаНовостей(ОбъектХДТО, Режим = "", КонтекстВыполнения = Неопределено)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='El relleno del objeto la Cinta de las novedades';ru='Заполнение объекта Лента новостей';en='Filling the object News feed'"));
				...
				Если ТипЗнч(ОбъектХДТО.categoriesRefList.categoryRef) = ТипСписокXDTO Тогда // Правильно сработает, даже если передали только одно значение.
					...
					Пока БылиУдаления = Истина Цикл
						...
						Для каждого ТекущаяКатегория Из лкОбъект.ДоступныеКатегорииНовостей Цикл
							...
							Если КатегорияЕстьНаСервере = Ложь Тогда
								ОписаниеРезультата = ОписаниеРезультата
									+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("es='De la cinta de las novedades %1 es quitada la categoría %2.';ru='Из ленты новостей %1 удалена категория %2.';en='Removed category%2 from news feed%1.'"),
					...
					Для каждого ТекущаяКатегория Из МассивДоступныхКатегорий Цикл
						...
						Если КатегорияЕстьНаКлиенте = Ложь Тогда
							ОписаниеРезультата = ОписаниеРезультата
								+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("es='En la cinta de las novedades %1 es añadida la categoría %2.';ru='В ленте новостей %1 добавлена категория %2.';en='Added% 2 category to news feed% 1.'"),
									СокрЛП(лкОбъект.Наименование),
...
Функция ПодключитьсяКВебСервисуОбновленияСтандартныхСписков(КонтекстВыполнения = Неопределено)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		НаименованиеПроцедурыФункции, // Идентификатор.
		НСтр("es='La conexión al servicio web de la renovación de los clasificadores';ru='Подключение к веб-сервису обновления классификаторов';en='Connect to the classifier update web service'"));
		...
		ОписаниеРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El comienzo de la carga de las listas estandartizadas "
"de MestopolozhenieWSDL = %1';ru='Начало загрузки стандартных списков"
"МестоположениеWSDL = %1';en='Start loading standard lists"
"LocationWSDL =%1 '"),
			МестоположениеWSDL)
			+ Символы.ПС;
		...
		Попытка
			...
			Если ТипЗнч(Прокси) <> ТипWSПрокси Тогда
				ОписаниеРезультата = ОписаниеРезультата
					+ НСтр("es='La conexión desgraciada al servicio web de la renovación de las listas estandartizadas';ru='Неудачное подключение к веб-сервису обновления стандартных списков';en='Failed to connect to the standard list update web service'");
			Иначе
				ОписаниеРезультата = ОписаниеРезультата
					+ НСтр("es='La conexión exitosa al servicio web de la renovación de las listas estandartizadas';ru='Успешное подключение к веб-сервису обновления стандартных списков';en='Successfully connecting to the standard list update web service'");
			КонецЕсли;
		...
		Исключение
			...
			ОписаниеРезультата = ОписаниеРезультата
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La conexión desgraciada al servicio web de la renovación de las listas estandartizadas por la causa: "
"%1';ru='Неудачное подключение к веб-сервису обновления стандартных списков по причине:"
"%1';en='Connection to the web service for updating standard lists failed due to:"
"%1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("es='БИП:Новости. La carga de los clasificadores';ru='БИП:Новости.Загрузка классификаторов';en='BIP: News.Loading classifiers'"), // ИмяСобытия
			НСтр("es='Las novedades. La carga de los clasificadores. La unión con el servicio web de la renovación de las listas estandartizadas';ru='Новости. Загрузка классификаторов. Соединение с веб-сервисом обновления стандартных списков';en='News. Classifiers loading. Connect to the standard list update web service'"), // ИдентификаторШага
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
