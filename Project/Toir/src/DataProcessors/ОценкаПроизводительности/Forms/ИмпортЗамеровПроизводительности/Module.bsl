...
Процедура ВыбратьФайлаИмпортаПредложено(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Если РасширениеРаботыСФайламиПодключено Тогда
		...
		ВыборФайла.Заголовок = НСтр("es='Escojan el fichero de la importación de las mediciones';ru='Выберите файл импорта замеров';en='Select a measurement import file'");
...
Процедура ИмпортПослеПроверкиСуществования(Существует, ДополнительныеПараметры) Экспорт
	Если Не Существует Тогда 
    	...
    	Сообщение.Текст = НСтр("es='Escojan el fichero para la importación.';ru='Выберите файл для импорта.';en='Select the file to import.'");
