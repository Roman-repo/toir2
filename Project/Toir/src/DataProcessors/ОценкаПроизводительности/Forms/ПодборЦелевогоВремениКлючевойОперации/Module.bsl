...
Процедура ПодобратьЦелевоеВремя(Команда)                             
	...
	Если РезультатПроверки Тогда
		...
		ДиаграммаЗамеровВремени.ОбластьПостроения.ШкалаЗначений.ТекстЗаголовка = НСтр("es='La cantidad de las mediciones, шт';ru='Количество замеров, шт';en='Number of measurements, pcs'");
...
Функция ПроверкаЗаполнения()
	...
	Если КлючеваяОперация.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("es='la operación clave.';ru='Не указана ключевая операция.';en='Key operation not specified.'");
	...
	Если Не ЗначениеЗаполнено(ТекущийApdex) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("es='No es indicado corriente APDEX.';ru='Не указан текущий APDEX.';en='The current APDEX is not specified.'");
	...
	Если Не ЗначениеЗаполнено(Период.ДатаНачала) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("es='del período.';ru='Не указана дата начала периода.';en='No start date for the period.'");
	...
	Если Не ЗначениеЗаполнено(Период.ДатаОкончания) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("es='del período.';ru='Не указана дата окончания периода.';en='No end date for the period.'");
		Сообщение.Поле = "Период";
		Сообщение.Сообщить();
		Успешно = Ложь;
	КонецЕсли;
...
Функция ПодборЦелевоеВремяНаСервере(ПараметрыПодбора)
	...
	Иначе
		РезультатПодбора.Вставить("ОписаниеОшибки", НСтр("es='No consiguió recibir los datos sobre las mediciones, prueben cambiar los ajustes.';ru='Не удалось получить данные о замерах, попробуйте изменить настройки.';en='Failed to get measurement data, try changing the settings.'"));
	...
	Если РезультатПодбора.КоличествоЗамеров = 0 Тогда
		РезультатПодбора.Вставить("ОписаниеОшибки",
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Son recibidas 0 mediciones por la operación clave %1. Cambien el período o escojan otra operación clave.';ru='Получено 0 замеров по ключевой операции %1. Измените период или выберите другую ключевую операцию.';en='Received 0 measurements for key operation%1. Change the period or select another key operation'"),
