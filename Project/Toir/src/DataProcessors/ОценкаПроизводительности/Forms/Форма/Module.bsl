...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщаяПроизводительностьСистемы.Пустая() Тогда
		Объект.ОбщаяПроизводительностьСистемы = НСтр("es='La capacidad total del sistema';ru='Общая производительность системы';en='Overall system performance'");
	Иначе
	...
	Исключение
		ТекстСообщения = НСтр("es='No consiguió cargar los ajustes.';ru='Не удалось загрузить настройки.';en='Failed to load settings.'");
...
Процедура УказатьAPDEX(Команда)
	...
	Если СтрокаТЧ[Элемент.Имя] = 0 Тогда
		ПоказатьПредупреждение(,НСтр("es='Faltan las mediciones de la productividad. "
"Contar el tiempo con destinación especial es imposible.';ru='Отсутствуют замеры производительности."
"Рассчитать целевое время невозможно.';en='There are no performance measurements."
"It is not possible to calculate the target time. '"));
		Возврат;
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("УказатьAPDEXЗавершение", ЭтотОбъект);
	Подсказка = НСтр("es='Introduzcan el significado APDEX deseable';ru='Введите желаемое значение APDEX';en='Enter the desired APDEX value'"); 
	ЗначениеAPDEX = 0;
...
Процедура УказатьAPDEXЗавершение(Знач ЗначениеAPDEX, Знач ДополнительныеПараметры) Экспорт
	...
	Если 0 > ЗначениеAPDEX Или ЗначениеAPDEX > 1 Тогда
		ПоказатьПредупреждение(,НСтр("es='Es introducido el índice APDEX incorrecto. "
"Los significados admisibles de 0 hasta 1.';ru='Введен неправильный показатель APDEX."
"Допустимые значения от 0 до 1.';en='Incorrect APDEX entered."
...
Процедура ОбновитьПоказатели(ЗначенияОтбора = Неопределено)
	...
	Если ТаблицаКлючевыхОпераций = Неопределено Тогда
		ОценкаПроизводительностиСлужебный.СообщитьПользователю(НСтр("es='El período es establecido no justo.';ru='Период установлен не верно.';en='The period is not set correctly.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьДиаграмму(ТаблицаКлючевыхОпераций)
	...
	ТекстЗаголовка = НСтр("es='El diagrama de la productividad con %1 por %2 - el paso: %3';ru='Диаграмма производительности с %1 по %2 - шаг: %3';en='Performance chart%1 through%2 - step:%3'");
...
Процедура ОбменятьПриоритеты(КлючеваяОперация1, Приоритет1, КлючеваяОперация2, Приоритет2)
	...
	Исключение
		ОтменитьТранзакцию();
 		ЗаписьЖурналаРегистрации(НСтр("es='La ejecución de la operación';ru='Выполнение операции';en='Operation'", Метаданные.ОсновнойЯзык.КодЯзыка), УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
 		ВызватьИсключение;
	КонецПопытки;
...
Функция ВычислитьДатыВременногоОтрезка(ДатаНачала, ДатаОкончания, ИндексПериода)
	...
	Если КоличествоШагов <= ИндексПериода Тогда
		ВызватьИсключение НСтр("es='La cantidad de los pasos no puede ser menos de índice.';ru='Количество шагов не может быть меньше индекса.';en='The number of steps cannot be less than the index.'");
	КонецЕсли;
	...
	Для Каждого СтрокаТЧ Из Объект.Производительность Цикл
		...
		Если СтрокаТЧ.ЦелевоеВремя = 0 
			...
			ОценкаПроизводительностиСлужебный.СообщитьПользователю(
				НСтр("es='El tiempo con destinación especial debe ser rellenado obligatoriamente.';ru='Целевое время обязательно должно быть заполнено.';en='The target time must be filled.'"),
				,
