...
Процедура ВыполнитьЗагрузкуДанных(ПараметрыЗагрузки = Неопределено) Экспорт
	...
	Исключение
		...
		СтрокаСообщения = НСтр("es='La falta a la carga de los datos: %1';ru='Ошибка при загрузке данных: %1';en='Error loading data:%1'");
	...
	Исключение
		ОбменДаннымиСлужебный.ОтключитьОбновлениеКлючейДоступа(Ложь);
		Информация = ИнформацияОбОшибке();
		СтрокаСообщения = НСтр("es='La falta a la desaparición de los objetos temporales creados por las referencias: %1';ru='Ошибка при удалении временных объектов, созданных по ссылкам: %1';en='Error deleting temporary objects created by links:%1'");
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			СтрокаСообщения, ПодробноеПредставлениеОшибки(Информация));
		ОбменДаннымиXDTOСервер.ЗаписатьВПротоколВыполнения(КомпонентыОбмена, СтрокаСообщения, , , , , Истина);
		КомпонентыОбмена.ФлагОшибки = Истина;
...
Процедура ВыполнитьАнализСообщенияОбмена(ПараметрыАнализа = Неопределено) Экспорт
	...
	Исключение
		СтрокаСообщения = НСтр("es='La falta durante el análisis de los datos: %1';ru='Ошибка при анализе данных: %1';en='Error parsing data:%1'");
		...
		Если КомпонентыОбмена.ОбменДаннымиСВнешнейСистемой Тогда
			...
			Если ЗначениеЗаполнено(КомпонентыОбмена.ИдентификаторКорреспондента)
				...
				ОбменДаннымиXDTOСервер.ЗаписатьВПротоколВыполнения(КомпонентыОбмена, 
					НСтр("es='Es cambiado el identificador del nudo del corresponsal.';ru='Изменен идентификатор узла корреспондента.';en='Correspondent node identifier changed.'"), , Ложь, , , Истина);
			КонецЕсли;
		КонецЕсли;
