...
Процедура ЗавершитьСеанс(Команда)
	...
	Если КоличествоВыделенныхСтрок = 0 Тогда
		ПоказатьПредупреждение(,НСтр("es='de las sesiones.';ru='Не выбраны пользователи для завершения сеансов.';en='No users selected to end sessions.'"));
		Возврат;
	ИначеЕсли КоличествоВыделенныхСтрок = 1 Тогда
		Если Элементы.СписокПользователей.ТекущиеДанные.Сеанс = НомерСеансаИнформационнойБазы Тогда
			ПоказатьПредупреждение(,НСтр("es='Es imposible acabar la sesión corriente. Para la salida del programa es posible cerrar la ventana principal del programa.';ru='Невозможно завершить текущий сеанс. Для выхода из программы можно закрыть главное окно программы.';en='Unable to end current session. To exit the program, you can close the main program tab.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		Если ЗапрашиватьПараметрыАдминистрированияИБ Тогда
			...
			ЗаголовокФормы = НСтр("es='La terminación de la sesión';ru='Завершение сеанса';en='Session completing'");
			ПоясняющаяНадпись = НСтр("es='Para la terminación de la sesión es necesario introducir los parámetros "
"de la administración del clúster de los servidores';ru='Для завершения сеанса необходимо ввести параметры"
"администрирования кластера серверов';en='To end the session, you must enter the parameters"
"server cluster administration"
"To load the address classifier from the folder'");
...
Процедура ОткрытьЖурналРегистрации()
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("es='Escojan a los usuarios para la presentación de la revista del registro.';ru='Выберите пользователей для просмотра журнала регистрации.';en='Select users to view the log.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПослеЗавершенияСеанса(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		Если ДополнительныеПараметры.НомераСеансов.Количество() > 1 Тогда
			...
			ТекстОповещения = НСтр("es='Las sesiones %1 son acabadas.';ru='Сеансы %1 завершены.';en='Sessions%1 completed.'");
			...
			ПоказатьОповещениеПользователя(НСтр("es='La terminación de las sesiones';ru='Завершение сеансов';en='Sessions completing'"),, ТекстОповещения);
			
		Иначе
			
			ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La sesión %1 es acabada.';ru='Сеанс %1 завершен.';en='Session%1 completed.'"), ДополнительныеПараметры.НомераСеансов[0]);
			ПоказатьОповещениеПользователя(НСтр("es='La terminación de la sesión';ru='Завершение сеанса';en='Session completing'"),, ТекстОповещения);
