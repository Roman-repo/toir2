...
Процедура Сохранить(Команда)
	...
	Результат.ТекстОшибки = НСтр("es='No consiguió conservar отсканированный el fichero.';ru='Не удалось сохранить отсканированный файл.';en='Failed to save the scanned file.'");
	ПринятьЗавершение(Результат, ПараметрыВыполнения);
КонецПроцедуры
	...
	Если КоличествоОшибок > 0 Тогда
		Если КоличествоОшибок = 1 Тогда
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió conservar el fichero por la causa: "
"%1';ru='Не удалось сохранить файл по причине:"
"%1';en='Failed to save file due to:"
"%1'"),
				ПолныйТекстВсехОшибок);
		Иначе
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió conservar algunos ficheros "
"(%1: %2)';ru='Не удалось сохранить некоторые файлы (%1):"
"%2';en='Failed to save some files (%1):"
"%2'"),
...
Процедура ПреобразоватьПеречисленияВПараметрыИПолучитьПредставление()
	...
	Если ИспользоватьImageMagickДляПреобразованияВPDF Тогда
		Если ФорматХраненияОдностраничный = Перечисления.ФорматыХраненияОдностраничныхФайлов.PDF Тогда
			...
			Представление = Представление + НСтр("es='El formato del almacenaje:';ru='Формат хранения:';en='Storage format:'") + " ";
			Представление = Представление + "PDF";
			Представление = Представление + ". ";
			Представление = Представление + НСтр("es='El formato del escaneo:';ru='Формат сканирования:';en='Scan format:'") + " ";
	...
	Если ВРег(ФорматКартинки) = "JPG" Тогда
		Представление = Представление +  НСтр("es='La cualidad:';ru='Качество:';en='Quality:'") + " " + Строка(КачествоJPG) + ". ";
	КонецЕсли;	
	
	Если ВРег(ФорматКартинки) = "TIF" Тогда
		Представление = Представление +  НСтр("es='La compresión:';ru='Сжатие:';en='Compression:'") + " " + Строка(СжатиеTIFF) + ". ";
	КонецЕсли;
	
	Представление = Представление + НСтр("es='El formato del almacenaje en muchas páginas:';ru='Формат хранения многостраничный:';en='Multi-page storage format:'") + " ";
	...
	Представление = Представление + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El permiso: %1 dpi. %2.';ru='Разрешение: %1 dpi. %2.';en='Permission:%1 dpi. %2.'") + " ",
		Строка(Разрешение), Строка(ЦветностьПеречисление));
	
	Если НЕ ПоворотПеречисление.Пустая() Тогда
		Представление = Представление +  НСтр("es='La vuelta:';ru='Поворот:';en='Turn:'")+ " " + Строка(ПоворотПеречисление) + ". ";
	КонецЕсли;	
	
	Если НЕ РазмерБумагиПеречисление.Пустая() Тогда
		Представление = Представление +  НСтр("es='La dimensión del papel:';ru='Размер бумаги:';en='Paper size:'") + " " + Строка(РазмерБумагиПеречисление) + ". ";
	КонецЕсли;	
	
	Если ДвустороннееСканирование = Истина Тогда
		Представление = Представление +  НСтр("es='El escaneo bilateral';ru='Двустороннее сканирование';en='Duplex scanning'") + ". ";
	КонецЕсли;	
...
Функция ТекстСообщенияОшибкиПреобразованияВPDF(ФайлРезультата)
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='""%1"". "
"Comprueben que es establecido el programa ImageMagick y "
"es indicado el camino justo al programa de la transformación en "
"PDF en forma de los ajustes del escaneo.';ru='Не найден файл ""%1""."
"Проверьте, что установлена программа ImageMagick и"
"указан правильный путь к программе преобразования в"
"PDF в форме настроек сканирования.';en='File ""%1"" not found."
"Verify that ImageMagick is installed and"
"the correct path to the conversion program in"
"PDF in the form of scan settings. ""'"),
		ФайлРезультата);
