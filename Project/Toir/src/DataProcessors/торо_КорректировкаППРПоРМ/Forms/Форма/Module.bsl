...
Процедура ПриОткрытии(Отказ)
	...
	//	ОтборОрганизация = "<По всем организациям>";
	//КонецЕсли;
	//
...
Процедура Выбрать(Команда)
	...
	Если МассивУдаления.Количество() <> 0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ВыбратьЗавершение", ЭтотОбъект, Новый Структура("МассивУдаления", МассивУдаления)), НСтр("es='¿Marcar las reparaciones escogidas en el documento ППР como anulado?';ru='Пометить выбранные ремонты в документе ППР как отмененные?';en='Mark selected repairs in the PPM document as canceled?'"),РежимДиалогаВопрос.ДаНет);
        Возврат;
	КонецЕсли;
