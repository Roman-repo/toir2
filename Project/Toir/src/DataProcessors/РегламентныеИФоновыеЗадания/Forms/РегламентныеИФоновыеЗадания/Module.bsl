...
""
"El trabajo con las tareas de reglamento y de fondo "
...
""
"Work with scheduled and background tasks"
...
Процедура ТаблицаРегламентныеЗаданияПередУдалением(Элемент, Отказ)
	...
	Если Элементы.ТаблицаРегламентныеЗадания.ВыделенныеСтроки.Количество() > 1 Тогда
		ПоказатьПредупреждение(, НСтр("es='Escojan una tarea de reglamento.';ru='Выберите одно регламентное задание.';en='Select one scheduled task.'"));
	...
	Иначе
		ПоказатьВопрос(
			...
			НСтр("es='¿Quitar la tarea de reglamento?';ru='Удалить регламентное задание?';en='Delete regulatory task?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
Процедура ВыполнитьРегламентноеЗаданиеВручную(Команда)
	...
	Если Элементы.ТаблицаРегламентныеЗадания.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("es='Escojan la tarea de reglamento.';ru='Выберите регламентное задание.';en='Select a scheduled task.'"));
		Возврат;
	КонецЕсли;
	...
	Для каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		...
		Если ТекущиеДанные.Параметризуемое И КоличествоВыделенныхЗаданий = 1 Тогда
			ПоказатьПредупреждение(, НСтр("es='La tarea escogida de reglamento no es posible cumplir a mano.';ru='Выбранное регламентное задание нельзя выполнить вручную.';en='The selected scheduled task cannot be completed manually.'"));
			Возврат;
		...
		Если ПараметрыВыполнения.ЗапускВыполнен Тогда
			...
			ПоказатьОповещениеПользователя(
				НСтр("es='Es puesto en marcha el procedimiento de la tarea de reglamento';ru='Запущена процедура регламентного задания';en='Scheduled task launched'"), ,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1. "
"El procedimiento es puesto en marcha en la tarea de fondo %2';ru='%1."
"Процедура запущена в фоновом задании %2';en='%1."
"Procedure started in background job% 2'"),
		...
		ИначеЕсли ПараметрыВыполнения.ПроцедураУжеВыполняется Тогда
			МассивСообщенийОбОшибках.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='El procedimiento de la tarea de reglamento ""%1"""
" se cumple ya en tarea ""%2"" de fondo comenzada %3.';ru='Процедура регламентного задания ""%1"""
"  уже выполняется в фоновом задании ""%2"", начатом %3.';en='Routine procedure ""%1"""
"   already running in background task ""%2"" started by%3.'"),
					ТекущиеДанные.Наименование,
	...
	Если КоличествоОшибок > 0 Тогда
		ЗаголовокТекстаПроОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Las tareas son cumplidas con las faltas (%1 de %2)';ru='Задания выполнены с ошибками (%1 из %2)';en='Tasks completed with errors (%1 of%2)'"),
		...
		Если КоличествоОшибок > 5 Тогда
			...
			Кнопки.Добавить(1, НСтр("es='Mostrar las faltas';ru='Показать ошибки';en='Show errors'"));
...
Процедура ОтменитьФоновоеЗадание(Команда)
	...
	Если Элементы.ТаблицаФоновыеЗадания.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("es='Escojan la tarea de fondo.';ru='Выберите фоновое задание.';en='Select a background job.'"));
		Возврат;
	КонецЕсли;
	...
	ПоказатьПредупреждение(,
		НСтр("es='La tarea es anulada, pero el estado de la anulación será establecido por el servidor solamente dentro de un tiempo, "
"será necesario probablemente renovar los datos a mano.';ru='Задание отменено, но состояние отмены будет установлено сервером только через некоторое время,"
"возможно потребуется обновить данные вручную.';en='The task is canceled, but the cancellation status will be set by the server only after some time,"
"you may need to update the data manually.'"));
...
Процедура ВыполнитьНеВФоне(Команда)
	...
	ПоказатьОповещениеПользователя(
		НСтр("es='Es cumplido el procedimiento de la tarea de reglamento';ru='Выполнена процедура регламентного задания';en='Scheduled task completed'"), ,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1. "
"El procedimiento es cumplido a mano sin tarea de fondo';ru='%1."
"Процедура выполнена вручную без фонового задания';en='%1."
"The procedure was performed manually without a background job'"),
			ТекущиеДанные.Наименование),
		БиблиотекаКартинок.ВыполнитьРегламентноеЗаданиеВручную);
...
Процедура СобытияЖурналаРегистрации(Команда)
	...
	Если Элементы.Задания.ТекущаяСтраница = Элементы.РегламентныеЗадания Тогда
		...
		Если Не ЗначениеЗаполнено(ТекущиеДанные.ДатаНачала) И Не ЗначениеЗаполнено(ТекущиеДанные.ДатаОкончания) Тогда
			ПоказатьПредупреждение(, НСтр("es='No hay acontecimientos para la presentación, puesto que la tarea de reglamento no se cumplía.';ru='Нет событий для просмотра, так как регламентное задание не выполнялось.';en='There are no events to view, as the scheduled task was not performed.'"));
			Возврат;
		КонецЕсли;	
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='<>';ru='<>';en='<>'"));
	...
	ОтборЭлемента.ПравоеЗначение = НСтр("es='<no es determinado>';ru='<не определено>';en='<не определено>'");
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТаблицаРегламентныеЗадания.Наименование");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = НСтр("es='El cálculo ППР';ru='Расчет ППР';en='Calculation of PPM'");
...
Процедура СообщитьОбОкончанииРучногоВыполненияРегламентногоЗадания()
	...
	Для каждого Оповещение Из ОповещенияОбОкончанииВыполнения Цикл
		...
		ПоказатьОповещениеПользователя(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='%1. "
"El procedimiento es acabado en la tarea de fondo %2';ru='%1."
"Процедура завершена в фоновом задании %2';en='%1."
"Procedure completed in background job% 2'"),
...
Функция ЗагрузкаРегламентныхЗаданий(ИдентификаторЗадания)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='La formación de la tabla de las tareas de reglamento';ru='Формирование таблицы регламентных заданий';en='Creating a table of scheduled tasks'");
...
Функция СформироватьТаблицуФоновыхЗаданийВФоне()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='Las tareas de reglamento. La renovación de la lista de las tareas de fondo';ru='Регламентные задания. Обновление списка фоновых заданий';en='Regulatory tasks. Updating the list of background jobs'");
	...
	Для каждого Задание Из ТекущаяТаблица Цикл
		...
		Если ЗначениеЗаполнено(Обновляемое.ИдентификаторРегламентногоЗадания) Тогда
			...
			Обновляемое.НаименованиеРегламентногоЗадания
				= ?(Строки.Количество() = 0, НСтр("es='<no es encontrado>';ru='<не найдено>';en='<не найдено>'"), Строки[0].Наименование);
		Иначе
...
Процедура УправлениеИнтерфейсом()
	...
	Если текСтрока <> Неопределено И текСтрока.Наименование = "Расчет ППР" тогда
		Элементы.ТаблицаРегламентныеЗаданияКонтекстноеМенюВыключитьРегламентноеЗадание.Доступность = Ложь;
	Иначе 
		Элементы.ТаблицаРегламентныеЗаданияКонтекстноеМенюВыключитьРегламентноеЗадание.Доступность = Истина;
	КонецЕсли;
КонецПроцедуры
