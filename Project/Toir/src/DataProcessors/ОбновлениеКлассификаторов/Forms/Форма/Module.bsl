...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Элементы.ДекорацияОписаниеРезультата.Заголовок = НСтр("es='El uso del tratamiento es inaccesible en el trabajo en el modelo del servicio.';ru='Использование обработки недоступно при работе в модели сервиса.';en='Using processing is not available when working in the service model.'");
...
Процедура ФайлОбновленияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ТекстПредложения = НСтр("es='Para la carga de los clasificadores del fichero es necesario "
"establecer la ampliación para el trabajo con los ficheros.';ru='Для загрузки классификаторов из файла необходимо"
"установить расширение для работы с файлами.';en='To download classifiers from a file, you must"
"install the extension for working with files.'");
...
Процедура ПроверитьПодключениеКПорталу1СИТС()
	...
	Если ПараметрыПолучения.Идентификаторы.Количество() = 0 Тогда
		УстановитьОтображениеИнформацииОбОшибке(
			ЭтотОбъект,
			НСтр("es='Faltan los clasificadores accesible para la renovación.';ru='Отсутствуют классификаторы доступные для обновления.';en='There are no classifiers available for updating.'"),
			Ложь,
			Ложь);
		УстановитьОтображениеЭлементовФормы(ЭтотОбъект);
		Возврат;
...
Процедура ЗаполнитьИнформациюОДоступныхОбновлениях(РезультатОперации, ВерсииКлассификаторов)
	...
	Иначе
		УстановитьОтображениеИнформацииОбОшибке(
			ЭтотОбъект,
			НСтр("es='de los clasificadores.';ru='Не найдены доступные обновления классификаторов.';en='No available classifier updates found.'"),
			Ложь,
			Ложь);
	КонецЕсли;
...
Процедура ИнформацияОДоступныхОбновленияхИзФайла()
	...
	Если Не ЗначениеЗаполнено(ФайлОбновления) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='de la renovación.';ru='Не выбран файл обновления.';en='No update file selected.'"),
			,
			"ФайлОбновления");
		Возврат;
	КонецЕсли;
	...
	Если КомпонентыПути.Расширение <> ".zip" Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='El formato equivocado del fichero.';ru='Неверный формат файла.';en='Invalid file format.'"),
			,
			"ФайлОбновления");
		Возврат;
	КонецЕсли;
...
Процедура ПодключеноРасширениеРаботыСФайлами(Знач РасширениеПодключено, Знач ДополнительныеПараметры) Экспорт
	...
	ДиалогВыбораФайла.Фильтр = НСтр("es='El archivo';ru='Архив';en='Archive'") + "(*.zip)|*.zip";
...
Процедура ОбработатьФайлОбновленияКлассификаторов(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ПомещенныеФайлы = Неопределено Или ПомещенныеФайлы.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='El fichero con las renovaciones no es cargado.';ru='Файл с обновлениями не загружен.';en='Update file is not loaded.'"));
		Возврат;
	КонецЕсли;
	...
	Элементы.ДекорацияСостояние.Заголовок   = НСтр("es='La transmisión de los ficheros al servidor de la aplicación.';ru='Передача файлов на сервер приложения.';en='Transferring files to the application server.'");
	УстановитьОтображениеЭлементовФормы(ЭтотОбъект);
...
Функция ОбработатьФайлОбновленияКлассификаторовНаСервере(АдресФайла, УникальныйИдентификатор)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='El tratamiento de los ficheros del clasificador en el servidor.';ru='Обработка файлов классификатора на сервере.';en='Processing classifier files on the server.'");
...
Процедура НачатьОбновлениеКлассификаторов()
	...
	Элементы.ДекорацияСостояние.Заголовок  = НСтр("es='Se cumple la renovación de los clasificadores. La renovación puede ocupar "
"de algunos minutos a las algunas depende de la dimensión de la renovación.';ru='Выполняется обновление классификаторов. Обновление может занять от"
"нескольких минут до нескольких часов в зависимости от размера обновления.';en='Classifiers are being updated. Updating may take from"
"minutes to hours depending on the size of the update.'");
...
Функция НачатьОбновлениеКлассификаторовНаСервере()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='La renovación de los clasificadores dados.';ru='Обновление данных классификаторов.';en='Updating classifiers data.'");
...
Процедура УстановитьОтображениеИнформацииОбОшибке(
	...
	Если ОтображатьЖР Тогда
		...
		ЧастиСтрок.Добавить(Символы.ПС);
		ЧастиСтрок.Добавить(НСтр("es='La información detallada de cm. En';ru='Подробную информацию см. в';en='See details in'"));
		ЧастиСтрок.Добавить(Символы.НПП);
		ЧастиСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("es='La revista del registro';ru='Журнале регистрации';en='Registration log'"),,,,"action:openLog"));
		ЧастиСтрок.Добавить(".");
...
Процедура УстановитьОтображениеУспешногоЗавершения(Форма)
	...
	Форма.Элементы.ДекорацияОписаниеРезультата.Заголовок = НСтр("es='La renovación de los clasificadores es con éxito acabada.';ru='Обновление классификаторов успешно завершено.';en='Classifier update completed successfully.'");
...
Функция ОбновлениеНеТребуется(Наименование)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='%1 (la renovación no es necesario)';ru='%1 (обновление не требуется)';en='% 1 (no update required)'"),
		Наименование);
