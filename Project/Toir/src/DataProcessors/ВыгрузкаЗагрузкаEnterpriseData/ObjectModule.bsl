...
Функция КомпонентыОбмена(НаправлениеОбмена, ВерсияФорматаОбменаПриЗагрузке = "") 
	...
	КонецЕсли;
	КомпонентыОбмена.КлючСообщенияЖурналаРегистрации = НСтр("es='El traslado de los datos a través del portapapeles del cambio';ru='Перенос данных через буфер обмена';en='Transferring data through the clipboard'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если МенеджерОбменаВнутренний Тогда
		...
		Если КомпонентыОбмена.МенеджерОбмена = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No es apoyada la versión del formato del cambio: <%1>.';ru='Не поддерживается версия формата обмена: <%1>.';en='The exchange format version is not supported: <%1>.'"), ТекВерсияФормата);
		КонецЕсли;
	КонецЕсли;
...
Функция РезультатВыгрузкиВXML(СтруктураПараметров, АдресДляРазмещенияРезультата = Неопределено) Экспорт
	...
	Если РезультатВыгрузки.ЕстьОшибки Тогда
		ТекстСообщения = НСтр("es='Durante la ejecución de la operación han surgido las faltas';ru='В ходе выполнения операции возникли ошибки';en='Errors occurred during the operation'") + ": "
			+ Символы.ПС + РезультатВыгрузки.ТекстОшибки
			+ Символы.ПС + НСтр("es='Los datos pueden ser descargados no por completo.';ru='Данные могут быть выгружены не полностью.';en='Data may not be fully downloaded.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
	ИначеЕсли Не РезультатВыгрузки.ЕстьВыгруженныеОбъекты Тогда
		ТекстСообщения = НСтр("es='No es encontrado ni un objeto a la descarga.';ru='Не найдено ни одного объекта к выгрузке.';en='No objects were found for unloading.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
"%2.';en='Event:%1."
"Handler: ПередКонвертацией.                                                Handler execution error."
"%2.'"),
...
Функция ЗагрузитьДанныеИзXML(ЧтениеXML)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента
		...
		РезультатЗагрузки.ТекстОшибки = НСтр("es='La falta de la lectura del mensaje de la carga. El formato equivocado del mensaje.';ru='Ошибка чтения сообщения загрузки. Неверный формат сообщения.';en='Error reading download message. Invalid message format.'");
...
Процедура ЗаполнениеПравилВыгрузки() Экспорт
	...
	УзелДереваСправочники.Наименование = НСтр("es='Las guías';ru='Справочники';en='Directories'");
	
	УзелДереваДокументы = ТаблицаПравилВыгрузки.Строки.Добавить();
	УзелДереваДокументы.ЭтоГруппа = Истина;
	УзелДереваДокументы.Наименование = НСтр("es='Los documentos';ru='Документы';en='Documents'");

	УзелДереваПВХ = ТаблицаПравилВыгрузки.Строки.Добавить();
	УзелДереваПВХ.ЭтоГруппа = Истина;
	УзелДереваПВХ.Наименование = НСтр("es='Los planes de los tipos de las características';ru='Планы видов характеристик';en='Features types plans'");
	...
	Для Каждого СтрокаПОД Из КомпонентыОбмена.ПравилаОбработкиДанных Цикл
		...
		Если Метаданные.Справочники.Содержит(ТекМД) Тогда
			...
			Представление = НСтр("es='La guía %1';ru='Справочник %1';en='Reference%1'");
		ИначеЕсли Метаданные.Документы.Содержит(ТекМД) Тогда
			...
			Представление = НСтр("es='El documento %1';ru='Документ %1';en='Document %1'");
...
Процедура ДобавитьДополнительныеНаборыКомпоновкиТабличныхЧастей(ЭлементыИсточника, ПараметрыДобавления)
	...
	Если МетаданныеОбъекта = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El nombre incorrecto metadado ""%1""';ru='Некорректное имя метаданных ""%1""';en=""Invalid metadata name '%1'"""),
...
Функция РазложитьФорматОбмена(Знач ФорматОбмена)
	...
	Если Версии.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El nombre Nekanonichesky del formato del cambio <%1>';ru='Неканоническое имя формата обмена <%1>';en='Non-canonical exchange format name <%1>'"), Результат.Версия);
	КонецЕсли;
