...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Исключение
		...
		ВызватьИсключение НСтр("es='En la base informativa no es apoyada la sincronización de los datos a través del formato universal';ru='В информационной базе не поддерживается синхронизация данных через универсальный формат';en='The data base does not support data synchronization through a universal format'");
	КонецПопытки;
	...
	Если ВариантОткрытияФормы = "ТолькоЗагрузка" Тогда
		ЭтотОбъект.Заголовок = НСтр("es='La carga dado EnterpriseData';ru='Загрузка данных EnterpriseData';en='Download EnterpriseData data'");
		Элементы.НадписьЗагрузкаВстроеннойОбработкой.Видимость = Истина;
	Иначе
		Элементы.НадписьЗагрузкаВстроеннойОбработкой.Видимость = Ложь;
		ЭтотОбъект.Заголовок = НСтр("es='La descarga y la carga dado EnterpriseData';ru='Выгрузка и загрузка данных EnterpriseData';en='Upload and Download EnterpriseData Data'");
	КонецЕсли;
	...
	Если МассивДоступныхВерсий.Количество() = 0 Тогда
		СтрокаПоддерживаемыеВерсииФормата = НСтр("es='No son descubiertas las versiones apoyadas del formato';ru='Не обнаружены поддерживаемые версии формата';en='No supported format versions found'");
		Элементы.СтрокаПоддерживаемыеВерсииФормата.ЦветТекста = Новый Цвет(255,0,0);
		Элементы.ФормаВыполнитьОперацию.Доступность = Ложь;
	Иначе
		СтрокаПоддерживаемыеВерсииФормата = НСтр("es='Las versiones apoyadas del formato:';ru='Поддерживаемые версии формата:';en='Supported format versions:'");
...
Процедура ПрерватьВыгрузкуЗагрузку(Команда)
	...
	Сообщение.Текст = НСтр("es='La ejecución de la operación es interrumpida.';ru='Выполнение операции прервано.';en='Operation aborted.'");
	Сообщение.Сообщить();
...
Процедура СохранитьНастройки(Команда)
	...
	ПолучитьФайл(АдресФайлаВХранилище, НСтр("es='El fichero настроек.xml';ru='Файл настроек.xml';en='Settings file.xml'"));
КонецПроцедуры
...
Процедура ЗагрузитьСообщение()
	Элементы.ДекорацияПоясняющийТекстДлительнойОперации.Заголовок = НСтр("es='Se cumple la carga de los datos...';ru='Выполняется загрузка данных...';en='Data loading in progress ...'");
	...
	Если ИсточникЗагрузки = 1 Тогда
		Если НЕ ЗначениеЗаполнено(ДанныеДляЗагрузкиXDTO) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("es='El campo textual con los datos para la carga no es rellenado.';ru='Текстовое поле с данными для загрузки не заполнено.';en='The text field with the data to download is empty.'"));
			Возврат;
		КонецЕсли;
...
Процедура ПриЗавершенииОперацииЗагрузки(Результат, ДополнительныеПараметры) Экспорт
	Если Результат = Неопределено Тогда
		ВызватьИсключение НСтр("es='La carga de los datos es acabada desgraciadamente.';ru='Загрузка данных завершена неудачно.';en='Data loading failed.'");
...
Процедура ВыгрузитьДанные()
	Элементы.ДекорацияПоясняющийТекстДлительнойОперации.Заголовок = НСтр("es='Se cumple la descarga de los datos...';ru='Выполняется выгрузка данных...';en='Uploading data ...'");
	Элементы.ВыгрузкаЗагрузка.ТекущаяСтраница = Элементы.Ожидание;
	УстановитьВидимостьДоступностьКнопок(Ложь);
	МассивСообщений = Неопределено;
...
Процедура ПриЗавершенииОперацииВыгрузки(Результат, ДополнительныеПараметры) Экспорт
	Если Результат = Неопределено Тогда
		ВызватьИсключение НСтр("es='La descarga de los datos es acabada desgraciadamente. Falta el resultado de la descarga.';ru='Выгрузка данных завершена неудачно. Отсутствует результат выгрузки.';en='Data upload failed. Missing upload result.'");
	ИначеЕсли Результат.Статус = "Ошибка" Тогда
		ВызватьИсключение Результат.КраткоеПредставлениеОшибки;
	КонецЕсли;
...
Процедура ПодключитьРасширениеРаботыСФайламиЗавершение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Не Подключено Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='Para la continuación es necesario establecer la ampliación del trabajo con los ficheros.';ru='Для продолжения необходимо установить расширение работы с файлами.';en='To continue, you must install the file extension.'"));
		Возврат;
	КонецЕсли;
	
	ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие);
	ДиалогОткрытияФайла.Фильтр = НСтр("es='Las cargas dadas';ru='Данные загрузки';en='Download Data'")+ "(*.xml)|*.xml";
...
Процедура ВыбратьФайлДляВыгрузкиНаКлиенте(ВыгружатьПослеВыбора = Ложь, АдресХраненияРезультата = "")
	...
	ДиалогОткрытияФайла.Фильтр = НСтр("es='Las descargas dadas';ru='Данные выгрузки';en='Upload data'")+ "(*.xml)|*.xml";
...
Функция ВыгрузитьДанныеНаСервере()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='La descarga dado EnterpriseData';ru='Выгрузка данных EnterpriseData';en='EnterpriseData Data Upload'");
...
Функция ЗагрузитьДанныеНаСервере()
	...
	МодульДлительныеОперации = ОбщегоНазначения.ОбщийМодуль("ДлительныеОперации");
	ПараметрыВыполнения = МодульДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='La carga dado EnterpriseData';ru='Загрузка данных EnterpriseData';en='Download EnterpriseData data'");
	РезультатФоновогоЗадания = МодульДлительныеОперации.ВыполнитьВФоне(ВыполняемыйМетод, ПараметрыЗадания, ПараметрыВыполнения);
	Возврат РезультатФоновогоЗадания;
	...
	Для Каждого СтрГруппыМД Из Объект.ТаблицаПравилВыгрузки.ПолучитьЭлементы() Цикл
		...
		Для Каждого СтрМД Из СтрГруппыМД.ПолучитьЭлементы() Цикл
			...
			ИначеЕсли СтрокиДополнения.Количество() = 0 Тогда
				...
				НовСтрока.ОтборСтрокой = НСтр("es='Todos los objetos';ru='Все объекты';en='All objects'");
...
Процедура НачалоВыбораМодуляМенеджера(РеквизитМодульМенеджера, СтандартнаяОбработка, ОбновлятьВыгрузку)
	...
	ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие);
	ДиалогОткрытияФайла.Фильтр = НСтр("es='El manager del cambio (*.epf)';ru='Менеджер обмена (*.epf)';en='Exchange manager (* .epf)'") + "|*.epf" ;
	ОповещениеВыбора = Новый ОписаниеОповещения("ВыбранФайл", ЭтотОбъект, 
...
Процедура СообщитьОбОкончанииОперации(Загрузка = Ложь)
	...
	Сообщение = Новый СообщениеПользователю();
	Если Загрузка Тогда
		Сообщение.Текст = НСтр("es='La carga de los datos es acabada.';ru='Загрузка данных завершена.';en='Data loading completed.'");
	Иначе
		Сообщение.Текст = НСтр("es='La descarga de los datos es acabada.';ru='Выгрузка данных завершена.';en='Data upload completed.'");
	КонецЕсли;
	Сообщение.Сообщить();
КонецПроцедуры
