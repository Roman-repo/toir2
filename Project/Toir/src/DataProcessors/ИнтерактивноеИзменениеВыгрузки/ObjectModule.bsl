...
Функция ЗаголовокГруппыОтбораВсехДокументов() Экспорт
	Возврат НСтр("es='Todos los documentos';ru='Все документы';en='All documents'");
КонецФункции
...
Функция ЗаголовокГруппыОтбораВсехСправочников() Экспорт
	Возврат НСтр("es='Todas las guías';ru='Все справочники';en='All directories'");
КонецФункции
...
Функция ИнициализироватьКомпоновщик(СписокИменМетаданных = Неопределено, ОграничиватьИспользованиеОтбором = Ложь, АдресСохраненияСхемы = Неопределено)
	...
	ПараметрАвтоматически.Значение = НСтр("es='Por las reglas generales';ru='По общим правилам';en='According to the general rules'");
	
	ПараметрДополнительно = Параметры.Найти("ПричинаРегистрацииДополнительно");
	ПараметрДополнительно.Значение = НСтр("es='Complementariamente';ru='Дополнительно';en='Additionally'");
	
	ПараметрПоСсылке = Параметры.Найти("ПричинаРегистрацииПоСсылке");
	ПараметрПоСсылке.Значение = НСтр("es='Por la referencia';ru='По ссылке';en='By link'");
...
Процедура ДобавитьДополнительныеНаборыКомпоновкиТабличныхЧастей(ЭлементыИсточника, ПараметрыДобавления)
	...
	Если МетаданныеОбъекта = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El nombre incorrecto de los metadatos ""%1"" para el registro sobre el nudo ""%2""';ru='Некорректное имя метаданных ""%1"" для регистрации на узле ""%2""';en='Invalid metadata name ""%1"" to register on node ""%2""'"),
...
Функция ТекстДополнительныхПараметров()
	...
	Если ВариантВыгрузки = 0 Тогда
		...
		Возврат НСтр("es='Sin datos adicionales.';ru='Без дополнительных данных.';en='Without additional data.'");
