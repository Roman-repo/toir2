...
Процедура ВариантыНастроекПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса   = НСтр("es='¿Quitar el ajuste ""%1""?';ru='Удалить настройку ""%1""?';en='Delete setting ""%1""?'");
...
Процедура СохранитьНастройку(Команда)
	...
	Если ПустаяСтрока(ПредставлениеТекущейНастройки) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='el nombre para el ajuste corriente.';ru='Не заполнено имя для текущей настройки.';en='The name for the current setting is not filled.'"), , "ПредставлениеТекущейНастройки");
		Возврат;
	КонецЕсли;
		
	Если ВариантыНастроек.НайтиПоЗначению(ПредставлениеТекущейНастройки)<>Неопределено Тогда
		ТекстЗаголовка = НСтр("es='La confirmación';ru='Подтверждение';en='Confirmation'");
		ТекстВопроса   = НСтр("es='¿Regrabar el ajuste existente ""%1""?';ru='Перезаписать существующую настройку ""%1""?';en='Overwrite existing setting ""%1""?'");
