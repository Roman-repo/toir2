...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.УзелИнформационнойБазы) Тогда
		Текст = НСтр("es='El ajuste del cambio para los datos no es encontrado.';ru='Настройка обмена данными не найдена.';en='Communication settings not found.'");
...
Процедура ДополнительнаяРегистрацияПередУдалением(Элемент, Отказ)
	...
	Если Количество>1 Тогда
		ТекстПредставления = НСтр("es='Las líneas escogidas';ru='выбранные строки';en='selected rows'");
	...
	ТекстВопроса = НСтр("es='¿Quitar de los datos adicionales %1?';ru='Удалить из дополнительных данных %1 ?';en='Delete%1 from additional data?'");    
	...
	ИначеЕсли ТипВыбранного= Тип("Структура") Тогда
		Если ВыбранноеЗначение.ДействиеВыбора=3 Тогда
			...
			Если Не ПустаяСтрока(ПредставлениеТекущейНастройки) И ПредставлениеНастройки<>ПредставлениеТекущейНастройки Тогда
				ТекстВопроса  = НСтр("es='¿Restablecer los ajustes ""%1""?';ru='Восстановить настройки ""%1""?';en=""Restore settings to '%1'?""");
...
Функция РедактированиеОтбораСтрокиДополнительныйСоставСервер(СтруктураВыбора)
	...
	ТекущиеДанные.Количество   = НСтр("es='No es contado';ru='Не рассчитано';en='Not calculated'");
...
Функция ДобавитьВДополнительныйСоставВыгрузки(Элемент)
	...
	Иначе
		...
		Строка.Количество = НСтр("es='No es contado';ru='Не рассчитано';en='Not calculated'");
		ОбновитьНадписьОбщегоКоличества();
	КонецЕсли;
...
Функция ОбновитьКоличествоСервер()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = 
		НСтр("es='El cálculo de la cantidad de los objetos para el envío a la sincronización';ru='Расчет количества объектов для отправки при синхронизации';en='Calculation of the number of objects to send during synchronization'");
...
Процедура ОбновитьНадписьОбщегоКоличества(Количество = Неопределено) 
	...
	Если Количество = Неопределено Тогда
		ТекстКоличества = НСтр("es='<no es contado>';ru='<не рассчитано>';en='<не рассчитано>'");
	Иначе
		ТекстКоличества = НСтр("es='De los objetos: %1';ru='Объектов: %1';en='%1 objects'");
...
Процедура СброситьНадписьКоличествТаблицы()
	ТекстКоличеств = НСтр("es='No es contado';ru='Не рассчитано';en='Not calculated'");
	Для Каждого Строка Из Объект.ДополнительнаяРегистрация Цикл
