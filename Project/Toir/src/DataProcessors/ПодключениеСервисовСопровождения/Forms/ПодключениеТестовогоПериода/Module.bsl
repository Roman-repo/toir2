...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Параметры.Идентификатор) Тогда
		ТекстИсключения = НСтр("es='del acompañamiento.';ru='Не заполнен идентификатор сервиса сопровождения.';en='The escort service identifier is not filled.'");
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы Тогда
		Если ПоказатьВопросПриЗавершенииРаботыПрограммы() Тогда
			Отказ = Истина;
			ТекстПредупреждения = НСтр("es='La conexión del período de test del servicio no es acabada.';ru='Подключение тестового периода сервиса не завершено.';en='The connection of the test period of the service is not completed.'");
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Функция ПодключитьТестовыйПериодНаСервере(
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='La conexión del período de test (activate).';ru='Подключение тестового периода (activate).';en='Connection of the test period (activate).'");
...
Функция ПроверитьПодключениеТестовогоПериодаНаСервере(
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='La comprobación del estado de la conexión del período de test.';ru='Проверка состояния подключения тестового периода.';en='Check the connection status of the test period.'");
...
Процедура ОтобразитьРезультатПодключения(ПараметрыОтображения)
	...
	ИначеЕсли СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияПодключенияСервисов.Подключен") Тогда
		Элементы.ДекорацияНадписьПодключен.Заголовок = НСтр("es='El servicio es con éxito conectado.';ru='Сервис успешно подключен.';en='Service successfully connected.'");
...
Процедура НастроитьОтображениеФормы(Форма)
	...
	ИначеЕсли СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияПодключенияСервисов.ПодключениеНедоступно") Тогда
		Элементы.ДекорацияИнформацияОбОшибке.Заголовок = НСтр("es='La falta a la recepción de los períodos accesibles de test: no son descubiertos "
"los períodos accesibles de test.';ru='Ошибка при получении доступных тестовых периодов:"
"Не обнаружены доступные тестовые периоды.';en='Error getting available test periods:"
"No available test periods found.'")
	КонецЕсли;
...
Процедура УстановитьВидимостьДоступность(Форма, Знач СостояниеПодключения, Знач Подключение = Ложь)
	...
	ИначеЕсли СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияПодключенияСервисов.ОшибкаПодключения") Тогда
		...
		Элементы.ФормаОтмена.Заголовок               = НСтр("es='Cerrar';ru='Закрыть';en='Close'");
		Элементы.ФормаПодключить.Заголовок           = НСтр("es='Cerrar';ru='Закрыть';en='Close'");
	ИначеЕсли СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияПодключенияСервисов.Подключение") Тогда
		Элементы.СтраницыПодключения.ТекущаяСтраница = Элементы.ГруппаОшибкаТаймаут;
		Элементы.ФормаОтмена.Видимость               = Истина;
		Элементы.ФормаОтмена.Заголовок               = НСтр("es='Cerrar';ru='Закрыть';en='Close'");
		Элементы.ФормаПодключить.Видимость           = Истина;
		Элементы.ФормаПодключить.Заголовок           = НСтр("es='Comprobar la conexión';ru='Проверить подключение';en='Check connection'");
	ИначеЕсли СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияПодключенияСервисов.Подключен") Тогда
		Элементы.СтраницыПодключения.ТекущаяСтраница = Элементы.ГруппаПодключенТестовыйПериод;
		Элементы.ФормаПодключить.Видимость           = Истина;
		Элементы.ФормаОтмена.Видимость               = Ложь;
		Элементы.ФормаПодключить.Заголовок           = НСтр("es='OK';ru='OK';en='OK'");
	ИначеЕсли СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияПодключенияСервисов.ПодключениеНедоступно") Тогда
		Элементы.ФормаПодключить.Видимость           = Ложь;
	ИначеЕсли ПодключениеДоступно(СостояниеПодключения) Тогда
		...
		Элементы.ФормаПодключить.Видимость           = Истина;
		Элементы.ФормаОтмена.Видимость               = Истина;
		Элементы.ФормаОтмена.Заголовок               = НСтр("es='Anular';ru='Отменить';en='Cancel'");
		Элементы.ФормаПодключить.Заголовок           = НСтр("es='Anular';ru='Отменить';en='Cancel'");
	КонецЕсли;
...
Процедура ЗаполнитьДанныеШапкиФормы(ОписательСервиса)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La conexión del servicio %1';ru='Подключение сервиса %1';en='Service connection %1'"),
	...
	ЧастиОписания.Добавить(Новый ФорматированнаяСтрока(НСтр("es='Las posibilidades del servicio:';ru='Возможности сервиса:';en='Service features:'"), Новый Шрифт(,,Истина)));
...
Процедура ДобавитьЭлементыТестовыхПериодов(ОписательПодключения, ОписательСервиса)
	...
	ЧастиОписания.Добавить(Новый ФорматированнаяСтрока(
		НСтр("es='Conecten';ru='Подключите';en='Connect'")
			+ " ",
		ЖирныйШрифт));
	ЧастиОписания.Добавить(
		Новый ФорматированнаяСтрока(
			НСтр("es='Gratis';ru='бесплатно';en='is free'"),
		ЖирныйШрифт,
		,
		,
		ОписательСервиса.URLУсловияПолучения));
	ЧастиОписания.Добавить(НСтр("es=':';ru=':';en=':'"));
	
	// Создание новых элементов форм.
	ГруппаФормы  = Элементы.ГруппаДоступныеТестовыеПериоды;
	НовыйЭлемент = Элементы.Добавить(НСтр("es='ЗаголовокТестовыеПериоды';ru='ЗаголовокТестовыеПериоды';en='ЗаголовокТестовыеПериоды'"), Тип("ДекорацияФормы"), ГруппаФормы);
	...
	Если ОписательПодключения.ТестовыеПериоды.Количество() = 1 Тогда
		...
		ИмяПоляНаименования      = НСтр("es='ТестовыйПериод1';ru='ТестовыйПериод1';en='ТестовыйПериод1'");
		ИмяГруппыНаименования    = НСтр("es='ТестовыйПериод1';ru='ТестовыйПериод1';en='ТестовыйПериод1'");
		ИмяГруппыОписания        = НСтр("es='ГруппаНаименование1';ru='ГруппаНаименование1';en='ГруппаНаименование1'");
		ИмяПоляОписания          = НСтр("es='ГруппаОписание1';ru='ГруппаОписание1';en='ГруппаОписание1'");
	...
	Иначе
		...
		Для Каждого ОписательТестовогоПериода Из ОписательПодключения.ТестовыеПериоды Цикл
			...
			ИмяПоляНаименования = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='ТестовыйПериод%1';ru='ТестовыйПериод%1';en='ТестовыйПериод%1'"),
				Сч);
			ИмяГруппыОписания   = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='ГруппаОписание%1';ru='ГруппаОписание%1';en='ГруппаОписание%1'"),
				Сч);
			ИмяПоляОписания     = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Описание%1';ru='Описание%1';en='Description%1'"),
				Сч);
