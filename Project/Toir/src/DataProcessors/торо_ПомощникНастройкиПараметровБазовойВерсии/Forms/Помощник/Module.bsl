...
Процедура ЗавершитьНастройку(Команда)
	...
	ПоказатьОповещениеПользователя(НСтр("es='El ajuste del programa es acabado';ru='Настройка программы завершена';en='Program setup completed'"), , НСтр("es='El ajuste necesario es acabado.';ru='Минимально необходимая настройка завершена.';en='The minimum required setup is complete.'"), БиблиотекаКартинок.Информация32);
	Закрыть();
КонецПроцедуры
...
Процедура ОбновитьСтрокуРеквизитовОрганизации(Форма, РеквизитыОрганизации = Неопределено)
	...
	Если Не РеквизитыОрганизации = Неопределено Тогда
		СтрокаРеквизитовОрганизации = СтрокаРеквизитовОрганизации + 
			?(ПустаяСтрока(РеквизитыОрганизации.ИНН), "", НСтр("es='EL INN:';ru='ИНН:';en='TIN:'") + Символы.НПП + РеквизитыОрганизации.ИНН); 
		СтрокаРеквизитовОрганизации = СтрокаРеквизитовОрганизации + 
			?(ПустаяСтрока(РеквизитыОрганизации.КПП), "", 
			?(ПустаяСтрока(СтрокаРеквизитовОрганизации), "", ";" + Символы.НПП) +
			НСтр("es='КПП:';ru='КПП:';en='PPC:'") + Символы.НПП + РеквизитыОрганизации.КПП); 
		
		СтрокаРеквизитовОрганизации = СтрокаРеквизитовОрганизации + 
		?(ПустаяСтрока(РеквизитыОрганизации.Префикс), "", 
				?(ПустаяСтрока(СтрокаРеквизитовОрганизации), "", ";" + Символы.НПП) + 
				НСтр("es='El prefijo:';ru='Префикс:';en='Prefix:'") + Символы.НПП + РеквизитыОрганизации.Префикс); 
	КонецЕсли;
...
Функция ПроверитьЗаполнениеКлиент()
	...
	Если ИмяСтраницыПерехода = "СтраницаОбщаяИнформация" Тогда
		Если ПустаяСтрока(НаименованиеПользователя) Тогда
			ЕстьОшибки = Истина;
			ОписаниеОшибки = НСтр("es='el usuario.';ru='Не заполнен пользователь.';en='The user is not full.'"); 
		...
		Если ПустаяСтрока(ПользовательИнфБазыИмя) Тогда
			...
			ОписаниеОшибки = ОписаниеОшибки + 
				...
				НСтр("es='la entrada.';ru='Не заполнено имя для входа.';en='Not filled name for login).'"); 
		...
		Если Не Пароль = ПодтверждениеПароля Тогда
			...
			ОписаниеОшибки = ОписаниеОшибки + 
				?(ПустаяСтрока(ОписаниеОшибки),"", Символы.ПС) + 
				НСтр("es='Las consignas introducidas no coinciden.';ru='Введенные пароли не совпадают.';en='The entered passwords do not match.'"); 
	...
	ИначеЕсли ИмяСтраницыПерехода = "СтраницаСведенияОрганизации" Тогда
		Если ПустаяСтрока(Наименование) Тогда
			ЕстьОшибки = Истина;
			ОписаниеОшибки = НСтр("es='de la organización.';ru='Не заполнено наименование организации.';en='The name of the organization is not filled.'"); 
		КонецЕсли;
	ИначеЕсли ИмяСтраницыПерехода = "СтраницаТОиР" Тогда
		Если НаборКонстант.ВалютаУправленческогоУчета.Пустая() Тогда
			ЕстьОшибки = Истина;
			ОписаниеОшибки = НСтр("es='del recuento de administración.';ru='Не заполнена валюта управленческого учета.';en='Management accounting currency is not filled.'"); 
		КонецЕсли;
		Если НаборКонстант.торо_ИерархияДляВводаНовыхОР.Пустая() Тогда
			ЕстьОшибки = Истина;
			ОписаниеОшибки = НСтр("es='de la reparación.';ru='Не заполнена иерархия для ввода новых объектов ремонта.';en='The hierarchy for entering new repair objects is not filled.'"); 
		КонецЕсли;
	КонецЕсли;
...
Процедура ВидимостьДоступностьСтильКонтактнойИнформации(ИмяГруппыКонтактнойИнформации)
	...
	Для Каждого Элемент Из ЭлементыКонтактнойИнформации Цикл 
		...
		Если ОписаниеЭлемента.ТипЭлемента = Перечисления.ТипыКонтактнойИнформации.Другое
			ИЛИ Найти(ОписаниеЭлемента.Представление, НСтр("es='Otra información';ru='Другая информация';en='Other information'")) > 0 
			ИЛИ Найти(ОписаниеЭлемента.Представление, НСтр("es='La dirección postal';ru='Почтовый адрес';en='Mail address'")) > 0 Тогда
...
Процедура УстановитьНазваниеКомандыРеквизитовОрганизации(ЭтаФорма)
	Если ПустаяСтрока(ЭтаФорма.СтрокаРеквизитовОрганизации) Тогда
		ЭтаФорма.Элементы.ИзменитьРеквизитыОрганизации.Заголовок = НСтр("es='Indicar';ru='Указать';en='Indicate'");
	Иначе
		ЭтаФорма.Элементы.ИзменитьРеквизитыОрганизации.Заголовок = НСтр("es='Cambiar';ru='Изменить';en='Change'");
	КонецЕсли;
КонецПроцедуры
