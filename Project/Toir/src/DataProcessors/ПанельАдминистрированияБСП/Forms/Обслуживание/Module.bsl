...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОбновлениеКонфигурации") Тогда
		...
		Если СтандартныеПодсистемыСервер.ЭтоБазоваяВерсияКонфигурации() Тогда
			Элементы.УстановкаОбновлений.Заголовок = НСтр("es='La instalación de las renovaciones';ru='Установка обновлений';en='Installing updates'");
			Элементы.УстановкаОбновлений.РасширеннаяПодсказка.Заголовок =
				НСтр("es='La renovación del programa del fichero sobre el disco local o en el catálogo de red.';ru='Обновление программы из файла на локальном диске или в сетевом каталоге.';en='Updating a program from a file on a local disk or in a network directory.'");
		КонецЕсли;
	Иначе
	...
	дс_ЛокализоватьСпособРезервногоКопирования();
	// << [Деснол] [Ушаков А.Д.] [03.10.2019] 
КонецПроцедуры
...
// >> [Деснол] [Ушаков А.Д.] [03.10.2019]
&НаСервере
Процедура дс_ЛокализоватьСпособРезервногоКопирования()
	СписокВыбора = Элементы.СпособРезервногоКопирования.СписокВыбора;
	ПереводыСписка = Новый Соответствие;
	
	ПереводыСписка.Вставить("На локальном компьютере", 	НСтр("es='En la computadora local';ru='На локальном компьютере';en='On the local computer'"));
	ПереводыСписка.Вставить("1С:Облачный архив", 		НСтр("es='1C: Archivo en la nube';ru='1С:Облачный архив';en='1C: Cloud archive'"));
	
	Для каждого ПунктСписка Из СписокВыбора Цикл
		ПунктСписка.Представление = ПереводыСписка[ПунктСписка.Значение];
	КонецЦикла;
КонецПроцедуры
// << [Деснол] [Ушаков А.Д.] [03.10.2019] 
