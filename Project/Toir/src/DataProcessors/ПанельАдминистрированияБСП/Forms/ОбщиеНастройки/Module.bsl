...
Процедура АдресПубликацииИнформационнойНачалоВыбораЗавершение(ИмяРеквизита)
	...
	Иначе
		ПоказатьПредупреждение(, НСтр("es='No consiguió automáticamente rellenar el campo, porque la aplicación de clientes no es conectada a través del servidor web.';ru='Не удалось автоматически заполнить поле, т.к. клиентское приложение не подключено через веб-сервер.';en='Failed to automatically fill in the field because the client application is not connected through the web server.'"));
	КонецЕсли;
