...
Функция ЗапуститьИзменениеПрефиксаИБВФоновомЗадании()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='El cambio del prefijo';ru='Изменение префикса';en='Prefix change'");
...
Процедура ПослеИзмененияПрефикса(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Если ФоновоеЗадание <> Неопределено
		...
		ПоказатьОповещениеПользователя(НСтр("es='El prefijo es cambiado.';ru='Префикс изменен.';en='Prefix is changed.'"));
	...
	Иначе
		...
		Если ФоновоеЗадание <> Неопределено Тогда
			ТекстОшибки = НСтр("es='No consiguió cambiar el prefijo. "
"Véase los detalles en la revista del registro.';ru='Не удалось изменить префикс."
"См. подробности в журнале регистрации.';en='Failed to change the prefix."
...
Процедура УстановитьВидимость()
	...
	Если РазделениеВключено Тогда
		Элементы.ОписаниеРаздела.Заголовок = НСтр("es='La sincronización de los datos con mis aplicaciones.';ru='Синхронизация данных с моими приложениями.';en='Data synchronization with my applications.'");
	КонецЕсли;
	...
	Если ПодсистемаСуществуетОбменДанными Тогда
		...
		Если РазделениеВключено Тогда
			...
			Элементы.ПрефиксУзлаРаспределеннойИнформационнойБазы.Заголовок = НСтр("es='El prefijo en este programa';ru='Префикс в этой программе';en='Prefix in this program'");
		Иначе
	...
	Если ПодсистемаСуществуетДатыЗапретаИзменения Тогда
		...
		Если РазделениеВключено
			...
			Элементы.ИспользоватьДатыЗапретаЗагрузки.РасширеннаяПодсказка.Заголовок =
				НСтр("es='La prohibición de la carga de los períodos dados pasados de otras aplicaciones. "
"No influye sobre la carga de los datos de los puestos de trabajo autónomos.';ru='Запрет загрузки данных прошлых периодов из других приложений."
"Не влияет на загрузку данных из автономных рабочих мест.';en='Prohibition of downloading historical data from other applications."
"Does not affect data loading from offline workstations. '");
		КонецЕсли;
	Иначе
