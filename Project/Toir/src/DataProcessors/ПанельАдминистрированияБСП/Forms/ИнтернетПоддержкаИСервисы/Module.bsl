...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВызовОнлайнПоддержки") Тогда
		Элементы.ГруппаИнтеграцияВызовОнлайнПоддержки.Видимость =
		   // ТОИР ++ 
			// Закомментировано до полного внедрения подсистемы, чтобы не вызывать вопросов у пользователей.
...
Процедура ЦентрМониторингаАдресСервисаПриИзменении(Элемент)
	...
	Исключение
		...
		ОписаниеОшибки = НСтр("es='La dirección del servicio';ru='Адрес сервиса';en='Service address'") + " "
			+ ЦентрМониторингаАдресСервиса + " "
			+ НСтр("es='No es la dirección admisible del servicio web para el envío de los informes del uso del programa.';ru='не является допустимым адресом веб-сервиса для отправки отчетов об использовании программы.';en='is not a valid web service address for sending reports on the use of the program.'"); 
...
Процедура ПриИзмененииСостоянияПодключенияОбсуждений(ОбсужденияПодключены = Неопределено)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.Обсуждения") Тогда
		...
		Если ОбсужденияПодключены Тогда 
			Элементы.ПодключитьОтключитьОбсуждения.Заголовок = НСтр("es='Desconectar';ru='Отключить';en='Disable'");
			Элементы.СостояниеПодключенияОбсуждений.Заголовок = НСтр("es='Las discusiones son conectadas.';ru='Обсуждения подключены.';en='Discussions are connected.'");
		Иначе 
			Элементы.ПодключитьОтключитьОбсуждения.Заголовок = НСтр("es='Conectar';ru='Подключить';en='Connect'");
			Элементы.СостояниеПодключенияОбсуждений.Заголовок = НСтр("es='La conexión de las discusiones no es cumplida.';ru='Подключение обсуждений не выполнено.';en='Discussion connection failed.'");
		КонецЕсли;
