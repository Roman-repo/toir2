...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСФайлами") Тогда
		ВызватьИсключение НСтр("es='los ficheros. "
"Se Recomienda desconectar la visibilidad de este panel.';ru='Не внедрена подсистема Работа с файлами."
"Рекомендуется отключить видимость этой панели.';en='The subsystem Work with files is not implemented."
"It is recommended that you disable the visibility of this panel. '");
	КонецЕсли;
...
Процедура ХранитьФайлыВТомахНаДискеПриИзменении(Элемент)
	...
	Попытка
		...
		Если Не ЕстьТомаХраненияФайлов() И НаборКонстант.ХранитьФайлыВТомахНаДиске Тогда
			ПоказатьПредупреждение(, НСтр("es='Es incluido el almacenaje de los ficheros en los tomos sobre el disco, pero los tomos no son ajustados. "
"Los ficheros añadidos se conservarán en la base informativa hasta que sea ajustado por lo menos un tomo del almacenaje de los ficheros.';ru='Включено хранение файлов в томах на диске, но тома еще не настроены."
"Добавляемые файлы будут сохраняться в информационной базе до тех пор, пока не будет настроен хотя бы один том хранения файлов.';en='File storage on volumes on disk is enabled, but volumes have not yet been configured."
"The added files will be stored in the infobase until at least one file storage volume is configured.'"));
		КонецЕсли;
	Исключение
...
Процедура МаксимальныйРазмерФайлаОбластиДанныхПриИзменении(Элемент)
	Если МаксимальныйРазмерФайлаОбластиДанных = 0 Тогда
		ТекстСообщения = НСтр("es='Campo ""la dimensión Máxima del fichero"" no es rellenado.';ru='Поле ""Максимальный размер файла"" не заполнено.';en='Field ""Maximum file size"" is empty.'");
