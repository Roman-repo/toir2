...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Элементы.ГруппаАвтоматическоеИзвлечениеТекстов.Видимость Тогда
		...
		Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
			...
			СписокВыбора[0].Представление = НСтр("es='Todas las estaciones de trabajo trabajan bajo la dirección la SO Windows';ru='Все рабочие станции работают под управлением ОС Windows';en='All workstations are running Windows'");
			
			СписокВыбора = Элементы.ИзвлекатьТекстыФайловНаСервереLinux.СписокВыбора;
			СписокВыбора[0].Представление = НСтр("es='Uno o plusieurs las estaciones de trabajo trabajan bajo la dirección de la SO Linux';ru='Одна или несколько рабочих станций работают под управлением ОС Linux';en='One or more workstations running Linux'");
		КонецЕсли;
	...
	Иначе
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("es='La dirección полнотекстовым por la búsqueda';ru='Управление полнотекстовым поиском';en='Full text search control'");
		Элементы.ОписаниеРаздела.Заголовок =
			НСтр("es='La inclusión y la desconexión полнотекстового de la búsqueda, la renovación del índice полнотекстового de la búsqueda.';ru='Включение и отключение полнотекстового поиска, обновление индекса полнотекстового поиска.';en='Enabling and disabling full-text search, updating the full-text search index.'");
	КонецЕсли;
...
Процедура ОбновитьИндекс(Команда)
	...
	ПоказатьОповещениеПользователя(НСтр("es='La búsqueda polnotekstovyy';ru='Полнотекстовый поиск';en='Full text search'"),, НСтр("es='El índice es con éxito renovado';ru='Индекс успешно обновлен';en='Index updated successfully'"));
КонецПроцедуры

&НаКлиенте
Процедура ОчиститьИндекс(Команда)
	ОчиститьИндексСервер();
	ПоказатьОповещениеПользователя(НСтр("es='La búsqueda polnotekstovyy';ru='Полнотекстовый поиск';en='Full text search'"),, НСтр("es='El índice es con éxito limpiado';ru='Индекс успешно очищен';en='Index cleared successfully'"));
КонецПроцедуры
...
Процедура ПроверитьИндекс(Команда)
	...
	Исключение
		ТекстСообщенияОбОшибке = 
			НСтр("es='En la actualidad la comprobación del índice es imposible, puesto que se cumple su limpieza o la renovación.';ru='В настоящее время проверка индекса невозможна, так как выполняется его очистка или обновление.';en='Currently, checking the index is not possible because it is being cleaned or updated.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщенияОбОшибке);
	КонецПопытки;
	
	ПоказатьОповещениеПользователя(НСтр("es='La búsqueda polnotekstovyy';ru='Полнотекстовый поиск';en='Full text search'"),, НСтр("es='El índice contiene los datos correctos';ru='Индекс содержит корректные данные';en='The index contains valid data.'"));
КонецПроцедуры
...
Процедура Подключаемый_ПриИзмененииРеквизита(Элемент, ОбновлятьИнтерфейс = Истина)
	...
	Если Результат.Свойство("НеУдалосьУстановитьРежимПолнотекстовогоПоиска") Тогда
		...
		ТекстВопроса = НСтр("es='Para el cambio del régimen полнотекстового de la búsqueda es necesaria la terminación de las sesiones de todos los usuarios, excepto corriente.';ru='Для изменения режима полнотекстового поиска требуется завершение сеансов всех пользователей, кроме текущего.';en='Changing the full-text search mode requires ending sessions of all users except the current one.'");
		...
		ИначеЕсли РеквизитПутьКДанным = "МаксимальныйРазмерИндексируемыхДанных"
			...
			Исключение
				ЗаписьЖурналаРегистрации(
					НСтр("es='La búsqueda polnotekstovyy';ru='Полнотекстовый поиск';en='Full text search'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "", ИндексПроверен = Ложь)
	...
	Если РеквизитПутьКДанным = ""
		...
		Если ИспользоватьПолнотекстовыйПоиск = 1 Тогда
			...
			Если ИндексПроверен И Не ИндексСодержитКорректныеДанные Тогда
				СтатусИндекса = НСтр("es='Es necesaria la limpieza y la renovación';ru='Требуется очистка и обновление';en='Cleaning and updating required'");
			ИначеЕсли ИндексАктуален Тогда
				СтатусИндекса = НСтр("es='La renovación no es necesario';ru='Обновление не требуется';en='No update required'");
			Иначе
				СтатусИндекса = НСтр("es='Es necesaria la renovación';ru='Требуется обновление';en='Update required'");
			КонецЕсли;
		Иначе
			ДатаАктуальностиИндекса = '00010101';
			ИндексАктуален = Ложь;
			СтатусИндекса = НСтр("es='La búsqueda polnotekstovyy es desconectada';ru='Полнотекстовый поиск отключен';en='Full text search disabled'");
		КонецЕсли;
	...
	Если Элементы.ГруппаАвтоматическоеИзвлечениеТекстов.Видимость
		...
		Иначе
			РасписаниеПредставление = НСтр("es='La extracción automática de los textos no se cumple.';ru='Автоматическое извлечение текстов не выполняется.';en='Auto text extraction is not performed.'");
		КонецЕсли;
