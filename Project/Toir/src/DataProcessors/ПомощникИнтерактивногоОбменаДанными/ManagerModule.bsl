...
Процедура ПриНачалеРегистрацииДанных(НастройкиРегистрации, ПараметрыОбработчика, ПродолжитьОжидание = Истина) Экспорт
	...
	КлючФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El registro de los datos para la descarga (%1)';ru='Регистрация данных для выгрузки (%1)';en='Logging data for upload (%1)'"),
...
Процедура ПриНачалеВыгрузкиДанныхДляСопоставления(НастройкиВыгрузки, ПараметрыОбработчика, ПродолжитьОжидание = Истина) Экспорт
	...
	КлючФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La descarga de los datos para comparación (%1)';ru='Выгрузка данных для сопоставления (%1)';en='Unload data for matching (%1)'"),
		НастройкиВыгрузки.УзелОбмена);

	Если ЕстьАктивныеФоновыеЗадания(КлючФоновогоЗадания) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La descarga de los datos para la comparación para ""%1"" se cumple ya.';ru='Выгрузка данных для сопоставления для ""%1"" уже выполняется.';en='Data mapping for matching for ""%1"" is already in progress.'"),
			НастройкиВыгрузки.УзелОбмена);
	КонецЕсли;
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(Новый УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La descarga de los datos para comparación (%1).';ru='Выгрузка данных для сопоставления (%1).';en='Upload data for matching (%1).'"),
		НастройкиВыгрузки.УзелОбмена);
	ПараметрыВыполнения.КлючФоновогоЗадания = КлючФоновогоЗадания;
	ПараметрыВыполнения.ЗапуститьНеВФоне    = Ложь;
	ПараметрыВыполнения.ЗапуститьВФоне      = Истина;
...
Процедура ЗарегистрироватьДанныеДляВыгрузки(Параметры, АдресРезультата) Экспорт
	...
	Исключение
		...
		Результат.СообщениеОбОшибке = НСтр("es='Ha surgido el problema a la adición de los datos a la descarga:';ru='Возникла проблема при добавлении данных к выгрузке:';en='There was a problem adding data to the upload:'") 
			+ Символы.ПС + КраткоеПредставлениеОшибки(Информация)
			+ Символы.ПС + НСтр("es='Es necesario cambiar las condiciones de la selección.';ru='Необходимо изменить условия отбора.';en='Required to change the selection conditions.'");
...
Процедура ВыгрузитьДанныеДляСопоставления(Параметры, АдресРезультата) Экспорт
	...
	Если Не Результат.ДанныеВыгружены
		...
		Результат.СообщениеОбОшибке = НСтр("es='A la ejecución de la descarga de los datos para la comparación han surgido las faltas (cm. La revista del registro).';ru='При выполнении выгрузки данных для сопоставления возникли ошибки (см. Журнал регистрации).';en='Errors occurred while performing data upload for comparison (see Log).'");
	КонецЕсли;
...
Функция РезультатАвтоматическогоСопоставленияДанных(Знач Корреспондент,
	...
	Если Результат.Отказ Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='No consiguió cargar los datos de ""%1"" (la etapa de la comparación automática de los datos).';ru='Не удалось загрузить данные из ""%1"" (этап автоматического сопоставления данных).';en=""Failed to load data from '%1' (automatic data matching step)."""),
	...
	Для Каждого СтрокаТаблицы Из ИнформацияСтатистики Цикл
		...
		Исключение
			...
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta: tipo ""%1"" no es determinado.';ru='Ошибка: тип ""%1"" не определен.';en=""Error: type '%1' is not defined."""), СтрокаТаблицы.ТипОбъектаСтрокой);
			Прервать;
		КонецПопытки;
