...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Параметры.Свойство("КодУзлаИнформационнойБазы") Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.УзелИнформационнойБазы) Тогда
			ВызватьИсключение НСтр("es='El ajuste del cambio para los datos no es encontrado.';ru='Настройка обмена данными не найдена.';en='Communication settings not found.'");
		КонецЕсли;
	...
	Иначе
		...
		ВызватьИсключение НСтр("es='La apertura directa del ayudante no es prevista.';ru='Непосредственное открытие помощника не предусмотрено.';en='Direct opening of the assistant is not provided.'");
	...
	Если Не ОбменДаннымиПовтИсп.ЭтоУзелУниверсальногоОбменаДанными(Объект.УзелИнформационнойБазы) Тогда
		ВызватьИсключение НСтр("es='Para el nudo escogido la ejecución del cambio para los datos con el ajuste no es prevista.';ru='Для выбранного узла выполнение обмена данными с настройкой не предусмотрено.';en='For the selected node, data exchange with the setting is not provided.'");
	КонецЕсли;
	...
	Если ВсеУзлы.Найти(Объект.УзелИнформационнойБазы) = Неопределено Тогда
		ВызватьИсключение НСтр("es='Para el nudo escogido la comparación de los datos no es prevista.';ru='Для выбранного узла сопоставление данных не предусмотрено.';en='No data mapping is provided for the selected node.'");
	КонецЕсли;
	...
	Если Не ПолучитьДанные И Не ОтправитьДанные Тогда
		ВызватьИсключение НСтр("es='El guión dado de la sincronización de los datos no es apoyado.';ru='Заданный сценарий синхронизации данных не поддерживается.';en='The specified data synchronization script is not supported.'");
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ПоказатьПодтверждениеЗакрытияПроизвольнойФормы(ЭтотОбъект, Отказ, ЗавершениеРаботы,
		НСтр("es='¿Salir del ayudante?';ru='Выйти из помощника?';en='Leave Assistant?'"), "ЗакрытьФормуБезусловно");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗакрытиеФормыСопоставленияОбъектов" Тогда
		...
		Если Отказ Тогда
			ПоказатьПредупреждение(, НСтр("es='A la recepción de la información de la estadística han surgido las faltas.';ru='При получении информации статистики возникли ошибки.';en='Errors occurred while getting statistics information.'"));
		Иначе
			
			РазвернутьДеревоИнформацииСтатистики(Параметр.КлючУникальности);
			
			ПоказатьОповещениеПользователя(НСтр("es='La recogida de la información es acabada';ru='Сбор информации завершен';en='Information collection completed'"));
		КонецЕсли;
...
Процедура ВыполнитьЗагрузкуДанныхДляСтроки(Команда)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		НСтрока = НСтр("es='Escojan el nombre de la tabla en el campo de la información estadística.';ru='Выберите имя таблицы в поле статистической информации.';en='Select a table name in the statistical information field.'");
	...
	Если ЕстьНесопоставленныеОбъекты Тогда
		НСтрока = НСтр("es='Hay unos objetos no confrontados. "
"A la carga de los datos serán creadas las tomas de los objetos no confrontados. ¿Continuar?';ru='Имеются несопоставленные объекты."
"При загрузке данных будут созданы дубли несопоставленных объектов. Продолжить?';en='There are unmatched objects."
"When loading data, duplicates of unmatched objects will be created. Continue?'");
...
Процедура ОткрытьФормуСопоставления(Команда)
	...
	Если Не ТекущиеДанные.ИспользоватьПредварительныйПросмотр Тогда
		ПоказатьПредупреждение(, НСтр("es='Para el tipo de datos no es posible cumplir la comparación de los objetos.';ru='Для типа данных нельзя выполнить сопоставление объектов.';en='You cannot match objects for a data type.'"));
		Возврат;
	КонецЕсли;
...
Процедура ДополнениеВыгрузкиОчисткаОбщегоОтбора(Команда)
	...
	ТекстЗаголовка = НСтр("es='La confirmación';ru='Подтверждение';en='Confirmation'");
	ТекстВопроса   = НСтр("es='La confirmación';ru='Подтверждение';en='Confirmation'");
...
Процедура ДополнениеВыгрузкиИсторияОтборов(Команда)
	...
	Текст = НСтр("es='Conservar el ajuste corriente...';ru='Сохранить текущую настройку...';en='Save current setting ...'");
...
Процедура ДополнениеВыгрузкиИсторияОтборовВыборИзМеню(Знач ВыбранныйЭлемент, Знач ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ПредставлениеНастройки)=Тип("Строка") Тогда
		...
		НСтр("es='¿Restablecer los ajustes ""%1""?';ru='Восстановить настройки ""%1""?';en=""Restore settings to '% 1'?"""), ПредставлениеНастройки);
...
Процедура ВыполнитьЗагрузкуДанныхДляСтрокиПродолжение(Знач ВыделенныеСтроки) 
	...
	Если Отказ Тогда
		НСтрока = НСтр("es='A la carga de los datos han surgido las faltas. "
"¿Pasar en la revista del registro?';ru='При загрузке данных возникли ошибки."
"Перейти в журнал регистрации?';en='Errors occurred while loading data."
"Go to the logbook?'");
	...
	ПоказатьОповещениеПользователя(НСтр("es='La carga de los datos es acabada.';ru='Загрузка данных завершена.';en='Data loading completed.'"));
КонецПроцедуры
...
Процедура ОткрытьКаталогОбменаИнформациейУзла()
	...
	Если ПустаяСтрока(ИмяКаталога) Тогда
		ПоказатьПредупреждение(, НСтр("es='para la información.';ru='Не задан каталог обмена информацией.';en='No information exchange directory specified.'"));
		Возврат;
	КонецЕсли;
		...
		Если ЗакончитьСопоставлениеДанных Тогда
			Если ЗагрузитьСообщениеПослеСопоставления Тогда
				Элементы.ДекорацияПодсказкаПереходаИнформацииСтатистики.Заголовок =
					НСтр("es='Presionen el botón ""Más"" para la terminación de la comparación de los datos y la carga del mensaje del cambio.';ru='Нажмите кнопку ""Далее"" для завершения сопоставления данных и загрузки сообщения обмена.';en='Click the ""Next"" button to complete the data mapping and download the exchange message.'");
			Иначе
				Элементы.ДекорацияПодсказкаПереходаИнформацииСтатистики.Заголовок =
					НСтр("es='Presionen el botón ""Conservar y cerrar"" para la terminación de la comparación de los datos y la salida del ayudante.';ru='Нажмите кнопку ""Сохранить и закрыть"" для завершения сопоставления данных и выхода из помощника.';en='Click the ""Save and Close"" button to complete the data mapping and exit the wizard.'");
			КонецЕсли;
		Иначе
			Элементы.ДекорацияПодсказкаПереходаИнформацииСтатистики.Заголовок =
				НСтр("es='Presionen el botón ""Conservar y cerrar"" para la conservación de los resultados de la comparación y la salida del ayudante. "
"Durante el lanzamiento siguiente del ayudante será posible continuar la comparación de los datos.';ru='Нажмите кнопку ""Сохранить и закрыть"" для сохранения результатов сопоставления и выхода из помощника."
"При следующем запуске помощника можно будет продолжить сопоставление данных.';en='Click the Save and Close button to save the results of the comparison and exit the assistant."
"The next time you run the wizard, you can continue to map data. '");
		КонецЕсли;
	Иначе
		Элементы.ДекорацияПодсказкаПереходаИнформацииСтатистики.Заголовок =
			НСтр("es='Presionen el botón ""Más"" para la sincronización de los datos.';ru='Нажмите кнопку ""Далее"" для синхронизации данных.';en='Click ""Next"" to synchronize data.'");
	КонецЕсли;
...
Процедура ИнициализироватьНастройкиТранспортаСообщенийОбмена()
	...
	Если Не ПропускатьСтраницуТранспорта Тогда
		...
		Если СписокВыбораТранспорта.Количество() = 0 Тогда
			СписокВыбораТранспорта.Добавить(Неопределено, НСтр("es='La conexión no es ajustada';ru='подключение не настроено';en='connection not configured'"));
...
Процедура УстановитьЗаголовокФормы()
	...
	ШаблонЗаголовка = НСтр("es='El cambio para los datos con ""%1""';ru='Обмен данными с ""%1""';en='Communication with ""%1""'");
	
	Если ПолученоСообщениеДляСопоставленияДанных Тогда
		ШаблонЗаголовка = НСтр("es='La comparación dado ""%1""';ru='Сопоставление данных ""%1""';en='Data mapping ""%1""'");
	ИначеЕсли ПолучитьДанные И ОтправитьДанные Тогда
		ШаблонЗаголовка = НСтр("es='La sincronización de los datos con ""%1""';ru='Синхронизация данных с ""%1""';en='Data synchronization with ""%1""'");
	ИначеЕсли ОтправитьДанные Тогда
		ШаблонЗаголовка = НСтр("es='El envío de los datos para ""%1""';ru='Отправка данных для ""%1""';en='Sending data for ""%1""'");
	ИначеЕсли ПолучитьДанные Тогда
		ШаблонЗаголовка = НСтр("es='La recepción de los datos de ""%1""';ru='Получение данных от ""%1""';en='Retrieving data from ""%1""'");
	КонецЕсли;
...
Процедура УстановитьОписаниеДополненияОбщегоОтбора()
	...
	Если НетОтбора Тогда
		Текст = НСтр("es='Todos los documentos';ru='Все документы';en='All documents'");
	КонецЕсли;
...
Процедура УстановитьОписаниеДополненияДетально()
	...
	НетОтбора = ПустаяСтрока(Текст);
	Если НетОтбора Тогда
		Текст = НСтр("es='Los datos adicionales no son escogidos';ru='Дополнительные данные не выбраны';en='No additional data selected'");
	КонецЕсли;
...
Процедура ОбновитьОтображениеЭлементовСтатусаОбменаДанными()
	...
	Если Элементы.СтраницыСтатусаЗагрузкиДанных.ТекущаяСтраница=Элементы.СтатусЗагрузкиНеопределено Тогда
		Элементы.ПерейтиВЖурналРегистрацииСобытийЗагрузкиДанных.Заголовок = НСтр("es='La carga de los datos no es hecha';ru='Загрузка данных не произведена';en='No data loading'");
	Иначе
	...
	Если Элементы.СтраницыСтатусаВыгрузкиДанных.ТекущаяСтраница=Элементы.СтатусВыгрузкиНеопределено Тогда
		Элементы.ПерейтиВЖурналРегистрацииСобытийВыгрузкиДанных.Заголовок = НСтр("es='La descarga de los datos no es hecha';ru='Выгрузка данных не произведена';en='No data upload'");
	Иначе
...
Процедура ОбработатьСтатусВыполненияФоновогоЗадания()
	...
	ИначеЕсли РезультатВыполненияФоновогоЗадания.Статус = "Отменено" Тогда
		СообщениеОбОшибке = НСтр("es='La acción es anulada por el usuario.';ru='Действие отменено пользователем.';en='Action canceled by user.'");
	КонецЕсли;
КонецПроцедуры
...
Функция Подключаемый_СтраницаНачала_ПриПереходеДалее(Отказ)
	...
	Если Объект.УзелИнформационнойБазы.Пустая() Тогда
		...
		НСтрока = НСтр("es='Indiquen el nudo de la base informativa';ru='Укажите узел информационной базы';en='Specify the infobase node'");
	...
	ИначеЕсли Объект.ВидТранспортаСообщенийОбмена.Пустая()
		...
		НСтрока = НСтр("es='Indiquen la variante de la conexión';ru='Укажите вариант подключения';en='Specify a connection option'");
	...
	ИначеЕсли ОбменЧерезВебСервис И ПустаяСтрока(WSПароль) Тогда
		...
		НСтрока = НСтр("es='la consigna.';ru='Не указан пароль.';en='No password specified.'");
...
Функция Подключаемый_СтраницаОжиданияПроверкиПодключения_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если ОбменЧерезВебСервис Тогда
		...
		Если Отказ Тогда
			ПоказатьПредупреждение(, НСтр("es='No consiguió cumplir la operación.';ru='Не удалось выполнить операцию.';en='The operation failed.'"));
		КонецЕсли;
	КонецЕсли;
...
Функция Подключаемый_СтраницаОжиданияАнализаДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("es='La recepción del mensaje del cambio en el catálogo temporal';ru='Получение сообщения обмена во временный каталог';en='Receiving an exchange message in a temporary directory'");
...
Функция Подключаемый_СтраницаОжиданияАнализаДанныхДлительнаяОперацияОкончание_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если ДлительнаяОперацияЗавершена
		...
		Если Не ЗначениеЗаполнено(Объект.ИмяФайлаСообщенияОбмена) Тогда
			...
			ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
			ПараметрыЗадания.НаименованиеЗадания  = НСтр("es='La recepción del fichero con los datos del mensaje del cambio en el catálogo temporal';ru='Получение файла с данными сообщения обмена во временный каталог';en='Receiving data file of exchange message in temporary directory'");
...
Процедура ЗавершениеПолученияДанныхВоВременныйКаталогНаСервере()
	...
	ШаблонСообщенияОбОшибке = НСтр("es='No consiguió cumplir la carga de los datos. Los detalles de cm. En la revista del registro';ru='Не удалось выполнить загрузку данных. Подробности см. в журнале регистрации';en='Failed to load data. See the log for details.'");
...
Функция Подключаемый_АнализДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("es='El análisis los mensajes dados del cambio';ru='Анализ данных сообщения обмена';en='Exchange message data analysis'");
...
Функция Подключаемый_СтраницаИнформацииСтатистики_ПриПереходеДалее(Отказ)
	...
	Кнопки.Добавить(КодВозвратаДиалога.Да,  НСтр("en='Resume';es='Continuar';ru='Продолжить'"));
	Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("en='cancel';es='La anulación';ru='Отмена'"));
	
	Сообщение = НСтр("es='No todos los datos son confrontados. La presencia de los datos no confrontados "
"puede llevar a la aparición de los elementos iguales en las listas de (las tomas). "
"¿Continuar?';ru='Не все данные сопоставлены. Наличие несопоставленных данных"
"может привести к появлению одинаковых элементов в списках (дублей)."
"Продолжить?';en='Not all data is mapped. Unmatched data"
"may lead to the appearance of identical items in lists (takes)."
"Continue?'");
...
Функция Подключаемый_ЗагрузкаДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("es='La carga de los datos del mensaje del cambio';ru='Загрузка данных из сообщения обмена';en='Loading data from an exchange message'");
	...
	Иначе
		...
		Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='No consiguió registrar los datos para la descarga.';ru='Не удалось зарегистрировать данные для выгрузки.';en='Failed to register data for upload.'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриНачалеВыгрузкиДанных(Отказ)
	...
	Иначе
		...
		ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
		ПараметрыЗадания.НаименованиеЗадания  = НСтр("es='La descarga de los datos en el mensaje del cambio';ru='Выгрузка данных в сообщение обмена';en='Uploading data to an exchange message'");
...
Процедура ПриЗавершенииВыгрузкиДанных()
	...
	Иначе
		...
		Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("es='No consiguió cumplir el cambio para los datos.';ru='Не удалось выполнить обмен данными.';en='Failed to exchange data.'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗавершениеВыгрузкиНаСервере()
	...
	Если РезультатВыполненияМетода.Отказ
		...
		СообщениеОбОшибке = НСтр("es='No consiguió cumplir el envío de los datos. Los detalles de cm. En la revista del registro';ru='Не удалось выполнить отправку данных. Подробности см. в журнале регистрации';en='Failed to send data. See the log for details.'");
	КонецЕсли;
