...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НачальнаяВыгрузка Тогда
		ТекстПредупреждения = НСтр("es='¿Anular la descarga inicial de los datos?';ru='Отменить начальную выгрузку данных?';en='Cancel initial data upload?'");
	Иначе
		ТекстПредупреждения = НСтр("es='¿Anular la descarga de los datos?';ru='Отменить выгрузку данных?';en='Cancel data upload?'");
	КонецЕсли;
...
Процедура ПроверитьВозможностьИспользованияФормы(Отказ = Ложь)
	...
	Если ОбменДаннымиПовтИсп.ЭтоУзелРаспределеннойИнформационнойБазы(Параметры.УзелОбмена) Тогда
		ТекстСообщения = НСтр("es='La descarga inicial no es apoyada para los nudos de las bases distribuidas informativas.';ru='Начальная выгрузка не поддерживается для узлов распределенных информационных баз.';en='Initial upload is not supported for nodes of distributed infobases.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, , , , Отказ);
		Возврат;
	КонецЕсли;
...
Функция Подключаемый_СтраницаЗапросПароля_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(WSПароль) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='Indiquen la consigna.';ru='Укажите пароль.';en='Specify a password.'"), , "WSПароль", , Отказ);
	КонецЕсли;
