...
Функция ИнформацияОПравилах() Экспорт
	...
	СтрокаИнформации = НСтр("es='Las reglas del registro de los objetos de esta base informativa (%1) de %2';ru='Правила регистрации объектов этой информационной базы (%1) от %2';en='Rules for registering objects of this infobase (%1) from%2'");
...
Процедура ВыполнитьПроверкуНаличияПланаОбмена()
	...
	Если Регистрация.ИмяПланаОбмена <> ИмяПланаОбменаДляЗагрузки Тогда
		...
		ОписаниеОшибки = НСтр("es='En las reglas del registro es indicado el plan del cambio %1, y la carga se cumple para el plan del cambio %2';ru='В правилах регистрации указан план обмена %1, а загрузка выполняется для плана обмена %2';en='Registration rules specify exchange plan%1, and download is performed for exchange plan%2'");
...
Функция ИнициализацияСообщений()
	...
	Сообщения.Вставить(0, НСтр("es='La falta interior';ru='Внутренняя ошибка';en='Internal error'", КодОсновногоЯзыка));
	Сообщения.Вставить(1, НСтр("es='La falta de la apertura del fichero de las reglas';ru='Ошибка открытия файла правил';en='Error opening rule file'", КодОсновногоЯзыка));
	Сообщения.Вставить(2, НСтр("es='La falta a la carga de las reglas';ru='Ошибка при загрузке правил';en='Error loading rules'", КодОсновногоЯзыка));
	Сообщения.Вставить(3, НСтр("es='La falta del formato de las reglas';ru='Ошибка формата правил';en='Rule format error'", КодОсновногоЯзыка));
	Сообщения.Вставить(4, НСтр("es='La falta a la recepción del fichero de las reglas para la lectura';ru='Ошибка при получении файла правил для чтения';en='Error getting read rules file'", КодОсновногоЯзыка));
	Сообщения.Вставить(5, НСтр("es='Las reglas cargadas del registro no son destinadas al plan corriente del cambio.';ru='Загружаемые правила регистрации не предназначены для текущего плана обмена.';en='Downloadable registration rules are not for the current exchange plan.'", КодОсновногоЯзыка));
