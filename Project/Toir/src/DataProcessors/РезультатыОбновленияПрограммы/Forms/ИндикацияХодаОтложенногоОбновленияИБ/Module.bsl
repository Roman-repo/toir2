...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь(, Истина) Тогда
		...
		Элементы.ПодсказкаГдеНайтиЭтуФорму.Заголовок = 
			НСтр("es='Se puede también controlar el curso del tratamiento de los datos de la versión del programa de la sección "
"""la Información"" sobre el escritorio, la orden ""la Descripción de los cambios del programa"".';ru='Ход обработки данных версии программы можно также проконтролировать из раздела"
"""Информация"" на рабочем столе, команда ""Описание изменений программы"".';en='The processing of the data version of the program can also be controlled from the section"
""" Information "" on the desktop, command "" Description of program changes "".'");
	...
	Иначе
		ЗаголовокОбновлениеЗавершено = НСтр("es='La versión del programa es con éxito renovada a la versión %1';ru='Версия программы успешно обновлена на версию %1';en='The version of the program was successfully updated to version% 1'");
	...
	Иначе
		...
		СведенияОбОбновлении = ОбновлениеИнформационнойБазыСлужебный.СведенияОбОбновленииИнформационнойБазы();
		ШаблонЗаголовка = НСтр("es='Los procedimientos adicionales del tratamiento de los datos son acabados %1 en %2';ru='Дополнительные процедуры обработки данных завершены %1 в %2';en='Additional data processing procedures completed %1 in %2'");
	...
	Элементы.ЗаголовокИнформации.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Se cumplen los procedimientos adicionales del tratamiento de los datos a la versión %1 "
"Trabajo con estos datos es temporalmente limitada';ru='Выполняются дополнительные процедуры обработки данных на версию %1"
"Работа с этими данными временно ограничена';en='Additional data processing procedures are running on version% 1"
"Work with this data is temporarily limited.'"), Метаданные.Версия);
...
Функция СообщениеОРезультатахОбновления(СведенияОбОбновлении)
	...
	Если ВсегоОбработчиков = УспешноВыполненоОбработчиков Тогда
		...
		Если ВсегоОбработчиков = 0 Тогда
			...
			ТекстСообщения = "";
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Todos los procedimientos de la renovación son cumplidos con éxito (%1)';ru='Все процедуры обновления выполнены успешно (%1)';en='All upgrade procedures completed successfully (% 1)'"), УспешноВыполненоОбработчиков);
		КонецЕсли;
		Элементы.КартинкаЗавершено.Картинка = БиблиотекаКартинок.Успешно32;
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='No consiguió cumplir todos los procedimientos (es cumplido %1 de %2)';ru='Не все процедуры удалось выполнить (выполнено %1 из %2)';en='Not all procedures succeeded (%1 of%2 completed)'"), 
...
Процедура ОбновитьИнформациюОХодеОбновления(СведенияОбОбновлении, ОбновлениеЗавершено = Ложь)
	...
	Элементы.ИнформацияСтатусОбновления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Es cumplido: %1 de %2';ru='Выполнено: %1 из %2';en='% 1 of% 2 completed'"),
...
Процедура ИзменитьРасписаниеПослеУстановкиРасписания(НовоеРасписание, ДополнительныеПараметры) Экспорт
	...
	Если НовоеРасписание <> Неопределено Тогда
		Если НовоеРасписание.ПериодПовтораВТечениеДня = 0 Тогда
			...
			КнопкиВопроса.Добавить("НастроитьРасписание", НСтр("es='Ajustar el horario';ru='Настроить расписание';en='Set schedule'"));
			КнопкиВопроса.Добавить("РекомендуемыеНастройки", НСтр("es='Establecer los ajustes recomendados';ru='Установить рекомендуемые настройки';en='Set recommended settings'"));
			
			ТекстСообщения = НСтр("es='Los procedimientos adicionales del tratamiento de los datos se cumplen por las porciones pequeñas, "
"por eso su trabajo correcto tiene que obligatoriamente dar el intervalo de la repetición después de la terminación. "
""
"Para esto en la ventana del ajuste del horario es necesario pasar en вкладку ""Del día"" "
"y rellenar el campo ""Repetir a través de"".';ru='Дополнительные процедуры обработки данных выполняются небольшими порциями,"
...
"и заполнить поле ""Повторять через"".';en='Additional data processing procedures are performed in small batches,"
"therefore, for their correct operation, it is necessary to set the retry interval after completion."
""
"To do this, in the schedule settings window, go to the "" Daytime"" tab"
"and fill in the field "" Repeat through ""'");
...
Процедура ОбработатьРезультатУстановкиИсправлений(ИнформацияПоИсправлениям)
	...
	Если ИнформацияПоИсправлениям.НеУстановлено > 0 Тогда
		...
		Ссылка = Новый ФорматированнаяСтрока(НСтр("es='No consiguió establecer las correcciones';ru='Не удалось установить исправления';en='Failed to install fixes'"),,,, "НеудачнаяУстановка");
		НадписьИсправления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='(%1 de %2)';ru='(%1 из %2)';en='(%1 of %2)'"), УспешноУстановлено, ВсегоПатчей);
	...
	Иначе
		Ссылка = Новый ФорматированнаяСтрока(НСтр("es='No consiguió establecer las correcciones';ru='Не удалось установить исправления';en='Failed to install fixes'"),,,, "УстановленныеИсправления");
		НадписьИсправления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Son con éxito establecidos (%1)';ru='успешно установлены (%1)';en='successfully installed (%1)'"), ВсегоПатчей);
		НадписьИсправления = Новый ФорматированнаяСтрока(Ссылка, " ", НадписьИсправления);
