...
Процедура ГиперссылкаОтложенноеОбновлениеНажатие(Элемент)
	...
	Иначе
		ТекстПредупреждения = НСтр("es='El tratamiento de los datos no se cumplía.';ru='Обработка данных еще не выполнялась.';en='Data processing has not yet been performed.'");
...
Процедура Приостановить(Команда)
	...
	ТекстВопроса = НСтр("es='La parada de los procedimientos adicionales del tratamiento de los datos "
"puede llevar al trabajo inestable o неработоспособности los programas. "
"Cumplir la desconexión se recomienda en caso del descubrimiento de la falta "
"en el procedimiento del tratamiento de los datos y solamente después de la consulta del servicio del apoyo, "
"porque los procedimientos del acabado de los datos pueden depender uno de otro.';ru='Остановка дополнительных процедур обработки данных"
...
"т.к. процедуры отработки данных могут зависеть друг от друга.';en='Stop additional data processing procedures"
"may lead to unstable operation or inoperability of the program."
"It is recommended to disconnect if an error is detected."
"in the data processing procedure and only after consultation with the support service,"
"because data processing procedures may depend on each other.'");
...
Процедура СформироватьТаблицуОтложенныхОбработчиков(ВыполненыВсеОбработчики = Истина, НачальноеЗаполнение = Ложь)
	...
	Если ОбработчикиНеВыполнялись Тогда
		Элементы.ТекстПояснения.Заголовок = НСтр("es='Se recomienda poner en marcha los procedimientos no cumplidos del tratamiento de los datos.';ru='Рекомендуется запустить невыполненные процедуры обработки данных.';en='It is recommended to run outstanding data processing procedures.'");
	Иначе
		Элементы.ТекстПояснения.Заголовок = НСтр("es='Se recomienda poner en marcha los procedimientos no cumplidos repetidamente.';ru='Невыполненные процедуры рекомендуется запустить повторно.';en='Failed procedures are recommended to be restarted.'");
	КонецЕсли;
...
"% 2'");
	...
	ШаблонИнформацияОПроцедуреОбновления = НСтр("es='%1%2Процедура ""%3"" tratamientos de los datos %4.';ru='%1%2Процедура ""%3"" обработки данных %4.';en='% 1% 2 ""% 3"" data processing procedure% 4.'");
	...
	Если СтрокаОбработчика.Статус = "Выполнено" Тогда
		...
		ПредставлениеСтатусаВыполнения = НСтр("es='Se ha acabado con éxito';ru='завершилась успешно';en='completed successfully'");
		СтрокаСписка.СтатусПредставление = НСтр("es='Es cumplido';ru='Выполнено';en='Done'");
	...
	ИначеЕсли СтрокаОбработчика.Статус = "Выполняется" Тогда
		...
		ПредставлениеСтатусаВыполнения = НСтр("es='En el momento actual se cumple';ru='в данный момент выполняется';en='currently running'");
		Если Прогресс <> Неопределено Тогда
			ШаблонСтатус = НСтр("es='Se cumple (%1 %)';ru='Выполняется (%1%)';en='In progress (% 1%)'");
			СтрокаСписка.СтатусПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСтатус, Прогресс)
		Иначе
			СтрокаСписка.СтатусПредставление = НСтр("es='Se cumple';ru='Выполняется';en='Performed'");
		КонецЕсли;
	ИначеЕсли СтрокаОбработчика.Статус = "Ошибка" Тогда
		
		ОбработчикиНеВыполнялись = Ложь;
		ВыполненыВсеОбработчики        = Ложь;
		ПредставлениеСтатусаВыполнения = НСтр("es='El procedimiento de ""%1"" tratamientos de los datos se ha acabado con la falta:';ru='Процедура ""%1"" обработки данных завершилась с ошибкой:';en='Procedure ""%1"" processing the data failed:'") + Символы.ПС + Символы.ПС;
		...
		СтрокаСписка.СтатусПредставление = НСтр("es='La falta';ru='Ошибка';en='Error '");
	...
	ИначеЕсли СтрокаОбработчика.Статус = "Приостановлен" Тогда
		...
		ОбработчикиНеВыполнялись = Ложь;
		ВыполненыВсеОбработчики        = Ложь;
		ПредставлениеСтатусаВыполнения = НСтр("es='Es parada por el administrador';ru='остановлена администратором';en='stopped by admin'");
		СтрокаСписка.СтатусПредставление = НСтр("es='Es parado';ru='Остановлено';en='Stopped'");
		СтрокаСписка.СтатусКартинка    = БиблиотекаКартинок.ЗнакСтоп;
	Иначе
		
		ВыполненыВсеОбработчики        = Ложь;
		ПредставлениеСтатусаВыполнения = НСтр("es='No se cumplía';ru='еще не выполнялась';en='not yet implemented'");
		СтрокаСписка.СтатусПредставление = НСтр("es='No se cumplía';ru='Не выполнялась';en='Not performed'");
	КонецЕсли;
...
Функция ДлительностьВыполненияПроцедурыОбновления(ДлительностьВыполнения)
	...
	ШаблонСекунды = НСтр("es='%1 segundos';ru='%1 сек.';en='% 1 sec'");
	ШаблонМинуты = НСтр("es='%1 minutos de %2 segundos';ru='%1 мин. %2 сек.';en='%1 min. % 2 sec'");
	ШаблонЧасы = НСтр("es='%1 h. %2 minutos';ru='%1 ч. %2 мин.';en='% 1 hr.% 2 min.'");
	...
	Если ДлительностьВСекундах < 1 Тогда
		Возврат НСтр("es='Menos de segundo';ru='менее секунды';en='less than a second'")
