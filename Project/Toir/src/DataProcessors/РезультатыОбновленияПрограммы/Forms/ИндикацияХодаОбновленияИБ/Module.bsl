...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТолькоОбновлениеПараметровРаботыПрограммы Тогда
		Заголовок = НСтр("es='La renovación de los parámetros del trabajo del programa';ru='Обновление параметров работы программы';en='Updating program operation parameters'");
	...
	ИначеЕсли РежимОбновленияДанных = "НачальноеЗаполнение" Тогда
		Заголовок = НСтр("es='El relleno inicial de los datos';ru='Начальное заполнение данных';en='Initial Data Fill'");
	...
	ИначеЕсли РежимОбновленияДанных = "ПереходСДругойПрограммы" Тогда
		Заголовок = НСтр("es='El tránsito de otro programa';ru='Переход с другой программы';en='Switching from another program'");
	...
	Иначе
		Заголовок = НСтр("es='La renovación de la versión del programa';ru='Обновление версии программы';en='Program version update'");
...
Функция ЗапуститьПроцедурыЗаполненияОтложенныхОбработчиков()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='La dirección многопоточной por el registro de los datos de la renovación aplazada';ru='Управление многопоточной регистрацией данных отложенного обновления';en='Control of multithreaded logging of deferred update data'");
