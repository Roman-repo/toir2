...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Текущая, Минимальная) < 0 Тогда
		ТекстУсловие                                    = НСтр("es='Es necesario';ru='необходимо';en='required'");
		ПродолжениеРаботыНевозможно                     = Истина;
		Элементы.РекомендуемаяВерсияПлатформы.Заголовок = Минимальная;
	Иначе
		ТекстУсловие                                    = НСтр("es='Se recomienda';ru='рекомендуется';en='recommended'");
	...
	Если ПродолжениеРаботыНевозможно Тогда
		...
		Заголовок = НСтр("es='Es necesario renovar la versión de la plataforma';ru='Необходимо обновить версию платформы';en='Required to upgrade platform version'");
	КонецЕсли;
