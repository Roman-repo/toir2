...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого ТекущееИсправление Из ОписаниеИсправлений Цикл
		...
		СтрокаИсправление.Наименование = ?(
			...
			ТекущееИсправление.Наименование + " " + НСтр("es='(La corrección es llamada)';ru='(Исправление отозвано)';en='(Bug revoked)'"),
		...
		Если СтрокаИсправление.ВозможноРазвертывание Тогда
			...
			СтрокаИсправление.ЗаголовокСсылкиДействия = НСтр("es='Más detalladamente';ru='Подробнее';en='More details'");
		КонецЕсли;
...
Процедура ИсправленияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ИсправленияЗаголовокСсылкиДействия" Тогда
		...
		Если ТекущиеДанные.Развернута Тогда
			...
			ТекущиеДанные.ЗаголовокСсылкиДействия = НСтр("es='Más detalladamente';ru='Подробнее';en='More details'");
		Иначе
			ТекущиеДанные.ОписаниеОтображаемое = ТекущиеДанные.Описание;
			ТекущиеДанные.Развернута = Истина;
			ТекущиеДанные.ЗаголовокСсылкиДействия = НСтр("es='Doblar';ru='Свернуть';en='Collapse'");
		КонецЕсли;
	КонецЕсли;
...
Процедура КомандаОК(Команда)
	...
	Иначе
		...
		Если Результат.Количество() = 0 Тогда
			ПоказатьПредупреждение(, НСтр("es='Escojan las correcciones para la instalación.';ru='Выберите исправления для установки.';en='Select the fixes to install.'"));
			Возврат;
		КонецЕсли;
...
Процедура ОбновитьНадписьВсегоВыбраноИсправлений(Форма)
	...
	Форма.ТекстВыбраноИсправлений = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Es escogido en total las correcciones %1 de %2';ru='Всего выбрано исправлений %1 из %2';en='Total fixes selected%1 from%2'"),
		ВыбраноКоличество,
