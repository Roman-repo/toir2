...
Функция ПолучитьСодержаниеКлассификатора(ЧитатьПринудительно = Ложь) Экспорт
	...
	Если Данные.Содержание = Неопределено Тогда
		...
		ТекущееСостояние = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La lectura del contenido del clasificador %1 …';ru='Чтение содержимого классификатора %1…';en='Reading the contents of the classifier%1 ...'"),
...
Процедура СохранитьКлассификатор(Источник, ИмяСправочника, УдалятьНеактуальные, Владелец, ИмяРеквизитаПолнНаименование, Прогресс) Экспорт
	...
	Попытка
		...
		ТекущееСостояние = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La conservación del clasificador en la guía «%1» …';ru='Сохранение классификатора в справочник «%1»…';en='Saving the classifier to the directory ""%1"" ...'"),
			МД.Синоним);
		...
		Иначе
			ВызватьИсключение НСтр("es='La fuente no puede ser conservada en la guía.';ru='Источник не может быть сохранен в справочник.';en='Source cannot be saved to the directory.'");
		КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("es='La carga ОКОФ';ru='Загрузка ОКОФ';en='Download OKOF'", Метаданные.ОсновнойЯзык.КодЯзыка),
...
Функция ДобавитьКлассификатор(СтрокаXMLИлиФайл, ПервичныйИсточник = "", MIMEТип = "", УникальныйИдентификатор = Неопределено) Экспорт
	...
	Исключение
		#Если Не ВнешнееСоединение Тогда
			ШаблонСообщения = НСтр("es='El fichero (%1) es estropeado o tiene el formato equivocado.';ru='Файл (%1) поврежден или имеет неверный формат.';en='File (%1) is corrupt or has the wrong format.'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, СтрокаXMLИлиФайл.Имя);
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
		#КонецЕсли
		ЗаписьЖурналаРегистрации(
			СтрШаблон(НСтр("es='El fichero (%1) es estropeado o tiene el formato equivocado.';ru='Файл (%1) поврежден или имеет неверный формат.';en='File (%1) is corrupt or has the wrong format.'", Метаданные.ОсновнойЯзык.КодЯзыка),
					...
					ИначеЕсли ЧтениеXML.ЛокальноеИмя = "data"
					   И ЧтениеXML.URIПространстваИмен = xmlnsКлассификатор Тогда
						...
						Если Данные.КоличествоЭлементов = 0 И Данные.МаксимальноеЗначениеПрогрессора = 0 Тогда
							ТекущееСостояние = НСтр("es='El cálculo de las posiciones del clasificador …';ru='Подсчет позиций классификатора…';en='Counting classifier positions ...'");
							Попытка
	...
	Если НормАкт = Неопределено Тогда
		Редакция = НСтр("es='<no es indicado>';ru='<не указана>';en='<не указана>'");
	Иначе
		Редакция = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='%1 %2%3 de %4';ru='%1 %2%3 от %4';en='%1%2%3 of%4'"),
			НормАкт.Вид,
...
Функция ПроверитьРазархивироватьФайл(Файл, ТипЧитателя, ПервичныйИсточник, MIMEТип = "",УникальныйИдентификатор = Неопределено)
	...
	Если НРег(Файл.Расширение) = ".aml" Тогда
		...
		ТекущееСостояние = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El desempaquetamiento del fichero (%1) …';ru='Распаковка файла (%1)…';en='Unpacking the file (%1) ...'"),
			Файл.Имя);
		...
		Исключение
			#Если Не ВнешнееСоединение Тогда
				ШаблонСообщения = НСтр("es='El fichero (%1) no consiguió распаковать: el fichero es estropeado, tiene el formato equivocado, o no basta el lugar sobre el disco.';ru='Файл (%1) не удалось распаковать: файл поврежден, имеет неверный формат, или не достаточно места на диске.';en='File (%1) could not be unzipped: the file is damaged, has the wrong format, or there is not enough disk space.'");
	...
	ИначеЕсли НРег(Файл.Расширение) = ".zip"
		...
		Для Каждого Элемент Из Zip.Элементы Цикл
			...
			Исключение
				#Если Не ВнешнееСоединение Тогда
					ШаблонСообщения = НСтр("es='No consiguió sacar el fichero (%1) de (%2): el archivo es estropeado, tiene el formato equivocado, o no basta el lugar sobre el disco.';ru='Файл (%1) не удалось извлечь из (%2): архив поврежден, имеет неверный формат, или не достаточно места на диске.';en='File (%1) could not be extracted from (%2): the archive is damaged, has the wrong format, or there is not enough disk space.'");
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El fichero (%1) no es el fichero del clasificador.';ru='Файл (%1) не является файлом классификатора.';en='File (%1) is not a classifier file.'"),
			Файл.Имя);
		Результат = Неопределено;
