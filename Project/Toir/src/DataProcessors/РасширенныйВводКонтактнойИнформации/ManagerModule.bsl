...
Функция СписокАвтоподбораНаселенногоПункта(Текст, ДополнительныеПараметры) Экспорт
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		...
		Результат.КраткоеПредставлениеОшибки = НСтр("es='Las noticias de domicilios faltan.';ru='Адресные сведения отсутствуют.';en='No address information.'");
		Возврат Результат;
	КонецЕсли;
...
Функция ДесериализацияАдресаОбщая(Знач ЗначенияПолей, Знач Представление, Знач ОжидаемыйТип = Неопределено)
	...
	Если ОжидаемыйТип <> Неопределено Тогда
		Если ОжидаемыйТип <> Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
			ВызватьИсключение НСтр("es='La falta десериализации de la información de contacto, se espera la dirección';ru='Ошибка десериализации контактной информации, ожидается адрес';en='Error deserializing contact information, expected address'");
		КонецЕсли;
	КонецЕсли;
...
Функция ОшибкиЗаполненияАдресаXDTO(Знач Адрес, ВидИнформации, РезультатГруппами = Ложь) Экспорт
	...
	Если НЕ УправлениеКонтактнойИнформациейСлужебный.ЭтоНациональныйАдрес(Адрес) Тогда
		...
		Если ФлагиПроверки.ТолькоНациональныйАдрес Тогда
			ТекстОшибки = НСтр("es='La dirección debe ser solamente rusa.';ru='Адрес должен быть только российским.';en='The address must be only Russian.'");
	...
	Если Не УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияЗаполнена(Адрес) Тогда
		...
		Если ФлагиПроверки.ОбязательноеЗаполнение Тогда
			...
			ТекстОшибки = НСтр("es='La dirección no es rellenada.';ru='Адрес не заполнен.';en='The address is empty.'");
	...
	Если РезультатГруппами Тогда
		...
		ИначеЕсли КоличествоОшибок > 0 Тогда
			...
			ВсеОшибки.Добавить(ГруппаОшибокАдреса(НаименованиеГруппыОшибки,
				НСтр("es='Las partes de la dirección no corresponden al clasificador de domicilios:';ru='Части адреса не соответствуют адресному классификатору:';en='Parts of the address do not match the address classifier:'")));
...
Процедура ТолькоЛатиницаВСтроке(Значение, Знач Результат, Знач РезультатГруппами)
	...
	Если Не СтроковыеФункцииКлиентСервер.ТолькоЛатиницаВСтроке(Значение, Ложь, "1234567890,") Тогда
		ТекстОшибки = НСтр("es='La dirección en el formato internacional debe ser tomada por el alfabeto latino.';ru='Адрес в международном формате должен быть набран латиницей.';en='The address in international format must be typed in Latin letters.'");
		Если РезультатГруппами Тогда
			Результат.Добавить(Новый Структура("Поля, ТипОшибки, Сообщение", Новый Массив,
...
Функция ОбщиеГруппыОшибокЗаполненияАдреса(Знач ДанныеАдреса, Знач ВидИнформации)
	...
	НеЗаполненыОбязательныеПоля = ГруппаОшибокАдреса("НеЗаполненыОбязательныеПоля",
		НСтр("es='los campos obligatorios:';ru='Не заполнены обязательные поля:';en='Required fields are not filled:'"));
	Результат.Добавить(НеЗаполненыОбязательныеПоля);
	
	НеУказаныСокращенияПолей = ГруппаОшибокАдреса("НеУказаныСокращенияПолей",
		НСтр("es='los campos:';ru='Не указано сокращение для полей:';en='Indicated abbreviation for fields is not specified:'"));
	Результат.Добавить(НеУказаныСокращенияПолей);
	
	НедопустимыеСимволыПолей = ГруппаОшибокАдреса("НедопустимыеСимволыПолей",
		НСтр("es='Son encontrados los símbolos inadmisibles en los campos:';ru='Найдены недопустимые символы в полях:';en='Invalid characters found in the fields:'"));
	Результат.Добавить(НедопустимыеСимволыПолей);
	
	НеСоответствуетДлинаПолей = ГруппаОшибокАдреса("НеСоответствуетДлинаПолей",
		НСтр("es='No corresponde establecido la longitud de los campos:';ru='Не соответствует установленной длина полей:';en='The length of the fields does not correspond to the established:'"));
	...
	Если ПустаяСтрока(Регион) Тогда
		ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "СубъектРФ",
			НСтр("es='de la dirección';ru='Не указан регион адреса';en='Address region not specified'"), "Регион");
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Регион) Тогда
		...
		Если ПустаяСтрока(ДанныеАдреса.AreaType) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, "СубъектРФ",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Para la región ""%1"" no es indicado el tipo (la república, la esfera, la ciudad etc.)';ru='Для региона ""%1"" не указан тип (республика, область, город и пр.)';en='For region ""%1"" the type is not specified (republic, region, city, etc.)'"), Регион), НСтр("es='La región';ru='Регион';en='Region'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСерверРФ.ТолькоКириллицаВСтроке(Регион, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='En el nombre de la región ""%1"" hay no unos símbolos cirílicos';ru='В названии региона ""%1"" есть не кириллические символы';en='Region name ""%1"" contains non-Cyrillic characters'"), Регион), НСтр("es='La región';ru='Регион';en='Region'"));
		КонецЕсли
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Район) Тогда
		...
		Если ПустаяСтрока(ДанныеАдреса.DistrictType) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Para la región ""%1"" no es indicado el tipo (la región, la colonia etc.)';ru='Для района ""%1"" не указан тип (район, поселение и пр.)';en='Type is not specified for district ""%1"" (district, settlement, etc.)'"), Район), НСтр("es='La región';ru='Район';en='Area'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСерверРФ.ТолькоКириллицаВСтроке(Район, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='En el nombre de la región ""%1"" hay no unos símbolos cirílicos';ru='В названии района ""%1"" есть не кириллические символы';en='District name ""%1"" contains non-Cyrillic characters'"), Район), НСтр("es='La región';ru='Район';en='Area'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Город) Тогда
		...
		Если ПустаяСтрока(ДанныеАдреса.CityType) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Para la ciudad ""%1"" no es indicado el tipo (la ciudad, el poblado, la colonia rural etc.)';ru='Для города ""%1"" не указан тип (город, поселок, сельское поселение и пр.)';en='Type is not specified for city ""%1"" (city, town, rural settlement, etc.)'"), Город), НСтр("es='La ciudad';ru='Город';en='City'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСерверРФ.ТолькоКириллицаВСтроке(Город, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='En el nombre de la ciudad ""%1"" hay no unos símbolos cirílicos';ru='В названии города ""%1"" есть не кириллические символы';en='There are non-Cyrillic characters in the name of the city ""%1""'"), Город), НСтр("es='La ciudad';ru='Город';en='City'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(НаселенныйПункт) Тогда
		...
		Если ПустаяСтрока(ДанныеАдреса.LocalityType) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Para la localidad ""%1"" no es indicado el tipo (la ciudad, la aldea, la aldea etc.)';ru='Для населенного пункта ""%1"" не указан тип (город, деревня, село и пр.)';en='The type (city, village, village, etc.) is not specified for the settlement ""%1""'"), НаселенныйПункт),
					НСтр("es='La localidad';ru='Населенный пункт';en='Locality'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСерверРФ.ТолькоКириллицаВСтроке(НаселенныйПункт, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='En el nombre de la localidad ""%1"" hay no unos símbolos cirílicos';ru='В названии населенного пункта ""%1"" есть не кириллические символы';en='The name of the settlement ""%1"" contains non-Cyrillic characters'"), НаселенныйПункт),
					НСтр("es='La localidad';ru='Населенный пункт';en='Locality'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Улица) Тогда
		...
		Если ПустаяСтрока(ДанныеАдреса.StreetType) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Para la calle ""%1"" no es indicado el tipo (la calle, el callejón, la carretera etc.)';ru='Для улицы ""%1"" не указан тип (улица, переулок, шоссе и пр.)';en='Type is not specified for street ""%1"" (street, lane, highway, etc.)'"), Улица), НСтр("es='La calle';ru='Улица';en='Street'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСерверРФ.ТолькоКириллицаВСтроке(Улица, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='En el nombre de la calle ""%1"" hay no unos símbolos cirílicos';ru='В названии улицы ""%1"" есть не кириллические символы';en='Street name ""%1"" contains non-Cyrillic characters'"), Улица), НСтр("es='La calle';ru='Улица';en='Street'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если Не ПустаяСтрока(ДанныеАдреса.ZipCode) Тогда
		...
		Если СтрДлина(ДанныеАдреса.ZipCode) <> 6 Или Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ДанныеАдреса.ZipCode) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				НСтр("es='El índice postal debe consistir de 6 cifras';ru='Почтовый индекс должен состоять из 6 цифр';en='Postcode must be 6 digits'"),
				НСтр("es='El índice';ru='Индекс';en='Index'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если ПустаяСтрока(Город) И ПустаяСтрока(НаселенныйПункт) И ПустаяСтрока(Район) Тогда
		Если РаботаСАдресами.ЭтоГородФедеральногоЗначения(Регион) = Ложь Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "Город",
				НСтр("es='La ciudad puede no ser indicado solamente en la región - la ciudad del significado federal';ru='Город может быть не указан только в регионе - городе федерального значения';en='The city may not be indicated only in the region - a city of federal significance'"),
				НСтр("es='La ciudad';ru='Город';en='City'"));
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "НаселПункт",
				НСтр("es='La localidad puede no ser indicado solamente en la región - la ciudad del significado federal';ru='Населенный пункт может быть не указан только в регионе - городе федерального значения';en='The locality may not be indicated only in the region - a city of federal significance'"),
				НСтр("es='La localidad';ru='Населенный пункт';en='Locality'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если ПустаяСтрока(ДанныеАдреса.houseNumber) И ДанныеАдреса.buildings.Количество() = 0 И ПустаяСтрока(ДанныеАдреса.stead) Тогда
		Поле = "Дом";
		ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
			НСтр("es='de la casa';ru='Не указан номер дома';en='House number not specified'"),
			НСтр("es='La casa';ru='Дом';en='House'"));
	КонецЕсли;
