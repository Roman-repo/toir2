...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Родитель) И Уровень > 1 Тогда
		...
		КраткоеПредставлениеОшибки = НСтр("es='Campo no contiene las noticias de domicilios para la elección.';ru='Поле не содержит адресных сведений для выбора.';en='The field does not contain address information for selection.'");
		Возврат;
	КонецЕсли;
...
Процедура ПроизвестиВыбор(Знач НомерСтроки)
	...
	ПредупреждениеНеактуальности = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La ""%1 dirección es no actual. "
"¿Continuar?';ru='Адрес ""%1"" неактуален."
"Продолжить?';en='Address ""%1"" ""is not relevant.'"), Данные.Представление);
		
	ЗаголовокПредупреждения = НСтр("es='La confirmación';ru='Подтверждение';en='Confirmation'");
...
Процедура ОповеститьВладельца(Знач Данные, Отказ = Ложь)
	...
	Иначе
		...
		Если Уровень = 7 Тогда
			Результат.КраткоеПредставлениеОшибки = НСтр("es='Los datos sobre las calles para la dirección introducida faltan';ru='Данные о улицах для введенного адреса отсутствуют';en='There is no street data for the entered address'");
		Иначе
			Результат.КраткоеПредставлениеОшибки = НСтр("es='Campo no contiene las noticias de domicilios para la elección.';ru='Поле не содержит адресных сведений для выбора.';en='The field does not contain address information for selection.'");
		КонецЕсли;
	КонецЕсли;
...
Функция ДанныеКлассификатораПоИдентификатору(Знач ПараметрыПоиска)
	...
	Результат.Вставить("КраткоеПредставлениеОшибки", НСтр("es='Campo no contiene las noticias de domicilios para la elección';ru='Поле не содержит адресных сведений для выбора';en='The field does not contain address information for selection.'"));
	Результат.Вставить("Отказ", Ложь);
	...
	Если ДанныеКлассификатора.Отказ Тогда
		...
		Результат.КраткоеПредставлениеОшибки = НСтр("es='La autoselección y la comprobación de la dirección son inaccesibles:';ru='Автоподбор и проверка адреса недоступны:';en='Auto-matching and address verification are not available:'") + Символы.ПС + ДанныеКлассификатора.КраткоеПредставлениеОшибки;
		Результат.Отказ = Истина;
		Возврат Результат;
