...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТолькоНациональныйАдрес Тогда
		...
		Если Страна <> ОсновнаяСтрана Тогда
			...
			ТекстПредупрежденияПриОткрытии = НСтр("es='La dirección es introducida es incorrecto: se permite la introducción solamente las direcciones nacionales. "
"El significado escardando ""el País"" era cambiado en %1, es necesario comprobar los otros campos.';ru='Адрес введен некорректно: допускается ввод только национальных адресов."
"Значение поля ""Страна"" было изменено на %1, необходимо проверить остальные поля.';en='The address is entered incorrectly: only national addresses are allowed."
"The value of the ""Country"" field has been changed to%1, you need to check the remaining fields.'");
	...
	Если Не ЕстьПравоЗагружатьКлассификатор Тогда
		Элементы.ИнформацияОЗагрузкеАдресныеСведений.Заголовок = НСтр("es='Para la autoselección y la comprobación de las direcciones se dirijan al administrador para la carga de las noticias de domicilios.';ru='Для автоподбора и проверки адресов обратитесь к администратору для загрузки адресных сведений.';en='For automatic selection and verification of addresses, contact the administrator to download the address information.'");
		Элементы.ЗагрузитьКлассификатор.Видимость = Ложь;
	КонецЕсли;
		...
		Если Результат.Отказ Тогда
			...
			Иначе
				Если НЕ ПодборПоИндексуДоступен Тогда
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El índice %1 no es encontrado en el clasificador de domicilios';ru='Индекс %1 не найден в адресном классификаторе';en='Index %1 not found in address classifier'"),
...
Процедура НаселенныйПунктОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ЕстьПравоЗагружатьКлассификатор И ВыбранноеЗначение.ПредлагатьЗагрузкуКлассификатора = Истина Тогда
		...
		ПредложениеЗагрузкиКлассификатора(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Los datos para ""%1"" no son cargados.';ru='Данные для ""%1"" не загружены.';en='Data for ""% 1"" not loaded.'"), 
...
Процедура ПослеВыбораУлицы(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		ОбработкаИзмененияАдреса();
	Иначе
		ПоказатьПредупреждение(, НСтр("es='La elección de la lista es inaccesible, т.к en el clasificador de domicilios falta la información sobre las calles para la dirección introducida.';ru='Выбор из списка недоступен, т.к в адресном классификаторе отсутствует информация о улицах для введенного адреса.';en='The selection from the list is not available, because the address classifier does not contain street information for the entered address.'"));
	КонецЕсли;
...
Процедура АдресНаДатуАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	Если СтрСравнить(Текст, НСтр("es='El comienzo del recuento';ru='начало учета';en='start of accounting'")) = 0 Или ПустаяСтрока(Текст) Тогда
...
Процедура АдресНаДатуПриИзменении(Элемент)
	...
	Если Не ВводНовогоАдреса Тогда
		...
		Если ЗначениеЗаполнено(Результат.ДействуетПо) Тогда
			ТекстИсторическийАдрес = " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Funciona por %1';ru='действует по %1';en='valid untill % 1'"), Формат(Результат.ДействуетПо - 10, "ДЛФ=DD"));
		Иначе
			ТекстИсторическийАдрес = НСтр("es='Funciona por el presente.';ru='действует по настоящее время.';en='valid untill the present.'");
		КонецЕсли;
	...
	ТекстНачалаУчета = НСтр("es='El comienzo del recuento';ru='начало учета';en='start of accounting'");
...
Процедура ПроверитьЗаполнениеАдреса(Команда)
	...
	Если РазрешитьВводАдресаВСвободнойФорме Тогда
		ПоказатьПредупреждение(, НСтр("es='La dirección no puede ser comprobada, puesto que él es introducido en la forma libre.';ru='Адрес не может быть проверен, так как он введен в свободной форме.';en='The address cannot be verified because it is entered in free form.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВвестиАдресВСвободнойФорме(Команда)
	...
	ИначеЕсли ЭтоНациональныйАдрес Тогда
		ТекстВопроса = НСтр("es='¿Introducir la dirección en la forma libre? "
"Las direcciones introducidas en la forma libre, pueden no pasar la comprobación por el clasificador de domicilios.';ru='Ввести адрес в свободной форме?"
"Адреса, введенные в свободной форме, могут не пройти проверку по адресному классификатору.';en='Enter the address in free form?"
"Addresses entered in free form may not pass the address classifier.'");
		Оповещение = Новый ОписаниеОповещения("ВвестиАдресВСвободнойФормеЗавершение", ЭтотОбъект);
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , , НСтр("es='La confirmación';ru='Подтверждение';en='Confirmation'"));
	Иначе
...
Процедура АдресныйКлассификаторУстарел(Команда)
	...
	СообщениеОПроверкеАдреса = НСтр("es='Las noticias ФИАС de domicilios cargadas en el programa, han caducado. "
"Se Recomienda cumplir la renovación que la comprobación de la dirección se cumpla correctamente.';ru='Адресные сведения ФИАС, загруженные в программу, устарели."
"Рекомендуется выполнить обновление, чтобы проверка адреса выполнялась корректно.';en='FIAS address information uploaded to the program is outdated."
"It is recommended that you upgrade to ensure that the address verification is performed correctly.'");
	...
	Иначе
		СообщениеОПроверкеАдреса = СообщениеОПроверкеАдреса + Символы.ПС
			+ НСтр("es='Para la renovación del clasificador de domicilios es necesario dirigirse al administrador.';ru='Для обновления адресного классификатора необходимо обратиться к администратору.';en='To update the address classifier, you must contact the administrator.'");
...
Процедура Подключаемый_ДобавитьСтроениеИлиПомещение(Команда)
	...
	Если НайденныеСтроки[0].Тип = "Помещение" Тогда
		Значение = ?(НайденныеСтроки[0].Значение <> НСтр("es='Otro';ru='Другое';en='Other'"), НайденныеСтроки[0].Значение, "");
...
Процедура ПоказатьСписокДомовИлиЗемельныхУчастков(ВключитьОтбор)
	...
	Иначе
		Если ЗначениеЗаполнено(ПредставлениеАдреса) Тогда
			ТекстПредупреждения = НСтр("es='Para la presentación de la lista de las casas rellenen los campos de la dirección.';ru='Для просмотра списка домов заполните поля адреса.';en='To view a list of houses, fill in the address fields.'");
		Иначе
			ТекстПредупреждения = НСтр("es='Para la presentación de la lista de las casas rellenen los campos de la dirección.';ru='Для просмотра списка домов заполните поля адреса.';en='To view a list of houses, fill in the address fields.'");
		КонецЕсли;
...
Процедура ВопросОбОбновлениеАдресногоКлассификатора(Знач СообщениеОПроверкеАдреса)
	...
	Кнопки.Добавить("ОбновитьКлассификатор", НСтр("es='Renovar el clasificador';ru='Обновить классификатор';en='Update classifier'"));
...
Процедура ОтобразитьПоляПоТипуАдреса()
	...
	Если ЭтоНациональныйАдрес Тогда
		...
		Элементы.Улица.Заголовок                     = НСтр("es='La calle';ru='Улица';en='Street'");
		Элементы.ОКТМО.Видимость                     = ВидКонтактнойИнформации.УказыватьОКТМО;
		ВебСервисИспользуется                        = ВебКлассификаторДоступен;
		...
		Элементы.НаселенныйПункт.ПодсказкаВвода      = НСтр("es='Introduzcan las primeras letras del nombre de la localidad';ru='Введите первые буквы названия населенного пункта';en='Enter the first letters of the name of the settlement'");
		Элементы.Улица.ПодсказкаВвода                = НСтр("es='Introduzcan las primeras letras del nombre de la calle';ru='Введите первые буквы названия улицы';en='Enter the first letters of the street name'");
	...
	Иначе
		...
		Иначе
			Элементы.Улица.Заголовок              = НСтр("es='La dirección';ru='Адрес';en='Address'");
...
Процедура ОпределитьОтображениеЭлементовНаФорме()
	...
	Если Не ЕстьЗагруженныйКлассификатор Тогда
		...
		Элементы.Дом.КнопкаВыбора                                 = Ложь;
		Элементы.Строение.КнопкаВыбора                            = Ложь;
		Элементы.НаселенныйПункт.ПодсказкаВвода                   = НСтр("es='Introduzcan el nombre de la localidad';ru='Введите название населенного пункта';en='Enter the name of the settlement'");
		Элементы.Улица.ПодсказкаВвода                             = НСтр("es='Introduzcan el nombre de la calle';ru='Введите название улицы';en='Enter street name'");
	КонецЕсли;
...
Процедура ОтобразитьИнформациюОДатахДействияАдреса(ДействуетС)
	...
	Элементы.ТекстПроДатуДействия.Заголовок = ТекстИсторическийАдрес;
	Элементы.АдресНаДату.ФорматРедактирования = ?(ЗначениеЗаполнено(АдресНаДату), "", "ДФ='""" + НСтр("es='El comienzo del recuento';ru='начало учета';en='start of accounting'") + """'");
...
Процедура ОтобразитьКоличествоЗаписейВИсторииИзменений()
	...
	Если НайденныеСтроки.Количество() > 1 Тогда
		Элементы.ИсторияИзмененийГиперссылка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La historia de cambios (%1)';ru='История изменений (%1)';en='Change history (%1)'"), НайденныеСтроки.Количество());
	...
	Иначе
		Элементы.ИсторияИзмененийГиперссылка.Заголовок = НСтр("es='La historia de los cambios';ru='История изменений';en='Change history'");
		Элементы.ИсторияИзмененийГиперссылка.Видимость = Истина;
	КонецЕсли;
...
Процедура СообщитьОбОшибкахЗаполнения(СписокОшибок, ПредупреждатьОбОтсутствии, ТребуетсяОбновление = Ложь)
	...
	Если КоличествоОшибок = 0 И ПредупреждатьОбОтсутствии Тогда
		...
		Если НЕ ВебСервисИспользуется И ТребуетсяОбновление Тогда
			Если ЕстьПравоЗагружатьКлассификатор Тогда
				СообщениеОПроверкеАдреса = НСтр("es='La dirección es introducida correctamente, pero las noticias de domicilios cargadas en el programa, hace mucho no se renovaban. "
"Se Recomienda cumplir la renovación y repetir la comprobación de la dirección de nuevo.';ru='Адрес введен корректно, но адресные сведения, загруженные в программу, давно не обновлялись."
"Рекомендуется выполнить обновление и повторить проверку адреса заново.';en='The address was entered correctly, but the address information downloaded to the program has not been updated for a long time."
"It is recommended that you upgrade and re-verify the address again.'");
				ВопросОбОбновлениеАдресногоКлассификатора(СообщениеОПроверкеАдреса);
			Иначе
				СообщениеОПроверкеАдреса = НСтр("es='La dirección es introducida correctamente, pero las noticias de domicilios cargadas en el programa, "
"hace mucho no se renovaban. Se Recomienda cumplir la renovación y repetir la comprobación de la dirección de nuevo. "
"La renovación del clasificador de domicilios tiene que dirigirse al administrador.';ru='Адрес введен корректно, но адресные сведения, загруженные в программу,"
"давно не обновлялись. Рекомендуется выполнить обновление и повторить проверку адреса заново."
"Для обновления адресного классификатора необходимо обратиться к администратору.';en='The address is entered correctly, but the address information loaded into the program,"
"have not been updated for a long time. It is recommended that you upgrade and re-verify the address again."
"To update the address classifier, you must contact the administrator.'");
				ПоказатьПредупреждение(, СообщениеОПроверкеАдреса);
			КонецЕсли;
		Иначе
			ПоказатьПредупреждение(, НСтр("es='La dirección es introducida correctamente';ru='Адрес введен корректно';en='Address entered correctly'"));
		КонецЕсли;
	...
	Если КоличествоОшибок = 1 Тогда
		...
		Если СписокОшибок[0].Пометка = Ложь И НЕ ВебСервисИспользуется Тогда
			ТекстВопроса = НСтр("es='La dirección no puede ser comprobada.';ru='Адрес не может быть проверен.';en='Address cannot be verified.'") + Символы.ПС 
				+ СписокОшибок[0].Представление + Символы.ПС + НСтр("es='¿Cargar el clasificador para la comprobación de la dirección?';ru='Загрузить классификатор для проверки адреса?';en='Download classifier for address verification?'");
	...
	Если Задание.Статус = "Выполнено" Тогда
		...
		Если ДоступностьКлассификатора <> Неопределено Тогда
			...
			Если ДоступностьКлассификатора.Отказ Тогда
				ТекстСообщенияСервиса = НСтр("es='La autoselección y la comprobación de la dirección son inaccesibles:';ru='Автоподбор и проверка адреса недоступны:';en='Auto-matching and address verification are not available:'") + Символы.ПС 
...
Функция ПроверитьДоступностьКлассификатораВФоне(Знач УникальныйИдентификатор)
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("es='La comprobación de la accesibilidad del servicio del clasificador de domicilios';ru='Проверка доступности сервиса адресного классификатора';en='Checking the availability of the address classifier service'");
...
Функция НаименованиеПриДобавлениеПроизвольногоПомещения()
	Возврат НСтр("es='Otro';ru='Другое';en='Other'");
КонецФункции
...
Процедура СформироватьМенюДобавленияСтроенийИПомещений()
	...
	Для каждого СтроениеИлиПомещение Из ДополнительныеСтроенияИПомещения Цикл
		...
		Если ЗначениеЗаполнено(СтроениеИлиПомещение.Ключ) Тогда
			...
			Команда.Подсказка                 = НСтр("es='Añadir';ru='Добавить';en='Add'") + " " + НРег(СтроениеИлиПомещение.Значение);
...
Процедура ПереключитьНаСтруктурированныйВвод(ТипАдреса = Неопределено)
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоАдресВСвободнойФорме(НаселенныйПунктДетально.AddressType) Тогда
		РазрешитьВводАдресаВСвободнойФорме = Ложь;
		ТекстВопроса = НСтр("es='Los cambios introducidos a mano, serán perdidos. "
"La dirección inicial será trasladada al comentario. "
"¿Continuar?';ru='Изменения, введенные вручную, будут потеряны."
"Исходный адрес будет перемещен в комментарий."
"Продолжить?';en='Manual changes will be lost."
"The source address will be moved to the comment."
"Continue?'");
...
Процедура ПереключитьНаСтруктурированныйВводЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Комментарий = ?(НЕ ПустаяСтрока(Комментарий), Комментарий + Символы.ПС, "")
			+ НСтр("es='La dirección inicial:';ru='Исходный адрес:';en='Source address:'") + " " + ПредставлениеАдреса;
	НаселенныйПунктДетально.comment = Комментарий;
...
Процедура СформироватьТекстНужнаПомощь()
	...
	ДополнительныйТекст = Символы.ПС + Символы.ПС + НСтр("es='También se puede introducir la dirección en la forma libre (cm. Todavía - la Dirección en la forma libre).';ru='Также адрес можно ввести в свободной форме (см. Еще - Адрес в свободной форме).';en='You can also enter the address in free form (see also - Address in free form).'");
	...
	Если ВидКонтактнойИнформации.МожноИзменятьСпособРедактирования И ВидКонтактнойИнформации.ПроверятьКорректность Тогда
		ДополнительныйТекст = ДополнительныйТекст + " "+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Para esto preliminarmente quitar el banderín ""Prohibir la introducción de las direcciones incorrectas"" en forma de la información de contacto ""%1"".';ru='Для этого предварительно снять флажок ""Запрещать ввод некорректных адресов"" в виде контактной информации ""%1"".';en='To do it, uncheck the box ""Prevent entering invalid addresses"" in the form of contact information ""%1"".'"),
