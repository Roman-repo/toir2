...
Процедура ОтобразитьПоляПоТипуАдреса()
	...
	Иначе
		...
		Элементы.СубъектРФ.Заголовок = НСтр("es='La región';ru='Регион';en='Region'");
...
Процедура ОбработкаВыбора(Элемент, Знач ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ПустаяСтрока(ВыбранноеЗначение) Тогда
		ТекстПредупреждения = НСтр("es='La elección de la lista es inaccesible, т.к en el clasificador de domicilios falta la información sobre las noticias de domicilios.';ru='Выбор из списка недоступен, т.к в адресном классификаторе отсутствует информация о адресных сведениях.';en='The choice from the list is not available, because the address classifier does not contain information about address information.'");
	...
	Если ЕстьПравоЗагружатьКлассификатор И Не КлассификаторДоступен И ВыбранноеЗначение.ПредлагатьЗагрузкуКлассификатора Тогда
		...
		ПредложениеЗагрузкиКлассификатора(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Los datos para ""%1"" no son cargados.';ru='Данные для ""%1"" не загружены.';en='Data for ""%1"" not loaded.'"), ВыбранноеЗначение.Представление),
