...
Процедура ПояснениеКЗаголовкуРегНомерОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "TechSupport" Тогда
		...
		ТекстСообщения =
			НСтр("es='No logra introducir el número de registro del producto informático.';ru='Не получается ввести регистрационный номер программного продукта.';en='Cannot enter the registration number of the software product.'");
		
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("es='El apoyo de Internet. La introducción del número de registro.';ru='Интернет-поддержка. Ввод регистрационного номера.';en='Online support. Enter registration number.'"),
			ТекстСообщения,
			,
			,
