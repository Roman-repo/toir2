...
Процедура НадписьСодержанияКонтентаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "TechSupport" Тогда
		...
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("es='El apoyo de Internet. El Apoyo De Internet del producto no resulta.';ru='Интернет-поддержка. Интернет-поддержка продукта не оказывается.';en='Online support. Online product support is not provided.'"),
			НСтр("es='A la conexión del Apoyo De Internet es representado el mensaje ""el Apoyo De Internet del producto no resulta"".';ru='При подключении Интернет-поддержки отображается сообщение ""Интернет-поддержка продукта не оказывается"".';en='When Internet support is connected, the message ""Internet product support is not provided."" Is displayed.'"),
			,
			,
