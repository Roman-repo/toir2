...
Процедура ОбработкаНавигационнойСсылкиТехПоддержка(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "TechSupport" Тогда
		...
		ТекстСообщения =
			НСтр("es='No logra enviar la demanda por el registro del participante del cambio ЭД. "
""
"%1';ru='Не получается отправить заявку на регистрацию участника обмена ЭД."
""
"%1';en='Cannot send an application for registration of a participant in the exchange of ED."
""
"%1'");
...
Процедура УстановитьСтатусФормы()
	...
	Если НЕ ЗначениеЗаполнено(СтатусЗаявки)
		...
		Элементы.ВыполнитьДействие.Заголовок = НСтр("es='Crear la demanda';ru='Создать заявку';en='Create request'");
	...
	ИначеЕсли СтатусЗаявки = "notconsidered" Тогда
		...
		Элементы.НадписьЗаявка.Заголовок = НСтр("es='La demanda №';ru='Заявка №';en='Application №'")
	...
	ИначеЕсли СтатусЗаявки = "rejected" Тогда
		...
		Элементы.ВыполнитьДействие.Заголовок = НСтр("es='Crear la demanda';ru='Создать заявку';en='Create request'");
		Элементы.ВыполнитьДействие.Видимость = Истина;
		Элементы.НадписьЗаявка1.Заголовок = НСтр("es='La demanda №';ru='Заявка №';en='Application №'")
			+ " " + ?(ЗначениеЗаполнено(НомерЗаявки), НомерЗаявки, "");
		Если ДатаЗаявки <> Дата(1,1,1) Тогда
