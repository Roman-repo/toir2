...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СтатусФормы = "new" ИЛИ НЕ ЗначениеЗаполнено(СтатусФормы) Тогда
		...
		Элементы.НадписьЗаголовкаФормы.Заголовок = НСтр("es='La demanda por el registro del participante del cambio ЭД';ru='Заявка на регистрацию участника обмена ЭД';en='Application for registration of a participant in the exchange of ED'");
		Элементы.НадписьСтатусаЗаявки.Заголовок = НСтр("es='Una nueva demanda';ru='Новая заявка';en='New application'");
	...
	ИначеЕсли СтатусФормы = "change" Тогда
		...
		Элементы.НадписьЗаголовкаФормы.Заголовок = НСтр("es='El cambio de los datos del participante del cambio ЭД';ru='Изменение данных участника обмена ЭД';en='Changing the data of a participant in an ED exchange'");
		Элементы.НадписьСтатусаЗаявки.Заголовок = НСтр("es='Una nueva demanda';ru='Новая заявка';en='New application'");
		Элементы.ОтправитьЗаявку.Видимость = Истина;
	...
	ИначеЕсли СтатусФормы = "show" Тогда
		...
		ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La demanda № %1 de %2';ru='Заявка №%1 от %2';en='Application № %1 from %2'"),
			НомерЗаявки,
	...
	Если НЕ ЗначениеЗаполнено(СтатусЗаявки) ИЛИ СтатусЗаявки = "none" Тогда
		...
		ТекстСтатуса = НСтр("es='Una nueva demanda.';ru='Новая заявка.';en='New application.'");
		Элементы.НадписьПричина.Видимость = Ложь;
		
	ИначеЕсли СтатусЗаявки = "notconsidered" Тогда
		
		ТекстСтатуса = НСтр("es='La demanda por el operador no es examinada.';ru='Заявка оператором еще не рассмотрена.';en='The application has not yet been reviewed by the operator.'");
		Элементы.НадписьПричина.Видимость = Ложь;
		
	ИначеЕсли СтатусЗаявки = "rejected" Тогда
		
		ТекстСтатуса = НСтр("es='La demanda por el operador es declinada.';ru='Заявка оператором отклонена.';en='The application was rejected by the operator.'");
		Элементы.НадписьПричина.Видимость = Истина;
		
	Иначе
		
		// Статус заявки - "obtained".
		ТекстСтатуса = НСтр("es='La demanda es con éxito cumplida por el operador';ru='Заявка успешно выполнена оператором';en='The application was successfully completed by the operator'");
		Элементы.НадписьПричина.Видимость = Ложь;
	...
	Если Не ПрограммноеЗакрытие Тогда
		...
		Если Модифицированность Тогда
			...
			Если ЗавершениеРаботы Тогда
				ТекстПредупреждения =
					НСтр("es='La demanda por el registro de los datos del participante del cambio para los documentos electrónicos no es enviada.';ru='Заявка на регистрацию данных участника обмена электронными документами не отправлена.';en='The application for registering the data of an electronic document exchange participant has not been sent.'");
			Иначе
				ОписаниеОповещения = Новый ОписаниеОповещения(
					"ПриОтветеНаВопросОЗакрытииМодифицированнойФормы",
					ЭтотОбъект);
				
				ТекстВопроса = НСтр("es='Las demandas dadas son cambiadas. ¿Cerrar la forma sin conservación de los datos?';ru='Данные заявки изменены. Закрыть форму без сохранения данных?';en='Application data changed. Close a form without saving data?'");
...
"%2'");
		...
		Если СтатусФормы = "new" ИЛИ ПустаяСтрока(СтатусФормы) Тогда
			ЧтоНеПолучилось = НСтр("es='No logra enviar la demanda por el registro del participante del cambio ЭД';ru='Не получается отправить заявку на регистрацию участника обмена ЭД';en='Cannot send an application for registration of an ED exchange participant'");
		ИначеЕсли СтатусФормы = "change" Тогда
			ЧтоНеПолучилось = НСтр("es='No logra enviar la demanda por el registro del participante del cambio ЭД';ru='Не получается отправить заявку на регистрацию участника обмена ЭД';en='Cannot send an application for registration of an ED exchange participant'");
		ИначеЕсли СтатусФормы = "show" Тогда
			ЧтоНеПолучилось = НСтр("es='No logra enviar la demanda por el cambio de los datos del participante del cambio ЭД';ru='Не получается отправить заявку на изменение данных участника обмена ЭД';en='Cannot send a request to change the data of the participant in the exchange of electronic communications'");
		КонецЕсли;
		...
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("es='El apoyo de Internet. La demanda por el registro del participante del cambio ЭД en 1S-Takskom';ru='Интернет-поддержка. Заявка на регистрацию участника обмена ЭД в 1С-Такском';en='Online support. Application for registration of a participant in the exchange of ED in 1C-Taxi'"),
			ТекстСообщения,
			"taxcom",
			,
...
Процедура ОтправитьЗаявку(Команда)
	...
	Если ПустаяСтрока(Организация) Тогда
		...
		Сообщение.Текст = НСтр("es='""la Organización""';ru='Не заполнено поле ""Организация""';en='The ""Organization"" field is not filled'");
	...
	Если ПустаяСтрока(Адрес) Тогда
		Ошибка    = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("es='""la Dirección""';ru='Не заполнено поле ""Адрес""';en='Missing Address field'");
	...
	Если ПустаяСтрока(Телефон) Тогда
		Ошибка    = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("es='""Teléfono""';ru='Не заполнено поле ""Телефон""';en='Phone field is not filled'");
	...
	Если ПустаяСтрока(КодРегиона) Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст =
			НСтр("es='"" (el Código de la región es necesario indicar en forma ""la Dirección del participante del cambio ЭД"")';ru='Не указан ""Код региона"" (Код региона нужно указать в форме ""Адрес участника обмена ЭД"")';en='The ""Region code"" is not specified (the Region code must be specified in the form ""Address of the participant in the exchange of electronic data"")'");
		Сообщение.Поле = "Адрес";
		Сообщение.Сообщить();
	КонецЕсли;
	...
	Если ПустаяСтрока(ИНН) Тогда
		Ошибка    = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("es='""el INN""';ru='Не заполнено поле ""ИНН""';en='The TIN field is not filled'");
		Сообщение.Поле = "ИНН";
		Сообщение.Сообщить();
	КонецЕсли;
	...
	Если ЮрФизЛицо <> "ЮрЛицо" И ЮрФизЛицо <> "ФизЛицо" Тогда
		Ошибка    = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("es='de la organización (jurídico o la persona física)';ru='Не выбран тип организации (юридическое или физическое лицо)';en='Type of organization not selected (legal or natural person)'");
	...
	Если ПустаяСтрока(КПП) И ЮрФизЛицо = "ЮрЛицо" Тогда
		Ошибка    = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("es='""КПП""';ru='Не заполнено поле ""КПП""';en='The ""PPC"" field is not filled'");
		Сообщение.Поле = "КПП";
		Сообщение.Сообщить();
	КонецЕсли;
	...
	Если ПустаяСтрока(ОГРН) И ЮрФизЛицо = "ЮрЛицо" Тогда
		Ошибка    = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("es='""ОГРН""';ru='Не заполнено поле ""ОГРН""';en='The OGRN field is not filled'");
		Сообщение.Поле = "ОГРН";
		Сообщение.Сообщить();
	КонецЕсли;
	...
	Если ПустаяСтрока(КодНалоговогоОргана) Тогда
		Ошибка    = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("es='""';ru='Не заполнено поле ""Код налогового органа""';en='The ""Tax authority code"" field is not filled'");
	...
	Если ПустаяСтрока(Фамилия) Тогда
		Ошибка    = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("es='""el Apellido""';ru='Не заполнено поле ""Фамилия""';en='Missing Last Name field'");
	...
	Если ПустаяСтрока(Имя) Тогда
		Ошибка    = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("es='""el Nombre""';ru='Не заполнено поле ""Имя""';en='Name field is not filled'");
		Сообщение.Поле = "Имя";
		Сообщение.Сообщить();
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(Телефон) Тогда
		Если СтрДлина(Телефон) > 20 Тогда
			Ошибка    = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("es='""Teléfono"" debe contener no más 20 símbolos';ru='""Телефон"" должен содержать не более 20 символов';en='""Phone"" must be no more than 20 characters'");
			Сообщение.Поле = "Телефон";
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(КодРегиона) Тогда
		Попытка
			...
			Если СтрДлина(СокрЛП(КодРегиона)) <> 2 Тогда
				Ошибка    = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("es='""El código de la región"" debe contener 2 cifras';ru='""Код региона"" должен содержать 2 цифры';en='Region code must contain 2 figures'");
				Сообщение.Поле = "Адрес";
				Сообщение.Сообщить();
			Иначе
				Если КодРегионаЧисло > 99 ИЛИ КодРегионаЧисло < 1 Тогда
					Ошибка    = Истина;
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = НСтр("es='""El código de la región"" debe ser de 01 hasta 99';ru='""Код региона"" должен быть от 01 до 99';en='Region code must be between 01 and 99'");
		...
		Исключение
			Ошибка    = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("es='En ""el Código de la región"" son usados los símbolos inadmisibles';ru='В ""Коде региона"" использованы недопустимые символы';en='Invalid characters used in region code'");
			Сообщение.Поле = "Адрес";
			Сообщение.Сообщить();
		КонецПопытки;
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(ИНН) Тогда
		Попытка
			...
			Если СтрДлина(СокрЛП(ИНН)) <> 12 И ЮрФизЛицо = "ФизЛицо" Тогда
				Ошибка    = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("es='""El INN"" debe contener 12 cifras';ru='""ИНН"" должен содержать 12 цифр';en='TIN must contain 12 figuress'");
				Сообщение.Поле = "ИНН";
				Сообщение.Сообщить();
			ИначеЕсли СтрДлина(СокрЛП(ИНН)) <> 10 И ЮрФизЛицо = "ЮрЛицо" Тогда
				Ошибка    = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("es='""El INN"" debe contener 10 cifras';ru='""ИНН"" должен содержать 10 цифр';en='TIN must contain 10 figures'");
				Сообщение.Поле = "ИНН";
				Сообщение.Сообщить();
			КонецЕсли;
		Исключение
			Ошибка    = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("es='En ""el INN"" son usados los símbolos inadmisibles';ru='В ""ИНН"" использованы недопустимые символы';en='""INN"" contains invalid characters'");
			Сообщение.Поле = "ИНН";
			Сообщение.Сообщить();
		КонецПопытки;
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(КПП) Тогда
		Попытка
			...
			Если СтрДлина(СокрЛП(КПП)) <> 9 Тогда
				Ошибка    = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("es='""КПП"" debe contener 9 cifras';ru='""КПП"" должен содержать 9 цифр';en='CAT must contain 9 figures'");
				Сообщение.Поле = "КПП";
				Сообщение.Сообщить();
			КонецЕсли;
		Исключение
			Ошибка    = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("es='En ""КПП"" son usados los símbolos inadmisibles';ru='В ""КПП"" использованы недопустимые символы';en='Invalid characters used in PPC'");
			Сообщение.Поле = "КПП";
			Сообщение.Сообщить();
		КонецПопытки;
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(ОГРН) Тогда
		Попытка
			...
			Если СтрДлина(СокрЛП(ОГРН)) <> 13  И ЮрФизЛицо = "ЮрЛицо" Тогда
				Ошибка    = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("es='""ОГРН"" debe contener 13 cifras';ru='""ОГРН"" должен содержать 13 цифр';en='OGRN must contain 13 figures'");
				Сообщение.Поле = "ОГРН";
				Сообщение.Сообщить();
			ИначеЕсли СтрДлина(СокрЛП(ОГРН)) <> 15  И ЮрФизЛицо = "ФизЛицо" Тогда
				Ошибка    = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("es='""ОГРН"" debe contener 15 cifras';ru='""ОГРН"" должен содержать 15 цифр';en='PSRN must contain 15 figures'");
				Сообщение.Поле = "ОГРН";
				Сообщение.Сообщить();
			КонецЕсли;
		Исключение
			Ошибка    = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("es='En ""ОГРН"" son usados los símbolos inadmisibles';ru='В ""ОГРН"" использованы недопустимые символы';en='OGRN uses invalid characters'");
			Сообщение.Поле = "ОГРН";
			Сообщение.Сообщить();
		КонецПопытки;
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(КодНалоговогоОргана) Тогда
		Попытка
			...
			Если СтрДлина(СокрЛП(КодНалоговогоОргана)) <> 4 Тогда
				Ошибка    = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("es='""El código del órgano fiscal"" debe contener 4 cifras';ru='""Код налогового органа"" должен содержать 4 цифры';en='""Tax authority code"" must contain 4 figures'");
				Сообщение.Поле = "КодНалоговогоОргана";
				Сообщение.Сообщить();
			КонецЕсли;
		Исключение
			Ошибка    = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("es='En ""el Código del órgano fiscal"" son usados los símbolos inadmisibles';ru='В ""Коде налогового органа"" использованы недопустимые символы';en='Invalid characters used in ""Tax authority code""'");
			Сообщение.Поле = "КодНалоговогоОргана";
			Сообщение.Сообщить();
		КонецПопытки;
	КонецЕсли;
