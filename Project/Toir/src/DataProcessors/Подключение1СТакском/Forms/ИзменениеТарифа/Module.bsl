...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстЗаголовка = НСтр("es='El período: con %1 por %2';ru='Период: с %1 по %2';en='Period: from%1 to%2'");
	...
	ЗаголовокЗаявки = НСтр("es='La demanda № %1 de %2';ru='Заявка №%1 от %2';en='Application № %1 from %2'");
	...
	Если Не ПрограммноеЗакрытие Тогда
		...
		Если Модифицированность Тогда
			...
			Если ЗавершениеРаботы Тогда
				ТекстПредупреждения =
					НСтр("es='La demanda por el cambio de la tarifa del cambio para los documentos electrónicos no es enviada.';ru='Заявка на изменение тарифа обмена электронными документами не отправлена.';en='The application for changing the rate of exchange of electronic documents has not been sent.'");
			Иначе
...
Процедура ОбработкаНавигационнойСсылкиСообщениеВТехПоддержку(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "TechSupport" Тогда
		...
		ТекстСообщения =
			НСтр("es='No logra cambiar la tarifa del trabajo con el operador del cambio ЭД. "
""
"%1';ru='Не получается изменить тариф работы с оператором обмена ЭД."
""
"%1';en='Cannot change the tariff for working with the ED exchange operator."
""
"%1'");
		...
		ИнтернетПоддержкаПользователейКлиент.ОтправитьСообщениеВТехПоддержку(
			НСтр("es='El apoyo de Internet. El cambio de la tarifa en 1S-Takskom.';ru='Интернет-поддержка. Изменение тарифа в 1С-Такском.';en='Online support. Tariff change in 1C-Taksky.'"),
			ТекстСообщения,
			"taxcom",
			,
...
Процедура УстановитьНадписьНаГиперссылкеОбновленияСтатуса()
	...
	ТекстЗаголовка = НСтр("es='Comprobar la ejecución de la demanda (se quedó %1 segundos)';ru='Проверить выполнение заявки (осталось %1 сек.)';en='Check the fulfillment of the application (%1 sec remaining)'");
