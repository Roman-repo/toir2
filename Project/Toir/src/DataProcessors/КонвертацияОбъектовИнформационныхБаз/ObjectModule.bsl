...
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		ИмяДействия = ?(ТолькоПолучитьУзелСсылки, НСтр("es='La conversión de la referencia al objeto';ru='Конвертация ссылки на объект';en='Object link conversion'"), НСтр("es='La conversión del objeto';ru='Конвертация объекта';en='Object conversion'"));
		
		ТекстСообщения = НСтр("es=""[ИмяДействия]: [el Objeto] ([TipOb'ekta]), ПКО: [ПКО] (НаименованиеПКО)"";ru='[ИмяДействия]: [Объект]([ТипОбъекта]), ПКО: [ПКО](НаименованиеПКО)';en='[ИмяДействия]: [Объект]([ТипОбъекта]), ПКО: [ПКО](НаименованиеПКО)'");
	...
	Если ЕстьГлобальныйОбработчикПередКонвертациейОбъекта Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеВыгрузкиОбработчикаПКО(64, ОписаниеОшибки(), ПКО, Источник, НСтр("es='ПередКонвертациейОбъекта (global)';ru='ПередКонвертациейОбъекта (глобальный)';en='ПередКонвертациейОбъекта (global)'"));
		КонецПопытки;
	...
	Если Не НеНужноВыгружатьПоСоответствиюЗначений Тогда
		...
		Если УзелСсылки = Неопределено Тогда
			...
			Если СтруктураСвойств.ИмяТипа = "Перечисление"
				...
				ЗП.Текст               = НСтр("es='En la regla de la conversión de los significados (ПКЗ) es necesario confrontar el significado de la Fuente al significado del Receptor. "
"Si el significado conveniente del receptor no existe, indicar el significado vacío.';ru='В правиле конвертации значений (ПКЗ) необходимо сопоставить значение Источника значению Приемника."
"Если подходящего значения приемника нет, то указать пустое значение.';en='In the value conversion rule (RPC), you must map the value of the Source to the value of the Receiver."
"If there is no suitable receiver value, then specify an empty value.'");
				//
...
Процедура ВыгрузитьДанныеПоПравилу(Правило) Экспорт
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = НСтр("es='LA REGLA DE LA DESCARGA DE LOS DATOS: %1 (%2)';ru='ПРАВИЛО ВЫГРУЗКИ ДАННЫХ: %1 (%2)';en='DATA UPLOAD RULE:%1 (%2)'");
...
Процедура ЗаписатьОбъектВИБ(Объект, Тип, ЗаписатьОбъект = Ложь, Знач ОтправкаНазад = Ложь) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		Если Не ЭтоРазделенныйОбъектМетаданных Тогда 
			...
			СтрокаСообщенияОбОшибке = НСтр("es='La tentativa del cambio de los datos no divididos (%1) en la sesión dividida.';ru='Попытка изменения неразделенных данных (%1) в разделенном сеансе.';en='Attempt to change unshared data (%1) in a split session.'");
...
Процедура ВыполнитьВыгрузкуДанных(ОбработкаДляЗагрузкиДанных = Неопределено) Экспорт
	...
	Если ЭтоОбменЧерезВнешнееСоединение() Тогда
		...
		Если ОбработкаДляЗагрузкиДанных().ФлагОшибки() Тогда
			...
			СтрокаСообщения = НСтр("es='La falta en la base-corresponsal: %1';ru='Ошибка в базе-корреспонденте: %1';en='Error in correspondent database:%1'");
...
Процедура ВыполнитьЗагрузкуДанных() Экспорт
	...
	Попытка
		...
		Попытка
			...
			Если ФлагОшибки() Тогда
				ВызватьИсключение НСтр("es='Han surgido las faltas a la carga de los datos.';ru='Возникли ошибки при загрузке данных.';en='There were errors loading data.'");
			КонецЕсли;
...
Процедура ВыполнитьЗагрузкуДанныхВИнформационнуюБазу(ТаблицыДляЗагрузки) Экспорт
	...
	СтрокаСообщения = НСтр("es='El comienzo del proceso del cambio para los datos para el nudo: %1';ru='Начало процесса обмена данными для узла: %1';en='Beginning of the data exchange process for the node:%1'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	// Запись в журнале регистрации.
	СтрокаСообщения = НСтр("es='%1, %2; Son tratados %3 objetos';ru='%1, %2; Обработано %3 объектов';en='%1; %2 items processed'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ВыполнитьАнализСообщенияОбмена(ПараметрыАнализа = Неопределено) Экспорт
	...
	Попытка
		...
		Попытка
			...
			Если ФлагОшибки() Тогда
				ВызватьИсключение НСтр("es='Han surgido las faltas durante el análisis de los datos.';ru='Возникли ошибки при анализе данных.';en='Errors occurred while analyzing data.'");
			КонецЕсли;
...
Процедура ВнешнееСоединениеОбработчикКонвертацииПередЗагрузкойДанных(Отказ) Экспорт
	...
	Если Не ПустаяСтрока(Конвертация.ПередЗагрузкойДанных) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(22, ОписаниеОшибки(), НСтр("es='ПередЗагрузкойДанных (la conversión)';ru='ПередЗагрузкойДанных (конвертация)';en='ПередЗагрузкойДанных (conversion)'"));
			Отказ = Истина;
		КонецПопытки;
...
Процедура ВнешнееСоединениеПослеЗагрузкиДанных() Экспорт
	...
	Если Не ФлагОшибки() Тогда
		...
		Если Не ПустаяСтрока(Конвертация.ПослеЗагрузкиДанных) Тогда
			...
			Исключение
				ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(23, ОписаниеОшибки(), НСтр("es='ПослеЗагрузкиДанных (la conversión)';ru='ПослеЗагрузкиДанных (конвертация)';en='ПослеЗагрузкиДанных (conversion)'"));
			КонецПопытки;
...
Функция ИнформацияОПравилах(ЭтоПравилаКорреспондента = Ложь) Экспорт
	...
	Если ЭтоПравилаКорреспондента Тогда
		СтрокаИнформации = НСтр("es='Las reglas de la conversión del corresponsal (%1) de %2';ru='Правила конвертации корреспондента (%1) от %2';en='Correspondent conversion rules (%1) from%2'");
	Иначе
		СтрокаИнформации = НСтр("es='Las reglas de la conversión de esta base informativa (%1) de %2';ru='Правила конвертации этой информационной базы (%1) от %2';en='Rules for converting this infobase (%1) from%2'");
	КонецЕсли;
...
Функция ОткрытьФайлВыгрузки()
	...
	Исключение
		ПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La falta a la apertura del fichero para la anotación del mensaje del cambio. "
"El nombre del fichero ""%1"". "
"La descripción de la falta: "
"%2';ru='Ошибка при открытии файла для записи сообщения обмена."
"Имя файла ""%1""."
"Описание ошибки:"
"%2';en='Error while opening file for recording exchange message."
"The file name is ""%1""."
"Error Description:"
"%2'"),
...
Процедура ИнициализироватьВедениеПротоколаОбмена()
	...
	Исключение
		ФайлПротоколаДанных = Неопределено;
		СтрокаСообщения = НСтр("es='La falta durante la tentativa de la grabación del fichero de la acta de los datos: %1. La descripción de la falta: %2';ru='Ошибка при попытке записи в файл протокола данных: %1. Описание ошибки: %2';en='An error occurred while trying to write to the data log file:%1. Error Description:%2'",
...
Процедура ВосстановитьПравилаИзВнутреннегоФормата() Экспорт
	...
	Если ВерсияФорматаХраненияПравил <> ВерсияФорматаХраненияПравилОбмена() Тогда
		ВызватьИсключение НСтр("es='La versión del formato del almacenaje de las reglas del cambio no corresponde esperado. "
"Es necesario cumplir la carga de las reglas del cambio repetidamente.';ru='Версия формата хранения правил обмена не соответствует ожидаемой."
"Требуется выполнить загрузку правил обмена повторно.';en='The version of the storage format for the exchange rules is not as expected."
...
Процедура НачатьЧтениеСообщения(ЧтениеСообщения, АнализДанных = Ложь)
	...
	Если ФайлОбмена.ТипУзла <> ТипУзлаXML.НачалоЭлемента Тогда
		ВызватьИсключение НСтр("es='La falta del formato del mensaje del cambio.';ru='Ошибка формата сообщения обмена.';en='Error message exchange format.'");
	КонецЕсли;
	...
	Если ТаблицаПравилКонвертации.Количество() = 0 Тогда
		...
		Если ФлагОшибки() Тогда
			ВызватьИсключение НСтр("es='A la carga de las reglas del cambio para los datos han surgido las faltas.';ru='При загрузке правил обмена данными возникли ошибки.';en='Errors occurred while loading communication rules.'");
		КонецЕсли;
	Иначе
	...
	Если Не ПустаяСтрока(Конвертация.ПередЗагрузкойДанных) Тогда
		...
		Исключение
			ВызватьИсключение ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(22, ОписаниеОшибки(), НСтр("es='ПередЗагрузкойДанных (la conversión)';ru='ПередЗагрузкойДанных (конвертация)';en='ПередЗагрузкойДанных (conversion)'"));
		КонецПопытки;
		
		Если Отказ Тогда
			ВызватьИсключение НСтр("es='La renuncia de la carga del mensaje del cambio en el elaborador PeredZagruzkoyDannyh (la conversión).';ru='Отказ от загрузки сообщения обмена в обработчике ПередЗагрузкойДанных (конвертация).';en='Failure to download exchange message in handler ПередЗагрузкойДанных (conversion).'");
		КонецЕсли;
	...
	Если ФайлОбмена.ЛокальноеИмя = "ИнформацияОТипахДанных" Тогда
		...
		Иначе
			...
			Если ФлагОшибки() Тогда
				ВызватьИсключение НСтр("es='A la carga de la información sobre los tipos de datos han surgido las faltas.';ru='При загрузке информации о типах данных возникли ошибки.';en='Errors occurred while loading data type information.'");
			КонецЕсли;
		КонецЕсли;
		ФайлОбмена.Прочитать();
	КонецЕсли;
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение НСтр("es='El bloqueo de la recepción de los datos no puede ser establecido en la transacción activa.';ru='Блокировка получения данных не может быть установлена в активной транзакции.';en='A data acquisition lock cannot be set in an active transaction.'");
	КонецЕсли;
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La falta de la instalación del bloqueo al cambio para los datos. "
"Probablemente, el cambio para los datos se cumple por otra sesión. "
""
"Los detalles: "
...
""
"Подробности:"
"%1';en='Error setting lock on data exchange."
"The data exchange might be in another session."
""
"Details:"
"%1'"),
...
Процедура ВыполнитьОбработчикПослеЗагрузкиДанных()
	...
	Если Не ПустаяСтрока(Конвертация.ПослеЗагрузкиДанных) Тогда
		...
		Исключение
			ВызватьИсключение ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(23, ОписаниеОшибки(), НСтр("es='ПослеЗагрузкиДанных (la conversión)';ru='ПослеЗагрузкиДанных (конвертация)';en='ПослеЗагрузкиДанных (conversion)'"));
		КонецПопытки;
	...
	Попытка
		...
		Если Отказ = Истина Тогда
			...
			Если Не ПустаяСтрока(ПричинаОтказа) Тогда
				...
				СтрокаСообщения = НСтр("es='La renuncia de la carga del mensaje del cambio en el elaborador PosleZagruzkiParametrov (la conversión) por la causa: %1';ru='Отказ от загрузки сообщения обмена в обработчике ПослеЗагрузкиПараметров (конвертация) по причине: %1';en='Failure to download exchange message in handler ПослеЗагрузкиПараметров (conversion) due to: %1'");
				СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, ПричинаОтказа);
				ВызватьИсключение СтрокаСообщения;
			Иначе
				ВызватьИсключение НСтр("es='La renuncia de la carga del mensaje del cambio en el elaborador PosleZagruzkiParametrov (la conversión).';ru='Отказ от загрузки сообщения обмена в обработчике ПослеЗагрузкиПараметров (конвертация).';en='Failure to download exchange message in handler ПослеЗагрузкиПараметров (conversion).'");
			КонецЕсли;
	...
	Пока НомерВариантаПоиска <= 10
		...
		Исключение
			...
			ЗаписатьИнформациюОбОшибкеЗагрузкиОбработчикаПКО(73, ОписаниеОшибки(), "", "", 
				ТипОбъекта, Неопределено, НСтр("es='La consecuencia de los campos de la búsqueda';ru='Последовательность полей поиска';en='Search field sequence'"));
...
Процедура ЗагрузитьТабличнуюЧасть(Объект, ИмяТабличнойЧасти, ОбщаяИнформацияОТипеДокумента, ПараметрыОбъекта, ПКО)
	...
	Исключение
		...
		Текст = НСтр("es='El nombre de la parte tabular: %1';ru='Имя табличной части: %1';en='Tabular name: %1'");
...
Процедура КомментарииКЗагрузкеОбъекта(НПП, ИмяПравила, Источник, ТипОбъекта, ГНПП = 0)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = НСтр("es='La carga del objeto № %1';ru='Загрузка объекта № %1';en='Loading object No.%1'");
...
Функция ПрочитатьНаборЗаписейРегистра()
	...
	Если ЕстьГлобальныйОбработчикПередЗагрузкойОбъекта Тогда
		...
		Исключение
			...
			ТипОбъекта, Неопределено, НСтр("es='ПередЗагрузкойОбъекта (global)';ru='ПередЗагрузкойОбъекта (глобальный)';en='ПередЗагрузкойОбъекта (global)'"));
			...
			Если ЕстьГлобальныйОбработчикПослеЗагрузкиОбъекта Тогда
				...
				Исключение
					...
					ЗаписатьИнформациюОбОшибкеЗагрузкиОбработчикаПКО(54, ОписаниеОшибки(), ИмяПравила, Источник,
						ТипОбъекта, Объект, НСтр("es='ПослеЗагрузкиОбъекта (global)';ru='ПослеЗагрузкиОбъекта (глобальный)';en='ПослеЗагрузкиОбъекта (global)'"));
			...
			Если ИмяТипаОбъекта = "Документ" Тогда
				...
				Если РежимЗагрузкиДанныхВИнформационнуюБазу() Тогда
					...
					Исключение
						...
						СтрокаСообщения = НСтр("es='La falta a la anotación del documento: %1. La descripción de la falta: %2';ru='Ошибка при записи документа: %1. Описание ошибки: %2';en='Error writing document:%1. Error Description:%2'");
...
Процедура ВыполнитьПереходНаНовыйОбмен()
	...
	СтрокаСообщения = НСтр("es='Es cumplido el tránsito automático a la sincronización de los datos a través del formato EnterpriseData.';ru='Выполнен автоматический переход на синхронизацию данных через формат EnterpriseData.';en='Automatically switched to data synchronization through the EnterpriseData format.'");
	ЗаписьЖурналаРегистрацииОбменДанными(СтрокаСообщения, УровеньЖурналаРегистрации.Информация);
	ПолеРезультатВыполненияОбмена = Перечисления.РезультатыВыполненияОбмена.Отменено;
	ВызватьИсключение НСтр("es='La sincronización de los datos del ajuste viejo es anulada.';ru='Синхронизация данных по старой настройке отменена.';en='Data synchronization on old setting is canceled.'");
КонецПроцедуры
...
Процедура ВыгрузитьГруппуСвойств(Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО, ПКГС, УзелКоллекцииСвойств, 
	...
	Если ВыгружатьГруппуЧерезФайл Тогда
		...
		Исключение
			...
			СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La falta a la creación del fichero temporal para la descarga de los datos. "
"El nombre del fichero ""%1"". "
"La descripción de la falta: "
"%2';ru='Ошибка при создании временного файла для выгрузки данных."
"Имя файла ""%1""."
"Описание ошибки:"
"%2';en='Error creating temporary file for uploading data."
"The file name is ""%1 ""."
"Error Description:"
"%2 '"),
...
Процедура ВызватьСобытияПередВыгрузкойОбъекта(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, 
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		ИмяСобытия = НСтр("es='ВыгрузкаОбъекта: %1';ru='ВыгрузкаОбъекта: %1';en='ВыгрузкаОбъекта: %1'");
	...
	Если ЕстьГлобальныйОбработчикПередВыгрузкойОбъекта Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиПВД(65, ОписаниеОшибки(), Правило.Имя, НСтр("es='ПередВыгрузкойОбъектаВыборки (global)';ru='ПередВыгрузкойОбъектаВыборки (глобальный)';en='ПередВыгрузкойОбъектаВыборки (global)'"), Объект);
		КонецПопытки;
...
Процедура ВызватьСобытияПослеВыгрузкиОбъекта(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, 
	...
	Если ЕстьГлобальныйОбработчикПослеВыгрузкиОбъекта Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиПВД(69, ОписаниеОшибки(), Правило.Имя, НСтр("es='ПослеВыгрузкиОбъектаВыборки (global)';ru='ПослеВыгрузкиОбъектаВыборки (глобальный)';en='ПослеВыгрузкиОбъектаВыборки (глобальный)'"), Объект);
		КонецПопытки;
	КонецЕсли;
...
Процедура ОтработатьУдалениеОбъекта(ДанныеОбУдаленииОбъекта, СтрокаСообщенияОбОшибке = "")
	...
	Если Конвертация.Свойство("ПередОтправкойИнформацииОбУдалении", ТекстСобытия) Тогда
		...
		Если Не ПустаяСтрока(ТекстСобытия) Тогда
			...
			Исключение
				СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(76, ОписаниеОшибки(), НСтр("es='ПередОтправкойИнформацииОбУдалении (la conversión)';ru='ПередОтправкойИнформацииОбУдалении (конвертация)';en='ПередОтправкойИнформацииОбУдалении (conversion)'"));
...
Функция РеквизитыРегистрацииСвойств(ТаблицаПКС)
	...
	Для Каждого ПКС Из МассивСтрокПКС Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("es='El cambio para los datos. La carga de las reglas de la conversión';ru='Обмен данными.Загрузка правил конвертации';en='Data exchange. Download conversion rules'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ВыполнитьПроверкуРеквизитовРегистрацииОбъектов(ТаблицаРеквизитовРегистрации)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаРеквизитовРегистрации Цикл
		...
		Исключение
			...
			СтрокаСообщения = НСтр("es='El tipo del objeto no es determinado: %1';ru='Тип объекта не определен: %1';en='Type of object not defined:%1'");
			...
			Для Каждого Реквизит Из СтрокаТаблицы.СтруктураРеквизитовРегистрации Цикл
				...
				Если ОбщегоНазначения.ЭтоЗадача(ОбъектМД) Тогда
					...
					Если НЕ (ОбъектМД.Реквизиты.Найти(Реквизит.Ключ) <> Неопределено
						...
						СтрокаСообщения = НСтр("es='Son incorrectamente indicados los requisitos del gorro del objeto ""%1"". El requisito ""%2"" no existe.';ru='Неправильно указаны реквизиты шапки объекта ""%1"". Реквизит ""%2"" не существует.';en='The details of the header of the object ""%1"" are incorrect. The requisite ""%2"" does not exist.'");
		...
		Иначе
			...
			Если МетаТаблица = Неопределено Тогда
				ЗаписатьВПротоколВыполнения(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='La parte tabular (la parte estandartizada tabular, el movimiento) ""%1"" objetos ""%2"" no existe.';ru='Табличная часть (стандартная табличная часть, движения) ""%1"" объекта ""%2"" не существует.';en='Tabular part (standard table part, movements) of ""%1"" of object ""%2"" does not exist.'"),
			...
			Для Каждого Реквизит Из СтрокаТаблицы.СтруктураРеквизитовРегистрации Цикл
				...
				Если Не РеквизитНайденВТабличнойЧастиРеквизитовРегистрацииОбъектов(МетаТаблица, Реквизит.Ключ) Тогда
					ЗаписатьВПротоколВыполнения(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='El requisito ""%3"" no existe en la parte tabular (la parte estandartizada tabular, los movimientos) ""%1"" objetos ""%2"".';ru='Реквизит ""%3"" не существует в табличной части (стандартной табличной части, движениях) ""%1"" объекта ""%2"".';en='Props ""%3"" does not exist in the tabular section (standard tabular section, movements) ""%1"" of the object ""%2"".'"),
...
Функция ИнициализироватьОбработки()
	...
	Если ОтладкаОбработчиковВыгрузки Или ОтладкаОбработчиковЗагрузки Тогда 
		ВызватьИсключение
			НСтр("es='El tratamiento exterior del ajuste cargado del fichero sobre el disco, no es apoyado.';ru='Внешняя обработка отладки, загружаемая из файла на диске, не поддерживается.';en='External debug processing loaded from a file on disk is not supported.'");
	КонецЕсли;
...
Процедура ОтключитьОбработкуДляОтладки()
	...
	Если ОбработкаВыгрузки <> Неопределено Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("es='El cambio para los datos';ru='Обмен данными';en='Data exchange'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
Процедура ИнициализацияСообщений()
	...
	СообщенияОбОшибках.Вставить(2,  НСтр("es='La falta del desempaquetamiento del fichero del cambio. El fichero es bloqueado.';ru='Ошибка распаковки файла обмена. Файл заблокирован.';en='Error unpacking file sharing. The file is locked.'"));
	СообщенияОбОшибках.Вставить(3,  НСтр("es='El fichero indicado gobernaba el cambio no existe.';ru='Указанный файл правил обмена не существует.';en='The specified exchange rule file does not exist.'"));
	СообщенияОбОшибках.Вставить(4,  НСтр("es='La falta a la creación del COM-objeto Msxml2.DOMDocument';ru='Ошибка при создании COM-объекта Msxml2.DOMDocument';en='Error creating COM object Msxml2.DOMDocument'"));
	СообщенияОбОшибках.Вставить(5,  НСтр("es='La falta de la apertura del fichero del cambio';ru='Ошибка открытия файла обмена';en='Error opening file sharing'"));
	СообщенияОбОшибках.Вставить(6,  НСтр("es='La falta a la carga de las reglas del cambio';ru='Ошибка при загрузке правил обмена';en='Error loading exchange rules'"));
	СообщенияОбОшибках.Вставить(7,  НСтр("es='La falta del formato de las reglas del cambio';ru='Ошибка формата правил обмена';en='Exchange rule format error'"));
	СообщенияОбОшибках.Вставить(8,  НСтр("es='Es indicado el nombre del fichero para la descarga de los datos';ru='Некорректно указано имя файла для выгрузки данных';en='Incorrect file name for uploading data'")); // не используется
	СообщенияОбОшибках.Вставить(9,  НСтр("es='La falta del formato del fichero del cambio';ru='Ошибка формата файла обмена';en='Exchange file format error'"));
	СообщенияОбОшибках.Вставить(10, НСтр("es='dado (el Nombre del fichero de los datos)';ru='Не указано имя файла для выгрузки данных (Имя файла данных)';en='Missing file name specified (Data file name)'"));
	СообщенияОбОшибках.Вставить(11, НСтр("es='La referencia al objeto no existente de los metadatos en las reglas del cambio';ru='Ссылка на несуществующий объект метаданных в правилах обмена';en='Reference to a nonexistent metadata object in exchange rules'"));
	СообщенияОбОшибках.Вставить(12, НСтр("es='del cambio (el Nombre del fichero de las reglas)';ru='Не указано имя файла с правилами обмена (Имя файла правил)';en='Missing exchange rule file name (Rules file name)'"));
			
	СообщенияОбОшибках.Вставить(13, НСтр("es='La falta del desempaquetamiento del fichero del cambio. El fichero es bloqueado.';ru='Ошибка распаковки файла обмена. Файл заблокирован.';en='Error unpacking file sharing. The file is locked.'"));
	СообщенияОбОшибках.Вставить(14, НСтр("es='El fichero indicado gobernaba el cambio no existe.';ru='Указанный файл правил обмена не существует.';en='The specified exchange rule file does not exist.'"));
	
	СообщенияОбОшибках.Вставить(15, НСтр("es='La falta de la apertura del fichero del cambio';ru='Ошибка открытия файла обмена';en='Error opening file sharing'"));
			
	СообщенияОбОшибках.Вставить(16, НСтр("es='La falta del formato de las reglas del cambio';ru='Ошибка формата правил обмена';en='Exchange rule format error'"));
	СообщенияОбОшибках.Вставить(17, НСтр("es='Es indicado el nombre del fichero para la descarga de los datos';ru='Некорректно указано имя файла для выгрузки данных';en='Incorrect file name for uploading data'"));
	СообщенияОбОшибках.Вставить(18, НСтр("es='La falta del formato del fichero del cambio';ru='Ошибка формата файла обмена';en='Exchange file format error'"));
	СообщенияОбОшибках.Вставить(19, НСтр("es='dado (el Nombre del fichero de los datos)';ru='Не указано имя файла для выгрузки данных (Имя файла данных)';en='Missing file name specified (Data file name)'"));
	СообщенияОбОшибках.Вставить(20, НСтр("es='La referencia al objeto no existente de los metadatos en las reglas del cambio';ru='Ссылка на несуществующий объект метаданных в правилах обмена';en='Reference to a nonexistent metadata object in exchange rules'"));
	СообщенияОбОшибках.Вставить(21, НСтр("es='del cambio (el Nombre del fichero de las reglas)';ru='Не указано имя файла с правилами обмена (Имя файла правил)';en='Missing exchange rule file name (Rules file name)'"));
	СообщенияОбОшибках.Вставить(22, НСтр("es='La falta en el elaborador del acontecimiento PeredZagruzkoyDannyh (conversión)';ru='Ошибка в обработчике события ПередЗагрузкойДанных (конвертация)';en='Error in the ПередЗагрузкойДанных event handler (conversion)'"));
	СообщенияОбОшибках.Вставить(23, НСтр("es='La falta de la recepción del significado de la propiedad del objeto (por nombre de las propiedades de la fuente)';ru='Ошибка получения значения свойства объекта (по имени свойства источника)';en='Error getting the property value of the object (by the name of the source property)'"));
	СообщенияОбОшибках.Вставить(24, НСтр("es='La falta de la recepción del significado de la propiedad del objeto (por nombre de las propiedades del receptor)';ru='Ошибка получения значения свойства объекта (по имени свойства приемника)';en='Error getting the value of an object property (by name of the receiver property)'"));
	СообщенияОбОшибках.Вставить(25, НСтр("es='La falta a la anotación del documento';ru='Ошибка при записи документа';en='Error while writing document'"));
	СообщенияОбОшибках.Вставить(26, НСтр("es='dado (el Nombre del fichero para la carga)';ru='Не указано имя файла для загрузки данных (Имя файла для загрузки)';en='File name to upload data (File name to upload) is not specified. '"));
	СообщенияОбОшибках.Вставить(27, НСтр("es='La falta en el elaborador del acontecimiento De PeredObrabotkoyPravilaOchistki';ru='Ошибка в обработчике события ПередОбработкойПравилаОчистки';en='Error in the ПередОбработкойПравилаОчистки event handler'"));
	СообщенияОбОшибках.Вставить(28, НСтр("es='La falta de la recepción del significado de la propiedad del objeto subordinado (por nombre de las propiedades de la fuente)';ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства источника)';en='Error getting the value of the property of the subordinate object (by the name of the source property)'"));
	СообщенияОбОшибках.Вставить(29, НСтр("es='La falta de la recepción del significado de la propiedad del objeto subordinado (por nombre de las propiedades del receptor)';ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства приемника)';en='Error getting the value of the property of the slave object (by the name of the receiver property)'"));
	
	СообщенияОбОшибках.Вставить(31, НСтр("es=""La falta en el elaborador del acontecimiento De PeredZagruzkoyOb'ekta"";ru='Ошибка в обработчике события ПередЗагрузкойОбъекта';en='Error in the ПередЗагрузкойОбъекта event handler'"));
	СообщенияОбОшибках.Вставить(32, НСтр("es=""La falta en el elaborador del acontecimiento de PriZagruzkeOb'ekta"";ru='Ошибка в обработчике события ПриЗагрузкеОбъекта';en='Error in the event handler ПриЗагрузкеОбъекта'"));
	СообщенияОбОшибках.Вставить(33, НСтр("es=""La falta en el elaborador del acontecimiento De PosleZagruzkiOb'ekta"";ru='Ошибка в обработчике события ПослеЗагрузкиОбъекта';en='Error in the event handler ПослеЗагрузкиОбъекта'"));
	СообщенияОбОшибках.Вставить(34, НСтр("es='La falta en el elaborador del acontecimiento PeredZagruzkoyDannyh (conversión)';ru='Ошибка в обработчике события ПередЗагрузкойДанных (конвертация)';en='Error in the ПередЗагрузкойДанных event handler (conversion)'"));
			
	СообщенияОбОшибках.Вставить(41, НСтр("es='La falta a la desaparición del objeto';ru='Ошибка при удалении объекта';en='Error deleting object'"));
	СообщенияОбОшибках.Вставить(42, НСтр("es='La falta a la anotación del documento';ru='Ошибка при записи документа';en='Error while writing document'"));
	СообщенияОбОшибках.Вставить(43, НСтр("es='La falta de la anotación del objeto';ru='Ошибка записи объекта';en='Error writing object'"));
			
	СообщенияОбОшибках.Вставить(45, НСтр("es='La falta en el elaborador del acontecimiento De PosleObrabotkiPravilaOchistki';ru='Ошибка в обработчике события ПослеОбработкиПравилаОчистки';en='Error in the event handler ПослеОбработкиПравилаОчистки'"));
		
	СообщенияОбОшибках.Вставить(48, НСтр("es='La falta en el elaborador del acontecimiento De PeredObrabotkoyVygruzki del grupo de las propiedades';ru='Ошибка в обработчике события ПередОбработкойВыгрузки группы свойств';en='Error in the ПередОбработкойВыгрузки event handler properties group'"));
	СообщенияОбОшибках.Вставить(49, НСтр("es='La falta en el elaborador del acontecimiento De PeredObrabotkoyPravilaVygruzki';ru='Ошибка в обработчике события ПередОбработкойПравилаВыгрузки';en='Error in the ПередОбработкойПравилаВыгрузки event handler'"));
	СообщенияОбОшибках.Вставить(50, НСтр("es='La falta en el elaborador del acontecimiento De PosleObrabotkiPravilaVygruzki';ru='Ошибка в обработчике события ПослеОбработкиПравилаВыгрузки';en='Error in the event handler ПослеОбработкиПравилаВыгрузки'"));
	СообщенияОбОшибках.Вставить(51, НСтр("es=""La falta en el elaborador del acontecimiento De PeredVygruzkoyOb'ekta"";ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';en='Error in the ПередВыгрузкойОбъекта event handler'"));
	СообщенияОбОшибках.Вставить(52, НСтр("es=""La falta en el elaborador del acontecimiento De PosleVygruzkiOb'ekta"";ru='Ошибка в обработчике события ПослеВыгрузкиОбъекта';en='Error in the event handler ПослеВыгрузкиОбъекта'"));
	СообщенияОбОшибках.Вставить(53, НСтр("es=""La falta en el elaborador global del acontecimiento PeredZagruzkoyOb'ekta (conversión)"";ru='Ошибка в глобальном обработчике события ПередЗагрузкойОбъекта (конвертация)';en='Error in the global event handler ПередЗагрузкойОбъекта (conversion)'"));
	СообщенияОбОшибках.Вставить(54, НСтр("es=""La falta en el elaborador del acontecimiento De PeredVygruzkoyOb'ekta"";ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';en='Error in the ПередВыгрузкойОбъекта event handler'"));
	СообщенияОбОшибках.Вставить(55, НСтр("es=""La falta en el elaborador del acontecimiento de PriVygruzkeOb'ekta"";ru='Ошибка в обработчике события ПриВыгрузкеОбъекта';en='Error in the event handler ПриВыгрузкеОбъекта'"));
	СообщенияОбОшибках.Вставить(56, НСтр("es=""La falta en el elaborador del acontecimiento De PosleVygruzkiOb'ekta"";ru='Ошибка в обработчике события ПослеВыгрузкиОбъекта';en='Error in the event handler ПослеВыгрузкиОбъекта'"));
	СообщенияОбОшибках.Вставить(57, НСтр("es='La falta en el elaborador del acontecimiento De PosleVygruzki (propiedad)';ru='Ошибка в обработчике события ПослеВыгрузки (свойства)';en='Error in the event handler ПослеВыгрузки (properties)'"));
	
	СообщенияОбОшибках.Вставить(62, НСтр("es='La falta en el elaborador del acontecimiento PeredVygruzkoyDannyh (conversión)';ru='Ошибка в обработчике события ПередВыгрузкойДанных (конвертация)';en='Error in the ПередВыгрузкойДанных event handler (conversion)'"));
	СообщенияОбОшибках.Вставить(63, НСтр("es='La falta en el elaborador del acontecimiento De PeredObrabotkoyVygruzki del grupo de las propiedades';ru='Ошибка в обработчике события ПередОбработкойВыгрузки группы свойств';en='Error in the ПередОбработкойВыгрузки event handler properties group'"));
	СообщенияОбОшибках.Вставить(64, НСтр("es='La falta en el elaborador del acontecimiento De PosleObrabotkiVygruzki del grupo de las propiedades';ru='Ошибка в обработчике события ПослеОбработкиВыгрузки группы свойств';en='Error in the event handler ПослеОбработкиВыгрузки properties group'"));
	СообщенияОбОшибках.Вставить(65, НСтр("es='La falta en el elaborador del acontecimiento PeredVygruzkoy (el objeto de la colección)';ru='Ошибка в обработчике события ПередВыгрузкой (объекта коллекции)';en='Error in the ПередВыгрузкой event handler (collection object)'"));
	СообщенияОбОшибках.Вставить(66, НСтр("es='La falta en el elaborador del acontecimiento de PriVygruzke (el objeto de la colección)';ru='Ошибка в обработчике события ПриВыгрузке (объекта коллекции)';en='Error in the event handler ПриВыгрузке (collection object)'"));
	СообщенияОбОшибках.Вставить(67, НСтр("es='La falta en el elaborador del acontecimiento De PosleVygruzki (el objeto de la colección)';ru='Ошибка в обработчике события ПослеВыгрузки (объекта коллекции)';en='Error in the event handler ПослеВыгрузки (collection object)'"));
	СообщенияОбОшибках.Вставить(68, НСтр("es=""La falta en el elaborador global del acontecimiento PeredZagruzkoyOb'ekta (conversión)"";ru='Ошибка в глобальном обработчике события ПередЗагрузкойОбъекта (конвертация)';en='Error in the global event handler ПередЗагрузкойОбъекта (conversion)'"));
	
	СообщенияОбОшибках.Вставить(69, НСтр("es='La falta en el elaborador del acontecimiento PeredVygruzkoy (propiedades)';ru='Ошибка в обработчике события ПередВыгрузкой (свойства)';en='Error in the ПередВыгрузкой event handler (properties)'"));
	
	СообщенияОбОшибках.Вставить(71, НСтр("es='La falta en el elaborador del acontecimiento De PosleVygruzki (propiedad)';ru='Ошибка в обработчике события ПослеВыгрузки (свойства)';en='Error in the event handler ПослеВыгрузки (properties)'"));
	
	СообщенияОбОшибках.Вставить(72, НСтр("es='La falta en el elaborador del acontecimiento PeredVygruzkoyDannyh (conversión)';ru='Ошибка в обработчике события ПередВыгрузкойДанных (конвертация)';en='Error in the ПередВыгрузкойДанных event handler (conversion)'"));
	
	СообщенияОбОшибках.Вставить(73, НСтр("es=""La falta en el elaborador global del acontecimiento PeredKonvertatsieyOb'ekta (conversión)"";ru='Ошибка в глобальном обработчике события ПередКонвертациейОбъекта (конвертация)';en='Error in the global event handler ПередКонвертациейОбъекта (conversion)'"));
	СообщенияОбОшибках.Вставить(74, НСтр("es=""La falta en el elaborador global del acontecimiento PeredVygruzkoyOb'ekta (conversión)"";ru='Ошибка в глобальном обработчике события ПередВыгрузкойОбъекта (конвертация)';en='Error in the global event handler ПередВыгрузкойОбъекта  (conversion)'"));
	
	СообщенияОбОшибках.Вставить(75, НСтр("es='La falta de la recepción de la propiedad del objeto subordinado de los datos entrantes';ru='Ошибка получения свойства подчиненного объекта из входящих данных';en='Error getting property of a slave object from incoming data'"));
	СообщенияОбОшибках.Вставить(76, НСтр("es='La falta de la recepción de la propiedad del objeto de los datos entrantes';ru='Ошибка получения свойства объекта из входящих данных';en='Error getting object properties from incoming data'"));
	СообщенияОбОшибках.Вставить(77, НСтр("es='La falta en el elaborador del acontecimiento PriPolucheniiInformatsiiObUdalenii (conversión)';ru='Ошибка в обработчике события ПриПолученииИнформацииОбУдалении (конвертация)';en='Error in the event handler ПриПолученииИнформацииОбУдалении (conversion)'"));
	
	СообщенияОбОшибках.Вставить(78, НСтр("es='La falta a la ejecución del algoritmo después de la carga de los significados de los parámetros';ru='Ошибка при выполнении алгоритма после загрузки значений параметров';en='An error occurred while executing the algorithm after loading parameter values'"));
	
	СообщенияОбОшибках.Вставить(79, НСтр("es=""La falta en el elaborador del acontecimiento De PosleVygruzkiOb'ektaVFayl"";ru='Ошибка в обработчике события ПослеВыгрузкиОбъектаВФайл';en='Error in the event handler ПослеВыгрузкиОбъектаВФайл'"));
	
	СообщенияОбОшибках.Вставить(80, НСтр("es='La falta de la instalación de la propiedad del elemento predestinado. "
"No es posible marcar a la desaparición el elemento predestinado. La marca a la desaparición para el objeto no es establecida.';ru='Ошибка установки свойства предопределенного элемента."
"Нельзя помечать на удаление предопределенный элемент. Пометка на удаление для объекта не установлена.';en='Error setting property of a predefined item."
"Can not flag a predefined item for deletion. Delete mark has not been set for the object.'"));
	//
	СообщенияОбОшибках.Вставить(83, НСтр("es='La falta en el elaborador del acontecimiento PosleZagruzkiPravilObmena (conversión)';ru='Ошибка в обработчике события ПослеЗагрузкиПравилОбмена (конвертация)';en='Error in the event handler ПослеЗагрузкиПравилОбмена (conversion)'"));
	СообщенияОбОшибках.Вставить(84, НСтр("es='La falta en el elaborador del acontecimiento PeredOtpravkoyInformatsiiObUdalenii (conversión)';ru='Ошибка в обработчике события ПередОтправкойИнформацииОбУдалении (конвертация)';en='Error in the ПередОтправкойИнформацииОбУдалении event handler (conversion)'"));
	
	СообщенияОбОшибках.Вставить(173, НСтр("es='La falta del bloqueo del nudo del cambio. Probablemente la sincronización de los datos se cumple ya';ru='Ошибка блокировки узла обмена. Возможно синхронизация данных уже выполняется';en='Error blocking the exchange node. Maybe data synchronization is already in progress'"));
	СообщенияОбОшибках.Вставить(174, НСтр("es='El mensaje del cambio era aceptado antes';ru='Сообщение обмена было принято ранее';en='The exchange message was received earlier.'"));
	СообщенияОбОшибках.Вставить(175, НСтр("es=""La falta en el elaborador del acontecimiento PeredPolucheniemIzmenennyhOb'ektov (conversión)"";ru='Ошибка в обработчике события ПередПолучениемИзмененныхОбъектов (конвертация)';en='Error in the ПередПолучениемИзмененныхОбъектов event handler (conversion)'"));
	СообщенияОбОшибках.Вставить(176, НСтр("es='La falta en el elaborador del acontecimiento PoslePolucheniyaInformatsiiObUzlahObmena (conversión)';ru='Ошибка в обработчике события ПослеПолученияИнформацииОбУзлахОбмена (конвертация)';en='Error in the event handler ПослеПолученияИнформацииОбУзлахОбмена (conversion)'"));
		
	СообщенияОбОшибках.Вставить(1000, НСтр("es='La falta a la creación del fichero temporal de la descarga de los datos';ru='Ошибка при создании временного файла выгрузки данных';en='Error creating temporary data upload file'"));
...
Процедура ЗагрузитьПравилаОбмена(Источник="",
	...
	Если РежимОбмена <> "Загрузка" И Конвертация.Свойство("ПослеЗагрузкиПравилОбмена", ТекстСобытияПослеЗагрузкиПравилОбмена)
		...
		Исключение
			СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(75, ОписаниеОшибки(), НСтр("es='ПослеЗагрузкиПравилОбмена (la conversión)';ru='ПослеЗагрузкиПравилОбмена (конвертация)';en='ПослеЗагрузкиПравилОбмена (conversion)'"));
...
Процедура УдалитьОбъектПоСсылке(Ссылка, СтрокаСообщенияОбОшибке)
	...
	Если Не ПустаяСтрока(Конвертация.ПриПолученииИнформацииОбУдалении) Тогда
		...
		Исключение
			СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(77, ОписаниеОшибки(), НСтр("es='ПриПолученииИнформацииОбУдалении (la conversión)';ru='ПриПолученииИнформацииОбУдалении (конвертация)';en='ПриПолученииИнформацииОбУдалении (conversion)'"));
			Отказ = Истина;
					...
					Если СтрокаТаблицы = Неопределено Тогда
						...
						Для Каждого Колонка Из ТаблицаДанныхСообщенияОбмена.Колонки Цикл
							...
							Если Колонка.ТипЗначения.СодержитТип(ТипСтрока) Тогда
								...
								СтрокаТаблицы[Колонка.Имя] = НСтр("es='La desaparición del objeto';ru='Удаление объекта';en='Deleting object'");
				...
				Попытка
					...
					Если Отказ = Истина Тогда
						...
						Если Не ПустаяСтрока(ПричинаОтказа) Тогда
							...
							СтрокаСообщения = НСтр("es='La carga de los datos es anulada por la causa: %1';ru='Загрузка данных отменена по причине: %1';en='Data upload canceled due to:%1'");
							СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, ПричинаОтказа);
							ВызватьИсключение СтрокаСообщения;
						Иначе
							ВызватьИсключение НСтр("es='La carga de los datos es anulada';ru='Загрузка данных отменена';en='Data upload canceled'");
						КонецЕсли;
...
Процедура ПрочитатьДанныеПоОбмену(ЧтениеСообщения, АнализДанных)
	...
	Если Не ЗначениеЗаполнено(УзелОбменаПолучатель)
		...
		СтрокаСообщения = НСтр("es='de los datos. El plan del cambio: %1, el Código: %2';ru='Не найден узел обмена для загрузки данных. План обмена: %1, Код: %2';en='An exchange node was not found for loading data. Exchange plan:%1, Code: %2'");
	...
	Если РежимЗагрузкиДанныхВИнформационнуюБазу() Тогда
		...
		Если НомерПринятого >= ЧтениеСообщения.НомерСообщения Тогда // Номер сообщения меньше либо равен ранее принятому.
			...
			ВызватьИсключение НСтр("es='El mensaje del cambio era aceptado antes';ru='Сообщение обмена было принято ранее';en='The exchange message was received earlier.'");
		КонецЕсли;
		...
		Если УдалитьРегистрациюИзменений Тогда // Удаляем регистрацию изменений.
			...
			Если ТранзакцияАктивна() Тогда
				ВызватьИсключение НСтр("es='La desaparición del registro de los cambios de los datos no puede ser cumplida en la transacción activa.';ru='Удаление регистрации изменений данных не может быть выполнено в активной транзакции.';en='Deleting the registration of data changes cannot be performed in the active transaction.'");
			КонецЕсли;
	...
	Если Не ПустаяСтрока(Конвертация.ПослеПолученияИнформацииОбУзлахОбмена) Тогда
		...
		Исключение
			ВызватьИсключение ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(176, ОписаниеОшибки(), НСтр("es='ПослеПолученияИнформацииОбУзлахОбмена (la conversión)';ru='ПослеПолученияИнформацииОбУзлахОбмена (конвертация)';en='ПослеПолученияИнформацииОбУзлахОбмена (conversion)'"));
		КонецПопытки;
	...
	Для Каждого ОбъектСоответствия Из ОбъектыДляОтложеннойЗаписи() Цикл
		...
		Попытка
			...
			Иначе
				...
				ОписаниеОшибки = НСтр("es='La falta de la comprobación del relleno de los requisitos';ru='Ошибка проверки заполнения реквизитов';en='Error checking details'");
...
Процедура ВыполнитьВыгрузку(СтрокаСообщенияОбОшибке = "")
	...
	Если Не ПустаяСтрока(Конвертация.ПередВыгрузкойДанных) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(62, ОписаниеОшибки(), НСтр("es='ПередВыгрузкойДанных (la conversión)';ru='ПередВыгрузкойДанных (конвертация)';en='ПередВыгрузкойДанных (conversion)'"));
			Отказ = Истина;
		КонецПопытки; 
	...
	Если Не Отказ И Не ПустаяСтрока(Конвертация.ПослеВыгрузкиДанных) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(63, ОписаниеОшибки(), НСтр("es='ПослеВыгрузкиДанных (la conversión)';ru='ПослеВыгрузкиДанных (конвертация)';en='ПослеВыгрузкиДанных (конвертация)'"));
		КонецПопытки;
...
Процедура ВыполнитьВыгрузкуЗарегистрированныхДанных(ЗаписьСообщения, СтрокаСообщенияОбОшибке, ТаблицаПравилВыгрузкиИспользуемые)
	...
	Если Не ПустаяСтрока(Конвертация.ПередПолучениемИзмененныхОбъектов) Тогда
		...
		Исключение
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(175, ОписаниеОшибки(), НСтр("es='ПередПолучениемИзмененныхОбъектов (la conversión)';ru='ПередПолучениемИзмененныхОбъектов (конвертация)';en='ПередПолучениемИзмененныхОбъектов (conversion)'"));
			Возврат;
		КонецПопытки;
		...
		Если ЭтоОбменЧерезВнешнееСоединение Тогда
			Если ЗагрузкаДанныхВыполняетсяВоВнешнемСоединении Тогда
				Если ОбработкаДляЗагрузкиДанных().ИспользоватьТранзакции Тогда
					...
					Если ОбработкаДляЗагрузкиДанных().ФлагОшибки() Тогда
						ВызватьИсключение(НСтр("es='La falta al envío de los datos.';ru='Ошибка при отправке данных.';en='Error sending data.'"));
					Иначе
						ВнешнееСоединение.ЗафиксироватьТранзакцию();
					КонецЕсли;
	...
	Исключение
		...
		ВызватьИсключение(НСтр("es='La falta al envío de los datos';ru='Ошибка при отправке данных';en='Error sending data'") + ": " + ОписаниеОшибки());
