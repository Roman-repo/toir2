...
Процедура ОбработатьВыборВСпискеКалендарей(СтандартнаяОбработка = Неопределено)
	...
	ПоказатьОповещениеПользователя(НСтр("es='Los calendarios son añadidos.';ru='Календари добавлены.';en='Calendars added.'"), , , БиблиотекаКартинок.Информация32);
	Закрыть();
...
Функция БазовыйКалендарь(КодБазовогоКалендаря)
	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El calendario con el código %1, indicado en calidad de básico, falta en la lista suministrado.';ru='Календарь с кодом %1, указанный в качестве базового, отсутствует в списке поставляемых.';en='Calendar with code%1, specified as the base, is not in the list of supplied ones.'"),
