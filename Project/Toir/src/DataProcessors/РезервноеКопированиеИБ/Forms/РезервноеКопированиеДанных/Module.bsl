...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначения.ЭтоWindowsКлиент() Тогда
		ВызватьИсключение НСтр("es='Es necesario ajustar la copia de reserva y la reconstitución de los datos por los medios del sistema de operaciones u otros medios extraños.';ru='Резервное копирование и восстановление данных необходимо настроить средствами операционной системы или другими сторонними средствами.';en='Data backup and restore must be configured using the operating system or other third-party tools.'");
	КонецЕсли;
	
	Если ОбщегоНазначения.ЭтоВебКлиент() Тогда
		ВызватьИсключение НСтр("es='La copia de reserva es inaccesible en el cliente web.';ru='Резервное копирование недоступно в веб-клиенте.';en='Backup is not available in the web client.'");
	КонецЕсли;
	
	Если НЕ ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		ВызватьИсключение НСтр("es='En el cliente-del servidor la variante del trabajo debe cumplir la copia de reserva por los medios extraños (los medios СУБД).';ru='В клиент-серверном варианте работы резервное копирование следует выполнять сторонними средствами (средствами СУБД).';en='In the client-server version of the work, backup should be performed using third-party means (DBMS means).'");
	КонецЕсли;
	...
	Иначе
		ТекстЗаголовка = НСтр("es='Por última vez la copia de reserva era pasada: %1';ru='В последний раз резервное копирование проводилось: %1';en='Last backup performed:%1'");
	...
	Если РучнойЗапуск Тогда
		...
		Элементы.Далее.Заголовок = НСтр("es='Conservar la copia de reserva';ru='Сохранить резервную копию';en='Save backup'");
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("es='¿Interrumpir la preparación para la copia de reserva de los datos?';ru='Прервать подготовку к резервному копированию данных?';en='Stop preparing for data backup?'");
...
&НаКлиенте
Процедура ПерейтиНаСтраницуРезервногоКопированияПослеПроверкиДоступаКИнформационнойБазе(РезультатПодключения, 
	Контекст) Экспорт
	...
	СоединенияИБВызовСервера.УстановитьБлокировкуСоединений(НСтр("es='Se cumple la reconstitución de la base informativa.';ru='Выполняется восстановление информационной базы.';en='The information base is being restored.'"), "РезервноеКопирование");
...
Процедура ПерейтиНаСтраницуРезультатовРезервногоКопирования()
	...
	Элементы.Отмена.Заголовок = НСтр("es='Cerrar';ru='Закрыть';en='Close'");
...
#Если ВебКлиент Тогда
	ТекстСообщения = НСтр("es='La creación de la copia de reserva no es accesible en el cliente web.';ru='Создание резервной копии не доступно в веб-клиенте.';en='Creating a backup is not available in the web client.'");
...
#Иначе
	...
	Если ПустаяСтрока(Объект.КаталогСРезервнымиКопиями) Тогда
		...
		ТекстСообщения = НСтр("es='la copia de reserva.';ru='Не выбран каталог для резервной копии.';en='No directory selected for backup.'");
	...
	ИначеЕсли НайтиФайлы(Объект.КаталогСРезервнымиКопиями).Количество() = 0 Тогда
		...
		ТекстСообщения = НСтр("es='Es indicado el catálogo no existente.';ru='Указан несуществующий каталог.';en='A directory that does not exist is specified.'");
		ЗафиксироватьОшибкуПроверкиРеквизитов(ТекстСообщения, "Объект.КаталогСРезервнымиКопиями", ВыдаватьОшибку);
		РеквизитыЗаполнены = Ложь;
	...
	Иначе
		...
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No hay acceso al catálogo con las copias de reserva "
"por la causa: "
"%1';ru='Нет доступа к каталогу с резервными копиями"
"по причине:"
"%1';en='No access to backup directory"
"due to:"
"%1'"),
	...
	Если ТребуетсяВводПароля И ПустаяСтрока(ПарольАдминистратораИБ) Тогда
		...
		ТекстСообщения = НСтр("es='del administrador.';ru='Не задан пароль администратора.';en='Administrator password not set.'");
...
Процедура ИстечениеВремениОжидания()
	...
	ОтключитьОбработчикОжидания("ПроверкаНаЕдинственностьПодключения");
	ТекстВопроса = НСтр("es='No consiguió desconectar a todos los usuarios de la base. ¿Pasar la copia de reserva? (Son posibles las faltas a la archivación)';ru='Не удалось отключить всех пользователей от базы. Провести резервное копирование? (возможны ошибки при архивации)';en='Failed to disconnect all users from the database. Back up? (possible errors during archiving)'");
	ТекстПояснения = НСтр("es='No consiguió desconectar al usuario.';ru='Не удалось отключить пользователя.';en='Failed to disconnect user.'");
...
Процедура ОтменитьПодготовку()
	...
	Элементы.НадписьНеУдалось.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1. "
"La preparación para la copia de reserva es anulada. La base informativa es desbloqueada.';ru='%1."
"Подготовка к резервному копированию отменена. Информационная база разблокирована.';en='%1."
"Backup preparation canceled. The infobase is unlocked.'"),
...
Процедура УстановитьЗаголовокКнопкиДалее(ЭтоКнопкаДалее)
	...
	Элементы.Далее.Заголовок = ?(ЭтоКнопкаДалее, НСтр("es='Más>';ru='Далее >';en='Next >'"), НСтр("es='Es preparado';ru='Готово';en='Done'"));
...
Функция ПолучитьПуть(РежимДиалога)
	...
	Иначе
		ДиалогОткрытияФайла.Заголовок= НСтр("en='selected file';es='Escojan el fichero';ru='Выберите файл'");
	КонецЕсли;	
...
#Если Не ВебКлиент И Не МобильныйКлиент Тогда
	...
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
		...
		"Информация", 
		НСтр("es='Se cumple la copia de reserva de la base informativa:';ru='Выполняется резервное копирование информационной базы:';en='The information base is backed up:'") + " " + ИмяГлавногоФайлаСкрипта);
...
Функция ПроцессВыполняется(Значение = Неопределено)
	...
	Если ПараметрыПриложения[ИмяПараметра] = Неопределено Тогда
		ПараметрыРезервногоКопирования = НастройкиРезервногоКопирования();
		РезервноеКопированиеИБКлиент.ЗаполнитьЗначенияГлобальныхПеременных(ПараметрыРезервногоКопирования);
		ПараметрыПриложения[ИмяПараметра].ПроцессВыполняется = Ложь;
	КонецЕсли;
