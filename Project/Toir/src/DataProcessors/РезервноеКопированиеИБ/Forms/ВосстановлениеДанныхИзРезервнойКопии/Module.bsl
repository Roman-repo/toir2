...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ЭтоВебКлиент()
		...
		ВызватьИсключение НСтр("es='La copia de reserva es inaccesible en el cliente web y el cliente móvil.';ru='Резервное копирование недоступно в веб-клиенте и мобильном клиенте.';en='Backup is not available in the web client and mobile client.'");
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("es='¿Interrumpir la preparación para la reconstitución de los datos?';ru='Прервать подготовку к восстановлению данных?';en='Stop preparing for data recovery?'");
...
Процедура ВыбратьФайлРезервнойКопии()
	...
	ДиалогОткрытияФайла.Фильтр = НСтр("es='La copia de reserva de la base (*.zip, *.1CD) | *.zip; *.1cd';ru='Резервная копия базы (*.zip, *.1CD)|*.zip;*.1cd';en='Database backup (* .zip, * .1CD) | * .zip; *. 1cd'");
	ДиалогОткрытияФайла.Заголовок= НСтр("es='Escojan el fichero de la copia de reserva';ru='Выберите файл резервной копии';en='Select backup file'");
...
#Если ВебКлиент Или МобильныйКлиент Тогда
	ТекстСообщения = НСтр("es='La reconstitución no es accesible en el cliente web y el cliente móvil.';ru='Восстановление не доступно в веб-клиенте и мобильном клиенте.';en='Recovery is not available in the web client and mobile client.'");
...
#Иначе
	...
	Если ПустаяСтрока(ИмяФайла) Тогда
		ТекстСообщения = НСтр("es='la copia de reserva.';ru='Не выбран файл с резервной копией.';en='No backup file selected.'");
	...
	Если ВРег(ФайлАрхива.Расширение) <> ".ZIP" И ВРег(ФайлАрхива.Расширение) <> ".1CD"  Тогда
		...
		ТекстСообщения = НСтр("es='El fichero escogido no es el archivo con la copia de reserva.';ru='Выбранный файл не является архивом с резервной копией.';en='The selected file is not a backup archive.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "Объект.ФайлЗагрузкиРезервнойКопии");
		Возврат Ложь;
	...
	Если ВРег(ФайлАрхива.Расширение) = ".1CD" Тогда
		...
		Если ВРег(ФайлАрхива.ИмяБезРасширения) <> "1CV8" Тогда
			ТекстСообщения = НСтр("es='El fichero escogido no es la copia de reserva (el nombre incorrecto del fichero de la base informativa).';ru='Выбранный файл не является резервной копией (неправильное имя файла информационной базы).';en='The selected file is not a backup (incorrect infobase file name).'");
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "Объект.ФайлЗагрузкиРезервнойКопии");
			Возврат Ложь;
		КонецЕсли;
	...
	Иначе 
		...
		Если ZipФайл.Элементы.Количество() <> 1 Тогда
			...
			ТекстСообщения = НСтр("es='El fichero escogido no es el archivo con la copia de reserva (contiene más de un fichero).';ru='Выбранный файл не является архивом с резервной копией (содержит более одного файла).';en='The selected file is not a backup archive (contains more than one file).'");
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "Объект.ФайлЗагрузкиРезервнойКопии");
			Возврат Ложь;
		...
		Если ВРег(ФайлВАрхиве.Расширение) <> "1CD" Тогда
			...
			ТекстСообщения = НСтр("es='El fichero escogido no es el archivo con la copia de reserva (no contiene el fichero de la base informativa).';ru='Выбранный файл не является архивом с резервной копией (не содержит файл информационной базы).';en='The selected file is not a backup archive (does not contain an infobase file).'");
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "Объект.ФайлЗагрузкиРезервнойКопии");
			Возврат Ложь;
		...
		Если ВРег(ФайлВАрхиве.ИмяБезРасширения) <> "1CV8" Тогда
			...
			ТекстСообщения = НСтр("es='El fichero escogido no es el archivo con la copia de reserva (el nombre incorrecto del fichero de la base informativa).';ru='Выбранный файл не является архивом с резервной копией (неправильное имя файла информационной базы).';en='The selected file is not a backup archive (incorrect infobase file name).'");
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "Объект.ФайлЗагрузкиРезервнойКопии");
			Возврат Ложь;
...
"La preparación para la reconstitución de los datos de la copia de reserva es anulada. La base informativa es desbloqueada.';ru='%1."
"Подготовка к восстановлению данных из резервной копии отменена. Информационная база разблокирована.';en='%1."
"Preparing to restore data from a backup is canceled. The infobase is unlocked.'"),
...
#Если Не ВебКлиент И Не МобильныйКлиент Тогда
	...
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
		...
		"Информация",
		НСтр("es='Se cumple la reconstitución de los datos de la base informativa:';ru='Выполняется восстановление данных информационной базы:';en='The data base information is being restored:'") + " " + ИмяГлавногоФайлаСкрипта);
