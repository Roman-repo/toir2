...
Функция МенеджерОбластиПоискаДублей(Знач ИмяОбластиПоискаДанных) Экспорт
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El tipo desconocido del objeto metadado ""%1""';ru='Неизвестный тип объекта метаданных ""%1""';en=""Unknown metadata object type '%1'"""), ИмяОбластиПоискаДанных);
КонецФункции
	...
	Для Каждого ИмяПоля Из ИменаПолейВВыборе Цикл
		...
		Если ДоступноеПолеКД = Неопределено Тогда
			ЗаписьЖурналаРегистрации(ПоискИУдалениеДублей.НаименованиеПодсистемы(Ложь),
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Campo ""%1"" no existe.';ru='Поле ""%1"" не существует.';en='Field ""%1"" does not exist.'"), Строка(ПолеКД)));
			Продолжить;
		КонецЕсли;
	...
	Пока СледующийЭлементВыборки(ВыборкаЭталонныхОбъектов) Цикл
		...
		Если СтруктураПолейПодобия.Количество() > 0 Тогда
			...
			Если НечеткийПоиск = Неопределено Тогда
				Результат.ОписаниеОшибки = 
					НСтр("es='No consiguió conectar el componente de la búsqueda imprecisa. Más detalladamente cm. En la revista del registro.';ru='Не удалось подключить компоненту нечеткого поиска. Подробнее см. в журнале регистрации.';en='Failed to connect the fuzzy search component. See the logbook for more details.'");
				Возврат Результат;
			КонецЕсли;
			Для Каждого КлючЗначение Из СтруктураПолейПодобия Цикл
				ИмяПоля = КлючЗначение.Ключ;
				...
				ИндексыСтрок = НечеткийПоиск.StringSearch(НРег(СтрокаДляПоиска), НРег(ИскомыеСтроки), "~", 10, 80, 90);
		...
		Если РазмерВозвращаемойПорции > 0 И (ТаблицаДублей.Количество() > РазмерВозвращаемойПорции) Тогда
			...
			Если Найдено > 0 И ТаблицаДублей.Количество() = 0 Тогда
				Результат.ОписаниеОшибки = НСтр("es='Es encontrado demasidas tomas de un elemento.';ru='Найдено слишком много дублей одного элемента.';en='Too many duplicates of one item found.'");
			Иначе
				Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Es encontrado demasidas tomas. Son mostrados solamente primero %1.';ru='Найдено слишком много дублей. Показаны только первые %1.';en='Too many duplicates found. Only the first %1 is shown.'"), РазмерВозвращаемойПорции);
			КонецЕсли;
			Прервать;
		КонецЕсли;
	КонецЦикла;
