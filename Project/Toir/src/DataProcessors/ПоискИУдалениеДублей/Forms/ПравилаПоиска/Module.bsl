...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Las reglas de la búsqueda de las tomas ""%1""';ru='Правила поиска дублей ""%1""';en=""Double search rules '%1'"""), Параметры.ПредставлениеОбластиОтбора);
...
Процедура УчитыватьПрикладныеПравилаПриИзменении(Элемент)
	...
	ТекстЗаголовка = НСтр("es='La prevención';ru='Предупреждение';en='Warning'");
	ТекстВопроса   = НСтр("es='La atención: la búsqueda y la desaparición de las tomas de los elementos sin tomar en cuenta las restricciones suministradas "
"puede llevar a la disonancia de los datos en el programa. "
""
"¿Desconectar el uso de las restricciones suministradas?';ru='Внимание: поиск и удаление дублей элементов без учета поставляемых ограничений"
"может привести к рассогласованию данных в программе."
""
"Отключить использование поставляемых ограничений?';en='Warning: search and removal of duplicate elements without regard to the supplied restrictions"
"may lead to inconsistent data in the program."
""
"Disable use of supplied restrictions?'");
...
Функция ОшибкиВыбора()
	...
	Возврат НСтр("es='Es necesario indicar por lo menos una regla de la búsqueda de las tomas.';ru='Необходимо указать хотя бы одно правило поиска дублей.';en='Required to specify at least one duplicate search rule.'");
КонецФункции
