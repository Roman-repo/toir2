...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РасширениеФайлаВСписке Тогда
		...
		Если Не ЗначениеЗаполнено(Кодировка) Тогда
			Кодировка = НСтр("es='Por reticencia';ru='По умолчанию';en='Default'");
		КонецЕсли;
	Иначе
...
Функция ОбработатьКомандуЗаписиВерсииФайла()
	...
	Если ПустаяСтрока(ЭтотОбъект.Объект.ПолноеНаименование) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='Para la continuación indiquen el nombre de la versión del fichero.';ru='Для продолжения укажите имя версии файла.';en='To continue, specify the file version name.'"), , "Наименование", "Объект");
		Возврат Ложь;
	КонецЕсли;
...
Функция ЗаписатьВерсиюФайла(Знач ПараметрОбъект = Неопределено)
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("es='Los ficheros. La falta de la anotación de la versión del fichero juntado';ru='Файлы.Ошибка записи версии присоединенного файла';en='Files. Error writing version of attached file'", ОбщегоНазначения.КодОсновногоЯзыка()),
