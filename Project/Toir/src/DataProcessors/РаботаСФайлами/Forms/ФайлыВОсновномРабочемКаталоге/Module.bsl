...
Процедура УдалитьИзЛокальногоКэшаФайловВыполнить()
	...
	ТекстВопроса = НСтр("es='¿Quitar los ficheros escogidos del catálogo de trabajo?';ru='Удалить выбранные файлы из рабочего каталога?';en='Delete selected files from working directory?'");
...
Процедура УдалитьИзЛокальногоКэшаФайловВЦикле(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.ЕстьЗанятыеФайлы Тогда
		ПоказатьПредупреждение(,
			НСтр("es='No es posible quitar del catálogo de trabajo los ficheros "
"ocupados para la redacción.';ru='Нельзя удалять из рабочего каталога файлы,"
"занятые для редактирования.';en='Can not delete files from the working directory,"
