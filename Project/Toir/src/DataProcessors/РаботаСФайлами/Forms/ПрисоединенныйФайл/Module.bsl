...
Процедура СтандартныеУстановитьПометкуУдаления(Команда)
	...
	Если Модифицированность Тогда
		Если ЭтотОбъект.Объект.ПометкаУдаления Тогда
			ТекстВопроса = НСтр("es='Para la ejecución de la acción es necesario anotar los cambios del fichero. "
"¿Anotar los cambios y quitar la marca a la desaparición del fichero "
"""%1""?';ru='Для выполнения действия требуется записать изменения файла."
"Записать изменения и снять пометку на удаление с файла"
"""%1""?';en='To complete the action, you need to write the file changes."
"Write changes and unmark the deletion from the file"
"""%1""?'");
		Иначе
			ТекстВопроса = НСтр("es='Para la ejecución de la acción es necesario anotar los cambios del fichero. "
"¿Anotar los cambios y marcar a la desaparición el fichero "
"""%1""?';ru='Для выполнения действия требуется записать изменения файла."
"Записать изменения и пометить на удаление файл"
"""%1""?';en='To complete the action, you need to write the file changes."
"Write changes and mark file for deletion"
"""%1""?'");
		КонецЕсли;
	Иначе
		Если ЭтотОбъект.Объект.ПометкаУдаления Тогда
			ТекстВопроса = НСтр("es='¿Quitar la marca a la desaparición del fichero "
"""%1""?';ru='Снять пометку на удаление с файла"
"""%1""?';en='Unmark file deletion"
"""%1""?'");
		Иначе
			ТекстВопроса = НСтр("es='¿Quitar la marca a la desaparición del fichero "
"""%1""?';ru='Снять пометку на удаление с файла"
...
Процедура СтандартнаяПеречитать(Команда)
	...
	ТекстВопроса = НСтр("es='Los datos son cambiados. ¿Volver a leer los datos?';ru='Данные изменены. Перечитать данные?';en='Data changed. Reread data?'");
...
Процедура УдалитьЭП(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("es='¿Quitar las firmas distinguidas?';ru='Удалить выделенные подписи?';en='Delete selected captions?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ОбновитьЗаголовок()
	...
	Если ЗначениеЗаполнено(ЭтотОбъект.Объект.Ссылка) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='%1 (el fichero Juntado)';ru='%1 (Присоединенный файл)';en='% 1 (Attached file)'"), Строка(ЭтотОбъект.Объект.Ссылка));
	Иначе
		Заголовок = НСтр("es='El fichero (Creación) juntado';ru='Присоединенный файл (Создание)';en='Attached file (Creation)'")
	КонецЕсли;
...
Процедура УстановитьДоступностьКнопок(Форма, Элементы)
	...
	Если Не ДоступнаПечатьСоШтампом Тогда
		...
		Элементы.Печать.Заголовок = НСтр("es='La prensa';ru='Печать';en='Printing'");
	Иначе
		Элементы.ПодменюПечать.Вид = ВидГруппыФормы.Подменю;
		Элементы.Печать.Заголовок = НСтр("es='En seguida a la impresora';ru='Сразу на принтер';en='Direct to printer'");
	КонецЕсли;
...
Функция ОбработатьКомандуЗаписиФайла()
	...
	Если ПустаяСтрока(ЭтотОбъект.Объект.Наименование) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("es='Para la continuación indiquen el nombre del fichero.';ru='Для продолжения укажите имя файла.';en='Please enter a file name to continue.'"), , "Наименование", "Объект");
		Возврат Ложь;
	КонецЕсли;
...
Процедура ОбновитьПояснениеОблачногоСервиса(ПрисоединенныйФайл)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюФайлов") Тогда
		...
		Если СведенияОСинхронизации.Количество() > 0 Тогда
			...
			ЧастиСтроки.Добавить(НСтр("es='El fichero es accesible solamente para la presentación, el trabajo con él tiene lugar en el servicio nuboso';ru='Файл доступен только для просмотра, работа с ним ведется в облачном сервисе';en='File is available for viewing only, work with it is carried out in the cloud service'"));
			...
			ЧастиСтроки.Добавить(НСтр("es='Es sincronizado';ru='Синхронизирован';en='Synchronized'"));
			ЧастиСтроки.Добавить(": ");
