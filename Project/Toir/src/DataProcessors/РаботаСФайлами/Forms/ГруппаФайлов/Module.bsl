...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(ОповещениеОбОтвете, НСтр("es='Los datos eran cambiados. ¿Conservar los cambios?';ru='Данные были изменены. Сохранить изменения?';en='Data has been changed. Save changes?'"), РежимДиалогаВопрос.ДаНетОтмена);
...
Процедура ОбновитьЗаголовок()
	...
	Если ЗначениеЗаполнено(ЭтотОбъект.Объект.Ссылка) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='%1 (el Grupo de los ficheros)';ru='%1 (Группа файлов)';en='%1 (File Group)'"), Строка(ЭтотОбъект.Объект.Ссылка));
	Иначе
		Заголовок = НСтр("es='El grupo de los ficheros (la Creación)';ru='Группа файлов (Создание)';en='File пroup (Creation)'")
	КонецЕсли;
...
Функция ЗаписатьФайл(Знач ПараметрОбъект = Неопределено)
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("es='Los ficheros. La falta de la anotación del grupo de los ficheros juntados';ru='Файлы.Ошибка записи группы присоединенных файлов';en='Files: Error writing a group of attached files'", ОбщегоНазначения.КодОсновногоЯзыка()),
