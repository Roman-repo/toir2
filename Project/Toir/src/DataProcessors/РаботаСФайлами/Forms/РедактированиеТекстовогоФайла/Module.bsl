...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не РежимРедактирования Тогда
		ЗаголовокСтрока = ЗаголовокСтрока + " " + НСтр("es='(Solamente la presentación)';ru='(только просмотр)';en='(view only)'");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(КодировкаТекстаФайла) Тогда
		...
		КонецЕсли;
	Иначе
		КодировкаПредставление = НСтр("es='Por reticencia';ru='По умолчанию';en='Default'");
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Los cambios en el fichero ""%1"" serán perdidos.';ru='Изменения в файле ""%1"" будут потеряны.';en='Changes to the file ""%1"" will be lost.'"), ИмяИРасширение);
		Возврат;
	КонецЕсли;
	...
	ТекстНапоминания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El fichero ""%1"" era cambiado. "
"¿Conservar los cambios?';ru='Файл ""%1"" был изменен."
"Сохранить изменения?';en='The file ""%1"" has been modified."
"Save changes?'"), 
		ИмяИРасширение);
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить("Сохранить", НСтр("es='Conservar';ru='Сохранить';en='Save'"));
	Кнопки.Добавить("НеСохранять", НСтр("es='No conservar';ru='Не сохранять';en='Do not save'"));
	...
	ПараметрыНапоминания.Вставить("Заголовок", НСтр("es='La atención';ru='Внимание';en='Warning! '"));
...
Процедура СохранитьКак(Команда)
	...
	Фильтр = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Todos los ficheros (*. El %1) | *. El %1';ru='Все файлы (*.%1)|*.%1';en='All files (*.% 1) | *.% 1'"), ДанныеФайла.Расширение);
	...
	Если ВыборФайла.Выбрать() Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("es='El fichero es con éxito conservado';ru='Файл успешно сохранен';en='File saved successfully'"), , ВыбранноеПолноеИмяФайла);
...
#Если ВебКлиент Тогда
	ПоказатьПредупреждение(, НСтр("es='La comparación de las versiones de los ficheros en el cliente web es inaccesible.';ru='Сравнение версий файлов в веб-клиенте недоступно.';en='Comparison of file versions in the web client is not available.'"));
	Возврат;
#ИначеЕсли МобильныйКлиент Тогда
	ПоказатьПредупреждение(, НСтр("es='La comparación de las versiones de los ficheros en el cliente móvil es inaccesible.';ru='Сравнение версий файлов в мобильном клиенте недоступно.';en='Comparison of file versions in the mobile client is not available.'"));
	Возврат;
#Иначе
