...
Процедура СохранитьПапку()
	...
	Если ПустаяСтрока(ПапкаДляЭкспорта) Или ПапкаДляЭкспорта = ПолучитьРазделительПути() Тогда
		ПоказатьПредупреждение(, НСтр("es='Es necesario indicar la carpeta.';ru='Необходимо указать папку.';en='Required to specify a folder.'"));
		Возврат;
	КонецЕсли;
	...
	Если Не КаталогВыгрузки.Существует() Тогда
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='La carpeta ""%1"" no es encontrada. "
"Escojan otra carpeta.';ru='Папка ""%1"" не найдена."
"Выберите другую папку.';en='The folder ""%1"" was not found."
"Select a different folder.'"),
	...
	Если Не КаталогВыгрузки.Существует() Тогда
		...
		Попытка
			...
			Если Не КаталогВыгрузки.Существует() Тогда
				ВызватьИсключение НСтр("es='Después de la creación exitosa подпапка no es encontrado.';ru='После успешного создания подпапка не найдена.';en='After successful creation, a subfolder was not found.'");
			КонецЕсли;
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='No consiguió crear подпапку ""%1"" en la carpeta ""%2"" por la causa: "
"%3';ru='Не удалось создать подпапку ""%1"" в папке ""%2"" по причине:"
"%3';en='Failed to create subfolder"" ""%1"" ""in folder ""%2"" due to:"
"%3 '"),
				Строка(ЧтоСохраняем),
				ПапкаДляЭкспорта,
...
Процедура СохранитьПапкуЗавершение(Результат, ПараметрыВыполнения) Экспорт
	Если Результат.Успех = Истина Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("es='La exportación de la carpeta';ru='Экспорт папки';en='Export folder'"),,
		             СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		               НСтр("es='Es con éxito acabada la exportación de la carpeta ""%1"" "
"en la carpeta sobre disco ""%2"".';ru='Успешно завершен экспорт папки ""%1"""
"в папку на диске ""%2"".';en='Export of folder ""%1"" completed successfully"
"to the folder on disk ""%2"".'"),
...
Процедура ОбойтиДеревоФайлов4(ПараметрыВыполнения)
	...
	Исключение
		ТекстОшибки = НСтр("es='La falta de la creación de la carpeta ""%1"":';ru='Ошибка создания папки ""%1"":';en='Error creating folder ""%1"":'");
...
Процедура ОбойтиДеревоФайлов9(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.ФайлНаДиске.Существует() И ПараметрыВыполнения.ФайлНаДиске.ЭтоКаталог() Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='En vez del fichero "
"""%1"" "
"hay una carpeta con el mismo nombre. "
""
"¿Repetir la exportación de este fichero?';ru='Вместо файла"
"""%1"""
"существует папка с таким же именем."
""
"Повторить экспорт этого файла?';en='Instead of file"
"""%1"""
"there is a folder with the same name."
""
"Retry exporting this file?'"),
		...
		Иначе
			Если Не ПараметрыВыполнения.ОбщиеПараметры.ДляВсехФайлов Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='En la carpeta ""%1"" "
"hay un fichero de ""%2"" "
"dimensión del fichero existente = %3 bytes, la fecha de cambio %4. "
"La dimensión del fichero conservado = %5 bytes, la fecha de cambio %6. "
""
"¿Sustituir el fichero existente por el fichero del depósito de los ficheros?';ru='В папке ""%1"""
"существует файл ""%2"""
"размер существующего файла = %3 байт, дата изменения %4."
"размер сохраняемого файла = %5 байт, дата изменения %6."
""
"Заменить существующий файл файлом из хранилища файлов?';en='In folder"" ""% 1"" """
"file exists """"% 2 """""
"existing file size =% 3 bytes, date modified% 4."
"size of saved file =% 5 bytes, date modified% 6."
""
"Replace existing file with file from file vault? ""'"),
					ПараметрыВыполнения.БазовыйКаталогСохраненияФайла,
...
Процедура ОбойтиДеревоФайлов15(ИнформацияОбОшибке, ПараметрыВыполнения)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='La falta del registro del fichero "
"""%1"". "
""
"%2.';ru='Ошибка записи файла"
"""%1""."
""
"%2.';en='Error writing file"
"""%1""."
""
"%2.'"),
		ПараметрыВыполнения.ПолноеИмяФайла,
