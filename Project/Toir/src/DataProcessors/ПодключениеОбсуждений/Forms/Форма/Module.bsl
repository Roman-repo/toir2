...
Процедура ПриПолученииКодаРегистрации()
	...
	Если ПустаяСтрока(АдресЭлектроннойПочты) Тогда 
		ПоказатьПредупреждение(, НСтр("es='La dirección del correo electrónico no es rellenada';ru='Адрес электронной почты не заполнен';en='Email address is blank'"));
		Возврат;
	КонецЕсли;
	
	Если Не ОбщегоНазначенияКлиентСервер.АдресЭлектроннойПочтыСоответствуетТребованиям(АдресЭлектроннойПочты) Тогда 
		ПоказатьПредупреждение(, НСтр("es='La dirección del correo electrónico contiene las faltas';ru='Адрес электронной почты содержит ошибки';en='Email address contains errors'"));
		Возврат;
	КонецЕсли;
...
Процедура ПриРегистрации()
	...
	Если ПустаяСтрока(КодРегистрации) Тогда 
		ПоказатьПредупреждение(, НСтр("es='El código del registro no es rellenado';ru='Код регистрации не заполнен';en='Registration code is not filled'"));
		Возврат;
	КонецЕсли;
...
Процедура ПриОтказеОтВводаКодаПодтверждения()
	...
	ПоказатьВопрос(Оповещение, 
		НСтр("es='A la renuncia de la introducción el código, mandado al correo electrónico, será no válida. "
"La continuación será posible solamente con la interpelación del nuevo código.';ru='При отказе от ввода высланный на электронную почту код станет недействительным."
"Продолжение будет возможно только с запросом нового кода.';en='If you refuse to enter the code sent by e-mail will become invalid."
"Continuation will only be possible with a request for a new code. '"), 
...
Процедура ОбновитьВидимостьКнопокКоманднойПанели(КонтейнерСостояний,
	...
	ИначеЕсли СостояниеРегистрации = "ТребуетсяРазблокировать" Тогда 
		...
		Зарегистрироваться.Заголовок = НСтр("es='Restablecer la conexión';ru='Восстановить подключение';en='Reconnect'");
		Назад.Видимость = Ложь;
	ИначеЕсли СостояниеРегистрации = "НеЗарегистрирована" Тогда 
		Зарегистрироваться.Видимость = Истина;
		Назад.Видимость = Ложь;
	...
	ИначеЕсли СостояниеРегистрации = "Зарегистрирована" Тогда 
		...
		Закрыть.Заголовок = НСтр("es='Es preparado';ru='Готово';en='Done'");
	КонецЕсли;
