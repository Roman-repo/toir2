...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.СписокКонтактнойИнформации) = Тип("Массив") Тогда
		...
		КонецЦикла;
		ЭтотОбъект.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La historia de cambios (%1)';ru='История изменений (%1)';en='Change history (%1)'"), ПредставлениеКонтактнойИнформации);
...
Процедура ИсторияПередУдалением(Элемент, Отказ)
	...
	Если НЕ Отказ Тогда
		...
		ПоказатьВопрос(Оповещение, НСтр("es='Quitar la dirección que funciona con';ru='Удалить адрес действующий с';en='Delete address valid from'") + " " + Формат(ДействуетС, "ДЛФ=DD")+ "?", РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
	Отказ = Истина;
КонецПроцедуры
...
Процедура СформироватьДанные(ВводНовогоАдреса = Ложь)
	...
	Для Индекс = 0 По История.Количество() - 1 Цикл
		...
		Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("es='No se permite la introducción de las direcciones con las fechas iguales.';ru='Не допускается ввод адресов с одинаковыми датами.';en='Entering addresses with the same dates is not allowed.'"),, "История[" + Строка(Индекс) + "].ДействуетС");
			Возврат;
		КонецЕсли;
	...
	Если НЕ ВводНовогоАдреса И ОтсутствуетНачальнаяДата Тогда
		ПоказатьПредупреждение(, НСтр("es='Falta la dirección que funciona de la fecha del comienzo de la gestión del recuento.';ru='Отсутствует адрес, действующий с даты начала ведения учета.';en='There is no address valid from the start of accounting.'"));
		Возврат;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("es='El significado inicial';ru='Начальное значение';en='Initial value'"));
