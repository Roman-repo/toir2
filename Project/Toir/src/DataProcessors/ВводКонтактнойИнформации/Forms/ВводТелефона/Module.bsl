...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Представление", "ПодсказкаВвода", НСтр("es='La representación';ru='Представление';en='Performance'"));
...
Процедура КодГородаПриИзменении(Элемент)
	...
	Если (КодСтраны = "+7" ИЛИ КодСтраны = "8") И СтрНачинаетсяС(КодГорода, "9") И СтрДлина(КодГорода) <> 3 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("es='Los códigos de teléfonos móviles que comienzan a la cifra 9 tienen la longitud fijada de 3 cifras, por ejemplo - 916.';ru='Коды мобильных телефонов начинающиеся на цифру 9 имеют фиксированную длину в 3 цифры, например - 916.';en='Mobile phone codes starting with the number 9 have a fixed length of 3 digits, for example - 916.'"),, "КодГорода");
	КонецЕсли;
...
Функция РезультатВыбора()
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("Обработка.ВводКонтактнойИнформации.Форма.ВводТелефона", "КодыСтраныИГорода", Коды, НСтр("es='Los códigos del país y la ciudad';ru='Коды страны и города';en='Country and city codes'"));
...
Функция ОшибкиЗаполненияТелефона()
	...
	Если СтрДлина(НомерТелефонаТолькоЦифры) > 15 Тогда
		СписокОшибок.Добавить("НомерТелефона", НСтр("es='El número de teléfono demasiado largo';ru='Номер телефона слишком длинный';en='Phone number is too long'"));
	КонецЕсли;
	
	Если НомерТелефонаСодержитНедопустимыеСимволы(ПолныйНомерТелефона) Тогда
		СписокОшибок.Добавить("НомерТелефона", НСтр("es='El número de teléfono contiene los símbolos inadmisibles';ru='Номер телефона содержит недопустимые символы';en='Phone number contains invalid characters'"));
	КонецЕсли;
	
	Если КодСтраны = "7" ИЛИ КодСтраны = "+7" Тогда
		Если СтрДлина(ТолькоЦифры(НомерТелефона)) > 7 Тогда
			СписокОшибок.Добавить("НомерТелефона", НСтр("es='En Rusia el número de teléfono no puede ser más de 7 cifras';ru='В России номер телефона не может быть больше 7 цифр';en='In Russia, the phone number cannot be more than 7 digits'"));
		КонецЕсли;
	КонецЕсли;
	
	Если СтрНачинаетсяС(КодГорода, "9") И СтрДлина(КодГорода) <> 3 Тогда
		СписокОшибок.Добавить("НомерТелефона", НСтр("es='En Rusia el número de teléfonos móviles deben contener 3 cifras';ru='В России номера мобильных телефонов должны содержать 3 цифры';en='In Russia, mobile phone numbers must contain 3 digits'"));
	КонецЕсли;
...
Процедура СообщитьОбОшибкахЗаполнения(СписокОшибок)
	...
	Если СписокОшибок.Количество()=0 Тогда
		ПоказатьПредупреждение(, НСтр("es='Teléfono es introducido correctamente.';ru='Телефон введен корректно.';en='Phone entered correctly.'"));
		Возврат;
	КонецЕсли;
