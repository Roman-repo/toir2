...
Процедура УстановитьКорневуюПапкуФайловДокументооборота(Элемент)
	...
	ПараметрыФормы.Вставить("Заголовок", 		НСтр("es='La elección de la carpeta de los ficheros';ru='Выбор папки файлов';en='File folder selection'"));
...
Процедура ОбновитьНастройкиОбновленияСвязанныхРеквизитов(Расписание = Неопределено)
	...
	СтрокаСсылки = Новый ФорматированнаяСтрока(НСтр("es='Cambiar el horario';ru='Изменить расписание';en='Change schedule'"),,,, "ИзменитьРасписание");
	МассивСтрок.Добавить(СтрокаСсылки);
	
	МассивСтрок.Добавить("  ");
	
	СтрокаСсылки = Новый ФорматированнаяСтрока(НСтр("es='Introducir el nombre del usuario y la consigna';ru='Ввести имя пользователя и пароль';en='Enter username and password'"),,,, "ВвестиИмяПользователяИПароль");
	МассивСтрок.Добавить(СтрокаСсылки);
