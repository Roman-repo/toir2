...
Процедура НастроитьВариантыОтчета(Настройки, НастройкиОтчета) Экспорт
	...
	НастройкиВарианта.Наименование = НСтр("es='Los recursos exteriores usados por el programa y los módulos adicionales';ru='Внешние ресурсы, используемые программой и дополнительными модулями';en='External resources used by the program and additional modules'");
	НастройкиВарианта.Описание = 
		НСтр("es='Los recursos de Internet, los componentes exteriores, las COM-clases etc. "
"Los parámetros del ambiente, que ayudarán al administrador "
"cumplir el ajuste del ordenador y pasar la auditoría de la seguridad.';ru='Интернет-ресурсы, внешние компоненты, COM-классы и прочее."
"Параметры окружения, которые помогут администратору"
"выполнить настройку компьютера и провести аудит безопасности.';en='Internet resources, external components, COM classes and more."
"Environment settings that will help the administrator"
"set up your computer and conduct a security audit. '");
	НастройкиВарианта.НастройкиДляПоиска.НаименованияПолей = 
		НСтр("es='El nombre y el identificador de la COM-clase "
"el Nombre del ordenador "
"la Dirección "
"la Lectura dado "
"al Registro de datos "
"el Nombre de la maqueta o el fichero de los componente "
"la suma De control "
"Modelo de la línea de mando "
"la Acta "
"la Dirección del Recurso De Internet "
"el Puerto';ru='Имя и идентификатор COM-класса"
"Имя компьютера"
"Адрес"
"Чтение данных"
"Запись данных"
...
"Порт';en='COM class name and identifier"
"Computer name"
"Address"
"Reading data"
"Data recording"
"Component Layout or File name"
"Check sum"
...
Функция ПредставлениеЗапросовРазрешенийНаИспользованиеВнешнихРесурсов(Знач ОперацииАдминистрирования, Знач ОписаниеДобавляемыхРазрешений, Знач ОписаниеУдаляемыхРазрешений, Знач КакТребуемые = Ложь) Экспорт
	...
	Для Каждого СтрокаТаблицыМодулей Из ТаблицаМодулей Цикл
		...
		Если Добавляемые.Количество() > 0 Тогда
			...
			Если КакТребуемые Тогда
				...
				ТекстШапки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Para %1 ""%2"" es necesario el uso de los recursos siguientes exteriores:';ru='Для %1 ""%2"" требуется использование следующих внешних ресурсов:';en='For % 1 ""% 2""  the use of the following external resources is required:'"),
			...
			Иначе
				...
				ТекстШапки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Para %1 ""%2"" serán concedidos los permisos siguientes para el uso de los recursos exteriores:';ru='Для %1 ""%2"" будут предоставлены следующие разрешения на использование внешних ресурсов:';en='For% 1 ""% 2"", the following permissions to use external resources will be granted:'"),
					НРег(Словарь.Родительный),
					НаименованиеМодуля);
		...
		Если Удаляемые.Количество() > 0 Тогда
			...
			Если КакТребуемые Тогда
				ВызватьИсключение НСтр("es='La interpelación incorrecta de los permisos';ru='Некорректный запрос разрешений';en='Invalid permission request'");
			КонецЕсли;
			
			ТекстШапки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Serán quitados los siguientes antes concedido para %1 ""%2"" permiso para el uso de los recursos exteriores:';ru='Будут удалены следующие ранее предоставленные для %1 ""%2"" разрешения на использование внешних ресурсов:';en='The following permissions to use external resources previously granted to% 1 ""% 2"" will be deleted:'"),
					НРег(Словарь.Родительный),
					НаименованиеМодуля);
...
Процедура СформироватьПредставлениеОпераций(ТабличныйДокумент, Знач Макет, Знач ОперацииАдминистрирования)
	...
	Для Каждого Описание Из ОперацииАдминистрирования Цикл
		...
		Если Описание.Операция = Перечисления.ОперацииАдминистрированияПрофилейБезопасности.Удаление Тогда
			...
			ТекстШапки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Será quitado el perfil de la seguridad para %1 ""%2"".';ru='Будет удален профиль безопасности для %1 ""%2"".';en='The security profile for% 1 ""% 2"" will be deleted.'"),
					НРег(Словарь.Родительный),
					НаименованиеМодуля);
					...
					Иначе
						...
						Если ТипРазрешения = "FileSystemAccess" Тогда
							...
							Если Разрешение.Path = "/temp" Тогда
								ДополнениеРазрешения.Вставить("Path", НСтр("es='El catálogo de los ficheros temporales';ru='Каталог временных файлов';en='Temporary files directory'"));
							КонецЕсли;
							
							Если Разрешение.Path = "/bin" Тогда
								ДополнениеРазрешения.Вставить("Path", НСтр("es='El catálogo, en que es establecido el servidor 1С:Предприятия';ru='Каталог, в который установлен сервер 1С:Предприятия';en='The directory in which the 1C: Enterprise server is installed'"));
							КонецЕсли;
...
Функция СловарьМодуляКонфигурации()
	...
	Результат.Вставить("Именительный", НСтр("es='El programa';ru='Программа';en='Program'"));
	Результат.Вставить("Родительный", НСтр("es='Los programas';ru='Программы';en='Programs'"));
