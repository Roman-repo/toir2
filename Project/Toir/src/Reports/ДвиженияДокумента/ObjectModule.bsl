...
Процедура ПередЗагрузкойНастроекВКомпоновщик(Контекст, КлючСхемы, КлючВарианта, Настройки, ПользовательскиеНастройки) Экспорт
	...
	Если ПараметрыОтчета.ДокументВладелец = Неопределено Тогда
		ВызватьИсключение НСтр("es='El informe de los movimientos del documento no es destinado al envío.';ru='Отчет о движениях документа не предназначен для рассылки.';en='The document movement report is not intended for distribution.'");
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Форма.Параметры.ПредставлениеВарианта = "Расшифровка" Тогда
		ВызватьИсключение НСтр("es='La acción escogida en el informe dado no en una forma accesible.';ru='Выбранное действие в данном отчете не доступно.';en='The selected action is not available in this report.'");
	КонецЕсли;
	...
	ИначеЕсли Не Форма.Параметры.Свойство("ПараметрКоманды") Тогда
		ВызватьИсключение НСтр("es='Para el comienzo del trabajo con el informe es necesario aprovecharse de la orden correspondiente en forma del documento que interesa.';ru='Для начала работы с отчетом необходимо воспользоваться соответствующей командой в форме интересующего документа.';en='To start working with the report, you must use the appropriate command in the form of the document of interest.'");
	КонецЕсли;
...
Функция НаборДанныхДляРегистраБухгалтерии(СвойстваРегистра, МетаданныеРегистра)
	...
	ЛокализацияДебет    = НСтр("es='Дт';ru='Дт';en='Dt'");
	ЛокализацияКредит   = НСтр("es='Кт';ru='Кт';en='Kt'");
	ЛокализацияСубконто = НСтр("es='Субконто';ru='Субконто';en='Subconto'");
	ЛокализацияСчет     = НСтр("es='La cuenta';ru='Счет';en='Account'");
...
Процедура ПодготовитьГоризонтальныйВариантРегистровНакопленияИСведений_Горизонталь(НастройкиКД, НаборДанныхКД, ЭлементДанных)
	...
	ПользовательскийЗаголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El registro %1 %2';ru='Регистр %1 %2';en='Register%1%2'"),
	...
	Если СтруктураСтандартныхРеквизитов.Количество() > 0 Тогда
		...
		ПараметрыГруппы.Вставить("ПредставлениеВыбраннойГруппы", НСтр("es='Los requisitos estandartizados';ru='Стандартные реквизиты';en='Standard details'"));
	...
	Если СтруктураИзмерений.Количество() > 0 Тогда
		...
		ПараметрыГруппы.Вставить("РасположениеВыбраннойГруппы",  Неопределено);
		ПараметрыГруппы.Вставить("ВыбраннаяГруппаПуста",         Ложь);
		ПараметрыГруппы.Вставить("ПредставлениеВыбраннойГруппы", НСтр("es='Las medidas';ru='Измерения';en='Measurements'"));
	...
	Если СтруктураРесурсов.Количество() > 0 Тогда
		...
		ПараметрыГруппы.Вставить("РасположениеВыбраннойГруппы",  Неопределено);
		ПараметрыГруппы.Вставить("ВыбраннаяГруппаПуста",         Ложь);
		ПараметрыГруппы.Вставить("ПредставлениеВыбраннойГруппы", НСтр("es='Los recursos';ru='Ресурсы';en='Resources'"));
	...
	Если СтруктураРеквизитов.Количество() > 0 Тогда
		...
		ПараметрыГруппы.Вставить("РасположениеВыбраннойГруппы",  Неопределено);
		ПараметрыГруппы.Вставить("ВыбраннаяГруппаПуста",         Ложь);
		ПараметрыГруппы.Вставить("ПредставлениеВыбраннойГруппы", НСтр("es='Los requisitos';ru='Реквизиты';en='Props'"));
...
Процедура ПодготовитьГоризонтальныйВариантРегистровРасчета(НастройкиКД, НаборДанныхКД, ЭлементДанных)
	...
	ВидРегистра            = "Расчета";
	ПредставлениеВидаРегистра = НСтр("es='Del cálculo';ru='Расчета';en='Calculation'");
	ИмяРегистра            = ЭлементДанных.ИмяРегистра;
	ПредставлениеРегистра  = ЭлементДанных.ПредставлениеРегистра;
...
Процедура ПодготовитьВертикальныйВариантРегистровНакопленияИСведений(НастройкиКД, НаборДанныхКД, ЭлементДанных)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El registro %1 %2';ru='Регистр %1 %2';en='Register%1%2'"),
		НРег(ПредставлениеВидаРегистра), ПредставлениеРегистра);
...
Процедура ПодготовитьГоризонтальныйВариантРегистровБухгалтерии(НастройкиКД, НаборДанныхКД, ЭлементДанных)
	...
	ПредставлениеВидаРегистра = НСтр("es='Las contabilidades';ru='Бухгалтерии';en='Bookkeeping'");
	ИмяРегистра            = ЭлементДанных.ИмяРегистра;
	ПредставлениеРегистра  = ЭлементДанных.ПредставлениеРегистра;
	...
	Для НомерСтроки = 1 По ДокументРезультат.ВысотаТаблицы Цикл
		...
		Если ГраницаПримечаний = ЭтажностьСтроки Тогда
			...
			ТекстПримечания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='En esta línea: %1';ru='В этой строке: %1';en='In this line:%1'"), СвойстваПримечания.Примечание + ".");
...
Процедура ПолучитьСвойстваРегистров(Контекст, ДокументВладелец = Неопределено)
	...
	Для Каждого Движение Из ДвиженияДокумента Цикл
		...
		Если ОбщегоНазначения.ЭтоРегистрНакопления(МетаданныеРегистра) Тогда
			...
			СвойстваРегистра.ПредставлениеВидаРегистра = НСтр("es='Las acumulaciones';ru='Накопления';en='Accumulations'");
		...
		ИначеЕсли ОбщегоНазначения.ЭтоРегистрСведений(МетаданныеРегистра) Тогда
			...
			СвойстваРегистра.ПредставлениеВидаРегистра = НСтр("es='De las noticias';ru='Сведений';en='Information'");
		...
		ИначеЕсли ОбщегоНазначения.ЭтоРегистрБухгалтерии(МетаданныеРегистра) Тогда
			...
			СвойстваРегистра.ПредставлениеВидаРегистра = НСтр("es='Las contabilidades';ru='Бухгалтерии';en='Bookkeeping'");
		...
		ИначеЕсли ОбщегоНазначения.ЭтоРегистрРасчета(МетаданныеРегистра) Тогда
			...
			СвойстваРегистра.ПредставлениеВидаРегистра = НСтр("es='Del cálculo';ru='Расчета';en='Calculation'");
		...
		ЗаголовокТаблицы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='Registro %1 ""%2""';ru='Регистр %1 ""%2""';en='Register%1 ""%2""'"), СокрЛП(СвойстваРегистра.ПредставлениеВидаРегистра), СвойстваРегистра.ПредставлениеРегистра);
	...
	Для Каждого СвойстваРегистра Из СвойстваРегистров Цикл 
		...
		Для Индекс = 0 По ЭтажностьСтроки - 1 Цикл
			...
			Для Каждого ВыбраннаяГруппа Из ВыбранныеГруппыПолей Цикл
				...
				Заголовок = "";
				Если ВыбраннаяГруппа.Заголовок = НСтр("es='Los requisitos estandartizados';ru='Стандартные реквизиты';en='Standard details'") Тогда
					Заголовок = НСтр("es='El requisito estandartizado';ru='Стандартный реквизит';en='Standard props'");
				ИначеЕсли ВыбраннаяГруппа.Заголовок = НСтр("es='Las medidas';ru='Измерения';en='Measurements'") Тогда
					Заголовок = НСтр("es='La medida';ru='Измерение';en='Measurement'");
				ИначеЕсли ВыбраннаяГруппа.Заголовок = НСтр("es='Los recursos';ru='Ресурсы';en='Resources'") Тогда
					Заголовок = НСтр("es='El recurso';ru='Ресурс';en='Resource'");
				ИначеЕсли ВыбраннаяГруппа.Заголовок = НСтр("es='Los requisitos';ru='Реквизиты';en='Props'") Тогда
					Заголовок = НСтр("es='El requisito';ru='Реквизит';en='Props'");
				ИначеЕсли ВыбраннаяГруппа.Заголовок = НСтр("es='Los datos del cálculo';ru='Данные расчета';en='Calculation data'") Тогда
					Заголовок = НСтр("es='Los datos del cálculo';ru='Данные расчета';en='Calculation data'");
				КонецЕсли;
...
Процедура ДоопределитьМакетСКД(СхемаКД, ИндексНабора, ВидРегистра, ПредставлениеВидаРегистра, ИмяРегистра)
	...
	ЗаголовокГруппировки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='Registro %1 ""%2""';ru='Регистр %1 ""%2""';en='Register%1 ""%2""'"),
		НРег(ПредставлениеВидаРегистра),ПредставлениеРегистра);
...
Функция СписокРегистров()
	...
	Для Каждого СвойстваРегистра Из СвойстваРегистров Цикл
		...
		ПредставлениеЭлемента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El registro %1 %2';ru='Регистр %1 %2';en='Register%1%2'"),
...
Процедура ВосстановитьОтборПоГруппировкамРегистров(Настройки)
	...
	Для Каждого ЭлементСтруктуры Из СтруктураОтчета Цикл
		...
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
			...
			НСтр("es='La selección de servicio';ru='Служебный отбор';en='Service selection'"),
			Истина,
