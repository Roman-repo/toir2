...
Процедура НастроитьВариантыОтчета(Настройки, НастройкиОтчета) Экспорт
	...
	НастройкиВарианта.Описание = 
		НСтр("es='Permite cumplir el monitoring de la actividad de los usuarios "
"en el programa (por cuanto es intenso y con que objetos trabajan los usuarios).';ru='Позволяет выполнять мониторинг активности пользователей"
"в программе (насколько интенсивно и с какими объектами работают пользователи).';en='Allows you to monitor user activity"
"in the program (how intensively and with what objects the users work).'");
	
	НастройкиВарианта = МодульВариантыОтчетов.ОписаниеВарианта(Настройки, НастройкиОтчета, "АктивностьПользователя");
	НастройкиВарианта.Описание = 
		НСтр("es='La información detallada sobre aquel, "
"con que objetos trabajaba el usuario en el programa.';ru='Подробная информация о том,"
"с какими объектами работал пользователь в программе.';en='Details on"
"what objects the user worked with in the program.'");
	
	НастройкиВарианта = МодульВариантыОтчетов.ОписаниеВарианта(Настройки, НастройкиОтчета, "КонтрольЖурналаРегистрации");
	НастройкиВарианта.Описание = НСтр("es='La lista de las anotaciones críticas de la revista del registro.';ru='Список критичных записей журнала регистрации.';en='List of critical log entries.'");
	...
	НастройкиВарианта.Описание = НСтр("es='Saca el gráfico de la ejecución de las tareas de reglamento en el programa.';ru='Выводит график выполнения регламентных заданий в программе.';en='Displays a schedule of scheduled tasks in the program.'");
...
Функция СтрокаНеделяГода(ДатаГода)
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La semana %1';ru='Неделя %1';en='Week%1'"), НеделяГода(ДатаГода));
КонецФункции
	...
	Для Каждого Элемент Из МаксМассивПользователей Цикл
		...
		Для Каждого ИмяПользователя Из ВременныйМассив Цикл
			Если ИмяПользователя = Элемент Тогда
				...
				ПользовательИНомер = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 (%2)';ru='%1 (%2)';en='%1(%2)'"),
					Элемент,
...
Функция СформироватьОтчетПоПродолжительностиРаботыРегламентныхЗаданий(ПараметрыЗаполнения) Экспорт
	...
	Если ВыводитьОтбор.Значение = ТипВыводаТекстаКомпоновкиДанных.Выводить
		...
		Если МинимальнаяПродолжительностьСеансовРегламентныхЗаданий > 0 Тогда
			РежимОтображенияИнтервалов = НСтр("es='Es desconectada la representación de los intervalos con la duración de cero';ru='Отключено отображение интервалов с нулевой продолжительностью';en='Disabled zero interval display'");
		Иначе
			РежимОтображенияИнтервалов = НСтр("es='Es desconectada la representación de los intervalos con la duración de cero';ru='Отключено отображение интервалов с нулевой продолжительностью';en='Disabled zero interval display'");
		КонецЕсли;
		...
		Для Каждого СтрокаОдновременноСессий Из ОдновременноСессий Цикл
			...
			Для Каждого Элемент Из МассивРегламентныхЗаданий Цикл
				Если Не ТипЗнч(Элемент) = Тип("Число")
					...
					Область.Параметры.СписокРегламентныхЗаданий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 (la sesión %2)';ru='%1 (сеанс %2)';en='% 1 (session% 2)'"),
						Элемент,
		...
		Если ДлительностьИнтервалаРегламентногоЗадания >= МинимальнаяПродолжительностьСеансовРегламентныхЗаданий Тогда
			Если ТекущееСобытие <> СтрокаРегламентныеЗадания.НазваниеСобытия Тогда
				Если ТекущееСобытие <> Неопределено
					...
					ТочкаНазвание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 (%2 de %3)';ru='%1 (%2 из %3)';en='% 1 (% 2 of% 3)'"),
						Точка.Значение,
				...
				ИначеЕсли Не ЗначениеЗаполнено(СтрокаРегламентныеЗадания.СобытиеМетаданные) Тогда
					ТочкаНазвание = НСтр("es='Las tareas de fondo';ru='Фоновые задания';en='Background tasks'");
					Точка = ДиаграммаГанта.УстановитьТочку(ТочкаНазвание);
					ОбщаяПродолжительностьРЗ = 0;
				КонецЕсли;
			КонецЕсли;
			...
			Интервал.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 - %2';ru='%1 - %2';en='%1-%2'"),
	...
	Для Каждого СтрокаДанныхЖурналаРегистрации Из ДанныеЖурналаРегистрации Цикл 
		...
		Если ТекущаяДата <> ДатаОдновременноРегламентныхЗаданий Тогда
			Если СтрокаТаблицы <> Неопределено Тогда
				...
				СтрокаТаблицы.ДатаОдновременноРегламентныхЗаданий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 - %2';ru='%1 - %2';en='%1-%2'"),
...
Функция РасшифровкаРегламентногоЗадания(Расшифровка) Экспорт
	...
	Если Расшифровка.Получить(8) = 0 Тогда
		РежимОтображенияИнтервалов = НСтр("es='Es incluida la representación de los intervalos con la duración de cero';ru='Включено отображение интервалов с нулевой продолжительностью';en='Zero duration interval display enabled'");
	Иначе
		РежимОтображенияИнтервалов = НСтр("es='Es incluida la representación de los intervalos con la duración de cero';ru='Включено отображение интервалов с нулевой продолжительностью';en='Zero duration interval display enabled'");
	КонецЕсли;
	...
	Область = Макет.ПолучитьОбласть("Таблица");
	Область.Параметры.ТипЗадания = НСтр("es='De reglamento';ru='Регламентное';en='Regulatory'");
...
Функция ПродолжительностьРегламентногоЗадания(ПродолжительностьРЗ)
	...
	ИначеЕсли ПродолжительностьРЗ <= 60 Тогда
		ОбщаяПродолжительностьРЗ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 сек';ru='%1 сек';en='% 1 sec'"), ПродолжительностьРЗ);
	ИначеЕсли 60 < ПродолжительностьРЗ <= 3600 Тогда
		...
		ОбщаяПродолжительностьРЗ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 minas %2 сек';ru='%1 мин %2 сек';en='% 1 min% 2 sec'"), ПродолжительностьМинуты, ПродолжительностьСекунды);
	ИначеЕсли ПродолжительностьРЗ > 3600 Тогда
		...
		ОбщаяПродолжительностьРЗ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='%1 h de %2 minas';ru='%1 ч %2 мин';en='% 1 h% 2 min'"), ПродолжительностьЧасы, ПродолжительностьМинуты);
	КонецЕсли;
