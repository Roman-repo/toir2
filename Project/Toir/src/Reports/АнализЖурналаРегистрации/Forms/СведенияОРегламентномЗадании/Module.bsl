...
Процедура НастроитьРасписаниеРегламентногоЗадания(Команда)
	...
	Иначе
		ПоказатьПредупреждение(,НСтр("es='Es imposible recibir el horario de la tarea de reglamento: la tarea de reglamento era quitada o no es indicado su nombre.';ru='Невозможно получить расписание регламентного задания: регламентное задание было удалено или не указано его наименование.';en='Unable to get the schedule of the scheduled task: the scheduled task was deleted or its name is not indicated.'"));
	КонецЕсли;
...
Процедура ПерейтиКЖурналуРегистрации(Команда)
	...
	Для Каждого Область Из Отчет.ВыделенныеОбласти Цикл
		...
		Если Расшифровка = Неопределено
			...
			ПоказатьПредупреждение(,НСтр("es='Escojan la línea o la célula de la sesión necesaria de la tarea';ru='Выберите строку или ячейку нужного сеанса задания';en='Select the row or cell of the desired job session'"));
			Возврат;
		КонецЕсли;
		ДатаНачала = Расшифровка.Получить(0);
		ДатаОкончания = Расшифровка.Получить(1);
