...
Процедура НастроитьВариантыОтчета(Настройки, НастройкиОтчета) Экспорт
	...
	НастройкиВарианта.Описание = НСтр("es='El informe universal de las guías, los documentos, los registros.';ru='Универсальный отчет по справочникам, документам, регистрам.';en='Universal report on directories, documents, registers.'");
...
Функция ДоступныеТипыОбъектовМетаданных()
	...
	Если ЕстьОбъектыТипаМетаданных(Метаданные.Справочники) Тогда
		ДоступныеЗначения.Добавить("Справочники", НСтр("es='La guía';ru='Справочник';en='Directory'"), , БиблиотекаКартинок.Справочник);
	КонецЕсли;
	
	Если ЕстьОбъектыТипаМетаданных(Метаданные.Документы) Тогда
		ДоступныеЗначения.Добавить("Документы", НСтр("es='El documento';ru='Документ';en='Document'"), , БиблиотекаКартинок.Документ);
	КонецЕсли;
	
	Если ЕстьОбъектыТипаМетаданных(Метаданные.РегистрыСведений) Тогда
		ДоступныеЗначения.Добавить("РегистрыСведений", НСтр("es='El registro de las noticias';ru='Регистр сведений';en='Information register'"), , БиблиотекаКартинок.РегистрСведений);
	КонецЕсли;
	
	Если ЕстьОбъектыТипаМетаданных(Метаданные.РегистрыНакопления) Тогда
		ДоступныеЗначения.Добавить("РегистрыНакопления", НСтр("es='El registro de la acumulación';ru='Регистр накопления';en='Accumulation register'"), , БиблиотекаКартинок.РегистрНакопления);
	КонецЕсли;
	
	Если ЕстьОбъектыТипаМетаданных(Метаданные.РегистрыБухгалтерии) Тогда
		ДоступныеЗначения.Добавить("РегистрыБухгалтерии", НСтр("es='El registro de la contabilidad';ru='Регистр бухгалтерии';en='Accounting register'"), , БиблиотекаКартинок.РегистрБухгалтерии);
	КонецЕсли;
	
	Если ЕстьОбъектыТипаМетаданных(Метаданные.РегистрыРасчета) Тогда
		ДоступныеЗначения.Добавить("РегистрыРасчета", НСтр("es='El registro del cálculo';ru='Регистр расчета';en='Calculation register'"), , БиблиотекаКартинок.РегистрРасчета);
	КонецЕсли;
	
	Если ЕстьОбъектыТипаМетаданных(Метаданные.ПланыВидовРасчета) Тогда
		ДоступныеЗначения.Добавить("ПланыВидовРасчета", НСтр("es='Los planes de los tipos del cálculo';ru='Планы видов расчета';en='Calculation types plans'"), , БиблиотекаКартинок.ПланВидовРасчета);
	КонецЕсли;
	
	Если ЕстьОбъектыТипаМетаданных(Метаданные.Задачи) Тогда
		ДоступныеЗначения.Добавить("Задачи", НСтр("es='Las tareas';ru='Задачи';en='Tasks'"), , БиблиотекаКартинок.Задача);
	КонецЕсли;
	
	Если ЕстьОбъектыТипаМетаданных(Метаданные.БизнесПроцессы) Тогда
		ДоступныеЗначения.Добавить("БизнесПроцессы", НСтр("es='Los procesos de negocios';ru='Бизнес-процессы';en='Business processes'"), , БиблиотекаКартинок.БизнесПроцесс);
	КонецЕсли;
...
Функция ДоступныеТаблицы(ТипОбъектаМетаданных, ИмяОбъектаМетаданных)
	...
	ДоступныеЗначения.Добавить("", НСтр("es='Los datos básicos';ru='Основные данные';en='Basic data'"));
	...
	ИначеЕсли ТипОбъектаМетаданных = "РегистрыСведений" Тогда 
		Если ОбъектМетаданных.ПериодичностьРегистраСведений
			...
			ДоступныеЗначения.Добавить("СрезПоследних", НСтр("es='El corte último';ru='Срез последних';en='Last cut'"));
			ДоступныеЗначения.Добавить("СрезПервых", НСтр("es='El corte primero';ru='Срез первых';en='First cut'"));
		КонецЕсли;
	ИначеЕсли ТипОбъектаМетаданных = "РегистрыНакопления" Тогда
		Если ОбъектМетаданных.ВидРегистра = Метаданные.СвойстваОбъектов.ВидРегистраНакопления.Остатки Тогда
			ДоступныеЗначения.Добавить("ОстаткиИОбороты", НСтр("es='Los restos y las vueltas';ru='Остатки и обороты';en='Balances and turnovers'"));
			ДоступныеЗначения.Добавить("Остатки", НСтр("es='Los restos';ru='Остатки';en='Balances'"));
			ДоступныеЗначения.Добавить("Обороты", НСтр("es='Las vueltas';ru='Обороты';en='Turnover'"));
		Иначе
			ДоступныеЗначения.Добавить("Обороты", НСтр("es='Las vueltas';ru='Обороты';en='Turnover'"));
		КонецЕсли;
	ИначеЕсли ТипОбъектаМетаданных = "РегистрыБухгалтерии" Тогда
		...
		ДоступныеЗначения.Добавить("Остатки", НСтр("es='Los restos';ru='Остатки';en='Balances'"));
		ДоступныеЗначения.Добавить("Обороты", НСтр("es='Las vueltas';ru='Обороты';en='Turnover'"));
		ДоступныеЗначения.Добавить("ОборотыДтКт", НСтр("es='Las vueltas De Dt/Kt';ru='Обороты Дт/Кт';en='Turnover Dt /Kt'"));
		ДоступныеЗначения.Добавить("ДвиженияССубконто", НСтр("es='Los movimientos con субконто';ru='Движения с субконто';en='Subconto movements'"));
	ИначеЕсли ТипОбъектаМетаданных = "РегистрыРасчета" Тогда 
		Если ОбъектМетаданных.ПериодДействия Тогда
			ДоступныеЗначения.Добавить("ДанныеГрафика", НСтр("es='Los datos del gráfico';ru='Данные графика';en='Chart data'"));
			ДоступныеЗначения.Добавить("ФактическийПериодДействия", НСтр("es='El período real de la acción';ru='Фактический период действия';en='Actual validity period'"));
		КонецЕсли;
	ИначеЕсли ТипОбъектаМетаданных = "ПланыВидовРасчета" Тогда
		Если ОбъектМетаданных.ЗависимостьОтВидовРасчета
			...
			ДоступныеЗначения.Добавить("БазовыеВидыРасчета", НСтр("es='Los tipos básicos del cálculo';ru='Базовые виды расчета';en='Basic types of calculation'"));
		КонецЕсли;
		
		ДоступныеЗначения.Добавить("ВедущиеВидыРасчета", НСтр("es='Los tipos principales del cálculo';ru='Ведущие виды расчета';en='Leading types of calculation'"));
		
		Если ОбъектМетаданных.ИспользованиеПериодаДействия Тогда 
			ДоступныеЗначения.Добавить("ВытесняющиеВидыРасчета", НСтр("es='Los tipos que desalojan del cálculo';ru='Вытесняющие виды расчета';en='Extrusion calculation types'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьИтогиРегистра(Знач ПараметрыОтчета, Знач СхемаКомпоновкиДанных)
	...
	Если ПараметрыОтчета.ТипОбъектаМетаданных = "РегистрыБухгалтерии" Тогда
		...
		ПолеСчет = ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], "Счет", НСтр("es='La cuenta';ru='Счет';en='Account'"));
		...
		Для НомерСубконто = 1 По КоличествоСубконто Цикл
			ПолеСубконто = ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], "Субконто" + НомерСубконто, НСтр("es='Субконто';ru='Субконто';en='Subconto'") + " " + НомерСубконто);
	...
	Для каждого Ресурс Из ОбъектМетаданных.Ресурсы Цикл
		...
		Если ПараметрыОтчета.ИмяТаблицы = "Обороты" Тогда
			...
			Если ПараметрыОтчета.ТипОбъектаМетаданных = "РегистрыБухгалтерии" Тогда
				ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "ОборотДт", Ресурс.Синоним + " " + НСтр("es='La vuelta de Dt';ru='оборот Дт';en='turnover Dt'"), Ресурс.Имя + "ОборотДт");
				ДобавитьПолеИтога(СхемаКомпоновкиДанных, Ресурс.Имя + "ОборотДт");
				ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "ОборотКт", Ресурс.Синоним + " " + НСтр("es='La vuelta de Kt';ru='оборот Кт';en='Kt turnover'"), Ресурс.Имя + "ОборотКт");
				ДобавитьПолеИтога(СхемаКомпоновкиДанных, Ресурс.Имя + "ОборотКт");
				
				Если НЕ Ресурс.Балансовый Тогда
					ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "КорОборот", Ресурс.Синоним + " " + НСтр("es='кор. La vuelta';ru='кор. оборот';en='box turnover'"), Ресурс.Имя + "КорОборот");
					ДобавитьПолеИтога(СхемаКомпоновкиДанных, Ресурс.Имя + "КорОборот");
					ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "КорОборотДт", Ресурс.Синоним + " " + НСтр("es='кор. La vuelta de Dt';ru='кор. оборот Дт';en='box turnover Dt'"), Ресурс.Имя + "КорОборотДт");
					ДобавитьПолеИтога(СхемаКомпоновкиДанных, Ресурс.Имя + "КорОборотДт");
					ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "КорОборотКт", Ресурс.Синоним + " " + НСтр("es='кор. La vuelta de Kt';ru='кор. оборот Кт';en='box turnover Kt'"), Ресурс.Имя + "КорОборотКт");
		...
		ИначеЕсли ПараметрыОтчета.ИмяТаблицы = "ДвиженияССубконто" Тогда
			...
			Иначе
				ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "Дт", Ресурс.Синоним + " " + НСтр("es='Дт';ru='Дт';en='Dt'"), Ресурс.Имя + "Дт");
				ДобавитьПолеИтога(СхемаКомпоновкиДанных, Ресурс.Имя + "Дт");
				ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "Кт", Ресурс.Синоним + " " + НСтр("es='Кт';ru='Кт';en='Kt'"), Ресурс.Имя + "Кт");
				ДобавитьПолеИтога(СхемаКомпоновкиДанных, Ресурс.Имя + "Кт");
			КонецЕсли;
			
		ИначеЕсли ПараметрыОтчета.ИмяТаблицы = "ОстаткиИОбороты" Тогда
			
			ПолеНабора = ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "НачальныйОстаток", Ресурс.Синоним + " " + НСтр("es='нач. El resto';ru='нач. остаток';en='beg. balance'"), Ресурс.Имя + "НачальныйОстаток");
			...
			Если ПараметрыОтчета.ТипОбъектаМетаданных = "РегистрыБухгалтерии" Тогда
				...
				ПолеНабора = ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "НачальныйОстатокДт", Ресурс.Синоним + " " + НСтр("es='нач. El resto de Dt';ru='нач. остаток Дт';en='beg. the remainder of Dt'"), Ресурс.Имя + "НачальныйОстатокДт");
				ПолеНабора.Роль.Остаток = Истина;
				ПолеНабора.Роль.ТипОстатка = ТипОстаткаКомпоновкиДанных.НачальныйОстаток;
				...
				ПолеНабора = ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "НачальныйОстатокКт", Ресурс.Синоним + " " + НСтр("es='нач. El resto de Kt';ru='нач. остаток Кт';en='beg. balance CT'"), Ресурс.Имя + "НачальныйОстатокКт");
				ПолеНабора.Роль.Остаток = Истина;
				ПолеНабора.Роль.ТипОстатка = ТипОстаткаКомпоновкиДанных.НачальныйОстаток;
				...
				ПолеНабора = ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "НачальныйРазвернутыйОстатокДт", Ресурс.Синоним + " " + НСтр("es='нач. El resto desenvuelto de Dt';ru='нач. развернутый остаток Дт';en='beg. detailed balance Dt'"), Ресурс.Имя + "НачальныйРазвернутыйОстатокДт");
				ПолеНабора.Роль.Остаток = Истина;
				ПолеНабора.Роль.ТипОстатка = ТипОстаткаКомпоновкиДанных.НачальныйОстаток;
				...
				ПолеНабора =ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "НачальныйРазвернутыйОстатокКт", Ресурс.Синоним + " " + НСтр("es='нач. El resto desenvuelto de Kt';ru='нач. развернутый остаток Кт';en='beg. expanded balance CT'"), Ресурс.Имя + "НачальныйРазвернутыйОстатокКт");
				ПолеНабора.Роль.Остаток = Истина;
				ПолеНабора.Роль.ТипОстатка = ТипОстаткаКомпоновкиДанных.НачальныйОстаток;
				ПолеНабора.Роль.ТипБухгалтерскогоОстатка = ТипБухгалтерскогоОстаткаКомпоновкиДанных.Нет;
				ПолеНабора.Роль.ПолеСчета = "Счет";
			...
			ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "Оборот", Ресурс.Синоним + " " + НСтр("es='La vuelta';ru='оборот';en='turnover'"), Ресурс.Имя + "Оборот");
			ДобавитьПолеИтога(СхемаКомпоновкиДанных, Ресурс.Имя + "Оборот");
			
			Если ПараметрыОтчета.ТипОбъектаМетаданных = "РегистрыНакопления" Тогда
				ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "Приход", Ресурс.Синоним + " " + НСтр("es='La llegada';ru='приход';en='coming'"), Ресурс.Имя + "Приход");
				ДобавитьПолеИтога(СхемаКомпоновкиДанных, Ресурс.Имя + "Приход");
				
				ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "Расход", Ресурс.Синоним + " " + НСтр("es='El gasto';ru='расход';en='consumption'"), Ресурс.Имя + "Расход");
			...
			ПолеНабора = ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "КонечныйОстаток", Ресурс.Синоним + " " + НСтр("es='La baza. El resto';ru='кон. остаток';en='closing balance'"), Ресурс.Имя + "КонечныйОстаток");
			...
			Если ПараметрыОтчета.ТипОбъектаМетаданных = "РегистрыБухгалтерии" Тогда
				...
				ПолеНабора = ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "КонечныйОстатокДт", Ресурс.Синоним + " " + НСтр("es='La baza. El resto de Dt';ru='кон. остаток Дт';en='closing balance Dt'"), Ресурс.Имя + "КонечныйОстатокДт");
				ПолеНабора.Роль.Остаток = Истина;
				ПолеНабора.Роль.ТипОстатка = ТипОстаткаКомпоновкиДанных.КонечныйОстаток;
				ПолеНабора.Роль.ТипБухгалтерскогоОстатка = ТипБухгалтерскогоОстаткаКомпоновкиДанных.Дебет;
				ПолеНабора.Роль.ПолеСчета = "Счет";
				...
				ПолеНабора = ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "КонечныйОстатокКт", Ресурс.Синоним + " " + НСтр("es='La baza. El resto de Kt';ru='кон. остаток Кт';en='closing balance Kt'"), Ресурс.Имя + "КонечныйОстатокКт");
				ПолеНабора.Роль.Остаток = Истина;
				ПолеНабора.Роль.ТипОстатка = ТипОстаткаКомпоновкиДанных.КонечныйОстаток;
				ПолеНабора.Роль.ТипБухгалтерскогоОстатка = ТипБухгалтерскогоОстаткаКомпоновкиДанных.Кредит;
				ПолеНабора.Роль.ПолеСчета = "Счет";
				...
				ПолеНабора = ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "КонечныйРазвернутыйОстатокДт", Ресурс.Синоним + " " + НСтр("es='La baza. El resto desenvuelto de Dt';ru='кон. развернутый остаток Дт';en='closing expanded balance Dt'"), Ресурс.Имя + "КонечныйРазвернутыйОстатокДт");
				ПолеНабора.Роль.Остаток = Истина;
				ПолеНабора.Роль.ТипОстатка = ТипОстаткаКомпоновкиДанных.КонечныйОстаток;
				ПолеНабора.Роль.ТипБухгалтерскогоОстатка = ТипБухгалтерскогоОстаткаКомпоновкиДанных.Нет;
				ПолеНабора.Роль.ПолеСчета = "Счет";
				...
				ПолеНабора = ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "КонечныйРазвернутыйОстатокКт", Ресурс.Синоним + " " + НСтр("es='La baza. El resto desenvuelto de Kt';ru='кон. развернутый остаток Кт';en='closing expanded balance Kt'"), Ресурс.Имя + "КонечныйРазвернутыйОстатокКт");
				ПолеНабора.Роль.Остаток = Истина;
				ПолеНабора.Роль.ТипОстатка = ТипОстаткаКомпоновкиДанных.КонечныйОстаток;
				ПолеНабора.Роль.ТипБухгалтерскогоОстатка = ТипБухгалтерскогоОстаткаКомпоновкиДанных.Нет;
				ПолеНабора.Роль.ПолеСчета = "Счет";
		...
		ИначеЕсли ПараметрыОтчета.ИмяТаблицы = "Остатки" Тогда
			ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "Остаток", Ресурс.Синоним + " " + НСтр("es='El resto';ru='остаток';en='balance'"), Ресурс.Имя + "Остаток");
			ДобавитьПолеИтога(СхемаКомпоновкиДанных, Ресурс.Имя + "Остаток");
			
			Если ПараметрыОтчета.ТипОбъектаМетаданных = "РегистрыБухгалтерии" Тогда
				ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "ОстатокДт", Ресурс.Синоним + " " + НСтр("es='El resto de Dt';ru='остаток Дт';en='balance of Dt'"), Ресурс.Имя + "ОстатокДт");
				ДобавитьПолеИтога(СхемаКомпоновкиДанных, Ресурс.Имя + "ОстатокДт");
				
				ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "ОстатокКт", Ресурс.Синоним + " " + НСтр("es='El resto de Kt';ru='остаток Кт';en='balance Kt'"), Ресурс.Имя + "ОстатокКт");
				ДобавитьПолеИтога(СхемаКомпоновкиДанных, Ресурс.Имя + "ОстатокКт");
				
				ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "РазвернутыйОстатокДт", Ресурс.Синоним + " " + НСтр("es='El resto desenvuelto de Dt';ru='развернутый остаток Дт';en='expanded balance Dt'"), Ресурс.Имя + "РазвернутыйОстатокДт");
				ДобавитьПолеИтога(СхемаКомпоновкиДанных, Ресурс.Имя + "РазвернутыйОстатокДт");
				
				ДобавитьПолеНабораДанных(СхемаКомпоновкиДанных.НаборыДанных[0], Ресурс.Имя + "РазвернутыйОстатокКт", Ресурс.Синоним + " " + НСтр("es='El resto desenvuelto de Kt';ru='развернутый остаток Кт';en='expanded balance Kt'"), Ресурс.Имя + "РазвернутыйОстатокКт");
...
Функция ДобавитьПоляПериодаВНаборДанных(НаборДанных)
	...
	СписокПериодов.Добавить("ПериодСекунда",   НСтр("es='El período el segundo';ru='Период секунда';en='Period second'"));
	СписокПериодов.Добавить("ПериодМинута",    НСтр("es='El período el minuto';ru='Период минута';en='Minute period'"));
	СписокПериодов.Добавить("ПериодЧас",       НСтр("es='El período la hora';ru='Период час';en='Period hour'"));
	СписокПериодов.Добавить("ПериодДень",      НСтр("es='El período el día';ru='Период день';en='Day period'"));
	СписокПериодов.Добавить("ПериодНеделя",    НСтр("es='El período la semana';ru='Период неделя';en='Week period'"));
	СписокПериодов.Добавить("ПериодДекада",    НСтр("es='El período la década';ru='Период декада';en='Decade period'"));
	СписокПериодов.Добавить("ПериодМесяц",     НСтр("es='El período el mes';ru='Период месяц';en='Month period'"));
	СписокПериодов.Добавить("ПериодКвартал",   НСтр("es='El período el trimestre';ru='Период квартал';en='Quarter period'"));
	СписокПериодов.Добавить("ПериодПолугодие", НСтр("es='El período el medio año';ru='Период полугодие';en='Half year period'"));
	СписокПериодов.Добавить("ПериодГод",       НСтр("es='El período el año';ru='Период год';en='Year period'"));
...
Процедура ДобавитьПоказатели(ПараметрыОтчета, НастройкиКД)
	...
	Если ПараметрыОтчета.ИмяТаблицы = "ОстаткиИОбороты" Тогда
		...
		ВыбранныеПоляНачальныйОстаток.Заголовок = НСтр("es='Нач. El resto';ru='Нач. остаток';en='The beginning balance'");
		...
		Если ПараметрыОтчета.ТипОбъектаМетаданных = "РегистрыНакопления" Тогда
			...
			ВыбранныеПоляПриход.Заголовок = НСтр("es='La llegada';ru='Приход';en='Coming'");
			...
			ВыбранныеПоляРасход.Заголовок = НСтр("es='El gasto';ru='Расход';en='Consumption'");
		...
		ИначеЕсли ПараметрыОтчета.ТипОбъектаМетаданных = "РегистрыБухгалтерии" Тогда
			...
			ВыбранныеПоляОбороты.Заголовок = НСтр("es='Las vueltas';ru='Обороты';en='Turnover'");
		...
		ВыбранныеПоляКонечныйОстаток.Заголовок = НСтр("es='La baza. El resto';ru='Кон. остаток';en='Closing balance'");
	...
	ИначеЕсли ПараметрыОтчета.ТипОбъектаМетаданных = "РегистрыБухгалтерии" Тогда
		Для Каждого Ресурс Из ОбъектМетаданных.Ресурсы Цикл
			...
			ВыбранныеПоля = НастройкиКД.Выбор;
			Если ПараметрыОтчета.ИмяТаблицы = "Обороты" Тогда
				ОтчетыСервер.ДобавитьВыбранноеПоле(ВыбранныеПоля, Ресурс.Имя + "ОборотДт", Ресурс.Синоним + " " + НСтр("es='La vuelta de Dt';ru='оборот Дт';en='turnover Dt'"));
				ОтчетыСервер.ДобавитьВыбранноеПоле(ВыбранныеПоля, Ресурс.Имя + "ОборотКт", Ресурс.Синоним + " " + НСтр("es='La vuelta de Kt';ru='оборот Кт';en='Kt turnover'"));
			ИначеЕсли ПараметрыОтчета.ИмяТаблицы = "ОборотыДтКт" Тогда
				Если Ресурс.Балансовый Тогда
					ОтчетыСервер.ДобавитьВыбранноеПоле(ВыбранныеПоля, Ресурс.Имя + "Оборот", Ресурс.Синоним + " " + НСтр("es='La vuelta';ru='оборот';en='turnover'"));
				Иначе
					ОтчетыСервер.ДобавитьВыбранноеПоле(ВыбранныеПоля, Ресурс.Имя + "ОборотДт", Ресурс.Синоним + " " + НСтр("es='La vuelta de Dt';ru='оборот Дт';en='turnover Dt'"));
					ОтчетыСервер.ДобавитьВыбранноеПоле(ВыбранныеПоля, Ресурс.Имя + "ОборотКт", Ресурс.Синоним + " " + НСтр("es='La vuelta de Kt';ru='оборот Кт';en='Kt turnover'"));
				КонецЕсли;
			ИначеЕсли ПараметрыОтчета.ИмяТаблицы = "Остатки" Тогда
				ОтчетыСервер.ДобавитьВыбранноеПоле(ВыбранныеПоля, Ресурс.Имя + "ОстатокДт", Ресурс.Синоним + " " + НСтр("es='El este. Дт';ru='ост. Дт';en='rest Dt'"));
				ОтчетыСервер.ДобавитьВыбранноеПоле(ВыбранныеПоля, Ресурс.Имя + "ОстатокКт", Ресурс.Синоним + " " + НСтр("es='El este. Кт';ru='ост. Кт';en='rest Kt'"));
			ИначеЕсли ПараметрыОтчета.ИмяТаблицы = "ОстаткиИОбороты" Тогда
				ОтчетыСервер.ДобавитьВыбранноеПоле(ВыбранныеПоляНачальныйОстаток, Ресурс.Имя + "НачальныйОстатокДт", Ресурс.Синоним + " " + НСтр("es='нач. El este. Дт';ru='нач. ост. Дт';en='beg. rest Dt'"));
				ОтчетыСервер.ДобавитьВыбранноеПоле(ВыбранныеПоляНачальныйОстаток, Ресурс.Имя + "НачальныйОстатокКт", Ресурс.Синоним + " " + НСтр("es='нач. El este. Кт';ru='нач. ост. Кт';en='beg. rest Ct'"));
				ОтчетыСервер.ДобавитьВыбранноеПоле(ВыбранныеПоляОбороты, Ресурс.Имя + "ОборотДт", Ресурс.Синоним + " " + НСтр("es='La vuelta de Dt';ru='оборот Дт';en='turnover Dt'"));
				ОтчетыСервер.ДобавитьВыбранноеПоле(ВыбранныеПоляОбороты, Ресурс.Имя + "ОборотКт", Ресурс.Синоним + " " + НСтр("es='La vuelta de Kt';ru='оборот Кт';en='Kt turnover'"));
				ОтчетыСервер.ДобавитьВыбранноеПоле(ВыбранныеПоляКонечныйОстаток, Ресурс.Имя + "КонечныйОстатокДт", " " + Ресурс.Синоним + НСтр("es='La baza. El este. Дт';ru='кон. ост. Дт';en='closing balance Dt'"));
				ОтчетыСервер.ДобавитьВыбранноеПоле(ВыбранныеПоляКонечныйОстаток, Ресурс.Имя + "КонечныйОстатокКт", " " + Ресурс.Синоним + НСтр("es='La baza. El este. Кт';ru='кон. ост. Кт';en='closing balance Kt'"));
			ИначеЕсли ПараметрыОтчета.ИмяТаблицы = "ДвиженияССубконто" Тогда
				Если Ресурс.Балансовый Тогда
					ОтчетыСервер.ДобавитьВыбранноеПоле(ВыбранныеПоля, Ресурс.Имя, Ресурс.Синоним);
				Иначе
					ОтчетыСервер.ДобавитьВыбранноеПоле(ВыбранныеПоля, Ресурс.Имя + "Дт", Ресурс.Синоним + " " + НСтр("es='Дт';ru='Дт';en='Dt'"));
					ОтчетыСервер.ДобавитьВыбранноеПоле(ВыбранныеПоля, Ресурс.Имя + "Кт", Ресурс.Синоним + " " + НСтр("es='Кт';ru='Кт';en='Kt'"));
				КонецЕсли;
			ИначеЕсли ПараметрыОтчета.ИмяТаблицы = "" Тогда
				Если Ресурс.Балансовый Тогда
					ОтчетыСервер.ДобавитьВыбранноеПоле(ВыбранныеПоля, Ресурс.Имя, Ресурс.Синоним);
...
Функция ИсточникДанных(ТипМенеджера, ИмяОбъекта)
	...
	Если Метаданные.НайтиПоПолномуИмени(ПолноеИмяОбъекта) = Неопределено Тогда 
		ЗаписьЖурналаРегистрации(НСтр("es='Las variantes de los informes. La instalación de la fuente de los datos del informe universal';ru='Варианты отчетов.Установка источника данных универсального отчета';en='Report options: installing a universal report data source'", 
			ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			Метаданные.Справочники.ВариантыОтчетов,,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='La fuente de los datos %1 falta';ru='Источник данных %1 отсутствует';en='Data source %1 missing'"), 
				ПолноеИмяОбъекта));
		Возврат Неопределено;
	КонецЕсли;
