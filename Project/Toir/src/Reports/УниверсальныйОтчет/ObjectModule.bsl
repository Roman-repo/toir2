...
//               //	Команда.Заголовок = НСтр("es='Mi orden...';ru='Моя команда...';en='My team...'");
//               //	
...
Процедура ПередЗагрузкойНастроекВКомпоновщик(Контекст, КлючСхемы, КлючВарианта, Настройки, ПользовательскиеНастройки) Экспорт
	...
	Если ТекущийКлючСхемы <> Неопределено И ТекущийКлючСхемы <> КлючСхемы Тогда
		...
		Иначе
			...
			// ТОиР++
			Если Контекст.НастройкиОтчета.Свойство("ПараметрыОтчета")
				И Контекст.НастройкиОтчета.ПараметрыОтчета.Свойство("ПараметрыОтбора") 
				И ТипЗнч(Контекст.НастройкиОтчета.ПараметрыОтчета.ПараметрыОтбора) = Тип("Структура") Тогда
				ОтчетыСервер.УстановитьФиксированныеОтборы(Контекст.НастройкиОтчета.ПараметрыОтчета.ПараметрыОтбора, Настройки, Контекст.НастройкиОтчета);
			КонецЕсли;
			// ТОиР-- 
		КонецЕсли;
...
// Параметры:
//   Форма - УправляемаяФорма - Форма отчета.
//   НовыеПользовательскиеНастройкиКД - НастройкиКомпоновкиДанных - Настройки, загружаемые в компоновщик.
//
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(Форма, НовыеПользовательскиеНастройкиКД) Экспорт
	
	торо_ПоказателиKPI.ОтчетРасшифровка_ПриЗагрузкеПользовательскихНастроекНаСервере(ЭтотОбъект, Форма, НовыеПользовательскиеНастройкиКД);
