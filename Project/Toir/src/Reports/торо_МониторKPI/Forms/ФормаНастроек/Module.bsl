...
Процедура СохранитьНастройкиВФайл(Команда)
	...
	#Иначе
		...
		ТекстСообщения = НСтр("es='La función no es accesible en el cliente web.';ru='Функция не доступна в веб-клиенте.';en='The function is not available in the web client.'");
...
Функция НайтиЭлементСпискаПоРеквизитам(ИмяСписка, СтруктураРеквизитов)
	...
	ШаблонСообщенияАтрибуты = НСтр("es='Para los elementos de lista ""%1"" no son dadas las propiedades necesarias. "
"El juego esperado de los atributos: %2.';ru='Для элементов списка ""%1"" не заданы необходимые свойства."
"Ожидаемый набор атрибутов: %2.';en='The list properties ""%1"" ""did not have the required properties. Expected attribute set:%2.""'");
									
	ШаблонСообщенияНеНайдено = НСтр("es='""%2"".';ru='Не найден элемент справочника ""%1"" с наименованием ""%2"".';en='The directory item ""%1"" with the name ""%2"" was not found.'");
	...
	Если ИмяСписка = "ПоказателиKPI" Тогда
		...
		Если НЕ ЕстьВсеРеквизиты Тогда
			ТекстСообщения = СтрШаблон(ШаблонСообщенияАтрибуты, НСтр("es='Los índices KPI';ru='Показатели KPI';en='KPI indicators'"), СтрокаОжидаемыхРеквизитов); 
	...
	ИначеЕсли ИмяСписка = "Организация" Тогда
		...
		Если НЕ СтруктураРеквизитов.Свойство("Наименование") Тогда
			ТекстСообщения = СтрШаблон(ШаблонСообщенияАтрибуты, НСтр("es='La organización';ru='Организация';en='Organization'"), "Наименование");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
			Возврат Неопределено;
		КонецЕсли;
		...
		Иначе
			ТекстСообщения = СтрШаблон(ШаблонСообщенияНеНайдено, НСтр("es='Las organizaciones';ru='Организации';en='Organizations'"), СтруктураРеквизитов.Наименование);
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;	
	...
	ИначеЕсли ИмяСписка = "Подразделение" Тогда
		// По наименованию.
		Если НЕ СтруктураРеквизитов.Свойство("Наименование") Тогда
			ТекстСообщения = СтрШаблон(ШаблонСообщенияАтрибуты, НСтр("es='La subdivisión';ru='Подразделение';en='Subdivision'"), "Наименование");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
			Возврат Неопределено;
		КонецЕсли;
		...
		Если Выборка.Следующий() Тогда
			Возврат Выборка.Ссылка;
		Иначе
			ТекстСообщения = СтрШаблон(ШаблонСообщенияНеНайдено, НСтр("es='La estructura de la empresa';ru='Структура предприятия';en='Company structure'"), СтруктураРеквизитов.Наименование);
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;	
	...
	ИначеЕсли ИмяСписка = "ОбъектРемонта" Тогда
		...
		Если НЕ СтруктураРеквизитов.Свойство("Наименование") Тогда
			ТекстСообщения = СтрШаблон(ШаблонСообщенияАтрибуты, НСтр("es='El objeto de la reparación';ru='Объект ремонта';en='Object of repair'"), "Наименование, ЭтоГруппа");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
			Возврат Неопределено;
		КонецЕсли;
      ...
      Запрос = Новый Запрос;
		...
		Если Выборка.Следующий() Тогда
			Возврат Выборка.Ссылка;
		Иначе
			ТекстСообщения = СтрШаблон(ШаблонСообщенияНеНайдено, НСтр("es='Los objetos de la reparación';ru='Объекты ремонта';en='Repair objects'"), СтруктураРеквизитов.Наименование);
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;	
...
Функция НайтиВариантАанализаПоказателяПоРеквизитам(СтруктураРеквизитов)
	...
	Если ЗначениеЗаполнено(СтруктураРеквизитов.ИДПоставляемойМодели) Тогда
		...
		Если Выборка.Следующий() Тогда
			Возврат Выборка.Ссылка;
		Иначе
			ШаблонСообщения = НСтр("es='con el identificador de modelo suministrado ""%2"". "
"Renueven el modelo suministrado de los índices en el sistema y prueben de nuevo.';ru='Не найден элемент справочника ""1"" с идентификатором поставляемой модели ""%2""."
"Обновите поставляемую модель показателей в системе и попробуйте снова.';en='The reference element ""1"" ""with the identifier of the supplied model ""%2"" was not found.  Update the supplied metrics model in the system and try again.'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, НСтр("es='Las variantes del análisis de los índices';ru='Варианты анализа показателей';en='Indicator analysis options'"), СтруктураРеквизитов.ИДПоставляемойМодели);
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
...
"""el Tipo del análisis"" = ""%3"", ""el Significado"" = ""%4"", ""el Índice"" = ""%5"".';ru='Не найден элемент справочника ""1"" с реквизитами ""Наименование""=""%2"","
"""Тип анализа""=""%3"", ""Значение""=""%4"", ""Показатель""=""%5"".';en='The reference element ""1"" ""was not found with the details ""Name"" = ""%2"","
"""Analysis type""=""%3"", ""Value"" = ""%4"", ""Metric"" =""%5"".'");
				ТекстСообщения = СтрШаблон(ШаблонСообщения, НСтр("es='Las variantes del análisis de los índices';ru='Варианты анализа показателей';en='Indicator analysis options'"), СтруктураРеквизитов.Наименование,
...
"Renueven el modelo suministrado de los índices en el sistema y prueben de nuevo.';ru='Не найден элемент справочника ""1"" с идентификатором поставляемой модели ""%2""."
"Обновите поставляемую модель показателей в системе и попробуйте снова.';en='The reference element ""1"" ""with the identifier of the supplied model ""%2"" was not found.  Update the supplied metrics model in the system and try again.'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, НСтр("es='Los índices de los procesos (KPI)';ru='Показатели процессов (KPI)';en='Process Indicators (KPI)'"), СтруктураРеквизитов.Владелец_ИДПоставляемойМодели);
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
	...
	Иначе
		...
		Иначе
			ШаблонСообщения = НСтр("es='""%2"", "
"""el Tipo del índice"" = ""%3"", ""la Variante del relleno"" = ""%4"".';ru='Не найден элемент справочника ""1"" с реквизитами ""Наименование""=""%2"","
"""Тип показателя""=""%3"", ""Вариант заполнения""=""%4"".';en='The reference element ""1"" was not found with the details ""Name""= ""%2"","
"""Metric type"" = ""%3"", ""Fill option"" =""%4"".'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, НСтр("es='Los índices de los procesos (KPI)';ru='Показатели процессов (KPI)';en='Process Indicators (KPI)'"), СтруктураРеквизитов.Владелец_Наименование,
