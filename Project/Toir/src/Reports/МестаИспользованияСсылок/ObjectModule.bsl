...
Функция ЗаголовокПоНаборуСсылок(Знач НаборСсылок)
	...
	Если ТипЗнч(НаборСсылок) = Тип("СписокЗначений") Тогда
		...
		Если ВсегоСсылок = 1 Тогда
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Los lugares del uso %1';ru='Места использования %1';en='Places of use %1'"), ОбщегоНазначения.ПредметСтрокой(НаборСсылок[0].Значение));
		ИначеЕсли ВсегоСсылок > 1 Тогда
			...
			Если ОдинаковыйТип Тогда
				Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Los lugares del uso de los elementos ""%1"" (%2)';ru='Места использования элементов ""%1"" (%2)';en='Places of use of elements ""%1"" (%2)'"), 
					НаборСсылок[0].Значение.Метаданные().Представление(),
					ВсегоСсылок);
			Иначе		
				Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("es='Los lugares del uso de los elementos (%1)';ru='Места использования элементов (%1)';en='Locations for using items (%1)'"), 
					ВсегоСсылок);
			КонецЕсли;
		КонецЕсли;
	...
	Возврат НСтр("es='Los lugares del uso de los elementos';ru='Места использования элементов';en='Places of use of elements'");
