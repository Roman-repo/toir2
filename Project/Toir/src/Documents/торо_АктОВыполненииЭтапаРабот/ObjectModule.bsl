...
Процедура ПроверитьЗавершенностьОстановочных(Отказ)
	...
	Если ПолучитьФункциональнуюОпцию("торо_ИспользоватьОстановочныеРемонты") Тогда
		...
		Если Выборка.Количество() > 0 Тогда
			Отказ = Истина;
			Пока Выборка.Следующий() Цикл
				ШаблонСообщения = НСтр("es='La reparación con el tipo <%1> por el objeto <%2> en periodo <%3 - %4> es de parada. ¡Para básico son acabadas no todas las reparaciones dependientes!';ru='Ремонт с видом <%1> по объекту <%2> за период <%3 - %4> является остановочным. Для основного завершены не все зависимые ремонты!';en='Repair with the view <%1> for the object <%2> for the period <%3 -%4> is a stop. For the main, not all dependent repairs have been completed!'");
...
Процедура ПроверитьНаличиеВведенныхНаОснованииВнутренныхПотреблений(Отказ)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ТекстСообщения = НСтр("es='¡En razon de este documento hay ya unos documentos pasados!';ru='На основании этого документа уже имеются проведенные документы!';en='Based on this document, the documents already posted are already available!'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,,,,Отказ);
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеТабличнойЧастиРемонтыОборудования(Отказ)
	...
	Пока ВыборкаОшибок.Следующий() Цикл
		ШаблонСообщения = НСтр("es='El defecto del objeto de la reparación ""%1"" es quitado del documento-razón ""%2""';ru='Дефект объекта ремонта ""%1"" удален из документа-основания ""%2""';en='Defect of repair object ""%1"" removed from the base document ""%2""'");
...
Процедура ДвиженияПоРегистру_торо_ИсторияЗапчастейОбъектаРемонта(РежимПроведения, ТаблицаЗапчастей, Отказ)
	...
	Для каждого ТекСтрока из ТабЗапчастей цикл
		Если НЕ ЗначениеЗаполнено(ТекСтрока.ОбъектРемонта) Тогда
			ТекстСообщения = НСтр("es='La falta de la realización por el registro ""la Historia de las partes de recambio del objeto de la reparación""';ru='Ошибка проведения по регистру ""История запчастей объекта ремонта""';en='Register error ""History of spare parts of the repair object""'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
			Возврат;	
		КонецЕсли;
...
Функция ПодготовитьТаблицуКонечныхЭлементовРемонтныхРабот()
	...
	|	И торо_АктОВыполненииЭтапаРаботРемонтныеРаботы.ПроцентВыполненияРабот <> 0
