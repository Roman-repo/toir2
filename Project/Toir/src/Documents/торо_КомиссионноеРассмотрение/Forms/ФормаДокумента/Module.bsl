...
Процедура ПланРемонтныхРаботОчистка(Элемент, СтандартнаяОбработка)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ПланРемонтныхРаботОчисткаЗавершение", ЭтотОбъект), НСтр("es='La parte tabular ""las Reparaciones"" será limpiada. ¿Continuar?';ru='Табличная часть ""Ремонты"" будет очищена. Продолжить?';en='Tabular part ""Repairs"" will be cleared. Continue?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура КандидатыПередУдалением(Элемент, Отказ)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("КандидатыПередУдалениемЗавершение",ЭтотОбъект),
						НСтр("es='Los datos en las partes vinculadas tabulares serán quitados. ¿Continuar?';ru='Данные в связанных табличных частях будут удалены. Продолжить?';en='Data in related tabular parts will be deleted. Continue?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ДанныеПоКандидатамПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если Элементы.Кандидаты.ТекущиеДанные = Неопределено Тогда
		ТекстСообщения = НСтр("es='El candidato no es escogido.';ru='Кандидат не выбран.';en='No candidate selected.'");
...
Процедура ДанныеПоКандидатамПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если Не ОтменаРедактирования И НоваяСтрока Тогда
		...
		Если Объект.ДанныеПоКандидатам.НайтиСтроки(Новый Структура("Критерий, Кандидат", ТекущиеДанныеПоКандидатам.Критерий, ТекущиеДанныеПоКандидатам.Кандидат)).Количество() > 1 Тогда
			...
			ТекстСообщения = НСтр("es='El criterio escogido es ya en la lista.';ru='Выбранный критерий уже есть в списке.';en='The selected criterion is already in the list.'");
...
Процедура ПодборРемонтныхРабот(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.ПланРемонтныхРабот) Тогда
		ТекстСообщения = НСтр("es='""';ru='Не заполнено поле ""План-график ППР""';en='The field ""PPR schedule"" is not filled'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
	Иначе
...
Функция ПолучитьПланНаСервере(СсылкаНаППР)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	торо_ПланГрафикРемонтаПланРемонтов.ID КАК ИД
	               |ПОМЕСТИТЬ ИДРемонтов
	               |ИЗ
	               |	Документ.торо_ПланГрафикРемонта.ПланРемонтов КАК торо_ПланГрафикРемонтаПланРемонтов
	               |ГДЕ
	               ...
	               |					Таб.ИД
	               |				ИЗ
	               |					ИДРемонтов КАК Таб)) КАК торо_РемонтыСрезПоследних
	               |ГДЕ
	               |	торо_РемонтыСрезПоследних.Регистратор ССЫЛКА Документ.торо_ПланГрафикРемонта
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               ...
	               |					Таб.ИД
	               |				ИЗ
	               |					ТолькоППрРемонты КАК Таб)) КАК ПРРСП
	               |ГДЕ
	               |	ПРРСП.Отменен = ЛОЖЬ
	               |	И ПРРСП.Замещен = ЛОЖЬ
	               |	И ПРРСП.СпособВыполнения = ЗНАЧЕНИЕ(Перечисление.СпособыСтроительства.Подрядный)
...
Процедура ДоговорПриИзменении(Элемент)
	Если ЭтаФорма.Объект.Подрядчик.Пустая() Тогда
		ТекстСообщения = НСтр("es='¡Campo ""el Contratista"" no es rellenado!';ru='Поле ""Подрядчик"" не заполнено!';en='The ""Contractor"" field is not filled!'");
...
Процедура ПодрядчикНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если СписокОтбора.Количество() = 0 Тогда
		ТекстСообщения = НСтр("es='"".';ru='Не заполнена закладка ""Данные по кандидатам"".';en='The tab ""Data on candidates"" is not filled.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
