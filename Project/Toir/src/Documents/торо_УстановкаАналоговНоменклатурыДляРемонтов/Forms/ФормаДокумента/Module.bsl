...
Процедура ОснованиеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ПоказатьФормуРедактированияМногострочногоТекста(Оповещение, Элемент.ТекстРедактирования, НСтр("es='La instrucción de la aplicación';ru='Указание по применению';en='Directions for use'"));
...
Процедура ОбновитьПредставлениеОбластиДействия()
	...
	Если ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		Шаблон = НСтр("es='La organización: %1;';ru='Организация: %1;';en='Organization:%1;'") + " ";
	...
	Если ЗначениеЗаполнено(Объект.ВидРемонта) Тогда
		...
		Шаблон = НСтр("es='El tipo de la reparación: %1;';ru='Вид ремонта: %1;';en='Type of repair: %1;'") + " ";
	...
	Если ЗначениеЗаполнено(Объект.Подразделение) Тогда
		...
		Шаблон = НСтр("es='La subdivisión: %1;';ru='Подразделение: %1;';en='Unit:%1;'") + " ";
	...
	Если ЗначениеЗаполнено(Объект.ТехКарта) Тогда
		...
		Шаблон = НСтр("es='La tarjeta tecnológica: %1;';ru='Технологическая карта: %1;';en='Technological map:%1;'") + " ";
	...
	Если ЗначениеЗаполнено(Объект.ОбъектРемонта) Тогда
		...
		Шаблон = НСтр("es='El objeto de la reparación: %1;';ru='Объект ремонта: %1;';en='Repair Object:%1;'") + " ";
	...
	Если ЗначениеЗаполнено(Объект.НаправлениеОбъектаРемонта) Тогда
		...
		Шаблон = НСтр("es='La dirección del objeto de la reparación: %1;';ru='Направление объекта ремонта: %1;';en='Repair facility direction: %1;'") + " ";
	...
	Если ЗначениеЗаполнено(Объект.СписокОбъектовРМ) Тогда 
		Шаблон = НСтр("es='La lista de los objetos de la medida de reglamento: %1;';ru='Список объектов регламентного мероприятия: %1;';en='List of scheduled maintenance items:%1;'") + " ";
	...
	Иначе
		ПредставлениеОбластиДействия = НСтр("es='Por la empresa en total';ru='По предприятию в целом';en='For the enterprise as a whole'");
	КонецЕсли;
