...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = НСтр("es='Los datos eran cambiados. ¿Llevar los cambios en el documento?';ru='Данные были изменены. Перенести изменения в документ?';en='Data has been changed. Transfer changes to a document?'");
...
Процедура ЗавершитьРедактирование(Отказ = Ложь)
	...
	Если НЕ ДляВсехОР Тогда
		Если ИспользоватьОрганизация и Не ЗначениеЗаполнено(Организация) Тогда
			ТекстСообщения = Нстр("es='Campo ""la Organización"" no es rellenado';ru='Поле ""Организация"" не заполнено';en='""Organization"" field is empty'");
	...
	Если НЕ ДляВсехРемонтов Тогда
		Если ИспользоватьПодразделение и Не ЗначениеЗаполнено(Подразделение) Тогда
			ТекстСообщения = Нстр("es='Campo ""la Subdivisión"" no es rellenado';ru='Поле ""Подразделение"" не заполнено';en='Field ""Unit"" is not filled'");
		...
		Если ИспользоватьВидРемонта и Не ЗначениеЗаполнено(ВидРемонта) Тогда
			ТекстСообщения = Нстр("es='Campo ""el Tipo de la reparación"" no es rellenado';ru='Поле ""Вид ремонта"" не заполнено';en='Field ""Type of repair"" is not filled'");
		...
		Если ИспользоватьТК и Не ЗначениеЗаполнено(ТехКарта) Тогда
			ТекстСообщения = Нстр("es='Campo ""la tarjeta Tecnológica"" no es rellenado';ru='Поле ""Технологическая карта"" не заполнено';en='Field ""Technological map"" is not filled'");
