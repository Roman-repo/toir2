...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ДатаНачала > ДатаОкончания
		...
		ТекстСообщения = НСтр("es='La fecha de la terminación no puede ser menos de fechas de la traducción';ru='Дата окончания не может быть меньше даты перевода';en='The end date cannot be less than the date of transfer'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "ДатаОкончания", ,Отказ);
		
	КонецЕсли;
	
	масСотрудников = Документы.КадровыйПеревод.СписокПодходящихСорудников(ДатаНачала, ДатаОкончания, Организация, "");
	Если масСотрудников.Найти(Сотрудник) = Неопределено Тогда
		ШаблонСообщения = НСтр("es='El empleado ""%1"" no trabaja en la organización ""%2"".';ru='Сотрудник ""%1"" не работает в организации ""%2"".';en=""Employee '%1' does not work in organization '%2'.""");
		ТекстСообщения = СтрШаблон(ШаблонСообщения, Сотрудник, Организация);
