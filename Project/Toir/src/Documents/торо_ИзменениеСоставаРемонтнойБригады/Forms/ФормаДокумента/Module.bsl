...
Процедура СоставРемонтнойБригадыПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если Объект.СоставРемонтнойБригады.НайтиСтроки(Новый Структура("Исполнитель, Квалификация", ТекДанные.Исполнитель, ТекДанные.Квалификация)).Количество() > 1 И НЕ ОтменаРедактирования Тогда
		ТекстСообщения = НСтр("es='Los ejecutores siguientes con la misma calificación son incluidos en la lista más de una vez:';ru='Следующие исполнители с одной и той же квалификацией внесены в список более одного раза:';en='The following performers with the same qualifications are listed more than once:'") + "  - " + ТекДанные.Исполнитель + " (" + ТекДанные.Квалификация + ")";
