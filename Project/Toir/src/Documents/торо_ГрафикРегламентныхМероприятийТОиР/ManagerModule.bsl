...
Функция ОбновитьДоступностьДляРедактирования(Ссылка, ВидОперации, ТаблицаID) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ТаблицаID.ID,
	               |	МИНИМУМ(ВЫБОР
	               |			КОГДА НЕ(ТИПЗНАЧЕНИЯ(торо_Ремонты.Регистратор) = ТИП(Документ.торо_НарядНаРегламентноеМероприятие)
	               |						ИЛИ ТИПЗНАЧЕНИЯ(торо_Ремонты.Регистратор) = ТИП(Документ.торо_АктОВыполненииРегламентногоМероприятия)
...
Функция РассчитатьСтоимостиРемонтов(ПланРемонтов, Ссылка, МассивДоступныхДляКорректировкиСтрок, ТекущаяСтрока = Неопределено) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |		ПО (торо_РегламентныеМероприятияИСпискиОбъектов.НормативныйРемонт = торо_ВерсииТехКартСрезПоследних.ИдентификаторТехКарты)
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
...
Процедура ПеревестиСтоимостиВТекущие(ПланРемонтов, Ссылка, ТекущаяСтрока = Неопределено) 
	...
	Иначе
		...
		ТабОбхода = ПланРемонтов.Скопировать(,"ВидРемонтныхРабот, СписокОбъектовРемонта");
		ТабОбхода.Свернуть("ВидРемонтныхРабот, СписокОбъектовРемонта");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("es='El gráfico de las medidas de reglamento';ru='График регламентных мероприятий';en='Schedule of Regulatory Activities'");
...
Функция ПечатьПланГрафикРегламентныхМероприятий(МассивОбъектов, ПараметрыПечати)
	...
	Для каждого ПланГрафикРемонта из МассивОбъектов Цикл
		...
		Если (ПланГрафикРемонта = Неопределено) Тогда
			ТекстСообщения = НСтр("es='"".';ru='Не выбран документ ""График регламентных мероприятий"".';en='The document ""Schedule of Regulatory Activities"" has not been selected.'");
...
Процедура ЗаполнитьПланРемонтов(ПланРемонтов, Основание, ДатаНачалаПланирования, ДатаОкончанияПланирования, Отборы=Неопределено, СсылкаТекДок = Неопределено) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ЕСТЬNULL(Акты.ДатаНачалаФакт, ЕСТЬNULL(Наряды.ДатаНачалаФакт, ИсхДанные.ДатаНачала)) КАК ДатаНач,
	               |	ЕСТЬNULL(Акты.ДатаОкончанияФакт, ЕСТЬNULL(Наряды.ДатаОкончанияФакт, ИсхДанные.ДатаОкончания)) КАК ДатаКон,
	               ...
	               |	ЛОЖЬ КАК Замещен,
	               |	ЕСТЬNULL(Акты.ДатаНачалаФакт, ЕСТЬNULL(Наряды.ДатаНачалаФакт, ИсхДанные.ДатаНачала)) КАК ДатаНачСт,
	               |	ЕСТЬNULL(Акты.ДатаОкончанияФакт, ЕСТЬNULL(Наряды.ДатаОкончанияФакт, ИсхДанные.ДатаОкончания)) КАК ДатаКонСт,
	               |	ЛОЖЬ КАК ЗамещенСт,
	               |	ИсхДанные.СтоимостьРемонта КАК СуммаРемонта
	               |ПОМЕСТИТЬ Финальная
	               |ИЗ
	               |	ИсхДанные КАК ИсхДанные
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.торо_Ремонты.СрезПоследних(
	               |				,
	               |				Регистратор ССЫЛКА Документ.торо_НарядНаРегламентноеМероприятие
	               |					И ID В
	               |						(ВЫБРАТЬ
	               |							ВТ_IDремонтов.ID
	               |						ИЗ
	               |							ВТ_IDремонтов)) КАК Наряды
	               |		ПО ИсхДанные.ИД = Наряды.ID
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.торо_Ремонты.СрезПоследних(
	               |				,
	               |				Регистратор ССЫЛКА Документ.торо_АктОВыполненииРегламентногоМероприятия
	               |					И ID В
	               |						(ВЫБРАТЬ
	               |							ВТ_IDремонтов.ID
	               |						ИЗ
	               |							ВТ_IDремонтов)) КАК Акты
	               |		ПО ИсхДанные.ИД = Акты.ID
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
