...
Процедура АктОВыполненииРаботПриИзменении(Элемент)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("АктОВыполненииРаботОчисткаЗавершение", ЭтотОбъект, Новый Структура("ПриИзменении", Истина)), НСтр("es='Las partes tabulares serán limpiadas. ¿Continuar?';ru='Табличные части будут очищены. Продолжить?';en='The tabular parts will be cleaned. Continue?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура АктОВыполненииРаботОчистка(Элемент, СтандартнаяОбработка)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("АктОВыполненииРаботОчисткаЗавершение", ЭтотОбъект, Новый Структура("ПриИзменении", Ложь)), НСтр("es='Las partes tabulares serán limpiadas. ¿Continuar?';ru='Табличные части будут очищены. Продолжить?';en='The tabular parts will be cleaned. Continue?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ОценкаДляВсехРаботПриИзменении(Элемент)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ОценкаДляВсехРаботПриИзмененииЗавершение", ЭтотОбъект), НСтр("es='Las partes tabulares de las apreciaciones y los criterios serán limpiadas. ¿Continuar?';ru='Табличные части оценок и критериев будут очищены. Продолжить?';en='The tabular parts of the ratings and criteria will be cleared. Continue?'"), РежимДиалогаВопрос.ДаНет); 
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("КонтрагентОчисткаЗавершение", ЭтотОбъект, Новый Структура("ПриИзменении", Истина)), НСтр("es='Las partes tabulares serán limpiadas. ¿Continuar?';ru='Табличные части будут очищены. Продолжить?';en='The tabular parts will be cleaned. Continue?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура КонтрагентОчистка(Элемент, СтандартнаяОбработка)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("КонтрагентОчисткаЗавершение", ЭтотОбъект, Новый Структура("ПриИзменении", Ложь)), НСтр("es='Las partes tabulares serán limpiadas. ¿Continuar?';ru='Табличные части будут очищены. Продолжить?';en='The tabular parts will be cleaned. Continue?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ОбъектыРемонтаПередУдалением(Элемент, Отказ)
	...
	Если Элементы.ОбъектыРемонта.ТекущиеДанные <> Неопределено Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ОбъектыРемонтаПередУдалениемЗавершение", ЭтотОбъект, ДопПараметры), НСтр("es='Los datos en las partes vinculadas tabulares serán quitados. ¿Continuar?';ru='Данные в связанных табличных частях будут удалены. Продолжить?';en='Data in related tabular parts will be deleted. Continue?'"), РежимДиалогаВопрос.ДаНет); 
	КонецЕсли;
...
Процедура ПараметрыОценкиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если Элементы.ОценкиПодрядчиков.ТекущиеДанные = Неопределено Тогда 
		ТекстСообщения = НСтр("es='de la apreciación.';ru='Не выбран вид оценки.';en='No rating type selected.'");
...
Процедура ПараметрыОценкиПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если Не ОтменаРедактирования И Объект.ПараметрыОценки.НайтиСтроки(Новый Структура("ID, ВидОценки, КритерийОценки", ТекущиеДанныеПараметровОценки.ID, ТекущиеДанныеПараметровОценки.ВидОценки, ТекущиеДанныеПараметровОценки.КритерийОценки)).Количество() > 1 Тогда
		ТекстСообщения = НСтр("es='¡El criterio dado de la apreciación era escogido ya! Escojan otro criterio de la apreciación.';ru='Данный критерий оценки уже был выбран! Выберите другой критерий оценки.';en='This evaluation criterion has already been selected! Choose a different evaluation criterion.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,,, Отказ);
	КонецЕсли;
...
Процедура ОценкиПодрядчиковПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если (Не Объект.ОценкаДляВсехРабот) И Элементы.ОбъектыРемонта.ТекущиеДанные = Неопределено Тогда
		ТекстСообщения = НСтр("es='de los trabajos de reparación.';ru='Не выбран вид ремонтных работ.';en='No type of repair work selected.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,,, Отказ);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ТекстСообщения = НСтр("es='el contratante.';ru='Не выбран контрагент.';en='No counterparty selected.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,,, Отказ);
	КонецЕсли; 
...
Процедура ОценкиПодрядчиковПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если Не ОтменаРедактирования И Объект.ОценкиПодрядчиков.НайтиСтроки(Новый Структура("ID, ВидОценки", Элемент.ТекущиеДанные.ID, Элемент.ТекущиеДанные.ВидОценки)).Количество() > 1 Тогда
		ТекстСообщения = НСтр("es='¡El tipo dado de la apreciación era escogido ya! Escojan otro tipo de la apreciación.';ru='Данный вид оценки уже был выбран! Выберите другой вид оценки.';en='This type of rating has already been selected! Choose a different type of rating.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,,, Отказ);
	КонецЕсли;
...
Процедура ДобавитьРемонты(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.АктОВыполненииРабот) Тогда
		...
		ТекстСообщения = НСтр("es='"".';ru='Не заполнено поле ""Акт о выполнении этапа работ"".';en='The ""Act on the completion of the work stage"" field is not filled.'");
