...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Для Каждого СтрокаТЧ Из Объект.НаработкаОбъектов Цикл
		Если СтрокаТЧ.ДатаРаботыПо <= СтрокаТЧ.ДатаРаботыС Тогда
			ТекстСообщения = НСтр("ru = 'В строке №" + Счётчик + " табличной части ""Наработка объектов"", поле ""Дата работы с"" не может быть больше или равно ""Дата работы по"".'");
...
Процедура НаработкаОбъектовПередУдалением(Элемент, Отказ)
	...
	Если НЕ СтрокаДоступнаДляРедактирования(СтрПоиска) Тогда
		...
		ТекстСообщения = СтрШаблон(ШаблонСообщения, ТекСтрокаНаработкаОбъектов.Объект, ТекСтрокаНаработкаОбъектов.Показатель, ТекСтрокаНаработкаОбъектов.ДатаРаботыПо);
...
Процедура ЗаполнитьПлановуюНаработку(ПроверятьПоПодобраннымОбъектам, УчитыватьПростои)
	...
	Если МассивСтрок.Количество() <> 0 Тогда
		...
		ШаблонВопроса = НСтр("es='Para los objetos de la reparación %1 (la línea con los números %2) no es rellenado el gráfico del trabajo de la maquinaria. "
"¿Cumplir el cálculo?';ru='Для объектов ремонта %1 (строки с номерами %2) не заполнен график работы оборудования."
"Выполнить расчет?';en='For repair objects%1 (lines with numbers%2), the equipment schedule is not filled."
"Make a calculation? '");
...
Процедура РаспространитьНаПодчиненныхДляТекущейСтроки(ТекСтрокаНаработкаОбъектов)
	...
	Если флагРаспространятьНаПодчиненных Тогда
		...
		ТаблицаИерархииОР = торо_РаботаСИерархией20.ПолучитьТаблицуИерархии(ТекСтрокаНаработкаОбъектов.Иерархия,Объект.Дата);
...
Процедура ЗаполнитьПоВышестоящемуОРНаСервере()
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	ТаблицаДанных.Иерархия КАК Иерархия
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ТаблицаКонецИнтервала.Объект КАК Объект,
	               |	МИНИМУМ(ТаблицаКонецИнтервала.КонецИнтервала) КАК КонецИнтервала,
	               |	ТаблицаКонецИнтервала.Показатель КАК Показатель,
	               |	ТаблицаКонецИнтервала.СредняяНаработка КАК СредняяНаработка,
	               |	ТаблицаКонецИнтервала.ДатаРаботыС КАК ДатаРаботыС
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ТаблицаНачалоИнтервала.Объект КАК Объект,
	               |	ТаблицаНачалоИнтервала.НачалоИнтервала КАК НачалоИнтервала,
	               |	ТаблицаКонецИнтервала.КонецИнтервала КАК КонецИнтервала,
	               |	ТаблицаКонецИнтервала.Показатель КАК Показатель,
	               |	ТаблицаНачалоИнтервала.СредняяНаработка КАК СредняяНаработка,
	               |	ТаблицаНачалоИнтервала.ДатаРаботыС КАК ДатаРаботыС,
	               |	ТаблицаНачалоИнтервала.РодительИерархии КАК РодительИерархии
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ТаблицаВозможныхИнтервалов.Объект КАК Объект,
	               |	МАКСИМУМ(ТаблицаВозможныхИнтервалов.НачалоИнтервала) КАК НачалоИнтервала,
	               |	ТаблицаВозможныхИнтервалов.КонецИнтервала КАК КонецИнтервала,
	               |	ТаблицаВозможныхИнтервалов.Показатель КАК Показатель,
	               |	ТаблицаВозможныхИнтервалов.СредняяНаработка КАК СредняяНаработка,
	               |	ТаблицаВозможныхИнтервалов.ДатаРаботыС КАК ДатаРаботыС,
	               |	ТаблицаВозможныхИнтервалов.РодительИерархии КАК РодительИерархии
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ТаблицаИнтерваловПромежуточная.Объект КАК Объект,
	               |	МИНИМУМ(ТаблицаИнтерваловПромежуточная.НачалоИнтервала) КАК НачалоИнтервала,
	               |	ТаблицаИнтерваловПромежуточная.КонецИнтервала КАК КонецИнтервала,
	               |	ТаблицаИнтерваловПромежуточная.Показатель КАК Показатель,
	               |	ТаблицаИнтерваловПромежуточная.СредняяНаработка КАК СредняяНаработка,
	               |	ТаблицаИнтерваловПромежуточная.ДатаРаботыС КАК ДатаРаботыС,
	               |	ТаблицаИнтерваловПромежуточная.РодительИерархии КАК РодительИерархии
	...
	Для Каждого СтрокаТЧ Из Объект.НаработкаОбъектов Цикл
		РодительИерархии =  торо_РаботаСИерархией20.ПолучитьТекущегоРодителяВИерархии(СтрокаТЧ.Объект, СтрокаТЧ.Иерархия, Объект.Дата);
...
Процедура НаработкаОбъектовПередНачаломИзменения(Элемент, Отказ)
	...
	Если НЕ СтрокаДоступнаДляРедактирования(СтрПоиска) Тогда
		...
		ТекстСообщения = СтрШаблон(ШаблонСообщения, ТекСтрокаНаработка.Объект, ТекСтрокаНаработка.Показатель, ТекСтрокаНаработка.ДатаРаботыПо);
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,,, Отказ);
	КонецЕсли;
...
Процедура УстановитьДоступностьЭлементов()
	...
	Элементы.НаработкаОбъектов.ТолькоПросмотр              = БлокироватьДляИзменения;
	Элементы.Заполнить.Доступность                         = НЕ БлокироватьДляИзменения;
	Элементы.НаработкаОбъектовСинхронизировать.Доступность = НЕ БлокироватьДляИзменения; 
