...
Процедура КоэффициентыПереводаПодразделениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Не СтандартнаяОбработка Тогда
		ТекстСообщения = НСтр("es='¡No es posible escoger la subdivisión sin organización escogida!';ru='Нельзя выбирать подразделение без выбранной организации!';en='Can not choose a unit without a selected organization!'");
...
Процедура КоэффициентыПереводаОбъектРемонтаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если ВыбранноеЗначение <> Неопределено Тогда
		...
		Если ЕстьОшибка Тогда
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("КоэффициентыПереводаОбъектРемонтаОбработкаВыбораЗавершение", ЭтотОбъект, Новый Структура("ВыбранноеЗначение, СтруктураОР, ТекСтрока", ВыбранноеЗначение, СтруктураОР, ТекСтрока)), НСтр("es='¿Rellenar los significados de los requisitos de la línea por los significados del objeto de la reparación?';ru='Заполнить значения реквизитов строки значениями из объекта ремонта?';en='Fill the values ​​of the details of the line with the values ​​from the repair object?'"), РежимДиалогаВопрос.ДаНетОтмена);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура КоэффициентыПереводаКлассификаторРемонтовНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Не СтандартнаяОбработка Тогда
		ТекстСообщения = НСтр("es='¡No es posible escoger el tipo del servicio sin dirección escogida o la organización o el objeto de la reparación!';ru='Нельзя выбирать вид обслуживания без выбранного направления или организации или объекта ремонта!';en='Can not choose the type of service without the chosen direction or organization or object of repair!'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
КонецПроцедуры
