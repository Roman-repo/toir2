...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Для Каждого СтрокаТЧ Из КопияТЧ Цикл 
		...
		Если торо_РаботаСИерархией20.ПеремещениеПриведетКЗацикливанию(МассивОР, ТекСтруктураИерархии, СтрокаТЧ.РодительИерархии) Тогда 
			ШаблонСообщения = НСтр("es='El traslado del objeto de la reparación llevará a la ciclación. El traslado en %1 es imposible.';ru='Перемещение объекта ремонта приведет к зацикливанию. Перемещение в %1 невозможно.';en='Moving the repair object will cause a loop. Moving to%1 is not possible.'");
...
Процедура СтруктураИерархииОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Объект.СтруктураИерархии <> ВыбранноеЗначение 
		...
		ТекстВопроса = НСтр("es='La tabla de las posiciones de los objetos de la reparación será limpiada. ¿Continuar?';ru='Таблица положений объектов ремонта будет очищена. Продолжить?';en='The table of repair objects will be cleared. Continue?'");
...
Процедура ПоложенияВСтруктуреИерархииПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Не ЗначениеЗаполнено(Объект.СтруктураИерархии) Тогда
		...
		Отказ = Истина;
		ПоказатьПредупреждение(Неопределено, НСтр("es='¡Rellenen el significado de la estructura de la jerarquía!';ru='Заполните значение структуры иерархии!';en='Fill in the value of the hierarchy structure!'"), 60);
...
Процедура ЗаполнитьСписокОРВТекущейИерархии()
	...
	Если ЗначениеЗаполнено(Объект.СтруктураИерархии) Тогда
		...
		Пока Выборка.Следующий() Цикл
			СписокОРВТекущейИерархии.Добавить(Выборка.Значение, Строка(Выборка.Наименование) + " ("+Выборка.Код+")");
