...
Функция ПараметрыОформленияВнутреннегоПотребления(МассивСсылок, ТекстПредупреждения) Экспорт
	...
	ШаблонСообщения = НСтр("es='Cerca de las disposiciones distinguidas se distingue el campo ""%ПредставлениеПоля del %""';ru='У выделенных распоряжений отличается поле ""%ПредставлениеПоля%""';en='Selected orders have a different field ""%ПредставлениеПоля%""'");
	ТекстСообщения = "";
	
	Если ВыборкаРеквизитыШапки.РазличныхОрганизаций > 1 Тогда
		ТекстСообщения = ТекстСообщения
							+ ?(ТекстСообщения = "", "", Символы.ПС)
							+ СтрЗаменить(ШаблонСообщения, "%ПредставлениеПоля%", НСтр("es='""La organización""';ru='""Организация""';en='""Organization""'"));
	...
	Если ВыборкаРеквизитыШапки.РазличныхСкладов > 1 Тогда
		ТекстСообщения = ТекстСообщения
							+ ?(ТекстСообщения = "", "", Символы.ПС)
							+ СтрЗаменить(ШаблонСообщения, "%ПредставлениеПоля%", НСтр("es='""El almacén""';ru='""Склад""';en='""Stock""'"));
	...
	Если ВыборкаРеквизитыШапки.РазличныхПодразделений > 1 Тогда
		ТекстСообщения = ТекстСообщения
							+ ?(ТекстСообщения = "", "", Символы.ПС)
							+ СтрЗаменить(ШаблонСообщения, "%ПредставлениеПоля%", НСтр("es='""La subdivisión""';ru='""Подразделение""';en='""Department""'"));
	...
	Если ВыборкаРеквизитыШапки.РазличныхСделок > 1 Тогда
		ТекстСообщения = ТекстСообщения
							+ ?(ТекстСообщения = "", "", Символы.ПС)
							+ СтрЗаменить(ШаблонСообщения, "%ПредставлениеПоля%", НСтр("es='""La transacción""';ru='""Сделка""';en='""Deal""'"));
	...
	Если ВыборкаРеквизитыШапки.РазличныхХозяйственныхОпераций > 1 Тогда
		ТекстСообщения = ТекстСообщения
							+ ?(ТекстСообщения = "", "", Символы.ПС)
							+ СтрЗаменить(ШаблонСообщения, "%ПредставлениеПоля%", НСтр("es='""La operación""';ru='""Операция""';en='""Operation""'"));
		
		Отказ = Истина;
	КонецЕсли;
	
	Если Отказ Тогда
		ТекстПредупреждения = НСтр("es='Es imposible formalizar el consumo interior en razon de las disposiciones escogidas.';ru='Невозможно оформить внутреннее потребление на основании выбранных распоряжений.';en='Unable to draw up domestic consumption based on selected orders.'")
								+ Символы.ПС
								+ ТекстСообщения;
	...
	Если Не РезультатЗапроса[1].Пустой() Тогда
		...
		Пока ВыборкаЗаказы.Следующий() Цикл
			...
			Если ВыборкаЗаказы.ЕстьОшибкиПроведен Тогда
				...
				ШаблонСообщения = НСтр("es='El documento %1 no es pasado. La introducción en razon del documento no pasado es prohibida.';ru='Документ %1 не проведен. Ввод на основании непроведенного документа запрещен.';en='Document %1 failed to post. Entering based on an undelivered document is prohibited.'");
				ТекстСообщения = СтрШаблон(ШаблонСообщения, ВыборкаЗаказы.Заказ);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,,,, Отказ);
				
			ИначеЕсли ВыборкаЗаказы.ЕстьОшибкиСтатус Тогда
					
				ШаблонСообщения = НСтр("es='El documento %1 se encuentra en el estatus ""%2"". La introducción a la razón es permitida solamente en los estatus ""a la ejecución"", ""es cerrado"".';ru='Документ %1 находится в статусе ""%2"". Ввод на основании разрешен только в статусах ""К выполнению"", ""Закрыт"".';en='Document %1 is in status ""%2"". Entering on the basis is allowed only in the statuses ""To completion"", ""Closed"".'");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("es='El encargo del consumo interior';ru='Заказ на внутреннее потребление';en='Domestic consumption order'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ЗаказНаВнутреннееПотребление") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("es='El encargo en interior la pereza de consumo';ru='Заказ на внутреннее потреблени';en='Domestic order'"),
...
Функция ПечатьЗаказНаВнутреннееПотребление(МассивОбъектов, ОбъектыПечати)
	...
	|		Характеристика.Наименование         КАК НаименованиеХарактеристики,
	|       Серия.Наименование                  КАК Серия,
	|		ВЫБОР
