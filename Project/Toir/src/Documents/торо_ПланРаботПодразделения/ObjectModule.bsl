...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ШаблонСообщения = НСтр("es='Hay un documento no pasado el registrador: %1';ru='Есть непроведенный документ регистратор: %1';en='There is a failed document registrar:%1'");
...
Процедура ДвиженияПоРегистру_торо_СкользящийПланРабот(Отказ)
	...
	Если РезультатЗапроса[1].Пустой() Тогда
		...
		КонецЦикла;
	Иначе
		ТекстСообщения = НСтр("es='Por las reparaciones del documento hay unos documentos más avanzados y de prioridad';ru='По ремонтам из документа имеются более поздние и приоритетные документы';en='For repairs from the document, there are later and priority documents'"); 
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,,,, Отказ);
	КонецЕсли; 
