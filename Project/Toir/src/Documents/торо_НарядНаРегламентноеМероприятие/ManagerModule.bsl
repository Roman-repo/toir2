...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("es='El vestido de la ejecución de las medidas de reglamento';ru='Наряд на выполнение регламентных мероприятий';en='Regulatory duty'");
...
Функция ПечатьНарядНаВыполнениеРегМероприятий(МассивОбъектов, ПараметрыПечати)
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Пока ВыборкаРемонтов.Следующий() Цикл
			...
			|			ТОГДА ВЫБОР
			|					КОГДА НЕ тчРемонтныеРаботы.РемонтнаяРабота.НаименованиеДляПечати = """"
			|						ТОГДА тчРемонтныеРаботы.РемонтнаяРабота.НаименованиеДляПечати
			...
			// Комиссия
			Если ЗначениеЗаполнено(ВыборкаДокументов.ПредседательКомиссии) Тогда
	...
	Если ЗначениеЗаполнено(ТехКарта) Тогда
		Для Каждого ОперацияРемонта Из ТехКарта.СписокОпераций Цикл
			Если ТипЗнч(ОперацияРемонта.Операция) = Тип("СправочникСсылка.торо_ТехнологическиеОперации") Тогда
				...
				НовСтр.Количество = ОперацияРемонта.Операция.Количество;
				НовСтр.ЕдиницаИзмерения = ОперацияРемонта.Операция.ЕдиницаИзмерения;
