...
Процедура ОбъектыРемонтаПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если НЕ (НоваяСтрока И ОтменаРедактирования) Тогда
		Если НЕ ЗначениеЗаполнено(ТекСтрока.ОбъектРемонта) Тогда 
			ТекстСообщения = НСтр("es='de la reparación!';ru='Не заполнен объект ремонта!';en='Repair object not filled!'");
...
Процедура ПоказателиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если ТекСтрокаОР = Неопределено Тогда
		...
		Отказ = Истина;
		ПоказатьПредупреждение(Неопределено, НСтр("es='¡Indiquen la maquinaria para que se fija el significado de los índices controlados!';ru='Укажите оборудование для которого фиксируется значение контролируемых показателей!';en='Specify the equipment for which the value of monitored indicators is recorded!'"), 20);
		               		
	ИначеЕсли Не ЗначениеЗаполнено(ТекСтрокаОР.ОбъектРемонта) Тогда
		
		Отказ = Истина;
		ПоказатьПредупреждение(Неопределено, НСтр("es='para que se fija el significado de los índices controlados!';ru='Не заполнено оборудование для которого фиксируется значение контролируемых показателей!';en='Not filled equipment for which the value of the monitored indicators is recorded!'"), 20);
...
Процедура Подбор(Команда)
	...
	Иначе       		
		...
		ПараметрыФормы = Новый Структура("КлючНазначенияИспользования, ЗакрыватьПриВыборе, СтруктураИерархии", "торо_УчетКонтролируемыхПоказателей", Ложь,ТекСтруктураИерархии);
...
Процедура ЗаполнитьКонтролируемыеПоказатели(Команда)
	...
	Иначе
		ТекстСообщения = НСтр("es='El relleno de los índices controlados es posible solamente el objeto de la reparación.';ru='Заполнение контролируемых показателей возможно только для объекта ремонта.';en='Filling of controlled indicators is possible only for the object of repair.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
	КонецЕсли; 
