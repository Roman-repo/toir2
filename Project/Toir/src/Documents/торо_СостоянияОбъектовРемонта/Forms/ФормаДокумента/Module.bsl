...
Процедура ОбъектыРемонтаОбъектРемонтаПриИзменении(Элемент)
	...
	Если НЕ ТекущиеДанные = Неопределено Тогда
		...
		Если торо_РаботаСИерархией20.ЕстьПодчиненныеОР(ТекущиеДанные.ОбъектРемонта, ТекСтруктураИерархии) Тогда
			ПоказатьВопрос(Новый ОписаниеОповещения("ОбъектыРемонтаОбъектРемонтаПриИзмененииЗавершение", ЭтотОбъект, Новый Структура("ТекущиеДанные", ТекущиеДанные)), НСтр("es='¿Cambiar el estado de los objetos subordinados de la reparación?';ru='Изменить состояние подчиненных объектов ремонта?';en='Change the status of subordinate repair facilities?'"), РежимДиалогаВопрос.ДаНет, 0);
		КонецЕсли;
...
Процедура ОбъектыРемонтаВидЭксплуатацииНаПериодПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ТекДанные.ВидЭксплуатацииНаПериод) Тогда
		...
		ТекДанные.КолонкаВидПускаНаПериод = РеквизитыВидаЭксплуатации.НеПростой;
		ТекДанные.КолонкаПричинаПростояНаПериод = 1-РеквизитыВидаЭксплуатации.НеПростой;
	Иначе
...
Процедура ОбъектыРемонтаПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если НЕ Копирование Тогда
		...
		ОткрытьФорму("Справочник.торо_ОбъектыРемонта.ФормаВыбора",,Элемент,,ВариантОткрытияОкна.ОтдельноеОкно,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
	...
	Если Объект.ВидЭксплуатации = ПредопределенноеЗначение("Перечисление.торо_ВидыОперацийОтклоненияВРаботеОборудования.ВидЭксплуатацииНаПериод") Тогда
		...
		Для Каждого СтрокаОР Из Объект.ОбъектыРемонта Цикл
			Если СтрокаОР.ДатаНачала > СтрокаОР.ДатаОкончания Тогда
				ШаблонСообщения = НСтр("es='¡Para el objeto de la reparación %1 en la línea № %2 fecha del comienzo del período es más grande que la fecha de la terminación del período!';ru='Для объекта ремонта %1 в строке № %2 дата начала периода больше даты окончания периода!';en='For repair object%1 in line №%2, the start date of the period is greater than the end date of the period!'");
				ТекстСообщения = СтрШаблон(ШаблонСообщения, СтрокаОР.ОбъектРемонта,СтрокаОР.НомерСтроки);
				ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,,, ФлагОтказа);
			КонецЕсли;
		КонецЦикла; 
...
Процедура УстановитьЗаголовокПодменюВидаОперации()
	...
	Элементы.ОбъектыРемонтаДатаОкончания.Заголовок  = ?(Объект.ВидОперации = Перечисления.торо_ВидыОперацийОтклоненияВРаботеОборудования.ВидЭксплуатацииНаПериод,НСтр("es='La terminación del período';ru='Окончание периода';en='End of period'"),НСтр("es='La fecha del cambio del estado';ru='Дата изменения состояния';en='Status change date'"));
	Элементы.ВидЭксплуатации.Заголовок = ?(Объект.ВидОперации = Перечисления.торо_ВидыОперацийОтклоненияВРаботеОборудования.ВидЭксплуатацииНаПериод,НСтр("es='El tipo de la explotación para el período';ru='Вид эксплуатации на период';en='Type of operation for the period'"),НСтр("es='El tipo de la explotación';ru='Вид эксплуатации';en='Type of operation'"));
КонецПроцедуры
...
Процедура РассчитатьНеобходимостьВидаПускаИПричиныПростояВтекущейСтроке(ТекДанные)
	...
	Если (ЗначениеЗаполнено(ТекДанные.ВидЭксплуатации) И Объект.ВидОперации = ВидНаДату)  
				ИЛИ (ЗначениеЗаполнено(ТекДанные.ВидЭксплуатацииНаПериод) И Объект.ВидОперации = ВидНаПериод) Тогда
...
Процедура ОбъектыРемонтаОбъектРемонтаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		...
		Если ВыбранноеЗначение.Свойство("ОбъектРемонта") Тогда
			...
			Если ВыбранноеЗначение.Свойство("Иерархия") Тогда
				НС.Иерархия = ВыбранноеЗначение.Иерархия;
			ИначеЕсли НЕ ЗначениеЗаполнено(НС.Иерархия) Тогда
				НС.Иерархия = ТекСтруктураИерархии;
			КонецЕсли;
