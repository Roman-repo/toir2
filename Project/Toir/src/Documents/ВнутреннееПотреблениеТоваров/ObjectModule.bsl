...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ФОИспользоватьХарактеристики Тогда 
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"&Характеристика","Товары.Характеристика КАК Характеристика");
	Иначе
	...
	Если ФОИспользоватьСерии Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&СерияЗ1", "ТабТовары.Серия = ТоварыНаСкладахОстатки.Серия");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&СерияЗ2", "ТаблицаОстатков.Серия = Таблица.Серия");
	Иначе
		...
		Запрос.УстановитьПараметр("СерияЗ1", Истина);
		Запрос.УстановитьПараметр("СерияЗ2", Истина);
	КонецЕсли;
	
	Если ФОИспользоватьХарактеристики Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ХарактеристикаЗ1", "ТабТовары.Характеристика = ТоварыНаСкладахОстатки.Характеристика");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ХарактеристикаЗ2", "ТаблицаОстатков.Характеристика = Таблица.Характеристика");
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Таблица.Характеристика,", "");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ТаблицаОстатков.Характеристика,", "");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ТабТовары.Характеристика,", "");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Таб.Характеристика,", "");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Характеристика,", "");
		Запрос.УстановитьПараметр("ХарактеристикаЗ1", Истина);
		Запрос.УстановитьПараметр("ХарактеристикаЗ2", Истина);
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстСообщения = НСтр("es='La nomenclatura ""%Номенклатура del %"", la característica ""%Характеристика del %"", una serie ""%Серия del %"", el almacén ""%Склад del %"". Por la línea %НомерСтроки del % es formalizado más que es indicado a disposición, en %Количество del % %Единица del %.';ru='Номенклатура ""%Номенклатура%"", характеристика ""%Характеристика%"", серия ""%Серия%"", склад ""%Склад%"". По строке %НомерСтроки% оформлено больше, чем указано в распоряжении, на %Количество% %Единица%.';en='Nomenclature ""%Номенклатура%"", characteristic ""%Характеристика%"", series ""%Серия%"", warehouse ""%Склад%"". For the line%НомерСтроки%, it is drawn up more than indicated at the disposal, by%Количество% %Единица%.'");
		...
		Если ФОИспользоватьХарактеристики Тогда
			ТекстСообщения = СтрЗаменить(ТекстСообщения,"%Характеристика%", Выборка.Характеристика);
		Иначе 
