...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗапретитьСоздание И Объект.Ссылка.Пустая() И НЕ Параметры.Свойство("МассивСтрокНоменклатуры") Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если ИспользоватьВнутреннееПотреблениеПоНесколькимЗаказам Тогда
		Элементы.ТоварыЗаполнитьПоЗаказу.Заголовок = НСтр("es='Rellenar por los encargos';ru='Заполнить по заказам';en='Fill in orders'");
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Для каждого Строка Из Объект.Товары Цикл
		Строка.СерииИспользуются = ПолучитьСерииИспользуются(Строка.Номенклатура);	
	КонецЦикла; 
...
Процедура НадписьЗаголовокЗаказыНаВнутреннееПотреблениеНажатие(Элемент, СтандартнаяОбработка)
	...
	ОткрытьФорму(
		...
		Новый Структура("СписокДокументов, Заголовок",
			СписокЗаказов,
			НСтр("es='Los encargos del consumo (amortización) interior (%КоличествоДокументов del %)';ru='Заказы на внутреннее потребление (списание) (%КоличествоДокументов%)';en='Domestic consumption orders (write-off)  (%КоличествоДокументов%)'")), ЭтаФорма,,,, Неопределено, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
Процедура ЗаполнитьПоЗаказу(Команда)
	...
	Если Объект.Товары.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоЗаказуЗавершение", ЭтотОбъект), НСтр("es='La tabla ""las Mercancías"" перезаполнена. ¿Continuar?';ru='Таблица ""Товары"" будет перезаполнена. Продолжить?';en='The ""Products"" table will be replenished. Continue?'"), РежимДиалогаВопрос.ДаНет);
        Возврат;
...
Процедура ОтвязатьОтЗаказа(Команда)
	...
	Иначе
		ТекстПредупреждения = НСтр("es='Escojan las líneas, que es necesario desatar del encargo.';ru='Выберите строки, которые необходимо отвязать от заказа.';en='Select the lines you want to untie from the order.'");
...
Процедура ПриИзмененииПотреблениеПоНесколькимЗаказам(Знач Оповещение)
	...
	Если Объект.ПотреблениеПоЗаказам Тогда
		...
		Если ИспользоватьВнутреннееПотреблениеПоНесколькимЗаказам Тогда
			...
			Если Объект.Товары.Количество() = 0 Тогда
				ТекстВопроса = НСтр("es='La lista las Mercancías será перезаполнен los restos de las mercancías que no han obrado por los encargos. ¿Continuar?';ru='Список Товары будет перезаполнен остатками непоступивших товаров по заказам. Продолжить?';en='The Goods list will be replenished with the remnants of unreceived goods for orders. Continue?'");
			Иначе
				ТекстВопроса = НСтр("es='Las líneas en la lista las Mercancías serán atadas a las líneas de los encargos. Las líneas, que es imposible atar a las líneas del encargo, serán quitados. ¿Continuar?';ru='Строки в списке Товары будут привязаны к строкам заказов. Строки, которые невозможно привязать к строкам заказа, будут удалены. Продолжить?';en='Lines in the Products list will be linked to order lines. Lines that cannot be linked to order lines will be deleted. Continue?'");
			КонецЕсли; 			
		КонецЕсли;
	...
	Иначе
		...
		Если Объект.Товары.Количество() > 0 Тогда
			...
			Если ИспользоватьВнутреннееПотреблениеПоНесколькимЗаказам Тогда
				ТекстВопроса = НСтр("es='Las líneas en la lista las Mercancías dejarán de ser vinculado con las líneas de los encargos. ¿Continuar?';ru='Строки в списке Товары перестанут быть связанными со строками заказов. Продолжить?';en='Lines in the Products list will no longer be associated with order lines. Continue?'");
			Иначе
				ТекстВопроса = НСтр("es='Las líneas en la lista las Mercancías dejarán de ser vinculado con las líneas del encargo. ¿Continuar?';ru='Строки в списке Товары перестанут быть связанными со строками заказа. Продолжить?';en='Lines in the Products list will no longer be associated with order lines. Continue?'");
			КонецЕсли;
		Иначе
...
Процедура ОбновитьИнформациюПоЗаказамВФорме(СписокЗаказов,
	...
	Если СписокЗаказов.Количество() > 1 Тогда
		Если НадписьВсегоЗаказов = Неопределено Тогда
			НадписьВсегоЗаказов = НСтр("es='En total encargos';ru='Всего заказов';en='Total orders'");
		КонецЕсли;
...
Процедура ОповеститьОбОкончанииОтвязкиСтрок(КоличествоОтработанныхСтрок, СтрокиОтвязаны = Истина)
	...
	Если СтрокиОтвязаны Тогда
		ТекстОповещения = НСтр("es='En el documento de los encargos es desatado las líneas (%% la Cantidad %%).';ru='В документе от заказов отвязано строк (%%Количество%%).';en='In the document lines are untied from orders (%%Количество%%).'");
		ТекстОповещения = СтрЗаменить(ТекстОповещения, "%%Количество%%", КоличествоОтработанныхСтрок);
		ПоказатьОповещениеПользователя(
			НСтр("es='Las líneas son desatadas';ru='Строки отвязаны';en='Lines are untied'"),
			,
			ТекстОповещения,
			БиблиотекаКартинок.Информация32);
	Иначе
		ПоказатьОповещениеПользователя(
			НСтр("es='Las líneas no son desatadas';ru='Строки не отвязаны';en='Lines are not untied'"),
			,
			НСтр("es='Ni una línea no era desatada.';ru='Ни одна строка не была отвязана.';en='None of lines were untied.'"),
			БиблиотекаКартинок.Информация32);
	КонецЕсли;
