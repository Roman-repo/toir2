...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='El ajuste de las selecciones para la cinta de las novedades %1';ru='Настройка отборов для ленты новостей %1';en='Selection preferences for news feed %1'"),
		...
		Если (Элементы.ОтборыПоЛентамНовостейПользовательские.ТекущиеДанные.РазрешеноНастраиватьПользователям = Истина) 
			...
			Если Элементы.ОтборыПоЛентамНовостейПользовательские.ТекущиеДанные.СписокЗначенийГлобальныхОтборов.Количество() > 0 Тогда
				ПоказатьОповещениеПользователя(
					НСтр("es='La parte de las selecciones no es quitada';ru='Часть отборов не удалена';en='Some selections not deleted'"),
					, // Навигационная ссылка
					НСтр("es='Las selecciones ajustadas por el administrador, no es posible quitar.';ru='Отборы, настроенные администратором, удалить нельзя.';en='Selections configured by the administrator cannot be deleted.'"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
					...
					Если (НайденнаяСтрока.СписокЗначенийОтборов.Количество() = 0) Тогда
						Если (НайденнаяСтрока.СписокЗначенийГлобальныхОтборов.Количество() = 0) Тогда
							Если НайденнаяСтрока.ОтбиратьПоКатегории Тогда
								НайденнаяСтрока.СписокЗначенийОтборовПредставление = НСтр("es='Introduzcan los significados de las selecciones';ru='Введите значения отборов';en='Enter selection values'");
							Иначе
								НайденнаяСтрока.СписокЗначенийОтборовПредставление = ""; // Вывести все значения
							КонецЕсли;
						Иначе
							НайденнаяСтрока.СписокЗначенийОтборовПредставление = НСтр("es='Es ajustado por el administrador:';ru='Настроено администратором:';en='Configured by administrator:'") + " ";
	...
	Иначе
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			Если (НайденнаяСтрока.ЗаполняетсяАвтоматически = Истина) Тогда
				НайденнаяСтрока.ПредставлениеДоступностиОтбора = НСтр("es='Se llena automáticamente, el ajuste es prohibido';ru='Заполняется автоматически, настройка запрещена';en='Automatically filled, setting is not allowed'");
			Иначе
				Если (НайденнаяСтрока.РазрешеноНастраиватьПользователям = Истина) Тогда
					Если НайденнаяСтрока.ОтбиратьПоКатегории = Истина Тогда
						НайденнаяСтрока.ПредставлениеДоступностиОтбора = НСтр("es='Quiero ajustar la selección';ru='Я хочу настроить отбор';en='I want to customize the selection'");
					Иначе
						НайденнаяСтрока.ПредставлениеДоступностиОтбора = НСтр("es='Mostrar todas las novedades por esta categoría';ru='Показывать все новости по этой категории';en='Show all news in this category'");
					КонецЕсли;
				Иначе
					Если НайденнаяСтрока.СписокЗначенийГлобальныхОтборов.Количество() > 0 Тогда
						// Если администратор ввел свои значения отбора и запретил редактировать.
						НайденнаяСтрока.ПредставлениеДоступностиОтбора = НСтр("es='La selección es ajustada ya por el administrador, el ajuste es prohibido';ru='Отбор уже настроен администратором, настройка запрещена';en='The selection is already configured by the administrator, the setting is prohibited'");
					Иначе
						// Если администратор просто запретил редактировать, не вводя доп.ограничений - будут получены все новости.
						НайденнаяСтрока.ПредставлениеДоступностиОтбора = НСтр("es='Este ajuste de la selección es prohibido por el administrador';ru='Эта настройка отбора запрещена администратором';en='This filtering setting is disabled by the administrator.'");
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
