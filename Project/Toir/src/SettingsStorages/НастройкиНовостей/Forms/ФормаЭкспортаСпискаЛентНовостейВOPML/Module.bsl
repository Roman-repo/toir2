...
Процедура ИмяФайлаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Фильтр = НСтр("es='El fichero xml';ru='Файл xml';en='Xml file'")
		+ "(*.xml)|*.xml";
	ДиалогОткрытияФайла.Фильтр = Фильтр;
	ДиалогОткрытияФайла.МножественныйВыбор = Ложь;
	ДиалогОткрытияФайла.Заголовок = НСтр("es='Indiquen el fichero, a que es necesario exportar la lista de las cintas de las novedades';ru='Укажите файл, в который надо экспортировать список лент новостей';en='Specify the file to which you want to export the list of news feeds'");
...
Процедура КомандаЭкспорт(Команда)
	...
	Если (ВРег(ЭтотОбъект.ФорматНовостей) <> ВРег("rss"))
		...
		Сообщение.Текст = НСтр("es='Escojan el formato correcto de las novedades';ru='Выберите правильный формат новостей';en='Choose the right news format'");
	...
	Если ПустаяСтрока(ЭтотОбъект.ИмяФайла) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("es='Indiquen el nombre del fichero, donde conservar la lista de las novedades';ru='Укажите имя файла, куда сохранить список новостей';en='Specify the file name where to save the news list'");
	...
	ПоказатьОповещениеПользователя(
		НСтр("es='La exportación es acabada';ru='Экспорт завершен';en='Export completed'"),
		ОбработкаНовостейКлиент.ПолучитьДействиеОткрытияФайла(ИмяФайла, "КакТекстВ1С"),
		НСтр("es='La lista de las cintas de las novedades es con éxito exportada al fichero. "
"Ahora en extraño rss-agregatore es posible escoger del menú [Importar la lista de las cintas de las novedades de OPML del fichero] e indicar el fichero recibido (a condición de que rss-agregator apoya esta posibilidad).';ru='Список лент новостей успешно экспортирован в файл."
"Теперь в стороннем rss-агрегаторе можно выбрать из меню [Импортировать список лент новостей из OPML файла] и указать полученный файл (при условии, что rss-агрегатор поддерживает эту возможность).';en='The list of news feeds has been successfully exported to a file."
"Now in the third-party rss aggregator, you can select from the menu [Import a list of news feeds from an OPML file] and specify the resulting file (provided that the rss aggregator supports this feature). '"),
	...
	ЗаписьХМЛ.ЗаписатьАтрибут("version", "1.1");
		ЗаписьХМЛ.ЗаписатьНачалоЭлемента("head");
			ЗаписьХМЛ.ЗаписатьНачалоЭлемента("title");
				ЗаписьХМЛ.ЗаписатьТекст(НСтр("es='La lista de las cintas de las novedades (1С)';ru='Список лент новостей (1С)';en='List of news feeds (1C)'"));
		...
		ЗаписьХМЛ.ЗаписатьНачалоЭлемента("body");
			ЗаписьХМЛ.ЗаписатьНачалоЭлемента("outline");
				ЗаписьХМЛ.ЗаписатьАтрибут("text", НСтр("es='Las novedades 1С';ru='Новости 1С';en='News 1C'"));
