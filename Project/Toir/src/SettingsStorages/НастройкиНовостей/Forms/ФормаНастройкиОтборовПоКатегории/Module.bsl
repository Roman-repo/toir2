...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтотОбъект.ЛентаНовостей.Пустая() Тогда
		ТекстСообщения = НСтр("es='de las novedades. "
"El ajuste de la selección es imposible.';ru='Не заполнено значение Ленты новостей."
"Настройка отбора невозможна.';en='The value of the News Feed is not filled."
"Set up selection is not possible. '");
	...
	Если ЭтотОбъект.КатегорияНовостей.Пустая() Тогда
		ТекстСообщения = НСтр("es='de las novedades. "
"El ajuste de la selección es imposible.';ru='Не заполнено значение Категории новостей."
"Настройка отбора невозможна.';en='Missing News Category value."
"Cannot set up selection. '");
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = ТекстСообщения;
		Сообщение.Сообщить();
		Отказ = Истина;
	...
	Если ЭтотОбъект.ТекущийПользователь <> Пользователи.ТекущийПользователь() Тогда
		ЭтотОбъект.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("es='El ajuste de las selecciones para la cinta de las novedades por la categoría (%1)';ru='Настройка отборов для ленты новостей по категории (%1)';en='Configuring selections for a news feed by category (%1)'"),
