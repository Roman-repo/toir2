...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления % (60*24*7) = 0 Тогда // 1 раз в неделю
		ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления_Представление = НСтр("es='La lista de las novedades se renueva semanalmente';ru='Список новостей обновляется еженедельно';en='News list updated weekly'");
	ИначеЕсли ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления % (60*24) = 0 Тогда // 1 раз в день
		ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления_Представление = НСтр("es='La lista de las novedades se renueva cada día';ru='Список новостей обновляется ежедневно';en='News list updated daily'");
	ИначеЕсли ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления % 60 = 0 Тогда // 1 раз в час
		ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления_Представление = НСтр("es='La lista de las novedades se renueva cada hora';ru='Список новостей обновляется ежечасно';en='News list updated hourly'");
	Иначе
		ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления_Представление =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La lista de las novedades se renueva cada %1 minutos';ru='Список новостей обновляется каждые %1 минут';en='News list updated every%1 minutes'"),
	...
	Если ЭтотОбъект.ТекущийПользователь <> Пользователи.ТекущийПользователь() Тогда
		ЭтотОбъект.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='Los ajustes de la representación de las novedades (%1)';ru='Настройки отображения новостей (%1)';en='News Display Settings (%1)'"),
	...
	Если ТипЗнч(ПараметрыФормы) = ТипСтруктура Тогда
		...
		Иначе
			ПоказатьПредупреждение(
				...
				НСтр("es='Al principio incluyan la recepción de las novedades por esta cinta.';ru='Вначале включите получение новостей по этой ленте.';en='First, enable news feeds on this feed.'"),
				0,
	...
	Для каждого ТекущаяСтрока Из ЭтотОбъект.ЛентыНовостей Цикл
		Если (ТекущаяСтрока.ЛентаНовостей = лкЛентаНовостей) ИЛИ (лкЛентаНовостей = Неопределено) Тогда
			...
			Если (ТекущаяСтрока.КоличествоВсехОтборов = 0) Тогда
				...
				ПредставлениеВсехОтборовПодробно = НСтр("es='Por esta cinta las selecciones no se disponen';ru='По этой ленте отборы не настраиваются';en='On this tape selections are not configured'");
			Иначе
				ПредставлениеВсехОтборовКратко               = "";
				...
				ПредставлениеВсехОтборовПодробно = ПредставлениеВсехОтборовПодробно
					...
					+ ?(ПустаяСтрока(ПредставлениеПользовательскихОтборов),
							НСтр("es='No hay selecciones permitidas para el ajuste por el usuario';ru='Нет отборов, разрешенных для настройки пользователем';en='No selections allowed for user customization'")
								+ Символы.ПС,
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='Las selecciones ajustadas por el usuario (se puede cambiar): "
"%1';ru='Отборы, настроенные пользователем (можно изменить):"
"%1';en='User-configured selections (can be changed):"
"%1'")
									+ Символы.ПС,
								ПредставлениеПользовательскихОтборов));
				...
				ПредставлениеВсехОтборовПодробно = ПредставлениеВсехОтборовПодробно
					+ ?(ПустаяСтрока(ПредставлениеАдминистративныхОтборов),
							"",
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='Las selecciones ajustadas por el administrador (no es posible cambiar): "
"%1';ru='Отборы, настроенные администратором (нельзя изменить):"
"%1';en='Selections configured by the administrator (cannot be changed):"
"%1'")
									+ Символы.ПС,
								ПредставлениеАдминистративныхОтборов));
				...
				ПредставлениеВсехОтборовПодробно = ПредставлениеВсехОтборовПодробно
					+ ?(ПустаяСтрока(ПредставлениеАвтоматическиЗаполняемыхОтборов),
							"",
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("es='Las selecciones automáticamente rellenadas (no es posible cambiar): "
"%1';ru='Автоматически заполняемые отборы (нельзя изменить):"
"%1';en='Automatically filled selections (cannot be changed):"
"%1'")
									+ Символы.ПС,
								ПредставлениеАвтоматическиЗаполняемыхОтборов));

				Если ПустаяСтрока(ПредставлениеВсехОтборовПодробно) Тогда
					ПредставлениеВсехОтборовКратко   = НСтр("es='Las selecciones no son ajustadas';ru='Отборы не настроены';en='No selections configured'");
					ПредставлениеВсехОтборовПодробно = НСтр("es='Las selecciones no son ajustadas';ru='Отборы не настроены';en='No selections configured'");
				Иначе
					ПредставлениеВсехОтборовКратко   = НСтр("es='Las selecciones no son ajustadas';ru='Отборы не настроены';en='No selections configured'");
				КонецЕсли;
			...
			Если ТекущаяСтрока.ОбязательныйКанал = Истина Тогда
				ПредставлениеВсехОтборовПодробно = 
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("es='Por esta cinta no es posible desconectar la recepción de las novedades (es tan ajustado por el administrador) "
"%1';ru='По этой ленте нельзя отключить получение новостей (так настроено администратором)"
"%1';en='You can’t turn off news feed on this feed (as configured by the administrator)"
"%1'")
							+ Символы.ПС,
						ПредставлениеВсехОтборовПодробно);
			КонецЕсли;
