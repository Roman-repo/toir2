...
Процедура ОбработкаСохранения(КлючОтчета, КлючВарианта, Настройки, ОписаниеНастроек, Пользователь)
	Если Не ВариантыОтчетовПовтИсп.ПравоДобавления() Тогда
		ВызватьИсключение НСтр("es='Es insuficientemente derecho para la conservación de las variantes de los informes';ru='Недостаточно прав для сохранения вариантов отчетов';en='Not enough rights to save report options'");
	КонецЕсли;
...
Функция ДобавитьВариантыВнешнегоОтчета(ВариантыОтчета, ПолноеИмяОтчета, ИмяОтчета) Экспорт 
	...
	Исключение
		ШаблонСообщения = НСтр("es='No consiguió recibir la lista de las variantes predestinadas del informe exterior ""%1: %2%3""';ru='Не удалось получить список предопределенных вариантов внешнего отчета ""%1"":%2%3';en='Failed to get the list of predefined variations of the external report ""%1"":%2%3'");
