...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("Расположение", Расположение) Тогда
		ВызватьИсключение НСтр("es='""la Disposición"".';ru='Не передан служебный параметр ""Расположение"".';en='The service parameter ""Location"" was not passed.'");
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El significado incorrecto del parámetro ""la Disposición"": ""%1"".';ru='Некорректное значение параметра ""Расположение"": ""%1"".';en='Invalid value for the Location parameter: ""%1"".'"), Строка(Расположение));
	КонецЕсли;
КонецПроцедуры
