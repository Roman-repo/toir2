...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Контекст.ЭтоВнешний Тогда
		...
		Элементы.ДекорацияЧтоБудетДальшеНовый.Заголовок = НСтр("es='Será conservada una nueva variante del informe.';ru='Будет сохранен новый вариант отчета.';en='The new version of the report will be saved.'");
		Элементы.ДекорацияЧтоБудетДальшеПерезапись.Заголовок = НСтр("es='Será regrabada la variante existente del informe.';ru='Будет перезаписан существующий вариант отчета.';en='The existing version of the report will be overwritten.'");
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	Если Не ЗначениеЗаполнено(Объект.Наименование) Тогда
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("es='Campo ""el Nombre"" no es rellenado';ru='Поле ""Наименование"" не заполнено';en='""Name"" field is empty'"),
			,
			"Наименование");
		Отказ = Истина;
	ИначеЕсли ВариантыОтчетов.НаименованиеЗанято(Контекст.ОтчетСсылка, ВариантСсылка, Объект.Наименование) Тогда
		ОбщегоНазначения.СообщитьПользователю(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='""%1"" es ocupado, es necesario indicar otro Nombre.';ru='""%1"" занято, необходимо указать другое Наименование.';en='""%1"" is busy, other Name must be specified.'"),
				Объект.Наименование),
			,
			"Наименование");
		Отказ = Истина;
...
Процедура ОписаниеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ПоказатьФормуРедактированияМногострочногоТекста(Оповещение, Элементы.Описание.ТекстРедактирования,
		НСтр("es='La descripción';ru='Описание';en='Description'"));
КонецПроцедуры
...
Процедура ВариантыОтчетаПередУдалением(Элемент, Отказ)
	...
	Если Не Контекст.ПолныеПраваНаВарианты И Не Вариант.АвторТекущийПользователь Тогда
		ТекстПредупреждения = НСтр("es='Es insuficientemente derecho para la desaparición de la variante del informe ""%1"".';ru='Недостаточно прав для удаления варианта отчета ""%1"".';en='Not enough rights to delete report option ""%1"".'");
	...
	Если Не Вариант.Пользовательский Тогда
		ПоказатьПредупреждение(, НСтр("es='Es imposible quitar la variante predestinada del informe.';ru='Невозможно удалить предопределенный вариант отчета.';en='Unable to delete predefined report option.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьПакет(Результат, Пакет) Экспорт
	...
	Если Пакет.ПроверитьСтраницу1 = Истина Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.Наименование) Тогда
			ТекстОшибки = НСтр("es='Campo ""el Nombre"" no es rellenado';ru='Поле ""Наименование"" не заполнено';en='""Name"" field is empty'");
		...
		Если Не Пакет.ВариантЭтоНовый Тогда
			...
			Если Не ПравоЗаписиВарианта(Вариант, Контекст.ПолныеПраваНаВарианты) Тогда
				ТекстОшибки = НСтр("es='Es insuficientemente derecho para el cambio de variante ""%1"". Es necesario escoger otra variante o cambiar el Nombre.';ru='Недостаточно прав для изменения варианта ""%1"". Необходимо выбрать другой вариант или изменить Наименование.';en=""Not enough rights to change option '%1'. You must select another option or change the Name.""");
			...
			Если Не Пакет.Свойство("ВопросНаПерезаписьПройден") Тогда
				Если Вариант.ПометкаУдаления = Истина Тогда
					ТекстВопроса = НСтр("es='La variante del informe ""%1"" es marcada a la desaparición. "
"¿Sustituir la variante, marcada a la desaparición, del informe?';ru='Вариант отчета ""%1"" помечен на удаление."
"Заменить помеченный на удаление вариант отчета?';en='Report option ""%1"" marked for deletion. Replace report option marked for deletion?'");
					КнопкаПоУмолчанию = КодВозвратаДиалога.Нет;
				Иначе
					ТекстВопроса = НСтр("es='¿Sustituir antes la variante conservada del informe ""%1""?';ru='Заменить ранее сохраненный вариант отчета ""%1""?';en='Replace previously saved report version ""% 1""?'");
...
Процедура ОткрытьВариантДляИзменения()
	...
	Если Не ПравоНастройкиВарианта(Вариант, Контекст.ПолныеПраваНаВарианты) Тогда
		ТекстПредупреждения = НСтр("es='No basta los derechos de admisión para el cambio de variante ""%1"".';ru='Недостаточно прав доступа для изменения варианта ""%1"".';en=""Not enough access rights to change option '%1'.""");
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстПредупреждения, Вариант.Наименование);
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Функция СформироватьСвободноеНаименование(Вариант, ВариантыОтчета)
	ШаблонИмениВарианта = СокрЛП(Вариант.Наименование) +" - "+ НСтр("es='La copia';ru='копия';en='copy'");
...
Процедура ПроверитьИЗаписатьВариантОтчета(Пакет)
	...
	Попытка
		...
		Если ЭтоНовыйВариантОтчета И ВариантыОтчетов.НаименованиеЗанято(Контекст.ОтчетСсылка, ВариантСсылка, Объект.Наименование) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='""%1"" es ocupado, es necesario indicar otro nombre.';ru='""%1"" занято, необходимо указать другое наименование.';en='""% 1"" is busy, other name must be specified.'"), Объект.Наименование);
