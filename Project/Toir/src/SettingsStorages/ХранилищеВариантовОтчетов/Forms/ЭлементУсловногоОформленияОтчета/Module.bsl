...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("КомпоновщикНастроек", КомпоновщикНастроек) Тогда
		ВызватьИсключение НСтр("es='el parámetro ""KomponovschikNastroek"" de servicio.';ru='Не передан служебный параметр ""КомпоновщикНастроек"".';en='""КомпоновщикНастроек"" service parameter was not passed.'");
	КонецЕсли;
	...
	Если Не Параметры.Свойство("ИдентификаторЭлементаСтруктурыНастроек", ИдентификаторЭлементаСтруктурыНастроек) Тогда
		ВызватьИсключение НСтр("es='el parámetro ""IdentifikatorElementaStrukturyNastroek"" de servicio.';ru='Не передан служебный параметр ""ИдентификаторЭлементаСтруктурыНастроек"".';en='The service parameter ""ИдентификаторЭлементаСтруктурыНастроек"" was not passed.'");
	КонецЕсли;
	Если Не Параметры.Свойство("ИдентификаторКД", ИдентификаторКД) Тогда
		ВызватьИсключение НСтр("es='el parámetro ""IdentifikatorKD"" de servicio.';ru='Не передан служебный параметр ""ИдентификаторКД"".';en='The service parameter ""ИдентификаторКД"" was not transferred.'");
	КонецЕсли;
	Если Не Параметры.Свойство("Наименование", Наименование) Тогда
		ВызватьИсключение НСтр("es='""el Nombre"".';ru='Не передан служебный параметр ""Наименование"".';en='The service parameter ""Name"" was not transferred.'");
	КонецЕсли;
	...
	Иначе
		...
		Если УзелКДИсточник = Неопределено Тогда
			ВызватьИсключение НСтр("es='del informe.';ru='Не найден узел отчета.';en='Report node not found.'");
		КонецЕсли;
		ЭлементКДИсточник = УзелКДИсточник.ПолучитьОбъектПоИдентификатору(ИдентификаторКД);
		Если ЭлементКДИсточник = Неопределено Тогда
			ВызватьИсключение НСтр("es='de la formalización condicional.';ru='Не найден элемент условного оформления.';en='No conditional element found.'");
		КонецЕсли;
