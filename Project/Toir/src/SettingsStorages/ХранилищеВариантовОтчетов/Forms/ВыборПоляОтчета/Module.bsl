...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("Режим", Режим) Тогда
		ВызватьИсключение НСтр("es='""el Régimen"".';ru='Не передан служебный параметр ""Режим"".';en='The service parameter ""Mode"" was not transferred.'");
	КонецЕсли;
	...
	ИначеЕсли Режим = "Отборы" Или Режим = "ВыбранныеПоля" Или Режим = "Сортировка" Или Режим = "ПоляГруппировки" Тогда
		ИмяТаблицы = Режим;
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("es='El significado incorrecto del parámetro ""el Régimen"": ""%1"".';ru='Некорректное значение параметра ""Режим"": ""%1"".';en='Invalid value for the ""Mode"" parameter: ""%1"".'"), Строка(Режим));
	КонецЕсли;
	...
	Если ИмяТаблицы = "ПоляГруппировки" Тогда
		...
		Если Режим = "СтруктураВарианта" Тогда
			...
			СтрокаДерева.Представление  = НСтр("es='<las anotaciones Detalladas>';ru='<Детальные записи>';en='<Детальные записи>'");
...
Процедура ВыбратьИЗакрыть()
	...
	Если ТипЗнч(ДоступноеПолеКД) = Тип("ДоступноеПолеКомпоновкиДанных")
		...
		Если ДоступноеПолеКД.Папка Тогда
			ПоказатьПредупреждение(, НСтр("es='Escojan el elemento';ru='Выберите элемент';en='Select item'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
