...
Процедура ВыбратьИЗакрыть()
	...
	Если Вариант.ИндексКартинки = 4 Тогда
		ТекстВопроса = НСтр("es='La variante escogida del informe es marcada a la desaparición. "
"¿Escoger este de la variante del informe?';ru='Выбранный вариант отчета помечен на удаление."
"Выбрать этот варианта отчета?';en='The selected report option is marked for deletion."
...
Процедура ОткрытьВариантДляИзменения()
	...
	Если Не ПравоИзмененияВарианта(Вариант, ПолныеПраваНаВарианты) Тогда
		ТекстПредупреждения = НСтр("es='Es insuficientemente derecho para el cambio de variante ""%1"".';ru='Недостаточно прав для изменения варианта ""%1"".';en=""Not enough rights to change option '%1'.""");
	...
	Для Каждого СведенияОВарианте Из ТаблицаВариантов Цикл
		...
		Если НаборСтрокДерева = Неопределено Тогда
			...
			Если СведенияОВарианте.Порядок = 3 Тогда
				ГруппаДерева.Наименование = НСтр("es='Marcado a la desaparición';ru='Помеченные на удаление';en='Deleted items'");
