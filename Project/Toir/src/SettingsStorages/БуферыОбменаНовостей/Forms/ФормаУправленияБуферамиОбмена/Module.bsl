...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("es='Es introducido el nombre del ajuste (%1), "
"pero no es anotado. "
""
"¿Anotar?';ru='Введено наименование настройки (%1),"
"но она не записана."
""
"Записать?';en='Enter setting name (%1),"
"but she is not recorded."
""
"To record?'"),
			...
			Если ЭтотОбъект.ТекущийБуферОбмена.Количество() > 0 Тогда
				Если НЕ ПустаяСтрока(ЭтотОбъект.КлючНастройки) Тогда
					Если ЭтотОбъект.СписокКлючейНастроек.НайтиПоЗначению(ЭтотОбъект.КлючНастройки) = Неопределено Тогда
						...
						СписокКнопок.Добавить("ЗаписатьИЗакрыть", НСтр("es='Anotar y cerrar';ru='Записать и закрыть';en='Write and close'"));
						СписокКнопок.Добавить("Закрыть", НСтр("es='Cerrar sin pedir hora';ru='Закрыть без записи';en='Close without recording'"));
						СписокКнопок.Добавить("Отмена", НСтр("es='No cerrar';ru='Не закрывать';en='Do not close'"));
						ПоказатьВопрос(ОписаниеОповещенияОЗакрытии, ТекстВопроса, СписокКнопок, 0, "ЗаписатьИЗакрыть", НСтр("es='El significado corriente del portapapeles del cambio no es conservado';ru='Текущее значение буфера обмена не сохранено';en='Current clipboard value not saved'"));
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура КомандаВыгрузитьСписокБуферовОбмена(Команда)
	...
	Диалог.Заголовок = НСтр("es='Escojan el fichero, donde anotar la lista de los portapapeles del cambio';ru='Выберите файл, куда записать список буферов обмена';en='Select the file where to write the clipboard list'");
...
Процедура КомандаЗагрузитьСписокБуферовОбмена(Команда)
	...
	Диалог.Заголовок = НСтр("es='Escojan el fichero con los portapapeles conservados antes del cambio';ru='Выберите файл с сохраненными ранее буферами обмена';en='Select a file with previously saved clipboards'");
	Диалог.МножественныйВыбор = Ложь;
	Диалог.Расширение = "xml";
...
Процедура ОбновитьИнформационныеСтроки()
	...
	Если ЭтотОбъект.ТекущийБуферОбмена.Количество() <= 0 Тогда
		ЭтотОбъект.ТекущийБуферОбмена_Представление =
			НСтр("es='El portapapeles vacío del cambio. "
"No es posible conservar el portapapeles vacío del cambio. "
"Al principio copien en él las líneas de los datos.';ru='Пустой буфер обмена."
"Нельзя сохранить пустой буфер обмена."
"Вначале скопируйте в него строки данных.';en='Empty clipboard."
"Cannot save empty clipboard."
"First copy the data rows into it.'");
...
Функция ПолучитьОписаниеВажности(Важность)
	...
	Если Важность = 1 Тогда
		Возврат НСтр("es='Muy importante';ru='Очень важная';en='Very important'");
	ИначеЕсли Важность = 2 Тогда
		Возврат НСтр("es='Importante';ru='Важная';en='Important'");
	Иначе
		Возврат НСтр("es='Regular';ru='Обычная';en='Usual'");
	КонецЕсли;
	...
	ЗаписьХМЛ.ОткрытьФайл(
		...
		ЗаписьХМЛ.ЗаписатьКомментарий(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La descarga de los portapapeles dados del cambio para %1, %2';ru='Выгрузка данных буферов обмена для %1, %2';en='Upload clipboard data for% 1,% 2'"),
				ЭтотОбъект.КлючОбъекта,
...
Процедура ПослеВыгрузкиСпискаБуферовОбмена(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если (ТипЗнч(ВыбранныеФайлы) = ТипМассив)
		...
		ПоказатьОповещениеПользователя(
			НСтр("es='Los datos son descargados.';ru='Данные выгружены.';en='Data uploaded.'"),
...
Процедура ПослеЗагрузкиСпискаБуферовОбмена(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если (ТипЗнч(ВыбранныеФайлы) = ТипМассив)
		...
		ПоказатьОповещениеПользователя(
			НСтр("es='Los datos son cargados.';ru='Данные загружены.';en='Data uploaded.'"),
			,
			,
