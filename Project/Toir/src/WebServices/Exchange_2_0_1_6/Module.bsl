...
Функция СоздатьОбменДанными(ИмяПланаОбмена, СтрокаПараметров, НастройкаОтборовXDTO, ЗначенияПоУмолчаниюXDTO)
	...
	Если Отказ Тогда
		Сообщение = НСтр("es='A la creación del ajuste del cambio en la segunda base informativa han surgido las faltas: %1';ru='При создании настройки обмена во второй информационной базе возникли ошибки: %1';en='Errors occurred while creating exchange settings in the second infobase:%1'");
...
Процедура ЗарегистрироватьДанныеДляНачальнойВыгрузки(Знач ИмяПланаОбмена, Знач КодУзла, ДлительнаяОперация, ИдентификаторОперации, ТолькоСправочники)
	...
	Иначе
		...
		Если ТолькоСправочники Тогда
			...
			НаименованиеЗадания = НСтр("es='El registro de los cambios de las guías para la descarga inicial.';ru='Регистрация изменений справочников для начальной выгрузки.';en='Registration of changes of directories for initial unloading.'");
		Иначе
			ИмяМетода = "ОбменДаннымиСервер.ЗарегистрироватьВсеДанныеКромеСправочниковДляНачальнойВыгрузкиВФоне";
			НаименованиеЗадания = НСтр("es='El registro de los cambios de todos los datos excepto las guías para la descarga inicial.';ru='Регистрация изменений всех данных кроме справочников для начальной выгрузки.';en='Registration of changes in all data except directories for initial unloading.'");
		КонецЕсли;
		...
		Иначе
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("es='La falta a la ejecución de la tarea de fondo: %1';ru='Ошибка при выполнении фонового задания: %1';en='Error completing background task:%1'"),
				НаименованиеЗадания);
		КонецЕсли;
