...
"en relación a esto no apoya el trabajo de los métodos del web-servicio.';ru='Подключаемая информационная база является файловой,"
"в связи с чем не поддерживает работу методов web-сервиса.';en='The infobase is a file base,"
"therefore, it does not support the operation of web service methods. '");
	...
	Если ПланыОбмена[ИмяПланаОбмена].НайтиПоКоду(КодУзлаПланаОбмена).Пустая() Тогда
		СообщениеОбОшибке = НСтр("es='El nudo dado del plan del cambio no es encontrado. Se dirijan al administrador de la aplicación.';ru='Заданный узел плана обмена не найден. Обратитесь к администратору приложения.';en='The specified exchange plan node was not found. Contact the application administrator.'");
		Возврат Ложь;
	КонецЕсли;
...
Функция ЗагрузитьДанныеВИнформационнуюБазу(ИмяПланаОбмена, КодУзлаПланаОбмена, ИдентификаторФайла, ИдентификаторФоновогоЗадания, СообщениеОбОшибке)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='La carga de los datos en la base informativa a través del web-servicio ""Enterprise Data Exchange""';ru='Загрузка данных в информационную базу через web-сервис ""Enterprise Data Exchange""';en='Uploading data to the infobase through the ""Enterprise Data Exchange"" web service'");
...
Функция ПодготовитьДанныеКПолучению(ИмяПланаОбмена, КодУзлаПланаОбмена, РазмерЧастиФайла, ИдентификаторФоновогоЗадания, СообщениеОбОшибке)
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(Новый УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("es='La preparación para la descarga de los datos de la base informativa a través del web-servicio ""Enterprise Data Exchange""';ru='Подготовка к выгрузке данных из информационной базы через web-сервис ""Enterprise Data Exchange""';en='Preparing to upload data from an infobase through the ""Enterprise Data Exchange"" web service'");
	ПараметрыВыполнения.КлючФоновогоЗадания = Строка(Новый УникальныйИдентификатор);
